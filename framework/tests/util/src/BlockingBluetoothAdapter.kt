...
open class BlockingBluetoothAdapter protected constructor() {
    companion object {
        ...
        private fun restorePermissions(permissions: Array<String>) =
        ...
        private fun replacePermissionsWith(vararg newPermissions: String): Array<String> {
            ...
            if (newPermissions.size == 0) {
                ...
                throw IllegalArgumentException(
                    "Invalid permissions replacement with no permissions."
                )
            }
        ...
        @Deprecated("Remove parameter")
        @JvmStatic
        fun enableBLE(
            @Suppress("UNUSED_PARAMETER") bluetoothAdapter: BluetoothAdapter,
        ...
        fun enableBLE(): Boolean {
            if (!state.oneOf(STATE_OFF)) {
                throw IllegalStateException(
                    "Invalid call to enableBLE while current state is: $state"
                )
            }
        ...
        fun disableBLE(): Boolean {
            if (!state.oneOf(STATE_BLE_ON)) {
                throw IllegalStateException(
                    "Invalid call to disableBLE while current state is: $state"
                )
            }
