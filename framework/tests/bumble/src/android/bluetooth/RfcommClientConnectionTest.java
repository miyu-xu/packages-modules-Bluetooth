...
public class RfcommClientConnectionTest {
    ...
    private RfcommProto.ServerId mServerId;
    ...
    public void ConnectToOpenServerSocketBondedInsecure() throws Exception {
        ...
        RfcommProto.StartServerRequest request =
                RfcommProto.StartServerRequest.newBuilder()
                        .setName("RFCOMM Server")
                        .setUuid(TEST_UUID)
                        .build();
        RfcommProto.StartServerResponse response = mBumble.rfcommBlocking().startServer(request);
        mServerId = response.getServer();
        ...
        Matcher<Intent> bondedMatchers =
                AllOf.allOf(
                        hasAction(BluetoothDevice.ACTION_BOND_STATE_CHANGED),
                        hasExtra(BluetoothDevice.EXTRA_DEVICE, mBumbleDevice),
                        hasExtra(BluetoothDevice.EXTRA_BOND_STATE, BluetoothDevice.BOND_BONDED));
        verify(mReceiver, timeout(BOND_INTENT_TIMEOUT.toMillis()))
                .onReceive(any(Context.class), MockitoHamcrest.argThat(bondedMatchers));
        ...
        RfcommProto.AcceptConnectionResponse connectionResponse =
    ...
    private void removeBondIfBonded(BluetoothDevice deviceToRemove) {
        ...
        if (bondedDevices.contains(deviceToRemove)) {
            ...
            Matcher<Intent> unbondedMatcher =
                    AllOf.allOf(
                            hasAction(BluetoothDevice.ACTION_BOND_STATE_CHANGED),
