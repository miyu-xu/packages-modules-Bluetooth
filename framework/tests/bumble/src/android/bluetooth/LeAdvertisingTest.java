...
public class LeAdvertisingTest {
    ...
    public void advertisingSet() throws Exception {
        ScanningResponse response = startAdvertising()
                                      .thenCompose(advAddress -> scanWithBumble(advAddress))
                                      .join();

        Log.i(LOG_TAG, "scan response: " + response);
        assertThat(response != null).isNotNull();
    }
    ...
    private CompletableFuture<ScanningResponse> scanWithBumble(String address) {
