...
public class AvrcpMediaPlayer extends MediaBrowserService {
    ...
    static final String DEFAULT_TITLE = "BumbleTestTitle";
    static final String DEFAULT_ARTIST = "BumbleTestArtist";
    static final String DEFAULT_ALBUM = "BumbleTestAlbum";
    static final long DEFAULT_NUM_TRACKS = 900913;
    static final long DEFAULT_TRACK_NUM = 900913;
    static final String DEFAULT_GENRE = "BumbleTestGenre";
    static final long DEFAULT_DURATION = 900913;
    ...
    /** Creates MediaMetadata from given values / keys */
    public static MediaMetadata createEditedMediaMetadata(
            List<String> stringKeys,
            List<String> stringValues,
            List<String> longKeys,
            List<Long> longValues) {
        if (stringKeys.size() != stringValues.size() || longKeys.size() != longValues.size()) {
            return null;
        }
        MediaMetadata.Builder builder = new MediaMetadata.Builder();

        for (int i = 0; i < stringKeys.size(); i++) {
            builder.putString(stringKeys.get(i), stringValues.get(i));
        }
        for (int i = 0; i < longKeys.size(); i++) {
            builder.putLong(longKeys.get(i), longValues.get(i));
        }
