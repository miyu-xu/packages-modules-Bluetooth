...
public class DckTest {
    ...
    // CCC DK Specfication R3 1.2.0 r14 section 19.2.1.2 Bluetooth Le Pairing
    ...
    fun testDiscoverDkGattService() {
        mBumble.dck().register(Empty.getDefaultInstance(), StreamObserverSpliterator<Empty>())
        ...
        val advertiseRequest =
            AdvertiseRequest.newBuilder()
                ...
                .build()
        mBumble.host().advertise(advertiseRequest, StreamObserverSpliterator<AdvertiseResponse>())
        ...
        val captor = ArgumentCaptor.forClass(BluetoothGatt::class.java)
        verify(gattCallback, timeout(1000))
            .onServicesDiscovered(captor.capture(), eq(BluetoothGatt.GATT_SUCCESS))
        bumbleGatt = captor.getValue()
