...
public class PairingTest {
    ...
    public void classicPairing() throws Exception {
        ...
        // Register pairing event handler
        ...
        CompletableFuture<Void> waitPairingRequestFuture = waitPairingRequestForDevice(refAddress);
        CompletableFuture<BluetoothDevice> waitConnectionFuture = waitACLConnectionForDevice(refAddress);
        CompletableFuture<Void> waitDisconnectionFuture = waitACLDisconnectionForDevice(refAddress);
        CompletableFuture<BluetoothDevice> waitBondedFuture = waitBondedForDevice(refAddress);
        ...
        waitConnectionFuture.get();

        // change security level
        CompletableFuture<Void> secureConnectionFuture = refSecureConnection(conn);
        waitPairingRequestFuture.get();
        PairingEvent pairingEvent = waitPairingEventFuture.get();
        assertThat(pairingEvent).isNotNull();

        // stream pairing event answer to REF
        answerObserver.onNext(PairingEventAnswer.newBuilder().setEvent(pairingEvent)
        ...
        // Wait bonding
        BluetoothDevice dev = waitBondedFuture.get();
        assertThat(dev.getBondState()).isEqualTo(BluetoothDevice.BOND_BONDED);
    ...
    private CompletableFuture<Void> waitPairingRequestForDevice(String addr) {
        CompletableFuture<Void> future = new CompletableFuture<Void>();
    ...
    private CompletableFuture<BluetoothDevice> waitACLConnectionForDevice(String addr) {
    ...
    private CompletableFuture<Void> waitACLDisconnectionForDevice(String addr) {
        CompletableFuture<Void> future = new CompletableFuture<Void>();
    ...
    private CompletableFuture<BluetoothDevice> waitBondedForDevice(String addr) {
        CompletableFuture<BluetoothDevice> future = new CompletableFuture<BluetoothDevice>();
