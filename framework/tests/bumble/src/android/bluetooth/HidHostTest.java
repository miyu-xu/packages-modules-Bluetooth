...
public class HidHostTest {
     ...
     *
     * <ol>
     *   <li>1. Android connects to Bumble via its MAC address
     *   <li>2. Android tries to create bond, emitting bonding intent 4. Android confirms the
     *       pairing via pairing request intent
     *   <li>3. Bumble confirms the pairing internally
     *   <li>4. Android tries to HID connect and verifies Connection state intent
     *   <li>5. Bumble Disconnetct the HID and Android verifies Connection state intent
     *   <li>6. Android removes the bond
     * </ol>
     */
    @Test
    public void connectHidDeviceTest() throws Exception {
        ...
        mBumble.hidBlocking().registerHidDevice(Empty.getDefaultInstance());
        ...
        mBumble.hidBlocking().disconnectHidHost(Empty.getDefaultInstance());
     ...
     *   <li>4. Android tries to HID connect and verifies Connection state intent
     *   <li>5. Bumble Disconnetct the HID and Android verifies Connection state intent
     *   <li>6. Android Disable the HID connection policy
     *   <li>7. Bumble connetct the HID and Android verifies Connection state intent
     * </ol>
     */
    @Test
    public void disableHidConnectionPolicyTest() throws Exception {
        ...
        mFutureConnectionIntent = SettableFuture.create();
        mBumble.hidBlocking().connectHidHost(Empty.getDefaultInstance());
    ...
    final Runnable mSetTransport =
            new Runnable() {
                @Override
                public void run() {
                    if (mService.getPreferredTransport(mDevice)
    ...
    @Test
    public void setPrefferedTransportTest() throws Exception {
        IntentFilter filter = new IntentFilter(BluetoothHidHost.ACTION_CONNECTION_STATE_CHANGED);
        mContext.registerReceiver(mConnectionStateReceiver, filter);
        mContext.registerReceiver(
                mConnectionStateReceiver, new IntentFilter(BluetoothDevice.ACTION_PAIRING_REQUEST));
        ...
        mFutureDisconnectionIntent = SettableFuture.create();
        mHandler.postDelayed(mSetTransport, 5000);
        assertThat(mFutureDisconnectionIntent.get()).isEqualTo(BluetoothProfile.STATE_DISCONNECTED);
