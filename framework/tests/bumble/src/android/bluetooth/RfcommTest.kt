...
class RfcommTest {
    ...
    */
    ...
    fun clientRfcommDeviceDisconnectedOnBleOnMode() {
        val BLE_SCAN_ALWAYS_AVAILABLE = "ble_scan_always_enabled"

        // Enable BLE_ON mode if disable Bluetooth
        mAdapter.disable()
        waittingBluetoothStates(BluetoothAdapter.STATE_OFF)
        ...
        mAdapter.enable()
        waittingBluetoothStates(BluetoothAdapter.STATE_ON)
    ...
    // helper to wait for Bluetooth states
    private fun waittingBluetoothStates(state: Int) {
        runBlocking(mScope.coroutineContext) {
            withTimeout(STATE_CHANGE_TIMEOUT.toMillis()) {
                // wait for Bluetooth states
                launch {
                    Log.i(TAG, "Waiting for waittingBluetoothStates: " + state)
                    mFlow
                        .filter { it.action == BluetoothAdapter.ACTION_STATE_CHANGED }
