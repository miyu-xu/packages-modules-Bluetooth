...
public class HidHostDualModeTest {
    ...
    public void setUp() throws Exception {
        ...
        if (Flags.removeInputDeviceOnVup()
            ...
            verifyConnectionState(mDevice, equalTo(TRANSPORT_BREDR), equalTo(STATE_DISCONNECTING));
            verifyConnectionState(mDevice, equalTo(TRANSPORT_LE), equalTo(STATE_CONNECTING));
            verifyConnectionState(mDevice, equalTo(TRANSPORT_BREDR), equalTo(STATE_DISCONNECTED));
            verifyConnectionState(mDevice, equalTo(TRANSPORT_LE), equalTo(STATE_CONNECTED));
        }
    ...
    public void tearDown() throws Exception {
        if (mDevice.getBondState() == BluetoothDevice.BOND_BONDED) {
            ...
            if (!Flags.removeInputDeviceOnVup()
                ...
                if (connected) {
                    verifyConnectionState(
                            mDevice, equalTo(TRANSPORT_LE), equalTo(STATE_DISCONNECTING));
                }
                verifyConnectionState(mDevice, equalTo(TRANSPORT_BREDR), equalTo(STATE_CONNECTING));
                if (connected) {
                    verifyConnectionState(
                            mDevice, equalTo(TRANSPORT_LE), equalTo(STATE_DISCONNECTED));
                }
                verifyConnectionState(mDevice, equalTo(TRANSPORT_BREDR), equalTo(STATE_CONNECTED));
            }
            removeBond(mDevice);
    ...
    public void setPreferredTransportTest() {
        // BR/EDR transport
        mHidService.setPreferredTransport(mDevice, TRANSPORT_BREDR);
        verifyConnectionState(mDevice, equalTo(TRANSPORT_LE), equalTo(STATE_DISCONNECTING));
        verifyConnectionState(mDevice, equalTo(TRANSPORT_BREDR), equalTo(STATE_CONNECTING));
        verifyConnectionState(mDevice, equalTo(TRANSPORT_LE), equalTo(STATE_DISCONNECTED));
        verifyConnectionState(mDevice, equalTo(TRANSPORT_BREDR), equalTo(STATE_CONNECTED));
