...
public class HidHostDualModeTest {
    ...
    public void hogpGetReportTest() throws Exception {
        ...
        ArgumentCaptor<byte[]> reportCaptor = ArgumentCaptor.forClass(byte[].class);
        verifyIntentReceived(
                hasAction(BluetoothHidHost.ACTION_REPORT),
                hasExtra(BluetoothHidHost.EXTRA_REPORT_BUFFER_SIZE, KEYBD_RPT_SIZE),
                hasExtra(BluetoothHidHost.EXTRA_REPORT, reportCaptor.capture()));
        assertThat(reportCaptor.getValue()).isNotNull();
        assertThat(reportCaptor.getValue().length).isGreaterThan(0);
        assertThat(reportCaptor.getValue()[0]).isEqualTo(KEYBD_RPT_ID);

        // Mouse report
        mHidService.getReport(mDevice, BluetoothHidHost.REPORT_TYPE_INPUT, (byte) MOUSE_RPT_ID, 0);
        verifyIntentReceived(
                hasAction(BluetoothHidHost.ACTION_REPORT),
                hasExtra(BluetoothHidHost.EXTRA_REPORT_BUFFER_SIZE, MOUSE_RPT_SIZE),
                hasExtra(BluetoothHidHost.EXTRA_REPORT, reportCaptor.capture()));
        assertThat(reportCaptor.getValue()).isNotNull();
        assertThat(reportCaptor.getValue().length).isGreaterThan(0);
        assertThat(reportCaptor.getValue()[0]).isEqualTo(MOUSE_RPT_ID);
    }
