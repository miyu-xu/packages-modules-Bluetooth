...
public class GattServerRunCallbackOnHandlerTest {
    ...
    public void callbackRunsOnMainThread() throws Exception {
        Handler mainThreadHandler = new Handler(Looper.getMainLooper());

        CompletableFuture<Thread> callbackThreadCheckFuture = new CompletableFuture<>();
        BluetoothGattServerCallback gattServerCallback =
                new BluetoothGattServerCallback() {
                    @Override
                    public void onConnectionStateChange(
                            BluetoothDevice device, int status, int newState) {
                        callbackThreadCheckFuture.complete(Thread.currentThread());
                    }
                };
    ...
    @Test
    public void callbackRunsOnTestRunnerThread() throws Exception {
        CompletableFuture<Thread> callbackThreadCheckFuture = new CompletableFuture<>();
        BluetoothGattServerCallback gattServerCallback =
                new BluetoothGattServerCallback() {
                    @Override
            ...
            Thread callbackThread =
                    callbackThreadCheckFuture
                            .completeOnTimeout(
                                    null, TIMEOUT_GATT_CONNECTION_MS, TimeUnit.MILLISECONDS)
                            .join();
            assertThat(callbackThread).isEqualTo(Thread.currentThread());
        } finally {
            gattServer.close();
        }
    }
