...
public class PairingTest {
    ...
    public void tearDown() throws Exception {
        ...
        if (bondedDevices.contains(mBumbleDevice)) {
            removeBond(null, mBumbleDevice);
        }
        if (bondedDevices.contains(mRemoteLeDevice)) {
            removeBond(null, mRemoteLeDevice);
        }
        mBumbleDevice = null;
     ...
     *  helper -> helper_registerNewIntentActionsInTestStep() which either modifies the
     *  intentReceiver instance, or creates one (if the caller has passed a `null`).
    ...
    private void testStep_BondBredr(IntentReceiver intentReceiver) {
        intentReceiver = helper_registerNewIntentActionsInTestStep(intentReceiver,
                BluetoothDevice.ACTION_BOND_STATE_CHANGED,
                BluetoothDevice.ACTION_ACL_CONNECTED,
                BluetoothDevice.ACTION_PAIRING_REQUEST);
    ...
    private StreamObserverSpliterator<SecureResponse>
            helper_OutgoingGattServiceDiscoveryWithIncomingLePairing(IntentReceiver intentReceiver) {
    ...
    private void removeBond(IntentReceiver intentReceiver, BluetoothDevice device) {
        intentReceiver = helper_registerNewIntentActionsInTestStep(intentReceiver,
                BluetoothDevice.ACTION_BOND_STATE_CHANGED);
    ...
    private void testStep_BondLe(IntentReceiver intentReceiver, BluetoothDevice device, OwnAddressType ownAddressType) {
        intentReceiver = helper_registerNewIntentActionsInTestStep(intentReceiver,
                BluetoothDevice.ACTION_BOND_STATE_CHANGED,
                BluetoothDevice.ACTION_ACL_CONNECTED,
                BluetoothDevice.ACTION_PAIRING_REQUEST);
    ...
    private void doTestIdentityAddressWithType(BluetoothDevice device,
        ...
        /**
         * Note: Since there was no IntentReceiver registered, passing the instance as NULL.
         * But, if there is an instance already present, that must be passed instead of NULL.
         */
    ...
    /**
     * Helper function to register intent actions, and get the IntentReceiver instance.
     *
     * @param intentReceiver IntentReceiver instance from the test caller
     * @param intentStrings Intent actions
     */
    private IntentReceiver helper_registerNewIntentActionsInTestStep(IntentReceiver intentReceiver, String... intentStrings) {
        /**
         * If intentReceiver is NULL, it indicates that the caller is a fresh test/testStep
         *  and a new IntentReceiver will be returned.
         *  else, update the intent actions and return the same instance.
         */
        if(intentReceiver == null)
            intentReceiver = new IntentReceiver.Builder(sTargetContext, intentStrings).build();
        else intentReceiver.registerIntentActions(intentStrings);
