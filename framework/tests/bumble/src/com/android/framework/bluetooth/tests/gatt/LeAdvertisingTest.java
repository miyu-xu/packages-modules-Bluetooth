...
public class LeAdvertisingTest {
    ...
    private CompletableFuture<Void> scan(String address) {
        ...
        withCancellation.run(() -> mHostStub.scan(request, responseObserver));

        future.whenComplete((input, exception) -> {
            CancellationListener cancellationListener = (io.grpc.Context c) -> future.complete(null);
