...
```shell
cd p/m/Blueooth/frameworks/bumble/src/android/bluetooth
...
To bond with Bumble, we initiate a connection and then verify that the connection was successful.
```kotlin
...
Finally, after our operations, we disconnect and ensure it was done gracefully.
