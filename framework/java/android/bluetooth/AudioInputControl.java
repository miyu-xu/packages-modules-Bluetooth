    ...
    private final IAudioInputCallback mCallback =
            new IAudioInputCallback.Stub() {
                ...
                public void onDescriptionChanged(String description) {
                    mDescriptor.mDescription = description;
                ...
                public void onGainStatusChanged(int status) {
                    mDescriptor.mStatus = status;
                    mCallbackWrapper.forEach(cb -> cb.onGainStatusChanged(status));
                }

                @Override
                @RequiresNoPermission
                public void onAudioInputStateChanged(Descriptor.AudioInputState inputState) {
                    if (mDescriptor.mAudioInputState.mGainSetting != inputState.mGainSetting) {
                        mDescriptor.mAudioInputState.mGainSetting = inputState.mGainSetting;
                        mCallbackWrapper.forEach(
                                cb -> cb.onGainStatusChanged(inputState.mGainSetting));
                    }
                    if (mDescriptor.mAudioInputState.mMute != inputState.mMute) {
                        mDescriptor.mAudioInputState.mMute = inputState.mMute;
                        mCallbackWrapper.forEach(cb -> cb.onMuteChanged(inputState.mMute));
                    }
                    if (mDescriptor.mAudioInputState.mGainMode != inputState.mGainMode) {
                        mDescriptor.mAudioInputState.mGainMode = inputState.mGainMode;
