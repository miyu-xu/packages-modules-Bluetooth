...
public final class BluetoothVolumeControl implements BluetoothProfile, AutoCloseable {
    ...
    public interface Callback {
         ...
         *     for choosing an VOCS instance ID
         * @hide
         */
        @Deprecated
        @SystemApi
         ...
         *
         * @param device remote device whose volume offset changed
         * @param instanceId identifier of VOCS instatnce on the remote device
     ...
     *     an VOCS instance. This method will always use the first instance.
     * @hide
     */
    @Deprecated
    @SystemApi
        ...
        if (service == null) {
            ...
            try {
                ...
                service.setVolumeOffset(
                        device, defaultInstanceId, volumeOffset, mAttributionSource, recv);
                recv.awaitResultNoInterrupt(getSyncTimeout()).getValue(null);
            } catch (RemoteException | TimeoutException e) {
                Log.e(TAG, e.toString() + "\n" + Log.getStackTraceString(new Throwable()));
            }
     ...
     *
     * @param device {@link BluetoothDevice} representing the remote device
     * @param instanceId identifier of VOCS instatnce on the remote device. Identifiers are
     *     numerated from 1. Number of them was notified by callbacks and it can be read using
     *     {@link #getNumberOfVolumeOffsetInstatnces(BluetoothDevice)}. Providing non existing
     *     instance ID will be ignored
    ...
    public int getNumberOfVolumeOffsetInstatnces(@NonNull BluetoothDevice device) {
        if (DBG) log("getNumberOfVolumeOffsetInstatnces(" + device + ")");
        final IBluetoothVolumeControl service = getService();
        if (service == null) {
            Log.w(TAG, "Proxy not attached to service");
            if (DBG) log(Log.getStackTraceString(new Throwable()));
        ...
        try {
            final SynchronousResultReceiver recv = SynchronousResultReceiver.get();
            service.getNumberOfVolumeOffsetInstatnces(device, mAttributionSource, recv);
            recv.awaitResultNoInterrupt(getSyncTimeout()).getValue(defaultValue);
        } catch (RemoteException | TimeoutException e) {
            Log.e(TAG, e.toString() + "\n" + Log.getStackTraceString(new Throwable()));
        }
