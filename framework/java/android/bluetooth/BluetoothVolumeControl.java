...
public final class BluetoothVolumeControl implements BluetoothProfile, AutoCloseable {
    ...
    public boolean isVolumeOffsetAvailable(@NonNull BluetoothDevice device) {
        ...
        return callService(
                s -> s.isVolumeOffsetAvailable(device, mAttributionSource), defaultValue);
    }
    ...
    public int getNumberOfVolumeOffsetInstances(@NonNull BluetoothDevice device) {
        ...
        return callService(
                s -> s.getNumberOfVolumeOffsetInstances(device, mAttributionSource), defaultValue);
    }
    ...
    public void setDeviceVolume(
        ...
        callService(s -> s.setDeviceVolume(device, volume, isGroupOperation, mAttributionSource));
    }

    private <R> R callService(
            RemoteExceptionFunction<IBluetoothVolumeControl, R> function, R defaultValue) {
        if (!mAdapter.isEnabled()) {
    ...
    private void callService(RemoteExceptionIgnoringConsumer<IBluetoothVolumeControl> consumer) {
        if (!mAdapter.isEnabled()) {
            Log.d(TAG, "BluetoothAdapter is not enabled");
            return;
        }
