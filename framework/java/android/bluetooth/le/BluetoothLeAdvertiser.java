...
public final class BluetoothLeAdvertiser {
    ...
    private int totalBytes(AdvertiseData data, boolean isFlagsIncluded) {
        ...
        if (!encryptionBytesAdded && data.getDeviceNameEnc()) {
        ...
        if (data.getServiceSolicitationUuids() != null) {
            ...
            if (!encryptionBytesAdded && data.getServiceSolicitationUuidsEnc()) {
                size += ENCRYPTION_OVERHEAD_BYTES;
                encryptionBytesAdded = true;
            }
        }
        ...
        if (!data.getTransportDiscoveryData().isEmpty()) {
            if (!encryptionBytesAdded && data.getTransportDiscoveryDataEnc()) {
                size += ENCRYPTION_OVERHEAD_BYTES;
                encryptionBytesAdded = true;
            }
        }
        ...
        if (!data.getServiceData().isEmpty()) {
            if (!encryptionBytesAdded && data.getServiceDataEnc()) {
                size += ENCRYPTION_OVERHEAD_BYTES;
                encryptionBytesAdded = true;
            }
        }
        ...
        if ((data.getManufacturerSpecificData().size()) > 0) {
            if (!encryptionBytesAdded && data.getManufacturerSpecificDataEnc()) {
                size += ENCRYPTION_OVERHEAD_BYTES;
                encryptionBytesAdded = true;
            }
        }
        if (data.getIncludeTxPowerLevel()) {
            ...
            if (!encryptionBytesAdded && data.getTxPowerLevelEnc()) {
                size += ENCRYPTION_OVERHEAD_BYTES;
                encryptionBytesAdded = true;
            }
        }
