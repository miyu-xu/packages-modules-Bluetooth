...
public final class ScanFilter implements Parcelable {
    ...
    public boolean matchesWithoutAddress(ScanResult scanResult) {
        ...
        if (mManufacturerId >= 0) {
            if(scanRecord.hasSamsungManuData() && isSamsungManufacturerFilter()){
                if (!matchesPartialSamsungManuData(mManufacturerData, mManufacturerDataMask,
                    scanRecord)){
                    return false;
                }
            } else  if (!matchesPartialData(mManufacturerData, mManufacturerDataMask,
                    scanRecord.getManufacturerSpecificData(mManufacturerId))) {
                return false;
            }
        }
