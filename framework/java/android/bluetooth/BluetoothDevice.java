...
public final class BluetoothDevice implements Parcelable, Attributable {
    ...
    /**
     * Register bond loss for this {@link BluetoothDevice}.
     *
     * <p>This API is used to update the bond loss count when a bond loss is detected (increment the
    ...
    @RequiresBluetoothConnectPermission
    @RequiresPermission(allOf = {BLUETOOTH_CONNECT, BLUETOOTH_PRIVILEGED})
    public void updateBondLossCount(boolean isBondLossDetected) {
        if (DBG) log("updateBondLossCount");
        final IBluetooth service = getService();
        if (service == null || !isBluetoothEnabled()) {
            Log.e(TAG, "Bluetooth is not enabled. Cannot register bond loss.");
            if (DBG) log(Log.getStackTraceString(new Throwable()));
        } else {
            try {
                service.updateBondLossCount(this, mAttributionSource, isBondLossDetected);
            } catch (RemoteException e) {
                Log.e(TAG, e.toString() + "\n" + Log.getStackTraceString(new Throwable()));
            }
        }
    ...
    /**
     * Get the bond loss count for this {@link BluetoothDevice}.
     *
     * @hide
     */
    @RequiresBluetoothConnectPermission
    @RequiresPermission(allOf = {BLUETOOTH_CONNECT, BLUETOOTH_PRIVILEGED})
    public int getBondLossCount() {
        final IBluetooth service = getService();
        if (service == null || !isBluetoothEnabled()) {
            Log.e(TAG, "Bluetooth is not enabled. Cannot get bond loss counter.");
            if (DBG) log(Log.getStackTraceString(new Throwable()));
        } else {
            try {
                return service.getBondLossCount(this, mAttributionSource);
            } catch (RemoteException e) {
                Log.e(TAG, e.toString() + "\n" + Log.getStackTraceString(new Throwable()));
            }
        }
