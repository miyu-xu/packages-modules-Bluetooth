...
public final class BluetoothAdapter {
    ...
    public @interface NotifyBluetoothQualityReportReadyReturnValues {}

    /**
     * Called by Bluetooth Quality Report Native Interface to inform the listeners about Bluetooth
     * Quality.
     * <p>
     * This method will return
     * {@link BluetoothStatusCodes#ERROR_BLUETOOTH_NOT_ALLOWED} if called outside system server.
     ...
     * @return whether callbacks invoked successfully
     * @throws NullPointerException if device or bluetoothQualityReport are null
     * @throws IllegalArgumentException if the device's address is invalid
     *
     * @hide
     */
    ...
    @NotifyBluetoothQualityReportReadyReturnValues
    public int notifyBluetoothQualityReportReady(@NonNull BluetoothDevice device,
            @NonNull BluetoothQualityReport bluetoothQualityReport) {
        if (DBG) Log.d(TAG, "notifyBluetoothQualityReportReady(" + device + ", "
                        + bluetoothQualityReport + ")");
        Objects.requireNonNull(device, "device cannot be null");
        Objects.requireNonNull(bluetoothQualityReport, "bluetoothQualityReport cannot be null");
        if (!BluetoothAdapter.checkBluetoothAddress(device.getAddress())) {
            throw new IllegalArgumentException("device cannot have an invalid address");
        }
        ...
        try {
            if (mService != null) {
                final SynchronousResultReceiver<Integer> recv = SynchronousResultReceiver.get();
                mService.notifyBluetoothQualityReportReady(device, bluetoothQualityReport,
                        mAttributionSource, recv);
                return recv.awaitResultNoInterrupt(getSyncTimeout()).getValue(defaultValue);
            }
        } catch (RemoteException e) {
