...
public final class BluetoothAdapter {
    ...
    private final Map<BluetoothProfile, Pair<BluetoothProfile.ServiceListener, Integer>>
            mProfileConnections = new ConcurrentHashMap<>();
    ...
    public boolean getProfileProxy(
        ...
        if (binder != null) {
            profileProxy.onServiceConnected(binder);
        }

        mProfileConnections.put(profileProxy, new Pair(listener, profile));

        if (binder != null) {
            mMainHandler.post(() -> listener.onServiceConnected(profile, profileProxy));
        }
    ...
    public void closeProfileProxy(@NonNull BluetoothProfile proxy) {
        ...
        Pair<BluetoothProfile.ServiceListener, Integer> connection =
                mProfileConnections.remove(proxy);
        if (connection != null) {
            if (proxy instanceof BluetoothLeCallControl callControl) {
                callControl.unregisterBearer();
            }

            connection.first.onServiceDisconnected(connection.second);
            ...
            new IBluetoothManagerCallback.Stub() {
                ...
                public void onBluetoothOn() {
                    mProfileConnections.forEach(
                            (proxy, connection) -> {
                                BluetoothProfile.ServiceListener listener = connection.first;
                                int profile = connection.second;
                ...
                public void onBluetoothOff() {
                    mProfileConnections.forEach(
                            (proxy, connection) -> {
                                ...
                                proxy.onServiceDisconnected();
                                mMainHandler.post(() -> listener.onServiceDisconnected(profile));
                            });
                }
            };
     ...
     * @hide
     */
    public @Nullable IBinder getProfile(int profile) {
