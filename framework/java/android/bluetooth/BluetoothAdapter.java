...
public final class BluetoothAdapter {
    ...
    private static final class HciVendorSpecificCallbackRegistration {
        private final BluetoothHciVendorSpecificCallback mCallback;
        private final Executor mExecutor;
        private final Set<Integer> mEventCodeSet;
    ...
    private final IBluetoothHciVendorSpecificCallback mHciVendorSpecificCallbackStub =
            new IBluetoothHciVendorSpecificCallback.Stub() {
                ...
                private void execute(Consumer<BluetoothHciVendorSpecificCallback> consumer) {
                    synchronized (mHciVendorSpecificCallbackRegistration) {
                        if (mHciVendorSpecificCallbackRegistration == null) {
                            return;
                        }
                        mHciVendorSpecificCallbackRegistration.execute(consumer);
                    }
                }

                @Override
                public void onCommandStatus(int ocf, int status) {
                    execute((cb) -> cb.onCommandStatus(ocf, status));
                }

                @Override
                public void onCommandComplete(int ocf, byte[] returnParameters) {
                    execute((cb) -> cb.onCommandComplete(ocf, returnParameters));
                }

                @Override
                public void onEvent(int code, byte[] data) {
                    execute((cb) -> cb.onEvent(code, data));
                }
            };
        ...
        try {
            synchronized (mHciVendorSpecificCallbackRegistration) {
                ...
                mHciVendorSpecificCallbackRegistration =
                        new HciVendorSpecificCallbackRegistration(callback, eventCodeSet, executor);
                mHciVendorSpecificCallbackRegistration.registerToService(
    ...
    public int unregisterBluetouthHciVendorCallback(
        ...
        try {
            synchronized (mHciVendorSpecificCallbackRegistration) {
                if (mHciVendorSpecificCallbackRegistration == null
                        || mHciVendorSpecificCallbackRegistration.mCallback != callback) {
    ...
    public int sendBluetoothHciVendorSpecificCommand(
        ...
        if (ocf < 0 || (ocf >= 0x150 && ocf < 0x160) || ocf > 0x3ff) {
