...
/**
 * Represents Bluetooth Audio Policies of an HF device, which describes the
 * preferences of allowing or disallowing audio based on the use cases. The HF
 * devices shall send objects of this class to send its preference to the AG
 * devices.
 *
 * <p> HF side applications on can use {@link BluetoothDevice#setAudioPolicy}
 * API to set and send a {@link BluetoothAudioPolicy} object containing the
 * preference/policy values. This object will be stored in the memory of HF
 * side, will be send to the AG side using Android Specific AT Commands and will
 * be stored in the AG side memory and database.
 *
 * <p> HF side API {@link BluetoothDevice#getAudioPolicy} can be used to retrieve
