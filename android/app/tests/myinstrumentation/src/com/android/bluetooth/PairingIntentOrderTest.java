...
public class PairingIntentOrderTest {

    private static final String LOG_TAG = "PairingIntentOrderTest";
    ...
    private static SecurityStorageGrpc.SecurityStorageBlockingStub mSecurityStorageBlockingStub;
    ...
    public static void setUpClass() throws Exception {
        ...
        if (mBluetoothAdapter.isEnabled()) {
            // TODO(b/27859763)
            // This will trigger application exit if not Mock
    ...
    public void setUp() throws Exception {
        ...
        ReadLocalAddressResponse response = mHostBlockingStub.withWaitForReady().readLocalAddress(Empty.getDefaultInstance());

        mRefAddressByteString = response.getAddress();

        // TODO:  How to get local address?
        //MacAddress localMacAddress = MacAddress.fromString(mBluetoothAdapter.getAddress());
        MacAddress localMacAddress = MacAddress.fromString("DA:4C:10:DE:17:02");
    ...
    public void classicPairing() throws Exception {
        ...
        DeleteBondRequest deleteBondReq = DeleteBondRequest.newBuilder().setPublic(mDutAddressByteString).build();
        ...
        StreamObserver<PairingEventAnswer> answerObserver = mSecurityStub.onPairing(pairingEventObserver);
            ...
            public void onReceive(android.content.Context context, Intent intent) {
                ...
                switch (action) {
                    ...
                    case BluetoothDevice.ACTION_PAIRING_REQUEST: {
                        ...
                        // set pairing confirmation on REF
                        answerObserver.onNext(PairingEventAnswer.newBuilder().setEvent(mPairingEvent).setConfirm(true).build());
                        break;
                    }
        ...
        ConnectRequest connReq = ConnectRequest.newBuilder().setAddress(mDutAddressByteString).build();
        StreamObserver<ConnectResponse> connectResponseObserver = new StreamObserver<ConnectResponse>(){
        ...
        SecureRequest secureReq = SecureRequest.newBuilder().setConnection(refConnection).setClassic(SecurityLevel.LEVEL2).build();
        StreamObserver<SecureResponse> secureResponseObserver = new StreamObserver<SecureResponse>(){
        ...
        // Disconnect devices
        DisconnectRequest disconnectReq = DisconnectRequest.newBuilder().setConnection(refConnection).build();
