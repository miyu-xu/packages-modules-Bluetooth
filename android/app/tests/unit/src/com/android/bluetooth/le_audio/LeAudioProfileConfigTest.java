...
public class LeAudioProfileConfigTest {
    ...
    public void enablePbpPublicBroadcastAssistant() {
        LeAudioProfileConfig.enableUseCase(LeAudioProfileConfig.PBP_ASSISTANT);

        Set<Integer> expectedProfilesEnabled = new HashSet();
        expectedProfilesEnabled.add(LeAudioProfileConfig.PBP_ASSISTANT);
        expectedProfilesEnabled.addAll(
                LeAudioProfileConfig.PBP_PUBLIC_BROADCAST_ASSISTANT_DEPENDENCIES);
        assertOnlyTheseProfilesAreEnabled(expectedProfilesEnabled);
    }
