...
public class HfpClientConnectionServiceTest {
    ...
    public void setUp() {
        ...
        doReturn("com.android.bluetooth.hfpclient").when(mHfpClientConnectionService).getPackageName();
        doReturn(mHfpClientConnectionService).when(mHfpClientConnectionService).getApplicationContext();
        doReturn(mMockResources).when(mHfpClientConnectionService).getResources();
        doReturn(true).when(mMockResources).getBoolean(R.bool.hfp_client_connection_service_support_emergency_call);

        doReturn(Context.TELECOM_SERVICE).when(mHfpClientConnectionService).getSystemServiceName(TelecomManager.class);
        doReturn(mMockTelecomManager).when(mHfpClientConnectionService).getSystemService(Context.TELECOM_SERVICE);
        doReturn(getPhoneAccount(TEST_DEVICE)).when(mMockTelecomManager).getPhoneAccount(any());

        doReturn(Context.BLUETOOTH_SERVICE).when(mHfpClientConnectionService).getSystemServiceName(BluetoothManager.class);
        doReturn(targetContext.getSystemService(BluetoothManager.class)).when(mHfpClientConnectionService).getSystemService(Context.BLUETOOTH_SERVICE);
    }
    ...
    private void setupDeviceConnection(BluetoothDevice device) throws Exception {
        mHfpClientConnectionService.onConnectionStateChanged(device, BluetoothProfile.STATE_CONNECTED, BluetoothProfile.STATE_CONNECTING);
    ...
    public void audioStateChanged_scoStateChanged() throws Exception {
        ...
        assertThat(block).isNotNull();
        assertThat(block.getDevice()).isEqualTo(TEST_DEVICE);
        assertThat(block.getAudioState()).isEqualTo(HeadsetClientHalConstants.AUDIO_STATE_CONNECTED);
    }
