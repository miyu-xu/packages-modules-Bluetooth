...
public class AvrcpControllerStateMachineTest {
    ...
    private static final int BROWSE_TIMEOUT_TEST_MILLIS = 10000;
    ...
    public void setUp() throws Exception {
        Log.i("AvrcpControllerStateMachineTest", "Setup");
    ...
    public void tearDown() throws Exception {
        Log.i("AvrcpControllerStateMachineTest", "TearDown");
    ...
    public void testNowPlayingListChangedWhileFetchingNowPlayingList_fetchAbortedAndRestarted() {
        ...
        verify(mAvrcpControllerService, times(1)).getNowPlayingListNative(
                eq(mTestAddress), eq(0), eq(19));
        mAvrcpStateMachine.sendMessage(
                AvrcpControllerStateMachine.MESSAGE_PROCESS_GET_FOLDER_ITEMS, new ArrayList<AvrcpItem>(nowPlayingList.subList(0, 20)));
        TestUtils.waitForLooperToFinishScheduledTask(mAvrcpStateMachine.getHandler().getLooper());
        verify(mAvrcpControllerService, times(1)).getNowPlayingListNative(
                eq(mTestAddress), eq(20), eq(39));

        // Force a now playing content invalidation and verify attempted download
        // mAvrcpStateMachine.sendMessage(AvrcpControllerStateMachine.MESSAGE_PROCESS_ADDRESSED_PLAYER_CHANGED);
        mAvrcpStateMachine.nowPlayingContentChanged();
        TestUtils.waitForLooperToFinishScheduledTask(mAvrcpStateMachine.getHandler().getLooper());
        ...
        mAvrcpStateMachine.sendMessage(
                AvrcpControllerStateMachine.MESSAGE_PROCESS_GET_FOLDER_ITEMS, new ArrayList<AvrcpItem>(nowPlayingList.subList(20, 25)));
        TestUtils.waitForLooperToFinishScheduledTask(mAvrcpStateMachine.getHandler().getLooper());
        ...
        mAvrcpStateMachine.sendMessage(
                AvrcpControllerStateMachine.MESSAGE_PROCESS_GET_FOLDER_ITEMS, new ArrayList<AvrcpItem>(updatedNowPlayingList.subList(0, 20)));
        mAvrcpStateMachine.sendMessage(
                AvrcpControllerStateMachine.MESSAGE_PROCESS_GET_FOLDER_ITEMS, new ArrayList<AvrcpItem>(updatedNowPlayingList.subList(20, 25)));
        mAvrcpStateMachine.sendMessage(
                AvrcpControllerStateMachine.MESSAGE_PROCESS_GET_FOLDER_ITEMS_OUT_OF_RANGE);
    ...
    public void testAddressedPlayerChangedWhileFetchingNowPlayingList_fetchAbortedAndRestarted() {
        ...
        mAvrcpStateMachine.sendMessage(AvrcpControllerStateMachine.MESSAGE_PROCESS_ADDRESSED_PLAYER_CHANGED, 1);
        TestUtils.waitForLooperToFinishScheduledTask(mAvrcpStateMachine.getHandler().getLooper());
    ...
    /**
     * Test making a browse request (arbitrarily now-playing here) while browsing is not connected.
     * The operation should time out
     */
    @Test
    public void testMakeBrowseRequestWithoutBrowseConnected_noFetchAndEmptyListNotified() {
        ...
        verify(mAvrcpControllerService,
                timeout(BROWSE_TIMEOUT_TEST_MILLIS).times(0)).getNowPlayingListNative(
                eq(mTestAddress), eq(0), eq(19));
    }
}
