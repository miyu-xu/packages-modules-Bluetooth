...
public class PbapClientBinderTest {
    ...
    public void testGetDevicesMatchingConnectionStates_afterCleanup_returnsEmptyList() {
        mPbapClientBinder.cleanup();
        int[] states = new int[] {BluetoothProfile.STATE_CONNECTED};
        List<BluetoothDevice> devices = mPbapClientBinder.getDevicesMatchingConnectionStates(states, mAttributionSource);
    ...
    public void testSetConnectionPolicy_afterCleanup_returnsFalse() {
        mPbapClientBinder.cleanup();
        int connectionPolicy = BluetoothProfile.CONNECTION_POLICY_ALLOWED;
        boolean result = mPbapClientBinder.setConnectionPolicy(mTestDevice, connectionPolicy, mAttributionSource);
