...
public class BluetoothOppReceiverTest {
    ...
    public void onReceive_withActionTransferComplete_noBroadcastSent() throws Exception {
        ...
        MockitoSession mockitoSession = ExtendedMockito.mockitoSession()
                .initMocks(this)
                .mockStatic(BluetoothOppReceiverTest.class)
                .spyStatic(ActivityManager.class)
                .startMocking();
        ExtendedMockito.doReturn(null).when(ActivityManager::getService);
        ExtendedMockito.doReturn(1).when(BluetoothOppReceiverTest::aaaa);
        activityScenario.onActivity(activity -> {
            mReceiver.onReceive(mContext, intent);
        });
        ...
        // context instead
        ExtendedMockito.verify(() -> BluetoothOppReceiverTest.aaaa()); //Toast.makeText(any(), any(CharSequence.class), anyInt()));
        verify(mContext, never()).sendBroadcast(any(), eq(Constants.HANDOVER_STATUS_PERMISSION),
                any());
        mockitoSession.finishMocking();
    }

    static int aaaa() throws Exception {
        Log.d("hieu","sadsd");
        return 0;
    }
}
