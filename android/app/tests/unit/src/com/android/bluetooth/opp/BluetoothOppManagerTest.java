...
public class BluetoothOppManagerTest {
    ...
    @Test
    public void saveSendingFileInfo_singleFile_restoreApplicationData_dataIsPersist() {
        BluetoothOppManager bluetoothOppManager = BluetoothOppManager.getInstance(mContext);
        bluetoothOppManager.mSendingFlag = true;
        bluetoothOppManager.saveSendingFileInfo("text/plain", "content:///abc/xyz.txt", false, true);
    ...
    @Test
    public void saveSendingFileInfo_multipleFiles_restoreApplicationData_dataIsPersist() {
        BluetoothOppManager bluetoothOppManager = BluetoothOppManager.getInstance(mContext);
        bluetoothOppManager.mSendingFlag = true;
        bluetoothOppManager.saveSendingFileInfo("text/plain", new ArrayList<Uri>(
                        List.of(Uri.parse("content:///abc/xyz.txt"),
                                Uri.parse("content:///123/456.txt"))), false,
                true);
    ...
    public void addToAcceptList_isAcceptedList_checkCorrectly() {
        ...
        bluetoothOppManager.addToAcceptlist(address3);
        assertThat(bluetoothOppManager.isAcceptlisted(address3)).isTrue();
    }

    @Test
    public void startTransfer_addMultiple_insertMultiple() {
        BluetoothOppManager bluetoothOppManager = BluetoothOppManager.getInstance(mContext);
        ...
        bluetoothOppManager.saveSendingFileInfo("text/plain", new ArrayList<Uri>(
                        List.of(Uri.parse("content:///abc/xyz.txt"),
                                Uri.parse("content:///123/456.txt"))), false,
                true);
        BluetoothDevice device = ((BluetoothManager) mContext.getSystemService(
                Context.BLUETOOTH_SERVICE)).getAdapter().getRemoteDevice(address);
        bluetoothOppManager.startTransfer(device);
        // add 2 files
        verify(mCallProxy, timeout(5_000).times(2)).contentResolverInsert(any(),
                nullable(Uri.class),
                nullable(ContentValues.class));
    }

    @Test
    public void startTransfer_addSingle_insertSingle() {
        BluetoothOppManager bluetoothOppManager = BluetoothOppManager.getInstance(mContext);
        String address = "AA:BB:CC:DD:EE:FF";
    ...
    public void cleanUpSendingFileInfo_fileInfoCleaned() {
        ...
        BluetoothOppManager.getInstance(mContext).saveSendingFileInfo("text/plain", uri.toString(), false,
                true);
