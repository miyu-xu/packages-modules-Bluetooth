...
public final class MsftAdvMonitorTest {
    ...
    public void testFastPairScanFilter() {
        ...
        String fastPairUuid = "0000fe2c-0000-1000-8000-00805f9b34fb";
        byte[] serviceData = new byte[] {(byte) 0xfc, (byte) 0x12, (byte) 0x8e};

        ParcelUuid serviceDataUuid = ParcelUuid.fromString(fastPairUuid);
        ScanFilter filter =
                new ScanFilter.Builder().setServiceData(serviceDataUuid, serviceData).build();
        MsftAdvMonitor monitor = new MsftAdvMonitor(filter);

        assertMonitorConstants(monitor);
        assertThat(monitor.getPatterns().length).isEqualTo(1);
