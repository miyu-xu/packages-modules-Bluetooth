...
public class AdapterServiceFactoryResetTest {
    ...
    public void setUp() throws PackageManager.NameNotFoundException {
        ...
        when(mMockContext.getSystemService(eq(Context.DEVICE_POLICY_SERVICE))).thenReturn(
                mMockDevicePolicyManager);
        when(mMockContext.getSystemServiceName(DevicePolicyManager.class))
                .thenReturn(Context.DEVICE_POLICY_SERVICE);
        when(mMockContext.getSystemService(eq(Context.ALARM_SERVICE))).thenReturn(mMockAlarmManager);
    ...
    private void doDisable() {
        ...
        verify(mMockContext, timeout(ONE_SECOND_MS).times(4))
                .startService(any());
