...
public class VolumeControlInputDescriptorTest {
    ...
    public void setFoo_withAllValidId_valuesAreUpdated() {
        for (int i = 0; i < NUMBER_OF_INPUT; i++) {
            assertThat(mDescriptor.getStatus(i)).isEqualTo(AudioInputStatus.INACTIVE);
            mDescriptor.setStatus(i, AudioInputStatus.ACTIVE);
            assertThat(mDescriptor.getStatus(i)).isEqualTo(AudioInputStatus.ACTIVE);
        }
    }

    @Test
    public void getStatus_whenNeverSet_defaultToInactive() {
        assertThat(mDescriptor.getStatus(VALID_ID)).isEqualTo(AudioInputStatus.INACTIVE);
    }

    @Test
    public void setStatus_withValidId_valueIsUpdated() {
        int newStatus = AudioInputStatus.ACTIVE;
    ...
    public void getType_whenNeverSet_defaultToUnspecified() {
        assertThat(mDescriptor.getType(VALID_ID)).isEqualTo(AudioInputType.UNSPECIFIED);
    }

    @Test
    public void setType_withValidId_valueIsUpdated() {
        int newType = AudioInputType.AMBIENT;
    ...
    public void setType_withInvalidId_valueIsNotUpdated() {
        int newType = AudioInputType.BLUETOOTH;
