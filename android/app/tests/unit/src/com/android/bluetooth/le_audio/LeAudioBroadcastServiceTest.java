...
public class LeAudioBroadcastServiceTest {
    ...
    void verifyBroadcastStarted(int broadcastId, BluetoothLeBroadcastSettings settings) {
        ...
        int subgroupSize = settings.getSubgroupSettings().size();
        byte[][] expectedArray = new byte[subgroupSize][];

        for (int i = 0; i < subgroupSize; i++) {
            byte[] metadata =
                    settings.getSubgroupSettings().get(i).getContentMetadata().getRawMetadata();
            expectedArray[i] = new byte[metadata.length + 1];
            expectedArray[i][0] = (byte) settings.getSubgroupSettings().get(i).getPreferredQuality();
            System.arraycopy(metadata, 0, expectedArray[i], 1, metadata.length);
        }
    ...
    public void testCreateBroadcastNative() {
        ...
        verifyBroadcastStarted(broadcastId, buildBroadcastSettingsFromMetadata(1, meta, code));
    }
    ...
    public void testCreateBroadcastNativeMultiGroups() {
        ...
        verifyBroadcastStarted(broadcastId, buildBroadcastSettingsFromMetadata(2, meta, code));
    }
    ...
    public void testCreateBroadcastNativeFailed() {
        ...
        BluetoothLeAudioContentMetadata meta = meta_builder.build();
        BluetoothLeBroadcastSettings settings = buildBroadcastSettingsFromMetadata(1, meta, code);
        mService.createBroadcast(settings);

        // Test data only has one subgroup
        byte[] metadata =
                settings.getSubgroupSettings().get(0).getContentMetadata().getRawMetadata();
        byte[][] expectedArray = new byte[1][metadata.length + 1];
        expectedArray[0][0] = (byte) settings.getSubgroupSettings().get(0).getPreferredQuality();
        System.arraycopy(metadata, 0, expectedArray[0], 1, metadata.length);
    ...
    public void testBroadcastInvalidBroadcastIdRequest() {
        ...
        mService.updateBroadcast(broadcastId,
                buildBroadcastSettingsFromMetadata(1, meta_builder.build(), null));
    ...
    public void testGetAllBroadcastMetadata() {
        ...
        meta_builder.setProgramInfo("Public broadcast info");
        BluetoothLeAudioContentMetadata meta = meta_builder.build();
        mService.createBroadcast(buildBroadcastSettingsFromMetadata(1, meta, code));
