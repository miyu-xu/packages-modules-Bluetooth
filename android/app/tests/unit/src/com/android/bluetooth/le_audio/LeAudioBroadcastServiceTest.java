...
public class LeAudioBroadcastServiceTest {
    ...
    public void testStackEventBroadcastCreated() {
        ...
        Assert.assertEquals(broadcastId, intent.getIntExtra(BluetoothLeAudio.EXTRA_LE_AUDIO_BROADCAST_INSTANCE_ID, -1));
        Assert.assertEquals(true,
                intent.getBooleanExtra(BluetoothLeAudio.EXTRA_LE_AUDIO_BROADCAST_INSTANCE_STATUS, false));
    }
    ...
    public void testStackEventBroadcastDestroyed() {
        ...
        Intent intent = TestUtils.waitForIntent(TIMEOUT_MS, mIntentQueue);
        Assert.assertNotNull(intent);
        Assert.assertEquals(BluetoothLeAudio.ACTION_LE_AUDIO_BROADCAST_DESTROYED, intent.getAction());
        Assert.assertEquals(broadcastId, intent.getIntExtra(BluetoothLeAudio.EXTRA_LE_AUDIO_BROADCAST_INSTANCE_ID, -1));
    }
    ...
    public void testStackEventBroadcastStateChanged() {
        ...
        mNativeInterface.onBroadcastStateChanged(broadcastId, LeAudioStackEvent.BROADCAST_STATE_STREAMING);

        Intent intent = TestUtils.waitForIntent(TIMEOUT_MS, mIntentQueue);
        Assert.assertNotNull(intent);
        Assert.assertEquals(BluetoothLeAudio.ACTION_LE_AUDIO_BROADCAST_STATE, intent.getAction());
        Assert.assertEquals(broadcastId, intent.getIntExtra(BluetoothLeAudio.EXTRA_LE_AUDIO_BROADCAST_INSTANCE_ID, -1));
        Assert.assertEquals(BluetoothLeAudio.BROADCAST_STATE_STREAMING, intent.getIntExtra(
                BluetoothLeAudio.EXTRA_LE_AUDIO_BROADCAST_STATE, BluetoothLeAudio.BROADCAST_STATE_STOPPED));
    }
    ...
    public void testStackEventBroadcastId() {
        ...
        Assert.assertEquals(instanceId, intent.getIntExtra(BluetoothLeAudio.EXTRA_LE_AUDIO_BROADCAST_INSTANCE_ID, -1));
