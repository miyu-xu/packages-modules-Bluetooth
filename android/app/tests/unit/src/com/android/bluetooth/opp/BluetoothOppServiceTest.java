...
public class BluetoothOppServiceTest {
    ...
    public void setUp() throws Exception {
        ...
        // BluetoothOppNotification (mNotifier) in BluetoothOppService can create another thread,
        // which may causes the thread to fail. Mocking it since we have no intention of testing
        // the thread
        ...
        doReturn(bluetoothOppNotification).when(mBluetoothMethodProxy).newBluetoothOppNotification(
                any());

        // if there is BluetoothShare.CONTENT_URI content provider client, a new thread to trim
        // database will be created. This can cause multi-thread problems. We have no intention to
        // test thread creation
        doReturn(null).when(mBluetoothMethodProxy).contentResolverAcquireContentProviderClient(
                any(), eq(BluetoothShare.CONTENT_URI));
    ...
    public void trimDatabase_trimsOldOrInvisibleRecords() {
        ...
        doReturn(1 /* any int is Ok */).when(mBluetoothMethodProxy).contentResolverDelete(
        ...
        doReturn(cursor).when(mBluetoothMethodProxy).contentResolverQuery(eq(contentResolver),
        ...
        verify(mBluetoothMethodProxy).contentResolverDelete(eq(contentResolver),
