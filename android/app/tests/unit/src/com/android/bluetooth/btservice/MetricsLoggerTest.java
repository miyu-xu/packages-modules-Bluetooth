...
public class MetricsLoggerTest {
    ...
    public void testDeviceNameUploading() {
        ...
        mTestableMetricsLogger.logEncryptedBluetoothDeviceName("AirpoDspro");
        Assert.assertEquals(1,
                mTestableMetricsLogger.mTestableDeviceNames.get("airpodspro").intValue());

        mTestableMetricsLogger.logEncryptedBluetoothDeviceName("AirpoDs-pro");
        Assert.assertEquals(2,
                mTestableMetricsLogger.mTestableDeviceNames.get("airpodspro").intValue());

        mTestableMetricsLogger.logEncryptedBluetoothDeviceName("Someone's AirpoDs");
        Assert.assertEquals(1,
                mTestableMetricsLogger.mTestableDeviceNames.get("airpods").intValue());

        mTestableMetricsLogger.logEncryptedBluetoothDeviceName("Who's Pixel 7");
        Assert.assertEquals(1,
                mTestableMetricsLogger.mTestableDeviceNames.get("pixel7").intValue());

        mTestableMetricsLogger.logEncryptedBluetoothDeviceName("My Pixel   7   - PRO");
        Assert.assertEquals(1,
                mTestableMetricsLogger.mTestableDeviceNames.get("pixel7pro").intValue());

        mTestableMetricsLogger.logEncryptedBluetoothDeviceName("abcgfDG gdfg");
        Assert.assertEquals(1,
                mTestableMetricsLogger.mTestableDeviceNames.get("abcgfdg").intValue());

        mTestableMetricsLogger.logEncryptedBluetoothDeviceName("陈的pixel 7手机");
        Assert.assertEquals(2,
                mTestableMetricsLogger.mTestableDeviceNames.get("pixel7").intValue());

        mTestableMetricsLogger.mTestableDeviceNames.clear();
        mTestableMetricsLogger.logEncryptedBluetoothDeviceName("SomeDevice1");
        Assert.assertTrue(mTestableMetricsLogger.mTestableDeviceNames.isEmpty());

        mTestableMetricsLogger.logEncryptedBluetoothDeviceName("Some Device-2");
        Assert.assertTrue(mTestableMetricsLogger.mTestableDeviceNames.isEmpty());
    }
}
