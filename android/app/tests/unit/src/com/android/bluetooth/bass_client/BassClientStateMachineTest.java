...
public class BassClientStateMachineTest {
    ...
    private boolean isChangingConnectionState(Class currentType, Class nextType) {
        if ((mBassClientStateMachine.getCurrentState().getClass() != nextType)
    ...
    private <T> void sendMessageAndVerifyTransition(Message msg, Class<T> type) {
        ...
        mBassClientStateMachine.sendMessage(msg);
        if (isChangingConnectionState(mBassClientStateMachine.getCurrentState().getClass(), type)) {
            // Verify that one connection state broadcast is executed
