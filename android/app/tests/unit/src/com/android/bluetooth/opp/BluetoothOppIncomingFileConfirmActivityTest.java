...
public class BluetoothOppIncomingFileConfirmActivityTest {
    ...
    public void onCreate_clickConfirmOk_saveUSER_CONFIRMATION_CONFIRMED()
        ...
        // but onclick function is not triggered. It works normally if sleep for a few seconds
        Thread.sleep(3_000);
    ...
    public void onTimeout_sendIntentWithUSER_CONFIRMATION_TIMEOUT_ACTION_finish() throws Exception {
        ...
        assertThat(activityScenario.getState()).isNotEqualTo(DESTROYED);
        Intent in = new Intent(BluetoothShare.USER_CONFIRMATION_TIMEOUT_ACTION);
        mTargetContext.sendBroadcast(in);

        // To work arround (possibly) Epresso's bug. The dialog button is clicked (no error throw)
        // but onclick function is not triggered. It works normally if sleep for a few seconds
        Thread.sleep(3_000);
        assertThat(activityScenario.getState()).isEqualTo(DESTROYED);
    }
