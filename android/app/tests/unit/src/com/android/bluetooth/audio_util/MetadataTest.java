...
public class MetadataTest {
    ...
    public void setUp() throws Exception {
        ...
        SystemProperties.set(Util.VFS_COVER_ART_ENABLED_PROPERTY, "true");
    ...
    public void tearDown() throws Exception {
        ...
        mTargetContext = null;
        mMockContext = null;
        SystemProperties.set(Util.VFS_COVER_ART_ENABLED_PROPERTY, "false");
    }
        ...
        MediaMetadata.Builder builder =
                new MediaMetadata.Builder()
                        ...
                        .putLong(
                                ...
                                Long.parseLong(SONG_TRACK_NUM))
                        .putLong(
                                MediaMetadata.METADATA_KEY_NUM_TRACKS,
                                Long.parseLong(SONG_NUM_TRACKS))
                        .putString(MediaMetadata.METADATA_KEY_GENRE, SONG_GENRE)
                        .putLong(
                                MediaMetadata.METADATA_KEY_DURATION, Long.parseLong(SONG_DURATION));
    ...
    /**
     * Make sure building with a MediaMetadata that contains URI art when URI art is disabled yields
     * no cover art.
     */
    @Test
    ...
    /**
     * Make sure you can create a Metadata from a MediaDescription that contains cover artwork as an
     * icon Uri
     */
    @Test
    ...
    public void testBuildMetadataFromMediaItemWithIconUri() {
        ...
        MediaItem item = getMediaItem(description);
        Metadata metadata =
                new Metadata.Builder().useContext(mMockContext).fromMediaItem(item).build();
        assertMetadata(
                SONG_MEDIA_ID,
                SONG_TITLE,
                SONG_ARTIST,
    ...
    public void testBuildMetadataFromQueueItemWithIconUri() {
        ...
        QueueItem queueItem = getQueueItem(description);
        Metadata metadata =
                new Metadata.Builder().useContext(mMockContext).fromQueueItem(queueItem).build();
        assertMetadata(
                SONG_MEDIA_ID,
                SONG_TITLE,
                SONG_ARTIST,
    ...
    /**
     * Make sure building with a QueueItem that contains URI art when URI art is disabled yields no
     * cover art.
     */
    @Test
    ...
    public void testBuildMetadataUseDefaults() {
        ...
        assertMetadata(
                Metadata.EMPTY_MEDIA_ID,
                Metadata.EMPTY_TITLE,
                Metadata.EMPTY_ARTIST,
                Metadata.EMPTY_ALBUM,
                Metadata.EMPTY_TRACK_NUM,
                Metadata.EMPTY_NUM_TRACKS,
                Metadata.EMPTY_GENRE,
                Metadata.EMPTY_DURATION,
                DEFAULT_IMAGE,
                metadata);
    }
    ...
    public void testBuildMetadataUseDefaultsAndPartialFields() {
        MediaDescription description = getMediaDescription(null, null, null);
        Metadata metadata =
                new Metadata.Builder().useDefaults().fromMediaDescription(description).build();
        assertMetadata(
                SONG_MEDIA_ID,
                SONG_TITLE,
                SONG_ARTIST,
    ...
    /**
     * Build a Metadata using the defaults. Call useDefaults at the end after other fields have been
     * extracted from the media framework objects. Ensure we don't overwrite existing values with
     * defaults. Ensure that the metadata created is the same (by field) as the one where
