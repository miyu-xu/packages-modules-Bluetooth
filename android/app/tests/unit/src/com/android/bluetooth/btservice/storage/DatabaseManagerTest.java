...
public final class DatabaseManagerTest {
    ...
    public void testSetGetAudioPolicyMetaData() {
        int badKey = 100;
        BluetoothAudioPolicy value = new BluetoothAudioPolicy(
    ...
    public void testDatabaseMigration_114_115() throws IOException {
        ...
        Cursor cursor = db.query("SELECT * FROM metadata");
        assertHasColumn(cursor, "call_pick_up_audio_policy", true);
        assertHasColumn(cursor, "connecting_audio_policy", true);
        assertHasColumn(cursor, "in_band_ring_audio_policy", true);
        while (cursor.moveToNext()) {
            // Check the new columns was added with default value
            assertColumnBlobData(cursor, "call_pick_up_audio_policy", null);
            assertColumnBlobData(cursor, "connecting_audio_policy", null);
            assertColumnBlobData(cursor, "in_band_ring_audio_policy", null);
        }
    }
    ...
    void testSetGetAudioPolicyMetadataCase(boolean stored,
        ...
        BluetoothAudioPolicy testPolicy = new BluetoothAudioPolicy();
        if (stored) {
            Metadata data = new Metadata(TEST_BT_ADDR);
            mDatabaseManager.mMetadataCache.put(TEST_BT_ADDR, data);
            mDatabase.insert(data);
