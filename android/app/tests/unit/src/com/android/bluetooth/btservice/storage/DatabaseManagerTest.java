...
public final class DatabaseManagerTest {
    ...
    public void setUp() throws Exception {
        ...
        doNothing().when(mAdapterService).onMetadataChanged(anyString(), anyInt(), any());
    ...
    public void testRemoveUnusedMetadata_WithSingleBondedDevice() {
        ...
        verify(mAdapterService).onMetadataChanged(OTHER_BT_ADDR1, 0, null);
    ...
    public void testRemoveUnusedMetadata_WithMultiBondedDevices() {
        ...
        verify(mAdapterService).onMetadataChanged(TEST_BT_ADDR, 0, null);
    ...
    void testSetGetCustomMetaCase(boolean stored, int key, byte[] value, boolean expectedResult) {
        ...
        if (stored) {
            ...
            verify(mAdapterService).onMetadataChanged(TEST_BT_ADDR, key, testValue);
            verifyTime++;
        }
        Assert.assertEquals(
        ...
        if (expectedResult) {
            ...
            verify(mAdapterService, times(verifyTime)).onMetadataChanged(TEST_BT_ADDR, key, value);
    ...
    public void setCustomMetadata_reentrantCallback_noDeadLock() throws Exception {
        ...
        doAnswer(answer).when(mAdapterService).onMetadataChanged(anyString(), anyInt(), any());
