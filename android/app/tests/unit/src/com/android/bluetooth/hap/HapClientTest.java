...
public class HapClientTest {
    ...
    public void setUp() throws Exception {
        ...
        doReturn(Arrays.asList(mDevice, mDevice2)).when(mCsipService).getGroupDevices(groupId2);
        ...
        doReturn(Arrays.asList(mDevice3)).when(mCsipService).getGroupDevices(0x03);
        doReturn(groups3).when(mCsipService).getGroupUuidMapByDevice(mDevice3);

        doReturn(Arrays.asList(mDevice)).when(mCsipService).getGroupDevices(0x01);
    ...
    public void testSetPresetNameForGroup() {
        doReturn(new ParcelUuid[]{BluetoothUuid.HAS}).when(mAdapterService)
                .getRemoteUuids(any(BluetoothDevice.class));
        int test_group = 0x02;
        for (BluetoothDevice device : mCsipService.getGroupDevices(test_group)) {
