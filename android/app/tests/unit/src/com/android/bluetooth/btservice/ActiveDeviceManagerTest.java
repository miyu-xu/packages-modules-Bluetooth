...
public class ActiveDeviceManagerTest {
    ...
    private void a2dpConnected(BluetoothDevice device, boolean supportHfp) {
        mDatabaseManager.setReturnForGetProfileConnectionPolicy(
                device,
    ...
    private class TestDatabaseManager extends DatabaseManager {
        ...
        public void setReturnForGetProfileConnectionPolicy(
                BluetoothDevice device, int profile, int ret) {
            SparseIntArray policy = mProfileConnectionPolicy.get(device);
            if (policy == null) {
                policy = new SparseIntArray();
            }
            policy.put(profile, ret);
            mProfileConnectionPolicy.put(device, policy);
        }
