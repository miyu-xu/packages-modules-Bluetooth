...
public class PbapClientStateMachineTest {
    ...
    //         PbapSdpRecord.FEATURE_DOWNLOADING | PbapSdpRecord.FEATURE_DATABASE_IDENTIFIER | PbapSdpRecord.FEATURE_FOLDER_VERSION_COUNTERS;
    ...
    ArgumentCaptor<PbapClientContactsStorage.Callback> mCaptor = ArgumentCaptor.forClass(PbapClientContactsStorage.Callback.class);
    ...
    public void setUp() throws Exception {
        ...
        doAnswer(invocation -> {
            ...
            return true;
        }).when(mMockStorage).addAccount(any(Account.class));

        doAnswer(invocation -> {
            Account account = (Account) invocation.getArgument(0);
            mMockedAccounts.remove(account);
            return true;
        }).when(mMockStorage).removeAccount(any(Account.class));

        doAnswer(invocation -> {
            return mMockedAccounts;
        }).when(mMockStorage).getStorageAccounts();

        mTestLooper = new TestLooper();

        mPbapClientStateMachine = new PbapClientStateMachine(mTestDevice, mMockStorage, mMockContext, mTestLooper.getLooper(), mMockCallback, mMockObexClient);
    ...
    public void tearDown() throws Exception {
        if (mPbapClientStateMachine != null) {
            // TODO: There isn't a stop?
            // mPbapClientStateMachine.stop();
        }
    }
    ...
    public void testDisconnected_receivedDisconnect_nothingHappens() {
        ...
        assertThat(mPbapClientStateMachine.getConnectionState()).isEqualTo(BluetoothProfile.STATE_DISCONNECTED);
        verify(mMockCallback, times(0)).onConnectionStateChanged(anyInt(), anyInt());
    }
    ...
    public void testDisconnected_receivedConnect_connectionStateChangesToConnecting() {
        ...
        assertThat(mPbapClientStateMachine.getConnectionState()).isEqualTo(BluetoothProfile.STATE_CONNECTING);
        verify(mMockCallback, times(1)).onConnectionStateChanged(eq(BluetoothProfile.STATE_DISCONNECTED), eq(BluetoothProfile.STATE_CONNECTING));
    }

    // enter disconnected from different state -> quit()
    ...
    @Test
    public void testConnecting_receivedSdpRecordWithTransportL2cap_connectionReqOnL2cap() {
        testDisconnected_receivedConnect_connectionStateChangesToConnecting();
        mPbapClientStateMachine.onSdpRecordReceived(makeSdpRecord(L2CAP_PSM, INVALID_RFCOMM, SUPPORTED_FEATURES, SUPPORTED_REPOSITORIES));
        mTestLooper.dispatchAll();
    ...
    @Test
    public void testConnecting_receivedSdpRecordWithTransportRfcomm_connectionReqOnRfcomm() {
        testDisconnected_receivedConnect_connectionStateChangesToConnecting();
        mPbapClientStateMachine.onSdpRecordReceived(makeSdpRecord(INVALID_L2CAP, RFCOMM_CHANNEL, SUPPORTED_FEATURES, SUPPORTED_REPOSITORIES));
        mTestLooper.dispatchAll();
    ...
    @Test
    public void testConnecting_receivedSdpRecordWithTransportL2capOrRfcomm_connectionReqOnL2cap() {
        testDisconnected_receivedConnect_connectionStateChangesToConnecting();
        mPbapClientStateMachine.onSdpRecordReceived(makeSdpRecord(L2CAP_PSM, RFCOMM_CHANNEL, SUPPORTED_FEATURES, SUPPORTED_REPOSITORIES));
        mTestLooper.dispatchAll();
    ...
    public void testConnecting_receivedObexConnection_transitionToConnected() {
        testConnecting_receivedSdpRecordWithTransportL2cap_connectionReqOnL2cap();
        setAndNotifyObexClientStatus(BluetoothProfile.STATE_DISCONNECTED, BluetoothProfile.STATE_CONNECTING);
        setAndNotifyObexClientStatus(BluetoothProfile.STATE_CONNECTING, BluetoothProfile.STATE_CONNECTED);
        mTestLooper.dispatchAll();

        assertThat(mPbapClientStateMachine.getConnectionState()).isEqualTo(BluetoothProfile.STATE_CONNECTED);
        verify(mMockCallback, times(1)).onConnectionStateChanged(eq(BluetoothProfile.STATE_CONNECTING), eq(BluetoothProfile.STATE_CONNECTED));
    }
    ...
    public void testConnecting_receivedConnectionTimeout_transitionToDisconnecting() {
        ...
        verify(mMockCallback, times(1)).onConnectionStateChanged(eq(BluetoothProfile.STATE_CONNECTING), eq(BluetoothProfile.STATE_DISCONNECTING));
        verify(mMockCallback, times(1)).onConnectionStateChanged(eq(BluetoothProfile.STATE_DISCONNECTING), eq(BluetoothProfile.STATE_DISCONNECTED));
        assertThat(mPbapClientStateMachine.getConnectionState()).isEqualTo(BluetoothProfile.STATE_DISCONNECTED);
    }
    ...
    public void testConnecting_receivedObexDisconnection_transitionToDisconnected() {
        testConnecting_receivedSdpRecordWithTransportL2cap_connectionReqOnL2cap();
        setAndNotifyObexClientStatus(BluetoothProfile.STATE_DISCONNECTED, BluetoothProfile.STATE_CONNECTING);
        setAndNotifyObexClientStatus(BluetoothProfile.STATE_CONNECTING, BluetoothProfile.STATE_DISCONNECTED);
        mTestLooper.dispatchAll();
    ...
    public void testConnected_storageReadyImmediatelyWithAccountReady_downloadStarted() {
        ...
        verify(mMockObexClient).requestPhonebookMetadata(anyString(), any(PbapApplicationParameters.class));
    }
    ...
    public void testConnected_receivedAccountRemoved_transitionToDisconnecting() {
        ...
        assertThat(mPbapClientStateMachine.getConnectionState()).isEqualTo(BluetoothProfile.STATE_DISCONNECTING);
        verify(mMockCallback, times(1)).onConnectionStateChanged(eq(BluetoothProfile.STATE_CONNECTED), eq(BluetoothProfile.STATE_DISCONNECTING));
    }
    ...
    public void testDownloading_onlyFavoritesSupported_favoritesDownloaded() {
        ...
        mPbapClientStateMachine.connect();
        mPbapClientStateMachine.onSdpRecordReceived(makeSdpRecord(L2CAP_PSM, INVALID_RFCOMM, SUPPORTED_FEATURES, PbapSdpRecord.REPOSITORY_FAVORITES));
        setAndNotifyObexClientStatus(BluetoothProfile.STATE_DISCONNECTED, BluetoothProfile.STATE_CONNECTING);
        setAndNotifyObexClientStatus(BluetoothProfile.STATE_CONNECTING, BluetoothProfile.STATE_CONNECTED);
        mTestLooper.dispatchAll();
    ...
    public void testDownloading_onlyLocalPhonebookSupported_localPhonebooksDownloaded() {
        ...
        mPbapClientStateMachine.connect();
        mPbapClientStateMachine.onSdpRecordReceived(makeSdpRecord(L2CAP_PSM, INVALID_RFCOMM, SUPPORTED_FEATURES, PbapSdpRecord.REPOSITORY_LOCAL_PHONEBOOK));
        setAndNotifyObexClientStatus(BluetoothProfile.STATE_DISCONNECTED, BluetoothProfile.STATE_CONNECTING);
        setAndNotifyObexClientStatus(BluetoothProfile.STATE_CONNECTING, BluetoothProfile.STATE_CONNECTED);
        mTestLooper.dispatchAll();
    ...
    public void testDownloading_onlySimPhonebookSupported_simPhonebooksDownloaded() {
        ...
        mPbapClientStateMachine.connect();
        mPbapClientStateMachine.onSdpRecordReceived(makeSdpRecord(L2CAP_PSM, INVALID_RFCOMM, SUPPORTED_FEATURES, PbapSdpRecord.REPOSITORY_SIM_CARD));
        setAndNotifyObexClientStatus(BluetoothProfile.STATE_DISCONNECTED, BluetoothProfile.STATE_CONNECTING);
        setAndNotifyObexClientStatus(BluetoothProfile.STATE_CONNECTING, BluetoothProfile.STATE_CONNECTED);
        mTestLooper.dispatchAll();
    ...
    public void testDownloading_noRepositoriesSupported_nothingDownloaded() {
        ...
        mMockedAccounts.add(getAccountForDevice(mTestDevice));
        doReturn(true).when(mMockStorage).isStorageReady();
        mPbapClientStateMachine.onSdpRecordReceived(makeSdpRecord(L2CAP_PSM, INVALID_RFCOMM, SUPPORTED_FEATURES, NO_REPOSITORIES_SUPPORTED));
        setAndNotifyObexClientStatus(BluetoothProfile.STATE_DISCONNECTED, BluetoothProfile.STATE_CONNECTING);
        setAndNotifyObexClientStatus(BluetoothProfile.STATE_CONNECTING, BluetoothProfile.STATE_CONNECTED);
        mTestLooper.dispatchAll();
    ...
    public void testDisconnecting_clientDisconnects_transitionToDisconnected() {
        ...
        setAndNotifyObexClientStatus(BluetoothProfile.STATE_CONNECTED, BluetoothProfile.STATE_DISCONNECTING);
        setAndNotifyObexClientStatus(BluetoothProfile.STATE_CONNECTING, BluetoothProfile.STATE_DISCONNECTED);
        mTestLooper.dispatchAll();
    ...
    // use SUPPORTED_FEATURES_CACHING
    // phonebook supports metadata
    // phonebook doesn't support metadata
    // partial downloads
    // download doesn't match metadata
    ...
    private void mockRemoteContacts(String phonebook, String dbIdentifier, String primaryVersion, String secondaryVersion, int numContacts) {
        doAnswer(invocation -> {
            ...
            PbapPhonebookMetadata metadata =
                new PbapPhonebookMetadata(pb, numContacts, dbIdentifier, primaryVersion, secondaryVersion);
            mObexCallback.onGetPhonebookMetadataComplete(ResponseCodes.OBEX_HTTP_OK, pb, metadata);
            return null;
        }).when(mMockObexClient).requestPhonebookMetadata(eq(phonebook), any(PbapApplicationParameters.class));
        ...
        doAnswer(invocation -> {
            ...
            mObexCallback.onPhonebookContactsDownloaded(ResponseCodes.OBEX_HTTP_OK, pb, book);
            return null;
        }).when(mMockObexClient).requestDownloadPhonebook(eq(phonebook), any(PbapApplicationParameters.class), any(Account.class));
    }
}
