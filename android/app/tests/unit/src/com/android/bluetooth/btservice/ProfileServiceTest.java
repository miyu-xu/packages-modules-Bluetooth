...
public class ProfileServiceTest {
    ...
    private void setAllProfilesState(int state, int invocationNumber) throws TimeoutException {
        ...
        verify(mMockAdapterService, timeout(PROFILE_START_MILLIS).times(
                profileCount * invocationNumber)).onProfileServiceStateChanged(
    ...
    public void setUp()
        ...
        Log.e(TAG, "WILLIAM — not yet config len is " +
                Config.getSupportedProfiles().length);
        String configWesh = "";
        ...
        // Despite calling on the Mock of adapterService, mockito cannot handle native method and will call the real method instead, allowing to initialize the native library
        // when(mMockAdapterService.initNative(anyBoolean(), anyBoolean(), anyInt(), any(), anyBoolean(), anyString())).thenCallRealMethod();
        doCallRealMethod().when(mMockAdapterService).initNative(anyBoolean(), anyBoolean(), anyInt(), any(), anyBoolean(), anyString());
