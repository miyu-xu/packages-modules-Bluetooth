...
public class BluetoothOppTransferTest {
    ...
    public void start_receiverRegistered() {
        ...
        mTransfer.start();
        Mockito.verify(mContext).registerReceiver(any(), any(IntentFilter.class));
    ...
    public void stop_unregisterRegistered() {
        ...
        mTransfer.start();
        mTransfer.stop();
        Mockito.verify(mContext).unregisterReceiver(any());
    }
...
//    }
    ...
    public void eventHandler_handleMessage_TRANSPORT_CONNECTED_obexSessionStarted() {
        ObexTransport transport = Mockito.mock(BluetoothObexTransport.class);
    ...
    public void eventHandler_handleMessage_MSG_SESSION_COMPLETE_batchFinished() {
        BluetoothOppShareInfo info = Mockito.mock(BluetoothOppShareInfo.class);
    ...
    public void eventHandler_handleMessage_MSG_CONNECT_TIMEOUT() {
        ...
        doAnswer(invocation -> {
            Intent in = invocation.getArgument(0);
            assertThat(in.getAction()).isEqualTo(BluetoothShare.USER_CONFIRMATION_TIMEOUT_ACTION);
            return null;
        }).when(mContext).sendBroadcast(any());
