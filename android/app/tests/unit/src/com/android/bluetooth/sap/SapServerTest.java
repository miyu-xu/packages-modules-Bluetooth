...
public class SapServerTest {
    ...
    public void setUp() throws Exception {
        ...
        mSapServer = new SapServer(mHandler, mTargetContext, mInputStream, mOutputStream);
    }
    ...
    @Test
    public void onConnectRequest_whenStateIsConnecting_sendsSapMessage() {
        ...
        // Getting SAP proxy is needed for sending the sap message.
        verify(mockReceiver, atLeastOnce()).getSapProxy();
    }

    @Test
    public void onConnectRequest_whenStateIsConnected_setsConnectionStatusToErrorConnection() {
        ...
        verify(mCallback, timeout(TIMEOUT_MS)).receiveMessage(eq(SAP_MSG_RFC_REPLY), argThat(
                new ArgumentMatcher<Object>() {
                    ...
                    public boolean matches(Object arg) {
                        ...
                        SapMessage m = (SapMessage) arg;
                        return m.getMsgType() == ID_CONNECT_RESP
                                && m.getConnectionStatus() == CON_STATUS_ERROR_CONNECTION;
                    }
                }
        ));
    }
    ...
    @Test
    public void onConnectRequest_whenStateIsCallOngoing_setsConnectionStatusToErrorConnection() {
        mSapServer.mSapHandler = mHandler;
    ...
    @Test
    public void sendRilMessage_whenSapProxyIsNull() throws Exception {
        SapRilReceiver mockReceiver = mock(SapRilReceiver.class);
        Object lock = new Object();
        when(mockReceiver.getSapProxyLock()).thenReturn(lock);
        ...
        verify(mCallback, timeout(TIMEOUT_MS)).receiveMessage(eq(SAP_MSG_RFC_REPLY), argThat(
                new ArgumentMatcher<Object>() {
                    ...
                    public boolean matches(Object arg) {
                        ...
                        return sapMsg.getMsgType() == ID_ERROR_RESP;
                    }
                }
        ));
    }
    ...
    @Test
    public void sendRilMessage_whenIllegalArgumentExceptionisThrown() throws Exception {
        SapRilReceiver mockReceiver = mock(SapRilReceiver.class);
        Object lock = new Object();
        ISap mockSapProxy = mock(ISap.class);
        when(mockReceiver.getSapProxyLock()).thenReturn(lock);
    ...
    @Test
    public void sendRilMessage_whenRemoteExceptionisThrown() throws Exception {
        SapRilReceiver mockReceiver = mock(SapRilReceiver.class);
        Object lock = new Object();
        ISap mockSapProxy = mock(ISap.class);
        when(mockReceiver.getSapProxyLock()).thenReturn(lock);
    ...
    @Test
    public void handleRilInd_whenStateIsActuallyConnected() {
        SapMessage msg = mock(SapMessage.class);
        when(msg.getMsgType()).thenReturn(ID_RIL_UNSOL_DISCONNECT_IND);
        when(msg.getDisconnectionType()).thenReturn(DISC_GRACEFULL);
        mSapServer.mSapHandler = mHandler;
        ...
        verify(mCallback, timeout(TIMEOUT_MS)).receiveMessage(eq(SAP_MSG_RFC_REPLY), argThat(
                new ArgumentMatcher<Object>() {
                    ...
                    public boolean matches(Object arg) {
                        ...
                        SapMessage sapMsg = (SapMessage) arg;
                        return sapMsg.getMsgType() == ID_DISCONNECT_IND;
                    }
                }
        ));
    }
    ...
    @Test
    public void handleRilInd_whenStateIsDisconnected_sendsDisconnectIndication() {
        NotificationManager notificationManager = mock(NotificationManager.class);
        when(mTargetContext.getSystemService(NotificationManager.class))
                .thenReturn(notificationManager);
        SapMessage msg = mock(SapMessage.class);
        ...
        verify(mCallback, timeout(TIMEOUT_MS)).receiveMessage(anyInt(), argThat(
                new ArgumentMatcher<Object>() {
                    @Override
                    public boolean matches(Object arg) {
                        if (!(arg instanceof SapMessage)) {
    ...
    @Test
    public void handleRfcommReply_disconnectRespMsg_whenInConnectedState_forcefullyShutDown()
            throws Exception {
        SapMessage msg = mock(SapMessage.class);
        when(msg.getMsgType()).thenReturn(ID_DISCONNECT_RESP);
    ...
    @Test
    public void handleRfcommReply_statusIndMsg_whenInDisonnectedState_doesNotSendMessage()
            throws Exception {
        SapMessage msg = mock(SapMessage.class);
    ...
    public void handleMessage_forRfcReplyMsg_callsHandleRfcommReply() {
        ...
        try {
            ...
            // For ID_CONNECT_RESP type and CON_STATUS_OK status, the state is changed to CONNECTED.
            assertThat(mSapServer.mState).isEqualTo(SapServer.SAP_STATE.CONNECTED);
        } finally {
            message.recycle();
        }
    ...
    @Test
    public void handleMessage_forRilConnectMsg_sendsNewConnectReqSapMsg() throws Exception {
        SapRilReceiver mockReceiver = mock(SapRilReceiver.class);
        Object lock = new Object();
        when(mockReceiver.getSapProxyLock()).thenReturn(lock);
        mSapServer.mRilBtReceiver = mockReceiver;
    ...
    @Test
    public void handleMessage_forRilReqMsg_sendsSapMsg() throws Exception {
        SapRilReceiver mockReceiver = mock(SapRilReceiver.class);
        ISap mockSapProxy = mock(ISap.class);
        Object lock = new Object();
        when(mockReceiver.getSapProxyLock()).thenReturn(lock);
    ...
    @Test
    public void handleMessage_forRilIndMsg_sendsClientMessage() throws Exception {
        SapMessage sapMsg = mock(SapMessage.class);
            ...
            verify(mCallback, timeout(TIMEOUT_MS)).receiveMessage(eq(SAP_MSG_RFC_REPLY), argThat(
                    new ArgumentMatcher<Object>() {
                        ...
                        public boolean matches(Object arg) {
                            ...
                            SapMessage rcvdMsg = (SapMessage) arg;
                            return rcvdMsg.getMsgType() == ID_DISCONNECT_IND
                                    && rcvdMsg.getDisconnectionType()
