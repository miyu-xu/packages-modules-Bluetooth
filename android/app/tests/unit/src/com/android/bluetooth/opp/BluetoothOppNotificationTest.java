...
public class BluetoothOppNotificationTest {
    ...
    static final int WORKAROUND_TIMEOUT = 5000;
    ...
    public void setUp() throws Exception {
        ...
        // Decline the incoming file, cannot user intended from Espresso.
    ...
    public void updateActiveNotification() throws InterruptedException {
        ...
        // But it turns out 3 seconds is pretty flaky, so use a bigger timeout
        Log.d("hieu-debug", "start waiting");
        Thread.sleep(WORKAROUND_TIMEOUT);
        Log.d("hieu-debug", "finish waiting and fling");

        title.getParent().swipe(Direction.LEFT, 1.0f);

        Log.d("hieu-debug", "done fling and check");
        // This notification should not be removable
        assertThat(device.findObject(By.text(titleString))).isNotNull();
    ...
    public void updateIncomingFileConfirmationNotification() throws InterruptedException {
        ...
        // All actions' titles are converted into upper case
        String confirmString = mTargetContext.getString(
                R.string.incoming_file_confirm_ok).toUpperCase();
        String declineString = mTargetContext.getString(
                R.string.incoming_file_confirm_cancel).toUpperCase();
