...
public class VolumeControlServiceTest {
    ...
    public void testServiceBinderVolumeOffsetMethods() throws Exception {
        ...
        final SynchronousResultReceiver<Integer> intRecv = SynchronousResultReceiver.get();
        int defaultNumberOfIds = 0;
        mServiceBinder.getNumberOfAudioOutputIds(mDevice, mAttributionSource, intRecv);
        int numberOfIds =
                intRecv.awaitResultNoInterrupt(Duration.ofMillis(TIMEOUT_MS))
                        .getValue(defaultNumberOfIds);
        Assert.assertEquals(2, numberOfIds);
    ...
    public void testServiceBinderRegisterCallbackWhenDeviceAlreadyConnected() throws Exception {
        ...
        verify(callback).onAudioLocationChanged(eq(mDevice), eq(1), eq(1));
        verify(callback).onAudioDescriptionChanged(eq(mDevice), eq(1), eq(testDevice1Desc1));

        verify(callback).onVolumeOffsetChanged(eq(mDevice), eq(2), eq(200));
        verify(callback).onAudioLocationChanged(eq(mDevice), eq(2), eq(2));
        verify(callback).onAudioDescriptionChanged(eq(mDevice), eq(2), eq(testDevice1Desc2));

        verify(callback).onVolumeOffsetChanged(eq(mDeviceTwo), eq(1), eq(250));
        verify(callback).onAudioLocationChanged(eq(mDeviceTwo), eq(1), eq(3));
        verify(callback).onAudioDescriptionChanged(eq(mDeviceTwo), eq(1), eq(testDevice2Desc));
        ...
        verify(callback).onAudioLocationChanged(eq(mDevice), eq(1), eq(0));
        verify(callback).onAudioDescriptionChanged(eq(mDevice), eq(1), eq(testDevice1Desc3));
    }
