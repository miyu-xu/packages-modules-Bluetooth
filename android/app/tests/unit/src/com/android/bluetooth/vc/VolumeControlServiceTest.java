...
public class VolumeControlServiceTest {
    ...
    @Test
    public void testServiceBinderTestUpdateNewRegisteredCallback()
            throws Exception {
        mFakeFlagsImpl.setFlag(
                Flags.FLAG_LEAUDIO_BROADCAST_VOLUME_CONTROL_FOR_CONNECTED_DEVICES, true);
        int groupId = 1;
        ...
        recv = SynchronousResultReceiver.get();
        mServiceBinder.updateNewRegistedCallback(callback_new_client, mAttributionSource, recv);
        recv.awaitResultNoInterrupt(Duration.ofMillis(TIMEOUT_MS)).getValue(null);
        Assert.assertEquals(size + 1, mService.mCallbacks.getRegisteredCallbackCount());
        ...
        verify(callback_new_client, times(1)).onDeviceVolumeChanged(eq(mDevice), eq(deviceOneVolume));
        verify(callback_new_client, times(1)).onDeviceVolumeChanged(eq(mDeviceTwo), eq(deviceTwoVolume));
    }
