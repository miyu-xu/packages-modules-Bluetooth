...
public class VolumeControlServiceTest {
    ...
    public void testServiceBinderRegisterCallbackWhenDeviceAlreadyConnected() throws Exception {
        ...
        generateDeviceOffestChangedMessageFromNative(mDevice, 1, 100);
        generateDeviceOffestChangedMessageFromNative(mDeviceTwo, 1, 200);
        ...
        verify(callback, times(1)).onVolumeOffsetChanged(eq(mDeviceTwo), eq(200));
        verify(callback, times(1)).onVolumeOffsetChanged(eq(mDevice), eq(100));

        generateDeviceOffestChangedMessageFromNative(mDevice, 1, 50);

        verify(callback, times(1)).onVolumeOffsetChanged(eq(mDevice), eq(50));
    }
    ...
    private void generateDeviceAvailableMessageFromNative(BluetoothDevice device, int numberOfExtOffsets) {
        // Send a message to trigger connection completed
    ...
    private void generateDeviceOffestChangedMessageFromNative(BluetoothDevice device, int extOffsetIndex, int offset) {
        // Send a message to trigger connection completed
        VolumeControlStackEvent event = new VolumeControlStackEvent(
                VolumeControlStackEvent.EVENT_TYPE_EXT_AUDIO_OUT_VOL_OFFSET_CHANGED);
        event.device = device;
        event.valueInt1 = extOffsetIndex; // external output index
        event.valueInt2 = offset; // offest value
        mService.messageFromNative(event);
    }
