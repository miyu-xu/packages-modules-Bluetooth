...
import static android.bluetooth.le.ScanSettings.CALLBACK_TYPE_AUTO_BATCH;
...
public class ScanManagerTest {
    ...
    private ScanSettings createScanSettings(int scanMode, boolean isBatch, boolean isAutoBatch) {
        ...
        if (isBatch && isAutoBatch) {
            int autoCallbackType = CALLBACK_TYPE_ALL_MATCHES | CALLBACK_TYPE_AUTO_BATCH;
            scanSettings = new ScanSettings.Builder().setScanMode(scanMode)
                    .setReportDelay(DEFAULT_SCAN_REPORT_DELAY_MS).setCallbackType(autoCallbackType)
                    .build();
        } else if (isBatch) {
            scanSettings = new ScanSettings.Builder().setScanMode(scanMode)
