...
public class BluetoothPbapObexServerTest {
    ...
    public void testOnConnect_whenUuidLengthIsWrong() {
        HeaderSet request = new HeaderSet();
        byte[] uuidWithWrongLength = new byte[] {0x00, 0x01, 0x02};
        request.setHeader(HeaderSet.TARGET, uuidWithWrongLength);
        HeaderSet reply = new HeaderSet();
    ...
    public void testOnConnect_whenIoExceptionIsThrownFromSettingWhoHeader() throws Exception {
        ...
        HeaderSet reply = new HeaderSet();
        doThrow(IOException.class).when(mPbapMethodProxy).setHeader(eq(reply), eq(HeaderSet.WHO), any());
    ...
    public void testOnConnect_whenIoExceptionIsThrownFromSettingTargetHeader() throws Exception {
        ...
        HeaderSet reply = new HeaderSet();
        doThrow(IOException.class).when(mPbapMethodProxy).setHeader(eq(reply), eq(HeaderSet.TARGET), any());
