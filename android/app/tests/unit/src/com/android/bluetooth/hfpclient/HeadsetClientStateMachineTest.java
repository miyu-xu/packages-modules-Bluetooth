...
public class HeadsetClientStateMachineTest {
    ...
    public void testVoiceRecognitionStateChange() {
        ...
        when(mHeadsetClientService.getConnectionPolicy(any(BluetoothDevice.class))).thenReturn(
                BluetoothProfile.CONNECTION_POLICY_ALLOWED);
        when(mNativeInterface.startVoiceRecognition(any(BluetoothDevice.class))).thenReturn(true);
        when(mNativeInterface.stopVoiceRecognition(any(BluetoothDevice.class))).thenReturn(true);
