...
public class HeadsetClientStateMachineTest {
    ...
    private int setUpServiceLevelConnection(int startBroadcastIndex, boolean atAndroidSupported) {
        ...
        setUpAtAndroid(atAndroidSupported);
     ...
     */
    private void setUpAtAndroid(boolean atAndroidSupported) {
     ...
     */
    @Test
    public void testAtAndroidRemoteNotSupported_StateTransition_setAudioPolicy() {
        // Setup connection state machine to be in connected state
        when(mHeadsetClientService.getConnectionPolicy(any(BluetoothDevice.class))).thenReturn(
                BluetoothProfile.CONNECTION_POLICY_ALLOWED);
        int expectedBroadcastIndex = 1;
        ...
        verify(mNativeInterface).sendAndroidAt(anyObject(), anyString());
    }
