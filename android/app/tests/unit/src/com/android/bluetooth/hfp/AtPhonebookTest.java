...
public class AtPhonebookTest {
    ...
    private static final String TEST_ERROR_STRING = "errorString";
    ...
    @Test
    public void handleCscsCommand_handledCorrectly() {
        mAtPhonebook.handleCscsCommand(TEST_ERROR_STRING, AtPhonebook.TYPE_READ, mTestDevice);
        verify(mNativeInterface).atResponseString(mTestDevice,
                "+CSCS: \"" + "UTF-8" + "\"");

        mAtPhonebook.handleCscsCommand(TEST_ERROR_STRING, AtPhonebook.TYPE_TEST, mTestDevice);
        verify(mNativeInterface).atResponseString(mTestDevice,
                "+CSCS: (\"UTF-8\",\"IRA\",\"GSM\")");

        mAtPhonebook.handleCscsCommand(TEST_ERROR_STRING, AtPhonebook.TYPE_SET, mTestDevice);
        ...
        mAtPhonebook.handleCscsCommand(TEST_ERROR_STRING, AtPhonebook.TYPE_UNKNOWN, mTestDevice);
        verify(mNativeInterface).atResponseCode(mTestDevice, HeadsetHalConstants.AT_RESPONSE_ERROR,
                BluetoothCmeError.TEXT_HAS_INVALID_CHARS);
    }

    @Test
    public void handleCpbsCommand_handledCorrectly() {
        mAtPhonebook.handleCpbsCommand(TEST_ERROR_STRING, AtPhonebook.TYPE_READ, mTestDevice);
        verify(mNativeInterface).atResponseString(mTestDevice,
                "+CPBS: \"" + "ME" + "\"," + 0 + "," + 256);

        mAtPhonebook.handleCpbsCommand(TEST_ERROR_STRING, AtPhonebook.TYPE_TEST, mTestDevice);
        verify(mNativeInterface).atResponseString(mTestDevice,
                "+CPBS: (\"ME\",\"SM\",\"DC\",\"RC\",\"MC\")");

        mAtPhonebook.handleCpbsCommand(TEST_ERROR_STRING, AtPhonebook.TYPE_SET, mTestDevice);
        verify(mNativeInterface).atResponseCode(mTestDevice, HeadsetHalConstants.AT_RESPONSE_ERROR,
                BluetoothCmeError.OPERATION_NOT_SUPPORTED);
        ...
        mAtPhonebook.handleCpbsCommand(TEST_ERROR_STRING, AtPhonebook.TYPE_UNKNOWN, mTestDevice);
        verify(mNativeInterface).atResponseCode(mTestDevice, HeadsetHalConstants.AT_RESPONSE_ERROR,
                BluetoothCmeError.TEXT_HAS_INVALID_CHARS);
    }
}