...
public class BassClientServiceTest {
    ...
    private void verifySendPastAndNoOthers() {
        expect.that(mStateMachines.size()).isEqualTo(2);
        for (BassClientStateMachine sm : mStateMachines.values()) {
            ArgumentCaptor<Message> messageCaptor = ArgumentCaptor.forClass(Message.class);
            verify(sm, atLeast(1)).sendMessage(messageCaptor.capture());
            ...
            if (sm.getDevice().equals(mCurrentDevice)) {
                count =
                        messageCaptor.getAllValues().stream()
                                .filter(
                                        m ->
                                                (m.what == BassClientStateMachine.SEND_PAST)
                ...
                count =
                        messageCaptor.getAllValues().stream()
                                .filter(m -> m.what != BassClientStateMachine.SEND_PAST)
                                .count();
    ...
    public void sendPast_concurrentWithResume() {
        prepareSynchronizedPairAndStopSearching();
        ...
        // Sync will sendPast and remove pending soure to add
        onSyncEstablished(mSourceDevice, TEST_SYNC_HANDLE);
        verifySendPastAndNoOthers();
    }
