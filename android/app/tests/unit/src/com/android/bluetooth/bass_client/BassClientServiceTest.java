...
public class BassClientServiceTest {
    ...
    private void injectRemoteSourceState(BassClientStateMachine sm, BluetoothLeBroadcastMetadata meta,
                                int sourceId, int paSynState, int encryptionState, byte[] badCode) {
   ...
   /**
    ...
    public void testGroupStickyFlagSetUnset() {
        ...
        verify(mStateMachines.get(mCurrentDevice1), atLeast(1)).sendMessage(messageCaptor.capture());
        msg = messageCaptor.getAllValues().stream()
                    .filter(m -> m.what == BassClientStateMachine.UPDATE_BCAST_SOURCE)
                    .findFirst();
    ...
    public void testAddRemoveMultipleSourcesForGroup() {
        ...
        for (BassClientStateMachine sm: mStateMachines.values()) {
            ...
            BluetoothLeBroadcastMetadata metaOther = createBroadcastMetadata(TEST_BROADCAST_ID + 20);
