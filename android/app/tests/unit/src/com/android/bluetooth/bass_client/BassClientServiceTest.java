...
public class BassClientServiceTest {
    ...
    public void testNotRemovingCachedBroadcastOnFailEstablishWithoutScanning() {
        ...
        // Add source to unsynced broadcast, causes synchronization first
        mBassClientService.addSource(device1, meta, true);
        handleHandoverSupport();
        inOrder.verify(mMethodProxy)
                .periodicAdvertisingManagerRegisterSync(
                        any(), any(), anyInt(), anyInt(), any(), any());

        // Error in syncEstablished causes soureLost, sourceAdd notification
        ...
        try {
            inOrderCallback
                    .verify(mCallback)
                    .onSourceAddFailed(
                            eq(device1), eq(meta), eq(BluetoothStatusCodes.ERROR_BAD_PARAMETERS));
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
