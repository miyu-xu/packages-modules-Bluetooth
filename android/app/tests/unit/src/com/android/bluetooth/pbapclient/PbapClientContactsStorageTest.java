...
public class PbapClientContactsStorageTest {
    ...
    private static final int DATA_PER_CONTACT = 2;
    ...
    // TODO: init with previous data -> data deleted
    ...
    @Test
    public void testInsertFavorites_validFavoritesList_contactsInserted() throws RemoteException, OperationApplicationException, NumberFormatException {
        testStartStorage_withoutExistingAccounts_storageReadyWithNoAccounts();
        BluetoothDevice device = TestUtils.getTestDevice(mAdapter, 1);
        Account account = mStorage.getStorageAccountForDevice(device);
        mStorage.addAccount(account);

        assertThat(mStorage.insertFavorites(account, getMockContacts(account, PbapPhonebook.FAVORITES_PATH, TEST_CONTACTS_SIZE))).isTrue();
        verifyDbAccounts(1);
    ...
    @Test
    public void testInsertLocalContacts() throws RemoteException, OperationApplicationException, NumberFormatException {
        testStartStorage_withoutExistingAccounts_storageReadyWithNoAccounts();
        BluetoothDevice device = TestUtils.getTestDevice(mAdapter, 1);
        Account account = mStorage.getStorageAccountForDevice(device);
        mStorage.addAccount(account);

        assertThat(mStorage.insertLocalContacts(account, getMockContacts(account, PbapPhonebook.LOCAL_PHONEBOOK_PATH, TEST_CONTACTS_SIZE))).isTrue();
        verifyDbAccounts(1);
        verifyDbFavorites(0);
        verifyDbRawContacts(TEST_CONTACTS_SIZE);
        verifyDbData(TEST_CONTACTS_SIZE * DATA_PER_CONTACT);
    ...
    @Test
    public void testInsertSimContacts() throws RemoteException, OperationApplicationException, NumberFormatException {
        testStartStorage_withoutExistingAccounts_storageReadyWithNoAccounts();
        BluetoothDevice device = TestUtils.getTestDevice(mAdapter, 1);
        Account account = mStorage.getStorageAccountForDevice(device);
        mStorage.addAccount(account);

        assertThat(mStorage.insertSimContacts(account, getMockContacts(account, PbapPhonebook.SIM_PHONEBOOK_PATH, TEST_CONTACTS_SIZE))).isTrue();
        verifyDbAccounts(1);
        verifyDbFavorites(0);
        verifyDbRawContacts(TEST_CONTACTS_SIZE);
        verifyDbData(TEST_CONTACTS_SIZE * DATA_PER_CONTACT);
    ...
    @Test
    public void testInsertIncomingCallHistory_validHistory_historyInserted() throws RemoteException, OperationApplicationException {
        testStartStorage_withoutExistingAccounts_storageReadyWithNoAccounts();
        BluetoothDevice device = TestUtils.getTestDevice(mAdapter, 1);
        Account account = mStorage.getStorageAccountForDevice(device);
        mStorage.addAccount(account);

        assertThat(mStorage.insertIncomingCallHistory(account, getMockCallHistory(account, CallLog.Calls.INCOMING_TYPE, TEST_CONTACTS_SIZE))).isTrue();
        verifyDbAccounts(0);
        verifyDbFavorites(0);
    ...
    @Test
    public void testInsertOutgoingCallHistory_validHistory_historyInserted() throws RemoteException, OperationApplicationException {
        testStartStorage_withoutExistingAccounts_storageReadyWithNoAccounts();
        BluetoothDevice device = TestUtils.getTestDevice(mAdapter, 1);
        Account account = mStorage.getStorageAccountForDevice(device);
        mStorage.addAccount(account);

        assertThat(mStorage.insertOutgoingCallHistory(account, getMockCallHistory(account, CallLog.Calls.OUTGOING_TYPE, TEST_CONTACTS_SIZE))).isTrue();
        verifyDbAccounts(0);
        verifyDbFavorites(0);
        verifyDbRawContacts(0);
        verifyDbData(0);
    ...
    @Test
    public void testInsertMissedCallHistory_validHistory_historyInserted() throws RemoteException, OperationApplicationException {
        testStartStorage_withoutExistingAccounts_storageReadyWithNoAccounts();
        BluetoothDevice device = TestUtils.getTestDevice(mAdapter, 1);
        Account account = mStorage.getStorageAccountForDevice(device);
        mStorage.addAccount(account);

        assertThat(mStorage.insertMissedCallHistory(account, getMockCallHistory(account, CallLog.Calls.MISSED_TYPE, TEST_CONTACTS_SIZE))).isTrue();
        verifyDbAccounts(0);
        verifyDbFavorites(0);
        verifyDbRawContacts(0);
        verifyDbData(0);
    ...
    public void testRemoveLocalContacts_localContactsRemoved() {
        ...
        addFakeAccount(device.getAddress());
        addFakeContacts(device.getAddress(), PbapPhonebook.LOCAL_PHONEBOOK_PATH, TEST_CONTACTS_SIZE);
    ...
    public void testInsertContacts_storageNotReady_insertFails() {
        ...
        assertThat(mStorage.insertFavorites(account, getMockContacts(account, PbapPhonebook.FAVORITES_PATH, TEST_CONTACTS_SIZE))).isFalse();
    }
    ...
    public void testInsertContacts_accountNull_insertFails() {
        ...
        assertThat(mStorage.insertFavorites(null, getMockContacts(account, PbapPhonebook.FAVORITES_PATH, TEST_CONTACTS_SIZE))).isFalse();
    }
    ...
    @Test
    public void testRemoveContacts_accountNull_removeFails() {
        testStartStorage_withoutExistingAccounts_storageReadyWithNoAccounts();
        BluetoothDevice device = TestUtils.getTestDevice(mAdapter, 1);
        Account account = mStorage.getStorageAccountForDevice(device);
        mStorage.addAccount(account);
    ...
    public void testInsertCallHistory_storageNotReady_insertFails() {
        ...
        assertThat(mStorage.insertIncomingCallHistory(account, getMockCallHistory(account, CallLog.Calls.INCOMING_TYPE, TEST_CONTACTS_SIZE))).isFalse();
    }
    ...
    public void testInsertCallHistory_accountNull_insertFails() {
        ...
        assertThat(mStorage.insertIncomingCallHistory(null, getMockCallHistory(account, CallLog.Calls.INCOMING_TYPE, TEST_CONTACTS_SIZE))).isFalse();
    }
    ...
    public void testInsertCallHistory_historyEmpty_insertFails() {
        ...
        assertThat(mStorage.insertIncomingCallHistory(account, new ArrayList<VCardEntry>())).isFalse();
    }
    ...
    private void verifyDbRawContacts(int numRawContacts) {
        SparseArray<FakeContactsProvider.FakeRawContact> rawContacts = mFakeContactsProvider.getRawContacts();
        assertThat(rawContacts.size()).isEqualTo(numRawContacts);
    }

    private void verifyDbFavorites(int numFavorites) {
        SparseArray<FakeContactsProvider.FakeRawContact> favorites = mFakeContactsProvider.getFavorites();
    ...
    private void verifyDbAccounts(int numAccounts) {
        SparseArray<FakeContactsProvider.FakeAccount> accounts = mFakeContactsProvider.getAccounts();
        assertThat(accounts.size()).isEqualTo(numAccounts);
    }

    private void verifyDbCallHistory(int numCallHistory) {
        SparseArray<FakeContactsProvider.FakeCallLog> calls = mFakeContactsProvider.getCallHistory();
