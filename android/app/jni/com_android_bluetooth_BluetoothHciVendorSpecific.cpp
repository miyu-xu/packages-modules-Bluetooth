...
#include "hardware/bt_hcivs.h"

using bluetooth::hcivs::BluetoothHciVendorSpecificInterface;
using bluetooth::hcivs::Cookie;
...
static jmethodID method_commandStatusDeliver;
static jmethodID method_commandCompleteDeliver;
static jmethodID method_eventDeliver;
static jobject mCallbacksObj = nullptr;

class BluetoothHciVendorSpecificCallbacksImpl
    : public bluetooth::hcivs::BluetoothHciVendorSpecificCallbacks {
public:
  ~BluetoothHciVendorSpecificCallbacksImpl() = default;

  void commandStatusDelivery(uint16_t ocf, uint8_t status, Cookie cookie) override {
    log::info("");
    std::shared_lock<std::shared_timed_mutex> lock(callbacks_mutex);

    CallbackEnv callbackEnv(__func__);
    if (!callbackEnv.valid() || mCallbacksObj == nullptr || method_commandStatusDeliver == NULL) {
      return;
    }

    auto j_cookie = toJByteArray(callbackEnv.get(), cookie);
    if (!j_cookie.get()) {
      log::error("Error while allocation byte array for cookie");
      return;
    }

    callbackEnv->CallVoidMethod(mCallbacksObj, method_commandStatusDeliver, (jint)ocf, (jint)status,
                                j_cookie.get());
  }

  void commandCompleteDelivery(uint16_t ocf, std::vector<uint8_t> return_parameters,
                               Cookie cookie) override {
    log::info("");
    std::shared_lock<std::shared_timed_mutex> lock(callbacks_mutex);

    CallbackEnv callbackEnv(__func__);
    if (!callbackEnv.valid() || mCallbacksObj == nullptr || method_commandCompleteDeliver == NULL) {
      return;
    }
    ...
    if (!j_return_parameters.get() || !j_cookie.get()) {
      log::error("Error while allocation byte array for return parameters or cookie");
      return;
    }

    callbackEnv->CallVoidMethod(mCallbacksObj, method_commandCompleteDeliver, (jint)ocf,
                                j_return_parameters.get(), j_cookie.get());
  }

  void eventDelivery(uint8_t code, std::vector<uint8_t> data) override {
    log::info("");
    std::shared_lock<std::shared_timed_mutex> lock(callbacks_mutex);

    CallbackEnv callbackEnv(__func__);
    if (!callbackEnv.valid() || mCallbacksObj == nullptr || method_eventDeliver == NULL) {
      return;
    }

    auto j_data = toJByteArray(callbackEnv.get(), data);
    if (!j_data.get()) {
      log::error("Error while allocation byte array for event data");
      return;
    }

    callbackEnv->CallVoidMethod(mCallbacksObj, method_eventDeliver, (jint)code, j_data.get());
  }

private:
  template <typename T>
  ScopedLocalRef<jbyteArray> toJByteArray(JNIEnv* env, const T& src) {
...
static void initNative(JNIEnv* env, jobject object) {
  ...
  sBluetoothHciVendorSpecificInterface =
          (BluetoothHciVendorSpecificInterface*)(btInf->get_profile_interface(BT_HCIVS_ID));
  if (sBluetoothHciVendorSpecificInterface == nullptr) {
...
int register_com_android_bluetooth_btservice_BluetoothHciVendorSpecific(JNIEnv* env) {
  ...
  const JNIJavaMethod javaMethods[] = {
          {"commandStatusDeliver", "(II[B)V", &method_commandStatusDeliver},
          {"commandCompleteDeliver", "(I[B[B)V", &method_commandCompleteDeliver},
          {"eventDeliver", "(I[B)V", &method_eventDeliver},
  };
