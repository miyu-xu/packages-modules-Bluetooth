...
Uuid jobject_to_uuid(JNIEnv* env, ScopedLocalRef<jobject> j_uuid) {
  if (j_uuid.get() == NULL) {
    assert();
  }

  return from_java_uuid(env->CallLongMethod(j_uuid.get(), uuidGetMsb),
                        env->CallLongMethod(j_uuid.get(), uuidGetLsb));
}
