...
public:
  ...
  void OnDistanceMeasurementResult(RawAddress address, uint32_t centimeter,
                                   ...
                                   int8_t confidence_level, double delaySpreadMeters,
                                   uint8_t detectedAttackLevel, double velocityMetersPerSecond,
                                   uint8_t method) {
    std::shared_lock<std::shared_mutex> lock(callbacks_mutex);
    CallbackEnv sCallbackEnv(__func__);
    if (!sCallbackEnv.valid() || !mDistanceMeasurementCallbacksObj) {
    ...
    sCallbackEnv->CallVoidMethod(
            ...
            error_altitude_angle, elapsedRealtimeNanos, confidence_level, delaySpreadMeters,
            detectedAttackLevel, velocityMetersPerSecond, method);
  }
};
...
static int register_com_android_bluetooth_gatt_distance_measurement(JNIEnv* env) {
  ...
  const JNIJavaMethod javaMethods[] = {
          ...
          {"onDistanceMeasurementResult", "(Ljava/lang/String;IIIIIIIIDIDI)V",
