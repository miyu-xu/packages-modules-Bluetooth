...
static BleScannerInterface* sScanner;
...
static void gattClientScanFilterEnableNative(JNIEnv* /* env */,
  ...
  if (!sGattIf) return;
  sScanner->ScanFilterEnable(enable,
...
static void startSyncNative(JNIEnv* env, jobject /* object */, jint sid,
                            ...
                            jint reg_id) {
  if (!sScanner) return;
  sScanner->StartSync(sid, str2addr(env, address), skip, timeout,
                              reg_id);
}
...
static void transferSetInfoNative(JNIEnv* env, jobject /* object */,
  ...
  if (!sScanner) return;
  sScanner->TransferSetInfo(str2addr(env, addr), service_data,
