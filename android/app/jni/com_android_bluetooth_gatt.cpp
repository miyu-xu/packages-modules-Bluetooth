...
=======
    JNIEnv* env, jobject object, jobject parameters, jbyteArray adv_data, jbyteArray adv_data_enc,
    jbyteArray scan_resp, jbyteArray scan_resp_enc, jobject periodic_parameters,
    jbyteArray periodic_data, jbyteArray periodic_data_enc, jint duration, jint maxExtAdvEvents,
    jbyteArray enc_key_material_value, jint reg_id, jint server_if) {
>>>>>>> ce43e86104... Encrypted Advertisement(Denver)
  if (!sGattIf) return;
  ...
  sGattIf->advertiser->StartAdvertisingSet(
      kAdvertiserClientIdJni, reg_id, base::Bind(&ble_advertising_set_started_cb, reg_id, server_if), params,
      data_vec, data_enc_vec ,scan_resp_vec, scan_resp_enc_vec, periodicParams, periodic_data_vec,
...
static void setAdvertisingDataNative(JNIEnv* env, jobject /* object */,
                                     jint advertiser_id, jbyteArray data, jbyteArray data_encrypt) {
  if (!sGattIf) return;
...
static int register_com_android_bluetooth_gatt_advertise_manager(JNIEnv* env) {
  const JNINativeMethod methods[] = {
      ...
      {"startAdvertisingSetNative",
       ...
       "[B[B[B[BLandroid/bluetooth/le/PeriodicAdvertisingParameters;[B[BII[BII)V",
