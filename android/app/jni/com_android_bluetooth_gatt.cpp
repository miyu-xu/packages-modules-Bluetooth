...
static void gattClientMsftAdvMonitorAdd(JNIEnv* env, jobject /* object*/, jobject msft_adv_monitor,
  ...
  jclass msftAdvMonitorAddressClazz =
          env->GetObjectClass(env->GetObjectClass(msft_adv_monitor_address));
  ...
  ScopedLocalRef<jobject> msft_adv_monitor_object(env, msftAdvMonitorClazz);
  ...
  for (int i = 0; i < numPatterns; i++) {
    ...
    if (patternByteArray.get() != nullptr) {
      ...
      for (int j = 0; j < env->GetArrayLength(patternByteArray.get()); j++) {
        native_msft_adv_monitor_pattern.pattern[j] = patternBytes[j];
      }
    }
...
static void gattClientMsftAdvMonitorRemove(JNIEnv* /* env */, jobject /* object */,
                                           int monitor_handle) {
  if (!sScanner) {
    return;
  }
...
static void gattClientMsftAdvMonitorEnable(JNIEnv* /* env */, jobject /* object */,
                                           jboolean enable) {
  if (!sScanner) {
    return;
  }
...
static int register_com_android_bluetooth_gatt_scan(JNIEnv* env) {
  const JNINativeMethod methods[] = {
          ...
          {"gattClientMsftAdvMonitorAdd",
           ...
           "MsftAdvMonitor$Patterns;Lcom/android/bluetooth/le_scan/MsftAdvMonitor$Address)V",
           (void*)gattClientMsftAdvMonitorAdd},
          {"gattClientMsftAdvMonitorRemove", "(I)V", (void*)gattClientMsftAdvMonitorRemove},
          {"gattClientMsftAdvMonitorEnable", "(Z)V", (void*)gattClientMsftAdvMonitorEnable},
  };
