...
static void listPlayerSettingsResponseNative(JNIEnv* env, jobject object,
  ...
  ALOGD("%s", __func__);
  jsize number_of_attributes = env->GetArrayLength(attributes);
  ...
  if (number_of_attributes != 0) {
    ...
    attributes_vector.reserve((size_t)number_of_attributes);
    std::copy(attributes_array, attributes_array + number_of_attributes,
...
static void listPlayerSettingValuesResponseNative(JNIEnv* env, jobject object,
  ...
  jsize number_of_values = env->GetArrayLength(values);
  ...
  if (number_of_values != 0) {
    ...
    values_vector.reserve(number_of_values);
    std::copy(values_array, values_array + number_of_values,
...
static void getPlayerSettingsResponseNative(JNIEnv* env, jobject object,
  ...
  jbyte* attributes_elements;
  jbyte* values_elements;
  uint8_t number_of_attributes = env->GetArrayLength(attributes);
  uint8_t number_of_values = env->GetArrayLength(values);

  if (number_of_attributes != 0) {
    attributes_elements = env->GetByteArrayElements(attributes, nullptr);
    PlayerAttribute* attributes_array =
        reinterpret_cast<PlayerAttribute*>(attributes_elements);
    attributes_vector.reserve(number_of_attributes);
    std::copy(attributes_array, attributes_array + number_of_attributes,
              std::back_inserter(attributes_vector));
  }
