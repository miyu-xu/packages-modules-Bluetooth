...
public:
  ...
  void OnExtAudioInStateChanged(const RawAddress& bd_addr, uint8_t ext_input_id, int8_t gain_val,
    ...
    ScopedLocalRef<jbyteArray> addr(sCallbackEnv.get(),
                                    sCallbackEnv->NewByteArray(sizeof(RawAddress)));
    if (!addr.get()) {
      log::error("Failed to new jbyteArray bd addr for OnExtAudioInStateChanged");
      return;
    }
  ...
  void OnExtAudioInStatusChanged(const RawAddress& bd_addr, uint8_t ext_input_id,
    ...
    ScopedLocalRef<jbyteArray> addr(sCallbackEnv.get(),
                                    sCallbackEnv->NewByteArray(sizeof(RawAddress)));
    if (!addr.get()) {
      log::error("Failed to new jbyteArray bd addr for OnExtAudioInStatusChanged");
      return;
    }
  ...
  void OnExtAudioInTypeChanged(const RawAddress& bd_addr, uint8_t ext_input_id,
    ...
    ScopedLocalRef<jbyteArray> addr(sCallbackEnv.get(),
                                    sCallbackEnv->NewByteArray(sizeof(RawAddress)));
    if (!addr.get()) {
      log::error("Failed to new jbyteArray bd addr for OnExtAudioInTypeChanged");
      return;
    }
  ...
  void OnExtAudioInGainPropsChanged(const RawAddress& bd_addr, uint8_t ext_input_id, uint8_t unit,
    ...
    ScopedLocalRef<jbyteArray> addr(sCallbackEnv.get(),
                                    sCallbackEnv->NewByteArray(sizeof(RawAddress)));
    if (!addr.get()) {
      log::error("Failed to new jbyteArray bd addr for OnExtAudioInGainPropsChanged");
      return;
    }
...
int register_com_android_bluetooth_vc(JNIEnv* env) {
  const JNINativeMethod methods[] = {
          ...
          {"getExtAudioInStateNative", "([BI)Z", (void*)getExtAudioInStateNative},
          {"getExtAudioInStatusNative", "([BI)Z", (void*)getExtAudioInStatusNative},
          {"getExtAudioInTypeNative", "([BI)Z", (void*)getExtAudioInTypeNative},
          {"getExtAudioInGainPropsNative", "([BI)Z", (void*)getExtAudioInGainPropsNative},
          {"getExtAudioInDescriptionNative", "([BI)Z", (void*)getExtAudioInDescriptionNative},
          {"setExtAudioInDescriptionNative", "([BILjava/lang/String;)Z",
           (void*)setExtAudioInDescriptionNative},
          {"setExtAudioInGainValueNative", "([BII)Z", (void*)setExtAudioInGainValueNative},
          {"setExtAudioInGainModeNative", "([BIZ)Z", (void*)setExtAudioInGainModeNative},
          {"setExtAudioInGainMuteNative", "([BIZ)Z", (void*)setExtAudioInGainMuteNative},
  };
