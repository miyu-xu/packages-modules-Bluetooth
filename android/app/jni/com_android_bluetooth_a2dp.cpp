...
static std::vector<btav_a2dp_codec_config_t> prepareCodecPreferences(
  ...
  for (int i = 0; i < numConfigs; i++) {
    ...
    jobject codecType = env->CallObjectMethod(
            jcodecConfig, android_bluetooth_BluetoothCodecConfig.getExtendedCodecType);
    jlong codecId = env->CallLongMethod(codecType, android_bluetooth_BluetoothCodecType.getCodecId);
    btav_a2dp_codec_index_t codecIndex = BTAV_A2DP_CODEC_INDEX_SOURCE_SBC;
    ...
    btav_a2dp_codec_config_t codec_config = {
            .codec_type = static_cast<btav_a2dp_codec_index_t>(codecIndex),
