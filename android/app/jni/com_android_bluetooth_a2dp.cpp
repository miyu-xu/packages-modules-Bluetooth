...
static void bta2dp_connection_state_callback(const RawAddress& bd_addr,
  ...
  log::info("{}: state: {}", bd_addr.ToString(), dump_av_conn_state(state));
...
static void bta2dp_audio_state_callback(const RawAddress& bd_addr,
  ...
  log::info("{}: state: {}", bd_addr.ToString(), dump_av_audio_state(state));
...
static void bta2dp_audio_config_callback(
  ...
  log::info(
      "{}: codec: {}, local codecs: {}, selectable codecs: {}",
      bd_addr.ToString(), codec_config.CodecNameStr(),
...
static jboolean connectA2dpNative(JNIEnv* env, jobject /* object */,
  ...
  log::info("sBluetoothA2dpInterface: {}, {}",
            fmt::ptr(sBluetoothA2dpInterface), bd_addr.ToString());
