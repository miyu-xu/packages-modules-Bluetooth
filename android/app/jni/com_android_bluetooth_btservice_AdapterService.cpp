...
static jboolean createBondNative(JNIEnv* env, jobject obj,
  ...
  uint8_t addr_type = static_cast<uint8_t>(addrWithType[ADDR_TYPE_POS]);

  int ret = BT_STATUS_SUCCESS;
  if (addr_type == BLE_ADDR_RANDOM && transport != TRANSPORT_BREDR) {
    ret = sBluetoothInterface->create_bond_with_addrtype(&bd_addr, addr_type,
                                                         transport);
  } else {
