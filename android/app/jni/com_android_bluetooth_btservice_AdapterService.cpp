...
static int get_properties(int num_properties, bt_property_t* properties,
  ...
  for (int i = 0; i < num_properties; i++) {
    ...
    if (!propVal.get()) {
      log::error("Error while allocation of array in ");
      return -1;
    }
...
static void remote_device_properties_callback(bt_status_t status,
  ...
  if (!addr.get()) {
    log::error("Error while allocation byte array in ");
    return;
  }
...
static void device_found_callback(int num_properties,
  ...
  for (int i = 0; i < num_properties; i++) {
    if (properties[i].type == BT_PROPERTY_BDADDR) {
      ...
      if (!addr.get()) {
        log::error("Address is NULL (unable to allocate) in ");
        return;
      }
...
static void bond_state_changed_callback(bt_status_t status, RawAddress* bd_addr,
  ...
  ScopedLocalRef<jbyteArray> addr(
      sCallbackEnv.get(), sCallbackEnv->NewByteArray(sizeof(RawAddress)));
  if (!addr.get()) {
    log::error("Address allocation failed in ");
    return;
  }
  sCallbackEnv->SetByteArrayRegion(addr.get(), 0, sizeof(RawAddress),
                                   (jbyte*)bd_addr);
...
static void pin_request_callback(RawAddress* bd_addr, bt_bdname_t* bdname,
  ...
  ScopedLocalRef<jbyteArray> addr(
      sCallbackEnv.get(), sCallbackEnv->NewByteArray(sizeof(RawAddress)));
  if (!addr.get()) {
    log::error("Error while allocating in: ");
    return;
  }
...
static bool initNative(JNIEnv* env, jobject obj, jboolean isGuest,
  ...
  if (ret != BT_STATUS_SUCCESS) {
    log::error("Error while setting the callbacks: {}\n", ret);
    sBluetoothInterface = NULL;
    return JNI_FALSE;
  }
  ret = sBluetoothInterface->set_os_callouts(&sBluetoothOsCallouts);
  if (ret != BT_STATUS_SUCCESS) {
    log::error("Error while setting Bluetooth callouts: {}\n", ret);
