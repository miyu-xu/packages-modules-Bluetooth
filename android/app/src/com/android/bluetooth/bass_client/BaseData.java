...
class BaseData {
    ...
    private static int sParseOffset;
    ...
    private static final int METADATA_UNKOWN_CODEC_LENGTH = 1;
    ...
    static BaseData parseBaseData(byte[] serviceData) {
        ...
        sParseOffset = 0;
        System.arraycopy(serviceData, sParseOffset, levelOne.presentationDelay, 0, 3);
        sParseOffset += METADATA_PRESENTATIONDELAY_LENGTH;
        levelOne.numSubGroups = serviceData[sParseOffset++];
        levelOne.print();
    ...
    private static BaseInformation parseLevelTwo(byte[] serviceData, int groupIndex) {
        ...
        node.numSubGroups = serviceData[sParseOffset++];
        if (serviceData[sParseOffset] == (byte) UNKNOWN_CODEC) {
            // Place It in the last byte of codecID
            System.arraycopy(serviceData, sParseOffset, node.codecId,
                    METADATA_CODEC_LENGTH - 1, METADATA_UNKOWN_CODEC_LENGTH);
            sParseOffset += METADATA_UNKOWN_CODEC_LENGTH;
            log("codecId is FE");
        } else {
            System.arraycopy(serviceData, sParseOffset, node.codecId,
                    0, METADATA_CODEC_LENGTH);
            sParseOffset += METADATA_CODEC_LENGTH;
        }
        node.codecConfigLength = serviceData[sParseOffset++];
        if (node.codecConfigLength != 0) {
            node.codecConfigInfo = new byte[(int) node.codecConfigLength];
            System.arraycopy(serviceData, sParseOffset, node.codecConfigInfo,
                    0, (int) node.codecConfigLength);
            sParseOffset += (int) node.codecConfigLength;
        }
        node.metaDataLength = serviceData[sParseOffset++];
        if (node.metaDataLength != 0) {
            node.metaData = new byte[(int) node.metaDataLength];
            System.arraycopy(serviceData, sParseOffset,
                    node.metaData, 0, (int) node.metaDataLength);
            sParseOffset += (int) node.metaDataLength;
        }
        return node;
    }
    ...
    private static BaseInformation parseLevelThree(byte[] serviceData) {
        ...
        node.index = serviceData[sParseOffset++];
        node.codecConfigLength = serviceData[sParseOffset++];
        if (node.codecConfigLength != 0) {
            node.codecConfigInfo = new byte[(int) node.codecConfigLength];
            System.arraycopy(serviceData, sParseOffset,
