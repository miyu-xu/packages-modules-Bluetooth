...
public class BassClientService extends ProfileService {
            ...
            mSearchScanCallback =
                    new ScanCallback() {
                        ...
                        public void onScanResult(int callbackType, ScanResult result) {
                            ...
                            sEventLogger.logd(
                                    TAG, "Broadcast Source Found: Broadcast ID: " + broadcastId);
                            ...
                            if (broadcastId != BassConstants.INVALID_BROADCAST_ID
                                ...
                                if (leaudioBroadcastExtractPeriodicScannerFromStateMachine()) {
                                    addSelectSourceSyncRequest(result, false);
                                } else {
                                    synchronized (mStateMachines) {
                                        for (BassClientStateMachine sm : mStateMachines.values()) {
                                            if (sm.isConnected()) {
        ...
        public void onSyncEstablished(
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                updatePeriodicAdvertisementResultMap(
                        ...
                        null,
                        null);
                // TODO add timeout to keep sync only by some time, but for what is that?
    ...
    private void cancelActiveSync(Integer syncHandle) {
        ...
        if (!activeSyncedSrc.isEmpty()
            ...
            if (syncHandle != null) {
                ...
                removeActiveSyncedSource(null);
            }
            if (getActiveSyncedSources().isEmpty()) {
                // all sources are removed, clean up
                // removeMessages(PSYNC_ACTIVE_TIMEOUT); TODO
                clearNotifiedFlags();
            }
        }
    }
    ...
    void addSelectSourceSyncRequest(ScanResult scanRes, boolean hasPriority) {
        sEventLogger.logd(
                TAG,
        ...
        if (scanRecord == null) {
            log("selectSource: ScanRecord empty");
            return;
        }
    ...
    void selectSource() {
        ...
        synchronized (mPeriodicAdvCallbacksMap) {
            if (mSourceSyncRequestsQueue.isEmpty()) {
                ...
                log("selectSource: already pending sync");
                return;
            } else {
                paCb = new PACallback();
        ...
        if (leaudioBroadcastExtractPeriodicScannerFromStateMachine()) {
            ...
            if (!isLocalBroadcast(sourceMetadata)
                ...
                if (broadcastId != BassConstants.INVALID_BROADCAST_ID
                    ...
                    addSelectSourceSyncRequest(getCachedBroadcast(broadcastId), true);
                    mPendingSourcesToAdd.add(new AddSourceData(sink, sourceMetadata, isGroupOp));
                } else {
                    // TODO sometimes go there because of lack broadcast in cache
                    log("AddSource: invalid broadcast, broadcastId: " + broadcastId);
                    mCallbacks.notifySourceAddFailed(
