...
public class BassClientService extends ProfileService {
            ...
            if (paResMap == null
                ...
                if (bId == BassConstants.INVALID_BROADCAST_ID) {
                    ...
                    if (leaudioBroadcastExtractPeriodicScannerFromStateMachine()) {
                        bId = getBroadcastIdForSyncHandle(BassConstants.INVALID_BROADCAST_ID);
    ...
    final class PACallback extends PeriodicAdvertisingCallback {
        ...
        public void onPeriodicAdvertisingReport(PeriodicAdvertisingReport report) {
            log("onPeriodicAdvertisingReport " + report.getSyncHandle());
            Integer bisCounter = mBisDiscoveryCounterMap.get(report.getSyncHandle());

            // Parse the BIS indices from report's service data
            if (bisCounter != null && bisCounter != 0) {
                if (parseScanRecord(report.getSyncHandle(), report.getData())) {
                    mBisDiscoveryCounterMap.put(report.getSyncHandle(), 0);
                } else {
                    bisCounter--;
                    mBisDiscoveryCounterMap.put(report.getSyncHandle(), bisCounter);
                    if (bisCounter == 0) {
                        cancelActiveSync(report.getSyncHandle());
                    }
                }
            }
        }
