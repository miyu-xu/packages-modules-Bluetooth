...
public class BassClientService extends ProfileService {
    ...
    private int areValidParametersToModifySource(
        ...
        if (stateMachine.hasPendingSourceOperation()) {
            Log.w(TAG, "modifySource: source operation already pending, device: " + device
                    + ", broadcastId: " + updatedMetadata.getBroadcastId());
            mCallbacks.notifySourceAddFailed(device, updatedMetadata,
    ...
    public void addSource(
        ...
        for (BluetoothDevice device : devices) {
            ...
            if (stateMachine.hasPendingSourceOperation()) {
                Log.w(TAG, "addSource: source operation already pending, device: " + device
        ...
        for (BluetoothDevice device : getConnectedDevices()) {
            synchronized (mStateMachines) {
                ...
                if (sm != null && sm.hasPendingSourceOperation(broadcastId)) {
                    Message message =
                            sm.obtainMessage(BassClientStateMachine.CANCEL_PENDING_SOURCE_OPERATION);
