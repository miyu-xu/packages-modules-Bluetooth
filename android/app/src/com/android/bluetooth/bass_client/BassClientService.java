...
public class BassClientService extends ProfileService {
    ...
    private class BassScanCallback extends IScannerCallback.Stub {
        private static final int SCANNER_NOT_INITIALIZED = -2;
        private static final int SCANNER_INITIALIZING = -1;

        private List<ScanFilter> mFilters = new ArrayList<ScanFilter>();
        private int mScannerId = SCANNER_NOT_INITIALIZED;

        void startRegistration(List<ScanFilter> filters) {
            synchronized (this) {
                if (mScannerId == SCANNER_INITIALIZING) {
                    Log.d(TAG, "startRegistration: Scanner is already initializing");
                ...
                if (controller == null) {
                    Log.d(TAG, "startRegistration: Scanner is null");
        ...
        void stopScan() {
            synchronized (this) {
                ScanController controller = mAdapterService.getBluetoothScanController();
                if (controller == null) {
                    Log.d(TAG, "stopScan: Scanner is null");
        ...
        public void onScannerRegistered(int status, int scannerId) {
            ...
            synchronized (this) {
                ...
                ScanController controller = mAdapterService.getBluetoothScanController();
                if (controller == null) {
                    Log.d(TAG, "onScannerRegistered: Scanner is null");
                    mCallbacks.notifySearchStartFailed(BluetoothStatusCodes.ERROR_UNKNOWN);
                    return;
                }
                controller.startScanInternal(scannerId, settings, mFilters);
        ...
        public void onScanResult(ScanResult result) {
            ...
            if (listOfUuids == null || !listOfUuids.containsKey(BassConstants.BAAS_UUID)) {
                Log.d(TAG, "Service data is invalid");
                return;
            }
    ...
    public void startSearchingForSources(List<ScanFilter> filters) {
        ...
        synchronized (mSearchScanCallbackLock) {
            ...
            if (leaudioBassScanWithInternalScanController()) {
                mBassScanCallback.startRegistration(filters);
