...
public class BassClientService extends ProfileService {
    ...
    PeriodicAdvertisementResult getPeriodicAdvertisementResult(BluetoothDevice device) {
        PeriodicAdvertisementResult res = null;
        if (mPeriodicAdvertisementResultMap != null) {
            res = mPeriodicAdvertisementResultMap.get(device);
        } else {
    ...
    PeriodicAdvertisementResult clearPeriodicAdvertisementResult(BluetoothDevice device) {
        PeriodicAdvertisementResult res = null;
        if (mPeriodicAdvertisementResultMap != null) {
            res = mPeriodicAdvertisementResultMap.remove(device);
        } else {
            Log.e(TAG, "getPeriodicAdvertisementResult: mPeriodicAdvertisementResultMap is null");
        }
        return res;
                    ...
                    if (scanRecord != null) {
                        ...
                        if (listOfUuids != null) {
                            if (listOfUuids.containsKey(
                                    BassConstants.BASS_UUID)) {
                                byte[] bId = listOfUuids.get(BassConstants.BASS_UUID);
                                int broadcastId = (0x00FF0000 & (bId[0] << 16));
                                broadcastId |= (0x0000FF00 & (bId[1] << 8));
                                broadcastId |= (0x000000FF & bId[2]);
                                BluetoothLeBroadcastMetadata metadata =
                                        new BluetoothLeBroadcastMetadata.Builder()
                                                .setSourceDevice(result.getDevice(),
                                                        result.getAddressType())
                                                .setSourceAdvertisingSid(result.getAdvertisingSid())
                                                .setBroadcastId(broadcastId)
                                                .setPaSyncInterval(
                                                        result.getPeriodicAdvertisingInterval())
                                                .build();
                                if (mBassUtils.mAutoAssistScanHandler != null) {
                                    Message msg = mBassUtils.mAutoAssistScanHandler
                                            .obtainMessage(BassConstants.AA_SCAN_SUCCESS);
                                    msg.obj = result;
                                    mBassUtils.mAutoAssistScanHandler.sendMessage(msg);
                                }
    ...
    void selectSource(BluetoothDevice sink, ScanResult result, boolean autoTrigger) {
        ...
        synchronized (mStateMachines) {
            ...
            message.obj = result;
            message.arg1 = autoTrigger ? stateMachine.AUTO : stateMachine.USER;
