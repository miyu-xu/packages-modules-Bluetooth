...
public class BassClientService extends ProfileService {
    ...
    private Map<BluetoothDevice, List<BluetoothDevice>> mActiveSourceMap;
    ...
    void removeActiveSyncedSource(BluetoothDevice scanDelegator, BluetoothDevice sourceDevice) {
        ...
        if (sourceDevice == null) {
            ...
            List<BluetoothDevice> sources = mActiveSourceMap.get(scanDelegator);
            if (sources != null) {
                sources.removeIf(e -> e.equals(sourceDevice));
                if (sources.isEmpty()) {
                    mActiveSourceMap.remove(scanDelegator);
                }
            }
    ...
    void addActiveSyncedSource(BluetoothDevice scanDelegator, BluetoothDevice sourceDevice) {
        ...
        log(
                "addActiveSyncedSource, scanDelegator: "
                        ...
                        + sourceDevice);
        if (sourceDevice != null) {
            if (!mActiveSourceMap.containsKey(scanDelegator)) {
                mActiveSourceMap.put(scanDelegator, new ArrayList<>());
            }
    ...
    List<BluetoothDevice> getActiveSyncedSource(BluetoothDevice scanDelegator) {
        if (mActiveSourceMap == null) {
            Log.e(TAG, "getActiveSyncedSource: mActiveSourceMap is null");
            return null;
        }

        List<BluetoothDevice> currentSources = mActiveSourceMap.get(scanDelegator);
    ...
    protected boolean start() {
        ...
        mActiveSourceMap = new HashMap<BluetoothDevice, List<BluetoothDevice>>();
    ...
    BluetoothDevice getDeviceForSyncHandle(int syncHandle) {
        ...
        if (device == null) {
            Log.w(TAG, "No sync handle found: " + syncHandle);
        }
        return device;
    }
    ...
    void selectSource(BluetoothDevice sink, ScanResult result, boolean autoTrigger) {
        ...
        List<BluetoothDevice> activeSyncedSrc = getActiveSyncedSource(sink);
        if (activeSyncedSrc != null
                && (activeSyncedSrc.size() >= MAX_ACTIVE_SYNCED_SOURCES_NUM
                        || activeSyncedSrc.contains(result.getDevice()))) {
            log(
                    "selectSource : found num of active sources: "
                            + activeSyncedSrc.size()
                            + ", is source synced: "
                            + activeSyncedSrc.contains(result.getDevice()));
            return;
        }
