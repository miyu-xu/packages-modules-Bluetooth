        ...
        if (mDialingOutTimeoutEvent != null) {
            for (Map.Entry<Integer, HashSet<BluetoothDevice>> entry :
                ...
                if ((mDialingOutTimeoutEvent.isScheduledForBroadcast(broadcastId))
                    ...
                    Log.i(
                            TAG, "connectionStateChanged: reconnected previousely synced device: "
                                    + device);
                    mHandler.removeCallbacks(mDialingOutTimeoutEvent);
                    mDialingOutTimeoutEvent = null;
                    break;
    ...
    public void handleDeviceDisconnection(BluetoothDevice sink, boolean isIntentional) {
        ...
        for (Map.Entry<Integer, HashSet<BluetoothDevice>> entry :
            ...
            if (isIntentional) {
                ...
                Log.d(
                        TAG, "handleIntendedDeviceDisconnection: No more potential broadcast "
                                + "(broadcast ID: "
                                + broadcastId
                ...
                if (devices.stream()
                    ...
                    Log.d(
                            TAG, "handleUnintendedDeviceDisconnection: No more potential broadcast "
                                    + "(broadcast ID: "
                                    + broadcastId
                                    + ") receivers - stopping broadcast");
