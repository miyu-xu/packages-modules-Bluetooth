...
public class BassClientService extends ProfileService {
    ...
    synchronized void connectionStateChanged(BluetoothDevice device, int fromState,
        ...
        if (toState == BluetoothProfile.STATE_DISCONNECTED) {
            ...
            LeAudioService leAudioService = mServiceFactory.getLeAudioService();
            if (leAudioService == null) {
                Log.d(TAG, "connectionStateChanged: No available LeAudioService");
                return;
            }

            /* In case if this may be reconnecting primary device which was synced to broadcast */
            for (Map.Entry<Integer, HashSet<BluetoothDevice>> entry :
                ...
                if (devices.contains(device) && leAudioService.isPrimaryDevice(device)) {
                    Log.i(
                            TAG,
                            "connectionStateChanged: reconnected previousely synced primary "
                                    + "device: "
                                    + device);
                    mHandler.removeCallbacks(mDialingOutTimeoutEvent);
                    mDialingOutTimeoutEvent = null;
