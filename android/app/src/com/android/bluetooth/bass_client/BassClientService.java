...
public class BassClientService extends ProfileService {
    ...
    private void localNotifySourceAdded(BluetoothDevice sink,
        ...
        if (mLocalBroadcastReceivers.containsKey(broadcastId)) {
            ...
            mLocalBroadcastReceivers.put(broadcastId,
                    new HashSet<BluetoothDevice>(Arrays.asList(sink)));
        }
    }
    ...
    /**
     * Stop broadcast if there are no potential receivers. This is called by BassClientStateMachine.
     */
    public void handleIntendedDeviceDisconnection(BluetoothDevice sink) {
        ...
        for (Map.Entry<Integer, HashSet<BluetoothDevice>> entry
            ...
            /* If there was a sink bonded with broadcast */
            if (devices.remove(sink) && devices.isEmpty()
                    && leAudioService.isPlaying(broadcastId)) {
                Log.d(TAG, "handleIntendedDeviceDisconnection: No more potential broadcast "
                        + "(broadcast ID: " + broadcastId + ") receivers - stopping broadcast");
