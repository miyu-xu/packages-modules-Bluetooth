...
public class BassClientService extends ProfileService {
    ...
    private boolean mAreModifiedAllowedContextOfActiveGroup = false;
    ...
    public void stop() {
        ...
        if (mAreModifiedAllowedContextOfActiveGroup) {
            LeAudioService leAudioService = mServiceFactory.getLeAudioService();
            if (leAudioService != null) {
                leAudioService.setActiveGroupAllowedContextMask(ALL_CONTEXTS, ALL_CONTEXTS);
            }
            mAreModifiedAllowedContextOfActiveGroup = false;
        }
    ...
    private boolean isDevicePartOfActiveUnicastGroup(BluetoothDevice device) {
        ...
        if (leAudioService.getActiveGroupId() == LE_AUDIO_GROUP_ID_INVALID) {
            return false;
        }
        ...
        for (BluetoothDevice device : getConnectedDevices()) {
            for (BluetoothLeBroadcastReceiveState receiveState : getAllSources(device)) {
                for (int i = 0; i < receiveState.getNumSubgroups(); i++) {
                    ...
                    /* Check if device is synced to any of BISes */
            ...
            if (leaudioAllowedContextMask()) {
                ...
                if (!mAreModifiedAllowedContextOfActiveGroup
                    ...
                    leAudioService.setActiveGroupAllowedContextMask(
                            ...
                            ALL_CONTEXTS);
                    mAreModifiedAllowedContextOfActiveGroup = true;
                }
            }
        } else {
