...
public class BCService extends ProfileService {
    ...
    private Map<BluetoothDevice, ArrayList<IBluetoothLeBroadcastAssistantCallback>>
            mAppCallbackMap =
                    new HashMap<
                            BluetoothDevice, ArrayList<IBluetoothLeBroadcastAssistantCallback>>();
     ...
     * @param addessType address type of device
    ...
    public void setActiveSyncedSource(BluetoothDevice scanDelegator, BluetoothDevice sourceDevice) {
        log(
                "setActiveSyncedSource: scanDelegator"
                        + scanDelegator
                        + ":: sourceDevice:"
                        + sourceDevice);
        ...
        synchronized (mStateMachines) {
            for (BluetoothDevice device : bondedDevices) {
                ...
                if (!ArrayUtils.contains(
                        featureUuids, new ParcelUuid(BassClientStateMachine.BASS_UUID))) {
                    continue;
                }
     ...
     */
    public synchronized boolean startScanOffload(
     ...
     */
    public synchronized boolean stopScanOffload(
            BluetoothDevice masterDevice, List<BluetoothDevice> devices) {
        ...
        for (BluetoothDevice dev : devices) {
            ...
            for (int i = 0; i < currentSourceInfos.size(); i++) {
                if (srcInfo.matches(currentSourceInfos.get(i))) {
                    ...
                    Log.e(
                            TAG,
                            "isValidBroadcastSourceAddition: fails for: "
                                    + dev
                                    + "&srcInfo"
                                    + srcInfo);
                    break;
     ...
     */
    public synchronized boolean addBroadcastSource(
     ...
     */
    public synchronized boolean updateBroadcastSource(
            BluetoothDevice masterDevice,
            List<BluetoothDevice> devices,
            BluetoothLeBroadcastSourceInfo srcInfo) {
     ...
     * Set broadcast code
     */
    public synchronized boolean setBroadcastCode(
            BluetoothDevice masterDevice,
            List<BluetoothDevice> devices,
            BluetoothLeBroadcastSourceInfo srcInfo) {
        ...
        Log.i(
                TAG,
                "setBroadcastCode for "
                        + devices
                        + "masterDevice"
                        + masterDevice
                        + "Broadcast PIN"
                        + srcInfo.getBroadcastCode());
     ...
     */
    public synchronized boolean removeBroadcastSource(
            BluetoothDevice masterDevice, List<BluetoothDevice> devices, byte sourceId) {

        int status = BluetoothLeBroadcastAssistantCallback.BASS_STATUS_INVALID_SOURCE_ID;
        Log.i(
                TAG,
                "removeBroadcastSource for "
                        + devices
                        + "masterDevice "
                        + masterDevice
                        + "removeBroadcastSource: sourceId:"
                        + sourceId);
    ...
    /**
     * updateBroadcastSourceInternal
     */
    public boolean updateBroadcastSourceInternal(
            BluetoothDevice device, BluetoothLeBroadcastSourceInfo srcInfo, boolean isGroupOp) {
        boolean ret = false;
        log(
                "updateBroadcastSourceInternal: device: "
                        + device
                        + "srcInfo"
                        + srcInfo
                        + "isGroupOp"
    ...
    protected boolean setBroadcastCodeInternal(
            BluetoothDevice device, BluetoothLeBroadcastSourceInfo srcInfo, boolean isGroupOp) {
        boolean ret = false;
        log(
                "setBroadcastCodeInternal: device: "
                        + device
                        + "srcInfo"
                        + srcInfo
                        + "isGroupOp"
    ...
    public boolean removeBroadcastSourceInternal(
        ...
        log(
                "removeBroadcastSourceInternal: device: "
                        + device
                        + "sourceId "
                        + sourceId
                        + "isGroupOp "
