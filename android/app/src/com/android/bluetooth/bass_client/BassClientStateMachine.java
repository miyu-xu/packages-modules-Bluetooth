...
final class BassClientStateMachine extends StateMachine {
    ...
    private static int num_of_receiver_states = 0;
    ...
    BluetoothLeBroadcastSourceInfo getBroadcastSourceInfoForSourceDevice(
        ...
        for (int i=0; i<currentSourceInfos.size(); i++) {
            ...
            if (device != null && device.equals(srcDevice)) {
                ...
                Log.e(TAG, "getBroadcastSourceInfoForSourceDevice: returns for: " +
    ...
    void parseBaseData(BluetoothDevice device, int syncHandle, byte[] serviceData) {
        ...
        if (base_ != null) {
            ...
            base_.print();
            if (mAutoTriggered == false) {
                mService.sendBroadcastSourceSelectedCallback(device,
    ...
    public boolean selectBroadcastSource(ScanResult scanRes, boolean groupOp,
    ...
    private void broadcastReceiverState(BluetoothLeBroadcastSourceInfo state,
            int index, int max_num_srcInfos) {
    ...
    private void checkAndUpdateBroadcastCode(BluetoothLeBroadcastSourceInfo srcInfo) {
        ...
        if (srcInfo.getEncryptionStatus() ==
            BluetoothLeBroadcastSourceInfo.LE_AUDIO_BROADCAST_SINK_ENC_STATE_CODE_REQUIRED &&
    ...
    private void processBroadcastReceiverState(byte[] receiverState,
        ...
        if (receiverState.length == 0 ||
           ...
           if (srcInfo.isEmpty()) {
            ...
            System.arraycopy(receiverState, BCAST_RCVR_STATE_SRC_ADDR_START_IDX, sourceAddress,
                                    0, BCAST_RCVR_STATE_SRC_ADDR_SIZE);
            ...
            System.arraycopy(receiverState, BCAST_RCVR_STATE_SRC_BCAST_ID_START_IDX,
                            broadcastIdBytes, 0, BROADCAST_SOURCE_ID_LENGTH);
            ...
            if (encryptionStatus ==
                ...
                System.arraycopy(receiverState, BCAST_RCVR_STATE_BADCODE_START_IDX,
                                    badBroadcastCode, 0, BCAST_RCVR_STATE_BADCODE_SIZE);
            ...
            for (int i = 0; i < numSubGroups; i++) {
                ...
                System.arraycopy(receiverState, offset, audioSyncIndex, 0,
                    BCAST_RCVR_STATE_BIS_SYNC_SIZE);
            ...
            srcInfo = new BluetoothLeBroadcastSourceInfo(sourceId,
        ...
        if (oldSourceInfo == null) {
            ...
            if (mBluetoothLeBroadcastSourceInfos.size() == NUM_OF_BROADCAST_RECEIVER_STATES) {
            ...
            if (oldSourceInfo.isEmpty() == true) {
                log("New Source Addition");
                sendPendingCallbacks(ADD_BCAST_SOURCE,
                          srcInfo, BluetoothGatt.GATT_SUCCESS);
                checkAndUpdateBroadcastCode(srcInfo);
            } else {
                if (isEmptyEntry) {
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
           ...
           if (newState == BluetoothProfile.STATE_CONNECTED && getConnectionState() !=
    ...
    void acquireAllBassChars() {
        ...
        if (service != null) {
            ...
            num_of_receiver_states = NUM_OF_BROADCAST_RECEIVER_STATES;
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                case CONNECT:
                    ...
                    if ((mBluetoothGatt = mDevice.connectGatt(mService, mIsAllowedList,
    ...
    private byte[] convertSourceInfoToAddSourceByteArray(BluetoothLeBroadcastSourceInfo srcInfo) {
        ...
        if (paRes == null) {
            ...
            sendPendingCallbacks(ADD_BCAST_SOURCE, srcInfo,
                       BluetoothLeBroadcastAssistantCallback.BASS_STATUS_SOURCE_UNAVAILABLE);
            return null;
        }
        ...
        if (paRes.mAddressType != (byte)BCService.INVALID_ADV_ADDRESS_TYPE) {
            res[1] = (byte)paRes.mAddressType;
        } else {
            res[1] = (byte)BROADCAST_ASSIST_ADDRESS_TYPE_PUBLIC;
        }
    ...
    private byte[] convertSourceInfoToUpdateSourceByteArray(
        ...
        int offset = 6;
        int bisIndexValue = 0;
        if (srcInfo.getAudioSyncState() ==
            BluetoothLeBroadcastSourceInfo.LE_AUDIO_BROADCAST_SINK_AUDIO_SYNC_STATE_SYNCHRONIZED) {
    ...
    private byte[] convertSourceInfoToSetBroadcastCodeByteArray(
        ...
        if (actualPIN == null) {
            ...
            System.arraycopy(actualPIN, 0, res, ((PIN_CODE_CMD_LEN)-actualPIN.length),
                    actualPIN.length);
            ...
            switch (message.what) {
                ...
                case DISCONNECT:
                    ...
                    if (mBluetoothGatt != null) {
                        ...
                        mBluetoothGatt = null;
                        cancelActiveSync(null);
                        //Trigger the CSip disconnection, don't worry about pass/failure
                ...
                case UPDATE_BCAST_SOURCE:
                    ...
                    if (mBluetoothGatt != null &&
                        ...
                        sendPendingCallbacks(UPDATE_BCAST_SOURCE, srcInfo,
                            BluetoothLeBroadcastAssistantCallback.BASS_STATUS_FAILURE);
                    }
                    break;
                case SET_BCAST_CODE:
                    ...
                    if (cmdType != QUEUED &&
                        ...
                        if (mBluetoothGatt != null &&
                          mBroadcastScanControlPoint != null) {
                            ...
                            sendPendingCallbacks(SET_BCAST_CODE, srcInfo,
                                BluetoothLeBroadcastAssistantCallback.BASS_STATUS_FAILURE);
                ...
                case REMOVE_BCAST_SOURCE:
                    ...
                    log("Removing Broadcast source: audioSource:" + audioSrc +
                            "sourceId:" + sourceId);
                    ...
                    if (mBluetoothGatt != null &&
                        ...
                        sendPendingCallbacks(REMOVE_BCAST_SOURCE,
                            new BluetoothLeBroadcastSourceInfo(INVALID_SRC_ID),
                            BluetoothLeBroadcastAssistantCallback.BASS_STATUS_FAILURE);
    ...
    void sendPendingCallbacks(int pendingOp, BluetoothLeBroadcastSourceInfo source,
			int status) {
        switch (pendingOp) {
            case START_SCAN_OFFLOAD:
                if (status != 0) {
