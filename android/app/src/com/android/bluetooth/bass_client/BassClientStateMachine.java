...
/**
 * Bluetooth Bassclient StateMachine. There is one instance per remote device. - "Disconnected" and
 * "Connected" are steady states. - "Connecting" and "Disconnecting" are transient states until the
 * connection / disconnection is completed. - "ConnectedProcessing" is an intermediate state to
 * ensure, there is only one Gatt transaction from the profile at any point of time
 *
 * <p>(Disconnected) | ^ CONNECT | | DISCONNECTED V | (Connecting)<--->(Disconnecting) | ^ CONNECTED
 * | | DISCONNECT V | (Connected) | ^ GATT_TXN | | GATT_TXN_DONE/GATT_TXN_TIMEOUT V |
 * (ConnectedProcessing) NOTES: - If state machine is in "Connecting" state and the remote device
 * sends DISCONNECT request, the state machine transitions to "Disconnecting" state. - Similarly, if
 * the state machine is in "Disconnecting" state and the remote device sends CONNECT request, the
 * state machine transitions to "Connecting" state. - Whenever there is any Gatt Write/read, State
 * machine will moved "ConnectedProcessing" and all other requests (add, update, remove source)
 * operations will be deferred in "ConnectedProcessing" state - Once the gatt transaction is done
 * (or after a specified timeout of no response), State machine will move back to "Connected" and
 * try to process the deferred requests as needed
 *
...
final class BassClientStateMachine extends StateMachine {
    ...
    BassClientStateMachine(BluetoothDevice device, BCService svc, Looper looper) {
    ...
    static BassClientStateMachine make(BluetoothDevice device, BCService svc, Looper looper) {
    ...
    BluetoothLeBroadcastSourceInfo getBroadcastSourceInfoForSourceId(int srcId) {
        ...
        for (int i = 0; i < currentSourceInfos.size(); i++) {
            ...
            if (srcId == sId) {
                ...
                Log.e(
                        TAG,
                        "getBroadcastSourceInfoForSourceId: returns for: "
                                + srcId
                                + "&srcInfo"
                                + srcInfo);
                return srcInfo;
    ...
    public boolean selectBroadcastSource(
        ...
        if (scanRecord != null) {
            ...
            mService.updatePAResultsMap(
                    ...
                    BCService.INVALID_SYNC_HANDLE,
                    BCService.INVALID_ADV_SID,
            ...
            new BluetoothGattCallback() {
                ...
                public void onCharacteristicChanged(
                    ...
                    if (characteristic.getUuid().equals(BASS_BCAST_RECEIVER_STATE)) {
                        log(
                                "onCharacteristicChanged is rcvr State :: "
                        ...
                        logByteArray(
                                "onCharacteristicChanged: Received ",
                                characteristic.getValue(),
                                0,
                                characteristic.getValue().length);
                        processBroadcastReceiverState(characteristic.getValue(), characteristic);
    ...
    private byte[] convertSourceInfoToSetBroadcastCodeByteArray(
        ...
        res[1] = srcInfo.getSourceId();
        log(
                "convertSourceInfoToSetBroadcastCodeByteArray: Source device : "
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case REMOVE_BCAST_SOURCE:
                    ...
                    log(
                            "Removing Broadcast source: audioSource:"
                                    + audioSrc
                                    + "sourceId:"
                                    + sourceId);
