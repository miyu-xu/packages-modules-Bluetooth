...
class BassClientStateMachine extends StateMachine {
    ...
    private final Map<Integer, BluetoothLeBroadcastReceiveState> mBluetoothLeBroadcastReceiveStates;
    ...
    private static ParcelUuid sBasicAudioUuid =
    ...
    BassClientStateMachine(BluetoothDevice device, BassClientService svc, Looper looper) {
        ...
        mBluetoothLeBroadcastReceiveStates = new HashMap<Integer,
        ...
        mNoReverse = DeviceConfig.getBoolean(DeviceConfig.NAMESPACE_BLUETOOTH,
                "persist.vendor.service.bt.nReverse", false);
    ...
    void parseScanRecord(int syncHandle, ScanRecord record) {
        ...
        byte[] advData = record.getServiceData(sBasicAudioUuid);
        if (advData != null) {
    ...
    private void broadcastReceiverState(
        ...
        mService.mCallbacks.notifyReceiveStateChanged(mDevice, sourceId, state);
    }
    ...
    private void processPASyncState(BluetoothLeBroadcastReceiveState recvState) {
        ...
        if (state == BluetoothLeBroadcastReceiveState.PA_SYNC_STATE_SYNCINFO_REQUEST) {
            ...
            BassClientService.PeriodicAdvertisementResult result =
                    mService.getPeriodicAdvertisementResult(
                    recvState.getSourceDevice());
            if (result != null) {
                int syncHandle = result.mSyncHandle;
                log("processPASyncState: syncHandle " + result.mSyncHandle);
    ...
    private void processBroadcastReceiverState(
        ...
        if (receiverState.length == 0
            ...
            if (mPendingOperation == REMOVE_BCAST_SOURCE) {
                ...
                if (mBluetoothLeBroadcastReceiveStates != null) {
                    mTempSourceId = (byte) mBluetoothLeBroadcastReceiveStates.size();
                }
                ...
                mTempSourceId++;
                recvState = new BluetoothLeBroadcastReceiveState(mTempSourceId,
                        BluetoothDevice.ADDRESS_TYPE_UNKNOWN, // sourceAddressType
                        null,   // sourceDevice
                        0,  // sourceAdvertisingSid
                        0,  // broadcastId
            ...
            numSubGroups = receiverState[BCAST_RCVR_STATE_BADCODE_START_IDX
                    + BCAST_RCVR_STATE_BADCODE_SIZE];
            int offset = BCAST_RCVR_STATE_BADCODE_START_IDX + BCAST_RCVR_STATE_BADCODE_SIZE + 1;
        ...
        if (oldRecvState == null) {
            ...
            if (oldRecvState.getSourceDevice() == null) {
                ...
                mService.mCallbacks.notifySourceAdded(mDevice,
                ...
                if (isEmptyEntry) {
                    ...
                    mService.mCallbacks.notifySourceRemoved(mDevice,
                            oldRecvState.getSourceId(),
                            BluetoothStatusCodes.REASON_LOCAL_APP_REQUEST);
                } else {
                    log("update to an existing recvState");
                    mService.updateSourceInternal(recvState.getSourceId(), mPendingMetadata);
                    mService.mCallbacks.notifySourceModified(mDevice,
                            recvState.getSourceId(), BluetoothStatusCodes.REASON_LOCAL_APP_REQUEST);
                    checkAndUpdateBroadcastCode(recvState);
                    processPASyncState(recvState);
                }
        ...
        index = recvState.getSourceId();
        if (index == -1) {
            log("processBroadcastReceiverState: invalid index");
            return;
        }
        broadcastReceiverState(recvState, index);
    }
                ...
                public void onServicesDiscovered(BluetoothGatt gatt, int status) {
                    ...
                    if (mDiscoveryInitiated) {
                        ...
                        if (status == BluetoothGatt.GATT_SUCCESS && mBluetoothGatt != null) {
                            mBluetoothGatt.requestMtu(BassConstants.BassMaxBytes);
    ...
    private byte[] convertMetadataToAddSourceByteArray(BluetoothLeBroadcastMetadata metaData) {
        ...
        int offset = 0;
        res[offset++] = mBassAddSourceOpcode;
        if (paRes.mAddressType != (byte) BassConstants.INVALID_ADV_ADDRESS_TYPE) {
            res[offset++] = (byte) paRes.mAddressType;
        } else {
        ...
        offset += 6;
        res[offset++] = (byte) paRes.mAdvSid;
        log("mBroadcastId: " + paRes.mBroadcastId);
        res[offset++] = (byte) (paRes.mBroadcastId & 0x00000000000000FF);
        res[offset++] = (byte) ((paRes.mBroadcastId & 0x000000000000FF00) >>> 8);
        res[offset++] = (byte) ((paRes.mBroadcastId & 0x0000000000FF0000) >>> 16);
        if (!mDefNoPAS) {
        ...
        res[offset++] = (byte) (paRes.mPAInterval & 0x00000000000000FF);
        res[offset++] = (byte) ((paRes.mPAInterval & 0x000000000000FF00) >>> 8);
        res[offset++] = base_.getNumberOfSubgroupsofBIG();
    ...
    private byte[] convertBroadcastMetadataToUpdateSourceByteArray(int sourceId,
        ...
        if (paRes == null) {
            ...
            mService.mCallbacks.notifySourceRemoveFailed(
        ...
        byte numSubGroups = base_.getNumberOfSubgroupsofBIG();
        res = new byte[updateSourceFixedLength + numSubGroups * 5];
        int offset = 0;
        res[offset++] = mBassUpdateSourceOpcode;
        ...
        BluetoothDevice existingSrcDevice = existingState.getSourceDevice();
        res[offset++] = (byte) 0xFF;
        res[offset++] = (byte) 0xFF;
            ...
            switch (message.what) {
                ...
                case ADD_BCAST_SOURCE:
                    ...
                    if (mBluetoothGatt != null && mBroadcastScanControlPoint != null) {
                        ...
                        mService.mCallbacks.notifySourceAddFailed(mDevice,
                ...
                case UPDATE_BCAST_SOURCE:
                    ...
                    if (mBluetoothGatt != null && mBroadcastScanControlPoint != null) {
                        ...
                        mService.mCallbacks.notifySourceModifyFailed(
                                mDevice, sourceId, BluetoothStatusCodes.ERROR_UNKNOWN);
                    }
                    break;
                case SET_BCAST_CODE:
                    ...
                    if (cmdType != QUEUED
                            && !isItRightTimeToUpdateBroadcastPin((byte) recvState.getSourceId())) {
                ...
                case REMOVE_BCAST_SOURCE:
                    ...
                    if (mBluetoothGatt != null && mBroadcastScanControlPoint != null) {
                        ...
                        mService.mCallbacks.notifySourceRemoveFailed(mDevice,
        ...
        switch (pendingOp) {
            ...
            case ADD_BCAST_SOURCE:
                if (!isSuccess(status)) {
                    ...
                    Message message = obtainMessage(STOP_SCAN_OFFLOAD);
                    sendMessage(message);
                    mService.mCallbacks.notifySourceAddFailed(mDevice, mPendingMetadata, status);
                    mPendingMetadata = null;
                }
                break;
            case UPDATE_BCAST_SOURCE:
                if (!mAutoTriggered) {
                    if (!isSuccess(status)) {
                        mService.mCallbacks.notifySourceModifyFailed(mDevice,
            ...
            case REMOVE_BCAST_SOURCE:
                if (!isSuccess(status)) {
                    mService.mCallbacks.notifySourceRemoveFailed(mDevice, mPendingSourceId, status);
                }
                break;
            case SET_BCAST_CODE:
