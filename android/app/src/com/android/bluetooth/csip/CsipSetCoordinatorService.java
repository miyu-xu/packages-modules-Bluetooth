...
public class CsipSetCoordinatorService extends ProfileService {
    ...
    private final Map<Integer, Set<BluetoothDevice>> mGroupIdToAvailableDevices = new HashMap<>();
    ...
    protected boolean stop() {
        ...
        mGroupIdToAvailableDevices.clear();
    ...
    private void handleDeviceAvailable(BluetoothDevice device, int groupId, int rank, UUID uuid,
        ...
        if (!mGroupIdToAvailableDevices.containsKey(groupId)) {
            mGroupIdToAvailableDevices.put(groupId, new HashSet<>());
        }
        mGroupIdToAvailableDevices.get(groupId).add(device);
        ...
        checkIfAllGroupMembersAvailable(groupId);
    }
     ...
     */
    private void checkIfAllGroupMembersAvailable(int groupId) {
        ...
        if (mLeAudioService != null) {
            if (!mGroupIdToAvailableDevices.containsKey(groupId)
                    || !mGroupIdToGroupSize.containsKey(groupId)
                    || mGroupIdToAvailableDevices.get(groupId).size()
                    < mGroupIdToGroupSize.get(groupId)) {
                return;
            }
            for (BluetoothDevice groupDevice : mGroupIdToAvailableDevices.get(groupId)) {
    ...
    void bondStateChanged(BluetoothDevice device, int bondState) {
        ...
        for (Map.Entry<Integer, Set<BluetoothDevice>> entry:
                mGroupIdToAvailableDevices.entrySet()) {
