...
public class PlayerSettingsManager {
    ...
    public void updatePlayerSettings(BluetoothAvrcpPlayerSettings settings,
        ...
        // If the update comes from the active player, replay to the native callbacks.
        ...
        if (playerPackageName.equals(getActivePlayerPackageName())) {
            mCallbackPool.consumeCallbacks(settings);
        }
    }
    ...
    void onListPlayerAttributeRequest(PlayerSettingsNativeCallback nativeCallback) {
        synchronized (mLock) {
            ...
            if (!mPlayerSettings.containsKey(activePlayerPkgName)) {
                ...
                nativeCallback.run(null);
                return;
            }
            getPlayerCallback(activePlayerPkgName).onRequestPlayerSettings();
            mCallbackPool.addCallback(nativeCallback);
        }
    }
    ...
    private NativeCallbackPool mCallbackPool = new NativeCallbackPool();
    ...
    public interface PlayerSettingsNativeCallback {
         ...
         */
        void run(@Nullable BluetoothAvrcpPlayerSettings settings);
    }

    /**
     * Native callback manager.
     *
     * Stores a pool of native callbacks to be called when remote device asks
     * for player settings informations and handles the timeout of callbacks.
     */
    private class NativeCallbackPool {
        // addCallback(PlayerSettingsNativeCallback)
        // - Adds callback to pool.
        // - Starts timer for timeout, probably need id.
        // - on timeout, remove callback from pool and send current params of
        //      active player if available or null.

        // consumeCallbacks(BluetoothAvrcpPlayerSettings)
        // - must be called when update received from active player.
        // - Run through all callbacks in pool and call them with given parameter.
