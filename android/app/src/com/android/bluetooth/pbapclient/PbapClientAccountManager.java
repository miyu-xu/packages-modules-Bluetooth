 ...
 *
 * Most AccountManager functions we want to use require the caller (us) to have a signature match
 ...
 *
 * Once the account list has been intitialized, clients can begin making calls to add, remove and
 * list accounts.
 */
class PbapClientAccountManager {
    ...
    public PbapClientAccountManager(Context context, Callback callback) {
        this(context, null, callback);
    }

    @VisibleForTesting
    public PbapClientAccountManager(Context context, HandlerThread handlerThread,
            Callback callback) {
     ...
     *
     * Initialization happens once the user is unlock and our account type is recognized by the
     ...
     *
     * This account should be used when making storage calls. Be sure the account is added and
     ...
     *
     * Storage must be initialized before calls to this function will be successful
     *
    ...
    public boolean removeAccount(Account account) {
        if (!mAccountsInitialized) {
            Log.w(TAG, "removeAccount(account=" + account
    ...
    private class AccountHandler extends Handler {
        ...
        private static int ACCOUNT_ADD_RETRY_MS = 1000;
     ...
     *
     * AccountManager functionality doesn't work until the user is unlocked. We need to hold our
     ...
     *
     * We're able to interact with our account when our account service is up and the
     ...
     *
     * We use a fake device address and our accoun ttype here to see if our account is visible yet.
     *
     * This function is used in conjunction with the handler and a polling scheme to see determine
     * when we're finally ready.
     *
     * Note: that this function uses the same restrictions as the other add and remove functions,
     ...
     *
     * Any exceptions generated cause this function to fail silently. In particular,
    ...
    public String dump() {
        ...
        sb.append("        Account Type Ready: ")
                .append(isAccountAuthenticationServiceReady()).append("\n");
