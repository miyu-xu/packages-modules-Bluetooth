 ...
 *
 * Most AccountManager functions we want to use requires the caller (us) to have a signature match
...
class PbapClientAccountManager {
    ...
    // TODO: onAccountsReady? may be a better pattern than just using changed.
    interface Callback {
        /**
    ...
    @VisibleForTesting
    public PbapClientAccountManager(Context context, HandlerThread handlerThread, Callback callback) {
    ...
    public List<Account> getAccounts() {
        ...
        synchronized (ACCOUNT_LOCK) {
            return new ArrayList<>(mAccounts);
        }
    }
    ...
    public boolean removeAccount(Account account) {
        if (!mAccountsInitialized) {
            Log.w(TAG, "removeAccount(account=" + account + "): Cannot remove account, not initialized");
            return false;
        }
        synchronized (ACCOUNT_LOCK) {
            List<Account> oldAccounts = new ArrayList<>(mAccounts);
    ...
    public String dump() {
        ...
        sb.append("        Account Type Ready: ").append(isAccountAuthenticationServiceReady()).append("\n");
