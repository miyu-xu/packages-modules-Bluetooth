...
class PbapClientContactsStorage {
    ...
    private void initialize(List<Account> accounts) {
        ...
        for (Account account : accounts) {
            if (getCachedPhonebooks(account).size() != 0) {
                Log.i(TAG, "intialize(): Found metadata for account=" + account + ". Leave it.");
                continue;
            }
    ...
    /**
     * Determine if storage on this device is capable of supporting contacts caching accross device
     * connections.
     *
