...
public class PbapClientService extends ProfileService {
    ...
    class PbapClientStateMachineCallback implements PbapClientStateMachine.Callback {
        ...
        public void onConnectionStateChanged(int oldState, int newState) {
            Log.v(TAG, "Device connection state changed, device=" + mDevice + ", old=" + oldState
                    + ", new=" + newState);
    ...
    @VisibleForTesting
    PbapClientService(Context context, PbapClientContactsStorage storage, Map<BluetoothDevice,
            PbapClientStateMachine> deviceMap) {
        super(context);
        mPbapClientAccountManager = null;
    ...
    private void registerSdpRecord() {
        ...
        mSdpHandle =
                nativeInterface.createPbapPceRecord(
                        SERVICE_NAME, PbapSdpRecord.VERSION_1_2);
    }
    ...
    private boolean addDevice(BluetoothDevice device) {
        ...
        synchronized (mPbapClientStateMachineMap) {
            ...
            if (stateMachine == null) {
                ...
                stateMachine = new PbapClientStateMachine(device, mPbapClientContactsStorage, this,
     ...
     *
     * This function gets the list of available Pbap Client accounts and deletes them. Deletion of
     * the account causes Contacts Provider to also delete the associated contacts data. We
    ...
    public void dump(StringBuilder sb) {
        ...
        if (Flags.pbapClientStorageRefactor()) {
            synchronized (mPbapClientStateMachineMap) {
                ProfileService.println(sb, "Devices (" + mPbapClientStateMachineMap.size()
                        + "/ " + MAXIMUM_DEVICES + ")");
    ...
    public void receiveSdpSearchRecord(
        ...
        if (uuid.equals(BluetoothUuid.PBAP_PSE)) {
            ...
            if (Flags.pbapClientStorageRefactor()) {
                ...
                smOld.obtainMessage(PbapClientStateMachineOld.MSG_SDP_COMPLETE,
                        new PbapSdpRecord(device, pseRecord)).sendToTarget();
            }
        }
    }
