...
class PbapClientStateMachine extends StateMachine {
    ...
    PbapClientStateMachine(BluetoothDevice device, PbapClientContactsStorage storage,
        ...
        mObexClient = new PbapClientObexClient(device, LOCAL_SUPPORTED_FEATURES,
    ...
    @VisibleForTesting
    PbapClientStateMachine(BluetoothDevice device, PbapClientContactsStorage storage,
            Context context, Looper looper, Callback callback, PbapClientObexClient obexClient) {
        super(TAG, looper);
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                case MSG_CONNECT:
                    ...
                    break;
                default:
                    warn("Disconnected: Received unhandled message, what="
    ...
    class Connecting extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                default:
                    warn("Connecting: Received unhandled message, what="
                            + eventToString(message.what));
                    return NOT_HANDLED;
            }
            return HANDLED;
    ...
    class Connected extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                default:
                    warn("Connected: received unhandled message, what="
                            + eventToString(message.what));
                    return NOT_HANDLED;
            }
            return HANDLED;
    ...
    class Downloading extends State {
            ...
            switch (message.what) {
                ...
                case MSG_PHONEBOOK_METADATA_RECEIVED:
                    ...
                    if (currentPhonebook != null && currentPhonebook.equals(phonebook)) {
                        ...
                        if (metadata.getSize() > 0) {
                            ...
                            warn("Downloading: no contacts for phonebook=" + currentPhonebook
                                    + ", skipping");
                            setNextPhonebookOrComplete();
                            break;
                        }
                    } else {
                        warn("Downloading: dropped metadata event for phonebook=" + phonebook
                                + ", current=" + currentPhonebook);
                    }
                    break;
                ...
                case MSG_PHONEBOOK_CONTACTS_RECEIVED:
                    ...
                    if (currentPhonebook != null && currentPhonebook.equals(phonebook)) {
                        ...
                        info("Downloading: received contacts, phonebook=" + phonebook + ", entries="
                                + numReceived + ", total=" + totalContactDownloaded + "/"
                                + totalContactsExpected);
                        if (numReceived != 0) {
                            storeDownloadedContacts(phonebook, contacts);
                        } else {
                            warn("Downloading: contacts empty for phonebook=" + phonebook
                        ...
                        if (totalContactDownloaded >= totalContactsExpected) {
                            ...
                            downloadPhonebook(currentPhonebook, totalContactDownloaded,
        ...
        private void setNextPhonebookOrComplete() {
            ...
            if (mPhonebooksToDownload.size() != 0) {
                ...
                debug("Downloading: Phonebook changed, old=" + currentPhonebook + ", new="
                        + nextPhonebook);
        ...
        private void downloadPhonebookMetadata(String path) {
            ...
            mObexClient.requestPhonebookMetadata(path,
                    new PbapApplicationParameters(
                            DEFAULT_PROPERTIES, DEFAULT_VCARD_VERSION,
                            PbapApplicationParameters.RETURN_SIZE_ONLY, 0));
        }
        ...
        private void downloadPhonebook(String path, int batchStart, int numToFetch) {
            ...
            info("Downloading: Download contents, phonebook=" + path + ", start=" + batchStart
                    + ", end=" + batchEnd);

            PbapApplicationParameters params =
                    new PbapApplicationParameters(DEFAULT_PROPERTIES, DEFAULT_VCARD_VERSION,
                            numToFetch, batchStart);
    ...
    class Disconnecting extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                default:
                    warn("Disconnecting: Received unhandled message, what="
                            + eventToString(message.what));
                    return NOT_HANDLED;
            }
            return HANDLED;
    ...
    class PbapClientObexClientCallback implements PbapClientObexClient.Callback {
        ...
        @Override
        public void onGetPhonebookMetadataComplete(int responseCode, String phonebook,
                PbapPhonebookMetadata metadata) {
            if (responseCode != ResponseCodes.OBEX_HTTP_OK) {
                warn("PullPhonebook for metadata failed, phonebook=" + phonebook + ", code="
                        + responseCode);
        ...
        @Override
        public void onPhonebookContactsDownloaded(int responseCode, String phonebook,
                PbapPhonebook contacts) {
            if (responseCode != ResponseCodes.OBEX_HTTP_OK) {
    ...
    public void dump(StringBuilder sb) {
        ...
        if (mSdpRecord != null) {
            ProfileService.println(sb, "    Server Version: "
        ...
        ProfileService.println(sb, "    Download Progress: "
                + totalContactDownloaded + "/" + totalContacts);
    }
}
