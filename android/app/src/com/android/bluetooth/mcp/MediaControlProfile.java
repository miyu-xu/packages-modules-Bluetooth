...
public class MediaControlProfile implements MediaControlServiceCallbacks {
    ...
    public class ListCallback implements MediaPlayerList.MediaUpdateCallback {
        ...
        public void run(MediaData data) {
            ...
            if (metadata || state || queue) {
                mEventLogger.logd(DBG, TAG, "onMediaUpdated: track_changed=" + metadata
                        + " state_changed=" + state + " queue_changed=" + queue);
            }
        ...
        public void run(boolean availablePlayers, boolean addressedPlayers, boolean uids) {
            mEventLogger.logd(DBG, TAG, "onFolderUpdated: available_players= " + availablePlayers
    ...
    void onCurrentPlayerStateUpdated(boolean stateChanged, boolean metadataChanged) {
        ...
        if (stateChanged) {
            if (mCurrentData.state != null) {
                ...
                mEventLogger.logd(DBG, TAG, "onCurrentPlayerStateUpdated state= " + playback_state
                        + ", supported opcodes= " + Request.SupportedOpcodes.toString(opcodes)
                        + ", track position= " + getDriftCorrectedTrackPosition(mCurrentData.state)
        ...
        if (metadataChanged) {
            if (mCurrentData.metadata != null) {
                mEventLogger.logd(DBG, TAG, "onCurrentPlayerStateUpdated metadata: title= '"
    ...
    public MediaControlProfile(@NonNull McpService mcpService) {
        ...
        mEventLogger = new BluetoothEventLogger(LOG_NB_EVENTS, "MediaControl GMCS event log");
    }
    ...
    public void onMediaControlRequest(Request request) {
        mEventLogger.logd(DBG, TAG, "GMCS onMediaControlRequest: opcode= "
    ...
    public void init() {
        ...
        synchronized (mServiceMap) {
            ...
            mEventLogger.logd(DBG, TAG, "Register MediaControlGattService instance ccid= " + ccid
                    + ", features= "
