...
public class MediaControlGattService implements MediaControlGattServiceInterface {
    ...
    int handleMediaControlPointRequest(BluetoothDevice device, byte[] value) {
        ...
        if (!isOpcodeSupported(opcode)) {
            Log.i(TAG, "handleMediaControlPointRequest: " + Request.Opcodes.toString(opcode) + " not supported");
            mHandler.post(() -> {
        ...
        if (getMediaControlPointRequestPayloadLength(opcode) != (value.length - payloadOffset)) {
            Log.w(TAG, "handleMediaControlPointRequest: " + Request.Opcodes.toString(opcode) + " bad payload length");
        ...
        if (DBG) {
            Log.d(TAG, "handleMediaControlPointRequest: sending " + Request.Opcodes.toString(opcode) + " request up");
        }
    ...
    private boolean isFeatureSupported(long featureBit) {
        if (DBG) {
            Log.w(TAG, "Feature " + ServiceFeature.toString(featureBit) + " support: " + ((mFeatures & featureBit) != 0));
        }
