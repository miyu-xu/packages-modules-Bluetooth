...
public class ContentProfileErrorReportUtils {
    ...
    @VisibleForTesting static final long MIN_PERIOD_BETWEEN_TWO_ERROR_REPORTS = 1_000;
     ...
     * @param type One of the followings: {@link
     ...
     *     each java file.
     */
    static synchronized void report(int profile, int fileNameEnum, int type, int tag) {
        if (!sEnabled) {
            return;
        }
    ...
    private static boolean isTooFrequentReport() {
        return SystemClock.uptimeMillis() - sLastReportTime < MIN_PERIOD_BETWEEN_TWO_ERROR_REPORTS;
    }
}
