...
public final class RfcommSessionizer {
    ...
    private synchronized void logSocketConnectionCompleteWithMetadata(
            boolean success,
            long latencyMs,
            int retriesBeforeCurrent,
            RfcommSocketMetadata socket) {
        ...
        var manufacturerBytes = Arrays.copyOfRange(addressBytes, 0, 3);
        ...
        // security
        var security = socket.mIsSecured ? 1 /* SECURE */ : 0 /* NONE */;

        BluetoothStatsLog.write(
                BluetoothStatsLog.BLUETOOTH_RFCOMM_CONNECTION_ATTEMPT_COMPLETE,
                success ? 0 /* SUCCESS */ : 1 /* UNKNOWN_FAILED */,
                (int) latencyMs,
                retriesBeforeCurrent,
                security,
                socket.mUuid != null ? socket.mUuid.toString() : "",
                socket.mPort > 0 ? socket.mPort : 0,
                socket.mAppUid,
                classOfDevice,
                manufacturerBytes,
                hashedName);
    ...
    public synchronized void logRfcommClientConnectionComplete(int attemptId, boolean success) {
        ...
        logSocketConnectionCompleteWithMetadata(success, latencyMs, retriesBeforeCurrent, socket);
    }
