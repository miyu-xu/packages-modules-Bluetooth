...
final class BluetoothQualityReportNativeInterface {
    ...
    private void bqrDeliver(
        ...
        try {
            int status = adapter.notifyBluetoothQualityReportReady(device, bqr);
            if (status != BluetoothStatusCodes.SUCCESS) {
                Log.e(TAG, "notifyBluetoothQualityReportReady failed, status: " + status);
            }
        } catch (Exception e) {
            Log.e(TAG, "bqrDeliver failed: notifyBluetoothQualityReportReady error", e);
            return;
        }
    }
