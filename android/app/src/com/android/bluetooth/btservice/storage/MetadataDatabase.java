...
public abstract class MetadataDatabase extends RoomDatabase {
    ...
    static final Migration MIGRATION_114_115 = new Migration(114, 115) {
        ...
        public void migrate(SupportSQLiteDatabase database) {
            try {
                database.execSQL(
                        "ALTER TABLE metadata ADD COLUMN `call_pick_up_audio_policy` "
                        + "INTEGER DEFAULT 0");
                database.execSQL(
                        "ALTER TABLE metadata ADD COLUMN `connecting_audio_policy` "
                        + "INTEGER DEFAULT 0");
                database.execSQL(
                        "ALTER TABLE metadata ADD COLUMN `in_band_ring_audio_policy` "
                        + "INTEGER DEFAULT 0");
            } catch (SQLException ex) {
                // Check if user has new schema, but is just missing the version update
                Cursor cursor = database.query("SELECT * FROM metadata");
                if (cursor == null
                        || cursor.getColumnIndex("call_pick_up_audio_policy") == -1) {
                    throw ex;
                }
            }
        }
