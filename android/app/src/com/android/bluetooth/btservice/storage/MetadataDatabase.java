 ...
 */
@Database(entities = {Metadata.class}, version = 118)
public abstract class MetadataDatabase extends RoomDatabase {
    ...
    @VisibleForTesting
    static final Migration MIGRATION_117_118 = new Migration(117, 118) {
        @Override
        public void migrate(SupportSQLiteDatabase database) {
            try {
                database.execSQL("ALTER TABLE metadata ADD COLUMN `isUserDisconnectedDevice` "
                        + "INTEGER NOT NULL DEFAULT 0");
            } catch (SQLException ex) {
                // Check if user has new schema, but is just missing the version update
                Cursor cursor = database.query("SELECT * FROM metadata");
                if (cursor == null || cursor.getColumnIndex("isUserDisconnectedDevice") == -1) {
                    throw ex;
                }
            }
        }
