...
final class BondStateMachine extends StateMachine {
    ...
    @VisibleForTesting
    void sendIntent(BluetoothDevice device, int newState, int reason, boolean isTimeout) {
        ...
        if (isTimeout && oldState == BluetoothDevice.BOND_BONDED
                && newState == BluetoothDevice.BOND_BONDED
        ...
        if (!isTimeout && newState == BluetoothDevice.BOND_BONDED
                && !mPendingBondedDevices.contains(device) && devProp != null
