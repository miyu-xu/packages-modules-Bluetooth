...
final class BondStateMachine extends StateMachine {
            ...
            switch (msg.what) {
                ...
                case SSP_REQUEST:
                    ...
                    Boolean displayPasskey =
                            (msg.getData() != null)
                                    ? msg.getData().getParcelable(DISPLAY_PASSKEY)
                                    : false;
    ...
    private void sendDisplayPinIntent(byte[] address, Optional<Integer> pin, int variant) {
        Intent intent = new Intent(BluetoothDevice.ACTION_PAIRING_REQUEST);
        intent.putExtra(BluetoothDevice.EXTRA_DEVICE, mRemoteDevices.getDevice(address));
        pin.ifPresent(pin -> intent.putExtra(BluetoothDevice.EXTRA_PAIRING_KEY, pin));
    ...
    void sspRequestCallback(byte[] address, byte[] name, int cod, int pairingVariant, int passkey) {
        ...
        if (displayPasskey) {
            ...
            bundle.putParcelable(BondStateMachine.DISPLAY_PASSKEY, true);
            msg.setData(bundle);
        }
        msg.arg2 = variant;
        sendMessage(msg);
