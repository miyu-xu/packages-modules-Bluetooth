...
class ActiveDeviceManager {
    ...
    // profile for
    private static final int PROFILE_PHONE_DEVICE = -100;
    ...
    private final List<BluetoothDevice> mActivatedDevicePriorityStack = new ArrayList<>();
    private final List<Integer> mActivatedProfilePriorityStack = new ArrayList<>();
    ...
    class ActiveDeviceManagerHandler extends Handler {
        ...
        void handleDeviceConnected(int profile, BluetoothDevice device) {
            if (getActivatedDeviceIndex(profile, device) >= 0) {
                // The device is already connected
                return;
            }
            int index = mActivatedProfilePriorityStack.size() - 1;
            if (index < 0
                    || mActivatedProfilePriorityStack.get(index) != BluetoothProfile.HEARING_AID) {
                // New connected device: select it as active
                if (setActiveDevice(profile, device)) {
                    inactivateOtherProfiles(profile);
                    mActivatedDevicePriorityStack.add(device);
                    mActivatedProfilePriorityStack.add(profile);
                    return;
                }
            }

            while (--index >= 0) {
                if (mActivatedProfilePriorityStack.get(index) != BluetoothProfile.HEARING_AID) {
                    mActivatedProfilePriorityStack.add(index + 1, profile);
                    mActivatedDevicePriorityStack.add(index + 1, device);
                    break;
                }
            }
            if (index < 0) {
                mActivatedProfilePriorityStack.add(0, profile);
                mActivatedDevicePriorityStack.add(0, device);
            }
        }
        ...
        void handleActiveDeviceChanged(int profile, BluetoothDevice device) {
            ...
            int index = getActivatedDeviceIndex(profile, device);
            int size = mActivatedProfilePriorityStack.size();
            if (index >= 0 && index == size - 1) {
                return;
            }
            if (index >= 0) {
                mActivatedProfilePriorityStack.remove(index);
                mActivatedDevicePriorityStack.remove(index);
            }
            inactivateOtherProfiles(profile);
            mActivatedProfilePriorityStack.add(profile);
            mActivatedDevicePriorityStack.add(device);
        ...
        void handleDeviceDisconnected(int profile, BluetoothDevice device) {
            ...
            if (index == size) {
                if (size >= 1) {
                    ...
                    for (int i = mActivatedProfilePriorityStack.size() - 1; i >= 0; i--) {
                        if (mActivatedProfilePriorityStack.get(i) == BluetoothProfile.HEARING_AID) {
                            activated = setActiveDevice(mActivatedProfilePriorityStack.get(i),
                                    mActivatedDevicePriorityStack.get(i));
                            break;
                        }
                    }
                    if (!activated) {
                        setActiveDevice(mActivatedProfilePriorityStack.get(size - 1),
                                mActivatedDevicePriorityStack.get(size - 1));
                    }
                } else {
                    setActiveDevice(profile, null);
                }
        ...
        int getActivatedDeviceIndex(int profile, BluetoothDevice device) {
            for (int i = mActivatedDevicePriorityStack.size() - 1; i >= 0; i--) {
                if (profile == mActivatedProfilePriorityStack.get(i)
                        && device.equals(mActivatedDevicePriorityStack.get(i))) {
                    return i;
                }
            }
            return -1;
    ...
    private void resetState() {
        mActivatedProfilePriorityStack.clear();
        mActivatedDevicePriorityStack.clear();
    }
    ...
    int getActiveProfile() {
        return mActivatedProfilePriorityStack.get(mActivatedProfilePriorityStack.size() - 1);
    }

    @VisibleForTesting
    BluetoothDevice getActiveDevice() {
        return mActivatedDevicePriorityStack.get(mActivatedDevicePriorityStack.size() - 1);
    }
