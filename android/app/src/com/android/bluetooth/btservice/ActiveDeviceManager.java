            ...
            if (wasTopPriority) {
                ...
                if (index < 0) {
                    ...
                    for (index = mActivatedDevices.size() - 1; index >= 0; index--) {
                        int profileToActivate = mActivatedDevices.getProfile(index);
                        if (profileToActivate == BluetoothProfile.HEARING_AID) {
                            continue;
                        }
                        if (setActiveDevice(profileToActivate,
                                mActivatedDevices.getDevice(index))) {
                            if (profileToActivate != profile) {
                                inactivateOtherProfiles(profileToActivate);
                            }
                        }
                    }
                }
