...
class BluetoothSocketManagerBinder extends IBluetoothSocketManager.Stub {
    ...
    @Override
    public int getL2capLocalChannelId(long id, AttributionSource source) {
        ...
        return service.getNative().getSocketL2capLocalChannelId(id);
    }

    @Override
    public int getL2capRemoteChannelId(long id, AttributionSource source) {
        AdapterService service = mService;
        if (service == null
                || !Utils.callerIsSystemOrActiveOrManagedUser(
        ...
        Utils.enforceBluetoothPrivilegedPermission(service);
        return service.getNative().getSocketL2capRemoteChannelId(id);
    }
