...
public class AdapterService extends Service {
    ...
    private final Map<Integer, Pair<Bundle, Integer>>
            mCsipGroupsPendingAudioProfileChanges = new HashMap<>();
    private final Map<Integer, BluetoothDevice> mDevicesPendingPreferredAudioProfileChanges =
            new HashMap<>();
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MESSAGE_PREFERRED_AUDIO_PROFILES_AUDIO_FRAMEWORK_TIMEOUT:
                    verboseLog("handleMessage() - "
                            + "MESSAGE_PREFERRED_PROFILE_CHANGE_AUDIO_FRAMEWORK_TIMEOUT");
                    removeFromPendingAudioProfileChanges((int) msg.obj);
                    break;
            }
        }
    ...
    private int setPreferredAudioProfiles(BluetoothDevice device, Bundle modeToProfileBundle) {
        ...
        synchronized (mCsipGroupsPendingAudioProfileChanges) {
            ...
            Bundle currentPreferences = getPreferredAudioProfiles(device);
            ...
            mHandler.post(() -> sendPreferredAudioProfileChangeToAudioFramework(
                    device, strippedPreferences, currentPreferences));
            return BluetoothStatusCodes.SUCCESS;
        }
    }
    ...
    private void sendPreferredAudioProfileChangeToAudioFramework(BluetoothDevice device,
        ...
        if (currentDuplex != newDuplex) {
            if (newDuplex == BluetoothProfile.HEADSET && mHeadsetService.getActiveDevice() != null
                ...
                // TODO(b/275426145): Add similar method for HFP in BluetoothProfileConnectionInfo
                numRequestsToAudioFw +=
                        mA2dpService.sendPreferredAudioProfileChangeToAudioFramework();
        ...
        Log.i(TAG, "sendPreferredAudioProfileChangeToAudioFramework: sent " + numRequestsToAudioFw
                + " request(s) to the Audio Framework for groupId: " + groupId);

        if (numRequestsToAudioFw > 0) {
            synchronized (mCsipGroupsPendingAudioProfileChanges) {
                mCsipGroupsPendingAudioProfileChanges.put(groupId,
                        new Pair<>(strippedPreferences, numRequestsToAudioFw));
                mDevicesPendingPreferredAudioProfileChanges.put(groupId, device);
            }
    ...
    private void removeFromPendingAudioProfileChanges(int groupId) {
        Log.i(TAG, "removeFromPendingAudioProfileChanges: Timeout on change for groupId="
                + groupId);
        synchronized (mCsipGroupsPendingAudioProfileChanges) {
            BluetoothDevice devicePendingChange =
                    mDevicesPendingPreferredAudioProfileChanges.remove(groupId);
            if (devicePendingChange == null) {
                Log.e(TAG,
                        "removeFromPendingAudioProfileChanges( " + groupId + ") no mapped device");
                return;
            }
            ...
            Bundle preferences = mCsipGroupsPendingAudioProfileChanges.remove(groupId).first;
            sendPreferredAudioProfilesCallbackToApps(devicePendingChange, preferences,
    ...
    private int notifyActiveDeviceChangeApplied(BluetoothDevice device) {
        ...
        synchronized (mCsipGroupsPendingAudioProfileChanges) {
            ...
            Bundle preferences = mCsipGroupsPendingAudioProfileChanges.get(groupId).first;
            int numRequestsToAudioFw = mCsipGroupsPendingAudioProfileChanges.get(groupId).second;
            ...
            if (numRequestsToAudioFw <= 1) {
                ...
                BluetoothDevice devicePendingChange =
                        mDevicesPendingPreferredAudioProfileChanges.remove(groupId);
                if (devicePendingChange == null) {
                    Log.i(TAG, "notifyActiveDeviceChangeApplied( " + device
                            + ") no change requested");
                    return BluetoothStatusCodes.ERROR_UNKNOWN;
                }
                sendPreferredAudioProfilesCallbackToApps(devicePendingChange, preferences,
                ...
                Log.i(TAG, "notifyActiveDeviceChangeApplied: Removing timeout from handler");
                mHandler.removeMessages(
                        MESSAGE_PREFERRED_AUDIO_PROFILES_AUDIO_FRAMEWORK_TIMEOUT, groupId);
            } else {
                mCsipGroupsPendingAudioProfileChanges.put(groupId, new Pair<>(preferences,
