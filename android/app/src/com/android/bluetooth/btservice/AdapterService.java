...
public class AdapterService extends Service {
    ...
    void updateAdapterState(int prevState, int newState) {
        ...
        for (int i = 0; i < n; i++) {
            try {
                ...
                Log.d("updateAdapterState() - Callback #" + i + " failed (" + e + ")");
            }
        }
    ...
    void cleanup() {
        ...
        if (mNativeAvailable) {
            Log.d("cleanup() - Cleaning up adapter native");
        ...
        if (mBluetoothKeystoreService != null) {
            Log.d("cleanup(): mBluetoothKeystoreService.cleanup()");
    ...
    public synchronized void enable(boolean quietMode) {
        ...
        if (mUserManager.hasUserRestrictionForUser(
            ...
            Log.d("enable() called when Bluetooth was disallowed");
            return;
        }
        if (Flags.fastBindToApp()) {
    ...
    void disable() {
        Log.d("disable() called with mRunningProfiles.size() = " + mRunningProfiles.size());
    ...
    public boolean isQuietModeEnabled() {
        Log.d("isQuietModeEnabled() - Enabled = " + mQuietmode);
        return mQuietmode;
    }

    public void updateUuids() {
        Log.d("updateUuids() - Updating UUIDs for bonded devices");
    ...
    boolean isMediaProfileConnected() {
        if (mA2dpService != null && mA2dpService.getConnectedDevices().size() > 0) {
            Log.d("isMediaProfileConnected. A2dp is connected");
            return true;
        } else if (mHearingAidService != null
                && mHearingAidService.getConnectedDevices().size() > 0) {
            Log.d("isMediaProfileConnected. HearingAid is connected");
            return true;
        } else if (mLeAudioService != null && mLeAudioService.getConnectedDevices().size() > 0) {
            Log.d("isMediaProfileConnected. LeAudio is connected");
            return true;
        } else {
            Log.d(
    ...
    private void dumpMetrics(FileDescriptor fd) {
        ...
        Log.d("dumpMetrics: native metrics size is " + nativeMetricsBytes.length);
        ...
        Log.d("dumpMetrics: combined metrics size is " + metricsBytes.length);
