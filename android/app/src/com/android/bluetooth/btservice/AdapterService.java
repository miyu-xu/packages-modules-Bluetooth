...
public class AdapterService extends Service {
    ...
    private int startRfcommListener(
        ...
        if (!attributionSource.isTrusted(this)) {
            Slog.e(TAG, "RFCOMM listener start attempted with untrusted attribution source");
            return BluetoothAdapter.RFCOMM_LISTENER_OPERATION_FAILED_DIFFERENT_APP;
        }
        ...
        if (listenerData == null) {
            Slog.d(TAG,
                    String.format(
                            ...
                            uuid.getUuid()));
            return BluetoothAdapter.RFCOMM_LISTENER_OPERATION_FAILED_NO_MATCHING_SERVICE_RECORD;
        }

        if (!attributionSource.isTrusted(this)
                || attributionSource.getUid() != listenerData.mAttributionSource.getUid()) {
            return BluetoothAdapter.RFCOMM_LISTENER_OPERATION_FAILED_DIFFERENT_APP;
        }
    ...
    private IncomingRfcommSocketInfo retrievePendingSocketForServiceRecord(
        ...
        if (listenerData == null) {
            socketInfo.status =
                    BluetoothAdapter.RFCOMM_LISTENER_OPERATION_FAILED_NO_MATCHING_SERVICE_RECORD;
            return socketInfo;
        }

        if (!attributionSource.isTrusted(this)
                || attributionSource.getUid() != listenerData.mAttributionSource.getUid()) {
            socketInfo.status = BluetoothAdapter.RFCOMM_LISTENER_OPERATION_FAILED_DIFFERENT_APP;
            return socketInfo;
        }
        ...
        if (socket == null) {
            socketInfo.status = BluetoothAdapter.RFCOMM_LISTENER_NO_SOCKET_AVAILABLE;
            return socketInfo;
        }
        ...
        socketInfo.status = BluetoothAdapter.RFCOMM_LISTENER_SUCCESS;
    ...
    private static class RfcommListenerData {
        ...
        void closeServerAndPendingSockets(Handler handler) {
            try {
                mServerSocket.close();
            } catch (IOException e) {
