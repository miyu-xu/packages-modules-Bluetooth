...
public class Config {
    ...
    private static final String LE_AUDIO_BROADCAST_SWITCHER_DISABLED_PROPERTY =
            "persist.bluetooth.leaudio_broadcast_switcher.disabled";
    ...
    static void init(Context ctx) {
        ...
        final boolean unicastEnabled =
                (getProfileMask(LeAudioService.class) & (1 << BluetoothProfile.LE_AUDIO)) != 0;

        if (unicastEnabled && broadcastDynamicSwitchSupported) {
            final String broadcastDynamicDisabled =
                    SystemProperties.get(LE_AUDIO_BROADCAST_SWITCHER_DISABLED_PROPERTY, "none");
            if (broadcastDynamicDisabled.equals("true")) {
                setBroadcastProfileStatus(false);
            } else if (broadcastDynamicDisabled.equals("false")) {
