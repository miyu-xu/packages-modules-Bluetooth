...
public class CompanionManager {
    ...
    static final String PROPERTY_LOW_MAX_SUBRATE_FACTOR = "bluetooth.subrate_mode.low_mmax.subrate";
     ...
     *
     * @param address the address of the Bluetooth device
     * @param requestedMode the requested Subrate Mode
     ...
     */
    public int verifyGattSubratingPriority(
            String address,
            @BluetoothGatt.SubrateMode int requestedMode,
            int subrateFactor,
            int latency,
            int contNum) {
        int companionType = isCompanionDevice(address) ? mCompanionType : COMPANION_TYPE_NONE;
        ...
        switch (companionType) {
            ...
            default:
                parameter =
                        verifyGattSubratingPriorityDefault(
                                requestedMode, subrateFactor, latency, contNum);
                break;
        }
        return parameter;
    }
    ...
    /**
     * Verifies the parameters matches the expects parameters from the request Due to bug
     * b/387344794 no verification done on contNum value
     *
     * @param requestedMode the original subrate mode requested
     * @param subrateFactor returned parameter from update
     * @param latency returned parameter from update
     * @param contNum returned parameter from update
     * @return verified subrate mode
     */
    private int verifyGattSubratingPriorityDefault(
            int requestedMode, int subrateFactor, int latency, int contNum) {
        int returnSubrateMode = BluetoothGatt.SUBRATE_MODE_SYSTEM;
        if (requestedMode == BluetoothGatt.SUBRATE_MODE_HIGH
                && mSubrateHighDefault[GATT_SUBRATE_MIN_SUBRATE_FACTOR] <= subrateFactor
        ...
        if (requestedMode == BluetoothGatt.SUBRATE_MODE_BALANCED
                && mSubrateBalanceDefault[GATT_SUBRATE_MIN_SUBRATE_FACTOR] <= subrateFactor
        ...
        if (requestedMode == BluetoothGatt.SUBRATE_MODE_LOW
                && mSubrateLowDefault[GATT_SUBRATE_MIN_SUBRATE_FACTOR] <= subrateFactor
        ...
        Log.d(TAG, "requested Subrate Mode: " + requestedMode);
        Log.d(TAG, "configured Subrate Factor: " + subrateFactor);
        Log.d(TAG, "configured Subrate Latency: " + latency);
        Log.d(TAG, "configured Subrate Con Num: " + contNum);
        Log.d(TAG, "verified Subrate Mode: " + returnSubrateMode);
        return returnSubrateMode;
    }

    /**
     * Gets the GATT connection subrating priority of the device
     *
     * @param address the address of the Bluetooth device
     ...
     * @param priority the priority of the connection, can be BluetoothGatt.SUBRATE_MODE_HIGH,
     *     BluetoothGatt.SUBRATE_MODE_LOW or BluetoothGatt.SUBRATE_MODE_BALANCED
     * @return the connection parameter in integer
     */
    public int getGattSubratingParameters(
            String address, int type, @BluetoothGatt.SubrateMode int priority) {
        int companionType = isCompanionDevice(address) ? mCompanionType : COMPANION_TYPE_NONE;
        int parameter;
        switch (companionType) {
            case COMPANION_TYPE_PRIMARY:
            case COMPANION_TYPE_SECONDARY:
            default:
                parameter = getGattSubratingParameterDefault(type, priority);
                break;
        }
        return parameter;
    }
    ...
    private int getGattSubratingParameterDefault(int type, int mode) {
        switch (mode) {
            case BluetoothGatt.SUBRATE_MODE_HIGH:
                return mSubrateHighDefault[type];
            case BluetoothGatt.SUBRATE_MODE_LOW:
                return mSubrateLowDefault[type];
        }
        return mSubrateBalanceDefault[type];
    }
}
