...
public class MetricsLogger {
    ...
    private static final int MAX_WORDS_ALLOWED_IN_DEVICE_NAME = 6;
    ...
    public boolean initBloomFilter() {
        try {
            File file = new File(BLOOMFILTER_FULL_PATH);
            if (!file.exists()) {
                Log.w(TAG, "MetricsLogger is creating a new Bloomfilter file");
                DeviceBloomfilterGenerator.generateDefaultBloomfilter(BLOOMFILTER_FULL_PATH);
            }

            FileInputStream in = new FileInputStream(new File(BLOOMFILTER_FULL_PATH));
    ...
    protected boolean logSanitizedBluetoothDeviceName(String deviceName) {
        ...
        if (words.length > MAX_WORDS_ALLOWED_IN_DEVICE_NAME) {
            // Sanity checking here to avoid excessively long sequences
            return false;
        }
    ...
    protected void statslogBluetoothDeviceNames(String matchedString, byte[] sha256) {
        Log.d(TAG, "Uploading sha256 hash of matched bluetooth device name: "
