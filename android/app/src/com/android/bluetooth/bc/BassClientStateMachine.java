...
//        mPublicAddrForcSrc = SystemProperties.getBoolean(
//                "persist.vendor.service.bt.pAddrForcSource", true);
        mForceSB = SystemProperties.getBoolean("persist.vendor.service.bt.forceSB", false);
//        mVcpForBroadcast = SystemProperties.getBoolean(
//              "persist.vendor.service.bt.vcpForBroadcast", true);
    }
...
*/
            ...
            mService.updatePAResultsMap(colocatedSrcDevice, colocatedAddressType,
                ...
                //Use advHandle to cache Base
                mService.updateBASE(mBAService.BroadcatGetAdvHandle(), localBase);
                mService.sendBroadcastSourceSelectedCallback(mDevice,
                    localBase.getBroadcastChannels(),
...
/*
   private void processPASyncState(BleBroadcastSourceInfo srcInfo) {
       ...
       if (srcInfo.getMetadataSyncState() ==
                BleBroadcastSourceInfo.BROADCAST_ASSIST_PA_SYNC_STATE_SYNCINFO_REQ) {
           log("Initiate PAST procedure");
            ...
            if (res != null) {
                ...
                if (mNoPast == false && syncHandle != BCService.INVALID_SYNC_HANDLE) {
                    if (isAddedBroadcastSourceIsLocal(srcInfo.getSourceDevice())) {
                        log("Collocated Case Initiate PAST for :" + mDevice + "syncHandle:" +
                                syncHandle + "serviceData" + serviceData);
                        ...
                        serviceData = serviceData &
                                        (~BROADCAST_ADV_ADDRESS_DONT_MATCHES_EXT_ADV_ADDRESS);
                        serviceData = serviceData |
                                        (BROADCAST_ADV_ADDRESS_DONT_MATCHES_SOURCE_ADV_ADDRESS);
                        try {
                            mPeriodicAdvManager.transferSetInfo(mDevice, serviceData,
                                        syncHandle,mPeriodicAdvCallback);
                        } catch (IllegalArgumentException ex) {
...
/*
   private List<BleBroadcastSourceChannel> getListOfBisIndices(int bisIndices,
            int subGroupId, byte[] metaData) {
       List<BleBroadcastSourceChannel> bcastIndices = new ArrayList<BleBroadcastSourceChannel>();
        int index =0;
        log("getListOfBisIndices:" + bisIndices);
        while (bisIndices != 0) {
            if ((bisIndices & 0x00000001) == 0x00000001) {
                BleBroadcastSourceChannel bI =
                    new BleBroadcastSourceChannel(index, Integer.toString(index), true,
                        subGroupId, metaData);
                bcastIndices.add(bI);
                log("Adding BIS index for :" + index);
            }
            bisIndices = bisIndices>>1;
            index++;
        }
        return bcastIndices;
...
//            Map<Integer, List<BleBroadcastSourceChannel>> bisIndexList =
//                                new HashMap<Integer, List<BleBroadcastSourceChannel>>();
...
//                bisIndexList.put(i, getListOfBisIndices(audioBisIndex, i, metaData));
            }
...
/*
    private int convertBisIndicesToIntegerValue(List<BleBroadcastSourceChannel> bisIndices,
                int subGroupId) {
        int audioBisIndex = 0;
        if (bisIndices != null) {
            for (int i=0; i<bisIndices.size(); i++) {
                if (bisIndices.get(i).getStatus() == true && bisIndices.get(i).getSubGroupId() ==
                                subGroupId) {
                    audioBisIndex = audioBisIndex | 1<<(bisIndices.get(i).getIndex()-1);
                    log( "set the bit" + bisIndices.get(i).getIndex() + "after:" + audioBisIndex);
                }
            }
        } else {
            log("bisIndices Channels are null");
...
*/
    ...
    private byte[] convertSourceInfoToAddSourceByteArray(BluetoothLeBroadcastSourceInfo srcInfo) {
        ...
        if (mPublicAddrForcSrc == false) {
            if (isLocalBroadcastSource(broadcastSource)){
                ...
                if (localBcastAddr == null) {
                    Log.w(TAG, "convertSourceInfoToAddSourceByteArray: localBCast not available");
                   ...
                   broadcastSource =
                       ...
                       log("convertSourceInfoToAddSourceByteArray: colocated case: " +
                                broadcastSource);
                }
            }
        }
        ...
        if (paRes == null) {
            ...
            sendPendingCallbacks(ADD_BCAST_SOURCE, srcInfo/*INVALID_SRC_ID*/,
                       BluetoothLeBroadcastAssistantCallback.BASS_STATUS_SOURCE_UNAVAILABLE);
            return null;
        }
        ...
        for (int i=0; i<base_.getNumberOfSubgroupsofBIG(); i++) {
            ...
            //int bisIndexValue = convertBisIndicesToIntegerValue(
            //        srcInfo.getBroadcastChannelsSyncStatus());
/*
            int bisIndexValue = convertBisIndicesToIntegerValue(
                                    mService.getBassUtils().selectBises(
                                    mDevice, srcInfo, base_), i);
...
*/
    ...
    private byte[] convertSourceInfoToUpdateSourceByteArray(BluetoothLeBroadcastSourceInfo srcInfo) {
        byte[] res;
...
*/
    ...
    private byte[] convertSourceInfoToSetBroadcastCodeByteArray(BluetoothLeBroadcastSourceInfo srcInfo) {
...
*/
                ...
                case UPDATE_BCAST_SOURCE:
                    ...
                    if (mBluetoothGatt != null &&
                        ...
                        sendPendingCallbacks(UPDATE_BCAST_SOURCE, srcInfo/*INVALID_SRC_ID*/,
                            BluetoothLeBroadcastAssistantCallback.BASS_STATUS_FAILURE);
                    }
                    break;
                case SET_BCAST_CODE:
                    ...
                    if (cmdType != QUEUED &&
                        ...
                        if (mBluetoothGatt != null &&
                          mBroadcastScanControlPoint != null) {
                            ...
                            sendPendingCallbacks(SET_BCAST_CODE, srcInfo/*INVALID_SRC_ID*/,
                                BluetoothLeBroadcastAssistantCallback.BASS_STATUS_FAILURE);
    ...
    void sendPendingCallbacks(int pendingOp, BluetoothLeBroadcastSourceInfo source/*byte sourceId*/
                    , int status) {
//        if (status != 0) {
...
//        }
        switch (pendingOp) {
            ...
            case ADD_BCAST_SOURCE:
                ...
                mService.sendAddBroadcastSourceCallback(mDevice, source/*sourceId*/, status);
                break;
            case UPDATE_BCAST_SOURCE:
                if (mAutoTriggered == false) {
                    mService.sendUpdateBroadcastSourceCallback(mDevice, source/*sourceId*/, status);
                } else {
                    mAutoTriggered = false;
                }
                break;
            case REMOVE_BCAST_SOURCE:
                mService.sendRemoveBroadcastSourceCallback(mDevice, source/*sourceId*/, status);
                break;
            case SET_BCAST_CODE:
                mService.sendSetBroadcastPINupdatedCallback(mDevice, source/*sourceId*/, status);
                break;
            default:
                    {
