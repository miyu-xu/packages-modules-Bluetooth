...
*/
  ...
  /**
    ...
    public void sendUpdateBroadcastSourceCallback(BluetoothDevice device, byte sourceId,
                    int status){
        ArrayList<IBleBroadcastAudioScanAssistCallback> cbs = mAppCallbackMap.get(device);
        if (cbs == null) {
            Log.e(TAG, "no App callback for this device" + device);
    ...
    public void sendRemoveBroadcastSourceCallback(BluetoothDevice device, byte sourceId,
                    int status){
        ArrayList<IBleBroadcastAudioScanAssistCallback> cbs = mAppCallbackMap.get(device);
        if (cbs == null) {
            Log.e(TAG, "no App callback for this device" + device);
    ...
    public void sendSetBroadcastPINupdatedCallback(BluetoothDevice device, byte sourceId,
                    int status){
        ArrayList<IBleBroadcastAudioScanAssistCallback> cbs = mAppCallbackMap.get(device);
        if (cbs == null) {
            Log.e(TAG, "no App callback for this device" + device);
    ...
    public void registerAppCallback (BluetoothDevice device,
                    IBleBroadcastAudioScanAssistCallback cb) {
...
*/
    ...
    private byte getSrcIdForCSMember(BluetoothDevice masterDevice,
        ...
        byte targetSrcId = -1;
        List<BleBroadcastSourceInfo> masterSrcInfos =
                        getAllBroadcastSourceInformation(masterDevice);
        List<BleBroadcastSourceInfo> memberSrcInfos =
                        getAllBroadcastSourceInformation(memberDevice);
...
//        }
    ...
    public boolean addBroadcastSourceInternal (BluetoothDevice device,
            BleBroadcastSourceInfo srcInfo, boolean isGroupOp) {
        boolean ret = false;
...
//        }
    ...
    public boolean removeBroadcastSourceInternal (BluetoothDevice device,
                        byte sourceId, boolean isGroupOp) {
         boolean ret = false;
...
//         }
    ...
    static class BluetoothSyncHelperBinder extends IBluetoothSyncHelper.Stub
        ...
        @Override
        public void registerAppCallback(BluetoothDevice device,
                IBleBroadcastAudioScanAssistCallback cb, AttributionSource source) {
            BCService service = getService();
            if (service == null|| !Utils.checkConnectPermissionForDataDelivery(
                                   service, source, TAG)) {
                return;
