...
final class BaseData {
            ...
            public byte[] bisIndices; //valid only if level = 2
            ...
            void printConsolidated() {
                    ...
                    if (consolidatedUniqueCodecInfo != null) {
                        for (Map.Entry<Integer,String> entry :
                                consolidatedUniqueCodecInfo.entrySet()) {
                            log("consolidatedUniqueCodecInfo:[" + entry.getKey() + "]:" +
                                    Arrays.toString(entry.getValue().getBytes()));
                        }
                    }

                    if (consolidatedUniqueMetadata != null) {
                        for (Map.Entry<Integer,String> entry :
                                consolidatedUniqueMetadata.entrySet()) {
                            log("consolidatedUniqueMetadata:[" + entry.getKey() + "]:" +
                                    Arrays.toString(entry.getValue().getBytes()));
                        }
                    }
            ...
            void print() {
                ...
                if (level == 2) {
                    ...
                    //log("bisIndices: " + Arrays.toString(bisIndices));
            ...
            BaseData(int numSubGroups, List<BisInfo> colocatedBisInfo,
                            Map<Integer, MetadataLtv> metaInfo) {
                if (metaInfo == null || colocatedBisInfo == null) {

                    Log.e(TAG, "BaseData Construction with Invalid parameters");
                ...
                if (colocatedBisInfo != null) {
                    mNumBISIndices = colocatedBisInfo.size();
...
/*
                List<BleBroadcastSourceChannel>  getBroadcastChannels() {
                    List<BleBroadcastSourceChannel> bChannels =
                                new ArrayList<BleBroadcastSourceChannel>();
                    for (int k=0; k<mNumBISIndices; k++) {
                        int index = levelThree.get(k).index;
                        String desc = levelThree.get(k).description;
                        //String desc = String.valueOf(index);
                        BleBroadcastSourceChannel bc = new BleBroadcastSourceChannel(
                        index, desc, false, levelThree.get(k).subGroupId,
                                levelThree.get(k).metaData);
                        bChannels.add(bc);
                    }
                    return bChannels;
                }
...
*/
                ...
                void updateUniquenessForLevelThree(int parentSubgroup, int startIdx,
                    ...
                    log("END: Level3: Uniqueness among children of " +
                                    parentSubgroup + "th Subgroup");
                    */
             }
