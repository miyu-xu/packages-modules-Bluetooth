...
/*
         public boolean isLocalLEAudioBroadcasting() {
             boolean ret = false;
             //String localLeABroadcast = SystemProperties.get(
                                        "persist.vendor.btstack.isLocalLeAB");
...
*/
        ...
        public boolean searchforBroadcastSources (BluetoothDevice srcDevice,
                                       ArrayList<IBluetoothLeBroadcastAssistantCallback> cbs
                                       ) {
                    ...
                    public void onScanResult(int callbackType, ScanResult result) {
                        ...
                        if (scanRecord != null) {
                            ...
                            if (listOfUuids != null) {
                                ...
                                boolean isBroadcastSource = listOfUuids.containsKey(
                                    ...
                                    if (cbs != null) {
                                        for (IBluetoothLeBroadcastAssistantCallback cb : cbs) {
                                           try {
                                               ...
                                               Log.e(TAG,
                                        "Exception while calling onBluetoothLeBroadcastSourceFound");
                                           }
                                        }
                                    } else {
                                        if (srcDevice.equals(result.getDevice())) {
                                            log("matching src Device found");
                                            Message msg = mAutoAssistScanHandler.obtainMessage(
                                                                                AA_SCAN_SUCCESS);
                                            msg.obj = result;
...
/*
                        ...
                        //Create a scan result for colocated case
...
*/
    ...
    public boolean stopSearchforBroadcastSources(BluetoothDevice srcDev,
                                        ArrayList<IBluetoothLeBroadcastAssistantCallback> cbs) {
...
//                              BluetoothLeBroadcastSourceInfo srcInfo, BaseData base)  {
/*
        boolean noPref = SystemProperties.getBoolean(
                        "persist.vendor.service.bt.bass_no_pref", false);
        if (noPref) {
...
*/
        ...
        //if (mPacsClientService == null) {
            log("selectBises: Pacs Service is null, pick BISes appropriately");
            //Pacs not available
            if (base != null) {
        ...
        if (mPacsClientService != null) {
            ...
            if (broadcastedCodecInfo != null) {
                for (int i=0; i<broadcastedCodecInfo.size(); i++) {
                    HashMap<Integer, String> consolidatedUniqueCodecInfo =
                                    broadcastedCodecInfo.get(i).consolidatedUniqueCodecInfo;
                    byte index = broadcastedCodecInfo.get(i).index;
                    if (consolidatedUniqueCodecInfo != null) {


                        byte[] bisChannelLocation =
                                        consolidatedUniqueCodecInfo.get(LOCATION).getBytes();
                        ...
                        byte[] bisSampleRate =
                            consolidatedUniqueCodecInfo.get(SAMPLE_RATE).getBytes();
                        byte bisSR = bisSampleRate[2];

                        //using bitwise operand as Location can be bitmask
                        if (isSampleRateSupported(device, bisSR) &&
                                        (reversebisLoc & supportedLocations) ==
                                        supportedLocations) {
                             log("matching location: bisLocation " + reversebisLoc + ":: "
                                                    + supportedLocations);
                             BleBroadcastSourceChannel bc =
                                        new BleBroadcastSourceChannel(index,
                                                String.valueOf(index), true);
                             bChannels.add(bc);
                        }
                     }
                }
