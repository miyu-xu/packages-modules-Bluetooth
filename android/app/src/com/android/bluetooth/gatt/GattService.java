    ...
    private int subrateModeRequest(int clientIf, BluetoothDevice device, int subrateMode) {
        ...
        subrateMin =
                manager.getGattSubratingParameters(
                        address, CompanionManager.GATT_SUBRATE_MIN_SUBRATE_FACTOR, subrateMode);
        subrateMax =
                manager.getGattSubratingParameters(
                        address, CompanionManager.GATT_SUBRATE_MAX_SUBRATE_FACTOR, subrateMode);
        maxLatency =
                manager.getGattSubratingParameters(
                        address, CompanionManager.GATT_SUBRATE_LATENCY, subrateMode);
        contNumber =
                manager.getGattSubratingParameters(
                        address, CompanionManager.GATT_SUBRATE_CONT_NUM, subrateMode);
    ...
    private int translateHciCode(int code) {
        ...
        switch (code) {
            case 0:
                ...
                // Hardware Failure
            case 3: // 0x03
                status = BluetoothStatusCodes.ERROR_HARDWARE_GENERIC;
                break;
                // Unsupported Command Local
            case 12: // 0x0C
                status = BluetoothStatusCodes.NOT_ALLOWED;
                break;
                // Unsupported Command Remote
            case 26: // 0x1A
                status = BluetoothStatusCodes.ERROR_REMOTE_OPERATION_NOT_SUPPORTED;
                break;
                // Insufficient Resources
            case 13: // 0x0D
            case 58: // 0x3A
                status = BluetoothStatusCodes.ERROR_LOCAL_NOT_ENOUGH_RESOURCES;
                break;
                // Invalid Parameters
            case 17: // 0x11
            case 18: // 0x12
