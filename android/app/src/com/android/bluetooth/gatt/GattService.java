...
public class GattService extends ProfileService {
    ...
    private static class BluetoothGattBinder extends IBluetoothGatt.Stub
        ...
        public void getAclHandle(int clientIf, String address, int transport,
                AttributionSource attributionSource, SynchronousResultReceiver receiver) {
            try {
                getAclHandle(clientIf, address, transport, attributionSource);
                receiver.send(null);
            } catch (RuntimeException e) {
                receiver.propagateException(e);
            }
        }
        private void getAclHandle(int clientIf, String address, int transport,
                AttributionSource attributionSource) {
            GattService service = getService();
            if (service == null) {
                return;
            }
            service.getAclHandle(clientIf, address, transport, attributionSource);
        }
    ...
    @RequiresPermission(android.Manifest.permission.BLUETOOTH_CONNECT)
    void getAclHandle(int clientIf, String address, int transport,
            AttributionSource attributionSource) {
        if (!Utils.checkConnectPermissionForDataDelivery(
                this, attributionSource, "GattService readRemoteRssi")) {
            return;
