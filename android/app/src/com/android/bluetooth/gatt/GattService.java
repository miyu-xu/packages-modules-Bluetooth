...
public class GattService extends ProfileService {
    ...
    void onGetAclHandle(int clientIf, String address, int transport, int handle)
        ...
        if (DBG) {
            Log.d(TAG,
                    "onGetAclHandle() - clientIf=" + clientIf + " address="
                    + BluetoothUtils.toAnonymizedAddress(address) + ", transport="
    ...
    int getAclHandle(int clientIf, String address, int transport,
            AttributionSource attributionSource) {
        if (!Utils.checkConnectPermissionForDataDelivery(
                this, attributionSource, "GattService getAclHandle")) {
            return BluetoothStatusCodes.ERROR_MISSING_BLUETOOTH_CONNECT_PERMISSION;
        }

        if (DBG) {
            Log.d(TAG, "getAclHandle() - address=" + BluetoothUtils.toAnonymizedAddress(address));
        }
