...
class PeriodicScanManager {
        ...
        for (Map.Entry<IBinder, SyncInfo> e : mSyncs.entrySet()) {
            if (e.getValue().id == syncHandle) {
                syncMap.put(e.getKey(),new SyncInfo(e.getValue().id,
                            e.getValue().adv_sid, e.getValue().address,
                            e.getValue().skip, e.getValue().timeout,
                            e.getValue().deathRecipient, e.getValue().callback));
            }
        }
        return syncMap;
    }
    ...
    private native void syncTransferNative(int pa_source, String address, int service_data,
                                           int sync_handle);

    private native void TransferSetInfoNative(int pa_source, String address, int service_data,
                                              int adv_handle);
}
