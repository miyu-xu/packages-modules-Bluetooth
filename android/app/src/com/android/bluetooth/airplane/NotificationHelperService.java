...
public class NotificationHelperService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        sendAirplaneModeNotification(intent.getStringExtra("NOTIFICATION_STATE"));
    ...
    private void sendAirplaneModeNotification(String notificationState) {
        ...
        if (notificationContent == null) {
            Log.e(TAG, "sendAirplaneModeNotification: unknown action for " + notificationState);
            return;
        }

        if (!isFirstTimeNotification(notificationState)) {
            Log.d(TAG, "sendAirplaneModeNotification: already displayed for " + notificationState);
            return;
        }
        Settings.Secure.putInt(getContentResolver(), notificationState, 1);

        Log.d(TAG, "sendAirplaneModeNotification: sending for " + notificationState);
        ...
        String help_link_url = getString(R.string.apm_learn_more_link);
                        ...
                        .setContentIntent(
                                PendingIntent.getActivity(
                                        ...
                                        new Intent(Intent.ACTION_VIEW)
                                                .setData(Uri.parse(help_link_url))
