...
<<<<<<< PATCH SET (bd030e Implement preferred transport interfaces in HID host)
    /**
...
=======
    private final Handler mHandler =
            new Handler() {
>>>>>>> BASE      (b76b0f Added logical changes for transport selection)
...
>>>>>>> BASE      (b76b0f Added logical changes for transport selection)
    ...
    List<BluetoothDevice> getDevicesMatchingConnectionStates(int[] states) {
        ...
        return mInputDevices.entrySet().stream()
                .filter(entry -> Arrays.asList(states).contains(entry.getValue().getState()))
