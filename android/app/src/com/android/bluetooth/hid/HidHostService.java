...
public class HidHostService extends ProfileService {
    ...
    private boolean connectionAllowed(BluetoothDevice device) {
        ...
        if (adapterService == null) {
            Log.w(TAG, "connectionAllowed: adapter service is null");
            return false;
        }
        // Check if this is an incoming connection in Quiet mode.
        if (adapterService.isQuietModeEnabled() && mTargetDevice == null) {
            Log.w(TAG, "connectionAllowed: return false as quiet mode enabled");
            return false;
        }
        ...
        if (bondState != BluetoothDevice.BOND_BONDED) {
            Log.w(TAG, "connectionAllowed: return false, bondState=" + bondState);
            return false;
        } else if (connectionPolicy != BluetoothProfile.CONNECTION_POLICY_UNKNOWN
                && connectionPolicy != BluetoothProfile.CONNECTION_POLICY_ALLOWED) {
            // Otherwise, reject the connection if connectionPolicy is not valid.
            Log.w(TAG, "connectionAllowed: return false, connectionPolicy=" + connectionPolicy);
            return false;
        }
        return true;
    }
