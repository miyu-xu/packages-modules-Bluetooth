...
public class LeAudioNativeInterface {
    ...
    @VisibleForTesting
    void onHealthBaseRecomendationAction(byte[] address, int action) {
        LeAudioStackEvent event =
        ...
        if (DBG) {
            Log.d(TAG, "onHealthBaseRecomendationAction: " + event);
        }
        sendMessageToService(event);
    }

    @VisibleForTesting
    void onHealthBaseGroupRecomendationAction(int groupId, int action) {
        LeAudioStackEvent event =
                new LeAudioStackEvent(
        ...
        if (DBG) {
            Log.d(TAG, "onHealthBaseGroupRecomendationAction: " + event);
        }
        sendMessageToService(event);
    }
    /**
