...
public class LeAudioService extends ProfileService {
    ...
    public void createBroadcast(BluetoothLeBroadcastSettings broadcastSettings) {
        ...
        if (getActiveGroupId() != LE_AUDIO_GROUP_ID_INVALID) {
            ...
            if (mFeatureFlags.leaudioBroadcastAudioHandoverPolicies()) {
                mLeAudioNativeInterface.setSinkListeningMode(true);
            }
    ...
    public void destroyBroadcast(int broadcastId) {
        ...
        if (mFeatureFlags.leaudioBroadcastAudioHandoverPolicies()) {
            mLeAudioNativeInterface.setSinkListeningMode(false);
        }
    ...
    private boolean updateActiveDevices(
        ...
        if (isActive) {
            ...
            } else if (wasSetSinkListeningMode()) {
                mLeAudioNativeInterface.setSinkListeningMode(false);
            }
        }
    ...
    private void handleGroupChangeRequest(int status) {
        /* Request change of INVALID group and re-activate fallback unicast group or request a
         * de-activation of some active group, what leads to broadcast resume
         */
        if (status == LeAudioStackEvent.STATUS_STREAMING_REQUESTED) {
            ...
            if (broadcastId.isEmpty() || (mBroadcastDescriptors.get(broadcastId.get()) == null)) {
                Log.e(TAG, "handleGroupChangeRequest: Broadcast to Unicast handover not possible");
                return;
            }

            mBroadcastIdDeactivatedForUnicastTransition = Optional.of(broadcastId.get());
            pauseBroadcast(broadcastId.get());
        } else if (status == LeAudioStackEvent.STATUS_STREAM_SUSPENDED) {
            removeActiveDevice(true);
        }
    }
    ...
    void messageFromNative(LeAudioStackEvent stackEvent) {
        ...
        if (stackEvent.type == LeAudioStackEvent.EVENT_TYPE_CONNECTION_STATE_CHANGED) {
            ...
            BassClientService bassClientService = null;
            ...
            switch (descriptor.mState) {
                ...
                case LeAudioStackEvent.BROADCAST_STATE_PAUSED:
                    ...
                    /* Avoid pausing when configured broadcast state comes */
        ...
        } else if (stackEvent.type == LeAudioStackEvent.EVENT_TYPE_SINK_MONITORING_STATUS) {
            int status = stackEvent.valueInt1;
            handleGroupChangeRequest(status);
        }
    }
    ...
    public void setInCall(boolean inCall) {
        ...
        if (mFeatureFlags.leaudioBroadcastAudioHandoverPolicies() && inCall
            ...
            handleGroupChangeRequest(LeAudioStackEvent.STATUS_STREAMING_REQUESTED);
            return;
        }
        ...
        if (mFeatureFlags.leaudioBroadcastAudioHandoverPolicies() && !inCall
                && mBroadcastIdDeactivatedForUnicastTransition.isPresent()) {
            handleGroupChangeRequest(LeAudioStackEvent.STATUS_STREAM_SUSPENDED);
        }
    }
