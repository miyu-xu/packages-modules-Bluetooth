...
public class LeAudioService extends ProfileService {
    ...
    boolean mIsPendingUnicastStreamRequest = false;
        ...
        synchronized (mGroupLock) {
            for (Map.Entry<Integer, LeAudioGroupDescriptor> entry : mGroupDescriptors.entrySet()) {
                ...
                if (descriptor.mActiveState == ACTIVE_STATE_ACTIVE) {
                    descriptor.mActiveState = ACTIVE_STATE_INACTIVE;
                    updateActiveDevices(
    ...
    public void confirmUnicastStreamRequest() {
        if (mIsPendingUnicastStreamRequest) {
            mIsPendingUnicastStreamRequest = false;
            mLeAudioNativeInterface.confirmUnicastStreamRequest();
        } else {
            Log.w(TAG, "There is no pending confirmation request");
        }
    }
    ...
    private boolean areAllGroupsInNotActiveState() {
        synchronized (mGroupLock) {
            for (Map.Entry<Integer, LeAudioGroupDescriptor> entry : mGroupDescriptors.entrySet()) {
                LeAudioGroupDescriptor descriptor = entry.getValue();
                if (descriptor.mActiveState != ACTIVE_STATE_INACTIVE) {
                    return false;
                }
            }
        }
    ...
    private void setActiveGroupWithDevice(BluetoothDevice device, boolean hasFallbackDevice) {
        ...
        synchronized (mGroupLock) {
            ...
            if (descriptor != null) {
                descriptor.mActiveState = ACTIVE_STATE_GETTING_ACTIVE;
            }
        }
    ...
    private void handleGroupTransitToInactive(int groupId) {
        synchronized (mGroupLock) {
            LeAudioGroupDescriptor descriptor = getGroupDescriptor(groupId);
            if (descriptor == null || (descriptor.mActiveState == ACTIVE_STATE_INACTIVE)) {
    ...
    private void handleSourceStreamStatusChange(int status) {
        ...
        /* Native wants streaming confirmation from service */
        if (status == LeAudioStackEvent.STATUS_LOCAL_STREAM_REQUESTED) {
            mIsPendingUnicastStreamRequest = true;
        }
