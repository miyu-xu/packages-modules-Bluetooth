...
public class LeAudioService extends ProfileService {
    ...
    private static final int ACTIVE_STATE_NOT_ACTIVE = 0x00;

    /** Indicates group is going to be activeted */
    private static final int ACTIVE_STATE_PENDING_ACTIVE = 0x01;
    ...
    private class LeAudioGroupDescriptor {
        LeAudioGroupDescriptor(boolean isInbandRingtonEnabled) {
            mIsConnected = false;
            mActiveState = ACTIVE_STATE_NOT_ACTIVE;
    ...
    protected boolean stop() {
        ...
        synchronized (mGroupLock) {
            for (Map.Entry<Integer, LeAudioGroupDescriptor> entry : mGroupDescriptors.entrySet()) {
                ...
                if (descriptor.mActiveState == ACTIVE_STATE_ACTIVE) {
                    descriptor.mActiveState = ACTIVE_STATE_NOT_ACTIVE;
                    updateActiveDevices(
                            group_id, descriptor.mDirection, AUDIO_DIRECTION_NONE, false, false);
                    break;
                }
            }
    ...
    private boolean areAllGroupsInNotActiveState() {
        synchronized (mGroupLock) {
            for (Map.Entry<Integer, LeAudioGroupDescriptor> entry : mGroupDescriptors.entrySet()) {
                LeAudioGroupDescriptor descriptor = entry.getValue();
                if ((descriptor.mActiveState == ACTIVE_STATE_PENDING_ACTIVE)
                        || (descriptor.mActiveState == ACTIVE_STATE_ACTIVE)) {
                    return false;
                }
            }
        }
     ...
     */
    private Integer updateActiveDevices(
            Integer groupId,
        ...
        return mActiveAudioOutDevice != null ? ACTIVE_STATE_ACTIVE : ACTIVE_STATE_NOT_ACTIVE;
    }
    ...
    private void setActiveGroupWithDevice(BluetoothDevice device, boolean hasFallbackDevice) {
        ...
        if (groupId != LE_AUDIO_GROUP_ID_INVALID) {
            ...
            if (descriptor == null) {
                Log.e(TAG, "setActiveGroupWithDevice: No valid descriptor for group: " + groupId);
                return;
            }

            descriptor.mActiveState = ACTIVE_STATE_PENDING_ACTIVE;
        }
    ...
    private void handleGroupTransitToActive(int groupId) {
        synchronized (mGroupLock) {
            ...
            if (descriptor == null || (descriptor.mActiveState == ACTIVE_STATE_ACTIVE)) {
                Log.e(TAG, "handleGroupTransitToActive: no descriptors for group: " + groupId
                        + " or group already active");
                return;
            }

            descriptor.mActiveState =
                    updateActiveDevices(
                            groupId, AUDIO_DIRECTION_NONE, descriptor.mDirection, true, false);
    ...
    private void handleGroupTransitToInactive(int groupId, boolean hasFallbackDevice) {
        synchronized (mGroupLock) {
            LeAudioGroupDescriptor descriptor = getGroupDescriptor(groupId);
            if (descriptor == null || (descriptor.mActiveState == ACTIVE_STATE_NOT_ACTIVE)) {
            ...
            descriptor.mActiveState = ACTIVE_STATE_NOT_ACTIVE;
            updateActiveDevices(
                    groupId, descriptor.mDirection, AUDIO_DIRECTION_NONE, false, hasFallbackDevice);
            /* Clear lost devices */
    ...
    void updateInbandRingtoneForTheGroup(int groupId) {
        ...
        synchronized (mGroupLock) {
            ...
            if (DBG) {
                Log.d(TAG, "groupId active state: " + groupDescriptor.mActiveState
                        + " ringtone supported: " + ringtoneContextAvailable);
            }

            boolean isRingtoneEnabled =
                    ((groupDescriptor.mActiveState == ACTIVE_STATE_ACTIVE)
            ...
            synchronized (mGroupLock) {
                ...
                if (descriptor != null) {
                    if (descriptor.mActiveState == ACTIVE_STATE_ACTIVE) {
                        descriptor.mActiveState =
                                updateActiveDevices(
                                        groupId, descriptor.mDirection, direction, true, false);
                        if (descriptor.mActiveState == ACTIVE_STATE_NOT_ACTIVE) {
    ...
    public void dump(StringBuilder sb) {
        ...
        synchronized (mGroupLock) {
            for (Map.Entry<Integer, LeAudioGroupDescriptor> groupEntry
                ...
                ProfileService.println(sb, "  activeState: " + groupDescriptor.mActiveState);
