...
public class HeadsetClientStateMachine extends StateMachine {
    ...
    boolean processAndroidSlcCommand(String atString, BluetoothDevice device) {
        ...
        int index = atString.lastIndexOf("+ANDROID:");
        if (index < 0) {
            return false;
        }

        String atValues = atString.substring(index);

        // Check if it is +ANDROID: (<feature1>, <feature2>, ...), the reply for AT+ANDROID=?
        int indexUpperQoute = atString.indexOf("(");
        int indexLowerQoute = atString.indexOf(")");
        if (indexUpperQoute >= 0 && indexLowerQoute >= 0 && indexUpperQoute < indexLowerQoute) {
            String featuresStr = atString.substring(indexUpperQoute, indexLowerQoute);
            Log.i(TAG, "processAndroidSlcCommand: " + atString);
            for (String feature : featuresStr.split(",")) {
                if (feature.contains(BluetoothSinkAudioPolicy.HFP_SET_SINK_AUDIO_POLICY_ID)) {
