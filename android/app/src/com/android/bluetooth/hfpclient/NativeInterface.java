...
public class NativeInterface {
    ...
    private void onAudioStateChanged(int state, byte[] address) {
        ...
        event.device = getDevice(address);
        if (DBG) {
            Log.d(TAG, "onAudioStateChanged: address " + Arrays.toString(address)
                    + " event " + event);
        }
        HeadsetClientService service = HeadsetClientService.getHeadsetClientService();
        if (service != null) {
    ...
    private void onVrStateChanged(int state, byte[] address) {
        ...
        event.device = getDevice(address);
        if (DBG) {
            Log.d(TAG, "onVrStateChanged: address " + Arrays.toString(address)
                    + " event " + event);
        }
    ...
    private void onNetworkState(int state, byte[] address) {
        ...
        event.device = getDevice(address);
        if (DBG) {
            Log.d(TAG, "onNetworkStateChanged: address " + Arrays.toString(address)
                    + " event " + event);
        }
    ...
    private void onNetworkSignal(int signal, byte[] address) {
        ...
        event.device = getDevice(address);
        if (DBG) {
            Log.d(TAG, "onNetworkSignal: address " + Arrays.toString(address)
                    + " event " + event);
        }
        HeadsetClientService service = HeadsetClientService.getHeadsetClientService();
        if (service != null) {
    ...
    private void onBatteryLevel(int level, byte[] address) {
        ...
        event.device = getDevice(address);
        if (DBG) {
            Log.d(TAG, "onBatteryLevel: address " + Arrays.toString(address) + " event " + event);
        }
        HeadsetClientService service = HeadsetClientService.getHeadsetClientService();
        if (service != null) {
            service.messageFromNative(event);
    ...
    private void onCurrentOperator(String name, byte[] address) {
        ...
        event.device = getDevice(address);
        if (DBG) {
            Log.d(TAG, "onCurrentOperator: address " + Arrays.toString(address)
                    + " event " + event);
        }
        HeadsetClientService service = HeadsetClientService.getHeadsetClientService();
        if (service != null) {
    ...
    private void onCall(int call, byte[] address) {
        ...
        event.device = getDevice(address);
        if (DBG) {
            Log.d(TAG, "onCall: address " + Arrays.toString(address) + " event " + event);
        }
        HeadsetClientService service = HeadsetClientService.getHeadsetClientService();
        if (service != null) {
            service.messageFromNative(event);
    ...
    private void onCallSetup(int callsetup, byte[] address) {
        ...
        event.device = getDevice(address);
        if (DBG) {
            Log.d(TAG, "onCallSetup: addr " + Arrays.toString(address) + " device" + event.device);
            Log.d(TAG, "onCallSetup: address " + Arrays.toString(address) + " event " + event);
        }
        HeadsetClientService service = HeadsetClientService.getHeadsetClientService();
        if (service != null) {
            service.messageFromNative(event);
    ...
    private void onCallHeld(int callheld, byte[] address) {
        ...
        event.device = getDevice(address);
        if (DBG) {
            Log.d(TAG, "onCallHeld: address " + Arrays.toString(address) + " event " + event);
        }
        HeadsetClientService service = HeadsetClientService.getHeadsetClientService();
        if (service != null) {
            service.messageFromNative(event);
    ...
    private void onRespAndHold(int respAndHold, byte[] address) {
        ...
        event.device = getDevice(address);
        if (DBG) {
            Log.d(TAG, "onRespAndHold: address " + Arrays.toString(address) + " event " + event);
        }
        HeadsetClientService service = HeadsetClientService.getHeadsetClientService();
        if (service != null) {
            service.messageFromNative(event);
    ...
    private void onClip(String number, byte[] address) {
        ...
        event.device = getDevice(address);
        if (DBG) {
            Log.d(TAG, "onClip: address " + Arrays.toString(address) + " event " + event);
        }
        HeadsetClientService service = HeadsetClientService.getHeadsetClientService();
        if (service != null) {
            service.messageFromNative(event);
    ...
    private void onCallWaiting(String number, byte[] address) {
        ...
        event.device = getDevice(address);
        if (DBG) {
            Log.d(TAG, "onCallWaiting: address " + Arrays.toString(address) + " event " + event);
        }
        HeadsetClientService service = HeadsetClientService.getHeadsetClientService();
        if (service != null) {
            service.messageFromNative(event);
    ...
    private void onCurrentCalls(int index, int dir, int state, int mparty, String number,
        ...
        event.device = getDevice(address);
        if (DBG) {
            Log.d(TAG, "onCurrentCalls: address " + Arrays.toString(address) + " event " + event);
        }
        HeadsetClientService service = HeadsetClientService.getHeadsetClientService();
        if (service != null) {
            service.messageFromNative(event);
    ...
    private void onVolumeChange(int type, int volume, byte[] address) {
        ...
        event.device = getDevice(address);
        if (DBG) {
            Log.d(TAG, "onVolumeChange: address " + Arrays.toString(address) + " event " + event);
        }
        HeadsetClientService service = HeadsetClientService.getHeadsetClientService();
        if (service != null) {
            service.messageFromNative(event);
    ...
    private void onCmdResult(int type, int cme, byte[] address) {
        ...
        event.device = getDevice(address);
        if (DBG) {
            Log.d(TAG, "onCmdResult: address " + Arrays.toString(address) + " event " + event);
        }
        HeadsetClientService service = HeadsetClientService.getHeadsetClientService();
        if (service != null) {
            service.messageFromNative(event);
    ...
    private void onSubscriberInfo(String number, int type, byte[] address) {
        ...
        event.device = getDevice(address);
        if (DBG) {
            Log.d(TAG, "onSubscriberInfo: address " + Arrays.toString(address)
                    + " event " + event);
        }
        HeadsetClientService service = HeadsetClientService.getHeadsetClientService();
        if (service != null) {
    ...
    private void onInBandRing(int inBand, byte[] address) {
        ...
        event.device = getDevice(address);
        if (DBG) {
            Log.d(TAG, "onInBandRing: address " + Arrays.toString(address) + " event " + event);
        }
        HeadsetClientService service = HeadsetClientService.getHeadsetClientService();
        if (service != null) {
            service.messageFromNative(event);
    ...
    private void onRingIndication(byte[] address) {
        ...
        event.device = getDevice(address);
        if (DBG) {
            Log.d(TAG, "onRingIndication: address " + Arrays.toString(address)
                    + " event " + event);
        }
        HeadsetClientService service = HeadsetClientService.getHeadsetClientService();
        if (service != null) {
    ...
    private void onUnknownEvent(String eventString, byte[] address) {
        ...
        if (DBG) {
            Log.d(TAG, "onUnknownEvent: address " + Arrays.toString(address) + " event " + event);
        }
        HeadsetClientService service = HeadsetClientService.getHeadsetClientService();
        if (service != null) {
            service.messageFromNative(event);
