...
public class BluetoothInCallService extends InCallService {
        ...
        if (mBluetoothCallHashMap.containsKey(call.getId())) {
            ...
            if (cause != null && cause.getCode() == DisconnectCause.OTHER) {
                ...
                if (Flags.maintainCallIndexAfterConference()) {
                    ...
                    if (cause.getReason().equals("IMS_MERGED_SUCCESSFULLY")) {
                        if (!mConferenceCallClccIndexMap.containsKey(getClccMapKey(call))) {
                            mConferenceCallClccIndexMap.put(getClccMapKey(call), call.mClccIndex);
                        }
                    }
                }
    ...
    private int getIndexForCall(BluetoothCall call) {
        ...
        if (Flags.maintainCallIndexAfterConference()) {
            int index_check = -1;
            index_check = mConferenceCallClccIndexMap.get(getClccMapKey(call));
            if (call.mClccIndex == index_check) {
                /**
                 * Remove the index from conference hashmap, this can be later added if call merges
                 * in conference
                 */
                mConferenceCallClccIndexMap.remove(getClccMapKey(call));
            }
        }
