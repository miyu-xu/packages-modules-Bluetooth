...
public class TransitionalScanHelper {
    ...
    public void onScannerRegistered(int status, int scannerId, long uuidLsb, long uuidMsb)
        ...
        ContextMap.App cbApp = mScannerMap.getByUuid(uuid);
        if (cbApp != null) {
            if (status == 0) {
                cbApp.id = scannerId;
    ...
    private void sendBatchScanResults(
            ContextMap.App app, ScanClient client, ArrayList<ScanResult> results) {
        try {
            if (app.callback != null) {
    ...
    public void onTrackAdvFoundLost(AdvtFilterOnFoundOnLostInfo trackingInfo)
        ...
        ContextMap.App app = mScannerMap.getById(trackingInfo.getClientIf());
    ...
    public void continuePiStartScan(int scannerId, ContextMap.App app) {
    ...
    /**
     * DeathReceipient handler used to unregister applications that disconnect ungracefully (ie.
