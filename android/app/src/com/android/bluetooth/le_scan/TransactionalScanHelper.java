 ...
 * @hide
 */
public class TransactionalScanHelper {

    private static final boolean DBG = GattServiceConfig.DBG;
    private static final String TAG = GattServiceConfig.TAG_PREFIX + "TransactionalScanHelper";
    ...
    public void start(GattService gattService,
        AdapterService mAdapterService,
        BluetoothAdapterProxy mBluetoothAdapterProxy,
    ...
    public void registerScanner(GattService gattService,
        IScannerCallback callback,
        WorkSource workSource,
        AttributionSource attributionSource) throws RemoteException {
        if (!Utils.checkScanPermissionForDataDelivery(
            gattService, attributionSource, "GattService registerScanner")) {
            return;
        }
        ...
        gattService.enforceImpersonatationPermissionIfNeeded(workSource);

        AppScanStats app = mScannerMap.getAppScanStatsByUid(Binder.getCallingUid());
        if (app != null && app.isScanningTooFrequently()
            && !Utils.checkCallerHasPrivilegedPermission(gattService)) {
        ...
        mScannerMap.add(uuid, workSource, callback, null, gattService);
        mScanManager.registerScanner(uuid);
    }

    @RequiresPermission(android.Manifest.permission.BLUETOOTH_SCAN)
    public void unregisterScanner(GattService gattService,
        int scannerId,
        AttributionSource attributionSource) {
        if (!Utils.checkScanPermissionForDataDelivery(
            gattService, attributionSource, "GattService unregisterScanner")) {
            return;
        }
    ...
    @RequiresPermission(android.Manifest.permission.BLUETOOTH_SCAN)
    public void startScan(GattService gattService,
        int scannerId,
        ...
        if (!Utils.checkScanPermissionForDataDelivery(
            gattService, attributionSource, "Starting GATT scan.")) {
            return;
        }

        gattService.enforcePrivilegedPermissionIfNeeded(settings);
        String callingPackage = attributionSource.getPackageName();
        settings = gattService.enforceReportDelayFloor(settings);
        gattService.enforcePrivilegedPermissionIfNeeded(filters);
    ...
    @RequiresPermission(android.Manifest.permission.BLUETOOTH_SCAN)
    public void registerPiAndStartScan(GattService service,
        ...
        if (!Utils.checkScanPermissionForDataDelivery(
            service, attributionSource, "Starting GATT scan.")) {
            return;
        }
    ...
    @RequiresPermission(android.Manifest.permission.BLUETOOTH_SCAN)
    public void flushPendingBatchResults(GattService service,
        int scannerId,
        AttributionSource attributionSource) {
        if (!Utils.checkScanPermissionForDataDelivery(
            service, attributionSource, "GattService flushPendingBatchResults")) {
            return;
        }
        if (DBG) {
    ...
    @RequiresPermission(android.Manifest.permission.BLUETOOTH_SCAN)
    public void stopScan(GattService service,
        PendingIntent intent,
        AttributionSource attributionSource) {
        if (!Utils.checkScanPermissionForDataDelivery(
            service, attributionSource, "GattService stopScan")) {
            return;
        }
     ...
     *************************************************************************/
    @RequiresPermission(android.Manifest.permission.BLUETOOTH_SCAN)
    public void registerSync(GattService service,
        ...
        AttributionSource attributionSource) {
        if (!Utils.checkScanPermissionForDataDelivery(
            service, attributionSource, "GattService registerSync")) {
            return;
        }
    ...
    @RequiresPermission(android.Manifest.permission.BLUETOOTH_SCAN)
    public void unregisterSync(GattService service,
        IPeriodicAdvertisingCallback callback,
        AttributionSource attributionSource) {
        if (!Utils.checkScanPermissionForDataDelivery(
            service, attributionSource, "GattService unregisterSync")) {
            return;
        }
        mPeriodicScanManager.stopSync(callback);
    }

    public void transferSync(GattService service,
        BluetoothDevice bda,
        int serviceData,
        int syncHandle,
        AttributionSource attributionSource) {
        if (!Utils.checkScanPermissionForDataDelivery(
            service, attributionSource, "GattService transferSync")) {
            return;
        }
        mPeriodicScanManager.transferSync(bda, serviceData, syncHandle);
    }

    public void transferSetInfo(GattService service,
        BluetoothDevice bda,
        int serviceData,
        int advHandle,
        IPeriodicAdvertisingCallback callback,
        AttributionSource attributionSource) {
        if (!Utils.checkScanPermissionForDataDelivery(
            service, attributionSource, "GattService transferSetInfo")) {
            return;
        }
    ...
    void onScanResultInternal(int eventType, int addressType, String address, int primaryPhy,
        ...
        for (ScanClient client : mScanManager.getRegularScanQueue()) {
            ...
            GattService.MatchResult matchResult = matchesFilters(client, result, originalAddress);
    ...
    private GattService.MatchResult matchesFilters(ScanClient client, ScanResult scanResult) {
    ...
    private GattService.MatchResult matchesFilters(ScanClient client, ScanResult scanResult,
        ...
        if (client.filters == null || client.filters.isEmpty()) {
            ...
            return new GattService.MatchResult(true, GattService.MatchOrigin.PSEUDO_ADDRESS);
        }
        for (ScanFilter filter : client.filters) {
            ...
            if (originalAddress != null
                ...
                return new GattService.MatchResult(true, GattService.MatchOrigin.ORIGINAL_ADDRESS);
            }
        }
        return new GattService.MatchResult(false, GattService.MatchOrigin.PSEUDO_ADDRESS);
    }
