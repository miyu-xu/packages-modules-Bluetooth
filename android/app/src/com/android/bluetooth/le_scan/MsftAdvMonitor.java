...
/* package */ class MsftAdvMonitor {
    ...
    static class Address {
        byte addr_type = 0;
        String bd_addr = "00:00:00:00:00:00";
    }
    ...
    public MsftAdvMonitor(ScanFilter filter) {
        ...
        // TODO magic numbers
        mMonitor.rssi_threshold_high = (byte) 0xBF; // (byte) 191
        mMonitor.rssi_threshold_low = (byte) 0xB0; // (byte) 176
        mMonitor.rssi_threshold_low_time_interval = 0x28; // 40
        mMonitor.rssi_sampling_period = 0x05; // hard coded to 500 ms
        mMonitor.condition_type = MSFT_CONDITION_TYPE_PATTERNS;

        if (filter.getServiceDataUuid() != null) {
            Pattern pattern = new Pattern();
            pattern.ad_type = (byte) 0x16; // Bluetooth Core Spec Part A, Section 1// (byte) 22
            pattern.start_byte = FILTER_PATTERN_START_POSITION;

            // pattern.pattern = new byte[] { (byte) 0x2c, (byte) 0xfe };
