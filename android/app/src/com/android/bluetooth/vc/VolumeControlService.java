...
public class VolumeControlService extends ProfileService {
    ...
    void onExtAudioInStateChanged(
        ...
        Log.d(TAG, logInfo);
        input.onStateChange(id, gainSetting, mute, gainMode);
    }
    ...
    static class BluetoothVolumeControlBinder extends IBluetoothVolumeControl.Stub
        ...
        private <R> R aics_getter(AudioInputControl.Descriptor descriptor, AttributionSource source, Function<VolumeControlInputDescriptor, R> fn, R defaultValue) {
            validateAudioInputControlDescriptor(descriptor);
        ...
        @Override
        public int getAudioInputType(AudioInputControl.Descriptor descriptor, AttributionSource source) {
            return aics_getter(descriptor, source, i->i.getType(descriptor.mInstanceId), AudioInputType.UNSPECIFIED);
        }

        @Override
        public int getAudioInputStatus(AudioInputControl.Descriptor descriptor, AttributionSource source) {
            return aics_getter(descriptor, source, i->i.getStatus(descriptor.mInstanceId), (int)AudioInputStatus.INACTIVE);
        }

        @Override
        public int getAudioInputGainSettingUnit(AudioInputControl.Descriptor descriptor, AttributionSource source) {
            return aics_getter(descriptor, source, i->i.getGainSettingUnit(descriptor.mInstanceId), 0);
        }

        @Override
        public int getAudioInputGainSettingMin(AudioInputControl.Descriptor descriptor, AttributionSource source) {
            return aics_getter(descriptor, source, i->i.getGainSettingMax(descriptor.mInstanceId), 0);
        }

        @Override
        public int getAudioInputGainSettingMax(AudioInputControl.Descriptor descriptor, AttributionSource source) {
            return aics_getter(descriptor, source, i->i.getGainSettingMin(descriptor.mInstanceId), 0);
        }
        @Override
        public String getAudioInputDescription(AudioInputControl.Descriptor descriptor, AttributionSource source) {
            return aics_getter(descriptor, source, i->i.getDescription(descriptor.mInstanceId), "");
        }
        @Override
        public boolean setAudioInputDescription(AudioInputControl.Descriptor descriptor, AttributionSource source, String description) {
            requireNonNull(description);
            //TODO implement
            return aics_getter(descriptor, source, i->{i.setDescription(descriptor.mInstanceId, description);return false;}, false);
        }
    }
