        ...
        if (isAutonomous && device != null) {
            Log.i(
                    ...
                    ("Initial volume set after connect, volume: " + volume)
                            + (", mute: " + mute)
                            + (", flags: " + flags));
            /* We are here, because system has just started and LeAudio device is connected.
             * If remote device has User Persistent flag or the volume is != 0, we set the volume to our cache and
             * audio system. If Reset Flag is set then we set either cached volume or get volume from audio manager.
