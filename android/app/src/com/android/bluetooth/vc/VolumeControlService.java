...
public class VolumeControlService extends ProfileService {
    ...
    int getNumberOfVolumeOffsetInstatnces(BluetoothDevice device) {
        VolumeControlOffsetDescriptor offsets = mAudioOffsets.get(device);
        if (offsets == null) {
            Log.i(TAG, " There is no offset service for device: " + device);
            return 0;
    ...
    void setVolumeOffset(BluetoothDevice device, int instatnceId, int volumeOffset) {
        VolumeControlOffsetDescriptor offsets = mAudioOffsets.get(device);
        if (offsets == null) {
        ...
        int numberOfInstances = offsets.size();
        if (instatnceId > numberOfInstances) {
            Log.e(
                    TAG,
        ...
        int value = offsets.getValue(instatnceId);
        if (value == volumeOffset) {
            /* Nothing to do - offset already applied */
            return;
        }

        mVolumeControlNativeInterface.setExtAudioOutVolumeOffset(device, instatnceId, volumeOffset);
    }
    ...
    static class BluetoothVolumeControlBinder extends IBluetoothVolumeControl.Stub
        ...
        @Override
        public void getNumberOfVolumeOffsetInstatnces(
                BluetoothDevice device,
                AttributionSource source,
                SynchronousResultReceiver receiver) {
            try {
                ...
                if (service != null) {
                    enforceBluetoothPrivilegedPermission(service);
                    defaultValue = service.getNumberOfVolumeOffsetInstatnces(device);
                }
                receiver.send(defaultValue);
            } catch (RuntimeException e) {
                receiver.propagateException(e);
        ...
        public void setVolumeOffset(
            ...
            try {
                ...
                if (service != null) {
                    enforceBluetoothPrivilegedPermission(service);
                    service.setVolumeOffset(device, instatnceId, volumeOffset);
                }
                receiver.send(null);
            } catch (RuntimeException e) {
                receiver.propagateException(e);
