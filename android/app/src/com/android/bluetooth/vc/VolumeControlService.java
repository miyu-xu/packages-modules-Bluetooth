...
public class VolumeControlService extends ProfileService {
    ...
    private static int sMusicMaxVolume = 0;
    private static int sMusicMinVolume = 0;
    private static int sVoiceCallMaxVolume = 0;
    private static int sVoiceCallMinVolume = 0;
    ...
    protected boolean start() {
        ...
        sMusicMaxVolume = mAudioManager.getStreamMaxVolume(AudioManager.STREAM_MUSIC);
        sMusicMinVolume = mAudioManager.getStreamMinVolume(AudioManager.STREAM_MUSIC);
        sVoiceCallMaxVolume = mAudioManager.getStreamMaxVolume(AudioManager.STREAM_VOICE_CALL);
        sVoiceCallMinVolume = mAudioManager.getStreamMinVolume(AudioManager.STREAM_VOICE_CALL);
    ...
    int getDeviceVolume(int streamType, int bleVolume) {
        ...
        int deviceMaxVolume = (streamType == AudioManager.STREAM_VOICE_CALL)
                ? sVoiceCallMaxVolume : sMusicMaxVolume;
        int deviceMinVolume = (streamType == AudioManager.STREAM_VOICE_CALL)
                ? sVoiceCallMinVolume : sMusicMinVolume;
    ...
    int getBluetoothContextualVolumeStream() {
        ...
        // TODO: AudioService has one more check here for voice playback active.
        // Check whether it is possible to bring it here.
        ...
        if (stackEvent.type == VolumeControlStackEvent.EVENT_TYPE_VOLUME_STATE_CHANGED) {
            handleVolumeControlChanged(stackEvent.device, stackEvent.valueInt1,
                                       stackEvent.valueInt2, stackEvent.valueBool1,
                                        // TODO: Get the actual isAutonomous value from native.
                                        true);
          return;
        }
