...
public class VolumeControlService extends ProfileService {
    ...
    public void setGroupVolume(int groupId, int volume) {
        ...
        /* Volume level 0 is muted / and everything above is unmuted */
        Boolean isGroupMuted = mGroupMuteCache.getOrDefault(groupId, false);
        if (volume > 0) {
            if (isGroupMuted) {
                Log.i(TAG, "Setting mute:" + false + " for group " + groupId);
                mGroupMuteCache.put(groupId, false);
                mVolumeControlNativeInterface.unmuteGroup(groupId);
            }
        } else {
            /* Actually it would be ok to just set the volume to 0, but we can mute anyway */
            if (!isGroupMuted) {
                Log.i(TAG, "Setting mute:" + true + " for group " + groupId);
