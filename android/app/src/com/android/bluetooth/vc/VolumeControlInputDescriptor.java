...
class VolumeControlInputDescriptor {
    ...
    private static class Descriptor {
        ...
        private final RemoteCallbackList<IAudioInputCallback> mCallbacks = new RemoteCallbackList<>();
    ...
    void setState(int id, int gainValue, int gainMode, int mute) {
        ...
        if (desc.mGainValue != gainValue) {
            desc.mGainValue = gainValue;
            mVolumeInputs[id].broadcast("onGainSetting", (c) -> c.onGainSettingChanged(gainValue));
        }
        if (desc.mGainMode != gainMode) {
            desc.mGainMode = gainMode;
            mVolumeInputs[id].broadcast("onGainMode", (c) -> c.onGainModeChanged(gainMode));
        }
        if (desc.mMute != mute) {
            desc.mMute = mute;
            mVolumeInputs[id].broadcast("onMuteChanged", (c) -> c.onMuteChanged(mute));
        }
    }
