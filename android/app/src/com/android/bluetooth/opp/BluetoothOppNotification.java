...
class BluetoothOppNotification {
    ...
    void updateIncomingFileConfirmNotification() {
        ...
        for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
            ...
            Notification publicVersionNotification = builder.build();

            Notification notification =
                    builder.setVisibility(Notification.VISIBILITY_PRIVATE)
                            .addAction(actionDecline)
                            .addAction(actionAccept)
                            .setPublicVersion(publicVersionNotification)
                            .build();
            mNotificationMgr.notify(NOTIFICATION_ID_PROGRESS, notification);
        }
        cursor.close();
    }
