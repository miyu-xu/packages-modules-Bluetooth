...
class ClassicConnect(base_test.BaseTestClass):
    ...
    def _connect_to_dut(self, io_cap):
        ...
        self.bumble.device.pairing_config_factory = lambda _: PairingConfig(delegate=Delegate(
            io_cap, self.dut, bumble_address))
        connect_resp = self.bumble.host.Connect(address=self.dut.address, wait_for_ready=True)
        asserts.assert_true(connect_resp.WhichOneof("result") == "connection", "Failed to connect")
