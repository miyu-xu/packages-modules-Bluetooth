...
if [[ "${ATEST_OUTPUT_DIR}" == "" ]];
then
  # Sample pip3 path, on Tradefed:
  # /tmp/tf-workfolder6068947767105837454/tf-container16946017916817032528/tf-workfolder1859486415479344244/stub-virtualenv/bin/pip3
  export PIP3_PATH=$(which pip3)
  if [[ "${PIP3_PATH}" != "" ]];
  then
    # Get to to root logging directory. Logging path based on above example:
    # /tmp/tf-workfolder6068947767105837454/tf-container16946017916817032528/tf-workfolder1859486415479344244
    export ATEST_OUTPUT_DIR="$(realpath "$(dirname "${PIP3_PATH}")/../..")"
  else
    # Fall back on atest-output dir matching pattern /tmp/tf-workfolder*.
    export ATEST_OUTPUT_DIR=$(ls -td /tmp/tf-workfolder* | head -n 1)
  fi
fi
...
# Make an acloud-pull directory in the atest's ouput directory.
