...
class Security(private val context: Context) : SecurityImplBase() {
  ...
  override fun pair(request: PairRequest, responseObserver: StreamObserver<Empty>) {
    grpcUnary(globalScope, responseObserver) {
      ...
      bluetoothDevice.createBond(when (request.transport) {
  ...
  override fun isPaired(
    request: IsPairedRequest,
    responseObserver: StreamObserver<IsPairedResponse>
  ) {
    grpcUnary(globalScope, responseObserver) {
      val device = request.address.toBluetoothDevice(bluetoothAdapter)
      val isBonded = device.bondState == BOND_BONDED
      val onTransport =
        when (request.transport!!) {
          Transport.TRANSPORT_UNSPECIFIED,
          Transport.UNRECOGNIZED -> true
          Transport.TRANSPORT_BREDR ->
            device.type == DEVICE_TYPE_CLASSIC || device.type == DEVICE_TYPE_DUAL
          Transport.TRANSPORT_LE -> device.type == DEVICE_TYPE_LE || device.type == DEVICE_TYPE_DUAL
        }
      Log.i(TAG, "IsPaired isBonded=${isBonded} onTransport=${onTransport} type=${device.type}")
      IsPairedResponse.newBuilder().setIsPaired(isBonded && onTransport).build()
    }
  }
}
