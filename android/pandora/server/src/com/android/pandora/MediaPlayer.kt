...
import pandora.MediaPlayerProto.SetShuffleModeResponse
import pandora.MediaPlayerProto.GetShuffleModeRequest
...
class MediaPlayer(val context: Context) : MediaPlayerImplBase(), Closeable {
  ...
  override fun setShuffleMode(
    ...
    responseObserver: StreamObserver<SetShuffleModeResponse>
  ) {
    grpcUnary(scope, responseObserver) {
      when (request.mode!!) {
        ShuffleMode.NONE ->
      ...
      SetShuffleModeResponse.getDefaultInstance()
    }
  }
