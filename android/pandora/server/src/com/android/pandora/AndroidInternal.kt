...
class AndroidInternal(val context: Context) : AndroidImplBase(), Closeable {
    ...
    override fun sendPing(
        ...
        responseObserver: StreamObserver<SendPingResponse>
    ) {
        grpcUnary<SendPingResponse>(scope, responseObserver) {
            val pingStatus =
                Runtime.getRuntime().exec("ping -I bt-pan -c 1 ${request.ipAddress}").waitFor()
            SendPingResponse.newBuilder().setPingStatus(pingStatus).build()
        }
    }
    ...
    suspend private fun waitAndSelectBluetoothDevice() {
        var selectJob =
            scope.async {
                device
                    .wait(
                        Until.findObject(By.textContains("Cuttlefish")),
                        BT_DEVICE_SELECT_WAIT_TIMEOUT
                    )
                    .click()
            }
        selectJob.await()
