...
class MediaPlayerBrowserService : MediaBrowserServiceCompat() {
  ...
  fun setShuffleMode() {
    ...
    Log.i(TAG, "setShuffleMode")
    val mediaControllerCompat: MediaControllerCompat = MediaControllerCompat(this, mediaSession.getSessionToken())
    ...
    for (item in QUEUE_START_INDEX..QUEUE_SIZE) {
      val metaData: MediaMetadataCompat =
        MediaMetadataCompat.Builder()
          ...
          .putString(MediaMetadataCompat.METADATA_KEY_ARTIST, if (item != QUEUE_SIZE) "Artist$item" else generateAlphanumericString(512))
          .putString(MediaMetadataCompat.METADATA_KEY_ALBUM, if (item != QUEUE_SIZE) "Album$item" else generateAlphanumericString(512))
