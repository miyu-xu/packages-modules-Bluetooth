...
public sealed class LeBroadcastCallback {
  public data class BroadcastStarted(val reason: Int, val broadcastId: Int) : LeBroadcastCallback()
  public data class BroadcastStartFailed(val reason: Int) : LeBroadcastCallback()
  public data class BroadcastStopped(val reason: Int, val broadcastId: Int) : LeBroadcastCallback()
  public data class BroadcastStopFailed(val reason: Int) : LeBroadcastCallback()
  public data class PlaybackStarted(val reason: Int, val broadcastId: Int) : LeBroadcastCallback()
  public data class PlaybackStopped(val reason: Int, val broadcastId: Int) : LeBroadcastCallback()
  public data class BroadcastUpdated(val reason: Int, val broadcastId: Int) : LeBroadcastCallback()
  public data class BroadcastUpdateFailed(val reason: Int, val broadcastId: Int) : LeBroadcastCallback()
  public data class BroadcastMetadataChanged(val broadcastId: Int, val metadata: BluetoothLeBroadcastMetadata) : LeBroadcastCallback()
}

/** Creates a coroutine flow from BluetoothLeBroadcast callbacks. */
public fun flowFromBluetoothLeBroadcastCallbacks(scope: CoroutineScope, bluetoothLeBroadcast: BluetoothLeBroadcast) :
    Flow<LeBroadcastCallback> = callbackFlow {
  val callback = object : BluetoothLeBroadcast.Callback {
    ...
    override fun onBroadcastUpdateFailed(reason: Int, broadcastId: Int) {
      scope.launch { send(LeBroadcastCallback.BroadcastUpdateFailed(reason, broadcastId)) }
    }

    override fun onBroadcastMetadataChanged(broadcastId: Int, metadata: BluetoothLeBroadcastMetadata) {
      scope.launch { send(LeBroadcastCallback.BroadcastMetadataChanged(broadcastId, metadata)) }
    }
  }
...
sealed class LeBroadcastCallbackResult<out T: LeBroadcastCallback, out E: LeBroadcastCallback> {
  ...
  data class Success<T : LeBroadcastCallback>(val value: T) : LeBroadcastCallbackResult<T, Nothing>()
  data class Failure<E : LeBroadcastCallback>(val value: E) : LeBroadcastCallbackResult<Nothing, E>()

  val isSuccess inline get() = this is Success<*>
  val isFailure inline get() = this is Failure<*>

  inline fun <U> valueOr(block: (E) -> U) = when (this) {
    is Success<T> -> value
    is Failure<E> -> block(value) as T
  }

  companion object {
    inline fun <reified T: LeBroadcastCallback, reified E: LeBroadcastCallback>
      wrap(result: LeBroadcastCallback) = when {
