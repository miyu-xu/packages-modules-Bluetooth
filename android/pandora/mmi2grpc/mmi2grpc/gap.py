...
class GAPProxy(ProfileProxy):
    ...
    def TSC_MMI_enter_security_id(self, pts_addr: bytes, **kwargs):
        ...
        if self.cached_passkey is not None:
            self.host.Log(text=f"Returning cached passkey entry {self.cached_passkey}")
            return str(self.cached_passkey)

        for event in self.pairing_events:
            if event.address == pts_addr and event.passkey_entry_notification:
                self.host.Log(text=f"Got passkey entry {event.passkey_entry_notification}")
    ...
    def _mmi_231(self, test: str, pts_addr: bytes, **kwargs):
        ...
        def after_that():
            # the 10s timeout is pretty stable (slower than bonding, faster than PTS timeout)
            sleep(10)

            # we want to force a disconnect *after* bonding finishes, so we toggle
            # Bluetooth (as disconnects only take effect after a timeout
            self.host.SoftReset()
