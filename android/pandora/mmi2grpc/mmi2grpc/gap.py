...
class GAPProxy(ProfileProxy):
    ...
    def TSC_MMI_iut_send_le_connect_request(self, test: str, pts_addr: bytes, **kwargs):
        ...
        if test in {"GAP/CONN/GCEP/BV-02-C", "GAP/DM/LEP/BV-06-C", "GAP/CONN/GCEP/BV-01-C"}:
            ...
            scans = self.host.Inquiry()
            for scan in scans:
                device_name = self.host.GetRemoteName(address=scan.address).name
    ...
    def TSC_MMI_iut_start_general_inquiry_found(self, pts_addr: bytes, **kwargs):
        ...
        inquiry_events = self.host.Inquiry()
        for event in inquiry_events:
            assert event.address == pts_addr, (event.address, pts_addr)
    ...
    def TSC_MMI_iut_start_general_discovery(self, **kwargs):
        ...
        self.scan_events = self.host.Scan()
    ...
    def TSC_MMI_iut_confirm_general_discovered_device(self, pts_addr: bytes, **kwargs):
        ...
        for event in self.scan_events:
            assert event.HasField("public")
            if (event.public == pts_addr and
                event.data.le_discoverability_mode == DiscoverabilityMode.DISCOVERABLE_GENERAL):
                self.scan_events.cancel()
                return "OK"
    ...
    def TSC_MMI_iut_confirm_limited_discovered_device(self, pts_addr: bytes, **kwargs):
        ...
        for event in self.scan_events:
            assert event.HasField("public")
            if (event.public == pts_addr and
                event.data.le_discoverability_mode == DiscoverabilityMode.DISCOVERABLE_LIMITED):
                self.scan_events.cancel()
                return "OK"
    ...
    def TSC_MMI_iut_confirm_limited_discovered_device_not_found(self, pts_addr: bytes, **kwargs):
        ...
        def search():
            ...
            for event in self.scan_events:
                assert event.HasField("public")
                if (event.public == pts_addr and
                    event.data.leDiscoverableMode == DiscoverabilityMode.DISCOVERABLE_LIMITED):
                    self.inquiry_events.cancel()
                    discovered = True
                    return
    ...
    def TSC_MMI_iut_start_general_discovery_DM(self, pts_addr: bytes, **kwargs):
        ...
        def search_le():
            ...
            scan_events = self.host.Scan()
            for event in scan_events:
                if (event.public == pts_addr and
                    event.data.leDiscoverableMode == DiscoverabilityMode.DISCOVERABLE_GENERAL):
                    scan_events.cancel()
                    discovered_le = True
                    return
