...
class GAPProxy(ProfileProxy):
    ...
    def TSC_MMI_iut_confirm_general_discovered_device(self, pts_addr: bytes, **kwargs):
        ...
        for event in self.scan_events:
            ...
            if event.public == pts_addr: # and flags_match(event.flags, GENERAL_MASK):
    ...
    def TSC_MMI_iut_confirm_limited_discovered_device(self, pts_addr: bytes, **kwargs):
        ...
        for event in self.scan_events:
            assert event.HasField("public")
            if event.public == pts_addr: # and flags_match(event.flags, LIMITED_MASK):
                self.scan_events.cancel()
                return "OK"
    ...
    def TSC_MMI_make_iut_not_connectable(self, **kwargs):
        ...
        self.host.SetDiscoverabilityMode(
            discoverability=DiscoverabilityMode.NOT_DISCOVERABLE)
