...
from pandora_experimental.gap_grpc import GAP
from pandora_experimental.gatt_grpc import GATT
from pandora_experimental.gatt_pb2 import GattService, GattCharacteristic
...
class GAPProxy(ProfileProxy):
    ...
    def TSC_MMI_iut_send_hci_connect_request(self, test: str, pts_addr: bytes, **kwargs):
        ...
        if test in {"GAP/SEC/SEM/BV-05-C", "GAP/SEC/SEM/BV-08-C"}:
            # we connect then pair, so we have to pair directly in this MMI
            self.pairing_events = self.security.OnPairing()
            self.security.Pair(connection=self.host.GetConnection(address=pts_addr).connection)
        else:
            self.connection = self.host.Connect(address=pts_addr, transport=Transport.TRANSPORT_BREDR).connection
    ...
    def TSC_MMI_iut_enter_handle_for_insufficient_authentication(self, pts_addr: bytes, **kwargs):
        ...
        # not sure which tests require this
    ...
    def TSC_MMI_iut_start_general_inquiry_found(self, pts_addr: bytes, **kwargs):
        ...
        # NOTE: it used to be what it was below, try re-enabling this if a test is failing
    ...
    def TSC_MMI_iut_enter_undirected_connectable_mode_non_discoverable_mode(self, **kwargs):
        ...
        self.host.SetDiscoverability(discoverability=DiscoverabilityMode.DISCOVERABILITY_NONE)
    ...
    def TSC_MMI_iut_send_le_disconnect_request(self, pts_addr: bytes, **kwargs):
        ...
        try:
            self.host.Disconnect(connection=self.host.GetConnection(address=pts_addr).connection)
        except Exception:
            pass
    ...
    def TSC_MMI_make_iut_not_connectable(self, **kwargs):
        ...
        self.host.SetDiscoverability(discoverability=DiscoverabilityMode.DISCOVERABILITY_NONE)
        self.host.SetConnectability(connectability=ConnectabilityMode.CONNECTABILITY_MODE_NOT_CONNECTABLE)
    ...
    @assert_description
    def TSC_MMI_press_ok_to_disconnect(self, **kwargs):
        """
    ...
    def TSC_MMI_iut_enter_non_connectable_mode_general_discoverable_mode(self, **kwargs):
        ...
        self.host.SetDiscoverability(discoverability=DiscoverabilityMode.DISCOVERABILITY_GENERAL)
        self.host.SetConnectability(connectability=ConnectabilityMode.CONNECTABILITY_MODE_NOT_CONNECTABLE)
    ...
    def TSC_MMI_iut_send_l2cap_connect_request(self, pts_addr: bytes, **kwargs):
        ...
        def after_that():
            sleep(5)
            self.host.Connect(address=pts_addr, transport=Transport.TRANSPORT_BREDR)
