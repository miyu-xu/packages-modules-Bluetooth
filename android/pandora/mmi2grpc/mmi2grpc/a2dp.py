...
class A2DPProxy(ProfileProxy):
    ...
    def TSC_AVDTP_mmi_iut_begin_streaming(self, test: str, **kwargs):
        ...
        if test == "A2DP/SRC/SUS/BV-01-I":
            ...
            time.sleep(1)
        if test not in "A2DP/SRC/SET/BV-03-I":  # Not initiating a2dp start again for this test case
            self.a2dp.Start(source=self.source)
        self.audio.start()
        return "OK"
    ...
    @match_description
    def TSC_A2DP_mmi_user_confirm_optional_string_attribute(self, name: str, **kwargs):
        """
