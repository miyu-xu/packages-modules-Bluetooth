...
class HFPProxy(ProfileProxy):
    ...
    def asyncWaitConnection(self, test: str, pts_addr: bytes, delay=WAIT_DELAY_BEFORE_CONNECTION):
        ...
        def waitConnectionCallback(self, test: str, pts_addr: bytes):
            ...
            if "HFP/HF" in test:
                self.slc = ServiceLevelConnection(handsfree=self.hfp.OpenHandsFree(connection=self.connection).handsfree)
            else:
                self.slc = ServiceLevelConnection(audiogateway=self.hfp.OpenAudioGateway(connection=self.connection).audiogateway)
