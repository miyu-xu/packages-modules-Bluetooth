...
class GATTProxy(ProfileProxy):
    ...
    def MMI_CONFIRM_PASSKEY(self, **kwargs):
        """
        Please verify the passKey is correct: XXXXXX
        """
    ...
    def MMI_CONFIRM_PRIMARY_SERVICE(self, description: str, **kwargs):
        ...
        assert all(uuid in list(map(lambda service: service.uuid, self.services)) for uuid in all_matches)
        return "OK"
    ...
    def MMI_IUT_DISCOVER_SERVICE_UUID(self, description: str, **kwargs):
        ...
        assert self.connection is not None
        uuid = formatUuid(re.findall("'([a0-Z9]*)'O", description)[0])
        self.characteristics = self.gatt.DiscoverCharacteristics(connection=self.connection, uuid=uuid).characteristics
        return "OK"
    ...
    def MMI_CONFIRM_ALL_CHARACTERISTICS_SERVICE(self, description: str, **kwargs):
        ...
        assert all(handle in list(map(lambda char: char.handle, self.characteristics)) for handle in all_matches)
        return "Yes"
    ...
    def MMI_IUT_DISCOVER_SERVICE_UUID_RANGE(self, description: str, **kwargs):
        ...
        """
        There is no Android API to discover characteristics for all services,
        so we have to trigger a global discovery.
        """
        self.services = self.gatt.DiscoverServices(connection=self.connection).services
        self.characteristics = getCharacteristicsRange(self.services, stringHandleToInt(handles[0]),\
                stringHandleToInt(handles[1]), uuid)
        return "OK"
...
def getCharacteristicsRange(services, start_handle, end_handle, uuid):
    ...
    for service in services:
        for characteristic in service.characteristics:
            if characteristic.handle >= start_handle and characteristic.handle <= end_handle and characteristic.uuid == uuid:
