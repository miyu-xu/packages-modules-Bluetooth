...
class GATTProxy(ProfileProxy):
    ...
    def MMI_IUT_CONFIRM_READ_INVALID_HANDLE(self, **kwargs):
        ...
        # BluetoothGatt#GATT_INVALID_OFFSET = 0x7.
        if self.read_value.status == 0x7:
            return "Yes"
        return "No"
    ...
    def MMI_IUT_CONFIRM_READ_NOT_PERMITTED(self, **kwargs):
        ...
        assert self.read_value is not None
        # BluetoothGatt#GATT_READ_NOT_PERMITTED = 0x2.
        if self.read_value.status == 0x2:
            return "Yes"
        return "No"
    ...
    def MMI_IUT_CONFIRM_READ_AUTHENTICATION(self, **kwargs):
        ...
        assert self.read_value is not None
        # BluetoothGatt#GATT_INSUFFICIENT_AUTHENTICATION = 0x5.
        # gatt_api#GATT_AUTH_FAIL = 0x89.
        self.gatt.CfmPassKey(connection=self.connection, passkey=0)
        if self.read_value.status == 0x5 or\
                self.read_value.status == 0x89 :
            return "Yes"
        return "No"
    ...
    def MMI_IUT_CONFIRM_ATTRIBUTE_NOT_FOUND(self, **kwargs):
        ...
        assert self.read_value is not None
        # There is no "NOT_FOUND" error code in Android for GATT not found characteristic.
        # BluetoothGatt#GATT_FAILURE = 0x101.
        if self.read_value.status == 0x101:
            return "Yes"
        return "No"
    ...
    def MMI_IUT_CONFIRM_READ_APPLICATION(self, **kwargs):
        ...
        assert self.read_value is not None
        # gatt_api#GATT_NO_RESOURCES = 0x80.
        if self.read_value.status == 0x80:
            return "Yes"
        return "No"
    ...
    def MMI_IUT_CONFIRM_READ_HANDLE_VALUE(self, description: str, **kwargs):
        ...
        if self.read_value == bytes_value:
            return "Yes"
        return "No"
    ...
    def _mmi_2004(self, description: str, **kwargs):
        """
        Please confirm that 6 digit number is matched with 451750.
        """

        assert self.connection is not None
        pin = int(re.findall(r' (\d{6}).', description)[0])
        self.gatt.CfmPassKey(connection=self.connection, passkey=pin)
        return re.findall(r' (\d{6}).', description)[0]
