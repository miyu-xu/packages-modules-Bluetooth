...
class GATTProxy(ProfileProxy):
    ...
    def MMI_IUT_CONFIRM_READ_INVALID_HANDLE(self, **kwargs):
        ...
        if self.read_value.status == self.read_value.StatusCodes.INVALID_HANDLE:
            return "Yes"
        return "No"
    ...
    def MMI_IUT_CONFIRM_READ_NOT_PERMITTED(self, **kwargs):
        ...
        if self.read_value.status == self.read_value.StatusCodes.READ_NOT_PERMITTED or\
                self.read_value.status == self.read_value.StatusCodes.UNKNOWN_ERROR:
            return "Yes"
        return "No"
    ...
    def MMI_IUT_CONFIRM_READ_AUTHENTICATION(self, **kwargs):
        ...
        assert self.read_value is not None
        if self.read_value.status == self.read_value.StatusCodes.INSUFFICIENT_AUTHENTICATION:
            return "Yes"
        return "No"
    ...
    def MMI_IUT_CONFIRM_ATTRIBUTE_NOT_FOUND(self, **kwargs):
        ...
        # Android read error doesn't return an error code so we have to also
        # compare to the generic error code here.
        if self.read_value.status == self.read_value.StatusCodes.ATTRIBUTE_NOT_FOUND or\
                self.read_value.status == self.read_value.StatusCodes.UNKNOWN_ERROR:
            return "Yes"
        return "No"
    ...
    def MMI_IUT_CONFIRM_READ_APPLICATION(self, **kwargs):
        ...
        assert self.read_value is not None
        if self.read_value.status == self.read_value.StatusCodes.APPLICATION_ERROR:
            return "Yes"
        return "No"
    ...
    def MMI_IUT_SEND_READ_DESCIPTOR_HANDLE(self, description: str, **kwargs):
        ...
        assert self.connection is not None
        handle = stringHandleToInt(re.findall("'([a0-Z9]*)'O", description)[0])
        self.read_value = self.gatt.ReadAttributeFromHandle(\
                connection=self.connection, handle=handle)
        return "OK"
