...
from pandora.gatt_pb2 import GattReadStatusCodes
...
class GATTProxy(ProfileProxy):
    ...
    def MMI_IUT_CONFIRM_READ_INVALID_HANDLE(self, **kwargs):
        ...
        if self.read_value.status == GattReadStatusCodes.INVALID_HANDLE:
            return "Yes"
        return "No"
    ...
    def MMI_IUT_CONFIRM_READ_NOT_PERMITTED(self, **kwargs):
        ...
        if self.read_value.status == GattReadStatusCodes.READ_NOT_PERMITTED or\
                self.read_value.status == GattReadStatusCodes.UNKNOWN_ERROR:
            return "Yes"
        return "No"
    ...
    def MMI_IUT_CONFIRM_READ_AUTHENTICATION(self, **kwargs):
        ...
        assert self.read_value is not None
        if self.read_value.status == GattReadStatusCodes.INSUFFICIENT_AUTHENTICATION:
            return "Yes"
        return "No"
    ...
    def MMI_IUT_CONFIRM_ATTRIBUTE_NOT_FOUND(self, **kwargs):
        ...
        # Android read error doesn't return an error code so we have to also
        # compare to the generic error code here.
        if self.read_value.status == GattReadStatusCodes.ATTRIBUTE_NOT_FOUND or\
                self.read_value.status == GattReadStatusCodes.UNKNOWN_ERROR:
            return "Yes"
        return "No"
    ...
    def MMI_IUT_CONFIRM_READ_APPLICATION(self, **kwargs):
        ...
        assert self.read_value is not None
        if self.read_value.status == GattReadStatusCodes.APPLICATION_ERROR:
            return "Yes"
        return "No"
