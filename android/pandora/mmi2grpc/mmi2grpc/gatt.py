...
class GATTProxy(ProfileProxy):
    ...
    def MMI_IUT_SEND_INDICATION(self, description: str, **kwargs):
        ...
        self.gatt.DiscoverServices()
        handle = re.findall("handle = '([a0-Z9]*)'O", description)
        self.gatt.WriteCharacteristic(handle=handle[0], size=4)

        return "OK"

    def MMI_IUT_SEND_READ_CHARACTERISTIC_HANDLE(self, description: str, pts_addr: bytes, **kwargs):
        """
        ...
        self.gatt.DiscoverServicesByUUID(uuid=handle, address=pts_addr)
        self.gatt.ReadCharacteristic(handle=handle)

        return "OK"

    def MMI_IUT_SEND_NOTIFICATION(self, **kwargs):
        """
        Please write to client characteristic configuration handle = 'XXXX'O to
