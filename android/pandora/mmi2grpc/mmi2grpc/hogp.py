...
class HOGPProxy(ProfileProxy):
    ...
    def IUT_SEND_WRITE_REQUEST(self, handle: str, properties: str, **kwargs):
        r"""
        Please send write request to handle (?P<handle>.*) with following value.
        Client
        Characteristic Configuration:
            Properties: \[0x00(?P<properties>.*)\]
        """

        self.gatt.WriteAttribute(
    ...
    def USER_CONFIRM_CHARACTERISTIC(self, body: str, **kwargs):
        ...
        PATTERN = re.compile(
            textwrap.dedent("""
                Attribute Handle = (.*)
                Characteristic Properties = (?P<properties>.*)
                Handle = (?P<handle>.*)
                UUID = (?P<uuid>.*)
                """.strip()))
        ...
        for block in blocks:
            ...
            targets.add((
                eval(match.group("properties")),
                eval(match.group("handle")),
                eval(match.group("uuid")),
            ))
    ...
    def USER_CONFIRM_CHARACTERISTIC_DESCRIPTOR(self, body: str, **kwargs):
        ...
        blocks = [block for block in body.split("\n") if block]
        blocks = [a + "\n" + b for a, b in zip(blocks[::2], blocks[1::2])]
    ...
    def USER_CONFIRM_SERVICE_HANDLE(self, service_name: str, body: str, **kwargs):
        ...
        PATTERN = re.compile(r"Start Handle: (?P<start_handle>.*)     End Handle: (?P<end_handle>.*)")
        ...
        for block in body.split("\n"):
            ...
            start_handle = PATTERN.match(block).group("start_handle")

            for service in services:
                if service.handle == int(start_handle, base=16):
                    assert (short_uuid(
                        service.uuid) == target_uuid[service_name]), "service UUID does not match expected type"
                    break
            else:
    ...
    def IUT_CONFIG_NOTIFICATION(self, value: str, **kwargs):
        ...
        Descriptor handle value: (?P<value>.*)
        """
    ...
    @match_description
    def USER_CONFIRM_READ_RESULT(self, characteristic_name: str, **kwargs):
        r"""
