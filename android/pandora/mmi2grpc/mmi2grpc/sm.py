...
from pandora.host_pb2 import HandlePairingEventRequest

# The tests needs the MMI to accept pairing confirmation request.
NEEDS_PAIRING_CONSENT = {
    "SM/CEN/EKS/BV-01-C",
    "SM/CEN/JW/BI-04-C",
    "SM/CEN/JW/BI-01-C",
}
...
class SMProxy(ProfileProxy):
    ...
    def MMI_ASK_IUT_PERFORM_PAIRING_PROCESS(self, test, **kwargs):
        ...
        if self.connection:
            ...
            if test in NEEDS_PAIRING_CONSENT:
                self.host.HandlePairingEvent(
                    connection=self.connection,
                    pairing_variant=HandlePairingEventRequest.PairingVariant.PAIRING_VARIANT_CONSENT,
                    pairing_confirmation_value=True)
            return "OK"
