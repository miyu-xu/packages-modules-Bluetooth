...
class RootCanal:
    ...
    def select_pts_dongle(self, dongle: Dongle):
        ...
        # Android is always first to connect to Android.
        (devices, _) = self._read_device_list()
        pts_id = max([id for (id, _) in devices])
        self.channel.send_command("select_config", [pts_id, str(dongle)])
    ...
    def _read_device_list(self):
        ...
        for line in response.split("\n"):
            ...
            if line.startswith("Devices") or line.startswith("Phys"):
                ...
                phy_id = int(parts[0])
                phy_name = parts[1]
                phy_devices = [int(id.strip()) for id in parts[2].split(",")]
                devices.append((phy_id, phy_name, phy_devices))
