...
class HAPProxy(ProfileProxy):
    ...
    def ORDER_IUT_SEND_PRESET_WRITE_NAME(self, test: str, index: str, **kwargs):
        ...
        # Android reads all presets after device is paired. Wait for initial preset info before sending next request.
            self.preset_changed = self.hap.WaitPresetChanged().presetChangedReceived
            assert self.preset_changed

        preset_name = ''.join(random.choice(string.ascii_lowercase) for i in range(10))

        self.hap.SetPresetName(connection=self.connection, index=int(index), name=preset_name)
    ...
    def IUT_ORDER_WRITE_READ_ALL_PRESET(self, test: str, **kwargs):
        ...
        self.preset_changed = self.hap.WaitPresetChanged()
        assert self.preset_changed

        responseMessage = self.hap.GetAllPresetsInfo(connection=self.connection)

        self.log(f"Received preset info:\n")
        for presetInfo in responseMessage.preset_info_list:
            self.log(f"    Index: {presetInfo.presetIndex}\n"
                     f"    Name: {presetInfo.presetName}\n"
                     f"    Writable: {presetInfo.isWritable}\n"
                     f"    Available: {presetInfo.isAvailable}\n")
    ...
    def IUT_READ_PRESET_INDEX(self, test: str, index: str, num_presets: str, **kwargs):
        ...
        responseMessage = self.hap.GetPresetInfo(connection=self.connection, index=int(index))

        self.log(f"Received preset info:\n"
                 f"    Index: {responseMessage.preset_info.presetIndex}\n"
                 f"    Name: {responseMessage.preset_info.presetName}\n"
                 f"    Writable: {responseMessage.preset_info.isWritable}\n"
                 f"    Available: {responseMessage.preset_info.isAvailable}\n")
    ...
    def IUT_ORDER_WRITE_SET_ACTIVE_PRESET_INDEX_SYNC_LOCALLY(self, test: str, index: str, **kwargs):
        ...
        self.hap.SelectPreset(connection=self.connection, index=int(index))
    ...
    def IUT_ORDER_WRITE_SET_NEXT_PRESET_INDEX_SYNC_LOCALLY(self, test: str, **kwargs):
        ...
        self.hap.SelectNextPreset(connection=self.connection)
    ...
    def IUT_ORDER_WRITE_SET_PREVIOUS_PRESET_INDEX_SYNC_LOCALLY(self, test: str, **kwargs):
        ...
        self.hap.SelectPreviousPreset(connection=self.connection)
    ...
    def IUT_ORDER_WRITE_SET_ACTIVE_PRESET_INDEX_DO_NOT_EXPECT_TO_RECEIVE(self, test: str, index: str, **kwargs):
        ...
        # This should be called and ignored by the stack
        self.hap.SelectPreset(connection=self.connection, index=int(index))
