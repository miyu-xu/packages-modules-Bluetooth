...
class AVRCPProxy(ProfileProxy):
    ...
    def __init__(self, channel):
        ...
        self.audio = AudioSignal(
            lambda frames: self.avrcp.PlaybackAudio(map(convert_frame, frames)),
    ...
    @assert_description
    def TSC_AVCTP_mmi_register_DisconnectCfm_CB(self, **kwargs):
        """
    ...
    @assert_description    
    def TSC_AVCTP_mmi_send_AVCT_Disconnect_Req(self, **kwargs):
        """
    ...
    #@assert_description
    #description does not match with function docstring of TSC_AVCTP_mmi_verify_ConnectInd_CB
    ...
    #@assert_description
    #description does not match with function docstring of TSC_AVCTP_mmi_register_ConnectInd_CB  
    ...
    @assert_description
    def TSC_AVCTP_mmi_register_MessageInd_CB_CT(self, **kwargs):
        """
        Using the Upper Tester register the function MessageInd_CBTest_System
        for callback on the AVCT_MessageRec_Ind event by sending an
        AVCT_EventRegistration command to the IUT with the following parameter
    ...
    @assert_description
    def TSC_AVCTP_mmi_send_fragmented_AVCT_SendMessage_CT(self, **kwargs):
        """
        Using the Upper Tester send an AVCT_SendMessage command to the IUT with
        the following parameter values:
           * BD_ADDR = BD_ADDRTest_System
           *
        Transaction = TRANSTest_System
           * Type = 1
           * PID = PIDTest_System
        * Data = ADDRESSdata_buffer (Buffer holding DATA[]Upper_Tester)
           *
        Length = LengthOf(DATA[]Upper_Tester) = 3*MTULowerTester - 6bytes
        """
    
        return "OK"
        
    @assert_description
    def TSC_AVCTP_mmi_send_fragmented_AVCT_SendMessage_TG(self, **kwargs):
        """
        Upon receipt of the appropriate notification from the IUT, use the Upper
        Tester to send an AVCT_SendMessage command to the IUT with the parameter
        values:
           * BD_ADDR = BD_ADDRTest_System
           * Transaction =
        TRANSTest_System
           * Type = 0x01 (Response Message)
           * PID =
        PIDTest_System
           * Data = ADDRESSdata_buffer (Buffer holding
        DATA[]Upper_Tester)
           * Length = LengthOf(DATA[]Upper_Tester) = 3 *
        MTULowerTester - 6 bytes
        """
