...
class L2CAPProxy(ProfileProxy):
    ...
    def MMI_IUT_SEND_LE_CREDIT_BASED_CONNECTION_REQUEST(self, test: str, pts_addr: bytes, **kwargs):
        ...
        try:
            ...
            self.channel = self.l2cap.Connect(connection=self.connection, le_credit_based=le_credit_based_channel).channel
    ...
    def MMI_TESTER_ENABLE_LE_CONNECTION(self, test: str, **kwargs):
        ...
        if test in tests_to_open_bluetooth_server_socket:
            ...
            if wait_connection_response.HasField('channel'):  # type: ignore
                self.channel = self.l2cap.WaitConnection(le_credit_based=le_credit_based_channel).channel  # type: ignore
            elif test in tests_connection_target_to_failed:
                self.test_status_map[test] = 'OK'  # type: ignore
                print(test, 'connection targets to fail', file=sys.stderr)
            else:
                raise Exception(wait_connection_response.error)  # type: ignore
