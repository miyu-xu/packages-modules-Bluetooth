...
from pandora.host_pb2 import Connection, CONNECTABLE, DISCOVERABLE_GENERAL
from pandora.security_pb2 import LEVEL2, PairingEventAnswer, SecurityLevel, SecureResponse, WaitSecurityResponse
from typing import Callable, Coroutine, Optional, Tuple


class SspTest(base_test.BaseTestClass):  # type: ignore[misc]
    '''
    ...
    async def _test_success(
        self, ref_io_capability: Optional[int],
        ...
        try:
            ...
            if dut_pairing_event.method_variant() in ('numeric_comparison', 'just_works'):
                assert ref_pairing_event.method_variant() in ('numeric_comparison', 'just_works')
                dut_pairing_stream.send_nowait(
                    PairingEventAnswer(event=dut_pairing_event, confirm=True)
                )
                ref_pairing_stream.send_nowait(
                    PairingEventAnswer(event=ref_pairing_event, confirm=True)
                )
    ...
    @avatar.asynchronous
    async def test_success_initiate_connection_initiate_pairing(self, ref_io_capability: Optional[int]) -> None:
    ...
    @avatar.asynchronous
    async def test_success_initiate_connection_accept_pairing(self, ref_io_capability: Optional[int]) -> None:
        if not isinstance(self.dut, BumblePandoraDevice):
            raise signals.TestSkip('TODO: Investigate Android failures.')
    ...
    @avatar.asynchronous
    async def test_success_accept_connection_initiate_pairing(self, ref_io_capability: Optional[int]) -> None:   
        # Connection/pairing task.
        async def connect_and_pair() -> Tuple[SecureResponse, WaitSecurityResponse]:
    ...
    @avatar.asynchronous
    async def test_success_accept_connection_accept_pairing(self, ref_io_capability: Optional[int]) -> None:
        # Connection/pairing task.
        async def connect_and_pair() -> Tuple[SecureResponse, WaitSecurityResponse]:
            ref_dut, dut_ref = await connect(self.ref, self.dut)
            return await pair(self.ref, ref_dut, self.dut, dut_ref, LEVEL2)
...
# Pairing task.
async def pair(
    initiator: PandoraDevice, initiator_connection: Connection,
    acceptor: PandoraDevice, acceptor_connection: Connection,
