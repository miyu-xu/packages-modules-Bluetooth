...
from bumble.smp import SMP_ENC_KEY_DISTRIBUTION_FLAG, SMP_ID_KEY_DISTRIBUTION_FLAG, SMP_SIGN_KEY_DISTRIBUTION_FLAG
...
class LeSecurityTest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    @avatar.parameterized(*itertools.product(ALL_IO_CAPABILITIES, ALL_KEY_DISTRIBUTION))  # type: ignore[misc]
    async def test_le_pairing_incoming(  # pytype: disable=wrong-arg-types
        self,
        ref_io_capability: Optional[PairingDelegate.IoCapability],
        ref_key_distribution_pair: Tuple[PairingDelegate.IoCapability, PairingDelegate.IoCapability],
    ) -> None:
