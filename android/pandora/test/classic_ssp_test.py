...
class ClassicSspTest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    async def setup_test(self) -> None:  # pytype: disable=wrong-arg-types
        ...
        if isinstance(self.dut, BumblePandoraDevice):
            # Override Bumble DUT device default IO capability.
            self.dut.server_config.io_capability = PairingDelegate.DISPLAY_OUTPUT_AND_YES_NO_INPUT
    ...
    async def _test_success(
        ...
        try:
            ...
            if dut_pairing_event.method_variant() in ('numeric_comparison', 'just_works'):
                ...
                confirm = True
                if (
                    dut_pairing_event.method_variant() == 'numeric_comparison' and
                    ref_pairing_event.method_variant() == 'numeric_comparison'
                ):
