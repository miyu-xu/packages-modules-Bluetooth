...
class AicsTest(base_test.BaseTestClass):
    ...
    async def setup_test(self) -> None:
        ...
        volume_control_service = VolumeControlService(included_services=[AICSService()])
        self.ref.device.add_service(volume_control_service)  # type: ignore
        self.dut_gatt = GATT(self.dut.channel)
    ...
    def test_do_not_discover_aics_as_primary_service(self) -> None:
        ...
        services = self.dut_gatt.DiscoverServices(dut_ref_connection).services
    ...
    def test_gatt_discover_aics_service_as_secondary_service(self) -> None:
        dut_ref_connection = self.connect_dut_to_ref()
