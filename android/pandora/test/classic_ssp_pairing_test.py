...
from bumble.hci import HCI_CENTRAL_ROLE, HCI_PERIPHERAL_ROLE, Address as BumbleAddress, HCI_Switch_Role_Command
...
from mobly.asserts import assert_equal, assert_in, assert_is_instance, assert_is_not_none, fail
...
class ClassicSspTests(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    def overwrite_ref_pairing_capability(
        self,
        io_capability: int,
        local_initiator_key_distribution: int,
        local_responder_key_distribution: int,
    ) -> None:
        default_pairing_config_factory = self.ref.device.pairing_config_factory

        def pairing_config_factory(connection: bumble.device.Connection,) -> PairingDelegate:
            return PairingConfig(
                ...
                delegate=PairingDelegate(
                    connection,
                    service=default_pairing_config_factory(connection).delegate.service,
    ...
    @asynchronous
    async def test_classic_pairing_incoming(self, ref_io_capability: int, ref_role: int) -> None:
    ...
    @asynchronous
    async def test_classic_pairing_outgoing(self, ref_io_capability: int, ref_role: int) -> None:
        # override reference device IO capability
        setattr(self.ref.device, 'io_capability', ref_io_capability)
