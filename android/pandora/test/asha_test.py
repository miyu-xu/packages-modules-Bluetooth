...
class AshaTest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    async def test_music_stop(self) -> None:
        ...
        audio_data = await self.get_audio_data(
            ref_asha=AioAsha(self.ref_left.aio.channel), ear=Ear.LEFT, connection=ref_dut, timeout=10
        )
    ...
    async def test_music_stop_dual_device(self) -> None:
        ...
        (audio_data_left, audio_data_right) = await asyncio.gather(
            self.get_audio_data(ref_asha=ref_left_asha, ear=Ear.LEFT, connection=ref_left_dut, timeout=10),
            self.get_audio_data(ref_asha=ref_right_asha, ear=Ear.RIGHT, connection=ref_right_dut, timeout=10),
        )
    ...
    async def test_music_audio_playback(self) -> None:
        ...
        await self.get_audio_data(ref_asha=ref_asha, ear=Ear.LEFT, connection=ref_left_dut, timeout=10)
        ...
        _, audio_data = await asyncio.gather(
            ...
            self.get_audio_data(ref_asha=ref_asha, ear=Ear.LEFT, connection=ref_left_dut, timeout=10),
        )
