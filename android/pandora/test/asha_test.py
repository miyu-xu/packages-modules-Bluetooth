...
class ASHATest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    def advertise(self, ref_address_type: OwnAddressType) -> AdvertiseResponse:
        ...
        self.ref.asha.Register(
            capability=ASHATest.CAPABILITY, hisyncid=ASHATest.HISYCNID
        )
        return self.ref.host.Advertise(
            legacy=True,
            connectable=True,
            data=DataTypes(
                complete_local_name=self.COMPLETE_LOCAL_NAME,
    ...
    def scan(self, dut_address_type: OwnAddressType) -> ScanningResponse:
        ...
        ref = None
        for peer in peers:
            if ASHATest.ASHA_UUID in peer.data.incomplete_service_class_uuids16:
                logging.debug(f"device has:{ASHATest.ASHA_UUID} UUID")
                ref = peer
                break
        peers.cancel()
    ...
    def test_advertising(self) -> None:
        ...
        expected_advertisement_data = (
            ...
            + "{:02x}".format(ASHATest.CAPABILITY)
