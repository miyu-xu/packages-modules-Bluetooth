...
class ASHATest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    async def test_music_start(self) -> None:
        ...
        asha_service = next((x for x in self.ref_left.device.gatt_server.attributes if isinstance(x, AshaService)))
        ...
        _, start_result = await asyncio.gather(dut_asha.Start(), start_future)
    ...
    async def test_set_volume(self) -> None:
        ...
        _, volume_result = await asyncio.gather(dut_asha.SetVolume(1), volume_future)
    ...
    async def test_music_stop(self) -> None:
        ...
        await dut_asha.Start()
        _, stop_result = await asyncio.gather(dut_asha.Stop(), stop_future)
