...
class ASHATest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    def get_le_psm_future(self, ref_device: PandoraDevice) -> asyncio.Future:
    ...
    def get_read_only_properties_future(self, ref_device: PandoraDevice) -> asyncio.Future:
        asha_service = next((x for x in ref_device.device.gatt_server.attributes if isinstance(x, AshaGattService)))
    ...
    def get_start_future(self, ref_device: PandoraDevice) -> asyncio.Future:
        asha_service = next((x for x in ref_device.device.gatt_server.attributes if isinstance(x, AshaGattService)))
    ...
    def get_stop_future(self, ref_device: PandoraDevice) -> asyncio.Future:
        asha_service = next((x for x in ref_device.device.gatt_server.attributes if isinstance(x, AshaGattService)))
    ...
    async def test_music_start_dual_device(self) -> None:
        ...
        await self.dut.aio.security.Secure(connection=dut_ref_left, le=LE_LEVEL3)
        ...
        assert_true(isinstance(start_result_left, dict), "")
        ...
        await self.dut.aio.security.Secure(connection=dut_ref_right, le=LE_LEVEL3)
