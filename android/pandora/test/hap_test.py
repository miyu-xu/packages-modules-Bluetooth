...
class HapTest(base_test.BaseTestClass):
    ...
    async def assertIdentiqPresetInDutAndRef(self, dut_connection_to_ref: Connection) -> None:
    ...
    async def test__set_non_existing_preset_as_active__verify_no_crash_and_no_update(self) -> None:
        ...
        dut_connection_to_ref = await self.setupHapConnection()
        AssertThat(toBumblePreset(
            (await
             self.hap_grpc.GetActivePreset(connection=dut_connection_to_ref)).preset_record)).IsEqualTo(foo_preset)

        await self.logcat.Log("Notify active update to non existing index")
        await self.has.set_active_preset_unchecked(non_existing_preset_index)
