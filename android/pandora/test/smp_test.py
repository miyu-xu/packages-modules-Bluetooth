...
from pandora.host_pb2 import RANDOM, PUBLIC, DataTypes, OwnAddressType, ScanningResponse
...
class SmpTest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    @asynchronous
    async def test_le_pairing_smp_security_req(self) -> None:
        if isinstance(self.ref, BumblePandoraDevice):
            ...
            self.ref.server_config.pairing_sc_enable=False
        ...
        scan = self.dut.aio.host.Scan(own_address_type=PUBLIC)
        ref = await anext((x async for x in scan if b'pause cafe' in x.data.manufacturer_specific_data))
        scan.cancel()

        pairing = asyncio.create_task(self.handle_pairing_events())
        (dut_ref_res, ref_dut_res) = await asyncio.gather(
            self.dut.aio.host.ConnectLE(own_address_type=PUBLIC, **ref.address_asdict()),
            anext(aiter(advertisement)),
        )
        ...
        connection.on(
            'connection_encryption_change', on_connection_encryption_change
        )
        ...
        class Session(smp.Session):
            ...
            def on_smp_pairing_request_command(
                ...
                assert_true(False, "Pairing request received")
