...
from bumble.smp import SMP_Identity_Address_Information_Command, Session
from bumble.hci import Address
from concurrent import futures
from contextlib import suppress
from mobly import base_test, test_runner
from mobly.asserts import assert_equal  # type: ignore
from pandora.host_pb2 import DataTypes, RANDOM, ScanningResponse
...
class SmpTest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    async def dut_pair(self, dut_address_type, ref_address_type) -> ScanningResponse:
    ...
    async def test_le_pairing_delete_dup_bond_record(self) -> None:
        ...
        def send_command(self, command):
            if isinstance(command, SMP_Identity_Address_Information_Command):
                ...
                command = SMP_Identity_Address_Information_Command(
        ...
        is_bonded = await self.dut.aio.security_storage.IsBonded(**ref1.address_asdict())
        assert is_bonded.value

        await self.ref.reset()

        ref2 = await self.dut_pair(dut_address_type=RANDOM, ref_address_type=RANDOM)
        is_bonded = await self.dut.aio.security_storage.IsBonded(**ref2.address_asdict())
        assert is_bonded.value
