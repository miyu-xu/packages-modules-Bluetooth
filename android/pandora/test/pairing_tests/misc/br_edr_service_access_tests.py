...
from mobly.asserts import assert_equal, fail, assert_raises, assert_is_not_none
...
class ServiceAccessTempBondingTest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    async def setup_test(self) -> None:
        ...
        async def start_pairing():
            await asyncio.gather(
                self.dut.aio.security.WaitSecurity(
                    connection=android_res.connection, classic=LEVEL2
                ),
                self.ref.aio.security.Secure(
                    connection=bumble_res.connection, classic=LEVEL2
                ),
            )
        ...
        android_addr = Address.from_string_for_transport(
            str(self.dut.address), Address.PUBLIC_DEVICE_ADDRESS
        )
    ...
    async def test_access_sdp_service(self):
        sdp_psm = 0x0001
        sdp_channel = self.acl_connection.create_l2cap_channel(
            spec=ClassicChannelSpec(psm=sdp_psm)
        )
        try:
    ...
    async def test_access_rfcomm_service(self):
        rfc_psm = 0x0003
        rfcomm_channel = self.acl_connection.create_l2cap_channel(
    ...
    def _parse_rfcomm_channel_from_sdp_service_attributes(self, attributes):
        ...
        for attribute in attributes:
            ...
            if (
                ...
                and len(attribute.value.value) >= 2
            ):
                proto0 = attribute.value.value[0]
                proto1 = attribute.value.value[1]

                if (
                    proto0.value[0].value == BT_L2CAP_PROTOCOL_ID
                    and proto1.value[0].value == BT_RFCOMM_PROTOCOL_ID
                ):
    ...
    async def test_access_rfcomm_mx_insecure_service(self):
        # async def start_insecure_rfcomm_mx_server():
