...
class BREDRAclConnectionBuilder(IAclConnectionBuilder):
    ...
    async def cleanup(self):
        if self.initiator_connection().result_variant() == 'connection':
            await asyncio.gather(self.initiator.Disconnect(await self.initiator_connection()),
...
class BumbleHidServiceAccessor(IServiceAccessor):
    ...
    async def verify_role_setup(self):
        assert_true(isinstance(self._acl_connection_builder.initiator, BumblePandoraDevice),
                    "acl initiator should be Bumble")
        assert_true(isinstance(self.initiator, BumblePandoraDevice),
                    "initiator should be Bumble")
