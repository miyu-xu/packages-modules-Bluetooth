...
class BREDRLegacyTestClass(base_test.BaseTestClass):
    ...
    async def accept_pairing(self, initiator_pairing_event_stream, responder_pairing_event_stream):
        ...
        init_ev = await asyncio.wait_for(init_pairing_fut, timeout=120.0)
        ...
        responder_ev = await asyncio.wait_for(responder_pairing_fut, timeout=120.0)
    ...
    async def test_dedicated_pairing_ref_initiate_1(self) -> None:
        ...
        android_pairing_res, bumble_pairing_res = await asyncio.wait_for(pairing_task, timeout=120.0)
        logging.debug(
    ...
    async def test_dedicated_pairing_dut_initiate_1(self) -> None:
        ...
        bumble_res, android_res = await asyncio.wait_for(acl_connection_task, timeout=120.0)
    ...
    async def test_general_pairing(self) -> None:
        ...
        try:
            connect_res = await asyncio.wait_for(hid_control_service_access_tsk, timeout=120.0)
        except:
