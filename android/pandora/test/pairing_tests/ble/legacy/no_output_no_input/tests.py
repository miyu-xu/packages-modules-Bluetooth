...
class BLELegNoInputNoOutputTestClass(base_test.BaseTestClass):
    ...
    async def accept_pairing(self, initiator_pairing_event_stream,
        ...
        if initiator_pairing_event_stream == self.bumble_pairing_stream:
            confirm_stream = responder_pairing_event_stream
        else:
            confirm_stream = initiator_pairing_event_stream

        pairing_fut = asyncio.create_task(anext(confirm_stream))
        pairing_ev = await asyncio.wait_for(pairing_fut, timeout=10.0)
