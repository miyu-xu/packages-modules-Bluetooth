...
class BLESCNoInputNoOutputTestClass(base_test.BaseTestClass):
    ...
    async def accept_pairing(self, initiator_pairing_event_stream,
        ...
        if initiator_pairing_event_stream == self.bumble_pairing_stream:
            confirm_stream_1 = responder_pairing_event_stream
            confirm_stream_2 = initiator_pairing_event_stream
        else:
            confirm_stream_1 = initiator_pairing_event_stream
            confirm_stream_2 = responder_pairing_event_stream

        pairing_fut_1 = asyncio.create_task(anext(confirm_stream_1))
        pairing_ev_1 = await asyncio.wait_for(pairing_fut_1, timeout=10.0)
        logging.debug(f'pairing_ev_1.method_variant():{pairing_ev_1.method_variant()}')
        assert_equal(pairing_ev_1.method_variant(), 'just_works')

        ans_1 = PairingEventAnswer(event=pairing_ev_1, confirm=True)
        confirm_stream_1.send_nowait(ans_1)

        # TODO:
        # FIXME:
        # why a confirmation is needed on bumble?
        pairing_fut_2 = asyncio.create_task(anext(confirm_stream_2))
        pairing_ev_2 = await asyncio.wait_for(pairing_fut_2, timeout=10.0)
        logging.debug(f'pairing_ev_1.method_variant():{pairing_ev_2.method_variant()}')
        assert_equal(pairing_ev_2.method_variant(), 'just_works')

        ans_2 = PairingEventAnswer(event=pairing_ev_2, confirm=True)
        confirm_stream_2.send_nowait(ans_2)
