...
class A2dpTest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    async def test_signaling_channel_and_streaming(self) -> None:
        ...
        async def receive_audio(timeout: float = 1.0):
            while True:
                try:
                    packet = await asyncio.wait_for(channel.expect_media(), timeout)
                    received_rtp_packets.append(packet)
                except TimeoutError:
                    logger.info(f"No media packets for > {timeout} seconds")
                    break

        # When AudioTrack finished streaming it sends Suspend to RD1.
        ...
        await asyncio.gather(asyncio.wait_for(receive_audio(), 5.0), acceptor_avdt_suspend(channel))
        assert channel.transport_queue.empty()
        assert len(received_rtp_packets) > 0
        logger.info(f"Received: {len(received_rtp_packets)} packets")
