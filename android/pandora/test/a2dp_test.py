...
from bumble.l2cap import (ChannelManager, ClassicChannel, ClassicChannelSpec, L2CAP_Connection_Response,
                          L2CAP_SIGNALING_CID)
...
class A2dpTest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    async def test_avdt_signaling_channel_connection_collision(self) -> None:
        ...
        class L2capConfigurationRequest:
            ...
            def set(self, connection: Connection, cid: int, request):
                self.connection = connection
                self.cid = cid
                self.request = request

            def get(self):
                return self.connection, self.cid, self.request

        global pending_configuration_request
        pending_configuration_request = L2capConfigurationRequest()

        class AvdtpL2capChannelManager(ChannelManager):
            ...
            def on_l2cap_connection_request(self, connection: Connection, cid: int, request) -> None:
                global pending_configuration_request
                if (request.psm == AVDTP_PSM and pending_configuration_request != None):
                    logger.info("<< RD1 rejects AVDTP connection request from DUT >>")
                    ...
                    logger.info("<< RD1 proceeds with first AVDTP channel configuration >>")
                    chan_connection, chan_cid, chan_request = pending_configuration_request.get()
                    pending_configuration_request = None
                    super().on_control_frame(chan_connection, chan_cid, chan_request)
                    return
                super().on_l2cap_connection_request(connection, cid, request)

        class AvdtClassicChannel(ClassicChannel):
            ...
            def on_connection_response(self, response):
                ...
                logger.info("<< RD1 connected DUT, configuration postponed >>")

            def on_configure_request(self, request) -> None:
                global pending_configuration_request
                if (pending_configuration_request != None):
                    logger.info("<< Block RD1 until DUT tries AVDTP channel connection >>")
                    pending_configuration_request.set(self.connection, self.source_cid, request)
                else:
                    super().on_configure_request(request)

        # Override L2CAP Channel Manager to control signaling
        self.ref1.device.l2cap_channel_manager = AvdtpL2capChannelManager(self.ref1.device)
        ...
        assert source_cid is not None, "source_cid should not be None"

        spec = ClassicChannelSpec(AVDTP_PSM)
        channel = AvdtClassicChannel(
        ...
        logger.info("<< RD1 connects DUT over AVDTP - first channel >>")
        await channel.connect()
        logger.info(f"<< Channel established: {channel} >>")
