...
from pandora.host_pb2 import PUBLIC, RANDOM, DataTypes, DiscoverabilityMode, OwnAddressType
...
class LeAdvertisingTest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    @parameterized(
        ...
        (E.ADV_SCAN_IND, 31),
        (E.ADV_NONCONN_IND, 0),
        (E.ADV_NONCONN_IND, 31),
    )
    def test_legacy_advertising(self, advertising_event_properties: int, advertising_data_length: int) -> None:
        ...
        connectable = (advertising_event_properties & E.CONNECTABLE) != 0
        scannable = (advertising_event_properties & E.SCANNABLE) != 0
        directed = (advertising_event_properties & E.DIRECTED) != 0

        manufacturer_specific_data_length = max(0, advertising_data_length - 5)  # Flags (3) + LV (2)
        manufacturer_specific_data = bytes([random.randint(1, 255) for n in range(manufacturer_specific_data_length)])
        advertising_data = (
