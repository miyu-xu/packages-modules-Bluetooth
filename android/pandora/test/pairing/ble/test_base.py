...
class BLEPairTestBase(PairTestBase):
    ...
    async def start_service_access(
        ...
        # add a GATT service with some characteristic
        ...
        # await dut_gatt.ExchangeMTU(mtu=512, connection=initiator_acl_connection)
...
class BLEPairTestBaseWithGeneralAndDedicatedPairingTests(BLEPairTestBaseWithGeneralPairingTests):
    ...
    async def test_dedicated_pairing_dut_initiate_2(self) -> None:
        ...
        bumble_pairing_res, android_pairing_res = await asyncio.wait_for(pairing_task, timeout=10.0)
        logging.debug(f'Pairing result: {android_pairing_res.result_variant()}/{bumble_pairing_res.result_variant()}')
