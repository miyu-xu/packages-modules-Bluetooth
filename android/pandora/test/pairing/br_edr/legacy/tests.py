...
class BREDRLegacyTestClass(base_test.BaseTestClass):
    ...
    async def accept_pairing(self, initiator_pairing_event_stream,
        ...
        init_ev_ans = PairingEventAnswer(event=init_ev,
        ...
        responder_ev_ans = PairingEventAnswer(event=responder_ev,
                                              pin=pairing_pin_code)
    ...
    async def test_general_pairing(self) -> None:
        ...
        # first initiate an ACL connection from bumble to android
        _, _ = await self._start_acl_connection(self.ref, self.dut)
        ...
        async def access_l2cap_service(psm):
            connector = bumble_acl_connection.create_l2cap_channel(
                spec=ClassicChannelSpec(psm=psm)
            )
        ...
        try:
            _ = await asyncio.wait_for(hid_interrupt_access_tsk, timeout=5.0)
        except:
