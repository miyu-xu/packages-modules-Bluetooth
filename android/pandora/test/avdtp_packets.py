...
class ServiceCapability(Packet):
    ...
    def parse(span: bytes) -> Tuple["ServiceCapability", bytes]:
        ...
        try:
            ...
            pass
        try:
            child, remain = HeaderCompressionCabability.parse(fields.copy(), payload)
            assert len(remain) == 0
            return child, span
        except Exception as exn:
            pass
        try:
            child, remain = MultiplexingCabability.parse(fields.copy(), payload)
            assert len(remain) == 0
            return child, span
        except Exception as exn:
            pass
...
@dataclass
class HeaderCompressionCabability(ServiceCapability):
    ...
    @staticmethod
    def parse(fields: dict, span: bytes) -> Tuple["HeaderCompressionCabability", bytes]:
        ...
        span = span[1:]
        return HeaderCompressionCabability(**fields), span

    def serialize(self, payload: bytes = None) -> bytes:
        _span = bytearray()
        if self.recovery > 1:
            print(
                f"Invalid value for field HeaderCompressionCabability::recovery: {self.recovery} > 1; the value will be truncated"
            )
            self.recovery &= 1
        if self.media > 1:
            print(
                f"Invalid value for field HeaderCompressionCabability::media: {self.media} > 1; the value will be truncated"
            )
            self.media &= 1
        if self.back_ch > 1:
            print(
                f"Invalid value for field HeaderCompressionCabability::back_ch: {self.back_ch} > 1; the value will be truncated"
            )
            self.back_ch &= 1
...
@dataclass
class MultiplexingCabability(ServiceCapability):
    ...
    @staticmethod
    def parse(fields: dict, span: bytes) -> Tuple["MultiplexingCabability", bytes]:
        ...
        span = bytes([])
        fields["payload"] = payload
        return MultiplexingCabability(**fields), span

    def serialize(self, payload: bytes = None) -> bytes:
        _span = bytearray()
        if self.frag > 1:
            print(
                f"Invalid value for field MultiplexingCabability::frag: {self.frag} > 1; the value will be truncated"
            )
            self.frag &= 1
