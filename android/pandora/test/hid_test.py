...
from pandora_experimental.hid_pb2 import HID_HOST_CONNECTED, HID_HOST_DISCONNECTED, HID_REPORT_TYPE_OUTPUT
...
def on_set_report_cb(report_id: int, report_type: int, report_size: int, data: bytes):
    ...
    print("SET_REPORT report_id: " + str(report_id) + "report_type: " + str(report_type) + "report_size " +
...
class HidTest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    async def test_hid_connect_and_disconnect(self) -> None:
        ...
        connection = await hid.GetHostConnectionState(address=self.ref.address)
        assert_equal(connection.state, HID_HOST_CONNECTED)

        await hid.SendHostReport(
        ...
        connection = await hid.DisconnectHost(address=self.ref.address)
        assert_equal(connection.state, HID_HOST_DISCONNECTED)
