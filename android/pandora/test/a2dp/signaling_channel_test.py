...
async def test_signaling_channel_as_source():
    ...
    signaling_channel_initiator = SignalingChannel(connection)

    await signaling_channel_initiator.initiate_signaling_channel()

    signaling_channel_initiator.send_signal(DiscoverCommand())

    result = await signaling_channel_initiator.expect_signal(
    ...
    signaling_channel_initiator.send_signal(GetAllCapabilitiesCommand(acp_seid=acp_seid))
    ...
    signaling_channel_initiator.send_signal(
        SetConfigurationCommand(acp_seid=acp_seid, service_capabilities=[result.service_capabilities[0]]))

    result = await signaling_channel_initiator.expect_signal(SetConfigurationResponse(transaction_label=any))
    result.show()

    signaling_channel_initiator.send_signal(OpenCommand(acp_seid=acp_seid))

    result = await signaling_channel_initiator.expect_signal(OpenResponse(transaction_label=any))
    result.show()
    ...
    await signaling_channel_initiator.initiate_transport_channel()

    signaling_channel_initiator.send_signal(StartCommand(acp_seid=acp_seid))

    result = await signaling_channel_initiator.expect_signal(StartResponse(transaction_label=any))
    result.show()
    ...
    await pump.start(signaling_channel_initiator.transport_channel)

    await rtp_packets_fully_received

    await pump.stop()

    signaling_channel_initiator.send_signal(CloseCommand(acp_seid=acp_seid))

    result = await signaling_channel_initiator.expect_signal(CloseResponse(transaction_label=any))
    result.show()

    await signaling_channel_initiator.disconnect_transport_channel()
...
async def test_signaling_channel_as_sink():
    ...
    def on_rtp_packet(packet):
        logger.info(f" RTP Packet received")
    ...
    signaling_channel_acceptor = SignalingChannel(dev_1_dev_0_conn)
    signaling_channel_acceptor.accept_signaling_channel()
    ...
    signaling_channel_initiator = SignalingChannel(dev_0_dev_1_conn)
    await signaling_channel_initiator.initiate_signaling_channel()

    signaling_channel_initiator.send_signal(DiscoverCommand())

    rcv_discover_cmd = await signaling_channel_acceptor.expect_signal(DiscoverCommand(transaction_label=any))

    seid_information = [SeidInformation(tsep=Tsep.SINK, media_type=avdtp.AVDTP_AUDIO_MEDIA_TYPE)]

    signaling_channel_acceptor.send_signal(
        DiscoverResponse(transaction_label=rcv_discover_cmd.transaction_label, seid_information=seid_information))
    ...
    initiator_to_acceptor_seid = result.seid_information[0].acp_seid

    signaling_channel_initiator.send_signal(GetAllCapabilitiesCommand(acp_seid=initiator_to_acceptor_seid))

    rcv_get_caps_cmd = await signaling_channel_acceptor.expect_signal(
        GetAllCapabilitiesCommand(acp_seid=initiator_to_acceptor_seid, transaction_label=any))
    ...
    signaling_channel_initiator.send_signal(
        SetConfigurationCommand(acp_seid=initiator_to_acceptor_seid,
                                service_capabilities=[result.service_capabilities[0]]))

    rcv_set_conf_cmd = await signaling_channel_acceptor.expect_signal(
        SetConfigurationCommand(transaction_label=any,
                                acp_seid=initiator_to_acceptor_seid,
                                service_capabilities=[result.service_capabilities[0]]))
    ...
    signaling_channel_initiator.send_signal(OpenCommand(acp_seid=initiator_to_acceptor_seid))

    rcv_open_cmd = await signaling_channel_acceptor.expect_signal(
        OpenCommand(transaction_label=any, acp_seid=initiator_to_acceptor_seid))

    signaling_channel_acceptor.send_signal(OpenResponse(transaction_label=rcv_open_cmd.transaction_label))
    ...
    signaling_channel_initiator.send_signal(StartCommand(acp_seid=initiator_to_acceptor_seid))

    rcv_start_cmd = await signaling_channel_acceptor.expect_signal(
        StartCommand(transaction_label=any, acp_seid=initiator_to_acceptor_seid))

    signaling_channel_acceptor.send_signal(StartResponse(transaction_label=rcv_start_cmd.transaction_label))

    result = await signaling_channel_initiator.expect_signal(StartResponse(transaction_label=any))
    result.show()

    signaling_channel_initiator.send_media(source_packets[0])
    signaling_channel_initiator.send_media(source_packets[1])
    signaling_channel_initiator.send_media(source_packets[2])

    await rtp_packets_fully_received

    signaling_channel_initiator.send_signal(CloseCommand(acp_seid=initiator_to_acceptor_seid))

    rcv_close_cmd = await signaling_channel_acceptor.expect_signal(
        CloseCommand(transaction_label=any, acp_seid=initiator_to_acceptor_seid))

    signaling_channel_acceptor.send_signal(CloseResponse(transaction_label=rcv_close_cmd.transaction_label))

    result = await signaling_channel_initiator.expect_signal(CloseResponse(transaction_label=any))
    result.show()

    await signaling_channel_initiator.disconnect_transport_channel()
    await signaling_channel_initiator.disconnect_signaling_channel()
