...
Run the tests:
    python signaling_channel_test.py
"""
...
async def test_signaling_channel_as_source():
    ...
    success = await signaling_channel_initiator.initiate_signaling_channel()
    assert success

    signaling_channel_initiator.send_command(DiscoverCommand())

    result = await signaling_channel_initiator.expect_sig(
    ...
    signaling_channel_initiator.send_command(GetAllCapabilitiesCommand(acp_seid=acp_seid))
    ...
    signaling_channel_initiator.send_command(
        SetConfigurationCommand(acp_seid=acp_seid, service_capabilities=[result.service_capabilities[0]]))

    result = await signaling_channel_initiator.expect_sig(SetConfigurationResponse(transaction_label=any))
    result.show()

    signaling_channel_initiator.send_command(OpenCommand(acp_seid=acp_seid))

    result = await signaling_channel_initiator.expect_sig(OpenResponse(transaction_label=any))
    result.show()
    ...
    success = await signaling_channel_initiator.initiate_rtp_channel()
    assert success

    signaling_channel_initiator.send_command(StartCommand(acp_seid=acp_seid))

    result = await signaling_channel_initiator.expect_sig(StartResponse(transaction_label=any))
    result.show()
    ...
    await pump.start(signaling_channel_initiator.rtp_l2cap_channel)

    await rtp_packets_fully_received

    await pump.stop()

    signaling_channel_initiator.send_command(CloseCommand(acp_seid=acp_seid))

    result = await signaling_channel_initiator.expect_sig(CloseResponse(transaction_label=any))
    result.show()

    await signaling_channel_initiator.disconnect_rtp_channel()
...
async def test_signaling_channel_as_sink():
    ...
    signaling_channel_initiator.send_command(GetAllCapabilitiesCommand(acp_seid=initiator_to_acceptor_seid))
    ...
    signaling_channel_initiator.send_command(OpenCommand(acp_seid=initiator_to_acceptor_seid))

    rcv_open_cmd = await signaling_channel_acceptor.expect_sig(
    ...
    signaling_channel_initiator.send_command(StartCommand(acp_seid=initiator_to_acceptor_seid))
    ...
    signaling_channel_initiator.send_command(CloseCommand(acp_seid=initiator_to_acceptor_seid))

    rcv_close_cmd = await signaling_channel_acceptor.expect_sig(
        CloseCommand(transaction_label=any, acp_seid=initiator_to_acceptor_seid))

    signaling_channel_acceptor.send_message(CloseResponse(transaction_label=rcv_close_cmd.transaction_label))
