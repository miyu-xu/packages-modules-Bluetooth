...
class SignalingChannel(EventEmitter):
    ...
    def accept_signaling_channel(self):
    ...
    def _on_pdu(self, pdu: bytes):
        self.sig_queue.put_nowait(pdu)

    def _on_avdtp_packet(self, packet):
        logger.debug(f"RTP Packet: {packet.hex()}")
        self.emit('rtp_packet', packet)

    async def _receive_signal(self) -> bytes:
        return await self.sig_queue.get()

    async def expect_signal(self, expected_sig: Union[SignalingPacket, type], timeout: float = 3) -> SignalingPacket:
        packet = await asyncio.wait_for(self._receive_signal(), timeout=timeout)
        ...
        if isinstance(expected_sig, type) and not isinstance(sig, expected_sig):
            ...
            sig.show()
            raise ValueError(f"Received unexpected event")
    ...
    def send_media(self, packet: avdtp.MediaPacket):
        self.transport_channel.send_pdu(bytes(packet))
