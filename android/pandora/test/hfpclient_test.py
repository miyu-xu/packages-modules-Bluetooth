...
class HfpClientTest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    async def make_hfp_connection(self) -> HfpProtocol:
        ...
        # Setup SDP records
        self.ref.device.sdp_service_records = make_sdp_records(HFP_VERSION_1_7, channel_number, 0)
...
def make_sdp_records(hfp_version: int, rfcomm_channel: int, ag_sdp_features: int) -> Dict[int, List[ServiceAttribute]]:
    return {
        0x00010001: [
            ServiceAttribute(
                SDP_SERVICE_RECORD_HANDLE_ATTRIBUTE_ID,
