...
from bumble.core import BT_HANDSFREE_SERVICE, BT_RFCOMM_PROTOCOL_ID
...
class HfpClientTest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    @avatar.parameterized(
        (True, False),)
    @avatar.asynchronous
    async def test_hf_indicator_setup(self, enhanced_driver_enabled) -> None:
        ...
        if enhanced_driver_enabled:
            self.dut.device.adb.shell(['setprop', PROPERTY_HF_INDICATOR_ENHANCED_DRIVER_SAFETY, 'true'])
        else:
            self.dut.device.adb.shell(['setprop', PROPERTY_HF_INDICATOR_ENHANCED_DRIVER_SAFETY, 'false'])
