...
from pandora.host_pb2 import RANDOM, DataTypes, Connection
from pandora_experimental.gatt_grpc_aio import GATT
from pandora_experimental.gatt_pb2 import SUCCESS
...
<<<<<<< PATCH SET (a571c7 Add Avatar test for b/271052144)
        dut, ref = self.devices
        assert isinstance(ref, BumbleDevice)
        self.dut, self.ref = dut, ref
=======
        self.dut, self.ref, *_ = self.devices
>>>>>>> BASE      (3a2eac Merge "BluetoothMetrics: Upload L2CAP CoC Server Metrics")
    ...
    async def test_read_characteristic_while_pairing(self) -> None:
        ...
        self.ref.device.pairing_config_factory = lambda _: PairingConfig(sc=True, mitm=False, bonding=True
                                                                        )  # type: ignore
        ...
        dut_gatt = GATT(self.dut.aio.channel)
        ...
        # android pops up a second pairing notif for some reason, accept it
        event = await anext(dut_pairing_events)
        dut_pairing_events.send_nowait(PairingEventAnswer(event=event, confirm=True))
