...
from bumble.gatt import Characteristic, Service, GATT_VOLUME_CONTROL_SERVICE, GATT_AUDIO_INPUT_CONTROL_SERVICE, GATT_PRIMARY_SERVICE_ATTRIBUTE_TYPE, GATT_SECONDARY_SERVICE_ATTRIBUTE_TYPE, UUID
...
class GattTest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    def test_discover_included_service2(self) -> None:
        ...
        self.ref.device.add_service(
            Service(PRIMARY_SERVICE_UUID, [], included_services=[Service(INCLUDED_SERVICE_UUID, [],
    ...
    @avatar.parameterized((GATT_PRIMARY_SERVICE_ATTRIBUTE_TYPE,), (GATT_SECONDARY_SERVICE_ATTRIBUTE_TYPE,))
    def test_discover_included_service(self, attribute_type: UUID) -> None:

        if attribute_type == GATT_SECONDARY_SERVICE_ATTRIBUTE_TYPE:
            raise signals.TestSkip("b/367467331: secondary services are not reported properly")

        PRIMARY_SERVICE_UUID = GATT_VOLUME_CONTROL_SERVICE
        INCLUDED_SERVICE_UUID = GATT_AUDIO_INPUT_CONTROL_SERVICE
        is_primary_service = True if attribute_type == GATT_PRIMARY_SERVICE_ATTRIBUTE_TYPE else GATT_SECONDARY_SERVICE_ATTRIBUTE_TYPE
        self.ref.device.add_service(
            Service(PRIMARY_SERVICE_UUID, [],
                    included_services=[Service(INCLUDED_SERVICE_UUID, [], primary=is_primary_service)]))  # type:ignore
