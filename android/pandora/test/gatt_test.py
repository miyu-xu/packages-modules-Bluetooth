...
class GattTest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    @avatar.asynchronous
    async def test_dut_remote_device_name(self) -> None:
        '''
        ...
        print("------------------------")
        print(dut.address_asdict())
        print(f"dut_address 1: {self.dut.address}")
        dut_address = Address(dut.address_asdict()['random'], Address.RANDOM_DEVICE_ADDRESS)
        print(f"dut_address 2: {dut_address.to_string()}")

        # ref_acl_connection = self.ref.device.find_connection_by_bd_addr(dut_address)
        ...
        assert_true(device_name.startswith("Cuttlefish x86"), "remote device name not correct")
