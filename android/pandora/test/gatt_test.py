...
class GattTest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    def test_discover_included_service(self, attribute_type: UUID) -> None:
        ...
        is_primary_service = True if attribute_type == GATT_PRIMARY_SERVICE_ATTRIBUTE_TYPE else False
        self.ref.device.add_service(  # type: ignore
            Service(PRIMARY_SERVICE_UUID, [],
                    included_services=[Service(INCLUDED_SERVICE_UUID, [], primary=is_primary_service)]))
