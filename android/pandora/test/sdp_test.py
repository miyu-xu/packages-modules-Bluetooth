...
class Sdp_Test(base_test.BaseTestClass):  # type: ignore[misc]
    '''
    ...
    async def test_sdp_connect_and_disconnect(self) -> None:
        ...
        connection = self.ref.device.lookup_connection(connection_handle)

        # Connect to the SDP Server
        self.ref.log.info(f'Connecting to SDP Server')
        sdp_client = SDP_Client(self.ref.device)
    ...
    async def test_sdp_list_services_and_attributes(self) -> None:
        ...
        assert bool(service_record_handles)
        ...
        for service_record_handle in service_record_handles:
            ...
            for attribute in attributes:
                if (attribute.id == SDP_SERVICE_CLASS_ID_LIST_ATTRIBUTE_ID):
                    if (ServiceAttribute.is_uuid_in_value(BT_AUDIO_SOURCE_SERVICE, attribute.value)):
                        service_found = True
