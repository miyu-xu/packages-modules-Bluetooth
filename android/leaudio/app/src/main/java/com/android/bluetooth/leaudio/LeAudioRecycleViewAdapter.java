...
public class LeAudioRecycleViewAdapter
    ...
    // As we scroll this methods rebinds devices bellow to our ViewHolders which are reused when
    ...
    private void setHasStateObservers(@NonNull ViewHolder holder,
        ...
        if (hap_svc_data != null) {
            ...
            hap_svc_data.hapFeaturesMutable.observe(this.parent, features -> {
                BitSet feature_set = BitSet.valueOf(new long[] {features});

                int hearing_aid_type_idx =
                        feature_set.get(BluetoothHapClient.FEATURE_BIT_NUM_TYPE_MONAURAL) ? 0
                                : (feature_set.get(BluetoothHapClient.FEATURE_BIT_NUM_TYPE_BANDED)
                                        ? 1
                                        : 2);
                ...
                String preset_synchronization_support = this.parent.getResources()
                        .getStringArray(R.array.preset_synchronization_support)[(feature_set
                                .get(BluetoothHapClient.FEATURE_BIT_NUM_SYNCHRONIZATED_PRESETS) ? 1
                                        : 0)];
                String independent_presets =
                        this.parent.getResources()
                                .getStringArray(R.array.independent_presets)[feature_set.get(
                                        BluetoothHapClient.FEATURE_BIT_NUM_INDEPENDENT_PRESETS) ? 1
                                                : 0];
                String dynamic_presets =
                        this.parent.getResources()
                                .getStringArray(R.array.dynamic_presets)[feature_set
                                        .get(BluetoothHapClient.FEATURE_BIT_NUM_DYNAMIC_PRESETS) ? 1
                                                : 0];
                String writable_presets_support =
                        this.parent.getResources()
                                .getStringArray(R.array.writable_presets_support)[feature_set.get(
                                        BluetoothHapClient.FEATURE_BIT_NUM_WRITABLE_PRESETS) ? 1
                                                : 0];
