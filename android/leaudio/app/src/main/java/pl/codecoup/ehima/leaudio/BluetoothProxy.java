...
public class BluetoothProxy {
    ...
    private IntentFilter adapterIntentFilter;
    ...
    private Map<Integer, UUID> mGroupLocks = new HashMap<>();

    private MutableLiveData<Boolean> enabledBluetoothMutable;
    private final BroadcastReceiver adapterIntentReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction();
            if (BluetoothAdapter.ACTION_STATE_CHANGED.equals(action)) {
                int toState = intent.getIntExtra(BluetoothAdapter.EXTRA_STATE, BluetoothAdapter.ERROR);
    ...
    private MutableLiveData<List<LeAudioDeviceStateWrapper>> allLeAudioDevicesMutable;
                ...
                if (device != null) {
                    Optional<LeAudioDeviceStateWrapper> valid_device_opt =
                            allLeAudioDevicesMutable.getValue().stream()
                                    .filter(state -> state.device.getAddress().equals(device.getAddress())).findAny();
                    ...
                    if (valid_device_opt.isPresent()) {
                        ...
                        switch (action) {
                            case BluetoothLeAudio.ACTION_LE_AUDIO_CONNECTION_STATE_CHANGED: {
                                final int toState = intent.getIntExtra(BluetoothLeAudio.EXTRA_STATE, -1);
                                if (toState == BluetoothLeAudio.STATE_CONNECTED || toState == BluetoothLeAudio.STATE_DISCONNECTED)
                                    svc_data.isConnectedMutable.postValue(toState == BluetoothLeAudio.STATE_CONNECTED);

                                group_id = bluetoothLeAudio.getGroupId(device);
                                svc_data.nodeStatusMutable.setValue(new Pair<>(group_id, BluetoothLeAudio.GROUP_NODE_ADDED));
                                svc_data.groupStatusMutable.setValue(new Pair<>(group_id, new Pair<>(-1, -1)));
                                break;
                            }
                            case BluetoothLeAudio.ACTION_LE_AUDIO_GROUP_NODE_STATUS_CHANGED:
                                group_id = intent.getIntExtra(BluetoothLeAudio.EXTRA_LE_AUDIO_GROUP_ID, BluetoothLeAudio.GROUP_ID_INVALID);
                                final int node_status = intent.getIntExtra(BluetoothLeAudio.EXTRA_LE_AUDIO_GROUP_NODE_STATUS, -1);
                                svc_data.nodeStatusMutable.setValue(new Pair<>(group_id, node_status));
                                svc_data.groupStatusMutable.setValue(new Pair<>(group_id, new Pair<>(-1, -1)));
                                break;
                        }
                    }
                } else {
                    final int group_id = intent.getIntExtra(BluetoothLeAudio.EXTRA_LE_AUDIO_GROUP_ID, BluetoothLeAudio.GROUP_ID_INVALID);
                    final int group_status = intent.getIntExtra(BluetoothLeAudio.EXTRA_LE_AUDIO_GROUP_STATUS, -1);
                    ...
                    if (group_id != BluetoothLeAudio.GROUP_ID_INVALID)
                        valid_devices = allLeAudioDevicesMutable.getValue().stream()
                                            .filter(state -> state.leAudioData.nodeStatusMutable.getValue() != null
                                                    && state.leAudioData.nodeStatusMutable.getValue().first.equals(group_id)).collect(Collectors.toList());
                        ...
                        switch (action) {
                            case BluetoothLeAudio.ACTION_LE_AUDIO_GROUP_STATUS_CHANGED:
                                for (LeAudioDeviceStateWrapper dev : valid_devices) {
                                    dev.leAudioData.groupStatusMutable.setValue(new Pair<>(group_id, new Pair<>(group_status, 0)));
                                }
                                break;
                        }
                    }
                    ...
                    if (valid_device_opt.isPresent()) {
                        ...
                        switch (action) {
                            case BluetoothHapClient.ACTION_HAP_CONNECTION_STATE_CHANGED: {
                                final int toState = intent.getIntExtra(BluetoothHapClient.EXTRA_STATE, -1);
                                svc_data.hapStateMutable.postValue(toState);
                                break;
                            }
                            case BluetoothHapClient.ACTION_HAP_DEVICE_AVAILABLE: {
                                final int features = intent.getIntExtra(BluetoothHapClient.EXTRA_HAP_FEATURES, -1);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (allLeAudioDevicesMutable.getValue() != null) {
                if (device != null) {
                    ...
                    if (valid_device_opt.isPresent()) {
                        LeAudioDeviceStateWrapper valid_device = valid_device_opt.get();
                        LeAudioDeviceStateWrapper.VolumeControlData svc_data = valid_device.volumeControlData;

                        switch (action) {
                            case BluetoothVolumeControl.ACTION_CONNECTION_STATE_CHANGED:
                                final int toState = intent.getIntExtra(BluetoothVolumeControl.EXTRA_STATE, -1);
                                if (toState == BluetoothVolumeControl.STATE_CONNECTED || toState == BluetoothVolumeControl.STATE_DISCONNECTED)
                                    svc_data.isConnectedMutable.postValue(toState == BluetoothVolumeControl.STATE_CONNECTED);
                                break;
                        }
                    }
                }
    ...
    public void initProfiles() {
        ...
        hapCallback = new BluetoothHapClient.Callback() {
            @Override
            public void onActivePresetChanged(BluetoothDevice device, int presetIndex) {
                Optional<LeAudioDeviceStateWrapper> valid_device_opt =
                allLeAudioDevicesMutable.getValue().stream()
                        .filter(state -> state.device.getAddress().equals(device.getAddress())).findAny();
            ...
            @Override
            public void onSelectActivePresetFailed(BluetoothDevice device, int statusCode) {
                Optional<LeAudioDeviceStateWrapper> valid_device_opt =
                        allLeAudioDevicesMutable.getValue().stream()
                        .filter(state -> state.device.getAddress().equals(device.getAddress())).findAny();
                ...
                svc_data.hapStatusMutable.postValue("Select preset failed with status "
            ...
            public void onSelectActivePresetForGroupFailed(int hapGroupId, int statusCode) {
                ...
                if (hapGroupId != BluetoothCsipSetCoordinator.GROUP_ID_INVALID)
                    valid_devices = allLeAudioDevicesMutable.getValue().stream()
                        .filter(state -> state.leAudioData.nodeStatusMutable.getValue() != null
                                && state.leAudioData.nodeStatusMutable.getValue().first.equals(hapGroupId)).collect(Collectors.toList());

                if (valid_devices != null) {
                    for (LeAudioDeviceStateWrapper device : valid_devices) {
                        device.hapData.hapStatusMutable.postValue(
                                "Select preset for group " + String.valueOf(hapGroupId)
                                + " failed with status " + String.valueOf(statusCode));
                    }
                }
            }
            ...
            public void onPresetInfoChanged(BluetoothDevice device,
                ...
                svc_data.hapStatusMutable.postValue(
                        "Preset list changed due to status " + String.valueOf(statusCode));
                svc_data.hapPresetsMutable.postValue(presetInfoList);
            }

            @Override
            public void onHapFeaturesAvailable(BluetoothDevice device, int hapFeatures) {
                Optional<LeAudioDeviceStateWrapper> valid_device_opt =
                allLeAudioDevicesMutable.getValue().stream()
                        .filter(state -> state.device.getAddress().equals(device.getAddress())).findAny();
                ...
                svc_data.hapStatusMutable.postValue(
                        "Hap features changed: " + String.valueOf(hapFeatures));
                svc_data.hapFeaturesMutable.postValue(hapFeatures);
            }
            ...
            public void onSetPresetNameFailed(BluetoothDevice device, int status) {
                ...
                svc_data.hapStatusMutable.postValue("Name set error: "
            ...
            public void onSetPresetNameForGroupFailed(int hapGroupId, int status) {
                ...
                if (valid_devices != null) {
                    for (LeAudioDeviceStateWrapper device : valid_devices) {
                        device.hapData.hapStatusMutable.postValue("Group Name set error: "
                                + String.valueOf(status));
                    }
                }
            }
    ...
    private void initLeAudioProxy() {
        if (bluetoothLeAudio == null) {
            bluetoothAdapter.getProfileProxy(this.application, profileListener, BluetoothProfile.LE_AUDIO);
        }
    ...
    private void initVolumeControlProxy() {
        bluetoothAdapter.getProfileProxy(this.application, profileListener, BluetoothProfile.VOLUME_CONTROL);
    ...
    private void cleanupVolumeControlProxy() {
        if (bluetoothVolumeControl != null) {
            bluetoothAdapter.closeProfileProxy(BluetoothProfile.VOLUME_CONTROL, bluetoothVolumeControl);
            application.unregisterReceiver(volumeControlIntentReceiver);
        }
    }

    private void initHapProxy() {
        bluetoothAdapter.getProfileProxy(this.application, profileListener, BluetoothProfile.HAP_CLIENT);

        intentFilter = new IntentFilter();
        intentFilter.addAction(BluetoothHapClient.ACTION_HAP_CONNECTION_STATE_CHANGED);
        intentFilter.addAction(BluetoothHapClient.ACTION_HAP_DEVICE_AVAILABLE);
    ...
    public void queryLeAudioDevices() {
        if (checkForEnabledBluetooth()) {
            ...
            for (BluetoothDevice dev : bluetoothAdapter.getBondedDevices()) {
                ...
                if (Arrays.asList(dev.getUuids() != null ? dev.getUuids() : new ParcelUuid[0]).contains(ParcelUuid.fromString(application.getString(R.string.svc_uuid_le_audio)))) {
                    if (state_wrapper.leAudioData == null)
                        state_wrapper.leAudioData = new LeAudioDeviceStateWrapper.LeAudioData();
                    valid_device = true;

                    if (bluetoothLeAudio != null) {
                        state_wrapper.leAudioData.isConnectedMutable.postValue(bluetoothLeAudio.getConnectionState(dev) == BluetoothLeAudio.STATE_CONNECTED ? true : false);
                        int group_id = bluetoothLeAudio.getGroupId(dev);
                        state_wrapper.leAudioData.nodeStatusMutable.setValue(new Pair<>(group_id, BluetoothLeAudio.GROUP_NODE_ADDED));
                        state_wrapper.leAudioData.groupStatusMutable.setValue(new Pair<>(group_id, new Pair<>(-1, -1)));
                    }
                }

                if (Arrays.asList(dev.getUuids() != null ? dev.getUuids() : new ParcelUuid[0]).contains(ParcelUuid.fromString(application.getString(R.string.svc_uuid_volume_control)))) {
                    if (state_wrapper.volumeControlData == null)
                        state_wrapper.volumeControlData = new LeAudioDeviceStateWrapper.VolumeControlData();
                    valid_device = true;

                    if (bluetoothVolumeControl != null) {
                        state_wrapper.volumeControlData.isConnectedMutable.postValue(bluetoothVolumeControl.getConnectionState(dev) == BluetoothVolumeControl.STATE_CONNECTED ? true : false);
                        // FIXME: We dont have the api to get the volume and mute states? :(
                    }
                }

                if (Arrays.asList(dev.getUuids() != null ? dev.getUuids() : new ParcelUuid[0]).contains(ParcelUuid.fromString(application.getString(R.string.svc_uuid_has)))) {
                    if (state_wrapper.hapData == null)
                        state_wrapper.hapData = new LeAudioDeviceStateWrapper.HapData();
                    valid_device = true;

                    if (bluetoothHapClient != null) {
                        state_wrapper.hapData.hapStateMutable.postValue(bluetoothHapClient
                                .getConnectionState(dev));
                        boolean is_connected = bluetoothHapClient.getConnectionState(dev) ==
                                BluetoothHapClient.STATE_CONNECTED ? true : false;
                        if (is_connected) {
                            // This is a hidden API
                            // state_wrapper.hapData.hapFeaturesMutable
                            //         .postValue(bluetoothHapClient.getFeatures(dev));
                            ...
                            state_wrapper.hapData.hapActivePresetIndexMutable
                                    .postValue(bluetoothHapClient.getActivePresetIndex(dev));
                        }
                    }
                }
    ...
    public void connectLeAudio(BluetoothDevice device, boolean connect) {
        if (bluetoothLeAudio != null) {
            if (connect) {
                ...
                bluetoothLeAudio.disconnect(device);
            }
        }
    }
    ...
    public void groupSet(BluetoothDevice device, Integer group_id) {
        if (bluetoothLeAudio != null)
            bluetoothLeAudio.groupAddNode(group_id, device);
    }

    public void groupUnset(BluetoothDevice device, Integer group_id) {
        if (bluetoothLeAudio != null)
            bluetoothLeAudio.groupRemoveNode(group_id, device);
    }
    ...
    public void groupSetLock(Integer group_id, boolean lock) {
        ...
        if (lock) {
            ...
            UUID uuid = bluetoothCsis.groupLock(group_id, mExecutor,
                ...
                if (((op_status == BluetoothStatusCodes.SUCCESS) ||
                    ...
                    allLeAudioDevicesMutable.getValue().forEach((dev_wrapper) -> {
                        if (dev_wrapper.leAudioData.nodeStatusMutable.getValue() != null
                                && dev_wrapper.leAudioData.nodeStatusMutable.getValue().first.equals(group_id)) {
                            dev_wrapper.leAudioData.groupLockStateMutable.postValue(new Pair<Integer, Boolean>(group, is_locked));
                        }
                    });
                } else {
                    // TODO: Set error status so it could be notified/toasted to the user
                }
            ...
            bluetoothCsis.groupUnlock(mGroupLocks.get(group_id));
            mGroupLocks.remove(group_id);
        }
    }
    ...
    public boolean hapReadPresetInfo(BluetoothDevice device, int preset_index) {
        ...
        BluetoothHapPresetInfo new_preset =  bluetoothHapClient.getPresetInfo(device, preset_index);
        if (new_preset == null)
            return false;
    ...
    public boolean hapPreviousGroupPreset(int group_id) {
        ...
        bluetoothHapClient.switchToPreviousPresetForGroup(group_id);
        return true;
    }
    ...
    public boolean hapNextGroupPreset(int group_id) {
        ...
        bluetoothHapClient.switchToNextPresetForGroup(group_id);
        return true;
    }
    ...
    public int hapGetHapGroup(BluetoothDevice device) {
        ...
        return bluetoothHapClient.getHapGroup(device);
    }

    private MutableLiveData<BluetoothLeBroadcastMetadata> mBroadcastUpdateMutableLive;
    private MutableLiveData<Pair<Integer /*reason*/, Integer /*broadcastId*/>>
            mBroadcastPlaybackStartedMutableLive;
    private MutableLiveData<Pair<Integer /*reason*/, Integer /*broadcastId*/>>
            mBroadcastPlaybackStoppedMutableLive;
    private MutableLiveData<Integer /*broadcastId*/> mBroadcastAddedMutableLive;
    private MutableLiveData<Pair<Integer /*reason*/, Integer /*broadcastId*/>>
            mBroadcastRemovedMutableLive;

    private MutableLiveData<String> mBroadcastStatusMutableLive;
    ...
    private final BluetoothLeBroadcast.Callback mBroadcasterCallback = new BluetoothLeBroadcast.Callback() {
        @Override
        public void onBroadcastStarted(int reason, int broadcastId) {
            if ((reason != BluetoothStatusCodes.REASON_LOCAL_APP_REQUEST) &&
                ...
                mBroadcastStatusMutableLive.postValue("Unable to create broadcast: "
                        + String.valueOf(broadcastId) + ", reason: " + String.valueOf(reason));
            }
        ...
        public void onBroadcastStartFailed(int reason) {
            mBroadcastStatusMutableLive.postValue("Unable to START broadcast due to reason: "
        ...
        public void onBroadcastStopFailed(int reason) {
            mBroadcastStatusMutableLive.postValue("Unable to STOP broadcast due to reason: "
                    + String.valueOf(reason));
        }
        ...
        public void onBroadcastUpdated(int reason, int broadcastId) {
            mBroadcastStatusMutableLive.postValue("Broadcast " + String.valueOf(broadcastId)
                    + "has been updated due to reason: " + String.valueOf(reason));
        }

        @Override
        public void onBroadcastUpdateFailed(int reason, int broadcastId) {
            mBroadcastStatusMutableLive.postValue("Unable to UPDATE broadcast "
                    + String.valueOf(broadcastId) + " due to reason: " + String.valueOf(reason));
        }
    ...
    public int getMaximumNumberOfBroadcast() {
        ...
        return mBluetoothLeBroadcast.getMaximumNumberOfBroadcast();
    }
