...
public class BluetoothManagerService extends IBluetoothManager.Stub {
    ...
    public int getState() {
        if (!isCallerSystem() && !checkIfCallerIsForegroundUser()) {
    ...
    private boolean checkBluetoothPermissions(AttributionSource attributionSource, String message,
        ...
        if (!isCallerSystem() && !isCallerShell() && !isCallerRoot()) {
    ...
    private static boolean isCallerShell() {
        return UserHandle.getAppId(Binder.getCallingUid()) == Process.SHELL_UID;
    }
    private static boolean isCallerRoot() {
        return UserHandle.getAppId(Binder.getCallingUid()) == Process.ROOT_UID;
    }
