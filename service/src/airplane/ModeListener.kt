...
internal class ToastNotification private constructor() {
    companion object {
        ...
        fun displayIfNeeded(resolver: ContentResolver, getUser: () -> Context) {
            ...
            Log.e(
                TAG,
                "WILLIAM - DO NOT SUBMIT - displayToastNotificationIfNeeded attempt on userContext"
            )
            ...
            Log.e(
                TAG,
                "WILLIAM - DO NOT SUBMIT - displayToastNotificationIfNeeded attempt on userContext for Toast with text" +
                    text
            )
...
// @kotlin.time.ExperimentalTime private val timeSource = TimeSource.Monotonic
...
private fun isApmEnhancementEnabled(resolver: ContentResolver): Boolean {
    return Settings.Global.getInt(resolver, APM_ENHANCEMENT, DEFAULT_APM_ENHANCEMENT_STATE) == 1
}

/** Airplane Enhancement Mode: Return true if the wifi should stays on during airplane mode */
private fun isWifiOnApm(resolver: ContentResolver, getUser: () -> Context): Boolean {
...
private fun hasUserToggledApm(getUser: () -> Context): Boolean {
    return Settings.Secure.getInt(getUser().contentResolver, APM_USER_TOGGLED_BLUETOOTH, 0) == 1
}

/** Airplane Enhancement Mode: Return true if the bluetooth should stays on during airplane mode */
private fun isBluetoothOnAPM(getUser: () -> Context): Boolean {
    return Settings.Secure.getInt(getUser().contentResolver, BLUETOOTH_APM_STATE, 0) == 1
}
