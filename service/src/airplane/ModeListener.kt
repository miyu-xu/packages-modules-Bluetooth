...
public fun initialize(
    ...
    setApmEnhancementDefaultValue(systemResolver)
...
/**
 * Wifi got support for "Airplane Enhancement Mode" prior to Bluetooth. In order for Wifi to be
 * aware that Bluetooth also support the feature, Bluetooth need to set the APM_ENHANCEMENT settings
 * to 1. This is done only if the APM_ENHANCEMENT settings is not already set.
 */
private fun setApmEnhancementDefaultValue(resolver: ContentResolver) {
    if (Settings.Global.getInt(resolver, APM_ENHANCEMENT, -1) != -1) {
        // The property has already been set, do not override the current value
        return
    }
