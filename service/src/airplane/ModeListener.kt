...
//     private get
//     set(value) = {
//         notificationManager.createNotificationChannels(value);
//         field = value
//     }
...
private fun updateMode(resolver: ContentResolver): Boolean {
    ...
    isOn =
        if (isSensitive(resolver)) {
            isGlobalModeOn(resolver) && enhancementMode.isOn(resolver)
        } else {
...
private lateinit var enhancementMode: EnhancementMode

private class EnhancementMode {
    private var lastEnabledTime: Long = 0
    private var isBluetoothOnBeforeApmToggle = false
    private var isBluetoothOnAfterApmToggle = false
    private var isMediaProfileConnectedBeforeApmToggle = false
    ...
    fun isOn(resolver: ContentResolver): Boolean {
        ...
        isBluetoothOnAfterApmToggle = shouldSkipAirplaneModeChange(resolver)
    ...
    private fun shouldSkipAirplaneModeChange(resolver: ContentResolver): Boolean {
        ...
        if (isApmEnhancementEnabled(resolver) && isBluetoothToggledOnApm(resolver)) {
            // return mAirplaneHelper.isBluetoothOn() && mAirplaneHelper.isBluetoothOnAPM()
        }
        ...
        // return mAirplaneHelper.isBluetoothOn() && mAirplaneHelper.isMediaProfileConnected()
        return false // TODO
    }
}
...
private fun isApmEnhancementEnabled(resolver: ContentResolver): Boolean {
    return Settings.Global.getInt(resolver, APM_ENHANCEMENT, 0) == 1
}

private fun isBluetoothToggledOnApm(resolver: ContentResolver): Boolean {
    // TODO(b/?): Should we really use user context?
    return Settings.Secure.getInt(resolver, APM_USER_TOGGLED_BLUETOOTH, 0) == 1
}
...
//             Log.e(
//                     TAG,
//                     "Unable to find Bluetooth package name with " + "APM notification
// resources");
//             return;
//         }
//         Resources resources =
...
//         if (isApmEnhancementEnabled()) {
//             setSettingsSecureInt(BLUETOOTH_APM_STATE, isOn ? BLUETOOTH_ON_APM :
// BLUETOOTH_OFF_APM);
