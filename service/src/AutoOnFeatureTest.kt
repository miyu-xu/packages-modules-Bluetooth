...
class AutoOnFeatureTest {
    ...
    private fun expectStorageTime() {
        shadowOf(looper).idle()
        val storageString = Settings.Secure.getString(resolver, Timer.STORAGE_KEY)
        assertThat(storageString).isNotNull()
        expect.that(LocalDateTime.parse(storageString)).isEqualTo(timerTarget)
    }

    private fun expectNoStorageTime() {
        shadowOf(looper).idle()
        assertThat(Settings.Secure.getString(resolver, Timer.STORAGE_KEY)).isNull()
    }
    ...
    fun setupTimer_whenBtOffAndUserEnabled_isScheduled() {
        ...
        expect.that(Timer.timer).isNotNull()
    }
    ...
    @Test
    fun registerHiddenListener_whenNothing_is() {
    ...
    @Test
    fun registerHiddenListener_whenAlreadyRegistered_is() {
        registerHiddenApiListener(looper, context, state, this::callback_on)
