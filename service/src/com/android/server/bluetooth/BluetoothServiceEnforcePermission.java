...
class BluetoothServiceEnforcePermission {
    private static final String TAG = BluetoothServiceEnforcePermission.class.getSimpleName();
    private static final boolean DBG = Log.isLoggable(TAG, Log.DEBUG);
    private static final boolean VDBG = Log.isLoggable(TAG, Log.VERBOSE);

    static int sSystemUiUid;

    private BluetoothServiceEnforcePermission() {}
    ...
    private static boolean isPrivileged(Context ctx, int pid, int uid) {
        return (ctx.checkPermission(BLUETOOTH_PRIVILEGED, pid, uid) == PERMISSION_GRANTED)
                || (ctx.checkPermission(NETWORK_SETTINGS, pid, uid) == PERMISSION_GRANTED)
    ...
    private static boolean isProfileOwner(Context ctx, int uid, String packageName) {
        Context userContext;
        try {
            userContext = ctx.createPackageContextAsUser(ctx.getPackageName(), 0,
    ...
    private static Pair<UserHandle, ComponentName> getDeviceOwner(Context ctx) {
    ...
    private static boolean isDeviceOwner(Context ctx, int uid, String packageName) {
        ...
        Pair<UserHandle, ComponentName> deviceOwner = getDeviceOwner(ctx);
    ...
    private static boolean isSystem(Context ctx, String packageName, int uid) {
        long ident = Binder.clearCallingIdentity();
        try {
            ApplicationInfo info = ctx.getPackageManager().getApplicationInfoAsUser(
    ...
    static boolean checkConnectPermissionForDataDelivery(Context ctx,
            AttributionSource source, String message) {
        final String permission = BLUETOOTH_CONNECT;
        PermissionManager pm = ctx.getSystemService(PermissionManager.class);
        if (pm == null) {
            return false;
        }
        AttributionSource currentSource =
                new AttributionSource.Builder(ctx.getAttributionSource())
                        ...
                        .build();
        final int result =
                pm.checkPermissionForDataDeliveryFromDataSource(permission, currentSource, message);
    ...
    private static boolean isCallerSystemUi(int callingAppId) {
        return callingAppId == sSystemUiUid;
    }
    ...
    static boolean checkIfCallerIsForegroundUser(UserManager userManager) {
        final int callingUid = Binder.getCallingUid();
        final UserHandle callingUser = UserHandle.getUserHandleForUid(callingUid);
        final long callingIdentity = Binder.clearCallingIdentity();
        final UserHandle parentUser = userManager.getProfileParent(callingUser);
        ...
        try {
            ...
            valid = callingUser == foregroundUser || parentUser == foregroundUser
                    || isCallerNfc(callingAppId) || isCallerSystemUi(callingAppId)
    ...
    @RequiresPermission(BLUETOOTH_CONNECT)
    private static boolean checkBluetoothPermissions(Context ctx, AttributionSource source,
            UserManager userManager, AppOpsManager appOpsManager, String message,
        ...
        if (!checkConnectPermissionForDataDelivery(ctx, source, message)) {
            return false;
        }
        return true;
    }
    ...
    static String callerCanToggle(Context ctx, AttributionSource source, UserManager userManager,
            AppOpsManager appOpsManager, String message, boolean requireForeground) {
        if (isBluetoothDisallowed(userManager)) {
            return "Bluetooth is not allowed";
        }

        if (!checkBluetoothPermissions(
                    ctx, source, userManager, appOpsManager, message, requireForeground)) {
