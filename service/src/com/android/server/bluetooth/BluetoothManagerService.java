...
class BluetoothManagerService {
    ...
    BluetoothManagerService(Context context,
            Function<Intent, ComponentName> getComponentForIntent) {
        mContext = context;
        mUserManager = requireNonNull(mContext.getSystemService(UserManager.class),
                "UserManager system service cannot be null");
        final AppOpsManager appOps = requireNonNull(mContext.getSystemService(AppOpsManager.class),
                "AppOpsManager system service cannot be null");

        mBinder = new BluetoothServiceBinder(this, context, mUserManager, appOps);
        ...
        BluetoothServiceEnforcePermission.sSystemUiUid = mSystemUiUid;
    ...
    boolean enableBle(String packageName, IBinder token) throws RemoteException {
        if (isAirplaneModeOn()) {
    ...
    boolean enable(String packageName) throws RemoteException {
        if (isSatelliteModeOn()) {
    ...
    boolean disable(String packageName, boolean persist) throws RemoteException {
        if (isSatelliteModeOn()) {
    ...
    boolean bindBluetoothProfileService(int bluetoothProfile, String serviceName,
    ...
    void unbindBluetoothProfileService(int bluetoothProfile,
            IBluetoothProfileServiceConnection proxy) {
        synchronized (mProfileServices) {
