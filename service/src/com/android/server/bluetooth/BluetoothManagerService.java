...
class BluetoothManagerService {
    ...
    IBluetooth registerAdapter(IBluetoothManagerCallback callback) {
        ...
        AdapterBinder binder = mAdapter;
        return binder != null ? binder.getAdapterBinder() : null;
    }
    ...
    private void adapterEnable() {
        if (!mState.oneOf(STATE_OFF)) {
            Log.d(TAG, "adapterEnable: Impossible transition from " + mState);
            return;
        }
    ...
    private void adapterDisable() {
        if (!mState.oneOf(STATE_ON)) {
            Log.d(TAG, "adapterDisable: Impossible transition from " + mState);
            return;
        }
    ...
    private void adapterStartBrEdr() {
        if (!mState.oneOf(STATE_BLE_ON)) {
            Log.d(TAG, "adapterStartBrEdr: Impossible transition from " + mState);
            return;
        }
    ...
    private void adapterStopBle() {
        if (!mState.oneOf(STATE_BLE_ON)) {
            Log.d(TAG, "adapterStopBle: Impossible transition from " + mState);
            return;
        }
    ...
    private ComponentName resolveSystemService(@NonNull Intent intent) {
        ...
        switch (results.size()) {
            case 0 -> {
                throw new IllegalStateException("No system services handle for intent " + intent);
            }
            case 1 -> {
            ...
            default -> {
                throw new IllegalStateException(
                        "Multiples system services can handle intent " + intent + ": " + results);
            }
        }
    }
