...
class BluetoothManagerService {
    ...
    private void disableBluetoothComponents() {
        ...
        try {
          PackageInfo packageInfo = pm.getPackageInfo(BLUETOOTH_PACKAGE_NAME,
                PackageManager.GET_ACTIVITIES |
            ...
            if (packageInfo.activities != null) {
                for (android.content.pm.ActivityInfo activityInfo : packageInfo.activities) {
                    if (pm.getComponentEnabledSetting(new ComponentName(BLUETOOTH_PACKAGE_NAME, activityInfo.name))
                            != PackageManager.COMPONENT_ENABLED_STATE_DISABLED) {
                         pm.setComponentEnabledSetting(
                            new ComponentName(BLUETOOTH_PACKAGE_NAME, activityInfo.name),
                            PackageManager.COMPONENT_ENABLED_STATE_DISABLED,
            ...
            //Disable services
             if (packageInfo.services != null) {
                for (android.content.pm.ServiceInfo serviceInfo : packageInfo.services) {
                    if (pm.getComponentEnabledSetting(new ComponentName(BLUETOOTH_PACKAGE_NAME, serviceInfo.name))
                            != PackageManager.COMPONENT_ENABLED_STATE_DISABLED) {
                         pm.setComponentEnabledSetting(
                            new ComponentName(BLUETOOTH_PACKAGE_NAME, serviceInfo.name),
                            PackageManager.COMPONENT_ENABLED_STATE_DISABLED,
                            PackageManager.DONT_KILL_APP, // Consider policy
                            userId);
            ...
            if (packageInfo.receivers != null) {
                for (android.content.pm.ActivityInfo receiverInfo : packageInfo.receivers) {
                    if (pm.getComponentEnabledSetting(new ComponentName(BLUETOOTH_PACKAGE_NAME, receiverInfo.name))
                            != PackageManager.COMPONENT_ENABLED_STATE_DISABLED) {
                        pm.setComponentEnabledSetting(
                            new ComponentName(BLUETOOTH_PACKAGE_NAME, receiverInfo.name),
                            PackageManager.COMPONENT_ENABLED_STATE_DISABLED,
                            PackageManager.DONT_KILL_APP, // Consider policy
                            userId);
            ...
            if (packageInfo.providers != null) {
                for (android.content.pm.ProviderInfo providerInfo : packageInfo.providers) {
                     if (pm.getComponentEnabledSetting(new ComponentName(BLUETOOTH_PACKAGE_NAME, providerInfo.name))
                            != PackageManager.COMPONENT_ENABLED_STATE_DISABLED) {
                        pm.setComponentEnabledSetting(
                            new ComponentName(BLUETOOTH_PACKAGE_NAME, providerInfo.name),
                            PackageManager.COMPONENT_ENABLED_STATE_DISABLED,
                            PackageManager.DONT_KILL_APP, // Consider policy
                            userId);
