...
class BluetoothManagerService {
    ...
    private void peekBluetoothComponents(String packageName) {
        PackageManager pm = mContext.getPackageManager();
        Log.i(TAG, "peekBluetoothComponents");
            ...
            if (packageInfo.services != null) {
                for (android.content.pm.ServiceInfo serviceInfo : packageInfo.services) {
                    ...
                    if (pm.getComponentEnabledSetting(componentName)
                            != PackageManager.COMPONENT_ENABLED_STATE_DISABLED) {
                        if (serviceInfo.isEnabled()) {
                            Log.e(TAG, "peekBluetoothComponents: manifest enabled component: " + serviceInfo.name);
                        } else {
                            Log.e(TAG, "peekBluetoothComponents: dynamical component: " + serviceInfo.name);
                        }
                    } else {
                        Log.e(TAG, "peekBluetoothComponents: disabled component: " + serviceInfo.name);
                    }
                }
            }
        } catch (PackageManager.NameNotFoundException e) {
