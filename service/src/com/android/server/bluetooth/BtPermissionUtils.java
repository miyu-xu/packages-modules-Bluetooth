...
class BtPermissionUtils {
     ...
     *
     * <p>Return a string contening a error description if state should not be toggled by this call
     */
    static String callerCanToggle(
            Context ctx,
            AttributionSource source,
    ...
    private static UserHandle getForegroundUser() {
        final long callingIdentity = Binder.clearCallingIdentity();
        try {
    ...
    static boolean checkIfCallerIsForegroundUser(UserManager userManager) {
        ...
        final UserHandle foregroundUser = getForegroundUser();
        ...
        if (!valid) {
            Log.d(
                    TAG,
                    "checkIfCallerIsForegroundUser: valid="
                            + valid
                            + " callingUser="
                            + callingUser
                            + " parentUser="
    ...
    @RequiresPermission(BLUETOOTH_CONNECT)
    private static boolean checkBluetoothPermissions(
            Context ctx,
            AttributionSource source,
            UserManager userManager,
            AppOpsManager appOpsManager,
