...
import static com.android.server.bluetooth.BluetoothServiceEnforcePermission.checkConnectPermissionForDataDelivery;
import static com.android.server.bluetooth.BluetoothServiceEnforcePermission.checkIfCallerIsForegroundUser;
import static com.android.server.bluetooth.BluetoothServiceEnforcePermission.getCallingAppId;
import static com.android.server.bluetooth.BluetoothServiceEnforcePermission.isCallerSystem;
...
class BluetoothServiceBinder extends IBluetoothManager.Stub {
    ...
    @Override
    public boolean enable(@NonNull AttributionSource source) throws RemoteException {
        requireNonNull(source, "AttributionSource cannot be null in enable");

        final String errorMsg = BluetoothServiceEnforcePermission.callerCanToggle(
                mContext, source, mUserManager, mAppOpsManager, "enable", true);
        if (!errorMsg.isEmpty()) {
    ...
    public boolean enableNoAutoConnect(AttributionSource source) {
        ...
        final String errorMsg = BluetoothServiceEnforcePermission.callerCanToggle(
                mContext, source, mUserManager, mAppOpsManager, "enableNoAutoConnect", false);
        if (!errorMsg.isEmpty()) {
            Log.d(TAG, "enableNoAutoConnect(): FAILED: " + errorMsg);
            return false;
        }

        if (!BluetoothServiceEnforcePermission.isCallerNfc(getCallingAppId())) {
    ...
    @Override
    public boolean disable(AttributionSource source, boolean persist) throws RemoteException {
        if (!persist) {
            BluetoothServiceEnforcePermission.enforcePrivileged(mContext);
        }

        final String errorMsg = BluetoothServiceEnforcePermission.callerCanToggle(
                mContext, source, mUserManager, mAppOpsManager, "disable", true);
        if (!errorMsg.isEmpty()) {
    ...
    @Override
    public int getState() {
        if (!isCallerSystem(getCallingAppId()) && !checkIfCallerIsForegroundUser(mUserManager)) {
    ...
    public boolean bindBluetoothProfileService(int bluetoothProfile, String serviceName,
        ...
        return mBluetoothManagerService.bindBluetoothProfileService(bluetoothProfile, serviceName,
                proxy);
    }
    ...
    public String getAddress(AttributionSource source) {
        ...
        if (!checkConnectPermissionForDataDelivery(mContext, source, "getAddress")) {
            return null;
        }
        ...
        if (mContext.checkCallingOrSelfPermission(LOCAL_MAC_ADDRESS)
                != PERMISSION_GRANTED) {
    ...
    public String getName(AttributionSource source) {
        ...
        if (!checkConnectPermissionForDataDelivery(mContext, source, "getName")) {
            return null;
        }
    ...
    public boolean onFactoryReset(AttributionSource source) {
        ...
        if (!checkConnectPermissionForDataDelivery(mContext, source, "onFactoryReset")) {
            return false;
        }
    ...
    @Override
    @RequiresPermission(BLUETOOTH_CONNECT)
    public boolean enableBle(AttributionSource source, IBinder token) throws RemoteException {
        ...
        final String errorMsg = BluetoothServiceEnforcePermission.callerCanToggle(
                mContext, source, mUserManager, mAppOpsManager, "enableBle", false);
        if (!errorMsg.isEmpty()) {
    ...
    @Override
    @RequiresPermission(BLUETOOTH_CONNECT)
    public boolean disableBle(AttributionSource source, IBinder token) throws RemoteException {
        ...
        final String errorMsg = BluetoothServiceEnforcePermission.callerCanToggle(
                mContext, source, mUserManager, mAppOpsManager, "disableBle", false);
        if (!errorMsg.isEmpty()) {
