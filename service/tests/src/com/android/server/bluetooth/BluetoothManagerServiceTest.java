...
public class BluetoothManagerServiceTest {
    ...
    @Parameters(name = "{0}")
    static Iterable<? extends Object> generateParameterizedFlagsValue() {
        final String[] flags = {
                        ...
                        .map(
                                flag ->
                                        Arrays.stream(values)
                                                .map(val -> Map.entry(flag, val))
                                                .collect(Collectors.toList()))
                        .collect(Collectors.toList());

        return Lists.cartesianProduct(flagValues).stream()
                .map(list -> Map.ofEntries(list.toArray(new Map.Entry[0])))
                .filter(BluetoothManagerServiceTest::filterFlags)
                .map(map -> new FlagsValue[] {new FlagsValue(map)})
                .collect(Collectors.toList());
    }
