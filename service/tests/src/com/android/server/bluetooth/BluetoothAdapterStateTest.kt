...
class BluetoothAdapterStateTest {
    ...
    fun testStateIsProperlyInit() {
        android.util.Log.d("BluetoothAdapterStateTest", "Initial state is " + mState)
    ...
    fun testStateDoesNotTimeoutWhenStateIsAlreadyCorrect() {
        val state = 10
        mState.set(state)
        assertThat(runBlocking { mState.waitForState(0.milliseconds, state) }).isTrue()
    }
    ...
    fun testStateConcurrent() {
        ...
        runBlocking<Unit> {
            coroutineScope {
                val waiter = async { mState.waitForState(50.milliseconds, state) }
                mState.set(state)
            ...
            coroutineScope {
                val waiter0 =
                    async(start = CoroutineStart.UNDISPATCHED) {
                        mState.waitForState(50.milliseconds, state0)
                    }
                val waiter1 =
                    async(start = CoroutineStart.UNDISPATCHED) {
                        mState.waitForState(50.milliseconds, state1)
                    }
                val waiter2 =
                    async(start = CoroutineStart.UNDISPATCHED) {
                        mState.waitForState(50.milliseconds, state2)
                    }
                val waiter3 =
                    async(start = CoroutineStart.UNDISPATCHED) {
                        mState.waitForState(50.milliseconds, -1)
                    }
                mState.set(state0)
                mState.set(state1)
                mState.set(state2)
    ...
    fun testStateCycle() {
        ...
        runBlocking<Unit> {
            mState.set(state0)
            mState.set(state1)
            assertThat(mState.waitForState(50.milliseconds, state0)).isFalse()
        }
    }
