...
public class BluetoothServiceBinderTest {
    ...
    @Mock AttributionSource mSource;
    ...
    public void setUp() throws Exception {
        ...
        final String permission = mContext.getSystemServiceName(PermissionManager.class);
        ...
        doReturn(mPermissionManager).when(mContext).getSystemService(eq(permission));
    ...
    public void tearDown() {
        ...
        // Do not call verifyMock here. If the test fail the inital error will be lost
    }
    ...
    public void enableNoAutoConnect() {
        ...
        doReturn(PERMISSION_GRANTED)
                ...
                .checkPermissionForDataDeliveryFromDataSource(
                        any(String.class), any(AttributionSource.class), any(String.class));

        // enableNoAutoConnect is only available for Nfc and will failed otherwise
    ...
    public void getAddress() {
        ...
        // TODO(b/280518177): manually acquire BLUETOOTH_CONNECT in test
    ...
    public void onFactoryReset() {
        ...
        assertFalse(mBinder.onFactoryReset(mSource));
    ...
    private void verifyAndClearMock(Object o) {
        assertTrue(Mockito.mockingDetails(o).isMock() || Mockito.mockingDetails(o).isSpy());
    ...
    private void verifyMock() {
        ...
        verifyAndClearMock(mPermissionManager);
        verifyAndClearMock(mSource);
    ...
    private void checkDisabled(BooleanSupplier binderCall) {
        ...
        assertFalse(binderCall.getAsBoolean());
    ...
    private void checkHardDenied(ThrowingRunnable binderCall, boolean requireForeground) {
        doReturn(false)
                .when(mUserManager)
                .hasUserRestrictionForUser(eq(UserManager.DISALLOW_BLUETOOTH), any());
        doReturn(PERMISSION_HARD_DENIED)
                .when(mPermissionManager)
                .checkPermissionForDataDeliveryFromDataSource(
                        any(String.class), any(AttributionSource.class), any(String.class));
    ...
    private void checkGranted(BooleanSupplier binderCall, boolean expectedResult) {
        ...
        verify(mUserManager).hasUserRestrictionForUser(eq(UserManager.DISALLOW_BLUETOOTH), any());
        verify(mAppOpsManager).checkPackage(anyInt(), eq(TAG));
        verify(mSource, atLeast(2)).getPackageName();
        if (!expectedResult) {
