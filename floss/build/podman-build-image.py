...
class DockerImageBuilder:
    ...
    def _docker_build(self):
        self.run_command(self.docker_binary + ' build', [self.docker_binary, 'build', '-t', self.build_tag, '.'])
    ...
    def _build_dpkg_and_commit(self):
        ...
        self.run_command(self.docker_binary + ' stop', [self.docker_binary, 'stop', '-t', '1', self.container_name], ignore_rc=True)
        self.run_command(self.docker_binary + ' rm', [self.docker_binary, 'rm', self.container_name], ignore_rc=True)
        ...
        self.run_command(self.docker_binary + ' run', [
            self.docker_binary, 'run', '--name', self.container_name, '--mount', mount_str, '-d', self.build_tag, 'tail', '-f',
            '/dev/null'
        ])
        ...
        try:
            ...
            self.run_command(self.docker_binary + ' commit', [self.docker_binary, 'commit', self.container_name, self.final_tag])
        finally:
            # Stop running the container and remove it
            self.run_command(self.docker_binary + ' stop', [self.docker_binary, 'stop', '-t', '1', self.container_name])
            self.run_command(self.docker_binary + ' rm', [self.docker_binary, 'rm', self.container_name])
...
def main():
    ...
    pib = DockerImageBuilder(workdir, rootdir, args.tag, args.use_docker)
    pib.build()
