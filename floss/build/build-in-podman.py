...
class FlossDockerRunner:
    ...
    def __init__(self, workdir, rootdir, image_tag, volume_name, container_name, staging_dir, use_docker):
        ...
        self.docker_binary = 'docker' if use_docker else 'podman'
    ...
    def _create_volume_if_needed(self):
        ...
        try:
            subprocess.check_output([self.docker_binary, 'volume', 'inspect', self.volume_name])
        except:
            self.run_command(self.docker_binary + ' volume create', [self.docker_binary, 'volume', 'create', self.volume_name])
    ...
    def start_container(self):
        ...
        self.run_command(self.docker_binary + ' run', [
            self.docker_binary, 'run', '--name', self.container_name, '--mount', mount_output_volume, '--mount', mount_src_dir,
            '-d', self.image_tag, 'tail', '-f', '/dev/null'
        ])

    def stop_container(self, ignore_error=False):
        """Stops the docker container for build."""
        self.run_command(self.docker_binary + ' stop', [self.docker_binary, 'stop', '-t', '1', self.container_name], ignore_rc=ignore_error)
        self.run_command(self.docker_binary + ' rm', [self.docker_binary, 'rm', self.container_name], ignore_rc=ignore_error)
    ...
    def do_build(self):
        ...
        try:
            ...
            for i, cmd in enumerate(self.BUILD_COMMANDS):
                self.run_command(self.docker_binary + ' exec #{}'.format(i), [self.docker_binary, 'exec', '-it', self.container_name] + cmd)
        finally:
    ...
    def print_do_build(self):
        ...
        docker_exec = [self.docker_binary, 'exec', '-it', self.container_name]
    ...
    def check_docker_runnable(self):
        try:
            subprocess.check_output([self.docker_binary, 'ps'], stderr=subprocess.STDOUT)
...
if __name__ == "__main__":
    ...
    fdr = FlossDockerRunner(workdir, rootdir, args.image_tag, args.volume_tag, args.container_name, staging, args.use_docker)
