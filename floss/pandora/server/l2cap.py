...
class L2CAPService(l2cap_grpc_aio.L2CAPServicer):
    ...
    async def CreateLECreditBasedChannel(self, request: l2cap_pb2.CreateLECreditBasedChannelRequest,
        ...
        try:
            ...
            connecting_sockets = self.bluetooth.socket_manager.connecting_sockets
            socket_info = connecting_sockets.get(socket_id, None)

            if socket_info is None:
                raise ValueError(f'Socket ID {socket_id} not found in connecting sockets.')
    ...
    async def AcceptL2CAPChannel(self, request: l2cap_pb2.AcceptL2CAPChannelRequest,
        ...
        try:
            ...
            name = utils.create_observer_name(observer)
            self.bluetooth.socket_manager.register_callback_observer(name, observer)
            socket_result = self.connection_server_socket.get(request.connection.cookie.value)
            ...
            listening_sockets = self.bluetooth.socket_manager.listening_sockets
            connections_info = listening_sockets.get(listener_id, ([], None))[1]

            if connections_info is None:
                raise ValueError(f'Listener ID {listener_id} not found in listening sockets.')
