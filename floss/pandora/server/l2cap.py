...
class L2CAPService(l2cap_grpc_aio.L2CAPServicer):
    ...
    def __init__(self, bluetooth: bluetooth_module.Bluetooth):
        ...
        # key = connection_value, val = current_socket_index.
        # Uses to track the current socket index for each connection.
    ...
    async def CreateLECreditBasedChannel(self, request: l2cap_pb2.CreateLECreditBasedChannelRequest,
        ...
        class CreateL2CAPObserver(socket_manager.SocketManagerCallbacks):
            ...
            def __init__(self, client: socket_manager.FlossSocketManagerClient, task):
                self.client = client
                self.task = task
    ...
    async def AcceptL2CAPChannel(self, request: l2cap_pb2.AcceptL2CAPChannelRequest,
        ...
        class AcceptL2CAPObserver(socket_manager.SocketManagerCallbacks):
            ...
            def on_handle_incoming_connection(self, listener_id, connection, *, dbus_unix_fd_list=None):
                ...
                fd = dbus_unix_fd_list.get(fd_handle)
                fd_dup = os.dup(fd)
                self.client.set_listening_socket_connection(listener_id, (connection, fd_dup))
    ...
    def _get_next_socket(self, connection_value):
        """Gets the next socket for the given connection."""
        connection_streams = self.connection_streams.get(connection_value)
        if not connection_streams:
            logging.error('No connection streams available for connection_value: %s', connection_value)
            return None

        current_index = self.current_index.get(connection_value, 0)
        current_socket = connection_streams[current_index]
        self.current_index[connection_value] = (current_index + 1) % len(connection_streams)
        return current_socket

    async def SendData(self, request: l2cap_pb2.SendDataRequest,
                       context: grpc.ServicerContext) -> l2cap_pb2.SendDataResponse:
        connection_value = request.connection.cookie.value
        output_stream = self._get_next_socket(connection_value)
        if output_stream:
            try:
    ...
    async def ReceiveData(self, request: l2cap_pb2.ReceiveDataRequest,
        ...
        connection_value = request.connection.cookie.value
        input_stream = self._get_next_socket(connection_value)
        if input_stream:
            try:
