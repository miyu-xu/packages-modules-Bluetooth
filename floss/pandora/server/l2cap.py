...
class L2CAPService(l2cap_grpc_aio.L2CAPServicer):
    ...
    async def ListenL2CAPChannel(self, request: l2cap_pb2.ListenL2CAPChannelRequest,
        ...
        try:
            ...
            if socket_id != socket_result['id']:
                raise RuntimeError(f'Failed to listen on the L2CAP channel with socket_id: {socket_id}.')
    ...
    async def CreateLECreditBasedChannel(self, request: l2cap_pb2.CreateLECreditBasedChannelRequest,
        ...
        try:
            ...
            observer = CreateL2CAPObserver(self.bluetooth.socket_manager,
                                           {'create_l2cap_channel': create_l2cap_channel})
            name = utils.create_observer_name(observer)
            self.bluetooth.socket_manager.register_callback_observer(name, observer)
            ...
            socket_id, fd = await asyncio.wait_for(create_l2cap_channel, timeout=5)
            if socket_id != socket_result['id']:
    ...
    async def AcceptL2CAPChannel(self, request: l2cap_pb2.AcceptL2CAPChannelRequest,
        ...
        try:
            ...
            observer = AcceptL2CAPObserver(self.bluetooth.socket_manager,
                                           {'accept_l2cap_channel': accept_l2cap_channel})
            name = utils.create_observer_name(observer)
            self.bluetooth.socket_manager.register_callback_observer(name, observer)
            connection_value = request.connection.cookie.value
            ...
            listener_id, fd = await asyncio.wait_for(accept_l2cap_channel, timeout=5)
            if listener_id != socket_id:
                raise RuntimeError(f'Failed to accept the L2CAP channel with socket_id: {socket_id}.')
    ...
    async def SendData(self, request: l2cap_pb2.SendDataRequest,
        ...
        if output_stream:
            ...
            logging.error('Output stream: %s not found for the connection: %s', output_stream, connection_value)
