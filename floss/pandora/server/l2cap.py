...
class L2CAPService(l2cap_grpc_aio.L2CAPServicer):
    ...
    def __init__(self, bluetooth: bluetooth_module.Bluetooth):
        ...
        # key = connection_value, val = [socket_info].
        self.connection_server_sockets = dict()

        # key = connection_value, val = [connection_sockets]
        # Where connection_sockets may represent either client or server sockets, facilitating communication channels
        # over UNIX domain stream connections.
        self.connection_streams = dict()
    ...
    async def ListenL2CAPChannel(self, request: l2cap_pb2.ListenL2CAPChannelRequest,
        ...
        try:
            ...
            if socket_result is None:
                context.abort(grpc.StatusCode.INTERNAL, f'Failed to call listen_using_{channel_type}_l2cap_channel.')
            ...
            self.connection_server_sockets.setdefault(request.connection.cookie.value, []).append(socket)
        finally:
    ...
    async def CreateLECreditBasedChannel(self, request: l2cap_pb2.CreateLECreditBasedChannelRequest,
        ...
        class CreateL2CAPObserver(socket_manager.SocketManagerCallbacks):
            ...
            def on_outgoing_connection_result(self, connecting_id, result, socket, *, dbus_unix_fd_list=None):
                ...
                socket_info = self.client.get_connecting_socket_info(connecting_id)
                if socket_info is None:
                    self.client.set_connecting_socket_info(connecting_id, (result, socket, None))
                    ...
                    fd_dup = os.dup(fd)
                    self.client.set_connecting_socket_info(connecting_id, (result, socket, fd_dup))
                else:
                    _, _, fd_dup = socket_info

                future = self.task['create_l2cap_channel']
                future.get_loop().call_soon_threadsafe(future.set_result, (result, fd_dup))
        ...
        try:
            ...
            if socket_result is None:
                context.abort(grpc.StatusCode.INTERNAL, f'Failed to call create_{channel_type}_l2cap_channel.')
            ...
            name = utils.create_observer_name(observer)
            self.bluetooth.socket_manager.register_callback_observer(name, observer)
            status, fd = await asyncio.wait_for(l2cap_channel_creation['create_l2cap_channel'], timeout=5)
            if status != floss_enums.BtStatus.SUCCESS:
                context.abort(
                    grpc.StatusCode.INTERNAL,
                    f'Failed to create the L2CAP channel with connecting_id: {connecting_id}. Status: {status}')

            if fd is None:
                context.abort(grpc.StatusCode.INTERNAL,
                              f'Failed to get the fd from L2CAP socket with socket_id: {connecting_id}')

            client_socket = socket_module.fromfd(fd, socket_module.AF_UNIX, socket_module.SOCK_STREAM)
            self.connection_streams.setdefault(connection_value, []).append(client_socket)
        finally:
            self.bluetooth.socket_manager.unregister_callback_observer(name, observer)
    ...
    async def AcceptL2CAPChannel(self, request: l2cap_pb2.AcceptL2CAPChannelRequest,
        ...
        class AcceptL2CAPObserver(socket_manager.SocketManagerCallbacks):
            ...
            def on_handle_incoming_connection(self, listener_id, connection, *, dbus_unix_fd_list=None):
                ...
                connections_info = self.client.get_listening_socket_connections(listener_id)
                if not connections_info:
                    ...
                    fd = dbus_unix_fd_list.get(fd_handle)
                    fd_dup = os.dup(fd)
                    self.client.set_connecting_socket_info(listener_id, (connection, fd_dup))
                else:
                    _, fd_dup = connections_info[0]
        ...
        connection_value = request.connection.cookie.value
        try:
            if connection_value in self.connection_server_sockets:
                socket_list = self.connection_server_sockets.get(connection_value)
                if not socket_list:
                    context.abort(grpc.StatusCode.INTERNAL,
                                  f'No sockets found for the connection_value: {connection_value}.')

                socket_id = socket_list[-1]['id']
                ...
                server_socket = socket_module.fromfd(fd, socket_module.AF_UNIX, socket_module.SOCK_STREAM)
                self.connection_streams.setdefault(connection_value, []).append(server_socket)
        finally:
            self.bluetooth.socket_manager.unregister_callback_observer(name, observer)
    ...
    async def SendData(self, request: l2cap_pb2.SendDataRequest,
        ...
        connection_value = request.connection.cookie.value
        output_streams = self.connection_streams.get(connection_value)
        if output_streams:
            try:
                output_streams[-1].send(request.data)
            except Exception as e:
                logging.error('Exception during writing to output stream: %s', e)
        else:
            logging.error('Output stream: %s not found for the connection_value: %s', output_streams[-1],
                          connection_value)
    ...
    async def ReceiveData(self, request: l2cap_pb2.ReceiveDataRequest,
        ...
        connection_value = request.connection.cookie.value
        input_streams = self.connection_streams.get(connection_value)
        if input_streams:
            try:
                data = input_streams[-1].recv(self.BUFFER_SIZE)
            ...
            logging.error('Input stream: %s not found for the connection_value: %s', input_streams[-1],
                          connection_value)
