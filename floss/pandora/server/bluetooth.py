...
class Bluetooth(object):
    ...
    def get_remote_property(self, address, property):
        return self.adapter_client.get_remote_property(address, property)

    def is_encrypted(self, address):
        connection_state = self.adapter_client.get_connection_state(address)
        if connection_state is None:
            logging.error(f'Failed to get connection state for address: {address}')
            return False
