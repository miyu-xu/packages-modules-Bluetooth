...
from autotest_lib.client.cros.bluetooth.floss import adapter_client
from autotest_lib.client.cros.bluetooth.floss import advertising_client
from autotest_lib.client.cros.bluetooth.floss import floss_enums
from autotest_lib.client.cros.bluetooth.floss import scanner_client
from autotest_lib.client.cros.bluetooth.floss import utils as floss_utils
from floss.pandora.server import bluetooth as bluetooth_module
from floss.pandora.server import utils
...
class HostService(host_grpc_aio.HostServicer):
    ...
    def __init__(self, server, bluetooth: bluetooth_module.Bluetooth):
        self.server = server
    ...
    async def FactoryReset(self, request: empty_pb2.Empty, context: grpc.ServicerContext) -> empty_pb2.Empty:
        # Looses all server states.
        self.waited_connections = set()

        # Trigger server to stop.
    ...
    async def Advertise(self, request: host_pb2.AdvertiseRequest,
        ...
        if own_address_type in (host_pb2.PUBLIC, host_pb2.RESOLVABLE_OR_PUBLIC):
            parameters['own_address_type'] = utils.OwnAddressType.PUBLIC
        elif own_address_type in (host_pb2.RANDOM, host_pb2.RESOLVABLE_OR_RANDOM):
            parameters['own_address_type'] = utils.OwnAddressType.RANDOM
    ...
    async def Scan(self, request: host_pb2.ScanRequest,
        ...
        try:
            ...
            name = utils.create_observer_name(observer)
            self.bluetooth.scanner_client.callbacks.add_observer(name, observer)
            ...
            while True:
                ...
                if scan_result['addr_type'] == utils.BleAddressType.BLE_ADDR_PUBLIC:
                    response.public = address
                elif scan_result['addr_type'] == utils.BleAddressType.BLE_ADDR_RANDOM:
                    response.random = address
                elif scan_result['addr_type'] == utils.BleAddressType.BLE_ADDR_PUBLIC_ID:
                    response.public_identity = address
                elif scan_result['addr_type'] == utils.BleAddressType.BLE_ADDR_RANDOM_ID:
            ...
            if name is not None and observer is not None:
                self.bluetooth.scanner_client.callbacks.remove_observer(name, observer)
