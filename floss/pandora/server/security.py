...
class SecurityService(security_grpc_aio.SecurityServicer):
    ...
    async def OnPairing(self, request: AsyncIterator[security_pb2.PairingEventAnswer],
        ...
        async def streaming_answers(self):
            while True:
                logging.info(f'OnPairing: Wait for pairing answer...')
                pairing_answer = await utils.anext(self.bluetooth.pairing_answers)
                logging.info('OnPairing: Pairing answer: %s address: %s', pairing_answer, address)
        ...
        try:
            ...
            while True:
                logging.info('OnPairing: Wait for pairing events...')
                address, name, variant, *variables = await self.bluetooth.pairing_events.get()
                logging.info('OnPairing: Pairing event: address: %s, name: %s, variant: %s, variables: %s', address,
                             name, variant, variables)
                ...
                if variant == floss_enums.PairingVariant.PASSKEY_CONFIRMATION:
                    ...
                    if transport == floss_enums.Transport.BREDR:
                        event.pin_code_notification.CopyFrom(pincode.encode())
                    elif transport == floss_enums.Transport.LE:
                        event.passkey_entry_notification.CopyFrom(int(pincode))
                    else:
                        logging.error('Cannot determine pairing variant from unknown transport.')
                        continue
                else:
