...
class SecurityService(security_grpc_aio.SecurityServicer):
    ...
    async def OnPairing(self, request: AsyncIterator[security_pb2.PairingEventAnswer],
        ...
        class PairingObserver(adapter_client.BluetoothCallbacks):
            ...
            def __init__(self, loop: asyncio.AbstractEventLoop, task, on_pairing_count):
                self.loop = loop
                self.task = task
                self.on_pairing_count = on_pairing_count
        ...
        async def streaming_answers(self):
            while True:
                ...
                logging.info('OnPairing[%s]: Wait for pairing answer...', self.on_pairing_count)
                ...
                logging.info('OnPairing[%s]: Pairing answer: %s address: %s', self.on_pairing_count, answer, address)
        ...
        try:
            ...
            while True:
                logging.info('OnPairing[%s]: Wait for pairing events...', self.on_pairing_count)
                ...
                logging.info('OnPairing[%s]: Pairing event: address: %s, name: %s, variant: %s, variables: %s',
                             self.on_pairing_count, address, name, variant, variables)
