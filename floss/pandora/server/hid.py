...
class HIDService(hid_grpc_aio.HIDServicer):
    ...
    /modules/Bluetooth/pandora/interfaces/pandora_experimental/hid.proto
    """
    ...
    async def SendHostReport(self, request: hid_pb2.SendHostReportRequest,
        ...
        class HIDReportObserver(qa_client.BluetoothQACallbacks):
            ...
            def on_set_hid_report_completed(self, status):
                if floss_enums.BtStatus(status) != floss_enums.BtStatus.SUCCESS:
                    logging.error('Failed to set HID report.')
        ...
        try:
            ...
            if floss_enums.BthReportType(request.report_type) not in floss_enums.BthReportType:
            ...
            status = await asyncio.wait_for(set_hid_report, timeout=5)
            if floss_enums.BtStatus(status) != floss_enums.BtStatus.SUCCESS:
                raise RuntimeError('Failed to set HID report.')
