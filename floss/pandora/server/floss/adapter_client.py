...
class FlossAdapterClient(BluetoothCallbacks, BluetoothConnectionCallbacks):
    ...
    def qa_legacy_proxy(self):
        ...
        return self.bus.get(self.ADAPTER_SERVICE, self.objpath)[self.QA_LEGACY_INTERFACE]
    ...
    def register_callbacks(self):
        ...
        if not self.callbacks:
            ...
            objpath = generate_dbus_cb_objpath(self.ADAPTER_CB_OBJ_NAME, self.hci)
        ...
        if not self.connection_callbacks:
            ...
            objpath = generate_dbus_cb_objpath(self.ADAPTER_CONN_CB_OBJ_NAME, self.hci)
        ...
        for device in bonded_devices:
            ...
            if success:
                ...
                dev = self.known_devices.get(address, self._make_device(address, name, bond_state=BondState.BONDED))
    ...
    def is_bonded(self, address):
        ...
        return address in self.known_devices and self.known_devices[address].get(
            'bond_state', BondState.NOT_BONDED) == BondState.BONDED
    ...
    def cancel_bond(self, address):
        ...
        @return Result of |CancelBondProcess|.
        """
        name = 'Test bond'
        if address in self.known_devices:
            name = self.known_devices[address]['name']
