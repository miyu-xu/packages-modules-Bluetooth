...
from autotest_lib.client.cros.bluetooth.floss.utils import (generate_dbus_cb_objpath, glib_call, glib_callback)
...
class FlossAdvertisingClient(BluetoothAdvertisingCallbacks):
    ...
    def on_advertising_set_started(self, reg_id, advertiser_id, tx_power, status):
        ...
        if advertiser_id in self.active_advs:
            logging.warn('The set of advertiser_id: %s, is already registered.', advertiser_id)
        else:
    ...
    def on_advertising_set_stopped(self, advertiser_id):
        ...
        if advertiser_id in self.active_advs:
            ...
            logging.warn('The set of advertiser_id: %s, not registered yet.', advertiser_id)

    @glib_callback()
    def on_advertising_enabled(self, advertiser_id, enable, status):
        """Handle advertising enable callback."""
        logging.debug('on_advertising_enabled: advertiser_id: %s, enable: %s, '
    ...
    def on_scan_response_data_set(self, advertiser_id, status):
        """Handle scan response data set callback."""
        logging.debug('on_scan_response_data_set: advertiser_id: %s, status: '
    ...
    def on_periodic_advertising_data_set(self, advertiser_id, status):
        """Handle periodic advertising data set callback."""
        logging.debug('on_periodic_advertising_data_set: advertiser_id: %s, status: '
                      '%s', advertiser_id, status)
    ...
    def register_advertiser_callback(self):
        ...
        objpath = generate_dbus_cb_objpath(self.ADVERTISING_CB_OBJ_NAME, self.hci)
