...
class RFCOMMService(rfcomm_grpc_aio.RFCOMMServicer):
    ...
    def __init__(self, bluetooth: bluetooth_module.Bluetooth):
        ...
        # key = socket_id, val = socket_info.
        self.connection_map = dict()

        # key = unique_id, val = connection_socket
        # Where connection_socket may represent either client or server socket, facilitating communication channels
    ...
    async def ConnectToServer(self, request: rfcomm_pb2.ConnectionRequest,
        ...
        try:
            ...
            status, fd = await asyncio.wait_for(rfcomm_channel_creation['create_rfcomm_channel'], timeout=5)
            if status != floss_enums.BtStatus.SUCCESS:
                context.abort(
                    grpc.StatusCode.INTERNAL,
                    f'Failed to create the RFCOMM channel with connecting_id: {connecting_id}. Status: {status}')
        ...
        try:
            if listener_id in self.server_map:
                ...
                if not self.bluetooth.close_socket(listener_id):
                    context.abort(grpc.StatusCode.INTERNAL, 'Failed to call close_socket.')

                status = await asyncio.wait_for(rfcomm_channel_stop['stop_rfcomm_channel'], timeout=5)
                if status != floss_enums.BtStatus.SUCCESS:
                    context.abort(grpc.StatusCode.INTERNAL,
                                  f'Failed to stop RFCOMM channel with listener_id: %s. Status: %s', listener_id,
                                  status)
    ...
    async def StartServer(self, request: rfcomm_pb2.ServerOptions,
        ...
        class StartServerObserver(socket_manager.SocketManagerCallbacks):
            ...
            def on_incoming_socket_ready(self, socket, status):
                ...
                future.get_loop().call_soon_threadsafe(future.set_result, (status, socket_id, socket))
        ...
        try:
            ...
            name = utils.create_observer_name(observer)
            self.bluetooth.socket_manager.register_callback_observer(name, observer)
            status, socket_id, socket = await asyncio.wait_for(rfcomm_channel_listener['start_rfcomm_server'],
                                                               timeout=5)
            if status != floss_enums.BtStatus.SUCCESS:
                context.abort(grpc.StatusCode.INTERNAL,
                              'Failed listening to RFCOMM channel with socket_id: %s. Status: %s', socket_id, status)
