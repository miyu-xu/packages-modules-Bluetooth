...
class BluetoothAdminPolicyCallbacks:
    ...
    def on_service_allowlist_changed(self, allowlist):
        ...
        Args:
            allowlist: List of uuid services.
        """
        pass
    ...
    def on_device_policy_effect_changed(self, device, new_policy_effect):
        ...
        Args:
            device: Remote device that is affected by the policy.
            new_policy_effect: The Policy that affects services and devices.
        """
        pass
...
class FlossAdminClient(BluetoothAdminPolicyCallbacks):
    ...
    def parse_dbus_device(device_dbus):
        ...
        Args:
            device_dbus: Variant dict with signature a{sv}.

        Returns:
            True and BluetoothDevice tuple on success parsing, otherwise False and None.
        """
        if 'address' in device_dbus and 'name' in device_dbus:
            return True, (str(device_dbus['address']), str(device_dbus['name']))
        return False, None
    ...
    def parse_dbus_policy_effect(optional_policy_effect):
        ...
        Args:
            optional_policy_effect: Optional value with signature a{sv}.

        Returns:
            True and PolicyEffect tuple on success parsing, otherwise False and None.
        """
        ...
        if ('service_blocked' in policy_optional_value and 'affected' in policy_optional_value):
            try:
                uuids = [uuid_module.UUID(bytes=bytes(u)) for u in policy_optional_value['service_blocked']]
            except ValueError:
                logging.exception("Failed to create UUID with values: %s", uuids)
                return False, None

            return True, (uuids, bool(policy_optional_value['affected']))
        return False, None

    class ExportedAdminPolicyCallbacks(observer_base.ObserverBase):
        """
        <node>
        ...
        def OnDevicePolicyEffectChanged(self, device, new_policy_effect):
            ...
            Args:
                ...
                new_policy_effect: The Policy that affects services and devices.
            """
            parsed_device, remote_device = FlossAdminClient.parse_dbus_device(device)
            if not parsed_device:
                logging.debug('OnDevicePolicyEffectChanged parse error: {}'.format(device))
                return

            parsed_policy, remote_policy_effect = (FlossAdminClient.parse_dbus_policy_effect(new_policy_effect))
            if not parsed_policy:
                logging.debug('OnDevicePolicyEffectChanged: {}'.format(new_policy_effect))
                return

            for observer in self.observers.values():
                observer.on_device_policy_effect_changed(remote_device, remote_policy_effect)
    ...
    def on_device_policy_effect_changed(self, device, new_policy_effect):
        ...
        Args:
            ...
            new_policy_effect: The Policy that affects services and devices.
        """
        logging.debug('on_device_policy_effect_changed: device: %s, '
                      'new_policy_effect: %s, ', device, new_policy_effect)

        address, name = device
        if address not in self.known_devices:
            self.known_devices[address] = {'address': address, 'name': name, 'new_policy_effect': new_policy_effect}
        else:
    ...
    def proxy(self):
        ...
        return self.bus.get(self.ADMIN_SERVICE, self.objpath)[self.ADMIN_INTERFACE]
    ...
    def register_admin_policy_callback(self):
        ...
        objpath = utils.generate_dbus_cb_objpath(self.ADMIN_CB_OBJ_NAME, self.hci)
    ...
    def get_device_policy_effect(self, address):
        ...
        Args:
            address: Address of device containing policy effect.

        Returns:
            PolicyEffect tuple as (List[UUID], bool) on success, None otherwise.
        """
        name = 'Test policy'
        if address in self.known_devices:
            name = self.known_devices[address]['name']

        device = {'address': GLib.Variant('s', address), 'name': GLib.Variant('s', name)}
        policy_effect_dbus = self.proxy().GetDevicePolicyEffect(device)
        parsed, policy_effect = FlossAdminClient.parse_dbus_policy_effect(policy_effect_dbus)
        if not parsed:
    ...
    def get_allowed_services(self):
        ...
        Returns:
            List of allowed services as type UUID on success, None otherwise.
        """
        uuids = self.proxy().GetAllowedServices()
        return [uuid_module.UUID(bytes=bytes(u)) for u in uuids]
    ...
    def set_allowed_services(self, services):
        ...
        Args:
            services: List of 128-bit service UUID.
    ...
    def is_service_allowed(self, uuid):
        ...
        Returns:
            Service allowed as boolean on success, None otherwise.
        """
