...
def parse_advertiging_data(adv_data: list[int]) -> host_pb2.DataTypes:
    ...
    # advertising data packet is repeted by many advertising data and each with following format:
    # | Lenghth (0) | Type (1) | Data Payload (2~N-1)
    # An example of advertising data packet: [2, 1, 6, 5, 3, 0, 24, 1, 24, 13, 9, 80, 84,
    # 83, 45, 71, 65, 80, 45, 49, 55, 48, 49, 3, 25, 0, 0].
    ...
    while index < len(adv_data):
        ...
        if data_type == floss_enums.AdvertisingDataType.COMPLETE_LOCAL_NAME:
            ...
            logging.error('Unsupported advertising data type to parse: %s', data_type)
...
def parse_complete_local_name(data: list[int]) -> str | None:
