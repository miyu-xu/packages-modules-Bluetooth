...
def playback(blocking=True, stdin=None, *args, **kargs):
    ...
    Returns:
        The process running the playback command. Note that if the blocking parameter is true, this will return
        a finished process.
    """
...
def capture_cmd(capture_file, block_size=None, duration=10,
    ...
    Args:
        ...
        block_size: The number of frames per callback(dictates latency).
        duration: Seconds to record. If it is None, duration is not set, and command will keep capturing audio until it
                  is terminated.
        sample_format: The sample format; possible choices: 'S16_LE', 'S24_LE', and 'S32_LE' default to S16_LE: signed
...
def get_cras_control_interface(private=False):
    ...
    Raises:
        ImportError if this is not called on Cros device.
    """
    try:
        import dbus
...
def get_selected_nodes():
    ...
    Returns:
        A tuple (output_nodes, input_nodes) where each field is a list of selected node IDs returned from Cras DBus
        API. Note that there may be multiple output/input nodes being selected at the same time.
    """
    output_nodes = []
    input_nodes = []
...
def node_type_is_plugged(node_type, nodes_info):
    ...
    This method is used in the AudioLoopbackDongleLabel class, where the call is executed on autotest server. Use
    get_cras_nodes instead if the call can be executed on Cros device.

    Since Cras only reports the plugged node in GetNodes, we can parse the return value to see if there is any node
    with the given type. For example, if INTERNAL_MIC is of intereset, the pattern we are looking for is:
...
def get_filtered_node_types(callback):
    ...
    Args:
        callback: A callback function which takes a node as input parameter and filter the node based on its return
                  value.

    Returns: A tuple (output_node_types, input_node_types) where each field is a list of node types defined in
             CRAS_NODE_TYPES, and their 'attribute_name' is True.
    """
    output_node_types = []
    input_node_types = []
    nodes = get_cras_nodes()
...
def get_selected_node_types():
    ...
    Returns:
         A tuple (output_node_types, input_node_types) where each field is a list of selected node types defined in
         CRAS_NODE_TYPES.
...
def set_selected_output_nodes(types):
    ...
    Note that Chrome UI uses SetActiveOutputNode of Cras DBus API to select one output node. Here we use add/remove
    active output node to support multiple nodes.
...
class CrasTestClient(object):
    ...
    def start_capturing_subprocess(self, capture_file, block_size=BLOCK_SIZE,
        ...
        Args:
            capture_file: The name of file the audio to be stored in.
            block_size: The number of frames per callback(dictates latency).
            duration: Seconds to record. If it is None, duration is not set, and will keep capturing audio until
                      terminated.
        ...
        Returns:
            True if the process is started successfully.
        """
        proc_cmd = capture_cmd(capture_file, block_size=block_size, duration=duration, sample_format=sample_format,
    ...
    def start_playing_subprocess(self, audio_file, block_size=BLOCK_SIZE,
        ...
        Args:
            ...
            block_size: The number of frames per callback(dictates latency).
            duration: Seconds to play. If it is None, duration is not set, and will keep playing audio until
                      terminated.
        ...
        if result:
            logging.info(f'start_playing_subprocess result: {result}')
    ...
    def stop_playing_subprocess(self):
        ...
        logging.info(f'stop_playing_subprocess result: {result}')
        if result:
            self._playing_proc = None
        return result


    def play(self, audio_file, block_size=BLOCK_SIZE, duration=DURATION, pin_device=PIN_DEVICE, channels=CHANNELS,
             rate=RATE):
        """Plays the audio file.

        This method will get blocked until it has completed playing back. If you do not want to get blocked, use
        start_playing_subprocess() above instead.
    ...
    def set_player_length(self, length):
        ...
        Media length is a part of metadata information. However, without specify its type to int64. dbus-python will
        guess the variant type to be int32 by default. Separate it from the metadata function to help prepare the data
        differently.

        Args:
            metadata: DBUS dictionary that contains a variant of int64.
        """
        try:
