...
impl TryFrom<&[u8]> for LinuxSnoopPacket {
    ...
    fn try_from(item: &[u8]) -> Result<Self, Self::Error> {
        ...
        let rest = item;
        let (size_bytes, rest) = rest.split_at(std::mem::size_of::<u32>());
        let (length_bytes, rest) = rest.split_at(std::mem::size_of::<u32>());
...
impl<'a> Iterator for LinuxSnoopReader<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match LinuxSnoopPacket::try_from(&data[0..bytes]) {
            Ok(mut p) => {
                if p.size > 0 {
                    let size: usize = p.size.try_into().unwrap();
...
impl<'a> LogParser {
    ...
    /// For a non-intrusive query, use |get_type|.
