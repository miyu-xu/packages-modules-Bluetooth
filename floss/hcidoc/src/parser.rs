...
/// Snoop timestamps start at year 0 instead of 1972 like unix timestamps. This
/// offset is used to represent Jan 1, 2000 AD and can be used to convert back
/// to unixtime.
const LINUX_SNOOP_Y2K_EPOCH_USECS: u64 = 0x00E03AB44A676000u64;
...
impl<'a> TryFrom<&'a LinuxSnoopPacket> for Packet {
    ...
    fn try_from(item: &'a LinuxSnoopPacket) -> Result<Self, Self::Error> {
        match PacketChild::try_from(item) {
            Ok(inner) => {
                let base_ts = i64::try_from(item.timestamp_magic_us - LINUX_SNOOP_Y2K_EPOCH_USECS)
                    .map_err(|e| format!("u64 conversion error: {}", e))?;

                let us_to_s = 1000000i64;
                let ts_secs = (base_ts / us_to_s) + LINUX_SNOOP_Y2K_OFFSET_IN_SECS;
