...
impl GeneralSnoopPacket for LinuxSnoopPacket {
    ...
    fn timestamp_us(&self) -> u64 {
        self.preamble.timestamp_magic_us
    }
    fn data(&self) -> &Vec<u8> {
        &self.data
    }
...
const SNOOP_Y2K_OFFSET_IN_SECS: i64 = 946684800i64;
...
/// to unixtime.
const SNOOP_Y2K_EPOCH_USECS: i64 = 0x00E03AB44A676000i64;
...
const SNOOP_OFFSET_TO_UNIXTIME_SECS: i64 =
    SNOOP_Y2K_OFFSET_IN_SECS - (SNOOP_Y2K_EPOCH_USECS / USECS_TO_SECS);
...
impl<'a> Iterator for H4SnoopReader<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if preamble.included_length > 0 {
            ...
            Some(Box::new(H4SnoopPacket { preamble, data: vec![], pkt_type: 0 }))
        }
    }
}
...
impl<'a> TryFrom<(usize, &'a dyn GeneralSnoopPacket)> for Packet {
    ...
    fn try_from(item: (usize, &'a dyn GeneralSnoopPacket)) -> Result<Self, Self::Error> {
        ...
        match PacketChild::try_from(packet) {
            Ok(inner) => {
                let base_ts = i64::try_from(packet.timestamp_us())
                    .map_err(|e| format!("u64 conversion error: {}", e))?;

                let ts_secs = (base_ts / USECS_TO_SECS) + SNOOP_OFFSET_TO_UNIXTIME_SECS;
