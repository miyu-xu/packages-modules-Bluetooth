...
enum ControllerSignal {
    ...
    LikelyExternalController, // The controller report name is not as expected, indicating an external controller is being used.
}
...
lazy_static! {
    static ref KNOWN_CONTROLLER_NAMES: Vec<String> = vec![
        ...
        String::from(""),                 // AC9260/AC9560/AC200/AC201/AC203/AX211/MVL8897/QCA6174A3/QCA6174A5/QC_WCN6856
    ];
}
const KNOWN_CONTROLLER_MANUFACTURE: [u16; 5] = [
    2,  // Intel.
    29, // Qualcomm
    70, // MediaTek
    72, // Marvell
...
impl ControllerRule {
    ...
    fn process_local_name(&mut self, local_name: &[u8; 248], packet: &Packet) {
        match String::from_utf8(local_name.to_vec()) {
            Ok(name) => {
            ...
            Err(err) => self.signals.push(Signal {
                index: packet.index,
                ts: packet.ts,
                tag: ControllerSignal::LikelyExternalController.into(),
            }),
    ...
    fn process_local_version(&mut self, version_info: &LocalVersionInformation, packet: &Packet) {
        if !KNOWN_CONTROLLER_MANUFACTURE.contains(&version_info.manufacturer_name) {
            self.signals.push(Signal {
                index: packet.index,
                ts: packet.ts,
                tag: ControllerSignal::LikelyExternalController.into(),
