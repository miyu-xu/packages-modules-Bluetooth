...
impl DeviceInformation {
    pub fn new(address: Address) -> Self {
        DeviceInformation {
            ...
            acls: HashMap::from([(Transport::BREDR, vec![]), (Transport::LE, vec![]), (Transport::Unknown, vec![])]),
            acl_state: HashMap::from([(Transport::BREDR, AclState::None), (Transport::LE, AclState::None), (Transport::Unknown, AclState::None)]),
        }
    }

    fn is_connection_active(&self, transport: Transport) -> bool {
        // not empty and last connection's end time is not set.
        return !self.acls[&transport].is_empty() && self.acls[&transport].last().unwrap().end_time == INVALID_TS;
    }

    fn get_or_allocate_connection(&mut self, handle: ConnectionHandle, transport: Transport) -> &mut AclInformation {
    ...
    fn report_connection_start(&mut self, handle: ConnectionHandle, transport: Transport, ts: NaiveDateTime) {
    ...
    fn report_connection_end(
        ...
        eprintln!("device {} receive disconnection of handle {} without corresponding connection at {}", self.address, handle, ts);
    }
        ...
        match &packet.inner {
            PacketChild::HciEvent(ev) => match ev.specialize() {
                ...
                EventChild::LeMetaEvent(ev) => match ev.specialize() {
                    LeMetaEventChild::LeConnectionComplete(ev) => {
                        ...
                        self.report_acl_state(&ev.get_peer_address(), Transport::LE, AclState::Initiating);
                        self.report_connection_start(
                            &ev.get_peer_address(),
                            ev.get_connection_handle(),
                            Transport::LE,
            ...
            PacketChild::HciCommand(cmd) => match cmd.specialize() {
                ...
                CommandChild::CreateConnection(cmd) => {
                    self.report_acl_state(&cmd.get_bd_addr(), Transport::BREDR, AclState::Initiating);
                    self.report_address_type(&cmd.get_bd_addr(), AddressType::BREDR);
                }
                CommandChild::AcceptConnectionRequest(cmd) => {
                    self.report_acl_state(&cmd.get_bd_addr(), Transport::BREDR, AclState::Accepting);
                    self.report_address_type(&cmd.get_bd_addr(), AddressType::BREDR);
                }
