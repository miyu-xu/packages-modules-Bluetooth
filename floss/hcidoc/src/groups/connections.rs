...
enum ConnectionSignal {
    ...
    FeatureNoReply,  // Host doesn't receive a response for a remote feature request. (b/300851411)
    FeatureError,    // Controller replies error for remote feature request. (b/292116133)
...
impl Into<&'static str> for ConnectionSignal {
    fn into(self) -> &'static str {
        match self {
            ...
            ConnectionSignal::FeatureError => "RemoteFeatureError",
    ...
    fn process_remote_feat_ev(
        ...
        if status != ErrorCode::Success {
            self.signals.push(Signal {
                index: packet.index,
                ts: packet.ts,
                tag: ConnectionSignal::FeatureError.into(),
            });
...
impl SecurityMode3Rule {
    ...
    fn process_connect_complete(
        ...
        if status == ErrorCode::Success && enable_encryption == Enable::Enabled {
            self.signals.push(Signal {
                index: packet.index,
                ts: packet.ts,
