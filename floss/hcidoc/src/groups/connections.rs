...
impl OddDisconnectionsRule {
    ...
    fn process_classic_connection(&mut self, address: Address, packet: &Packet) {
        self.last_connection_attempt = Some(address.clone());
    ...
    fn process_setup_sync_connection(&mut self, handle: ConnectionHandle, packet: &Packet) {
    ...
    fn process_accept_sync_connection(&mut self, address: Address, packet: &Packet) {
        self.last_sco_connection_attempt = Some(address.clone());
        if let Some(p) = self.sco_connection_attempt.insert(address, packet.clone()) {
            self.reportable.push((
                p.ts,
    ...
    fn process_le_create_connection(
        ...
        self.last_le_connection_attempt = Some(address.clone());
        self.last_le_connection_filter_policy = Some(policy);
        if let Some(p) = self.le_connection_attempt.insert(address.clone(), packet.clone()) {
            self.reportable.push((
                p.ts,
    ...
    fn process_command_status(&mut self, status: ErrorCode, opcode: OpCode, packet: &Packet) {
        ...
        match opcode {
            OpCode::CreateConnection | OpCode::AcceptConnectionRequest => {
                last_address = self.last_connection_attempt.take()
            }
            ...
            | OpCode::EnhancedAcceptSynchronousConnection => {
                last_address = self.last_sco_connection_attempt.take()
            }

            OpCode::LeCreateConnection | OpCode::LeExtendedCreateConnection => {
                last_address = self.last_le_connection_attempt.take()
            }
    ...
    fn process_sync_conn_complete_ev(
        ...
        match self.sco_connection_attempt.remove(&address) {
            Some(_) => {
                if status == ErrorCode::Success {
                    self.active_handles.insert(handle, (packet.ts, address));
                } else {
            ...
            PacketChild::HciCommand(cmd) => match cmd.specialize() {
                CommandChild::AclCommand(aclpkt) => match aclpkt.specialize() {
                    ...
                    AclCommandChild::ScoConnectionCommand(sco_conn) => {
                        match sco_conn.specialize() {
                            ScoConnectionCommandChild::SetupSynchronousConnection(ssc) => {
                                self.process_setup_sync_connection(
                            ...
                            ScoConnectionCommandChild::EnhancedSetupSynchronousConnection(esc) => {
                                self.process_setup_sync_connection(
                                    esc.get_connection_handle(),
                                    packet,
                                );
                            }
                            ScoConnectionCommandChild::AcceptSynchronousConnection(asc) => {
                                self.process_accept_sync_connection(asc.get_bd_addr(), packet);
                            }
                            ScoConnectionCommandChild::EnhancedAcceptSynchronousConnection(
                                easc,
                            ) => {
                                self.process_accept_sync_connection(easc.get_bd_addr(), packet);
                            }
