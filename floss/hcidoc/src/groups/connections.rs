...
impl Rule for LinkKeyMismatchRule {
    // Currently I only have logs for BREDR device. Will add LE when available.
    fn process(&mut self, packet: &Packet) {
        match &packet.inner {
            PacketChild::HciEvent(ev) => match ev.specialize() {
                ...
                EventChild::AuthenticationComplete(ev) => {
                    //let address = Address { bytes: [0x00, 0x00, 0x00, 0x00, 0x00, 0x00] };
                    //let address = self.handles.get(&ev.get_connection_handle()).unwrap().clone();
                    //self.reportAddressAuthFailure(&address, &packet);
            ...
            PacketChild::HciCommand(cmd) => match cmd.specialize() {
                CommandChild::SecurityCommand(cmd) => match cmd.specialize() {
                    SecurityCommandChild::LinkKeyRequestReply(cmd) => {
                        if let Some(LinkKeyMismatchState::Requested) =
                            self.states.get(&cmd.get_bd_addr())
                        {
                            self.states.insert(cmd.get_bd_addr(), LinkKeyMismatchState::Replied);
                        }
                    }
