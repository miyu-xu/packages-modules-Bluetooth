...
use hcidoc_packets::hci::{
    Acl, Address, AuthenticatedPayloadTimeoutExpired, CommandChild,
    DisconnectReason, Enable, ErrorCode, EventChild,
    InitiatorFilterPolicy, LeMetaEventChild,
    NumberOfCompletedPackets, OpCode,
};
    ...
    fn process(&mut self, packet: &Packet) {
        match &packet.inner {
            PacketChild::HciCommand(cmd) => match cmd.specialize() {
                ...
                CommandChild::LeRemoveDeviceFromFilterAcceptList(
                    lrac,
                ) => {
