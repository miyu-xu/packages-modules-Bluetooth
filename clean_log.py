...
def sanitize_fmt_arg(arg: str) -> str:
    ...
    if not re.fullmatch(r'\(.*?\).*?\(.*?\)', arg):
        arg = re.sub(r'^\((?P<inner>.*?)\)$', r'\g<inner>', arg)
...
def log_converter(match: re.Match) -> Optional[str]:
    ...
    for specifier in specifier_matcher.finditer(fmt_string):
        ...
        elif matched_arg := re.fullmatch(r"loghex\((.*?)\)", arg):
            assert specifier.group(0) == "{}"
            new_fmt_string += "0x{:x}"
            new_fmt_args.append(matched_arg.group(1))

        elif matched_arg := re.fullmatch(r"std::to_string\((.*?)\)", arg):
            assert specifier.group(0) == "{}"
            new_fmt_string += "{}"
            new_fmt_args.append(matched_arg.group(1))

        elif matched_arg := re.fullmatch(r"(.*?) \? \"true\" : \"false\"", arg):
            assert specifier.group(0) == "{}"
            new_fmt_string += "{}"
            new_fmt_args.append(matched_arg.group(1))
