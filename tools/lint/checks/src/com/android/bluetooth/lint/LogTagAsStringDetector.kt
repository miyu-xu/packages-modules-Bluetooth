 ...
 *
 * The recommended fix is to set the tag based on the _remove_ the creation of the variable. This
 * fix is easy to make, but it is hard to fix all the downstream usages of the variable.
 */
class LogTagAsStringDetector : Detector(), SourceCodeScanner {
    ...
    companion object {
        const val LOG_TAG_FORMATTING_WARNING =
            """Logging tags should named TAG, be static and final, and match the name of the class
            they're created for."""

        val ISSUE =
            Issue.create(
                id = "LogTagShouldMatchClassname",
                briefDescription = LOG_TAG_FORMATTING_WARNING,
                explanation =
                    """Using a logging tag that matches a given class name makes finding log lines
                    simple and consistent. This, together with asserting that Log framework calls
                    always use a static TAG variable, will also make sure classes always log to the
                    same tag.""",
    ...
    private fun isClassLiteralForOwningClass(
        ...
        // if (!(expression is UClassLiteralExpression)) {
        //     return false
        // }

        // val qualifiedClass = owningClass.qualifiedName ?: return false
        // val classLiteralType = expression.type ?: return false
        // if(classLiteralType.equalsToText(qualifiedClass)) {
        //     debug("LogTagAsStringDetector", "guess this works?", "    ")
        // }
