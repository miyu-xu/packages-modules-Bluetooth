...
class GuardedLogLineDetector : Detector(), SourceCodeScanner {
    ...
    companion object {
        const val GUARDED_LOG_INVOCATION_ERROR =
            ...
            isLoggable(). The Log framework does this check for you. Remove the surrounding if block
            and call to log completely unguarded"""

        val ISSUE =
            Issue.create(
                id = "DontGuardLogLines",
                briefDescription =
                    """Dont guard log invocations with if blocks using log
                                   enforcement variables or isLoggable()""",
                explanation =
