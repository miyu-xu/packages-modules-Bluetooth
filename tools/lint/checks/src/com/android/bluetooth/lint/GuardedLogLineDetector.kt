...
class GuardedLogLineDetector : Detector(), SourceCodeScanner {
    ...
    private val LOG_ENFORCEMENT_TYPE_NONE = 0
    private val LOG_ENFORCEMENT_TYPE_VAR = 1
    private val LOG_ENFORCEMENT_TYPE_IS_LOGGABLE = 2
        ...
        return object : UElementHandler() {
            override fun visitCallExpression(node: UCallExpression) {
                ...
                while (ifStatement != null) {
                    ...
                    if (enforcementType == LOG_ENFORCEMENT_TYPE_VAR) {
                        context.report(
                            issue = ISSUE,
                        ...
                        return
                    } else if (enforcementType == LOG_ENFORCEMENT_TYPE_IS_LOGGABLE) {
                        context.report(
                            issue = WARNING,
                            location = context.getNameLocation(ifStatement),
                            message = GUARDED_LOG_INVOCATION_ERROR,
        ...
        when (node) {
            ...
            is UPolyadicExpression -> {
                for (subExpression in node.operands) {
                    ...
                    if (enforcementType != LOG_ENFORCEMENT_TYPE_NONE) {
