...
class LogEnforcementVariableCreationDetector : Detector(), SourceCodeScanner {
    ...
    companion object {
        const val LOG_ENFORCEMENT_VARIABLE_USAGE_ERROR =
            """Dont create log enforcement variables to enforce which logs fire. The Log framework
            does this check for you. Remove this variable and update any log invocations to be
            unguarded."""

        val ISSUE =
            Issue.create(
                id = "DontCreateLogEnforcementVariables",
                briefDescription =
                    """Dont create log enforcement variables to govern which logs
                    fire. The Log framework does this check for you.""",
                explanation =
                    """The BT stack defines a process default log level, which allows the Android
                    Log framework (For Java, Kotlin, _and_ Native) to properly enforce log levels
                    for us. Using our own variables for enforcement causes inconsistency in log
                    output and double checks against the log level each time we log. Please delete
                    this variable and use the Log functions unguarded in your code.""",
