...
def generate_packet_serializer_test(packet: ast.PacketDeclaration, tests: List[object]) -> str:
    """Generate the implementation of unit tests for the selected packet."""
    ...
    def build_packet(decl: ast.Declaration, var: str, initializer: object) -> (str, List[str]):
        ...
        for field in fields:
            ...
            value = initializer['payload'] if isinstance(field, (ast.PayloadField, ast.BodyField)) else initializer.get(field_id, None)
            ...
            if isinstance(field, ast.ScalarField):
                ...
                elements = []
                for (n, value) in enumerate(value):
                    (element, intermediate_declarations) = build_packet(
                        field.type, f'{field_var}_{n}', value)
    ...
    for (test_nr, test) in enumerate(tests):
        ...
        generated_tests.append(
            ...
            TEST_F(SerializerTest, {packet_id}_Case{test_nr}) {{
    ...
    for decl in file.declarations:
        ...
        if isinstance(decl, ast.PacketDeclaration):
            ...
            if matching_tests:
                ...
                output.write(generate_packet_serializer_test(decl, matching_tests))
...
def main() -> int:
    ...
    parser.add_argument('--using-namespace',
                        ...
                        default=[],
                        action='append',
                        help='Added using namespace directives')
