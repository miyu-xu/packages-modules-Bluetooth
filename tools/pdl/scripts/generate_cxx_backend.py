...
class FieldSerializer:
    ...
    def get_payload_field_size(self,
                               var: Optional[str],
                               payload: ast.PayloadField,
                               decl: ast.Declaration) -> str:
    ...
    def serialize_array_element_(self, field: ast.ArrayField, var: str):
        ...
        if field.width:
            ...
            element_size = int(field.width / 8)
            self.append_(f"pdl::packet::Builder::write<{backing_type}, {element_size}>(output, {var});")
        elif isinstance(field.type, ast.EnumDeclaration):
            backing_type = get_cxx_scalar_type(field.type.width)
            element_size = int(field.type.width / 8)
            self.append_(
                f"pdl::packet::Builder::write<{backing_type}, {element_size}>(" +
    ...
    def serialize_array_field_(self, field: ast.ArrayField, var: str):
        ...
        if field.width == 8:
            self.append_(
                f"output.insert(output.end(), {var}.begin(), {var}.end());")
        else:
    ...
    def pack_bit_fields_(self):
        ...
        if len(value) == 0:
            self.append_(f"pdl::packet::Builder::write<{backing_type}, {size}>(output, 0);")
        elif len(value) == 1:
            self.append_(f"pdl::packet::Builder::write<{backing_type}, {size}>(output, {value[0]});")
        else:
            self.append_(f"pdl::packet::Builder::write<{backing_type}, {size}>(output, {' | '.join(value)});")
...
def generate_packet_field_members(packet: ast.Declaration, view: bool) -> List[str]:
...
def generate_packet_field_serializers(packet: ast.Declaration) -> List[str]:
    ...
    for field in core.get_packet_fields(packet):
        ...
        if constraint and constraint.tag_id:
            fixed_field = ast.FixedField(enum_id=field.type_id, tag_id=constraint.tag_id, loc=field.loc, kind='fixed_field')
...
def generate_array_field_size_getters(decl: ast.Declaration) -> str:
    ...
    for field in fields:
        ...
        if size:
            getters.append(dedent("""\
                size_t Get{accessor_name}Size() const {{
                    return {size};
                }}
                """).format(accessor_name=to_pascal_case(field.id),
                            size=size))
        else:
            getters.append(dedent("""\
                size_t Get{accessor_name}Size() const {{
...
def generate_packet_view_field_parsers(packet: ast.PacketDeclaration) -> str:
    ...
    if packet.fields:
        ...
        parser = FieldParser(extract_arrays=False, byteorder=packet.file.byteorder)
        for f in packet.fields:
            parser.parse(f)
        parser.done()
...
def generate_packet_builder(packet: ast.PacketDeclaration) -> str:
    ...
    builder_constructor = generate_struct_constructor(packet, constructor_name=class_name)
    ...
    return dedent("""\
        ...
        """).format(class_name=f'{packet.id}Builder',
                    ...
                    array_field_size_getters=indent_block(array_field_size_getters, 1))


def generate_struct_constructor(struct: ast.StructDeclaration, constructor_name: str) -> str:
    ...
    return dedent("""\
        {constructor_name}({constructor_params})
            : {constructor_initializers} {{}}""").format(constructor_name=constructor_name,
...
def generate_struct_field_parsers(struct: ast.StructDeclaration) -> str:
    ...
    parser = FieldParser(extract_arrays=True, byteorder=struct.file.byteorder)
...
def generate_struct_declaration(struct: ast.StructDeclaration) -> str:
    ...
    struct_constructor = generate_struct_constructor(struct, constructor_name=struct.id)
