...
class FieldParser:
    ...
    def parse_typedef_field_(self, field: ast.TypedefField):
        ...
        self.append_(dedent("""\
            if (!{field_type}::Parse(span, &{field_id}_)) {{
                return false;
            }}""".format(field_type=field.type.id,
                         field_id=field.id)))
    ...
    def parse_array_field_lite_(self, field: ast.ArrayField):
        ...
        if padded_size:
            ...
            self.append_("{")
            self.append_(f"pdl::packet::slice remaining_span = span.subrange({padded_size}, span.size() - {padded_size});")
    ...
    def parse_array_field_full_(self, field: ast.ArrayField):
        ...
        if size is not None:
            ...
            if field.width:
                ...
                self.append_(f"        return false;")
                self.append_("    }")
                self.append_(f"    {field.id}_.push_back({element_type}(temp_span.read<{backing_type}, {element_size}>()));")
            else:
    ...
    for field in fields:
        if isinstance(field, (ast.PayloadField, ast.BodyField)):
            members.append("pdl::packet::slice payload_;")
              ...
              isinstance(packet, ast.PacketDeclaration)):
            members.append(f"pdl::packet::slice {field.id}_;")
        elif (isinstance(field, ast.ArrayField) and
              isinstance(packet, ast.StructDeclaration)):
            element_type = field.type_id or get_cxx_scalar_type(field.width)
...
def run(input: argparse.FileType, output: argparse.FileType,
        custom_type_headers: List[str],
        custom_type_namespace: Optional[str],
...
def main() -> int:
    ...
    parser.add_argument('--custom-type-namespace',
                        type=str,
                        help='Declaration namespace of custom types')
