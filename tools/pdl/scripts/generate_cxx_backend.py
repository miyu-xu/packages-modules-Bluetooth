    ...
    def parse_array_field_full_(self, field: ast.ArrayField):
        ...
        if size is not None:
            ...
            if field.width:
                ...
                self.append_("    }")
                self.append_(f"    {field.id}_.push_back(temp_span.read_{self.byteorder}<{element_type}, {element_size}>());")
        ...
        # following the array.
            ...
            self.append_(f"        return false;")
            self.append_("    }")
            self.append_(f"    {field.id}_.push_back({element_type}(span.read_{self.byteorder}<{backing_type}, {element_size}>()));")
            self.append_("}")
        else:
            self.append_(f"while (span.size() > 0) {{")
            self.append_(f"    {element_type} element;")
...
class FieldSerializer:
    ...
    def serialize_array_element_(self, field: ast.ArrayField, var: str):
        ...
        if field.width:
            ...
            element_size = int(field.width / 8)
            self.append_(f"pdl::packet::Builder::write_{self.byteorder}<{backing_type}, {element_size}>(output, {var});")
        elif isinstance(field.type, ast.EnumDeclaration):
            backing_type = get_cxx_scalar_type(field.type.width)
            element_size = int(field.type.width / 8)
            self.append_(
                f"pdl::packet::Builder::write_{self.byteorder}<{backing_type}, {element_size}>(" +
    ...
    def pack_bit_fields_(self):
        ...
        if len(value) == 0:
            ...
            self.append_(f"pdl::packet::Builder::write_{self.byteorder}<{backing_type}, {size}>(output, {' | '.join(value)});")
...
def generate_scalar_array_field_accessor(field: ast.ArrayField) -> str:
    ...
    return dedent("""\
        ...
        return std::move(elements);""").format(field_id=field.id, backing_type=backing_type, element_size=element_size, byteorder=byteorder)
...
def run(input: argparse.FileType, output: argparse.FileType, namespace: Optional[str],
        include_header: List[str], using_namespace: List[str]):
