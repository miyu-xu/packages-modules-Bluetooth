...
def desugar(grammar: Grammar):
    ...
    **The original grammar object is modified inline.**"""
    ...
    for d in grammar.declarations:
        ...
        if (isinstance(d, PacketDeclaration) or isinstance(d, StructDeclaration)):
            fields = []
            for f in d.fields:
...
def get_packet_field(packet: Union[PacketDeclaration, StructDeclaration], id: str) -> Optional[Field]:
    ...
    packet or its ascendance."""
    for f in packet.fields:
        if hasattr(f, 'id') and f.id == id:
            return f
...
def get_derived_packets(decl: Union[PacketDeclaration, StructDeclaration]
    ...
    for d in decl.grammar.declarations:
        if type(d) is type(decl) and d.parent_id == decl.id:
            if (len(d.fields) == 1 and
                (isinstance(d.fields[0], PayloadField) or isinstance(d.fields[0], PayloadField))):
...
def get_field_size(field: Field, skip_payload: bool = False) -> Optional[int]:
    ...
    if (isinstance(field, ScalarField) or isinstance(field, SizeField) or isinstance(field, CountField) or
            isinstance(field, ReservedField)):
        return field.width
    ...
    elif ((isinstance(field, PayloadField) or isinstance(field, BodyField)) and skip_payload):
        return 0
...
def get_declaration_size(decl: Declaration, skip_payload: bool = False) -> Optional[int]:
    ...
    if (isinstance(decl, EnumDeclaration) or isinstance(decl, CustomFieldDeclaration) or
            isinstance(decl, ChecksumDeclaration)):
        return decl.width

    elif (isinstance(decl, PacketDeclaration) or isinstance(decl, StructDeclaration)):
        parent = decl.parent
...
def get_array_field_size(field: ArrayField) -> Union[None, int, Field]:
    ...
    for f in field.parent.fields:
        if ((isinstance(f, SizeField) or isinstance(f, CountField)) and f.field_id == field.id):
            return f
    return None
...
def is_bit_field(field: Field) -> bool:
    ...
    if (isinstance(field, ScalarField) or isinstance(field, SizeField) or isinstance(field, CountField) or
            isinstance(field, FixedField) or isinstance(field, ReservedField)):
        return True
