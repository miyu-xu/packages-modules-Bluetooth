...
class FieldSerializer:
    ...
    def extend_(self, value: str, length: int):
        if length == 1:
            ...
            self.append_(
                f"span.extend(int.to_bytes({value}, length={length}," +
    ...
    def serialize_bit_field_(self, field: ast.Field):
        ...
        for shift, width, field in self.chunk:
            if isinstance(field, ast.ScalarField):
                ...
                self.append_(
                    f"    print(f\"Invalid value for field {field.parent.id}::{field.id}:" +
                ...
                self.append_(
                    f"    print(f\"Invalid length for field {field.parent.id}::{field.field_id}:" +
                    f"  {{len(self.{field.field_id})}} > {max_count}; the array will be truncated\")")
                self.append_(f"    del self.{field.field_id}[{max_value}:]")
    ...
    def serialize_typedef_field_(self, field: ast.TypedefField):
        ...
        if self.shift != 0:
            raise Exception('Typedef field does not start on an octet boundary')
        if (isinstance(field.type, ast.StructDeclaration) and
    ...
    def serialize(self, field: ast.Field):
        ...
        # Payload and body fields.
        elif (isinstance(field, ast.PayloadField) or
              isinstance(field, ast.BodyField)):
...
def generate_toplevel_packet_serializer(packet: ast.Declaration) -> List[str]:
    """Generate the serialize() function for a toplevel Packet or Struct
       declaration."""
    serializer = FieldSerializer(byteorder=packet.grammar.byteorder)
...
def generate_derived_packet_serializer(packet: ast.Declaration) -> List[str]:
    ...
    for f in packet.fields:
        serializer.serialize(f)
    return ['span = bytearray()'] + serializer.code + [
        f'return {packet.parent.id}.serialize(self, payload = bytes(span))']
...
def generate_default_field_values(decl: ast.Declaration, constraints: List[ast.Constraint]) -> List[str]:
    ...
    for f in decl.fields:
        if isinstance(f, (ast.SizeField, ast.CountField, ast.FixedField, ast.ReservedField, ast.PaddingField)):
            continue
            ...
            elif isinstance(f.type, ast.ChecksumFieldDeclaration):
                continue
            elif isinstance(f.type, ast.CustomFieldDeclaration):
                initializers.append(f"{f.id}={f.function}()")
    ...
    if decl.parent_id:
        initializers.extend(
            generate_default_field_values(decl.parent, decl.constraints))
