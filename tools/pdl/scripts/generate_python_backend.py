...
class FieldParser:
    ...
    def parse_array_field_(self, field: ast.ArrayField):
        ...
        # Parse elements item by item as a vector.
            ...
            self.append_(f"{field.id} = []")
            self.append_(f"for n in range(0, {count}):")
        ...
        # length).
            ...
            if element_width != 1:
                ...
                self.append_(f"{field.id}_count = {array_size} / {element_width}")
                array_count = f'{field.id}_count'
            self.append_(f"{field.id} = []")
            self.append_(f"for n in range(0, {array_count}):")
            span = ('span[n:n + 1]' if element_width == 1 else f'span[n * {element_width}:(n + 1) * {element_width}]')
            self.parse_array_element_static_(field, span)
            self.append_(f"fields['{field.id}'] = {field.id}")
            if size is not None:
    ...
    def parse_typedef_field_(self, field: ast.TypedefField):
        ...
        if width is None:
            ...
            if not isinstance(field.type, ast.ChecksumDeclaration):
                span = f'span[{self.offset}:{end_offset}]'
                self.unchecked_append_(f"fields[{field.id}] = {field.type_id}.parse({span})")
            self.offset = end_offset
    ...
    def parse_checksum_field_(self, field: ast.ChecksumField):
        ...
        self.append_(f"    raise Exception('Invalid checksum computation')")
...
def generate_field_parser(packet: ast.Field) -> List[str]:
    """Generate the statement for parsing a field."""
    return []
...
def generate_custom_field_declaration_check(decl: ast.CustomFieldDeclaration) -> str:
    ...
    return dedent("""\
        ...
        if not isinstance({custom_field_name}, type):
            raise Exception('{custom_field_name} is not a type name')
        if not hasattr({custom_field_name}, 'parse'):
...
def run(input: argparse.FileType, output: argparse.FileType, custom_type_location: Optional[str]):
    ...
    if len(custom_types) > 0 and custom_type_location is not None:
    ...
    for d in grammar.declarations:
        ...
        elif (isinstance(d, ast.PacketDeclaration) or isinstance(d, ast.StructDeclaration)):
