...
fn generate_packet_decl(
    ...
    let plain_fields = fields.iter().map(|field| Field::from(field).generate(parse_quote!()));
    code.push_str(&quote_block! {
        #[derive(Debug)]
        struct #data_name {
            #(#plain_fields,)*
    ...
    let pub_fields = fields.iter().map(|field| Field::from(field).generate(parse_quote!(pub)));
    code.push_str(&quote_block! {
        #[derive(Debug)]
    ...
    fn test_generate_chunk_write_field_adjustments_48bit() {
        ...
        assert_expr_eq(
            ...
            quote! {
                ...
                let chunk = chunk | (((self.c as u64) << 11) & 0x3ff);
                let chunk = chunk | (((self.d as u64) << 21) & 0x3ffff);
                let chunk = chunk | (((self.e as u64) << 39) & 0x1ff);
            },
        );
    }
}
