...
mod tests {
    ...
    /// Create a test case for the given PDL `code`.
    ///
    /// The test will compare the generated Rust code for declaration
    /// `n` with a previously saved snapshot. The snapshot is read
    /// from `"tests/generated/{name}_{endianess}.rs"`. Touch the
    /// missing files and use `UPDATE_SNAPSHOTS=1 cargo test` to
    /// populate them.
    ///
    /// The `code` must not have an endianness declaration, instead
    /// you must supply either "little_endian" or "big_endian" as
    /// `endianness`.
    macro_rules! make_decl_test {
        ($name:ident, $n:expr, $code:expr, $endianess:expr) => {
            paste! {
                #[test]
                fn [< test_ $name _ $endianess >]() {
                    let file = parse_str(concat!($endianess, "_packets\n", $code));
                    let scope = lint::Scope::new(&file).unwrap();
                    let decl = &file.declarations[$n];
                    let actual_code = generate_decl(&scope, &file, decl);
                    assert_snapshot_eq(
                        concat!("tests/generated/", stringify!($name), "_", $endianess, ".rs"),
                        &rustfmt(&actual_code),
                    );
                }
            }
    ...
    /// Create test cases for the given PDL `code`.
    ///
    /// The `code` must not have an endianness declaration: we will
    /// automatically generate unit tests for both
    /// "little_endian_packets" and "big_endian_packets".
    macro_rules! test_decl {
        ($name:ident, $n:expr, $code:expr $(,)?) => {
            make_decl_test!($name, $n, $code, "little_endian");
            make_decl_test!($name, $n, $code, "big_endian");
        };
    }

    test_decl!(packet_decl_empty, 0, "packet Foo {}");

    test_decl!(packet_decl_8bit_scalar, 0, " packet Foo { x:  8 }");
    test_decl!(packet_decl_16bit_scalar, 0, "packet Foo { x: 16 }");
    test_decl!(packet_decl_24bit_scalar, 0, "packet Foo { x: 24 }");
    test_decl!(packet_decl_32bit_scalar, 0, "packet Foo { x: 32 }");
    test_decl!(packet_decl_40bit_scalar, 0, "packet Foo { x: 40 }");
    test_decl!(packet_decl_64bit_scalar, 0, "packet Foo { x: 64 }");
    ...
    test_decl!(enum_decl, 0, " enum Foo : 8 { A = 1, B = 2 }");

    test_decl!(packet_decl_8bit_enum, 1, " enum Foo :  8 { A = 1, B = 2 } packet Bar { x: Foo }");
    test_decl!(packet_decl_16bit_enum, 1, "enum Foo : 16 { A = 1, B = 2 } packet Bar { x: Foo }");
    test_decl!(packet_decl_24bit_enum, 1, "enum Foo : 24 { A = 1, B = 2 } packet Bar { x: Foo }");
    test_decl!(packet_decl_64bit_enum, 1, "enum Foo : 64 { A = 1, B = 2 } packet Bar { x: Foo }");
    ...
    test_decl!(packet_decl_8bit_scalar_array, 0, " packet Foo { x:  8[3] }");
    test_decl!(packet_decl_16bit_scalar_array, 0, "packet Foo { x: 16[5] }");
    test_decl!(packet_decl_24bit_scalar_array, 0, "packet Foo { x: 24[7] }");
    ...
    test_decl!(
        packet_decl_16bit_enum_array,
        1,
        "enum Foo : 16 { A = 1, B = 2 } packet Bar { x: Foo[5] }"
    );
    test_decl!(
        packet_decl_24bit_enum_array,
        1,
        "enum Foo : 24 { A = 1, B = 2 } packet Bar { x: Foo[7] }"
    );
}
