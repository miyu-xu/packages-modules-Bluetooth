...
fn generate_packet_size_getter(
    ...
    for field in fields {
        ...
        dynamic_widths.push(match field {
            ast::Field::Payload { .. } | ast::Field::Body { .. } => quote!(self.payload.len()),
            ...
            ast::Field::Array { id, width, .. } => {
                let id = format_ident!("{id}");
                match decl {
                    Some(ast::Decl::Struct { .. } | ast::Decl::CustomField { .. }) => quote! {
...
mod tests {
        ...
        ($name:ident, $code:expr, $endianness:ident) => {
            paste! {
                ...
                fn [< test_ $name _ $endianness >]() {
                    ...
                    let file = parse_str(&format!("{endianness}_packets\n{}", $code));
                    let scope = lint::Scope::new(&file).unwrap();
                    for decl in &file.declarations {
                        if let Some(id) = decl.id() {
                            let actual_code = generate_decl(&scope, &file, decl);
                            assert_snapshot_eq(
                                &format!("tests/generated/{name}_{endianness}_{id}.rs"),
                                &rustfmt(&actual_code),
                            );
                        }
                    }
    ...
    macro_rules! test_decl {
        ($name:ident, $code:expr $(,)?) => {
            make_decl_test!($name, $code, little_endian);
            make_decl_test!($name, $code, big_endian);
        };
    }

    test_decl!(packet_decl_empty, "packet Foo {}");

    test_decl!(packet_decl_8bit_scalar, " packet Foo { x:  8 }");
    test_decl!(packet_decl_24bit_scalar, "packet Foo { x: 24 }");
    test_decl!(packet_decl_64bit_scalar, "packet Foo { x: 64 }");
    ...
    test_decl!(packet_decl_8bit_enum, " enum Foo :  8 { A = 1, B = 2 } packet Bar { x: Foo }");
    test_decl!(packet_decl_24bit_enum, "enum Foo : 24 { A = 1, B = 2 } packet Bar { x: Foo }");
    test_decl!(packet_decl_64bit_enum, "enum Foo : 64 { A = 1, B = 2 } packet Bar { x: Foo }");
