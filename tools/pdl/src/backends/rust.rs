...
fn generate_enum_decl(id: &str, tags: &[ast::Tag], width: usize) -> proc_macro2::TokenStream {
    ...
    tags.iter().for_each(|tag| match tag {
        ...
        ast::Tag::Range(tag) => {
            tag.tags.iter().for_each(|tag| variants.push(format_tag_ident(&tag.id)));
    ...
    tags.iter().for_each(|tag| match tag {
        ast::Tag::Value(tag) => {
            ...
            from_cases.push(quote! { #value => #name::#id })
        }
        ast::Tag::Range(tag) => {
            tag.tags.iter().for_each(|tag| {
                let id = format_tag_ident(&tag.id);
                let value = format_value(tag.value);
                from_cases.push(quote! { #value => #name::#id })
            });
            ...
            from_cases.push(quote! { #start ..= #end => #name::#id(Private(value)) })
        }
    });
    ...
    // Generate a default case if the enum is not complete.
    if !is_complete {
        variants.push(quote! { Unknown(Private<#backing_type>) });
        from_cases.push(quote! { value => #name::Unknown(Private(value)) });
        into_cases.push(quote! { #name::Unknown(Private(value)) => value });
    }

    // Generate an unreachable case if the enum is complete but the enum size
    // is lower than the backing type size: the value being truncated to the
    // correct size this case is unreachable.
    if is_complete && backing_type.width != width {
        from_cases.push(quote! { _ => unreachable!() });
    }

    // Generate the value mask for the truncating converter from uN
    // to the enum values.
    let and_mask = (backing_type.width != width).then(|| {
        let mask = format_value(scalar_max(width));
        quote! { & #mask }
    });
    ...
    quote! {
        ...
        impl From<#backing_type> for #name {
            fn from(value: #backing_type) -> Self {
                match value #and_mask {
                    #(#from_cases,)*
                }
            }
        }
...
fn generate_decl(
    ...
    match &decl.desc {
        ...
        ast::DeclDesc::Enum { id, tags, width } => generate_enum_decl(id, tags, *width).to_string(),
