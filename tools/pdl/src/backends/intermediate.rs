...
pub struct ComputedOffsetWithAlignment<'a> {
    offset: ComputedOffset<'a>,
    alignment: u8,
}
...
fn compute_getters<'a>(schema: &Schema<'a>, fields: &'a [ast::Field]) -> PacketOrStruct<'a> {
        ...
        let next_pos = match field {
            ...
            ast::Field::Typedef { id, type_id, .. } => {
                ...
                match schema.structs[type_id.as_str()].0.length {
                    ...
                    PacketOrStructLength::NeedsExternal => {
                        let end_offset = if computed_values
                            .contains_key(&ComputedValueId::FieldSize(id))
                        {
    ...
    return PacketOrStruct { computed_values, computed_offsets, length };
}
