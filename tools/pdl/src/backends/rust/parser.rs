...
impl<'a> FieldParser<'a> {
    ...
    fn add_bit_field(&mut self, field: &'a ast::Field) {
        ...
        for Chunk { offset, width, field } in self.chunk.drain(..) {
            ...
            if offset > 0 {
                let shift = proc_macro2::Literal::usize_unsuffixed(offset);
                v = quote! { (#v >> #shift) }
            }
            if width % 8 != 0 {
    ...
    fn add_array_field(
        ...
        match shape {
            Shape {
                ...
                self.code.push(quote! {
                    let __case_1__ = "111";
                    let (head, #span) = #span.split_at(#size_field as usize);
                    let mut #id = Vec::new();
                    while !head.is_empty() {
                        #id.push(#parse_element?);
                    }
                });
                //todo!("1: {}", self.packet_name);
            }
            Shape {
                element_width: None,
                count_static: Some(_),
            ...
            Shape {
                ...
                self.code.push(quote! {
                    let __case_3__ = "333";
                    let #id = (0..#count_field).map(|_| #parse_element).collect::<Result<Vec<_>>>()?;
                });
                //todo!("3: {}", self.packet_name);
            }
            Shape {
                element_width: None,
                count_static: None,
