...
impl<'a> FieldParser<'a> {
    ...
    pub fn add(&mut self, field: &'a ast::Field) {
        match field {
            ...
            ast::Field::Padding { .. } => todo!("Padding fields are not supported"),
            ...
            ast::Field::Body { .. } => self.add_payload_field(None),
            _ => todo!("{field:?}"),
        }
    }
    ...
    fn find_size_field(&self, id: &str) -> Option<proc_macro2::Ident> {
        match self.packet_scope()?.sizes.get(id)? {
            ast::Field::Size { .. } => Some(size_field_ident(id)),
            _ => None,
        }
    }
    ...
    fn add_payload_field(&mut self, _size_modifier: Option<&str>) {
        ...
        if self.shift != 0 {
            ...
            // consume span of rounded size
        } else {
            // consume span
        }

        if let Some(ast::Field::Size { field_id, .. }) = payload_size_field {
    ...
    pub fn done(&mut self) {
        ...
        if let Some(children) = self.scope.children.get(self.packet_name) {
            ...
            for child in children.iter() {
                match child {
                    ast::Decl::Packet { id, constraints, .. }
                    | ast::Decl::Struct { id, constraints, .. } => {
