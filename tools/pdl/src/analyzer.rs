...
pub enum ErrorCode {
    ...
    FixedValueOutOfRange = 29,
    E30 = 30,
    E31 = 31,
    E32 = 32,
    DuplicatePayloadField = 33,
    MissingPayloadField = 34,
    RedundantArraySize = 35,
}
...
pub struct ParentIter<'s, 'd, A: Annotation + Default> {
    scope: &'s Scope<'d, A>,
    decl: &'d crate::ast::Decl<A>,
}

impl<'s, 'd, A: Annotation + Default> Iterator for ParentIter<'s, 'd, A> {
    type Item = &'d crate::ast::Decl<A>;

    fn next(&mut self) -> std::option::Option<<Self as Iterator>::Item> {
        if let Some(parent) = self.scope.get_parent(self.decl) {
            self.decl = parent;
            Some(parent)
        } else {
            None
...
impl<'d, A: Annotation + Default> Scope<'d, A> {
    ...
    pub fn iter_parents<'s>(&'s self, decl: &'d crate::ast::Decl<A>) -> ParentIter<'s, 'd, A> {
        ParentIter { scope: self, decl }
    }
            ...
            match &field.desc {
                ...
                FieldDesc::Count { field_id, .. } | FieldDesc::ElementSize { field_id, .. } => {
                    if let Some(prev) = size_for_id.insert(field_id, field) {
                        diagnostics.push(
                            Diagnostic::error()
                                .with_code(ErrorCode::DuplicateCountField)
                    ...
                    match decl.fields().find(|field| field.id() == Some(field_id)) {
                        None => diagnostics.push(
                            Diagnostic::error()
                                .with_code(ErrorCode::UndeclaredCountIdentifier)
                                .with_message(format!(
                                    "undeclared {} identifier `{}`",
                                    field.kind(),
                                    field_id
                        ...
                        Some(Field { loc, .. }) => diagnostics.push(
                            Diagnostic::error()
                                .with_code(ErrorCode::InvalidCountIdentifier)
                                .with_message(format!(
                                    "invalid {} identifier `{}`",
                                    field.kind(),
                                    field_id
