...
#[derive(Debug, Default)]
pub struct Scope<'d, A: Annotation + Default> {
...
struct FieldIter<'s, 'd, A: Annotation + Default> {
    scope: &'s Scope<'d, A>,
...
impl<'d, A: Annotation + Default> Scope<'d, A> {
    fn new(file: &'d crate::ast::File<A>) -> Result<Scope<'d, A>, Diagnostics> {
    ...
    /// if it has one.
    fn get_parent(&self, decl: &crate::ast::Decl<A>) -> Option<&'d crate::ast::Decl<A>> {
        decl.parent_id().and_then(|parent_id| self.typedef.get(parent_id).cloned())
    }

    /// Iterate over the declaration and its parent's fields.
    fn iter_fields<'s>(&'s self, decl: &'d crate::ast::Decl<A>) -> FieldIter<'s, 'd, A> {
...
fn check_decl_identifiers(
    ...
    fn bfs<'d>(
        ...
        let parent_decl = parent_id.and_then(|id| scope.typedef.get(id));
        match (&decl.desc, parent_decl) {
            (DeclDesc::Packet { .. }, None) | (DeclDesc::Struct { .. }, None) => diagnostics.push(
                Diagnostic::error()
                    .with_message(format!("undeclared parent identifier `{}`", parent_id.unwrap()))
                    .with_labels(vec![decl.loc.primary()])
            ...
            (_, Some(parent_decl)) => bfs(parent_decl, context, scope, diagnostics),
            _ => (),
        }
            ...
            DeclDesc::Test { type_id, .. } => match scope.typedef.get(type_id) {
                None => diagnostics.push(
                    Diagnostic::error()
                        .with_message(format!("undeclared test identifier `{}`", type_id))
                        .with_labels(vec![decl.loc.primary()])
                        .with_notes(vec!["hint: expected packet, struct identifier".to_owned()]),
                ),
                Some(Decl { desc: DeclDesc::Packet { .. }, .. })
                | Some(Decl { desc: DeclDesc::Struct { .. }, .. }) => (),
                Some(typedef_decl) => diagnostics.push(
                    Diagnostic::error()
                        .with_message(format!("invalid test identifier `{}`", type_id))
                        .with_labels(vec![
                            decl.loc.primary(),
                            typedef_decl.loc.secondary().with_message(format!(
                                "`{}` is declared here as {}",
                                type_id,
                                typedef_decl.kind()
                            )),
...
fn check_payload_fields(file: &parser_ast::File) -> Result<(), Diagnostics> {
    ...
    fn requires_payload(file: &parser_ast::File, decl: &parser_ast::Decl) -> bool {
        file.declarations
            ...
            .any(|decl| decl.fields().next().is_some())
    }
            ...
            match &field.desc {
                FieldDesc::Array { id, size: Some(size), .. } => {
                        ...
                        .next()
                    {
                        diagnostics.push(
                            Diagnostic::error()
                                .with_message("redundant array size field".to_owned())
                                .with_labels(vec![
    ...
    fn annotate_field(
        ...
        field.annotate(match &field.desc {
            ...
            | FieldDesc::Group { group_id: type_id, .. } => {
                ast::FieldAnnotation { size: scope.get(type_id).unwrap().size }
            }
            FieldDesc::Array { width: Some(width), size: Some(size), .. } => {
                ast::FieldAnnotation { size: ast::Size::Static(*size * *width) }
            }
            FieldDesc::Array { id, width, type_id, size, .. } => {
                let array_size = decl.fields().find(|field| match &field.desc {
                    FieldDesc::Size { field_id, .. } => field_id == id,
                    _ => false,
                });

                // Do not compute the element size when we know that the size
                // dynamic to avoid running into infinite loops with
                // recursive structures.
                let size = if array_size.is_some() {
                    ast::Size::Dynamic
                } else {
                    let element_size = match (width, type_id) {
                        (Some(width), _) => ast::Size::Static(*width),
                        (_, Some(type_id)) => scope.get(type_id).unwrap().size,
                        _ => unreachable!(),
                    };
                    let element_count = match size {
                        Some(size) => ast::Size::Static(*size),
                        None => ast::Size::Unknown,
                    };
                    element_size * element_count
                };

                ast::FieldAnnotation { size }
            }
        })
    }
...
pub fn analyze(file: &parser_ast::File) -> Result<ast::File, Diagnostics> {
    ...
    check_test_declarations(file, &scope)?;
    ...
    // check_checksumfields(file, &scope)?;
