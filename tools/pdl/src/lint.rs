...
fn lint_padding(
    ...
    None => result.push(
        Diagnostic::error()
            ...
            .with_notes(vec!["hint: padding fields must be placed after an array field".to_owned()])
        ),
    Some(Field::Array { .. }) => (),
    Some(preceding_field) => result.push(
        Diagnostic::error()
            .with_message(
                format!("padding field cannot be placed after {} field", preceding_field.kind()))
            .with_labels(vec![
                padding_loc.primary(),
                preceding_field.loc().secondary()])
            .with_notes(vec!["hint: padding fields must be placed after an array field".to_owned()])
        )
    }
}
...
fn lint_field(
    ...
    match field.0.last().unwrap() {
        ...
        Field::Padding { width, .. } => lint_padding(scope, packet_scope, field, *width, result),
        Field::Reserved { .. }
        | Field::Scalar { .. }
        | Field::Body { .. }
        | Field::Payload { .. } => (),
...
mod test {
    ...
    macro_rules! lint_success {
        ($name:ident, $text:literal) => {
            ...
            fn $name() {
                ...
                let file = parse_inline(&mut db, "stdin".to_owned(),
                    $text.to_owned()).expect("parsing failure");
    ...
    lint_failure!(test_packet_redeclared,
        r#"
    ...
    lint_success!(test_packet_checksum_start,
        r#"
        little_endian_packets
    ...
    lint_failure!(test_padding_cannot_be_first_field,
        r#"
        little_endian_packets
        struct Test {
            _padding_[10],
    ...
    lint_failure!(test_padding_cannot_follow_scalar_field,
        r#"
        little_endian_packets
        struct Test {
            scalar: 8,
