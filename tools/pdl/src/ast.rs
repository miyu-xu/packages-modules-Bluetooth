...
pub enum DeclDesc<Af> {
    ...
    Group { id: String, fields: Vec<Field<Af>> },
    #[serde(rename = "test_declaration")]
    Test { type_id: String, test_cases: Vec<TestCase> },
}

#[derive(Debug, Serialize)]
pub struct Decl<Ad, Af> {
    pub loc: SourceRange,
    #[serde(skip_serializing)]
    pub annot: Ad,
    #[serde(flatten)]
    pub desc: DeclDesc<Af>,
}

#[derive(Debug, Serialize)]
pub struct File<Ad, Af> {
    pub version: String,
    pub file: FileId,
    pub comments: Vec<Comment>,
    pub endianness: Endianness,
    pub declarations: Vec<Decl<Ad, Af>>,
}
...
impl<Ad, Af> File<Ad, Af> {
    pub fn new(file: FileId) -> File<Ad, Af> {
        File {
...
impl<Ad, Af> Decl<Ad, Af> {
    ...
    pub fn checksum(loc: SourceRange, id: String, function: String, width: usize) -> Decl<Ad, Af>
    where
        Ad: Default,
    {
        Decl { loc, annot: Default::default(), desc: DeclDesc::Checksum { id, function, width } }
    }
    ...
    where
        Ad: Default,
    {
        Decl { loc, annot: Default::default(), desc: DeclDesc::CustomField { id, function, width } }
    }

    pub fn enum_(loc: SourceRange, id: String, width: usize, tags: Vec<Tag>) -> Decl<Ad, Af>
    where
        Ad: Default,
    {
        Decl { loc, annot: Default::default(), desc: DeclDesc::Enum { id, width, tags } }
    }
    ...
    pub fn group(loc: SourceRange, id: String, fields: Vec<Field<Af>>) -> Decl<Ad, Af>
    where
        Ad: Default,
    {
        Decl { loc, annot: Default::default(), desc: DeclDesc::Group { id, fields } }
    }
}
