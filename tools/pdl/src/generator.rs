...
///
/// Returns a `Vec` with results, which is either a piece of code or a
/// `Diagnostic` value. Inspect the severity of the returned
/// diagnostics to determine if the code can be used:
/// `Severity::Error` and `Severity::Bug` indicate that the code
/// should not be used.
///
/// The code is not formatted, pipe it through `rustfmt` to get
/// readable source code.
pub fn generate_rust(sources: &ast::SourceDatabase, grammar: &ast::Grammar) -> Vec<Result<String>> {
    let source = match sources.get(grammar.file) {
        ...
        Err(err) => {
            return vec![Err(Diagnostic::bug().with_message(format!("unknown error: {:?}", err)))]
        }
    };
    ...
    let mut results = vec![generate_preamble(Path::new(source.name()))];
    for decl in &grammar.declarations {
        match generate_decl(grammar, &packets, &children, decl) {
            Ok(decl_code) => results.push(Ok(format!("{decl_code}\n\n"))),
            Err(diagnostic) => {
                results.push(Err(diagnostic.with_labels(vec![decl.loc().secondary()])))
            }
        }
    }
