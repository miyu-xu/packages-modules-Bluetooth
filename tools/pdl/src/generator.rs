...
//fn other_err(err: &str) -> std::io::Error {
//std::io::
//}
...
fn generate_field_parser(
    ...
    match field {
        ast::Field::Scalar { id, width, .. } => {
            ...
            let getter = match grammar.endianness {
                Some(ast::Endianness { value: ast::EndiannessValue::BigEndian, .. }) => {
                    format_ident!("from_be_bytes")
                }
                Some(ast::Endianness { value: ast::EndiannessValue::LittleEndian, .. }) => {
                    format_ident!("from_le_bytes")
                }
                None => {
                    return Err(Error::Unsupported(String::from(
                        "Missing endianness value in grammar",
                    )))
                }
            };
...
fn generate_field_writer(
    ...
    match field {
        ast::Field::Scalar { id, width, .. } => {
            ...
            let writer = match grammar.endianness {
                Some(ast::Endianness { value: ast::EndiannessValue::BigEndian, .. }) => {
                    format_ident!("to_be_bytes")
                }
                Some(ast::Endianness { value: ast::EndiannessValue::LittleEndian, .. }) => {
                    format_ident!("to_le_bytes")
                }
                None => {
                    return Err(Error::Unsupported(String::from(
                        "Missing endianness value in grammar",
    ...
    match decl {
        ast::Decl::Packet { id, fields, parent_id, .. } => {
            ...
            let parent = match parent_id {
                Some(parent_id) => match packets.get(parent_id.as_str()) {
                    ...
                    _ => return Err(Error::Unsupported(format!("Could not find {parent_id}"))),
                },
                None => quote! {},
            };
            ...
            let field_names = fields
                ...
                .map(|field| match field {
                    ast::Field::Scalar { id, .. } => Ok(format_ident!("{id}")),
                    _ => Err(Error::Unsupported(format!("Field: {:?}", field))),
                })
                .collect::<Result<Vec<_>, _>>()?;
            let mut offset = 0;
...
mod tests {
    ...
    fn parse(text: &str) -> ast::Grammar {
        ...
        parse_inline(&mut db, "stdin", text).expect("parsing failure")
    }
