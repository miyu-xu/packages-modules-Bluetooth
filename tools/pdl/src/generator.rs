    ...
    match decl {
        ast::Decl::Packet { id, fields, parent_id, .. } => {
            ...
            let child_decl_name = match child_decls.get(id.as_str()) {
                Some(children) => children
                    ...
                    .map(|child_decl| match child_decl {
                        ast::Decl::Packet { id, .. } => format_ident!("{id}"),
                        _ => panic!("Expected Decl::Packet, found {:?}", child_decl),
                    })
                    .collect::<Vec<_>>(),
                None => vec![],
            };
            let has_children = !child_decl_name.is_empty();
            ...
            let field_parsers = fields
                ...
                .map(|field| {
                    let parser =
                        generate_field_parser(&grammar.endianness.value, id, field, offset);
            ...
            let field_writers = fields
                ...
                .collect::<Result<Vec<_>>>()?;
            let total_field_size =
                syn::Index::from(fields.iter().map(get_field_size).sum::<usize>());
...
mod tests {
    ...
    fn test_generate_packet_decl_empty() {
        ...
        let actual_code = generate_decl(&grammar, &packets, &child_decls, decl).unwrap();
