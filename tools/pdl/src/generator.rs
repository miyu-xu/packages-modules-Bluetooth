...
mod tests {
    ...
    fn test_generate_packet_decl_complex_little_endian() {
        ...
        let actual_code = generate_decl(&grammar, &packets, &children, decl).unwrap();
        assert_snapshot_eq(
            "test/generated/packet_decl_complex_little_endian.rs",
            &rustfmt(&actual_code),
        );
    }
    ...
    fn test_generate_packet_decl_complex_big_endian() {
        ...
        let actual_code = generate_decl(&grammar, &packets, &children, decl).unwrap();
        assert_snapshot_eq(
            "test/generated/packet_decl_complex_big_endian.rs",
            &rustfmt(&actual_code),
        );
    }
    ...
    fn test_mask_field_value() {
        ...
        assert_eq!(
            mask_field_value(&format_ident!("a"), 24).unwrap().to_string(),
            quote! { let a = a & 0xffffff; }.to_string(),
        );
    }
}
