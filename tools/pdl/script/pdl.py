...
This tool only accept valid pdl file.
...
def rule(reg):
    """
    Create a regex that match multiple spaces instead of one
    """
...
def parse_fields(data):
    ...
    for field in filter(len, re.split("\s*,\s*", data)):
        m = re.match(g(identifier, 'name'), field)
        name = m['name']
        rest = field[m.end():]

        if name == "_checksum_start_":
            m = re.match(rule(f' \( {g(identifier, "field_id")} \)'), rest)
            fields.append({
                'kind': 'checksum_field',
                'field_id': m['field_id'],
            })
            ...
            m = re.match(rule(f' \( {g(identifier, "field_id")} \) : {g(integer, "width")}'), rest)
...
def parse_declarations(data):
    ...
    while data:
        [verb, data] = re.split("\s", data.lstrip(), 1)
        ...
        if verb == "little_endian_packets":
            ...
            def parse_tag(tag):
                m = re.match(rule(f'(?P<id>\w+) = (?P<value>{integer})'), tag)
