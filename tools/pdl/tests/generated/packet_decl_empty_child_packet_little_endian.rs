...
impl FooDataChild {
    fn get_total_size(&self) -> usize {
        match self {
            FooDataChild::Empty(value) => value.get_total_size(),
...
impl FooData {
    ...
    fn parse(mut bytes: &mut Cell<&[u8]>) -> Result<Self> {
        ...
        let child = match () {
            () => {
                ...
                let child_data = EmptyData::parse(&mut cell)?;
                if !cell.get().is_empty() {
                    return Err(Error::InvalidPacketError);
                }
                FooDataChild::Empty(Arc::new(child_data))
            }
    ...
    fn write_to(&self, buffer: &mut BytesMut) {
        ...
        match &self.child {
            FooDataChild::Empty(child) => child.write_to(buffer),
