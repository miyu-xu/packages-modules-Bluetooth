...
impl FooData {
    ...
    fn parse(mut bytes: &[u8]) -> Result<Self> {
        ...
        let __case_6__ = "666";
        let a =
            (0..a_count).map(|_| Ok::<_, Error>(bytes.get_u16())).collect::<Result<Vec<_>>>()?;
        Ok(Self { a })
    }
...
impl BarData {
    ...
    fn parse(mut bytes: &[u8]) -> Result<Self> {
        ...
        if bytes.remaining() < x_size as usize {
            return Err(Error::InvalidLengthError {
                obj: "Bar".to_string(),
        ...
        let __case_1__ = "111";
        let (head, bytes) = bytes.split_at(x_size as usize);
        let mut x = Vec::new();
        while !head.is_empty() {
            x.push(Foo::parse(bytes)?);
        }
        Ok(Self { x })
    }
    fn write_to(&self, buffer: &mut BytesMut) {
