...
impl FooData {
    ...
    fn write_to(&self, buffer: &mut BytesMut) {
        let a = self.a;
        let a = a & 0x7;
        buffer[0..1].copy_from_slice(&a.to_le_bytes()[0..1]);
        let b = self.b;
        let b = (b << 3) | ((buffer[0] as u8) & 0x7);
        buffer[0..2].copy_from_slice(&b.to_le_bytes()[0..2]);
        let c = self.c;
        let c = c & 0x1f;
        let c = (c << 3) | ((buffer[1] as u8) & 0x7);
        buffer[1..2].copy_from_slice(&c.to_le_bytes()[0..1]);
        let d = self.d;
        let d = d & 0xffffff;
        buffer[2..5].copy_from_slice(&d.to_le_bytes()[0..3]);
        let e = self.e;
        let e = e & 0xfff;
        buffer[5..7].copy_from_slice(&e.to_le_bytes()[0..2]);
        let f = self.f;
        let f = f & 0xf;
        let f = (f << 4) | ((buffer[6] as u8) & 0xf);
        buffer[6..7].copy_from_slice(&f.to_le_bytes()[0..1]);
    }
