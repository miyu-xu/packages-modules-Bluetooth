...
#define SET_HANDLER(command_name, method)                                     \
  ...
  SET_HANDLER("set_features", SetFeatures)
#undef SET_HANDLER
}
...
void TestCommandHandler::SetFeatures(const std::vector<std::string>& args) {
  if (args.size() != 4) {
    response_string_ =
        "TestCommandHandler 'set_features' takes four arguments: the index "
  ...
  if (value >= 2) {
    response_string_ =
        "TestCommandHandler 'set_features' can only set bits to 0 or 1";
    send_response_(response_string_);
    return;
  }
  ...
  if (device == nullptr) {
    response_string_ =
        "TestCommandHandler 'set_features' requires a valid device";
    send_response_(response_string_);
    return;
  }

  auto hci_device = dynamic_cast<DualModeController*>(device.get());
  if (hci_device == nullptr) {
    response_string_ =
        "TestCommandHandler 'set_features' can only be applied to an HCI "
        "device";
    send_response_(response_string_);
    return;
  }
  ...
  if (!ok) {
    response_string_ =
        "TestCommandHandler 'set_features' failed to set feature " +
  ...
  response_string_ =
      "TestCommandHandler 'set_features' successfully set feature " +
