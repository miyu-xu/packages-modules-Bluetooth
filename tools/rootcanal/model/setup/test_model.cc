...
PhyDevice::Identifier TestModel::AddDevice(std::shared_ptr<Device> device) {
  ...
  if (unused_device_ids_.empty()) {
    ...
    device_id = unused_device_ids_.front();
    unused_device_ids_.pop_front();
  }
...
void TestModel::RemoveDevice(PhyDevice::Identifier device_id) {
  ...
  unused_device_ids_.insert(
      std::lower_bound(unused_device_ids_.begin(), unused_device_ids_.end(),
                       device_id),
      device_id);
}
