...
class LinkLayerController {
 public:
  ...
  // HCI LE Set Random Address command (Vol 4, Part E § 7.8.115).
...
public:
  ...
  uint8_t GetEncryptionKeySize() const {
    return min_encryption_key_size_;
  }
  ...
  void SetClassOfDevice(ClassOfDevice class_of_device) { class_of_device_ = class_of_device; }
  ...
  void SetExtendedInquiryData(std::vector<uint8_t> const &extended_inquiry_data) {
...
private:
  ...
  std::optional<Address> random_address_;
  ...
  uint8_t min_encryption_key_size_ {10};
