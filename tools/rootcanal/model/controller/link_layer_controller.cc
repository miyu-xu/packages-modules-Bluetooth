...
void LinkLayerController::ScanIncomingLeExtendedAdvertisingPdu(
  ...
  if (!scannable_advertising) {
    ...
    // The ScanA field of the scanning PDU is generated using the
    // Resolving List’s Local IRK value and the Resolvable Private Address
    // Generation procedure (see Section 1.3.2.2), or the address is
    ...
    LOG_INFO(
        "Sending LE Scan request to advertising address %s(%hhx) with "
        "scanning "
        "address %s(%hhx)",
        advertising_address.ToString().c_str(),
        advertising_address.GetAddressType(),
...
void LinkLayerController::ConnectIncomingLeExtendedAdvertisingPdu(
  ...
  if (initiator_.pending_connect_request) {
    LOG_VERB(
        "Extended advertising ignored because an LE Connect request is "
        "already "
        "pending");
    return;
  }
  ...
  // When an initiator receives a directed connectable advertising event
  // that contains a resolvable private address for the target’s address
  // (TargetA field) and address resolution is enabled, the Link Layer shall
  // resolve the private address using the resolving list’s Local IRK
  // values. An initiator that has been instructed by the Host to use
  ...
  // The Link Layer shall use resolvable private addresses for the
  // initiator’s device address (InitA field) when initiating connection
  // establishment with an associated device that exists in the Resolving
  // List.
  AddressWithType initiating_address;
  switch (initiator_.own_address_type) {
    case bluetooth::hci::OwnAddressType::PUBLIC_DEVICE_ADDRESS:
