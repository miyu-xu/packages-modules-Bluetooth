...
#if 0
  ResolveAdvertisingAddress(advertising_address, advertising_address_type);
  ResolveTargetAddress(target_address, target_address_type);

  if (target_address != Address::kEmpty &&
      target_address != address_ &&
      target_address != random_address_) {
    return;
  }
#endif
...
void LinkLayerController::IncomingLeExtendedAdvertisingPdu(
  ...
  (void)incoming;
  (void)rssi;
  LOG_INFO("Support for extended advertising PDUs is not yet implemented");
  ASSERT(false);
}
