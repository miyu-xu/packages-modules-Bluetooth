...
void LinkLayerController::IncomingPageResponsePacket(
  ...
  bluetooth::hci::Role role =
    ...
    response.GetTryRoleSwitch() ? bluetooth::hci::Role::PERIPHERAL :
...
void LinkLayerController::MakePeripheralConnection(const Address& bd_addr,
  ...
  bluetooth::hci::Role role = try_role_switch && connections_.IsRoleSwitchAllowedForPendingConnection() ?
    bluetooth::hci::Role::CENTRAL : bluetooth::hci::Role::PERIPHERAL;
...
ErrorCode LinkLayerController::SwitchRole(Address bd_addr,
  ...
  // which a connection already exists.
  ...
  if (connections_.GetScoHandle(bd_addr) != kReservedHandle) {
    LOG_INFO("role switch rejected because an Sco link is opened with"
             " the target device");
    return ErrorCode::COMMAND_DISALLOWED;
  }
  ...
  if (connection.GetMode() == AclConnectionState::kSniffMode) {
    LOG_INFO("role switch rejected because the acl connection is in sniff mode");
    return ErrorCode::COMMAND_DISALLOWED;
  }
  ...
  if (role != connection.GetRole()) {
    ...
    ScheduleTask(kNoDelayMs, [this, bd_addr, role]() {
      send_event_(bluetooth::hci::RoleChangeBuilder::Create(ErrorCode::ROLE_SWITCH_FAILED,
                                                            bd_addr, role));
    });
  }
  ...
  if (!connection.IsRoleSwitchEnabled()) {
    ...
    bluetooth::hci::Role new_role =
      connection.GetRole() == bluetooth::hci::Role::CENTRAL ?
        bluetooth::hci::Role::PERIPHERAL : bluetooth::hci::Role::CENTRAL;
