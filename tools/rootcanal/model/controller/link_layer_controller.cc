    ...
    uint8_t advertising_sid)
{
  ...
  if (synchronizing_.has_value()) {
    LOG_INFO("LE Periodic Advertising Create Sync command is currently pending");
    return ErrorCode::COMMAND_DISALLOWED;
  }
  ...
  le_periodic_advertiser_list_.emplace_back(
      PeriodicAdvertiserListEntry{advertiser_address_type, advertiser_address, advertising_sid});
  return ErrorCode::SUCCESS;
}
...
ErrorCode LinkLayerController::LePeriodicAdvertisingCreateSync(
      ...
      bluetooth::hci::PeriodicSyncCteType sync_cte_type) {
  ...
  if ((sync_cte_type & sync_cte_type_mask) == sync_cte_type_mask) {
    LOG_INFO("Sync_CTE_Type is configured to ignore all types of advertisement");
    return ErrorCode::COMMAND_DISALLOWED;
  }
  ...
  bool has_synchronized_train = std::any_of(synchronized_.begin(), synchronized_.end(),
    [](auto &state) {
  ...
  if (!options.use_periodic_advertiser_list_ && has_synchronized_train) {
    LOG_INFO(
      ...
      advertiser_address_type.ToString().c_str(),
      advertising_sid);
  ...
  if (!options.disable_reporting_ &&
      options.enable_duplicate_filtering_ &&
      !properties_.SupportsLLFeature(LLFeaturesBits::PERIODIC_ADVERTISING_ADI_SUPPORT)) {
    LOG_INFO("reporting and duplicate filtering are enabled in the options,"
  ...
  if (options.disable_reporting_ &&
      !properties_.SupportsCommand(bluetooth::hci::OpCodeIndex::LE_SET_PERIODIC_ADVERTISING_RECEIVE_ENABLE)) {
    LOG_INFO(
...
ErrorCode LinkLayerController::LePeriodicAdvertisingCreateSyncCancel() {
  ...
  if (IsLeEventUnmasked(SubeventCode::PERIODIC_ADVERTISING_SYNC_ESTABLISHED)) {
    ScheduleTask(0ms, [this] {
      send_event_(bluetooth::hci::LePeriodicAdvertisingSyncEstablished::Create(
