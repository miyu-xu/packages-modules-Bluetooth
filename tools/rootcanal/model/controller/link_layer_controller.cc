...
ErrorCode LinkLayerController::LePeriodicAdvertisingCreateSync(
  ...
  bool has_synchronized_train =
      std::any_of(synchronized_.begin(), synchronized_.end(), [](auto& state) {
        return state.advertiser_address_type =
                   advertiser_address_type && state.advertiser_address =
                       advertiser_address && state.advertising_sid =
                           advertising_sid;
      });
  if (!options.use_periodic_advertiser_list_ && has_synchronized_train) {
    LOG_INFO(
        ...
        advertiser_address_type.ToString().c_str(), advertising_sid);
  ...
  if (IsLeEventUnmasked(SubeventCode::PERIODIC_ADVERTISING_SYNC_ESTABLISHED)) {
    ScheduleTask(0ms, [this] {
      send_event_(bluetooth::hci::LePeriodicAdvertisingSyncEstablished::Create(
          ErrorCode::OPERATION_CANCELLED_BY_HOST, sync_handle, advertising_sid,
          advertiser_address_type, advertiser_address, advertiser_phy,
          periodic_advertising_interval, advertiser_clock_accuracy));
    });
  }
