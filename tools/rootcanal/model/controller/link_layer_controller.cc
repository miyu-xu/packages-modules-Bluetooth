...
static bluetooth::hci::PhyType select_phy(uint8_t phys, bluetooh::hci::PhyType current) {
...
// Helper to generate the LL_PHY_UPDATE_IND mask for the selected mask.
// The mask is non zero only if the phy has changed.
static bluetooth::hci::PhyType indicate_phy(bluetooh::hci::PhyType selected, bluetooh::hci::PhyType current) {
...
void LinkLayerController::IncomingLlPhyReq(
  ...
  if (connection.GetRole() == bluetooth::hci::Role::CENTRAL) {
    ...
    if ((phy_c_to_p != connection.GetTxPhy() ||
         phy_p_to_c != connection.GetTxPhy()) &&
...
void LinkLayerController::IncomingLlPhyRsp(
  ...
  // Always notify the host when the phy selection has changed
...
void LinkLayerController::IncomingLlPhyUpdateInd(
  ...
  bluetooth::hci::PhyType tx_phy = select_phy(
      phy_update_ind.GetPhyPToC(), connection.GetTxPhy());
  bluetooth::hci::PhyType rx_phy = select_phy(
      phy_update_ind.GetPhyCToP(), connection.GetRxPhy());

  // Update local state, and notify the host.
  // TODO: the notification is sent only when the local host is initiator
