...
ErrorCode LinkLayerController::LePeriodicAdvertisingCreateSync(
  ...
  bool has_synchronized_train =
      std::any_of(synchronized_.begin(), synchronized_.end(), [&](auto& [_, state]) {
        return state.advertiser_address_type == advertiser_address_type &&
               state.advertiser_address == advertiser_address &&
               state.advertising_sid == advertising_sid;
      });
...
void LinkLayerController::IncomingLePeriodicAdvertisingPdu(
  ...
  bluetooth::hci::AdvertiserAddressType advertiser_address_type =
    bluetooth::hci::AdvertiserAddressType(incoming.GetAdvertisingAddressType());
  Address advertiser_address = incoming.GetAdvertisingAddress();
  uint8_t advertising_sid = incoming.GetAdvertisingSid();

  // Check if the periodic advertising PDU matches the pending
  // Create Sync command.
  if (synchronizing_.has_value() &&
      synchronizing_->advertiser_address_type == advertiser_address_type &&
      synchronizing_->advertiser_address == advertiser_address &&
    ...
    if (IsLeEventUnmasked(SubeventCode::PERIODIC_ADVERTISING_SYNC_ESTABLISHED)) {
      send_event_(
          bluetooth::hci::LePeriodicAdvertisingSyncEstablishedBuilder::Create(
              ...
              advertiser_address_type, advertiser_address,
              bluetooth::hci::SecondaryPhyType::LE_1M,
              incoming.GetAdvertisingInterval(),
              bluetooth::hci::ClockAccuracy::PPM_500));
    }

    // Update the synchronization state.
    synchronized_.insert(sync_handle, Synchronized {
      .advertiser_address_type = advertiser_address_type,
      .advertiser_address = advertiser_address,
      .advertising_sid = advertising_sid,
  ...
  for (auto& [_, sync] : synchronized_) {
    ...
    if (IsLeEventUnmasked(SubeventCode::PERIODIC_ADVERTISING_REPORT)) {
      ...
      do {
        ...
        bluetooth::hci::DataStatus = remaining_size <= max_fragment_size
                                  ? bluetooth::hci::DataStatus::COMPLETE
                                  : bluetooth::hci::DataStatus::CONTINUING;
        std::vector<uint8_t> fragment_data(
            ...
            advertising_data.begin() + offset + fragment_size);
        offset += fragment_size;
        send_event_(bluetooth::hci::LePeriodicAdvertisingReport::Create(
            sync.sync_handle, incoming.GetTxPower(), rssi,
            bluetooth::hci::CteType::NO_CONSTANT_TONE_EXTENSION,
            data_status, fragment_data));
      } while (offset < advertising_data.size());
    }

    // Refresh the timeout for the sync disconnection.
    sync.timeout = std::chrono::steady_clock::now() +
      6 * slots(sync.advertising_interval);
  }
}
...
void LinkLayerController::LeSynchronization() {
  ...
  for (auto& [_, sync] : synchronized_) {
    ...
    if (IsLeEventUnmasked(SubeventCode::PERIODIC_ADVERTISING_SYNC_LOST)) {
      send_event_(bluetooth::hci::PeriodicAdvertisingSyncLostBuilder::Create(
        sync_handle));
    }
  }

  for (auto sync_handle : removed_sync_handles) {
    synchronized_.remove(sync_handle);
  }
}
