...
bool LinkLayerController::FilterAcceptListBusy() {
  ...
  for (auto const& entry : extended_advertisers_) {
    ExtendedAdvertiser const& advertiser = entry.second;
...
bool LinkLayerController::HandleExtendedConnectInd(
  ...
  if (advertiser.IsDirected()) {
    if (advertiser.GetTargetAddress() != initiating_address) {
      return false;
    }
  } else {
    // Check if initiator address is in the filter accept list
...
void LinkLayerController::IncomingLeConnectPacket(
  ...
  for (auto& entry : extended_advertisers_) {
    if (HandleExtendedConnectInd(entry.second, connect)) {
      return;
    }
  }
}
...
void LinkLayerController::GenerateExtendedScanResponse(
...
void LinkLayerController::IncomingLeScanPacket(
  ...
  for (auto& entry : extended_advertisers_) {
    GenerateExtendedScanResponse(entry.second, resolved_scanning_address,
                                 advertising_address);
  }
}
