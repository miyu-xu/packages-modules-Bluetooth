...
void LinkLayerController::IncomingPageResponsePacket(
  ...
  if (connections_.IsRoleSwitchAllowed(handle) && response.GetTryRoleSwitch()) {
...
void LinkLayerController::MakePeripheralConnection(const Address& addr,
   ...
   * change event */
  // TODO(b/274476773): Add a config option for this quirk
  auto role = try_role_switch && connections_.IsRoleSwitchAllowed(handle)
...
ErrorCode LinkLayerController::SwitchRole(Address addr,
  ...
  if (!connections_.IsRoleSwitchAllowed(handle)) {
    return ErrorCode::ROLE_CHANGE_NOT_ALLOWED;
  }
...
void LinkLayerController::IncomingRoleSwitchRequest(
  ...
  ASSERT(request.IsValid());
  // Already checked by initiator controller
  ASSERT(connections_.IsRoleSwitchAllowed(handle));
