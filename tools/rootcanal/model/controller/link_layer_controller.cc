...
void LinkLayerController::ScanIncomingLeExtendedAdvertisingPdu(
  ...
  if (should_send_advertising_report &&
    ...
    size_t nr_fragments =
        (advertising_data.size() + max_fragment_size - 1) / max_fragment_size;
    if (nr_fragments == 0) {
      // Still send an advertising report if the advertising data is empty.
      nr_fragments = 1;
    }

    for (size_t nr = 0, offset = 0; nr < nr_fragments;
         offset += max_fragment_size, nr++) {
