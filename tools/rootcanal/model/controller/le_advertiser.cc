...
ErrorCode LinkLayerController::LeSetAdvertisingEnable(bool advertising_enable) {
  ...
  switch (advertiser.peer_address_type) {
  ...
  AddressWithType peer_address{advertiser.peer_address, peer_address_type};
  ...
  std::optional<AddressWithType> resolvable_address =
      GenerateResolvablePrivateAddress(peer_address, true);
  ...
  if (advertiser.own_address_type == OwnAddressType::RANDOM_DEVICE_ADDRESS &&
      random_address.GetAddress() == Address::kEmpty) {
    LOG_INFO(
  ...
  // Invalid HCI Command Parameters (0x12).
  if (advertiser.own_address_type ==
          OwnAddressType::RESOLVABLE_OR_RANDOM_ADDRESS &&
      !resolvable_address && random_address.GetAddress() == Address::kEmpty) {
    LOG_INFO(
  ...
  switch (advertiser.own_address_type) {
    ...
    case OwnAddressType::RESOLVABLE_OR_PUBLIC_ADDRESS:
      advertiser.advertising_address =
          resolvable_address ? resolvable_address.value() : public_address;
      break;
    case OwnAddressType::RESOLVABLE_OR_RANDOM_ADDRESS:
      advertiser.advertising_address =
          resolvable_address ? resolvable_address.value() : random_address;
      break;
  }
  ...
  switch (advertiser.advertising_type) {
    ...
    case AdvertisingType::ADV_DIRECT_IND_LOW: {
      ...
      std::optional<AddressWithType> peer_resolvable_address =
          GenerateResolvablePrivateAddress(peer_address, false);
      advertiser.target_address = peer_resolvable_address
                                      ? peer_resolvable_address.value()
                                      : peer_address;
      break;
    }
    default:
...
void LinkLayerController::LeAdvertising() {
  ...
  if (legacy_advertiser_.IsEnabled() &&
      (now - legacy_advertiser_.last_event) >= legacy_advertiser_.interval) {
    legacy_advertiser_.last_event = now;
