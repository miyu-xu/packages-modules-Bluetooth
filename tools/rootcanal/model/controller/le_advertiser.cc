...
ErrorCode LinkLayerController::LeSetPeriodicAdvertisingParameters(
  ...
  if (advertiser.advertising_event_properties.connectable_ ||
    ...
    LOG_INFO("the periodic advertising set %02x specifies scannable,"
  ...
  if (advertiser.periodic_advertising_data.size() >
      GetMaxPeriodicAdvertisingDataLength(slots(periodic_advertising_interval_max))) {
    LOG_INFO("the length of the periodic advertising data exceeds the maximum"
...
ErrorCode LinkLayerController::LeSetPeriodicAdvertisingData(
  ...
  if (advertiser.periodic_advertising_enable &&
      operation != Operation::COMPLETE_ADVERTISEMENT &&
      operation != Operation::UNCHANGED_DATA) {
    LOG_INFO("periodic advertising is enabled and the operation is not"
  ...
  if (advertising_data.empty() &&
      operation != Operation::COMPLETE_ADVERTISEMENT &&
      operation != Operation::UNCHANGED_DATA) {
    LOG_INFO("periodic advertising data is empty is enabled and the operation"
  ...
  if (advertiser.periodic_advertising_data.size() > properties_.le_max_advertising_data_length) {
    LOG_INFO("the length of the combined periodic advertising data exceeds"
...
ErrorCode LinkLayerController::LeSetPeriodicAdvertisingEnable(
  ...
  if (!enable) {
    advertiser.periodic_advertising_enable = false;
    advertiser.periodic_advertising_enable_latch = false;
    return ErrorCode::SUCCESS;
  }
  ...
  if (GetMaxPeriodicAdvertisingDataLength(advertiser.periodic_advertising_interval)) {
    LOG_INFO("the length of the combined periodic advertising data exceeds"
             " the maximum that the controller can transmit within the current"
             " periodic advertising interval");
    return ErrorCode::PACKET_TOO_LONG;
  ...
  if (include_adi && !properties_.SupportsLLFeature(LLFeaturesBits::PERIODIC_ADVERTISING_ADI_SUPPORT)) {
    LOG_INFO("include ADI is true but the controller does not support the"
             " Periodic Advertising ADI Supported feature",
             static_cast<int>(advertising_handle));
    return ErrorCode::UNSUPPORTED_FEATURE_OR_PARAMETER_VALUE;
  }

  advertiser.periodic_advertising_enable = true;
  return ErrorCode::SUCCESS;
}

static uint16_t ExtendedAdvertiser::GetMaxPeriodicAdvertisingDataLength(
