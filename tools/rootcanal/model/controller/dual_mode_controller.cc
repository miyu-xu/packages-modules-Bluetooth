...
#else
  auto command_view =
      bluetooth::hci::IoCapabilityRequestReplyView::Create(command);
  ASSERT(command_view.IsValid());
...
#else
  auto command_view =
      bluetooth::hci::UserConfirmationRequestReplyView::Create(command);
  ASSERT(command_view.IsValid());
...
#else
  auto command_view =
      bluetooth::hci::UserConfirmationRequestNegativeReplyView::Create(command);
  ASSERT(command_view.IsValid());
...
void DualModeController::PinCodeRequestReply(CommandView command) {
  ...
  link_layer_controller_.ForwardToLm(command);
#else
  auto command_view = bluetooth::hci::PinCodeRequestReplyView::Create(command);
  ASSERT(command_view.IsValid());
...
#else
  auto command_view =
      bluetooth::hci::PinCodeRequestNegativeReplyView::Create(command);
  ASSERT(command_view.IsValid());
  LOG_INFO("%s", GetAddress().ToString().c_str());
...
#else
  auto command_view =
      bluetooth::hci::UserPasskeyRequestReplyView::Create(command);
  ASSERT(command_view.IsValid());
...
#else
  auto command_view =
      bluetooth::hci::UserPasskeyRequestNegativeReplyView::Create(command);
  ASSERT(command_view.IsValid());
...
#else
  auto command_view =
      bluetooth::hci::RemoteOobDataRequestReplyView::Create(command);
  ASSERT(command_view.IsValid());
...
#else
  auto command_view =
      bluetooth::hci::RemoteOobDataRequestNegativeReplyView::Create(command);
  ASSERT(command_view.IsValid());
...
#else
  auto command_view =
      bluetooth::hci::IoCapabilityRequestNegativeReplyView::Create(command);
  ASSERT(command_view.IsValid());
...
#else
  auto command_view =
      bluetooth::hci::RemoteOobExtendedDataRequestReplyView::Create(command);
  ASSERT(command_view.IsValid());
...
#else
  auto command_view =
      bluetooth::hci::SendKeypressNotificationView::Create(command);
  ASSERT(command_view.IsValid());
...
#else
  auto command_view =
      bluetooth::hci::AuthenticationRequestedView::Create(command);
  ASSERT(command_view.IsValid());
  uint16_t handle = command_view.GetConnectionHandle();
...
#else
  auto command_view =
      bluetooth::hci::SetConnectionEncryptionView::Create(command);
  ASSERT(command_view.IsValid());
  uint16_t handle = command_view.GetConnectionHandle();
...
void DualModeController::LinkKeyRequestReply(CommandView command) {
  ...
  link_layer_controller_.ForwardToLm(command);
#else
  auto command_view = bluetooth::hci::LinkKeyRequestReplyView::Create(command);
  ASSERT(command_view.IsValid());
  Address addr = command_view.GetBdAddr();
...
#else
  auto command_view =
      bluetooth::hci::LinkKeyRequestNegativeReplyView::Create(command);
  ASSERT(command_view.IsValid());
  Address addr = command_view.GetBdAddr();
