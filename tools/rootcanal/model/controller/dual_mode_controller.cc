...
#define CHECK_PACKET_VIEW(view)                     \
  do {                                              \
    if (!CheckPacketView(view, __func__)) {         \
      ASSERT_LOG(false, "received invalid packet"); \
      return;                                       \
    }                                               \
  } while (0)
...
DualModeController::DualModeController(ControllerProperties properties)
  ...
  // Ignore invalid packets by default.
  invalid_packet_handler_ = [](uint32_t, InvalidPacketReason, std::string,
