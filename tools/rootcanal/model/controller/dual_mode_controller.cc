...
void DualModeController::SendCommandStatusUnknownOpCodeEvent(
  ...
  send_event_(gd_hci::CommandStatusBuilder::Create(
      ErrorCode::UNKNOWN_HCI_COMMAND, kNumCommandPackets,
      bluetooth::hci::OpCode(op_code),
...
void DualModeController::HandleCommand(
  ...
  // Respond with the status code Unknown Command.
    ...
    SendCommandStatusUnknownOpCodeEvent(opcode);
...
void DualModeController::LeVendorCap(CommandView command) {
  ...
  if (caps.empty()) {
    SendCommandStatusUnknownOpCodeEvent(
