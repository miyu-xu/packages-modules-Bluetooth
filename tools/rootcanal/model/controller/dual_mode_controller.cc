...
void DualModeController::ReadBufferSize(CommandView command) {
  ...
  send_event_(bluetooth::hci::ReadBufferSizeCompleteBuilder::Create(
      kNumCommandPackets, ErrorCode::SUCCESS,
      properties_.acl_data_packet_length,
      properties_.sco_data_packet_length,
...
void DualModeController::EnhancedFlush(CommandView command) {
  ...
  if (link_layer_controller_.IsEventUnmasked(gd_hci::EventCode::ENHANCED_FLUSH_COMPLETE)) {
...
void DualModeController::ReadLocalName(CommandView command) {
  ...
  size_t len = std::min(link_layer_controller_.GetName().size(), (size_t)248);
  std::copy_n(link_layer_controller_.GetName().begin(), len, local_name.begin());
