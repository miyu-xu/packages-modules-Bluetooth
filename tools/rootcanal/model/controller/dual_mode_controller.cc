...
void DualModeController::GetControllerDebugInfo(CommandView command) {
  auto command_view = bluetooth::hci::LeGetControllerDebugInfoInfoView::Create(command);
  ASSERT(command_view.IsValid());

  DEBUG(id_, "<< Get Controller Debug Info");

  send_event_(bluetooth::hci::GetControllerDebugInfoCompleteBuilder::Create(kNumCommandPackets, ErrorCode::SUCCESS));
}
