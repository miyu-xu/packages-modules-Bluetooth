...
void DualModeController::IntelDdcConfigRead(CommandView command) {
  ...
  switch (id) {
    case 0x0204:
      send_event_(bluetooth::hci::CommandCompleteBuilder::Create(kNumCommandPackets, OpCode::INTEL_DDC_CONFIG_READ,
                                                  std::vector<uint8_t> { static_cast<uint8_t>(ErrorCode::SUCCESS), 0x04, 0x02, 0x01, 0x10 }));
      break;
    default:
      send_event_(bluetooth::hci::CommandCompleteBuilder::Create(kNumCommandPackets, OpCode::INTEL_DDC_CONFIG_READ,
                                                  std::vector<uint8_t> { static_cast<uint8_t>(ErrorCode::INVALID_HCI_COMMAND_PARAMETERS), 0x00, 0x00 }));
      break;
  }
}
