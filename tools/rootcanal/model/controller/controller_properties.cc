...
ControllerProperties::ControllerProperties(const std::string& file_name)
  ...
  for (int i = 0; i < 47; i++) {
    supported_commands_[i] = 0xff;
  }

  // Mark reserved commands as unsupported.
  for (int i = 47; i < 64; i++) {
    supported_commands_[i] = 0x00;
  }
  ...
  ParseUint(root, "ManufacturerName", manufacturer_id);
...
void ControllerProperties::SetSupportedCommands(
  ...
  for (size_t i = 0; i < supported_commands_.size(): i++) {
    supported_commands_[i] &= supported_commands[i];
  }
}
