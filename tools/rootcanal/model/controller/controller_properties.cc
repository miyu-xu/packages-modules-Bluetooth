...
static std::array<uint8_t, 64> SupportedCommands() {
  OpCodeIndex supported_commands[] = {
      ...
      OpCodeIndex::REMOTE_NAME_REQUEST, OpCodeIndex::REMOTE_NAME_REQUEST_CANCEL,
      ...
      OpCodeIndex::WRITE_LOCAL_NAME, OpCodeIndex::READ_LOCAL_NAME,
...
ControllerProperties::ControllerProperties(const std::string& file_name)
  ...
  if (!CheckSupportedCommands()) {
    LOG_INFO(
        ...
        "sure"
        " that the supported commands are correct w.r.t. the rules described"
        " in Vol 4, Part E § 3 Overview of commands and events");
  }
  ...
  ParseBool(root, "le_transport", le_transport);
  ParseBool(root, "br_transport", br_transport);
  ...
  if (le_transport && br_transport) {
    lmp_features[0] |= LMPFeaturesPage0Bits::SIMULTANEOUS_LE_AND_BR_CONTROLLER;
    lmp_features[0] |= LMPFeaturesPage0Bits::LE_SUPPORTED_CONTROLLER;
  } else if (le_transport) {
    lmp_features[0] &= ~LMPFeaturesPage0Bits::SIMULTANEOUS_LE_AND_BR_CONTROLLER;
    lmp_features[0] |= LMPFeaturesPage0Bits::LE_SUPPORTED_CONTROLLER;
  } else if (br_transport) {
    lmp_features[0] &= ~LMPFeaturesPage0Bits::SIMULTANEOUS_LE_AND_BR_CONTROLLER;
    lmp_features[0] &= ~LMPFeaturesPage0Bits::LE_SUPPORTED_CONTROLLER;
  } else {
    LOG_ERROR("le_transport or br_transport must be set in the configuration");
    br_transport = true;
    le_transport = true;
    lmp_features[0] |= LMPFeaturesPage0Bits::SIMULTANEOUS_LE_AND_BR_CONTROLLER;
    lmp_features[0] |= LMPFeaturesPage0Bits::LE_SUPPORTED_CONTROLLER;
...
bool ControllerProperties::CheckSupportedCommands() const {
  ...
  // supported, otherwise excluded. C24: Mandatory if the LE Controller supports
  ...
  // Connection State, otherwise excluded. C25: Mandatory if LE Feature
  ...
  // otherwise excluded. C39: Mandatory if LE Feature (Connected Isochronous
  ...
  // otherwise optional. C55: Mandatory if LE Feature (Connected Isochronous
  ...
  // excluded. C57: Mandatory if LE Feature (Connection Subrating) is supported,
  // otherwise excluded.
  ...
  // C64: Optional if the Controller supports transmitting packets,
  // otherwise excluded.
  auto c64 = optional;
  ...
  // excluded. C96: Optional if the LE Controller supports Connection State,
  // otherwise excluded.
  auto c96 = mandatory;
  ...
  // otherwise excluded. C102: Mandatory if the Change Connection Link Key
  // command is supported, otherwise excluded. C103: Mandatory if the Periodic
  ...
  // excluded. C105: Mandatory if the Read Remote Version Information command is
  // supported, otherwise excluded. C106: Mandatory if the Remote Name Request
  // command is supported, otherwise excluded.
  auto c106 =
  ...
  // otherwise excluded. C111: Mandatory if the Write Authentication Enable
  // command is supported, otherwise excluded.
  ...
  // excluded. C133: Mandatory if HV2, HV3, or multi-slot or EDR ACL packets are
  // supported, otherwise excluded.
  ...
  // excluded. C145: Mandatory if any event in event mask page 2 is supported,
  // otherwise optional.
  auto c145 = mandatory;
  ...
  // otherwise excluded. C148: Optional if any of the Connection Complete,
