...
bool crash_callback(const void* crash_context, size_t crash_context_size,
  ...
  if (crash_context_size >=
    ...
    log::error("Process crashed, signal: {}[{}], tid: {}",
  ...
  for (const auto& frame : data.frames) {
    log::error("{}", unwinder.FormatFrame(frame).c_str());
  }
  return true;
}
...
int main(int argc, char** argv) {
  ...
  log::info("starting rootcanal");

  if (FLAGS_test_port > UINT16_MAX) {
    log::error("test_port out of range: {}", FLAGS_test_port);
    return -1;
  }

  if (FLAGS_hci_port > UINT16_MAX) {
    log::error("hci_port out of range: {}", FLAGS_hci_port);
    return -1;
  }

  if (FLAGS_link_port > UINT16_MAX) {
    log::error("link_port out of range: {}", FLAGS_link_port);
    return -1;
  }

  if (FLAGS_link_ble_port > UINT16_MAX) {
    log::error("link_ble_port out of range: {}", FLAGS_link_ble_port);
    return -1;
  }
