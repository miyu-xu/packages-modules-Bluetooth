...
DEFINE_string(configuration, "",
              "controller configuration (see config.proto)");
...
int main(int argc, char** argv) {
  ...
  if (FLAGS_configuration.size() > 0) {
    configuration_str = FLAGS_configuration;
  } else if (FLAGS_configuration_file.size() > 0) {
  ...
  TestEnvironment root_canal(
      ...
      static_cast<int>(FLAGS_test_port),
      static_cast<int>(FLAGS_hci_port),
      static_cast<int>(FLAGS_link_port),
      static_cast<int>(FLAGS_link_ble_port),
      configuration_str,
