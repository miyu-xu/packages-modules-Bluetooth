...
fn send_complete(ctx: &impl Context, status: hci::ErrorCode) -> Result<(), ()> {
    ctx.send_hci_event(
...
fn send_confirm(ctx: &impl Context) {
...
pub async fn initiate(ctx: &impl Context) -> Result<(), ()> {
        ...
        match ctx
                ...
                .await
            {
                ...
                Either::Right(_) => {
                    ...
                    return send_complete(ctx, hci::ErrorCode::AuthenticationFailure);
                }
            }
    };
    let responder = {
    ...
    match auth_method {
        ...
        | AuthenticationMethod::NumericComparisonUserConfirm => {
            ...
            if user_confirmation_request(ctx).await.is_err() {
                ...
                send_complete(ctx, hci::ErrorCode::AuthenticationFailure)?;
            }
        }
        AuthenticationMethod::PasskeyEntry => {
    ...
    let _ = send_complete(ctx, hci::ErrorCode::Success);
