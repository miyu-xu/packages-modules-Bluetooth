...
pub async fn initiate(ctx: &impl Context) {
    ...
    {
        Either::Left(_reply) => {
            // Send the initial challenge to determine if the remote device
            // has saved the link key, or deleted the bond. The authentication
            // request fails with error PIN_OR_KEY_MISSING if the remote
            // rejects the challenge.
            match send_challenge(ctx, 0, _reply.get_link_key().clone()).await {
