...
impl CisParameters {
    fn new(cig_config: &CigConfig, cis_config: &CisConfig) -> CisParameters {
        ...
        let nse = cis_config.bn_p_to_c.unwrap_or(std::cmp::max(bn_c_to_p, bn_p_to_c));
...
impl IsoManager {
    ...
    fn new_cis_connection_handle(&self) -> u16 {
        (0x100..0x200).find(|handle| !self.cis_connections.contains_key(handle)).unwrap()
    }
    ...
    pub fn hci_le_set_cig_parameters(&mut self, packet: hci::LeSetCigParametersPacket) {
        ...
        if !(0xff..=0xf_ffff).contains(&sdu_interval_c_to_p)
            || !(0xff..=0xf_ffff).contains(&sdu_interval_p_to_c)
        {
            println!(
    ...
    pub fn ll_cis_rsp(&mut self, acl_connection_handle: u16, _packet: llcp::CisRsp) {
        ...
        if let Some(cis_connection_handle) = cis_connection_handle {
            ...
            self.send_hci_event(hci::LeCisEstablishedBuilder {
                ...
                phy_c_to_p: hci::SecondaryPhyType::from_u8(parameters.phy_c_to_p as u8).unwrap(),
                phy_p_to_c: hci::SecondaryPhyType::from_u8(parameters.phy_p_to_c as u8).unwrap(),
                nse: parameters.nse,
...
fn iso_interval(
    ...
    if framed {
        ...
        let iso_interval =
            num_integer::lcm(num_integer::lcm(sdu_interval_c_to_p, sdu_interval_p_to_c), 1250);
