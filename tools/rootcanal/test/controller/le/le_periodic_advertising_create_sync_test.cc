...
TEST_F(LePeriodicAdvertisingCreateSyncTest, CreateUsingPublicAddress) {
  ASSERT_EQ(controller_.LePeriodicAdvertisingCreateSync(
                PeriodicAdvertisingOptions(false, false, false),
                0, AdvertiserAddressType::PUBLIC_DEVICE_OR_IDENTITY_ADDRESS,
                Address{1}, 0, 0x100, 0),
            ErrorCode::SUCCESS);
}
...
TEST_F(LePeriodicAdvertisingCreateSyncTest, CreateUsingPeriodicAdvertiserList) {
  ASSERT_EQ(controller_.LePeriodicAdvertisingCreateSync(
                PeriodicAdvertisingOptions(true, false, false),
                0, AdvertiserAddressType::PUBLIC_DEVICE_OR_IDENTITY_ADDRESS,
...
TEST_F(LePeriodicAdvertisingCreateSyncTest, InvalidSyncCteMask) {
  ASSERT_EQ(controller_.LePeriodicAdvertisingCreateSync(
                ...
                static_cast<uint8_t>(PeriodicSyncCteType::AVOID_TYPE_THREE_CONSTANT_TONE_EXTENSION) |
                static_cast<uint8_t>(PeriodicSyncCteType::AVOID_NO_CONSTANT_TONE_EXTENSION), 0x100, 0),
            ErrorCode::COMMAND_DISALLOWED);
}
