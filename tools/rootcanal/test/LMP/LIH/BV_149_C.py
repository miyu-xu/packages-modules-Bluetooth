...
class Test(ControllerTest):
    ...
    async def test(self):
        ...
        await self.expect_ll(ll.Page(source_address=controller.address,
                                     destination_address=peer_address,
                                     allow_role_switch=False))

        controller.send_ll(ll.PageResponse(source_address=peer_address,
        ...
        controller.send_cmd(
            hci.SwitchRole(bd_addr=peer_address, role=hci.Role.CENTRAL))

        await self.expect_evt(hci.SwitchRoleStatus(status=ErrorCode.SUCCESS, num_hci_command_packets=1))

        await self.expect_evt(hci.RoleChange(status=ErrorCode.ROLE_SWITCH_FAILED, bd_addr=peer_address, role=hci.Role.CENTRAL))
        ...
        await self.expect_evt(hci.RoleChange(status=ErrorCode.ROLE_CHANGE_NOT_ALLLOWED, bd_addr=peer_address, role=hci.Role.CENTRAL))
