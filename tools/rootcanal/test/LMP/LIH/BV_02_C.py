...
class Test(ControllerTest):
    ...
    async def test(self):
        ...
        controller.send_ll(ll.RoleSwitchRequest())

        await self.expect_ll(ll.RoleSwitchResponse(status=ErrorCode.SUCCESS))

        await self.expect_evt(hci.RoleChange(status=ErrorCode.SUCCESS, bd_addr=peer_address, role=hci.Role.PERIPHERAL))

        controller.send_ll(ll.RoleSwitchRequest())

        await self.expect_ll(ll.RoleSwitchResponse(status=ErrorCode.SUCCESS))

        await self.expect_evt(hci.RoleChange(status=ErrorCode.SUCCESS, bd_addr=peer_address, role=hci.Role.CENTRAL))
