...
class Test(ControllerTest):
    ...
    async def test_inquiry_result_all_devices(self):
        ...
        controller.send_cmd(hci.Inquiry(lap=hci.Lap(lap=0), inquiry_length=1, num_responses=0))

        await self.expect_evt(hci.InquiryStatus(status=ErrorCode.SUCCESS, num_hci_command_packets=1))
