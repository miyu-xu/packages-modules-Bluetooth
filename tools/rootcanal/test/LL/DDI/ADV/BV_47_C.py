...
class Test(ControllerTest):
    ...
    async def steps_2_13(self, maximum_advertising_data_length: int, advertising_event_properties: int,
                         data_length: int, fragment_preference: FragmentPreference,
                         duration: int, max_extended_advertising_events: int):
        controller = self.controller
        ...
        controller.send_cmd(
            hci.LeSetExtendedAdvertisingParameters(
        ...
        for n in range(num_fragments):
            ...
            controller.send_cmd(
                hci.LeSetExtendedAdvertisingDataRaw(
                    advertising_handle=0,
                    operation=operation,
                    advertising_data=advertising_data[fragment_offset:fragment_offset + fragment_length]))
        ...
        controller.send_cmd(hci.LeSetExtendedAdvertisingEnable(
        ...
        await self.expect_evt(hci.LeSetExtendedAdvertisingEnableComplete(status=ErrorCode.SUCCESS, num_hci_command_packets=1))
        ...
        for n in range(repeat):
            await self.expect_ll(ll.LeExtendedAdvertisingPdu(
        ...
        if max_extended_advertising_events > 0:
            ...
            await self.expect_evt(hci.LeAdvertisingSetTerminated(
