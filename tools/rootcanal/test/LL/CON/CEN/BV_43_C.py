...
class Test(ControllerTest):
    ...
    async def test(self):
        ...
        controller.send_ll(ll.LlPhyRsp(
            source_address=peer_address,
            destination_address=controller.address,
            tx_phys=0x1,
            rx_phys=0x1))

        await.self.expect_ll(ll.LlPhyUpdateInd(
            source_address=controller.address,
            destination_address=peer_address,
            tx_phys=0x0,
            rx_phys=0x0))
        ...
        for test_round in test_rounds:
            await self.steps_3_9(peer_address, acl_connection_handle, **vars(test_round))

    async def steps_1_9(self, peer_address: Address, connection_handle: int, req_tx_phys: int, req_rx_phys: int):
        controller = self.controller
        ...
        # table.
        controller.send_ll(ll.LlPhyReq(
            source_address=peer_address,
            destination_address=controller.address,
            req_tx_phys,
            req_rx_phys))
        ...
        phy_update_ind = await self.expect_ll(ll.LlPhyUpdateInd(
            source_address=controller.address,
            destination_address=peer_address,
            phy_c_to_p=self.Any,
            phy_p_to_c=self.Any))
        ...
        if phy_update_ind.phy_c_to_p != 0 || phy_update_ind.phy_p_to_c != 0:
            await self.expect_evt(
