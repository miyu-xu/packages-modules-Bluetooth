...
class Test(ControllerTest):
    ...
    async def test(self):
        ...
        phy_c_to_p = 0x1
        phy_p_to_c = 0x1
        for tes_round in test_rounds:
            (phy_c_to_p, phy_p_to_c) = await self.steps_1_9(acl_connection_handle, phy_c_to_p, phy_p_to_c, **vars(test_round))

    async def steps_1_9(self, connection_handle: int, phy_c_to_p: int, phy_p_to_c: int,
        ...
        # Table 4.77.
        await self.expect_ll(ll.LlPhyReq(tx_phys=req_tx_phys, rx_phys=req_rx_phys))

        controller.send_ll(ll.LlPhyRsp(tx_phys=rsp_tx_phys, rx_phys=rsp_rx_phys))
        ...
        phy_update_ind = await self.expect_ll(ll.LlPhyUpdateInd(phy_c_to_p=self.Any, phy_p_to_c=self.Any))
        ...
        await self.expect_evt(ll.LePhyUpdateComplete(connection_handle=connection_handle,
