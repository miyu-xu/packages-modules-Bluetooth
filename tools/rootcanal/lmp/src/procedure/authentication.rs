...
fn generate_sres(random_number: &[u8; 16], key: &[u8; 16]) -> [u8; 4] {
    ...
    hasher.update(key);
    hasher.finish()[0..4].try_into().unwrap()
}
    ...
    .await
    {
        Either::Left(au_rand) => {
            match request_link_key_from_host(ctx).await {
                ...
                None => {
                    ctx.send_lmp_packet(lmp::NotAcceptedBuilder { transaction_id: 0, not_accepted_opcode: lmp::Opcode::AuRand, error_code: 0x06 }.build());
                }
            }
        },
