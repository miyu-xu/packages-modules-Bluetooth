...
pub async fn initiate(ctx: &impl Context) -> Result<(), ()> {
    ...
    if auth_result.is_ok() {
        let key_type = link_key_type(ctx, initiator, responder).await;
        ctx.send_hci_event(
            hci::LinkKeyNotificationBuilder { bd_addr: ctx.peer_address(), key_type, link_key }
                .build(),
        );
    }
