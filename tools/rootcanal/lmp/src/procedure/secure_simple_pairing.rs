...
use crate::procedure::ec::{DhKey, PrivateKey, PublicKey};
    ...
    let dh_key = {
        ...
        let private_key =
            if features::supported_on_both_page1(ctx, SecureConnectionsHostSupport).await {
                PrivateKey::generate_p256(ctx)
            } else {
                PrivateKey::generate_p192(ctx)
            };
...
pub async fn respond(ctx: &impl Context, request: lmp::IoCapabilityReqPacket) -> Result<(), ()> {
    ...
    let dh_key = {
        ...
        let private_key = match peer_public_key {
            PublicKey::P192(_) => PrivateKey::generate_p192(ctx),
            PublicKey::P256(_) => PrivateKey::generate_p256(ctx),
        };
        let local_public_key = private_key.derive();
        send_public_key(ctx, 0, local_public_key).await;
        private_key.shared_secret(peer_public_key)
...
#[cfg(test)]
mod tests {
    use crate::procedure::ec::PrivateKey;
    ...
    fn sample_p192_public_key(context: &crate::test::TestContext) -> [u8; 16] {
        let part = *context.round.borrow();
        PrivateKey::generate_p192(context).derive().as_slice()[part * 16..(part + 1) * 16]
            .try_into()
            .unwrap()
    }
