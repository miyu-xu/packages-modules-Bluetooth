...
fn p192_group() -> EcGroup {
    ...
    let p = BigNum::from_slice(&k_p).unwrap();
    let mut a = BigNum::from_slice(&k_p).unwrap();
    a.sub_word(3).unwrap();
    let b = BigNum::from_slice(&k_b).unwrap();
    let mut x = BigNum::from_slice(&k_x).unwrap();
    let mut y = BigNum::from_slice(&k_y).unwrap();
    let order = BigNum::from_slice(&k_n).unwrap();
    let p192_group = EcGroup::from_components(p, a, b, &mut ctx).unwrap();
    let generator = EcPoint::new(p192_group.as_ref()).unwrap();
    generator
        .as_ref()
        .affine_coordinates_gfp(p192_group.as_ref(), &mut x, &mut y, &mut ctx)
        .unwrap();
    p192_group.as_ref().set_generator(generator, order, BigNum::from_u32(1).unwrap()).unwrap();
