...
impl PrivateKey {
    pub fn generate_p192(ctx: &impl Context) -> Self {
        let mut key = BigInt::from_signed_bytes_le(
            &ctx.generate_random_bytes::<{ P192r1::PRIVATE_KEY_SIZE }>(),
        );
        ...
        buf.resize(P192r1::PRIVATE_KEY_SIZE, 0);
    ...
    pub fn generate_p256(ctx: &impl Context) -> Self {
        let mut key = BigInt::from_signed_bytes_le(
            &ctx.generate_random_bytes::<{ P256r1::PRIVATE_KEY_SIZE }>(),
        );
        // Avoid zero and return 1 for tests
        if key.is_negative() {
            key = -key;
        ...
        let mut buf = key.to_signed_bytes_le();
        buf.resize(P256r1::PRIVATE_KEY_SIZE, 0);
