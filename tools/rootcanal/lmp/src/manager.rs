...
impl LinkManagerError {
    fn is_fatal(&self) -> bool {
        match self {
            // Incoming LMP packets corresponding to an unknown peer should be dropped
            // e.g. if we have disconnected halfway through a procedure
            LinkManagerError::UnknownPeer => false,
            // Need to fatal here to avoid credit imbalance
            LinkManagerError::UnhandledHciPacket => true,
            // The C++ side has already added the link, so if we cannot
            // do the same we should fatal
...
impl LinkManager {
    ...
    pub fn add_link(self: &Rc<Self>, peer: hci::Address) -> Result<(), LinkManagerError> {
        let index = self
            ...
            .ok_or(LinkManagerError::MaxNumberOfLink)?;
