...
impl Drop for TestContext {
    fn drop(&mut self) {
        assert!(self.in_lmp_packets.borrow().is_empty());
        assert!(self.out_lmp_packets.borrow().is_empty());
        assert!(self.hci_commands.borrow().is_empty());
        assert!(self.hci_events.borrow().is_empty());
    }
}
...
pub fn poll(future: Pin<&mut impl Future<Output = ()>>) -> Poll<()> {
    future.poll(&mut task::Context::from_waker(&Arc::new(NoopWake).into()))
}
