  ...
  switch (event.GetEventCode()) {
    case EventCode::LE_META_EVENT: {
      ...
      switch (le_meta_event.GetSubeventCode()) {
        case SubeventCode::ADVERTISING_REPORT:           // TODO
        case SubeventCode::EXTENDED_ADVERTISING_REPORT:  // TODO
        default:
          break;
      }
      break;
...
static std::vector<uint8_t> generate_device_name(size_t device_nr,
  ...
  std::snprintf(reinterpret_cast<char*>(output.data()), output.size(),
                "#%02zd device", device_nr);
  return output;
}
...
std::vector<uint8_t> PcapFilter::ChangeDeviceName(
  ...
  std::vector<uint8_t> new_device_name =
      generate_device_name(device_name_map.size(), device_name.size());
