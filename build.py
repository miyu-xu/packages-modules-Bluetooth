...
USE_DEFAULTS = {
    ...
    'test': False,
    # Temporary workaround to prevent common-mk BUILD.gn from including sysroot in all compiler invocations,
    # breaking cross compiles. See http://cs/chromeos_public/src/platform2/common-mk/BUILD.gn?l=116
...
class HostBuild():
    ...
    def _generate_target_rustflags(self, triplet):
        ...
        Order matters here, e.g. setting linker after link-arg makes link-args not apply.
    """
        rust_flags = [
            '-C',
            'linker=clang',
    ...
    def configure_environ(self):
        ...
        self.env['RUSTFLAGS'] = self._generate_rustflags if not self.cross_compile else self._generate_target_rustflags(self.triplet)
    ...
    def _gn_configure(self):
        ...
        gn_args = {
            ...
            'external_cflags': [] if not self.cross_compile else ["-Os","-pipe","-g","-fno-exceptions","-fno-unwind-tables","-fno-asynchronous-unwind-tables","-ffunction-sections","-fdata-sections","-Wno-deprecated-declarations"],
            'external_cxxflags': ["-DNDEBUG"] if not self.cross_compile else ["-DNDEBUG","-Os","-pipe","-g","-fno-exceptions","-fno-unwind-tables","-fno-asynchronous-unwind-tables","-ffunction-sections","-fdata-sections","-Wno-deprecated-declarations"],
            'enable_werror': False,
        }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--triplet', help='Cross compile for a specific target triplet of the form <arch>-<vendor>-<sys>-<abi>. Builds for the host if unset.', default="")
    ...
    parser.add_argument(
        '--no-vendored-rust', help='Don\'t use vendored rust crates', default=False, action='store_true')
