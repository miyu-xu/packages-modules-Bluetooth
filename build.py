...
class HostBuild():
    ...
    def _generate_rustflags(self, machine):
    ...
    def configure_environ(self):
        ...
        self.env['RUSTFLAGS'] = self._generate_rustflags() + ' ' + \
                                self._generate_target_rustflags(host_machine)
...
class Bootstrap():
    ...
    def _print_missing_packages(self):
        ...
        if self.host_arch == 'amd64':
            ...
            raise Exception("Host architecture is not supported by build.py")
...
if __name__ == '__main__':
    ...
    if host_machine in SUPPORTED_MACHINES:
        ...
        raise Exception("Host machine %s currently not supported by build.py" % host_machine)
