...
use bluetooth_offload_hci::{self as hci, EventToBytes, Module};
...
fn reset() {
    ...
    m.in_evt(
        &hci::CommandComplete {
            ...
            return_parameters: hci::ResetComplete { status: 0 }.into(),
        }
        .to_bytes(),
    );
    ...
    m.in_evt(
        &hci::CommandComplete {
            num_hci_command_packets: 0,
            return_parameters: hci::LeReadBufferSizeV2Complete {
                status: 0,
