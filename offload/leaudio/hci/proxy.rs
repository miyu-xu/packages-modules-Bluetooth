...
impl Module for LeAudioModule {
    ...
    fn in_evt(&self, data: &[u8]) {
        match Event::from_bytes(data) {
            ...
            Ok(Event::NumberOfCompletedPackets(ref e)) => {
                let (stack_event, _) = {
                    let mut stack_event = hci::NumberOfCompletedPackets { handles: Vec::new() };
                    let mut audio_event = hci::NumberOfCompletedPackets { handles: Vec::new() };
                    let state = self.state.lock().unwrap();
