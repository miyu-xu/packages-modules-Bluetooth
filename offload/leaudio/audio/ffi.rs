...
use crate::codec::{CodecCConfig, Lc3CConfig, OpusCConfig};
...
/// Agreggation of the codec type and his configuration,
...
impl CCodec {
    pub fn config(&self) -> Box<dyn CodecCConfig> {
        match self.r#type {
            ...
            CCodecType::Lc3 => unsafe { Box::new(self.config.lc3) },

            // SAFETY: The C Code gives an Opus configuration type according
            //         to the selected Opus codec type.
            CCodecType::Opus => unsafe { Box::new(self.config.opus) },
        }
    }
}
