...
impl CodecCConfig for Lc3CConfig {
    ...
    fn new_encoder(&self, channels: usize, max_sdu_size: usize) -> Box<dyn Encode> {
        Box::new(Encoder::new(channels, max_sdu_size, self))
    }
}
...
impl Encoder {
    fn new(channels: usize, max_sdu_size: usize, config: &Lc3CConfig) -> Self {
        ...
        let frame_samples = unsafe { lc3_hr_frame_samples(hr_mode, dt_us, sr_hz) } as usize;
...
impl EncoderInstances {
    fn new(channels: usize, hr_mode: bool, dt_us: c_int, sr_hz: c_int) -> Self {
        ...
        for _ in 0..channels {
            ...
            let instance =
                unsafe { lc3_hr_setup_encoder(hr_mode, dt_us, sr_hz, 0, alloc(layout).cast()) };
