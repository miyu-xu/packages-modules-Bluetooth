 ...
 */
void *__swoff_leaudio_setup(const IsoStream iso_streams[], size_t num_iso_streams,
 ...
 * interface functions.
 */
void __swoff_leaudio_drop(void *handle);
 ...
 */
int __swoff_leaudio_write(void *handle, const uint8_t *data, size_t len);
...
public:
  LeAudioStream(const std::vector<IsoStream> iso_streams, const AudioConfig &audio_config,
    ...
    handle_ = __swoff_leaudio_setup(iso_streams.data(), iso_streams.size(), &audio_config,
                                    &callbacks_c);
  }

  ~LeAudioStream() { __swoff_leaudio_drop(handle_); }

  int write(const std::vector<uint8_t> data) {
    return __swoff_leaudio_write(handle_, data.data(), data.size());
  }
