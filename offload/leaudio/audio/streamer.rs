...
use crate::codec::{Encode, PcmFrame};
use crate::ffi::{CCodec, CIsoStream};
use std::cmp::min;
...
impl<Cb: Callbacks> Streamer<Cb> {
    pub fn new(
        ...
        let codec_channels = match iso.streams().count() {
            1 => iso.streams[0].channels,
            _ => 1,
        };

        if !matches!(bitdepth, 16 | 24 | 32) {
            return Err(format!("Invalid bitdepth: {}", bitdepth));
        }

        codec.config().validate(codec_channels)?;
    ...
    pub fn enable(&self, handle: u16, max_sdu_size: usize, sdu_interval_us: u32) {
        ...
        if matches!(*state, State::Idle) {
            let sample_rate = self.codec.config().sample_rate();
            let frame_len = ((sample_rate as u64 * sdu_interval_us as u64) / 1_000_000) as usize;
            let fifo = Arc::new(Fifo::new(self.bitdepth, frame_len));
    ...
    pub fn write(&self, chunk: &[u8]) -> Result<(), String> {
        match *self.state.lock().unwrap() {
            State::Running(ref fifo, _) => Ok(fifo.write(chunk)?),
...
impl Worker {
        ...
        F: Fn(u16, u16, &[u8]) + Send + 'static,
    {
        ...
        let thread = thread::spawn(move || {
            let mut worker = WorkerThread::new(fifo, codec, interval_us, max_sdu_size, send);
            let mut clocker = Clocker::new(interval_us);
...
where
    F: Fn(u16, u16, &[u8]),
{
    fn new(fifo: Arc<Fifo>, codec: CCodec, interval_us: u32, max_sdu_size: usize, send: F) -> Self {
        let sample_rate = codec.config().sample_rate();
        let frame_len = ((sample_rate as u64 * interval_us as u64) / 1_000_000) as usize;
        Self { fifo, codec, frame_len, max_sdu_size, streams: [None, None], send }
    }

    fn schedule(&mut self, iso: IsoState, sequence_number: u64) {
        let Some(frame) = self.fifo.get(self.frame_len) else {
            log::error!("PCM Data unavailable for SDU: {}", sequence_number);
            return;
        };
        ...
        for (i, iso) in iso.streams.iter().enumerate() {
            ...
            let stream = self.streams[i].get_or_insert_with(|| WorkerStream {
                sn0: sequence_number,
                encoder: self.codec.config().to_encoder(iso.channels),
            });
            ...
            (self.send)(
                ...
                &stream.encoder.encode(&pcm, self.max_sdu_size),
            );
        }
    }
}
...
impl Fifo {
    ...
    pub fn write(&self, mut chunk: &[u8]) -> Result<(), String> {
        let mut queue = self.queue.lock().unwrap();
        let cvar = &self.cvar;
        ...
        while !chunk.is_empty() {
            ...
            let size = min(chunk.len(), queue.capacity() - queue.len());
            queue.extend(&chunk[..size]);
            chunk = &chunk[size..];
        }
