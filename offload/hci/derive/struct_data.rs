...
fn parse_type_kind(ty: &syn::Type) -> Result<TypeKind, Error> {
    Ok(match ty {
        syn::Type::Path(v) => {
            ...
            match ident.to_string().as_str() {
                "u8" | "u16" | "u32" => TypeKind::Primitive(ident.clone()),
                "Vec" => match &segment.arguments {
                    syn::PathArguments::AngleBracketed(params) => match &params.args[0] {
                        syn::GenericArgument::Type(ty) => match parse_type_kind(ty)? {
                            TypeKind::Primitive(ident) => TypeKind::VecPrimitive(ident),
                            TypeKind::Any() => TypeKind::VecAny(),
        ...
        syn::Type::Array(v) => {
            let syn::Type::Path(ref elem) = *v.elem else {
                return Err(Error::new(ty.span(), "Unsupported array type"));
            };
            if elem.path.segments.len() > 1 || elem.path.segments[0].ident != "u8" {
                return Err(Error::new(ty.span(), "Only Byte array supported"));
            };
            TypeKind::Bytes()
        }
...
pub(crate) fn derive_read(name: &syn::Ident, data: &syn::DataStruct) -> Result<TokenStream, Error> {
    ...
    for field in &data.fields {
        ...
        let fn_token = match parse_type_kind(&field.ty)? {
            TypeKind::Primitive(ty) => match ty.to_string().as_str() {
                "u8" => {
            ...
            TypeKind::Any() => quote_spanned! { ident.span() => read()? },
            TypeKind::VecAny() => quote_spanned! { ident.span() => read_vec()? },
            TypeKind::Bytes(..) => quote_spanned! { ident.span() => read_bytes()? },
        };
...
pub(crate) fn derive_write(
    ...
    for field in &data.fields {
        ...
        let fn_token = match parse_type_kind(&field.ty)? {
            ...
            TypeKind::Any() => quote_spanned! { ident.span() => write(&self.#ident) },
            TypeKind::VecAny() => quote_spanned! { ident.span() => write_vec(&self.#ident) },
            TypeKind::Bytes(..) => quote_spanned! { ident.span() => write_bytes(&self.#ident) },
        };
