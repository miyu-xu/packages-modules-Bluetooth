...
impl Event {
    ...
    pub fn from_bytes(data: &[u8]) -> Self {
        ...
        Self::dispatch_read(code, &mut r).unwrap_or(Event::Malformed(Some(code)))
    }
...
impl EventToBytes for DisconnectionComplete {}
impl EventToBytes for CommandComplete {}
impl EventToBytes for CommandStatus {}
impl EventToBytes for NumberOfCompletedPackets {}
impl EventToBytes for LeCisEstablished {}
impl EventToBytes for LeCreateBigComplete {}
impl EventToBytes for LeTerminateBigComplete {}
...
fn test_le_create_big_complete() {
    ...
    let Event::LeCreateBigComplete(e) = Event::from_bytes(&dump) else { panic!() };
    assert_eq!(e.status, 0);
...
fn test_le_terminate_big_complete() {
    ...
    let Event::LeTerminateBigComplete(e) = Event::from_bytes(&dump) else { panic!() };
    assert_eq!(e.big_handle, 0x00);
    assert_eq!(e.reason, 0x16);
    assert_eq!(e.to_bytes(), &dump[..]);
}
