...
impl From<ResetComplete> for ReturnParameters {
    fn from(v: ResetComplete) -> ReturnParameters {
        Self::Reset(v)
    }
}

impl From<LeReadBufferSizeV2Complete> for ReturnParameters {
    fn from(v: LeReadBufferSizeV2Complete) -> ReturnParameters {
        Self::LeReadBufferSizeV2(v)
    }
}
...
impl Read for ReturnParameters {
    fn read(r: &mut Reader) -> Option<Self> {
        Some(match r.read_u16()?.into() {
            Reset::OPCODE => Self::Reset(r.read()?),
            LeReadBufferSizeV1::OPCODE => Self::LeReadBufferSizeV1(r.read()?),
...
impl Write for ReturnParameters {
    fn write(&self, w: &mut Writer) {
        match self {
            Self::Reset(p) => {
                w.write(&Reset::OPCODE);
                w.write(p);
            }
            Self::LeReadBufferSizeV1(p) => {
                w.write(&LeReadBufferSizeV1::OPCODE);
                w.write(p);
            }
            Self::LeReadBufferSizeV2(p) => {
                w.write(&LeReadBufferSizeV2::OPCODE);
                w.write(p);
            }
            Self::LeSetCigParameters(p) => {
                w.write(&LeSetCigParameters::OPCODE);
                w.write(p);
            }
            Self::LeRemoveCig(p) => {
                w.write(&LeRemoveCig::OPCODE);
                w.write(p);
            }
            Self::LeSetupIsoDataPath(p) => {
                w.write(&LeSetupIsoDataPath::OPCODE);
                w.write(p);
            }
            Self::LeRemoveIsoDataPath(p) => {
                w.write(&LeRemoveIsoDataPath::OPCODE);
                w.write(p);
            }
            Self::Other(..) => panic!(),
        };
...
impl CommandToBytes for Reset {}
impl CommandToBytes for LeSetCigParameters {}
impl CommandToBytes for LeCreateCis {}
impl CommandToBytes for LeRemoveCig {}
impl CommandToBytes for LeSetupIsoDataPath {}
impl CommandToBytes for LeRemoveIsoDataPath {}
...
fn test_reset() {
    ...
    let Command::Reset(c) = Command::from_bytes(&dump) else { panic!() };
...
fn test_le_set_cig_parameters_complete() {
    ...
    assert_eq!(p.connection_handle.len(), 2);
    assert_eq!(p.connection_handle[0], 0x60);
    assert_eq!(p.connection_handle[1], 0x61);
    assert_eq!(e.to_bytes(), &dump[..]);
}
