...
impl Writer {
    ...
    pub(crate) fn write_vec<T: Write>(&mut self, slice: &[T]) {
        self.write_u8(slice.len().try_into().unwrap());
        for item in slice {
            self.write(item);
        }
    }

    pub(crate) fn write_vec_u8(&mut self, slice: &[u8]) {
        self.write_u8(slice.len().try_into().unwrap());
        self.put(slice);
    }

    pub(crate) fn write_vec_u16(&mut self, slice: &[u16]) {
        self.write_u8(slice.len().try_into().unwrap());
        for v in slice {
            self.write_u16(*v);
        }
