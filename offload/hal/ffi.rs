...
/// `handle` is allocated to an `Option<T: Callbacks>`; It must be valid from the
...
//SAFETY: We allow the interface be executed from any thread.
//        The `Sync` trait is inherited by the Mutex that locks it.
...
impl<T: Callbacks> Ffi<T> {
    pub(crate) fn new(intf: CInterface) -> Self {
        Self { intf: Mutex::new(intf), wrapper: Arc::new(RwLock::new(None)) }
    }
    ...
    pub(crate) fn initialize(&self, client: T) {
        ...
        // SAFETY: The callback address is just borrowed;
        //         the wrapper object has the same lifetime as `Self`.
        unsafe {
            let wrapper = Arc::into_raw(self.wrapper.clone());
            (intf.initialize)(intf.handle, &CCallbacks::new(wrapper));
    ...
    pub(crate) fn send_command(&self, data: &[u8]) {
        ...
        // SAFETY: The HAL C Code has initialized the `CInterface` with a valid
...
impl CCallbacks {
    ...
    unsafe fn unwrap_client<T: Callbacks, F: FnOnce(&T)>(handle: *mut c_void, f: F) {
        // SAFETY: The `handle` points the wrapper object; it was allocated at the creation
        //         of the `Ffi` object and remain alive until its destruction.
        unsafe {
    ...
    extern "C" fn initialization_complete<T: Callbacks>(handle: *mut c_void, status: CStatus) {
        ...
        //         the same lifetine as the base `Ffi` instance.
        unsafe {
    ...
    extern "C" fn event_received<T: Callbacks>(handle: *mut c_void, data: *const u8, len: usize) {
        // SAFETY: The vendor HAL returns `handle` pointing `wrapper` object which has
        //         the same lifetine as the base `Ffi` instance. `data` points to a buffer
