...
impl CCallbacks {
    ...
    fn new<T: Callbacks>(wrapper: &Arc<RwLock<Option<T>>>) -> Self {
        Self {
            handle: Arc::as_ptr(wrapper).cast(),
    ...
    /// and not yet destroyed (this is in fact an `Arc<RwLock<Option<T>>>`).
    unsafe fn unwrap_client<T: Callbacks, F: FnOnce(&T)>(handle: *mut c_void, f: F) {
        let wrapper: *const RwLock<Option<T>> = handle.cast();

        // SAFETY: The `handle` points the `Arc<RwLock<Option<T>>>` wrapper object; it was allocated
