...
impl<T: Callbacks> Ffi<T> {
    ...
    pub(crate) fn send_command(&self, data: &[u8]) {
        // SAFETY: The vendor HAL is supposed to do the right things
        unsafe {
            let intf = self.intf.lock().unwrap();
    ...
    pub(crate) fn close(&self) {
        // SAFETY: Callback function addresses remain valid after closure
        unsafe {
            let intf = self.intf.lock().unwrap();
...
impl CCallbacks {
    ...
    fn unwrap_client<T: Callbacks, F: FnOnce(&T)>(handle: *mut c_void, f: F) {
        // SAFETY: The vendor HAL should returns us a valid `handle` on the wrapper,
        //         which has the same lifetime as the base `Ffi` instane.
        unsafe {
    ...
    extern "C" fn event_received<T: Callbacks>(handle: *mut c_void, data: *const u8, len: usize) {
        // SAFETY: The vendor HAL should return us valid address of packet data
        unsafe {
