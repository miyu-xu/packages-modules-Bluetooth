...
void LogLinkLayerConnectionEvent(const RawAddress* address,
  ...
  if (ret < 0) {
    log::warn(
        "failed to log status {:x}, reason {:x} from cmd {:x}, event {:x}, "
        "ble_event {:x} for {}, handle {}, type {:x}, error {}",
...
void LogHciTimeoutEvent(uint32_t hci_cmd) {
  ...
  if (ret < 0) {
    log::warn("failed for opcode {:x}, error {}", hci_cmd, ret);
  }
}
...
void LogRemoteVersionInfo(uint16_t handle, uint8_t status, uint8_t version,
  ...
  if (ret < 0) {
    log::warn(
        "failed for handle {}, status {:x}, version {:x}, manufacturer_name "
        "{:x}, subversion {:x}, error {}",
...
void LogReadRssiResult(const RawAddress& address, uint16_t handle,
  ...
  if (ret < 0) {
    log::warn("failed for {}, handle {}, status {:x}, rssi {} dBm, error {}",
...
void LogReadFailedContactCounterResult(const RawAddress& address,
  ...
  if (ret < 0) {
    log::warn(
        "failed for {}, handle {}, status {:x}, failed_contact_counter {} "
        "packets, error {}",
...
void LogReadTxPowerLevelResult(const RawAddress& address, uint16_t handle,
  ...
  if (ret < 0) {
    log::warn(
        "failed for {}, handle {}, status {:x}, transmit_power_level {} "
        "packets, error {}",
...
void LogSmpPairingEvent(const RawAddress& address, uint8_t smp_cmd,
  ...
  if (ret < 0) {
    log::warn(
        "failed for {}, smp_cmd {:x}, direction {}, smp_fail_reason {:x}, "
        "error {}",
...
void LogClassicPairingEvent(const RawAddress& address, uint16_t handle, uint32_t hci_cmd, uint16_t hci_event,
  ...
  if (ret < 0) {
    log::warn(
        "failed for {}, handle {}, hci_cmd {:x}, hci_event {:x}, cmd_status "
        "{:x}, reason {:x}, event_value {}, error {}",
...
void LogSdpAttribute(const RawAddress& address, uint16_t protocol_uuid,
  ...
  if (ret < 0) {
    log::warn("failed for {}, protocol_uuid {:x}, attribute_id {:x}, error {}",
...
void LogBluetoothHalCrashReason(const RawAddress& address, uint32_t error_code,
  ...
  if (ret < 0) {
    log::warn(
        "failed for {}, error_code {:x}, vendor_error_code {:x}, error {}",
