...
void LogLinkLayerConnectionEvent(const RawAddress* address,
  ...
  if (ret < 0) {
    log::warn(
        ...
        *address, connection_handle, loghex(link_type),
        ret);
  }
}
...
void LogA2dpAudioUnderrunEvent(const RawAddress& address,
  ...
  if (ret < 0) {
    log::warn(
        ...
        "error {}",
        address, encoding_interval_nanos,
...
void LogA2dpPlaybackEvent(const RawAddress& address, int playback_state,
  ...
  if (ret < 0) {
    log::warn(
        ...
        "{}",
        address, playback_state, audio_coding_mode,
        ret);
  }
}
...
void LogReadRssiResult(const RawAddress& address, uint16_t handle,
  ...
  if (ret < 0) {
    log::warn("failed for {}, handle {}, status {}, rssi {} dBm, error {}",
              address, handle, loghex(cmd_status),
              rssi, ret);
  }
}
...
void LogSmpPairingEvent(const RawAddress& address, uint8_t smp_cmd,
  ...
  if (ret < 0) {
    log::warn(
        ...
        address, loghex(smp_cmd), direction,
...
void LogSocketConnectionState(
  ...
  if (ret < 0) {
    log::warn(
        ...
        address, port, type, connection_state,
...
void LogManufacturerInfo(const RawAddress& address,
  ...
  if (ret < 0) {
    log::warn(
        ...
        address, source_type, source_name,
