...
class MessageLoopThreadTest : public ::testing::Test {
 ...
 protected:
  ...
  void SetUp() override {
    message_loop_thread = MessageLoopThread::Create("test_thread");
  }
...
TEST_F(MessageLoopThreadTest, get_weak_ptr) {
  std::weak_ptr<MessageLoopThread> message_loop_thread_ptr;
  message_loop_thread_ptr = message_loop_thread->GetWeakPtr();
  ASSERT_FALSE(message_loop_thread_ptr.expired());
  // Remove the last shared_ptr to the message loop thread.
  message_loop_thread = nullptr;
  ASSERT_TRUE(message_loop_thread_ptr.expired());
}
...
TEST_F(MessageLoopThreadTest, test_name) {
  ...
  ASSERT_EQ("test_thread", my_name);
  ASSERT_EQ("test_thread", message_loop_thread->GetName());
}
...
TEST_F(MessageLoopThreadTest, shut_down_multi_thread) {
  message_loop_thread->StartUp();
  auto thread = std::thread(&MessageLoopThread::ShutDown, message_loop_thread.get());
  message_loop_thread->ShutDown();
  thread.join();
}
...
TEST_F(MessageLoopThreadTest, start_up_multi_thread) {
  message_loop_thread->StartUp();
  auto thread = std::thread(&MessageLoopThread::StartUp, message_loop_thread.get());
  thread.join();
}
...
TEST_F(MessageLoopThreadTest, test_post_twice) {
  ...
  message_loop_thread->Post(
      base::BindOnce([](MessageLoopThread* thread,
                     ...
                     message_loop_thread.get(), &counter));
  message_loop_thread->Post(
      base::BindOnce([](MessageLoopThread* thread,
