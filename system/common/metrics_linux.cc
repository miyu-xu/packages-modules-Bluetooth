...
void A2dpSessionMetrics::Update(const A2dpSessionMetrics& metrics) {
  ...
  LOG_DEBUG(
      "To be added A2DP session: audio_duration_ms=%lld, "
      "media_timer_min_ms=%ld, media_timer_max_ms=%ld, media_timer_avg_ms=%ld, "
      "total_scheduling_count=%lld, buffer_overruns_max_count=%ld, "
      "buffer_overruns_total=%ld, buffer_underruns_average=%f, "
      "buffer_underruns_count=%lld, codec_index=%lld, is_a2dp_offload=%d",
      metrics.audio_duration_ms, metrics.media_timer_min_ms,
      metrics.media_timer_max_ms, metrics.media_timer_avg_ms,
      metrics.total_scheduling_count, metrics.buffer_overruns_max_count,
      metrics.buffer_overruns_total, metrics.buffer_underruns_average,
      metrics.buffer_underruns_count, metrics.codec_index,
      metrics.is_a2dp_offload);
  LOG_DEBUG(
      "Updated total session: audio_duration_ms=%lld, "
      "media_timer_min_ms=%ld, media_timer_max_ms=%ld, media_timer_avg_ms=%ld, "
      "total_scheduling_count=%lld, buffer_overruns_max_count=%ld, "
      "buffer_overruns_total=%ld, buffer_underruns_average=%f, "
      "buffer_underruns_count=%lld, codec_index=%lld, is_a2dp_offload=%d",
      this->audio_duration_ms, this->media_timer_min_ms,
      this->media_timer_max_ms, this->media_timer_avg_ms,
      this->total_scheduling_count, this->buffer_overruns_max_count,
      this->buffer_overruns_total, this->buffer_underruns_average,
      this->buffer_underruns_count, this->codec_index, this->is_a2dp_offload);
}
...
void LogLinkLayerConnectionEvent(const RawAddress* address,
  ...
  // TODO(enshuo): Resolve the statslog_bt header for non-Android build
  int ret = 0;
  if (ret < 0) {
...
void LogA2dpAudioUnderrunEvent(const RawAddress& address,
  ...
  // TODO(enshuo): Resolve the statslog_bt header for non-Android build
  LOG_DEBUG(
      "[BLUETOOTH_A2DP_AUDIO_UNDERRUN_REPORTED] - address=%s, "
      "encoding_interval_nanos=%lld, num_missing_pcm_bytes=%d",
      address.ToString().c_str(), encoding_interval_nanos,
...
void LogA2dpAudioOverrunEvent(const RawAddress& address,
  ...
  // TODO(enshuo): Resolve the statslog_bt header for non-Android build
  LOG_DEBUG(
      "[BLUETOOTH_A2DP_AUDIO_OVERRUN_REPORTED] - address=%s, "
      "encoding_interval_nanos=%lld, num_missing_pcm_bytes=%d, "
      "num_dropped_encoded_frames=%d, num_dropped_encoded_bytes=%d",
      address.ToString().c_str(), num_dropped_buffers,
...
void LogClassicPairingEvent(const RawAddress& address, uint16_t handle,
  ...
  if (ret < 0) {
    LOG(WARNING) << __func__ << ": failed for " << address << ", handle "
                 ...
                 << ", reason " << loghex(reason_code) << ", event_value "
                 << event_value << ", error " << ret;
  }
}
