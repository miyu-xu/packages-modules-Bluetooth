...
void A2dpSessionMetrics::Update(const A2dpSessionMetrics& metrics) {
  ...
  LOG_DEBUG(
      ...
      "media_timer_min_ms=%d, media_timer_max_ms=%d, media_timer_avg_ms=%d, "
      "total_scheduling_count=%lld, buffer_overruns_max_count=%d, "
      "buffer_overruns_total=%d, buffer_underruns_average=%f, "
      "buffer_underruns_count=%d, codec_index=%d, is_a2dp_offload=%d",
      (long long)metrics.audio_duration_ms, metrics.media_timer_min_ms,
      metrics.media_timer_max_ms, metrics.media_timer_avg_ms,
      (long long)metrics.total_scheduling_count,
      metrics.buffer_overruns_max_count, metrics.buffer_overruns_total,
      metrics.buffer_underruns_average, metrics.buffer_underruns_count,
      (int)metrics.codec_index, metrics.is_a2dp_offload);
  LOG_DEBUG(
      ...
      "buffer_overruns_total=%d, buffer_underruns_average=%f, "
      "buffer_underruns_count=%d, codec_index=%d, is_a2dp_offload=%d",
      (long long)this->audio_duration_ms, this->media_timer_min_ms,
      this->media_timer_max_ms, this->media_timer_avg_ms,
      (long long)this->total_scheduling_count, this->buffer_overruns_max_count,
      this->buffer_overruns_total, this->buffer_underruns_average,
      this->buffer_underruns_count, (int)this->codec_index,
      this->is_a2dp_offload);
}
...
void LogLinkLayerConnectionEvent(const RawAddress* address,
  ...
  // TODO(b/300256611): Resolve the statslog_bt header for non-Android build
  int ret = 0;
  if (ret < 0) {
...
void LogA2dpAudioUnderrunEvent(const RawAddress& address,
  ...
  LOG_DEBUG(
      ...
      address.ToString().c_str(), (long long unsigned)encoding_interval_millis,
...
void LogA2dpAudioOverrunEvent(const RawAddress& address,
  ...
  LOG_DEBUG(
      ...
      "encoding_interval_millis=%llu, num_missing_pcm_bytes=%d, "
