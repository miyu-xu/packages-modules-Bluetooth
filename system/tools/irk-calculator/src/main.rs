...
fn main() {
    ...
    // TODO(optedoblivion): Convert to subcommands
...
fn _validate_address_char(i: usize, address: &String) {
    match Some(parse_hex(address.chars().nth(i).unwrap().to_string().as_str())) {
        None => {
...
fn test_generate_rpa() {
    ...
    _validate_address_char(0, &address);
    _validate_address_char(1, &address);
    assert_eq!(address.chars().nth(2).unwrap(), ':');
    _validate_address_char(3, &address);
    _validate_address_char(4, &address);
    assert_eq!(address.chars().nth(5).unwrap(), ':');
    _validate_address_char(6, &address);
    _validate_address_char(7, &address);
    assert_eq!(address.chars().nth(8).unwrap(), ':');
    _validate_address_char(9, &address);
    _validate_address_char(10, &address);
    assert_eq!(address.chars().nth(11).unwrap(), ':');
    _validate_address_char(12, &address);
    _validate_address_char(13, &address);
}
