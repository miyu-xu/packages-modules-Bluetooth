...
public:
  ...
  virtual void commandStatusDelivery(uint16_t ocf, uint8_t status, Cookie cookie) = 0;
  virtual void commandCompleteDelivery(uint16_t ocf, std::vector<uint8_t> return_parameters,
                                       Cookie cookie) = 0;
  virtual void eventDelivery(uint8_t code, std::vector<uint8_t> data) = 0;
};
...
public:
  ...
  /** Register the bluetooth HCI vendor-specific callbacks */
