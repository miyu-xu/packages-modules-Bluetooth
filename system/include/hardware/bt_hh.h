...
/* address type */
typedef enum {
  BTHH_BLE_ADDR_PUBLIC = 0x00,
  BTHH_BLE_ADDR_RANDOM,
  BTHH_BLE_ADDR_PUBLIC_ID,
  BTHH_BLE_ADDR_RANDOM_ID,
  BTHH_BLE_ADDR_ANONYMOUS = 0xff
} bthh_address_type_t;

/* transport type */
typedef enum {
  BTHH_TRANSPORT_AUTO = 0x00,
  BTHH_TRANSPORT_BR_EDR,
  BTHH_BT_TRANSPORT_LE
} bthh_transport_type_t;
...
typedef struct {
  ...
  bt_status_t (*connect)(RawAddress* bd_addr, bthh_address_type_t addr_type,
                         bthh_transport_type_t transport);

  /** dis-connect from hid device */
  bt_status_t (*disconnect)(RawAddress* bd_addr, bthh_address_type_t addr_type,
                            bthh_transport_type_t transport);
  ...
  bt_status_t (*set_info)(RawAddress* bd_addr, bthh_address_type_t addr_type,
                          bthh_transport_type_t transport,
                          bthh_hid_info_t hid_info);
  ...
  bt_status_t (*set_idle_time)(RawAddress* bd_addr,
                               bthh_address_type_t addr_type,
                               bthh_transport_type_t transport,
                               uint8_t idleTime);

  /** Send a GET_REPORT to HID device. */
  bt_status_t (*get_report)(RawAddress* bd_addr, bthh_address_type_t addr_type,
                            bthh_transport_type_t transport,
                            bthh_report_type_t reportType, uint8_t reportId,
                            int bufferSize);

  /** Send a GET_REPORT_REPLY to HID driver. */
  bt_status_t (*get_report_reply)(RawAddress* bd_addr,
                                  bthh_address_type_t addr_type,
                                  bthh_transport_type_t transport,
  ...
  bt_status_t (*set_report)(RawAddress* bd_addr, bthh_address_type_t addr_type,
                            bthh_transport_type_t transport,
                            bthh_report_type_t reportType, char* report);

  /** Send data to HID device. */
  bt_status_t (*send_data)(RawAddress* bd_addr, bthh_address_type_t addr_type,
