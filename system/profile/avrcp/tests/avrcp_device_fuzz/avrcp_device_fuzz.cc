...
public:
  ...
  virtual void SetBrowsedPlayer(uint16_t player_id, SetBrowsedPlayerCallback browse_cb) {}
  virtual void PlayItem(uint16_t player_id, bool now_playing, std::string media_id) {}
...
public:
  ...
  virtual bool is_peer_in_silence_mode(const RawAddress& peer_address) { return false; }
  virtual void connect_audio_sink_delayed(uint8_t handle, const RawAddress& peer_address) {
    return;
  }
