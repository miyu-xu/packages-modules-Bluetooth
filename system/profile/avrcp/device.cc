...
void Device::SetBrowseMtu(uint16_t browse_mtu) {
  logger::info("{}: browse_mtu = {}", ADDRESS_TO_LOGGABLE_STR(address_),
               browse_mtu);
  browse_mtu_ = browse_mtu;
}

void Device::SetBipClientStatus(bool connected) {
  logger::info("{}: connected = {}", ADDRESS_TO_LOGGABLE_STR(address_),
               connected);
...
void Device::VendorPacketHandler(uint8_t label,
  ...
  logger::verbose("pdu={}", pkt->GetCommandPdu());

  if (!pkt->IsValid()) {
    logger::warn("{}: Request packet is not valid",
    ...
    switch (pkt->GetCommandPdu()) {
      ...
      case CommandPdu::REGISTER_NOTIFICATION: {
        ...
        if (register_notification->GetEvent() != Event::VOLUME_CHANGED) {
          logger::warn("{}: Unhandled register notification received: {}",
                       ADDRESS_TO_LOGGABLE_STR(address_),
      ...
      case CommandPdu::SET_ABSOLUTE_VOLUME:
        ...
        break;
      default:
        logger::warn("{}: Unhandled Response: pdu={}",
  ...
  switch (pkt->GetCommandPdu()) {
    ...
    case CommandPdu::LIST_PLAYER_APPLICATION_SETTING_ATTRIBUTES: {
      if (player_settings_interface_ == nullptr) {
        logger::error("Player Settings Interface not initialized.");
        auto response = RejectBuilder::MakeBuilder(pkt->GetCommandPdu(),
                                                   Status::INVALID_COMMAND);
        send_message(label, false, std::move(response));
        return;
    ...
    case CommandPdu::LIST_PLAYER_APPLICATION_SETTING_VALUES: {
      ...
      if (attribute < PlayerAttribute::EQUALIZER ||
        ...
        logger::warn("{}: Player Setting Attribute is not valid",
                     ADDRESS_TO_LOGGABLE_STR(address_));
        auto response = RejectBuilder::MakeBuilder(pkt->GetCommandPdu(),
                                                   Status::INVALID_PARAMETER);
        send_message(label, false, std::move(response));
      ...
      for (size_t i = 0; i < attributes.size(); i++) {
        ...
        if (attributes[i] == PlayerAttribute::REPEAT) {
          ...
          if (value < PlayerRepeatValue::OFF ||
            ...
            logger::warn("{}: Player Repeat Value is not valid",
                         ADDRESS_TO_LOGGABLE_STR(address_));
            invalid_request = true;
            break;
          }
          ...
          if (value < PlayerShuffleValue::OFF ||
            ...
            logger::warn("{}: Player Shuffle Value is not valid",
                         ADDRESS_TO_LOGGABLE_STR(address_));
            invalid_request = true;
            break;
          }
    ...
    default: {
      logger::error("{}: Unhandled Vendor Packet: {}",
...
void Device::HandleGetCapabilities(
  ...
  logger::verbose("capability={}", pkt->GetCapabilityRequested());
  ...
  switch (pkt->GetCapabilityRequested()) {
    ...
    default: {
      logger::warn("{}: Unhandled Capability: {}",
                   ADDRESS_TO_LOGGABLE_STR(address_),
...
void Device::HandleNotification(
  ...
  logger::verbose("event={}", pkt->GetEventRegistered());
  ...
  switch (pkt->GetEventRegistered()) {
    ...
    default: {
      logger::error("{}: Unknown event registered. Event ID={}",
                    ADDRESS_TO_LOGGABLE_STR(address_),
...
void Device::RegisterVolumeChanged() {
  ...
  if (label == MAX_TRANSACTION_LABEL) {
    logger::fatal("{}: Abandon all hope, something went catastrophically wrong",
                  ADDRESS_TO_LOGGABLE_STR(address_));
  }
...
void Device::HandleVolumeChanged(
  ...
  logger::verbose("interim={}", pkt->IsInterim());
  ...
  if (!IsActive()) {
    logger::verbose("Ignoring volume changes from non active device");
    return;
  }

  volume_ = pkt->GetVolume();
  volume_ &= ~0x80;  // remove RFA bit
  logger::verbose("Volume has changed to {}", (uint32_t)volume_);
  volume_interface_->SetVolume(volume_);
}

void Device::SetVolume(int8_t volume) {
  // TODO (apanicke): Implement logic for Multi-AVRCP
  logger::verbose("volume={}", (int)volume);
  if (volume == volume_) {
    logger::warn("{}: Ignoring volume change same as current volume level",
                 ADDRESS_TO_LOGGABLE_STR(address_));
    return;
  }
...
void Device::TrackChangedNotificationResponse(uint8_t label, bool interim,
  ...
  if (interim) {
    ...
    logger::verbose("Device not registered for update");
    return;
  }
  ...
  if (stack_config_get_interface()->get_pts_avrcp_test()) {
    logger::warn("{}: pts test mode", ADDRESS_TO_LOGGABLE_STR(address_));
  ...
  for (const SongInfo& song : song_list) {
    ...
    if (curr_song_id == song.media_id) {
      logger::verbose("Found media ID match for {}", song.media_id);
  ...
  if (uid == 0) {
    ...
    logger::error("{}: No match for media ID found",
                  ADDRESS_TO_LOGGABLE_STR(address_));
  }
...
void Device::PlaybackStatusNotificationResponse(uint8_t label, bool interim,
  ...
  if (!interim && state_to_send == last_play_status_.state) {
    logger::verbose("Not sending notification due to no state update {}",
                    ADDRESS_TO_LOGGABLE_STR(address_));
    return;
  }
...
void Device::PlaybackPosNotificationResponse(uint8_t label, bool interim,
  ...
  if (!interim && last_play_status_.position == status.position) {
    logger::warn("{}: No update to play position",
                 ADDRESS_TO_LOGGABLE_STR(address_));
    return;
  }
  ...
  if (status.state == PlayState::PLAYING && !IsInSilenceMode()) {
    logger::verbose("Queue next play position update");
...
void Device::AddressedPlayerNotificationResponse(
  ...
  logger::verbose("curr_player_id={}", (unsigned int)curr_player);
...
void Device::GetPlayStatusResponse(uint8_t label, PlayStatus status) {
  logger::verbose("position={} duration={} state={}", status.position,
...
void Device::MessageReceived(uint8_t label, std::shared_ptr<Packet> pkt) {
  ...
  logger::verbose("opcode={}", pkt->GetOpcode());
      ...
      if (pass_through_packet->GetOperationId() == 0x44 &&
        ...
        media_interface_->GetPlayStatus(base::Bind(
            [](base::WeakPtr<Device> d, PlayStatus s) {
              ...
              if (!d->IsActive()) {
                logger::info("Setting {} to be the active device",
...
void Device::HandlePlayItem(uint8_t label,
  ...
  logger::verbose("scope={} uid={}", pkt->GetScope(), pkt->GetUid());
  ...
  switch (pkt->GetScope()) {
    ...
    case Scope::VFS:
      ...
      break;
    default:
      logger::warn("{}: Unknown scope for play item",
                   ADDRESS_TO_LOGGABLE_STR(address_));
  }

  if (media_id == "") {
    logger::verbose("Could not find item");
...
void Device::HandleSetAddressedPlayer(
  ...
  logger::verbose("PlayerId={}", pkt->GetPlayerId());

  if (curr_player != pkt->GetPlayerId()) {
    logger::verbose("Reject invalid addressed player ID");
...
void Device::ListPlayerApplicationSettingAttributesResponse(
  ...
  logger::verbose("num_of_attributes={}", num_of_attributes);
  if (num_of_attributes > 0) {
    for (auto attribute : attributes) {
      logger::verbose("attribute={}", attribute);
    }
  }
  auto response =
...
void Device::ListPlayerApplicationSettingValuesResponse(
  ...
  logger::verbose("attribute={}, number_of_values={}", attribute,
                  number_of_values);

  if (number_of_values > 0) {
    if (attribute == PlayerAttribute::REPEAT) {
      for (auto value : values) {
        logger::verbose("value={}", static_cast<PlayerRepeatValue>(value));
      }
    } else if (attribute == PlayerAttribute::SHUFFLE) {
      for (auto value : values) {
        logger::verbose("value={}", static_cast<PlayerShuffleValue>(value));
      }
    } else {
      logger::verbose("value={}", loghex(values.at(0)));
    }
  }
...
void Device::GetPlayerApplicationSettingValueResponse(
    ...
    std::vector<uint8_t> values) {
  for (size_t i = 0; i < attributes.size(); i++) {
    logger::verbose("attribute={}",
                    static_cast<PlayerAttribute>(attributes[i]));
    if (attributes[i] == PlayerAttribute::REPEAT) {
      logger::verbose("value={}", static_cast<PlayerRepeatValue>(values[i]));
    } else if (attributes[i] == PlayerAttribute::SHUFFLE) {
      logger::verbose("value={}", static_cast<PlayerShuffleValue>(values[i]));
    } else {
      logger::verbose("value={}", loghex(values.at(0)));
    }
  }
...
void Device::SetPlayerApplicationSettingValueResponse(uint8_t label,
                                                      ...
                                                      bool success) {
  if (!success) {
    logger::error("{}: Set Player Application Setting Value failed",
                  ADDRESS_TO_LOGGABLE_STR(address_));
...
void Device::BrowseMessageReceived(uint8_t label,
  ...
  switch (pkt->GetPdu()) {
    ...
    case BrowsePdu::GET_TOTAL_NUMBER_OF_ITEMS:
      ...
      break;
    default:
      logger::warn("{}: pdu={}", ADDRESS_TO_LOGGABLE_STR(address_),
                   pkt->GetPdu());
      auto response =
...
void Device::HandleGetFolderItems(uint8_t label,
  ...
  if (!pkt->IsValid()) {
    ...
    logger::warn("{}: Get folder items request packet is not valid",
                 ADDRESS_TO_LOGGABLE_STR(address_));
  ...
  logger::verbose("scope={}", pkt->GetScope());
  ...
  switch (pkt->GetScope()) {
    ...
    case Scope::NOW_PLAYING:
      ...
      break;
    default:
      logger::error("{}: scope={}", ADDRESS_TO_LOGGABLE_STR(address_),
                    pkt->GetScope());
      auto response = GetFolderItemsResponseBuilder::MakePlayerListBuilder(
...
void Device::GetTotalNumberOfItemsMediaPlayersResponse(
  ...
  logger::verbose("num_items={}", list.size());
...
void Device::HandleChangePath(uint8_t label,
  ...
  logger::verbose("direction={} uid={}", pkt->GetDirection(),
                  loghex(pkt->GetUid()));

  if (pkt->GetDirection() == Direction::DOWN &&
      vfs_ids_.get_media_id(pkt->GetUid()) == "") {
    logger::error("{}: No item found for UID={}",
  ...
  if (pkt->GetDirection() == Direction::DOWN) {
    ...
    logger::verbose("Pushing Path to stack: \"{}\"", CurrentFolder());
  } else {
    // Don't pop the root id off the stack
    if (current_path_.size() > 1) {
      current_path_.pop();
    } else {
      logger::error("{}: Trying to change directory up past root.",
                    ADDRESS_TO_LOGGABLE_STR(address_));
      auto builder =
          ChangePathResponseBuilder::MakeBuilder(Status::DOES_NOT_EXIST, 0);
      send_message(label, true, std::move(builder));
    ...
    logger::verbose("Popping Path from stack: new path=\"{}\"",
                    CurrentFolder());
  }
...
void Device::HandleGetItemAttributes(
  ...
  switch (pkt->GetScope()) {
    ...
    case Scope::VFS:
      ...
      break;
    default:
      logger::error("{}: UNKNOWN SCOPE FOR HANDLE GET ITEM ATTRIBUTES",
                    ADDRESS_TO_LOGGABLE_STR(address_));
      break;
  }
}
...
void Device::GetItemAttributesNowPlayingResponse(
  ...
  logger::verbose("uid={}", loghex(pkt->GetUid()));
  ...
  logger::verbose("media_id=\"{}\"", media_id);

  SongInfo info;
  if (song_list.size() == 1) {
    logger::verbose("Send out the only song in the queue as now playing song.");
...
void Device::GetMediaPlayerListResponse(
  ...
  for (auto it = players.begin(); it != players.end(); it++) {
    if (it->id == curr_player) {
      logger::verbose(" Adding player to first spot: {}", it->name);
      auto temp_player = *it;
      players.erase(it);
...
void Device::GetVFSListResponse(uint8_t label,
  ...
  logger::verbose("start_item={} end_item={}", pkt->GetStartItem(),
                  pkt->GetEndItem());
...
void Device::HandleSetBrowsedPlayer(
  ...
  logger::verbose("player_id={}", pkt->GetPlayerId());
...
void Device::SetBrowsedPlayerResponse(
  ...
  logger::verbose("success={} root_id=\"{}\" num_items={}", success, root_id,
                  num_items);
...
void Device::SendMediaUpdate(bool metadata, bool play_status, bool queue) {
  ...
  CHECK(media_interface_);
  logger::verbose("Metadata={} : play_status= {} : queue={} : is_silence={}",
...
void Device::HandleTrackUpdate() {
  ...
  if (!track_changed_.first) {
    logger::warn("Device is not registered for track changed updates");
    return;
  }
...
void Device::HandlePlayStatusUpdate() {
  ...
  if (!play_status_changed_.first) {
    logger::warn("Device is not registered for play status updates");
    return;
  }
...
void Device::HandleNowPlayingUpdate() {
  ...
  if (!now_playing_changed_.first) {
    logger::warn("Device is not registered for now playing updates");
    return;
  }
...
void Device::HandlePlayerSettingChanged(std::vector<PlayerAttribute> attributes,
  ...
  if (!player_setting_changed_.first) {
    logger::warn("Device is not registered for player settings updates");
    return;
  }

  for (size_t i = 0; i < attributes.size(); i++) {
    logger::verbose(" attribute: {}", attributes[i]);
    if (attributes[i] == PlayerAttribute::SHUFFLE) {
      logger::verbose(" value: {}", (PlayerShuffleValue)values[i]);
    } else if (attributes[i] == PlayerAttribute::REPEAT) {
      logger::verbose(" value: {}", (PlayerRepeatValue)values[i]);
    } else {
      logger::verbose(" value: {}", values[i]);
    }
  }
...
void Device::PlayerSettingChangedNotificationResponse(
    ...
    std::vector<uint8_t> values) {
  logger::verbose("interim: {}", interim);
  for (size_t i = 0; i < attributes.size(); i++) {
    logger::verbose(" attribute: {}", attributes[i]);
    if (attributes[i] == PlayerAttribute::SHUFFLE) {
      logger::verbose(" value: {}", (PlayerShuffleValue)values[i]);
...
void Device::HandlePlayPosUpdate() {
  ...
  if (!play_pos_changed_.first) {
    logger::warn("Device is not registered for play position updates");
    return;
  }
...
void Device::HandleAvailablePlayerUpdate() {
  ...
  if (!avail_players_changed_.first) {
    logger::warn("Device is not registered for available player updates");
    return;
  }
...
void Device::HandleAddressedPlayerUpdate() {
  ...
  if (!addr_player_changed_.first) {
    logger::warn("{}: Device is not registered for addressed player updates",
                 ADDRESS_TO_LOGGABLE_STR(address_));
    return;
  }
  media_interface_->GetMediaPlayerList(base::Bind(
...
void Device::DeviceDisconnected() {
  logger::info("{} : Device was disconnected",
               ADDRESS_TO_LOGGABLE_STR(address_));
