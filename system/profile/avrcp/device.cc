...
void Device::VendorPacketHandler(uint8_t label,
  ...
  switch (pkt->GetCommandPdu()) {
    ...
    case CommandPdu::SET_PLAYER_APPLICATION_SETTING_VALUE: {
      ...
      DEVICE_LOG(ERROR) << set_player_setting_value_request->ToString();
...
void Device::ListPlayerApplicationSettingAttributesResponse(
  ...
  if (num_of_attributes > 0) {
    for (auto attribute : attributes) {
      DEVICE_LOG(ERROR) << __func__ << ": attribute=" << attribute;
    }
  }
  auto response =
...
void Device::ListPlayerApplicationSettingValuesResponse(
  ...
  auto response =
      ListPlayerApplicationSettingValuesResponseBuilder::MakeBuilder(values);
  send_message(label, false, std::move(response));
}
...
void Device::HandlePlayerSettingChanged(std::vector<PlayerAttribute> attributes,
  ...
  for (size_t i = 0; i < attributes.size(); i++) {
    DEVICE_LOG(ERROR) << " attribute: " << attributes[i] << std::endl;
    if (attributes[i] == PlayerAttribute::SHUFFLE) {
      DEVICE_LOG(ERROR) << " value: " << (PlayerShuffleValue)values[i]
                        << std::endl;
    } else if (attributes[i] == PlayerAttribute::REPEAT) {
      DEVICE_LOG(ERROR) << " value: " << (PlayerRepeatValue)values[i]
                        << std::endl;
    } else {
      DEVICE_LOG(ERROR) << " value: " << std::to_string(values[i]) << std::endl;
    }
  }
...
void Device::PlayerSettingChangedNotificationResponse(
    ...
    std::vector<uint8_t> values) {
  DEVICE_LOG(ERROR) << __func__;
  DEVICE_LOG(ERROR) << " interim: " << interim << std::endl;
  for (size_t i = 0; i < attributes.size(); i++) {
    DEVICE_LOG(ERROR) << " attribute: " << attributes[i] << std::endl;
    if (attributes[i] == PlayerAttribute::SHUFFLE) {
      DEVICE_LOG(ERROR) << " value: " << (PlayerShuffleValue)values[i]
