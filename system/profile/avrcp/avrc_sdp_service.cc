...
namespace bluetooth {
namespace avrc {
std::shared_ptr<AvrcSdpService> AvrcSdpService::instance_ =
    nullptr;  // Static member definition

std::shared_ptr<AvrcSdpService> AvrcSdpService::Get() {
  static std::once_flag onceFlag;
  std::call_once(onceFlag,
                 []() { instance_ = std::make_shared<AvrcSdpService>(); });
  return instance_;
}

uint16_t AvrcSdpService::AddRecord(
...
uint16_t AvrcSdpService::EnableCovertArt(uint16_t service_uuid,
...
uint16_t AvrcSdpService::DisableCovertArt(uint16_t service_uuid) {
  if (service_uuid == UUID_SERVCLASS_AV_REM_CTRL_TARGET) {
...
uint16_t AvrcSdpService::RemoveRecord(uint16_t service_uuid) {
  if (service_uuid == UUID_SERVCLASS_AV_REM_CTRL_TARGET) {
