...
static uint8_t convertRcFeature(uint16_t features) {
  uint8_t feature = 0;
  if ((features & BTA_AV_FEAT_ADV_CTRL) && (features & BTA_AV_FEAT_RCTG)) {
    feature = (feature | static_cast<uint8_t>(RcFeature::RC_FEAT_ABSOLUTE_VOLUME));
  }
  if (features & BTA_AV_FEAT_METADATA) {
    feature = (feature | static_cast<uint8_t>(RcFeature::RC_FEAT_METADATA));
  }
  if (features & BTA_AV_FEAT_BROWSE) {
    feature = (feature | static_cast<uint8_t>(RcFeature::RC_FEAT_BROWSE));
  }
  if (features & BTA_AV_FEAT_COVER_ARTWORK) {
      feature = (feature | static_cast<uint8_t>(RcFeature::RC_FEAT_COVERART));
  }
    ...
    case AVRC_OPEN_IND_EVT: {
      ...
      for (auto it = feature_map_.begin(); it != feature_map_.end(); it++) {
        if (*peer_addr == it->first) {
          device_map_[handle]->SetRcFeatures(convertRcFeature(it->second));
          break;
        }
      }
    } break;
...
void ConnectionHandler::SdpCb(RawAddress bdaddr, SdpCallback cb, tSDP_DISCOVERY_DB* disc_db,
  ...
  for (auto it = device_map_.begin(); it != device_map_.end(); it++) {
    if (bdaddr == it->second->GetAddress()) {
      device_map_[it->first]->SetRcFeatures(convertRcFeature(peer_features));
      break;
    }
  }
