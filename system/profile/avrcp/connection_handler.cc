...
bool ConnectionHandler::ConnectDevice(const RawAddress& bdaddr) {
  ...
  auto connection_lambda = [](ConnectionHandler* instance_,
    ...
    log::info("{} SDP Completed features={}", __PRETTY_FUNCTION__,
              loghex(features));
...
bool ConnectionHandler::AvrcpConnect(bool initiator, const RawAddress& bdaddr) {
  ...
  log::info("{}: handle={} status= {}", __PRETTY_FUNCTION__, loghex(handle),
            loghex(status));
...
void ConnectionHandler::InitiatorControlCb(uint8_t handle, uint8_t event,
  ...
  log::info("{}: handle={} result={} addr={}", __PRETTY_FUNCTION__,
            loghex(handle), loghex(result),
            (peer_addr ? ADDRESS_TO_LOGGABLE_STR(*peer_addr) : "none"));

  switch (event) {
    case AVRC_OPEN_IND_EVT: {
      log::info("{}: Connection Opened Event", __PRETTY_FUNCTION__);
    ...
    case AVRC_CLOSE_IND_EVT: {
      log::info("{}: Connection Closed Event", __PRETTY_FUNCTION__);
    ...
    case AVRC_BROWSE_OPEN_IND_EVT: {
      log::info("{}: Browse Open Event", __PRETTY_FUNCTION__);
    ...
    case AVRC_BROWSE_CLOSE_IND_EVT:
      log::info("{}: Browse Close Event", __PRETTY_FUNCTION__);
      break;
    default:
    ...
    case AVRC_OPEN_IND_EVT: {
      ...
      auto sdp_lambda = [](ConnectionHandler* instance_, uint8_t handle,
        ...
        if (instance_->device_map_.find(handle) ==
          ...
          log::warn("{}: No device found for handle: {}", __PRETTY_FUNCTION__,
                    loghex(handle));
          return;
        }
...
void ConnectionHandler::SdpCb(RawAddress bdaddr, SdpCallback cb,
                              ...
                              uint16_t status) {
  log::verbose("{}: SDP lookup callback received", __PRETTY_FUNCTION__);

  if (status == SDP_CONN_FAILED && !retry) {
    log::warn("{}: SDP Failure retry again", __PRETTY_FUNCTION__);
    SdpLookup(bdaddr, cb, true);
    return;
  } else if (status != AVRC_SUCCESS) {
    log::error("{}: SDP Failure: status = {}", __PRETTY_FUNCTION__,
               (unsigned int)status);
    cb.Run(status, 0, 0);
    osi_free(disc_db);
    return;
  ...
  if (sdp_record != nullptr) {
    log::info("{}: Device {} supports remote control", __PRETTY_FUNCTION__,
              ADDRESS_TO_LOGGABLE_STR(bdaddr));
    ...
    if ((sdp_->FindAttributeInRec(sdp_record, ATTR_ID_BT_PROFILE_DESC_LIST)) !=
      ...
      log::verbose("{}: Device {} peer avrcp version={}", __PRETTY_FUNCTION__,
      ...
      if (peer_avrcp_version >= AVRC_REV_1_3) {
        ...
        log::verbose("{}: Device {} supports metadata", __PRETTY_FUNCTION__,
                     ADDRESS_TO_LOGGABLE_STR(bdaddr));
        peer_features |= (BTA_AV_FEAT_VENDOR | BTA_AV_FEAT_METADATA);
      }
      if (peer_avrcp_version >= AVRC_REV_1_4) {
        /* get supported categories */
        log::verbose("{} Get Supported categories", __PRETTY_FUNCTION__);
        ...
        if (sdp_attribute != NULL &&
          ...
          if (categories & AVRC_SUPF_CT_BROWSE) {
            log::verbose("{}: Device {} supports browsing", __PRETTY_FUNCTION__,
                         ADDRESS_TO_LOGGABLE_STR(bdaddr));
