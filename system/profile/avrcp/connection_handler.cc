...
bool IsAbsoluteVolumeEnabled(const RawAddress* bdaddr) {
  ...
  if (strncmp(volume_disabled, "true", 4) == 0) {
    logger::info("Absolute volume disabled by property");
    return false;
  }
  if (interop_match_addr(INTEROP_DISABLE_ABSOLUTE_VOLUME, bdaddr)) {
    logger::info("Absolute volume disabled by IOP table");
    return false;
  }
  return true;
}
...
bool ConnectionHandler::ConnectDevice(const RawAddress& bdaddr) {
  ...
  for (const auto& pair : device_map_) {
    if (bdaddr == pair.second->GetAddress()) {
      logger::warn("Already connected to device with address {}",
                   ADDRESS_TO_LOGGABLE_STR(bdaddr));
      return false;
    }
  }
  ...
  auto connection_lambda = [](ConnectionHandler* instance_,
    ...
    logger::info("{} SDP Completed features={}", __PRETTY_FUNCTION__,
                 loghex(features));
...
bool ConnectionHandler::AvrcpConnect(bool initiator, const RawAddress& bdaddr) {
  logger::info("Connect to device {}", ADDRESS_TO_LOGGABLE_STR(bdaddr));
  ...
  logger::info("{}: handle={} status= {}", __PRETTY_FUNCTION__, loghex(handle),
               loghex(status));
...
void ConnectionHandler::InitiatorControlCb(uint8_t handle, uint8_t event,
  ...
  logger::info("{}: handle={} result={} addr={}", __PRETTY_FUNCTION__,
               loghex(handle), loghex(result),
               (peer_addr ? ADDRESS_TO_LOGGABLE_STR(*peer_addr) : "none"));

  switch (event) {
    case AVRC_OPEN_IND_EVT: {
      logger::info("{}: Connection Opened Event", __PRETTY_FUNCTION__);
    ...
    case AVRC_CLOSE_IND_EVT: {
      logger::info("{}: Connection Closed Event", __PRETTY_FUNCTION__);
    ...
    case AVRC_BROWSE_OPEN_IND_EVT: {
      logger::info("{}: Browse Open Event", __PRETTY_FUNCTION__);
      ...
      if (device_map_.find(handle) == device_map_.end()) {
        logger::warn("Browse Opened received from device that doesn't exist");
        return;
      }
    ...
    case AVRC_BROWSE_CLOSE_IND_EVT:
      logger::info("{}: Browse Close Event", __PRETTY_FUNCTION__);
      break;
    default:
      logger::error("Unknown AVRCP Control event");
      break;
  }
}
  ...
  switch (event) {
    case AVRC_OPEN_IND_EVT: {
      ...
      if (btif_av_src_sink_coexist_enabled() &&
        ...
        logger::warn("peer is src, close new avrcp cback");
        if (device_map_.find(handle) != device_map_.end()) {
          std::lock_guard<std::recursive_mutex> lock(device_map_lock);
          feature_map_.erase(device_map_[handle]->GetAddress());
          device_map_[handle]->DeviceDisconnected();
      ...
      logger::info("{}: Performing SDP on connected device. address={}",
                   __PRETTY_FUNCTION__, ADDRESS_TO_LOGGABLE_STR(*peer_addr));
      auto sdp_lambda = [](ConnectionHandler* instance_, uint8_t handle,
        ...
        if (instance_->device_map_.find(handle) ==
          ...
          logger::warn("{}: No device found for handle: {}",
                       __PRETTY_FUNCTION__, loghex(handle));
          return;
        }
      ...
      if (SdpLookup(*peer_addr, base::Bind(sdp_lambda, this, handle), false)) {
        ...
        // connection.
        logger::error(
            "{}: SDP search failed for handle: {}, closing connection",
            __PRETTY_FUNCTION__, loghex(handle));
...
void ConnectionHandler::MessageCb(uint8_t handle, uint8_t label, uint8_t opcode,
                                  tAVRC_MSG* p_msg) {
  if (device_map_.find(handle) == device_map_.end()) {
    logger::error("Message received for unconnected device: handle={}",
                  loghex(handle));
    return;
  }
  ...
  if (opcode == AVRC_OP_BROWSE) {
    if (btif_av_src_sink_coexist_enabled() && btif_av_both_enable()) {
      if (p_msg->browse.hdr.ctype == AVCT_RSP) {
        logger::verbose("ignore response handle {}", (unsigned int)handle);
        return;
      }
    }
    logger::verbose("Browse Message received on handle {}",
                    (unsigned int)handle);
    device_map_[handle]->BrowseMessageReceived(label, BrowsePacket::Parse(pkt));
    return;
  }

  logger::verbose("Message received on handle {}", (unsigned int)handle);
...
void ConnectionHandler::SdpCb(RawAddress bdaddr, SdpCallback cb,
                              ...
                              uint16_t status) {
  logger::verbose("{}: SDP lookup callback received", __PRETTY_FUNCTION__);

  if (status == SDP_CONN_FAILED && !retry) {
    logger::warn("{}: SDP Failure retry again", __PRETTY_FUNCTION__);
    SdpLookup(bdaddr, cb, true);
    return;
  } else if (status != AVRC_SUCCESS) {
    logger::error("{}: SDP Failure: status = {}", __PRETTY_FUNCTION__,
                  (unsigned int)status);
    cb.Run(status, 0, 0);
    osi_free(disc_db);
    return;
  ...
  if (sdp_record != nullptr) {
    logger::info("{}: Device {} supports remote control", __PRETTY_FUNCTION__,
                 ADDRESS_TO_LOGGABLE_STR(bdaddr));
    ...
    if ((sdp_->FindAttributeInRec(sdp_record, ATTR_ID_BT_PROFILE_DESC_LIST)) !=
      ...
      logger::verbose("{}: Device {} peer avrcp version={}",
                      __PRETTY_FUNCTION__, ADDRESS_TO_LOGGABLE_STR(bdaddr),
      ...
      if (peer_avrcp_version >= AVRC_REV_1_3) {
        ...
        logger::verbose("{}: Device {} supports metadata", __PRETTY_FUNCTION__,
                        ADDRESS_TO_LOGGABLE_STR(bdaddr));
        peer_features |= (BTA_AV_FEAT_VENDOR | BTA_AV_FEAT_METADATA);
      }
      if (peer_avrcp_version >= AVRC_REV_1_4) {
        /* get supported categories */
        logger::verbose("{} Get Supported categories", __PRETTY_FUNCTION__);
        ...
        if (sdp_attribute != NULL &&
          ...
          logger::verbose("{}Get Supported categories SDP ATTRIBUTES != null",
                          __PRETTY_FUNCTION__);
          uint16_t categories = sdp_attribute->attr_value.v.u16;
          if (categories & AVRC_SUPF_CT_CAT2) {
            logger::verbose("{}: Device {} supports advanced control",
                            __PRETTY_FUNCTION__,
                            ADDRESS_TO_LOGGABLE_STR(bdaddr));
          ...
          if (categories & AVRC_SUPF_CT_BROWSE) {
            logger::verbose("{}: Device {} supports browsing",
                            __PRETTY_FUNCTION__,
                            ADDRESS_TO_LOGGABLE_STR(bdaddr));
  ...
  sdp_record = sdp_->FindServiceInDb(disc_db, UUID_SERVCLASS_AV_REM_CTRL_TARGET,
                                     nullptr);
  if (sdp_record != nullptr) {
    logger::verbose("{}: Device {} supports remote control target",
                    __PRETTY_FUNCTION__, ADDRESS_TO_LOGGABLE_STR(bdaddr));
    ...
    logger::verbose("{}: Device {} peer avrcp target version={}",
                    __PRETTY_FUNCTION__, ADDRESS_TO_LOGGABLE_STR(bdaddr),
...
void ConnectionHandler::SendMessage(
  ...
  logger::info("SendMessage to handle={}", loghex(handle));
...
void ConnectionHandler::RegisterVolChanged(const RawAddress& bdaddr) {
  logger::info("Attempting to RegisterVolChanged device {}",
               ADDRESS_TO_LOGGABLE_STR(bdaddr));
  for (auto it = device_map_.begin(); it != device_map_.end(); it++) {
    if (bdaddr == it->second->GetAddress()) {
