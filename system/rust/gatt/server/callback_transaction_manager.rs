...
use super::gatt_database::GattDatastore;
...
impl CallbackTransactionManager {
    async fn read_characteristic(
        ...
        let (trans_id, rx) = {
            let mut pending = self.pending_transactions.borrow_mut();
            let trans_id = TransactionId(pending.next_transaction_id);
            pending.next_transaction_id += 1;

            let (tx, rx) = oneshot::channel();
            let pending_transactions = pending.pending_transactions.get_mut(&conn_id);
        ...
        let address = RawAddress([1, 2, 3, 4, 5, 6]); // DO NOT SUBMIT
        self.callbacks.on_server_read_characteristic(
            address,
            conn_id,
            trans_id,
            handle,
    ...
    // TODO: make this thing specialize for isolated servers
    pub fn get_datastore(self: &Rc<Self>) -> CallbackGattDatastore {
        CallbackGattDatastore { callback_manager: self.clone() }
    }
}

pub struct CallbackGattDatastore {
    callback_manager: Rc<CallbackTransactionManager>,
}

#[async_trait(?Send)]
impl GattDatastore for CallbackGattDatastore {
    async fn read_characteristic(
        &self,
        conn_id: ConnectionId,
        handle: AttHandle,
    ) -> Result<AttAttributeDataChild, AttErrorCode> {
        self.callback_manager.read_characteristic(conn_id, handle).await
    }
    ...
    async fn write_characteristic(
        ...
        self.callback_manager.write_characteristic(conn_id, handle, data).await
    }
}
