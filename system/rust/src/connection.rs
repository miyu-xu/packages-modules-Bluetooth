...
impl ConnectionManager {
    ...
    async fn is_connected(&self, address: AddressWithType, state: &mut ConnectionManagerState) -> bool {
    ...
    async fn on_le_connect(
        ...
        state.attempts.process_connection(canonical_address, self.address_resolver.as_ref(), result).await;
    ...
    async fn reconcile_state(&self, state: &mut ConnectionManagerState) {
        state
            .acceptlist_manager
            .drive_to_state(determine_target_state(&state.attempts.active_attempts(), self.address_resolver.as_ref()).await);
    }
}
...
mod test {
    ...
    fn test_resolved_background_connection() {
        block_on_locally(async {
            ...
            connection_manager.as_ref().add_background_connection(CLIENT_1, ADDRESS_1).await.unwrap();
            mock_le_manager.on_le_connect(ADDRESS_1, ErrorCode::SUCCESS);
