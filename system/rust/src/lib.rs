...
impl RustModuleRunner {
    ...
    #[allow(dead_code)]
    fn send(&self, f: BoxedMainThreadCallback) -> Result<(), String> {
        match self {
            Self::NotStarted => Err("Not started yet".to_string()),
            Self::Ended { .. } | Self::Stopping => Err("Runner ended".to_string()),
            Self::Running { tx } => tx.send(f).map_err(|e| format!("Failed to send: {e:?}")),
        }
    }

    /// Retrieve a clone of the sender, which can be passed through FFI to send messages.
    /// Returns None if the runner has either ended or has not started yet.
    fn sender(&self) -> Option<mpsc::UnboundedSender<BoxedMainThreadCallback>> {
        let Self::Running { tx } = self else {
            return None;
        };
        Some(tx.clone())
    }
...
thread_local! {
    ...
    static MAIN_THREAD_TX: mpsc::UnboundedSender<BoxedMainThreadCallback> = {
        let Some(sender) = GLOBAL_MODULE_RUNNER.lock().unwrap().sender() else {
            error!("Failed to get sender from runner");
            panic!("Initialization issue");
        };
        sender
    }
}
...
{
    if let Err(e) = MAIN_THREAD_TX.with(|sender| sender.send(Box::new(f))) {
        error!("Failed to do_in_rust_thread: {e:?}");
