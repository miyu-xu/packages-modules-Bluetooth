...
void AddressResolverShim::ResolveAddress(
    core::AddressWithType address,
    ::rust::Box<ResolveAddressCallback> on_resolved) const {
  do_in_main_thread(
      FROM_HERE,
      base::BindOnce(
          [](core::AddressWithType original_address,
             ::rust::Box<ResolveAddressCallback> on_resolved) {
            if (original_address.address_type == core::AddressType::Public) {
              Invoke(std::move(on_resolved), original_address);
              return;
            }
            auto* p_dev_rec =
