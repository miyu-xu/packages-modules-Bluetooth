...
use super::{Role, AddressManager, Connection, hci_operations::{HciConnectProxy, HciEvent, ErrorCode}};
...
impl HciConnectionStateMachine {
    ...
    fn on_connection_complete(
        ...
        let out = if role == Role::Central {
            self.state = InitiatorState::StableAt(InitiatorStableState::Stopped);
            if status == ErrorCode::SUCCESS {
                // if we got a successful connection, remain in the stopped state
                // until a further command is received
                self.requested_state = InitiatorStableState::Stopped;
                Some(Connection { remote_address: address, role })
            } else {
                // otherwise, we may (or may not) just immediately restart
                // initiation, so don't touch the requested_state
                None
            }
        } else {
            None
