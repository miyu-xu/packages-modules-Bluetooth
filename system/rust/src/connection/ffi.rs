...
mod inner {
    ...
    unsafe extern "C++" {
        ...
        fn register_rust_callbacks(self: Pin<&mut Self>, callbacks: Box<LeAclManagerCallbackShim>);
    }
...
pub struct LeAclManagerCallbackShim(Box<dyn LeAclManagerConnectionCallbacks>);

impl LeAclManagerCallbackShim {
    fn on_le_connect_success(&self, address: AddressWithType) {
        self.0.on_le_connect(address, Ok(LeConnection { remote_address: address }))
    }

    fn on_le_connect_fail(&self, address: AddressWithType, status: u8) {
        self.0.on_le_connect(address, Err(ErrorCode(status)))
    }
...
impl InactiveLeAclManager for LeAclManagerImpl {
    ...
    fn register_callbacks(
        ...
        self.0
            .pin_mut()
            .register_rust_callbacks(Box::new(LeAclManagerCallbackShim(Box::new(callbacks))));
        self
    }
}
