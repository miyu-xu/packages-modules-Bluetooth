...
mod inner {
    ...
    unsafe extern "C++" {
        ...
        fn StoreHciCallbacksFromRust(
            ...
            on_disconnect: fn(address: AddressWithType),
        );
...
impl HciConnectProxy for HciConnectProxyImpl {
    ...
    fn add_to_accept_list(&self, address: AddressWithType) {
        self.0.add_to_filter_accept_list(address)
    }

    fn remove_from_accept_list(&self, address: AddressWithType) {
        self.0.remove_from_filter_accept_list(address);
    }

    fn disconnect(&self, _conn: Connection) {
        todo!()
    }
}
