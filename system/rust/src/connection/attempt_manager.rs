...
struct ConnectionAttemptData {
    conn_tx: Option<oneshot::Sender<LeConnection>>,
}
...
impl ConnectionAttempts {
    ...
    pub fn remove_unconditionally(&self, address: AddressWithType) {
        self.active_attempts.borrow_mut().retain(|attempt, _| attempt.remote_address != address);
    }

    /// Cancel all connection attempts from this client
    pub fn remove_client(&self, client: ConnectionManagerClient) {
        self.active_attempts.borrow_mut().retain(|attempt, _| attempt.client != client);
    }

    /// List all active connection attempts
    ...
    /// Handle a succesful connection by notifying clients and resolving direct connect attempts
    pub fn process_connection(&self, conn: LeConnection) {
