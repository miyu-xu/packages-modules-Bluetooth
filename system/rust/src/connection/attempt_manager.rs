...
impl ConnectionAttempts {
    ...
    > {
        ...
        Ok(PendingConnectionAttempt {
            id,
            f: async move { rx.await.map_err(|_| ConnectionFailure::Cancelled) },
        })
    }
    ...
    pub fn process_connection(&mut self, conn: LeConnection) {
        ...
        for attempt in interested_clients {
            if attempt.mode == ConnectionMode::Direct {
                ...
                let _ = self.attempts.remove(&attempt).unwrap().conn_tx.unwrap().send(conn);
            } else {
...
mod test {
    ...
    fn test_resolved_direct_connection() {
        block_on_locally(async {
            ...
            attempts.process_connection(CONNECTION_1);
    ...
    fn test_incoming_connection_while_another_is_pending() {
        block_on_locally(async {
            ...
            attempts.process_connection(CONNECTION_2);
