...
mod test {
    ...
    fn test_queued_writes() {
        ...
        tokio_test::block_on(async {
            ...
            assert_eq!(
                Ok(handler.process_packet(request.try_into().unwrap(), 255).await),
            ...
            assert_eq!(
                Ok(handler
                    .process_packet(
                        att::AttReadRequest { attribute_handle: AttHandle(3).into() }
                            .try_into()
                            .unwrap(),
                        255
                    )
            ...
            assert_eq!(
                Ok(handler
                    .process_packet(
                        att::AttExecuteWriteRequest { commit: 1 }.try_into().unwrap(),
                        255
                    )
                    .await),
    ...
    fn test_cancelling_queued_writes() {
        ...
        tokio_test::block_on(async {
            ...
            assert_eq!(
                Ok(handler
                    .process_packet(
                        att::AttExecuteWriteRequest { commit: 0 }.try_into().unwrap(),
                        255
                    )
                    .await),
                att::AttExecuteWriteResponse {}.try_into()
    ...
    fn test_queued_write_to_different_datastores() {
        ...
        tokio_test::block_on(async {
            ...
            assert_eq!(
                Ok(handler.process_packet(request.clone().try_into().unwrap(), 255).await),
                att::AttErrorResponse {
            ...
            assert_eq!(
                Ok(handler
                    .process_packet(
                        att::AttReadRequest { attribute_handle: handle1.into() }
                            .try_into()
                            .unwrap(),
                        255
                    )
            ...
            assert_eq!(
                Ok(handler
                    .process_packet(
                        att::AttReadRequest { attribute_handle: handle2.into() }
                            .try_into()
                            .unwrap(),
                        255
                    )
