...
pub async fn handle_read_request<T: AttDatabase>(
    ...
    let Some(attr) = db.get_attribute(handle) else {
...
mod test {
    ...
    use crate::{
        gatt::{
            ids::AttHandle,
            server::{
                att_database::{AttAttribute, AttPermissions, Uuid},
    ...
    fn do_read_request_with_handle_and_mtu(
        ...
        tokio_test::block_on(handle_read_request((&att_view).into(), mtu, db))
    }
    ...
    fn test_simple_read() {
        ...
        assert_eq!(
            ...
            AttChild::AttReadResponse(AttReadResponseBuilder {
                value: build_att_data(AttAttributeDataChild::RawData(vec![4, 5].into()))
            })
        )
    }
