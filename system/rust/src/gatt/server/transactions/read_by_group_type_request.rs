use crate::{
    ...
    gatt::{
        ...
        server::{
            att_database::{
                AttDatabase, PRIMARY_SERVICE_DECLARATION_UUID, SECONDARY_SERVICE_DECLARATION_UUID,
            },
...
use super::{
    att_filter_by_type::{filter_attributes_by_type, AttributeWithValue},
...
pub async fn handle_read_by_group_type_request<T: AttDatabase>(
    ...
    // > ATT_ERROR_RSP PDU
    // > then this attribute cannot be included in an ATT_READ_BY_GROUP_TYPE_RSP PDU
    // > and the
    // > attributes before this attribute shall be returned.
    //
    ...
    match filter_attributes_by_type(db, attrs, group_type, mtu - 6).await {
        Ok(attrs) => {
            for AttributeWithValue { attr, value } in attrs {
                if !matches.push(AttReadByGroupTypeDataElementBuilder {
                    handle: attr.handle.into(),
                    end_group_handle: find_group_end(all_attrs.iter().cloned(), attr)
...
mod test {
    ...
    fn test_attribute_truncation() {
        ...
        let response =
            tokio_test::block_on(handle_read_by_group_type_request((&att_view).into(), 7, &db))
                .unwrap();
    ...
    fn test_limit_total_size() {
        ...
        let response =
            tokio_test::block_on(handle_read_by_group_type_request((&att_view).into(), 9, &db))
                .unwrap();
