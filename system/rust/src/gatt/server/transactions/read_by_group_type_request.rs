...
pub async fn handle_read_by_group_type_request<T: AttDatabase>(
    ...
    for AttAttribute { handle, type_: uuid, permissions } in db.list_attributes() {
        if handle.0 < request.get_starting_handle().get_handle()
            ...
            match db.read_attribute(handle).await {
                ...
                Ok(value) => {
                    ...
                    if let Some(curr_elem) = &curr_elem {
                        // serialization failure will lead to an error packet anyway, so it doesn't
                        // matter what happens here anyway
...
mod test {
    ...
    fn test_simple_grouping() {
        // arrange
        let grouping_uuid = Uuid::new([1, 2, 3, 4]);
