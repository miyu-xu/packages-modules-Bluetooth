use crate::{
    ...
    gatt::{
        ids::AttHandle,
        server::{att_database::AttDatabase, utils::PayloadAccumulator},
    },
    packets::{
...
pub async fn handle_read_by_type_request<T: AttDatabase>(
    ...
    // > ATT_ERROR_RSP PDU
    // > then this attribute cannot be included in an ATT_READ_BY_TYPE_RSP PDU
    // > and the
    ...
    match filter_attributes_by_type(db, attrs, request_type, mtu - 4).await {
        Ok(attrs) => {
...
mod test {
    ...
    fn test_single_matching_attr() {
        ...
        // act
        let response =
            tokio_test::block_on(handle_read_by_type_request((&att_view).into(), 31, &db)).unwrap();
    ...
    fn test_limit_total_size() {
        ...
        let response =
            tokio_test::block_on(handle_read_by_type_request((&att_view).into(), 8, &db)).unwrap();
