...
mod test {
    use crate::{
        gatt::server::{
            gatt_database::{AttPermissions, Uuid},
        ...
        if let AttChild::AttFindInformationResponse(response) = response {
                ...
                response._child_
            {
                ...
                assert_eq!(
                    ...
                    Uuid128Builder::from(Uuid::from([1, 2, 3, 4])).data.to_vec()
                );
                assert_eq!(response.data[1].handle.handle, 4);
                assert_eq!(
                    response.data[1].uuid.data.to_vec(),
                    Uuid128Builder::from(Uuid::from([1, 2, 3, 5])).data.to_vec()
                );
            } else {
                unreachable!()
            }
    ...
    fn test_short_uuids() {
        ...
        if let AttChild::AttFindInformationResponse(response) = response {
                ...
                response._child_
            {
                ...
                assert_eq!(
                    response.data[0].uuid.data.to_vec(),
                    Uuid16Builder::try_from(Uuid::from([1, 2])).unwrap().data.to_vec()
                );
                assert_eq!(response.data[1].handle.handle, 4);
                assert_eq!(
                    response.data[1].uuid.data.to_vec(),
                    Uuid16Builder::try_from(Uuid::from([1, 3])).unwrap().data.to_vec()
                );
            } else {
                unreachable!()
            }
