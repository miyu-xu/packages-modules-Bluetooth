...
use crate::gatt::server::att_database::{
    AttAttribute, CHARACTERISTIC_UUID, PRIMARY_SERVICE_DECLARATION_UUID,
    SECONDARY_SERVICE_DECLARATION_UUID,
};
...
#[cfg(test)]
mod test {
    use crate::gatt::{ids::AttHandle, server::gatt_database::AttPermissions};
    ...
    fn test_characteristic_group_terminated_by_service() {
        ...
        let group_end = find_group_end(attrs.into_iter(), attr(AttHandle(10), CHARACTERISTIC_UUID));
    ...
    fn test_empty_non_terminated_group() {
        let attrs = vec![attr(AttHandle(10), CHARACTERISTIC_UUID)];
    ...
    fn test_non_grouping_attribute() {
        ...
        let group_end = find_group_end(attrs.into_iter(), attr(AttHandle(10), OTHER_UUID));
