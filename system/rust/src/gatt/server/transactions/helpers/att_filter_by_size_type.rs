...
use crate::{
    core::uuid::Uuid,
    gatt::server::att_database::{AttAttribute, AttDatabase, StableAttDatabase},
...
/// size limit.
///
/// Returns an iterator of attributes in the range and matching the
/// type, with the max number of elements such that each attribute has the same
/// size.
///
/// Attributes are truncated to the attr_size limit before size comparison.
pub async fn filter_attributes_by_type(
...
mod test {
    ...
    use crate::{
        ...
        gatt::{
            ids::AttHandle,
            server::{
                att_database::{AttAttribute, StableAttDatabase},
    ...
    fn test_single_matching_attr() {
        ...
        // act
        let response = tokio_test::block_on(filter_attributes_by_type(
            &db,
    ...
    fn test_read_failure_on_subsequent_attr() {
        ...
        // assert: we reply with the first attribute, but not the second
        assert_eq!(
            response.collect::<Vec<_>>(),
            vec![AttributeWithValue {
                attr: db.find_attribute(AttHandle(3)).unwrap(),
