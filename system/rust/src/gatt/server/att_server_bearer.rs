...
impl<T: AttDatabase + Clone + 'static> AttServerBearer<T> {
    ...
    pub fn send_indication(
        ...
        let locked_indication_handler = this.indication_handler.lock();
        let pending_mtu = this.mtu.snapshot();
        ...
        async move {
            ...
            let mut indication_handler = locked_indication_handler
                .await
                .ok_or(IndicationError::SendError(SendError::ConnectionDropped))?;
            // then, if MTU negotation is taking place, wait for it to complete
            let mtu = pending_mtu
                .await
                .ok_or(IndicationError::SendError(SendError::ConnectionDropped))?;
    ...
    pub fn handle_packet(this: &WeakBoxRef<Self>, packet: AttView<'_>) {
        match classify_opcode(packet.get_opcode()) {
            ...
            OperationType::Confirmation => this.pending_confirmation.on_confirmation(),
    ...
    /// operations
    pub fn handle_mtu_event(this: &WeakBoxRef<Self>, mtu_event: MtuEvent) -> Result<()> {
        this.mtu.handle_event(mtu_event)
    }
    ...
    fn handle_request(this: &WeakBoxRef<Self>, packet: AttView<'_>) {
        ...
        this.curr_request.replace(match curr_request {
            AttRequestState::Idle(mut request_handler) => {
                ...
                let mtu = this.mtu.snapshot_or_default();
...
mod test {
    ...
    fn test_indication_confirmation() {
        block_on_locally(async {
            ...
            let pending_send = spawn_local(AttServerBearer::send_indication(
                &conn.as_ref(),
                VALID_HANDLE,
    ...
    fn test_sequential_indications() {
        block_on_locally(async {
            ...
            let pending_send1 = spawn_local(AttServerBearer::send_indication(
                &conn.as_ref(),
                VALID_HANDLE,
                AttAttributeDataChild::RawData([1, 2, 3].into()),
            ));
            ...
            let pending_send2 = spawn_local(AttServerBearer::send_indication(
                &conn.as_ref(),
                VALID_HANDLE,
                AttAttributeDataChild::RawData([1, 2, 3].into()),
            ));
    ...
    fn test_single_indication_pending_mtu() {
        block_on_locally(async {
            // arrange: pending MTU negotation
            let (conn, mut rx) = open_connection();
            AttServerBearer::handle_mtu_event(&conn.as_ref(), MtuEvent::OutgoingRequest).unwrap();

            // act: try to send an indication with a large payload size
            spawn_local(AttServerBearer::send_indication(
                &conn.as_ref(),
                VALID_HANDLE,
                AttAttributeDataChild::RawData((1..50).collect()),
            ));
            // then resolve the MTU negotiation with a large MTU
            AttServerBearer::handle_mtu_event(&conn.as_ref(), MtuEvent::IncomingResponse(100))
                .unwrap();
    ...
    fn test_single_indication_pending_mtu_fail() {
        block_on_locally(async {
            ...
            // act: try to send an indication with a large payload size
            let pending_mtu = spawn_local(AttServerBearer::send_indication(
                &conn.as_ref(),
                VALID_HANDLE,
                AttAttributeDataChild::RawData((1..50).collect()),
            ));
            // then resolve the MTU negotiation with a small MTU
            AttServerBearer::handle_mtu_event(&conn.as_ref(), MtuEvent::IncomingResponse(32))
                .unwrap();

            // assert: the indication failed to send
            assert!(matches!(
                pending_mtu.await.unwrap(),
                Err(IndicationError::DataExceedsMtu { .. })
            ));
        });
    }
    ...
    fn test_queued_indication_pending_mtu_uses_mtu_on_dequeue() {
        block_on_locally(async {
            ...
            let (conn, mut rx) = open_connection();
            let _ = try_await(AttServerBearer::send_indication(
                &conn.as_ref(),
                VALID_HANDLE,
                AttAttributeDataChild::RawData([1, 2, 3].into()),
            ))
            ...
            AttServerBearer::handle_mtu_event(&conn.as_ref(), MtuEvent::OutgoingRequest).unwrap();
            AttServerBearer::handle_mtu_event(&conn.as_ref(), MtuEvent::IncomingResponse(512))
                .unwrap();
