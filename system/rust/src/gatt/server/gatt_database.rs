...
/// A GattDescriptor consists of a handle, type_, and permissions (similar to a GattCharacteristic)
/// It is guaranteed that the handle of the GattDescriptor is after the handle of the characteristic
/// value attribute, and before the next characteristic/service declaration
#[derive(Debug, Clone)]
...
impl<T: GattDatastore + ?Sized> GattDatabase<T> {
                ...
                AttAttributeBackingValue::Static(
                    GattCharacteristicDeclarationValueBuilder {
                        properties: GattCharacteristicPropertiesBuilder {
                            ...
                            read: characteristic
                                .permissions
                                .contains(AttPermissions::READABLE)
                                .into(),
                            write_without_response: 0,
                            write: characteristic
                                .permissions
                                .contains(AttPermissions::WRITABLE)
                                .into(),
                            notify: 0,
    ...
    pub fn get_att_database(this: &SharedBox<Self>, conn_id: ConnectionId) -> AttDatabaseImpl<T> {
        AttDatabaseImpl { gatt_db: this.downgrade(), conn_id }
    }
}
    ...
    T: GattDatastore + ?Sized,
{
    async fn read_attribute(
        ...
        let (value, datastore) = self.gatt_db.with(|gatt_db| {
            ...
            if !attr.attribute.permissions.contains(AttPermissions::READABLE) {
    ...
    async fn write_attribute(
        ...
        let (value, datastore) = self.gatt_db.with(|gatt_db| {
            ...
            if !attr.attribute.permissions.contains(AttPermissions::WRITABLE) {
...
mod test {
    ...
    fn test_read_empty_db() {
        ...
        let att_db = GattDatabase::get_att_database(&gatt_db, CONN_ID);
    ...
    fn test_unreadable_characteristic() {
        ...
        let characteristic_value = tokio_test::block_on(
            GattDatabase::get_att_database(&gatt_db, CONN_ID)
                .read_attribute(CHARACTERISTIC_VALUE_HANDLE),
        );
    ...
    fn test_multiple_descriptors() {
        ...
        let attributes = GattDatabase::get_att_database(&gatt_db, CONN_ID).list_attributes();
