...
impl WeakAttClient {
    ...
    pub fn list_attributes(&self) -> Vec<AttAttribute> {
        self.with(
            |client| if let Some(client) = client { client.list_attributes() } else { Vec::new() },
        )
    }
    ...
    /// Queues a write for later execution.  As per the specification, the offset and data length
    /// are not validated until execution time.
