...
/// The AttTransaction ensures that only one transaction can take place at a time
pub struct AttServerBearer<T: AttDatabase> {
    curr_operation: RefCell<AttTransaction<T>>,
...
mod test {
    ...
    fn open_connection(
    ) -> (Rc<AttServerBearer<TestAttDatabase>>, UnboundedReceiver<AttBuilder>) {
