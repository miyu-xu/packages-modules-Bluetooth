...
pub enum MockDatastoreEvents {
    ...
    /// A characteristic was read on a given handle. The oneshot is used to return the value read.
    ReadCharacteristic(
        ConnectionId,
        AttHandle,
        oneshot::Sender<Result<AttAttributeDataChild, AttErrorCode>>,
    ),
    /// A characteristic was written to on a given handle. The oneshot is used to return whether the write suceeded.
    WriteCharacteristic(
        ConnectionId,
        AttHandle,
