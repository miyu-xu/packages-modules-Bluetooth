...
pub struct AttMtu {
    /// The MTU we have committed to (i.e. sent a REQ and got a RESP)
    previous_mtu: Cell<usize>,
    /// The MTU we have commited or are about to commit to (if a REQ is pending)
    stable_mtu: SharedMutex<usize>,
    /// Lock guard held if we are currrently performing MTU negotation
...
mod test {
    ...
    #[test]
    fn test_guaranteed_mtu_during_client_negotation() {
        // arrange
        let mtu = AttMtu::new();
    ...
    #[test]
    fn test_mtu_blocking_snapshot_during_client_negotation() {
