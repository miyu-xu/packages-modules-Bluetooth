...
mod test {
    ...
    use crate::{packets::{AttBuilder, AttReadRequestBuilder, Serializable}, gatt::ids::AttHandle};
    ...
    fn test_advertiser_id_reuse() {
        ...
        // but we should not be isolated since this is a new advertiser reusing the old ID
        assert!(conn_id.is_none())
    }

    #[test]
    fn test_server_id_reuse() {
        let mut arbiter = Arbiter::new();
