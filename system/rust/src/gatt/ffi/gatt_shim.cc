    ...
    case AttributeBackingType::CHARACTERISTIC:
      do_in_jni_thread(
          FROM_HERE, base::BindRepeating(
                         callbacks.request_read_characteristic_cb, conn_id,
                         trans_id, addr.value(), attr_handle, offset, is_long));
      break;
    case AttributeBackingType::DESCRIPTOR:
      do_in_jni_thread(FROM_HERE,
                       base::BindRepeating(callbacks.request_read_descriptor_cb,
...
void GattServerCallbacks::OnServerWrite(
  ...
  switch (attr_type) {
    case AttributeBackingType::CHARACTERISTIC:
      do_in_jni_thread(
          FROM_HERE,
          base::BindRepeating(callbacks.request_write_characteristic_cb,
                              conn_id, trans_id, addr.value(), attr_handle,
                              offset, need_response, is_prepare,
                              base::Owned(buf), value.size()));
      break;
    case AttributeBackingType::DESCRIPTOR:
      do_in_jni_thread(
          FROM_HERE,
          base::BindRepeating(callbacks.request_write_descriptor_cb, conn_id,
...
void GattServerCallbacks::OnIndicationSentConfirmation(uint16_t conn_id,
                                                       int status) const {
  do_in_jni_thread(FROM_HERE, base::BindRepeating(callbacks.indication_sent_cb,
                                                  conn_id, status));
}
...
void GattServerCallbacks::OnExecute(uint16_t conn_id, uint32_t trans_id,
  ...
  do_in_jni_thread(FROM_HERE,
                   base::BindRepeating(callbacks.request_exec_write_cb, conn_id,
