...
fn records_to_service(service_records: &[GattRecord<'_>]) -> Result<GattServiceWithHandle> {
    ...
    Ok(GattServiceWithHandle { handle: AttHandle(handle), type_: uuid.into(), characteristics })
}
