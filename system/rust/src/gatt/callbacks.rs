...
pub enum GattWriteType {
    /// Reliable, expects a response (WRITE_REQ)
    Request,
    /// Transactional, expects a response but should not be committed yet (PREPARE_WRITE_REQ)
    Prepare { offset: u32 },
...
impl GattWriteType {
    /// Whether we need a response from the upper layer
    pub fn needs_response(&self) -> bool {
        match self {
            GattWriteType::Request => true,
            GattWriteType::Prepare { .. } => true,
            GattWriteType::Command => false,
        }
    }
}
...
impl From<GattWriteRequestType> for GattWriteType {
    fn from(value: GattWriteRequestType) -> Self {
        match value {
            GattWriteRequestType::Request => GattWriteType::Request,
            GattWriteRequestType::Prepare { offset } => GattWriteType::Prepare { offset },
        }
    }
}
