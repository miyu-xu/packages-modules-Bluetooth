//! These are the interfaces between the GattModule and JNI. The synchronous interface is mapped to the
//! asynchronous interface using the CallbackTransactionManager;
...
pub trait GattCallbacks {
    ...
    /// Invoked when a client tries to write a characteristic. Expects a response using bluetooth::gatt::send_response();
...
/// This interface is an "async" version of the above, and is passed directly into the GattModule
#[async_trait(?Send)]
