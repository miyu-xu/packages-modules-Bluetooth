...
impl RfcommTracker {
    ...
    pub fn log_rfcomm_client_connection_attempt_start(
        ...
        info!("logging RFCOMM connection attempt start {attempt_id:?} from {app_uid:?}");
        self.sockets.insert(
            attempt_id,
            RfcommConnectionAttempt { peer_addr, is_secured, app_uid, start_time: Instant::now() },
        );
    }
    ...
    pub fn log_rfcomm_client_connection_complete(&mut self, attempt_id: AttemptId, success: bool) {
        let Some(attempt) = self.sockets.get(&attempt_id).copied() else {
