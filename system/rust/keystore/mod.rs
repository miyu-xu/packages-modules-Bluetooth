...
use crate::{
    ...
    MainThreadTx, MAIN_THREAD_TX, utils::DynamicObjectProxy,
};

const ENCRYPTED_STRING: &str = "encrypted";

#[allow(missing_docs)]
pub struct KeystoreModule<'modules, 'jni> {
    jni_module: DynamicObjectProxy<'modules, 'jni, &'jni (dyn KeystoreJniCallbacks + 'jni)>,
    storage_module: &'modules crate::storage::StorageModule,
    parameter_provider_module: &'modules crate::parameter_provider::ParameterProvider,
...
impl<'modules, 'jni> KeystoreModule<'modules, 'jni>
where
    'jni: 'modules,
{
    /// Constructor. Depends on `jni_module` to proxy reads/writes into Java. Depends on `storage_module` to (a) inject
    /// the KeystoreInterfaceImpl itself into the callback interface, and (b) to read/write data on startup if we need
    /// to encrypt/decrypt the persisted data on disk.
    pub fn new(
        jni_module: DynamicObjectProxy<'modules, 'jni, &'jni (dyn KeystoreJniCallbacks + 'jni)>,
        storage_module: &'modules crate::storage::StorageModule,
        parameter_provider_module: &'modules crate::parameter_provider::ParameterProvider,
        ...
        // do this before providing the keystore interface, so writes go directly to disk and are not proxied by us
        out.fix_storage_layer();
        // then, inject the proxy so future writes go through us if needed
    ...
    fn store_key(&self, prefix: &str, decrypted: &str) {
        ...
        let decrypted = decrypted.to_owned();
        self.jni_module.with(move |keystore| {
            keystore.set_encrypt_key_or_remove_key_callback(&prefix, &decrypted)
        });
    }

    fn get_key(&self, prefix: &str) -> String {
        self.key_cache.borrow().get(prefix).cloned().unwrap_or_default()
    }
    ...
    pub fn fix_storage_layer(&self) {
        let mut mutation_ptr = self.storage_module.modify();
