...
where
    'b: 'a,
{
    /// Proxies and caches get/set calls from the Storage module into the BluetoothKeystoreService
    pub keystore_module: &'a keystore::KeystoreModule<'a, 'b>,
}
...
pub struct JniCallbacks<'a> {
    /// Callbacks out of the KeystoreModule
    pub keystore_jni: &'a (dyn keystore::KeystoreJniCallbacks + 'a),
}
...
impl GlobalModuleRegistry {
    ...
    pub fn start<'a, 'b, T: ObjectProxy<'a, T = JniCallbacks<'a>>>(jni_callbacks: &'b T)
    where
        'a: 'b,
    {
        ...
        // Next, prepare the JNI proxies
        let jni_module = jni_callbacks.specialize(|modules| &mut modules.keystore_jni);

        // Finally we have the pure-Rust modules
        let keystore_module = keystore::KeystoreModule::new(
            jni_module.type_erase(),
            &storage_module,
...
thread_local! {
    ...
    /// JNI / direct FFI should use do_on_rust_thread for convenience, but objects passed into C++ as callbacks should
