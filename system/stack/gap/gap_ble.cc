...
void gap_attr_db_init(void) {
  ...
  if (IS_FLAG_ENABLED(encrypted_advertising_data)) {
    ...
    gatt_attr[3].attr_value.conn_param.int_max =
        GAP_PREFER_CONN_INT_MAX; /* 6 */
    gatt_attr[3].attr_value.conn_param.int_min =
        GAP_PREFER_CONN_INT_MIN; /* 0 */
    gatt_attr[3].attr_value.conn_param.latency =
        GAP_PREFER_CONN_LATENCY; /* 0 */
...
void gap_chk_encr_data(RawAddress bda, uint16_t conn_id, tGAP_ATTR db_attr) {
  ...
  if (conn_id == GATT_INVALID_CONN_ID) {
    LOG_ERROR("Unable to find conn_id for %s", ADDRESS_TO_LOGGABLE_CSTR(bda));
    return;
  }
