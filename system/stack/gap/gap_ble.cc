...
bool send_cl_disc_request(tGAP_CLCB& clcb) {
  ...
  (void)GATTC_Discover(clcb.conn_id, static_cast<tGATT_DISC_TYPE>(disc_type), s_handle, e_handle,
                       char_uuid);
  return true;
}
...
bool send_cl_write_request(tGAP_CLCB& clcb) {
  ...
  (void)GATTC_Write(clcb.conn_id, GATT_WRITE, &ccc_value);
  return true;
}
  ...
  if (clcb.connected) {
    if (com::android::bluetooth::flags::encrypted_advertising_data()) {
      if (!send_cl_request(clcb) && (clcb.enc_key_stage <= GAP_ENC_KEY_CONNECTING)) {
        ...
        (void)GATT_Disconnect(clcb.conn_id);
        clcb_dealloc(clcb);
      }
    } else {
...
void client_disc_cmpl_cback(uint16_t conn_id, tGATT_DISC_TYPE disc_type, tGATT_STATUS status) {
  ...
  log::debug("status={}, enc_key_stage={}, disc_type={}", status, p_clcb->enc_key_stage, disc_type);
...
void gap_attr_db_init(void) {
  ...
  if (com::android::bluetooth::flags::encrypted_advertising_data()) {
    ...
    /* Add a GAP service */
    (void)GATTS_AddService(gatt_if, service, sizeof(service) / sizeof(btgatt_db_element_t));
...
void gap_chk_encr_data(RawAddress bda, uint16_t conn_id, tGAP_ATTR db_attr) {
  ...
  if (conn_id == GATT_INVALID_CONN_ID) {
    log::error("Unable to find conn_id for {}", bda);
    return;
  }

  uint8_t encr_material[ENC_KEY_MATERIAL_LEN];
  memcpy(encr_material, &db_attr.attr_value.enc_key_material, ENC_KEY_MATERIAL_LEN);
  (void)GATTS_HandleValueIndication(conn_id, db_attr.handle, sizeof(encr_material), encr_material);
}
        ...
        case GATT_UUID_GAP_ENC_KEY_MATERIAL:
          if (com::android::bluetooth::flags::encrypted_advertising_data()) {
            if (std::memcmp((const void*)&db_attr.attr_value.enc_key_material,
              ...
              for (auto& cb : gap_clcbs) {
                if (cb.connected) {
                  log::debug(" BDA: {} conn_id: {}", cb.bda, cb.conn_id);
...
void gap_ble_config_cccd_enc_key_cmpl(UNUSED_ATTR bool status, const RawAddress& bda,
                                      UNUSED_ATTR uint16_t length, UNUSED_ATTR char* p_name) {
  tGAP_CLCB* p_clcb = find_clcb_by_bd_addr(bda);

  if (p_clcb == NULL) {
    log::debug("p_clcb is NULL for {}", bda);
    return;
  }
  ...
  it = std::find(p_clcb->enc_key_char_handles.begin(), p_clcb->enc_key_char_handles.end(),
...
void btm_ble_read_enc_key_cmpl(bool status, const RawAddress& bda, uint16_t length, char* p_data) {
  ...
  if (len <= 0) {
    ...
    uint8_t* concatEncKeyMaterial = (uint8_t*)osi_malloc((length + len) * sizeof(uint8_t));
    if (btif_storage_get_enc_key_material(&bda, concatEncKeyMaterial, &len) == BT_STATUS_SUCCESS) {
      if (length > 0) {
        memcpy(concatEncKeyMaterial + len, p_data, length);
        btif_storage_remove_enc_key_material(&bda);
        btif_storage_set_enc_key_material(bda, concatEncKeyMaterial, length + len);
      }
    }
    osi_free(concatEncKeyMaterial);
  }
