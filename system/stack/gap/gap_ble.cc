...
void client_disc_cmpl_cback(uint16_t conn_id, tGATT_DISC_TYPE disc_type,
  ...
  LOG_DEBUG("enc_key_stage: %d", +p_clcb->enc_key_stage);
...
void btm_ble_read_enc_key_cmpl(bool status, const RawAddress& bda,
  ...
  size_t len =
      btif_config_get_bin_length(bda.ToString().c_str(), "ENC_KEY_MATERIAL");
  if (len > 0) {
    if (btif_storage_get_enc_key_material((RawAddress*)&bda,
                                          (uint8_t*)&prev_data[0],
                                          (int*)&len) == BT_STATUS_SUCCESS) {
      if (length > 0) {
        ...
        btif_storage_set_enc_key_material((RawAddress*)&bda, concat_arr,
                                          (length + len));
      }
    }
  } else {
    ...
    if (p_data) {
      btif_storage_set_enc_key_material((RawAddress*)&bda, (uint8_t*)p_data,
                                        length);
    }
  }

  /*Dont increment p_clcb->enc_key_stage as we want to be in Read enc key
