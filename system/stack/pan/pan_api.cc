...
tPAN_RESULT PAN_SetRole(uint8_t role, std::string p_user_name,
  ...
  if (pan_cb.role == role) {
    LOG_INFO("PAN role already was set to: %d", role);
    return PAN_SUCCESS;
  }

  /* Register all the roles with SDP */
  LOG_INFO("PAN_SetRole() called with role 0x%x", role);
  ...
  pan_cb.role = role;
  LOG_INFO("PAN role set to: %d", role);
  ...
  *handle = BNEP_INVALID_HANDLE;
  ...
  if (src_role == PAN_ROLE_CLIENT) {
    if ((pan_cb.num_conns > 1) || (pan_cb.num_conns && (!pcb))) {
      ...
      */
      LOG_ERROR("Cannot make PANU connections when there are more than one "
                "connection");
  ...
  LOG_DEBUG("PAN_Connect() current active role set to %d", src_role);
...
tPAN_RESULT PAN_Disconnect(uint16_t handle) {
  ...
  if (result != BNEP_SUCCESS) {
    LOG_INFO("Error in closing PAN connection");
    return PAN_FAILURE;
  }

  LOG_INFO("PAN connection closed");
  return PAN_SUCCESS;
}
...
tPAN_RESULT PAN_WriteBuf(uint16_t handle, const RawAddress& dst,
  ...
  if (pan_cb.active_role == PAN_ROLE_CLIENT) {
    ...
    if (result == BNEP_IGNORE_CMD) {
      LOG_DEBUG("PAN ignored data write for PANU connection");
    ...
    LOG_DEBUG("PAN successfully wrote data for the PANU connection");
    return PAN_SUCCESS;
  }
  ...
  if (result == BNEP_IGNORE_CMD) {
    LOG_DEBUG("PAN ignored data buf write to PANU");
    pcb->write.errors++;
  ...
  LOG_DEBUG("PAN successfully sent data buf to the PANU");
...
tPAN_RESULT PAN_SetProtocolFilters(uint16_t handle, uint16_t num_filters,
  ...
  LOG_INFO("PAN successfully sent protocol filters for handle %d", handle);
  return PAN_SUCCESS;
}
...
tPAN_RESULT PAN_SetMulticastFilters(uint16_t handle, uint16_t num_mcast_filters,
  ...
  LOG_INFO("PAN successfully sent multicast filters for handle %d", handle);
  return PAN_SUCCESS;
}
