...
tPAN_RESULT PAN_SetRole(uint8_t role, std::string p_user_name,
  ...
  if (pan_cb.role == role) {
    LOG_DEBUG("PAN role already was set to: %d", role);
    return PAN_SUCCESS;
  }

  /* Register all the roles with SDP */
  LOG_DEBUG("PAN_SetRole() called with role 0x%x", role);
  ...
  pan_cb.role = role;
  LOG_DEBUG("PAN role set to: %d", role);
...
tPAN_RESULT PAN_Disconnect(uint16_t handle) {
  ...
  if (result != BNEP_SUCCESS) {
    LOG_DEBUG("Error in closing PAN connection");
    return PAN_FAILURE;
  }

  LOG_DEBUG("PAN connection closed");
  return PAN_SUCCESS;
}
...
tPAN_RESULT PAN_SetProtocolFilters(uint16_t handle, uint16_t num_filters,
  ...
  LOG_DEBUG("PAN successfully sent protocol filters for handle %d", handle);
  return PAN_SUCCESS;
}
...
tPAN_RESULT PAN_SetMulticastFilters(uint16_t handle, uint16_t num_mcast_filters,
  ...
  LOG_DEBUG("PAN successfully sent multicast filters for handle %d", handle);
  return PAN_SUCCESS;
}
