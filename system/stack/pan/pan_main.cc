...
void pan_conn_ind_cb(uint16_t handle, const RawAddress& p_bda,
  ...
  if (!is_valid_interaction) {
    LOG_ERROR("PAN Connection failed because of invalid PAN profile roles "
  ...
  */
  ...
  if (pcb) {
    ...
    if (pcb->con_state == PAN_STATE_CONNECTED) {
      LOG_INFO("PAN Role changing New Src 0x%x Dst 0x%x", remote_uuid16,
               local_uuid16);
  ...
  LOG_DEBUG("New connection indication for handle %d", handle);
  ...
  LOG_INFO("PAN connection destination UUID is 0x%x", local_uuid16);
  /* Set the latest active PAN role */
  pan_cb.active_role = req_role;
  pcb->src_uuid = local_uuid16;
  pcb->dst_uuid = remote_uuid16;
...
void pan_connect_state_cb(uint16_t handle,
  ...
  LOG_INFO("pan_connect_state_cb - for handle %d, result %d", handle, result);
  ...
  if (result != BNEP_SUCCESS) {
    ...
    if (pcb->con_state != PAN_STATE_CONNECTED &&
      ...
      LOG_INFO("restoring the connection state to active");
  ...
  if (pan_cb.pan_bridge_req_cb && pcb->src_uuid == UUID_SERVCLASS_NAP) {
    LOG_INFO("PAN requesting for bridge");
    (*pan_cb.pan_bridge_req_cb)(pcb->rem_bda, true);
  }
}
...
void pan_data_buf_ind_cb(uint16_t handle, const RawAddress& src,
  ...
  if (pcb->src_uuid != UUID_SERVCLASS_PANU) {
    if (dst.address[0] & 0x01) {
      LOG_DEBUG("PAN received broadcast packet on handle %d, src uuid 0x%x",
                handle, pcb->src_uuid);
