  ...
  */
  ...
  if (pcb) {
    ...
    if (pcb->con_state == PAN_STATE_CONNECTED) {
      LOG_DEBUG("PAN Role changing New Src 0x%x Dst 0x%x", remote_uuid16,
                local_uuid16);
  ...
  LOG_DEBUG("PAN connection destination UUID is 0x%x", local_uuid16);
  /* Set the latest active PAN role */
  pan_cb.active_role = req_role;
  pcb->src_uuid = local_uuid16;
  pcb->dst_uuid = remote_uuid16;
...
void pan_connect_state_cb(uint16_t handle,
  ...
  LOG_DEBUG("pan_connect_state_cb - for handle %d, result %d", handle, result);
  ...
  if (result != BNEP_SUCCESS) {
    ...
    if (pcb->con_state != PAN_STATE_CONNECTED &&
      ...
      LOG_DEBUG("restoring the connection state to active");
  ...
  if (pan_cb.pan_bridge_req_cb && pcb->src_uuid == UUID_SERVCLASS_NAP) {
    LOG_DEBUG("PAN requesting for bridge");
    (*pan_cb.pan_bridge_req_cb)(pcb->rem_bda, true);
  }
}
