...
bool gatt_check_if_anybody_interested_in_connection(const RawAddress& bda) {
  if (size_t i = 1; i < GATT_MAX_APPS; i++) {
    tGATT_REG* p_reg = &gatt_cb.cl_rcb[i - 1];
    if (p_reg->direct_connect_request.count(bda) > 0) {
      log::debug("gatt_if {} interested in connection to {}", i, bd_addr, transport);
      return true;
    }
  }
  return false;
...
bool gatt_cancel_open(tGATT_IF gatt_if, const RawAddress& bda) {
  ...
  if (!p_tcb) {
    if (com::android::bluetooth::flags::gatt_reconnect_on_bt_on_fix()) {
      ...
      if (!gatt_check_if_anybody_interested_in_connection(bda)) {
