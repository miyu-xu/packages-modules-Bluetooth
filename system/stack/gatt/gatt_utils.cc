  ...
  if (!p_tcb) {
    if (IS_FLAG_ENABLED(gatt_reconnect_on_bt_on_fix)) {
      ...
      if (!p_reg) {
        ...
        p_reg->direct_connect_request.erase(
            std::remove(p_reg->direct_connect_request.begin(),
                        p_reg->direct_connect_request.end(), bda),
            p_reg->direct_connect_request.end());
      }
      return true;
    }
  ...
  for (uint8_t i = 0; i < GATT_MAX_APPS; i++) {
    ...
    if (IS_FLAG_ENABLED(gatt_reconnect_on_bt_on_fix)) {
      ...
      if (iter != p_reg->direct_connect_request.end()) {
        ...
        p_reg->direct_connect_request.erase(iter);
      }
    }
  }
}
