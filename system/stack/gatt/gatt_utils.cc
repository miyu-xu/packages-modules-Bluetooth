...
tGATT_TCB* gatt_allocate_tcb_by_bdaddr(const RawAddress& bda,
  ...
  for (int i = 0; i < GATT_MAX_PHY_CHANNEL; i++) {
    ...
    *p_tcb = tGATT_TCB();
    ...
    p_tcb->peer_bda = bda;
    p_tcb->eatt = 0;
    p_tcb->pending_mtu_exchange_flag = false;
...
void gatt_set_pending_mtu_exchange_flag(const RawAddress& bda) {
  /* This function shall be called only for BLE */
  ...
  p_tcb->pending_mtu_exchange_flag = true;
}
...
bool gatt_is_pending_mtu_exchange(const RawAddress& bda,
  ...
  return p_tcb->pending_mtu_exchange_flag;
}
