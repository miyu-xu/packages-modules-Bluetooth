...
inline std::string gatt_security_action_text(const tGATT_SEC_ACTION& action) {
  switch (action) {
    ...
    default:
      return std::format("UNKNOWN[{}]", action);
  }
}
...
struct tTCB_STATE_HISTORY {
  ...
  std::string ToString() const {
    return std::format("{}, {}, state: {}, {}", ADDRESS_TO_LOGGABLE_CSTR(address), bt_transport_text(transport), gatt_channel_state_text(state), holders_info);
  }
};
