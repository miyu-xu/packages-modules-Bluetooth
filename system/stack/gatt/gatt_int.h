...
#if (GATT_CONFORMANCE_TESTING == TRUE)
void gatt_set_err_rsp(bool enable, uint8_t req_op_code,
                             uint8_t err_status);
#endif
...
tGATT_STATUS attp_send_sr_msg(tGATT_TCB& tcb, uint16_t cid,
                                     BT_HDR* p_msg);
...
uint32_t gatt_add_sdp_record(const bluetooth::Uuid& uuid,
...
uint8_t gatt_build_uuid_to_stream(uint8_t** p_dst,
                                         const bluetooth::Uuid& uuid);
void gatt_sr_get_sec_info(const RawAddress& rem_bda,
                                 tBT_TRANSPORT transport,
...
tGATT_STATUS gatt_send_error_rsp(tGATT_TCB& tcb, uint16_t cid,
                                        uint8_t err_code, uint8_t op_code,
                                        uint16_t handle, bool deq);

bool gatt_is_srv_chg_ind_pending(tGATT_TCB* p_tcb);
tGATTS_SRV_CHG* gatt_is_bda_in_the_srv_chg_clt_list(
...
bool gatt_auto_connect_dev_remove(tGATT_IF gatt_if,
...
uint32_t gatt_sr_enqueue_cmd(tGATT_TCB& tcb, uint16_t cid,
...
bool gatt_tcb_get_cid_available_for_indication(
...
tGATT_SR_CMD* gatt_sr_get_cmd_by_trans_id(tGATT_TCB* p_tcb,
                                                 uint32_t trans_id);
...
void gatt_sr_update_cback_cnt(tGATT_TCB& p_tcb, uint16_t cid,
                                     tGATT_IF gatt_if, bool is_inc,
                                     bool is_reset_first);
void gatt_sr_update_prep_cnt(tGATT_TCB& tcb, tGATT_IF gatt_if,
...
tGATT_TCB* gatt_find_tcb_by_addr(const RawAddress& bda,
                                        tBT_TRANSPORT transport);
bool gatt_send_ble_burst_data(const RawAddress& remote_bda,
                                     BT_HDR* p_buf);
...
tGATT_STATUS gatt_send_write_msg(tGATT_TCB& p_tcb, tGATT_CLCB* p_clcb,
                                        ...
                                        uint8_t* p_data);
void gatt_cleanup_upon_disc(const RawAddress& bda,
                                   tGATT_DISCONN_REASON reason,
                                   tBT_TRANSPORT transport);
void gatt_end_operation(tGATT_CLCB* p_clcb, tGATT_STATUS status,
                               void* p_data);

void gatt_act_discovery(tGATT_CLCB* p_clcb);
void gatt_act_read(tGATT_CLCB* p_clcb, uint16_t offset);
void gatt_act_write(tGATT_CLCB* p_clcb, uint8_t sec_act);
tGATT_CLCB* gatt_cmd_dequeue(tGATT_TCB& tcb, uint16_t cid,
                                    uint8_t* p_opcode);
...
/* gatt_db.cc */
void gatts_init_service_db(tGATT_SVC_DB& db,
...
tGATT_STATUS gatts_write_attr_perm_check(
