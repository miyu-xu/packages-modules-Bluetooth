...
tGATT_STATUS GATTS_AddService(tGATT_IF gatt_if, btgatt_db_element_t* service,
  ...
  log::verbose(
      "handles needed={}, s_hdl={:x}, e_hdl={:x}, uuid={}, is_primary={}",
  ...
  for (int i = 0; i < count - 1; i++, el++) {
    ...
    if (el->type == BTGATT_DB_CHARACTERISTIC) {
      ...
      if (((el->properties & GATT_CHAR_PROP_BIT_AUTH) &&
        ...
        log::verbose("Invalid configuration property={:x}, perm={:x}",
  ...
  log::verbose("allocated el s_hdl={:x}, e_hdl={:x}, type={:x}, sdp_hdl={:x}",
...
bool GATTS_DeleteService(tGATT_IF gatt_if, Uuid* p_svc_uuid,
  ...
  log::verbose("released handles s_hdl={:x}, e_hdl={:x}",
...
void GATTS_StopService(uint16_t service_handle) {
  log::info("service = {:x}", service_handle);

  auto it = gatt_sr_find_i_rcb_by_handle(service_handle);
  if (it == gatt_cb.srv_list_info->end()) {
    log::error("service_handle={:x} is not in use", service_handle);
    return;
  }
...
tGATT_STATUS GATTS_SendRsp(uint16_t conn_id, uint32_t trans_id,
  ...
  log::verbose("conn_id={:x}, trans_id={:x}, status={:x}", conn_id, trans_id,
  ...
  if (!sr_res_p) {
    log::error("conn_id={:x} waiting for other op_code", conn_id);
...
tGATT_STATUS GATTC_Discover(uint16_t conn_id, tGATT_DISC_TYPE disc_type,
  ...
  if ((p_tcb == NULL) || (p_reg == NULL) || (disc_type >= GATT_DISC_MAX)) {
    log::error("Illegal param: disc_type={} conn_id={:x}", disc_type, conn_id);
    return GATT_ILLEGAL_PARAMETER;
  }
  ...
  if (!GATT_HANDLE_IS_VALID(start_handle) ||
    ...
    log::warn(
        "Illegal parameter conn_id={:x}, disc_type={}, s_handle={:x}, "
        "e_handle={:x}",
  ...
  if (!p_clcb) {
    log::warn(
        "No resources conn_id={:x}, disc_type={}, s_handle={:x}, e_handle={:x}",
        conn_id, disc_type, start_handle, end_handle);
    return GATT_NO_RESOURCES;
  }
  ...
  log::info("conn_id={:x}, disc_type={}, s_handle={:x}, e_handle={:x}", conn_id,
            disc_type, start_handle, end_handle);
...
tGATT_STATUS GATTC_Read(uint16_t conn_id, tGATT_READ_TYPE type,
  ...
  log::verbose("conn_id={:x}, type={:x}", conn_id, type);

  if ((p_tcb == NULL) || (p_reg == NULL) || (p_read == NULL) ||
      ((type >= GATT_READ_MAX) || (type == 0))) {
    log::error("illegal param: conn_id={:x}, type={:x}", conn_id, type);
    return GATT_ILLEGAL_PARAMETER;
  }
...
tGATT_STATUS GATTC_ExecuteWrite(uint16_t conn_id, bool is_execute) {
  ...
  log::verbose("conn_id={:x}, is_execute={}", conn_id, is_execute);

  if ((p_tcb == NULL) || (p_reg == NULL)) {
    log::error("Illegal param: conn_id={:x}", conn_id);
    return GATT_ILLEGAL_PARAMETER;
  }
