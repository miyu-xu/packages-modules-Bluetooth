...
bool IsTargetedAnnoucement(const uint8_t* p_eir, uint16_t eir_len) {
  ...
  while ((p_service_data = AdvertiseDataParser::GetFieldByType(
    ...
    STREAM_TO_UINT8(annoucement_type, p_tmp);
    LOG_DEBUG("Found annoucement_type 0x%02x", annoucement_type);
    if (annoucement_type == 0x01) {
      return true;
    }
  }
  return false;
...
static void target_annoucement_observe_results_cb(tBTM_INQ_RESULTS* p_inq,
                                                  const uint8_t* p_eir,
                                                  uint16_t eir_len) {
  ...
  if (!IsTargetedAnnoucement(p_eir, eir_len)) {
    LOG_DEBUG("Not a targeted annoucement for device %s",
              ADDRESS_TO_LOGGABLE_CSTR(addr));
    return;
  }

  LOG_INFO("Found targeted annoucement for device %s", addr.ToString().c_str());
  ...
  auto app_id = *(it->second.doing_targeted_announcements.begin());
...
void target_annoucements_filtering_set(bool enable) {
  LOG_DEBUG("enable %d", enable);
  /* Safe to call as if there is no support for filtering, this call will be
   * ignored. */
  bluetooth::shim::set_target_annoucements_filter(enable);
  BTM_BleTargetAnnoucementObserve(enable,
                                  target_annoucement_observe_results_cb);
}

/** Add a device to the background connection list for cap announcements.
 * Returns
...
bool background_connect_targeted_announcement_add(tAPP_ID app_id,
  ...
  if (num_of_targeted_announcements_users() == 1) {
    target_annoucements_filtering_set(true);
  }
...
bool background_connect_remove(uint8_t app_id, const RawAddress& address) {
  ...
  auto num_of_targeted_annoucements_before_remove =
      it->second.doing_targeted_announcements.size();
  ...
  if ((num_of_targeted_annoucements_before_remove > 0) &&
...
void reset(bool after_reset) {
  bgconn_dev.clear();
  if (!after_reset) {
    target_annoucements_filtering_set(false);
    BTM_AcceptlistClear();
  }
}
...
bool direct_connect_remove(uint8_t app_id, const RawAddress& address) {
  ...
  /* Let see if the device was connected due to Target Annoucements.*/
