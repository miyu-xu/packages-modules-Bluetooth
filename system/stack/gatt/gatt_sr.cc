...
static void gatts_process_mtu_req(tGATT_TCB& tcb, uint16_t cid, uint16_t len,
  ...
  if (mtu < GATT_DEF_BLE_MTU_SIZE) {
    ...
    gatt_sr_msg.mtu = GATT_DEF_BLE_MTU_SIZE;
  } else if (mtu > GATT_MAX_MTU_SIZE) {
    tcb.payload_size = GATT_MAX_MTU_SIZE;
    gatt_sr_msg.mtu = GATT_MAX_MTU_SIZE;
  } else if (tcb.pending_mtu_exchange_value != 0) {
    tcb.payload_size = std::min(mtu, tcb.pending_mtu_exchange_value);
    gatt_sr_msg.mtu = tcb.pending_mtu_exchange_value;
  } else {
    tcb.payload_size = std::min(mtu, (uint16_t)(GATT_MAX_MTU_SIZE));
    gatt_sr_msg.mtu = GATT_MAX_MTU_SIZE;
  }

  LOG_INFO("MTU request from %s with their MTU %d, resulted MTU is %d",
           tcb.peer_bda.ToString().c_str(), mtu, tcb.payload_size);
