...
static void gatt_send_conn_cback(tGATT_TCB* p_tcb) {
  ...
  if (!p_tcb->app_hold_link.empty() && p_tcb->att_lcid == L2CAP_ATT_CID) {
    ...
    if (bluetooth::common::init_flags::finite_att_timeout_is_enabled()) {
      // otherwise, release the idle timeout. Note that we held the timeout
      // initially to allow service discovery to complete. Note also that the
      // L2CAP idle timeout still exists (and dominates this one, for now), this
      // just releases the timeout at the ATT fixed channel.
      GATT_SetIdleTimeout(p_tcb->peer_bda, GATT_LINK_IDLE_TIMEOUT_WHEN_NO_APP,
                          p_tcb->transport);
    }
  }
