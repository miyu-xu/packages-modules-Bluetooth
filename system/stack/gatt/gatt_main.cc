...
void gatt_send_srv_chg_ind(const RawAddress& peer_bda) {
  static uint16_t sGATT_DEFAULT_START_HANDLE = (uint16_t)osi_property_get_int32(
      "bluetooth.gatt.default_start_handle.value", GATT_GATT_START_HANDLE);
  static uint16_t sGATT_LAST_HANDLE = (uint16_t)osi_property_get_int32(
      "bluetooth.gatt.last_handle.value", 0xFFFF);
