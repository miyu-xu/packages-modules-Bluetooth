...
bool direct_connect_add(uint8_t app_id, const RawAddress& address, tBLE_ADDR_TYPE addr_type) {
  ...
  if (it != bgconn_dev.end()) {
    ...
    if (info.doing_direct_conn.count(app_id)) {
      ...
      bluetooth::os::LogMetricBluetoothEvent(bluetooth::ToGdAddress(address), android::bluetooth::EventType::LE_CONNECTION_REJECTED, android::bluetooth::State::ATTEMPT_IN_PROGRESS);
      return false;
    }
