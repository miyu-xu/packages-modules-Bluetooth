  ...
  if (!in_acceptlist) {
    ...
    if (is_targeted_announcement_enabled) {
      ...
      if (accept_list_is_full()) {
        log::warn("accept list is full, can't add {}", address);
        return false;
      }
...
bool direct_connect_add(uint8_t app_id, const RawAddress& address, tBLE_ADDR_TYPE addr_type) {
  ...
  if (!in_acceptlist) {
    if (accept_list_is_full()) {
      log::warn("accept list is full, can't add {}", address_with_type);
      return false;
    }

    if (!bluetooth::shim::ACL_AcceptLeConnectionFrom(address_with_type, true)) {
    ...
    // connection. There is no API for that yet.
    bluetooth::shim::ACL_AcceptLeConnectionFrom(address_with_type, true);
  }
