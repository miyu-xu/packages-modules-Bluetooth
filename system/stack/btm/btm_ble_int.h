...
void btm_ble_process_periodic_adv_sync_est_evt(uint8_t len,
                                                      const uint8_t* p);
...
tBTM_STATUS btm_ble_start_encrypt(const RawAddress& bda, bool use_stk,
                                         Octet16* p_stk);
void btm_ble_link_encrypted(const RawAddress& bd_addr,
                                   uint8_t encr_enable);
...
bool btm_ble_get_enc_key_type(const RawAddress& bd_addr,
                                     uint8_t* p_key_types);

void btm_sec_save_le_key(const RawAddress& bd_addr,
...
tBTM_SEC_DEV_REC* btm_ble_resolve_random_addr(
