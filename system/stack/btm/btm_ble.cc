...
void BTM_BleSetPrefConnParams(const RawAddress& bd_addr, uint16_t min_conn_int,
  ...
  LOG_DEBUG("BTM_BleSetPrefConnParams min: %u  max: %u  latency: %u  \
                    tout: %u",
  ...
  if ((p_rec = btm_find_dev(bd_addr)) != NULL &&
    ...
    switch (key_type) {
      ...
      case BTM_LE_KEY_LENC:
        ...
        LOG_VERBOSE("BTM_LE_KEY_LENC key_type=0x%x DIV=0x%x key_size=0x%x "
                    "sec_level=0x%x",
    ...
    switch (event) {
      ...
      case SMP_COMPLT_EVT:
        if (btm_cb.api.p_le_callback) {
           ...
           * capability... */
          LOG_VERBOSE("btm_cb.api.p_le_callback=0x%p",
