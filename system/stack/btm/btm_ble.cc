...
bool BTM_GetRemoteDeviceName(const RawAddress& bd_addr, BD_NAME bd_name) {
  ...
  if (btif_storage_get_remote_device_property(&bd_addr, &prop_name) ==
      BT_STATUS_SUCCESS) {
    LOG_DEBUG("%s, NV name = %s", __func__, bdname.name);
...
void BTM_BleSetPrefConnParams(const RawAddress& bd_addr, uint16_t min_conn_int,
  ...
  LOG_INFO("BTM_BleSetPrefConnParams min: %u  max: %u  latency: %u  \
                    tout: %u",
    ...
    *p_dev_type = BT_DEVICE_TYPE_BREDR;
    ...
    if (p_inq_info != NULL) {
      ...
      *p_addr_type = p_inq_info->results.ble_addr_type;
      ...
      LOG_DEBUG("btm_find_dev_type - unknown device, BR/EDR assumed");
    }
...
tBTM_SEC_ACTION btm_ble_determine_security_act(bool is_originator,
  ...
  if (is_originator) {
    if ((security_required & BTM_SEC_OUT_FLAGS) == 0 &&
      ...
      LOG_DEBUG("%s No security required for outgoing connection", __func__);
      return BTM_SEC_OK;
    }
    ...
    if ((security_required & BTM_SEC_IN_FLAGS) == 0 &&
      ...
      LOG_DEBUG("%s No security required for incoming connection", __func__);
      return BTM_SEC_OK;
    }
  ...
  LOG_DEBUG("%s ble_sec_act %d", __func__, ble_sec_act);
...
void btm_ble_increment_sign_ctr(const RawAddress& bd_addr, bool is_local) {
  ...
  LOG_DEBUG("btm_ble_increment_sign_ctr is_local=%d", is_local);
  ...
  if (p_dev_rec != NULL) {
    ...
    LOG_DEBUG("is_local=%d local sign counter=%d peer sign counter=%d",
...
bool btm_ble_get_enc_key_type(const RawAddress& bd_addr, uint8_t* p_key_types) {
  ...
  LOG_DEBUG("btm_ble_get_enc_key_type");
...
bool btm_get_local_div(const RawAddress& bd_addr, uint16_t* p_div) {
  ...
  *p_div = 0;
  ...
  LOG_DEBUG("btm_get_local_div status=%d (1-OK) DIV=0x%x", status, *p_div);
  return status;
}
...
void btm_sec_save_le_key(const RawAddress& bd_addr, tBTM_LE_KEY_TYPE key_type,
  ...
  LOG_DEBUG("btm_sec_save_le_key key_type=0x%x pass_to_application=%d",
    ...
    switch (key_type) {
      case BTM_LE_KEY_PENC:
        ...
        if (p_keys->penc_key.sec_level == SMP_SEC_AUTHENTICATED)
          ...
          p_rec->sec_flags &= ~BTM_SEC_LE_LINK_KEY_AUTHED;
        LOG_DEBUG("BTM_LE_KEY_PENC key_type=0x%x sec_flags=0x%x sec_leve=0x%x",
      ...
      case BTM_LE_KEY_LENC:
        ...
        LOG_DEBUG("BTM_LE_KEY_LENC key_type=0x%x DIV=0x%x key_size=0x%x "
                  "sec_level=0x%x",
                  p_rec->ble.key_type, p_rec->ble.keys.div,
                  p_rec->ble.keys.key_size, p_rec->ble.keys.sec_level);
        break;
      ...
      case BTM_LE_KEY_LCSRK: /* local CSRK has been delivered */
        ...
        LOG_DEBUG("BTM_LE_KEY_LCSRK key_type=0x%x DIV=0x%x scrk_sec_level=0x%x "
                  "local_counter=%d",
                  p_rec->ble.key_type, p_rec->ble.keys.div,
  ...
  if (p_rec) {
    LOG_DEBUG("sec_flags=0x%x", p_rec->sec_flags);
  }
}
...
void btm_ble_update_sec_key_size(const RawAddress& bd_addr,
  ...
  LOG_DEBUG("btm_ble_update_sec_key_size enc_key_size = %d", enc_key_size);
...
void btm_ble_link_sec_check(const RawAddress& bd_addr,
  ...
  LOG_DEBUG("btm_ble_link_sec_check auth_req =0x%x", auth_req);
  ...
  if (p_dev_rec->is_security_state_encrypting() ||
    ...
    *p_sec_req_act = BTM_BLE_SEC_REQ_ACT_DISCARD;
    ...
    LOG_DEBUG("dev_rec sec_flags=0x%x", p_dev_rec->sec_flags);
  ...
  LOG_DEBUG("cur_sec_level=%d req_sec_level=%d sec_req_act=%d", cur_sec_level,
...
tBTM_STATUS btm_ble_set_encryption(const RawAddress& bd_addr,
  ...
  LOG_DEBUG("btm_ble_set_encryption sec_act=0x%x role_central=%d", sec_act,
            p_rec->role_central);
  ...
  switch (sec_act) {
    ...
    case BTM_BLE_SEC_ENCRYPT_MITM:
      ...
      if (sec_req_act == BTM_BLE_SEC_REQ_ACT_NONE ||
        ...
        LOG_DEBUG("%s, no action needed. Ignore", __func__);
        cmd = BTM_SUCCESS;
        break;
      }
      if (link_role == HCI_ROLE_CENTRAL) {
...
tBTM_STATUS btm_ble_start_encrypt(const RawAddress& bda, bool use_stk,
  ...
  LOG_DEBUG("btm_ble_start_encrypt");
...
void btm_ble_link_encrypted(const RawAddress& bd_addr, uint8_t encr_enable) {
  ...
  LOG_DEBUG("btm_ble_link_encrypted encr_enable=%d", encr_enable);

  enc_cback = p_dev_rec->is_security_state_le_encrypting();

  smp_link_encrypted(bd_addr, encr_enable);

  LOG_DEBUG(" p_dev_rec->sec_flags=0x%x", p_dev_rec->sec_flags);
...
void btm_ble_ltk_request_reply(const RawAddress& bda, bool use_stk,
  ...
  LOG_DEBUG("btm_ble_ltk_request_reply");
  p_cb->enc_handle = p_rec->ble_hci_handle;
  p_cb->key_size = p_rec->ble.keys.key_size;

  LOG_ERROR("key size = %d", p_rec->ble.keys.key_size);
  if (use_stk) {
...
uint8_t btm_ble_io_capabilities_req(tBTM_SEC_DEV_REC* p_dev_rec,
  ...
  LOG_DEBUG("btm_ble_io_capabilities_req");
  ...
  if ((callback_rc == BTM_SUCCESS) || (BTM_OOB_UNKNOWN != p_data->oob_data)) {
    ...
    if (btm_cb.devcb.keep_rfu_in_auth_req) {
      LOG_DEBUG("btm_ble_io_capabilities_req keep_rfu_in_auth_req = %u",
    ...
    if (!(p_data->auth_req & SMP_AUTH_BOND)) {
      LOG_DEBUG("Non bonding: No keys should be exchanged");
      p_data->init_keys = 0;
      p_data->resp_keys = 0;
    }

    LOG_DEBUG("btm_ble_io_capabilities_req 3: auth_req:%d", p_data->auth_req);
    LOG_DEBUG("btm_ble_io_capabilities_req 4: i_keys=0x%x r_keys=0x%x",
              p_data->init_keys, p_data->resp_keys);

    LOG_DEBUG("btm_ble_io_capabilities_req 5: p_data->io_cap = %d auth_req:%d",
...
tBTM_STATUS btm_proc_smp_cback(tSMP_EVT event, const RawAddress& bd_addr,
  ...
  LOG_DEBUG("btm_proc_smp_cback event = %d", event);
    ...
    switch (event) {
      ...
      case SMP_SEC_REQUEST_EVT:
        if (event == SMP_SEC_REQUEST_EVT &&
          ...
          LOG_DEBUG("%s: Ignoring SMP Security request", __func__);
          break;
        }
      ...
      case SMP_COMPLT_EVT:
        if (btm_cb.api.p_le_callback) {
           ...
           * capability... */
          LOG_DEBUG("btm_cb.api.p_le_callback=0x%p", btm_cb.api.p_le_callback);
        ...
        if (event == SMP_COMPLT_EVT) {
          ...
          LOG_DEBUG("after update result=%d sec_level=0x%x sec_flags=0x%x", res,
                    p_data->cmplt.sec_level, p_dev_rec->sec_flags);

          if (p_data->cmplt.is_pair_cancel &&
              btm_cb.api.p_bond_cancel_cmpl_callback) {
            LOG_DEBUG("Pairing Cancel completed");
          ...
          if (res != BTM_SUCCESS) {
            if (!btm_cb.devcb.no_disc_if_pair_fail &&
              ...
              LOG_DEBUG("Pairing failed - prepare to remove ACL");
              l2cu_start_post_bond_timer(p_dev_rec->ble_hci_handle);
            } else {
              LOG_DEBUG("Pairing failed - Not Removing ACL");
              p_dev_rec->sec_state = BTM_SEC_STATE_IDLE;
            }
          }
#else
          ...
          LOG_DEBUG("btm_cb pairing_state=%x pairing_flags=%x pin_code_len=%x",
                    btm_cb.pairing_state, btm_cb.pairing_flags,
                    btm_cb.pin_code_len);
...
bool BTM_BleDataSignature(const RawAddress& bd_addr, uint8_t* p_text,
  ...
  LOG_DEBUG("%s-Start to generate Local CSRK", __func__);
  pp = p_buf;
  /* prepare plain text */
  if (p_text) {
  ...
  LOG_DEBUG("%s p_mac = %p", __func__, p_mac);
  LOG_DEBUG("p_mac[0] = 0x%02x p_mac[1] = 0x%02x p_mac[2] = 0x%02x p_mac[3] = "
            "0x%02x",
            *p_mac, *(p_mac + 1), *(p_mac + 2), *(p_mac + 3));
  LOG_DEBUG("p_mac[4] = 0x%02x p_mac[5] = 0x%02x p_mac[6] = 0x%02x p_mac[7] = "
            "0x%02x",
...
bool BTM_BleVerifySignature(const RawAddress& bd_addr, uint8_t* p_orig,
  ...
  if (p_rec == NULL || (p_rec && !(p_rec->ble.key_type & BTM_LE_KEY_PCSRK))) {
    ...
    LOG_DEBUG("%s rcv_cnt=%d >= expected_cnt=%d", __func__, counter,
...
static void btm_notify_new_key(uint8_t key_type) {
  ...
  LOG_DEBUG("btm_notify_new_key key_type=%d", key_type);

  if (btm_cb.api.p_le_key_callback) {
    switch (key_type) {
      case BTM_BLE_KEY_TYPE_ID:
        LOG_DEBUG("BTM_BLE_KEY_TYPE_ID");
        p_local_keys = (tBTM_BLE_LOCAL_KEYS*)&btm_cb.devcb.id_keys;
        break;

      case BTM_BLE_KEY_TYPE_ER:
        LOG_DEBUG("BTM_BLE_KEY_TYPE_ER");
        p_local_keys =
...
void btm_ble_set_no_disc_if_pair_fail(bool disable_disc) {
  LOG_DEBUG("btm_ble_set_disc_enable_if_pair_fail disable_disc=%d",
            disable_disc);
...
void btm_ble_set_test_mac_value(bool enable, uint8_t* p_test_mac_val) {
  LOG_DEBUG("btm_ble_set_test_mac_value enable=%d", enable);
...
void btm_ble_set_keep_rfu_in_auth_req(bool keep_rfu) {
  LOG_DEBUG("btm_ble_set_keep_rfu_in_auth_req keep_rfus=%d", keep_rfu);
