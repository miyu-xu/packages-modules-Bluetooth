...
typedef struct {
 ...
 public:
  void init(size_t pkt_size) {
    ...
    msbc_buf = (uint8_t*)osi_calloc(buf_size);
  }
  ...
  void mark_read(size_t len) {
    if (read_offset + len > write_offset) {
      LOG_WARN("Trying to mark read offset beyond write offset.");
      return;
    }
...
size_t decode(const uint8_t** out_data) {
  ...
  if (frame_head == nullptr) {
    LOG_DEBUG("No valid mSBC packet to decode %lu, %lu",
              (unsigned long)msbc_info->read_offset,
              (unsigned long)msbc_info->write_offset);
