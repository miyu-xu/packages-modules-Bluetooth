 ...
 public:
  ...
  size_t write(const std::vector<uint8_t>& input) {
    if (input.size() > decode_buf_avail_len()) {
      log::warn("Cannot write input with size {} into decode_buf with {} empty space.", input.size(), decode_buf_avail_len());
      return 0;
    }
...
struct tBTM_LC3_INFO {
 ...
 public:
  ...
  size_t write(const std::vector<uint8_t>& input) {
    if (input.size() > decode_buf_avail_len()) {
      log::warn("Cannot write input with size {} into decode_buf with {} empty space.", input.size(), buf_size - decode_buf_wo);
      return 0;
    }
