 ...
 * value as a fallback. */
constexpr size_t btm_wbs_supported_pkt_size[] = {BTM_MSBC_PKT_LEN, 24, 48, 72,
                                                 0};
...
typedef struct {
 ...
 public:
  ...
  uint8_t* fill_msbc_pkt_template() {
    ...
    if (encode_buf_wo + BTM_MSBC_PKT_LEN > buf_size) {
  ...
  size_t mark_pkt_dequeued() {
    if (encode_buf_ro + packet_size > encode_buf_wo) {
      LOG_ERROR("Trying to mark read offset beyond write offset.");
      return 0;
    }
