...
/* Supported SCO packet sizes for mSBC. The frame parsing
...
struct tBTM_WBS_INFO {
  ...
  size_t packet_size; /* SCO packet size supported by lower layer */
  size_t buf_size; /* The size of the buffer, determined by the packet_size. */

  uint8_t* wbs_decode_buf;  /* Buffer to store packets to decode */
...
void cleanup(int* num_decoded_frames, double* packet_loss_ratio) {
