...
typedef struct {
 ...
 public:
  ...
  size_t mark_pkt_dequeued() {
    LOG_DEBUG(
        "Try to mark an encoded packet dequeued: ro:%zu wo:%lu pkt_size:%lu",
        encode_buf_ro, encode_buf_wo, packet_size);
...
size_t enqueue_packet(const uint8_t* data, size_t pkt_size) {
  ...
  if (pkt_size != msbc_info->packet_size) {
    LOG_WARN(
        "Ignoring the coming packet with size %zu that is inconsistent with "
        "the HAL reported packet size %zu",
        pkt_size, msbc_info->packet_size);
    return 0;
  }
  ...
  if (msbc_info->write(data, pkt_size) != pkt_size) {
    LOG_DEBUG("Fail to write packet with size %zu to buffer", pkt_size);
    return 0;
  }
...
size_t decode(const uint8_t** out_data) {
  ...
  if (frame_head == nullptr) {
    LOG_DEBUG("No valid mSBC packet to decode %zu, %zu",
              msbc_info->decode_buf_ro, msbc_info->decode_buf_wo);
...
size_t encode(const uint8_t* data, size_t len) {
  ...
  if (len < BTM_MSBC_CODE_SIZE) {
    LOG_DEBUG("Insufficient PCM frames to be encoded into a mSBC packet");
    return 0;
  }
  ...
  encoded_size = hfp_msbc_encode_frames((int16_t*)data, pkt_body);
  if (encoded_size != BTM_MSBC_PKT_FRAME_LEN) {
    LOG_WARN("Encoding invalid packet size: %zu", encoded_size);
