...
void init() {
  ...
  cached_codecs.push_back({
          ...
          .sco_packet_size = kDefaultPacketSize,
  });
  ...
  if (osi_property_get_bool("bluetooth.hfp.swb.supported", false)) {
    cached_codecs.push_back({
            .codec = codec::LC3,
            .data_path = ESCO_DATA_PATH_PCM,
            .sco_packet_size = kDefaultPacketSize,
  ...
  for (const auto& c : cached_codecs) {
    bluetooth::log::info("Caching HFP codec {}, data path {}, sco_packet_size {}",
                         (uint64_t)c.codec, c.data_path, c.sco_packet_size);
  }
}
...
// Software path (!offload_enabled) implies support of SWB.
bool get_swb_supported() {
  return !offload_enabled || osi_property_get_bool("bluetooth.hfp.swb.supported", false);
}

// Checks the supported codecs
bt_codecs get_codec_capabilities(uint64_t codecs) {
  bt_codecs codec_list = {.offload_capable = offload_enabled};
...
bool get_offload_supported() { return offload_enabled; }

// Check if hardware offload is enabled
bool get_offload_enabled() { return offload_enabled; }
