...
public:
  ...
  std::string ToString() const {
    return std::format("bredr_linkkey_known:{:c},le_linkkey_known:{:c},bond_type:{},bredr_linkkey_type:{},ble_enc_key_size:{},bredr_authenticated:{:c},le_authenticated:{:c},16_digit_key_authenticated:{:c},bredr_encrypted:{:c},le_encrypted:{:c}", is_link_key_known() ? 'T' : 'F', is_le_link_key_known() ? 'T' : 'F', bond_type_text(bond_type), linkkey_type_text(link_key_type), enc_key_size, is_device_authenticated() ? 'T' : 'F', is_le_device_authenticated() ? 'T' : 'F', is_le_link_16_digit_key_authenticated() ? 'T' : 'F', is_device_encrypted() ? 'T' : 'F', is_le_device_encrypted() ? 'T' : 'F');
  }
};
...
public:
  ...
  std::string ToString() const {
    return std::format("{} {:6s} cod:{} remote_info:{:<14s} sm4:0x{:02x} SecureConn:{:c} name:\"{}\"sec_prop:{}", ADDRESS_TO_LOGGABLE_CSTR(bd_addr), DeviceTypeText(device_type), dev_class_text(dev_class), remote_version_info.ToString(), sm4, (remote_supports_secure_connections) ? 'T' : 'F', PRIVATE_NAME(reinterpret_cast<char const*>(sec_bd_name)), sec_rec.ToString());
  }
