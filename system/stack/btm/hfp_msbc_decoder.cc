...
bool hfp_msbc_decoder_init() {
  if (hfp_msbc_decoder) {
    LOG_WARN("%s: Trying to init the decoder when there is a exiting one",
             __func__);
    hfp_msbc_decoder_cleanup();
  }

  hfp_msbc_decoder = (tHFP_MSBC_DECODER*)calloc(1, sizeof(tHFP_MSBC_DECODER));
  if (hfp_msbc_decoder == NULL) {
    LOG_ERROR("%s: Out of memory when allocating the decoder", __func__);
    return false;
  }
  ...
  status =
      OI_CODEC_SBC_DecoderConfigureMSbc(&hfp_msbc_decoder->decoder_context);
  if (!OI_SUCCESS(status)) {
...
void hfp_msbc_decoder_cleanup(void) {
  free(hfp_msbc_decoder);
  hfp_msbc_decoder = NULL;
}
...
bool hfp_msbc_decoder_decode_packet(BT_HDR* p_buf, uint8_t** out_buf) {
  ...
  if (hfp_msbc_decoder == NULL) {
    LOG_ERROR("%s: Decoder hasn't been initialized", __func__);
    return false;
  }
  ...
  if (!OI_SUCCESS(status) || out_avail != 240 || oi_size != 0) {
    LOG_ERROR("%s: Decoding failure: %d, %llu, %llu", __func__, status,
              out_avail, oi_size);
    return false;
  }
