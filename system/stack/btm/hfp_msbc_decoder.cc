...
#define HFP_MSBC_PKT_LEN 58
#define HFP_MSBC_FRAME_LEN 240
...
bool hfp_msbc_decoder_decode_packet(const uint8_t* i_buf, int16_t* o_buf,
                                    size_t out_len) {
  if (out_len < HFP_MSBC_FRAME_LEN) {
    LOG_ERROR(
        "Output buffer's size %lu is less than one complete mSBC frame %d",
        (unsigned long)out_len, HFP_MSBC_FRAME_LEN);
    return false;
  }
  ...
  if (!OI_SUCCESS(status) || out_avail != HFP_MSBC_FRAME_LEN || oi_size != 0) {
