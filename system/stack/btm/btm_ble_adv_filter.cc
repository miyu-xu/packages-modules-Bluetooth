...
static uint8_t btm_ble_ocf_to_condtype(uint8_t ocf) {
  ...
  switch (ocf) {
    ...
    case BTM_BLE_META_PF_SRVC_DATA:
      ...
      break;
    case BTM_BLE_META_PF_AD_DATA_TYPE:
      cond_type = BTM_BLE_PF_AD_DATA_TYPE;
      break;
 ...
 * condition.
 */
static void BTM_LE_PF_ad_data_type(tBTM_BLE_SCAN_COND_OP action,
                                   tBTM_BLE_PF_FILT_INDEX filt_index,
  ...
  uint8_t* p = param;
  UINT8_TO_STREAM(p, BTM_BLE_META_PF_AD_DATA_TYPE);
  UINT8_TO_STREAM(p, action);
  UINT8_TO_STREAM(p, filt_index);
  ...
  btu_hcif_send_cmd_with_cb(
      FROM_HERE, HCI_BLE_ADV_FILTER, param, len,
      base::Bind(&btm_flt_update_cb, BTM_BLE_META_PF_AD_DATA_TYPE, cb));
...
static uint8_t btm_ble_cs_update_pf_counter(tBTM_BLE_SCAN_COND_OP action,
  ...
  if (BTM_BLE_PF_ADDR_FILTER == cond_type ||
      ...
      BTM_BLE_PF_SRVC_DATA_PATTERN == cond_type || BTM_BLE_PF_AD_DATA_TYPE)
    p_bd_addr = NULL;
...
void BTM_LE_PF_set(tBTM_BLE_PF_FILT_INDEX filt_index,
  ...
  for (const ApcfCommand& cmd : commands) {
    ...
    switch (cmd.type) {
      ...
      case BTM_BLE_PF_AD_DATA_TYPE: {
        BTM_LE_PF_ad_data_type(action, filt_index, cmd.ad_type, cmd.data,
...
void BTM_LE_PF_clear(tBTM_BLE_PF_FILT_INDEX filt_index,
  ...
  /* clear the general filter entry */
  {
    ...
    BTM_LE_PF_ad_data_type(BTM_BLE_SCAN_COND_CLEAR, filt_index, 0, {}, {},
                           fDoNothing);
