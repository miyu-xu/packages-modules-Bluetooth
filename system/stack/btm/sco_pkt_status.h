...
typedef struct {
 ...
 public:
  void init() {
    memset(data, 0, BTM_PKT_STATUS_LEN);
  ...
  void update(bool is_lost) {
    ...
    if (offset == 0) {
      ts = bluetooth::common::time_get_os_monotonic_raw_us();
    }
    offset++;
    if (offset == size) {
      offset = 0;
  ...
  std::string data_to_hex_string() {
    ...
    for (i = 0; i < len; ++i) {
      ...
      if ((i + 1) % 8 == 0) {
        s += bluetooth::common::StringFormat("%.2x", byte);
        byte = 0;
      }
    }

    if (i % 8) s += bluetooth::common::StringFormat("%.2x", byte);
