...
tBTM_STATUS BTM_SetDiscoverability(uint16_t inq_mode) {
  ...
  LOG_INFO("BTM_SetDiscoverability");
  ...
  LOG_INFO("BTM_SetDiscoverability: mode %d [NonDisc-0, Lim-1, Gen-2]",
           inq_mode);
...
void BTM_EnableInterlacedInquiryScan() {
  LOG_INFO("BTM_EnableInterlacedInquiryScan");
...
void BTM_EnableInterlacedPageScan() {
  LOG_INFO("BTM_EnableInterlacedPageScan");
...
tBTM_STATUS BTM_SetConnectability(uint16_t page_mode) {
  ...
  LOG_INFO("BTM_SetConnectability page scan interval  = (%d * 0.625)ms",
           interval);
  ...
  LOG_INFO("BTM_SetConnectability: mode %d [NonConn-0, Conn-1]", page_mode);
...
void BTM_CancelInquiry(void) {
  tBTM_INQUIRY_VAR_ST* p_inq = &btm_cb.btm_inq_vars;
  LOG_INFO("BTM_CancelInquiry called");
...
tBTM_STATUS BTM_CancelRemoteDeviceName(void) {
  ...
  LOG_INFO("BTM_CancelRemoteDeviceName()");
...
void btm_inq_stop_on_ssp(void) {
  ...
  LOG_DEBUG("btm_inq_stop_on_ssp: no_inc_ssp=%d inq_active:0x%x state:%d ",
...
#if (BTM_INQ_DEBUG == TRUE)
  LOG_DEBUG("btm_clr_inq_db: inq_active:0x%x state:%d",
            btm_cb.btm_inq_vars.inq_active, btm_cb.btm_inq_vars.state);
#endif
  std::lock_guard<std::mutex> lock(inq_db_lock_);
  tINQ_DB_ENT* p_ent = inq_db_;
...
#if (BTM_INQ_DEBUG == TRUE)
  LOG_DEBUG("inq_active:0x%x state:%d", btm_cb.btm_inq_vars.inq_active,
            btm_cb.btm_inq_vars.state);
#endif
}
...
void btm_process_inq_results(const uint8_t* p, uint8_t hci_evt_len,
  ...
  for (xx = 0; xx < num_resp; xx++) {
    ...
    if (btm_inq_find_bdaddr(bda)) {
      /* LOG_DEBUG("BDA seen before %s", ADDRESS_TO_LOGGABLE_CSTR(bda));
       */
      ...
      if ((rssi != 0) && p_i &&
        ...
        LOG_DEBUG("update RSSI new:%d, old:%d", i_rssi, p_cur->rssi);
        p_cur->rssi = i_rssi;
        update = true;
      }
...
tBTM_STATUS BTM_WriteEIR(BT_HDR* p_buff) {
  if (controller_get_interface()->supports_extended_inquiry_response()) {
    LOG_INFO("Write Extended Inquiry Response to controller");
...
void btm_set_eir_uuid(const uint8_t* p_eir, tBTM_INQ_RESULTS* p_results) {
  ...
  LOG_INFO("btm_set_eir_uuid eir_complete_list=0x%02X",
