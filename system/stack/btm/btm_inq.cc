...
const uint16_t BTM_EIR_UUID_LKUP_TBL[BTM_EIR_MAX_SERVICES] = {
  ...
  UUID_SERVCLASS_SERIAL_PORT, UUID_SERVCLASS_LAN_ACCESS_USING_PPP, UUID_SERVCLASS_DIALUP_NETWORKING,
  UUID_SERVCLASS_IRMC_SYNC, UUID_SERVCLASS_OBEX_OBJECT_PUSH, UUID_SERVCLASS_OBEX_FILE_TRANSFER,
  UUID_SERVCLASS_IRMC_SYNC_COMMAND, UUID_SERVCLASS_HEADSET, UUID_SERVCLASS_CORDLESS_TELEPHONY,
  UUID_SERVCLASS_AUDIO_SOURCE, UUID_SERVCLASS_AUDIO_SINK, UUID_SERVCLASS_AV_REM_CTRL_TARGET,
  ...
  UUID_SERVCLASS_IMAGING, UUID_SERVCLASS_IMAGING_RESPONDER, UUID_SERVCLASS_IMAGING_AUTO_ARCHIVE,
  UUID_SERVCLASS_IMAGING_REF_OBJECTS, UUID_SERVCLASS_HF_HANDSFREE, UUID_SERVCLASS_AG_HANDSFREE,
  UUID_SERVCLASS_DIR_PRT_REF_OBJ_SERVICE,
  /*    UUID_SERVCLASS_REFLECTED_UI,              */
  UUID_SERVCLASS_BASIC_PRINTING, UUID_SERVCLASS_PRINTING_STATUS, UUID_SERVCLASS_HUMAN_INTERFACE,
  UUID_SERVCLASS_CABLE_REPLACEMENT, UUID_SERVCLASS_HCRP_PRINT, UUID_SERVCLASS_HCRP_SCAN,
  /*    UUID_SERVCLASS_COMMON_ISDN_ACCESS,        */
  /*    UUID_SERVCLASS_VIDEO_CONFERENCING_GW,     */
  /*    UUID_SERVCLASS_UDI_MT,                    */
  /*    UUID_SERVCLASS_UDI_TA,                    */
  /*    UUID_SERVCLASS_VCP,                       */
  UUID_SERVCLASS_SAP, UUID_SERVCLASS_PBAP_PCE, UUID_SERVCLASS_PBAP_PSE, UUID_SERVCLASS_PHONE_ACCESS,
  UUID_SERVCLASS_HEADSET_HS, UUID_SERVCLASS_PNP_INFORMATION,
  ...
  UUID_SERVCLASS_MESSAGE_ACCESS, UUID_SERVCLASS_MESSAGE_NOTIFICATION, UUID_SERVCLASS_HDP_SOURCE,
...
tBTM_STATUS BTM_CancelRemoteDeviceName(void) {
  ...
  if (btm_cb.btm_inq_vars.remname_active) {
    ...
    if (is_le) {
      ...
      bluetooth::shim::ACL_CancelRemoteNameRequest(btm_cb.btm_inq_vars.remname_bda);
      if (com::android::bluetooth::flags::rnr_reset_state_at_cancel()) {
        btm_process_remote_name(&btm_cb.btm_inq_vars.remname_bda, nullptr, 0, HCI_ERR_UNSPECIFIED);
      }
    }
    return BTM_CMD_STARTED;
  } else {
...
tBTM_STATUS btm_initiate_rem_name(const RawAddress& remote_bda, uint64_t timeout_ms,
  ...
  if (btm_cb.btm_inq_vars.remname_active) {
    ...
    if (p_i && (p_i->inq_info.results.inq_result_type & BT_DEVICE_TYPE_BREDR)) {
      ...
      bluetooth::shim::ACL_RemoteNameRequest(remote_bda, p_cur->results.page_scan_rep_mode,
...
void btm_process_remote_name(const RawAddress* bda, const BD_NAME bdn, uint16_t /* evt_len */,
  ...
  if (btm_cb.btm_inq_vars.remname_active) {
    ...
    log::info(
            ...
            hci_status_code_text(hci_status), on_le_link, btm_cb.btm_inq_vars.remname_active);
  }
}
