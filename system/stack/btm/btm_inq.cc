...
tBTM_STATUS BTM_CancelRemoteDeviceName(void) {
  ...
  if (is_le) {
    ...
    bluetooth::shim::ACL_CancelRemoteNameRequest(btm_cb.btm_inq_vars.rnr.remname_bda);
...
void btm_process_remote_name(const RawAddress* bda, const BD_NAME bdn, uint16_t /* evt_len */,
  ...
  bool on_le_link;
  if (com::android::bluetooth::flags::rnr_store_device_type()) {
    on_le_link = (btm_cb.btm_inq_vars.rnr.remname_dev_type == BT_DEVICE_TYPE_BLE);
  } else {
  ...
  if (btm_cb.btm_inq_vars.rnr.remname_active) {
    ...
    log::info(
            ...
            hci_status_code_text(hci_status), on_le_link, btm_cb.btm_inq_vars.rnr.remname_active);
  }
}
...
void btm_inq_rmt_name_failed_cancelled(void) {
  ...
  if (btm_cb.btm_inq_vars.rnr.remname_active) {
    btm_process_remote_name(&btm_cb.btm_inq_vars.rnr.remname_bda, NULL, 0, HCI_ERR_UNSPECIFIED);
  }
