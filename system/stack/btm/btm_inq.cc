...
tBTM_STATUS BTM_CancelRemoteDeviceName(void) {
  ...
  if (is_le) {
    ...
    if (com::android::bluetooth::flags::rnr_reset_state_at_cancel()) {
      btm_process_remote_name(&btm_cb.rnr.remname_bda, nullptr, 0,
                              HCI_ERR_UNSPECIFIED);
    }
  }
  return (BTM_CMD_STARTED);
...
tBTM_STATUS btm_initiate_rem_name(const RawAddress& remote_bda,
  ...
  if (btm_cb.rnr.remname_active) {
    ...
    alarm_set_on_mloop(btm_cb.rnr.remote_name_timer, timeout_ms,
...
void btm_process_remote_name(const RawAddress* bda, const BD_NAME bdn,
  ...
  if (btm_cb.rnr.remname_active) {
    if (rem_name.bd_addr == RawAddress::kEmpty ||
        rem_name.bd_addr == btm_cb.rnr.remname_bda) {
      log::info(
    ...
    log::info(
        ...
        "le_link:{} rnr_active:{}",
        rem_name.bd_addr.ToRedactedStringForLogging(),
        btm_cb.rnr.remname_bda.ToRedactedStringForLogging(),
        hci_status_code_text(hci_status), on_le_link,
        btm_cb.rnr.remname_active);
  }
}
...
void btm_inq_rmt_name_failed_cancelled(void) {
  ...
  if (btm_cb.rnr.remname_active) {
    btm_process_remote_name(&btm_cb.rnr.remname_bda, NULL, 0,
                            HCI_ERR_UNSPECIFIED);
  }
