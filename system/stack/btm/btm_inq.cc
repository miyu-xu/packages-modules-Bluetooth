...
<<<<<<< PATCH SET (00aacc stack::btm::tBTM_CB Extract rnr data fields into structure)
  if (btm_cb.btm_inq_vars.rnr.remname_active) {
    ...
    if (is_le) {
      ...
      if (com::android::bluetooth::flags::rnr_reset_state_at_cancel()) {
        btm_process_remote_name(&btm_cb.btm_inq_vars.rnr.remname_bda, nullptr,
                                0, HCI_ERR_UNSPECIFIED);
      }
    }
    return (BTM_CMD_STARTED);
  } else
...
>>>>>>> BASE      (d56005 Merge "Revert^2 "stack::inq Finalize extract inquiry API"" i)
    return (BTM_WRONG_MODE);
  }
