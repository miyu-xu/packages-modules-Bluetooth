...
void btm_log_history_scan_mode(uint8_t scan_mode) {
  ...
  BTM_LogHistory(kBtmLogTag, RawAddress::kEmpty, "Classic updated",
                 std::format("inquiry_scan_enable:{:c} page_scan_enable:{:c}", (scan_mode & HCI_INQUIRY_SCAN_ENABLED) ? 'T' : 'F', (scan_mode & HCI_PAGE_SCAN_ENABLED) ? 'T' : 'F'));
...
void BTM_CancelInquiry(void) {
  ...
  BTM_LogHistory(kBtmLogTag, RawAddress::kEmpty, "Classic inquiry canceled",
                 std::format("duration_s:{:6.3f} results:{} std:{} rssi:{} ext:{}", duration_ms / 1000.0, (unsigned long)btm_cb.neighbor.classic_inquiry.results, btm_cb.btm_inq_vars.inq_cmpl_info.resp_type[BTM_INQ_RESULT_STANDARD], btm_cb.btm_inq_vars.inq_cmpl_info.resp_type[BTM_INQ_RESULT_WITH_RSSI], btm_cb.btm_inq_vars.inq_cmpl_info.resp_type[BTM_INQ_RESULT_EXTENDED]));
  ...
  if (btm_cb.btm_inq_vars.inq_active) {
    ...
    if (btm_cb.btm_inq_vars.inqparms.mode == 0) {
      ...
      BTM_LogHistory(kBtmLogTag, RawAddress::kEmpty, "Classic inquiry complete",
                     std::format("duration_s:{:6.3f} results:{} inq_active:0x{:02x} std:{} rssi:{} ext:{} status:{}", (end_time_ms - btm_cb.neighbor.classic_inquiry.start_time_ms) / 1000.0, (unsigned long)btm_cb.neighbor.classic_inquiry.results, inq_active, btm_cb.btm_inq_vars.inq_cmpl_info.resp_type[BTM_INQ_RESULT_STANDARD], btm_cb.btm_inq_vars.inq_cmpl_info.resp_type[BTM_INQ_RESULT_WITH_RSSI], btm_cb.btm_inq_vars.inq_cmpl_info.resp_type[BTM_INQ_RESULT_EXTENDED], hci_error_code_text(status)));
