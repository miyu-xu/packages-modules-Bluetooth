...
tBTM_STATUS BTM_SetDiscoverability(uint16_t inq_mode) {
  ...
  LOG_DEBUG("BTM_SetDiscoverability");
  ...
  LOG_DEBUG("BTM_SetDiscoverability: mode %d [NonDisc-0, Lim-1, Gen-2]",
            inq_mode);
...
void BTM_EnableInterlacedInquiryScan() {
  LOG_DEBUG("BTM_EnableInterlacedInquiryScan");
...
void BTM_EnableInterlacedPageScan() {
  LOG_DEBUG("BTM_EnableInterlacedPageScan");
...
tBTM_STATUS BTM_SetConnectability(uint16_t page_mode) {
  ...
  LOG_DEBUG("BTM_SetConnectability page scan interval  = (%d * 0.625)ms",
            interval);
  ...
  LOG_DEBUG("BTM_SetConnectability: mode %d [NonConn-0, Conn-1]", page_mode);
...
void BTM_CancelInquiry(void) {
  tBTM_INQUIRY_VAR_ST* p_inq = &btm_cb.btm_inq_vars;
  LOG_DEBUG("BTM_CancelInquiry called");
...
tBTM_STATUS BTM_CancelRemoteDeviceName(void) {
  ...
  LOG_DEBUG("BTM_CancelRemoteDeviceName()");
...
tBTM_STATUS BTM_WriteEIR(BT_HDR* p_buff) {
  if (controller_get_interface()->supports_extended_inquiry_response()) {
    LOG_DEBUG("Write Extended Inquiry Response to controller");
...
void btm_set_eir_uuid(const uint8_t* p_eir, tBTM_INQ_RESULTS* p_results) {
  ...
  LOG_DEBUG("btm_set_eir_uuid eir_complete_list=0x%02X",
