  ...
  if (start) {
    ...
    if (!btm_sec_cb.ble_ctr_cb.is_ble_scan_active()) {
      ...
      btm_send_hci_set_scan_params(
          ...
          btm_sec_cb.ble_ctr_cb.addr_mgnt_cb.own_addr_type, BTM_BLE_DEFAULT_SFP);
...
void btm_ble_process_adv_pkt_cont(uint16_t evt_type, tBLE_ADDR_TYPE addr_type,
  ...
  tBTM_INQ_RESULTS_CB* p_obs_results_cb = btm_sec_cb.ble_ctr_cb.p_obs_results_cb;
...
bool btm_ble_set_topology_mask(tBTM_BLE_STATE_MASK request_state_mask) {
  ...
  btm_sec_cb.ble_ctr_cb.cur_states |= (request_state_mask & BTM_BLE_STATE_ALL_MASK);
  return true;
}
