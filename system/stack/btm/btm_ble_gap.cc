...
void BTM_BleReadControllerFeatures(tBTM_BLE_CTRL_FEATURES_CBACK* p_vsc_cback) {
  ...
  if (GetController()->GetVendorCapabilities().dynamic_audio_buffer_support_) {
    // Copy the results from ControllerDabReadCapability
    uint32_t codec_mask = 0;
    if (GetController()->GetControllerDabSbcSupported()) {
      codec_mask |= 1;
      btm_cb.dynamic_audio_buffer_cb[0].default_buffer_time =
          GetController()->GetControllerDabSbcCapability().default_time_ms_;
      btm_cb.dynamic_audio_buffer_cb[0].maximum_buffer_time =
          GetController()->GetControllerDabSbcCapability().maximum_time_ms_;
      btm_cb.dynamic_audio_buffer_cb[0].minimum_buffer_time =
          GetController()->GetControllerDabSbcCapability().minimum_time_ms_;
    }
    if (GetController()->GetControllerDabAacSupported()) {
      codec_mask |= 0x02;
      btm_cb.dynamic_audio_buffer_cb[1].default_buffer_time =
          GetController()->GetControllerDabAacCapability().default_time_ms_;
      btm_cb.dynamic_audio_buffer_cb[1].maximum_buffer_time =
          GetController()->GetControllerDabAacCapability().maximum_time_ms_;
      btm_cb.dynamic_audio_buffer_cb[1].minimum_buffer_time =
          GetController()->GetControllerDabAacCapability().minimum_time_ms_;
    }
    if (GetController()->GetControllerDabAptxSupported()) {
      codec_mask |= 0x04;
      btm_cb.dynamic_audio_buffer_cb[2].default_buffer_time =
          GetController()->GetControllerDabAptxCapability().default_time_ms_;
      btm_cb.dynamic_audio_buffer_cb[2].maximum_buffer_time =
          GetController()->GetControllerDabAptxCapability().maximum_time_ms_;
      btm_cb.dynamic_audio_buffer_cb[2].minimum_buffer_time =
          GetController()->GetControllerDabAptxCapability().minimum_time_ms_;
    }
    if (GetController()->GetControllerDabAptxHdSupported()) {
      codec_mask |= 0x08;
      btm_cb.dynamic_audio_buffer_cb[3].default_buffer_time =
          GetController()->GetControllerDabAptxHdCapability().default_time_ms_;
      btm_cb.dynamic_audio_buffer_cb[3].maximum_buffer_time =
          GetController()->GetControllerDabAptxHdCapability().maximum_time_ms_;
      btm_cb.dynamic_audio_buffer_cb[3].minimum_buffer_time =
          GetController()->GetControllerDabAptxHdCapability().minimum_time_ms_;
    }
    if (GetController()->GetControllerDabLdacSupported()) {
      codec_mask |= 0x10;
      btm_cb.dynamic_audio_buffer_cb[4].default_buffer_time =
          GetController()->GetControllerDabLdacCapability().default_time_ms_;
      btm_cb.dynamic_audio_buffer_cb[4].maximum_buffer_time =
          GetController()->GetControllerDabLdacCapability().maximum_time_ms_;
      btm_cb.dynamic_audio_buffer_cb[4].minimum_buffer_time =
          GetController()->GetControllerDabLdacCapability().minimum_time_ms_;
    }
    LOG_INFO(
        ": codec_mask = 0x%08x (original) 0x%08x (known codecs)",
        GetController()->GetVendorCapabilities().dynamic_audio_buffer_support_,
        codec_mask);

    // Copy reserved buffer times.
    if (reserved.size() != BTM_CODEC_TYPE_MAX_RECORDS - 5) {
      LOG_ERROR("Buffer time reserved codecs are out of date");
    }
    auto reserved = GetController()->GetControllerDabReservedCapability();
    for (int i = 5; i < BTM_CODEC_TYPE_MAX_RECORDS && (i - 5) < reserved.size();
         i++) {
      btm_cb.dynamic_audio_buffer_cb[i].default_buffer_time =
          reserved[i - 5].default_time_ms_;
      btm_cb.dynamic_audio_buffer_cb[i].maximum_buffer_time =
          reserved[i - 5].maximum_time_ms_;
      btm_cb.dynamic_audio_buffer_cb[i].minimum_buffer_time =
          reserved[i - 5].minimum_time_ms_;
    }
  }
