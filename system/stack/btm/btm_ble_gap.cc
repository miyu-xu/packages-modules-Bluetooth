...
bool BTM_BleConfigPrivacy(bool privacy_mode) {
  ...
  if (!privacy_mode) /* if privacy disabled, always use public address */
  {
    ...
    /* This is a Floss only flag. Random address is only needed when disabling
     * LL privacy in ChromeOS */
    if (com::android::bluetooth::flags::separate_host_privacy_and_llprivacy()) {
  ...
  } else /* privacy is turned on*/
  {
     ...
     * disabled */
    btm_cb.ble_ctr_cb.addr_mgnt_cb.own_addr_type = BLE_ADDR_RANDOM;
    /* This is a Floss only flag. Android has LL privacy enabled by default, the
     * address policy is random when privacy mode is enabled */
    if (com::android::bluetooth::flags::separate_host_privacy_and_llprivacy()) {
