 ...
 *  mapping under the appropriate appearanceToCODForCategoryXXXX macro.
 */
static DEV_CLASS btm_ble_appearance_to_cod(uint16_t appearance) {
  DEV_CLASS dev_class = kDevClassEmpty;

  #define addSwitchCases(appearance, value, service, major, minor) \
    case appearance: \
      dev_class[0] = service; \
      dev_class[1] = major; \
      dev_class[2] = minor; \
  ...
  switch (appearance) {
    appearanceToCOD(addSwitchCases);
    // No need of adding default case
  };

  #undef addSwitchCases
  return dev_class;
}
