...
void BTM_BleTargetAnnoucementObserve(bool enable,
                                     tBTM_INQ_RESULTS_CB* p_results_cb) {
  if (bluetooth::shim::is_gd_shim_enabled()) {
    bluetooth::shim::BTM_BleTargetAnnoucementObserve(enable, p_results_cb);
    // NOTE: passthrough, no return here. GD would send the results back to BTM,
    // and it needs the callbacks set properly.
  }

  if (enable) {
    btm_cb.ble_ctr_cb.p_target_annoucement_obs_results_cb = p_results_cb;
  } else {
    btm_cb.ble_ctr_cb.p_target_annoucement_obs_results_cb = NULL;
  }
}
...
void btm_ble_process_adv_pkt_cont(uint16_t evt_type, tBLE_ADDR_TYPE addr_type,
  ...
  tBTM_INQ_RESULTS_CB* p_target_annoucement_obs_results_cb =
      btm_cb.ble_ctr_cb.p_target_annoucement_obs_results_cb;
  if (p_target_annoucement_obs_results_cb) {
    (p_target_annoucement_obs_results_cb)(
