...
void btm_queue_start_sync_req(uint8_t sid, RawAddress address, uint16_t skip,
                              uint16_t timeout) {
  BTM_TRACE_DEBUG("%s: address = %s, sid = %d", __func__,
                  address.ToString().c_str(), sid);
  sync_node_t node = {};
  node.sid = sid;
...
void btm_ble_periodic_adv_sync_established(uint8_t status, uint16_t sync_handle,
  ...
  BTM_TRACE_DEBUG(
      "[PSync]%s: status=%d, sync_handle=%d, s_id=%d, "
      "addr_type=%d, adv_phy=%d,adv_interval=%d, clock_acc=%d",
      __func__, status, sync_handle, adv_sid, address_type, phy, interval,
      adv_clock_accuracy);
...
void btm_ble_periodic_adv_report(uint16_t sync_handle, uint8_t tx_power,
  ...
  BTM_TRACE_DEBUG(
      "[PSync]%s: sync_handle = %d, tx_power = %d, rssi = %d,"
      "cte_type = %d, data_status = %d, data_len = %d",
      __func__, sync_handle, tx_power, rssi, cte_type, data_status, data_len);
...
void btm_ble_biginfo_adv_report_rcvd(uint8_t* p, uint16_t param_len) {
  ...
  BTM_TRACE_DEBUG(
      "[PAST]%s:sync_handle %d, num_bises = %d, nse = %d,"
      "iso_interval = %d, bn = %d, pto = %d, irc = %d, max_pdu = %d "
      "sdu_interval = %d, max_sdu = %d, phy = %d, framing = %d, encryption  = "
      "%d",
      __func__, sync_handle, num_bises, nse, iso_interval, bn, pto, irc,
...
void btm_ble_periodic_adv_sync_tx_rcvd(uint8_t* p, uint16_t param_len) {
  ...
  BTM_TRACE_DEBUG(
      "[PAST]%s: status = %d, conn_handle = %d, service_data = %d,"
      " sync_handle = %d, adv_sid = %d, address_type = %d, addr = %s,"
      " adv_phy = %d, pa_int = %d, clk_acc = %d",
      __func__, status, conn_handle, service_data, sync_handle, adv_sid,
      address_type, addr.ToString().c_str(), adv_phy, pa_int, clk_acc);
