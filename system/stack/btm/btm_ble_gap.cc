...
tBTM_STATUS BTM_BleObserve(bool start, uint8_t duration, tBTM_INQ_RESULTS_CB* p_results_cb,
  ...
  if (start) {
    ...
    BTM_LogHistory(kBtmLogTag, RawAddress::kEmpty, "Le observe stopped",
                   std::format("duration_s:{:6.3f} results:{:<3}", (double)duration_timestamp / 1000.0, (unsigned long)btm_cb.neighbor.le_observe.results));
    status = tBTM_STATUS::BTM_CMD_STARTED;
    btm_ble_stop_observe();
  } else {
...
static void btm_ble_stop_scan(void) {
  ...
  BTM_LogHistory(
          ...
          std::format("duration_s:{:6.3f} results:{:<3}", (double)duration_timestamp / 1000.0, (unsigned long)btm_cb.neighbor.le_legacy_scan.results));
  btm_send_hci_scan_enable(BTM_BLE_SCAN_DISABLE, BTM_BLE_DUPLICATE_ENABLE);
...
void btm_ble_stop_inquiry(void) {
  ...
  BTM_LogHistory(
          ...
          std::format("duration_s:{:6.3f} results:{:<3}", (double)duration_timestamp / 1000.0, (unsigned long)btm_cb.neighbor.le_inquiry.results));
