...
static void sync_queue_advance() {
  ...
  if (sync_queue && !list_is_empty(sync_queue)) {
    ...
    LOG_INFO("%s: queue_advance", __func__);
...
static void btm_queue_sync_next() {
  ...
  LOG_INFO("%s: executing sync request SID=%04X, bd_addr=%s",
...
void btm_ble_periodic_adv_sync_established(uint8_t status, uint16_t sync_handle, uint8_t adv_sid,
     uint8_t address_type, const RawAddress& addr, uint8_t phy, uint16_t interval,
     uint8_t adv_clock_accuracy) {
  BTM_TRACE_DEBUG("[PSync]%s: status=%d, sync_handle=%d, s_id=%d, "
                  "addr_type=%d, adv_phy=%d,adv_interval=%d, clock_acc=%d",__func__, status,
                  sync_handle, adv_sid, address_type, phy, interval, adv_clock_accuracy);
...
void btm_ble_periodic_syc_transfer_cmd_cmpl(uint8_t status, uint16_t conn_handle) {
  BTM_TRACE_DEBUG("[PAST]%s: status = %d, conn_handle =%d", __func__, status, conn_handle);
...
void btm_ble_periodic_syc_transfer_param_cmpl(uint8_t status) {
  BTM_TRACE_DEBUG("[PAST]%s: status = %d, conn_handle =%d", __func__, status);
}
...
void BTM_BlePeriodicSyncTransfer(RawAddress addr, uint16_t service_data,
  ...
  if (!HCI_LE_PERIODIC_ADVERTISING_SYNC_TRANSFER_RECIPIENT(p_acl->peer_le_features)) {
  ...
  if (BleScanningManager::IsInitialized()) {
    BleScanningManager::Get()->PeriodicAdvSyncTransfer(addr, service_data,
        sync_handle, base::Bind(&btm_ble_periodic_syc_transfer_cmd_cmpl));
  }
}
...
void BTM_BlePeriodicSyncSetInfo(RawAddress addr, uint16_t service_data,
  ...
  p_sync_transfer->cb = cb;
  if (BleScanningManager::IsInitialized()) {
    BleScanningManager::Get()->PeriodicAdvSetInfoTransfer(addr, service_data, adv_handle,
...
void btm_ble_periodic_adv_sync_tx_rcvd(uint8_t *p, uint16_t param_len) {
  ...
  BTM_TRACE_DEBUG("[PAST]%s: status = %d, conn_handle = %d, service_data = %d,"
                  " sync_handle = %d, adv_sid = %d, address_type = %d, addr = %s,"
                  " adv_phy = %d, pa_int = %d, clk_acc = %d",
                  __func__, status, conn_handle, service_data, sync_handle,
                  adv_sid, address_type, addr.ToString().c_str(), adv_phy,
                  pa_int, clk_acc);
...
void BTM_BlePeriodicSyncTxParameters(RawAddress addr, uint8_t mode,
  ...
  if (BleScanningManager::IsInitialized()) {
    BleScanningManager::Get()->SetPeriodicAdvSyncTransferParams(addr, mode, skip, timeout,
        cte_type, true,
