...
void btm_ble_batchscan_filter_track_adv_vse_cback(uint8_t len,
  ...
  LOG_DEBUG("btm_ble_batchscan_filter_track_adv_vse_cback called with event:%x",
            sub_event);
  ...
  if (HCI_VSE_SUBCODE_BLE_TRACKING_SUB_EVT == sub_event &&
    ...
    LOG_DEBUG("track_adv_vse_cback called: %d, %d, %d", adv_data.filt_index,
...
void BTM_BleSetStorageConfig(uint8_t batch_scan_full_max,
  ...
  LOG_DEBUG("%s: %d, %d, %d, %d, %d", __func__, ble_batchscan_cb.cur_state,
...
void BTM_BleEnableBatchScan(tBTM_BLE_BATCH_SCAN_MODE scan_mode,
                            ...
                            tBTM_BLE_DISCARD_RULE discard_rule,
                            Callback<void(uint8_t /* status */)> cb) {
  LOG_DEBUG("%s: %d, %d, %d, %d, %d", __func__, scan_mode, scan_interval,
...
void BTM_BleDisableBatchScan(base::Callback<void(uint8_t /* status */)> cb) {
  LOG_DEBUG(" BTM_BleDisableBatchScan");
...
void BTM_BleReadScanReports(tBTM_BLE_BATCH_SCAN_MODE scan_mode,
  ...
  LOG_DEBUG("%s; %d", __func__, scan_mode);
...
void btm_ble_batchscan_init(void) {
  LOG_DEBUG(" btm_ble_batchscan_init");
