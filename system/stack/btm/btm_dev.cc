...
bool btm_dev_support_role_switch(const RawAddress& bd_addr) {
  if (BTM_IsScoActiveByBdaddr(bd_addr)) {
    LOG_DEBUG("%s Role switch is not allowed if a SCO is up", __func__);
    return false;
  }

  tBTM_SEC_DEV_REC* p_dev_rec = btm_find_dev(bd_addr);
  if (p_dev_rec == nullptr) {
    LOG_DEBUG("%s Unknown address for role switch", __func__);
    return false;
  }

  if (!controller_get_interface()->supports_central_peripheral_role_switch()) {
    LOG_DEBUG("%s Local controller does not support role switch", __func__);
    return false;
  }

  if (p_dev_rec->remote_supports_hci_role_switch) {
    LOG_DEBUG("%s Peer controller supports role switch", __func__);
    return true;
  }
  ...
  LOG_DEBUG("%s Peer controller does not support role switch", __func__);
  return false;
}
...
tBTM_SEC_DEV_REC* btm_find_or_alloc_dev(const RawAddress& bd_addr) {
  tBTM_SEC_DEV_REC* p_dev_rec;
  LOG_INFO("btm_find_or_alloc_dev");
