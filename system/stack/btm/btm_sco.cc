...
void btm_route_sco_data(BT_HDR* p_msg) {
  ...
  while (written) {
    ...
    if (avail) {
      ...
      if (btm_pcm_buf_write_offset + read > BTM_SCO_DATA_SIZE_MAX) {
        LOG_ERROR("Read more data than available buffer size.");
        break;
      }
    ...
    if (active_sco->is_wbs()) {
      ...
      LOG_DEBUG("%lu bytes of data is encoded", (unsigned long)rc);
