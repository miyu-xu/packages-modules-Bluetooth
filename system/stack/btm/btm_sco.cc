...
void btm_sco_chk_pend_unpark(tHCI_STATUS hci_status, uint16_t hci_handle) {
  ...
  for (uint16_t xx = 0; xx < BTM_MAX_SCO_LINKS; xx++, p++) {
    uint16_t acl_handle = BTM_GetHCIConnHandle(p->esco.data.bd_addr, BT_TRANSPORT_BR_EDR);
...
void btm_sco_chk_pend_rolechange(uint16_t hci_handle) {
  ...
  for (xx = 0; xx < BTM_MAX_SCO_LINKS; xx++, p++) {
    if ((p->state == SCO_ST_PEND_ROLECHANGE) &&
        ((acl_handle = BTM_GetHCIConnHandle(p->esco.data.bd_addr, BT_TRANSPORT_BR_EDR)) ==
         hci_handle))
...
tBTM_SCO_DEBUG_DUMP BTM_GetScoDebugDump() {
  ...
  if (debug_dump.codec_id != UUID_CODEC_MSBC && debug_dump.codec_id != UUID_CODEC_LC3) {
    return debug_dump;
  }
