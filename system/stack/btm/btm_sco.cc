...
static void btm_esco_conn_rsp(uint16_t sco_inx, uint8_t hci_status,
  ...
  if (hci_status != HCI_SUCCESS || p_sco == NULL) {
    ...
    if (controller_get_interface()
      ...
      LOG_VERBOSE("%s: txbw 0x%x, rxbw 0x%x, lat 0x%x, retrans 0x%02x, "
                  "pkt 0x%04x, path %u",
      ...
      if (p->state != SCO_ST_PEND_UNPARK) {
        if (is_orig) {
          ...
          if (!acl_is_switch_role_idle(*remote_bda, BT_TRANSPORT_BR_EDR)) {
            LOG_DEBUG("Role Change is in progress for ACL handle 0x%04x",
                      acl_handle);
...
static tBTM_STATUS BTM_ChangeEScoLinkParms(uint16_t sco_inx,
  ...
  if (p_sco->esco.data.link_type == BTM_LINK_TYPE_SCO ||
    ...
    LOG_DEBUG("%s: SCO Link for handle 0x%04x, pkt 0x%04x", __func__,
  ...
  } else /* eSCO is supported and the link type is eSCO */
  {
    ...
    LOG_DEBUG("%s -> eSCO Link for handle 0x%04x", __func__, p_sco->hci_handle);
    LOG_DEBUG("   txbw 0x%x, rxbw 0x%x, lat 0x%x, retrans 0x%02x, pkt 0x%04x",
    ...
    LOG_DEBUG("%s: txbw 0x%x, rxbw 0x%x, lat 0x%x, retrans 0x%02x, pkt 0x%04x",
