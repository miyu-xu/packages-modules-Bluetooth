...
bool btm_pcm_buf_write_mirror = false;
bool btm_pcm_buf_read_mirror = false;
...
void incr_btm_pcm_buf_offset(size_t& offset, bool& mirror, size_t bsize,
                             size_t amount) {
    ...
    while (written) {
      ...
      if (avail) {
        ...
        log::assert_that(
            ...
            "read",
            codec, (unsigned long)read, (unsigned long)avail);

        if (BTM_SCO_DATA_SIZE_MAX - btm_pcm_buf_write_offset > read) {
          std::copy(packet_buf, packet_buf + BTM_SCO_DATA_SIZE_MAX,
                    btm_pcm_buf + btm_pcm_buf_write_offset);
        } else {
          std::copy(
              packet_buf,
              packet_buf + BTM_SCO_DATA_SIZE_MAX - btm_pcm_buf_write_offset,
              btm_pcm_buf + btm_pcm_buf_write_offset);
          std::copy(
              packet_buf + BTM_SCO_DATA_SIZE_MAX - btm_pcm_buf_write_offset,
              packet_buf + BTM_SCO_DATA_SIZE_MAX, btm_pcm_buf);
        }
      ...
      incr_btm_pcm_buf_offset(btm_pcm_buf_write_offset,
                              btm_pcm_buf_write_mirror, BTM_SCO_DATA_SIZE_MAX,
                              read);
      ...
      if (BTM_SCO_DATA_SIZE_MAX - btm_pcm_buf_read_offset > data_len) {
        ...
        std::copy(
            btm_pcm_buf,
            btm_pcm_buf + data_len -
                (BTM_SCO_DATA_SIZE_MAX - btm_pcm_buf_read_offset),
            packet_buf + (BTM_SCO_DATA_SIZE_MAX - btm_pcm_buf_read_offset));
      }
      ...
      incr_btm_pcm_buf_offset(btm_pcm_buf_read_offset, btm_pcm_buf_read_mirror,
                              BTM_SCO_DATA_SIZE_MAX, rc);
