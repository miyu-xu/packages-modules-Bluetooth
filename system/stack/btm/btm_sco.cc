...
void btm_route_sco_data(BT_HDR* p_msg) {
  ...
  if (active_sco->is_wbs()) {
    while (written) {
      wp = (uint8_t*)btm_pcm_buf + btm_pcm_buf_write_offset;
      avail = BTM_SCO_DATA_SIZE_MAX - btm_pcm_buf_write_offset;
      if (avail) {
        data_len = written < avail ? written : avail;
        read = bluetooth::audio::sco::read(wp, data_len);
        if (read != data_len) {
  ...
  for (xx = 0; xx < BTM_MAX_SCO_LINKS; xx++, p++) {
    if (((p->state == SCO_ST_CONNECTING) || (p->state == SCO_ST_LISTENING) ||
      ...
      if (p->is_inband()) {
        ...
        ASSERT_LOG(sizeof(btm_pcm_buf) % BTM_MSBC_CODE_SIZE == 0,
                   "The size of PCM buffer %lu should be divisible by "
                   "BTM_MSBC_CODE_SIZE %lu",
                   (unsigned long)sizeof(btm_pcm_buf), BTM_MSBC_CODE_SIZE);
