...
static void btm_esco_conn_rsp(uint16_t sco_inx, uint8_t hci_status,
  ...
  if (hci_status != HCI_SUCCESS || p_sco == NULL) {
    ...
    if (controller_get_interface()
      ...
      LOG_DEBUG("%s: txbw 0x%x, rxbw 0x%x, lat 0x%x, retrans 0x%02x, "
                "pkt 0x%04x, path %u",
                __func__, p_setup->transmit_bandwidth,
      ...
      if (p->state != SCO_ST_PEND_UNPARK) {
        if (is_orig) {
          ...
          if (!acl_is_switch_role_idle(*remote_bda, BT_TRANSPORT_BR_EDR)) {
            LOG_INFO("Role Change is in progress for ACL handle 0x%04x",
                     acl_handle);
...
tBTM_STATUS BTM_RemoveSco(uint16_t sco_inx) {
  ...
  if (BTM_ReadPowerMode(p->esco.data.bd_addr, &state) &&
      (state == BTM_PM_ST_PENDING)) {
    LOG_DEBUG("%s: BTM_PM_ST_PENDING for ACL mapped with SCO Link 0x%04x",
...
static tBTM_STATUS BTM_ChangeEScoLinkParms(uint16_t sco_inx,
  ...
  if (p_sco->esco.data.link_type == BTM_LINK_TYPE_SCO ||
    ...
    LOG_INFO("%s: SCO Link for handle 0x%04x, pkt 0x%04x", __func__,
  ...
  } else /* eSCO is supported and the link type is eSCO */
  {
    ...
    LOG_INFO("%s -> eSCO Link for handle 0x%04x", __func__, p_sco->hci_handle);
    LOG_INFO("   txbw 0x%x, rxbw 0x%x, lat 0x%x, retrans 0x%02x, pkt 0x%04x",
    ...
    LOG_INFO("%s: txbw 0x%x, rxbw 0x%x, lat 0x%x, retrans 0x%02x, pkt 0x%04x",
...
static uint16_t btm_sco_voice_settings_to_legacy(enh_esco_params_t* p_params) {
  ...
  LOG_DEBUG("%s: voice setting for legacy 0x%03x", __func__, voice_settings);
