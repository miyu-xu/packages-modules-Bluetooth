...
tBTM_STATUS BTM_SetBleDataLength(const RawAddress& bd_addr, uint16_t tx_pdu_length,
  ...
  if (p_dev_rec->is_le_datalen_set_by_prevclient() && !prev_client) {
    log::info("Data length set by prev client can't be overridden");
    return BTM_SUCCESS;
  }

  /* preveliged client can override to have lesser Data length & this can happen
   * multiple times from preveliged clients.
   * */
  ...
  if (prev_client) {
    p_dev_rec->set_is_datalen_set_by_prevclient(true);
  }
