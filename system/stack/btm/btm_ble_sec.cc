...
void BTM_SecurityGrant(const RawAddress& bd_addr, uint8_t res) {
  ...
  LOG_VERBOSE("bd_addr:%s, res:%d", ADDRESS_TO_LOGGABLE_CSTR(bd_addr),
...
tBTM_STATUS BTM_SetBleDataLength(const RawAddress& bd_addr,
  ...
  LOG_INFO("bd_addr:%s, tx_pdu_length:%d",
           ADDRESS_TO_LOGGABLE_CSTR(bd_addr), tx_pdu_length);
...
void btm_sec_save_le_key(const RawAddress& bd_addr, tBTM_LE_KEY_TYPE key_type,
  ...
  LOG_VERBOSE("key_type=0x%x pass_to_application=%d", key_type, pass_to_application);
...
void btm_ble_link_sec_check(const RawAddress& bd_addr,
  ...
  LOG_VERBOSE("bd_addr:%s, auth_req=0x%x", ADDRESS_TO_LOGGABLE_CSTR(bd_addr), auth_req);
...
tBTM_STATUS btm_ble_start_encrypt(const RawAddress& bda, bool use_stk,
  ...
  LOG_VERBOSE("bd_addr:%s, use_stk:%d", ADDRESS_TO_LOGGABLE_CSTR(bd_addr), use_stk);
...
void btm_ble_link_encrypted(const RawAddress& bd_addr, uint8_t encr_enable) {
  ...
  LOG_VERBOSE("bd_addr:%s, encr_enable=%d", ADDRESS_TO_LOGGABLE_CSTR(bd_addr), encr_enable);
...
uint8_t btm_ble_io_capabilities_req(tBTM_SEC_DEV_REC* p_dev_rec,
  ...
  LOG_VERBOSE("p_dev_rec->bd_addr:%s", ADDRESS_TO_LOGGABLE_CSTR(p_dev_rec->bd_addr));
  ...
  if ((callback_rc == BTM_SUCCESS) || (BTM_OOB_UNKNOWN != p_data->oob_data)) {
    ...
    LOG_VERBOSE("4:i_keys=0x%x r_keys=0x%x",
                p_data->init_keys, p_data->resp_keys);

    LOG_VERBOSE("5:p_data->io_cap=%d auth_req:%d", p_data->io_cap, p_data->auth_req);
    ...
    if (!(p_data->auth_req & SMP_SC_SUPPORT_BIT)) {
      ...
      */
      LOG_VERBOSE("SC not supported -> No LK derivation, no keypress notifications");
      p_data->auth_req &= ~SMP_KP_SUPPORT_BIT;
      p_data->init_keys &= ~SMP_SEC_KEY_TYPE_LK;
      p_data->resp_keys &= ~SMP_SEC_KEY_TYPE_LK;
    }

    LOG_VERBOSE("6:IO_CAP:%d oob_data:%d auth_req:0x%02x",
...
void btm_ble_connected(const RawAddress& bda, uint16_t handle, uint8_t enc_mode,
  ...
  LOG_INFO("Update timestamp for ble connection:%s", ADDRESS_TO_LOGGABLE_CSTR(bd_addr));
...
tBTM_STATUS btm_proc_smp_cback(tSMP_EVT event, const RawAddress& bd_addr,
  ...
  LOG_VERBOSE("bd_addr:%s, event=%", ADDRESS_TO_LOGGABLE_CSTR(bd_addr), event);
