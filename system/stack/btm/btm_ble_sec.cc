...
static void btm_ble_user_confirmation_req(const RawAddress& bd_addr, tBTM_LE_EVT event,
...
static void btm_ble_sec_req(const RawAddress& bd_addr, tBTM_LE_EVT_DATA* p_data) {
  tBTM_SEC_DEV_REC* p_dev_rec = btm_find_dev(bd_addr);
...
static void btm_ble_complete_evt(const RawAddress& bd_addr, tBTM_LE_EVT_DATA* p_data) {
  tBTM_SEC_DEV_REC* p_dev_rec = btm_find_dev(bd_addr);
...
tBTM_STATUS btm_proc_smp_cback(tSMP_EVT event, const RawAddress& bd_addr, tSMP_EVT_DATA* p_data) {
  ...
  switch (event) {
    ...
    case SMP_SC_OOB_REQ_EVT:
      btm_ble_user_confirmation_req(bd_addr, static_cast<tBTM_LE_EVT>(event),
                                    reinterpret_cast<tBTM_LE_EVT_DATA*>(p_data));
      break;

    case SMP_SEC_REQUEST_EVT:
      btm_ble_sec_req(bd_addr, reinterpret_cast<tBTM_LE_EVT_DATA*>(p_data));
      break;
    ...
    case SMP_COMPLT_EVT:
      btm_ble_complete_evt(bd_addr, reinterpret_cast<tBTM_LE_EVT_DATA*>(p_data));
      break;
    ...
    case SMP_SIRK_VERIFICATION_REQ_EVT: {
      ...
      if (res != tBTM_STATUS::BTM_CMD_STARTED) {
        log::warn("SMP SIRK verification failed:{}", btm_status_text(res));
        return res;
      }
      break;
    }
