...
static void btm_ble_get_auth_req(tBTM_SEC_DEV_REC* p_dev_rec, tBTM_LE_AUTH_REQ *p_auth_req) {
  ...
  if (com::android::bluetooth::flags::peripheral_auth_req() && !p_dev_rec->role_central &&
      p_dev_rec->sec_rec.le_link == tSECURITY_STATE::AUTHENTICATING &&
    ...
    // Trying to encrypt the link with bonded device as a peripheral
    if ((p_dev_rec->sec_rec.security_required & BTM_SEC_IN_MITM) ||
        p_dev_rec->sec_rec.ble_keys.sec_level == SMP_SEC_AUTHENTICATED) {
      // MITM protection required or existing bond record has MITM protection
      *p_auth_req |= BTM_LE_AUTH_REQ_MITM;
    } else {
      // No MITM protection required and no bond record
      *p_auth_req &= ~BTM_LE_AUTH_REQ_MITM;
    }
