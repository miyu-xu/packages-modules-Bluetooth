...
void set_codec_datapath(esco_coding_format_t coding_format) {
  ...
  if (get_offload_enabled()) {
    /* TODO(jrwu): expect the data content to be represented differently */
    std::vector<uint8_t> data = {codec->codec == codec::MSBC};

    btm_configure_data_path(btm_data_direction::CONTROLLER_TO_HOST, 0x01, data);
    btm_configure_data_path(btm_data_direction::HOST_TO_CONTROLLER, 0x01, data);
  }
}
