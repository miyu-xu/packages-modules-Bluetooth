...
void set_codec_datapath(enh_esco_params_t* params) {
  ...
  switch (esco_coding_to_codec(params->transmit_coding_format.coding_format)) {
    case codec::MSBC:
      ...
      LOG_WARNING(
          "Format (%u) is not supported or enabled. Fallback to "
          "MSBC_TRANSPARENT.",
    ...
    case codec::MSBC_TRANSPARENT:
      if (found = get_single_codec(codec::MSBC_TRANSPARENT, &codec)) {
        break;
      };
      LOG_WARNING("Format (%u) is not supported. Fallback to CVSD.",
                  codec::MSBC_TRANSPARENT);
      [[fallthrough]];  // If transparent mSBC doesn't work out, try CVSD
    // Default to CVSD
    case codec::CVSD:
    default:
