...
void cache_codec_capabilities(struct mgmt_rp_get_codec_capabilities* rp) {
  ...
  auto codecs = controller_get_interface()->GetLocalSupportedBrEdrCodecIds();

  for (uint8_t codec_id : codecs) {
    // TODO(b/316077719): Query the codec capabilities and fill in c.inner.data.
    ...
    switch (codec_id) {
      ...
      case kCodecMsbc:
        ...
        break;
      default:
        LOG_WARN("Unsupported codec ID: %u", codec_id);
        continue;
    }
