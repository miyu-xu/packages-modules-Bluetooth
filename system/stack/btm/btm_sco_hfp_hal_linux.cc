...
constexpr char isoc_alt_path_format[] = "/sys/class/bluetooth/hci{}/device/isoc_alt";
...
void notify_sco_connection_change(RawAddress device, bool is_connected, int codec) {
  ...
  std::fstream fs(isoc_alt_path, std::fstream::out);
  if (!fs.is_open()) {
    bluetooth::log::warn("Failed to open USB alt settings, err:{}", std::strerror(errno));
    return;
  }
