...
void cache_codec_capabilities(struct mgmt_rp_get_codec_capabilities* rp) {
  ...
  if (rp->transparent_wbs_supported) {
    cached_codecs.push_back({
            ...
            .sco_packet_size = rp->wbs_pkt_len,
    });
  }
  ...
  for (const auto& c : cached_codecs) {
    bluetooth::log::info("Caching HFP codec {}, data path {}, data len {}, sco_packet_size {}",
                         (uint64_t)c.codec, c.data_path, c.data.size(), c.sco_packet_size);
  }
}
