...
static void decode_controller_support() {
  ...
  LOG_DEBUG("Local supported SCO packet types: 0x%04x",
            btm_cb.btm_sco_pkt_types_supported);
...
void BTM_VendorSpecificCommand(uint16_t opcode, uint8_t param_len,
  ...
  LOG_INFO("BTM: %s: Opcode: 0x%04X, ParamLen: %i.", __func__, opcode,
           param_len);
...
tBTM_STATUS BTM_RegisterForVSEvents(tBTM_VS_EVT_CB* p_cb, bool is_register) {
  ...
  for (i = 0; i < BTM_MAX_VSE_CALLBACKS; i++) {
    if (btm_cb.devcb.p_vend_spec_cb[i] == NULL) {
      ...
      if (!is_register) {
        ...
        LOG_INFO("BTM Deregister For VSEvents is successfully");
      }
      return (BTM_SUCCESS);
    }
  }
  ...
  if (is_register) {
    if (free_idx < BTM_MAX_VSE_CALLBACKS) {
      ...
      LOG_INFO("BTM Register For VSEvents is successfully");
    } else {
...
void BTM_WritePageTimeout(uint16_t timeout) {
  LOG_INFO("BTM: BTM_WritePageTimeout: Timeout: %d.", timeout);
...
void BTM_WriteVoiceSettings(uint16_t settings) {
  LOG_INFO("BTM: BTM_WriteVoiceSettings: Settings: 0x%04x.", settings);
...
tBTM_STATUS BTM_EnableTestMode(void) {
  ...
  LOG_INFO("BTM: BTM_EnableTestMode");
...
tBTM_STATUS BTM_DeleteStoredLinkKey(const RawAddress* bd_addr,
  ...
  LOG_INFO("BTM: BTM_DeleteStoredLinkKey: delete_all_flag: %s",
