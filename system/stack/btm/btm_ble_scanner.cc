...
class BleScanningManagerImpl
 ...
 public:
  ...
  void PeriodicScanStart(uint8_t options, uint8_t set_id, uint8_t adv_addr_type,
                         ...
                         uint16_t sync_timeout, uint8_t sync_cte_type) override {
  ...
  void SetPeriodicAdvSyncTransferParams(const RawAddress& bd_addr, uint8_t mode,
    ...
    GetHciInterface()->SetPeriodicAdvSyncTransferParams(bd_addr, mode,
                                                        skip, sync_timeout,
                                                        cte_type, set_defaults,
                                                        command_complete);
  }

  void OnPeriodicScanResult(uint16_t sync_handle, uint8_t tx_power,
  ...
  void OnPeriodicScanEstablished(
    ...
    btm_ble_periodic_adv_sync_established(status, sync_handle, set_id,
        adv_addr_type, adv_addr, adv_phy, adv_interval, adv_clock_accuracy);
  }
