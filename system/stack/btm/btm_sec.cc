 ...
 *
 * Description      When an RNR event is recieved from the controller executes
 ...
 *
 * Description      When an RNR event is recieved from the controller adds
 *                  the name to the device record, if valid.
 *
 * Returns          SecurityDeviceRecord pointer if record is found for
 *                  given bluetooth device address with updated name if
 *                  the name is valid.
 *                  nullptr if record is not found
 *                  Name is not updated if name is not valid.
 *
 ******************************************************************************/
...
void btm_sec_rmt_name_request_complete(const RawAddress* p_bd_addr, const uint8_t* p_bd_name,
  ...
  tBTM_SEC_DEV_REC* p_dev_rec = btm_rnr_add_name_to_security_record(p_bd_addr, p_bd_name, status);
  if (p_dev_rec == nullptr) {
    return;
  }
