...
void btm_io_capabilities_req(const RawAddress& p) {
  ...
  LOG_VERBOSE("State: %sm, Security Mode: %d, Device security Flags: 0x%04x",
              btm_pair_state_descr(btm_sec_cb.pairing_state),
...
void btm_rem_oob_req(const uint8_t* p) {
  ...
  LOG_VERBOSE("BDA: %s", ADDRESS_TO_LOGGABLE_STR(p_bda));
  p_dev_rec = btm_find_dev(p_bda);
    ...
    case BTM_PAIR_STATE_WAIT_DISCONNECT:
      ...
      if (p_dev_rec == NULL) {
        LOG_ERROR(
            ...
            "%s" ADDRESS_TO_LOGGABLE_CSTR(p_cb->pairing_bda));
        break;
      }
      btm_sec_send_hci_disconnect(
          p_dev_rec, HCI_ERR_AUTH_FAILURE, p_dev_rec->hci_handle,
