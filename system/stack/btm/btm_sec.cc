...
tBTM_STATUS BTM_SecBond(const RawAddress& bd_addr, tBLE_ADDR_TYPE addr_type,
  ...
  if (transport == BT_TRANSPORT_AUTO) {
    ...
    bool is_ble_discoverable = false;
    tBTM_INQ_INFO* p_inq_info = BTM_InqDbRead(bd_addr);
    if (p_inq_info && com::android::bluetooth::flags::auto_transport_pairing()) {
      inq_result_type = p_inq_info->results.inq_result_type;
      is_ble_discoverable = (inq_result_type == BT_DEVICE_TYPE_BLE);
    }
    if (addr_type == BLE_ADDR_PUBLIC) {
      transport = (get_btm_client_interface().ble.BTM_UseLeLink(bd_addr) ||
                   is_ble_discoverable) ? BT_TRANSPORT_LE: BT_TRANSPORT_BR_EDR;
    } else {
