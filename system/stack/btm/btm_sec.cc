...
void tBTM_SEC_CB::change_pairing_state(tBTM_PAIRING_STATE new_state) {
  ...
  LOG_DEBUG("Pairing state changed %s => %s pairing_flags:0x%x",
            btm_pair_state_descr(pairing_state),
            btm_pair_state_descr(new_state), pairing_flags);

  if (pairing_state != new_state) {
    BTM_LogHistory(
        kBtmLogTag, pairing_bda, "Pairing state changed",
        base::StringPrintf("%s => %s", btm_pair_state_descr(pairing_state),
