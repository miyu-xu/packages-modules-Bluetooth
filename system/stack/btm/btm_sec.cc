...
static bool is_sec_state_equal(void* data, void* context) {
  ...
  if (p_dev_rec->sec_rec.le_link == *state) {
    return false;
  }
...
tBTM_STATUS btm_sec_bond_by_transport(const RawAddress& bd_addr,
  ...
  if (transport == BT_TRANSPORT_LE) {
    ...
    if (SMP_Pair(bd_addr, addr_type) == SMP_STARTED) {
      ...
      p_dev_rec->sec_rec.le_link = tSECURITY_STATE::BTM_SEC_STATE_AUTHENTICATING;
      btm_sec_cb.change_pairing_state(BTM_PAIR_STATE_WAIT_AUTH_COMPLETE);
...
tBTM_STATUS BTM_SecBondCancel(const RawAddress& bd_addr) {
  ...
  if (btm_sec_cb.pairing_flags & BTM_PAIR_FLAGS_LE_ACTIVE) {
    if (p_dev_rec->sec_rec.le_link == tSECURITY_STATE::BTM_SEC_STATE_AUTHENTICATING) {
  ...
  if ((btm_sec_cb.pairing_state != BTM_PAIR_STATE_IDLE) &&
    ...
    if (p_dev_rec->hci_handle != HCI_INVALID_HANDLE) {
      ...
      if (p_dev_rec->sec_rec.classic_link == tSECURITY_STATE::BTM_SEC_STATE_DISCONNECTING) {
        return (BTM_CMD_STARTED);
      }
...
tBTM_STATUS BTM_SetEncryption(const RawAddress& bd_addr,
  ...
  if (bluetooth::common::init_flags::encryption_in_busy_state_is_enabled()) {
    ...
    if (enqueue) {
      log::warn("Security Manager: Enqueue request in state:{}", security_state_text(state));
      btm_sec_queue_encrypt_request(bd_addr, transport, p_callback, p_ref_data,
                                    sec_act);
      return BTM_CMD_STARTED;
    }
  } else {
    if (p_dev_rec->sec_rec.p_callback || (state != tSECURITY_STATE::BTM_SEC_STATE_IDLE)) {
...
// TODO: rename to IsLeSecurityPending, this is
...
static tBTM_STATUS btm_sec_send_hci_disconnect(tBTM_SEC_DEV_REC* p_dev_rec,
  ...
  if (conn_handle == p_dev_rec->hci_handle) {
    ...
    p_dev_rec->sec_rec.classic_link = tSECURITY_STATE::BTM_SEC_STATE_DISCONNECTING;
  } else if (conn_handle == p_dev_rec->ble_hci_handle) {
    if (p_dev_rec->sec_rec.le_link == tSECURITY_STATE::BTM_SEC_STATE_DISCONNECTING) {
      // Already sent ble disconnect
      return BTM_CMD_STARTED;
    }
    p_dev_rec->sec_rec.le_link = tSECURITY_STATE::BTM_SEC_STATE_DISCONNECTING;
  } else {
    log::error(
  ...
  if (chk_acp_auth_done) {
    ...
    if ((0 == (p_dev_rec->sec_rec.sec_flags & BTM_SEC_AUTHENTICATED)) ||
      ...
      */
      ...
      p_dev_rec->sec_rec.p_callback = p_callback;
      p_dev_rec->sec_rec.classic_link = tSECURITY_STATE::BTM_SEC_STATE_DELAY_FOR_ENC;
...
tBTM_STATUS btm_sec_mx_access_request(const RawAddress& bd_addr,
  ...
  if ((p_dev_rec->sec_rec.p_callback) ||
    ...
    if (p_dev_rec->sec_rec.classic_link != tSECURITY_STATE::BTM_SEC_STATE_IDLE) {
...
void btm_sec_abort_access_req(const RawAddress& bd_addr) {
  ...
  if ((p_dev_rec->sec_rec.classic_link != tSECURITY_STATE::BTM_SEC_STATE_AUTHORIZING) &&
      (p_dev_rec->sec_rec.classic_link != tSECURITY_STATE::BTM_SEC_STATE_AUTHENTICATING)) {
    return;
  }

  p_dev_rec->sec_rec.classic_link = tSECURITY_STATE::BTM_SEC_STATE_IDLE;
...
void btm_sec_rmt_name_request_complete(const RawAddress* p_bd_addr,
  ...
  if (status == HCI_SUCCESS) {
    log::debug(
            ...
            security_state_text(p_dev_rec->sec_rec.classic_link));
  ...
  const bool is_security_state_getting_name =
          (p_dev_rec->sec_rec.classic_link == tSECURITY_STATE::BTM_SEC_STATE_GETTING_NAME);
...
void btm_simple_pair_complete(const RawAddress bd_addr, uint8_t status) {
  ...
  if (status == HCI_SUCCESS) {
    ...
    if (p_dev_rec->sec_rec.classic_link != tSECURITY_STATE::BTM_SEC_STATE_AUTHENTICATING) {
...
void btm_sec_encrypt_change(uint16_t handle, tHCI_STATUS status,
  ...
  log::debug(
          ...
          security_state_text(p_dev_rec->sec_rec.le_link),
          security_state_text(p_dev_rec->sec_rec.classic_link), p_dev_rec->sec_rec.sec_flags);
  ...
  if (!p_dev_rec->sec_rec.is_security_state_bredr_encrypting()) {
    if (tSECURITY_STATE::BTM_SEC_STATE_DELAY_FOR_ENC == p_dev_rec->sec_rec.classic_link) {
      p_dev_rec->sec_rec.classic_link = tSECURITY_STATE::BTM_SEC_STATE_IDLE;
      log::verbose("clearing callback. p_dev_rec={}, p_callback={}",
                   fmt::ptr(p_dev_rec),
...
void btm_sec_disconnected(uint16_t handle, tHCI_REASON reason,
  ...
  if (transport == BT_TRANSPORT_LE) {
    p_dev_rec->sec_rec.le_link = tSECURITY_STATE::BTM_SEC_STATE_IDLE;
  ...
  if (com::android::bluetooth::flags::
    ...
    if (p_dev_rec->sec_rec.le_link == tSECURITY_STATE::BTM_SEC_STATE_ENCRYPTING &&
        transport != BT_TRANSPORT_LE) {
    ...
    if ((p_dev_rec->sec_rec.classic_link == tSECURITY_STATE::BTM_SEC_STATE_AUTHENTICATING ||
         p_dev_rec->sec_rec.classic_link == tSECURITY_STATE::BTM_SEC_STATE_ENCRYPTING) &&
        transport != BT_TRANSPORT_BR_EDR) {
      log::debug(
...
tBTM_STATUS btm_sec_execute_procedure(tBTM_SEC_DEV_REC* p_dev_rec) {
  ...
  log::debug("security_required:0x{:x} security_flags:0x{:x} le_link:{}[{}] classic_link:{}[{}]",
             ...
             security_state_text(static_cast<tSECURITY_STATE>(p_dev_rec->sec_rec.le_link)),
             p_dev_rec->sec_rec.le_link,
             security_state_text(static_cast<tSECURITY_STATE>(p_dev_rec->sec_rec.classic_link)),
             p_dev_rec->sec_rec.classic_link);
...
static void btm_sec_auth_timer_timeout(void* data) {
  ...
  } else if (p_dev_rec->sec_rec.classic_link == tSECURITY_STATE::BTM_SEC_STATE_AUTHENTICATING) {
