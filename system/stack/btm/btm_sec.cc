...
tBTM_STATUS btm_sec_bond_by_transport(const RawAddress& bd_addr,
  ...
  BTM_TRACE_DEBUG("sec mode: %d sm4:x%x", btm_sec_cb.security_mode, p_dev_rec->sm4);
  if (!controller_get_interface()->supports_simple_pairing() ||
...
uint8_t BTM_GetSecurityMode() {
  return btm_sec_cb.security_mode;
}
...
void btm_sec_dev_reset(void) {
  ...
  BTM_TRACE_DEBUG("btm_sec_dev_reset sec mode: %d", btm_sec_cb.security_mode);
}
...
void btm_sec_rmt_name_request_complete(const RawAddress* p_bd_addr,
  ...
  if ((!p_bd_addr &&
       !BTM_IsAclConnectionUp(btm_sec_cb.connecting_bda, BT_TRANSPORT_BR_EDR)) ||
   ...
   * now */
  if ((btm_sec_cb.pairing_state == BTM_PAIR_STATE_WAIT_LOCAL_PIN) && p_bd_addr &&
...
void btm_io_capabilities_req(const RawAddress& p) {
  ...
  BTM_TRACE_DEBUG("%s:Security mode: %d", __func__, btm_sec_cb.security_mode);
...
void btm_simple_pair_complete(const uint8_t* p) {
  ...
  BTM_TRACE_EVENT(
      ...
      btm_pair_state_descr(btm_sec_cb.pairing_state), status, p_dev_rec->sec_state);
  ...
  */
  ...
  if (btm_sec_cb.api.p_auth_complete_callback) {
    ...
    if ((old_state != BTM_PAIR_STATE_IDLE) || (status != HCI_SUCCESS))
      (*btm_sec_cb.api.p_auth_complete_callback)(p_dev_rec->bd_addr,
                                             p_dev_rec->dev_class,
                                             p_dev_rec->sec_bd_name, status);
  }
  ...
  switch (p_cb->pairing_state) {
    ...
    case BTM_PAIR_STATE_WAIT_LOCAL_PIN:
      ...
      if (btm_sec_cb.api.p_auth_complete_callback) {
        if (p_dev_rec == NULL) {
          name[0] = 0;
          (*btm_sec_cb.api.p_auth_complete_callback)(p_cb->pairing_bda, NULL, name,
    ...
    case BTM_PAIR_STATE_WAIT_LOCAL_IOCAPS:
      // TODO(optedoblivion): Inject OOB_DATA_PRESENT Flag
      btsnd_hcic_io_cap_req_reply(p_cb->pairing_bda, btm_sec_cb.devcb.loc_io_caps,
...
void btm_sec_pin_code_request(const uint8_t* p_event) {
  ...
  LOG_DEBUG("Controller requests PIN code device:%s state:%s",
            ADDRESS_TO_LOGGABLE_CSTR(p_bda), btm_pair_state_descr(btm_sec_cb.pairing_state));
...
tBTM_SEC_SERV_REC* btm_sec_find_first_serv(bool is_originator, uint16_t psm) {
  ...
  if (is_originator && btm_sec_cb.p_out_serv && btm_sec_cb.p_out_serv->psm == psm) {
...
static void btm_sec_change_pairing_state(tBTM_PAIRING_STATE new_state) {
  ...
  if (btm_sec_cb.pairing_state != new_state) {
    BTM_LogHistory(kBtmLogTag, btm_sec_cb.pairing_bda, "Pairing state changed",
                   base::StringPrintf(
                       "%s => %s", btm_pair_state_descr(btm_sec_cb.pairing_state),
