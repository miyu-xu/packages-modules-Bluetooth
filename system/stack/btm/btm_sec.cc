...
static void btm_sec_rmt_host_support_feat_evt(
    const bluetooth::inquiry::RemoteHostSupportedFeaturesResult& result);

static void btm_sec_rmt_name_request_complete(
 ...
 *
 * Description      check device is authenticated by using 16 digit pin or
 *MITM
 *
 * Returns          bool    true or false
 *
 ...
 *
 * Description      Get security flags for the device on a particular
 *transport
 *
...
tBTM_STATUS btm_sec_bond_by_transport(const RawAddress& bd_addr,
  ...
  if ((btm_cb.security_mode == BTM_SEC_MODE_SP ||
    ...
    if ((p_dev_rec->sm4 & BTM_SM4_CONN_PEND) == 0) {
      ...
      status = BTM_ReadRemoteDeviceName(
          bd_addr,
          bluetooth::inquiry::RemoteNameRequestCallbacks::forFeaturesAndName(
              btm_sec_rmt_host_support_feat_evt,
              btm_sec_rmt_name_request_complete),
          BT_TRANSPORT_BR_EDR, &btm_cb.pending_remname_handle);
    } else {
  ...
  if ((btm_cb.pairing_state != BTM_PAIR_STATE_IDLE) &&
    ...
    } else /*HCI link is not up */
    {
      ...
      if (btm_cb.pairing_state == BTM_PAIR_STATE_GET_REM_NAME) {
        BTM_CancelRemoteDeviceName(btm_cb.pending_remname_handle);
...
static void btm_sec_rmt_name_request_complete(
    const bluetooth::inquiry::RemoteNameRequestResult& result) {
  const RawAddress* p_bd_addr = &result.bd_addr;
  const uint8_t* p_bd_name = (uint8_t*)result.remote_bd_name;
  tHCI_STATUS status = result.hci_status;
  ...
  tBTM_SEC_DEV_REC* p_dev_rec = btm_find_dev(*p_bd_addr);
  ...
  if (btm_cb.pairing_state == BTM_PAIR_STATE_GET_REM_NAME) {
    if (p_bd_addr && btm_cb.pairing_bda == *p_bd_addr) {
      ...
      BTM_TRACE_ERROR("%s: wrong BDA, remote name request failed", __func__);
      NotifyBondingChange(*p_dev_rec, HCI_ERR_UNSPECIFIED);
    };
    return;
  }
        ...
        if ((status != HCI_SUCCESS) &&
          ...
          if (p_dev_rec->sec_flags & BTM_SEC_NAME_KNOWN) {
            ...
            if (BTM_ReadRemoteDeviceName(
                    p_dev_rec->bd_addr,
                    bluetooth::inquiry::RemoteNameRequestCallbacks::
                        forFeaturesAndName(btm_sec_rmt_host_support_feat_evt,
                                           btm_sec_rmt_name_request_complete),
                    BT_TRANSPORT_BR_EDR,
                    &btm_cb.pending_remname_handle) != BTM_CMD_STARTED) {
...
static bool btm_sec_start_get_name(tBTM_SEC_DEV_REC* p_dev_rec) {
  ...
  BTM_ReadRemoteDeviceName(
      p_dev_rec->bd_addr,
      bluetooth::inquiry::RemoteNameRequestCallbacks::forFeaturesAndName(
          btm_sec_rmt_host_support_feat_evt, btm_sec_rmt_name_request_complete),
      BT_TRANSPORT_BR_EDR, &btm_cb.pending_remname_handle);
  return true;
}
