...
bool BTM_SecRegister(const tBTM_APPL_INFO* p_cb_info) {
  LOG_DEBUG("%s application registered", __func__);

  LOG_INFO("%s p_cb_info->p_le_callback == 0x%p", __func__,
           p_cb_info->p_le_callback);
  if (p_cb_info->p_le_callback) {
    LOG_DEBUG("%s SMP_Register( btm_proc_smp_cback )", __func__);
...
bool BTM_SetSecurityLevel(bool is_originator, const char* p_name,
  ...
  LOG_DEBUG("%s : sec: 0x%x", __func__, sec_level);
...
uint8_t BTM_SecClrService(uint8_t service_id) {
  ...
  for (i = 0; i < BTM_SEC_MAX_SERVICE_RECORDS; i++, p_srec++) {
    ...
    if ((p_srec->security_flags & BTM_SEC_IN_USE) &&
      ...
      LOG_DEBUG("BTM_SEC_CLR[%d]: id %d", i, service_id);
...
uint8_t BTM_SecClrServiceByPsm(uint16_t psm) {
  ...
  for (i = 0; i < BTM_SEC_MAX_SERVICE_RECORDS; i++, p_srec++) {
    ...
    if ((p_srec->security_flags & BTM_SEC_IN_USE) && (p_srec->psm == psm)) {
      LOG_DEBUG("BTM_SEC_CLR[%d]: id %d ", i, p_srec->service_id);
      p_srec->security_flags = 0;
      num_freed++;
    }
  }
  LOG_DEBUG("BTM_SecClrServiceByPsm psm:0x%x num_freed:%d", psm, num_freed);
...
void BTM_PINCodeReply(const RawAddress& bd_addr, tBTM_STATUS res,
  ...
  LOG_DEBUG("BTM_PINCodeReply(): PairState: %s   PairFlags: 0x%02x  PinLen:%d  "
            "Result:%d",
...
tBTM_STATUS btm_sec_bond_by_transport(const RawAddress& bd_addr,
  ...
  LOG_DEBUG("BTM_SecBond: Remote sm4: 0x%x  HCI Handle: 0x%04x", p_dev_rec->sm4,
  ...
  LOG_VERBOSE("sec mode: %d sm4:x%x", btm_cb.security_mode, p_dev_rec->sm4);
  if (!controller_get_interface()->supports_simple_pairing() ||
...
tBTM_STATUS BTM_SecBondCancel(const RawAddress& bd_addr) {
  ...
  LOG_DEBUG("BTM_SecBondCancel()  State: %s flags:0x%x",
            btm_pair_state_descr(btm_cb.pairing_state), btm_cb.pairing_flags);
  p_dev_rec = btm_find_dev(bd_addr);
...
void BTM_ConfirmReqReply(tBTM_STATUS res, const RawAddress& bd_addr) {
  LOG_DEBUG("BTM_ConfirmReqReply() State: %s  Res: %u",
...
void BTM_PasskeyReqReply(tBTM_STATUS res, const RawAddress& bd_addr,
                         uint32_t passkey) {
  LOG_DEBUG("BTM_PasskeyReqReply: State: %s  res:%d",
            btm_pair_state_descr(btm_cb.pairing_state), res);
...
void BTM_RemoteOobDataReply(tBTM_STATUS res, const RawAddress& bd_addr,
  ...
  LOG_DEBUG("%s() - State: %s res: %d", __func__,
            btm_pair_state_descr(btm_cb.pairing_state), res);
...
static bool btm_sec_is_upgrade_possible(tBTM_SEC_DEV_REC* p_dev_rec,
  ...
  LOG_VERBOSE("%s() is_possible: %d sec_flags: 0x%x", __func__, is_possible,
              p_dev_rec->sec_flags);
  return is_possible;
}
...
tBTM_STATUS btm_sec_l2cap_access_req_by_requirement(
  ...
  LOG_VERBOSE("%s()  sm4:0x%x, sec_flags:0x%x, security_required:0x%x chk:%d",
  ...
  if (BTM_SEC_IS_SM4(p_dev_rec->sm4)) {
    if ((p_dev_rec->security_required & BTM_SEC_MODE4_LEVEL4) &&
      ...
      LOG_VERBOSE("%s: sec_flags:0x%x", __func__, p_dev_rec->sec_flags);
    } else {
  ...
  if (rc != BTM_CMD_STARTED) {
    LOG_VERBOSE("%s: p_dev_rec=%p, clearing callback. old p_callback=%p",
...
void btm_sec_conn_req(const RawAddress& bda, uint8_t* dc) {
   ...
   * sequence */
  if (!controller_get_interface()->get_is_ready()) {
    LOG_DEBUG("Security Manager: connect request when device not ready");
  ...
  if ((btm_cb.pairing_state != BTM_PAIR_STATE_IDLE) &&
    ...
    LOG_DEBUG("Security Manager: reject connect request from bonding device");
...
void btm_create_conn_cancel_complete(const uint8_t* p, uint16_t evt_len) {
  ...
  LOG_DEBUG("btm_create_conn_cancel_complete(): in State: %s  status:%d",
  ...
  if (btm_cb.pairing_state == BTM_PAIR_STATE_IDLE) {
    ...
    while ((p_e = (tBTM_SEC_QUEUE_ENTRY*)fixed_queue_try_dequeue(bq)) != NULL) {
      ...
      if (BTM_IsAclConnectionUp(p_e->bd_addr, p_e->transport)) {
        if (p_e->psm != 0) {
          LOG_DEBUG("%s PSM:0x%04x Is_Orig:%u", __func__, p_e->psm,
                    p_e->is_orig);
...
void btm_sec_dev_reset(void) {
  ...
  LOG_VERBOSE("btm_sec_dev_reset sec mode: %d", btm_cb.security_mode);
}
...
void btm_sec_abort_access_req(const RawAddress& bd_addr) {
  ...
  LOG_VERBOSE("%s: clearing callback. p_dev_rec=%p, p_callback=%p", __func__,
...
void btm_sec_rmt_name_request_complete(const RawAddress* p_bd_addr,
  ...
  if (p_dev_rec != nullptr) {
    ...
    if (status == HCI_SUCCESS) {
      ...
      p_dev_rec->sec_flags |= BTM_SEC_NAME_KNOWN;
      LOG_DEBUG("setting BTM_SEC_NAME_KNOWN sec_flags:0x%x",
                p_dev_rec->sec_flags);
    } else {
      LOG_WARN(
  ...
  if ((btm_cb.pairing_state == BTM_PAIR_STATE_WAIT_LOCAL_PIN) && p_bd_addr &&
    ...
    LOG_DEBUG("%s() delayed pin now being requested flags:0x%x, "
              "(p_pin_callback=0x%p)",
    ...
    if ((btm_cb.pairing_flags & BTM_PAIR_FLAGS_PIN_REQD) == 0 &&
      ...
      LOG_DEBUG("%s() calling pin_callback", __func__);
  ...
  if (btm_cb.pairing_state == BTM_PAIR_STATE_GET_REM_NAME) {
    if (p_bd_addr && btm_cb.pairing_bda == *p_bd_addr) {
      LOG_DEBUG("%s() continue bonding sm4: 0x%04x, status:0x%x", __func__,
      ...
      if (BTM_SEC_IS_SM4_UNKNOWN(p_dev_rec->sm4)) {
        ...
        LOG_VERBOSE("%s IS_SM4_UNKNOWN Flags:0x%04x", __func__,
                    btm_cb.pairing_flags);
        if ((btm_cb.pairing_flags & BTM_PAIR_FLAGS_REJECTED_CONNECT) == 0)
          p_dev_rec->sm4 |= BTM_SM4_KNOWN;
      }

      LOG_VERBOSE("%s, SM4 Value: %x, Legacy:%d,IS SM4:%d, Unknown:%d",
  ...
  if (p_dev_rec->sm4 & BTM_SM4_REQ_PEND) {
    LOG_DEBUG("waiting for remote features!!");
    return;
  }
...
void btm_sec_rmt_host_support_feat_evt(const uint8_t* p) {
  ...
  LOG_DEBUG("btm_sec_rmt_host_support_feat_evt  sm4: 0x%x  p[0]: 0x%x",
            p_dev_rec->sm4, p[0]);
  ...
  if (BTM_SEC_IS_SM4_UNKNOWN(p_dev_rec->sm4)) {
    ...
    LOG_DEBUG("btm_sec_rmt_host_support_feat_evt sm4: 0x%x features[0]: 0x%x",
...
void btm_io_capabilities_req(const RawAddress& p) {
  ...
  if ((btm_cb.security_mode == BTM_SEC_MODE_SC) &&
    ...
    LOG_DEBUG("%s: Device security mode is SC only."
  ...
  LOG_VERBOSE("%s:Security mode: %d", __func__, btm_cb.security_mode);

  p_dev_rec->sm4 |= BTM_SM4_TRUE;

  LOG_DEBUG("%s: State: %s  Flags: 0x%04x", __func__,
            btm_pair_state_descr(btm_cb.pairing_state), btm_cb.pairing_flags);
  ...
  if (btm_cb.pairing_disabled) {
    ...
    LOG_VERBOSE("%s: Pairing is not allowed -> fail pairing.", __func__);
  ...
  if (btm_cb.security_mode == BTM_SEC_MODE_SC) {
    ...
    LOG_VERBOSE("%s: for device in \"SC only\" mode set auth_req to 0x%02x",
  ...
  LOG_DEBUG("%s: State: %s  IO_CAP:%d oob_data:%d auth_req:%d", __func__,
...
void btm_read_local_oob_complete(uint8_t* p, uint16_t evt_len) {
  ...
  LOG_DEBUG("btm_read_local_oob_complete:%d", status);
  if (status == HCI_SUCCESS) {
...
static bool btm_sec_auth_retry(uint16_t handle, uint8_t status) {
  ...
  if ((btm_cb.pairing_state == BTM_PAIR_STATE_IDLE) &&
    ...
    LOG_VERBOSE("%s Retry for missing key sm4:x%x sec_flags:0x%x", __func__,
...
void btm_sec_encrypt_change(uint16_t handle, tHCI_STATUS status,
  ...
  if ((status == HCI_SUCCESS) && encr_enable &&
    ...
    if (p_dev_rec->rmt_auth_req == BTM_AUTH_SP_NO &&
      ...
      derive_ltk = false;
      LOG_VERBOSE("%s: BR key is temporary, skip derivation of LE LTK",
                  __func__);
    }
    tHCI_ROLE role = HCI_ROLE_UNKNOWN;
    BTM_GetRole(p_dev_rec->bd_addr, &role);
    if (p_dev_rec->new_encryption_key_is_p256) {
      if (btm_sec_use_smp_br_chnl(p_dev_rec) && role == HCI_ROLE_CENTRAL &&
        ...
        LOG_VERBOSE("%s start SM over BR/EDR", __func__);
  ...
  if (!p_dev_rec->is_security_state_bredr_encrypting()) {
    if (BTM_SEC_STATE_DELAY_FOR_ENC == p_dev_rec->sec_state) {
      p_dev_rec->sec_state = BTM_SEC_STATE_IDLE;
      LOG_VERBOSE("%s: clearing callback. p_dev_rec=%p, p_callback=%p",
                  __func__, p_dev_rec, p_dev_rec->p_callback);
      p_dev_rec->p_callback = NULL;
...
void btm_sec_link_key_notification(const RawAddress& p_bda,
  ...
  if (ltk_derived_lk) {
    if (btm_cb.api.p_link_key_callback) {
      LOG_VERBOSE("%s() Save LTK derived LK (key_type = %d)", __func__,
    ...
    if ((p_dev_rec->link_key_type == BTM_LKEY_TYPE_UNAUTH_COMB_P_256) ||
      ...
      LOG_VERBOSE("%s set new_encr_key_256 to %d", __func__,
  ...
  if ((!(p_dev_rec->sec_flags & BTM_SEC_NAME_KNOWN) &&
    ...
    LOG_DEBUG("rmt_io_caps:%d, sec_flags:x%x, dev_class[1]:x%02x",
...
void btm_sec_link_key_request(const uint8_t* p_event) {
  ...
  if ((btm_cb.pairing_state == BTM_PAIR_STATE_WAIT_PIN_REQ) &&
    ...
    LOG_DEBUG("btm_sec_link_key_request() rejecting link key req "
...
static void btm_sec_pairing_timeout(UNUSED_ATTR void* data) {
  ...
  LOG_DEBUG("%s  State: %s   Flags: %u", __func__,
...
void btm_sec_pin_code_request(const uint8_t* p_event) {
  ...
  if (!p_cb->pairing_disabled && (p_cb->cfg.pin_type == HCI_PIN_TYPE_FIXED)) {
    LOG_DEBUG("btm_sec_pin_code_request fixed pin replying");
    btm_sec_change_pairing_state(BTM_PAIR_STATE_WAIT_AUTH_COMPLETE);
  ...
  if (btm_cb.pin_code_len != 0) {
    LOG_DEBUG("btm_sec_pin_code_request bonding sending reply");
  ...
  /* Notify upper layer of PIN request and start expiration timer */
     ...
     * completed */
    if (p_dev_rec->sec_flags & BTM_SEC_NAME_KNOWN) {
      LOG_DEBUG("btm_sec_pin_code_request going for callback");
      ...
      LOG_DEBUG("btm_sec_pin_code_request going for remote name");
...
tBTM_STATUS btm_sec_execute_procedure(tBTM_SEC_DEV_REC* p_dev_rec) {
  ...
  if (!(p_dev_rec->sec_flags & BTM_SEC_ENCRYPTED) &&
      ...
      (p_dev_rec->hci_handle != HCI_INVALID_HANDLE)) {
    LOG_DEBUG("Security Manager: Start encryption");
  ...
  LOG_DEBUG("Security Manager: access granted");
...
static bool btm_sec_queue_mx_request(const RawAddress& bd_addr, uint16_t psm,
  ...
  LOG_DEBUG("%s() PSM: 0x%04x  Is_Orig: %u  security_required: 0x%x", __func__,
...
static bool btm_sec_check_prefetch_pin(tBTM_SEC_DEV_REC* p_dev_rec) {
  ...
  if ((major == BTM_COD_MAJOR_AUDIO) &&
    ...
    LOG_DEBUG("%s() Skipping pre-fetch PIN for carkit COD Major: 0x%02x Minor: "
              "0x%02x",
    ...
    if (btm_cb.pin_code_len) {
      ...
      if (btm_cb.api.p_pin_callback &&
        ...
        LOG_VERBOSE("%s() PIN code callback called", __func__);
