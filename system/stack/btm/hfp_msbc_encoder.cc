...
bool hfp_msbc_encoder_init(void) {
  if (hfp_msbc_encoder) {
    LOG_WARN("%s: Trying to init the encoder when there is a exiting one",
             __func__);
    hfp_msbc_encoder_cleanup();
  }

  hfp_msbc_encoder = (tHFP_MSBC_ENCODER*)calloc(1, sizeof(tHFP_MSBC_ENCODER));
  if (hfp_msbc_encoder == NULL) {
    LOG_ERROR("%s: Out of memory when allocating the encoder", __func__);
    return false;
  }
