...
#define ISO_LINK_QUALITY_SIZE 31
  ...
  BT_HDR* prepare_ts_hci_packet(uint16_t iso_handle,
    ...
    UINT32_TO_STREAM(packet_data, 0);
  ...
  void send_iso_data(uint16_t iso_handle, const uint8_t* data,
     ...
     */
    iso->sync_info.seq_nb = (iso->sync_info.seq_nb + 1) & 0xffff;
    ...
    BT_HDR* packet =
        prepare_ts_hci_packet(iso_handle, iso->sync_info.seq_nb, data_len);
  ...
  void handle_iso_data(BT_HDR* p_msg) {
    ...
    uint16_t new_calc_seq_nb = (iso->sync_info.seq_nb + 1) & 0xffff;

    evt.evt_lost = ((1 << 16) + new_calc_seq_nb - iso->sync_info.seq_nb) & 0xffff;
    if (evt.evt_lost > 0) {
