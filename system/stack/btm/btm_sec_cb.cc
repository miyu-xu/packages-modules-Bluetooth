...
bool tBTM_SEC_CB::querySecFlag(const RawAddress bd_addr,
                               tBT_TRANSPORT transport,
                               FlagQueryMethod bredr_mothod,
                               FlagQueryMethod ble_method) {
  tBTM_SEC_DEV_REC* p_dev_rec;
  tBTM_SEC_REC* sec_rec;
  p_dev_rec = btm_find_dev(bd_addr);
  if (p_dev_rec) {
    sec_rec = &p_dev_rec->sec_rec;
    if (transport == BT_TRANSPORT_BR_EDR) {
      return (sec_rec->*bredr_mothod)();
    } else if (transport == BT_TRANSPORT_LE) {
      return (sec_rec->*ble_method)();
    }
...
bool tBTM_SEC_CB::IsDeviceEncrypted(const RawAddress bd_addr,
                                    tBT_TRANSPORT transport) {
  return querySecFlag(bd_addr, transport, &tBTM_SEC_REC::is_device_encrypted,
                      &tBTM_SEC_REC::is_le_device_encrypted);
}

bool tBTM_SEC_CB::IsLinkKeyAuthenticated(const RawAddress bd_addr,
                                         tBT_TRANSPORT transport) {
  return querySecFlag(bd_addr, transport,
                      &tBTM_SEC_REC::is_link_key_authenticated,
                      &tBTM_SEC_REC::is_le_link_key_authenticated);
}

bool tBTM_SEC_CB::IsDeviceAuthenticated(const RawAddress bd_addr,
                                        tBT_TRANSPORT transport) {
  return querySecFlag(bd_addr, transport,
                      &tBTM_SEC_REC::is_device_authenticated,
                      &tBTM_SEC_REC::is_le_device_authenticated);
}

bool tBTM_SEC_CB::IsLinkKeyKnown(const RawAddress bd_addr,
                                 tBT_TRANSPORT transport) {
  return querySecFlag(bd_addr, transport, &tBTM_SEC_REC::is_link_key_known,
                      &tBTM_SEC_REC::is_le_link_key_known);
}

#define BTM_NO_AVAIL_SEC_SERVICES ((uint16_t)0xffff)
bool tBTM_SEC_CB::SetServiceSecurityLevel(bool is_originator,
                                          const char* p_name,
                                          uint8_t service_id,
