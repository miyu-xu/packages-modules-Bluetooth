...
void l2c_fcr_send_S_frame(tL2C_CCB* p_ccb, uint16_t function_code,
  ...
  if ((((ctrl_word & L2CAP_FCR_SUP_BITS) >> L2CAP_FCR_SUP_SHIFT) == 1) ||
    ...
    LOG_WARN("L2CAP eRTM Tx S-frame  CID: 0x%04x  ctrlword: 0x%04x  Type: %s  "
             "ReqSeq: %u  P: %u  F: %u",
             p_ccb->local_cid, ctrl_word,
             SUP_types[(ctrl_word & L2CAP_FCR_SUP_BITS) >> L2CAP_FCR_SUP_SHIFT],
             (ctrl_word & L2CAP_FCR_REQ_SEQ_BITS) >>
    ...
    if (num_lost >= p_ccb->our_cfg.fcr.tx_win_sz) {
      ...
      if (p_fcrb->srej_sent) {
        ...
        if ((tx_seq == next_srej) &&
          ...
          LOG_WARN("process_i_frame() CID: 0x%04x  frame dropped in Srej Sent "
        ...
        if (num_lost > 1) {
          ...
          if (!fixed_queue_is_empty(p_fcrb->srej_rcv_hold_q)) {
            LOG_ERROR("process_i_frame() CID: 0x%04x  sending SREJ tx_seq:%d "
                      "hold_q.count:%zu",
