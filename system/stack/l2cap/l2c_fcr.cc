...
void l2c_fcr_send_S_frame(tL2C_CCB* p_ccb, uint16_t function_code,
  ...
  if ((((ctrl_word & L2CAP_FCR_SUP_BITS) >> L2CAP_FCR_SUP_SHIFT) == 1) ||
    ...
    L2LOG_WARN(
        ...
        (ctrl_word & L2CAP_FCR_F_BIT) >> L2CAP_FCR_F_BIT_SHIFT);
    L2LOG_WARN("                  Buf Len: %u", p_buf->len);
  } else {
    L2LOG_INFO(
        "L2CAP eRTM Tx S-frame  CID: 0x%04x  ctrlword: 0x%04x  Type: %s  "
        "ReqSeq: %u  P: %u  F: %u",
        ...
        (ctrl_word & L2CAP_FCR_P_BIT) >> L2CAP_FCR_P_BIT_SHIFT,
        (ctrl_word & L2CAP_FCR_F_BIT) >> L2CAP_FCR_F_BIT_SHIFT);
    L2LOG_INFO("                  Buf Len: %u", p_buf->len);
  }
...
void l2c_fcr_proc_pdu(tL2C_CCB* p_ccb, BT_HDR* p_buf) {
  ...
  if (p_buf->len < min_pdu_len) {
    L2LOG_WARN("Rx L2CAP PDU: CID: 0x%04x  Len too short: %u", p_ccb->local_cid,
               p_buf->len);
    osi_free(p_buf);
    return;
  }
  ...
  if (l2c_fcr_rx_get_fcs(p_buf) != fcs) {
    L2LOG_WARN("Rx L2CAP PDU: CID: 0x%04x  BAD FCS", p_ccb->local_cid);
    osi_free(p_buf);
    return;
  }
...
void l2c_lcc_proc_pdu(tL2C_CCB* p_ccb, BT_HDR* p_buf) {
  ...
  if (p_ccb->is_first_seg) {
    if (p_buf->len < sizeof(sdu_length)) {
      L2LOG_ERROR("%s: buffer length=%d too small. Need at least 2.", __func__,
                  p_buf->len);
      /* Discard the buffer */
      osi_free(p_buf);
      return;
    ...
    if (sdu_length < p_buf->len) {
      L2LOG_ERROR("%s: Invalid sdu_length: %d", __func__, sdu_length);
      /* Discard the buffer */
      osi_free(p_buf);
      return;
    }
    ...
    L2LOG_DEBUG("%s SDU Length = %d", __func__, sdu_length);
    p_data->offset = 0;
    ...
    if (p_buf->len > (p_ccb->ble_sdu_length - p_data->len)) {
      L2LOG_ERROR("%s: buffer length=%d too big. max=%d. Dropped", __func__,
...
static void process_s_frame(tL2C_CCB* p_ccb, BT_HDR* p_buf,
  ...
  if (p_buf->len != 0) {
    L2LOG_WARN("Incorrect S-frame Length (%d)", p_buf->len);
  }

  L2LOG_DEBUG("process_s_frame ctrl_word 0x%04x fcrb_remote_busy:%d", ctrl_word,
              p_fcrb->remote_busy);
  ...
  if (all_ok) {
    ...
    L2LOG_DEBUG("process_s_frame hit_max_retries");
  }
...
static void process_i_frame(tL2C_CCB* p_ccb, BT_HDR* p_buf, uint16_t ctrl_word,
      ...
      if (p_fcrb->srej_sent) {
        ...
        if (num_lost > 1) {
          ...
          if (!fixed_queue_is_empty(p_fcrb->srej_rcv_hold_q)) {
            L2LOG_ERROR("process_i_frame() CID: 0x%04x  sending SREJ tx_seq:%d "
                        "hold_q.count:%zu",
  ...
  if (!do_sar_reassembly(p_ccb, p_buf, ctrl_word)) {
    L2LOG_WARN("process_i_frame() CID: 0x%04x  reassembly failed",
               p_ccb->local_cid);
    l2cu_disconnect_chnl(p_ccb);
    return;
  }
...
static bool do_sar_reassembly(tL2C_CCB* p_ccb, BT_HDR* p_buf,
  ...
  if ((sar_type == L2CAP_FCR_UNSEG_SDU) || (sar_type == L2CAP_FCR_START_SDU)) {
    ...
    if ((sar_type == L2CAP_FCR_START_SDU) &&
      ...
      L2LOG_WARN("SAR start packet too short: %u", p_buf->len);
      packet_ok = false;
    }
  } else {
    if (p_fcrb->p_rx_sdu == NULL) {
      L2LOG_WARN("SAR - got unexpected cont or end SDU");
      packet_ok = false;
    }
  }
  ...
  if ((packet_ok) && (sar_type != L2CAP_FCR_UNSEG_SDU)) {
    ...
    if (sar_type == L2CAP_FCR_START_SDU) {
      ...
      if (p_fcrb->rx_sdu_len > p_ccb->max_rx_mtu) {
        L2LOG_WARN("SAR - SDU len: %u  larger than MTU: %u", p_fcrb->rx_sdu_len,
                   p_ccb->max_rx_mtu);
        packet_ok = false;
      } else {
    ...
    */
    if (list_ack != NULL) {
      for (; node_ack != list_end(list_ack); node_ack = list_next(node_ack)) {
        ...
        L2LOG_DEBUG("retransmit_i_frames()   cur seq: %u  looking for: %u",
                    buf_seq, tx_seq);
    ...
    if (!p_buf) {
      L2LOG_ERROR("retransmit_i_frames() UNKNOWN seq: %u  q_count: %zu", tx_seq,
                  fixed_queue_length(p_ccb->fcrb.waiting_for_ack_q));
      return (true);
    }
  } else {
...
BT_HDR* l2c_fcr_get_next_xmit_sdu_seg(tL2C_CCB* p_ccb,
  ...
  */
  ...
  if (p_buf->len > max_pdu) {
    ...
    if (p_xmit != NULL) {
      ...
      p_xmit->layer_specific = p_buf->layer_specific;
              ...
              correctly */
    {
      L2LOG_ERROR("L2CAP - cannot get buffer for segmentation, max_pdu: %u",
                  max_pdu);
      return (NULL);
    }
  ...
  if (p_ccb->peer_cfg.fcr.mode == L2CAP_FCR_ERTM_MODE) {
    ...
    if (!p_wack) {
      L2LOG_ERROR("L2CAP - no buffer for xmit cloning, CID: 0x%04x  Length: %u",
...
uint8_t l2c_fcr_chk_chan_modes(tL2C_CCB* p_ccb) {
  ...
  if (!(p_ccb->p_lcb->peer_ext_fea & L2CAP_EXTFEA_ENH_RETRANS) &&
    ...
    L2LOG_WARN("L2CAP - Peer does not support our desired channel types");
...
void l2c_fcr_adj_our_rsp_options(tL2C_CCB* p_ccb, tL2CAP_CFG_INFO* p_cfg) {
  ...
  if (p_cfg->fcr_present) {
    ...
    if (p_ccb->peer_cfg.fcr.tx_win_sz > p_ccb->our_cfg.fcr.tx_win_sz) {
      L2LOG_DEBUG("%s: adjusting requested tx_win_sz from %i to %i", __func__,
...
bool l2c_fcr_renegotiate_chan(tL2C_CCB* p_ccb, tL2CAP_CFG_INFO* p_cfg) {
  ...
  if (p_cfg->result == L2CAP_CFG_UNACCEPTABLE_PARAMS) {
    ...
    if (p_ccb->our_cfg.fcr.mode != peer_mode) {
      if ((--p_ccb->fcr_cfg_tries) == 0) {
        ...
        L2LOG_WARN("l2c_fcr_renegotiate_chan (Max retries exceeded)");
      }
      ...
      switch (p_ccb->our_cfg.fcr.mode) {
        case L2CAP_FCR_ERTM_MODE:
           ...
           * supported */
          L2LOG_DEBUG("%s(Trying Basic)", __func__);
  ...
  if (p_ccb->our_cfg.fcr.mode != peer_mode) {
    L2LOG_WARN("L2C CFG:  Channels incompatible (local %d, peer %d)",
  ...
  if (fcr_ok == L2CAP_PEER_CFG_OK) {
    ...
    if (p_cfg->fcr_present && p_cfg->fcr.mode != L2CAP_FCR_BASIC_MODE) {
      ...
      if (p_cfg->fcr.mps > max_retrans_size) {
        L2LOG_DEBUG("CFG: Overriding MPS to %d (orig %d)", max_retrans_size,
                    p_cfg->fcr.mps);
