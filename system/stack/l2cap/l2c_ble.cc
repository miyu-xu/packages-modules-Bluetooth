...
void l2cble_process_sig_cmd(tL2C_LCB* p_lcb, uint8_t* p, uint16_t pkt_len) {
  ...
  switch (cmd_code) {
      ...
      for (int i = 0; i < num_of_channels; i++) {
        ...
        if (rcid != 0) {
          ...
          if (temp_p_ccb != nullptr) {
            LOG_ERROR("Already Allocated Destination cid. "
                      "rcid = %d "
                      "send peer_disc_req",
                      rcid);
    ...
    case L2CAP_CMD_CREDIT_BASED_RECONFIG_REQ: {
      ...
      LOG_VERBOSE("Recv L2CAP_CMD_CREDIT_BASED_RECONFIG_REQ with "
                  "mtu = %d, "
                  "mps = %d, "
                  "num_of_channels = %d",
    ...
    case L2CAP_CMD_CREDIT_BASED_RECONFIG_RES: {
      ...
      LOG_VERBOSE("Recv L2CAP_CMD_CREDIT_BASED_RECONFIG_RES for "
                  "result = 0x%04x",
                  result);
    ...
    case L2CAP_CMD_BLE_CREDIT_BASED_CONN_REQ:
      ...
      LOG_VERBOSE("Recv L2CAP_CMD_BLE_CREDIT_BASED_CONN_REQ with "
                  "mtu = %d, "
                  "mps = %d, "
                  "initial credit = %d",
...
void l2c_ble_link_adjust_allocation(void) {
  ...
  LOG_DEBUG("l2c_ble_link_adjust_allocation  num_hipri: %u  num_lowpri: %u  "
...
void l2cble_process_rc_param_request_evt(uint16_t handle, uint16_t int_min,
  ...
  if (p_lcb != NULL) {
    ...
    if ((p_lcb->conn_update_mask & L2C_BLE_CONN_UPDATE_DISABLE) == 0) {
      ...
      LOG_DEBUG("L2CAP - LE - update currently disabled");
      p_lcb->conn_update_mask |= L2C_BLE_NEW_CONN_PARAM;
