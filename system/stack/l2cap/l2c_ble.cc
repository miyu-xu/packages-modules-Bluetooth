...
bool l2cble_conn_comp(uint16_t handle, tHCI_ROLE role, const RawAddress& bda,
  ...
  if (com::android::bluetooth::flags::initial_conn_params_p1()) {
    // TODO: Check the conn params belongs to adjusted aggressive params.
    uint16_t min_conn_interval_aggressive = get_min_conn_interval_aggressive();
    uint16_t max_conn_interval_aggressive = get_max_conn_interval_aggressive();
