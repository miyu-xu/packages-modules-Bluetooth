...
void l2cble_process_conn_update_evt(uint16_t handle, uint8_t status,
  ...
  if (!p_lcb) {
    L2LOG_WARN("%s: Invalid handle: %d", __func__, handle);
    return;
  }
  ...
  if (status != HCI_SUCCESS) {
    L2LOG_WARN("%s: Error status: %d", __func__, status);
  }
  ...
  L2LOG_DEBUG("%s: conn_update_mask=%d , subrate_req_mask=%d", __func__,
...
void l2cble_process_sig_cmd(tL2C_LCB* p_lcb, uint8_t* p, uint16_t pkt_len) {
  ...
  if ((p + cmd_len) > p_pkt_end) {
    L2LOG_WARN("L2CAP - LE - format error, pkt_len: %d  cmd_len: %d  code: %d",
  ...
  switch (cmd_code) {
    ...
    case L2CAP_CMD_CREDIT_BASED_CONN_RES:
      ...
      L2LOG_DEBUG("Recv L2CAP_CMD_CREDIT_BASED_CONN_RES");
      ...
      if (!p_ccb) {
        L2LOG_DEBUG(" Cannot find matching connection req");
      ...
      if (con_info.l2cap_result == L2CAP_LE_RESULT_NO_PSM ||
        ...
        L2LOG_ERROR("L2CAP - not accepted. Status %d", con_info.l2cap_result);
        l2cble_handle_connect_rsp_neg(p_lcb, &con_info);
        return;
      }
      ...
      if (mtu < L2CAP_CREDIT_BASED_MIN_MTU ||
          mps < L2CAP_CREDIT_BASED_MIN_MPS || mps > L2CAP_LE_MAX_MPS) {
        L2LOG_ERROR("L2CAP - invalid params");
        con_info.l2cap_result = L2CAP_LE_RESULT_INVALID_PARAMETERS;
        l2cble_handle_connect_rsp_neg(p_lcb, &con_info);
        return;
      }
      ...
      if (num_of_channels != p_lcb->pending_ecoc_conn_cnt) {
        L2LOG_ERROR("Incorrect response."
      ...
      for (int i = 0; i < num_of_channels; i++) {
        ...
        if (rcid != 0) {
          ...
          if (temp_p_ccb != nullptr) {
            L2LOG_ERROR("Already Allocated Destination cid. "
                        "rcid = %d "
                        "send peer_disc_req",
                        rcid);
    ...
    case L2CAP_CMD_CREDIT_BASED_RECONFIG_REQ: {
      ...
      L2LOG_DEBUG("Recv L2CAP_CMD_CREDIT_BASED_RECONFIG_REQ with "
                  "mtu = %d, "
                  "mps = %d, "
                  "num_of_channels = %d",
      ...
      for (int i = 0; i < num_of_channels; i++) {
        ...
        if (!p_ccb) {
          L2LOG_WARN("L2CAP - rcvd config req for non existing cid: 0x%04x",
                     rcid);
    ...
    case L2CAP_CMD_CREDIT_BASED_RECONFIG_RES: {
      ...
      L2LOG_DEBUG("Recv L2CAP_CMD_CREDIT_BASED_RECONFIG_RES for "
                  "result = 0x%04x",
                  result);
    ...
    case L2CAP_CMD_BLE_CREDIT_BASED_CONN_REQ:
      ...
      L2LOG_DEBUG("Recv L2CAP_CMD_BLE_CREDIT_BASED_CONN_REQ with "
                  "mtu = %d, "
                  "mps = %d, "
                  "initial credit = %d",
      ...
      p_ccb = l2cu_find_ccb_by_remote_cid(p_lcb, rcid);
      if (p_ccb) {
        L2LOG_WARN("L2CAP - rcvd conn req for duplicated cid: 0x%04x", rcid);
      ...
      p_rcb = l2cu_find_ble_rcb_by_psm(con_info.psm);
      if (p_rcb == NULL) {
        L2LOG_WARN("L2CAP - rcvd conn req for unknown PSM: 0x%04x",
                   con_info.psm);
      ...
      if (p_ccb == NULL) {
        L2LOG_ERROR("L2CAP - unable to allocate CCB");
      ...
      if (mtu < L2CAP_LE_MIN_MTU || mps < L2CAP_LE_MIN_MPS ||
        ...
        L2LOG_ERROR("L2CAP do not like the params");
        l2cu_reject_ble_connection(p_ccb, id, L2CAP_CONN_NO_RESOURCES);
        break;
      }
    ...
    case L2CAP_CMD_BLE_CREDIT_BASED_CONN_RES:
      L2LOG_DEBUG("Recv L2CAP_CMD_BLE_CREDIT_BASED_CONN_RES");
      /* For all channels, see whose identifier matches this id */
      for (temp_p_ccb = p_lcb->ccb_queue.p_first_ccb; temp_p_ccb;
           temp_p_ccb = temp_p_ccb->p_next_ccb) {
        if (temp_p_ccb->local_id == id) {
      ...
      if (p_ccb) {
        L2LOG_DEBUG("I remember the connection req");
        if (p + 10 > p_pkt_end) {
          LOG(ERROR) << "invalid read";
          return;
        }
        ...
        L2LOG_DEBUG("I DO NOT remember the connection req");
        con_info.l2cap_result = L2CAP_LE_RESULT_INVALID_SOURCE_CID;
        l2c_csm_execute(p_ccb, L2CEVT_L2CAP_CONNECT_RSP_NEG, &con_info);
      }
      break;
    ...
    case L2CAP_CMD_BLE_FLOW_CTRL_CREDIT:
      ...
      if (p_ccb == NULL) {
        L2LOG_DEBUG("%s Credit received for unknown channel id %d", __func__,
                    lcid);
        break;
      }
      ...
      L2LOG_DEBUG("%s Credit received", __func__);
      break;
    ...
    default:
      L2LOG_WARN("L2CAP - LE - unknown cmd code: %d", cmd_code);
      l2cu_send_peer_cmd_reject(p_lcb, L2CAP_CMD_REJ_NOT_UNDERSTOOD, id, 0, 0);
      break;
  }
}
...
void l2c_ble_link_adjust_allocation(void) {
  ...
  L2LOG_INFO("l2c_ble_link_adjust_allocation  num_hipri: %u  num_lowpri: %u  "
  ...
  for (yy = 0, p_lcb = &l2cb.lcb_pool[0]; yy < MAX_L2CAP_LINKS; yy++, p_lcb++) {
    if (p_lcb->in_use && p_lcb->transport == BT_TRANSPORT_LE) {
      ...
      L2LOG_INFO("        SentNotAcked: %d  RRUnacked: %d",
...
void l2cble_process_rc_param_request_evt(uint16_t handle, uint16_t int_min,
  ...
  if (p_lcb != NULL) {
    ...
    if ((p_lcb->conn_update_mask & L2C_BLE_CONN_UPDATE_DISABLE) == 0) {
      ...
      L2LOG_INFO("L2CAP - LE - update currently disabled");
      p_lcb->conn_update_mask |= L2C_BLE_NEW_CONN_PARAM;
    ...
    L2LOG_WARN("No link to update connection parameter")
  }
}
...
void l2cble_credit_based_conn_req(tL2C_CCB* p_ccb) {
  ...
  if (p_ccb->p_lcb && p_ccb->p_lcb->transport != BT_TRANSPORT_LE) {
    L2LOG_WARN("LE link doesn't exist");
    return;
  }
...
void l2cble_sec_comp(const RawAddress* bda, tBT_TRANSPORT transport,
  ...
  if (!p_lcb) {
    L2LOG_WARN("%s: security complete for unknown device. bda=%s", __func__,
  ...
  if (!fixed_queue_is_empty(p_lcb->le_sec_pending_q)) {
    ...
    if (!p_buf) {
      L2LOG_WARN("%s Security complete for request not initiated from L2CAP",
                 __func__);
      return;
    }
    ...
    if (status != BTM_SUCCESS) {
      ...
      if (sec_act == BTM_SEC_ENCRYPT_MITM) {
        if (BTM_IsLinkKeyAuthed(p_bda, transport))
          ...
          L2LOG_DEBUG("%s MITM Protection Not present", __func__);
          (*(p_buf->p_callback))(p_bda, BT_TRANSPORT_LE, p_buf->p_ref_data,
                                 BTM_FAILED_ON_SECURITY);
        }
      } else {
        L2LOG_DEBUG("%s MITM Protection not required sec_act = %d", __func__,
                    p_lcb->sec_act);
...
void L2CA_AdjustConnectionIntervals(uint16_t* min_interval,
  ...
  if (GetInterfaceToProfiles()
    ...
    L2LOG_DEBUG("%s: Have Hearing Aids. Min. interval is set to %d", __func__,
                phone_min_interval);
  }

  if (*min_interval < phone_min_interval) {
    L2LOG_DEBUG("%s: requested min_interval=%d too small. Set to %d", __func__,
  ...
  if (*max_interval < phone_min_interval) {
    L2LOG_DEBUG("%s: requested max_interval=%d too small. Set to %d", __func__,
...
static void l2cble_start_subrate_change(tL2C_LCB* p_lcb) {
  ...
  L2LOG_DEBUG("%s: subrate_req_mask=%d conn_update_mask=%d", __func__,
              p_lcb->subrate_req_mask, p_lcb->conn_update_mask);

  if (p_lcb->subrate_req_mask & L2C_BLE_SUBRATE_REQ_PENDING) {
    L2LOG_DEBUG("%s: returning L2C_BLE_SUBRATE_REQ_PENDING ", __func__);
    return;
  }

  if (p_lcb->subrate_req_mask & L2C_BLE_SUBRATE_REQ_DISABLE) {
    L2LOG_DEBUG("%s: returning L2C_BLE_SUBRATE_REQ_DISABLE ", __func__);
    return;
  }
  ...
  if (!(p_lcb->subrate_req_mask & L2C_BLE_NEW_SUBRATE_PARAM) ||
      (p_lcb->conn_update_mask & L2C_BLE_UPDATE_PENDING) ||
      (p_lcb->conn_update_mask & L2C_BLE_NEW_CONN_PARAM)) {
    L2LOG_DEBUG("%s: returning L2C_BLE_NEW_SUBRATE_PARAM", __func__);
    return;
  }
  ...
  L2LOG_DEBUG("%s: Sending HCI cmd for subrate req", __func__);
