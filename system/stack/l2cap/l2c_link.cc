...
static void l2c_link_send_to_lower_ble(tL2C_LCB* p_lcb, BT_HDR* p_buf) {
  ...
  p_lcb->sent_not_acked++;
  p_buf->layer_specific = 0;
  uint16_t max_le_acl_len = shim::GetController()->GetLeBufferSize().le_data_packet_length_;
  uint16_t num_of_frags = (p_buf->len / max_le_acl_len) + ((p_buf->len % max_acl_len) ? 1 : 0);
