  ...
  switch (cmd_code) {
    ...
    case L2CAP_CMD_BLE_UPDATE_REQ:
      l2cu_send_peer_cmd_reject(p_lcb, L2CAP_CMD_REJ_MTU_EXCEEDED, signal_id,
                                L2CAP_DEFAULT_MTU, 0);
      L2LOG_WARN("Dumping first Command (%d)", cmd_code);
      return true;
...
void l2cu_send_peer_cmd_reject(tL2C_LCB* p_lcb, uint16_t reason, uint8_t rem_id,
  ...
  if (p_buf == NULL) {
    L2LOG_WARN("L2CAP - no buffer cmd_rej");
    return;
  }
...
void l2cu_send_peer_connect_req(tL2C_CCB* p_ccb) {
  ...
  if (p_buf == NULL) {
    L2LOG_WARN("L2CAP - no buffer for conn_req");
    return;
  }
...
void l2cu_send_credit_based_reconfig_req(tL2C_CCB* p_ccb,
  ...
  p_buf = l2cu_build_header(p_lcb, cmd_len, L2CAP_CMD_CREDIT_BASED_RECONFIG_REQ,
                            p_lcb->signal_id);
  if (p_buf == NULL) {
    L2LOG_WARN("l2cu_send_reconfig_req - no buffer");
    return;
  }
  ...
  L2LOG_DEBUG("l2cu_send_reconfig_req number of cids: %d mtu:%d mps:%d",
...
void l2cu_send_peer_config_rej(tL2C_CCB* p_ccb, uint8_t* p_data,
  ...
  L2LOG_DEBUG("l2cu_send_peer_config_rej: data_len=%d, rej_len=%d", data_len,
              rej_len);
  ...
  len1 = 0xFFFF - len;
  if (rej_len > len1) {
    L2LOG_ERROR("L2CAP - cfg_rej pkt size exceeds buffer design max limit.");
    return;
  }
...
/* Put in HCI header - handle + pkt boundary */
        ...
        if ((cfg_len + L2CAP_CFG_OPTION_OVERHEAD) <= data_len) {
          if ((cfg_code & 0x80) == 0) {
            if (buf_space >= (cfg_len + L2CAP_CFG_OPTION_OVERHEAD)) {
              ...
              L2LOG_WARN("L2CAP - cfg_rej exceeds allocated buffer");
  ...
  L2LOG_DEBUG("L2CAP - cfg_rej pkt hci_len=%d, l2cap_len=%d", len,
...
void l2cu_send_peer_disc_req(tL2C_CCB* p_ccb) {
  ...
  if ((!p_ccb) || (p_ccb->p_lcb == NULL)) {
    L2LOG_ERROR("%s L2CAP - ccb or lcb invalid", __func__);
    return;
  }
  ...
  if (p_buf == NULL) {
    L2LOG_WARN("L2CAP - no buffer for disc_req");
    return;
  }
...
void l2cu_send_peer_disc_rsp(tL2C_LCB* p_lcb, uint8_t remote_id,
  ...
  p_buf = l2cu_build_header(p_lcb, L2CAP_DISC_RSP_LEN, L2CAP_CMD_DISC_RSP,
                            remote_id);
  if (p_buf == NULL) {
    L2LOG_WARN("L2CAP - no buffer for disc_rsp");
    return;
  }
...
void l2cu_send_peer_echo_rsp(tL2C_LCB* p_lcb, uint8_t signal_id,
  ...
  if (!signal_id || signal_id == p_lcb->cur_echo_id) {
    ...
    L2LOG_WARN("L2CAP ignoring duplicate echo request (%d)", signal_id);
    return;
  } else
  ...
  if (p_buf == NULL) {
    L2LOG_WARN("L2CAP - no buffer for echo_rsp");
    return;
  }
...
void l2cu_send_peer_info_req(tL2C_LCB* p_lcb, uint16_t info_type) {
  ...
  if (p_buf == NULL) {
    L2LOG_WARN("L2CAP - no buffer for info_req");
    return;
  }

  L2LOG_INFO("l2cu_send_peer_info_req: type 0x%04x", info_type);
...
void l2cu_send_peer_info_rsp(tL2C_LCB* p_lcb, uint8_t remote_id,
  ...
  if (p_buf == NULL) {
    L2LOG_WARN("L2CAP - no buffer for info_rsp");
    return;
  }
...
void l2cu_enqueue_ccb(tL2C_CCB* p_ccb) {
  ...
  */
  ...
  if ((!p_ccb->in_use) || (p_q == NULL)) {
    L2LOG_ERROR("%s: CID: 0x%04x ERROR in_use: %u  p_lcb: %p", __func__,
                p_ccb->local_cid, p_ccb->in_use, p_ccb->p_lcb);
    return;
  }

  L2LOG_DEBUG("l2cu_enqueue_ccb CID: 0x%04x  priority: %d", p_ccb->local_cid,
              p_ccb->ccb_priority);
...
void l2cu_dequeue_ccb(tL2C_CCB* p_ccb) {
  ...
  L2LOG_DEBUG("l2cu_dequeue_ccb  CID: 0x%04x", p_ccb->local_cid);
...
void l2cu_change_pri_ccb(tL2C_CCB* p_ccb, tL2CAP_CHNL_PRIORITY priority) {
  if (p_ccb->ccb_priority != priority) {
    ...
    if ((p_ccb->p_next_ccb != NULL) || (p_ccb->p_prev_ccb != NULL)) {
      L2LOG_DEBUG("Update CCB list in logical link");
...
void l2cu_release_ccb(tL2C_CCB* p_ccb) {
  ...
  L2LOG_DEBUG("l2cu_release_ccb: cid 0x%04x  in_use: %u", p_ccb->local_cid,
              p_ccb->in_use);
    ...
    if (p_lcb->link_state == LST_CONNECTED) {
      ...
      if (!p_lcb->ccb_queue.p_first_ccb) {
        if (p_lcb->transport == BT_TRANSPORT_LE &&
          ...
          L2LOG_WARN("%s - disconnecting the LE link", __func__);
          l2cu_no_dynamic_ccbs(p_lcb);
        }
      }
    }
...
void l2cu_disconnect_chnl(tL2C_CCB* p_ccb) {
  ...
  if (local_cid >= L2CAP_BASE_APPL_CID) {
    ...
    L2LOG_WARN("L2CAP - disconnect_chnl CID: 0x%04x", local_cid);
    ...
    L2LOG_ERROR("L2CAP - disconnect_chnl CID: 0x%04x Ignored", local_cid);
  }
}
...
void l2cu_create_conn_br_edr(tL2C_LCB* p_lcb) {
  ...
  for (uint8_t xx = 0; xx < MAX_L2CAP_LINKS; xx++, p_lcb_cur++) {
    ...
    if (BTM_IsScoActiveByBdaddr(p_lcb_cur->remote_bd_addr)) {
      L2LOG_DEBUG("%s Central peripheral switch not allowed when SCO active",
                  __func__);
      continue;
    }
...
bool l2cu_set_acl_priority(const RawAddress& bd_addr, tL2CAP_PRIORITY priority,
  ...
  LOG_INFO("SET ACL PRIORITY %d", priority);

  /* Find the link control block for the acl channel */
  p_lcb = l2cu_find_lcb_by_bd_addr(bd_addr, BT_TRANSPORT_BR_EDR);
  if (p_lcb == NULL) {
    L2LOG_WARN("L2CAP - no LCB for L2CA_SetAclPriority");
    return (false);
  }
...
void l2cu_resubmit_pending_sec_req(const RawAddress* p_bda) {
  ...
  L2LOG_DEBUG("l2cu_resubmit_pending_sec_req  p_bda: 0x%08x", p_bda);
  ...
  if (p_bda) {
    ...
    if (p_lcb) {
      ...
      L2LOG_WARN("l2cu_resubmit_pending_sec_req - unknown BD_ADDR");
    }
  } else {
...
void l2cu_adjust_out_mps(tL2C_CCB* p_ccb) {
  ...
  if (packet_size <= (L2CAP_PKT_OVERHEAD + L2CAP_FCR_OVERHEAD +
    ...
    L2LOG_ERROR("l2cu_adjust_out_mps bad packet size: %u  will use MPS: %u",
    ...
    L2LOG_DEBUG("l2cu_adjust_out_mps use %d   Based on peer_cfg.fcr.mps: %u  "
                "packet_size: %u",
...
void l2cu_no_dynamic_ccbs(tL2C_LCB* p_lcb) {
  ...
  for (xx = 0; xx < L2CAP_NUM_FIXED_CHNLS; xx++) {
    if ((p_lcb->p_fixed_ccbs[xx] != NULL) &&
      ...
      if (p_lcb->p_fixed_ccbs[xx]->fixed_chnl_idle_tout ==
        ...
        L2LOG_DEBUG("%s NO IDLE timeout set for fixed cid 0x%04x", __func__,
  ...
  L2LOG_DEBUG("l2cu_no_dynamic_ccbs() with_active_local_clients=%d",
  ...
  if (timeout_ms == 0) {
    L2LOG_DEBUG("l2cu_no_dynamic_ccbs() IDLE timer 0, disconnecting link");
...
void l2cu_send_peer_ble_par_req(tL2C_LCB* p_lcb, uint16_t min_int,
  ...
  if (p_buf == NULL) {
    L2LOG_WARN("l2cu_send_peer_ble_par_req - no buffer");
    return;
  }
...
void l2cu_send_peer_ble_par_rsp(tL2C_LCB* p_lcb, uint16_t reason,
  ...
  if (p_buf == NULL) {
    L2LOG_WARN("l2cu_send_peer_ble_par_rsp - no buffer");
    return;
  }
...
void l2cu_send_peer_ble_credit_based_conn_req(tL2C_CCB* p_ccb) {
  ...
  if (p_buf == NULL) {
    L2LOG_WARN("l2cu_send_peer_ble_credit_based_conn_req - no buffer");
    return;
  }
  ...
  L2LOG_DEBUG("l2cu_send_peer_ble_credit_based_conn_req PSM:0x%04x local_cid:%d\
                mtu:%d mps:%d initial_credit:%d",
              p_ccb->p_rcb->real_psm, p_ccb->local_cid, mtu, mps,
              initial_credit);
...
void l2cu_send_peer_credit_based_conn_req(tL2C_CCB* p_ccb) {
  ...
  if (p_buf == NULL) {
    L2LOG_WARN("%s - no buffer", __func__);
    return;
  }
  ...
  L2LOG_DEBUG("%s PSM:0x%04x mtu:%d mps:%d initial_credit:%d, cids_cnt %d",
...
void l2cu_reject_ble_coc_connection(tL2C_LCB* p_lcb, uint8_t rem_id,
  ...
  if (p_buf == NULL) {
    L2LOG_WARN("l2cu_reject_ble_coc_connection - no buffer");
    return;
  }
...
void l2cu_reject_credit_based_conn_req(tL2C_LCB* p_lcb, uint8_t rem_id,
  ...
  p_buf = l2cu_build_header(p_lcb, rsp_len, L2CAP_CMD_CREDIT_BASED_CONN_RES,
                            rem_id);
  if (p_buf == NULL) {
    L2LOG_WARN("l2cu_reject_credit_based_conn_req - no buffer");
    return;
  }
...
void l2cu_send_ble_reconfig_rsp(tL2C_LCB* p_lcb, uint8_t rem_id,
  ...
  L2LOG_DEBUG("l2cu_send_ble_reconfig_rsp result 0x04%x", result);

  p_buf = l2cu_build_header(p_lcb, L2CAP_CMD_CREDIT_BASED_RECONFIG_RES_LEN,
                            L2CAP_CMD_CREDIT_BASED_RECONFIG_RES, rem_id);
  if (p_buf == NULL) {
    L2LOG_WARN("l2cu_send_peer_ble_credit_based_conn_res - no buffer");
    return;
  }
...
void l2cu_send_peer_ble_credit_based_conn_res(tL2C_CCB* p_ccb,
  ...
  L2LOG_DEBUG("l2cu_send_peer_ble_credit_based_conn_res");
  p_buf =
...
void l2cu_send_peer_ble_credit_based_disconn_req(tL2C_CCB* p_ccb) {
  ...
  p_buf = l2cu_build_header(p_lcb, L2CAP_DISC_REQ_LEN, L2CAP_CMD_DISC_REQ,
                            p_lcb->signal_id);
  if (p_buf == NULL) {
    L2LOG_WARN("l2cu_send_peer_ble_credit_based_disconn_req - no buffer");
    return;
  }
...
static void send_congestion_status_to_all_clients(tL2C_CCB* p_ccb,
  ...
  if (p_ccb->p_rcb && p_ccb->p_rcb->api.pL2CA_CongestionStatus_Cb) {
    L2LOG_DEBUG("L2CAP - Calling CongestionStatus_Cb (%d), CID: 0x%04x "
                "xmit_hold_q.count: %u  buff_quota: %u",
                status, p_ccb->local_cid,
