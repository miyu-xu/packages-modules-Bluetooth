...
void l2cu_send_peer_config_rsp(tL2C_CCB* p_ccb, tL2CAP_CFG_INFO* p_cfg) {
  ...
  uint16_t cfg_result = static_cast<uint16_t>(p_cfg->result);
...
void l2cu_send_peer_config_rej(tL2C_CCB* p_ccb, uint8_t* p_data, uint16_t data_len,
  ...
  uint16_t cfg_result = static_cast<uint16_t>(tL2CAP_CFG_RESULT::L2CAP_CFG_UNKNOWN_OPTIONS);
  UINT16_TO_STREAM(p, cfg_result);
...
void l2cu_send_peer_ble_par_rsp(tL2C_LCB* p_lcb, tL2CAP_CFG_RESULT reason, uint8_t rem_id) {
  ...
  uint16_t cfg_reason = static_cast<uint16_t>(reason);
  UINT16_TO_STREAM(p, cfg_reason);
