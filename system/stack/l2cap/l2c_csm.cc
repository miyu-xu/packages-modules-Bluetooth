...
static void l2c_csm_closed(tL2C_CCB* p_ccb, tL2CEVT event, void* p_data) {
  ...
  log::debug("LCID: 0x{:04x}  st: CLOSED  evt: {} psm: {}", p_ccb->local_cid,
             l2c_csm_get_event_name(event), bt_psm_uint_to_text(p_ccb->p_rcb->psm));
...
static void l2c_csm_orig_w4_sec_comp(tL2C_CCB* p_ccb, tL2CEVT event, void* p_data) {
  ...
  log::debug("{} - LCID: 0x{:04x}  st: ORIG_W4_SEC_COMP  evt: {} psm: {}",
             ...
             p_ccb->local_cid, l2c_csm_get_event_name(event), bt_psm_uint_to_text(p_ccb->p_rcb->psm));
  ...
  switch (event) {
    ...
    case L2CEVT_L2CAP_CONNECT_RSP_NEG: /* Peer rejected connection */
      ...
      if (p_lcb->transport == BT_TRANSPORT_LE) {
        ...
        (*p_ccb->p_rcb->api.pL2CA_Error_Cb)(
                local_cid, psm_to_l2cap_result(p_ccb->p_rcb->psm, p_ci->l2cap_result, L2CAP_CONN_OTHER_ERROR));
      }
      bluetooth::shim::CountCounterMetrics(
    ...
    case L2CEVT_TIMEOUT:
      ...
      if (p_ccb->ecoc) {
        ...
        l2cu_release_ccb(p_ccb);
        (*p_ccb->p_rcb->api.pL2CA_Error_Cb)(local_cid, psm_to_l2cap_result(p_ccb->p_rcb->psm, p_ci->l2cap_result, L2CAP_CONN_OTHER_ERROR));
        bluetooth::shim::CountCounterMetrics(
      ...
      if (cfg_result == L2CAP_PEER_CFG_OK) {
        ...
        if (p_ccb->config_done & OB_CFG_DONE) {
          if (p_ccb->remote_config_rsp_result == L2CAP_CFG_OK) {
            ...
            if (p_ccb->connection_initiator == L2CAP_INITIATOR_LOCAL) {
              (*p_ccb->p_rcb->api.pL2CA_Error_Cb)(p_ccb->local_cid, psm_to_l2cap_result(p_ccb->p_rcb->psm, p_ccb->remote_config_rsp_result, L2CAP_CFG_FAILED_NO_REASON));
              bluetooth::shim::CountCounterMetrics(
    ...
    case L2CEVT_L2CAP_CONFIG_RSP_NEG: /* Peer config error rsp */
      ...
      if (!l2c_fcr_renegotiate_chan(p_ccb, p_cfg)) {
        log::debug("Calling Config_Rsp_Cb(), CID: 0x{:04x}, Failure: {}", p_ccb->local_cid,
                   p_cfg->result);
        if (p_ccb->connection_initiator == L2CAP_INITIATOR_LOCAL) {
          (*p_ccb->p_rcb->api.pL2CA_Error_Cb)(p_ccb->local_cid, psm_to_l2cap_result(p_ccb->p_rcb->psm, p_cfg->result, L2CAP_CFG_FAILED_NO_REASON));
          bluetooth::shim::CountCounterMetrics(
