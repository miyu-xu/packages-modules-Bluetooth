...
static void l2c_csm_indicate_connection_open(tL2C_CCB* p_ccb) {
  ...
  PowerTelemetry::GetInstance()->LogChannelConnected(
...
static void l2c_csm_open(tL2C_CCB* p_ccb, tL2CEVT event, void* p_data) {
  ...
  switch (event) {
    case L2CEVT_LP_DISCONNECT_IND: /* Link was disconnected */
      LOG_DEBUG("Calling Disconnect_Ind_Cb(), CID: 0x%04x  No Conf Needed",
                p_ccb->local_cid);
      PowerTelemetry::GetInstance()->LogChannelDisconnected(
          CHANNEL_TYPE_L2CAP, p_ccb->local_cid, p_ccb->remote_id,
          p_ccb->p_lcb->remote_bd_addr);
      l2cu_release_ccb(p_ccb);
      if (p_ccb->p_rcb)
    ...
    case L2CEVT_L2CAP_DATA: /* Peer data packet rcvd    */
      ...
      if (p_ccb->p_rcb->psm == BT_PSM_RFCOMM) {
        PowerTelemetry::GetInstance()->LogRxBytes(
            CHANNEL_TYPE_RFCOMM, p_ccb->local_cid, p_ccb->remote_id,
            p_ccb->p_lcb->remote_bd_addr, ((BT_HDR*)p_data)->len);
      } else {
        PowerTelemetry::GetInstance()->LogRxBytes(
            CHANNEL_TYPE_L2CAP, p_ccb->local_cid, p_ccb->remote_id,
            p_ccb->p_lcb->remote_bd_addr, ((BT_HDR*)p_data)->len);
    ...
    case L2CEVT_L2CA_DATA_WRITE: /* Upper layer data to send */
      if (p_data) {
        ...
        l2c_link_check_send_pkts(p_ccb->p_lcb, 0, NULL);
        if (p_ccb->p_rcb->psm == BT_PSM_RFCOMM) {
          PowerTelemetry::GetInstance()->LogTxBytes(
              CHANNEL_TYPE_RFCOMM, p_ccb->local_cid, p_ccb->remote_id,
              p_ccb->p_lcb->remote_bd_addr, ((BT_HDR*)p_data)->len);
        } else {
          PowerTelemetry::GetInstance()->LogTxBytes(
