...
static void l2c_csm_indicate_connection_open(tL2C_CCB* p_ccb) {
  ...
  power_telemetry::GetInstance().LogChannelConnected(
      power_telemetry::kChannelL2cap, p_ccb->local_cid, p_ccb->remote_id,
  ...
  switch (event) {
    ...
    case L2CEVT_L2CAP_DATA: /* Peer data packet rcvd    */
      if (p_data && (p_ccb->p_rcb)) {
        ...
        const int channel_type = p_ccb->p_rcb->psm == BT_PSM_RFCOMM
                                     ? power_telemetry::kChannelRfcomm
                                     : power_telemetry::kChannelL2cap;
        power_telemetry::GetInstance().LogRxBytes(
            channel_type, p_ccb->local_cid, p_ccb->remote_id,
      ...
      if (p_data) {
        ...
        if (p_ccb->p_rcb->psm == BT_PSM_RFCOMM) {
          power_telemetry::GetInstance().LogTxBytes(
              power_telemetry::kChannelRfcomm, p_ccb->local_cid,
              p_ccb->remote_id, p_ccb->p_lcb->remote_bd_addr, package_len);
        } else {
          power_telemetry::GetInstance().LogTxBytes(
              power_telemetry::kChannelL2cap, p_ccb->local_cid,
              p_ccb->remote_id, p_ccb->p_lcb->remote_bd_addr, package_len);
        }
      }
      break;
