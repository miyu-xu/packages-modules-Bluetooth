...
void L2CA_Deregister(uint16_t psm) {
  ...
  L2LOG_INFO("L2CAP - L2CA_Deregister() called for PSM: 0x%04x", psm);
  ...
  if (p_rcb != NULL) {
    ...
    L2LOG_WARN("L2CAP - PSM: 0x%04x not found for deregistration", psm);
  }
}
...
uint16_t L2CA_AllocateLePSM(void) {
  ...
  L2LOG_INFO("%s: last psm=%d", __func__, psm);
  while (!done) {
    count++;
    if (count > LE_DYNAMIC_PSM_RANGE) {
      L2LOG_ERROR("%s: Out of free BLE PSM", __func__);
      return 0;
    }
    ...
    if (!l2cb.le_dyn_psm_assigned[psm - LE_DYNAMIC_PSM_START]) {
      ...
      if (l2cu_find_ble_rcb_by_psm(psm)) {
        L2LOG_WARN("%s: supposedly-free PSM=%d have allocated rcb!", __func__,
                   psm);
        continue;
      }

      l2cb.le_dyn_psm_assigned[psm - LE_DYNAMIC_PSM_START] = true;
      L2LOG_DEBUG("%s: assigned PSM=%d", __func__, psm);
      done = true;
      break;
    }
  }
...
void L2CA_FreeLePSM(uint16_t psm) {
  L2LOG_INFO("%s: to free psm=%d", __func__, psm);

  if ((psm < LE_DYNAMIC_PSM_START) || (psm > LE_DYNAMIC_PSM_END)) {
    L2LOG_ERROR("%s: Invalid PSM=%d value!", __func__, psm);
    return;
  }

  if (!l2cb.le_dyn_psm_assigned[psm - LE_DYNAMIC_PSM_START]) {
    L2LOG_WARN("%s: PSM=%d was not allocated!", __func__, psm);
  }
...
uint16_t L2CA_ConnectReq(uint16_t psm, const RawAddress& p_bd_addr) {
  ...
  if (p_lcb->link_state == LST_CONNECTED) {
     ...
     * arrives
     */
    L2LOG_DEBUG("L2CAP API - link disconnecting: RETRY LATER");

    /* Save ccb so it can be started after disconnect is finished */
    p_lcb->p_pending_ccb = p_ccb;
  }

  L2LOG_INFO("L2CAP - L2CA_conn_req(psm: 0x%04x) returned CID: 0x%04x", psm,
             p_ccb->local_cid);
...
void L2CA_DeregisterLECoc(uint16_t psm) {
  L2LOG_INFO("%s called for PSM: 0x%04x", __func__, psm);

  tL2C_RCB* p_rcb = l2cu_find_ble_rcb_by_psm(psm);
  if (p_rcb == NULL) {
    L2LOG_WARN("%s PSM: 0x%04x not found for deregistration", __func__, psm);
    return;
  }
...
uint16_t L2CA_ConnectLECocReq(uint16_t psm, const RawAddress& p_bd_addr,
  ...
  /* Fail if we have not established communications with the controller */
  if (!BTM_IsDeviceUp()) {
    L2LOG_WARN("%s BTU not ready", __func__);
    return 0;
  }
  ...
  tL2C_RCB* p_rcb = l2cu_find_ble_rcb_by_psm(psm);
  if (p_rcb == NULL) {
    L2LOG_WARN("%s No BLE RCB, PSM: 0x%04x", __func__, psm);
    return 0;
  }
  ...
  if (p_lcb == NULL) {
    ...
    if ((p_lcb == NULL)
      ...
      L2LOG_WARN("%s conn not started for PSM: 0x%04x  p_lcb: 0x%08x", __func__,
                 psm, p_lcb);
      return 0;
    }
  }
  ...
  tL2C_CCB* p_ccb = l2cu_allocate_ccb(p_lcb, 0);
  if (p_ccb == NULL) {
    L2LOG_WARN("%s no CCB, PSM: 0x%04x", __func__, psm);
    return 0;
  }
  ...
  if (p_lcb->link_state == LST_CONNECTED) {
    if (p_ccb->p_lcb->transport == BT_TRANSPORT_LE) {
      L2LOG_DEBUG("%s LE Link is up", __func__);
   ...
   */
    ...
    L2LOG_DEBUG("%s link disconnecting: RETRY LATER", __func__);

    /* Save ccb so it can be started after disconnect is finished */
    p_lcb->p_pending_ccb = p_ccb;
  }

  L2LOG_INFO("%s(psm: 0x%04x) returned CID: 0x%04x", __func__, psm,
             p_ccb->local_cid);
...
bool L2CA_GetPeerLECocConfig(uint16_t lcid, tL2CAP_LE_CFG_INFO* peer_cfg) {
  L2LOG_INFO("%s CID: 0x%04x", __func__, lcid);

  tL2C_CCB* p_ccb = l2cu_find_ccb_by_cid(NULL, lcid);
  if (p_ccb == NULL) {
    L2LOG_ERROR("%s No CCB for CID:0x%04x", __func__, lcid);
    return false;
  }
...
bool L2CA_ConnectCreditBasedRsp(const RawAddress& p_bd_addr, uint8_t id,
  ...
  if (!p_ccb) {
    L2LOG_ERROR("%s No CCB for CID:0x%04x", __func__, p_lcb->pending_lead_cid);
    return false;
  }
  ...
  if (p_ccb->remote_id != id) {
    L2LOG_WARN("%s bad id. Expected: %d  Got: %d", __func__, p_ccb->remote_id,
               id);
    return false;
  }
...
std::vector<uint16_t> L2CA_ConnectCreditBasedReq(uint16_t psm,
  ...
  if (!p_cfg) {
    L2LOG_WARN("%s p_cfg is NULL", __func__);
    return allocated_cids;
  }
  ...
  tL2C_LCB* p_lcb = l2cu_find_lcb_by_bd_addr(p_bd_addr, BT_TRANSPORT_LE);
  if (p_lcb == NULL) {
    L2LOG_WARN("%s No link available", __func__);
    return allocated_cids;
  }

  if (p_lcb->link_state != LST_CONNECTED) {
    L2LOG_WARN("%s incorrect link state: %d", __func__, p_lcb->link_state);
    return allocated_cids;
  }
...
bool L2CA_ReconfigCreditBasedConnsReq(const RawAddress& bda,
  ...
  L2LOG_INFO("L2CA_ReconfigCreditBasedConnsReq() ");

  if (lcids.empty()) {
    L2LOG_WARN("L2CAP - no lcids given to %s", __func__);
    return (false);
  }
  ...
  for (uint16_t cid : lcids) {
    ...
    if (!p_ccb) {
      L2LOG_WARN("L2CAP - no CCB for L2CA_cfg_req, CID: %d", cid);
      return (false);
    }

    if ((p_ccb->local_conn_cfg.mtu > p_cfg->mtu) ||
        (p_ccb->local_conn_cfg.mps > p_cfg->mps)) {
      L2LOG_WARN("L2CAP - MPS or MTU reduction, CID: %d", cid);
      return (false);
    }
  }

  if (p_cfg->mtu > L2CAP_MTU_SIZE) {
    L2LOG_WARN("L2CAP - adjust MTU: %u too large", p_cfg->mtu);
    p_cfg->mtu = L2CAP_MTU_SIZE;
  }

  /* Mark all the p_ccbs which going to be reconfigured */
  for (uint16_t cid : lcids) {
    L2LOG_INFO(" cid: %d", cid);
    p_ccb = l2cu_find_ccb_by_cid(NULL, cid);
    if (!p_ccb) {
...
bool L2CA_SetTxPriority(uint16_t cid, tL2CAP_CHNL_PRIORITY priority) {
  ...
  L2LOG_INFO("L2CA_SetTxPriority()  CID: 0x%04x, priority:%d", cid, priority);

  /* Find the channel control block. We don't know the link it is on. */
  p_ccb = l2cu_find_ccb_by_cid(NULL, cid);
  if (p_ccb == NULL) {
    L2LOG_WARN("L2CAP - no CCB for L2CA_SetTxPriority, CID: %d", cid);
    return (false);
  }
...
bool L2CA_RemoveFixedChnl(uint16_t fixed_cid, const RawAddress& rem_bda) {
  ...
  if ((fixed_cid < L2CAP_FIRST_FIXED_CHNL) ||
      ...
      (l2cb.fixed_reg[fixed_cid - L2CAP_FIRST_FIXED_CHNL].pL2CA_FixedData_Cb ==
       NULL)) {
    L2LOG_ERROR("L2CA_RemoveFixedChnl()  Invalid CID: 0x%04x", fixed_cid);
    return (false);
  }
...
uint8_t L2CA_DataWrite(uint16_t cid, BT_HDR* p_data) {
  L2LOG_INFO("L2CA_DataWrite()  CID: 0x%04x  Len: %d", cid, p_data->len);
...
bool L2CA_SetChnlFlushability(uint16_t cid, bool is_flushable) {
  ...
  p_ccb = l2cu_find_ccb_by_cid(NULL, cid);
  if (p_ccb == NULL) {
    L2LOG_WARN("L2CAP - no CCB for L2CA_SetChnlFlushability, CID: %d", cid);
    return (false);
  }

  p_ccb->is_flushable = is_flushable;

  L2LOG_INFO("L2CA_SetChnlFlushability()  CID: 0x%04x  is_flushable: %d", cid,
             is_flushable);
...
uint16_t L2CA_FlushChannel(uint16_t lcid, uint16_t num_to_flush) {
  ...
  if (!p_ccb || (p_ccb->p_lcb == NULL)) {
    L2LOG_WARN("L2CA_FlushChannel()  abnormally returning 0  CID: 0x%04x",
               lcid);
    return (0);
  }
  p_lcb = p_ccb->p_lcb;
  ...
  if (num_to_flush != L2CAP_FLUSH_CHANS_GET) {
    ...
    L2LOG_INFO("L2CA_FlushChannel (QUERY)  CID: 0x%04x", lcid);
  }
  ...
  L2LOG_DEBUG("L2CA_FlushChannel()  flushed: %u + %u,  num_left: %u",
...
void L2CA_SetMediaStreamChannel(uint16_t local_media_cid, bool status) {
  ...
  if (status) {
    ...
    if (set_channel < 0) {
      L2LOG_ERROR("%s: No empty slot found to set media channel", __func__);
      return;
    }
    ...
    if (set_channel < 0) {
      L2LOG_ERROR("%s: The channel %d not found in active media channels",
                  __func__, local_media_cid);
      return;
    }
    ...
    L2LOG_INFO("%s: Reset A2DP media snoop filtering for local_cid: %d",
               __func__, local_media_cid);
  }
