...
void L2CA_Deregister(uint16_t psm) {
  ...
  LOG_DEBUG("L2CAP - L2CA_Deregister() called for PSM: 0x%04x", psm);
...
uint16_t L2CA_AllocateLePSM(void) {
  ...
  LOG_DEBUG("%s: last psm=%d", __func__, psm);
  while (!done) {
    count++;
...
void L2CA_FreeLePSM(uint16_t psm) {
  LOG_DEBUG("%s: to free psm=%d", __func__, psm);
...
uint16_t L2CA_ConnectReq(uint16_t psm, const RawAddress& p_bd_addr) {
  ...
  LOG_DEBUG("L2CAP - L2CA_conn_req(psm: 0x%04x) returned CID: 0x%04x", psm,
            p_ccb->local_cid);
...
void L2CA_DeregisterLECoc(uint16_t psm) {
  LOG_DEBUG("%s called for PSM: 0x%04x", __func__, psm);
...
uint16_t L2CA_ConnectLECocReq(uint16_t psm, const RawAddress& p_bd_addr,
  ...
  LOG_DEBUG("%s(psm: 0x%04x) returned CID: 0x%04x", __func__, psm,
            p_ccb->local_cid);
...
bool L2CA_GetPeerLECocConfig(uint16_t lcid, tL2CAP_LE_CFG_INFO* peer_cfg) {
  LOG_DEBUG("%s CID: 0x%04x", __func__, lcid);
...
bool L2CA_ReconfigCreditBasedConnsReq(const RawAddress& bda,
  ...
  LOG_DEBUG("L2CA_ReconfigCreditBasedConnsReq() ");
...
bool L2CA_SetTxPriority(uint16_t cid, tL2CAP_CHNL_PRIORITY priority) {
  ...
  LOG_DEBUG("L2CA_SetTxPriority()  CID: 0x%04x, priority:%d", cid, priority);
...
uint8_t L2CA_DataWrite(uint16_t cid, BT_HDR* p_data) {
  LOG_DEBUG("L2CA_DataWrite()  CID: 0x%04x  Len: %d", cid, p_data->len);
...
bool L2CA_SetChnlFlushability(uint16_t cid, bool is_flushable) {
  ...
  LOG_DEBUG("L2CA_SetChnlFlushability()  CID: 0x%04x  is_flushable: %d", cid,
            is_flushable);
...
uint16_t L2CA_FlushChannel(uint16_t lcid, uint16_t num_to_flush) {
  ...
  if (num_to_flush != L2CAP_FLUSH_CHANS_GET) {
    ...
    LOG_DEBUG("L2CA_FlushChannel (QUERY)  CID: 0x%04x", lcid);
  }
...
void L2CA_SetMediaStreamChannel(uint16_t local_media_cid, bool status) {
  ...
  if (status) {
    ...
    LOG_DEBUG("%s: Reset A2DP media snoop filtering for local_cid: %d",
              __func__, local_media_cid);
  }
