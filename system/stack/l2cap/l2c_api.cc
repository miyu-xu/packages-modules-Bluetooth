...
void L2CA_Dumpsys(int fd) {
  ...
  LOG_DUMPSYS(fd, "Registered classic clients:");
  {
    const tL2C_RCB* p_rcb = &l2cb.rcb_pool[0];
    for (uint16_t xx = 0; xx < MAX_L2CAP_CLIENTS; xx++, p_rcb++) {
      if (!p_rcb->in_use) continue;
      LOG_DUMPSYS(fd, "  PSM:0x%04x[0x%04x]", p_rcb->psm, p_rcb->real_psm);
    }
  }
  LOG_DUMPSYS(fd, "Registered le clients:");
  {
    const tL2C_RCB* p_rcb = &l2cb.ble_rcb_pool[0];
    for (uint16_t xx = 0; xx < BLE_MAX_L2CAP_CLIENTS; xx++, p_rcb++) {
      if (!p_rcb->in_use) continue;
      LOG_DUMPSYS(fd, "  LE_PSM:0x%04x[0x%04x]", p_rcb->psm, p_rcb->real_psm);
    }
  }
