...
uint16_t L2CA_Register(uint16_t psm, const tL2CAP_APPL_INFO& p_cb_info, bool enable_snoop,
  ...
  */
  ...
  p_rcb->real_psm = psm;
  p_rcb->ertm_info =
          p_ertm_info == nullptr ? tL2CAP_ERTM_INFO{ L2CAP_FCR_BASIC_MODE } : *p_ertm_info;
  p_rcb->my_mtu = my_mtu;
  ...
  if (p_lcb->link_state == LST_CONNECTED) {
    if (p_ccb->p_lcb->transport == BT_TRANSPORT_LE) {
      ...
      do_in_main_thread(FROM_HERE,
                        base::BindOnce(&l2c_csm_execute, base::Unretained(p_ccb),
...
uint8_t L2CA_LECocDataWrite(uint16_t cid, BT_HDR* p_data) { return L2CA_DataWrite(cid, p_data); }
