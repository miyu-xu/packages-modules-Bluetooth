...
void l2c_rcv_acl_data(BT_HDR* p_msg) {
  ...
  if (pkt_type == L2CAP_PKT_CONTINUE) {
    L2LOG_WARN("L2CAP - received packet continuation");
    osi_free(p_msg);
    return;
  }
  ...
  if (hci_len < L2CAP_PKT_OVERHEAD || hci_len != p_msg->len - 4) {
    ...
    L2LOG_WARN("L2CAP - got incorrect hci header");
    osi_free(p_msg);
    return;
  }
  ...
  if (rcv_cid >= L2CAP_BASE_APPL_CID) {
    ...
    if (!p_ccb) {
      L2LOG_WARN("L2CAP - unknown CID: 0x%04x", rcv_cid);
      osi_free(p_msg);
      return;
    }
  }
  ...
  if (l2cap_len != p_msg->len) {
    L2LOG_WARN("L2CAP - bad length in pkt. Exp: %d  Act: %d", l2cap_len,
               p_msg->len);
    osi_free(p_msg);
    return;
  }
...
uint8_t l2c_data_write(uint16_t cid, BT_HDR* p_data, uint16_t flags) {
  ...
  if (!p_ccb) {
    L2LOG_WARN("L2CAP - no CCB for L2CA_DataWrite, CID: %d", cid);
    osi_free(p_data);
  ...
  if (p_ccb->cong_sent) {
    L2LOG_ERROR("L2CAP - CID: 0x%04x cannot send, already congested  "
