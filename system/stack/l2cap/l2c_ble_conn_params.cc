...
void l2cble_start_conn_update(tL2C_LCB* p_lcb) {
  ...
  if (p_lcb->conn_update_mask & L2C_BLE_CONN_UPDATE_DISABLE) {
    ...
    if (p_lcb->conn_update_mask & L2C_BLE_NOT_DEFAULT_PARAM) {
      if (com::android::bluetooth::flags::initial_conn_params_p1()) {
        min_conn_int = osi_property_get_int32(kPropertyMinConnIntervalAggressive.c_str(),
                                              kMinConnIntervalAggressive);
        max_conn_int = osi_property_get_int32(kPropertyMaxConnIntervalAggressive.c_str(),
        ...
        if (p_lcb->conn_update_mask & L2C_BLE_AGGRESSIVE_INITIAL_PARAM) {
          // Check whether we can still use the same connection parameters for service discovery.
    ...
    if (p_lcb->conn_update_mask & L2C_BLE_NEW_CONN_PARAM) {
      ...
      if (p_lcb->IsLinkRoleCentral() ||
        ...
        if (com::android::bluetooth::flags::initial_conn_params_p1() &&
          ...
          p_lcb->min_interval = kMinConnIntervalRelaxed;
          p_lcb->max_interval = kMaxConnIntervalRelaxed;
          p_lcb->conn_update_mask &= ~L2C_BLE_AGGRESSIVE_INITIAL_PARAM;
        }
...
void l2cble_process_rc_param_request_evt(uint16_t handle, uint16_t int_min, uint16_t int_max,
  ...
  // TODO: Where are this params actually applied?
