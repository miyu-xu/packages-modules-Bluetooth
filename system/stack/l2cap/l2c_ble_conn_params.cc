...
// TODO: Where should put these constants so that this can be used in both le_impl.h and here?
...
void L2CA_LockBleConnParamsForServiceDiscovery(const RawAddress& rem_bda, bool lock) {
  ...
  if (lock == p_lcb->conn_update_blocked_by_service_discovery) {
    ...
    if (!lock && p_lcb->relax_conn_params_after_service_discovery &&
        com::android::bluetooth::flags::initial_conn_params_p1()) {
      // This handles the case where the service discovery is skipped due to GATT cache.
      p_lcb->conn_update_mask &= ~L2C_BLE_NOT_DEFAULT_PARAM;
      p_lcb->conn_update_mask |= L2C_BLE_NEW_CONN_PARAM;
      log::info("Need to relax connection parameters. Keep running...");
    } else {
      return;
    }
  }
...
void l2cble_start_conn_update(tL2C_LCB* p_lcb) {
  ...
  if ((p_lcb->conn_update_mask & L2C_BLE_UPDATE_PENDING) ||
    ...
    log::info("XXX - Step 2-X Should not come here");
    return;
  }
  log::info("XXX - Step 3");

  if (p_lcb->conn_update_mask & L2C_BLE_CONN_UPDATE_DISABLE) {
    log::info("XXX - Step 3-X: When Lock is true");
    ...
    if (p_lcb->conn_update_mask & L2C_BLE_NOT_DEFAULT_PARAM) {
      if (com::android::bluetooth::flags::initial_conn_params_p1()) {
        if (p_lcb->relax_conn_params_after_service_discovery) {
        ...
        min_conn_int = kConnIntervalAggressiveMin;
        max_conn_int = kConnIntervalAggressiveMax;
      } else {
      ...
      if (p_lcb->IsLinkRoleCentral() ||
        ...
        log::info("XXX - Step 3-X-A-1 Changing parameters to [{}, {}] for service discovery",
        ...
        log::info("XXX - Step 3-X-A-2 Should not come here");
    ...
    if (p_lcb->conn_update_mask & L2C_BLE_NEW_CONN_PARAM) {
      log::info("XXX - Step 5. Is my role central?={}", p_lcb->IsLinkRoleCentral());

      /* if both side 4.1, or we are central device, send HCI command */
      if (p_lcb->IsLinkRoleCentral() ||
          (bluetooth::shim::GetController()->SupportsBleConnectionParametersRequest() &&
           acl_peer_supports_ble_connection_parameters_request(p_lcb->remote_bd_addr))) {
        log::info("XXX - Step 5-A Good");

        uint16_t min_interval = p_lcb->min_interval;
        uint16_t max_interval = p_lcb->max_interval;

        if (com::android::bluetooth::flags::initial_conn_params_p1() &&
            p_lcb->relax_conn_params_after_service_discovery) {
          // Q: When does the p_lcb->min_interval is changed?
          log::info("Trying to relax conn. params! addr={}", p_lcb->remote_bd_addr);
          p_lcb->min_interval = kConnIntervalRelaxedMin;
          p_lcb->max_interval = kConnIntervalRelaxedMax;
          p_lcb->relax_conn_params_after_service_discovery = false;
        }
        ...
        log::info("XXX - Step 5-X: Should not come here!");
