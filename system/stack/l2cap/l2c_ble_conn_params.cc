...
void l2cble_start_conn_update(tL2C_LCB* p_lcb) {
  ...
  if (p_lcb->conn_update_mask & L2C_BLE_CONN_UPDATE_DISABLE) {
    ...
    if (p_lcb->conn_update_mask & L2C_BLE_NOT_DEFAULT_PARAM) {
      if (com::android::bluetooth::flags::initial_conn_params_p1()) {
        min_conn_int = get_min_conn_interval_aggressive();
        max_conn_int = get_max_conn_interval_aggressive();
        log::info("min_conn_int={}, max_conn_int={}", min_conn_int, max_conn_int);

        if (p_lcb->conn_update_mask & L2C_BLE_AGGRESSIVE_INITIAL_PARAM) {
          log::info("Already using aggressive connection parameters.");
          p_lcb->conn_update_mask &= ~L2C_BLE_NOT_DEFAULT_PARAM;
          p_lcb->conn_update_mask |= L2C_BLE_NEW_CONN_PARAM;
          return;
        }
    ...
    if (p_lcb->conn_update_mask & L2C_BLE_NEW_CONN_PARAM) {
      ...
      if (p_lcb->IsLinkRoleCentral() ||
        ...
        if (com::android::bluetooth::flags::initial_conn_params_p1() &&
          ...
          p_lcb->min_interval = get_min_conn_interval_relaxed();
          p_lcb->max_interval = get_max_conn_interval_relaxed();
          p_lcb->conn_update_mask &= ~L2C_BLE_AGGRESSIVE_INITIAL_PARAM;
        }
