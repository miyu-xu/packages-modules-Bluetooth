  ...
  *request.mutable_config() = config;
  ...
  std::unique_ptr<dbus::Response> dbus_response =
      codec_manager_
          ...
          .value_or(nullptr);
  if (!dbus_response) {
    LOG(ERROR) << "DBus `CodecInit` call failed.";
    return -ECOMM;
  }
  ...
  if (response.socket_token().empty()) {
    LOG(ERROR) << "DBus `CodecInit` returned empty socket token.";
    return -EBADMSG;
  }
...
void CodecClient::cleanup() {
  ...
  std::unique_ptr<dbus::Response> dbus_response =
      codec_manager_
          ...
          .value_or(nullptr);
  if (!dbus_response) {
    LOG(ERROR) << "Failed to send dbus message to mmc service.";
  }
  return;
}
...
int CodecClient::transcode(uint8_t* i_buf, int i_len, uint8_t* o_buf,
  ...
  DCHECK(i_buf != nullptr && o_buf != nullptr);
