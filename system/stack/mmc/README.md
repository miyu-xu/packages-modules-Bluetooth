...
To reduce the privilge of audio codec in BT daemon, MMC uses minijail to launch and sandbox external codec libraries.
...
### 4. Add codec client in BT process
  * BT process accesses library via codec client
    ...
    * `transcode`: pass input and output buffer and specify the input data size and the output buffer capacity. the transcode return value will be the output data length on success, and negative error number otherwise.
