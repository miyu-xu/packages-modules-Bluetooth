...
void StartSocketListener(int fd, std::promise<void> task_ended,
                         std::unique_ptr<MmcInterface> codec_server) {
  int client_fd = accept(fd, NULL, NULL);
  uint8_t i_buf[kMaximumBufferSize];
  uint8_t o_buf[kMaximumBufferSize];
  ...
  while (1) {
    ...
    int rc = recv(client_fd, i_buf, kMaximumBufferSize, MSG_NOSIGNAL);
    if (rc <= 0) {
    ...
    memset(o_buf, 0, sizeof(o_buf));
    int transcode_rc =
        codec_server->transcode(i_buf, rc, o_buf, kMaximumBufferSize);
    if (transcode_rc < 0) {
      LOG(ERROR) << "Failed to transcode: " << strerror(-transcode_rc);
      break;
    }

    rc = send(client_fd, o_buf, transcode_rc, MSG_NOSIGNAL);
    if (rc <= 0) {
...
void Service::CodecInit(dbus::MethodCall* method_call,
  ...
  if (request.config().has_hfp_lc3_decoder_param()) {
    ...
    codec_server->init(request.config());
  ...
  if (bind(skt_fd_, (struct sockaddr*)&addr, sizeof(struct sockaddr_un)) ==
      -1) {
    std::move(sender).Run(dbus::ErrorResponse::FromMethodCall(
        method_call, kMmcServiceError,
  ...
  if (listen(skt_fd_, kClientMaximum) == -1) {
    std::move(sender).Run(dbus::ErrorResponse::FromMethodCall(
        method_call, kMmcServiceError,
  ...
  if (!StartWorkerThread(skt_fd_, std::move(codec_server))) {
    std::move(sender).Run(dbus::ErrorResponse::FromMethodCall(
...
bool Service::StartWorkerThread(int fd,
                                std::unique_ptr<MmcInterface> codec_server) {
  // Each thread has its associated future. Main thread checks the
  ...
  // Real-time scheudling increases thread priority.
  // Without it, the thread could still works.
  if (!thread_pool_.back().first->EnableRealTimeScheduling()) {
    LOG(ERROR) << "Failed to enable real time scheduling";
  }

  if (!thread_pool_.back().first->DoInThread(
          FROM_HERE,
          base::BindOnce(&StartSocketListener, fd, std::move(task_ended),
