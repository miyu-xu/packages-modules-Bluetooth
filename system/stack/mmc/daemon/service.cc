...
void StartSocketListener(int fd, struct sockaddr_un addr,
  ...
  while (1) {
    ...
    int rc = recv(client_fd, i_buf.data(), kMaximumBufferSize, MSG_NOSIGNAL);
    if (rc <= 0) {
    ...
    // Start transcode.
    int transcoded_data_length = codec_server->transcode(
        i_buf.data(), rc, o_buf.data(), kMaximumBufferSize);
    if (transcoded_data_length < 0) {
      LOG(ERROR) << "Failed to transcode: "
                 << strerror(-transcoded_data_length);
      break;
    }

    rc = send(client_fd, o_buf.data(), transcoded_data_length, MSG_NOSIGNAL);
    if (rc <= 0) {
