...
#include "mmc/codec_server/a2dp_aac_mmc_encoder_linux.h"
...
void Service::CodecInit(dbus::MethodCall* method_call,
  ...
  } else if (request.config().has_a2dp_aac_encoder_linux_param()) {
    codec_server = std::make_unique<A2dpAacEncoderLinux>();
  } else {
    std::move(sender).Run(dbus::ErrorResponse::FromMethodCall(
