...
class Service final {
 public:
  Service(base::OnceClosure shutdown_callback);
 ...
 private:
  ...
  // Response:
  //   |CodecInitResponse| with socket token, if |CodecInit| succeeded.
  ...
  // Creates a thread in the thread pool and makes it listen on the socket fd.
  bool StartWorkerThread(int fd, std::unique_ptr<MmcInterface> codec_server);
