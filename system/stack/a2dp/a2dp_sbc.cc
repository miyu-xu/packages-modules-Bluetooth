...
static void A2DP_AdjustBitpool(tA2DP_SBC_CIE* p_ie) {
  log::verbose("Rajat A2DP_AdjustBitpool");
...
static bool A2DP_BuildInfoSbc(uint8_t media_type, const tA2DP_SBC_CIE* p_ie, uint8_t* p_result) {
  log::verbose("Rajat A2DP_BuildInfoSbc");
  ...
  if(true/*Flag to be added*/) {
    A2DP_AdjustBitpool(p_ie);
            ...
            (p_ie->max_bitpool > A2DP_SBC_IE_MAX_BITPOOL)) {
    log::verbose("Rajat A2DP_AdjustBitpool: Invalid bitpool value");
    return false;
  }
...
static tA2DP_STATUS A2DP_ParseInfoSbc(tA2DP_SBC_CIE* p_ie, const uint8_t* p_codec_info,
  ...
  if(/*flag-to-be-added*/true) {
    log::verbose("Rajat A2DP_ParseInfoSbc : flag is true");
    A2DP_AdjustBitpool(p_ie);
  }
