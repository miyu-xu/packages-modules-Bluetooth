...
static void A2DP_AdjustBitpool(tA2DP_SBC_CIE* p_ie) {
  ...
  if(p_ie->min_bitpool < A2DP_SBC_IE_MIN_BITPOOL) {
    log::verbose("min_bitpool value adjusted from: {} to {}",
  ...
  if(p_ie->max_bitpool > A2DP_SBC_IE_MAX_BITPOOL) {
    log::verbose("max_bitpool value adjusted from: {} to {}",
...
static bool A2DP_BuildInfoSbc(uint8_t media_type, const tA2DP_SBC_CIE* p_ie, uint8_t* p_result) {
  ...
  tA2DP_SBC_CIE p_ie_temp = *p_ie;
  A2DP_AdjustBitpool(&p_ie_temp);

  if((p_ie_temp.min_bitpool > p_ie_temp.max_bitpool) ||
            (p_ie_temp.min_bitpool < A2DP_SBC_IE_MIN_BITPOOL) ||
            (p_ie_temp.min_bitpool > A2DP_SBC_IE_MAX_BITPOOL) ||
            (p_ie_temp.max_bitpool < A2DP_SBC_IE_MIN_BITPOOL) ||
            (p_ie_temp.max_bitpool > A2DP_SBC_IE_MAX_BITPOOL)) {
    log::verbose("Invalid bitpool value min: {}, max: {}",
                    p_ie_temp.min_bitpool, p_ie_temp.max_bitpool);
    return false;
  }
  ...
  *p_result++ = p_ie_temp.min_bitpool;
  *p_result = p_ie_temp.max_bitpool;
