...
bool A2DP_CodecEquals(const uint8_t* p_codec_info_a, const uint8_t* p_codec_info_b) {
  ...
  if (!codec_id_a.has_value() || !codec_id_b.has_value() ||
      codec_id_a != codec_id_b) {
    return false;
  }
  ...
  log::error("unsupported codec id 0x{:x}", codec_id_a);
  return false;
}
...
int A2DP_GetTrackSampleRate(const uint8_t* p_codec_info) {
  ...
  log::error("unsupported codec id 0x{:x}", codec_id);
  return -1;
}
