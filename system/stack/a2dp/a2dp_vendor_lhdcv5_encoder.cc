...
typedef int32_t (*tLHDC_SET_EXT_FUNC)(HANDLE_LHDCV5_BT hLhdcParam, LHDCV5_EXT_FUNC_T field,
                                      bool enabled, void* priv, uint32_t privDataLen);

static const char* LHDC_INIT_ENCODER_NAME = "lhdcv5BT_init_encoder";
typedef int32_t (*tLHDC_INIT_ENCODER)(HANDLE_LHDCV5_BT hLhdcParam, uint32_t samplingFreq,
...
typedef int32_t (*tLHDC_ENCODE)(HANDLE_LHDCV5_BT hLhdcParam, void* pInPcm, uint32_t pcmBytes,
...
typedef int (*tLHDCV5_EXT_SET_CONFIG)(HANDLE_LHDCV5_BT hLhdcParam, const char* userBuf,
                                      const int bufLen);

static const char* LHDCV5_EXT_GET_CONFIG_NAME = "lhdcv5BT_get_user_exconfig";
typedef int (*tLHDCV5_EXT_GET_CONFIG)(HANDLE_LHDCV5_BT hLhdcParam, const char* userBuf,
                                      const int bufLen);

static const char* LHDC_EXT_SET_DATA_NAME = "lhdcv5BT_set_user_exdata";
typedef void (*tLHDCV5_EXT_SET_DATA)(HANDLE_LHDCV5_BT hLhdcParam, const char* userBuf,
                                     const int bufLen);

static const char* LHDCV5_EXT_GET_APIVER_NAME = "lhdcv5BT_get_user_exApiver";
typedef int (*tLHDCV5_EXT_GET_APIVER)(HANDLE_LHDCV5_BT hLhdcParam, const char* userBuf,
                                      const int bufLen);
#endif
...
static void* load_func(const char* func_name) {
  ...
  if (func_ptr == NULL) {
    LOG_ERROR("%s: cannot find function '%s' in the encoder library: %s", __func__, func_name,
              dlerror());
...
bool A2DP_VendorLoadEncoderLhdcV5(void) {
  ...
  if (lhdc_encoder_lib_handle == nullptr) {
    LOG_ERROR("%s: cannot open LHDC encoder library %s: %s", __func__, LHDC_ENCODER_LIB_NAME,
              dlerror());
    return false;
  }
...
bool A2DP_VendorUnloadEncoderLhdcV5(void) {
  ...
  LOG_DEBUG("%s: has_lhdc_handle %d", __func__, a2dp_lhdc_encoder_cb.has_lhdc_handle);
...
static void a2dp_vendor_lhdcv5_encoder_update(uint16_t peer_mtu, A2dpCodecConfig* a2dp_codec_config,
  ...
  *p_config_updated = false;
  ...
  LOG_DEBUG("%s: (feeding param) sample_rate=%u bits_per_sample=%u channel_count=%u", __func__,
  ...
  LOG_DEBUG(
          "%s: (encode param) sample_rate=%u pcm_fmt=%d peer_mtu=%d mtu=%d "
          "maxBitRateIdx=%u minBitRateIdx=%u isLLEnabled=%u isLLessEnabled=%u quality_mode=%s(%d)",
          __func__,
  ...
  if (p_encoder_params->isLLessEnabled == 1 && p_encoder_params->sample_rate == 96000) {
    ...
    if (A2DP_VendorHasARFlagLhdcV5(&ret_value8, p_codec_info)) {
      ...
      if (lib_ret != LHDCV5_FRET_SUCCESS) {
        LOG_ERROR("%s: [lib_ret] lhdc_set_ext_func AR(0x%X) %d", __func__, LHDCV5_EXT_FUNC_AR,
                  lib_ret);
        goto fail;
      }
    }
    ...
    if (A2DP_VendorHasJASFlagLhdcV5(&ret_value8, p_codec_info)) {
      lib_ret = lhdc_set_ext_func(a2dp_lhdc_encoder_cb.lhdc_handle, LHDCV5_EXT_FUNC_JAS,
                                  (bool)ret_value8, NULL, 0);
      if (lib_ret != LHDCV5_FRET_SUCCESS) {
        LOG_ERROR("%s: [lib_ret] lhdc_set_ext_func JAS(0x%X) %d", __func__, LHDCV5_EXT_FUNC_JAS,
                  lib_ret);
        goto fail;
      }
    }
...
void a2dp_vendor_lhdcv5_send_frames(uint64_t timestamp_us) {
  ...
  LOG_VERBOSE("%s: Sending %d frames per iteration, %d iterations", __func__, nb_frame,
              nb_iterations);
...
static void a2dp_lhdcv5_get_num_frame_iteration(uint8_t* num_of_iterations, uint8_t* num_of_frames,
  ...
  LOG_VERBOSE("%s: samples_per_frame=%u pcm_bytes_per_frame=%u nb_frame=%u", __func__,
...
static void a2dp_lhdcV5_encode_frames(uint8_t nb_frame) {
  ...
  lib_ret = lhdc_get_block_size(a2dp_lhdc_encoder_cb.lhdc_handle, &samples_per_frame);
  if (lib_ret != LHDCV5_FRET_SUCCESS) {
    LOG_ERROR("%s: [lib_ret] lhdc_get_block_size error %d", __func__, lib_ret);
    return;
  }
  pcm_bytes_per_frame = samples_per_frame * a2dp_lhdc_encoder_cb.feeding_params.channel_count *
                        a2dp_lhdc_encoder_cb.feeding_params.bits_per_sample / 8;
  ...
  while (nb_frame) {
...
#endif
        LOG_VERBOSE("%s: nb_frame(%d) - written:%d, out_frames:%d", __func__, nb_frame, written,
                    out_frames);
        p_buf->len += written;
    ...
    if (p_buf->len) {
      ...
      *((uint32_t*)(p_buf + 1)) = a2dp_lhdc_encoder_cb.timestamp;
      ...
      LOG_VERBOSE("%s: nb_frame(%d) - remain_nb_frame:%d", __func__, (nb_frame + 1),
                  remain_nb_frame);
  ...
  if (time_now_ms - time_prev_ms >= 1000) {
    ...
    if (mcpsStat_time_dur_cnt_lhdcv5 <= MCPS_FRAME_ENCODE_DUR) {  // how long we stop statistics
       ...
       */
      LOG_DEBUG(
              "[MCPS_STAT]: [cnt:%u time_now_ms(%u)-time_prev_ms(%u)=(%u) %.2fs] [%u %u %d kbps] "
              "frmPerTheSec(%u) avgFrmPerSec(%.2f) avgUsPerfrm:(%.2f)us, avgMCPS:(%.3f)",
...
static bool a2dp_lhdcv5_read_feeding(uint8_t* read_buffer, uint32_t* bytes_read) {
  ...
  LOG_VERBOSE("%s: expected read bytes %u, actual read bytes %u", __func__, read_size,
              nb_byte_read);

  // TODO: what to do if not alignment?
  if ((nb_byte_read % bytes_per_sample) != 0) {
    LOG_DEBUG("%s: PCM data not alignment. The audio sample is shfit %d bytes!", __func__,
...
void A2dpCodecConfigLhdcV5Source::debug_codec_dump(int fd) {
  ...
  if (lib_ret == LHDCV5_FRET_SUCCESS) {
    dprintf(fd, "  LHDC transmission bitrate (Kbps)                        : %d\n", lib_value);
  }
