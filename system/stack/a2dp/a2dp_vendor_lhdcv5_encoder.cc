...
#include "a2dp_vendor_lhdcv5_encoder_ffi.h"
...
typedef int32_t (*tLHDC_GET_HANDLE)(uint32_t version, HANDLE_LHDC_BT* hLhdcParam);

static const char* LHDC_FREE_HANDLE_NAME = "lhdcv5BT_free_handle";
typedef int32_t (*tLHDC_FREE_HANDLE)(HANDLE_LHDC_BT hLhdcParam);

static const char* LHDC_GET_BITRATE_NAME = "lhdcv5BT_get_bitrate";
typedef int32_t (*tLHDC_GET_BITRATE)(HANDLE_LHDC_BT hLhdcParam, uint32_t* bitrate);

static const char* LHDC_SET_BITRATE_NAME = "lhdcv5BT_set_bitrate";
typedef int32_t (*tLHDC_SET_BITRATE)(HANDLE_LHDC_BT hLhdcParam, uint32_t bitrateInx);

static const char* LHDC_SET_MAX_BITRATE_NAME = "lhdcv5BT_set_max_bitrate";
typedef int32_t (*tLHDC_SET_MAX_BITRATE)(HANDLE_LHDC_BT hLhdcParam, uint32_t maxBitrateInx);

static const char* LHDC_SET_MIN_BITRATE_NAME = "lhdcv5BT_set_min_bitrate";
typedef int32_t (*tLHDC_SET_MIN_BITRATE)(HANDLE_LHDC_BT hLhdcParam, uint32_t minBitrateInx);

static const char* LHDC_AUTO_ADJUST_BITRATE_NAME = "lhdcv5BT_adjust_bitrate";
typedef int32_t (*tLHDC_AUTO_ADJUST_BITRATE)(HANDLE_LHDC_BT hLhdcParam, uint32_t queueLength);

static const char* LHDC_SET_EXT_FUNC = "lhdcv5BT_set_ext_func_state";
typedef int32_t (*tLHDC_SET_EXT_FUNC)(HANDLE_LHDC_BT hLhdcParam, LHDCV5_EXT_FUNC_T field,
                                      bool enabled, void* priv, uint32_t privDataLen);

static const char* LHDC_INIT_ENCODER_NAME = "lhdcv5BT_init_encoder";
typedef int32_t (*tLHDC_INIT_ENCODER)(HANDLE_LHDC_BT hLhdcParam, uint32_t samplingFreq,
                                      uint32_t bitsPerSample, uint32_t bitrateInx, uint32_t mtu,
                                      uint32_t interval, uint32_t is_lossless_enable);

static const char* LHDC_GET_BLOCK_SIZE = "lhdcv5BT_get_block_Size";
typedef int32_t (*tLHDC_GET_BLOCK_SIZE)(HANDLE_LHDC_BT hLhdcParam, uint32_t* samplesPerFrame);

static const char* LHDC_ENCODE_NAME = "lhdcv5BT_encode";
typedef int32_t (*tLHDC_ENCODE)(HANDLE_LHDC_BT hLhdcParam, void* pInPcm, uint32_t pcmBytes,
                                uint8_t* pOutBuf, uint32_t outBufBytes, uint32_t* pOutByte,
                                uint32_t* pOut_frames);

#ifdef LHDC_LOSSLESS_RAW_SUPPORT
static const char* LHDC_SET_LOSSLESS_RAW_ENABLE = "lhdcv5BT_set_lossless_raw_enable";
typedef int32_t (*tLHDC_SET_LOSSLESS_RAW_ENABLE)(HANDLE_LHDC_BT hLhdcParam, uint32_t enabled);
#endif
...
typedef int (*tLHDCV5_EXT_SET_CONFIG)(HANDLE_LHDC_BT hLhdcParam, const char* userBuf,
                                      const int bufLen);

static const char* LHDCV5_EXT_GET_CONFIG_NAME = "lhdcv5BT_get_user_exconfig";
typedef int (*tLHDCV5_EXT_GET_CONFIG)(HANDLE_LHDC_BT hLhdcParam, const char* userBuf,
                                      const int bufLen);

static const char* LHDC_EXT_SET_DATA_NAME = "lhdcv5BT_set_user_exdata";
typedef void (*tLHDCV5_EXT_SET_DATA)(HANDLE_LHDC_BT hLhdcParam, const char* userBuf,
                                     const int bufLen);

static const char* LHDCV5_EXT_GET_APIVER_NAME = "lhdcv5BT_get_user_exApiver";
typedef int (*tLHDCV5_EXT_GET_APIVER)(HANDLE_LHDC_BT hLhdcParam, const char* userBuf,
                                      const int bufLen);
#endif
...
static void* load_func(const char* func_name) {
  ...
  if (func_ptr == NULL) {
    log::error("cannot find function '{}' in the encoder library: {}", func_name, dlerror());
...
bool A2DP_VendorLoadEncoderLhdcV5(void) {
  ...
  if (lhdc_encoder_lib_handle == nullptr) {
    log::error("cannot open LHDC encoder library {}: {}", LHDC_ENCODER_LIB_NAME, dlerror());
    return false;
  }
...
int A2dpCodecConfigLhdcV5Source::getEncoderExtendFuncUserApiVer(const char* /*version*/,
                                                                const int /*clen*/) {
  return BT_STATUS_FAIL;
}
int A2dpCodecConfigLhdcV5Source::getEncoderExtendFuncUserConfig(const char* /*userConfig*/,
                                                                const int /*clen*/) {
  return BT_STATUS_FAIL;
}
int A2dpCodecConfigLhdcV5Source::setEncoderExtendFuncUserConfig(const char* /*userConfig*/,
                                                                const int /*clen*/) {
  return BT_STATUS_FAIL;
}
bool A2dpCodecConfigLhdcV5Source::setEncoderExtendFuncUserData(const char* /*codecData*/,
                                                               const int /*clen*/) {
  return false;
}
#else
...
int A2dpCodecConfigLhdcV5Source::getEncoderExtendFuncUserConfig(const char* userConfig,
  ...
  if (lhdcv5_ext_get_config != nullptr) {
    log::verbose("lhdc_ext_API(GetConfig): clen:{}", clen);
...
int A2dpCodecConfigLhdcV5Source::setEncoderExtendFuncUserConfig(const char* userConfig,
  ...
  if (lhdcv5_ext_set_config != nullptr) {
    log::verbose("lhdc_ext_API(SetConfig): clen:{}", clen);
...
bool A2dpCodecConfigLhdcV5Source::setEncoderExtendFuncUserData(const char* codecData,
  ...
  if (lhdcv5_ext_set_data != nullptr) {
    log::verbose("lhdc_ext_API(SetData): clen:{}", clen);
...
static void a2dp_vendor_lhdcv5_encoder_update(uint16_t peer_mtu, A2dpCodecConfig* a2dp_codec_config,
  ...
  *p_config_updated = false;
  ...
  log::debug("lhdc handle addr = %p", a2dp_lhdc_encoder_cb.lhdc_handle);
  ...
  lib_ret = lhdc_init_encoder(a2dp_lhdc_encoder_cb.lhdc_handle, p_encoder_params->sample_rate,
                              p_encoder_params->pcm_fmt, p_encoder_params->quality_mode_index,
                              max_mtu_len, (uint32_t)a2dp_vendor_lhdcv5_get_encoder_interval_ms(),
  ...
  lib_ret = lhdc_set_max_bitrate(a2dp_lhdc_encoder_cb.lhdc_handle,
                                 p_encoder_params->max_target_bitrate);
  if (lib_ret != LHDC_FRET_SUCCESS) {
    log::error("[lib_ret] set_max_bitrate {}", lib_ret);
    goto fail;
  }
  lib_ret = lhdc_set_min_bitrate(a2dp_lhdc_encoder_cb.lhdc_handle,
  ...
  if (p_encoder_params->isLLessRawEnabled == 1) {
    ...
    log::debug("set lossless raw enable {}", p_encoder_params->isLLessRawEnabled);
  }
#endif
  ...
  if (recFile == NULL) {
    ...
    log::debug("create recode file = %p", recFile);
  }
  if (pcmFile == NULL) {
    pcmFile = fopen(V5ENC_PCM_NAME, "wb");
    log::debug("create recode file = %p", pcmFile);
  }
#endif
  return;
...
void a2dp_vendor_lhdcv5_feeding_reset(void) {
  ...
  if (p_encoder_params->quality_mode_index == LHDC_QUALITY_AUTO) {
    if (lhdc_set_bitrate != NULL && a2dp_lhdc_encoder_cb.has_lhdc_handle) {
      lhdc_set_bitrate(a2dp_lhdc_encoder_cb.lhdc_handle, LHDC_QUALITY_CTRL_RESET_ABR);
    }
  }
