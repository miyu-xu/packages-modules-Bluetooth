...
static bool is_codec_version_supported(uint8_t version, bool is_source) {
  ...
  log::debug("versoin unsupported! peer:{} local:{}", version, p_a2dp_lhdcv5_caps->version);
  return false;
}
...
static tA2DP_STATUS A2DP_ParseInfoLhdcV5(tA2DP_LHDCV5_CIE* p_ie, const uint8_t* p_codec_info,
  ...
  log::debug(
          ...
          "LLESS96K({}) LLESSRaw({})]",
          is_capability, p_ie->sampleRate, p_ie->bitsPerSample, p_ie->version, p_ie->frameLenType,
          p_ie->maxTargetBitrate, p_ie->minTargetBitrate, p_ie->hasFeatureAR, p_ie->hasFeatureJAS,
          p_ie->hasFeatureMETA, p_ie->hasFeatureLL, p_ie->hasFeatureLLESS48K,
          p_ie->hasFeatureLLESS24Bit, p_ie->hasFeatureLLESS96K,
#ifdef LHDC_LOSSLESS_RAW_SUPPORT
...
tA2DP_STATUS A2dpCodecConfigLhdcV5Base::setCodecConfig(const uint8_t* p_peer_codec_info,
   ...
   *******************************************/
  {
    ...
    log::debug("featureLLESS: enabled? <{}> [Peer:0x{:02X} Local:0x{:02X} User:{} 24bit:{} 96K:{}]",
               result_config_cie.hasFeatureLLESS48K ? "Y" : "N", sink_info_cie.hasFeatureLLESS48K,
               p_a2dp_lhdcv5_caps->hasFeatureLLESS48K, hasUserSet ? "Y" : "N",
