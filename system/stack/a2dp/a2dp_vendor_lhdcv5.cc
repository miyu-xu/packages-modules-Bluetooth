...
UNUSED_ATTR static const tA2DP_LHDC_FEATURE_POS a2dp_lhdcv5_source_spec_all[] = {
        a2dp_lhdcv5_source_spec_JAS,       a2dp_lhdcv5_source_spec_AR,
        a2dp_lhdcv5_source_spec_META,      a2dp_lhdcv5_source_spec_LL,
  ...
  switch (featureCode) {
    case LHDCV5_FEATURE_CODE_JAS: {
      ret = LHDCV5_CHECK_IN_A2DP_SPEC(cfgsPtr->_codec_user_config_,
                                      a2dp_lhdcv5_source_spec_JAS.inSpecBank,
                                      a2dp_lhdcv5_source_spec_JAS.value);
      return ret;
    } break;
    case LHDCV5_FEATURE_CODE_AR: {
      return LHDCV5_CHECK_IN_A2DP_SPEC(cfgsPtr->_codec_user_config_,
                                       a2dp_lhdcv5_source_spec_AR.inSpecBank,
                                       a2dp_lhdcv5_source_spec_AR.value);
    } break;
    case LHDCV5_FEATURE_CODE_META: {
      return LHDCV5_CHECK_IN_A2DP_SPEC(cfgsPtr->_codec_user_config_,
                                       a2dp_lhdcv5_source_spec_META.inSpecBank,
                                       a2dp_lhdcv5_source_spec_META.value);
    } break;
    ...
    case LHDCV5_FEATURE_CODE_LLESS: {
      return LHDCV5_CHECK_IN_A2DP_SPEC(cfgsPtr->_codec_user_config_,
                                       a2dp_lhdcv5_source_spec_LLESS.inSpecBank,
                                       a2dp_lhdcv5_source_spec_LLESS.value);
    } break;
#ifdef LHDC_LOSSLESS_RAW_SUPPORT
    case LHDCV5_FEATURE_CODE_LLESS_RAW: {
      return LHDCV5_CHECK_IN_A2DP_SPEC(cfgsPtr->_codec_user_config_,
                                       a2dp_lhdcv5_source_spec_LLESS_RAW.inSpecBank,
                                       a2dp_lhdcv5_source_spec_LLESS_RAW.value);
    } break;
#endif
    default:
      break;
...
static bool is_codec_version_supported(uint8_t version, bool is_source) {
  ...
  log::debug("versoin unsupported! peer:{} local:{}", version,
            p_a2dp_lhdcv5_caps->version);
  return false;
}
...
static tA2DP_STATUS A2DP_BuildInfoLhdcV5(uint8_t media_type, const tA2DP_LHDCV5_CIE* p_ie,
  ...
  *p_result++ = para;
  ...
  log::debug(
          ...
          tmpInfo[6], tmpInfo[7], tmpInfo[8], tmpInfo[9], tmpInfo[10], tmpInfo[11], tmpInfo[12],
...
static tA2DP_STATUS A2DP_ParseInfoLhdcV5(tA2DP_LHDCV5_CIE* p_ie, const uint8_t* p_codec_info,
  ...
  p_ie->hasFeatureLLESS24Bit =
          ((*p_codec_info & A2DP_LHDCV5_FEATURE_LLESS24BIT) != 0) ? true : false;
  ...
  log::debug(
          "isCap[{}] SR[{:02X}] BPS[{:02X}] Ver[{:02X}] FL[{:02X}] "
          "MBR[{:02X}] mBR[{:02X}] Feature[AR({}) JAS({}) META({}) LL({}) LLESS({}) LLESS24({}) "
          "LLESS96K({}) LLESSRaw({})]",
          is_capability, p_ie->sampleRate,
          p_ie->bitsPerSample, p_ie->version, p_ie->frameLenType, p_ie->maxTargetBitrate,
          p_ie->minTargetBitrate, p_ie->hasFeatureAR, p_ie->hasFeatureJAS, p_ie->hasFeatureMETA,
          p_ie->hasFeatureLL, p_ie->hasFeatureLLESS48K, p_ie->hasFeatureLLESS24Bit,
...
bool A2DP_IsVendorSinkCodecSupportedLhdcV5(const uint8_t* p_codec_info) {
  ...
  if (!is_codec_version_supported(cfg_cie.version, IS_SNK)) {
...
bool A2DP_IsPeerSourceCodecSupportedLhdcV5(const uint8_t* p_codec_info) {
  ...
  if (!is_codec_version_supported(cfg_cie.version, IS_SRC)) {
      log::error("unsupported version 0x{:X}", cfg_cie.version);
      return false;
  }
...
static tA2DP_STATUS A2DP_CodecInfoMatchesCapabilityLhdcV5(const tA2DP_LHDCV5_CIE* p_cap,
  ...
  log::debug("FREQ peer: 0x{:x}, capability 0x{:x}", cfg_cie.sampleRate,
            p_cap->sampleRate);
...
std::string A2DP_VendorCodecInfoStringLhdcV5(const uint8_t* p_codec_info) {
  ...
  a2dp_status = A2DP_ParseInfoLhdcV5(&lhdc_cie, p_codec_info, true);
  if (a2dp_status != A2DP_SUCCESS) {
    res << fmt::format("A2DP_ParseInfoLhdcV5 fail: 0x{:x}", a2dp_status);
    return res.str();
  }
  ...
  res << fmt::format("\tbits_depth: {} bits (0x{:x})\n", field, lhdc_cie.bitsPerSample);
  ...
  AppendField(&field,
              ...
              "400Kbps");
  res << fmt::format("\tMax target-rate: {} (0x{:x})\n", field, lhdc_cie.maxTargetBitrate & A2DP_LHDCV5_MAX_BIT_RATE_MASK);
  ...
  AppendField(&field,
              ((lhdc_cie.minTargetBitrate & A2DP_LHDCV5_MIN_BIT_RATE_MASK) ==
               A2DP_LHDCV5_MIN_BIT_RATE_64K),
              "64Kbps");
  res << fmt::format("\tMin target-rate: {} (0x{:x})\n", field, lhdc_cie.minTargetBitrate & A2DP_LHDCV5_MIN_BIT_RATE_MASK);
...
tA2DP_STATUS A2dpCodecConfigLhdcV5Base::setCodecConfig(const uint8_t* p_peer_codec_info, bool is_capability,
  ...
  do {
    ...
    if (select_audio_sample_rate(&codec_audio_config_, sampleRate, &result_config_cie,
                                 &codec_config_)) {
      log::debug("select sample rate from audio: 0x{:02X}",
                result_config_cie.sampleRate);
      break;
    }
    ...
    if (select_best_sample_rate(
      ...
      log::debug("select sample rate from default: 0x{:02X}",
                result_config_cie.sampleRate);
      break;
    }
    ...
    if (select_best_sample_rate(sampleRate, &result_config_cie, &codec_config_)) {
      log::debug("select sample rate from best match: 0x{:02X}",
                result_config_cie.sampleRate);
      break;
    }
  } while (false);
  ...
  do {
    ...
    if (select_audio_bits_per_sample(&codec_audio_config_, bitsPerSample, &result_config_cie,
                                     &codec_config_)) {
      log::debug("select bit per sample from audio: 0x{:02X}",
                result_config_cie.bitsPerSample);
      break;
    }
    ...
    if (select_best_bits_per_sample(
                ...
                &result_config_cie, &codec_config_)) {
      log::debug("select bit per sample from default: 0x{:02X}",
                result_config_cie.bitsPerSample);
      break;
    }
  ...
  if ((codec_user_config_.codec_specific_3 & A2DP_LHDC_VENDOR_FEATURE_MASK) !=
    ...
    log::debug("LHDC features tag check fail, default UI status[LL] => {}",
              hasUserSet ? "true" : "false");

    // Feature: AR (default UI: OFF)
    hasUserSet = false;
    A2DP_UpdateFeatureToA2dpConfigLhdcV5(&allCfgPack, LHDCV5_FEATURE_CODE_AR,
                                         A2DP_LHDC_TO_A2DP_CODEC_USER_,
                                         (hasUserSet ? true : false));
    log::debug("LHDC features tag check fail, default UI status[AR] => {}",
              hasUserSet ? "true" : "false");

    // Feature: Lossless (default UI: OFF)
    hasUserSet = false;
    A2DP_UpdateFeatureToA2dpConfigLhdcV5(&allCfgPack, LHDCV5_FEATURE_CODE_LLESS,
                                         A2DP_LHDC_TO_A2DP_CODEC_USER_,
                                         (hasUserSet ? true : false));
    log::debug("LHDC features tag check fail, default UI status[LLESS] => {}",
              hasUserSet ? "true" : "false");

#ifdef LHDC_LOSSLESS_RAW_SUPPORT
    // Feature: Lossless Raw(default UI: OFF)
    hasUserSet = false;
    A2DP_UpdateFeatureToA2dpConfigLhdcV5(&allCfgPack, LHDCV5_FEATURE_CODE_LLESS_RAW,
                                         A2DP_LHDC_TO_A2DP_CODEC_USER_,
                                         (hasUserSet ? true : false));
    log::debug("LHDC features tag check fail, default UI status[LLESS Raw] => {}",
              hasUserSet ? "true" : "false");
#endif
  }
   ...
   *******************************************/
  {
    ...
    A2DP_UpdateFeatureToA2dpConfigLhdcV5(
            &allCfgPack, LHDCV5_FEATURE_CODE_JAS,
    ...
    log::debug("featureJAS: enabled? <{}> [Peer:0x{:02X} Local:0x{:02X}]",
              result_config_cie.hasFeatureJAS ? "Y" : "N", sink_info_cie.hasFeatureJAS,
   ...
   *******************************************/
  {
    ...
    A2DP_UpdateFeatureToA2dpConfigLhdcV5(
            &allCfgPack, LHDCV5_FEATURE_CODE_AR,
            (A2DP_LHDC_TO_A2DP_CODEC_CONFIG_ | A2DP_LHDC_TO_A2DP_CODEC_CAP_ |
             A2DP_LHDC_TO_A2DP_CODEC_SELECT_CAP_ | A2DP_LHDC_TO_A2DP_CODEC_USER_),
            false);
    // update
    ...
    log::debug("featureAR: enabled? <{}> [Peer:0x{:02X} Local:0x{:02X} User:{}]",
              result_config_cie.hasFeatureAR ? "Y" : "N", sink_info_cie.hasFeatureAR,
              p_a2dp_lhdcv5_caps->hasFeatureAR, hasUserSet ? "Y" : "N");
  }
   ...
   *******************************************/
  {
    ...
    A2DP_UpdateFeatureToA2dpConfigLhdcV5(
            &allCfgPack, LHDCV5_FEATURE_CODE_META,
            (A2DP_LHDC_TO_A2DP_CODEC_CONFIG_ | A2DP_LHDC_TO_A2DP_CODEC_CAP_ |
             A2DP_LHDC_TO_A2DP_CODEC_SELECT_CAP_ | A2DP_LHDC_TO_A2DP_CODEC_USER_),
            false);
    // update
    ...
    log::debug("featureMETA: enabled? <{}> [Peer:0x{:02X} Local:0x{:02X}]",
              result_config_cie.hasFeatureMETA ? "Y" : "N", sink_info_cie.hasFeatureMETA,
   ...
   *******************************************/
  {
    ...
    hasUserSet =
            A2DP_IsFeatureInUserConfigLhdcV5(&allCfgPack, LHDCV5_FEATURE_CODE_LLESS);  // UI-control

    A2DP_UpdateFeatureToA2dpConfigLhdcV5(
            &allCfgPack, LHDCV5_FEATURE_CODE_LLESS,
            (A2DP_LHDC_TO_A2DP_CODEC_CONFIG_ | A2DP_LHDC_TO_A2DP_CODEC_CAP_ |
             A2DP_LHDC_TO_A2DP_CODEC_SELECT_CAP_ | A2DP_LHDC_TO_A2DP_CODEC_USER_),
            false);
    ...
    log::debug("featureLLESS: enabled? <{}> [Peer:0x{:02X} Local:0x{:02X} User:{} 24bit:{} 96K:{}]",
              result_config_cie.hasFeatureLLESS48K ? "Y" : "N",
              sink_info_cie.hasFeatureLLESS48K, p_a2dp_lhdcv5_caps->hasFeatureLLESS48K,
              hasUserSet ? "Y" : "N", result_config_cie.hasFeatureLLESS24Bit ? "Y" : "N",
   ...
   *******************************************/
  {
    ...
    hasUserSet = A2DP_IsFeatureInUserConfigLhdcV5(&allCfgPack,
                                                  LHDCV5_FEATURE_CODE_LLESS_RAW);  // UI-control

    A2DP_UpdateFeatureToA2dpConfigLhdcV5(
            &allCfgPack, LHDCV5_FEATURE_CODE_LLESS_RAW,
            (A2DP_LHDC_TO_A2DP_CODEC_CONFIG_ | A2DP_LHDC_TO_A2DP_CODEC_CAP_ |
             A2DP_LHDC_TO_A2DP_CODEC_SELECT_CAP_ | A2DP_LHDC_TO_A2DP_CODEC_USER_),
            false);
    ...
    log::debug("featureLLESSRaw: enabled? <{}> [Peer:0x{:02X} Local:0x{:02X} User:{}]",
              result_config_cie.hasFeatureLLESSRaw ? "Y" : "N", sink_info_cie.hasFeatureLLESSRaw,
              p_a2dp_lhdcv5_caps->hasFeatureLLESSRaw, hasUserSet ? "Y" : "N");
  }
#endif
  ...
  if (result_config_cie.hasFeatureLLESS48K == true) {
    ...
    if (bitRateQmode != A2DP_LHDC_QUALITY_HIGH1) {
      ...
      result_config_cie.hasFeatureLLESSRaw = false;
#endif
      log::debug("disable lossless due to peer Max Bit Rate not NO LIMIT");
    }
  ...
  if (result_config_cie.hasFeatureAR == true) {
    ...
    if (codec_user_config_.sample_rate != BTAV_A2DP_CODEC_SAMPLE_RATE_48000) {
      ...
      result_config_cie.sampleRate = A2DP_LHDCV5_SAMPLING_FREQ_48000;
      log::debug("set 48KHz sample Rate for running AR");
    }
  }
  ...
  if (result_config_cie.hasFeatureLLESS48K == true) {
    ...
    if (result_config_cie.hasFeatureLLESSRaw == true) {
      ...
      if (result_config_cie.hasFeatureLLESS96K == false) {
        ...
        codec_user_config_.sample_rate = BTAV_A2DP_CODEC_SAMPLE_RATE_48000;
        result_config_cie.sampleRate = A2DP_LHDCV5_SAMPLING_FREQ_48000;
        log::debug("(lossless raw) set 48KHz sample rate");
      } else {
        ...
        if (codec_user_config_.sample_rate != BTAV_A2DP_CODEC_SAMPLE_RATE_48000 &&
          ...
          codec_user_config_.sample_rate = BTAV_A2DP_CODEC_SAMPLE_RATE_48000;
          result_config_cie.sampleRate = A2DP_LHDCV5_SAMPLING_FREQ_48000;
          log::debug("(lossless raw) default 48KHz sample rate");
        } else {
          //  48 or 96KHz => use sample rate from user
          log::debug("(lossless raw) apply {} sample rate",
                    lhdcV5_sampleRate_toString(result_config_cie.sampleRate));
        }
      }
    } else {
      ...
      if (result_config_cie.hasFeatureLLESS96K == false) {
        ...
        codec_user_config_.sample_rate = BTAV_A2DP_CODEC_SAMPLE_RATE_48000;
        result_config_cie.sampleRate = A2DP_LHDCV5_SAMPLING_FREQ_48000;
        log::debug("(lossless) fixed 48KHz sample rate");
      } else {
        ...
        if (codec_user_config_.sample_rate != BTAV_A2DP_CODEC_SAMPLE_RATE_48000 &&
          ...
          codec_user_config_.sample_rate = BTAV_A2DP_CODEC_SAMPLE_RATE_48000;
          result_config_cie.sampleRate = A2DP_LHDCV5_SAMPLING_FREQ_48000;
          log::debug("(lossless) default 48KHz sample rate");
        } else {
          // user configure 48/96KHz
          //  => use sample rate from user
          log::debug("(lossless) apply {} sample rate",
                    lhdcV5_sampleRate_toString(result_config_cie.sampleRate));
        }
      }
#ifdef LHDC_LOSSLESS_RAW_SUPPORT
  ...
  if (result_config_cie.hasFeatureAR == true) {
    ...
    if (codec_user_config_.bits_per_sample != BTAV_A2DP_CODEC_BITS_PER_SAMPLE_16 &&
      ...
      codec_user_config_.bits_per_sample = BTAV_A2DP_CODEC_BITS_PER_SAMPLE_24;
      result_config_cie.bitsPerSample = A2DP_LHDCV5_BIT_FMT_24;
      log::debug("set 24 bits per sample for running AR");
    }
  }
    ...
    if (result_config_cie.hasFeatureLLESS24Bit == true) {
      if (codec_user_config_.bits_per_sample != BTAV_A2DP_CODEC_BITS_PER_SAMPLE_16 &&
        ...
        if (bitsPerSample & A2DP_LHDCV5_BIT_FMT_24) {
          ...
          codec_user_config_.bits_per_sample = BTAV_A2DP_CODEC_BITS_PER_SAMPLE_24;
          result_config_cie.bitsPerSample = A2DP_LHDCV5_BIT_FMT_24;
          log::debug("(lossless-24bit): use default 24 mode");
        } else {
          ...
          result_config_cie.bitsPerSample = A2DP_LHDCV5_BIT_FMT_16;
          log::debug("(lossless-24bit): use default 16 bit mode");
        }
      } else {
        // bitrate supported, adopt user's configuration
        if (codec_user_config_.bits_per_sample == BTAV_A2DP_CODEC_BITS_PER_SAMPLE_24) {
          log::debug("(lossless-24bit): use 24 bit mode");
        } else {
          log::debug("(lossless-24bit): use 16 bit mode");
        }
      }
    } else {
      ...
      codec_user_config_.bits_per_sample = BTAV_A2DP_CODEC_BITS_PER_SAMPLE_16;
      result_config_cie.bitsPerSample = A2DP_LHDCV5_BIT_FMT_16;
      log::debug("(lossless-16bit): 16 bit mode only");
    }
  }
  ...
  if ((result_config_cie.hasFeatureLLESS48K == true) &&
    ...
    if (result_config_cie.hasFeatureLLESS24Bit == true) {
      if (result_config_cie.bitsPerSample == A2DP_LHDCV5_BIT_FMT_24) {
        log::debug("(lossless 96KHz) : 24bit OK");
      } else {
        codec_config_.bits_per_sample = BTAV_A2DP_CODEC_BITS_PER_SAMPLE_24;
        codec_capability_.bits_per_sample = BTAV_A2DP_CODEC_BITS_PER_SAMPLE_24;
        codec_user_config_.bits_per_sample = BTAV_A2DP_CODEC_BITS_PER_SAMPLE_24;
        result_config_cie.bitsPerSample = A2DP_LHDCV5_BIT_FMT_24;
        log::debug(
                "(lossless 96KHz) : unsupported bit depth but 24bit capable, reconfigure to "
                "24bit");
      }
    } else {
      codec_config_.sample_rate = BTAV_A2DP_CODEC_SAMPLE_RATE_48000;
      ...
      codec_user_config_.bits_per_sample = BTAV_A2DP_CODEC_BITS_PER_SAMPLE_16;
      result_config_cie.bitsPerSample = A2DP_LHDCV5_BIT_FMT_16;
      log::debug("(lossless 96KHz) : 16 and 24bit not capable, reconfigure to 48KHz 16bit");
    }
  }
  ...
  if (result_config_cie.hasFeatureLLESS48K == true) {
    ...
    if (qualityMode != A2DP_LHDC_QUALITY_ABR) {
      log::debug("reset non-supported quality mode ({}) to ({}) for lossless",
                lhdcV5_QualityModeBitRate_toString(qualityMode),
                lhdcV5_QualityModeBitRate_toString(A2DP_LHDC_QUALITY_ABR));
      codec_user_config_.codec_specific_1 &= ~(A2DP_LHDC_VENDOR_CMD_MASK | A2DP_LHDC_QUALITY_MASK);
      codec_user_config_.codec_specific_1 |= (A2DP_LHDC_QUALITY_MAGIC_NUM | A2DP_LHDC_QUALITY_ABR);
    ...
    if (qualityMode != A2DP_LHDC_QUALITY_ABR) {
      ...
      if (result_config_cie.sampleRate == A2DP_LHDCV5_SAMPLING_FREQ_96000 ||
        ...
        if (qualityMode < A2DP_LHDC_QUALITY_LOW3) {
          ...
          log::debug("upgrade quality mode to 0x{:02X} due to higher sample rates",
                    qualityMode);
        }
      }
      ...
      if (result_config_cie.sampleRate == A2DP_LHDCV5_SAMPLING_FREQ_44100 ||
        ...
        if (qualityMode >= A2DP_LHDC_QUALITY_HIGH1 && qualityMode != A2DP_LHDC_QUALITY_ABR) {
          ...
          log::debug("downgrade quality mode to 0x{:02X} due to lower sample rates",
                    qualityMode);
        }
      }
    }
  ...
  if (is_capability) {
    status =
            A2DP_BuildInfoLhdcV5(AVDT_MEDIA_TYPE_AUDIO, &sink_info_cie, ota_codec_peer_capability_);
  } else {
    status = A2DP_BuildInfoLhdcV5(AVDT_MEDIA_TYPE_AUDIO, &sink_info_cie, ota_codec_peer_config_);
  }
  CHECK(status == A2DP_SUCCESS);

  status = A2DP_BuildInfoLhdcV5(AVDT_MEDIA_TYPE_AUDIO, &result_config_cie, ota_codec_config_);
  CHECK(status == A2DP_SUCCESS);
  return A2DP_SUCCESS;
...
bool A2dpCodecConfigLhdcV5Base::setPeerCodecCapabilities(const uint8_t* p_peer_codec_capabilities) {
  ...
  status = A2DP_BuildInfoLhdcV5(AVDT_MEDIA_TYPE_AUDIO, &peer_info_cie, ota_codec_peer_capability_);
  CHECK(status == A2DP_SUCCESS);
  return true;
...
static bool A2DP_LhdcV5GetFeatureInfoV2(uint8_t* featVector) {
  ...
  if (featVector == nullptr) {
    log::debug("null buffer!");
    return false;
  }

  featNum = (sizeof(a2dp_lhdcv5_source_spec_all) / sizeof(tA2DP_LHDC_FEATURE_POS));
  if (featNum <= 0) {
    // log::debug( "no any features recorded in specifics, nothing to do");
    return true;
  }
  ...
  for (int i = 0; i < featNum; i++) {
    ...
    featVector[idx + 1] =
            (a2dp_lhdcv5_source_spec_all[i].inSpecBank | a2dp_lhdcv5_source_spec_all[i].bitPos);
    idx += 2;
  }
...
static bool A2DP_LhdcV5GetA2DPSpecificV2(btav_a2dp_codec_config_t* a2dpCfg, uint8_t* buf,
                                         uint32_t len) {
  if (len < LHDCV5_EXTEND_API_A2DP_SPEC_TOTAL_SIZE_V2) {
    log::debug("payload size too small {} ", len);
    return false;
  }

  /* retrieve specifics value */
  if (!(swapInt64toByteArray(&buf[LHDCV5_EXTEND_API_A2DP_SPEC1_HEAD_V2],
                             a2dpCfg->codec_specific_1) &&
        swapInt64toByteArray(&buf[LHDCV5_EXTEND_API_A2DP_SPEC2_HEAD_V2],
                             a2dpCfg->codec_specific_2) &&
        swapInt64toByteArray(&buf[LHDCV5_EXTEND_API_A2DP_SPEC3_HEAD_V2],
                             a2dpCfg->codec_specific_3) &&
        swapInt64toByteArray(&buf[LHDCV5_EXTEND_API_A2DP_SPEC4_HEAD_V2],
                             a2dpCfg->codec_specific_4))) {
    log::debug("copy specifics error");
    return false;
  }

  /* retrieve bitmap info of LHDCv5 features */
  if (!A2DP_LhdcV5GetFeatureInfoV2(&buf[LHDCV5_EXTEND_API_A2DP_SPEC_INFO_HEAD_V2])) {
    log::debug("get feature info error!");
    return false;
  }
  return true;
}

static int A2DP_getEncoderExtendFuncA2DPTypeLhdcV5(A2dpCodecConfig* codecCfg, uint8_t* buf,
                                                   uint32_t buflen) {
  ...
  if (buflen < (LHDCV5_EXTEND_API_A2DP_SPEC_VER_SIZE + LHDCV5_EXTEND_API_A2DP_SPEC_CODE_SIZE)) {
    log::debug("buf len too small {}", buflen);
    ...
    case LHDCV5_EXTEND_API_A2DP_SPEC_CODE:
      switch (buf[LHDCV5_EXTEND_API_A2DP_SPEC_ID_HEAD]) {
        ...
        case LHDCV5_EXTEND_API_A2DP_SPEC_AUDIO_CFG:
          ...
          break;
        default:
          log::debug("unknown a2dp codec config");
          return BT_STATUS_FAIL;
      }
      break;
    default:
      log::debug("invalid ext API command(0x{:X})", exFuncCode);
      return BT_STATUS_FAIL;
  }
  ...
  switch (exFuncVer) {
    case LHDCV5_EXTEND_API_A2DP_SPEC_VER2:
      if (!A2DP_LhdcV5GetA2DPSpecificV2(&codec_config_tmp, buf, buflen)) {
        log::debug("get specifics error");
        return BT_STATUS_FAIL;
      }
      break;
    default:
      log::debug("invalid ext API version(0x{:X})", exFuncVer);
      return BT_STATUS_FAIL;
  }
...
int A2dpCodecConfigLhdcV5Base::getLhdcExtendAPIVersion(A2dpCodecConfig* peerCodec, const char* buf,
                                                       const int clen) {
  int result = BT_STATUS_FAIL;

  if (peerCodec == nullptr || buf == nullptr || clen <= 0) {
    log::debug("invalid input clen {}", clen);
    return BT_STATUS_FAIL;
  }

  log::verbose("{}:(buf:{} clen:{})", is_source_ ? "SRC" : "SNK", fmt::ptr(buf), clen);

  if (is_source_) {
    result = A2dpCodecConfigLhdcV5Source::getEncoderExtendFuncUserApiVer(buf, clen);
  } else {
    log::debug("sink not supported");
    return BT_STATUS_FAIL;
  }
...
int A2dpCodecConfigLhdcV5Base::getLhdcExtendAPIConfig(A2dpCodecConfig* peerCodec, const char* buf,
                                                      const int clen) {
  int result = BT_STATUS_FAIL;
...
int A2dpCodecConfigLhdcV5Base::setLhdcExtendAPIConfig(A2dpCodecConfig* peerCodec, const char* buf,
                                                      const int clen) {
  int result = BT_STATUS_FAIL;
...
bool A2dpCodecConfigLhdcV5Base::setLhdcExtendAPIData(A2dpCodecConfig* peerCodec, const char* buf,
                                                     const int clen) {
  if (peerCodec == nullptr || buf == nullptr || clen <= 0) {
    log::debug("invalid input clen {}", clen);
    return BT_STATUS_FAIL;
