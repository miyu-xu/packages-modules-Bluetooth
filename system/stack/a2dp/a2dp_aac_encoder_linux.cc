...
static_assert(false, "AAC cannot be included.")
#endif

    static_assert(BTA_AV_CO_CP_SCMS_T == FALSE);
...
// TODO(): Run the APIs in a sandbox.
class FFmpegInterface {
 public:
  ...
  int encode_pcm(uint8_t* i_buf, int i_len, int bit_depth, uint8_t* o_buf) {
    ...
    if (bit_depth != 16) {
      LOG_ERROR("%s: Unsupported bit depth %d", __func__, bit_depth);
      av_frame_free(&frame);
      return -EINVAL;
    }
    ...
    if (rc == -EAGAIN) {
      LOG_INFO("%s: Encoder is buffering...", __func__);
      av_frame_free(&frame);
      av_packet_free(&pkt);
      return 0;
    } else if (rc < 0) {
    ...
    if (cap < pkt->size + A2DP_AAC_MAX_PREFIX_SIZE) {
      LOG_ERROR("%s: Dropped pkt: size=%d, cap=%d", __func__, pkt->size, cap);
      return -EFBIG;
    } else {
      int fsize = pkt->size;
...
typedef struct {
  ...
  uint32_t timestamp;  // Timestamp embeded into the BT frames
...
void a2dp_aac_encoder_init(const tA2DP_ENCODER_INIT_PEER_PARAMS* p_peer_params,
  ...
  a2dp_aac_encoder_cb = tA2DP_AAC_ENCODER_CB{
      ...
      .timestamp = 0,  // TODO: randomize on init (RFC 6416)
      .feeding_params =
          {
