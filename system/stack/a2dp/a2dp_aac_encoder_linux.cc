...
static void a2dp_aac_encoder_update(A2dpCodecConfig* a2dp_codec_config,
  ...
  *p_config_updated = false;
  ...
  if (!a2dp_aac_encoder_cb.aac_context) {
    ...
    auto check_sample_fmt = [](const AVCodec* codec, enum AVSampleFormat fmt) {
    ...
    if (!check_sample_fmt(codec, AV_SAMPLE_FMT_S16)) {
      LOG_ERROR("%s: Codec does not support s16 format.", __func__);
      return;
    }

    int error = avcodec_open2(ctx, codec, NULL);
    if (error < 0) {
      LOG_ERROR("%s: Could not open codec: '%s'", av_err2str(error));
      return;
    }
  ...
  p_encoder_params->input_channels_n =
      aac_param_value;                             // aac_info.inputChannels;
    ...
    do {
      ...
      if (a2dp_aac_read_feeding(read_buffer, &bytes_read)) {
        ...
        if (av_frame_get_buffer(frame, 0) < 0) {
          LOG_ERROR("%s: failed to get buffer for frame");
          av_frame_free(&frame);
          return;
        }

        frame->pts = frame_pts;
        frame_pts += frame->nb_samples;  // TODO: use .timestamp?

        AVPacket* pkt = av_packet_alloc();

        int error = avcodec_send_frame(ctx, frame);
        if (error < 0) {
          LOG_ERROR("%s: send_frame: '%s'", av_err2str(error));
          a2dp_aac_encoder_cb.stats.media_read_total_dropped_packets++;
          osi_free(p_buf);
          av_frame_free(&frame);
          av_packet_free(&pkt);
        ...
        assert(bytes_read >= pcm_bytes_per_frame);
        memcpy(frame->data, read_buffer, pcm_bytes_per_frame);

        error = avcodec_receive_packet(ctx, pkt);
        if (error < 0) {
          LOG_ERROR("%s: receive_packet: '%s'", av_err2str(error));
          a2dp_aac_encoder_cb.stats.media_read_total_dropped_packets++;
          osi_free(p_buf);
          av_frame_free(&frame);
          av_packet_free(&pkt);
