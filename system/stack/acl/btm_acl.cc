...
void btm_read_remote_features_complete(uint16_t handle, uint8_t* features) {
  ...
  char key[64];
  snprintf(key, sizeof(key), "%s%s%x", IOT_CONF_KEY_RT_SUPP_FEATURES, "_", 0);
  std::string key_str{key};

  DEVICE_IOT_CONFIG_ADDR_SET_BIN(p_acl_cb->remote_addr, key_str,
...
void btm_read_remote_ext_features_complete(uint16_t handle, uint8_t page_num,
  ...
  char key[64];
  snprintf(key, sizeof(key), "%s%s%x", IOT_CONF_KEY_RT_EXT_FEATURES, "_",
           page_num);
  std::string key_str{key};
...
bool acl_set_peer_le_features_from_handle(uint16_t hci_handle,
  ...
  char key[64];
  snprintf(key, sizeof(key), "%s%s%x", IOT_CONF_KEY_LE_RT_FEATURES, "_", 0);
  std::string key_str{key};

  DEVICE_IOT_CONFIG_ADDR_SET_BIN(p_acl->remote_addr, key_str,
