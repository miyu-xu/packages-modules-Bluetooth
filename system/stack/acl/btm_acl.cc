...
void acl_process_supported_features(uint16_t handle, uint64_t features) {
  ...
  log::debug(
      "Copied supported feature pages handle:{} current_page_number:%hhu "
      "features:{}",
...
void acl_process_extended_features(uint16_t handle, uint8_t current_page_number,
  ...
  if (current_page_number > HCI_EXT_FEATURES_PAGE_MAX) {
    log::warn("Unable to process current_page_number:%hhu");
    return;
  }
  tACL_CONN* p_acl = internal_.acl_get_connection_from_handle(handle);
  if (p_acl == nullptr) {
  ...
  log::debug(
      "Copied extended feature pages handle:{} current_page_number:%hhu "
      "max_page_number:%hhu features:{}",
      handle, current_page_number);
