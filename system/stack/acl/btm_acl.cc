...
void on_acl_br_edr_connected(const RawAddress& bda, uint16_t handle,
  ...
  PowerTelemetry::GetInstance()->LogAclLinkDetails(handle, bda, true);
...
void btm_acl_disconnected(tHCI_STATUS status, uint16_t handle,
  ...
  PowerTelemetry::GetInstance()->LogAclLinkDetails(handle, addr, false);
...
void acl_send_data_packet_br_edr(const RawAddress& bd_addr, BT_HDR* p_buf) {
    ...
    PowerTelemetry::GetInstance()->LogAclPktDetails(ACL_PKT_TX, p_buf->len);
...
void acl_rcv_acl_data(BT_HDR* p_msg) {
  ...
  PowerTelemetry::GetInstance()->LogAclPktDetails(ACL_PKT_RX, p_msg->len);
