...
tBTM_STATUS BTM_SetPowerMode(uint8_t pm_id, const RawAddress& remote_bda,
  ...
  log::info(
      "Setting power mode for peer:{} current_mode:{}[%hhu] new_mode:{}[%hhu]",
      ADDRESS_TO_LOGGABLE_CSTR(remote_bda),
      power_mode_state_text(p_cb->state).c_str(), p_cb->state);
...
static tBTM_STATUS btm_pm_snd_md_req(uint16_t handle, uint8_t pm_id,
  ...
  if (p_cb->state == mode) {
    log::info(
        "Link already in requested mode pm_id:%hhu link_ind:{} mode:{}[%hhu]",
        pm_id, link_ind);
...
void btm_pm_proc_mode_change(tHCI_STATUS hci_status, uint16_t hci_handle,
  ...
  log::info("Power mode switched from {}[%hhu] to {}[%hhu]",
