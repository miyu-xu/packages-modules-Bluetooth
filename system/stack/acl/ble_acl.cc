...
static bool acl_ble_common_connection(
    ...
    uint16_t conn_timeout, bool is_discovering) {
...
void acl_ble_enhanced_connection_complete(
    ...
    tBLE_ADDR_TYPE peer_addr_type, bool is_discovering) {
  connection_manager::on_connection_complete(address_with_type.bda);
  ...
  if (!acl_ble_common_connection(resolved_address_with_type, handle, role,
                                 is_in_security_db, conn_interval, conn_latency,
                                 conn_timeout, is_discovering)) {
    LOG_WARN("Unable to create enhanced ble acl connection");
    return;
  }

  btm_ble_refresh_local_resolvable_private_addr(resolved_address_with_type.bda,
                                                local_rpa);

  if (peer_addr_type & BLE_ADDR_TYPE_ID_BIT)
    btm_ble_refresh_peer_resolvable_private_addr(
        resolved_address_with_type.bda, peer_rpa,
        tBTM_SEC_BLE::BTM_BLE_ADDR_RRA);
  btm_ble_update_mode_operation(role, &resolved_address_with_type.bda,
                                HCI_SUCCESS);
