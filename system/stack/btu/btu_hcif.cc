...
void btu_hcif_proc_sp_req_evt(tBTM_SP_EVT event, const uint8_t* p) {
  ...
  switch (event) {
    case BTM_SP_CFM_REQ_EVT:
      ...
      btm_proc_sp_req_evt(BTM_SP_CFM_REQ_EVT, bda, value);
      break;
    case BTM_SP_KEY_NOTIF_EVT:
      STREAM_TO_UINT32(value, p);
      btm_proc_sp_req_evt(BTM_SP_KEY_NOTIF_EVT, bda, value);
      break;
    case BTM_SP_KEY_REQ_EVT:
      // No value needed.
      btm_proc_sp_req_evt(BTM_SP_KEY_REQ_EVT, bda, value);
      break;
  }
}
...
void btu_hcif_read_local_oob_complete(const uint8_t* p, uint16_t evt_len) {
  ...
  btm_read_local_oob_complete(evt_data, status);
  return;
