...
static uint8_t* add_attr(uint8_t* p, uint8_t* p_end, tSDP_DISCOVERY_DB* p_db,
  ...
  switch (attr_type) {
    case UINT_DESC_TYPE:
      if ((is_additional_list != 0) && (attr_len == 2)) {
        ...
        if (id != ATTR_ID_PROTOCOL_DESC_LIST)
          ...
          /* LOG_DEBUG ("SDP - attr nest level:%d(list)", nest_level); */
    ...
    case DATA_ELE_ALT_DESC_TYPE:
      ...
      /* LOG_DEBUG ("SDP - attr nest level:%d", nest_level); */
      if (nest_level >= MAX_NEST_LEVELS) {
        LOG_ERROR("SDP - attr nesting too deep");
        return p_attr_end;
      }
      if (is_additional_list != 0 ||
        ...
        nest_level |= SDP_ADDITIONAL_LIST_MASK;
      /* LOG_DEBUG ("SDP - attr nest level:0x%x(finish)", nest_level); */
