...
static const tSDP_RECORD* sdp_upgrade_pse_record(const tSDP_RECORD* p_rec,
  ...
  status &= SDP_AddProfileDescriptorListToRecord(
      &pbap_102_sdp_rec, UUID_SERVCLASS_PHONE_ACCESS, sdpPseLocalRecord.profile_version);
...
**
** Description     Save pbap 1.2 sdp record attributes values, which would be  used for dynamic version upgrade.
**
** Returns         BOOLEAN
**
***************************************************************************************/
void sdp_save_local_pse_record_attributes(int32_t rfcomm_channel_number,
    ...
    uint32_t supported_repositories) {
  SDP_TRACE_WARNING("rfcomm_channel_number: 0x%x, l2cap_psm: 0x%x profile_version: 0x%x"
