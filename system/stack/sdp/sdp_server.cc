   ...
   if ((p_attr->id != ATTR_ID_BT_PROFILE_DESC_LIST) ||(p_attr->len < SDP_PROFILE_DESC_LENGTH))
    {
       ...
       /* As per current DB implementation UUID is condidered as 16 bit */
        ...
        bool is_blacklisted_1_7 = interop_match_addr_or_name(INTEROP_HFP_1_7_ALLOWLIST,&remote_address,&btif_storage_get_remote_device_property);
            /* For PTS we should update AG's HFP version as 1.7 */
        if (!(is_blacklisted_1_7) && !(osi_property_get_bool("vendor.bt.pts.certification", false)))
            {
             return false;
            }
