  ...
  for (xx = p_ccb->cont_info.next_attr_index; xx < attr_seq.num_attr; xx++) {
    ...
    if (p_attr) {
      if (is_service_avrc_target) {
        uint16_t avrcp_version = sdpu_set_avrc_target_version(p_attr, &(p_ccb->device_address));
        uint16_t version = sdpu_is_avrcp_profile_description_list(p_attr);
        SDP_TRACE_ERROR("SDP attr too big: avrcp_version return = %x,utility = %x",
                          avrcp_version, version);
        sdpu_set_avrc_target_features(p_rec->attribute[1], p_attr, &(p_ccb->device_address), avrcp_version);
      }
