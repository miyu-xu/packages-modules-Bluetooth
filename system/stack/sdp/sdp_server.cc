   ...
   if ((p_attr->id != ATTR_ID_BT_PROFILE_DESC_LIST) ||(p_attr->len < SDP_PROFILE_DESC_LENGTH))
    {
       ...
       /* As per current DB implementation UUID is condidered as 16 bit */
        if (((p_attr->value_ptr[3] << SDP_PROFILE_DESC_LENGTH) | (p_attr->value_ptr[4])) != UUID_SERVCLASS_HF_HANDSFREE)
		{
          return false;
        }
        bool is_allowlisted_1_7 = interop_match_addr_or_name(INTEROP_HFP_1_7_ALLOWLIST,&remote_address,&btif_storage_get_remote_device_property);
            /* For PTS we should update AG's HFP version as 1.7 */
        if (!(is_allowlisted_1_7) && !(osi_property_get_bool("vendor.bt.pts.certification", false)))
            {
             return false;
            }
       p_attr->value_ptr[PROFILE_VERSION_POSITION] = HFP_PROFILE_MINOR_VERSION_7;
       SDP_TRACE_INFO("%s SDP Change HFP Version = %d for %s",__func__,p_attr->value_ptr[PROFILE_VERSION_POSITION],ADDRESS_TO_LOGGABLE_CSTR(remote_address));
       return true;
}
/******************************************************************************
 *
