...
#include <cutils/properties.h> // dynamic HFP  version
...
static void process_service_search_attr_req(tCONN_CB* p_ccb, uint16_t trans_num,
                                            uint16_t param_len, uint8_t* p_req,
                                            uint8_t* p_req_end);
bool sdp_change_hfp_version (const tSDP_ATTRIBUTE *p_attr, RawAddress remote_address); //dynamic HFP version
/******************************************************************************/
...
bool sdp_change_hfp_version (const tSDP_ATTRIBUTE *p_attr, RawAddress remote_address)
{
   ...
   // bool is_blacklisted_1_8 = FALSE; //black listing
        ...
        (p_attr->len >= SDP_PROFILE_DESC_LENGTH))
    {
                ...
                UUID_SERVCLASS_HF_HANDSFREE)
        {
            is_blacklisted_1_7 = interop_match_addr_or_name(INTEROP_HFP_1_7_BLACKLIST,
                                                           &remote_address, &btif_storage_get_remote_device_property);
...
static void process_service_attr_req(tCONN_CB* p_ccb, uint16_t trans_num,
  ...
  bool is_cont = false;
  bool is_hfp_fallback = false; //Dynamic HFP version
  uint16_t attr_len;
  ...
  for (xx = p_ccb->cont_info.next_attr_index; xx < attr_seq.num_attr; xx++) {
    ...
    if (p_attr) {
	  ...
	  is_hfp_fallback = sdp_change_hfp_version (p_attr, p_ccb->device_address); //Dynamic HFP version
