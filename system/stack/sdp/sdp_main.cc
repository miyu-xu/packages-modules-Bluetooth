...
static void sdp_config_ind(uint16_t l2cap_cid, tL2CAP_CFG_INFO* p_cfg) {
  ...
  LOG_DEBUG("SDP - Rcvd cfg ind, sent cfg cfm, CID: 0x%x", l2cap_cid);
}
...
static void sdp_config_cfm(uint16_t l2cap_cid, uint16_t initiator,
  ...
  LOG_DEBUG("SDP - Rcvd cfg cfm, CID: 0x%x", l2cap_cid);
...
tCONN_CB* sdp_conn_originate(const RawAddress& p_bd_addr) {
  ...
  LOG_DEBUG("%s: SDP - Originate started for peer %s", __func__,
            ADDRESS_TO_LOGGABLE_CSTR(p_bd_addr));
...
void sdp_disconnect(tCONN_CB* p_ccb, tSDP_REASON reason) {
  tCONN_CB& ccb = *p_ccb;
  LOG_DEBUG("SDP - disconnect  CID: 0x%x", ccb.connection_id);
...
static void sdp_disconnect_cfm(uint16_t l2cap_cid,
  ...
  LOG_DEBUG("SDP - Rcvd L2CAP disc cfm, CID: 0x%x", l2cap_cid);
...
void sdp_conn_timer_timeout(void* data) {
  ...
  LOG_DEBUG("SDP - CCB timeout in state: %d  CID: 0x%x", ccb.con_state,
            ccb.connection_id);
