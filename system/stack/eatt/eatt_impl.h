...
struct eatt_impl {
  ...
  void eatt_l2cap_connect_ind(const RawAddress& bda,
    ...
    /* Android lets the Central to create EATT, but some PTS test cases
     * wants us to do it anyway.*/
    if (stack_config_get_interface()
  ...
  void eatt_retry_after_collision_if_needed(eatt_device* eatt_dev) {
    ...
    if (role == HCI_ROLE_CENTRAL) {
      ...
      connect_eatt_wrap(eatt_dev);
                   ...
                   ->get_pts_eatt_peripheral_collision_support()) {
      /* This is only for the PTS. Android does not setup EATT when peripheral.
       */
      usleep(300 * 1000);
      connect_eatt_wrap(eatt_dev);
    }
  ...
  void eatt_l2cap_collision_ind(const RawAddress& bda) {
    ...
    /* Remote wanted to setup channels as well. Let's retry his request when we
     * are done with ours.*/
  ...
  void connect(const RawAddress& bd_addr) {
    ...
    if (stack_config_get_interface()->get_pts_connect_eatt_unconditionally()) {
      /* For PTS just start connecting EATT right away but with some delay to
       * let PTS to configure it self. */
