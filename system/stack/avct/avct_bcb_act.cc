...
void avct_bcb_open_ind(tAVCT_BCB* p_bcb, tAVCT_LCB_EVT* p_data) {
  ...
  if (p_ccb_bind == nullptr) {
    log::warn("Ignoring incoming browse request from peer:{} lcid:0x{:04x}", p_bcb->peer_addr,
              p_bcb->ch_lcid);
  ...
  if (!p_bcb->p_tx_msg) {
    log::warn("Received browse packet with no browse data peer:{} lcid:{}", p_bcb->peer_addr,
              p_bcb->ch_lcid);
    return;
  }
...
void avct_bcb_discard_msg(tAVCT_BCB* p_bcb, tAVCT_LCB_EVT* p_data) {
  ...
  if (p_bcb->p_tx_msg) {
    ...
    log::verbose("ch_state: {}, allocated:{}->{}", avct_ch_state_text(p_bcb->ch_state),
