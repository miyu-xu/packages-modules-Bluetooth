...
static BT_HDR* avct_lcb_msg_asmbl(tAVCT_LCB* p_lcb, BT_HDR* p_buf) {
  ...
  /* single packet */
    ...
    if (p_lcb->p_rx_msg != NULL)
      LOG_WARN("Got single during reassembly");
  ...
  /* start packet */
    ...
    if (p_lcb->p_rx_msg != NULL)
      LOG_WARN("Got start during reassembly");
  ...
  if (android::sysprop::bluetooth::A2dp::src_sink_coexist().value_or(false)) {
    ...
    for (i = 0; i < AVCT_NUM_CONN; i++, p_ccb++) {
      if (p_ccb->allocated && (p_ccb->p_lcb == p_lcb) &&
        ...
        LOG_DEBUG("%s, find int handle %d", __func__, i);
        is_originater = true;
      }
    }
    ...
    for (i = 0; i < AVCT_NUM_CONN; i++, p_ccb++) {
       ...
       */
      LOG_DEBUG("%s, %d ccb to lcb, alloc %d, lcb %p, role %d, pid 0x%x",
...
void avct_lcb_send_msg(tAVCT_LCB* p_lcb, tAVCT_LCB_EVT* p_data) {
  ...
  LOG_DEBUG("%s tx_q_count:%d", __func__, fixed_queue_length(p_lcb->tx_q));
  return;
}
...
void avct_lcb_msg_ind(tAVCT_LCB* p_lcb, tAVCT_LCB_EVT* p_data) {
  ...
  if (cr_ipid == AVCT_CR_IPID_INVALID) {
    LOG_WARN("Invalid cr_ipid", cr_ipid);
    osi_free_and_reset((void**)&p_data->p_buf);
    return;
  }
