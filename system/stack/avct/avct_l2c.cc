...
static bool avct_l2c_is_passive(tAVCT_LCB* p_lcb) {
  ...
  for (i = 0; i < AVCT_NUM_CONN; i++, p_ccb++) {
    if (p_ccb->allocated && (p_ccb->p_lcb == p_lcb)) {
      LOG_DEBUG("avct_l2c_is_ct control:x%x", p_ccb->cc.control);
...
void avct_l2c_connect_ind_cback(const RawAddress& bd_addr, uint16_t lcid,
  ...
  /* else we already have a channel for this peer */
    ...
    if (!avct_l2c_is_passive(p_lcb) || (p_lcb->ch_state == AVCT_CH_OPEN)) {
      ...
      LOG_DEBUG("avct_l2c_connect_ind_cback conflict_lcid:0x%x",
                p_lcb->conflict_lcid);
    }
  }

  if (p_lcb) {
    LOG_DEBUG("avct_l2c_connect_ind_cback: 0x%x, res: %d, ch_state: %d", lcid,
  ...
  if (p_lcb)
    LOG_DEBUG("ch_state cni: %d ", p_lcb->ch_state);
}
...
static void avct_on_l2cap_error(uint16_t lcid, uint16_t result) {
  ...
  if (p_lcb->ch_state == AVCT_CH_CONN) {
    LOG_DEBUG("avct_l2c_connect_cfm_cback conflict_lcid:0x%x",
              p_lcb->conflict_lcid);
    ...
    LOG_DEBUG("ERROR avct_l2c_config_cfm_cback L2CA_DisconnectReq %d ",
              p_lcb->ch_state);
    /* store result value */
...
void avct_l2c_connect_cfm_cback(uint16_t lcid, uint16_t result) {
  ...
  if (p_lcb != NULL) {
    LOG_DEBUG("avct_l2c_connect_cfm_cback lcid:0x%x result: %d ch_state: %d, "
              "conflict_lcid:0x%x",
    ...
    if (p_lcb->ch_state == AVCT_CH_CONN) {
      ...
      LOG_DEBUG("avct_l2c_connect_cfm_cback ch_state: %d, conflict_lcid:0x%x",
    ...
    LOG_DEBUG("ch_state cnc: %d ", p_lcb->ch_state);
  }
}
...
void avct_l2c_config_cfm_cback(uint16_t lcid, uint16_t initiator,
  ...
  p_lcb = avct_lcb_by_lcid(lcid);
  if (p_lcb != NULL) {
    LOG_DEBUG("avct_l2c_config_cfm_cback: 0x%x, ch_state: %d,", lcid,
              p_lcb->ch_state);
    /* if in correct state */
    ...
    LOG_DEBUG("ch_state cfc: %d ", p_lcb->ch_state);
  }
}
...
void avct_l2c_config_ind_cback(uint16_t lcid, tL2CAP_CFG_INFO* p_cfg) {
  ...
  p_lcb = avct_lcb_by_lcid(lcid);
  if (p_lcb != NULL) {
    LOG_DEBUG("avct_l2c_config_ind_cback: 0x%x, ch_state: %d", lcid,
              p_lcb->ch_state);
...
void avct_l2c_disconnect_ind_cback(uint16_t lcid, bool ack_needed) {
  ...
  p_lcb = avct_lcb_by_lcid(lcid);
  if (p_lcb != NULL) {
    LOG_DEBUG("avct_l2c_disconnect_ind_cback: 0x%x, ch_state: %d", lcid,
              p_lcb->ch_state);
    tAVCT_LCB_EVT avct_lcb_evt;
    avct_lcb_evt.result = result;
    avct_lcb_event(p_lcb, AVCT_LCB_LL_CLOSE_EVT, &avct_lcb_evt);
    LOG_DEBUG("ch_state di: %d ", p_lcb->ch_state);
  }
}
...
void avct_l2c_disconnect(uint16_t lcid, uint16_t result) {
  ...
  p_lcb = avct_lcb_by_lcid(lcid);
  if (p_lcb != NULL) {
    LOG_DEBUG("avct_l2c_disconnect_cfm_cback: 0x%x, ch_state: %d, res: %d",
    ...
    avct_lcb_event(p_lcb, AVCT_LCB_LL_CLOSE_EVT, &avct_lcb_evt);
    LOG_DEBUG("ch_state dc: %d ", p_lcb->ch_state);
  }
}
...
void avct_l2c_congestion_ind_cback(uint16_t lcid, bool is_congested) {
  ...
  LOG_DEBUG("avct_l2c_congestion_ind_cback: 0x%x", lcid);
  /* look up lcb for this channel */
  p_lcb = avct_lcb_by_lcid(lcid);
  if (p_lcb != NULL) {
    tAVCT_LCB_EVT avct_lcb_evt;
...
void avct_l2c_data_ind_cback(uint16_t lcid, BT_HDR* p_buf) {
  ...
  LOG_DEBUG("avct_l2c_data_ind_cback: 0x%x", lcid);
  /* look up lcb for this channel */
  p_lcb = avct_lcb_by_lcid(lcid);
  if (p_lcb != NULL) {
