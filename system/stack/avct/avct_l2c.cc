...
void avct_l2c_connect_ind_cback(const RawAddress& bd_addr, uint16_t lcid, uint16_t /* psm */,
  ...
  if (p_lcb) {
    log::verbose("Received remote connection request peer:{} lcid:0x{:x}, res:{}, ch_state: {}",
                 bd_addr, lcid, l2cap_result_code_text(result),
                 avct_ch_state_text(p_lcb->ch_state));
  } else {
    log::info("Ignoring remote connection request peer:{} lcid:0x{:x}, res:{}, ch_state: {}",
