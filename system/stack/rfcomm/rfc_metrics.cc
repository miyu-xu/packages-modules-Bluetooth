...
// logged after port closed -> SDP succeeded
void port_collect_attempt_metrics(tPORT* p_port) {
  bool initiated_as_client = !(p_port->is_server);
  // client initiates SDP, server did not so sdp status is N/A
  BtaStatus sdp_status =
          (initiated_as_client) ? BtaStatus::BTA_STATUS_SUCCESS : BtaStatus::BTA_STATUS_UNKNOWN;
  ...
  uint64_t open_duration_ms =
          (sm_cb.close_timestamp - sm_cb.open_timestamp) / 1000;  // to milliseconds
  ...
  os::LogMetricRfcommConnectionAtClose(
          ...
          static_cast<int>(open_duration_ms), static_cast<int>(p_port->app_uid), sdp_status,
          initiated_as_client);
}
