...
void port_collect_attempt_metrics(tPORT* p_port) {
  ...
  auto open_duration =
          static_cast<int32_t>((close_timestamp - sm_cb.open_timestamp) / 1000);  // to milliseconds

  bluetooth::os::LogMetricRfcommConnectionAtClose(
          toPortResult(sm_cb.close_reason), toSecurity(p_port->sec_mask),
          toPortState(sm_cb.second_state_prior), toPortState(sm_cb.state_prior), open_duration,
          static_cast<int32_t>(p_port->app_uid));
}
