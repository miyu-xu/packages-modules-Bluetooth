...
int RFCOMM_RemoveConnection(uint16_t handle) {
  ...
  LOG_INFO("RFCOMM_RemoveConnection() handle:%d", handle);
  ...
  if (!p_port->in_use || (p_port->state == PORT_CONNECTION_STATE_CLOSED)) {
    LOG_INFO("RFCOMM_RemoveConnection() Not opened:%d", handle);
    return (PORT_SUCCESS);
  }
...
int PORT_SetEventCallback(uint16_t port_handle, tPORT_CALLBACK* p_port_cb) {
  ...
  LOG_INFO("PORT_SetEventCallback() handle:%d", port_handle);
...
int PORT_SetDataCOCallback(uint16_t port_handle,
  ...
  LOG_INFO("PORT_SetDataCOCallback() handle:%d cb 0x%x", port_handle,
           p_port_cb);
...
int PORT_SetEventMask(uint16_t port_handle, uint32_t mask) {
  ...
  LOG_INFO("PORT_SetEventMask() handle:%d mask:0x%x", port_handle, mask);
...
int PORT_CheckConnection(uint16_t handle, RawAddress* bd_addr,
  ...
  tPORT* p_port = &rfc_cb.port.port[handle - 1];
  LOG_DEBUG("%s: handle=%d, in_use=%d, port_state=%d, p_mcb=%p, peer_ready=%d, "
            "rfc_state=%d",
...
int PORT_SetState(uint16_t handle, tPORT_STATE* p_settings) {
  ...
  LOG_INFO("PORT_SetState() handle:%d", handle);
  ...
  LOG_INFO("PORT_SetState() handle:%d FC_TYPE:0x%x", handle,
           p_settings->fc_type);
...
int PORT_GetState(uint16_t handle, tPORT_STATE* p_settings) {
  ...
  LOG_INFO("PORT_GetState() handle:%d", handle);
...
int PORT_FlowControl_MaxCredit(uint16_t handle, bool enable) {
  ...
  LOG_INFO("PORT_FlowControl() handle:%d enable: %d", handle, enable);
...
int PORT_ReadData(uint16_t handle, char* p_data, uint16_t max_len,
  ...
  LOG_INFO("PORT_ReadData() handle:%d max_len:%d", handle, max_len);
  ...
  *p_len = 0;
  ...
  if (*p_len == 1) {
    LOG_INFO("PORT_ReadData queue:%d returned:%d %x", p_port->rx.queue_size,
             *p_len, (p_data[0]));
  } else {
    LOG_INFO("PORT_ReadData queue:%d returned:%d", p_port->rx.queue_size,
             *p_len);
  }
...
static int port_write(tPORT* p_port, BT_HDR* p_buf) {
  ...
  if (p_port->tx.peer_fc || !p_port->rfc.p_mcb ||
    ...
    LOG_INFO("PORT_Write : Data is being sent");
...
int PORT_WriteDataCO(uint16_t handle, int* p_len) {
  ...
  LOG_INFO("PORT_WriteDataCO() handle:%d", handle);
  *p_len = 0;
  ...
  if (!p_port->p_data_co_callback(handle, (uint8_t*)&available,
    ...
    LOG_ERROR("p_data_co_callback DATA_CO_CALLBACK_TYPE_INCOMING_SIZE failed, "
              "available:%d",
              available);
    return (PORT_UNKNOWN_ERROR);
  }
  ...
  while (available) {
    ...
    LOG_INFO("PORT_WriteData %d bytes", length);
...
int PORT_WriteData(uint16_t handle, const char* p_data, uint16_t max_len,
  ...
  LOG_INFO("PORT_WriteData() max_len:%d", max_len);
