...
static void rfc_set_port_settings(tPORT_SETTINGS* port_settings, MX_FRAME* p_frame);
...
void rfc_process_rpn(tRFC_MCB* p_mcb, bool is_command, bool is_request, MX_FRAME* p_frame) {
  ...
  if (p_port == nullptr) {
    ...
    if (is_command) {
      memset(&port_settings, 0, sizeof(tPORT_SETTINGS));
 ...
 *
 * Description      This function sets the tPORT_SETTINGS structure given a
 *                  p_frame.
 *
 ******************************************************************************/

void rfc_set_port_settings(tPORT_SETTINGS* port_settings, MX_FRAME* p_frame) {
