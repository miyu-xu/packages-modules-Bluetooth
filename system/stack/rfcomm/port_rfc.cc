...
void PORT_DlcEstablishInd(tRFC_MCB* p_mcb, uint8_t dlci, uint16_t mtu) {
  ...
  if (com::android::bluetooth::flags::indicate_rfcomm_connection_complete_after_msc()) {
    if ((!p_port->rfc_cfg_info.data_path_present ||
         p_port->rfc_cfg_info.data_path != BTSOCK_DATA_PATH_HARDWARE_OFFLOAD) &&
...
void PORT_ControlInd(tRFC_MCB* p_mcb, uint8_t dlci, tPORT_CTRL* p_pars) {
  ...
  if (com::android::bluetooth::flags::indicate_rfcomm_connection_complete_after_msc()) {
    if (p_port->rfc_cfg_info.data_path_present &&
        p_port->rfc_cfg_info.data_path == BTSOCK_DATA_PATH_HARDWARE_OFFLOAD) {
      log::verbose("port_ctrl: 0x{:x}", p_port->port_ctrl);
      if (p_port->port_ctrl == PORT_CTRL_SETUP_COMPLETED) {
        if (p_port->p_mgmt_callback) {
          p_port->p_mgmt_callback(PORT_SUCCESS, p_port->handle);
          log_counter_metrics(
