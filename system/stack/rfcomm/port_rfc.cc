...
int port_open_continue(tPORT* p_port) {
  LOG_INFO("port_open_continue, p_port:%p", p_port);
  ...
  if (p_mcb->state == RFC_MX_STATE_CONNECTED) {
    ...
    LOG_DEBUG("port_open_continue: mx state(%d) mx channel is openning",
              p_mcb->state);
  }
  return (PORT_SUCCESS);
}
...
void PORT_StartCnf(tRFC_MCB* p_mcb, uint16_t result) {
  ...
  LOG_INFO("%s: result %d", __func__, result);
  ...
  for (int i = 0; i < MAX_RFC_PORTS; i++, p_port++) {
    if (p_port->rfc.p_mcb == p_mcb) {
      ...
      if (result == RFCOMM_SUCCESS) {
        LOG_INFO("%s: dlci %d", __func__, p_port->dlci);
        RFCOMM_ParameterNegotiationRequest(p_mcb, p_port->dlci, p_port->mtu);
      } else {
...
void PORT_StartInd(tRFC_MCB* p_mcb) {
  ...
  LOG_INFO("PORT_StartInd");

  p_port = &rfc_cb.port.port[0];
  for (i = 0; i < MAX_RFC_PORTS; i++, p_port++) {
    if ((p_port->rfc.p_mcb == NULL) || (p_port->rfc.p_mcb == p_mcb)) {
      LOG_DEBUG("PORT_StartInd, RFCOMM_StartRsp RFCOMM_SUCCESS: p_mcb:%p",
                p_mcb);
...
void PORT_ParNegInd(tRFC_MCB* p_mcb, uint8_t dlci, uint16_t mtu, uint8_t cl,
                    uint8_t k) {
  LOG_INFO("%s: bd_addr=%s, dlci=%d, mtu=%d", __func__,
  ...
  if (!p_port) {
    ...
    LOG_INFO("%s: port_handles[dlci:%d]:%d->%d", __func__, dlci,
...
void PORT_ParNegCnf(tRFC_MCB* p_mcb, uint8_t dlci, uint16_t mtu, uint8_t cl,
                    uint8_t k) {
  LOG_INFO("PORT_ParNegCnf dlci:%d mtu:%d cl: %d k: %d", dlci, mtu, cl, k);
  tPORT* p_port = port_find_mcb_dlci_port(p_mcb, dlci);
  if (!p_port) {
...
void PORT_DlcEstablishInd(tRFC_MCB* p_mcb, uint8_t dlci, uint16_t mtu) {
  ...
  LOG_DEBUG("PORT_DlcEstablishInd p_mcb:%p, dlci:%d mtu:%di, p_port:%p", p_mcb,
            dlci, mtu, p_port);
  VLOG(1) << __func__
...
void PORT_DlcEstablishCnf(tRFC_MCB* p_mcb, uint8_t dlci, uint16_t mtu,
  ...
  LOG_INFO("PORT_DlcEstablishCnf dlci:%d mtu:%d result:%d", dlci, mtu, result);
...
void PORT_ControlInd(tRFC_MCB* p_mcb, uint8_t dlci, tPORT_CTRL* p_pars) {
  ...
  LOG_INFO("PORT_ControlInd DTR_DSR : %d, RTS_CTS : %d, RI : %d, DCD : %d",
...
void PORT_DataInd(tRFC_MCB* p_mcb, uint8_t dlci, BT_HDR* p_buf) {
  ...
  LOG_INFO("PORT_DataInd with data length %d, p_mcb:%p,p_port:%p,dlci:%d",
  ...
  if ((p_port->rx.queue_size + p_buf->len > PORT_RX_CRITICAL_WM) ||
    ...
    LOG_INFO("PORT_DataInd. Buffer over run. Dropping the buffer");
    osi_free(p_buf);
...
void PORT_FlowInd(tRFC_MCB* p_mcb, uint8_t dlci, bool enable_data) {
  ...
  LOG_INFO("PORT_FlowInd fc:%d", enable_data);
  ...
  if (p_port->tx.queue_size > 0) {
    ...
    while (!p_port->tx.peer_fc && p_port->rfc.p_mcb &&
      ...
      if (p_buf != NULL) {
        ...
        LOG_DEBUG("Sending RFCOMM_DataReq tx.queue_size=%d",
