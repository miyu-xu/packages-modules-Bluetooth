...
tPORT* port_allocate_port(uint8_t dlci, const RawAddress& bd_addr) {
  ...
  for (int loop_counter = 0; loop_counter < MAX_RFC_PORTS;
    ...
    if (!p_port->in_use) {
      ...
      LOG_DEBUG("%s: rfc_cb.port.port[%d]:%p chosen, "
...
void port_select_mtu(tPORT* p_port) {
  ...
  if (p_port->mtu == 0) {
    ...
    if (packet_size == 0) {
      ...
      if ((L2CAP_MTU_SIZE + L2CAP_PKT_OVERHEAD) >= packet_size) {
        ...
        LOG_DEBUG("%s: selected %d based on connection speed", __func__,
                  p_port->mtu);
      } else {
        p_port->mtu = L2CAP_MTU_SIZE - RFCOMM_DATA_OVERHEAD;
        LOG_DEBUG("%s: selected %d based on l2cap PDU size", __func__,
                  p_port->mtu);
      }
    }
  } else {
    LOG_DEBUG("%s: application selected %d", __func__, p_port->mtu);
  }
  ...
  LOG_DEBUG("%s: credit_rx_max %d, credit_rx_low %d, rx_buf_critical %d",
...
void port_release_port(tPORT* p_port) {
  LOG_DEBUG("%s p_port: %p state: %d keep_handle: %d", __func__, p_port,
  ...
  if (p_port->rfc.state == RFC_STATE_CLOSED) {
    ...
    if (p_port->keep_port_handle) {
      LOG_DEBUG("%s Re-initialize handle: %d", __func__, p_port->handle);
      ...
      LOG_DEBUG("%s Clean-up handle: %d", __func__, p_port->handle);
  ...
  /* else using TS 07.10 flow control */
    ...
    /* else want to disable flow from peer */
      ...
      /* level and flow control has not been sent to the peer do it now */
               ...
               !p_port->rx.peer_fc) {
        LOG_INFO("PORT_DataInd Data reached HW. Sending FC set.");
