...
tRFC_MCB* rfc_alloc_multiplexer_channel(const RawAddress& bd_addr,
  ...
  LOG_DEBUG("rfc_alloc_multiplexer_channel:is_initiator:%d", is_initiator);

  for (i = 0; i < MAX_BD_CONNECTIONS; i++) {
    LOG_DEBUG("rfc_alloc_multiplexer_channel rfc_cb.port.rfc_mcb[%d].state:%d",
...
void rfc_timer_start(tRFC_MCB* p_mcb, uint16_t timeout) {
  LOG_INFO("%s - timeout:%d seconds", __func__, timeout);
...
void rfc_inc_credit(tPORT* p_port, uint8_t credit) {
  if (p_port->rfc.p_mcb->flow == PORT_FC_CREDIT) {
    ...
    LOG_INFO("rfc_inc_credit:%d", p_port->credit_tx);
