...
void RFCOMM_DisconnectInd(uint16_t lcid, bool is_conf_needed) {
  log::verbose("lcid:{:x}, is_conf_needed:{}", lcid, is_conf_needed);
  tRFC_MCB* p_mcb = rfc_find_lcid_mcb(lcid);
  if (!p_mcb) {
    log::warn("no mcb for lcid {:x}", lcid);
    return;
  }
...
void RFCOMM_BufDataInd(uint16_t lcid, BT_HDR* p_buf) {
  ...
  if (!p_mcb) {
    log::warn("Cannot find RFCOMM multiplexer for lcid {:x}", lcid);
    osi_free(p_buf);
    return;
  }
  ...
  if (event == RFC_EVENT_BAD_FRAME) {
    log::warn("Bad RFCOMM frame from lcid={:x}, bd_addr={}, p_mcb={}", lcid,
  ...
  if (p_port == nullptr || !p_port->rfc.p_mcb) {
    ...
    if (event != RFC_EVENT_SABME) {
      log::warn("no for none-SABME event, lcid={:x}, bd_addr={}, p_mcb={}",
                lcid, p_mcb->bd_addr, fmt::ptr(p_mcb));
      if ((p_mcb->is_initiator && !rfc_cb.rfc.rx_frame.cr) ||
          (!p_mcb->is_initiator && rfc_cb.rfc.rx_frame.cr)) {
        log::error("Disconnecting RFCOMM, lcid={:x}, bd_addr={}, p_mcb={}",
                   lcid, p_mcb->bd_addr, fmt::ptr(p_mcb));
    ...
    if (p_port == nullptr) {
      log::error(
          "Disconnecting RFCOMM, no port for dlci {}, lcid={:x}, bd_addr={}, "
          "p_mcb={}",
...
tRFC_MCB* rfc_find_lcid_mcb(uint16_t lcid) {
  ...
  if (p_mcb != nullptr) {
    if (p_mcb->lcid != lcid) {
      log::warn("LCID reused lcid=:{:x}, current_lcid={:x}", lcid, p_mcb->lcid);
      return nullptr;
    }
  }
  return p_mcb;
