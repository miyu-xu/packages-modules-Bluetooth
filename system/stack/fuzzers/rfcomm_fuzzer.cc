...
static int ServerInit(FuzzedDataProvider& fdp, uint16_t* server_handle) {
  ...
  status = PORT_SetEventCallback(*server_handle, port_event_cback);
  return status;
}
...
static int ClientInit(FuzzedDataProvider& fdp, uint16_t* client_handle) {
  ...
  status = PORT_SetEventMask(*client_handle, PORT_EV_RXCHAR);
  if (status != PORT_SUCCESS) {
    return status;
  }
  status = PORT_SetEventCallback(*client_handle, port_event_cback);
  return status;
}
