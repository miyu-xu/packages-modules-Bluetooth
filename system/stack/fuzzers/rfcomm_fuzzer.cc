...
void port_mgmt_cback(uint32_t code, uint16_t port_handle) {
...
static void Cleanup(uint16_t* server_handle) {
  RFCOMM_RemoveServer(*server_handle);
}

static void ServerInit(FuzzedDataProvider& fdp, uint16_t* server_handle) {
  RFCOMM_Init();
  ...
  RFCOMM_CreateConnectionWithSecurity(uuid, scn, true, mtu, kDummyAddr,
                                      server_handle, port_mgmt_cback, 0);
  PORT_SetEventMask(*server_handle, PORT_EV_RXCHAR);
  PORT_SetEventCallback(*server_handle, port_event_cback);
}

static void FuzzAsServer(FuzzedDataProvider& fdp) {
  auto server_handle = fdp.ConsumeIntegralInRange<uint16_t>(1, MAX_RFC_PORTS);
  ServerInit(fdp, &server_handle);
...
static void ClientInit(FuzzedDataProvider& fdp, uint16_t* client_handle) {
  RFCOMM_Init();
  ...
  RFCOMM_CreateConnectionWithSecurity(uuid, scn, false, mtu, kDummyAddr,
                                      client_handle, port_mgmt_cback, 0);
  PORT_SetEventMask(*client_handle, PORT_EV_RXCHAR);
  PORT_SetEventCallback(*client_handle, port_event_cback);
}

static void FuzzAsClient(FuzzedDataProvider& fdp) {
  auto client_handle = fdp.ConsumeIntegralInRange<uint16_t>(1, MAX_RFC_PORTS);

  ClientInit(fdp, &client_handle);
