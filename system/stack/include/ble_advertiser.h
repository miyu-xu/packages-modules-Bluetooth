...
class BleAdvertisingManager {
 public:
  ...
  virtual void StartAdvertisingSet(
      ...
      std::vector<uint8_t> periodic_data, std::vector<uint8_t> periodic_data_enc ,
      uint16_t duration, uint8_t maxExtAdvEvents, std::vector<uint8_t> enc_key_value,
  ...
  virtual void SetData(uint8_t inst_id, bool is_scan_rsp,
                       std::vector<uint8_t> data, std::vector<uint8_t> data_enc ,MultiAdvCb cb) = 0;
