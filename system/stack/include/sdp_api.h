...
struct tLegacyStackSdpApi {
   ...
   * Description      This function cancels an active query to an SDP server.
   *
   * Returns          true if discovery cancelled, false if a matching activity
   *is not found.
   *
   ******************************************************************************/
   ...
   *                  The difference between this API function and the function
   *                  SDP_ServiceSearchRequest is that this one does a
   *                  combined ServiceSearchAttributeRequest SDP function with
   *the user data piggyback
   *
   * Returns          true if discovery started, false if failed.
   *
   ******************************************************************************/
   ...
   *
   * NOTE             the only difference between this function and the previous
   *                  function "SDP_FindServiceInDb()" is that this function
   *takes a Uuid input.
   *
   * Returns          Pointer to record containing service class, or NULL
   *
   ...
   *beginning of the database, else continue from the next record after
   *                  p_start_rec.
   *
   * Returns          Pointer to record containing service class, or NULL
   *
   ...
   *database. This would be through the SDP database maintenance API. The record
   *is created empty, teh application should then call "add_attribute" to add
   ...
   *                  maintenance API.
   *
   *                  If a record handle of 0 is passed, all records are
   *deleted.
   *
   * Returns          true if succeeded, else false
   *
   ...
   *record. This would be through the SDP database maintenance API. If the
   *sequence already exists in the record, it is replaced with the new sequence.
   *
   * Returns          true if added OK, else false
   *
   ******************************************************************************/
   ...
   *to a record. This would be through the SDP database maintenance API. If the
   *protocol list already exists in the record, it is replaced with the new
   *list.
   *
   * Returns          true if added OK, else false
   *
   ...
   *to a record. This would be through the SDP database maintenance API. If the
   *version already exists in the record, it is replaced with the new one.
   *
   * Returns          true if added OK, else false
   *
   ******************************************************************************/
...
const struct tLegacyStackSdpApi* get_legacy_stack_sdp_api();
