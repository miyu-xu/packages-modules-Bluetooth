...
/// Generate the `tA2DP_CODEC_ID` tag value for a vendor codec identified
/// by the input `company_id` and `codec_id`.
consteval static uint64_t VendorCodecId(uint16_t company_id, uint16_t codec_id) {
  return 0xff
      | (static_cast<uint64_t>(company_id) << 8)
      | (static_cast<uint64_t>(codec_id) << 24);
}
...
CodecId ParseCodecId(uint8_t const media_codec_capabilities[]);

}  // namespace bluetooth::a2bp
