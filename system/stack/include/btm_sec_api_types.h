...
inline std::string security_mode_text(const tSECURITY_MODE& security_mode) {
  switch (security_mode) {
    ...
    default:
      return android::base::StringPrintf("UNKNOWN[%hhu]", security_mode);
  }
}
...
inline std::string linkkey_type_text(const int linkkey_type) {
  switch (linkkey_type) {
    ...
    default:
      return android::base::StringPrintf("UNKNOWN[0x%02x]", linkkey_type);
  }
}
...
inline std::string sp_evt_to_text(const tBTM_SP_EVT evt) {
  ...
  return android::base::StringPrintf("UNKNOWN[%hhu]", evt);
}
...
inline std::string io_capabilities_text(const tBTM_IO_CAP& io_caps) {
  switch (io_caps) {
    ...
    default:
      return android::base::StringPrintf("UNKNOWN[%hhu]", io_caps);
  }
}
...
inline std::string bond_type_text(const tBTM_BOND_TYPE& bond_type) {
  switch (bond_type) {
    ...
    default:
      return android::base::StringPrintf("UNKNOWN[%hhu]", bond_type);
  }
}
