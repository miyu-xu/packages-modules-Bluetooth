...
inline size_t bd_name_copy(BD_NAME bd_name_dest, const BD_NAME bd_name_src) {
  return osi_strlcpy(reinterpret_cast<char*>(bd_name_dest), reinterpret_cast<const char*>(bd_name_src),
                 kBdNameLength + 1);
}
...
inline void bd_name_from_char_pointer(BD_NAME bd_name_dest, const char* bd_name_char) {
  ...
  size_t src_len = osi_strlcpy(reinterpret_cast<char*>(bd_name_dest), bd_name_char, sizeof(BD_NAME));
