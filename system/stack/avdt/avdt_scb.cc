...
#if (AVDT_DEBUG == TRUE)
  LOG_INFO("%s: SCB hdl=%d event=%d/%s state=%s p_avdt_scb=%p scb_index=%d",
...
AvdtpScb* avdt_scb_alloc(uint8_t peer_id,
  ...
  for (int i = 0; i < AVDT_NUM_SEPS; i++, p_scb++) {
    if (!p_scb->allocated) {
      ...
      LOG_DEBUG("%s: allocated (handle=%d, psc_mask:0x%x)", __func__,
...
void avdt_scb_dealloc(AvdtpScb* p_scb, UNUSED_ATTR tAVDT_SCB_EVT* p_data) {
  LOG_DEBUG("%s: hdl=%d", __func__, avdt_scb_to_hdl(p_scb));
  p_scb->Recycle();
}
...
AvdtpScb* avdt_scb_by_hdl(uint8_t hdl) {
  ...
  LOG_DEBUG("%s: SCB for handle %d found: p_scb=%p scb_index=%d", __func__, hdl,
            p_scb, p_scb->stream_config.scb_index);
  return p_scb;
}
...
uint8_t avdt_scb_verify(AvdtpCcb* p_ccb, uint8_t state, uint8_t* p_seid,
  ...
  LOG_DEBUG("avdt_scb_verify state %d", state);
