...
uint8_t avdt_ad_type_to_tcid(uint8_t type, AvdtpScb* p_scb) {
  ...
  log::verbose("type: {} -> tcid: {}", tc_type_text(type), tcid);
  return tcid;
}
...
static uint8_t avdt_ad_tcid_to_type(uint8_t tcid) {
  ...
  */
  uint8_t type = ((tcid + AVDT_CHAN_NUM_TYPES - 2) % (AVDT_CHAN_NUM_TYPES - 1)) + 1;
  log::verbose("tcid: {} -> type: {}", tcid, tc_type_text(type));
  return type;
}
...
uint8_t avdt_ad_tc_tbl_to_idx(AvdtpTransportChannel* p_tbl) {
  uint8_t index = (uint8_t)(p_tbl - avdtp_cb.ad.tc_tbl);
