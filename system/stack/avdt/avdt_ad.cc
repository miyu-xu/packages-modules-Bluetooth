...
AvdtpScb* AvdtpAdaptationLayer::LookupAvdtpScb(
  ...
  LOG_DEBUG("%s: ccb_idx:%d tcid:%d scb_hdl:%d", __func__, tc.ccb_idx, tc.tcid,
            re.scb_hdl);
...
uint8_t avdt_ad_type_to_tcid(uint8_t type, AvdtpScb* p_scb) {
  ...
  LOG_DEBUG("%s: type:%d, tcid: %d", __func__, type, tcid);
  return tcid;
}
...
static uint8_t avdt_ad_tcid_to_type(uint8_t tcid) {
  ...
  LOG_DEBUG("tcid: %d, type: %d", tcid, type);
  return type;
}
...
uint8_t avdt_ad_tc_tbl_to_idx(AvdtpTransportChannel* p_tbl) {
  LOG_DEBUG("avdt_ad_tc_tbl_to_idx: %ld", (long)(p_tbl - avdtp_cb.ad.tc_tbl));
...
void avdt_ad_tc_close_ind(AvdtpTransportChannel* p_tbl) {
  ...
  LOG_DEBUG("%s: tcid: %d, old: %d", __func__, p_tbl->tcid, close.old_tc_state);
...
void avdt_ad_tc_open_ind(AvdtpTransportChannel* p_tbl) {
  ...
  LOG_DEBUG("%s: p_tbl:%p state:%d ccb_idx:%d tcid:%d scb_hdl:%d", __func__,
...
void avdt_ad_open_req(uint8_t type, AvdtpCcb* p_ccb, AvdtpScb* p_scb,
  ...
  LOG_DEBUG("avdt_ad_open_req: type: %d, role: %d, tcid:%d", type, role,
            p_tbl->tcid);
  ...
  if (type == AVDT_CHAN_SIG) {
    ...
    LOG_DEBUG("avdtp_cb.ad.rt_tbl[%d][%d].scb_hdl = %d", avdt_ccb_to_idx(p_ccb),
...
void avdt_ad_close_req(uint8_t type, AvdtpCcb* p_ccb, AvdtpScb* p_scb) {
  ...
  LOG_DEBUG("avdt_ad_close_req state: %d", p_tbl->state);
