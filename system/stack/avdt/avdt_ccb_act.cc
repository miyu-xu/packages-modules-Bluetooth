...
void avdt_ccb_hdl_discover_cmd(AvdtpCcb* p_ccb, tAVDT_CCB_EVT* p_data) {
  ...
  LOG_DEBUG("%s: p_ccb index=%d", __func__, avdt_ccb_to_idx(p_ccb));
...
void avdt_ccb_snd_start_cmd(AvdtpCcb* p_ccb, tAVDT_CCB_EVT* p_data) {
  ...
  if (avdt_msg.hdr.err_param == 0) {
    LOG_DEBUG("%s: AVDT_SIG_START", __func__);
    ...
    for (i = 0; i < p_data->msg.multi.num_seps; i++) {
      ...
      if (p_scb != NULL) {
        LOG_DEBUG("%s: AVDT_SCB_MSG_START_REJ_EVT: i=%d", __func__, i);
...
void avdt_ccb_set_disconn(AvdtpCcb* p_ccb, tAVDT_CCB_EVT* p_data) {
  /*
  LOG_INFO("avdt_ccb_set_disconn:conn:x%x, api:x%x",
...
void avdt_ccb_ll_closed(AvdtpCcb* p_ccb, UNUSED_ATTR tAVDT_CCB_EVT* p_data) {
  ...
  LOG_DEBUG("%s peer %s", __func__, ADDRESS_TO_LOGGABLE_CSTR(p_ccb->peer_addr));
...
void avdt_ccb_ll_opened(AvdtpCcb* p_ccb, tAVDT_CCB_EVT* p_data) {
  ...
  LOG_DEBUG("%s peer %s BtaAvScbIndex=%d p_ccb=%p", __func__,
            ADDRESS_TO_LOGGABLE_CSTR(p_ccb->peer_addr), p_ccb->BtaAvScbIndex(),
            p_ccb);
