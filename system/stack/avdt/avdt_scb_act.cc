 ...
 *
 * Description      This function checks if the mandatory codec should be prioritize
 *                  despite the remote peer codec config choice.
 *
 *
 * Returns          bool.
 *
 ******************************************************************************/
static bool avdt_scb_prioritize_mandatory_codec(RawAddress peer_addr, uint8_t sep_type,
                                                uint8_t* codec_info) {
  if (!com::android::bluetooth::flags::avdt_prioritize_mandatory_codec()) {
    return false;
  }
  A2dpType local_a2dp_type = sep_type == AVDT_TSEP_SNK ? A2dpType::kSource : A2dpType::kSink;
  ...
  log::verbose("local_a2dp_type: {}, codec: {}, is_mandatory_codec_preferred {}",
               static_cast<int>(local_a2dp_type), A2DP_CodecIndexStr(codec_index),
