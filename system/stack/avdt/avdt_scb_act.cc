...
void avdt_scb_hdl_setconfig_cmd(AvdtpScb* p_scb, tAVDT_SCB_EVT* p_data) {
  LOG_DEBUG("%s: p_scb->in_use=%d p_avdt_scb=%p scb_index=%d", __func__,
  ...
  if (!p_scb->in_use) {
    ...
    if (A2DP_GetCodecType(p_scb->stream_config.cfg.codec_info) ==
      ...
      if (p_scb->p_ccb != p_ccb) {
        LOG_ERROR("%s: mismatch in AVDTP SCB/CCB state: (p_scb->p_ccb=%p != "
                  "p_ccb=%p): "
    ...
    LOG_DEBUG("%s: calling avdt_scb_rej_in_use()", __func__);
...
void avdt_scb_hdl_tc_open(AvdtpScb* p_scb, tAVDT_SCB_EVT* p_data) {
  ...
  LOG_DEBUG("%s: psc_mask: cfg: 0x%x, req:0x%x, cur: 0x%x", __func__,
...
void avdt_scb_snd_abort_req(AvdtpScb* p_scb,
  ...
  LOG_DEBUG("%s: p_scb->p_ccb=%p", __func__, p_scb->p_ccb);
...
void avdt_scb_snd_reconfig_req(AvdtpScb* p_scb, tAVDT_SCB_EVT* p_data) {
  LOG_DEBUG("%s: p_scb->peer_seid=%d p_data->msg.hdr.seid=%d", __func__,
...
void avdt_scb_clr_pkt(AvdtpScb* p_scb, UNUSED_ATTR tAVDT_SCB_EVT* p_data) {
  ...
  if (p_scb->p_pkt != NULL) {
    ...
    LOG_DEBUG("Dropped stored media packet");
