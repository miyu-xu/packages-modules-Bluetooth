...
void avdt_scb_hdl_setconfig_rsp(AvdtpScb* p_scb, tAVDT_SCB_EVT* p_data) {
  LOG_DEBUG("avdt_scb_hdl_setconfig_rsp");
...
void avdt_scb_hdl_delay_rpt_cmd(AvdtpScb* p_scb, tAVDT_SCB_EVT* p_data) {
  LOG_DEBUG("avdt_scb_hdl_delay_rpt_cmd");
  tAVDT_EVT_HDR single;
  ...
  if (p_scb->p_ccb) {
    if ((p_scb->cs.cfg.psc_mask & AVDT_PSC_DELAY_RPT)) {
      ...
      if (p_scb->role == AVDT_CONF_INT) {
        ...
        LOG_DEBUG("avdt_scb_hdl_delay_rpt_cmd: Initiate open request");
...
void avdt_scb_hdl_delay_rpt_rsp(AvdtpScb* p_scb, tAVDT_SCB_EVT* p_data) {
  LOG_DEBUG("avdt_scb_hdl_delay_rpt_rsp");
  (*p_scb->stream_config.p_avdt_ctrl_cback)(
      avdt_scb_to_hdl(p_scb),
      p_scb->p_ccb ? p_scb->p_ccb->peer_addr : RawAddress::kEmpty,
