...
enum tTRANSPORT_CHANNEL_TYPE : uint8_t {
  AVDT_CHAN_SIG = 0,    /* signaling channel */
  AVDT_CHAN_MEDIA = 1,  /* media channel */
  AVDT_CHAN_REPORT = 2, /* reporting channel */
  AVDT_CHAN_NUM_TYPES = 3
};
...
inline std::string tc_state_text(uint8_t state) {
  ...
  switch (state_) {
    ...
    default:
      return std::string("UNKNOWN[") + std::to_string(state_) + std::string("]");
  }
}
...
inline std::string tc_type_text(uint8_t type) {
  ...
  switch (type_) {
    ...
    default:
      return std::string("UNKNOWN[") + std::to_string(type_) + std::string("]");
  }
}
