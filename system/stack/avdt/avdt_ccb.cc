...
void avdt_ccb_event(AvdtpCcb* p_ccb, uint8_t event, tAVDT_CCB_EVT* p_data) {
...
#if (AVDT_DEBUG == TRUE)
  LOG_INFO("%s: CCB ccb=%d event=%s state=%s p_ccb=%p", __func__,
  ...
  for (i = 0; i < AVDT_CCB_ACTIONS; i++) {
    ...
    LOG_DEBUG("%s: event=%s state=%s action=%d", __func__,
...
AvdtpCcb* avdt_ccb_alloc(const RawAddress& bd_addr) {
  ...
  for (int i = 0; i < AVDT_NUM_LINKS; i++, p_ccb++) {
    if (!p_ccb->allocated) {
      ...
      LOG_DEBUG("%s: allocated (index %d) for peer %s", __func__, i,
...
AvdtpCcb* avdt_ccb_alloc_by_channel_index(const RawAddress& bd_addr,
  ...
  p_ccb->Allocate(bd_addr);
  LOG_DEBUG("%s: allocated (index %d) peer=%s p_ccb=%p", __func__,
...
void avdt_ccb_dealloc(AvdtpCcb* p_ccb, UNUSED_ATTR tAVDT_CCB_EVT* p_data) {
  LOG_DEBUG("%s: deallocated (index %d) peer=%s p_ccb=%p", __func__,
            avdt_ccb_to_idx(p_ccb), ADDRESS_TO_LOGGABLE_CSTR(p_ccb->peer_addr),
            p_ccb);
  p_ccb->ResetCcb();
}
