...
bool SMP_Register(tSMP_CALLBACK* p_cback) {
  LOG_DEBUG("SMP_Register state=%d", smp_cb.state);
...
tSMP_STATUS SMP_Pair(const RawAddress& bd_addr, tBLE_ADDR_TYPE addr_type) {
  ...
  LOG_DEBUG("%s: state=%d br_state=%d flag=0x%x, bd_addr=%s", __func__,
...
bool SMP_PairCancel(const RawAddress& bd_addr) {
  ...
  LOG_DEBUG("SMP_CancelPair state=%d flag=0x%x ", p_cb->state, p_cb->flags);
...
void SMP_PasskeyReply(const RawAddress& bd_addr, uint8_t res,
  ...
  LOG_DEBUG("SMP_PasskeyReply: Key: %d  Result:%d", passkey, res);
...
void SMP_ConfirmReply(const RawAddress& bd_addr, uint8_t res) {
  ...
  LOG_DEBUG("%s: Result:%d", __func__, res);
...
void SMP_OobDataReply(const RawAddress& bd_addr, tSMP_STATUS res, uint8_t len,
  ...
  LOG_DEBUG("%s State: %d  res:%d", __func__, smp_cb.state, res);
...
void SMP_SecureConnectionOobDataReply(uint8_t* p_data) {
  ...
  LOG_DEBUG("%s req_oob_type: %d, loc_oob_data.present: %d, "
  ...
  switch (p_cb->req_oob_type) {
    ...
    case SMP_OOB_BOTH:
      ...
      break;
    default:
      LOG_DEBUG("Unexpected OOB data type requested. Fail OOB");
      data_missing = true;
      break;
  }
