...
void smp_generate_srand_mrand_confirm(tSMP_CB* p_cb,
  ...
  send_ble_rand(BindOnce(
      [](tSMP_CB* p_cb, uint64_t rand) {
        uint8_t* rand_ptr = (uint8_t*)&rand;
        std::copy(rand_ptr, rand_ptr + sizeof(uint64_t), p_cb->rand.data());
        /* generate 64 MSB of MRand or SRand */
        send_ble_rand(BindOnce(
            [](tSMP_CB* p_cb, uint64_t rand) {
              uint8_t* rand_ptr = (uint8_t*)&rand;
              std::copy(rand_ptr, rand_ptr + sizeof(uint64_t),
                        p_cb->rand.data() + sizeof(uint64_t));
...
static void smp_generate_y(tSMP_CB* p_cb, uint64_t rand) {
  ...
  uint8_t* rand_ptr = (uint8_t*)&rand;
  std::copy(rand_ptr, rand_ptr + BT_OCTET8_LEN, p_cb->enc_rand);
  Octet16 rand16{};
  std::copy(rand_ptr, rand_ptr + BT_OCTET8_LEN, rand16.data());
...
void smp_create_private_key(tSMP_CB* p_cb, tSMP_INT_DATA* p_data) {
  ...
  send_ble_rand(BindOnce(
      [](tSMP_CB* p_cb, uint64_t rand) {
        uint8_t* rand_ptr = (uint8_t*)&rand;
        std::copy(rand_ptr, rand_ptr + sizeof(uint64_t), p_cb->private_key);
        send_ble_rand(BindOnce(
            [](tSMP_CB* p_cb, uint64_t rand) {
              uint8_t* rand_ptr = (uint8_t*)&rand;
              std::copy(rand_ptr, rand_ptr + sizeof(uint64_t),
                        p_cb->private_key + sizeof(uint64_t));
              send_ble_rand(BindOnce(
                  [](tSMP_CB* p_cb, uint64_t rand) {
                    uint8_t* rand_ptr = (uint8_t*)&rand;
                    std::copy(rand_ptr, rand_ptr + sizeof(uint64_t),
                              p_cb->private_key + 2 * sizeof(uint64_t));
                    send_ble_rand(BindOnce(
                        [](tSMP_CB* p_cb, uint64_t rand) {
                          uint8_t* rand_ptr = (uint8_t*)&rand;
                          std::copy(rand_ptr, rand_ptr + sizeof(uint64_t),
                                    p_cb->private_key + 3 * sizeof(uint64_t));
                          smp_process_private_key(p_cb);
                        },
                        p_cb));
                  },
