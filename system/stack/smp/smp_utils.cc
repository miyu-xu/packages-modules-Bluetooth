...
void smp_log_metrics(const RawAddress& bd_addr, bool is_outgoing,
  ...
  uint16_t metric_cmd =
      is_over_br ? SMP_METRIC_COMMAND_BR_MASK : SMP_METRIC_COMMAND_LE_MASK;
  ...
  log_smp_pairing_event(bd_addr, metric_cmd, direction, failure_reason);
}
...
void smp_proc_pairing_cmpl(tSMP_CB* p_cb) {
  ...
  {
    auto direction = android::bluetooth::DirectionEnum::DIRECTION_UNKNOWN;
