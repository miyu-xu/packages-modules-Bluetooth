...
void smp_rsp_timeout(UNUSED_ATTR void* data) {
  ...
  LOG_INFO("%s state:%d br_state:%d", __func__, p_cb->state, p_cb->br_state);
...
void smp_delayed_auth_complete_timeout(UNUSED_ATTR void* data) {
  ...
  if (smp_get_state() == SMP_STATE_BOND_PENDING) {
    LOG_INFO("%s sending delayed auth complete.", __func__);
    tSMP_INT_DATA smp_int_data;
    smp_int_data.status = SMP_SUCCESS;
...
static BT_HDR* smp_build_security_request(UNUSED_ATTR uint8_t cmd_code,
  ...
  LOG_INFO("opcode=%d auth_req=0x%x", SMP_OPCODE_SEC_REQ, p_cb->loc_auth_req);
...
void smp_convert_string_to_tk(Octet16* tk, uint32_t passkey) {
  uint8_t* p = tk->data();
  tSMP_KEY key;
  LOG_INFO("smp_convert_string_to_tk");
...
bool smp_command_has_valid_fixed_length(tSMP_CB* p_cb) {
  ...
  LOG_DEBUG("%s for cmd code 0x%02x", __func__, cmd_code);

  if (p_cb->rcvd_cmd_len != smp_cmd_size_per_spec[cmd_code]) {
    LOG_WARN("Rcvd from the peer cmd 0x%02x with invalid length "
...
bool smp_pairing_request_response_parameters_are_valid(tSMP_CB* p_cb) {
  ...
  LOG_DEBUG("%s for cmd code 0x%02x", __func__, p_cb->rcvd_cmd_code);

  if (io_caps >= BTM_IO_CAP_MAX) {
    LOG_WARN("Rcvd from the peer cmd 0x%02x with IO Capability "
  ...
  if (!((oob_flag == SMP_OOB_NONE) || (oob_flag == SMP_OOB_PRESENT))) {
    LOG_WARN("Rcvd from the peer cmd 0x%02x with OOB data flag value "
             "(0x%02x) out of range).",
  ...
  if (!((bond_flag == SMP_AUTH_NO_BOND) || (bond_flag == SMP_AUTH_BOND))) {
    LOG_WARN("Rcvd from the peer cmd 0x%02x with Bonding_Flags value (0x%02x) "
             "out of range).",
  ...
  if ((enc_size < SMP_ENCR_KEY_SIZE_MIN) ||
    ...
    LOG_WARN("Rcvd from the peer cmd 0x%02x with Maximum Encryption "
...
bool smp_pairing_keypress_notification_is_valid(tSMP_CB* p_cb) {
  ...
  if (keypress_notification >= SMP_SC_KEY_OUT_OF_RANGE) {
    LOG_WARN("Rcvd from the peer cmd 0x%02x with Pairing Keypress "
...
tSMP_ASSO_MODEL smp_select_association_model(tSMP_CB* p_cb) {
  ...
  LOG_INFO("%s", __func__);
  LOG_DEBUG("%s p_cb->peer_io_caps = %d p_cb->local_io_capability = %d",
            __func__, p_cb->peer_io_caps, p_cb->local_io_capability);
  LOG_DEBUG("%s p_cb->peer_oob_flag = %d p_cb->loc_oob_flag = %d", __func__,
            p_cb->peer_oob_flag, p_cb->loc_oob_flag);
  LOG_DEBUG("%s p_cb->peer_auth_req = 0x%02x p_cb->loc_auth_req = 0x%02x",
            __func__, p_cb->peer_auth_req, p_cb->loc_auth_req);
  LOG_DEBUG("%s p_cb->secure_connections_only_mode_required = %s", __func__,
  ...
  LOG_DEBUG("use_sc_process = %d, h7 use = %d",
...
uint8_t smp_calculate_random_input(uint8_t* random, uint8_t round) {
  ...
  LOG_DEBUG("random: 0x%02x, round: %d, i: %d, j: %d", random[i], round, i, j);
  ri = ((random[i] >> j) & 1) | 0x80;
  LOG_DEBUG("%s ri=0x%02x", __func__, ri);
  return ri;
}
...
bool smp_check_commitment(tSMP_CB* p_cb) {
  ...
  LOG_DEBUG("%s: Commitment check succeeds", __func__);
  return true;
}
...
void smp_save_secure_connections_long_term_key(tSMP_CB* p_cb) {
  ...
  LOG_DEBUG("%s-Save LTK as local LTK key", __func__);
  ...
  LOG_DEBUG("%s-Save LTK as peer LTK key", __func__);
...
void smp_calculate_f5_mackey_and_long_term_key(tSMP_CB* p_cb) {
  ...
  LOG_INFO("%s is completed", __func__);
}
...
bool smp_request_oob_data(tSMP_CB* p_cb) {
  ...
  LOG_DEBUG("req_oob_type = %d", req_oob_type);
