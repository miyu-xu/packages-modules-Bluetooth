...
static void smp_update_key_mask(tSMP_CB* p_cb, uint8_t key_type, bool recv) {
  ...
  LOG_DEBUG("updated local_i_key = %02x, local_r_key = %02x", p_cb->local_i_key,
            p_cb->local_r_key);
}
...
void smp_send_enc_info(tSMP_CB* p_cb, tSMP_INT_DATA* p_data) {
  ...
  LOG_DEBUG("%s: p_cb->loc_enc_size = %d", __func__, p_cb->loc_enc_size);
...
void smp_proc_sec_req(tSMP_CB* p_cb, tSMP_INT_DATA* p_data) {
  ...
  LOG_DEBUG("%s: auth_req=0x%x", __func__, auth_req);

  p_cb->cb_evt = SMP_EVT_NONE;

  btm_ble_link_sec_check(p_cb->pairing_bda, auth_req, &sec_req_act);

  LOG_DEBUG("%s: sec_req_act=0x%x", __func__, sec_req_act);

  switch (sec_req_act) {
    case BTM_BLE_SEC_REQ_ACT_ENCRYPT:
      LOG_DEBUG("%s: BTM_BLE_SEC_REQ_ACT_ENCRYPT", __func__);
...
void smp_proc_pair_cmd(tSMP_CB* p_cb, tSMP_INT_DATA* p_data) {
  ...
  LOG_DEBUG("%s: pairing_bda=%s", __func__,
  ...
  } else /* Central receives pairing response */
  {
    ...
    if (p_cb->secure_connections_only_mode_required &&
      ...
      LOG_ERROR("Central requires secure connection only mode "
...
void smp_br_process_pairing_command(tSMP_CB* p_cb, tSMP_INT_DATA* p_data) {
  ...
  if (p_cb->role == HCI_ROLE_PERIPHERAL) {
    ...
    LOG_DEBUG("%s central rcvs valid PAIRING RESPONSE."
...
void smp_br_check_authorization_request(tSMP_CB* p_cb, tSMP_INT_DATA* p_data) {
  LOG_DEBUG("%s rcvs i_keys=0x%x r_keys=0x%x (i-initiator r-responder)",
  ...
  ** Set local_r_key on central to expect only these keys. */
  ...
  LOG_DEBUG("%s: use h7 = %d", __func__, p_cb->key_derivation_h7_used);
...
void smp_br_select_next_key(tSMP_CB* p_cb, tSMP_INT_DATA* p_data) {
  LOG_DEBUG("%s role=%d (0-central) r_keys=0x%x i_keys=0x%x", __func__,
...
void smp_key_pick_key(tSMP_CB* p_cb, tSMP_INT_DATA* p_data) {
  ...
  LOG_DEBUG("%s key_to_dist=0x%x", __func__, key_to_dist);
  while (i < SMP_KEY_DIST_TYPE_MAX) {
    LOG_DEBUG("key to send = %02x, i = %d", key_to_dist, i);

    if (key_to_dist & (1 << i)) {
      LOG_DEBUG("smp_distribute_act[%d]", i);
    ...
    if (smp_get_state() == SMP_STATE_BOND_PENDING) {
      if (p_cb->derive_lk) {
        ...
        if (!(p_dev_rec->sec_flags & BTM_SEC_LE_LINK_KEY_AUTHED) &&
          ...
          LOG_DEBUG("%s BR key is higher security than existing LE keys, don't "
                    "derive LK from LTK",
                    __func__);
        } else {
      ...
      if (p_cb->total_tx_unacked == 0) {
        ...
        if (!alarm_is_scheduled(p_cb->delayed_auth_timer_ent)) {
          LOG_DEBUG("%s delaying auth complete.", __func__);
...
void smp_decide_association_model(tSMP_CB* p_cb, tSMP_INT_DATA* p_data) {
  ...
  LOG_DEBUG("%s Association Model = %d", __func__,
            p_cb->selected_association_model);
  ...
  switch (p_cb->selected_association_model) {
    ...
    case SMP_MODEL_PASSKEY:
      ...
      LOG_INFO("p_cb->sec_level =%d (SMP_SEC_AUTHENTICATED) ", p_cb->sec_level);
    ...
    case SMP_MODEL_KEY_NOTIF:
      p_cb->sec_level = SMP_SEC_AUTHENTICATED;
      LOG_DEBUG("Need to generate Passkey");
  ...
  LOG_INFO("sec_level=%d ", p_cb->sec_level);
...
void smp_process_io_response(tSMP_CB* p_cb, tSMP_INT_DATA* p_data) {
  ...
  } else /* plan to send pairing respond */
  {
    ...
    if (p_cb->secure_connections_only_mode_required &&
        (!(p_cb->le_secure_connections_mode_is_used) ||
         (p_cb->selected_association_model == SMP_MODEL_SEC_CONN_JUSTWORKS))) {
      LOG_ERROR("Peripheral requires secure connection only mode "
...
void smp_idle_terminate(tSMP_CB* p_cb, tSMP_INT_DATA* p_data) {
  if (p_cb->flags & SMP_PAIR_FLAGS_WE_STARTED_DD) {
    LOG_DEBUG("Pairing terminated at IDLE state.");
...
void smp_start_secure_connection_phase1(tSMP_CB* p_cb, tSMP_INT_DATA* p_data) {
  ...
  if (p_cb->selected_association_model == SMP_MODEL_SEC_CONN_JUSTWORKS) {
    p_cb->sec_level = SMP_SEC_UNAUTHENTICATE;
    LOG_INFO("p_cb->sec_level =%d (SMP_SEC_UNAUTHENTICATE) ", p_cb->sec_level);
  } else {
    p_cb->sec_level = SMP_SEC_AUTHENTICATED;
    LOG_INFO("p_cb->sec_level =%d (SMP_SEC_AUTHENTICATED) ", p_cb->sec_level);
  }
  ...
  switch (p_cb->selected_association_model) {
    ...
    case SMP_MODEL_SEC_CONN_PASSKEY_DISP:
      ...
      LOG_DEBUG("Need to generate SC Passkey");
      smp_generate_passkey(p_cb, NULL);
      break;
    case SMP_MODEL_SEC_CONN_OOB:
    ...
    case SMP_MODEL_SEC_CONN_NUM_COMP:
      ...
      } else /* i.e. central */
      {
        if (p_cb->flags & SMP_PAIR_FLAG_HAVE_PEER_COMM) {
           ...
           * for remote nonce*/
          LOG_DEBUG("central in assoc mode = %d "
    ...
    case SMP_MODEL_SEC_CONN_NUM_COMP:
      ...
      if (p_cb->selected_association_model == SMP_MODEL_SEC_CONN_JUSTWORKS) {
        if (!GetInterfaceToProfiles()->config->isAndroidTVDevice() &&
          ...
          LOG_DEBUG("JUST WORKS showing Consent Dialog");
          p_cb->cb_evt = SMP_CONSENT_REQ_EVT;
          smp_set_state(SMP_STATE_WAIT_NONCE);
          smp_sm_event(p_cb, SMP_SC_DSPL_NC_EVT, NULL);
        } else {
...
void smp_process_secure_connection_oob_data(tSMP_CB* p_cb,
  ...
  if (p_sc_oob_data->loc_oob_data.present) {
    ...
    LOG_INFO("%s: local OOB randomizer is absent", __func__);
    p_cb->local_random = {0};
  }

  if (!p_sc_oob_data->peer_oob_data.present) {
    LOG_INFO("%s: peer OOB data is absent", __func__);
    p_cb->peer_random = {0};
  } else {
...
bool smp_proc_ltk_request(const RawAddress& bda) {
  LOG_DEBUG("%s state = %d", __func__, smp_cb.state);
  bool match = false;
...
void smp_br_process_link_key(tSMP_CB* p_cb, tSMP_INT_DATA* p_data) {
  ...
  tBTM_SEC_DEV_REC* p_dev_rec = btm_find_dev(p_cb->pairing_bda);
  if (p_dev_rec) {
    LOG_DEBUG("%s: dev_type = %d ", __func__, p_dev_rec->device_type);
    p_dev_rec->device_type |= BT_DEVICE_TYPE_BLE;
  } else {
    LOG_ERROR("%s failed to find Security Record", __func__);
  }

  LOG_DEBUG("%s: LTK derivation from LK successfully completed", __func__);
  smp_save_secure_connections_long_term_key(p_cb);
  smp_update_key_mask(p_cb, SMP_SEC_KEY_TYPE_ENC, false);
