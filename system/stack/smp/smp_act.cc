...
void smp_pairing_cmpl(tSMP_CB* p_cb, tSMP_INT_DATA* /* p_data */) {
  ...
  } else if ((p_cb->flags & SMP_PAIR_FLAGS_WE_STARTED_DD) && (p_cb->role == HCI_ROLE_PERIPHERAL) &&
             (p_cb->state == SMP_STATE_IDLE)) {
    log::warn(
            "smp_pairing_cmpl tx unacked is not zero and role is peripheral, proc pairing comple");
    smp_proc_pairing_cmpl(p_cb);
  }
}
