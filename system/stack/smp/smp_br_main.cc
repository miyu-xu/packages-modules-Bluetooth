...
void smp_br_state_machine_event(tSMP_CB* p_cb, tSMP_BR_EVENT event,
  ...
  LOG_INFO("main %s", __func__);
  if (curr_state >= SMP_BR_STATE_MAX) {
    LOG_DEBUG("Invalid br_state: %d", curr_state);
    return;
  }
  ...
  LOG_DEBUG("SMP Role: %s State: [%s (%d)], Event: [%s (%d)]",
  ...
  if ((event <= SMP_BR_MAX_EVT) &&
    ...
    LOG_DEBUG("Ignore event [%s (%d)] in state [%s (%d)]",
              smp_get_br_event_name(event), event,
  ...
  LOG_DEBUG("result state = %s", smp_get_br_state_name(p_cb->br_state));
}
