 ...
 protected:
  void SetUp() override {
    ...
    test::mock::osi_thread::thread_new.body =
      ...
      pthread_create(&thread->pthread_, NULL, run_message_loop, &start_arg);
...
void runme(BORROWED void* context) {
  data_t* data = static_cast<data_t*>(context);
  RawAddress p_bda =
...
TEST_F(BtmDmInqDbThreadedTest, btm_inq_db_new) {
  ...
  for (size_t j = 0; j < kEntriesPerThread; j++) {
    for (size_t i = 0; i < kNumberOfThreads; i++) {
      ...
      ASSERT_TRUE(
          thread_post(threads[i], runme, TO_OWNED static_cast<void*>(data)));
    }
  }
...
void runme2(BORROWED void* context) {
  data2_t* data = static_cast<data2_t*>(context);
...
TEST_F(BtmDmInqDbThreadedTest, btm_inq_find_bdaddr) {
  ...
  for (size_t j = 0; j < kEntriesPerThread; j++) {
    for (size_t i = 0; i < kNumberOfThreads; i++) {
      data2_t* data = static_cast<data2_t*>(calloc(sizeof(data2_t), 1));
      data->thread_id = i;
      data->offset = (int)j;
      ASSERT_TRUE(
          thread_post(threads[i], runme2, TO_OWNED static_cast<void*>(data)));
    }
  }
