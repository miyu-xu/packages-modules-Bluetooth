...
TEST_F(ScoHciTest, ScoOverHciOpenFail) {
  bluetooth::audio::sco::open();
  ASSERT_EQ(increment_mock_function_call_count_get()["UIPC_Init"], 1);
  ASSERT_EQ(increment_mock_function_call_count_get()["UIPC_Open"], 0);
  bluetooth::audio::sco::cleanup();

  // UIPC is nullptr and shouldn't require an actual call of UIPC_Close;
  ASSERT_EQ(increment_mock_function_call_count_get()["UIPC_Close"], 0);
}

TEST_F(ScoHciWithOpenCleanTest, ScoOverHciOpenClean) {
  ASSERT_EQ(increment_mock_function_call_count_get()["UIPC_Init"], 1);
  ASSERT_EQ(increment_mock_function_call_count_get()["UIPC_Open"], 1);
  ASSERT_EQ(mock_uipc_init_ret, nullptr);

  mock_uipc_init_ret = std::make_unique<tUIPC_STATE>();
  // Double open will override uipc
  bluetooth::audio::sco::open();
  ASSERT_EQ(increment_mock_function_call_count_get()["UIPC_Init"], 2);
  ASSERT_EQ(increment_mock_function_call_count_get()["UIPC_Open"], 2);
  ASSERT_EQ(mock_uipc_init_ret, nullptr);

  bluetooth::audio::sco::cleanup();
  ASSERT_EQ(increment_mock_function_call_count_get()["UIPC_Close"], 1);
...
TEST_F(ScoHciTest, ScoOverHciReadNoOpen) {
  ...
  ASSERT_EQ(increment_mock_function_call_count_get()["UIPC_Read"], 0);
}
...
TEST_F(ScoHciWithOpenCleanTest, ScoOverHciRead) {
  ...
  ASSERT_EQ(increment_mock_function_call_count_get()["UIPC_Read"], 1);
}

TEST_F(ScoHciTest, ScoOverHciWriteNoOpen) {
  uint8_t buf[100];
  bluetooth::audio::sco::write(buf, sizeof(buf));
  ASSERT_EQ(increment_mock_function_call_count_get()["UIPC_Send"], 0);
}
...
TEST_F(ScoHciWithOpenCleanTest, ScoOverHciWrite) {
  ...
  ASSERT_EQ(increment_mock_function_call_count_get()["UIPC_Send"], 1);

  // Send fails
  mock_uipc_send_ret = false;
  ASSERT_EQ(bluetooth::audio::sco::write(buf, sizeof(buf)), size_t(0));
  ASSERT_EQ(increment_mock_function_call_count_get()["UIPC_Send"], 2);
}
