...
TEST_F(ScoHciWbsWithInitCleanTest, WbsDecode) {
  ...
  ASSERT_EQ(bluetooth::audio::sco::wbs::decode(&decoded),
            size_t(BTM_MSBC_CODE_SIZE));
  ASSERT_NE(decoded, nullptr);
  for (size_t i = 0; i < BTM_MSBC_CODE_SIZE; i++) ASSERT_EQ(decoded[i], 0);
...
TEST_F(ScoHciWbsWithInitCleanTest, WbsEncodeDequeuePackets) {
  ...
  for (size_t i = 0; i < 5; i++) {
    msbc_zero_packet[1] = h2_header_frames_count[i % 4];
    ASSERT_EQ(bluetooth::audio::sco::wbs::encode(data, sizeof(data)),
              sizeof(data));
    ASSERT_EQ(bluetooth::audio::sco::wbs::dequeue_packet(&encoded), size_t(60));
    ASSERT_NE(encoded, nullptr);
    for (size_t j = 0; j < 60; j++) {
      ASSERT_EQ(encoded[j], msbc_zero_packet[j]);
    }
  }
}
*/
TEST_F(ScoHciWbsWithInitCleanTest, WbsPlc) {
  int16_t sine[16] = {0, 100,  200,  300,  400,  300,  200,  100,
  ...
  // Simulate a run without packet loss
  for (size_t i = 0, sample_idx = 0; i < 50; i++) {
    // Data is a 1000Hz sine wave
    for (size_t j = 0; j < 120; j++, sample_idx++)
      data[j] = sine[sample_idx % 16];
    ASSERT_EQ(bluetooth::audio::sco::wbs::encode(data, sizeof(data)),
              sizeof(data));
    ASSERT_EQ(bluetooth::audio::sco::wbs::dequeue_packet(&encoded), size_t(60));
    ASSERT_NE(encoded, nullptr);
  ...
  // Simulate a run with packet loss
  for (size_t i = 0, sample_idx = 0; i <= plc_idx; i++) {
    // Data is a 1000Hz sine wave
    for (size_t j = 0; j < 120; j++, sample_idx++)
      data[j] = sine[sample_idx % 16];
    ASSERT_EQ(bluetooth::audio::sco::wbs::encode(data, sizeof(data)),
    ...
    // Receive the packet. Substitute to invalid when packet loss.
    ASSERT_EQ(bluetooth::audio::sco::wbs::enqueue_packet(
                  i != plc_idx ? encoded : invalid_pkt, 60),
              size_t(60));
    ASSERT_EQ(bluetooth::audio::sco::wbs::decode(&decoded),
              size_t(BTM_MSBC_CODE_SIZE));
    ASSERT_NE(decoded, nullptr);
  ...
  for (size_t i = 0; i < 120; i++)
    EXPECT_THAT(ptr[i] - expect_data[i], AllOf(Ge(-3), Le(3)))
        << "PLC data " << ptr[i] << " deviates from expected "
        << expect_data[i] << " at index " << i;
  //        ASSERT_EQ(*((int16_t*)(decoded + 2 * i)), expect_data[i]);
}
