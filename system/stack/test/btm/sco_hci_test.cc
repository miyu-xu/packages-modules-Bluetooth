...
// NOTE: there is no guarantee about this from the spec
...
TEST_F(ScoHciSwbWithInitCleanTest, SwbDecode) {
  ...
  for (size_t i = 0; i < BTM_LC3_CODE_SIZE; i++) {
    ASSERT_THAT(decoded[i], AllOf(Ge(-3), Le(3)))
        << "PLC data " << decoded[i] << " deviates from expected " << 0
        << " at index " << i;
  }
...
TEST_F(ScoHciSwbWithInitCleanTest, SwbEncodeDequeuePackets) {
  ...
  for (size_t i = 0; i < 5; i++) {
    ...
    ASSERT_NE(encoded, nullptr);
    ASSERT_EQ(encoded[1], h2_header_frames_count[i % 4]);

    const std::vector<uint8_t> payload(encoded, encoded + size_t(60));
    ASSERT_EQ(bluetooth::audio::sco::swb::enqueue_packet(payload, false), true);
    ASSERT_EQ(bluetooth::audio::sco::swb::decode(&decoded),
              size_t(BTM_LC3_CODE_SIZE));
    ASSERT_NE(decoded, nullptr);
...
// TODO: implement this with github.com/google/liblc3/issues/16 in note.
