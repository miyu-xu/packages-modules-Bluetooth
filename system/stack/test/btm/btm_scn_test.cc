...
TEST_F(BtmAllocateSCNTest, all_scn_available) {
  uint8_t scn = BTM_AllocateSCN();
  ASSERT_EQ(scn, 2);  // When all SCN are available assign the first SCN (exc. 1
                      // which is reserved)
}
...
TEST_F(BtmAllocateSCNTest, scn_available_after_available_index) {
  ...
  uint8_t scn = BTM_AllocateSCN();
  ASSERT_EQ(scn, 9);  // All indexes upto 7 are occupied; hence index 8 i.e. scn
                      // 9 should return
}
