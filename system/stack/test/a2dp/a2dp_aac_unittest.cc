...
constexpr char kWavFile[] = "raw_data/pcm1644s.wav";
...
static WavReader wav_reader = WavReader(kWavFile);
...
class A2dpAacTest : public AllocationTestHarness {
 protected:
  ...
  void SetCodecConfig() {
    ...
    ASSERT(codec_config_);
    ASSERT(a2dp_codecs_->setSinkCodecConfig(kCodecInfoAacCapability, true,
  ...
  void InitializeEncoder(a2dp_source_read_callback_t read_cb,
    ...
    if (encoder_iface_ != nullptr) {
      ...
      ASSERT_LOG(false, "unable to initialize the encoder.");
    }
  }
  ...
  void InitializeDecoder(decoded_data_callback_t data_cb) {
    if (decoder_iface_ != nullptr) {
      ...
      ASSERT_LOG(false, "Unable to initialize the decoder");
    }
  }
...
TEST_F(A2dpAacTest, a2dp_source_read_underflow) {
  ...
  usleep(kA2dpTickUs);
  timestamp_us = bluetooth::common::time_get_os_monotonic_raw_us();
  encoder_iface_->send_frames(timestamp_us);
...
TEST_F(A2dpAacTest, decoded_data_cb_not_invoked_when_empty_packet) {
  auto data_cb = +[](uint8_t* p_buf, uint32_t len) {
    ASSERT_LOG(false, "Decoded data callback invoked when there is no data.");
  };
...
TEST_F(A2dpAacTest, decoded_data_cb_invoked) {
  ...
  decoder_iface_->decode_packet(packet);
  osi_free(packet);
  ASSERT(log_capture_->Find(kDecodedDataCallbackIsInvoked));
}
