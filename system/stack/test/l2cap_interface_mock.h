...
class MockL2capInterface : public Interface {
public:
  ...
  MOCK_METHOD(uint16_t, L2CA_ConnectReq, (uint16_t psm, const RawAddress& p_bd_addr));
  ...
  MOCK_METHOD(bool, L2CA_SetLeGattTimeout, (const RawAddress& rem_bda, uint16_t idle_tout));

  MOCK_METHOD(bool, L2CA_MarkLeLinkAsActive, (const RawAddress& rem_bda));
  ...
  MOCK_METHOD(bool, L2CA_SubrateRequest,
              (const RawAddress& rem_bda, uint16_t subrate_min, uint16_t subrate_max,
               uint16_t max_latency, uint16_t cont_num, uint16_t timeout));
  MOCK_METHOD(bool, L2CA_GetPeerLECocConfig, (uint16_t cid, tL2CAP_LE_CFG_INFO* peer_cfg));
  ...
  MOCK_METHOD(bool, L2CA_GetRemoteCid, (uint16_t cid, uint16_t* rcid));
  MOCK_METHOD(bool, L2CA_GetPeerChannelId, (uint16_t lcid, uint16_t* rcid));
  MOCK_METHOD(uint16_t, L2CA_FlushChannel, (uint16_t cid, uint16_t num_to_flush));
  ...
  MOCK_METHOD(bool, L2CA_RemoveFixedChnl, (uint16_t fixed_cid, const RawAddress& rem_bda));
 ...
 */
void SetMockInterface(l2cap::Interface* mock_l2cap_interface);
