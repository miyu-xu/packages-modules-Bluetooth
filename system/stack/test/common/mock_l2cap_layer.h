...
class MockL2capInterface : public L2capInterface {
  ...
  MOCK_METHOD4(Register,
               uint16_t(uint16_t psm, const tL2CAP_APPL_INFO& p_cb_info, bool enable_snoop,
                        tL2CAP_ERTM_INFO* p_ertm_info));
  MOCK_METHOD2(ConnectRequest, uint16_t(uint16_t psm, const RawAddress& bd_addr));
  MOCK_METHOD5(ConnectResponse,
               bool(const RawAddress& bd_addr, uint8_t id, uint16_t lcid, uint16_t result,
                    uint16_t status));
  ...
  MOCK_METHOD3(ConnectCreditBasedReq,
               std::vector<uint16_t>(uint16_t psm, const RawAddress& bd_addr,
