...
static const std::vector<std::function<void(FuzzedDataProvider*)>> sdp_operations = {
  ...
  [](FuzzedDataProvider* fdp) -> void {
    ...
    std::shared_ptr<tSDP_DISCOVERY_DB> p_db(reinterpret_cast<tSDP_DISCOVERY_DB*>(malloc(db_size)),
                                            free);
    if (p_db) {
  ...
  [](FuzzedDataProvider* fdp) -> void {
    [[maybe_unused]] bool rc = get_legacy_stack_sdp_api()->db.SDP_FindServiceInDb(
            getArbitraryVectorElement(fdp, sdp_db_vect, true).get(),
            fdp->ConsumeIntegral<uint16_t>(), generateArbitrarySdpDiscRecord(fdp, true).get());
  },
  ...
  [](FuzzedDataProvider* fdp) -> void {
    ...
    tSDP_DISC_REC* p_rec = generateArbitrarySdpDiscRecord(fdp, false).get();
    [[maybe_unused]] bool rc = get_legacy_stack_sdp_api()->record.SDP_FindProtocolListElemInRec(
  ...
  [](FuzzedDataProvider* fdp) -> void {
    ...
    if (p_proto_list) {
      if (p_proto_list[0]) {
        [[maybe_unused]] bool rc = get_legacy_stack_sdp_api()->handle.SDP_AddAdditionProtoLists(
  ...
  [](FuzzedDataProvider* fdp) -> void {
    [[maybe_unused]] bool rc = get_legacy_stack_sdp_api()->handle.SDP_AddProfileDescriptorList(
            getArbitraryVectorElement(fdp, sdp_record_handles, true),
  ...
  [](FuzzedDataProvider* fdp) -> void {
    [[maybe_unused]] bool rc = get_legacy_stack_sdp_api()->handle.SDP_AddLanguageBaseAttrIDList(
            getArbitraryVectorElement(fdp, sdp_record_handles, true),
            fdp->ConsumeIntegral<uint16_t>(), fdp->ConsumeIntegral<uint16_t>(),
  ...
  [](FuzzedDataProvider* fdp) -> void {
    ...
    [[maybe_unused]] bool rc = get_legacy_stack_sdp_api()->handle.SDP_AddServiceClassIdList(
            getArbitraryVectorElement(fdp, sdp_record_handles, true), num_services, service_uuids);
