...
std::vector<std::function<void(FuzzedDataProvider*)>> a2dp_codec_operations = {
  ...
  [](FuzzedDataProvider* fdp) -> void {
    ...
    if (a2dp_init_runs <= A2DP_MAX_INIT_RUNS) {
      std::shared_ptr<A2dpCodecs> codecs = getArbitraryVectorElement(fdp, a2dp_codecs_vect, false);
      if (codecs) {
        a2dp_init_runs++;
        codecs->init();
      }
  ...
  [](FuzzedDataProvider* fdp) -> void {
    ...
    const uint8_t* peer_codec_info = getArbitraryVectorElement(fdp, a2dp_codec_info_vect, false);
  ...
  [](FuzzedDataProvider* fdp) -> void {
    ...
    std::unique_ptr<uint8_t, void (*)(void*)> p_result_codec_config(
            reinterpret_cast<uint8_t*>(calloc(500, sizeof(uint8_t))), free);
    if (p_result_codec_config) {
      codecs->setSinkCodecConfig(peer_codec_info, fdp->ConsumeBool(), p_result_codec_config.get(),
                                 fdp->ConsumeBool());
    }
  },
  ...
  [](FuzzedDataProvider* fdp) -> void {
    ...
    const tA2DP_ENCODER_INIT_PEER_PARAMS p_peer_params = getArbitraryA2dpEncoderInitPeerParams(fdp);
  ...
  [](FuzzedDataProvider* fdp) -> void {
    ...
    const uint8_t* p_ota_codec_config = getArbitraryVectorElement(fdp, a2dp_codec_info_vect, false);
