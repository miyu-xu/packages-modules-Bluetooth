...
void sdp_cb(tSDP_RESULT result) {
...
TEST_F(StackSdpMainTest, sdp_service_search_request_queuing_race_condition) {
  // start first request
  ASSERT_TRUE(SDP_ServiceSearchRequest(addr, sdp_db, sdp_cb));
  ...
  sdp_disconnect(p_ccb1, SDP_SUCCESS);
  sdp_cb.reg_info.pL2CA_DisconnectCfm_Cb(p_ccb1->connection_id, 0, &cfg);

  const int cid2 = L2CA_ConnectReq2_cid;
  ASSERT_NEQ(cid1, cid2);  // The callback a queued a new request
