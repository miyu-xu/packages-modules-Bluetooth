...
TEST_F(StackSdpInitTest, multiple_pending) {
  auto* p_ccb = sdp_conn_originate(addr);
  ASSERT_NE(nullptr, p_ccb);
  auto* p_ccb2 = sdp_conn_originate(addr);
  ASSERT_NE(nullptr, p_ccb2);
  auto* p_ccb3 = sdp_conn_originate(addr);
  ASSERT_NE(nullptr, p_ccb3);
  auto* p_ccb4 = sdp_conn_originate(addr);
  ASSERT_NE(nullptr, p_ccb4);

  ASSERT_STREQ(sdp_state_text(p_ccb->con_state).c_str(),
               sdp_state_text(tSDP_STATE::CONN_SETUP).c_str());
  ASSERT_STREQ(sdp_state_text(p_ccb2->con_state).c_str(),
               sdp_state_text(tSDP_STATE::CONN_PEND).c_str());
  ASSERT_STREQ(sdp_state_text(p_ccb3->con_state).c_str(),
               sdp_state_text(tSDP_STATE::CONN_PEND).c_str());
  ASSERT_STREQ(sdp_state_text(p_ccb4->con_state).c_str(),
               sdp_state_text(tSDP_STATE::CONN_PEND).c_str());
