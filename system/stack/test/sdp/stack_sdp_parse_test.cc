...
class StackSdpAsClientParseTest : public StackSdpParserInitTest {
  ...
  const bluetooth::Uuid p_uuid_list[1]{
    bluetooth::Uuid::From128BitBE(bluetooth::os::GenerateRandom<bluetooth::Uuid::kNumBytes128>()),
  };
...
TEST_F_WITH_FLAGS(StackSdpAsClientParseTest, sdp_disc_server_rsp_packets00,
  ...
  p_sdp_rec = p_db_->p_first_rec;
  ASSERT_EQ(true,
            get_legacy_stack_sdp_api()->record.SDP_FindServiceUUIDInRec_128bit(p_sdp_rec,
                                                                               &uuid_list[0]));
  ...
  // Browse Group List
  ASSERT_NE(nullptr,
            get_legacy_stack_sdp_api()->record.SDP_FindAttributeInRec(p_sdp_rec,
