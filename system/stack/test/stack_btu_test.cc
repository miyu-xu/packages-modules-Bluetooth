...
TEST_F(StackBtuTest, btm_sco_connected_called) {
  uint8_t p_cmd[10];  // garbage data for testing
  btu_hcif_hdl_command_status(HCI_SETUP_ESCO_CONNECTION, HCI_ERR_UNSPECIFIED,
                              p_cmd, nullptr);
  ASSERT_EQ(1, mock_function_count_map["btm_sco_connected"]);
}