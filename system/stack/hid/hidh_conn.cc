...
static void hidh_l2cif_connect_ind(const RawAddress& bd_addr, uint16_t l2cap_cid, uint16_t psm,
  ...
  BTM_LogHistory(
          ...
          std::format("{} state:{}", (psm == HID_PSM_CONTROL) ? "control" : "interrupt", hid_conn::state_text(p_hcon->conn_state)));
...
static void hidh_l2cif_connect_cfm(uint16_t l2cap_cid, tL2CAP_CONN result) {
  ...
  BTM_LogHistory(
          ...
          std::format("control:0x{:04x} interrupt:0x{:04x} state:{}", p_hcon->ctrl_cid, p_hcon->intr_cid, hid_conn::state_text(p_hcon->conn_state)));
  return;
}
