...
static void hidh_l2cif_connect_ind(const RawAddress& bd_addr,
  ...
  LOG_DEBUG("HID-Host Rcvd L2CAP conn ind, PSM: 0x%04x  CID 0x%x", psm,
            l2cap_cid);
...
static void hidh_l2cif_config_ind(uint16_t l2cap_cid, tL2CAP_CFG_INFO* p_cfg) {
  ...
  LOG_DEBUG("HID-Host Rcvd cfg ind, sent cfg cfm, CID: 0x%x", l2cap_cid);
...
static void hidh_l2cif_config_cfm(uint16_t l2cap_cid, uint16_t initiator,
  ...
  LOG_DEBUG("HID-Host Rcvd cfg cfm, CID: 0x%x", l2cap_cid);
...
static void hidh_l2cif_disconnect_ind(uint16_t l2cap_cid, bool ack_needed) {
  ...
  LOG_DEBUG("HID-Host Rcvd L2CAP disc, CID: 0x%x", l2cap_cid);
...
static void hidh_l2cif_disconnect(uint16_t l2cap_cid) {
  ...
  if (l2cap_cid == p_hcon->ctrl_cid) {
    ...
    p_hcon->intr_cid = 0;
    if (p_hcon->ctrl_cid) {
      LOG_DEBUG("HID-Host Initiating L2CAP Ctrl disconnection");
...
static void hidh_l2cif_cong_ind(uint16_t l2cap_cid, bool congested) {
  ...
  LOG_DEBUG("HID-Host Rcvd L2CAP congestion status, CID: 0x%x  Cong: %d",
            l2cap_cid, congested);
