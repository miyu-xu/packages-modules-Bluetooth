...
static void hidd_l2cif_connect_ind(const RawAddress& bd_addr, uint16_t cid,
  ...
  LOG_INFO("%s: psm=%04x cid=%04x", __func__, psm, cid);
...
static void hidd_l2cif_connect_cfm(uint16_t cid, uint16_t result) {
  ...
  LOG_INFO("%s: cid=%04x result=%d", __func__, cid, result);
 ...
 ******************************************************************************/
static void hidd_l2cif_config_ind(uint16_t cid, tL2CAP_CFG_INFO* p_cfg) {
  LOG_INFO("%s: cid=%04x", __func__, cid);
...
static void hidd_l2cif_disconnect_ind(uint16_t cid, bool ack_needed) {
  ...
  LOG_INFO("%s: cid=%04x ack_needed=%d", __func__, cid, ack_needed);
  ...
  if ((p_hcon->ctrl_cid == 0) && (p_hcon->intr_cid == 0)) {
    LOG_INFO("%s: INTR and CTRL disconnected", __func__);
...
static void hidd_l2cif_cong_ind(uint16_t cid, bool congested) {
  ...
  LOG_INFO("%s: cid=%04x congested=%d", __func__, cid, congested);
  ...
  switch (msg_type) {
    ...
    case HID_TRANS_SET_IDLE:
      if (p_msg->len != 2) {
        ...
        LOG_DEBUG("%s: idle_time = %d", __func__, hd_cb.device.idle_time);
