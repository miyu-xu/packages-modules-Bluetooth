...
static tAVRC_STS avrc_pars_vendor_rsp(tAVRC_MSG_VENDOR* p_msg,
  ...
  LOG_DEBUG("%s ctype:0x%x pdu:0x%x, len:%d/0x%x vendor_len=0x%x", __func__,
...
/* case AVRC_PDU_ABORT_CONTINUATION_RSP:   0x41 */
    ...
    case AVRC_PDU_REGISTER_NOTIFICATION: /* 0x31 */
      ...
      LOG_DEBUG("%s PDU reg notif response:event %x, volume %x", __func__,
...
static tAVRC_STS avrc_pars_browse_rsp(tAVRC_MSG_BROWSE* p_msg,
  ...
  if (p_msg->browse_len == 0) {
    LOG_ERROR("%s length ", p_msg->browse_len);
  ...
  LOG_DEBUG("%s pdu:%d, pkt_len:%d", __func__, pdu, pkt_len);
  ...
  switch (pdu) {
    case AVRC_PDU_GET_FOLDER_ITEMS: {
      ...
      LOG_DEBUG("%s pdu %d status %d pkt_len %d uid counter %d item count %d",
      ...
      for (int i = 0; i < get_item_rsp->item_count; i++) {
        ...
        LOG_DEBUG("%s item type %d", __func__, curr_item->item_type);
        switch (curr_item->item_type) {
          ...
          case AVRC_ITEM_FOLDER: {
            ...
            LOG_DEBUG("%s type %d playable %d cs %d name len %d", __func__,
          ...
          case AVRC_ITEM_MEDIA: {
            ...
            LOG_DEBUG("%s media type %d charset id %d len %d attr ct %d",
            ...
            for (int jk = 0; jk < media->attr_count; jk++) {
              ...
              LOG_DEBUG("%s media attr id %d cs %d name len %d", __func__,
        ...
        LOG_DEBUG("%s pkt_len %d min_len %d", __func__, pkt_len, min_len);
    ...
    case AVRC_PDU_CHANGE_PATH: {
      ...
      LOG_DEBUG("%s pdu %d status %d item count %d", __func__,
    ...
    case AVRC_PDU_SET_BROWSED_PLAYER: {
      ...
      for (uint32_t i = 0; i < set_br_pl_rsp->folder_depth; i++) {
        ...
        if (pkt_len < min_len) goto browse_length_error;
        LOG_DEBUG("%s AVRC_PDU_SET_BROWSED_PLAYER item: %d len: %d", __func__,
...
static tAVRC_STS avrc_ctrl_pars_vendor_rsp(tAVRC_MSG_VENDOR* p_msg,
  ...
  uint32_t min_len = 0;
  BE_STREAM_TO_UINT16(len, p);
  LOG_DEBUG("%s ctype:0x%x pdu:0x%x, len:%d  vendor_len=0x%x", __func__,
  ...
  switch (p_result->pdu) {
    ...
    case AVRC_PDU_GET_CAPABILITIES:
      ...
      LOG_DEBUG("%s cap id = %d, cap_count = %d ", __func__,
    ...
    case AVRC_PDU_LIST_PLAYER_APP_ATTR:
      ...
      LOG_DEBUG("%s attr count = %d ", __func__,
    ...
    case AVRC_PDU_LIST_PLAYER_APP_VALUES:
      ...
      LOG_DEBUG("%s value count = %d ", __func__,
