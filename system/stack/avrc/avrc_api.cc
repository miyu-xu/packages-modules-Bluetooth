...
void avrc_flush_cmd_q(uint8_t handle) {
  LOG_DEBUG("AVRC: Flushing command queue for handle=0x%02x", handle);
...
void avrc_process_timeout(void* data) {
  ...
  LOG_DEBUG("AVRC: command timeout (handle=0x%02x, label=0x%02x)",
...
void avrc_send_next_vendor_cmd(uint8_t handle) {
  ...
  while ((p_next_cmd = (BT_HDR*)fixed_queue_try_dequeue(
    ...
    LOG_DEBUG("AVRC: Dequeuing command 0x%08x (handle=0x%02x, label=0x%02x)",
...
void avrc_start_cmd_timer(uint8_t handle, uint8_t label, uint8_t msg_mask) {
  ...
  LOG_DEBUG("AVRC: starting timer (handle=0x%02x, label=0x%02x)", handle,
            label);
...
static uint16_t avrc_send_continue_frag(uint8_t handle, uint8_t label) {
  ...
  LOG_DEBUG("%s handle = %u label = %u len = %d", __func__, handle, label,
            p_pkt->len);
  ...
  if (pkt_type != AVRC_PKT_SINGLE && cr == AVCT_RSP) {
    ...
    if (pkt_type == AVRC_PKT_START) {
      ...
      *pp_pkt = p_rcb->p_rmsg;
      ...
      LOG_DEBUG("Received a CONTINUE/END without no corresponding START \
                                (or previous fragmented response was dropped)");
      drop_code = 5;
      osi_free(p_pkt);
      *pp_pkt = NULL;
      ...
      if (pkt_type == AVRC_PKT_END) {
        ...
        *p_data++ = AVRC_PKT_SINGLE;
        ...
        LOG_DEBUG("end frag:%d, total len:%d, offset:%d", p_pkt->len,
...
static void avrc_msg_cback(uint8_t handle, uint8_t label, uint8_t cr,
  ...
  if (cr == AVCT_REJ) {
    ...
    LOG_DEBUG("AVRC: stopping timer (handle=0x%02x)", handle);
    alarm_cancel(avrc_cb.ccb_int[handle].tle);
  }
  ...
  if (p_pkt->layer_specific == AVCT_DATA_BROWSE) {
    ...
    LOG_DEBUG("%s handle:%d, ctype:%d, offset:%d, len: %d", __func__, handle,
...
uint16_t AVRC_Open(uint8_t* p_handle, tAVRC_CONN_CB* p_ccb,
  ...
  LOG_DEBUG("%s role: %d, control:%d status:%d, handle:%d", __func__, cc.role,
...
uint16_t AVRC_Close(uint8_t handle) {
  LOG_DEBUG("%s handle:%d", __func__, handle);
  avrc_flush_cmd_q(handle);
...
uint16_t AVRC_MsgReq(uint8_t handle, uint8_t label, uint8_t ctype,
  ...
  LOG_DEBUG("%s handle = %u label = %u ctype = %u len = %d", __func__, handle,
  ...
  if ((cr == AVCT_RSP) && (chk_frag)) {
    if (p_pkt->len > AVRC_MAX_CTRL_DATA_LEN) {
      ...
      if (p_start != NULL) {
        ...
        *p_data++ = AVRC_PKT_START;
        ...
        /* prepare the left over for as an end fragment */
        avrc_prep_end_frag(handle);
        LOG_DEBUG("%s p_pkt len:%d/%d, next len:%d", __func__, p_pkt->len, len,
                  p_fcb->p_fmsg->len);
      } else {
    ...
    LOG_DEBUG("AVRC: Enqueuing command 0x%08x (handle=0x%02x, label=0x%02x)",
              p_pkt, handle, label);
