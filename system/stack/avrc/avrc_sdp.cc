...
uint16_t AVRC_AddRecord(uint16_t service_uuid, const char* p_service_name,
  ...
  LOG_DEBUG("%s: Add AVRCP SDP record, uuid: %x, profile_version: 0x%x, "
  ...
  if (profile_version > AVRC_REV_1_3) {
    ...
    if (browse_supported) {
      LOG_DEBUG("%s: Add Browsing PSM to additonal protocol descriptor"
                " lists",
                __func__);
    ...
    if (profile_version >= AVRC_REV_1_6 &&
        ...
        cover_art_psm > 0) {
      LOG_DEBUG("%s: Add AVRCP BIP PSM to additonal protocol descriptor"
                " lists, psm: 0x%x",
    ...
    if (num_additional_protocols > 0) {
      LOG_DEBUG("%s: Add %d additonal protocol descriptor lists", __func__,
                num_additional_protocols);
...
uint16_t AVRC_RemoveRecord(uint32_t sdp_handle) {
  LOG_DEBUG("%s: remove AVRCP SDP record", __func__);
