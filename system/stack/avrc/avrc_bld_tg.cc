...
static tAVRC_STS avrc_bld_get_capability_rsp(tAVRC_GET_CAPS_RSP* p_rsp,
  ...
  if (!(AVRC_IS_VALID_CAP_ID(p_rsp->capability_id))) {
    LOG_ERROR("%s bad parameter. p_rsp: %x", __func__, p_rsp);
...
static tAVRC_STS avrc_bld_app_setting_text_rsp(
  ...
  for (xx = 0; xx < p_rsp->num_attr; xx++) {
    if (len_left < (p_rsp->p_attrs[xx].str_len + 4)) {
      LOG_ERROR("%s out of room (str_len:%d, left:%d)", __func__, xx,
...
static void avrc_build_attribute_entries(int num_attrs,
  ...
  LOG_DEBUG("%s num_attrs: %d, remaining_buffer_capacity: %d", __func__,
  ...
  for (int index = 0; index < num_attrs; index++) {
    LOG_DEBUG("%s attr id[%d]: %d", __func__, index, p_attrs[index].attr_id);
    ...
    if (remaining_buffer_capacity < 0) {
      LOG_WARN("%s not enough buffer space for attr_id[%d]: %d,"
  ...
  *pp_data = p_data;
  LOG_DEBUG("%s filled attributes, remaining_buffer_capacity: %d", __func__,
            num_attrs, remaining_buffer_capacity);
}
...
static tAVRC_STS avrc_bld_get_elem_attrs_rsp(tAVRC_GET_ATTRS_RSP* p_rsp,
  ...
  if (remaining_buffer_capacity < 5) {
    LOG_ERROR("%s not enough buffer for packet header",
  ...
  ;
  if (remaining_buffer_capacity < 0) {
    LOG_ERROR("%s not enough buffer capacity for response");
    return AVRC_STS_BAD_PARAM;
  }
  /* Fill in the Attribute ID, Character Set, Length and Values */
...
static tAVRC_STS avrc_bld_notify_rsp(tAVRC_REG_NOTIF_RSP* p_rsp,
  ...
  LOG_INFO("%s event_id %d", __func__, p_rsp->event_id);
...
static tAVRC_STS avrc_bld_next_rsp(tAVRC_NEXT_RSP* p_rsp, BT_HDR* p_pkt) {
  ...
  LOG_INFO("%s: target_pdu: 0x%02x", __func__, p_rsp->target_pdu);
  return AVRC_STS_NO_ERROR;
}
...
static tAVRC_STS avrc_bld_rejected_rsp(tAVRC_RSP* p_rsp, BT_HDR* p_pkt) {
  ...
  LOG_INFO("%s: status=%d, pdu:x%x, opcode=%x", __func__, p_rsp->status,
           p_rsp->pdu, opcode);
  ...
  LOG_DEBUG("%s pdu:x%x, Opcode:%x", __func__, *p_start, opcode);
  UINT16_TO_BE_STREAM(p_data, 1);
...
static tAVRC_STS avrc_bld_set_browsed_player_rsp(tAVRC_SET_BR_PLAYER_RSP* p_rsp,
  ...
  LOG_DEBUG("len_left:%d, mtu:%d ", len_left, mtu);
...
static tAVRC_STS avrc_bld_get_folder_items_rsp(tAVRC_GET_ITEMS_RSP* p_rsp,
  ...
  LOG_DEBUG("len:%d, len_left:%d, num:%d", len, len_left, item_count);
  ...
  for (xx = 0;
    ...
    LOG_DEBUG("len:%d, len_left:%d, num:%d, item_len:%d", len, len_left,
              item_count, item_len);
  } /* for item_count */
...
static tAVRC_STS avrc_bld_get_item_attrs_rsp(tAVRC_GET_ATTRS_RSP* p_rsp,
  ...
  LOG_DEBUG("%s: remaining_buffer_capacity:%d, mtu:%d", __func__,
            remaining_buffer_capacity, mtu);
  if (remaining_buffer_capacity < 5) {
...
static BT_HDR* avrc_bld_init_rsp_buffer(tAVRC_RESPONSE* p_rsp) {
  ...
  LOG_INFO("%s: pdu=%x, opcode=%x/%x", __func__, p_rsp->pdu, opcode,
           p_rsp->rsp.opcode);
  if (opcode != p_rsp->rsp.opcode && p_rsp->rsp.status != AVRC_STS_NO_ERROR &&
    ...
    LOG_INFO("%s opcode=%x", __func__, opcode);
  }
...
tAVRC_STS AVRC_BldResponse(uint8_t handle, tAVRC_RESPONSE* p_rsp,
  ...
  if (!p_rsp || !pp_pkt) {
    LOG_INFO("%s Invalid parameters passed. p_rsp=%p, pp_pkt=%p", __func__,
             p_rsp, pp_pkt);
    return AVRC_STS_BAD_PARAM;
  }
  ...
  if (*pp_pkt == NULL) {
    *pp_pkt = avrc_bld_init_rsp_buffer(p_rsp);
    if (*pp_pkt == NULL) {
      LOG_INFO("%s Failed to initialize response buffer", __func__);
      return AVRC_STS_INTERNAL_ERR;
    }
  ...
  LOG_INFO("%s pdu=%x status=%x", __func__, p_rsp->rsp.pdu, p_rsp->rsp.status);
  ...
  LOG_INFO("%s returning %d", __func__, status);
  return status;
}
