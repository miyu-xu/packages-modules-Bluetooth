...
static tAVRC_STS avrc_bld_notify_rsp(tAVRC_REG_NOTIF_RSP* p_rsp,
  ...
  LOG_DEBUG("%s event_id %d", __func__, p_rsp->event_id);
...
static tAVRC_STS avrc_bld_next_rsp(tAVRC_NEXT_RSP* p_rsp, BT_HDR* p_pkt) {
  ...
  LOG_DEBUG("%s: target_pdu: 0x%02x", __func__, p_rsp->target_pdu);
  return AVRC_STS_NO_ERROR;
}
...
static tAVRC_STS avrc_bld_rejected_rsp(tAVRC_RSP* p_rsp, BT_HDR* p_pkt) {
  ...
  LOG_DEBUG("%s: status=%d, pdu:x%x, opcode=%x", __func__, p_rsp->status,
            p_rsp->pdu, opcode);
...
static BT_HDR* avrc_bld_init_rsp_buffer(tAVRC_RESPONSE* p_rsp) {
  ...
  LOG_DEBUG("%s: pdu=%x, opcode=%x/%x", __func__, p_rsp->pdu, opcode,
            p_rsp->rsp.opcode);
  if (opcode != p_rsp->rsp.opcode && p_rsp->rsp.status != AVRC_STS_NO_ERROR &&
    ...
    LOG_DEBUG("%s opcode=%x", __func__, opcode);
  }
...
tAVRC_STS AVRC_BldResponse(uint8_t handle, tAVRC_RESPONSE* p_rsp,
  ...
  if (!p_rsp || !pp_pkt) {
    LOG_DEBUG("%s Invalid parameters passed. p_rsp=%p, pp_pkt=%p", __func__,
              p_rsp, pp_pkt);
    return AVRC_STS_BAD_PARAM;
  }
  ...
  if (*pp_pkt == NULL) {
    *pp_pkt = avrc_bld_init_rsp_buffer(p_rsp);
    if (*pp_pkt == NULL) {
      LOG_DEBUG("%s Failed to initialize response buffer", __func__);
      return AVRC_STS_INTERNAL_ERR;
    }
  ...
  LOG_DEBUG("%s pdu=%x status=%x", __func__, p_rsp->rsp.pdu, p_rsp->rsp.status);
  ...
  LOG_DEBUG("%s returning %d", __func__, status);
  return status;
}
