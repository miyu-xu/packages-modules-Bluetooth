...
class HasClientTestBase : public ::testing::Test {
 protected:
  ...
  void set_sample_database(
    ...
    ON_CALL(gatt_queue,
        ...
        .WillByDefault(Invoke([this, address](uint16_t conn_id, uint16_t handle,
          ...
          auto pp = value.data();
          auto len = value.size();
          uint8_t op, index, num_of_indexes;
          ...
          switch (static_cast<::le_audio::has::PresetCtpOpcode>(op)) {
            case ::le_audio::has::PresetCtpOpcode::READ_PRESETS:
              if (len < 2) {
                ...
                STREAM_TO_UINT8(num_of_indexes, pp);
                len -= 2;
                ASSERT_EQ(0u, len);

                InjectNotifyReadPresetsResponse(conn_id, address, handle, value,
                                                indicate, index, num_of_indexes,
                                                cb, cb_data);
              }
              break;
  ...
  void InjectNotifyReadPresetsResponse(
      ...
      std::vector<uint8_t> value, bool indicate, int index, int num_of_indexes,
    ...
    while (1) {
      bool last =
          preset == std::prev(presets.end()) || num_of_notif == num_of_indexes;
