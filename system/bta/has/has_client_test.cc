...
TEST_F(HasClientTest, test_preset_set_name_invalid_index) {
  ...
  EXPECT_CALL(*callbacks,
              OnSetPresetNameError(std::variant<RawAddress, int>(test_address), 0x40,
...
TEST_F(HasClientTest, test_preset_set_name_to_long) {
  ...
  EXPECT_CALL(
          gatt_queue,
          WriteCharacteristic(test_conn_id, HasDbBuilder::kPresetsCtpValHdl, _, GATT_WRITE, _, _))
          .Times(0);
...
TEST_F(HasClientTest, test_preset_group_set_name) {
  ...
  EXPECT_CALL(gatt_queue,
              WriteCharacteristic(GetTestConnId(test_address1), HasDbBuilder::kPresetsCtpValHdl, _,
                                  GATT_WRITE, _, _))
          .Times(1);
  EXPECT_CALL(gatt_queue,
              WriteCharacteristic(GetTestConnId(test_address2), HasDbBuilder::kPresetsCtpValHdl, _,
                                  GATT_WRITE, _, _))
          .Times(1);
...
TEST_F(HasClientTest, test_presets_get_name_invalid_index) {
  ...
  EXPECT_CALL(*callbacks,
              OnPresetInfoError(std::variant<RawAddress, int>(test_address), 128,
                                ErrorCode::INVALID_PRESET_INDEX));
...
TEST_F(HasClientTest, test_select_group_preset_invalid_group) {
  ...
  EXPECT_CALL(*callbacks,
              OnActivePresetSelectError(std::variant<RawAddress, int>(unlucky_group),
...
TEST_F(HasClientTest, test_select_group_preset_valid_preset_sync_supported) {
  ...
  EXPECT_CALL(
          gatt_queue,
          WriteCharacteristic(test_conn_id1, HasDbBuilder::kPresetsCtpValHdl, _, GATT_WRITE, _, _))
          .Times(0);
  EXPECT_CALL(
          gatt_queue,
          WriteCharacteristic(test_conn_id2, HasDbBuilder::kPresetsCtpValHdl, _, GATT_WRITE, _, _))
          .Times(1);
...
TEST_F(HasClientTest, test_select_preset_invalid) {
  ...
  EXPECT_CALL(*callbacks,
              OnActivePresetSelectError(std::variant<RawAddress, int>(test_address),
                                        ErrorCode::INVALID_PRESET_INDEX))
          .Times(1);
