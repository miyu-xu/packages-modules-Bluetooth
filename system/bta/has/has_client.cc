 ...
 * connected and has his characteristics discovered. All the characteristics
 ...
 * about the successfull HAS device connection and it's features and the list
...
class HasClientImpl : public HasClient {
 ...
 private:
  ...
  void OnHasActivePresetValue(
    ...
    /* If svc not marked valid, this might be the the last validation step. */
    MarkDeviceValidIfInInitialDiscovery(*device);
  ...
  bool StartInitialHasDetailsReadAndValidation(const gatt::Service& service,
     ...
     */
    if (device->SupportsPresets()) {
      /* Subscribe for control point notifications on EATT and indications
       * otherwise.
       */
      SubscribeForNotifications(device->conn_id, device->addr,
                                device->cp_handle, device->cp_ccc_handle,
                                gatt_profile_get_eatt_support(device->addr));
  ...
  bool OnHasServiceFound(const gatt::Service& service, void* context) {
    ...
    /* Initally validate and store GATT service discovery data */
  ...
  void SubscribeForNotifications(uint16_t conn_id, const RawAddress& address,
                                 ...
                                 bool additional_indications = false) {
    uint16_t ccc_val =
        GATT_CHAR_CLIENT_CONFIG_NOTIFICATION |
        (additional_indications ? GATT_CHAR_CLIENT_CONFIG_INDICTION : 0);
