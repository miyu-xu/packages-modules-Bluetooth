...
private:
  ...
  void UpdateGroupMaxActiveDevices(HasDevice* device, int group_id) {
    ...
    if (!LeAudioClient::IsLeAudioClientRunning()) {
      log::warn("LeAudioClient not running");
      return;
    }

    int desired_group_size = GetExpectedGroupSize(device->GetFeatures());

    auto addresses = LeAudioClient::Get()->GetGroupDevices(group_id);
    if (!addresses.empty()) {
      for (auto const& addr : addresses) {
        if (addr == device->addr) {
          continue;
        ...
        if (has_device != devices_.end()) {
          desired_group_size =
                  std::max(desired_group_size, GetExpectedGroupSize(has_device->GetFeatures()));
        }
      }
    }

    LeAudioClient::Get()->SetDesiredGroupSize(group_id, desired_group_size);
  }
