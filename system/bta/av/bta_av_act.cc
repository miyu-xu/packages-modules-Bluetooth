...
void bta_av_del_rc(tBTA_AV_RCB* p_rcb) {
  ...
  if (p_rcb->handle != BTA_AV_RC_HANDLE_NONE) {
    ...
    LOG_DEBUG("%s: handle: %d status=0x%x, rc_acp_handle:%d, idx:%d", __func__,
...
static void bta_av_rc_ctrl_cback(uint8_t handle, uint8_t event,
  ...
  LOG_DEBUG("%s: handle: %d event=0x%x", __func__, handle, event);
...
void bta_av_rc_opened(tBTA_AV_CB* p_cb, tBTA_AV_DATA* p_data) {
  ...
  if (btif_av_both_enable()) {
    ...
    LOG_VERBOSE("local features:0x%x peer_features:0x%x, peer_ct_feature:0x%x, "
...
void bta_av_conn_chg(tBTA_AV_DATA* p_data) {
  ...
  if (p_data->conn_chg.is_up) {
    ...
    if (p_scb) {
      ...
      if (p_lcb && p_cb->rc_acp_handle != BTA_AV_RC_HANDLE_NONE &&
        ...
        LOG_VERBOSE("%s: rc_acp is connected && conn_chg on same addr "
  ...
  LOG_VERBOSE("%s: audio:%x up:%d conn_msk:0x%x chk_restore:%d "
              "audio_open_cnt:%d",
