...
void bta_av_del_rc(tBTA_AV_RCB* p_rcb) {
  ...
  if (p_rcb->handle != BTA_AV_RC_HANDLE_NONE) {
    if (p_rcb->shdl) {
      ...
      if (p_scb) {
        LOG_DEBUG("%s: shdl:%d, srch:%d rc_handle:%d", __func__, p_rcb->shdl,
    ...
    LOG_INFO("%s: handle: %d status=0x%x, rc_acp_handle:%d, idx:%d", __func__,
    ...
    LOG_INFO("%s: end del_rc handle: %d status=0x%x, rc_acp_handle:%d, lidx:%d",
...
static void bta_av_rc_ctrl_cback(uint8_t handle, uint8_t event,
  ...
  if (btif_av_both_enable() && peer_addr != NULL &&
    ...
    if (event == AVRC_CLOSE_IND_EVT || event == AVRC_OPEN_IND_EVT) {
      LOG_DEBUG("%s: resend close event", __func__);
  ...
  LOG_INFO("%s: handle: %d event=0x%x", __func__, handle, event);
...
static void bta_av_rc_msg_cback(uint8_t handle, uint8_t label, uint8_t opcode,
  ...
  LOG_DEBUG("%s: handle: %u opcode=0x%x", __func__, handle, opcode);
...
uint8_t bta_av_rc_create(tBTA_AV_CB* p_cb, uint8_t role, uint8_t shdl,
  ...
  if (lidx == (BTA_AV_NUM_LINKS + 1)) {
    ...
    p_cb->rc_acp_idx = (i + 1);
    LOG_DEBUG("%s: rc_acp_handle:%d idx:%d", __func__, p_cb->rc_acp_handle,
              p_cb->rc_acp_idx);
  }
  LOG_DEBUG(
...
tBTA_AV_LCB* bta_av_find_lcb(const RawAddress& addr, uint8_t op) {
  ...
  LOG_DEBUG("%s: address: %s op:%d", __func__, ADDRESS_TO_LOGGABLE_CSTR(addr),
            op);
  for (xx = 0; xx < BTA_AV_NUM_LINKS; xx++) {
    ...
    if ((mask & p_cb->conn_lcb) && p_cb->lcb[xx].addr == addr) {
      ...
      if (op == BTA_AV_LCB_FREE) {
        ...
        LOG_DEBUG("%s: conn_lcb: 0x%x", __func__, p_cb->conn_lcb);
      }
      break;
    }
  }
...
void bta_av_rc_opened(tBTA_AV_CB* p_cb, tBTA_AV_DATA* p_data) {
  ...
  for (i = 0; i < BTA_AV_NUM_STRS; i++) {
    ...
    if (p_scb && p_scb->PeerAddress() == p_data->rc_conn_chg.peer_addr) {
      ...
      LOG_DEBUG("%s: shdl:%d, srch %d", __func__, i + 1, p_scb->rc_handle);
      shdl = i + 1;
  ...
  LOG_DEBUG("%s: local features %d peer features %d", __func__, p_cb->features,
  ...
  if (p_cb->rcb[i].lidx == (BTA_AV_NUM_LINKS + 1) && shdl != 0) {
    ...
    if (p_rcb) {
      ...
      LOG_DEBUG("%s: switching RCB rc_acp_handle:%d idx:%d", __func__,
  ...
  if (btif_av_both_enable()) {
    ...
    LOG_DEBUG("local features:0x%x peer_features:0x%x, peer_ct_feature:0x%x, "
    ...
    if ((p_cb->features & BTA_AV_FEAT_BROWSE) &&
      ...
      if ((p_cb->rcb[i].status & BTA_AV_RC_ROLE_MASK) == BTA_AV_RC_ROLE_INT) {
        LOG_DEBUG("%s: opening AVRC Browse channel", __func__);
  ...
  rc_open.status = BTA_AV_SUCCESS;
  LOG_DEBUG("%s: local features:x%x peer_features:x%x", __func__,
            p_cb->features, rc_open.peer_features);
  LOG_DEBUG("%s: cover art psm:x%x", __func__, rc_open.cover_art_psm);
...
void bta_av_proc_rsp(tAVRC_RESPONSE* p_rc_rsp) {
  ...
  LOG_DEBUG("%s: ver: 0x%x", __func__, rc_ver);
...
tBTA_AV_EVT bta_av_proc_meta_cmd(tAVRC_RESPONSE* p_rc_rsp,
  ...
  if (p_vendor->vendor_len == 0) {
    ...
    LOG_DEBUG("%s: p_vendor->vendor_len == 0", __func__);
  ...
  *p_ctype = AVRC_RSP_REJ;
  ...
  if (p_vendor->hdr.subunit_type != AVRC_SUB_PANEL) {
    LOG_DEBUG("%s: SUBUNIT must be PANEL", __func__);
    /* reject it */
    evt = 0;
    ...
    LOG_DEBUG("%s: Invalid pdu/ctype: 0x%x, %d", __func__, pdu,
              p_vendor->hdr.ctype);
        ...
        if (u16 != 1) {
          ...
          if (u8 == AVRC_CAP_COMPANY_ID) {
            ...
            *p_ctype = AVRC_RSP_IMPL_STBL;
            ...
            LOG_DEBUG("%s: Invalid capability ID: 0x%x", __func__, u8);
...
void bta_av_rc_msg(tBTA_AV_CB* p_cb, tBTA_AV_DATA* p_data) {
  ...
  LOG_DEBUG("%s: opcode=%x, ctype=%x", __func__, p_data->rc_msg.opcode,
  ...
  if (p_data->rc_msg.opcode == AVRC_OP_PASS_THRU) {
    ...
    if ((p_data->rc_msg.msg.hdr.ctype == AVRC_CMD_CTRL) ||
      ...
      LOG_DEBUG("%s: ctype %d", __func__, p_data->rc_msg.msg.hdr.ctype)
    ...
    /* id response type is not impl, we have to release label */
      ...
      if ((p_data->rc_msg.msg.pass.op_id == AVRC_ID_VENDOR) &&
        ...
        LOG_DEBUG("%s: Vendor Unique data len = %d", __func__,
                  p_data->rc_msg.msg.pass.pass_len);
...
void bta_av_rc_close(tBTA_AV_CB* p_cb, tBTA_AV_DATA* p_data) {
  ...
  if (handle < BTA_AV_NUM_RCB) {
    ...
    LOG_DEBUG("%s: handle: %d, status=0x%x", __func__, p_rcb->handle,
              p_rcb->status);
    if (p_rcb->handle != BTA_AV_RC_HANDLE_NONE) {
      if (p_rcb->shdl) {
        p_scb = bta_av_cb.p_scb[p_rcb->shdl - 1];
...
void bta_av_stream_chg(tBTA_AV_SCB* p_scb, bool started) {
  ...
  LOG_DEBUG("%s: peer %s started:%s started_msk:0x%x", __func__,
...
void bta_av_conn_chg(tBTA_AV_DATA* p_data) {
  ...
  if (p_data->conn_chg.is_up) {
    ...
    if (p_scb) {
      if (p_lcb) {
        ...
        for (i = 0; i < BTA_AV_NUM_RCB; i++) {
          if (bta_av_cb.rcb[i].lidx == p_lcb->lidx) {
            ...
            LOG_DEBUG("%s: conn_chg up[%d]: %d, status=0x%x, shdl:%d, lidx:%d",
      ...
      LOG_DEBUG("%s: rc_acp_handle:%d rc_acp_idx:%d", __func__,
                p_cb->rc_acp_handle, p_cb->rc_acp_idx);
      ...
      if (p_lcb && p_cb->rc_acp_handle != BTA_AV_RC_HANDLE_NONE &&
        ...
        LOG_DEBUG("%s: rc_acp is connected && conn_chg on same addr "
        ...
        if (p_lcb_rc->conn_msk &&
          ...
          LOG_DEBUG("%s: update rc_acp shdl:%d/%d srch:%d", __func__, index + 1,
          ...
          if (p_rcb2) {
            ...
            LOG_DEBUG("%s: new rc_acp_handle:%d, idx:%d", __func__,
                      p_cb->rc_acp_handle, p_cb->rc_acp_idx);
            p_rcb2->lidx = (BTA_AV_NUM_LINKS + 1);
            LOG_DEBUG("%s: rc2 handle:%d lidx:%d/%d", __func__, p_rcb2->handle,
                      p_rcb2->lidx, p_cb->lcb[p_rcb2->lidx - 1].lidx);
          }
          p_rcb->lidx = p_lcb->lidx;
          LOG_DEBUG("%s: rc handle:%d lidx:%d/%d", __func__, p_rcb->handle,
    ...
    LOG_DEBUG("%s: shdl:%d", __func__, index + 1);
    for (i = 0; i < BTA_AV_NUM_RCB; i++) {
      LOG_DEBUG("%s: conn_chg dn[%d]: %d, status=0x%x, shdl:%d, lidx:%d",
  ...
  LOG_DEBUG("%s: audio:%x up:%d conn_msk:0x%x chk_restore:%d "
            "audio_open_cnt:%d",
            __func__, p_cb->conn_audio, p_data->conn_chg.is_up, conn_msk,
...
static uint8_t bta_av_find_lcb_index_by_scb_and_address(
  ...
  LOG_DEBUG("%s: peer_address: %s conn_lcb: 0x%x", __func__,
...
void bta_av_sig_chg(tBTA_AV_DATA* p_data) {
  ...
  if (event == AVDT_CONNECT_IND_EVT) {
    LOG_DEBUG("%s: AVDT_CONNECT_IND_EVT: peer %s", __func__,
    ...
    if (!p_lcb) {
      ...
      LOG_DEBUG("%s: start sig timer %d", __func__, p_data->hdr.offset);
      if (p_data->hdr.offset == AVDT_ACP) {
        LOG_DEBUG("%s: Incoming L2CAP acquired, set state as incoming",
                  __func__);
        ...
        LOG_DEBUG("%s: Re-start timer for AVDTP service", __func__);
    ...
    /* disconnected. */
    LOG_DEBUG("%s: bta_av_cb.conn_lcb=0x%x", __func__, bta_av_cb.conn_lcb);

    p_lcb = bta_av_find_lcb(p_data->str_msg.bd_addr, BTA_AV_LCB_FREE);
    if (p_lcb && (p_lcb->conn_msk || bta_av_cb.conn_lcb)) {
      LOG_DEBUG("%s: conn_msk: 0x%x", __func__, p_lcb->conn_msk);
      /* clean up ssm  */
      for (xx = 0; xx < BTA_AV_NUM_STRS; xx++) {
        if (p_cb->p_scb[xx] &&
          ...
          LOG_DEBUG("%s: Closing timer for AVDTP service", __func__);
  ...
  LOG_DEBUG("%s: bta_av_cb.conn_lcb=0x%x after sig_chg", __func__,
            p_cb->conn_lcb);
}
...
void bta_av_signalling_timer(UNUSED_ATTR tBTA_AV_DATA* p_data) {
  ...
  for (xx = 0; xx < BTA_AV_NUM_LINKS; xx++) {
    p_lcb = &p_cb->lcb[xx];
    mask = 1 << xx;
    LOG_DEBUG("%s: index=%d conn_lcb=0x%x peer=%s conn_mask=0x%x lidx=%d",
    ...
    if (mask & p_cb->conn_lcb) {
      ...
      if (!p_lcb->conn_msk) {
        LOG_DEBUG("%s hndl 0x%x", __func__, p_scb->hndl);
...
static void bta_av_accept_signalling_timer_cback(void* data) {
  ...
  if (p_scb) {
    LOG_DEBUG("%s: coll_mask=0x%02x", __func__, p_scb->coll_mask);

    if (p_scb->coll_mask & BTA_AV_COLL_INC_TMR) {
      p_scb->coll_mask &= ~BTA_AV_COLL_INC_TMR;

      if (bta_av_is_scb_opening(p_scb)) {
        LOG_DEBUG("%s: stream state opening: SDP started = %d", __func__,
        ...
        LOG_DEBUG("%s: stream state incoming", __func__);
...
tBTA_AV_FEAT bta_av_check_peer_features(uint16_t service_uuid) {
  ...
  LOG_DEBUG("%s: service_uuid:x%x", __func__, service_uuid);
  ...
  while (true) {
    ...
    if ((get_legacy_stack_sdp_api()->record.SDP_FindAttributeInRec(
      ...
      get_legacy_stack_sdp_api()->record.SDP_FindProfileVersionInRec(
          p_rec, UUID_SERVCLASS_AV_REMOTE_CONTROL, &peer_rc_version);
      LOG_DEBUG("%s: peer_rc_version 0x%x", __func__, peer_rc_version);
  ...
  LOG_DEBUG("%s: peer_features:x%x", __func__, peer_features);
...
tBTA_AV_FEAT bta_avk_check_peer_features(uint16_t service_uuid) {
  ...
  while (p_rec) {
    LOG_DEBUG("%s: found Service record for x%x", __func__, service_uuid);
    ...
    if ((get_legacy_stack_sdp_api()->record.SDP_FindAttributeInRec(
      ...
      bool val = get_legacy_stack_sdp_api()->record.SDP_FindProfileVersionInRec(
          p_rec, UUID_SERVCLASS_AV_REMOTE_CONTROL, &peer_rc_version);
      LOG_DEBUG("%s: peer_rc_version for TG 0x%x, profile_found %d", __func__,
                peer_rc_version, val);
...
uint16_t bta_avk_get_cover_art_psm() {
  LOG_DEBUG("%s: searching for cover art psm", __func__);
        ...
        if (SDP_DISC_ATTR_TYPE(p_protocol_list->attr_len_type)
          ...
          while (p_protocol) {
            ...
            if (protocol_has_l2cap && protocol_has_obex) {
              LOG_DEBUG("%s: found psm 0x%x", __func__, psm);
              return psm;
            }
  ...
  LOG_DEBUG("%s: could not find a BIP psm", __func__);
  return 0x0000;
}
...
void bta_av_rc_disc_done_all(UNUSED_ATTR tBTA_AV_DATA* p_data) {
  ...
  LOG_DEBUG("%s: bta_av_rc_disc_done disc:x%x", __func__, p_cb->disc);
  if (!p_cb->disc) {
    return;
  }
  ...
  if ((p_cb->disc & BTA_AV_CHNL_MSK) == BTA_AV_CHNL_MSK) {
    ...
    if (((p_cb->disc & BTA_AV_HNDL_MSK) - 1) < BTA_AV_NUM_STRS) {
      LOG_DEBUG("%s: wrong data bta_av_rc_disc_done disc:x%x", __func__,
                p_cb->disc);
  ...
  if (p_cb->sdp_a2dp_snk_handle) {
    ...
    LOG_DEBUG("%s: populating rem ctrl target bip psm 0x%x", __func__,
              cover_art_psm);
    ...
    if (p_rec != NULL &&
      ...
      SDP_FindProfileVersionInRec(p_rec, UUID_SERVCLASS_AV_REMOTE_CONTROL,
                                  &peer_rc_version);
      if (peer_rc_version <= AVRC_REV_1_3) {
        LOG_DEBUG("%s: Using AVRCP 1.3 Capabilities with remote device",
                  __func__);
  ...
  LOG_DEBUG("peer_tg_features 0x%x, peer_ct_features 0x%x, features 0x%x",
  ...
  if (rc_handle == BTA_AV_RC_HANDLE_NONE) {
    ...
    // Send PSM data
    LOG_DEBUG("%s: Send PSM data. rc_psm = %#x", __func__, cover_art_psm);
    p_cb->rcb[rc_handle].cover_art_psm = cover_art_psm;
    tBTA_AV bta_av_psm = {
        .rc_cover_art_psm =
            {
...
void bta_av_rc_disc_done(UNUSED_ATTR tBTA_AV_DATA* p_data) {
  ...
  LOG_DEBUG("%s: peer_features 0x%x, features 0x%x", __func__, peer_features,
            p_cb->features);
  ...
  if (rc_handle == BTA_AV_RC_HANDLE_NONE) {
    ...
    // Send PSM data
    LOG_DEBUG("%s: Send PSM data", __func__);
    ...
    LOG_DEBUG("%s: rc_psm = 0x%x", __func__, rc_psm.cover_art_psm);
...
void bta_av_rc_closed(tBTA_AV_DATA* p_data) {
  ...
  p_scb = NULL;
  LOG_DEBUG("%s: rc_handle:%d, address:%s", __func__, p_msg->handle,
            ADDRESS_TO_LOGGABLE_CSTR(p_msg->peer_addr));
  for (i = 0; i < BTA_AV_NUM_RCB; i++) {
    p_rcb = &p_cb->rcb[i];
    LOG_DEBUG("%s: rcb[%d] rc_handle:%d, status=0x%x, shdl:%d, lidx:%d",
    ...
    if (p_rcb->handle == p_msg->handle) {
      if (btif_av_src_sink_coexist_enabled() && p_rcb->shdl &&
        ...
        if (p_scb && !(p_scb->PeerAddress() == p_msg->peer_addr)) {
          LOG_DEBUG("%s: handle%d %s error p_scb or addr", __func__, i,
                    ADDRESS_TO_LOGGABLE_CSTR(p_scb->PeerAddress()));
          conn = true;
          continue;
        }
      ...
      p_rcb->peer_tg_features = 0;
      p_cb->rc_feature = {};
      LOG_DEBUG("%s: shdl:%d, lidx:%d", __func__, p_rcb->shdl, p_rcb->lidx);
      if (p_rcb->shdl) {
        if ((p_rcb->shdl - 1) < BTA_AV_NUM_STRS) {
          p_scb = bta_av_cb.p_scb[p_rcb->shdl - 1];
        }
        if (p_scb) {
          ...
          if (p_scb->rc_handle == p_rcb->handle)
            p_scb->rc_handle = BTA_AV_RC_HANDLE_NONE;
          LOG_DEBUG("%s: shdl:%d, srch:%d", __func__, p_rcb->shdl,
                    p_scb->rc_handle);
        }
        p_rcb->shdl = 0;
...
void bta_av_rc_disc(uint8_t disc) {
  ...
  LOG_DEBUG("%s: disc: 0x%x, bta_av_cb.disc: 0x%x", __func__, disc,
            bta_av_cb.disc);
  ...
  if ((disc & BTA_AV_CHNL_MSK) == BTA_AV_CHNL_MSK) {
    ...
    if (p_scb) {
      LOG_DEBUG("%s: rc_handle %d", __func__, p_scb->rc_handle);
      peer_addr = p_scb->PeerAddress();
    }
  }
  ...
  if (!peer_addr.IsEmpty()) {
    ...
    if (AVRC_FindService(UUID_SERVCLASS_AV_REMOTE_CONTROL, peer_addr,
      ...
      p_cb->disc = disc;
      LOG_DEBUG("%s: disc 0x%x", __func__, p_cb->disc);
    }
  }
}
...
void bta_av_dereg_comp(tBTA_AV_DATA* p_data) {
  ...
  if (p_scb) {
    LOG_DEBUG("%s: deregistered %d(h%d)", __func__, p_scb->chnl, p_scb->hndl);
    ...
    if (!p_cb->reg_audio) {
      ...
      if (is_new_avrcp_enabled()) {
        ...
        LOG_DEBUG("%s: newavrcp is not enabled. Remove SDP record", __func__);
  ...
  LOG_DEBUG("%s: audio 0x%x, disable:%d", __func__, p_cb->reg_audio,
            p_cb->disabling);
