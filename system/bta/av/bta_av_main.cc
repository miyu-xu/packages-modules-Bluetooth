...
static tBTA_AV_SCB* bta_av_find_scb(tBTA_AV_CHNL chnl, uint8_t app_id) {
  ...
  for (int xx = 0; xx < BTA_AV_NUM_STRS; xx++) {
    if ((bta_av_cb.p_scb[xx] != nullptr) &&
      ...
      LOG_DEBUG("%s: found at: %d", __func__, xx);
...
void bta_av_conn_cback(UNUSED_ATTR uint8_t handle, const RawAddress& bd_addr,
      ...
      event == AVDT_DISCONNECT_IND_EVT)
  {
    ...
    if (event == AVDT_DISCONNECT_IND_EVT) {
      ...
      LOG_DEBUG("%s: CONN_IND is ACP:%d", __func__, p_data->hdr.err_param);
    }
    ...
    if (p_scb) {
      LOG_DEBUG("%s: bta_handle x%x, role x%x", __func__, p_scb->hndl,
                p_scb->role);
    }
...
static void bta_av_api_register(tBTA_AV_DATA* p_data) {
  ...
  if (bta_av_cb.disabling || (bta_av_cb.features == 0)) {
    LOG_WARN("%s: AV instance (features=%#x, reg_audio=%#x) is not "
             "ready for app_id %d",
  ...
  LOG_DEBUG("%s: profile: 0x%x", __func__, profile_initialized);
  ...
  do {
    ...
    if ((btif_av_src_sink_coexist_enabled() &&
      ...
      if (bta_av_cb.features & (BTA_AV_FEAT_RCTG)) {
        ...
        if (is_new_avrcp_enabled()) {
          LOG_DEBUG("%s: newavrcp is the owner of the AVRCP Target SDP "
                    "record. Don't create the SDP record",
                    __func__);
        } else {
          LOG_DEBUG("%s: newavrcp is not enabled. Create SDP record", __func__);
    ...
    LOG_DEBUG("%s: nsc_mask: 0x%x", __func__, avdtp_stream_config.nsc_mask);
    ...
    LOG_DEBUG("%s: reg_audio: 0x%x", __func__, bta_av_cb.reg_audio);
  } while (0);
  ...
  if (btif_av_src_sink_coexist_enabled()) {
    ...
    if ((profile_initialized == UUID_SERVCLASS_AUDIO_SINK) &&
      ...
      if (!strncmp(AVRCP_1_3_STRING, avrcp_version,
        ...
        LOG_DEBUG("%s: AVRCP 1.3 capabilites used", __func__);
        p_bta_av_cfg = &bta_av_cfg_compatibility;
      }
    }
  }
...
static void bta_av_sys_rs_cback(UNUSED_ATTR tBTA_SYS_CONN_STATUS status,
  ...
  LOG_DEBUG("%s: peer %s new_role:%d hci_status:0x%x bta_av_cb.rs_idx:%d",
  ...
  if ((bta_av_cb.rs_idx != 0) && (bta_av_cb.rs_idx != peer_idx)) {
    ...
    if (p_scb && p_scb->q_tag == BTA_AV_Q_TAG_OPEN) {
      LOG_DEBUG("%s: peer %s rs_idx:%d, bta_handle:0x%x q_tag:%d", __func__,
      ...
      if (HCI_SUCCESS == hci_status || HCI_ERR_NO_CONNECTION == hci_status) {
        ...
        LOG_ERROR("%s: peer %s (p_scb peer %s) role switch failed: new_role:%d "
                  "hci_status:0x%x",
...
void bta_av_sm_execute(tBTA_AV_CB* p_cb, uint16_t event, tBTA_AV_DATA* p_data) {
  LOG_INFO("%s: AV event=0x%x(%s) state=%d(%s)", __func__, event,
