...
#include "profile/avrcp/avrc_sdp_service.h"
  ...
  do {
    ...
    if ((btif_av_src_sink_coexist_enabled() &&
      ...
      if (bta_av_cb.features & (BTA_AV_FEAT_RCTG)) {
        ...
        if (com::android::bluetooth::flags::avrcp_sdp_records()) {
          /*
           * For audio source skip registration of SDP records if new AVRCP
    ...
    if ((btif_av_src_sink_coexist_enabled() &&
      ...
      if (bta_av_cb.features & (BTA_AV_FEAT_RCCT)) {
        ...
        if (com::android::bluetooth::flags::avrcp_sdp_records()) {
          const std::shared_ptr<AvrcSdpService>& avrcp_sdp_service =
              AvrcSdpService::Get();
          // Add control record only for source and new avrcp service is not
          // enabled.
          if (profile_initialized == UUID_SERVCLASS_AUDIO_SINK ||
              (profile_initialized == UUID_SERVCLASS_AUDIO_SOURCE &&
               !is_new_avrcp_enabled())) {
