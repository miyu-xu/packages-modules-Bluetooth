...
static void bta_av_api_register(tBTA_AV_DATA* p_data) {
  ...
  do {
    if (bluetooth::common::init_flags::a2dp_src_sink_coexist_is_enabled()) {
      ...
      if (nullptr == p_scb) p_scb = bta_av_alloc_scb(registr.chnl);
      /** @} */
    } else {
    ...
    if ((bluetooth::common::init_flags::a2dp_src_sink_coexist_is_enabled() && !(bta_av_cb.reg_role & (1 << local_role))) ||
      (!bluetooth::common::init_flags::a2dp_src_sink_coexist_is_enabled() && bta_av_cb.reg_audio == 0)) {
    /** @} */
    ...
    /** src and sink coexist, we can be src or sink any time.
        * if don't register this role, register it @{ */
    if ((bluetooth::common::init_flags::a2dp_src_sink_coexist_is_enabled() && !(bta_av_cb.reg_role & (1 << local_role))) ||
      (!bluetooth::common::init_flags::a2dp_src_sink_coexist_is_enabled() && !bta_av_cb.reg_audio)) {
    /** @} */
