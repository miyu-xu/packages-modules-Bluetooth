...
void bta_av_str_stopped(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  if (p_data && p_data->api_stop.suspend) {
    log::verbose("peer {} suspending: {}, sup:{}, suspend_local_sent: {}",
                  p_scb->PeerAddress(), start, p_scb->suspend_sup, p_scb->suspend_local_sent);
...
void bta_av_suspend_cfm(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  log::verbose("peer {} bta_handle:0x{:x} audio_open_cnt:{} err_code:{} scb_started:{}", p_scb->PeerAddress(),
