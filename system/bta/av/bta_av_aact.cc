...
static uint8_t bta_av_get_scb_handle(tBTA_AV_SCB* p_scb, uint8_t local_sep) {
  ...
  LOG_DEBUG("%s: local sep_type %d not found", __func__, local_sep)
...
static uint8_t bta_av_get_scb_sep_type(tBTA_AV_SCB* p_scb,
  ...
  LOG_DEBUG("%s: avdt_handle %d not found", __func__, tavdt_handle)
...
static void bta_av_save_addr(tBTA_AV_SCB* p_scb, const RawAddress& bd_addr) {
  LOG_DEBUG("%s: peer=%s recfg_sup:%d, suspend_sup:%d", __func__,
...
void bta_av_st_rc_timer(tBTA_AV_SCB* p_scb, UNUSED_ATTR tBTA_AV_DATA* p_data) {
  LOG_DEBUG("%s: rc_handle:%d, use_rc: %d", __func__, p_scb->rc_handle,
            p_scb->use_rc);
...
void bta_av_proc_stream_evt(uint8_t handle, const RawAddress& bd_addr,
  ...
  if (p_scb) {
    ...
    LOG_INFO("%s: stream event bd_addr: %s scb_index: %u", __func__,
...
void bta_av_sink_data_cback(uint8_t handle, BT_HDR* p_pkt, uint32_t time_stamp,
  ...
  int index = 0;
  tBTA_AV_SCB* p_scb;
  LOG_DEBUG("%s: avdt_handle: %d pkt_len=0x%x  offset = 0x%x "
...
static void bta_av_a2dp_sdp_cback(bool found, tA2DP_Service* p_service,
  ...
  LOG_DEBUG("%s: peer %s : found=%s", __func__,
            ADDRESS_TO_LOGGABLE_CSTR(peer_address), (found) ? "true" : "false");
...
static void bta_av_adjust_seps_idx(tBTA_AV_SCB* p_scb, uint8_t avdt_handle) {
  LOG_DEBUG("%s: codec: %s", __func__, A2DP_CodecName(p_scb->cfg.codec_info));
  for (int i = 0; i < BTAV_A2DP_CODEC_INDEX_MAX; i++) {
    LOG_DEBUG("%s: avdt_handle: %d codec: %s", __func__,
...
void bta_av_switch_role(tBTA_AV_SCB* p_scb, UNUSED_ATTR tBTA_AV_DATA* p_data) {
  ...
  LOG_DEBUG("%s: peer %s wait:0x%x", __func__,
...
void bta_av_role_res(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  LOG_DEBUG("%s: peer %s q_tag:%d, wait:0x%x, role:0x%x", __func__,
  ...
  LOG_DEBUG("%s: peer %s wait:0x%x, role:0x%x", __func__,
            ADDRESS_TO_LOGGABLE_CSTR(p_scb->PeerAddress()), p_scb->wait,
            p_scb->role);
}
...
void bta_av_delay_co(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  LOG_DEBUG("%s: peer %s bta_handle:0x%x delay:%d", __func__,
...
void bta_av_do_disc_a2dp(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  LOG_DEBUG("%s: peer_addr: %s use_rc: %d switch_res:%d, oc:%d", __func__,
  ...
  LOG_DEBUG("%s: ok_continue: %d wait:0x%x, q_tag: %d", __func__, ok_continue,
  ...
  LOG_DEBUG("%s: Initiate SDP discovery for peer %s : uuid_int=0x%x "
            "sdp_uuid=0x%x",
            __func__, ADDRESS_TO_LOGGABLE_CSTR(p_scb->PeerAddress()),
  ...
  if (find_service_status != A2DP_SUCCESS) {
    LOG_ERROR("%s: A2DP_FindService() failed for peer %s uuid_int=0x%x "
...
void bta_av_config_ind(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  LOG_DEBUG("%s: peer %s bta_handle:0x%x local_sep:%d", __func__,
            ADDRESS_TO_LOGGABLE_CSTR(p_scb->PeerAddress()), p_scb->hndl,
            local_sep);
  ...
  if ((p_evt_cfg->num_codec == 0) ||
    ...
    LOG_DEBUG("%s: SEID: %d use_rc: %d cur_psc_mask:0x%x", __func__,
...
void bta_av_disconnect_req(tBTA_AV_SCB* p_scb,
  ...
  LOG_INFO("%s: conn_lcb: 0x%x peer_addr: %s", __func__, bta_av_cb.conn_lcb,
           ADDRESS_TO_LOGGABLE_CSTR(p_scb->PeerAddress()));
...
void bta_av_setconfig_rsp(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  if (p_data->ci_setconfig.err_code == AVDT_SUCCESS) {
    ...
    LOG_DEBUG("%s: recfg_needed:%d role:0x%x num:%d", __func__,
    ...
    for (i = 1; i < num; i++) {
      LOG_DEBUG("%s: sep_info[%d] SEID: %d", __func__, i, p_seid[i - 1]);
...
void bta_av_str_opened(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  LOG_DEBUG("%s: peer %s bta_handle: 0x%x", __func__,
            ADDRESS_TO_LOGGABLE_CSTR(p_scb->PeerAddress()), p_scb->hndl);
  ...
  LOG_DEBUG("%s: l2c_cid: 0x%x stream_mtu: %d", __func__, p_scb->l2c_cid,
            p_scb->stream_mtu);
...
void bta_av_do_close(tBTA_AV_SCB* p_scb, UNUSED_ATTR tBTA_AV_DATA* p_data) {
  LOG_DEBUG("%s: p_scb->co_started=%d", __func__, p_scb->co_started);
...
void bta_av_connect_req(tBTA_AV_SCB* p_scb, UNUSED_ATTR tBTA_AV_DATA* p_data) {
  LOG_DEBUG("%s: peer %s coll_mask=0x%02x", __func__,
  ...
  if (p_scb->coll_mask & BTA_AV_COLL_INC_TMR) {
    ...
    LOG_INFO("%s: updated coll_mask=0x%02x", __func__, p_scb->coll_mask);
    return;
  }
...
void bta_av_disc_results(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  LOG_DEBUG("%s: peer %s bta_handle: 0x%x initiator UUID 0x%x", __func__,
            ADDRESS_TO_LOGGABLE_CSTR(p_scb->PeerAddress()), p_scb->hndl,
            uuid_int);
  ...
  if (btif_av_src_sink_coexist_enabled()) {
    ...
    LOG_DEBUG("both_enable=%d, uuid_int=0x%x, incoming=%d",
    ...
    if (btif_av_both_enable() && p_scb->open_api.incoming) {
      if (uuid_int == UUID_SERVCLASS_AUDIO_SOURCE && num_snks == 0 &&
          num_srcs > 0) {
        p_scb->uuid_int = UUID_SERVCLASS_AUDIO_SINK;
        LOG_DEBUG(" change UUID to 0x%x, num_snks=%u, num_srcs=%u",
...
void bta_av_save_caps(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  LOG_DEBUG("%s: peer %s bta_handle:0x%x num_seps:%d sep_info_idx:%d wait:0x%x",
            __func__, ADDRESS_TO_LOGGABLE_CSTR(p_scb->PeerAddress()),
            p_scb->hndl, p_scb->num_seps, p_scb->sep_info_idx, p_scb->wait);
  LOG_DEBUG("%s: codec: %s", __func__,
  ...
  if (p_scb->p_cos->getcfg(p_scb->hndl, p_scb->PeerAddress(), cfg.codec_info,
    ...
    p_scb->sep_info_idx++;
    LOG_DEBUG("%s: result: next sep_info_idx:%d", __func__,
              p_scb->sep_info_idx);
  } else {
    // All capabilities found
    getcap_done = true;
    LOG_DEBUG("%s: result: done sep_info_idx:%d", __func__,
              p_scb->sep_info_idx);
  }
  LOG_DEBUG("%s: codec: %s", __func__,
  ...
  if (getcap_done) {
    LOG_DEBUG("%s: getcap_done: num_seps:%d sep_info_idx:%d wait:0x%x",
...
void bta_av_open_failed(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  if (is_av_opened) {
    ...
    LOG_ERROR("%s: there is already an active connection: peer_addr=%s chnl=%d "
              "hndl=0x%x status=%d starting=%d edr=%d",
              __func__, ADDRESS_TO_LOGGABLE_CSTR(bta_av_data.open.bd_addr),
              bta_av_data.open.chnl, bta_av_data.open.hndl,
...
void bta_av_getcap_results(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  LOG_DEBUG("%s: peer %s bta_handle:0x%x num_codec:%d psc_mask=0x%x", __func__,
            ADDRESS_TO_LOGGABLE_CSTR(p_scb->PeerAddress()), p_scb->hndl,
            p_scb->peer_cap.num_codec, p_scb->cfg.psc_mask);
  LOG_DEBUG("%s: media type 0x%x, 0x%x", __func__, media_type,
            p_scb->media_type);
  LOG_DEBUG("%s: codec: %s", __func__,
  ...
  if ((p_scb->peer_cap.num_codec != 0) && (media_type == p_scb->media_type) &&
    ...
    LOG_DEBUG("%s: result: sep_info_idx=%d", __func__, p_scb->sep_info_idx);
    LOG_DEBUG("%s: codec: %s", __func__,
              A2DP_CodecInfoString(p_scb->cfg.codec_info).c_str());

    LOG_DEBUG("%s: initiator UUID = 0x%x", __func__, uuid_int);
    if (uuid_int == UUID_SERVCLASS_AUDIO_SOURCE)
    ...
    LOG_DEBUG("%s: peer %s bta_handle:0x%x sep_idx:%d sep_info_idx:%d "
              "cur_psc_mask:0x%x",
              __func__, ADDRESS_TO_LOGGABLE_CSTR(p_scb->PeerAddress()),
              p_scb->hndl, p_scb->sep_idx, p_scb->sep_info_idx,
              p_scb->cur_psc_mask);

    if ((uuid_int == UUID_SERVCLASS_AUDIO_SINK) &&
        (p_scb->seps[p_scb->sep_idx].p_app_sink_data_cback != NULL)) {
      LOG_DEBUG("%s: configure decoder for Sink connection", __func__);
      tBTA_AV_MEDIA av_sink_codec_info = {
          .avk_config =
              {
...
void bta_av_str_stopped(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  if (p_data && p_data->api_stop.suspend) {
    LOG_DEBUG("%s: peer %s suspending: %d, sup:%d", __func__,
    ...
    suspend_rsp.status = BTA_AV_SUCCESS;
    suspend_rsp.initiator = true;
    LOG_INFO("%s: status %d", __func__, suspend_rsp.status);
...
void bta_av_reconfig(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  LOG_DEBUG("%s: r:%d, s:%d idx: %d (o:%d)", __func__, p_scb->recfg_sup,
  ...
  if ((p_scb->rcfg_idx == p_scb->sep_info_idx) && p_rcfg->suspend &&
    ...
    LOG_DEBUG("%s: Close/Open started: %d state: %d num_protect: %d", __func__,
...
void bta_av_start_ok(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  if ((p_scb->avdt_handle == p_scb->seps[p_scb->sep_idx].av_handle) &&
    ...
    p_scb->wait &= ~(BTA_AV_WAIT_ACP_CAPS_ON);
    LOG_DEBUG("%s: local SEP type is SNK new wait is 0x%x", __func__,
              p_scb->wait);
  }
  ...
  LOG_DEBUG("%s: peer %s wait:0x%x use_rtp_header_marker_bit:%s", __func__,
            ADDRESS_TO_LOGGABLE_CSTR(p_scb->PeerAddress()), p_scb->wait,
...

  {
    ...
    LOG_DEBUG("%s: peer %s suspending: %d, role:0x%x, init %d", __func__,
...
void bta_av_suspend_cfm(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  LOG_DEBUG("%s: peer %s bta_handle:0x%x audio_open_cnt:%d err_code:%d",
            __func__, ADDRESS_TO_LOGGABLE_CSTR(p_scb->PeerAddress()),
...
void bta_av_rcfg_str_ok(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  p_scb->l2c_cid = AVDT_GetL2CapChannel(p_scb->avdt_handle);
  LOG_DEBUG("%s: peer %s bta_handle:0x%x l2c_cid:%d", __func__,
            ADDRESS_TO_LOGGABLE_CSTR(p_scb->PeerAddress()), p_scb->hndl,
            p_scb->l2c_cid);
...
void bta_av_rcfg_connect(tBTA_AV_SCB* p_scb, UNUSED_ATTR tBTA_AV_DATA* p_data) {
  ...
  p_scb->cong = false;
  p_scb->num_recfg++;
  LOG_DEBUG("%s: num_recfg: %d", __func__, p_scb->num_recfg);
  if (p_scb->num_recfg > BTA_AV_RECONFIG_RETRY) {
...
void bta_av_suspend_cont(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  LOG_DEBUG("%s: err_code=%d", __func__, err_code);
  ...
  if (err_code) {
    ...
    LOG_DEBUG("%s: calling AVDT_ReconfigReq", __func__);
    /* reconfig the stream */
...
void bta_av_rcfg_open(tBTA_AV_SCB* p_scb, UNUSED_ATTR tBTA_AV_DATA* p_data) {
  LOG_DEBUG("%s: peer %s bta_handle:0x%x num_disc_snks:%d", __func__,
            ADDRESS_TO_LOGGABLE_CSTR(p_scb->PeerAddress()), p_scb->hndl,
            p_scb->num_disc_snks);
  ...
  if (p_scb->num_disc_snks == 0) {
    ...
    LOG_DEBUG("%s: calling AVDT_OpenReq()", __func__);
    LOG_DEBUG("%s: codec: %s", __func__,
              A2DP_CodecInfoString(p_scb->cfg.codec_info).c_str());
...
void bta_av_open_rc(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  LOG_DEBUG("%s: use_rc: %d, wait: 0x%x role: 0x%x", __func__, p_scb->use_rc,
            p_scb->wait, p_scb->role);
...
void offload_vendor_callback(tBTM_VSC_CMPL* param) {
  ...
  if (param->param_len) {
    LOG_DEBUG("%s: param_len = %d status = %d", __func__, param->param_len,
              param->p_param_buf[0]);
  ...
  if (value.status == 0) {
    ...
    switch (sub_opcode) {
      case VS_HCI_A2DP_OFFLOAD_STOP:
        LOG_DEBUG("%s: VS_HCI_STOP_A2DP_MEDIA successful", __func__);
        break;
      case VS_HCI_A2DP_OFFLOAD_START:
        if (bta_av_cb.offload_start_pending_hndl) {
          LOG_DEBUG("%s: VS_HCI_START_A2DP_MEDIA successful", __func__);
    ...
    LOG_DEBUG("%s: Offload failed for subopcode= %d", __func__, sub_opcode);
...
void bta_av_offload_req(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  tBT_A2DP_OFFLOAD offload_start;
  LOG_DEBUG("%s: stream %s, audio channels open %d", __func__,
  ...
  /* TODO(eisenbach): RE-IMPLEMENT USING VSC OR HAL EXTENSION
    ...
    if (L2CA_GetConnectionConfig(
      ...
      LOG_DEBUG("%s: l2cmtu %d lcid 0x%02X rcid 0x%02X lm_handle
  0x%02X",
...
void bta_av_offload_rsp(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  LOG_DEBUG("%s: stream %s status %s", __func__,
...
static void bta_av_offload_codec_builder(tBTA_AV_SCB* p_scb,
  ...
  LOG_DEBUG("%s:codec_index = %d", __func__, codec_index);
  switch (codec_index) {
  ...
  LOG_DEBUG("%s: SCMS-T_enable status: %d, "
...
void bta_av_api_set_peer_sep(tBTA_AV_DATA* p_data) {
  LOG_DEBUG("%s, bd_addr=%s, sep:%d", __func__,
