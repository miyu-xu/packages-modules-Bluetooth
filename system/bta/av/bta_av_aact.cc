...
void bta_av_reconfig(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  LOG_VERBOSE(" Debug_log_ Enter bta_av_reconfig");
  ...
  LOG_VERBOSE("p_scb->sep_info_idx=%d p_scb->rcfg_idx=%d p_rcfg->sep_info_idx=%d",
            p_scb->sep_info_idx, p_scb->rcfg_idx, p_rcfg->sep_info_idx);
  LOG_VERBOSE("Peer capable codec: %s",
            A2DP_CodecInfoString(p_scb->peer_cap.codec_info).c_str());
  LOG_VERBOSE("Current codec: %s",
            A2DP_CodecInfoString(p_scb->cfg.codec_info).c_str());
  LOG_VERBOSE("Reconfig codec: %s",
  ...
  LOG_VERBOSE(" Debug_log_ p_rcfg->suspend: %d", p_rcfg->suspend);
   LOG_VERBOSE(" Debug_log_ rcfg_idx:%d,sep_info_idx:%d,suspend:%d,recfg_sup:%d,suspend_sup:%d",
                     p_scb->rcfg_idx,
                     p_scb->sep_info_idx,
                     p_rcfg->suspend,
                     p_scb->recfg_sup,
  ...
  if ((p_scb->rcfg_idx == p_scb->sep_info_idx) && p_rcfg->suspend &&
    ...
    LOG_VERBOSE(" Debug_log_ bta_av_reconfig: 1st if condition");
    if (p_scb->started) {
      LOG_VERBOSE(" Debug_log_ bta_av_reconfig: 1st if condition inside 1st if condition");
      ...
      LOG_VERBOSE(" Debug_log_ bta_av_reconfig: else condition inside 1st if condition");
      // Reconfigure
    ...
    LOG_VERBOSE(" Debug_log_ bta_av_reconfig: 1st else condition");
    ...
    if (p_scb->started) {
      LOG_VERBOSE(" Debug_log_ bta_av_reconfig: p_scb->started is true");
      ...
      LOG_VERBOSE(" Debug_log_ bta_av_reconfig: p_scb->started is false");
