...
void bta_av_setconfig_rej(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  tBTA_AV bta_av_data = {
      .reject =
          {
              .bd_addr = p_data->str_msg.bd_addr, .hndl = p_scb->hndl,
          },
  };
...
void bta_av_start_ok(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
...

  {
    ...
    if (suspend) {
      ...
      p_scb->co_started = false;
      tBTA_AV_API_STOP stop = {
          .hdr = {}, .suspend = true, .flush = false, .reconfig_stop = false,
      };
    ...
    switch (sub_opcode) {
      ...
      case VS_HCI_A2DP_OFFLOAD_START:
        if (bta_av_cb.offload_start_pending_hndl) {
          ...
          LOG_INFO(
              "%s: No pending start command due to AVDTP suspend immediately",
              __func__);
        }
...
void bta_av_api_set_peer_sep(tBTA_AV_DATA* p_data) {
  ...
  APPL_TRACE_DEBUG("%s, bd_addr=%s, sep:%d", __func__,
                   p_data->peer_sep.addr.ToString().c_str(),
                   p_data->peer_sep.sep);
  p_scb = bta_av_addr_to_scb(p_data->peer_sep.addr);
  if (p_scb) {
