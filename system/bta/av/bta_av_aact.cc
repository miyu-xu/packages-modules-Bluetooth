...
void bta_av_do_close(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* /* p_data */) {
  ...
  if (!L2CA_FlushChannel(p_scb->l2c_cid, L2CAP_FLUSH_CHANS_ALL)) {
    log::warn("Unable to flush L2CAP channel peer:{} cid:{}",
              p_scb->PeerAddress(), p_scb->l2c_cid);
  }
...
void bta_av_str_stopped(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  if (BTA_AV_CHNL_AUDIO == p_scb->chnl) {
    ...
    if (p_data && p_data->api_stop.flush) {
      if (!L2CA_FlushChannel(p_scb->l2c_cid, L2CAP_FLUSH_CHANS_ALL)) {
        log::warn("Unable to flush L2CAP ALL channels peer:{} cid:{}",
                  p_scb->PeerAddress(), p_scb->l2c_cid);
      }
    }
  }
...
void bta_av_suspend_cont(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  if (err_code) {
    if (AVDT_ERR_CONNECT == err_code) {
      ...
      /* drop the buffers queued in L2CAP */
      const uint16_t queued_buffers =
          L2CA_FlushChannel(p_scb->l2c_cid, L2CAP_FLUSH_CHANS_ALL);
      log::error("suspend rejected, closing queued buffers:{} peer:{} cid:{}",
                 p_scb->PeerAddress(), p_scb->l2c_cid, queued_buffers);
      AVDT_CloseReq(p_scb->avdt_handle);
    }
  } else {
...
void bta_av_rcfg_cfm(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  if ((err_code != 0) || disable_avdtp_reconfigure) {
    ...
    const uint16_t queued_buffers =
        L2CA_FlushChannel(p_scb->l2c_cid, L2CAP_FLUSH_CHANS_ALL);
    log::error("reconfig rejected, closing queued buffers:{} peer:{} cid:{}",
               p_scb->PeerAddress(), p_scb->l2c_cid, queued_buffers);
    AVDT_CloseReq(p_scb->avdt_handle);
  } else {
