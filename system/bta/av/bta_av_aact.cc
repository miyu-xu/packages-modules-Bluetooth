...
/* Time to wait for open from SNK when signalling is initiated from SNK. */
/* If not, we will open from SRC. */
...
void bta_av_open_at_inc(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  if (p_scb->coll_mask & BTA_AV_COLL_INC_TMR) {
    ...
    /* SNK did not start signalling or it was incomplete, API was called N seconds timeout. */
 ...
 *                  and SNK did not send open command.
 *
 * Returns          void
 *
 ******************************************************************************/
static void bta_av_accept_open_timer_cback(void* data) {
  ...
  if (p_scb == nullptr) {
    log::error("SCB not found for index {}", inx);
    return;
  }
  AVDT_AbortReq(p_scb->avdt_handle);
