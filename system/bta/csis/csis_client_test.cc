...
TEST_F(CsisClientTest, test_storage_calls) {
  ...
  ASSERT_EQ(0,
            increment_mock_function_call_count_get()["btif_storage_load_bonded_csis_devices"]);
  TestAppRegister();
  ASSERT_EQ(1,
            increment_mock_function_call_count_get()["btif_storage_load_bonded_csis_devices"]);

  ASSERT_EQ(0, increment_mock_function_call_count_get()["btif_storage_update_csis_info"]);
  ASSERT_EQ(0, increment_mock_function_call_count_get()["btif_storage_set_csis_autoconnect"]);
  TestConnect(test_address);
  InjectConnectedEvent(test_address, 1);
  GetSearchCompleteEvent(1);
  ASSERT_EQ(1, increment_mock_function_call_count_get()["btif_storage_set_csis_autoconnect"]);
  ASSERT_EQ(1, increment_mock_function_call_count_get()["btif_storage_update_csis_info"]);

  ASSERT_EQ(0, increment_mock_function_call_count_get()["btif_storage_remove_csis_device"]);
  CsisClient::Get()->RemoveDevice(test_address);
  ASSERT_EQ(1, increment_mock_function_call_count_get()["btif_storage_remove_csis_device"]);
