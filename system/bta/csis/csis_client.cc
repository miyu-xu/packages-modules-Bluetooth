...
class CsisClientImpl : public CsisClient {
  static constexpr uint8_t CSIS_STORAGE_LAYOUT_MAGIC_10 = 0x10;
  static constexpr size_t CSIS_STORAGE_ENTRY_SZ_10 =
 ...
 public:
  ...
  std::map<uint8_t, uint8_t> DeserializeSets(const RawAddress& addr,
    ...
    if (magic == CSIS_STORAGE_LAYOUT_MAGIC_10) {
      uint8_t num_sets;
      STREAM_TO_UINT8(num_sets, ptr);

      if (in.size() <
          CSIS_STORAGE_HEADER_SZ + (num_sets * CSIS_STORAGE_ENTRY_SZ_10)) {
    ...
    } else if (magic == CSIS_STORAGE_CURRENT_LAYOUT_MAGIC) {
      uint8_t num_sets;
      STREAM_TO_UINT8(num_sets, ptr);
