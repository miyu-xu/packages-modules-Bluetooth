...
class CsisClientImpl : public CsisClient {
 ...
 private:
  ...
  void CheckForGroupInInqDb(std::shared_ptr<CsisGroup> csis_group) {
    ...
    for (tBTM_INQ_INFO* inq_ent = BTM_InqDbFirst(); inq_ent != nullptr;
      ...
      if (csis_group->GetDesiredSize() > 0 &&
          (csis_group->GetCurrentSize() == csis_group->GetDesiredSize())) {
        LOG_WARN("Group is already completed. Some other device use same SIRK");
        break;
      }
