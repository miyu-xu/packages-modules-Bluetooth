 ...
 private:
  ...
  bool sdf(const RawAddress& address, const Octet16& encrypted_sirk,
    ...
    Octet16 s1 = crypto_toolbox::aes_cmac(zero_key, (uint8_t*)(msg1.c_str()),
                                          msg1.size());
    LOG_VERBOSE("s1 (le) %s", base::HexEncode(s1.data(), 16).c_str());
    ...
    LOG_VERBOSE("T (le) %s", base::HexEncode(T.data(), 16).c_str());
    ...
    LOG_VERBOSE("K1 (le) %s", base::HexEncode(k1.data(), 16).c_str());

    for (int i = 0; i < 16; i++) sirk[i] = encrypted_sirk[i] ^ k1[i];

    LOG_VERBOSE("SIRK (le)%s", base::HexEncode(sirk.data(), 16).c_str());
    return true;
  }
