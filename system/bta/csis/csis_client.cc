...
private:
  ...
  const gatt::Service* FindCommonAudioService(const std::list<gatt::Service>* services) {
    ...
    if (cas) {
        log::debug("UUID: {} handle {}", cas->uuid, cas->handle);
    } else {
        log::debug("not found");
    }
    ...
    if (!device->is_gatt_service_valid) {
      ...
      for (auto& svrc : *all_services) {
        ...
        if (context_uuid == kCsisServiceUuid ||
            (context_uuid != kCasServiceUuid && context_uuid != kCasExServiceUuid)) {
          continue;
        }
      ...
      for (auto& svrc : *all_services) {
        ...
        if (context_uuid == kCasServiceUuid || context_uuid == kCasExServiceUuid) {
          ...
          log::debug("context_uuid {} -> {}", context_uuid, kCasServiceUuid);
