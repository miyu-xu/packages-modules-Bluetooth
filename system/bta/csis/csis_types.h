...
class CsisInstance {
 public:
  ...
  void SetLockState(CsisLockState state) {
    bluetooth::log::debug("current lock state: {}, new lock state: {}",
  ...
  void SetRank(uint8_t rank) {
    bluetooth::log::debug("current rank: {}, new rank: {}",
                          static_cast<int>(rank_), static_cast<int>(rank));
    rank_ = rank;
  }

  void SetGroupId(int group_id) {
    bluetooth::log::info("set group id: {}, instance handle: 0x{:04x}",
                         group_id, svc_data.start_handle);
    group_id_ = group_id;
  }
 ...
 public:
  ...
  std::shared_ptr<CsisInstance> GetCsisInstanceByOwningHandle(uint16_t handle) {
    ...
    for (const auto& [h, inst] : csis_instances_) {
      if (handle >= inst->svc_data.start_handle &&
        ...
        hdl = h;
        bluetooth::log::verbose("found 0x{:04x}", hdl);
        break;
      }
    }
  ...
  void SetCsisInstance(uint16_t handle,
    ...
    if (csis_instances_.count(handle)) {
      bluetooth::log::debug("instance is already here: {}",
                            csis_instance->GetUuid().ToString());
      return;
    }

    csis_instances_.insert({handle, csis_instance});
    bluetooth::log::debug("instance added: 0x{:04x}, device {}", handle,
  ...
  void SetExpectedGroupIdMember(int group_id) {
    bluetooth::log::info("Expected Group ID: {}, for member: {} is set",
                         group_id, ADDRESS_TO_LOGGABLE_CSTR(addr));
    expected_group_id_member_ = group_id;
  }

  void SetPairingSirkReadFlag(bool flag) {
    bluetooth::log::info(
        "Pairing flag for Group ID: {}, member: {} is set to {}",
...
class CsisGroup {
 public:
  ...
  void SetSirk(Octet16& sirk) {
    if (sirk_available_) {
      bluetooth::log::debug("Updating SIRK");
    }
  ...
  void SetDiscoveryState(CsisDiscoveryState state) {
    bluetooth::log::debug(
        "current discovery state: {}, new discovery state: {}",
  ...
  bool IsAvailableForCsisLockOperation(void) {
    ...
    auto iter = std::find_if(
        devices_.begin(), devices_.end(), [id, &number_of_connected](auto& d) {
          if (!d->IsConnected()) {
            bluetooth::log::debug("Device {} is not connected in group {}",
                                  ADDRESS_TO_LOGGABLE_CSTR(d->addr), id);
            return false;
          }
          auto inst = d->GetCsisInstanceByGroupId(id);
          if (!inst) {
            bluetooth::log::debug("Instance not available for group {}", id);
            return false;
          }
          number_of_connected++;
          bluetooth::log::debug("Device {},  lock state: {}",
                                ADDRESS_TO_LOGGABLE_CSTR(d->addr),
                                (int)inst->GetLockState());
          return inst->GetLockState() == CsisLockState::CSIS_STATE_LOCKED;
        });

    bluetooth::log::debug("Locked set: {}, number of connected {}",
                          iter != devices_.end(), number_of_connected);
  ...
  void SortByCsisRank(void) {
    ...
    std::sort(devices_.begin(), devices_.end(), [id](auto& dev1, auto& dev2) {
      ...
      if (!inst1 || !inst2) {
        ...
        bluetooth::log::debug("Device  {} is not connected.",
                              inst1 == nullptr
