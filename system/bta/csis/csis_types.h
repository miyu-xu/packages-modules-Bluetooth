...
class CsisGroup {
 public:
  ...
  static bool is_rsi_match_sirk(const RawAddress& rsi, const Octet16& sirk) {
    ...
    LOG_VERBOSE("Prand %s", base::HexEncode(rand.data(), 3).c_str());

    LOG_VERBOSE("SIRK %s", base::HexEncode(sirk.data(), 16).c_str());
    /* generate X = E irk(R0, R1, R2) and R is random address 3 LSO */
    Octet16 x = crypto_toolbox::aes_128(sirk, rand);

    LOG_VERBOSE("X %s", base::HexEncode(x.data(), 16).c_str());

    rand[0] = rsi.address[5];
    rand[1] = rsi.address[4];
    rand[2] = rsi.address[3];

    LOG_VERBOSE("Hash %s", base::HexEncode(rand.data(), 3).c_str());
