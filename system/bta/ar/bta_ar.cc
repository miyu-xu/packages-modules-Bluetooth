...
void bta_ar_reg_avrc(uint16_t service_uuid, const char* service_name,
  ...
  if (service_uuid == UUID_SERVCLASS_AV_REM_CTRL_TARGET) {
    ...
    /** src and sink coexist, we can be src or sink any time. @{ */
    else if (bluetooth::common::init_flags::a2dp_src_sink_coexist_is_enabled()) {
      p = temp;
    ...
    if (bta_ar_cb.sdp_ct_handle == 0) {
      ...
      if (bluetooth::common::init_flags::a2dp_src_sink_coexist_is_enabled() && bta_ar_cb.ct_ver < profile_version) {
