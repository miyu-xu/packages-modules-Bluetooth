...
static void bta_dm_sdp_callback(tBTA_DM_SDP_STATE sdp_state,
...
static void bta_dm_free_sdp_db(tBTA_DM_SDP_STATE &sdp_state);
...
static void bta_dm_sdp_callback(tBTA_DM_SDP_STATE sdp_state,
  ...
  if (bta_dm_discovery_get_state() == BTA_DM_DISCOVER_IDLE) {
    bta_dm_free_sdp_db(sdp_state);
    return;
  }

  do_in_main_thread(FROM_HERE,
                    base::BindOnce(&bta_dm_sdp_result, sdp_status, sdp_state));
}
...
static void bta_dm_sdp_result(tSDP_STATUS sdp_result,
  ...
  if ((sdp_result == SDP_SUCCESS) || (sdp_result == SDP_NO_RECS_MATCH) ||
    ...
    do {
      ...
      if (sdp_state.service_index == (BTA_USER_SERVICE_ID + 1)) {
        ...
        p_sdp_rec = get_legacy_stack_sdp_api()->db.SDP_FindServiceInDb(
            sdp_state.p_sdp_db, service, p_sdp_rec);
      }
      ...
      if (sdp_state.service_index == BTA_MAX_SERVICE_ID) {
        ...
        do {
          ...
          p_sdp_rec = get_legacy_stack_sdp_api()->db.SDP_FindServiceInDb(
              sdp_state.p_sdp_db, 0, p_sdp_rec);
          if (p_sdp_rec) {
            Uuid service_uuid;
    ...
    if (bluetooth::common::init_flags::
      ...
      bta_dm_store_audio_profiles_version(sdp_state.p_sdp_db);
    }

#if TARGET_FLOSS
    tSDP_DI_GET_RECORD di_record;
    if (get_legacy_stack_sdp_api()->device_id.SDP_GetDiRecord(
            1, &di_record, sdp_state.p_sdp_db) == SDP_SUCCESS) {
    ...
    if (sdp_state.p_sdp_db != NULL &&
        sdp_state.p_sdp_db->raw_used != 0 &&
        sdp_state.p_sdp_db->raw_data != NULL) {
      log::verbose("raw_data used = 0x{:x} raw_data_ptr = 0x{}",
                    sdp_state.p_sdp_db->raw_used,
                    fmt::ptr(sdp_state.p_sdp_db->raw_data));

      sdp_state.p_sdp_db->raw_data =
          NULL;  // no need to free this - it is a global assigned.
      sdp_state.p_sdp_db->raw_used = 0;
      sdp_state.p_sdp_db->raw_size = 0;
    } else {
    ...
    if (sdp_state.p_sdp_db)
      osi_free_and_reset((void**)&sdp_state.p_sdp_db);
...
static void bta_dm_find_services(const RawAddress& bd_addr,
  ...
  get_legacy_stack_sdp_api()->service.SDP_InitDiscoveryDb(
      sdp_state.p_sdp_db, BTA_DM_SDP_DB_SIZE, 1, &uuid, 0, NULL);

  memset(sdp_state.g_disc_raw_data_buf, 0, sizeof(sdp_state.g_disc_raw_data_buf));
  sdp_state.p_sdp_db->raw_data = sdp_state.g_disc_raw_data_buf;

  sdp_state.p_sdp_db->raw_size = MAX_DISC_RAW_DATA_BUF;

  if (!get_legacy_stack_sdp_api()->service.SDP_ServiceSearchAttributeRequest2(
          bd_addr, sdp_state.p_sdp_db,
          base::BindRepeating(&bta_dm_sdp_callback, sdp_state))) {
    /*
...
static void bta_dm_discover_services(tBTA_DM_API_DISCOVER& discover) {
  ...
  bta_dm_find_services(bd_addr, tBTA_DM_SDP_STATE{
