...
static void bta_dm_sdp_result(tBTA_DM_SDP_RESULT& sdp_event) {
  ...
  p_sdp_rec = NULL;
  /* find a service record, report it */
  while ((p_sdp_rec = get_legacy_stack_sdp_api()->db.SDP_FindServiceInDb_128bit(
        bta_dm_discovery_cb.p_sdp_db, p_sdp_rec)) != nullptr) {
...
static void bta_dm_find_services(const RawAddress& bd_addr) {
  ...
  if (!get_legacy_stack_sdp_api()->service.SDP_ServiceSearchAttributeRequest(
    ...
    /* If discovery is not successful with this device, then proceed with the
     * next one. */
    osi_free_and_reset((void**)&bta_dm_discovery_cb.p_sdp_db);
...
static void bta_dm_discover_services(tBTA_DM_API_DISCOVER& discover) {
  ...
  if (HID_HostSDPDisable(bd_addr)) {
    log::debug("peer:{} with HIDSDPDisable attribute.", bd_addr);
  ...
  if (transport == BT_TRANSPORT_LE) {
    ...
    log::info("bta_dm_discovery: starting SDP discovery on {}", bd_addr);
