...
static void bta_dm_discover_services(tBTA_DM_API_DISCOVER& discover) {
  ...
  if (transport == BT_TRANSPORT_LE) {
    log::info("starting GATT discovery on {}", bda_addr);
    // set the raw data buffer here
    memset(g_disc_raw_data_buf, 0, sizeof(g_disc_raw_data_buf));
    /* start GATT for service discovery */
    btm_dm_start_gatt_discovery(bda_addr);
    return;
  } else {
    log::info("bta_dm_discovery: starting SDP discovery on {}", bda_addr);
    bta_dm_discovery_cb.sdp_results = false;
    bta_dm_find_services(bda_addr);
    return;
  }
