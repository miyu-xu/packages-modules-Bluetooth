...
std::unique_ptr<BtaDmSdpPerformer> get_sdp_performer(RawAddress bd_addr) {
  return std::make_unique<BtaDmSdpPerformer>(bd_addr);
}

// Factory method that can be overridden for testing
auto sdp_performer_factory = get_sdp_performer;

}  // namespace

void bta_dm_disc_override_sdp_performer_for_testing(
    std::unique_ptr<BtaDmSdpPerformer> (*factory_method)(RawAddress)) {
  sdp_performer_factory = factory_method;
}
...
static void bta_dm_discover_services(tBTA_DM_API_DISCOVER& discover) {
  ...
  bta_dm_discovery_cb.sdp_performer = sdp_performer_factory(bd_addr);
