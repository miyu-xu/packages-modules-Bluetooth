...
static void bta_dm_search_clear_queue() {
  ...
  if (bluetooth::common::InitFlags::
    ...
    bta_dm_search_cb.pending_discovery_queue =
        std::queue<tBTA_DM_API_DISCOVER>();
  }
}
...
static void bta_dm_gatt_disc_complete(uint16_t conn_id, tGATT_STATUS status) {
  ...
  /* no more services to be discovered */
  disc_result.result = (status == GATT_SUCCESS) ? BTA_SUCCESS : BTA_FAILURE;
  log::verbose("service found: 0x{:08x}", bta_dm_search_cb.services_found);
  disc_result.services = bta_dm_search_cb.services_found;
  disc_result.bd_addr = bta_dm_search_cb.peer_bdaddr;
  disc_result.device_type |= BT_DEVICE_TYPE_BLE;
    ...
    case BTA_DM_SEARCH_IDLE:
      switch (event) {
        case BTA_DM_API_SEARCH_EVT:
          ...
          if (!std::holds_alternative<tBTA_DM_API_SEARCH>(*msg))
        ...
        case BTA_DM_API_DISCOVER_EVT:
          ...
          if (!std::holds_alternative<tBTA_DM_API_DISCOVER>(*msg))
            log::fatal("bad message type!");
    ...
    case BTA_DM_SEARCH_ACTIVE:
      switch (event) {
        case BTA_DM_REMT_NAME_EVT:
          if (!std::holds_alternative<tBTA_DM_REMOTE_NAME>(*msg))
            log::fatal("bad message type!");
        ...
        case BTA_DM_DISCOVERY_RESULT_EVT:
          if (!std::holds_alternative<tBTA_DM_SVC_RES>(*msg))
            log::fatal("bad message type!");
    ...
    case BTA_DM_DISCOVER_ACTIVE:
      switch (event) {
        case BTA_DM_REMT_NAME_EVT:
          ...
          break;
        case BTA_DM_SDP_RESULT_EVT:
          if (!std::holds_alternative<tBTA_DM_SDP_RESULT>(*msg))
            log::fatal("bad message type!");
...
static void bta_dm_disc_reset() {
  ...
  bta_dm_search_cb.p_pending_search.reset();
  bta_dm_search_cb.pending_discovery_queue = std::queue<tBTA_DM_API_DISCOVER>();
...
void bta_dm_search_result(tBTA_DM_SVC_RES& disc_result) {
