...
static void bta_dm_disc_result(tBTA_DM_SVC_RES& disc_result) {
  ...
  if (!disc_result.is_gatt_over_ble) {
    bta_dm_discovery_cb.transport &= ~BT_TRANSPORT_BR_EDR;
    ...
    bta_dm_discovery_cb.transport &= ~BT_TRANSPORT_LE;
  ...
  if (!bta_dm_discovery_cb.transport) {
...
static void bta_dm_discover_services(tBTA_DM_API_DISCOVER& discover) {
  ...
  if (transport == BT_TRANSPORT_LE) {
    if (bta_dm_discovery_cb.transport & BT_TRANSPORT_LE) {
  ...
  if (bta_dm_discovery_cb.transport & BT_TRANSPORT_BR_EDR) {
 ...
 ******************************************************************************/
static void bta_dm_gatt_disc_complete(uint16_t conn_id, tGATT_STATUS status) {
  bool sdp_pending = bta_dm_discovery_cb.transport & BT_TRANSPORT_BR_EDR;
  bool le_pending = bta_dm_discovery_cb.transport & BT_TRANSPORT_LE;

  log::verbose("conn_id = {}, status = {}, sdp_pending = {}, le_pending = {}", conn_id, status, sdp_pending, le_pending);

  if (com::android::bluetooth::flags::bta_dm_discover_both() && sdp_pending && !le_pending) {
    /* LE Service discovery finished, and services were reported, but SDP is not finished yet. gatt_close_timer closed the connection, and we received this callback because of disconnnection */
    return;
  }
