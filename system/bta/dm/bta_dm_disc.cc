...
static void bta_dm_sdp_result(tSDP_STATUS sdp_result,
  ...
  if ((sdp_result == SDP_SUCCESS) || (sdp_result == SDP_NO_RECS_MATCH) ||
    ...
    if (sdp_state->p_sdp_db != NULL && sdp_state->p_sdp_db->raw_used != 0 &&
...
static void bta_dm_find_services(const RawAddress& bd_addr,
  ...
  sdp_state->p_sdp_db = (tSDP_DISCOVERY_DB*)sdp_state->sdp_db_buffer;
  ...
  memset(sdp_state->g_disc_raw_data_buf, 0,
         sizeof(sdp_state->g_disc_raw_data_buf));
