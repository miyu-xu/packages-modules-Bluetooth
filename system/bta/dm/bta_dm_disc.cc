...
static void bta_dm_search_start(tBTA_DM_MSG* p_data) {
  ...
  LOG_DEBUG("%s avoid_scatter=%d", __func__, p_bta_dm_cfg->avoid_scatter);
...
static bool bta_dm_read_remote_device_name(const RawAddress& bd_addr,
  ...
  if (btm_status == BTM_CMD_STARTED) {
    LOG_DEBUG("%s: BTM_ReadRemoteDeviceName is started", __func__);

    return (true);
  } else if (btm_status == BTM_BUSY) {
    LOG_DEBUG("%s: BTM_ReadRemoteDeviceName is busy", __func__);
...
static void bta_dm_sdp_result(tBTA_DM_MSG* p_data) {
  ...
  if ((p_data->sdp_event.sdp_result == SDP_SUCCESS) ||
    ...
    LOG_DEBUG("sdp_result::0x%x", p_data->sdp_event.sdp_result);
    do {
      p_sdp_rec = NULL;
    ...
    LOG_DEBUG("%s services_found = %04x", __func__,
              bta_dm_search_cb.services_found);
    ...
    if (bta_dm_search_cb.services_to_search) {
      ...
      if (bta_dm_search_cb.p_sdp_db != NULL &&
        ...
        LOG_DEBUG("%s raw_data used = 0x%x raw_data_ptr = 0x%p", __func__,
        ...
        LOG_DEBUG("%s raw data size is 0 or raw_data is null!!", __func__);
      }
      ...
      if (scn_found) {
        ...
        LOG_INFO(" Piggy back the SCN over result field  SCN=%d",
...
static void bta_dm_search_result(tBTA_DM_MSG* p_data) {
  LOG_DEBUG("%s searching:0x%04x, result:0x%04x", __func__,
...
static void bta_dm_find_services(const RawAddress& bd_addr) {
  while (bta_dm_search_cb.service_index < BTA_MAX_SERVICE_ID) {
    ...
    if (bta_dm_search_cb.services_to_search &
      ...
      LOG_DEBUG("bta_dm_search_cb.services = %04x***********",
                bta_dm_search_cb.services);
 ...
 ******************************************************************************/
static void bta_dm_discover_next_device(void) {
  LOG_DEBUG("bta_dm_discover_next_device");
...
static void bta_dm_discover_device(const RawAddress& remote_bd_addr) {
  ...
  if (bta_dm_search_cb.p_btm_inq_info) {
    LOG_DEBUG("%s appl_knows_rem_name %d", __func__,
  ...
  if (bta_dm_search_cb.services) {
    ...
    if (bta_dm_search_cb.services_to_search) {
      ...
      if (bta_dm_search_cb.p_btm_inq_info) {
        LOG_DEBUG("%s p_btm_inq_info 0x%p results.device_type 0x%x "
...
static void bta_dm_service_search_remname_cback(const RawAddress& bd_addr,
  ...
  LOG_DEBUG("%s name=<%s>", __func__, bd_name);
...
static void bta_dm_observe_results_cb(tBTM_INQ_RESULTS* p_inq,
  ...
  tBTA_DM_SEARCH result;
  tBTM_INQ_INFO* p_inq_info;
  LOG_DEBUG("bta_dm_observe_results_cb");
...
static void bta_dm_observe_cmpl_cb(void* p_result) {
  ...
  LOG_DEBUG("bta_dm_observe_cmpl_cb");
 ...
 ******************************************************************************/
static void bta_dm_gatt_disc_complete(uint16_t conn_id, tGATT_STATUS status) {
  LOG_DEBUG("%s conn_id = %d", __func__, conn_id);
  ...
  LOG_DEBUG("%s service found: 0x%08x", __func__,
            bta_dm_search_cb.services_found);
  p_msg->disc_result.result.disc_res.services = bta_dm_search_cb.services_found;
 ...
 ******************************************************************************/
static void bta_dm_gattc_callback(tBTA_GATTC_EVT event, tBTA_GATTC* p_data) {
  LOG_DEBUG("bta_dm_gattc_callback event = %d", event);
