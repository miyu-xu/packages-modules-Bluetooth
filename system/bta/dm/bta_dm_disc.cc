...
static void bta_dm_search_sm_execute_safe(tBTA_DM_EVT event, std::unique_ptr<DM_MSG_SAFE> msg);
static void bta_dm_search_sendmsg(tBTA_DM_EVT event, std::unique_ptr<DM_MSG_SAFE> msg) {
    if (do_in_main_thread(
          FROM_HERE,
          base::BindOnce(&bta_dm_search_sm_execute_safe, event, std::move(msg))) !=
      BT_STATUS_SUCCESS) {
...
static void bta_dm_disc_result(tBTA_DM_DISC_RESULT &disc_result) {
  ...
  bool is_gatt_over_ble = ((disc_result.result.disc_res.device_type &
...
static void bta_dm_queue_search(tBTA_DM_API_SEARCH &search) {
  ...
  memcpy(bta_dm_search_cb.p_pending_search, &search, sizeof(tBTA_DM_API_SEARCH));
...
static void bta_dm_find_services(const RawAddress& bd_addr) {
  ...
  if (bta_dm_search_cb.service_index >= BTA_MAX_SERVICE_ID) {
    std::unique_ptr<DM_MSG_SAFE> msg = std::make_unique<DM_MSG_SAFE>(tBTA_DM_DISC_RESULT{});
...
static void bta_dm_sdp_callback(UNUSED_ATTR const RawAddress& bd_addr,
                                tSDP_STATUS sdp_status) {
    bta_dm_search_sendmsg(BTA_DM_SDP_RESULT_EVT, std::make_unique<DM_MSG_SAFE>(tBTA_DM_SDP_RESULT{.sdp_result= sdp_status}));
}
...
static void bta_dm_remname_cback(const tBTM_REMOTE_DEV_NAME* p_remote_name) {
  ...
  if (bta_dm_search_cb.transport == BT_TRANSPORT_LE) {
    ...
    std::unique_ptr<DM_MSG_SAFE> msg = std::make_unique<DM_MSG_SAFE>(tBTA_DM_REMOTE_NAME{});
...
void bta_dm_sdp_result(tBTA_DM_SDP_RESULT& sdp_event) { ::bta_dm_sdp_result(sdp_event); }
  ...
  switch (bta_dm_search_get_state()) {
      ...
      switch (event) {
        case BTA_DM_API_SEARCH_EVT:
          ...
          std::visit(overloaded{
            [](tBTA_DM_API_SEARCH msg) { bta_dm_search_start(msg); },
        ...
        case BTA_DM_API_DISCOVER_EVT:
          ...
          std::visit(overloaded{
            [](tBTA_DM_API_DISCOVER msg) { bta_dm_discover(msg); },
            [](auto) { log::fatal("bad message type!"); }
          }, *msg.get());
          break;
    ...
    case BTA_DM_SEARCH_ACTIVE:
      switch (event) {
        case BTA_DM_REMT_NAME_EVT:
          std::visit(overloaded{
            [](tBTA_DM_REMOTE_NAME msg) { bta_dm_remote_name_cmpl(msg); },
            [](auto) { log::fatal("bad message type!"); }
          }, *msg.get());
          break;
        ...
        case BTA_DM_DISCOVERY_RESULT_EVT:
          std::visit(overloaded{
            [](tBTA_DM_DISC_RESULT msg) { bta_dm_search_result(msg); },
            [](auto) { log::fatal("bad message type!"); }
          }, *msg.get());
          break;
        case BTA_DM_DISC_CLOSE_TOUT_EVT:
        ...
        case BTA_DM_API_DISCOVER_EVT:
          std::visit(overloaded{
            [](tBTA_DM_API_DISCOVER &msg) { bta_dm_queue_disc(msg); },
            [](auto) { log::fatal("bad message type!"); }
          }, *msg.get());
    ...
    case BTA_DM_SEARCH_CANCELLING:
      switch (event) {
        case BTA_DM_API_SEARCH_EVT:
          std::visit(overloaded{
            [](tBTA_DM_API_SEARCH &msg) { bta_dm_queue_search(msg); },
            [](auto) { log::fatal("bad message type!"); }
          }, *msg.get());
          break;
        case BTA_DM_API_DISCOVER_EVT:
    ...
    case BTA_DM_DISCOVER_ACTIVE:
      switch (event) {
        ...
        case BTA_DM_SDP_RESULT_EVT:
          std::visit(overloaded{
            [](tBTA_DM_SDP_RESULT &msg) { bta_dm_sdp_result(msg); },
            [](auto) { log::fatal("bad message type!"); }
          }, *msg.get());
          break;
        case BTA_DM_SEARCH_CMPL_EVT:
        ...
        case BTA_DM_DISCOVERY_RESULT_EVT:
          std::visit(overloaded{
            [](tBTA_DM_DISC_RESULT &msg) { bta_dm_disc_result(msg); },
            [](auto) { log::fatal("bad message type!"); }
          }, *msg.get());
          break;
        case BTA_DM_API_SEARCH_EVT:
