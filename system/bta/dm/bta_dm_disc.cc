...
static void bta_dm_search_sm_execute_safe(tBTA_DM_EVT event,
                                          std::unique_ptr<DM_MSG_SAFE> msg);
static void bta_dm_search_sendmsg(tBTA_DM_EVT event,
                                  std::unique_ptr<DM_MSG_SAFE> msg) {
  if (do_in_main_thread(FROM_HERE,
                        base::BindOnce(&bta_dm_search_sm_execute_safe, event,
                                       std::move(msg))) != BT_STATUS_SUCCESS) {
    log::error("bta_dm_search_sendmsg failed");
  }
}
...
static void bta_dm_sdp_result(tBTA_DM_SDP_RESULT& sdp_event) {
  ...
  if ((sdp_event.sdp_result == SDP_SUCCESS) ||
    ...
    if (bta_dm_search_cb.services_to_search) {
      ...
      std::unique_ptr<DM_MSG_SAFE> msg =
          std::make_unique<DM_MSG_SAFE>(tBTA_DM_DISC_RESULT{});
      ...
      bta_dm_search_sm_execute_safe(BTA_DM_DISCOVERY_RESULT_EVT,
                                    std::move(msg));
    }
  } else {
    BTM_LogHistory(
    ...
    bta_dm_search_sm_execute_safe(BTA_DM_DISCOVERY_RESULT_EVT, std::move(msg));
  }
}
...
static void bta_dm_queue_search(tBTA_DM_API_SEARCH& search) {
  ...
  bta_dm_search_cb.p_pending_search.reset(new DM_MSG_SAFE(search));
...
static void bta_dm_execute_queued_request() {
  ...
  if (p_pending_discovery) {
    ...
    bta_dm_search_sendmsg(BTA_DM_API_DISCOVER_EVT,
                          std::make_unique<DM_MSG_SAFE>(
                              tBTA_DM_API_DISCOVER{*p_pending_discovery}));
  } else if (bta_dm_search_cb.p_pending_search) {
    log::info("Start pending search");
    bta_dm_search_sendmsg(BTA_DM_API_SEARCH_EVT,
...
static void bta_dm_find_services(const RawAddress& bd_addr) {
  ...
  if (bta_dm_search_cb.service_index >= BTA_MAX_SERVICE_ID) {
    ...
    bta_dm_search_sendmsg(BTA_DM_DISCOVERY_RESULT_EVT, std::move(msg));
  }
}
...
static void bta_dm_discover_next_device(void) {
  ...
  if (bta_dm_search_cb.p_btm_inq_info != NULL) {
    ...
    bta_dm_search_sendmsg(BTA_DM_SEARCH_CMPL_EVT, nullptr);
  }
}
...
static void bta_dm_sdp_callback(UNUSED_ATTR const RawAddress& bd_addr,
                                tSDP_STATUS sdp_status) {
  bta_dm_search_sendmsg(BTA_DM_SDP_RESULT_EVT,
                        std::make_unique<DM_MSG_SAFE>(
...
static void bta_dm_remname_cback(const tBTM_REMOTE_DEV_NAME* p_remote_name) {
  ...
  std::unique_ptr<DM_MSG_SAFE> msg =
      std::make_unique<DM_MSG_SAFE>(tBTA_DM_REMOTE_NAME{});
  ...
  bta_dm_search_sendmsg(BTA_DM_REMT_NAME_EVT, std::move(msg));
}
...
static void bta_dm_gatt_disc_complete(uint16_t conn_id, tGATT_STATUS status) {
  ...
  if (conn_id != GATT_INVALID_CONN_ID) {
    ...
    if (bta_dm_search_cb.gatt_close_timer != nullptr) {
      ...
      bta_dm_search_sm_execute_safe(BTA_DM_DISC_CLOSE_TOUT_EVT, nullptr);
    }
  } else {
    ...
    case BTA_DM_SEARCH_IDLE:
      switch (event) {
        case BTA_DM_API_SEARCH_EVT:
          ...
          std::visit(overloaded{[](tBTA_DM_API_SEARCH msg) {
                                  bta_dm_search_start(msg);
                                },
                                [](auto) { log::fatal("bad message type!"); }},
                     *msg.get());
          break;
        case BTA_DM_API_DISCOVER_EVT:
          bta_dm_search_set_state(BTA_DM_DISCOVER_ACTIVE);
          std::visit(
              overloaded{[](tBTA_DM_API_DISCOVER msg) { bta_dm_discover(msg); },
                         [](auto) { log::fatal("bad message type!"); }},
              *msg.get());
          break;
    ...
    case BTA_DM_SEARCH_ACTIVE:
      switch (event) {
        case BTA_DM_REMT_NAME_EVT:
          std::visit(overloaded{[](tBTA_DM_REMOTE_NAME msg) {
        ...
        case BTA_DM_DISCOVERY_RESULT_EVT:
          std::visit(overloaded{[](tBTA_DM_DISC_RESULT msg) {
                                  bta_dm_search_result(msg);
                                },
                                [](auto) { log::fatal("bad message type!"); }},
                     *msg.get());
          break;
        case BTA_DM_DISC_CLOSE_TOUT_EVT:
          ...
          break;
        case BTA_DM_API_DISCOVER_EVT:
          std::visit(overloaded{[](tBTA_DM_API_DISCOVER& msg) {
                                  bta_dm_queue_disc(msg);
                                },
                                [](auto) { log::fatal("bad message type!"); }},
                     *msg.get());
    ...
    case BTA_DM_DISCOVER_ACTIVE:
      switch (event) {
        case BTA_DM_REMT_NAME_EVT:
          ...
          break;
        case BTA_DM_SDP_RESULT_EVT:
          std::visit(overloaded{[](tBTA_DM_SDP_RESULT& msg) {
...
void bta_dm_disc_start_device_discovery(tBTA_DM_SEARCH_CBACK* p_cback) {
  bta_dm_search_sm_execute_safe(
      BTA_DM_API_SEARCH_EVT, std::make_unique<DM_MSG_SAFE>(tBTA_DM_API_SEARCH{
                                 .services = 0, .p_cback = p_cback}));
}

void bta_dm_disc_stop_device_discovery() {
  bta_dm_search_sm_execute_safe(BTA_DM_API_SEARCH_CANCEL_EVT, nullptr);
}
...
void bta_dm_disc_start_service_discovery(service_discovery_callbacks cbacks,
  ...
  bta_dm_search_sm_execute_safe(
      ...
      std::make_unique<DM_MSG_SAFE>(tBTA_DM_API_DISCOVER{
