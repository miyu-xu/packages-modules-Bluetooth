...
gatt_interface_t default_gatt_interface = {
        .BTA_GATTC_CancelOpen =
                [](tGATT_IF client_if, const RawAddress& remote_bda, bool is_direct) {
                  gatt_history_.Push(std::format("{:<32s} bd_addr:{} client_if:{} is_direct:{:c}", "GATTC_CancelOpen", ADDRESS_TO_LOGGABLE_CSTR(remote_bda), static_cast<uint16_t>(client_if), (is_direct) ? 'T' : 'F'));
                  BTA_GATTC_CancelOpen(client_if, remote_bda, is_direct);
                },
        .BTA_GATTC_Refresh =
                [](const RawAddress& remote_bda) {
                  gatt_history_.Push(std::format("{:<32s} bd_addr:{}", "GATTC_Refresh", ADDRESS_TO_LOGGABLE_CSTR(remote_bda)));
                  BTA_GATTC_Refresh(remote_bda);
                },
        .BTA_GATTC_GetGattDb =
                [](tCONN_ID conn_id, uint16_t start_handle, uint16_t end_handle,
                   btgatt_db_element_t** db, int* count) {
                  gatt_history_.Push(std::format("{:<32s} conn_id:{} start_handle:{} end:handle:{}", "GATTC_GetGattDb", static_cast<uint16_t>(conn_id), start_handle, end_handle));
        ...
        .BTA_GATTC_AppRegister =
                [](tBTA_GATTC_CBACK* p_client_cb, BtaAppRegisterCallback cb, bool eatt_support) {
                  gatt_history_.Push(std::format("{:<32s} eatt_support:{:c}", "GATTC_AppRegister", (eatt_support) ? 'T' : 'F'));
                  BTA_GATTC_AppRegister(p_client_cb, cb, eatt_support);
                },
        .BTA_GATTC_Close =
                [](tCONN_ID conn_id) {
                  gatt_history_.Push(std::format("{:<32s} conn_id:{}", "GATTC_Close", static_cast<uint16_t>(conn_id)));
                  BTA_GATTC_Close(conn_id);
                },
        .BTA_GATTC_ServiceSearchRequest =
                [](tCONN_ID conn_id, const bluetooth::Uuid* p_srvc_uuid) {
                  gatt_history_.Push(std::format("{:<32s} conn_id:{}", "GATTC_ServiceSearchRequest", static_cast<uint16_t>(conn_id)));
                  if (p_srvc_uuid) {
        ...
        .BTA_GATTC_Open =
                [](tGATT_IF client_if, const RawAddress& remote_bda,
                  ...
                  gatt_history_.Push(std::format("{:<32s} bd_addr:{} client_if:{} type:0x{:x} opportunistic:{:c}", "GATTC_Open", ADDRESS_TO_LOGGABLE_CSTR(remote_bda), static_cast<uint16_t>(client_if), connection_type, (opportunistic) ? 'T' : 'F'));
