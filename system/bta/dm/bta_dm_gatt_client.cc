...
gatt_interface_t default_gatt_interface = {
        .BTA_GATTC_CancelOpen =
                [](tGATT_IF client_if, const RawAddress& remote_bda, bool is_direct) {
                  gatt_history_.Push(android::base::StringPrintf(
        ...
        .BTA_GATTC_Refresh =
                [](const RawAddress& remote_bda) {
                  gatt_history_.Push(
                          android::base::StringPrintf("%-32s bd_addr:%s", "GATTC_Refresh",
        ...
        .BTA_GATTC_AppRegister =
                [](tBTA_GATTC_CBACK* p_client_cb, BtaAppRegisterCallback cb, bool eatt_support) {
                  gatt_history_.Push(android::base::StringPrintf("%-32s eatt_support:%c",
                                                                 "GATTC_AppRegister",
                                                                 (eatt_support) ? 'T' : 'F'));
        ...
        .BTA_GATTC_Close =
                [](tCONN_ID conn_id) {
                  gatt_history_.Push(android::base::StringPrintf("%-32s conn_id:%hu", "GATTC_Close",
                                                                 static_cast<uint16_t>(conn_id)));
                  BTA_GATTC_Close(conn_id);
                },
        .BTA_GATTC_ServiceSearchRequest =
                [](tCONN_ID conn_id, const bluetooth::Uuid* p_srvc_uuid) {
                  gatt_history_.Push(android::base::StringPrintf("%-32s conn_id:%hu",
