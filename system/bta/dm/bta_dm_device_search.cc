...
static void bta_dm_remote_name_cmpl(const tBTA_DM_REMOTE_NAME& remote_name_msg) {
  BTM_LogHistory(
          ...
          std::format("status:{} state:{} name:\"{}\"", hci_status_code_text(remote_name_msg.hci_status), bta_dm_state_text(bta_dm_search_get_state()), PRIVATE_NAME(reinterpret_cast<char const*>(remote_name_msg.bd_name))));
