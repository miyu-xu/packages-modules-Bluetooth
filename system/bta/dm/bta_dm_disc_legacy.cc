...
static void post_disc_evt(tBTA_DM_EVT event, std::unique_ptr<tBTA_DM_MSG> msg) {
  if (do_in_main_thread(FROM_HERE,
                        base::BindOnce(&bta_dm_search_sm_execute, event, std::move(msg))) !=
      BT_STATUS_SUCCESS) {
...
struct gatt_interface_t {
  ...
  .BTA_GATTC_CancelOpen =
          [](tGATT_IF client_if, const RawAddress& remote_bda, bool is_direct) {
            disc_gatt_history_.Push(base::StringPrintf(
                    ...
                    ADDRESS_TO_LOGGABLE_CSTR(remote_bda), client_if, (is_direct) ? 'T' : 'F'));
  ...
  .BTA_GATTC_GetGattDb =
          [](uint16_t conn_id, uint16_t start_handle, uint16_t end_handle, btgatt_db_element_t** db,
             int* count) {
  ...
  .BTA_GATTC_AppRegister =
          [](tBTA_GATTC_CBACK* p_client_cb, BtaAppRegisterCallback cb, bool eatt_support) {
            disc_gatt_history_.Push(base::StringPrintf("%-32s eatt_support:%c", "GATTC_AppRegister",
                                                       (eatt_support) ? 'T' : 'F'));
  ...
  .BTA_GATTC_ServiceSearchRequest =
          [](uint16_t conn_id, const bluetooth::Uuid* p_srvc_uuid) {
            disc_gatt_history_.Push(
                    base::StringPrintf("%-32s conn_id:%hu", "GATTC_ServiceSearchRequest", conn_id));
            if (p_srvc_uuid) {
  ...
  .BTA_GATTC_Open =
          [](tGATT_IF client_if, const RawAddress& remote_bda, tBTM_BLE_CONN_TYPE connection_type,
             bool opportunistic) {
            disc_gatt_history_.Push(
                    base::StringPrintf("%-32s bd_addr:%s client_if:%hu type:0x%x opportunistic:%c",
                                       "GATTC_Open", ADDRESS_TO_LOGGABLE_CSTR(remote_bda),
                                       client_if, connection_type, (opportunistic) ? 'T' : 'F'));
...
static void bta_dm_search_cmpl() {
  ...
  bool send_gatt_results =
          bluetooth::common::init_flags::always_send_services_if_gatt_disc_done_is_enabled()
...
static void bta_dm_search_clear_queue() {
  bta_dm_search_cb.p_pending_search.reset();
  if (bluetooth::common::InitFlags::IsBtmDmFlushDiscoveryQueueOnSearchCancel()) {
