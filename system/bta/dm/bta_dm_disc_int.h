...
class BtaDmSdpPerformer {
 public:
  ...
  virtual ~BtaDmSdpPerformer() {}
  virtual void FindServices();
  virtual void OnResult(tSDP_STATUS sdp_status);
...
typedef base::RepeatingCallback<std::unique_ptr<BtaDmSdpPerformer>(RawAddress)>
    SdpPerformerFactory;
