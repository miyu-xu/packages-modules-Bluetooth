...
typedef enum : uint16_t {
  ...
  /* TODO: BTA_DM_DISCOVERY_CMPL_EVT ? */
} tBTA_DM_SERVICE_DISCOVERY_EVT;

inline std::string bta_dm_event_text(const tBTA_DM_DEVICE_SEARCH_EVT& event) {
  switch (event) {
...
inline std::string bta_dm_event_text(
...
typedef struct {
  ...
  tBTA_DM_SERVICE_DISCOVERY_STATE discovery_state;
...
namespace fmt {
template <>
struct formatter<tBTA_DM_SERVICE_DISCOVERY_EVT>
    : enum_formatter<tBTA_DM_SERVICE_DISCOVERY_EVT> {};
template <>
struct formatter<tBTA_DM_DEVICE_SEARCH_EVT>
    : enum_formatter<tBTA_DM_DEVICE_SEARCH_EVT> {};
template <>
