...
typedef struct {
  ...
  std::string ToString() const {
    return base::StringPrintf("p_search_cback:%p p_btm_inq_info:%p services:0x%x services_to_search:%x services_found:%x state:%hu peer_bdaddr:%s name_discover_done:%u peer_name:%s p_pending_search:%p pending_discovery_queue_sz:%zu wait_disc:%u sdp_results:%u uuid:%s peer_scn:%hhu transport:%s client_if:%hu uuid_to_search:%u gatt_disc_active:%u conn_id:%hu pending_close_bda:%s",
                       p_search_cback, p_btm_inq_info, services, services_to_search, services_found, state, PRIVATE_ADDRESS(peer_bdaddr), name_discover_done, peer_name, p_pending_search, fixed_queue_length(pending_discovery_queue), wait_disc, sdp_results, uuid.ToString().c_str(), peer_scn, bt_transport_text(transport).c_str(), client_if, uuid_to_search, gatt_disc_active, conn_id, PRIVATE_ADDRESS(pending_close_bda));
  }
