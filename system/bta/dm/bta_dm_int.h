...
void bta_dm_add_blekey(const RawAddress& bd_addr,
                              tBTA_LE_KEY_VALUE blekey,
                              tBTM_LE_KEY_TYPE key_type);
void bta_dm_add_ble_device(const RawAddress& bd_addr,
...
void bta_dm_ble_set_conn_params(const RawAddress&, uint16_t, uint16_t,
                                       uint16_t, uint16_t);
...
tBTA_DM_PEER_DEVICE* bta_dm_find_peer_device(
...
void bta_dm_set_default_event_mask_except(uint64_t mask,
                                                 uint64_t le_mask);
