...
typedef struct {
  ...
  std::string ToString() const {
    return std::format("peer:{} sys_name:{} app_id:{} state:{} new_request:{}", ADDRESS_TO_LOGGABLE_CSTR(peer_bdaddr), BtaIdSysText(id), app_id, bta_sys_conn_status_text(state), new_request ? "true" : "false");
  }
} tBTA_DM_SRVCS;
