...
void BTA_EnableTestMode(void) {
  do_in_main_thread(
      FROM_HERE, base::BindRepeating(base::IgnoreResult(BTM_EnableTestMode)));
}
...
void BTA_DmSetDeviceName(const char* p_name) {
  ...
  do_in_main_thread(FROM_HERE, base::BindRepeating(bta_dm_set_dev_name, name));
}
...
void BTA_DmBond(const RawAddress& bd_addr, tBLE_ADDR_TYPE addr_type,
  ...
  do_in_main_thread(
      FROM_HERE, base::BindRepeating(bta_dm_bond, bd_addr, addr_type, transport,
                                     device_type));
}
...
void BTA_DmBondCancel(const RawAddress& bd_addr) {
  do_in_main_thread(FROM_HERE,
                    base::BindRepeating(bta_dm_bond_cancel, bd_addr));
}
...
void BTA_DmPinReply(const RawAddress& bd_addr, bool accept, uint8_t pin_len,
  ...
  do_in_main_thread(FROM_HERE,
                    base::BindRepeating(bta_dm_pin_reply, base::Passed(&msg)));
}
...
void BTA_DmLocalOob(void) {
  do_in_main_thread(FROM_HERE, base::BindRepeating(BTM_ReadLocalOobData));
}
...
void BTA_DmConfirm(const RawAddress& bd_addr, bool accept) {
  do_in_main_thread(FROM_HERE,
                    base::BindRepeating(bta_dm_confirm, bd_addr, accept));
}
...
void BTA_DmAddDevice(const RawAddress& bd_addr, DEV_CLASS dev_class,
  ...
  do_in_main_thread(FROM_HERE,
                    base::BindRepeating(bta_dm_add_device, base::Passed(&msg)));
}
...
tBTA_STATUS BTA_DmRemoveDevice(const RawAddress& bd_addr) {
  do_in_main_thread(FROM_HERE,
                    base::BindRepeating(bta_dm_remove_device, bd_addr));
  return BTA_SUCCESS;
}
...
void BTA_DmAddBleKey(const RawAddress& bd_addr, tBTA_LE_KEY_VALUE* p_le_key,
  ...
  do_in_main_thread(FROM_HERE, base::BindRepeating(bta_dm_add_blekey, bd_addr,
                                                   *p_le_key, key_type));
}
...
void BTA_DmAddBleDevice(const RawAddress& bd_addr, tBLE_ADDR_TYPE addr_type,
  ...
  do_in_main_thread(
      FROM_HERE,
      base::BindRepeating(bta_dm_add_ble_device, bd_addr, addr_type, dev_type));
}
...
void BTA_DmBlePasskeyReply(const RawAddress& bd_addr, bool accept,
                           uint32_t passkey) {
  do_in_main_thread(
      FROM_HERE, base::BindRepeating(bta_dm_ble_passkey_reply, bd_addr, accept,
                                     accept ? passkey : 0));
}
...
void BTA_DmBleConfirmReply(const RawAddress& bd_addr, bool accept) {
  do_in_main_thread(FROM_HERE, base::BindRepeating(bta_dm_ble_confirm_reply,
                                                   bd_addr, accept));
}
...
void BTA_DmBleSecurityGrant(const RawAddress& bd_addr,
  ...
  do_in_main_thread(FROM_HERE,
                    base::BindRepeating(BTM_SecurityGrant, bd_addr, res));
}
...
void BTA_DmSetBlePrefConnParams(const RawAddress& bd_addr,
  ...
  do_in_main_thread(
      FROM_HERE,
      base::BindRepeating(bta_dm_ble_set_conn_params, bd_addr, min_conn_int,
...
void BTA_DmBleUpdateConnectionParams(const RawAddress& bd_addr,
  ...
  do_in_main_thread(
      FROM_HERE,
      base::BindRepeating(bta_dm_ble_update_conn_params, bd_addr, min_int,
...
void BTA_DmBleConfigLocalPrivacy(bool privacy_enable) {
  do_in_main_thread(
      FROM_HERE,
      base::BindRepeating(bta_dm_ble_config_local_privacy, privacy_enable));
}
...
void BTA_DmBleGetEnergyInfo(tBTA_BLE_ENERGY_INFO_CBACK* p_cmpl_cback) {
  do_in_main_thread(
      FROM_HERE, base::BindRepeating(bta_dm_ble_get_energy_info, p_cmpl_cback));
}

/** This function is to set maximum LE data packet size */
void BTA_DmBleRequestMaxTxDataLength(const RawAddress& remote_device) {
  do_in_main_thread(FROM_HERE, base::BindRepeating(bta_dm_ble_set_data_length,
                                                   remote_device));
}
...
void BTA_DmSetEncryption(const RawAddress& bd_addr, tBT_TRANSPORT transport,
  ...
  do_in_main_thread(
      FROM_HERE, base::BindRepeating(bta_dm_set_encryption, bd_addr, transport,
                                     p_callback, sec_act));
}
...
void BTA_DmCloseACL(const RawAddress& bd_addr, bool remove_dev,
                    tBT_TRANSPORT transport) {
  do_in_main_thread(FROM_HERE, base::BindRepeating(bta_dm_close_acl, bd_addr,
...
void BTA_DmBleObserve(bool start, uint8_t duration,
  ...
  do_in_main_thread(FROM_HERE, base::BindRepeating(bta_dm_ble_observe, start,
...
void BTA_DmBleScan(bool start, uint8_t duration_sec, bool low_latency_scan) {
  APPL_TRACE_API("%s:start = %d ", __func__, start);
  do_in_main_thread(FROM_HERE,
                    base::BindRepeating(bta_dm_ble_scan, start, duration_sec,
                                        low_latency_scan));
}
...
void BTA_DmBleCsisObserve(bool observe, tBTA_DM_SEARCH_CBACK* p_results_cb) {
  ...
  do_in_main_thread(FROM_HERE, base::BindRepeating(bta_dm_ble_csis_observe,
                                                   observe, p_results_cb));
}
...
void BTA_DmClearEventFilter(void) {
  ...
  do_in_main_thread(FROM_HERE, base::BindRepeating(bta_dm_clear_event_filter));
}
...
void BTA_DmClearEventMask(void) {
  ...
  do_in_main_thread(FROM_HERE, base::BindRepeating(bta_dm_clear_event_mask));
}
...
void BTA_DmClearFilterAcceptList(void) {
  ...
  do_in_main_thread(FROM_HERE,
                    base::BindRepeating(bta_dm_clear_filter_accept_list));
}
...
void BTA_DmLeRand(LeRandCallback cb) {
  ...
  do_in_main_thread(FROM_HERE, base::BindRepeating(bta_dm_le_rand, cb));
}
...
void BTA_DmDisconnectAllAcls() {
  APPL_TRACE_API("BTA_DmLeRand");
  do_in_main_thread(FROM_HERE, base::BindRepeating(bta_dm_disconnect_all_acls));
}
...
void BTA_DmSetEventFilterConnectionSetupAllDevices() {
  ...
  do_in_main_thread(FROM_HERE,
                    base::BindRepeating(
                        bta_dm_set_event_filter_connection_setup_all_devices));
}
...
void BTA_DmAllowWakeByHid(
  ...
  do_in_main_thread(FROM_HERE,
                    base::BindRepeating(bta_dm_allow_wake_by_hid,
...
void BTA_DmRestoreFilterAcceptList(
  ...
  do_in_main_thread(FROM_HERE,
                    base::BindRepeating(bta_dm_restore_filter_accept_list,
                                        std::move(le_devices)));
}
...
void BTA_DmSetDefaultEventMaskExcept(uint64_t mask, uint64_t le_mask) {
  ...
  do_in_main_thread(
      FROM_HERE,
      base::BindRepeating(bta_dm_set_default_event_mask_except, mask, le_mask));
}
...
void BTA_DmSetEventFilterInquiryResultAllDevices() {
  ...
  do_in_main_thread(
      FROM_HERE,
      base::BindRepeating(bta_dm_set_event_filter_inquiry_result_all_devices));
}
...
void BTA_DmBleResetId(void) {
  ...
  do_in_main_thread(FROM_HERE, base::BindRepeating(bta_dm_ble_reset_id));
}
...
void BTA_DmBleSubrateRequest(const RawAddress& bd_addr, uint16_t subrate_min,
  ...
  do_in_main_thread(
      FROM_HERE,
      base::BindRepeating(bta_dm_ble_subrate_request, bd_addr, subrate_min,
