...
void BTA_DmSetBlePrefConnParams(const RawAddress& bd_addr, uint16_t min_conn_int,
  ...
  do_in_main_thread(base::BindOnce(bta_dm_ble_set_conn_params, bd_addr, min_conn_int,
...
void BTA_DmBleUpdateConnectionParams(const RawAddress& bd_addr, uint16_t min_int, uint16_t max_int,
                                     ...
                                     uint16_t max_ce_len) {
  do_in_main_thread(base::BindOnce(bta_dm_ble_update_conn_params, bd_addr, min_int,
...
void BTA_DmBleConfigLocalPrivacy(bool privacy_enable) {
  if (com::android::bluetooth::flags::synchronous_bta_sec()) {
    ...
    do_in_main_thread(base::BindOnce(bta_dm_ble_config_local_privacy, privacy_enable));
  }
}
...
void BTA_DmBleSubrateRequest(const RawAddress& bd_addr, uint16_t subrate_min, uint16_t subrate_max,
  ...
  log::verbose("");
  do_in_main_thread(base::BindOnce(bta_dm_ble_subrate_request, bd_addr, subrate_min,
