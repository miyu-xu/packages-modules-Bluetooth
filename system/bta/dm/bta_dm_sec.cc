...
static void bta_dm_bond_retry_cback(void* data);
...
void bta_dm_bond(const RawAddress& bd_addr, tBLE_ADDR_TYPE addr_type, tBT_TRANSPORT transport,
  ...
  if (status == tBTM_STATUS::BTM_BUSY) {
    ...
    if (p_msg) {
      ...
      log::debug("Queueing bond request as RNR might be active");
      alarm_set_on_mloop(bta_dm_cb.bond_retry_timer, BTA_DM_BOND_RETRY_TIMER_MS,
                         bta_dm_bond_retry_cback, p_msg);
      return;
    }
  }
