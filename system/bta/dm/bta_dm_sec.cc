...
static tBTM_STATUS bta_dm_pin_cback(const RawAddress& bd_addr, DEV_CLASS dev_class,
  ...
  if (bd_name[0] == 0) {
    ...
    if ((get_rnr_interface().BTM_ReadRemoteDeviceName(bd_addr, bta_dm_pinname_cback,
    ...
    case BTM_SP_KEY_NOTIF_EVT:
      ...
      if (BTM_SP_CFM_REQ_EVT == event) {
        ...
        if (p_data->cfm_req.bd_name[0] == 0) {
          ...
          {
            const tBTM_STATUS btm_status = get_rnr_interface().BTM_ReadRemoteDeviceName(
      ...
      if (BTM_SP_KEY_NOTIF_EVT == event) {
        ...
        if (p_data->key_notif.bd_name[0] == 0) {
          ...
          if ((get_rnr_interface().BTM_ReadRemoteDeviceName(
