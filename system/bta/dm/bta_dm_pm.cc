...
/* Sysprop path for min and max sniff parameters */
#ifndef PROPERTY_SNIFF_MAX_INTERVALS
#define PROPERTY_SNIFF_MAX_INTERVALS \
  "bluetooth.core.classic.sniff_max_intervals"
#endif

#ifndef PROPERTY_SNIFF_MIN_INTERVALS
#define PROPERTY_SNIFF_MIN_INTERVALS \
  "bluetooth.core.classic.sniff_min_intervals"
#endif

#ifndef PROPERTY_SNIFF_ATTEMPTS
#define PROPERTY_SNIFF_ATTEMPTS "bluetooth.core.classic.sniff_attempts"
#endif

#ifndef PROPERTY_SNIFF_TIMEOUTS
#define PROPERTY_SNIFF_TIMEOUTS "bluetooth.core.classic.sniff_timeouts"
#endif
...
tBTM_PM_PWR_MD get_sniff_entry(uint8_t index) {
  ...
  // value
  std::vector<uint32_t> default_list(BTA_DM_PM_PARK_IDX, 0);
  std::vector<uint32_t> max =
      osi_property_get_uintlist(PROPERTY_SNIFF_MAX_INTERVALS, default_list);
  if (index >= max.size() || max == default_list) {
    return p_bta_dm_pm_md[index];
  }
  std::vector<uint32_t> min =
      osi_property_get_uintlist(PROPERTY_SNIFF_MIN_INTERVALS, default_list);
  if (index >= min.size() || min == default_list) {
    return p_bta_dm_pm_md[index];
  }
  std::vector<uint32_t> attempt =
      osi_property_get_uintlist(PROPERTY_SNIFF_ATTEMPTS, default_list);
  if (index >= attempt.size() || attempt == default_list) {
    return p_bta_dm_pm_md[index];
  }
  std::vector<uint32_t> timeout =
      osi_property_get_uintlist(PROPERTY_SNIFF_TIMEOUTS, default_list);
  if (index >= timeout.size() || timeout == default_list) {
    return p_bta_dm_pm_md[index];
  }
