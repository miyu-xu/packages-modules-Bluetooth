...
>>>>>>> 9f09a477e8f (Add GATT sequencing to dumpsys)
    .BTA_GATTC_GetGattDb =
...
<<<<<<< HEAD
    ...
    .BTA_GATTC_ServiceSearchRequest = BTA_GATTC_ServiceSearchRequest,
=======
    .BTA_GATTC_Close =
        [](uint16_t conn_id) {
          gatt_history_.Push(
...
void bta_dm_proc_close_evt(const tBTA_GATTC_CLOSE& close) {
  ...
  if (close.remote_bda == bta_dm_search_cb.peer_bdaddr) {
    ...
    LOG_WARN("Recieved close event for unknown peer:%s",
             ADDRESS_TO_LOGGABLE_CSTR(close.remote_bda));
  }
