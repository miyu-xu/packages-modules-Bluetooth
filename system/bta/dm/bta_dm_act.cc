...
<<<<<<< PATCH SET (3203c2 Fix unpairing failure when peer initiate reconnection at the)
...
>>>>>>> BASE      (bf4557 Merge "CS: Update opcode and subevent code" into main)
    ...
    if (i == bta_dm_cb.device_list.count) {
      log::info(
          ...
          "notified. Remove sec", bd_addr, transport);
...
>>>>>>> BASE      (bf4557 Merge "CS: Update opcode and subevent code" into main)
    ...
    /* Make sure device is not in acceptlist before we disconnect */
    if (!GATT_CancelConnect(0, other_address, false)) {
      log::warn("Unable to cancel GATT connect peer:{} is_direct:{}",
                other_address, false);
    }
    ...
    if (i == bta_dm_cb.device_list.count) {
      log::info(
          ...
          "notified. Remove sec.", other_address, other_transport);
