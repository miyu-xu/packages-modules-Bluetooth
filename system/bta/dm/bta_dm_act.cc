...
void bta_dm_disc_rmt_name(tBTA_DM_MSG* p_data) {
  ...
  const tBTA_DM_DISC_RES* disc_res = static_cast<tBTA_DM_DISC_RES*>(&p_data->rem_name.result.disc_res);

  BTM_LogHistory(kBtmLogTag, disc_res->bd_addr, "Remote name completed",
                 base::StringPrintf("status:%s name:\"%s\" service:0x%x device_type:%s num_uuids:%zu",
                                    bta_status_text(disc_res->result).c_str(), disc_res->bd_name,
                                    disc_res->services, DeviceTypeText(disc_res->device_type).c_str(),
                                    disc_res->num_uuids));

  tBTM_INQ_INFO* p_btm_inq_info = BTM_InqDbRead(p_data->rem_name.result.disc_res.bd_addr);
  if (p_btm_inq_info) {
    if (p_data->rem_name.result.disc_res.bd_name[0]) {
...
void bta_dm_sdp_result(tBTA_DM_MSG* p_data) {
  ...
  if ((p_data->sdp_event.sdp_result == SDP_SUCCESS) ||
    ...
    if (bta_dm_search_cb.services_to_search) {
      ...
      BTM_LogHistory(kBtmLogTag, bta_dm_search_cb.peer_bdaddr, "Discovery completed",
    ...
    BTM_LogHistory(kBtmLogTag, bta_dm_search_cb.peer_bdaddr, "Result failed",
