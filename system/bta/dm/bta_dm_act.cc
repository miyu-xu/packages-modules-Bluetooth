...
static uint8_t bta_dm_csis_verifiction_cback(const RawAddress& bd_addr);
...
const tBTM_APPL_INFO bta_security = {
    ...
    .p_csis_verification_callback = &bta_dm_csis_verifiction_cback};
...
void bta_dm_ble_csis_sec_cb_register(tBTA_DM_SEC_CBACK* p_cback) {
  /* Save the callback to be called when a request of member validation will be
   * needed. */
  APPL_TRACE_DEBUG("%s", __func__);
  bta_dm_cb.p_sec_csis_cback = p_cback;
}

void bta_dm_ble_csis_confirm_device_reply(const RawAddress& bd_addr,
                                          bool accept) {
  APPL_TRACE_DEBUG("%s", __func__);
  BTM_BleCsisConfirmDeviceReply(bd_addr,
                                accept ? BTM_SUCCESS : BTM_NOT_AUTHORIZED);
}
...
/*******************************************************************************
 *
 * Function         bta_dm_csis_verifiction_cback
 *
 * Description      Callback requesting CSIS member verification
 *
 *
 * Returns          void
 *
...
static uint8_t bta_dm_csis_verifiction_cback(const RawAddress& bd_addr) {
  ...
  if (bta_dm_cb.p_sec_csis_cback) {
    APPL_TRACE_DEBUG("%s callback called", __func__);
    bta_dm_cb.p_sec_csis_cback(BTA_DM_CSIS_VERIFICATION_REQ_EVT, &sec_event);
    return BTM_CMD_STARTED;
  }
