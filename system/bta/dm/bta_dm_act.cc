...
void bta_dm_remote_name_cmpl(const tBTA_DM_MSG* p_data) {
  ...
  if (bta_dm_search_cb.p_search_cback != nullptr) {
    ...
    if (remote_name_msg.hci_status == HCI_SUCCESS) {
      memcpy(search_data.disc_res.bd_name, remote_name_msg.bd_name,
             BD_NAME_LEN);
    }
  ...
  // service discovery request, otherwise just use the address
  // now that we know the name.
  bta_dm_discover_device(
      (bta_dm_search_cb.peer_bdaddr == remote_name_msg.bd_addr)
          ? remote_name_msg.bd_addr
...
static void bta_dm_remname_cback(const tBTM_REMOTE_DEV_NAME* p) {
  ...
  *p_msg = {
  ...
  strlcpy((char*)p_msg->remote_name_msg.bd_name,
          (char*)p_remote_name->remote_bd_name, BD_NAME_LEN + 1);
  bta_sys_sendmsg(p_msg);
}
