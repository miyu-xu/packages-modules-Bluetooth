...
void bta_dm_set_dev_name(const std::string& name) {
  bluetooth::shim::GetController()->WriteLocalName(name);
  bta_dm_set_eir();
}
...
static void bta_dm_set_eir() {
  ...
  std::string local_name = bluetooth::shim::GetController()->GetLocalName();
  size_t local_name_len = local_name.size();
  ...
  if (local_name.size() > p_bta_dm_eir_cfg->bta_dm_eir_min_name_len) {
    ...
    if (local_name.size() > (free_eir_length - 4 - num_uuid * Uuid::kNumBytes16)) {
      local_name_len =
              find_utf8_char_boundary(local_name.data(), p_bta_dm_eir_cfg->bta_dm_eir_min_name_len);
  ...
  memcpy(p, local_name.data(), local_name_len);
  p += local_name_len;
