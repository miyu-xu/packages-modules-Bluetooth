...
static void bta_dm_remname_cback(void* p) {
  ...
  if (bta_dm_search_cb.peer_bdaddr == p_remote_name->bd_addr) {
    ...
    if (p_remote_name->hci_status == HCI_ERR_CONNECTION_EXISTS) {
      ...
      LOG_INFO(
          "Assuming command failed due to disconnection hci_status:%s peer:%s",
          hci_error_code_text(p_remote_name->hci_status),
          ADDRESS_TO_LOGGABLE_CSTR(p_remote_name->remote_bd_name));
    } else {
      LOG_INFO(
          "ignoring remote name response for the wrong address exp:%s act:%s",
