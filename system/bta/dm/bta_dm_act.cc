...
static void disconnect_peer(const RawAddress& bd_addr, tBT_TRANSPORT transport) {
  if (BTM_IsAclConnectionUp(bd_addr, transport)) {
    ...
    for (int i = 0; i < bta_dm_cb.device_list.count; i++) {
      ...
      LOG_ERROR("Found Peer: %s with transport %s", peer_device.peer_bdaddr.ToString().c_str(), ((peer_device.transport == BT_TRANSPORT_LE) ? "BT_TRANSPORT_LE" : "BT_TRANSPORT_BR_EDR"));
      // Set the connection state to indicate removal, then disconnect for matchin address
...
void bta_dm_remove_device(const RawAddress& bd_addr) {
  ...
  bool classic_address_connected = BTM_ReadConnectedTransportAddress(&classic_address, BT_TRANSPORT_BR_EDR);
  ...
  bool le_address_connected = BTM_ReadConnectedTransportAddress(&le_address, BT_TRANSPORT_LE);
...
//  // Now let's try the pseudo address
//  RawAddress other_address = bd_addr;
//  // LE; Is this a pseudo address that has a connected identiy address?
//  // LE; Is this a identity address that has a connected pseudo address?
//  // DCK LE Only; Need to be able to accept an identity address to remove bond.
//  if (btm_random_pseudo_to_identity_addr(other_address, type)
//          || btm_identity_addr_to_random_pseudo(other_address, type, false)) {
//    if (BTM_IsAclConnectionUp(other_address, BT_TRANSPORT_LE)) {
//      disconnect_peer(other_address, BT_TRANSPORT_LE);
//    }
//  }
