...
enum class tBTA_JV_SDP_ACT {
  NONE = 0,
  YES,   /* waiting for SDP result */
  CANCEL /* waiting for cancel complete */
};

inline std::string bta_jv_sdp_act_text(const tBTA_JV_SDP_ACT& state) {
  switch (state) {
    CASE_RETURN_TEXT(tBTA_JV_SDP_ACT::NONE);
    CASE_RETURN_TEXT(tBTA_JV_SDP_ACT::YES);
    CASE_RETURN_TEXT(tBTA_JV_SDP_ACT::CANCEL);
  }
}

namespace fmt {
template <>
struct formatter<tBTA_JV_SDP_ACT> : enum_formatter<tBTA_JV_SDP_ACT> {};
}  // namespace fmt
...
struct tBTA_JV_CB {
  ...
  tBTA_JV_SDP_ACT sdp_active{tBTA_JV_SDP_ACT::NONE};
