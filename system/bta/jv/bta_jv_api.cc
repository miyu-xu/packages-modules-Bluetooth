...
tBTA_JV_STATUS BTA_JvEnable(tBTA_JV_DM_CBACK* p_cback) {
  ...
  do_in_main_thread(FROM_HERE, base::BindOnce(&bta_jv_enable, p_cback));
...
void BTA_JvDisable(void) {
  ...
  do_in_main_thread(FROM_HERE, base::BindOnce(&bta_jv_disable));
}
...
void BTA_JvGetChannelId(int conn_type, uint32_t id, int32_t channel) {
  ...
  do_in_main_thread(FROM_HERE,
                    base::BindOnce(&bta_jv_get_channel_id, conn_type, channel, id, id));
}
...
tBTA_JV_STATUS BTA_JvFreeChannel(uint16_t channel, int conn_type) {
  ...
  do_in_main_thread(FROM_HERE, base::BindOnce(&bta_jv_free_scn, conn_type, channel));
  return BTA_JV_SUCCESS;
}
...
tBTA_JV_STATUS BTA_JvStartDiscovery(const RawAddress& bd_addr,
  ...
  do_in_main_thread(FROM_HERE,
                    base::BindOnce(&bta_jv_start_discovery, bd_addr, num_uuid,
...
tBTA_JV_STATUS BTA_JvCreateRecordByUser(uint32_t rfcomm_slot_id) {
  ...
  do_in_main_thread(FROM_HERE, base::BindOnce(&bta_jv_create_record, rfcomm_slot_id));
  return BTA_JV_SUCCESS;
}
...
tBTA_JV_STATUS BTA_JvDeleteRecord(uint32_t handle) {
  ...
  do_in_main_thread(FROM_HERE, base::BindOnce(&bta_jv_delete_record, handle));
  return BTA_JV_SUCCESS;
}
...
tBTA_JV_STATUS BTA_JvL2capClose(uint32_t handle) {
  ...
  do_in_main_thread(
      FROM_HERE, base::BindOnce(&bta_jv_l2cap_close, handle, &bta_jv_cb.l2c_cb[handle]));
  return BTA_JV_SUCCESS;
}
...
tBTA_JV_STATUS BTA_JvL2capStopServer(uint16_t local_psm,
  ...
  do_in_main_thread(
      FROM_HERE, base::BindOnce(&bta_jv_l2cap_stop_server, local_psm, l2cap_socket_id));
  return BTA_JV_SUCCESS;
}
...
tBTA_JV_STATUS BTA_JvL2capWrite(uint32_t handle, uint32_t req_id, BT_HDR* msg,
  ...
  do_in_main_thread(FROM_HERE, base::BindOnce(&bta_jv_l2cap_write, handle, req_id, msg,
...
tBTA_JV_STATUS BTA_JvRfcommConnect(tBTA_SEC sec_mask, tBTA_JV_ROLE role,
  ...
  do_in_main_thread(FROM_HERE,
                    base::BindOnce(&bta_jv_rfcomm_connect, sec_mask, remote_scn,
...
tBTA_JV_STATUS BTA_JvRfcommClose(uint32_t handle, uint32_t rfcomm_slot_id) {
  ...
  do_in_main_thread(FROM_HERE,
                    base::BindOnce(&bta_jv_rfcomm_close, handle, rfcomm_slot_id));
  return BTA_JV_SUCCESS;
}
...
tBTA_JV_STATUS BTA_JvRfcommStartServer(tBTA_SEC sec_mask, tBTA_JV_ROLE role,
  ...
  do_in_main_thread(FROM_HERE,
                    base::BindOnce(&bta_jv_rfcomm_start_server, sec_mask, local_scn,
...
tBTA_JV_STATUS BTA_JvRfcommStopServer(uint32_t handle,
  ...
  do_in_main_thread(FROM_HERE,
                    base::BindOnce(&bta_jv_rfcomm_stop_server, handle, rfcomm_slot_id));
  return BTA_JV_SUCCESS;
}
...
tBTA_JV_STATUS BTA_JvRfcommWrite(uint32_t handle, uint32_t req_id) {
  ...
  do_in_main_thread(FROM_HERE, base::BindOnce(&bta_jv_rfcomm_write, handle, req_id, p_cb,
...
tBTA_JV_STATUS BTA_JvSetPmProfile(uint32_t handle, tBTA_JV_PM_ID app_id,
  ...
  do_in_main_thread(FROM_HERE,
                    base::BindOnce(&bta_jv_set_pm_profile, handle, app_id, init_st));
  return BTA_JV_SUCCESS;
}
