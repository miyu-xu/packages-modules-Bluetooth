...
tBTA_JV_RFC_CB* bta_jv_alloc_rfc_cb(uint16_t port_handle,
  ...
  for (i = 0; i < BTA_JV_MAX_RFC_CONN; i++) {
    if (0 == bta_jv_cb.rfc_cb[i].handle) {
      ...
      log::verbose("port_handle={}, handle={:x}", port_handle, p_cb->handle);
...
static tBTA_JV_STATUS bta_jv_free_rfc_cb(tBTA_JV_RFC_CB* p_cb,
  ...
  log::verbose(
      "max_sess={}, curr_sess={}, p_pcb={}, user={}, state={}, jv handle={:x}",
  ...
  switch (p_pcb->state) {
    ...
    default:
      log::warn(
          "failed, ignore port state= {}, scn={}, p_pcb= {}, jv handle={:x}, "
  ...
  if (tBTA_JV_STATUS::SUCCESS == status) {
    ...
    if (port_status != PORT_SUCCESS) {
      status = tBTA_JV_STATUS::FAILURE;
      log::warn(
          "Remove jv handle={:x}, state={}, port_status={}, port_handle={}, "
          "close_pending={}",
...
static tBTA_JV_STATUS bta_jv_free_set_pm_profile_cb(uint32_t jv_handle) {
  ...
  for (i = 0; i < BTA_JV_PM_MAX_NUM; i++) {
    ...
    if ((bta_jv_cb.pm_cb[i].state != BTA_JV_PM_FREE_ST) &&
      ...
      log::verbose(
          "jv_handle={:x}, idx={}app_id={}, bd_counter={}, appid_counter={}",
      ...
      if (BTA_JV_RFCOMM_MASK & jv_handle) {
        ...
        if (hi < BTA_JV_MAX_RFC_CONN && bta_jv_cb.rfc_cb[hi].p_cback &&
          ...
          if (p_pcb) {
            if (NULL == p_pcb->p_pm_cb)
              log::warn(
                  "jv_handle={:x}, port_handle={}, i={}, no link to pm_cb?",
        ...
        if (jv_handle < BTA_JV_MAX_L2C_CONN) {
          ...
          if (NULL == p_l2c_cb->p_pm_cb)
            log::warn("jv_handle={:x}, i={} no link to pm_cb?", jv_handle, i);
...
static tBTA_JV_PM_CB* bta_jv_alloc_set_pm_profile_cb(uint32_t jv_handle,
  ...
  for (i = 0; i < BTA_JV_PM_MAX_NUM; i++) {
    ...
    if (bta_jv_cb.pm_cb[i].state == BTA_JV_PM_FREE_ST) {
      ...
      log::verbose(
          "handle={:x}, app_id={}, idx={}, BTA_JV_PM_MAX_NUM={}, pp_cb={}",
  ...
  log::warn("handle={:x}, app_id={}, return NULL", jv_handle, app_id);
  return NULL;
}
...
static void bta_jv_set_free_psm(uint16_t psm) {
  ...
  if (free_index != -1) {
    ...
    log::verbose("Recycling PSM={:x}", psm);
  } else {
    log::error("unable to free psm={:x} no more free slots", psm);
  }
}
  ...
  switch (type) {
    ...
    case BTA_JV_CONN_TYPE_L2CAP:
      ...
      if (psm == 0) {
        ...
        log::verbose("returned PSM={:x}", psm);
      }
      break;
...
static void bta_jv_l2cap_client_cback(uint16_t gap_handle, uint16_t event,
  ...
  log::verbose("gap_handle={}, evt={:x}", gap_handle, event);
...
static void bta_jv_port_event_cl_cback(uint32_t code, uint16_t port_handle) {
  ...
  log::verbose("code={:x}, port_handle={}, handle={}", code, port_handle,
               p_cb->handle);
  ...
  for (i = 0; i < MAX_RFC_PORTS; i++) {
    ...
    if (rfc_handle && bta_jv_cb.port_cb[i].rfcomm_slot_id == rfcomm_slot_id) {
      ...
      *cb = &bta_jv_cb.rfc_cb[rfc_handle - 1];
      log::verbose(
          "FOUND rfc_cb_handle={:x}, port.jv_handle={:x}, state={}, "
          "rfc_cb->handle={:x}",
...
static void bta_jv_port_mgmt_sr_cback(uint32_t code, uint16_t port_handle) {
  ...
  log::verbose("code={}, port_handle={:x}, handle={:x}, p_pcb{}, user={}", code,
               port_handle, p_cb->handle, fmt::ptr(p_pcb),
               p_pcb->rfcomm_slot_id);
  ...
  if (p_cb->max_sess > 1) {
    ...
    if (used < p_cb->max_sess && listen == 1 && si) {
      ...
      if (RFCOMM_CreateConnectionWithSecurity(
        ...
        log::verbose("p_pcb->handle={:x}, curr_sess={}", p_pcb->handle,
                     p_cb->curr_sess);
      } else {
        log::error("RFCOMM_CreateConnection failed");
        return NULL;
...
void bta_jv_set_pm_profile(uint32_t handle, tBTA_JV_PM_ID app_id,
  ...
  log::verbose("handle={:x}, app_id={}, init_st={}", handle, app_id, init_st);
...
static void bta_jv_pm_state_change(tBTA_JV_PM_CB* p_cb,
                                   const tBTA_JV_CONN_STATE state) {
  log::verbose(
      "p_cb={}, handle={:x}, busy/idle_state={}, app_id={}, conn_state={}",
      fmt::ptr(p_cb), p_cb->handle, p_cb->state, p_cb->app_id, state);
