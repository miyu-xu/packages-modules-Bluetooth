  ...
  switch (event) {
    case GAP_EVT_CONN_OPENED:
      if (!com::android::bluetooth::flags::bt_offload_socket_api()) {
        ...
        uint16_t local_mtu, remote_mtu, local_mps, remote_mps, local_credit, remote_credit;
        uint16_t local_cid, remote_cid, acl_handle;
        evt_data.l2c_open.rem_bda = *GAP_ConnGetRemoteAddr(gap_handle);
        if (GAP_GetLeChannelInfo(gap_handle, &local_mtu, &remote_mtu, &local_mps, &remote_mps,
                                 &local_credit, &remote_credit, &local_cid, &remote_cid,
                                 &acl_handle) != PORT_SUCCESS) {
          log::warn("Unable to get GAP channel info handle:{}", gap_handle);
        }
        evt_data.l2c_open.rx_mtu = local_mtu;
