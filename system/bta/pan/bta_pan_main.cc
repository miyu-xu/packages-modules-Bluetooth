...
tBTA_PAN_SCB* bta_pan_scb_alloc(void) {
  ...
  for (i = 0; i < BTA_PAN_NUM_CONN; i++, p_scb++) {
    if (!p_scb->in_use) {
      p_scb->in_use = true;
      LOG_DEBUG("bta_pan_scb_alloc %d", i);
      break;
    }
  }
...
void bta_pan_sm_execute(tBTA_PAN_SCB* p_scb, uint16_t event,
  ...
  LOG_INFO("PAN scb=%d event=0x%x state=%d", bta_pan_scb_to_idx(p_scb), event,
           p_scb->state);
...
void bta_pan_scb_dealloc(tBTA_PAN_SCB* p_scb) {
  LOG_DEBUG("bta_pan_scb_dealloc %d", bta_pan_scb_to_idx(p_scb));
