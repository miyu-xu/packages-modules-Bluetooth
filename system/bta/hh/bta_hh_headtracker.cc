...
bool bta_hh_headtracker_supported(tBTA_HH_DEV_CB* p_dev_cb) {
  ...
  if (p_dev_cb->hid_srvc.headtracker_support == BTA_HH_UNKNOWN) {
    ...
    auto bd_addr = ToGdAddress(p_dev_cb->link_spec.addrt.bda);
    auto uuids = bluetooth::shim::GetStorage()
                     ->GetDeviceByLegacyKey(bd_addr)
                     .GetServiceUuids();
    if (uuids &&
        std::find(uuids->begin(), uuids->end(),
                  hci::Uuid::From128BitBE(
                      ANDROID_HEADTRACKER_SERVICE_UUID.To128BitBE())) !=
            uuids->end()) {
      p_dev_cb->hid_srvc.headtracker_support = BTA_HH_AVAILABLE;
    }
    log::verbose("Headtracker support: {}",
                 p_dev_cb->hid_srvc.headtracker_support);
  }
