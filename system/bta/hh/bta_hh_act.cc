...
void bta_hh_open_cmpl_act(tBTA_HH_DEV_CB* p_cb, const tBTA_HH_DATA* p_data) {
  ...
  BTM_LogHistory(kBtmLogTag, p_cb->link_spec.addrt.bda, "Opened",
                 std::format("{} initiator:{}", bt_transport_text(p_cb->link_spec.transport), (p_cb->incoming_conn) ? "remote" : "local"));
...
void bta_hh_close_act(tBTA_HH_DEV_CB* p_cb, const tBTA_HH_DATA* p_data) {
  ...
  std::string overlay_fail = std::format("{} {} {}", (l2cap_conn_fail) ? "l2cap_conn_fail" : "", (l2cap_req_fail) ? "l2cap_req_fail" : "", (l2cap_cfg_fail) ? "l2cap_cfg_fail" : "");
  BTM_LogHistory(
          kBtmLogTag, p_cb->link_spec.addrt.bda, "Closed",
          std::format("{} reason {} {}", (p_cb->link_spec.transport == BT_TRANSPORT_LE) ? "le" : "classic", hid_status_text(hid_status), overlay_fail));
