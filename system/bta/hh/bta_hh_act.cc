...
void bta_hh_sdp_cmpl(tBTA_HH_DEV_CB* p_cb, const tBTA_HH_DATA* p_data) {
  ...
  if (status != BTA_HH_OK) {
    ...
    if ((status == BTA_HH_ERR_SDP) && (p_cb->incoming_conn) &&
        (p_cb->app_id == 0)) {
      log::error("SDP failed for  incoming conn hndl:{}",
 ...
 *
 ******************************************************************************/
void bta_hh_transport_select(tBTA_HH_DEV_CB* p_cb, const tBTA_HH_DATA* p_data) {
  ...
  p_cb->link_spec.transport =
      le_preferred ? BT_TRANSPORT_LE : BT_TRANSPORT_BR_EDR;
}
...
void bta_hh_open_cmpl_act(tBTA_HH_DEV_CB* p_cb, const tBTA_HH_DATA* p_data) {
  ...
  conn.le_hid = p_cb->link_spec.transport;
  ...
  switch (p_dev_info->sub_event) {
    case BTA_HH_ADD_DEV_EVT: /* add a device */
      ...
      if (p_cb->hid_handle == BTA_HH_INVALID_HANDLE) {
        ...
        if (transport == BT_TRANSPORT_LE) {
        ...
        } else if (p_data->api_maintdev.link_spec.transport ==
                   BT_TRANSPORT_BR_EDR) {
          ...
          log::error("un expected BT transport");
        }
