...
static void bta_hh_sdp_cback(uint16_t result, uint16_t attr_mask,
  ...
  if ((result == SDP_SUCCESS) && (p_cb != NULL)) {
    ...
    LOG_INFO("%s: p_cb: %p result 0x%02x, attr_mask 0x%02x, handle %x",
...
static void bta_hh_di_sdp_cback(UNUSED_ATTR const RawAddress& bd_addr,
  ...
  tHID_STATUS ret;
  LOG_INFO("%s: p_cb: %p result 0x%02x", __func__, p_cb, result);
  ...
  if (((result == SDP_SUCCESS) || (result == SDP_NO_RECS_MATCH)) &&
    ...
    if (ret == HID_SUCCESS) {
      ...
      LOG_DEBUG("%s:  HID_HostGetSDPRecord failed: Status 0x%2x", __func__,
                ret);
    }
  }
...
void bta_hh_sdp_cmpl(tBTA_HH_DEV_CB* p_cb, const tBTA_HH_DATA* p_data) {
  ...
  LOG_DEBUG("%s:  status 0x%2X", __func__, p_data->status);
  ...
  if (status == BTA_HH_OK) {
    ...
    if (!p_cb->incoming_conn) {
      ...
      LOG_DEBUG("%s: HID_HostOpenDev returned=%d", __func__, ret);
      if (ret == HID_SUCCESS || ret == HID_ERR_ALREADY_CONN) {
         ...
         */
        LOG_DEBUG("%s: connection already in progress", __func__);
        return;
      } else {
        LOG_DEBUG("%s: HID_HostOpenDev failed: Status 0x%2X", __func__, ret);
...
static void bta_hh_bredr_conn(tBTA_HH_DEV_CB* p_cb, const tBTA_HH_DATA* p_data) {
  ...
  if (p_cb->app_id) {
    ...
    LOG_DEBUG("%s: skip SDP for known devices", __func__);
...
void bta_hh_open_act(tBTA_HH_DEV_CB* p_cb, const tBTA_HH_DATA* p_data) {
  ...
  LOG_INFO("%s:  Device[%d] connected", __func__, dev_handle);
...
void bta_hh_handsk_act(tBTA_HH_DEV_CB* p_cb, const tBTA_HH_DATA* p_data) {
  LOG_DEBUG("HANDSHAKE received for: event = %s data= %d",
  ...
  switch (p_cb->w4_evt) {
    ...
    default:
      ...
      LOG_DEBUG("unknown transaction type");
      break;
  }
...
void bta_hh_ctrl_dat_act(tBTA_HH_DEV_CB* p_cb, const tBTA_HH_DATA* p_data) {
  ...
  LOG_DEBUG("Ctrl DATA received w4: event[%s]",
  ...
  switch (p_cb->w4_evt) {
    ...
    case BTA_HH_SET_IDLE_EVT:
      FALLTHROUGH_INTENDED; /* FALLTHROUGH */
    default:
      LOG_DEBUG("invalid  transaction type for DATA payload: 4_evt[%s]",
                bta_hh_get_w4_event(p_cb->w4_evt));
      break;
  }
  ...
  if (p_cb->is_le_device)
    ...
    if (status != HID_SUCCESS) {
      ...
      switch (p_data->api_sndcmd.t_type) {
        ...
        default:
          LOG_DEBUG("%s: cmd type = %d", __func__, p_data->api_sndcmd.t_type);
          break;
      }
...
static void bta_hh_cback(uint8_t dev_handle, const RawAddress& addr,
  ...
  LOG_DEBUG("%s::HID_event [%s]", __func__, bta_hh_hid_event_name(event));
