  ...
  if ((result == SDP_SUCCESS) && (p_cb != NULL)) {
    ...
    if (bta_hh_tod_spt(p_cb, sdp_rec->sub_class)) {
      ...
      if (p_cb->hid_handle == BTA_HH_INVALID_HANDLE) {
        ...
        if (HID_HostAddDev(p_cb->addr.addrt.bda, attr_mask, &hdl) == HID_SUCCESS) {
          status = BTA_HH_OK;
          /* update cb_index[] map */
...
static void bta_hh_di_sdp_cback(UNUSED_ATTR const RawAddress& bd_addr,
  ...
  if (((result == SDP_SUCCESS) || (result == SDP_NO_RECS_MATCH)) &&
    ...
    ret = HID_HostGetSDPRecord(p_cb->addr.addrt.bda, bta_hh_cb.p_disc_db,
...
static void bta_hh_start_sdp(tBTA_HH_DEV_CB* p_cb, const tBTA_HH_DATA* p_data) {
  if (!bta_hh_cb.p_disc_db) {
    ...
    if (get_legacy_stack_sdp_api()->device_id.SDP_DiDiscover(
            p_data->api_conn.dev_addr.addrt.bda, bta_hh_cb.p_disc_db,
...
static void bta_hh_bredr_conn(tBTA_HH_DEV_CB* p_cb, const tBTA_HH_DATA* p_data) {
  ...
  if (p_cb->app_id) {
    ...
    if (p_cb->hid_handle == BTA_HH_INVALID_HANDLE) {
      uint8_t hdl;
      if (HID_HostAddDev(p_cb->addr.addrt.bda, p_cb->attr_mask, &hdl) == HID_SUCCESS) {
 ...
 *
 ******************************************************************************/
void btif_hh_remove_device(tTypedAddressTransport bd_addr);
...
void bta_hh_open_cmpl_act(tBTA_HH_DEV_CB* p_cb, const tBTA_HH_DATA* p_data) {
  ...
  BTM_LogHistory(kBtmLogTag, p_cb->addr.addrt.bda, "Opened",
                 base::StringPrintf(
  ...
  {
    /* inform role manager */
    bta_sys_conn_open(BTA_ID_HH, p_cb->app_id, p_cb->addr.addrt.bda);
...
void bta_hh_handsk_act(tBTA_HH_DEV_CB* p_cb, const tBTA_HH_DATA* p_data) {
  ...
  bta_sys_idle(BTA_ID_HH, p_cb->app_id, p_cb->addr.addrt.bda);
  return;
}
/*******************************************************************************
 *
...
void bta_hh_ctrl_dat_act(tBTA_HH_DEV_CB* p_cb, const tBTA_HH_DATA* p_data) {
  ...
  bta_sys_busy(BTA_ID_HH, p_cb->app_id, p_cb->addr.addrt.bda);
  bta_sys_idle(BTA_ID_HH, p_cb->app_id, p_cb->addr.addrt.bda);
...
void bta_hh_close_act(tBTA_HH_DEV_CB* p_cb, const tBTA_HH_DATA* p_data) {
  ...
  /* inform role manager */
  bta_sys_conn_close(BTA_ID_HH, p_cb->app_id, p_cb->addr.addrt.bda);
  ...
  if (p_cb->is_le_device)
    ...
    if (status != HID_SUCCESS) {
      ...
      if (p_data->api_sndcmd.t_type != HID_TRANS_CONTROL) {
        ...
        bta_sys_sco_close(BTA_ID_HH, p_cb->app_id, p_cb->addr.addrt.bda);
