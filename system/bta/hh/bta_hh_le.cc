...
static void bta_hh_le_parse_report_map_charac(
...
static void bta_hh_le_parse_report_charac(tBTA_HH_DEV_CB* p_dev_cb,
  ...
  if (p_rpt->rpt_type != BTA_HH_RPTT_INPUT) return;
  ...
  for (const gatt::Characteristic& charac : service->characteristics) {
    ...
    switch (uuid16) {
      ...
      case GATT_UUID_HID_INFORMATION:
        ...
        break;
      case GATT_UUID_HID_REPORT_MAP:
        bta_hh_le_parse_report_map_charac(p_dev_cb, charac);
        break;

      case GATT_UUID_HID_REPORT:
        bta_hh_le_parse_report_charac(p_dev_cb, charac);
        break;
...
static void bta_hh_le_srvc_search_cmpl(tBTA_GATTC_SEARCH_CMPL* p_data) {
  ...
  for (const gatt::Service& service : *services) {
    if (service.uuid == Uuid::From16Bit(UUID_SERVCLASS_LE_HID) &&
      ...
      gap_service = &service;
    } else if (IS_FLAG_ENABLED(android_headtracker_service) &&
