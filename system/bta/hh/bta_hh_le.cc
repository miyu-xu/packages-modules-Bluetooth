...
static bool bta_hh_le_iso_data_callback(const RawAddress& addr,
  ...
  tBTA_HH_DEV_CB* p_dev_cb = bta_hh_le_find_dev_cb_by_bda(addr);
  if (p_dev_cb == nullptr) {
    LOG_WARN("Device not connected: %s", ADDRESS_TO_LOGGABLE_CSTR(addr));
    return false;
  }

  bta_hh_co_data(p_dev_cb->hid_handle, data, size, p_dev_cb->mode, 0,
                 p_dev_cb->dscp_info.ctry_code, p_dev_cb->addr,
                 BTA_HH_APP_ID_LE);
  return true;
}
