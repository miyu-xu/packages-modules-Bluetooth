...
static void bta_hh_le_open_cmpl(tBTA_HH_DEV_CB* p_cb) {
  if (p_cb->disc_active == BTA_HH_LE_DISC_NONE) {
    ...
    // if (device is in interop lib)
    BTA_GATTC_ConfigureMTU(p_cb->conn_id, 517);
...
void bta_hh_le_save_report_map(tBTA_HH_DEV_CB* p_dev_cb, uint16_t len, uint8_t* desc) {
  ...
  if (len > 0) {
    // if (device is in interop lib)
    len = 605;
    uint8_t quirk[] = {0x31, 0x81, 0x02, 0xC0, 0x05, 0x0D, 0x09, 0x54, 0x25, 0x05, 0x75, 0x07,
                       0x95, 0x01, 0x81, 0x02, 0x05, 0x01, 0x05, 0x09, 0x19, 0x01, 0x29, 0x01,
                       0x15, 0x00, 0x25, 0x01, 0x75, 0x01, 0x95, 0x01, 0x81, 0x02, 0x05, 0x0D,
                       0x55, 0x0C, 0x66, 0x01, 0x10, 0x47, 0xFF, 0xFF, 0x00, 0x00, 0x27, 0xFF,
                       0xFF, 0x00, 0x00, 0x75, 0x10, 0x95, 0x01, 0x09, 0x56, 0x81, 0x02, 0x85,
                       0x12, 0x09, 0x55, 0x09, 0x59, 0x25, 0x0F, 0x75, 0x08, 0x95, 0x01, 0xB1,
                       0x02, 0x06, 0x00, 0xFF, 0x85, 0x11, 0x09, 0xC5, 0x15, 0x00, 0x26, 0xFF,
                       0x00, 0x75, 0x08, 0x96, 0x00, 0x01, 0xB1, 0x02, 0xC0};
    p_srvc->rpt_map = (uint8_t*)osi_malloc(len);

    uint8_t* pp = desc;
    STREAM_TO_ARRAY(p_srvc->rpt_map, pp, len);
    memcpy(&(p_srvc->rpt_map[512]), quirk, 93);
