...
static void get_iop_device_rpt_map(tBTA_HH_LE_HID_SRVC* p_srvc, uint16_t* len, uint8_t* desc) {
  static const uint8_t remaining_report[] = {
  ...
  *len = *len + sizeof(remaining_report);
  p_srvc->rpt_map = (uint8_t*)osi_malloc(*len);
  STREAM_TO_ARRAY(p_srvc->rpt_map, desc, GATT_MAX_ATTR_LEN);
  memcpy(&(p_srvc->rpt_map[GATT_MAX_ATTR_LEN]), remaining_report, sizeof(remaining_report));
}
void bta_hh_le_save_report_map(tBTA_HH_DEV_CB* p_dev_cb, uint16_t len, uint8_t* desc) {
  ...
  if (len > 0) {
    ...
    if (len == GATT_MAX_ATTR_LEN &&
        interop_match_vendor_product_ids(INTEROP_HID_LONG_REPORT, p_dev_cb->dscp_info.vendor_id,
