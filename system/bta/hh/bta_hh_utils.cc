...
uint8_t bta_hh_find_cb(const RawAddress& bda) {
  ...
  for (xx = 0; xx < BTA_HH_MAX_DEVICE; xx++) {
    ...
    if ((bda == bta_hh_cb.kdev[xx].addr && !bda.IsEmpty())) {
      ...
      LOG_DEBUG("found kdev_cb[%d] hid_handle = %d ", xx,
      ...
      LOG_DEBUG("in_use ? [%d] kdev[%d].hid_handle = %d state = [%d]",
...
#if (BTA_HH_DEBUG == TRUE)
  LOG_DEBUG("bta_hh_find_cb:: index = %d while max = %d", xx,
            BTA_HH_MAX_DEVICE);
#endif
...
void bta_hh_update_di_info(tBTA_HH_DEV_CB* p_cb, uint16_t vendor_id,
                           ...
                           uint8_t ctry_code) {
#if (BTA_HH_DEBUG == TRUE)
  LOG_DEBUG("vendor_id = 0x%2x product_id = 0x%2x version = 0x%2x", vendor_id,
            product_id, version);
#endif
...
#if (BTA_HH_DEBUG == TRUE)
  LOG_DEBUG("subclass = 0x%2x", sub_class);
#endif
...
#if (BTA_HH_DEBUG == TRUE)
      LOG_INFO("bta_hh_tod_spt sub_class:0x%x supported", sub_class);
#endif
      return true;
    }
  }
#if (BTA_HH_DEBUG == TRUE)
  LOG_INFO("bta_hh_tod_spt sub_class:0x%x NOT supported", sub_class);
#endif
  return false;
}
...
#if (BTA_HH_DEBUG == TRUE)
    LOG_DEBUG("bta_hh_dev_handle_to_cb_idx dev_handle = %d index = %d",
              dev_handle, index);
#endif
  } else
...
void bta_hh_trace_dev_db(void) {
  ...
  LOG_DEBUG("bta_hh_trace_dev_db:: Device DB list********************");

  for (xx = 0; xx < BTA_HH_MAX_DEVICE; xx++) {
    LOG_DEBUG("kdev[%d] in_use[%d]  handle[%d] ", xx, bta_hh_cb.kdev[xx].in_use,
              bta_hh_cb.kdev[xx].hid_handle);

    LOG_DEBUG("\t\t\t attr_mask[%04x] state [%d] sub_class[%02x] index = %d",
