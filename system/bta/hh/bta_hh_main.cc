    ...
    if (bta_hh_cb.p_cback != NULL) {
      switch (event) {
        ...
        case BTA_HH_API_OPEN_EVT:
          ...
          /* build cback data */
          cback_data.conn.addr.addrt.bda = ((tBTA_HH_API_CONN*)p_data)->dev_addr.addrt.bda;
        ...
        case BTA_HH_API_MAINT_DEV_EVT:
          ...
          if (p_data->api_maintdev.sub_event == BTA_HH_ADD_DEV_EVT) {
            cback_data.dev_info.addr.addrt.bda = p_data->api_maintdev.dev_addr.addrt.bda;
...
bool bta_hh_hdl_event(const BT_HDR_RIGID* p_msg) {
  ...
  if (p_msg->event == BTA_HH_API_OPEN_EVT) {
    index = bta_hh_find_cb(((tBTA_HH_API_CONN*)p_msg)->dev_addr.addrt.bda);
  } else if (p_msg->event == BTA_HH_API_MAINT_DEV_EVT) {
    /* if add device */
    if (((tBTA_HH_MAINT_DEV*)p_msg)->sub_event == BTA_HH_ADD_DEV_EVT) {
      index = bta_hh_find_cb(((tBTA_HH_MAINT_DEV*)p_msg)->dev_addr.addrt.bda);
    ...
    index = bta_hh_find_cb(((tBTA_HH_CBACK_DATA*)p_msg)->dev_addr.addrt.bda);
  } else {
    index = bta_hh_dev_handle_to_cb_idx((uint8_t)p_msg->layer_specific);
  }
