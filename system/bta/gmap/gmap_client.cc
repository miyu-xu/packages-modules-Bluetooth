...
void GmapClient::DebugDump(int fd) {
  ...
  stream << "GmapClient device: " << addr_ << " enabled: " << IsGmapClientEnabled() << "\n";
  stream << "GmapClient Role Handle: " << role_handle_ << " Role: " << role_ << "\n";
  stream << "GmapClient UGT Handle: " << UGT_feature_handle_ << " UGT Feature: " << UGT_feature_
         << "\n";
...
bool GmapClient::parseGmapRole(uint16_t len, const uint8_t *value) {
  ...
  log::info(", Gaming Audio Service Role:\t Role: %d", role_);
  return true;
}

bool GmapClient::parseUGTFeature(uint16_t len, const uint8_t *value) {
  ...
  log::info(", Gaming Audio Service UGT Feature:\t UGT Feature: %d", UGT_feature_);
  return true;
}
