...
impl Client {
    ...
    fn discover_all_primary_services(&mut self) -> impl Stream<Item = gatt::Service> + '_ {
        self.database
            .primary_services
            .cache_complete_request(self.inner.discover_all_primary_services())
    }
    ...
    fn discover_primary_service_by_service_uuid(
        ...
        self.database
            .primary_services
            .cache_request(
                self.inner.discover_primary_service_by_service_uuid(uuid),
                cache_is_complete,
                mark_cache_complete,
            )
    ...
    fn discover_all_characteristics_of_a_service(
        ...
        cached_service
            .characteristics
            .cache_complete_request(self.inner.discover_all_characteristics_of_a_service(service))
    }
    ...
    fn discover_all_characteristic_descriptors(
        ...
        cached_characteristic.descriptors.cache_complete_request(
            self.inner.discover_all_characteristic_descriptors(characteristic),
        )
    }
}
...
impl<T, U> Cache<T>
where
    T: From<U> + Deref<Target = U>,
    U: Clone + PartialEq + 'static,
{
    fn cache_request<'a>(
        &'a mut self,
        request: impl Stream<Item = U> + 'a,
        is_complete: bool,
        mark_complete: impl FnOnce(&'a mut Self) + 'a,
    ) -> impl Stream<Item = U> + 'a {
        stream!({
