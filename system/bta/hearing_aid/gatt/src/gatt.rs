    ...
    pub fn discover_all_primary_services(&mut self) -> impl Stream<Item = Service> + '_ {
        stream!({
            ...
            loop {
                ...
                {
                    let uuid = if let Ok(crate::att_packets::ServiceAttributeValueUuid16 { uuid }) =
                        attribute_value.try_into()
                    {
                        uuid.into()
                    } else if let Ok(crate::att_packets::ServiceAttributeValueUuid128 { uuid }) =
                        attribute_value.try_into()
                    {
                        uuid.into()
                    } else {
            ...
            loop {
                ...
                } in attribute_data_list.iter()
                {
                    ...
                    }) = attribute_value.try_into()
                    {
                        ...
                        let result =
                            self.bearer.transaction(att::ReadReq { attribute_handle }).await;

                        let attribute_value = match result {
                            Ok(att::ReadRsp { ref attribute_value }) => attribute_value,
                            Err(_) => {
                                // TODO: Handle errors
                                break;
                            }
                        ...
                        let Ok(crate::att_packets::ServiceAttributeValueUuid128 { uuid }) =
                            attribute_value.try_into()
                        else {
                            // TODO: Handle errors
                            break;
                ...
                Ok(att::ReadByTypeRsp {
                    attribute_data_list: [
                        att::AttributeData {
                            ...
                            attribute_value: crate::att_packets::AttributeValue { payload: vec![] },
                        },
                        att::AttributeData {
