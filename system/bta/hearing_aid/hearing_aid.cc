...
constexpr uint16_t MIN_CE_LEN_20MS_CI_LE_AUDIO = 0x0000;
...
HearingAidIsoTrafficCallbacks* iso_traffic_callbacks;
...
class HearingAidImpl : public HearingAid {
 ...
 public:
  ...
  HearingAidImpl(bluetooth::hearing_aid::HearingAidCallbacks* callbacks,
    ...
    IsoManager::GetInstance()->RegisterOnIsoTrafficActiveCallbacks(
        iso_traffic_callbacks);
  }

  void IsoTrafficEventCb(uint16_t event_type, void* data) {
    switch (event_type) {
      /*kIsoEventBigOnCreateCmpl and kIsoEventCigOnCreateCmpl both are value 0*/
  ...
  uint16_t UpdateBleConnParams(const RawAddress& address) {
    ...
    switch (default_data_interval_ms) {
      ...
      case HA_INTERVAL_20_MS:
        ...
        // Because when CIG is connected, controller might not be able to
        // update connection event length successfully.
        // So if CIG is running, we use a small ce length to connect first,
        // then update to a better value later on
        if (is_iso_running) {
          min_ce_len = MIN_CE_LEN_20MS_CI_LE_AUDIO;
          max_ce_len = MIN_CE_LEN_20MS_CI_LE_AUDIO;
...
class HearingAidIsoTrafficCallbacksImpl : public HearingAidIsoTrafficCallbacks {
  void OnIsoTrafficEvent(uint8_t event, void* data) override {
    if (instance) instance->IsoTrafficEventCb(event, data);
  };
};

HearingAidIsoTrafficCallbacksImpl isoTrafficCallbacksImpl;
...
void HearingAid::Initialize(
  ...
  iso_traffic_callbacks = &isoTrafficCallbacksImpl;
