...
static void write_rpt_ctl_cfg_cb(uint16_t conn_id, tGATT_STATUS status,
  ...
  if (status != GATT_SUCCESS) {
    LOG_ERROR("%s: handle= %hu, conn_id=%hu, status= %s, length=%u", __func__,
              handle, conn_id, loghex(static_cast<uint8_t>(status)).c_str(),
              len);
  }
}
...
inline void encoder_state_init() {
  if (encoder_state_left != nullptr) {
    LOG_WARN("%s: encoder already initialized", __func__);
    return;
  }
...
class HearingAidImpl : public HearingAid {
 ...
 public:
  ...
  HearingAidImpl(bluetooth::hearing_aid::HearingAidCallbacks* callbacks,
    ...
    if ((default_data_interval_ms != HA_INTERVAL_10_MS) &&
      ...
      LOG_ERROR("%s: invalid interval= %ums. Overwrriting back to default",
                __func__, default_data_interval_ms);
      default_data_interval_ms = HA_INTERVAL_20_MS;
    }
    VLOG(2) << __func__
    ...
    if (overwrite_min_ce_len) {
      LOG_INFO("%s: Overwrites MIN_CE_LEN=%u", __func__, overwrite_min_ce_len);
    }
  ...
  uint16_t UpdateBleConnParams(const RawAddress& address) {
    ...
    switch (default_data_interval_ms) {
      ...
      case HA_INTERVAL_20_MS:
        ...
        break;
      default:
        LOG_ERROR("%s::Error: invalid default_data_interval_ms=%u", __func__,
  ...
  void OnGattConnected(tGATT_STATUS status, uint16_t conn_id,
    ...
    if (!hearingDevice) {
       ...
       * might happen */
      LOG_WARN("%s Closing connection to non hearing-aid device, address=%s",
               __func__, address.ToStringForLogging().c_str());
      BTA_GATTC_Close(conn_id);
      return;
    }

    LOG_INFO("%s: address=%s, conn_id=%u", __func__,
             address.ToStringForLogging().c_str(), conn_id);
    ...
    if (status != GATT_SUCCESS) {
      ...
      LOG_INFO("%s Failed to connect to Hearing Aid device", __func__);
    ...
    if (controller_get_interface()->supports_ble_2m_phy()) {
      LOG_INFO("%s %s set preferred 2M PHY", __func__,
  ...
  void OnConnectionUpdateComplete(uint16_t conn_id, tBTA_GATTC* p_data) {
    ...
    if (p_data) {
      if (p_data->conn_update.status == 0) {
        ...
        switch (hearingDevice->connection_update_status) {
          case COMPLETED:
            if (!same_conn_interval) {
              LOG_WARN(
                  "%s : Unexpected change. Redo. connection interval=%u, "
                  "expected=%u, conn_id=%u, connection_update_status=%u",
                  __func__, p_data->conn_update.interval,
          ...
          case STARTED:
            if (same_conn_interval) {
              LOG_INFO("%s: Connection update completed. conn_id=%u, device=%s",
                       __func__, conn_id,
                       hearingDevice->address.ToStringForLogging().c_str());
              hearingDevice->connection_update_status = COMPLETED;
            } else {
              LOG_WARN(
                  "%s: Ignored. Different connection interval=%u, expected=%u, "
        ...
        LOG_INFO(
            "%s: error status=%s, conn_id=%u,device=%s, "
        ...
        if (hearingDevice->connection_update_status == STARTED) {
          // Redo this connection interval change.
          LOG_ERROR("%s: Redo Connection Interval change", __func__);
          hearingDevice->connection_update_status = AWAITING;
        }
      }
    } else {
  ...
  void OnReadRssiComplete(const RawAddress& address, int8_t rssi_value) {
    HearingDevice* hearingDevice = hearingDevices.FindByAddress(address);
    if (!hearingDevice) {
      LOG_INFO("%s Skipping unknown device %s", __func__,
               address.ToStringForLogging().c_str());
      return;
    }
    ...
    if (hearingDevice->read_rssi_count <= 0) {
      LOG_ERROR("%s: device=%s, invalid read_rssi_count=%d", __func__,
                address.ToStringForLogging().c_str(),
    ...
    if (hearingDevice->read_rssi_count == READ_RSSI_NUM_TRIES) {
      ...
      LOG_INFO("%s: store time, device=%s, rssi=%d", __func__,
  ...
  void OnEncryptionComplete(const RawAddress& address, bool success) {
    ...
    if (!success) {
      LOG_ERROR("%s encryption failed", __func__);
    ...
    LOG_INFO("%s: %s", __func__, address.ToStringForLogging().c_str());
    ...
    if (hearingDevice->audio_control_point_handle &&
      ...
      LOG_INFO("%s: %s: do BTA_GATTC_ServiceSearchRequest", __func__,
               address.ToStringForLogging().c_str());
  ...
  void OnPhyUpdateEvent(uint16_t conn_id, uint8_t tx_phys, uint8_t rx_phys,
    ...
    if (status != GATT_SUCCESS) {
      LOG_WARN("%s %s phy update fail with status: %hu", __func__,
               hearingDevice->address.ToStringForLogging().c_str(), status);
      return;
    }
    if (tx_phys == PHY_LE_2M && rx_phys == PHY_LE_2M) {
      LOG_INFO("%s %s phy update to 2M successful", __func__,
               hearingDevice->address.ToStringForLogging().c_str());
      return;
    }
    LOG_INFO(
        "%s %s phy update successful but not target phy, try again. tx_phys: "
        "%u,rx_phys: %u",
        __func__, hearingDevice->address.ToStringForLogging().c_str(), tx_phys,
        rx_phys);
  ...
  void OnServiceChangeEvent(const RawAddress& address) {
    ...
    LOG_INFO("%s: address=%s", __func__, address.ToStringForLogging().c_str());
    hearingDevice->first_connection = true;
  ...
  void OnServiceSearchComplete(uint16_t conn_id, tGATT_STATUS status) {
    ...
    if (status != GATT_SUCCESS) {
      ...
      LOG_ERROR("%s Service discovery failed", __func__);
      if (hearingDevice->first_connection) {
    ...
    for (const gatt::Service& tmp : *services) {
      if (tmp.uuid == Uuid::From16Bit(UUID_SERVCLASS_GATT_SERVER)) {
        LOG_INFO("%s Found UUID_SERVCLASS_GATT_SERVER, handle=%s", __func__,
        ...
        LOG_INFO("%s Found Hearing Aid service, handle=%s", __func__,
                 loghex(tmp.handle).c_str());
        service = &tmp;
      }
    }
    ...
    for (const gatt::Characteristic& charac : service->characteristics) {
      if (charac.uuid == READ_ONLY_PROPERTIES_UUID) {
        ...
        if (!hearingDevice->audio_status_ccc_handle) {
          LOG_ERROR("%s: cannot find Audio Status CCC descriptor", __func__);
          continue;
        }

        LOG_INFO("%s: audio_status_handle=%s, ccc=%s", __func__,
        ...
        LOG_WARN("%s Unknown characteristic found:%s", __func__,
  ...
  void ReadPSM(HearingDevice* hearingDevice) {
    if (hearingDevice->read_psm_handle) {
      LOG_INFO("%s Reading PSM %s, device=%s", __func__,
  ...
  void OnNotificationEvent(uint16_t conn_id, uint16_t handle, uint16_t len,
    ...
    if (!device) {
      LOG_INFO("%s: Skipping unknown device, conn_id=%s", __func__,
               loghex(conn_id).c_str());
      return;
    }

    if (device->audio_status_handle != handle) {
      LOG_INFO("%s: Mismatched handle, %s!=%s", __func__,
    ...
    if (len < 1) {
      LOG_ERROR("%s: Data Length too small, len=%u, expecting at least 1",
                __func__, len);
      return;
    }

    if (value[0] != 0) {
      LOG_INFO("%s: Invalid returned status. data=%s", __func__,
               loghex(value[0]).c_str());
      return;
    }

    LOG_INFO("%s: audio status success notification. command_acked=%u",
             __func__, device->command_acked);
  ...
  void OnReadOnlyPropertiesRead(uint16_t conn_id, tGATT_STATUS status,
    ...
    if (version != 0x01) {
      LOG_WARN("%s Unknown version: %s", __func__, loghex(version).c_str());
      return;
    }

    // version 0x01 of read only properties:
    if (len < 17) {
      LOG_WARN("%s Read only properties too short: %s", __func__,
               loghex(len).c_str());
      return;
    }
    ...
    if (capabilities & CAPABILITY_RESERVED) {
      LOG_WARN("%s reserved capabilities are set", __func__);
    }
    ...
    if (!(codecs & (1 << CODEC_G722_16KHZ))) {
      LOG_WARN("%s  Mandatory codec, G722@16kHz not supported", __func__);
    }
  }
  ...
  void OnAudioStatus(uint16_t conn_id, tGATT_STATUS status, uint16_t handle,
    ...
    LOG_INFO("%s %s", __func__, base::HexEncode(value, len).c_str());
  }
  ...
  void OnPsmRead(uint16_t conn_id, tGATT_STATUS status, uint16_t handle,
    ...
    if (status != GATT_SUCCESS) {
      LOG_ERROR("%s Error reading PSM for device %s", __func__,
                hearingDevice->address.ToStringForLogging().c_str());
      return;
    }

    if (len > 2) {
      LOG_ERROR("%s Bad PSM Lengh", __func__);
      return;
    }
  ...
  void ConnectSocket(HearingDevice* hearingDevice, uint16_t psm) {
    ...
    if (gap_handle == GAP_INVALID_HANDLE) {
      LOG_ERROR("%s UNABLE TO GET gap_handle", __func__);
      return;
    }

    hearingDevice->gap_handle = gap_handle;
    LOG_INFO("%s Successfully sent GAP connect request", __func__);
  }
  ...
  void OnDeviceReady(const RawAddress& address) {
    HearingDevice* hearingDevice = hearingDevices.FindByAddress(address);
    if (!hearingDevice) {
      LOG_INFO("%s Device not connected to profile %s", __func__,
               address.ToStringForLogging().c_str());
      return;
    }
    ...
    LOG_INFO("%s: audio_status_handle=%s, audio_status_ccc_handle=%s", __func__,
    ...
    if (register_status != GATT_SUCCESS) {
      LOG_ERROR("%s: BTA_GATTC_RegisterForNotifications failed, status=%s",
                __func__,
    ...
    LOG_INFO("%s: address=%s, hi_sync_id=%s, codec_in_use=%s, audio_running=%i",
             __func__, address.ToStringForLogging().c_str(),
  ...
  void OnAudioSuspend(const std::function<void()>& stop_audio_ticks) {
    ...
    if (!audio_running) {
      LOG_WARN("%s: Unexpected audio suspend", __func__);
    } else {
      LOG_INFO("%s: audio_running=%i", __func__, audio_running);
    }
    audio_running = false;
    stop_audio_ticks();
    ...
    for (auto& device : hearingDevices.devices) {
      ...
      if (!device.playback_started) {
        LOG_WARN("%s: Playback not started, skip send Stop cmd, device=%s",
                 __func__, device.address.ToStringForLogging().c_str());
      } else {
        LOG_INFO("%s: send Stop cmd, device=%s", __func__,
  ...
  void OnAudioResume(const std::function<void()>& start_audio_ticks) {
    ...
    if (audio_running) {
      LOG_ERROR("%s: Unexpected Audio Resume", __func__);
    } else {
      LOG_INFO("%s: audio_running=%i", __func__, audio_running);
    }
    ...
    if (!audio_running) {
      LOG_INFO("%s: No device (0/%d) ready to start", __func__,
               GetDeviceCount());
      return;
    }
  ...
  void SendStart(HearingDevice* device) {
    ...
    if (!audio_running) {
      if (!device->playback_started) {
        LOG_INFO("%s: Skip Send Start since audio is not running, device=%s",
                 __func__, device->address.ToStringForLogging().c_str());
      } else {
        LOG_ERROR("%s: Audio not running but Playback has started, device=%s",
                  __func__, device->address.ToStringForLogging().c_str());
      }
      return;
    }
    ...
    if (device->playback_started) {
      LOG_ERROR("%s: Playback already started, skip send Start cmd, device=%s",
                __func__, device->address.ToStringForLogging().c_str());
    } else {
      start[4] = GetOtherSideStreamStatus(device);
      LOG_INFO(
          "%s: send Start cmd, volume=%s, audio type=%s, device=%s, other side "
          "streaming=%s",
          __func__, loghex(start[3]).c_str(), loghex(start[2]).c_str(),
          device->address.ToStringForLogging().c_str(),
  ...
  static void StartAudioCtrlCallbackStatic(uint16_t conn_id,
                                           ...
                                           void* data) {
    if (status != GATT_SUCCESS) {
      LOG_ERROR("%s: handle=%u, conn_id=%u, status=%s", __func__, handle,
                conn_id, loghex(static_cast<uint8_t>(status)).c_str());
      return;
    }
    if (!instance) {
  ...
  void StartAudioCtrlCallback(uint16_t conn_id) {
    HearingDevice* hearingDevice = hearingDevices.FindByConnId(conn_id);
    if (!hearingDevice) {
      LOG_ERROR("%s Skipping unknown device, conn_id=%s", __func__,
                loghex(conn_id).c_str());
      return;
    }
    LOG_INFO("%s: device: %s", __func__,
             hearingDevice->address.ToStringForLogging().c_str());
  ...
  bool NeedToDropPacket(HearingDevice* target_side, HearingDevice* other_side) {
    ...
    if (target_current_credit == L2CAP_LE_CREDIT_MAX) {
      LOG_ERROR("%s: Get target side credit value fail.", __func__);
      return true;
    }
    ...
    if (other_current_credit == L2CAP_LE_CREDIT_MAX) {
      LOG_ERROR("%s: Get other side credit value fail.", __func__);
      return true;
    }
  ...
  void OnAudioDataReady(const std::vector<uint8_t>& data) {
    ...
    if (num_samples % 2 != 0)
      LOG_ALWAYS_FATAL("%s num_samples is not even: %d", __func__, num_samples);
    ...
    if (left == nullptr && right == nullptr) {
      LOG_WARN("%s: No more (0/%d) devices ready", __func__, GetDeviceCount());
    ...
    if (left) {
      ...
      if (packets_in_chans) {
        ...
        if (NeedToDropPacket(left, right)) {
          LOG_INFO("%s %s triggers dropping, %u packets in channel", __func__,
                   left->address.ToStringForLogging().c_str(),
                   packets_in_chans);
          need_drop = true;
          left->audio_stats.trigger_drop_count++;
        } else {
          LOG_INFO("%s %s skipping %u packets", __func__,
                   left->address.ToStringForLogging().c_str(),
                   packets_in_chans);
  ...
  void SendAudio(uint8_t* encoded_data, uint16_t packet_size,
    ...
    *p = seq_counter;
    ...
    if (result != BT_PASS) {
      LOG_ERROR("%s Error sending data: %s", __func__, loghex(result).c_str());
    }
  }
  ...
  void GapCallback(uint16_t gap_handle, uint16_t event, tGAP_CB_DATA* data) {
    ...
    if (!hearingDevice) {
      LOG_INFO("%s Skipping unknown device, gap_handle=%u", __func__,
               gap_handle);
      return;
    }
    ...
    switch (event) {
      case GAP_EVT_CONN_OPENED: {
        ...
        LOG_INFO("%s GAP_EVT_CONN_OPENED %s, tx_mtu=%u, init_credit=%u",
                 __func__, address.ToStringForLogging().c_str(), tx_mtu,
                 init_credit);
      ...
      case GAP_EVT_CONN_CLOSED:
        LOG_INFO(
            "%s: GAP_EVT_CONN_CLOSED: %s, playback_started=%i, "
            "accepting_audio=%i",
            __func__, hearingDevice->address.ToStringForLogging().c_str(),
      ...
      case GAP_EVT_CONN_DATA_AVAIL: {
        ...
        if (bytes_read < 4) {
          LOG_WARN("%s Wrong data length", __func__);
          return;
        }
  ...
  void DumpRssi(int fd, const HearingDevice& device) {
    ...
    for (auto& rssi_logs : stats->rssi_history) {
      ...
      if (!strftime(temptime, sizeof(temptime), "%H:%M:%S", tstamp)) {
        LOG_ERROR("%s: strftime fails. tm_sec=%d, tm_min=%d, tm_hour=%d",
                  __func__, tstamp->tm_sec, tstamp->tm_min, tstamp->tm_hour);
  ...
  void Disconnect(const RawAddress& address) override {
    ...
    LOG_INFO("%s: %s, playback_started=%i, accepting_audio=%i", __func__,
             hearingDevice->address.ToStringForLogging().c_str(),
             hearingDevice->playback_started, hearingDevice->accepting_audio);
    ...
    LOG_INFO("%s: No more (0/%d) devices ready", __func__, GetDeviceCount());
    DoDisconnectAudioStop();
  }
  ...
  void DoDisconnectCleanUp(HearingDevice* hearingDevice) {
    if (hearingDevice->connection_update_status != COMPLETED) {
      LOG_INFO("%s: connection update not completed. Current=%u, device=%s",
               __func__, hearingDevice->connection_update_status,
               hearingDevice->address.ToStringForLogging().c_str());
    ...
    hearingDevice->accepting_audio = false;
    LOG_INFO("%s: device=%s, playback_started=%i", __func__,
             hearingDevice->address.ToStringForLogging().c_str(),
 ...
 private:
    ...
    for (const gatt::Characteristic& charac : service->characteristics) {
      if (charac.uuid == Uuid::From16Bit(GATT_UUID_GATT_SRV_CHGD)) {
        ...
        if (!hearingDevice->service_changed_ccc_handle) {
          LOG_ERROR("%s: cannot find service changed CCC descriptor", __func__);
          continue;
        }
        LOG_INFO("%s service_changed_ccc=%s", __func__,
  ...
  uint16_t find_ccc_handle(uint16_t conn_id, uint16_t char_handle) {
    ...
    if (!p_char) {
      LOG_WARN("%s: No such characteristic: %u", __func__, char_handle);
      return 0;
    }
  ...
  void send_state_change(HearingDevice* device, std::vector<uint8_t> payload) {
    if (device->conn_id != 0) {
      if (device->service_changed_rcvd) {
        LOG_INFO(
            "%s: service discover is in progress, skip send State Change cmd.",
            __func__);
        return;
      }
      // Send the data packet
      LOG_INFO("%s: Send State Change. device=%s, status=%s", __func__,
               device->address.ToStringForLogging().c_str(),
  ...
  switch (event) {
    ...
    case BTA_GATTC_NOTIF_EVT:
      ...
      if (!p_data->notify.is_notify || p_data->notify.len > GATT_MAX_ATTR_LEN) {
        LOG_ERROR("%s: rejected BTA_GATTC_NOTIF_EVT. is_notify=%i, len=%u",
                  __func__, p_data->notify.is_notify, p_data->notify.len);
        break;
      }
...
void HearingAid::Initialize(
  ...
  if (instance) {
    LOG_ERROR("%s Already initialized!", __func__);
  }
...
void HearingAid::AddFromStorage(const HearingDevice& dev_info,
  ...
  if (!instance) {
    LOG_ERROR("%s Not initialized yet", __func__);
  }
...
int HearingAid::GetDeviceCount() {
  if (!instance) {
    LOG_INFO("%s: Not initialized yet", __func__);
    return 0;
  }
