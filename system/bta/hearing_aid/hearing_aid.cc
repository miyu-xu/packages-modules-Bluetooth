...
class HearingAidImpl : public HearingAid {
 ...
 public:
  ...
  uint16_t UpdateBleConnParams(const RawAddress& address) {
    ...
    if (overwrite_min_ce_len != -1) {
      LOG_WARN("min_ce_len=%u for device %s is overwritten to %d",
               ADDRESS_TO_LOGGABLE_CSTR(address), min_ce_len,
               overwrite_min_ce_len);
      min_ce_len = overwrite_min_ce_len;
    }
    if (overwrite_max_ce_len != -1) {
      LOG_WARN("max_ce_len=%u for device %s is overwritten to %d",
    ...
    LOG_INFO(
        "L2CA_UpdateBleConnParams for device %s min_ce_len:%d "
        "max_ce_len:%d",
