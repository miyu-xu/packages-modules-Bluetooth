...
class HearingAidImpl : public HearingAid {
 ...
 public:
  ...
  void Connect(const RawAddress& address) override {
    ...
    BTA_GATTC_Open(gatt_if, address, BTA_GATT_DIRECT_CONNECTION, false);
  }

  void AddToAcceptlist(const RawAddress& address) override {
    VLOG(2) << __func__ << " address: " << address;
    hearingDevices.Add(HearingDevice(address, true));
    BTA_GATTC_Open(gatt_if, address, BTA_GATT_BKG_CONNECT_ALLOW_LIST, false);
  }
  ...
  void AddFromStorage(const HearingDevice& dev_info, uint16_t is_acceptlisted) {
    ...
    if (is_acceptlisted) {
      ...
      BTA_GATTC_Open(gatt_if, dev_info.address, BTA_GATT_BKG_CONNECT_ALLOW_LIST,
                     false);
    }
