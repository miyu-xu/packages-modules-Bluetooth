  ...
  void send_state_change(HearingDevice* device, std::vector<uint8_t> payload) {
    if (device->conn_id != 0) {
      ...
      log::info("Send State Change. device={}, status={}",
                device->address, loghex(payload[1]));
      BtaGattQueue::WriteCharacteristic(
