...
constexpr uint16_t MIN_CE_LEN_20MS_CI_LE_AUDIO_MUSIC = 0x000C;
...
class HearingAidImpl : public HearingAid {
 ...
 public:
  ...
  HearingAidImpl(bluetooth::hearing_aid::HearingAidCallbacks* callbacks,
    ...
    //    IsoManager::GetInstance()->Start(); // do we need start
  ...
  uint16_t UpdateBleConnParams(const RawAddress& address) {
    ...
    switch (default_data_interval_ms) {
      ...
      case HA_INTERVAL_20_MS:
        LOG_INFO("is_cig_running %d", is_cig_running);
  ...
  void OnDeviceReady(const RawAddress& address) {
    ...
    if (needs_parameter_update) {
      LOG_INFO(
          "L2CA_UpdateBleConnParams min_ce_len:%d "
          "max_ce_len:%d",
          MIN_CE_LEN_20MS_CI_LE_AUDIO_MUSIC, MAX_CE_LEN_20MS_CI);
      L2CA_UpdateBleConnParams(address, CONNECTION_INTERVAL_20MS_PARAM,
                               CONNECTION_INTERVAL_20MS_PARAM, 0x000A,
                               0x0064 /*1s*/, MIN_CE_LEN_20MS_CI_LE_AUDIO_MUSIC,
                               MAX_CE_LEN_20MS_CI);
    }
