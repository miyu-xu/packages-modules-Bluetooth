...
tBTA_HF_CLIENT_CB* bta_hf_client_find_cb_by_rfc_handle(uint16_t handle) {
  for (int i = 0; i < HF_CLIENT_MAX_DEVICES; i++) {
    ...
    LOG_DEBUG("%s: cb rfc_handle %d alloc %d conn_handle %d", __func__, handle,
...
bool bta_hf_client_allocate_handle(const RawAddress& bd_addr,
  ...
  for (int i = 0; i < HF_CLIENT_MAX_DEVICES; i++) {
    ...
    *p_handle = client_cb->handle;
    LOG_DEBUG("%s: marking CB handle %d to true", __func__, client_cb->handle);
...
bool bta_hf_client_hdl_event(const BT_HDR_RIGID* p_msg) {
  LOG_DEBUG("%s: %s (0x%x)", __func__, bta_hf_client_evt_str(p_msg->event),
            p_msg->event);
...
void bta_hf_client_sm_execute(uint16_t event, tBTA_HF_CLIENT_DATA* p_data) {
  ...
  if (client_cb->state == BTA_HF_CLIENT_OPEN_ST) {
    LOG_INFO("HF Client evt : State %d (%s), Event 0x%04x (%s)",
             client_cb->state, bta_hf_client_state_str(client_cb->state), event,
  ...
  if (in_state != client_cb->state) {
    ...
    if (client_cb->state == BTA_HF_CLIENT_INIT_ST) {
      ...
      LOG_DEBUG("%s: marking CB handle %d to false", __func__,
                client_cb->handle);
...
void bta_hf_client_slc_seq(tBTA_HF_CLIENT_CB* client_cb, bool error) {
  LOG_DEBUG("bta_hf_client_slc_seq cmd: %u", client_cb->at_cb.current_cmd);
  ...
  switch (client_cb->at_cb.current_cmd) {
    ...
    default: {
      ...
      LOG_ERROR("HFPClient: Failed to create SLCdue to unexpected AT command, "
                "disconnecting (%u)",
                client_cb->at_cb.current_cmd);
