...
void bta_hf_client_rfc_do_open(tBTA_HF_CLIENT_DATA* p_data) {
  ...
  if (status == PORT_SUCCESS) {
    ...
    bta_collect_rfc_metrics_after_port_fail(
            static_cast<tPORT_RESULT>(status), client_cb->sdp.sdp_initiated, client_cb->sdp.status,
            client_cb->peer_addr, 0, BTA_SEC_AUTHENTICATE | BTA_SEC_ENCRYPT, true,
            (client_cb->sdp.sdp_start_ms - client_cb->sdp.sdp_end_ms));
