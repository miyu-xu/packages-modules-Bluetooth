...
static void bta_hf_client_mgmt_cback(uint32_t code, uint16_t port_handle) {
  ...
  LOG_DEBUG("%s: code = %d, port_handle = %d serv = %d", __func__, code,
  ...
  if (code != PORT_SUCCESS && client_cb != NULL &&
    ...
    LOG_DEBUG("bta_hf_client_mgmt_cback ignoring handle:%d", port_handle);
    return;
  }
  ...
  if (code == PORT_SUCCESS) {
    if (client_cb && port_handle == client_cb->conn_handle) { /* out conn */
      ...
      LOG_DEBUG("%s: allocating a new CB for incoming connection", __func__);
...
void bta_hf_client_start_server() {
  ...
  if (bta_hf_client_cb_arr.serv_handle > 0) {
    LOG_DEBUG("%s: already started, handle: %d", __func__,
  ...
  LOG_DEBUG("%s: started rfcomm server with handle %d", __func__,
            bta_hf_client_cb_arr.serv_handle);

  if (port_status == PORT_SUCCESS) {
    bta_hf_client_setup_port(bta_hf_client_cb_arr.serv_handle);
  } else {
    LOG_DEBUG("%s: RFCOMM_CreateConnection returned error:%d", __func__,
              port_status);
  }
}
...
void bta_hf_client_close_server() {
  LOG_DEBUG("%s: %d", __func__, bta_hf_client_cb_arr.serv_handle);

  if (bta_hf_client_cb_arr.serv_handle == 0) {
    LOG_DEBUG("%s: already stopped", __func__);
    return;
  }
...
void bta_hf_client_rfc_do_open(tBTA_HF_CLIENT_DATA* p_data) {
  ...
  if (RFCOMM_CreateConnectionWithSecurity(
    ...
    LOG_DEBUG("bta_hf_client_rfc_do_open : conn_handle = %d",
