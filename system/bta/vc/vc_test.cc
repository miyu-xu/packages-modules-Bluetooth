...
public:
  ...
  MOCK_METHOD((void), OnExtAudioInStatusChanged,
              (const RawAddress& address, uint8_t ext_input_id, uint8_t status), (override));
  MOCK_METHOD((void), OnExtAudioInTypeChanged,
              (const RawAddress& address, uint8_t ext_input_id, uint8_t type), (override));
...
TEST_F(VolumeControlCallbackTest, test_audio_input_status_changed) {
  ...
  EXPECT_CALL(*callbacks, OnExtAudioInStatusChanged(test_address, 1, 0x03));
...
TEST_F(VolumeControlValueGetTest, test_get_ext_audio_in_status) {
  ...
  EXPECT_CALL(*callbacks, OnExtAudioInStatusChanged(test_address, 2, 0x01));
  cb(conn_id, GATT_SUCCESS, handle, (uint16_t)value.size(), value.data(), cb_data);
}
...
TEST_F(VolumeControlValueGetTest, test_get_ext_audio_in_type) {
  ...
  EXPECT_TRUE(cb);
  std::vector<uint8_t> value({0x04});
  EXPECT_CALL(*callbacks, OnExtAudioInTypeChanged(test_address, 1, 0x04));
  cb(conn_id, GATT_SUCCESS, handle, (uint16_t)value.size(), value.data(), cb_data);
}
