...
TEST_F(VolumeControlCallbackTest, test_audio_input_state_changed__muted) {
  std::vector<uint8_t> value({0x03, (uint8_t)MuteField::MUTED, 0x02, 0x04});
  EXPECT_CALL(*callbacks, OnExtAudioInStateChanged(test_address, 1, 0x03, 0x02, MuteField::MUTED));
  GetNotificationEvent(0x0032, value);
}

TEST_F(VolumeControlCallbackTest, test_audio_input_state_changed__disabled) {
  std::vector<uint8_t> value({0x03, (uint8_t)MuteField::DISABLED, 0x02, 0x04});
  EXPECT_CALL(*callbacks,
              OnExtAudioInStateChanged(test_address, 1, 0x03, 0x02, MuteField::DISABLED));
  GetNotificationEvent(0x0032, value);
}
...
TEST_F(VolumeControlValueGetTest, test_get_ext_audio_in_state) {
  ...
  EXPECT_TRUE(cb);
  std::vector<uint8_t> value({0x01, (uint8_t)MuteField::NOT_MUTED, 0x02, 0x03});
  EXPECT_CALL(*callbacks,
              OnExtAudioInStateChanged(test_address, 1, 0x01, 0x02, MuteField::NOT_MUTED));
  cb(conn_id, GATT_SUCCESS, handle, (uint16_t)value.size(), value.data(), cb_data);
}
