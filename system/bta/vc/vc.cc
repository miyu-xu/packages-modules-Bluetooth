...
public:
  ...
  void OnCharacteristicValueChanged(uint16_t conn_id, tGATT_STATUS status, uint16_t handle,
    ...
    if (!device) {
      bluetooth::log::info("unknown conn_id={:#x}", conn_id);
      return;
    }
  ...
  void OnVolumeControlStateReadOrNotified(VolumeControlDevice* device, uint16_t len, uint8_t* value,
                                          bool is_notification) {
    if (len != 3) {
      bluetooth::log::info("{}, malformed len={:#x}", device->address, len);
      return;
    }
  ...
  void OnExtAudioOutStateChanged(VolumeControlDevice* device, VolumeOffset* offset, uint16_t len,
                                 uint8_t* value) {
    if (len != 3) {
      bluetooth::log::info("{}, id={:#x}, malformed len={:#x}", device->address, offset->id, len);
      return;
    }
    ...
    bluetooth::log::info("{}, len:{}", device->address, base::HexEncode(value, len));
    bluetooth::log::info("id={:#x} offset: {:#x} counter: {:#x}", offset->id, offset->offset,
  ...
  void OnExtAudioOutLocationChanged(VolumeControlDevice* device, VolumeOffset* offset, uint16_t len,
    ...
    bluetooth::log::info("{}, len:{}", device->address, base::HexEncode(value, len));
    bluetooth::log::info("id={:#x}location {:#x}", offset->id, offset->location);
  ...
  void OnExtAudioInputStateChanged(VolumeControlDevice* device, VolumeAudioInput* input,
    ...
    if (len != 4) {
      bluetooth::log::info("{}, id={}, malformed len={:#x}", device->address, input->id, len);
      return;
    }
  ...
  void OnExtAudioInGainSettingChanged(VolumeControlDevice* device, VolumeAudioInput* input,
    ...
    bluetooth::log::info("{}, len:{}", device->address, base::HexEncode(value, len));
    bluetooth::log::info("id={:#x} gain unit {:#x} gain min {:#x} gain max {:#x}", input->id,
                         input->gain_settings.unit, input->gain_settings.min,
  ...
  void OnGattWriteCcc(uint16_t connection_id, tGATT_STATUS status, uint16_t handle, uint16_t len,
    ...
    VolumeControlDevice* device = volume_control_devices_.FindByConnId(connection_id);
    if (!device) {
      bluetooth::log::info("unknown connection_id={:#x}", connection_id);
  ...
  static void chrc_multi_read_callback_static(uint16_t conn_id, tGATT_STATUS status,
    ...
    while (position != total_len) {
      ...
      if (position + len >= total_len) {
        bluetooth::log::warn(
                "Multi read was too long, value truncated conn_id: 0x{:04x} handle: 0x{:04x}",
                conn_id, hdl);
        break;
      }
