...
TEST_F(VolumeControlDevicesTest, test_control_point_operation) {
  ...
  for (int i = 5; i > 0; i--) {
    ...
    EXPECT_CALL(gatt_queue,
                WriteCharacteristic(connection_id, control_point_handle, data_expected, GATT_WRITE,
                                    _, _));
  }
...
TEST_F(VolumeControlDeviceTest, test_enqueue_initial_requests) {
  ...
  for (auto const& handle_pair : expected_to_read_write) {
    ...
    EXPECT_CALL(gatt_queue,
                WriteDescriptor(_, handle_pair.second, register_for_notification_data, GATT_WRITE,
                                _, _));
