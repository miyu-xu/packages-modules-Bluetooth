...
class MockAudioHalClient : public AudioHalClient {
 public:
  MockAudioHalClient() = default;
  MOCK_METHOD((AudioHalClient::LeAudioSink*), AcquireUnicastSink, (),
              (override));
  MOCK_METHOD((AudioHalClient::LeAudioSource*), AcquireUnicastSource, (),
              (override));
  MOCK_METHOD((AudioHalClient::LeAudioSource*), AcquireBroadcastSource, (),
              (override));
  MOCK_METHOD((void), Release, (AudioHalClient::LeAudioSink*), (override));
  MOCK_METHOD((void), Release, (AudioHalClient::LeAudioSource*), (override));
};

class MockLeAudioSinkHalClient : public AudioHalClient::LeAudioSink {
 public:
  MockLeAudioSinkHalClient() = default;
  MOCK_METHOD((bool), Start,
              (const LeAudioCodecConfiguration& codecConfiguration,
               AudioHalClient::LeAudioSinkCallbacks* audioReceiver),
              (override));
...
class MockLeAudioSourceHalClient : public AudioHalClient::LeAudioSource {
 public:
  MockLeAudioSourceHalClient() = default;
  MOCK_METHOD((bool), Start,
              (const LeAudioCodecConfiguration& codecConfiguration,
               AudioHalClient::LeAudioSourceCallbacks* audioReceiver),
              (override));
  MOCK_METHOD((void), Stop, (), (override));
  MOCK_METHOD((void), ConfirmStreamingRequest, (), (override));
  MOCK_METHOD((void), CancelStreamingRequest, (), (override));
...
class UnicastTestNoInit : public Test {
 protected:
  void SetUpMockAudioHal() {
    ...
    ON_CALL(mock_audio_hal_client_, AcquireUnicastSource)
        .WillByDefault([this]() -> AudioHalClient::LeAudioSource* {
          if (!is_audio_unicast_source_acquired) {
            is_audio_unicast_source_acquired = true;
            return &mock_le_audio_source_hal_client_;
          }

          return nullptr;
        });
    ON_CALL(mock_audio_hal_client_,
            Release(Matcher<AudioHalClient::LeAudioSource*>(_)))
        .WillByDefault([this](AudioHalClient::LeAudioSource* inst) -> void {
          if (is_audio_unicast_source_acquired) {
            is_audio_unicast_source_acquired = false;
          }
        });
    ...
    ON_CALL(mock_audio_hal_client_, AcquireUnicastSink)
        .WillByDefault([this]() -> AudioHalClient::LeAudioSink* {
          if (!is_audio_unicast_sink_acquired) {
            is_audio_unicast_sink_acquired = true;
            return &mock_le_audio_sink_hal_client_;
          }

          return nullptr;
        });
    ON_CALL(mock_audio_hal_client_,
            Release(Matcher<AudioHalClient::LeAudioSink*>(_)))
        .WillByDefault([this](AudioHalClient::LeAudioSink* inst) -> void {
          if (is_audio_unicast_sink_acquired) {
            is_audio_unicast_sink_acquired = false;
          }
        });
  ...
  void SetUp() override {
    ...
    LeAudioClient::SetAudioHalClientForTesting(&mock_audio_hal_client_);
 ...
 protected:
  ...
  void SinkAudioResume(void) {
    ...
    do_in_main_thread(FROM_HERE,
                      base::BindOnce(
                          [](AudioHalClient::LeAudioSourceCallbacks* cb) {
                            cb->OnAudioResume();
                          },
  ...
  void StartStreaming(audio_usage_t usage, audio_content_type_t content_type,
    ...
    if (usage == AUDIO_USAGE_VOICE_COMMUNICATION ||
      ...
      do_in_main_thread(FROM_HERE,
                        base::BindOnce(
                            [](AudioHalClient::LeAudioSinkCallbacks* cb) {
                              cb->OnAudioResume();
                            },
  ...
  MockAudioHalClient mock_audio_hal_client_;
  ...
  AudioHalClient::LeAudioSourceCallbacks* unicast_source_hal_cb_ = nullptr;
  AudioHalClient::LeAudioSinkCallbacks* unicast_sink_hal_cb_ = nullptr;
...
TEST_F(UnicastTest, TwoEarbudsWithSourceSupporting32kHz) {
  ...
  // Audio sessions are started only when device gets active
  EXPECT_CALL(mock_le_audio_source_hal_client_, Start(_, _)).Times(1);
  EXPECT_CALL(mock_le_audio_sink_hal_client_, Start(expected_af_sink_config, _))
      .Times(1);
  LeAudioClient::Get()->GroupSetActive(group_id);
  SyncOnMainLoop();
}
