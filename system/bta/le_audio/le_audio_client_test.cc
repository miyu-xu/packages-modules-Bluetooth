              ...
              for (LeAudioDevice* device = group->GetFirstActiveDevice(); device != nullptr;
                ...
                for (auto& ase : device->ases_) {
                  ...
                  if (ase.direction == bluetooth::le_audio::types::kLeAudioDirectionSink) {
                    group->SetStreamingMetadataContexts(metadata_context_types.sink, ase.direction);
                  } else {
                    group->SetStreamingMetadataContexts(metadata_context_types.source,
                                                        ase.direction);
                  }
                }
              }
...
TEST_F(UnicastTest, TestStreamingContextTypeBehaviour) {
   ...
   * 3. Remove Available Contexts
   * 4. Verify GetAvailableContexts()
   */
