...
protected:
  ...
  void SetUpMockGatt() {
              ...
              do_in_main_thread(
                      ...
                      base::BindOnce(
                              [](std::map<uint16_t, std::unique_ptr<NiceMock<MockDeviceWrapper>>>*
                                ...
                                auto get_char_value_helper =
                                        [&](NiceMock<MockDeviceWrapper>& device, uint16_t handle) {
                                          ...
                                          if (svc == device.services.end()) {
                                            return std::make_pair(GATT_ERROR,
                                                                  std::vector<uint8_t>());
                                          }
                                ...
                                std::tie(status, value) = get_char_value_helper(*device, handle);
    ...
    ON_CALL(mock_gatt_queue_, ReadMultiCharacteristic(_, _, _, _, _))
            .WillByDefault(Invoke([&](uint16_t conn_id, tBTA_GATTC_MULTI& handles,
              ...
              do_in_main_thread(
                      ...
                      base::BindOnce(
                              [](std::map<uint16_t, std::unique_ptr<NiceMock<MockDeviceWrapper>>>*
                                         peer_devices,
                                 uint16_t conn_id, tBTA_GATTC_MULTI handles, bool variable_len,
                                ...
                                for (int i = 0; i < handles.num_attr; i++) {
                                  ...
                                  std::copy(curr_val.begin(), curr_val.end(),
                                            value.data() + value_end);
                                  value_end += curr_val.size();
                                }
                                cb(conn_id, GATT_SUCCESS, handles, value_end, value.data(),
                                   cb_data);
                              },
                              &peer_devices, conn_id, handles, variable_len, cb, cb_data));
            }));
  }
  ...
  public:
    ...
    struct ascs_mock : public IGattHandlers {
      ...
      MOCK_METHOD((std::pair<GattStatus, std::vector<uint8_t>>), OnGetCharacteristicValue,
                  (uint16_t handle), (override));
      MOCK_METHOD((void), OnWriteCharacteristic,
                  (uint16_t handle, std::vector<uint8_t> value, tGATT_WRITE_TYPE wrtie_type,
                   GATT_WRITE_OP_CB cb, void* cb_data),
                  (override));
    };
