...
stack_config_t mock_stack_config{
  ...
  .get_pts_force_le_audio_multiple_contexts_metadata =
          get_pts_force_le_audio_multiple_contexts_metadata,
  .get_pts_le_audio_disable_ases_before_stopping = get_pts_le_audio_disable_ases_before_stopping,
  .get_all = get_all,
};
...
class UnicastTestNoInit : public Test {
  ...
  void InjectContextTypes(const RawAddress& test_address, uint16_t conn_id, uint16_t handle,
    ...
    std::vector<uint8_t> contexts = { (uint8_t)(sink_ctxs.value()),
                                      (uint8_t)(sink_ctxs.value() >> 8),
                                      (uint8_t)(source_ctxs.value()),
                                      (uint8_t)(source_ctxs.value() >> 8) };
                  ...
                  if (ase.direction == bluetooth::le_audio::types::kLeAudioDirectionSource) {
                    ...
                    if (iter == stream_conf->stream_params.source.stream_locations.end()) {
                      ...
                      log::info(
                              ...
                              "Handle: {}, Audio Channel Allocation: {}, Source "
                              "Number "
                        ...
                        std::remove_if(stream_conf->stream_params.sink.stream_locations.begin(),
                                       ...
                                       [leAudioDevice, &stream_conf](auto& pair) {
                                         ...
                                         if (ases.sink) {
                                           ...
                                           log::info(
                                                   "Sink Number Of Devices: {}, Sink Number "
                                                   "Of "
                                                   "Channels: {}",
                                                   stream_conf->stream_params.sink.num_of_devices,
                                                   stream_conf->stream_params.sink.num_of_channels);
                ...
                stream_conf->stream_params.source.stream_locations.erase(
                        std::remove_if(
                                ...
                                [leAudioDevice, &stream_conf](auto& pair) {
                                  ...
                                  log::info(
                                          ", source to delete. Cis handle: {}, ase "
                                          "pointer: {}",
...
public:
  ...
  void DisconnectLeAudioWithGattClose(
    ...
    do_in_main_thread(FROM_HERE,
                      base::Bind(&LeAudioClient::Disconnect, base::Unretained(LeAudioClient::Get()),
                                 address));
    SyncOnMainLoop();
    Mock::VerifyAndClearExpectations(&mock_btm_interface_);
    Mock::VerifyAndClearExpectations(&mock_gatt_interface_);
  ...
  void UpdateLocalSourceMetadata(std::vector<struct playback_track_metadata> tracks,
    ...
    unicast_source_hal_cb_->OnAudioMetadataUpdate(source_metadata, DsaMode::DISABLED);
  }
  ...
  void SetSampleDatabaseEarbudsValid(
    ...
    if (add_pacs) {
      ...
      ON_CALL(*peer_devices.at(conn_id)->pacs, OnReadCharacteristic(_, _, _))
              .WillByDefault([this, conn_id, snk_allocation, src_allocation, sample_freq,
                              sink_channel_cnt, source_channel_cnt,
                              gatt_status](uint16_t handle, GATT_READ_OP_CB cb, void* cb_data) {
                ...
                if (gatt_status == GATT_SUCCESS) {
                  ...
                  if (is_ase_sink_request) {
                    value = {
                      ...
                      // State
                      static_cast<uint8_t>(
                              bluetooth::le_audio::types::AseState::BTA_LE_AUDIO_ASE_STATE_IDLE),
...
RawAddress GetTestAddress(uint8_t index) {
  ...
  RawAddress result = {
    { 0xC0, 0xDE, 0xC0, 0xDE, 0x00, index }
  };
  return result;
}
...
TEST_F_WITH_FLAGS(UnicastTestHealthStatus, ConnectOneEarbudEmpty_withHealthStatus,
...
TEST_F_WITH_FLAGS(UnicastTestHealthStatus, ConnectOneEarbudNoPacs_withHealthStatus,
                  REQUIRES_FLAGS_ENABLED(ACONFIG_FLAG(TEST_BT,
                                                      leaudio_enable_health_based_actions))) {
  const RawAddress test_address0 = GetTestAddress(0);
  SetSampleDatabaseEarbudsValid(1, test_address0, codec_spec_conf::kLeAudioLocationStereo,
...
TEST_F_WITH_FLAGS(UnicastTestHealthStatus, ConnectOneEarbudNoAscs_withHealthStatus,
                  REQUIRES_FLAGS_ENABLED(ACONFIG_FLAG(TEST_BT,
                                                      leaudio_enable_health_based_actions))) {
  const RawAddress test_address0 = GetTestAddress(0);
  SetSampleDatabaseEarbudsValid(1, test_address0, codec_spec_conf::kLeAudioLocationStereo,
...
TEST_F_WITH_FLAGS(UnicastTestHealthStatus, ConnectOneEarbudWithInvalidCsis_withHealthStatus,
                  REQUIRES_FLAGS_ENABLED(ACONFIG_FLAG(TEST_BT,
                                                      leaudio_enable_health_based_actions))) {
  const RawAddress test_address0 = GetTestAddress(0);
  SetSampleDatabaseEarbudsValid(1, test_address0, codec_spec_conf::kLeAudioLocationStereo,
...
TEST_F_WITH_FLAGS(UnicastTestHealthStatus, ConnectOneEarbudDisable_withHealthStatus,
                  REQUIRES_FLAGS_ENABLED(ACONFIG_FLAG(TEST_BT,
                                                      leaudio_enable_health_based_actions))) {
  const RawAddress test_address0 = GetTestAddress(0);
  int conn_id = 1;
...
TEST_F_WITH_FLAGS(UnicastTestHealthStatus, ConnectOneEarbudConsiderDisabling_withHealthStatus,
                  REQUIRES_FLAGS_ENABLED(ACONFIG_FLAG(TEST_BT,
                                                      leaudio_enable_health_based_actions))) {
  const RawAddress test_address0 = GetTestAddress(0);
  int conn_id = 1;
  ...
  EXPECT_CALL(mock_storage_load, Call()).WillOnce([&]() {
    do_in_main_thread(
            ...
            base::Bind(&LeAudioClient::AddFromStorage, test_address0, autoconnect,
                       ...
                       codec_spec_conf::kLeAudioLocationFrontLeft, 0xff, 0xff, std::move(handles),
...
TEST_F_WITH_FLAGS(UnicastTest, GroupSetActive_CurrentCodecSentOfActive,
  ...
  btle_audio_codec_config_t output_config = { .codec_type = LE_AUDIO_CODEC_INDEX_SOURCE_LC3,
                                              ...
                                              .frame_duration =
                                                      LE_AUDIO_FRAME_DURATION_INDEX_10000US,
...
TEST_F(UnicastTest, TwoEarbudsStopConversational_StartStreamSonification) {
  ...
  types::BidirectionalPair<types::AudioContexts> contexts = {
    .sink = types::AudioContexts(initial_context), .source = types::AudioContexts(initial_context)
  };
  EXPECT_CALL(mock_state_machine_, StartStream(_, initial_context, contexts, _)).Times(1);
...
TEST_F(UnicastTest, TwoEarbudsStreamingContextSwitchReconfigure) {
  ...
  types::BidirectionalPair<types::AudioContexts> contexts = {
    .sink = types::AudioContexts(types::LeAudioContextType::MEDIA), .source = types::AudioContexts()
  };
  EXPECT_CALL(mock_state_machine_,
...
TEST_F(UnicastTest, TwoReconfigureAndVerifyEnableContextType) {
  ...
  EXPECT_CALL(mock_state_machine_,
              StartStream(_, types::LeAudioContextType::CONVERSATIONAL, conversiational_contexts,
                          ccids))
          .Times(AtLeast(1));
...
TEST_F(UnicastTest, StartStreamToUnsupportedContextTypeUsingUnspecified) {
  ...
  BidirectionalPair<AudioContexts> contexts = { .sink = types::AudioContexts(
                                                        types::LeAudioContextType::UNSPECIFIED),
