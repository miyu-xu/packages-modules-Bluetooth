...
//  return
//  std::vector<::le_audio::set_configurations::AudioSetConfiguration>(0);
//}
//}  // namespace le_audio
//}  // namespace audio
//}  // namespace bluetooth
...
class UnicastTestNoInit : public Test {
 protected:
  void SetUpMockAudioHal() {
    ...
    //// Broadcast Source
    // ON_CALL(mock_broadcast_audio_source_, Start(_, _))
    //    .WillByDefault(
    //        [this](const LeAudioCodecConfiguration& codec_configuration,
    //               LeAudioClientAudioSinkReceiver* audioReceiver) {
    //          audio_broadcast_sink_receiver_ = audioReceiver;
    //          return true;
    //        });
    // ON_CALL(mock_broadcast_audio_source_, Acquire).WillByDefault([this]() ->
    // void* {
    //  if (!is_audio_unicast_source_acquired) {
    //    is_audio_unicast_source_acquired = true;
    //    return &mock_broadcast_audio_source_;
    //  }

    //  return nullptr;
    //});
    // ON_CALL(mock_broadcast_audio_source_, Release)
    //    .WillByDefault([this](const void* inst) -> void {
    //      if (is_audio_broadcast_hal_source_acquired) {
    //        is_audio_broadcast_hal_source_acquired = false;
    //      }
    //    });
 ...
 protected:
  ...
  void UpdateMetadata(audio_usage_t usage, audio_content_type_t content_type,
    ...
    if (reconfigure_existing_stream) {
      ...
      EXPECT_CALL(*mock_unicast_audio_source_, SuspendedForReconfiguration()).Times(0);
    }
  ...
  void StartStreaming(audio_usage_t usage, audio_content_type_t content_type,
    ...
    EXPECT_CALL(*mock_unicast_audio_source_, ConfirmStreamingRequest()).Times(1);
    do_in_main_thread(FROM_HERE,
                      base::BindOnce(
