...
class LeAudioClientImpl : public LeAudioClient {
 public:
  ...
  void SetDeviceAsRemovePendingAndStopGroup(LeAudioDevice* leAudioDevice) {
    LOG_INGO("device %s", leAudioDevice->address_.ToString().c_str());
    leAudioDevice->SetConnectionState(DeviceConnectState::REMOVE_PENDING);
  ...
  void HandlePendingDeviceRemove(LeAudioDeviceGroup* group) {
    for (auto device = group->GetFirstDevice(); device != nullptr;
         device = group->GetNextDevice(device)) {
      if (device->GetConnectionState() == DeviceConnectState::REMOVE_PENDING) {
