...
public:
  ...
  void AseInitialStateReadRequest(LeAudioDevice* leAudioDevice) {
    ...
    for (int i = 0; i < ases_num; i++) {
      ...
      if (i != 0 && (i % GATT_MAX_READ_MULTI_HANDLES == 0)) {
        ...
        BtaGattQueue::ReadMultiCharacteristic(leAudioDevice->conn_id_, multi_read, true,
                                              OnGattReadMultiRspStatic, notify_flag_ptr);
        memset(multi_read.handles, GATT_MAX_READ_MULTI_HANDLES, sizeof(uint16_t));
      }
  ...
  static void OnGattReadMultiRspStatic(uint16_t conn_id, tGATT_STATUS status,
    ...
    if (status == GATT_DATABASE_OUT_OF_SYNC) {
      ...
      log::error("Failed to read multiple attributes, status: 0x%02x", +status);
      return;
    }
    ...
    while (position != total_len) {
      ...
      if (position + len >= total_len) {
        log::warn("Multi read was too long, value truncated!");
        break;
      }
