...
class LeAudioClientImpl : public LeAudioClient {
 public:
    ...
    for (int i = 0; i < ases_num; i++) {
      ...
      if (i != 0 && (i % GATT_MAX_READ_MULTI_HANDLES == 0)) {
        ...
        BtaGattQueue::ReadMultiCharacteristic(
            leAudioDevice->conn_id_, multi_read, true, OnGattReadMultiRspStatic,
            notify_flag_ptr);
  ...
  static void OnGattReadMultiRspStatic(uint16_t conn_id, tGATT_STATUS status,
                                       ...
                                       uint16_t total_len, uint8_t* value,
                                       void* data) {
    if (!instance) return;
    ...
    if (status == GATT_DATABASE_OUT_OF_SYNC) {
      ...
      LOG_ERROR("Failed to read multiple attributes, status: 0x%02x", +status);
      return;
    }
    ...
    while (position != total_len) {
      ...
      if (position + len >= total_len) {
        LOG_WARN("Multi read was too long, value truncated!");
        break;
      }
    ...
    if (handles.num_attr - 1 != index) {
      LOG_WARN("Attempted to read %d handles, but received just %d values",
