...
class LeAudioClientImpl : public LeAudioClient {
 public:
  ...
  bool IsCompatibleContextForVoiceBackChannel(LeAudioContextType context_type) {
      ...
      case AudioState::IDLE:
        switch (audio_sender_state_) {
          ...
          case AudioState::STARTED:
            ...
            if (group->GetState() ==
                AseState::BTA_LE_AUDIO_ASE_STATE_STREAMING) {
              if (IsCompatibleContextForVoiceBackChannel(
                      current_context_type_)) {
  ...
  void OnAudioSourceMetadataUpdate(const sink_metadata_t& sink_metadata) {
    ...
    while (track_count) {
      ...
      if (tracks->source != AUDIO_SOURCE_INVALID)
