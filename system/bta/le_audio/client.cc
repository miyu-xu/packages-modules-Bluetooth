...
public:
  ...
  void LeAudioCharValueHandle(tCONN_ID conn_id, uint16_t hdl, uint16_t len, uint8_t* value,
    ...
    if (leAudioDevice->audio_locations_.sink &&
      ...
      /* Disconnect the non-active device. */
      if (!leAudioDevice->IsMemberActive() &&
          leAudioDevice->IsConnected() &&
          !leAudioDevice->HaveAnyCisConnected()) {
  ...
  void OnGattDisconnected(tCONN_ID conn_id, tGATT_IF /*client_if*/, RawAddress address,
    ...
    if (reason == GATT_CONN_TERMINATE_LOCAL_HOST) {
      if (leAudioDevice->autoconnect_flag_ && leAudioDevice->IsMemberActive()) {
    ...
    if (leAudioDevice->autoconnect_flag_) {
      if (group->IsAnyDeviceConnected() && leAudioDevice->IsMemberActive()) {
  ...
  void OnDeviceDetachedFromStream(LeAudioDevice* leAudioDevice) {
    ...
    if (group->GetTargetState() == bluetooth::le_audio::types::AseState::BTA_LE_AUDIO_ASE_STATE_IDLE) {
      return;
    }
