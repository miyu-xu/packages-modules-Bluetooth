...
class LeAudioClientImpl : public LeAudioClient {
 public:
  ...
  std::vector<int16_t> other_channel_cache;
  uint32_t other_channel_timestamp = 0;
  uint32_t other_channel_is_left;
  ...
  void SendAudioData(uint8_t* data, uint16_t size, uint16_t cis_conn_hdl,
    ...
    if (cis_conn_hdl == left_cis_handle) {
      ...
      LOG(ERROR) << __func__ << "Received data for unknown handle: "
                 << loghex(cis_conn_hdl);
      return;
    }
    ...
    if (left_cis_handle && right_cis_handle) {
      ...
      // AF == Audio Framework
      if (other_channel_timestamp == 0) {
        /* First packet received, cache it. We need both channel data to send it
         * to AF. */
        other_channel_cache = pcm_data_decoded;
        other_channel_timestamp = timestamp;
        other_channel_is_left = is_left;
      } else {
        /* We received either data for the other audio channel, or another
         * packet for same channel */

        if (other_channel_is_left != is_left) {
          /* It's data for the 2nd channel */
          if (timestamp == other_channel_timestamp) {
            /* Ready to mix data and send out to AF */
            if (is_left)
              SendAudioDataToAF(true, af_is_stereo, &other_channel_cache,
                                &pcm_data_decoded);
            else
              SendAudioDataToAF(true, af_is_stereo, &pcm_data_decoded,
                                &other_channel_cache);

            other_channel_timestamp = 0;
          } else {
            ...
            if (other_channel_is_left)
              SendAudioDataToAF(false, af_is_stereo, &other_channel_cache,
                                nullptr);
            else
              SendAudioDataToAF(false, af_is_stereo, nullptr,
                                &other_channel_cache);
          ...
          /* Data for same channel received. 2nd chanel is down/not sending data
           */
      ...
      SendAudioDataToAF(false, af_is_stereo, &pcm_data_decoded, nullptr);
    }
  }
