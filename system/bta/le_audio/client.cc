...
class LeAudioClientImpl : public LeAudioClient {
 public:
  ...
  void SendAudioData(uint8_t* data, uint16_t size, uint16_t cis_conn_hdl,
    ...
    // AF == Audio Framework
    if (other_channel_timestamp_ == 0) {
      /* First packet received, cache it. We need both channel data to send it
       * to AF. */
      other_channel_cache_ = pcm_data_decoded;
      other_channel_timestamp_ = timestamp;
      other_channel_is_left_ = is_left;
    } else {
      /* We received either data for the other audio channel, or another
       * packet for same channel */

      if (other_channel_is_left_ != is_left) {
        /* It's data for the 2nd channel */
        if (timestamp == other_channel_timestamp_) {
          ...
          if (is_left) {
            ...
            SendAudioDataToAF(true /* bt_got_stereo */, af_is_stereo,
                              &pcm_data_decoded, &other_channel_cache_);
          }

          other_channel_timestamp_ = 0;
        } else {
          ...
          if (other_channel_is_left_) {
            SendAudioDataToAF(false /* bt_got_stereo */, af_is_stereo,
                              &other_channel_cache_, nullptr);
          } else {
            SendAudioDataToAF(false /* bt_got_stereo */, af_is_stereo, nullptr,
                              &other_channel_cache_);
          }
 ...
 private:
  ...
  std::vector<int16_t> other_channel_cache_;
  uint32_t other_channel_timestamp_ = 0;
  uint32_t other_channel_is_left_;
