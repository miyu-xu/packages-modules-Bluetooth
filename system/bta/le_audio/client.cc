...
public:
  ...
  static void OnGattReadMultiRspStatic(uint16_t conn_id, tGATT_STATUS status,
    ...
    if (status != GATT_SUCCESS) {
      log::error("Failed to read multiple attributes, conn_id: 0x{:04x}, status: 0x{:02x}", conn_id, +status);
      return;
    }
    ...
    while (position != total_len) {
      ...
      if (position + len >= total_len) {
        log::warn("Multi read was too long, value truncated conn_id: 0x{:04x} handle: 0x{:04x}", conn_id, hdl);
        break;
      }
