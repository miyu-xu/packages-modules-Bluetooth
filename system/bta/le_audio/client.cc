...
public:
  ...
  BidirectionalPair<AudioContexts> DirectionalRealignMetadataAudioContexts(
     ...
     */
    const auto detect_game_on_local_source_for_microphone_only_devices = true;
    bool take_unresumed_local_source_metadata =
            detect_game_on_local_source_for_microphone_only_devices &&
    ...
    if (is_other_direction_bidir) {
      if (!(is_streaming_other_direction || is_releasing_for_reconfiguration_other_direction) &&
          !take_unresumed_local_source_metadata) {
        log::debug(
