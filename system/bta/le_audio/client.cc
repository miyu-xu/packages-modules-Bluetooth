...
AudioHalClient::LeAudioSourceCallbacks* audioSinkReceiver;
AudioHalClient::LeAudioSinkCallbacks* audioSourceReceiver;
AudioHalClient* audioHalClient;
...
class LeAudioClientImpl : public LeAudioClient {
 public:
  ...
  void UpdateContextAndLocations(LeAudioDeviceGroup* group,
    ...
    auto contexts_updated = group->UpdateGroupAudioContextTypeAvailability(
    ...
    if (contexts_updated || location_update) {
      callbacks_->OnAudioConf(group->audio_directions_, group->group_id_,
                              ...
                              group->GetGroupAvailableContexts().value());
    }
  }
  ...
  void group_add_node(const int group_id, const RawAddress& address,
    ...
    if (aseGroups_.FindById(old_group_id) != nullptr) {
      ...
      auto contexts_updated =
          old_group->UpdateGroupAudioContextTypeAvailability(
              old_group->GetGroupAvailableContexts());
      ...
      if (group_conf_changed) {
        callbacks_->OnAudioConf(old_group->audio_directions_, old_group_id,
                                ...
                                old_group->GetGroupAvailableContexts().value());
      }
    }
  ...
  void group_remove_node(LeAudioDeviceGroup* group, const RawAddress& address,
    ...
    bool contexts_updated = group->UpdateGroupAudioContextTypeAvailability(
        group->GetGroupAvailableContexts());
  ...
  bool GroupStream(const int group_id, LeAudioContextType context_type,
    ...
    if (!group->GetGroupAvailableContexts().test(context_type)) {
  ...
  void GroupSetActive(const int group_id) override {
    ...
    if (!le_audio_source_hal_client_) {
      le_audio_source_hal_client_ = audioHalClient->AcquireUnicastSource();
      if (!le_audio_source_hal_client_) {
    ...
    if (!le_audio_sink_hal_client_) {
      le_audio_sink_hal_client_ = audioHalClient->AcquireUnicastSink();
      if (!le_audio_sink_hal_client_) {
        LOG(ERROR) << __func__ << ", could not acquire audio sink interface";
        audioHalClient->Release(le_audio_source_hal_client_);
        return;
      }
    }
  ...
  void LeAudioCharValueHandle(uint16_t conn_id, uint16_t hdl, uint16_t len,
    ...
    if (snk_pac_ent != leAudioDevice->snk_pacs_.end()) {
       ...
       */
      if (group && group->UpdateGroupAudioContextTypeAvailability(
                       leAudioDevice->GetAvailableContexts())) {
        callbacks_->OnAudioConf(group->audio_directions_, group->group_id_,
                                group->snk_audio_locations_.to_ulong(),
                                group->src_audio_locations_.to_ulong(),
      ...
      if (updated_avail_contexts.any()) {
        ...
        if (group) {
          ...
          if (group->IsInTransition() ||
            ...
            group->SetPendingGroupAvailableContextsChange(
  ...
  void OnLeAudioSourceMetadataUpdate(
    ...
    metadata_context_types_ |= GetAllowedAudioContextsFromSourceMetadata(
        source_metadata, group->GetGroupAvailableContexts());
  ...
  void OnLeAudioSinkMetadataUpdate(
  ...
  void HandlePendingAvailableContextsChange(LeAudioDeviceGroup* group) {
    ...
    auto contexts = group->GetPendingGroupAvailableContextsChange();
    if (contexts.any()) {
      auto success = group->UpdateGroupAudioContextTypeAvailability(contexts);
      if (success) {
        callbacks_->OnAudioConf(group->audio_directions_, group->group_id_,
                                group->snk_audio_locations_.to_ulong(),
                                group->src_audio_locations_.to_ulong(),
      ...
      group->ClearPendingGroupAvailableContextsChange();
    }
  }
  ...
  void updateOffloaderIfNeeded(LeAudioDeviceGroup* group) {
    ...
    if (stream_conf->sink_offloader_changed || stream_conf->sink_is_initial) {
      ...
      CodecManager::GetInstance()->UpdateActiveSourceAudioConfig(
          *stream_conf, remote_delay_ms,
          std::bind(&AudioHalClient::LeAudioSource::UpdateAudioConfigToHal,
                    le_audio_source_hal_client_, std::placeholders::_1));
    ...
    if (stream_conf->source_offloader_changed ||
      ...
      CodecManager::GetInstance()->UpdateActiveSinkAudioConfig(
          *stream_conf, remote_delay_ms,
          std::bind(&AudioHalClient::LeAudioSink::UpdateAudioConfigToHal,
                    le_audio_sink_hal_client_, std::placeholders::_1));
 ...
 private:
  ...
  AudioHalClient::LeAudioSink* le_audio_sink_hal_client_;
  ...
  void ClientAudioIntefraceRelease() {
    ...
    if (le_audio_sink_hal_client_) {
      ...
      audioHalClient->Release(le_audio_sink_hal_client_);
      le_audio_sink_hal_client_ = nullptr;
    }
    le_audio::MetricsCollector::Get()->OnStreamEnded(active_group_id_);
  }
...
class SourceCallbacksImpl : public AudioHalClient::LeAudioSourceCallbacks {
 public:
  ...
  void OnAudioMetadataUpdate(
    ...
    if (instance) instance->OnLeAudioSourceMetadataUpdate(source_metadata);
  }
};

class SinkCallbacksImpl : public AudioHalClient::LeAudioSinkCallbacks {
 public:
  void OnAudioSuspend(std::promise<void> do_suspend_promise) override {
  ...
  void OnAudioMetadataUpdate(
    ...
    if (instance) instance->OnLeAudioSinkMetadataUpdate(sink_metadata);
  }
};
...
void LeAudioClient::Initialize(
  ...
  if (!audioHalClient) audioHalClient = AudioHalClient::Get();
...
void LeAudioClient::DebugDump(int fd) {
  ...
  AudioHalClient::LeAudioSink::DebugDump(fd);
  AudioHalClient::LeAudioSource::DebugDump(fd);
...
void LeAudioClient::SetAudioHalClientForTesting(AudioHalClient* halClient) {
  if (audioHalClient) {
    LOG(WARNING) << __func__ << ", audio hal clients already initialized";
    return;
  }
