...
void LeAudioClient::Initialize(
  ...
  if(GmapServer::IsGmapServerEnabled()){
      ...
      // TODO: may change, now pixel offloader support
      UGG_feature |= static_cast<uint8_t>(bluetooth::gmap::UGGFeatureBitMask::MultiplexFeatureSupport);
      UGG_feature |=static_cast<uint8_t>(bluetooth::gmap::UGGFeatureBitMask::MultisinkFeatureSupport);
      for(auto& capa:capabilities){
          if(capa.sample_rate == bluetooth::le_audio::LE_AUDIO_SAMPLE_RATE_INDEX_48000HZ){
              UGG_feature |= static_cast<uint8_t>(bluetooth::gmap::UGGFeatureBitMask::NinetySixKbpsSourceFeatureSupport);
              break;
          }
      }
