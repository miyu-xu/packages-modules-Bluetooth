...
public:
  ...
  void OnActiveMembersListChangedCb(int group_id, const std::vector<RawAddress>& addresses) {
    ...
    for (LeAudioDevice* leAudioDevice = group->GetFirstDevice(); leAudioDevice != nullptr;
      ...
      bool was_csis_member_active = leAudioDevice->IsCsisMemberActive();

      leAudioDevice->is_csis_active_ = std::find(addresses.begin(), addresses.end(),
                                                 leAudioDevice->address_) != addresses.end();

      if (was_csis_member_active != leAudioDevice->IsCsisMemberActive()) {
        handleDeviceAvailabilityChanged(leAudioDevice, group);
      }
    }
  }
  ...
  void GroupSetActive(const int group_id) override {
    ...
    if (!group->IsAnyCsisMemberActive()) {
      log::error("None of the members is active", static_cast<int>(group_id));
      callbacks_->OnGroupStatus(group_id, GroupStatus::INACTIVE);
      return;
    }
  ...
  void handleDeviceAvailabilityChanged(LeAudioDevice* leAudioDevice, LeAudioDeviceGroup* group) {
    log::debug("{} group_id: {}", leAudioDevice->address_, group->group_id_);
  ...
  void AttachToStreamingGroupIfNeeded(LeAudioDevice* leAudioDevice) {
    ...
    if (group->DesiredSize() <= group->NumOfOngoing()) {
    ...
    if (!leAudioDevice->IsCsisMemberActive()) {
      log::info("{} is inactive CSIS member", leAudioDevice->address_);
      return;
    }
...
public:
  void OnConnectionState(const RawAddress& addr, bluetooth::csis::ConnectionState state) override {
    /* to implement if needed */
  }
  ...
  void OnSetMemberAvailable(const RawAddress& address, int group_id) override {
    /* to implement if needed */
  }
