...
public:
  ...
  void ReconfigureAfterVbcClose() {
    log::debug("active_group_id_: {}", active_group_id_);
  ...
  bool IsReconfigurationTimeoutRunning(
    ...
    if (alarm_is_scheduled(reconfiguration_timeout_)) {
      log::debug(" is {} group_id: {}, to check: {}, scheduled: {}",
                 group_id == reconfiguration_group_ ? "runnning" : " not running", group_id,
  ...
  void StartReconfigurationTimeout(int group_id) {
     ...
     * What we want to achive with this timeout, that BT stack will resume the stream with
    ...
    switch (status) {
      ...
      case GroupStreamStatus::IDLE: {
        ...
        if (group) {
          ...
          if (group->IsPendingConfiguration()) {
            log::info(
                    "Pending confguration for group_id: {} pre_configuration_context_type_ : {} -> "
                    "configuration_context_type_ {}",
            ...
            ApplyRemoteMetadataAudioContextPolicy(group, remote_contexts, remote_direction);
            log::info(
                    "Pending confguration 2 pre_configuration_context_type_ : {} -> "
                    "configuration_context_type_ {}",
