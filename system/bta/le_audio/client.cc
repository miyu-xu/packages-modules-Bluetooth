...
class LeAudioClientImpl : public LeAudioClient {
 public:
  ...
  LeAudioClientImpl(
      ...
      : gatt_if_(0),
        ...
        sink_listener_notified_status_(std::nullopt),
    ...
    if (direction == le_audio::types::kLeAudioDirectionSink) {
      ...
      if (group->IsStreaming()) {
        callbacks_->OnUnicastMonitorModeStatus(
            ...
            StreamMonitoringStatus::STREAMING);
      }
    } else {
  ...
  void Dump(int fd) {
    ...
    if (sink_listener_notified_status_) {
      dprintf(fd, "  Local sink notified state: %d\n",
              sink_listener_notified_status_.value());
    }
  ...
  void notifyAudioLocalSink(StreamMonitoringStatus status) {
    if (sink_listener_notified_status_ != status) {
      LOG_INFO("Stram monitoring status changed to: %d",
               static_cast<int>(status));
      sink_listener_notified_status_ = status;
      callbacks_->OnUnicastMonitorModeStatus(
  ...
  void OnLocalAudioSinkResume() {
    ...
    if (active_group_id_ == bluetooth::groups::kGroupUnknown) {
      if (sink_listening_mode_ && !sink_listener_notified_status_) {
        notifyAudioLocalSink(StreamMonitoringStatus::STREAMING_REQUESTED);
      }
      CancelLocalAudioSinkStreamingRequest();
      return;
    }
 ...
 private:
  ...
  /* Status whis has been notified to Service */
  std::optional<StreamMonitoringStatus> sink_listener_notified_status_;
