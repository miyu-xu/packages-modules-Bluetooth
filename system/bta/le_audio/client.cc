...
class LeAudioClientImpl : public LeAudioClient {
 public:
  ...
  bool IsCacheDecoderChannelsSatisfied(LeAudioDeviceGroup* group) {
    ...
    decoder_expected_sdu_seq_nb = std::nullopt;
  ...
  void SendAudioData(uint8_t* data, uint16_t size, uint16_t cis_conn_hdl,
    ...
    if (decoder_expected_sdu_seq_nb &&
        (seq_nb != decoder_expected_sdu_seq_nb)) {
      LOG(ERROR) << __func__ << ", Not expected SDU came with sequence number: "
                 << static_cast<int>(seq_nb);

      /* Flush cache data, since channel data sequence is broken */
      decoder_expected_sdu_seq_nb = std::nullopt;
      decoder_channels_data_cache.clear();
    ...
    uint32_t audio_locations_per_cis = (*stream_ent).second;
    uint8_t codec_frames_blocks_per_sdu =
        group->stream_conf.source_codec_frames_blocks_per_sdu;

    size_t required_byte_count =
        group->stream_conf.source_octets_per_codec_frame *
        codec_frames_blocks_per_sdu * popcount(audio_locations_per_cis);

    if (required_byte_count < size) {
      LOG(ERROR) << "Insufficient data for decoding and send, required: "
                 << static_cast<int>(required_byte_count)
                 << ", received: " << static_cast<int>(size);
      return;
    }

    if (!CacheDecoderChannelsData(data, group, audio_locations_per_cis,
                                  codec_frames_blocks_per_sdu))
      return;
 ...
 private:
  ...
  std::optional<uint16_t> decoder_expected_sdu_seq_nb;
