...
inline lc3_pcm_format bits_to_lc3_bits(uint8_t bits_per_sample) {
  return bits_per_sample == 16 ? LC3_PCM_FORMAT_S16 : LC3_PCM_FORMAT_S24;
}
...
class LeAudioClientImpl : public LeAudioClient {
 public:
  ...
  void mono_blend(const std::vector<uint8_t>& buf, int bytes_per_sample,
                  size_t frames) {
    ...
    } else if (bytes_per_sample == 3 || bytes_per_sample == 4) {
      int32_t* out = (int32_t*)buf.data();
      for (size_t i = 0; i < frames; ++i) {
        const int32_t* in = out;
        int accum = 0;
        accum += *in++;
        accum += *in++;
        accum /= 2;  // round to 0
  ...
  void PrepareAndSendToTwoDevices(
    ...
    if (!mono) {
      ...
      mono_blend(data, bytes_per_sample,
                 number_of_required_samples_per_channel);
      if (left_cis_handle) {
        lc3_encode(lc3_encoder_left, bits_per_sample, data.data(), 2,
                   chan_left_enc.size(), chan_left_enc.data());
      ...
      if (right_cis_handle) {
        lc3_encode(lc3_encoder_right, bits_per_sample, data.data(), 2,
  ...
  void PrepareAndSendToSingleDevice(
    ...
    if (num_channels == 1) {
      ...
      auto err = lc3_encode(lc3_encoder_left, bits_per_sample, data.data(), 2,
