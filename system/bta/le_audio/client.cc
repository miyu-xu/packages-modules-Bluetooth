...
class LeAudioClientImpl : public LeAudioClient {
 public:
  ...
  void GroupSetActive(const int group_id) override {
     ...
     */
    if (!UpdateConfigAndCheckIfReconfigurationIsNeeded(
            group_id, LeAudioContextType::MEDIA)) {
  ...
  void PrepareAndSendToTwoDevices(
    ...
    if (!mono) {
      ...
      lc3_encode(lc3_encoder_right, ((const int16_t*)data.data()) + 1,
                 2 * pitch, chan_right_enc.data(), chan_right_enc.size());
    } else {
      std::vector<int16_t> chan_left;
      std::vector<int16_t> chan_right;
      ...
      if (left_cis_handle) {
        lc3_encode(lc3_encoder_left, (const int16_t*)chan_left.data(), pitch,
                   chan_left_enc.data(), chan_left_enc.size());
      }

      if (right_cis_handle) {
        lc3_encode(lc3_encoder_right, (const int16_t*)chan_right.data(), pitch,
  ...
  void PrepareAndSendToSingleDevice(
    ...
    if (num_channels == 1) {
      ...
      lc3_encode(lc3_encoder_left, (const int16_t*)chan_mono.data(), pitch,
    ...
    switch (audio_sender_state_) {
      ...
      case AudioState::IDLE:
        switch (audio_receiver_state_) {
          ...
          case AudioState::RELEASING:
          case AudioState::READY_TO_RELEASE:
            /* If group is reconfiguring, just let it be */
