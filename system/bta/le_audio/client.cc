...
class LeAudioClientImpl : public LeAudioClient {
 public:
  ...
  bool CacheDecoderChannelsData(uint8_t* data, LeAudioDeviceGroup* group,
    ...
    for (int block_count = 0; block_count < codec_frames_blocks_per_sdu;
      ...
      for (auto& location :
        ...
        decoder_channels_data_cache[location]
            .insert(decoder_channels_data_cache[location].end(),
            &data[begin_index], &data[end_index]);
        channel_count++;
  ...
  void SendAudioData(uint8_t* data, uint16_t size, uint16_t cis_conn_hdl,
    ...
    std::vector<int16_t> pcm_data_decoded(
        lc3_decoder->lc3Config.NF * current_source_codec_config.num_channels *
            group->stream_conf.source_codec_frames_blocks_per_sdu,
        0);

    if (!DecodeChannelsDataCache(group, pcm_data_decoded)) return;
  ...
  void SuspendAudio(void) {
    ...
    decoder_required_channel_data_size.clear();
  }
    ...
    switch (event_type) {
      case bluetooth::hci::iso_manager::kIsoEventCisDataAvailable: {
        ...
        SendAudioData(event->p_msg->data + event->p_msg->offset,
                      ...
                      event->cis_conn_hdl, event->seq_nb);
      } break;
