...
class LeAudioClientImpl : public LeAudioClient {
  ...
  void OnDeviceAutonomousStateTransitionTimeout(LeAudioDevice* leAudioDevice) {
    log::error("Device {}, failed to complete autonomous transition", leAudioDevice->address_);
  ...
  void GroupSetActive(const int group_id) override {
    ...
    if (previous_active_group == bluetooth::groups::kGroupUnknown) {
       ...
       * the new group so the group change is correctly handled in
  ...
  void OnLocalAudioSourceMetadataUpdate(source_metadata_v7 source_metadata, DsaMode dsa_mode) {
    if (active_group_id_ == bluetooth::groups::kGroupUnknown) {
  ...
  BidirectionalPair<AudioContexts> DirectionalRealignMetadataAudioContexts(
    ...
    BidirectionalPair<AudioContexts> remote_metadata = {
      .sink = local_metadata_context_types_.source, .source = local_metadata_context_types_.sink
    };
  ...
  bool ReconfigureOrUpdateRemote(LeAudioDeviceGroup* group, int remote_direction) {
     ...
     */
    auto no_reconfigure_contexts = LeAudioContextType::NOTIFICATIONS |
            LeAudioContextType::SOUNDEFFECTS | LeAudioContextType::INSTRUCTIONAL |
  ...
  void OnStateMachineStatusReportCb(int group_id, GroupStreamStatus status) {
    ...
    switch (status) {
      case GroupStreamStatus::STREAMING: {
        log::assert_that(group_id == active_group_id_, "invalid group id {}!={}", group_id,
                         active_group_id_);
      ...
      case GroupStreamStatus::SUSPENDING:
        if (active_group_id_ != bluetooth::groups::kGroupUnknown &&
           ...
           * it means that it is some internal state machine error. This is very unlikely
           * and for now just Inactivate the group.
           */
...
class CallbacksImpl : public LeAudioGroupStateMachine::Callbacks {
  ...
  void OnDeviceAutonomousStateTransitionTimeout(LeAudioDevice* leAudioDevice) override {
    if (instance) {
...
class SourceCallbacksImpl : public LeAudioSourceAudioHalClient::Callbacks {
  ...
  void OnAudioMetadataUpdate(source_metadata_v7 source_metadata, DsaMode dsa_mode) override {
    if (instance) {
      instance->OnLocalAudioSourceMetadataUpdate(std::move(source_metadata), dsa_mode);
    }
  }
};
