...
class LeAudioClientImpl : public LeAudioClient {
 public:
  ...
  LeAudioClientImpl(
    ...
    if (bluetooth::common::InitFlags::IsLeAudioHealthBaseActionsEnabled()) {
   ...
   * called and will veirfy if the remote device has a group_id properly set.
  ...
  void LeAudioHealthSendRecommendation(const RawAddress& address, int group_id,
    ...
    if (address != RawAddress::kEmpty &&
      ...
      callbacks_->OnHealthBaseRecommendationAction(address, action);
    }

    if (group_id != bluetooth::groups::kGroupUnknown &&
        aseGroups_.FindById(group_id)) {
      callbacks_->OnHealthBaseGroupRecommendationAction(group_id, action);
    }
  }
