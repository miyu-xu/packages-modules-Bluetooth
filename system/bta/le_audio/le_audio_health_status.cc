...
using le_audio::LeAudioRecomendationActionCb;
...
class LeAudioHealthStatusImpl : public LeAudioHealthStatus {
 public:
  ...
  void RegisterCallback(LeAudioRecomendationActionCb cb) override {
  ...
  void AddStatisticForDevice(const RawAddress& address,
    ...
    if (dev->latest_recomendation_ != action) {
      dev->latest_recomendation_ = action;
      send_recomendation_for_device(address, action);
      return;
    }
  }
  ...
  void AddStatisticForGroup(int group_id,
    ...
    switch (type) {
      case LeAudioHealthGroupStatType::STREAM_CREATE_SUCCESS:
        ...
        if (group->latest_recomendation_ == LeAudioHealthBasedAction::NONE) {
          return;
        }
        break;
    ...
    if (group->latest_recomendation_ != action) {
      group->latest_recomendation_ = action;
      send_recomendation_for_group(group_id, action);
    }
  }
 ...
 private:
  ...
  std::vector<LeAudioRecomendationActionCb> callbacks_;
  ...
  void send_recomendation_for_device(const RawAddress& address,
                                     LeAudioHealthBasedAction recomendation) {
    LOG_DEBUG("%s, %s", ADDRESS_TO_LOGGABLE_CSTR(address),
              ToString(recomendation).c_str());
    /* Notify new user about known groups */
    for (auto& cb : callbacks_) {
      cb.Run(address, kGroupUnknown, recomendation);
    }
  }

  void send_recomendation_for_group(
      int group_id, const LeAudioHealthBasedAction recomendation) {
    LOG_DEBUG("group_id: %d, %s", group_id, ToString(recomendation).c_str());
    /* Notify new user about known groups */
    for (auto& cb : callbacks_) {
      cb.Run(RawAddress::kEmpty, group_id, recomendation);
    }
  }
  ...
  void register_callback(LeAudioRecomendationActionCb cb) {
