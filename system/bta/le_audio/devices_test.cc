...
class LeAudioAseConfigurationTest : public Test {
 protected:
  void SetUp() override {
    ...
    auto codec_location = ::le_audio::types::CodecLocation::HOST;
  ...
  void SetUpMockCodecManager(::le_audio::types::CodecLocation location) {
    ...
    ON_CALL(*mock_codec_manager_, GetCodecConfig)
        .WillByDefault(
            Invoke([](::le_audio::types::LeAudioContextType ctx_type) {
              return ::le_audio::AudioSetConfigurationProvider::Get()
  ...
  bool TestGroupAseConfigurationVerdict(
    ...
    for (ase* ase = data.device->GetFirstActiveAse(); ase;
      ...
      if (ase->direction == kLeAudioDirectionSink)
        active_channel_num_snk += config_base.GetAllocatedChannelCount();
      else
        active_channel_num_src += config_base.GetAllocatedChannelCount();
    }
