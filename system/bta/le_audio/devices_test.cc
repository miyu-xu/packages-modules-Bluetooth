...
static constexpr ::le_audio::types::LeAudioCodecId LeAudioCodecIdLc3 = {
...
class PublishedAudioCapabilitiesBuilder {
 public:
  ...
  void Add(const CodecCapabilitySetting& setting,
    ...
    const LeAudioLc3Config config =
        std::get<LeAudioLc3Config>(setting.config());
...
#ifdef OS_ANDROID
static const std::vector<std::tuple<bool, const char*, const char*>>
    kLeAudioSetConfigs = {};
#else
static const std::vector<std::tuple<bool, const char*, const char*>>
    kLeAudioSetConfigs = {
        {true, "audio_set_scenarios.bfbs", "audio_set_scenarios.json"},
        {false, "audio_set_configurations.bfbs",
...
TEST_F(LeAudioAseConfigurationTest, test_reconnection_media) {
  ...
  ASSERT_TRUE(group_snk_audio_location ==
              (::le_audio::codec_spec_conf::kLeAudioLocationFrontLeft |
               ::le_audio::codec_spec_conf::kLeAudioLocationFrontRight));
