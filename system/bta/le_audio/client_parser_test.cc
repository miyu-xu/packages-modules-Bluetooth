...
TEST(LeAudioClientParserTest, testParsePacsNullLtv) {
  ...
  auto codec_spec_caps = pac_recs[0].codec_spec_caps_ltvs->Values();
  ...
  auto ltv_raw = pac_recs[0].codec_spec_caps_ltvs->RawPacket();
  ltv_raw.push_back(0x00);
  ASSERT_EQ(ltv_raw, pac_recs[0].codec_spec_caps);
}
...
TEST(LeAudioClientParserTest, testParsePacsEmptyMeta) {
  ...
  // Validate the raw data from ltv matches the original pac record data buffer
  ASSERT_EQ(pac_recs[0].codec_spec_caps_ltvs->RawPacket(),
            pac_recs[0].codec_spec_caps);
}
...
TEST(LeAudioClientParserTest, testParsePacsMultipleRecords) {
  ...
  auto codec_spec_caps1 = record1.codec_spec_caps_ltvs->Values();
  ...
  // Validate the raw data from ltv matches the original pac record data buffer
  ASSERT_EQ(record1.codec_spec_caps_ltvs->RawPacket(), record1.codec_spec_caps);
  ...
  ASSERT_EQ(record2.codec_spec_caps.size(), 7u);
  ASSERT_EQ(record2.codec_spec_caps_ltvs->Size(), 0u);
  ASSERT_EQ(0, memcmp(record2.codec_spec_caps.data(), value + 28,
                      record2.codec_spec_caps.size()));
...
TEST(LeAudioClientParserTest, testParsePacsVendorCodecRecords) {
  ...
  ASSERT_EQ(pac_recs[0].codec_id.vendor_company_id, 0x0203u);
  ASSERT_EQ(pac_recs[0].codec_id.vendor_codec_id, 0x0405u);
  ASSERT_FALSE(pac_recs[0].codec_spec_caps_ltvs.has_value());
  ASSERT_EQ(0x0Au, pac_recs[0].codec_spec_caps.size());
}
