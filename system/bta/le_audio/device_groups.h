...
class LeAudioDeviceGroup {
 public:
  ...
  inline void SetAllowedContextMask(
    ...
    group_allowed_contexts_mask_ = context_types;
    log::debug(
        "group id: {}, allowed contexts sink: {}, allowed contexts source: "
        "{}",
        group_id_, group_allowed_contexts_mask_.sink.to_string(),
        group_allowed_contexts_mask_.source.to_string());
  }
  ...
  types::AudioContexts GetAllowedContextMask(
    ...
    if (direction < types::kLeAudioDirectionBoth) {
      log::debug(
          ...
          group_id_, group_allowed_contexts_mask_.sink.to_string(),
          group_allowed_contexts_mask_.source.to_string());
      return group_allowed_contexts_mask_.get(direction);
    } else {
      return types::get_bidirectional(group_allowed_contexts_mask_);
    }
  }
 ...
 private:
   ...
   */
  types::BidirectionalPair<types::AudioContexts> group_allowed_contexts_mask_;
