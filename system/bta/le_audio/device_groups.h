...
public:
  ...
  inline void SetStreamingMetadataContexts(types::AudioContexts& metadata, int remote_direction) {
    ...
    if (remote_direction == types::kLeAudioDirectionSink) {
      streaming_metadata_context_type_.sink = metadata;
    } else {
      streaming_metadata_context_type_.source = metadata;
    }
  }
  ...
  types::AudioContexts GetAvailableContexts(int direction = types::kLeAudioDirectionBoth) const {
    ...
    log::debug(
            "group id: {}, streaming context sink: {}, streaming contexts source: {}, available "
            "context sink: {}, available contexts source: {}",
