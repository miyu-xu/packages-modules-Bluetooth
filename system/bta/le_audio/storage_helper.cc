...
bool serializePacs(const le_audio::types::PublishedAudioCapabilities& pacs,
  ...
  LOG_DEBUG("Serialize %d of pac characteristics",
            static_cast<int>(num_of_pacs));
  ...
  for (auto pac_tuple : pacs) {
    ...
    LOG_DEBUG(" Handle: 0x%04x, ccc handle: 0x%04x, pac count: %d", handle,
    ...
    for (const auto& pac : pac_recs) {
      ...
      LOG_DEBUG("Pac size %d", static_cast<int>(pac_len));
      ...
      /* Codec caps */
      LOG_DEBUG("Codec capability size %d",
      ...
      /* Metadata */
      LOG_DEBUG("Metadata size %d", static_cast<int>(pac.metadata.size()));
...
bool deserializePacs(LeAudioDevice* leAudioDevice,
  ...
  while (num_of_pacs_chars--) {
    ...
    LOG_DEBUG(" Handle: 0x%04x, ccc handle: 0x%04x, pac_count: %d",
...
bool DeserializeAses(le_audio::LeAudioDevice* leAudioDevice,
  ...
  LOG_DEBUG("Loading %d Ases", num_of_ases);
  /* sets entries */
  while (num_of_ases--) {
    uint16_t handle;
    uint16_t ccc_handle;
