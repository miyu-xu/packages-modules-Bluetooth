...
void LeAudioDevice::Dump(int fd) {
  ...
  if (conn_id_ != GATT_INVALID_CONN_ID) {
    ...
    if (connecting_actively_) {
      device_state = "Connecting";
    } else if (background_connect_) {
      device_state = "Background connect";
    } else if (known_service_handles_) {
      device_state = "Known device - not connecting";
    } else {
