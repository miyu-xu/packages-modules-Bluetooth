...
uint32_t PickAudioLocation(types::LeAudioConfigurationStrategy strategy,
  ...
  LOG_DEBUG("strategy: %d, locations: 0x%lx, group locations: %lx",
...
void LeAudioDevice::Dump(int fd) {
  ...
  if (ases_.size() > 0) {
    stream << "\n\t== ASEs == \n\t";
    stream
        << "id  active dir     cis_id  cis_handle  sdu  latency retry#  state";
    for (auto& ase : ases_) {
      stream << std::setfill('\xA0') << "\n\t" << std::left << std::setw(4)
             ...
             << std::setw(6) << ase.max_sdu_size << std::left << std::setw(8)
             << ase.max_transport_latency << std::left << std::setw(8)
