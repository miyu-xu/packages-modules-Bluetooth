...
void LeAudioDeviceGroup::UpdateGroupAudioContextTypeAvailability(void) {
  LOG_DEBUG(" group id: %d, active contexts: %s", group_id_,
            group_available_contexts_.to_string().c_str());
  UpdateGroupAudioContextTypeAvailability(group_available_contexts_);
}

/* Returns true if support for any type in the whole group has changed,
 * otherwise false. */
bool LeAudioDeviceGroup::UpdateGroupAudioContextTypeAvailability(
...
void LeAudioDeviceGroup::Dump(int fd, int active_group_id) {
  ...
  stream << "\n    == Group id: " << group_id_
         ...
         << "      group available contexts: " << GetGroupAvailableContexts()
