  ...
  for (const auto& ent : (*audio_set_conf).confs) {
    ...
    for (auto* device = GetFirstDeviceWithActiveContext(context_type);
      ...
      if (device->GetConnectionState() !=
          types::DeviceConnectState::CONNECTED) {
        LOG_WARN(
...
void LeAudioDevice::SetConnectionState(types::DeviceConnectState state) {
  LOG_DEBUG(" %s --> %s",
...
types::DeviceConnectState LeAudioDevice::GetConnectionState(void) {
...
void LeAudioDevices::Add(const RawAddress& address,
                         types::DeviceConnectState state, int group_id) {
