...
bool LeAudioDevice::ConfigureAses(
  ...
  for (; needed_ase && ase; needed_ase--) {
    ...
    if (ase->state != AseState::BTA_LE_AUDIO_ASE_STATE_STREAMING) {
      ...
      auto config_base = LeAudioCodecConfigCore::FromLtvMap(ent.codec.params);
      ase->max_sdu_size = config_base.GetChannelCountPerIsoStream() *
                          config_base.octets_per_codec_frame.value_or(0) *
                          config_base.codec_frames_blocks_per_sdu.value_or(1);
