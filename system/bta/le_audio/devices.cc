...
bool LeAudioDeviceGroup::UpdateAudioSetConfigurationCache(
  ...
  if (context_to_configuration_cache_map.count(ctx_type) != 0) {
    auto [is_invalid, existing_conf] =
        context_to_configuration_cache_map.at(ctx_type);
    update_config = (new_conf != existing_conf);
    /* Just mark it as still valid */
    if (!update_config && is_invalid) {
      context_to_configuration_cache_map.at(ctx_type).first = false;
      return false;
    }
  }

  if (update_config) {
    context_to_configuration_cache_map[ctx_type] = std::pair(false, new_conf);
...
bool LeAudioDeviceGroup::IsAudioSetConfigurationAvailable(
  ...
  if (context_to_configuration_cache_map.count(group_context_type) != 0) {
    return context_to_configuration_cache_map.at(group_context_type).second !=
           nullptr;
  }
  return false;
}
