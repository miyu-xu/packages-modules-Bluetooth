...
void LeAudioDeviceGroup::ClearSinksFromConfiguration(void) {
  ...
  stream_conf.sink_offloader_streams_audio_allocation.clear();
  stream_conf.sink_offloader_streams_connection.clear();
...
void LeAudioDeviceGroup::ClearSourcesFromConfiguration(void) {
  ...
  stream_conf.source_offloader_streams_audio_allocation.clear();
  stream_conf.source_offloader_streams_connection.clear();
...
void LeAudioDeviceGroup::CreateStreamVectorForOffloader(uint8_t direction) {
  ...
  std::vector<std::pair<uint16_t, uint32_t>>*
      offloader_streams_audio_allocation;
  std::vector<std::pair<uint16_t, uint32_t>>* offloader_streams_connection;
  std::string tag;
  ...
  if (direction == le_audio::types::kLeAudioDirectionSource) {
    ...
    offloader_streams_connection =
        &stream_conf.sink_offloader_streams_connection;
    tag = "Sink";
  ...
  if (offloader_streams_audio_allocation->size() > 0) {
  ...
  offloader_streams_audio_allocation->clear();
  offloader_streams_connection->clear();
  *changed_flag = true;
  ...
  *not_all_cises_connected = false;
  ...
  for (auto& cis_entry : cises_) {
    if ((cis_entry.type == CisType::CIS_TYPE_BIDIRECTIONAL ||
      ...
      offloader_streams_audio_allocation->emplace_back(
          std::make_pair(cis_entry.conn_handle, allocation));
      if (downmix_fallback_) {
        offloader_streams_connection->emplace_back(
