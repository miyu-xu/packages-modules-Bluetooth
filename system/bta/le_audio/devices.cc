...
void LeAudioDevice::RegisterPACs(
  ...
  for (const struct types::acs_ac_record& pac : *pac_recs) {
    ...
    if (utils::IsCodecCapabilityUsingLtvFormat(pac.codec_id) &&
        pac.codec_spec_caps_ltvs.has_value()) {
      debug_str << pac.codec_spec_caps_ltvs->ToString(
          "", types::CodecCapabilitiesLtvFormat);
    } else {
      debug_str << base::HexEncode(pac.codec_spec_caps.data(),
                                   pac.codec_spec_caps.size());
    }
    debug_str << "\n\tMetadata: "
...
uint8_t LeAudioDevice::GetSupportedAudioChannelCounts(uint8_t direction) const {
  ...
  for (const auto& pac_tuple : pacs) {
    ...
    for (const auto pac : pac_recs) {
      ...
      auto supported_channel_count_ltv = pac.codec_spec_caps_ltvs->Find(
  ...
  if (pacs.size() > 0) {
    for (auto& pac : pacs) {
      ...
      for (auto& record : std::get<1>(pac)) {
        ...
        if (utils::IsCodecCapabilityUsingLtvFormat(record.codec_id)) {
          stream << record.codec_spec_caps_ltvs->ToString(
              "\t\t\t", types::CodecCapabilitiesLtvFormat);
        } else {
          stream << "\t\t\t"
                 << base::HexEncode(record.codec_spec_caps.data(),
                                    record.codec_spec_caps.size());
        }
        stream << "\t\tMetadata: "
