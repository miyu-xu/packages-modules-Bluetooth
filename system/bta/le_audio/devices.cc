...
bool LeAudioDeviceGroup::UpdateAudioSetConfigAvailability(void) {
  LOG_DEBUG(" group id: %d, available contexts: %s", group_id_,
            group_available_contexts_.to_string().c_str());
  return UpdateAudioSetConfigAvailability(group_available_contexts_);
}

/* Returns true if support for any type in the whole group has changed,
 * otherwise false. */
bool LeAudioDeviceGroup::UpdateAudioSetConfigAvailability(
