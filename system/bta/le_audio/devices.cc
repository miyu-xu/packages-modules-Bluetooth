...
#include "le_audio/client_parser.h"
...
bool LeAudioDevice::IsMemberActive(void) const {
  if (IsDynamicSetMember()) {
    return cas_prop_.test(bluetooth::le_audio::client_parser::cas::kAcceptorPropBitMemberActive);
  }
  return true;
}
...
void LeAudioDevice::Dump(std::stringstream& stream) {
  ...
  stream << "      ● Device address: " << address_.ToRedactedStringForLogging() << ", "
         ...
         << ", available_for_stream: " << IsAvailableForStream()
...
bool LeAudioDevice::ActivateConfiguredAses(
  ...
  if (!IsAvailableForStream()) {
    log::warn("Device {} is not available for stream", address_);
    return false;
  }
