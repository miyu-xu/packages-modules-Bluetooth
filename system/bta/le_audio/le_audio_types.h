...
struct LeAudioLc3Config {
  ...
  uint16_t GetOctetsPerCodecFrame(void) const {
    if (octets_per_codec_frame) return *octets_per_codec_frame;
...
struct CodecCapabilitySetting {
  ...
  /* Channel allocation for codec */
  uint32_t GetConfigChannelAllocation() const;
  /* Octets per one codec frame */
  uint16_t GetConfigOctetsPerCodecFrame() const;
  /* # of frames per one sdu */
  uint32_t GetConfigCodecFramesBlocksPerSdu() const;
