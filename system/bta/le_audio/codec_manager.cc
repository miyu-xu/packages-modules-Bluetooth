 ...
 public:
  ...
  void UpdateActiveAudioConfig(
    ...
    for (auto direction : {le_audio::types::kLeAudioDirectionSink,
      ...
      if (stream_params.get(direction).stream_locations.empty()) return;
      ...
      if (stream_map.is_initial ||
        ...
        unicast_cfg.stream_map = stream_map.streams_map_current;
      }
      // TODO: set the default value 16 for now, would change it if we support
      // mode bits_per_sample
      unicast_cfg.bits_per_sample = 16;
      unicast_cfg.sampling_rate =
          stream_params.get(direction).sample_frequency_hz;
      unicast_cfg.frame_duration =
          stream_params.get(direction).frame_duration_us;
      unicast_cfg.octets_per_frame =
          stream_params.get(direction).octets_per_codec_frame;
      unicast_cfg.blocks_per_sdu =
          stream_params.get(direction).codec_frames_blocks_per_sdu;
      unicast_cfg.peer_delay_ms = delays_ms.get(direction);
  ...
  void UpdateCisConfiguration(const std::vector<struct types::cis>& cises,
    ...
    if (stream_map.streams_map_target.size() == 0) {
