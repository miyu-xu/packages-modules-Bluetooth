 ...
 public:
  ...
  void UpdateSupportedBroadcastConfig(
    ...
    for (const auto& adsp_audio_set_conf : adsp_capabilities) {
      ...
      // Per LC3 spec 3.2.5, bitrate = (8000 * nbytes) / (frame duration in
      // milliseconds)
  ...
  const broadcast_offload_config* GetBroadcastOffloadConfig() {
    ...
    LOG(INFO) << __func__ << " stream_map.size() -> "
              ...
              << " blocks_per_sdu -> "
              << supported_broadcast_config[0].blocks_per_sdu
              << " codec_bitrate -> "
              << supported_broadcast_config[0].codec_bitrate
              << " retransmission_number -> "
              << supported_broadcast_config[0].retransmission_number
