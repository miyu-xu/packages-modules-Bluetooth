...
struct codec_manager_impl {
 public:
  ...
  const AudioSetConfiguration* GetCodecConfig(
      types::LeAudioContextType ctx_type,
    ...
    auto confs = GetSupportedCodecConfigurations(ctx_type);
    return non_vendor_config_matcher(ctx_type, confs);
  }
...
const AudioSetConfiguration* CodecManager::GetCodecConfig(
    types::LeAudioContextType ctx_type,
    std::function<const set_configurations::AudioSetConfiguration*(
        types::LeAudioContextType context_type,
        const set_configurations::AudioSetConfigurations* confs)>
