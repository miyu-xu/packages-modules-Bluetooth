...
struct codec_manager_impl {
 public:
  ...
  bool UpdateActiveUnicastAudioHalClient(
    ...
    if (is_active) {
      if (unicast_local_source_hal_client != nullptr) {
        log::error("Trying to overriding previous source hal client {}",
                   fmt::ptr(unicast_local_source_hal_client));
        return false;
      }

      if (unicast_local_sink_hal_client != nullptr) {
        log::error("Overriding previous sink hal client {}",
      ...
      // TODO determine if use_aidl_extensions should be set here

      return true;
    }

    if (source_unicast_client != unicast_local_source_hal_client) {
    ...
    if (sink_unicast_client != unicast_local_sink_hal_client) {
      log::error("local source session does not match {} != {}",
  ...
  bool UpdateActiveBroadcastAudioHalClient(
    ...
    broadcast_local_source_hal_client = false;
 ...
 private:
  ...
  /* Inidicator to use aidl_extensions feature. It will be used by the codec
   * manager when codec implementation is different then CodecLocation::HOST
   */
  bool use_aidl_extensions = false;
};
