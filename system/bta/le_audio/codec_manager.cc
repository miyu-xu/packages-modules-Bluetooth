 ...
 private:
  ...
  void storeLocalCapa(
    ...
    for (auto adsp : adsp_capabilities) {
      log::debug("{}, number of confs {}", adsp.name, (int)(adsp.confs.size()));
      for (auto conf : adsp.confs) {
        log::debug(
            "codecId: {} dir: {}, dev_cnt: {} ase_cnt: {}, strategy: {}, "
            "sample_freq: {}, interval {}, channel_cnt: {}",
            conf.codec.id.coding_format,
            (conf.direction == types::kLeAudioDirectionSink ? "sink"
                                                            : "source"),
            conf.device_cnt, conf.ase_cnt, getStrategyString(conf.strategy),
