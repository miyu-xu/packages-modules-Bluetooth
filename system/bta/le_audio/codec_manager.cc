...
namespace le_audio {
// The mapping for smapling rate, frame duration, and the QoS config
 ...
 public:
  ...
  void UpdateSupportedBroadcastConfig(
    ...
    for (const auto& adsp_audio_set_conf : adsp_capabilities) {
      ...
      auto it = bcast_high_reliability_qos[broadcast_config.sampling_rate].find(
          broadcast_config.frame_duration);
      if (it !=
          bcast_high_reliability_qos[broadcast_config.sampling_rate].end()) {
        broadcast_config.retransmission_number =
            (it->second).getRetransmissionNumber();
        broadcast_config.max_transport_latency =
            (it->second).getMaxTransportLatency();
        supported_broadcast_config.push_back(broadcast_config);
      } else {
        LOG(ERROR) << __func__
                   << ", Cannot find the correspoding QoS config for the "
                      "sampling_rate: "
                   << broadcast_config.sampling_rate
                   << ", frame_duration: " << broadcast_config.frame_duration;
      }

      LOG(INFO) << __func__ << ", broadcast_config sampling_rate: "
                << broadcast_config.sampling_rate;
    }
  }
  ...
  const broadcast_offload_config* GetBroadcastOffloadConfig() {
    if (supported_broadcast_config.empty()) {
      LOG(ERROR) << __func__ << ": There is no valid broadcast offload config";
      return nullptr;
    }

    LOG(INFO) << __func__ << ", stream_map.size(): "
