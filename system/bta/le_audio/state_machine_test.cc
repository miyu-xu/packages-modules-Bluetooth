  ...
  EXPECT_CALL(mock_callbacks_,
          ...
          .WillOnce([group](int group_id, uint8_t direction) {
            ...
            for (auto const& info : stream_config.stream_map) {
              ...
              ASSERT_EQ(codec_specific::kLc3CodingFormat, info.codec_id.coding_format);
              ASSERT_EQ(0lu, info.codec_id.vendor_company_id);
              ASSERT_EQ(0lu, info.codec_id.vendor_codec_id);
              ASSERT_NE(info.address, RawAddress::kEmpty);
              ASSERT_NE(info.stream_handle, 0);
              ASSERT_NE(info.codec_config.Size(), 0lu);
