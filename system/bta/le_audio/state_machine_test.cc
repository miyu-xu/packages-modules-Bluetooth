...
class MockLeAudioGroupStateMachineCallbacks : public LeAudioGroupStateMachine::Callbacks {
  ...
  MOCK_METHOD((void), OnDeviceAutonomousStateTransitionTimeout, (LeAudioDevice * leAudioDevice),
              (override));
};
...
#define UNIQUE_CIS_CONN_HANDLE(cig_id, cis_index) (cig_id << 8 | cis_index)
  ...
  void InjectCachedConfiguratuibForActiveAses(LeAudioDeviceGroup* group, LeAudioDevice* device) {
    for (auto& ase : device->ases_) {
      if (!ase.active) {
        continue;
      }
  ...
  static void InsertPacRecord(std::vector<types::acs_ac_record>& recs,
...
TEST_F(StateMachineTest, testStreamMultipleMedia_OneMemberHasNoAses) {
   ...
   */
  EXPECT_CALL(gatt_queue,
              WriteCharacteristic(lastDevice->conn_id_, lastDevice->ctp_hdls_.val_hdl, _,
                                  GATT_WRITE_NO_RSP, _, _))
          .Times(3);
...
TEST_F(StateMachineTest, testHandlingAutonomousCodecConfigStateOnConnection) {
   ...
   */
  EXPECT_CALL(gatt_queue,
              WriteCharacteristic(firstDevice->conn_id_, firstDevice->ctp_hdls_.val_hdl, _,
                                  GATT_WRITE_NO_RSP, _, _))
          .Times(4);
...
TEST_F(StateMachineTest, testAutonomousDisableTimeout) {
  ...
  // Check if timeout is fired
  EXPECT_CALL(mock_callbacks_, OnDeviceAutonomousStateTransitionTimeout(lastDevice));
...
TEST_F(StateMachineTest, testAutonomousDisableSuccess) {
  ...
  InjectAseStateNotification(ase_sink, lastDevice, group, ascs::kAseStateQoSConfigured,
  ...
  InjectAseStateNotification(ase_source, lastDevice, group, ascs::kAseStateDisabling,
                             &disabling_params);
  InjectAseStateNotification(ase_source, lastDevice, group, ascs::kAseStateQoSConfigured,
