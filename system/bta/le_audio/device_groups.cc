...
bool LeAudioDeviceGroup::ReloadAudioLocations(void) {
  ...
  for (const auto& device : leAudioDevices_) {
    auto shared_ptr = device.lock();
    if (shared_ptr && shared_ptr->GetConnectionState() == DeviceConnectState::CONNECTED) {
      if (shared_ptr->audio_locations_.sink) {
        updated_audio_locations.sink =
                updated_audio_locations.sink.value_or(0) | shared_ptr->audio_locations_.sink->value;
      }
      if (shared_ptr->audio_locations_.source) {
        updated_audio_locations.source = updated_audio_locations.source.value_or(0) |
                                         shared_ptr->audio_locations_.source->value;
      }
    }
  }
