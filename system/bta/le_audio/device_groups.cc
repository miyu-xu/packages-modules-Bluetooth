...
bool LeAudioDeviceGroup::IsAudioSetConfigurationSupported(
  ...
  for (const auto& ent : (*audio_set_conf).confs) {
    ...
    for (auto* device = GetFirstDevice();
      ...
      if (needed_ase > 0) {
        log::debug("Device has too less ASEs. Still needed ases {}",
                   needed_ase);
        return false;
      }
    ...
    if (required_device_cnt > 0) {
      ...
      log::debug("could not configure all the devices");
      return false;
    }
  }
...
bool LeAudioDeviceGroup::ConfigureAses(
  ...
  for (const auto& ent : (*audio_set_conf).confs) {
    log::debug("Looking for requirements: {},  - {}", audio_set_conf->name,
