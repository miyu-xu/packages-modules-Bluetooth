  ...
  for (; ase != nullptr; ase = leAudioDevice->GetNextActiveAse(ase)) {
    ...
    if (ase->direction == types::kLeAudioDirectionSink) {
      ...
      if (cis_id == kInvalidCisId) {
        ...
        log::info("ASE ID: {}, assigned Bi-Directional CIS ID: {} for Sink ASE", ase->id,
                  ase->cis_id);
      } else {
        log::info("ASE ID: {}, assigned Uni-Directional CIS ID: {} for Sink ASE", ase->id,
                  ase->cis_id);
      }
    ...
    if (cis_id == kInvalidCisId) {
      ...
      log::info("ASE ID: {}, assigned Bi-Directional CIS ID: {} for Source ASE", ase->id,
                ase->cis_id);
    } else {
      log::info("ASE ID: {}, assigned Uni-Directional CIS ID: {} for Source ASE", ase->id,
                ase->cis_id);
    }
