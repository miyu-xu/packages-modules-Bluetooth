...
AudioContexts GetAudioContextsFromSourceMetadata(const source_metadata_v7& source_metadata) {
  ...
  for (size_t i = 0; i < source_metadata.track_count; i++) {
    ...
    if (isMetadataTagPresent(source_metadata.tracks[i].tags, "VX_AOSP_SAMPLESOUND")) {
...
AudioContexts GetAudioContextsFromSinkMetadata(const sink_metadata_v7& sink_metadata) {
    ...
    for (uint8_t fd_bit = codec_spec_conf::kLeAudioCodecFrameDur7500us;
      ...
      if (!capa.HasSupportedAudioChannelCounts()) {
        bluetooth::le_audio::btle_audio_codec_config_t config = {
          .codec_type =
                  utils::translateBluetoothCodecFormatToCodecType(record.codec_id.coding_format),
...
static bool IsCodecConfigSettingSupported(
  ...
  if (utils::IsCodecUsingLtvFormat(codec_id)) {
    log::assert_that(!pac.codec_spec_caps.IsEmpty(),
                     "Codec specific capabilities are not parsed aproprietly.");
