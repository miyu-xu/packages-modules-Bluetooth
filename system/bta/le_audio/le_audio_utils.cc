...
bool IsAseConfigMatchedWithPreferredRequirements(
  ...
  if (ase_confs.empty() || reqs.empty()) {
    return false;
  }

  auto ase_config = ase_confs.at(0).codec.params.GetAsCoreCodecConfig();
  auto req_config = reqs.at(0).params.GetAsCoreCodecConfig();
  ...
  /* Channel counts */
  if (ase_confs.at(0).codec.GetChannelCountPerIsoStream() != channel_cnt_per_ase) {
    log::debug("Ase cfg: Allocated channel count= {:#x}",
               ase_confs.at(0).codec.GetChannelCountPerIsoStream());
