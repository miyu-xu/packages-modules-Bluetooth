...
bool IsAseConfigMatchedWithPreferredRequirements(
        ...
        btle_audio_codec_index_t PreferCodecType) {
  ...
  for (auto i = 0; i < static_cast<int>(reqs.size()); ++i) {
    ...
    if ((ent.codec.id.vendor_codec_id == types::kLeAudioCodingFormatLC3 &&
         PreferCodecType !=
        ...
        (ent.codec.id.vendor_codec_id == types::kLeAudioCodingFormatAptxLe &&
         PreferCodecType !=
           bluetooth::le_audio::btle_audio_codec_index_t::LE_AUDIO_CODEC_INDEX_SOURCE_APTX_LE) ||
