 ...
 private:
  ...
  void CigCreate(LeAudioDeviceGroup* group) {
    ...
    for (struct cis& cis : group->cises_) {
      ...
      max_sdu_size_stom =
        group->GetMaxSduSize(le_audio::types::kLeAudioDirectionSource, cis.id);

      rtn_mtos = group->GetRtn(le_audio::types::kLeAudioDirectionSink, cis.id);
      rtn_stom = group->GetRtn(le_audio::types::kLeAudioDirectionSource, cis.id);

      /* TODO: Add support for CIS specific PHY assignment */
      cis_cfg.phy_mtos = group->GetPhyBitmask(le_audio::types::kLeAudioDirectionSink);
      cis_cfg.phy_stom = group->GetPhyBitmask(le_audio::types::kLeAudioDirectionSource);
