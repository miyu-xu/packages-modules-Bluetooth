...
class LeAudioGroupStateMachineImpl : public LeAudioGroupStateMachine {
 public:
  ...
  void ProcessHciNotifOnCigCreate(LeAudioDeviceGroup* group, uint8_t status,
    ...
    log_history_->AddLogHistory(
        kLogHciEvent, group->group_id_, RawAddress::kEmpty,
        kLogCisCreateOp + "STATUS=" + std::to_string(status));
  ...
  void ProcessHciNotifyOnCigRemoveRecovering(uint8_t status,
    ...
    log_history_->AddLogHistory(
        kLogHciEvent, group->group_id_, RawAddress::kEmpty,
        kLogCigRemoveOp + " STATUS=" + std::to_string(status));
    if (status != HCI_SUCCESS) {
      LOG_ERROR(
  ...
  void ProcessHciNotifSetupIsoDataPath(LeAudioDeviceGroup* group,
    ...
    log_history_->AddLogHistory(
        ...
        kLogSetDataPathOp + "c_h:" + std::to_string(conn_handle) +
  ...
  void ProcessHciNotifRemoveIsoDataPath(LeAudioDeviceGroup* group,
    ...
    log_history_->AddLogHistory(
        kLogHciEvent, group->group_id_, leAudioDevice->address_,
        kLogRemoveDataPathOp + "STATUS=" + std::to_string(status));
    ...
    if (do_disconnect) {
      ...
      log_history_->AddLogHistory(
          ...
          kLogCisDisconnectOp + "c_h:" + std::to_string(conn_hdl));
    }
  }
  ...
  void ProcessHciNotifCisEstablished(
    ...
    log_history_->AddLogHistory(
        kLogHciEvent, group->group_id_, leAudioDevice->address_,
        kLogCisEstablishedOp + "c_h:" + std::to_string(event->cis_conn_hdl) +
  ...
  static void RemoveDataPathByCisHandle(LeAudioDevice* leAudioDevice,
    ...
    LeAudioLogHistory::Get()->AddLogHistory(
        ...
        kLogRemoveDataPathOp + " c_h:" + std::to_string(cis_conn_hdl));
  }
  ...
  void ProcessHciNotifCisDisconnected(
    ...
    log_history_->AddLogHistory(
        kLogHciEvent, group->group_id_, leAudioDevice->address_,
        kLogCisDisconnectedOp + "c_h:" + std::to_string(event->cis_conn_hdl) +
 ...
 private:
  ...
  static void PrepareDataPath(int group_id, const struct ase* ase) {
    ...
    LeAudioLogHistory::Get()->AddLogHistory(
        ...
        kLogSetDataPathOp + "c_h:" + std::to_string(ase->cis_conn_hdl),
        "direction: " + std::to_string(param.data_path_dir));
  ...
  void DisconnectCisIfNeeded(LeAudioDeviceGroup* group,
    ...
    log_history_->AddLogHistory(
        kLogStateMachineTag, group->group_id_, leAudioDevice->address_,
        kLogCisDisconnectOp + "c_h:" + std::to_string(ase->cis_conn_hdl));
  }
