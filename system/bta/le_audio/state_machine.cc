...
using le_audio::types::LeAudioCodecConfigBase;
...
class LeAudioGroupStateMachineImpl : public LeAudioGroupStateMachine {
 ...
 private:
  ...
  void AddCisToStreamConfiguration(LeAudioDeviceGroup* group,
    ...
    // TODO: Support vendor codec parameters
    auto base_config = LeAudioCodecConfigBase::FromLtvMap(ase->codec_config);

    params.num_of_devices++;
    params.num_of_channels += base_config.channel_count;

    if (!base_config.audio_channel_allocation.has_value()) {
      LOG_WARN("ASE has invalid audio location");
    }
    auto ase_audio_channel_allocation =
        base_config.audio_channel_allocation.value_or(0);
    ...
    if (params.sample_frequency_hz == 0) {
      params.sample_frequency_hz = base_config.GetSamplingFrequencyHz();
    } else {
      ASSERT_LOG(
          params.sample_frequency_hz == base_config.GetSamplingFrequencyHz(),
          "sample freq mismatch: %d!=%d", params.sample_frequency_hz,
          base_config.GetSamplingFrequencyHz());
    }

    if (params.octets_per_codec_frame == 0) {
      params.octets_per_codec_frame = *base_config.octets_per_codec_frame;
    } else {
      ASSERT_LOG(
          params.octets_per_codec_frame == *base_config.octets_per_codec_frame,
          "octets per frame mismatch: %d!=%d", params.octets_per_codec_frame,
          *base_config.octets_per_codec_frame);
    }
    ...
    if (params.frame_duration_us == 0) {
      params.frame_duration_us = base_config.GetFrameDurationUs();
    } else {
      ASSERT_LOG(params.frame_duration_us == base_config.GetFrameDurationUs(),
                 "frame_duration_us: %d!=%d", params.frame_duration_us,
                 base_config.GetFrameDurationUs());
    }
