...
class LeAudioGroupStateMachineImpl : public LeAudioGroupStateMachine {
  ...
  void ProcessHciNotifRemoveIsoDataPath(LeAudioDeviceGroup* group, LeAudioDevice* leAudioDevice,
    ...
    if (status != HCI_SUCCESS) {
      log::error("failed to remove ISO data path, reason: 0x{:0x} - continiung stream closing",
                 status);
  ...
  void applyDsaDataPath(LeAudioDeviceGroup* group, LeAudioDevice* leAudioDevice,
    ...
    bluetooth::hci::iso_manager::iso_data_path_params param = {
      ...
      .codec_id_company = bluetooth::le_audio::types::kLeAudioCodecHeadtracking.vendor_company_id,
      .codec_id_vendor = bluetooth::le_audio::types::kLeAudioCodecHeadtracking.vendor_codec_id,
  ...
  static bool CisCreateForDevice(LeAudioDeviceGroup* group, LeAudioDevice* leAudioDevice) {
    ...
    do {
      ...
      uint16_t acl_handle = BTM_GetHCIConnHandle(leAudioDevice->address_, BT_TRANSPORT_LE);
  ...
  static bool CisCreate(LeAudioDeviceGroup* group) {
    ...
    do {
      ...
      do {
        ...
        uint16_t acl_handle = BTM_GetHCIConnHandle(leAudioDevice->address_, BT_TRANSPORT_LE);
        conn_pairs.push_back(
                { .cis_conn_handle = ase->cis_conn_hdl, .acl_conn_handle = acl_handle });
  ...
  static void PrepareDataPath(int group_id, struct ase* ase) {
    bluetooth::hci::iso_manager::iso_data_path_params param = {
      ...
      .codec_id_company = ase->data_path_configuration.isoDataPathConfig.codecId.vendor_company_id,
      .codec_id_vendor = ase->data_path_configuration.isoDataPathConfig.codecId.vendor_codec_id,
  ...
  void ScheduleAutonomousOperationTimer(AseState target_state, LeAudioDevice* leAudioDevice,
                                        struct ase* ase) {
    ase->autonomous_target_state_ = target_state;
    ase->autonomous_operation_timer_ = alarm_new("LeAudioAutonomousOperationTimeout");
    alarm_set_on_mloop(
            ase->autonomous_operation_timer_, kAutonomousTransitionTimeoutMs,
            [](void* data) {
              LeAudioDevice* leAudioDevice = static_cast<LeAudioDevice*>(data);
              instance->state_machine_callbacks_->OnDeviceAutonomousStateTransitionTimeout(
                      leAudioDevice);
            },
            leAudioDevice);
  }
  ...
  void DisconnectCisIfNeeded(LeAudioDeviceGroup* group, LeAudioDevice* leAudioDevice,
                             struct ase* ase) {
    log::debug(
  ...
  void AseStateMachineProcessReleasing(
    ...
    switch (ase->state) {
      ...
      case AseState::BTA_LE_AUDIO_ASE_STATE_QOS_CONFIGURED:
        ...
        if (group->cig.GetState() == CigState::CREATED && group->HaveAllCisesDisconnected() &&
            getDeviceTryingToAttachTheStream(group) == nullptr) {
          RemoveCigForGroup(group);
        }
  ...
  void ProcessAutonomousDisable(LeAudioDevice* leAudioDevice, struct ase* ase) {
    ...
    if (bidirection_ase->state == AseState::BTA_LE_AUDIO_ASE_STATE_QOS_CONFIGURED) {
      /* Bi-direction ASEs are now disabled */
      if ((ase->autonomous_target_state_ == AseState::BTA_LE_AUDIO_ASE_STATE_QOS_CONFIGURED) &&
    ...
    if (!alarm_is_scheduled(bidirection_ase->autonomous_operation_timer_)) {
      ScheduleAutonomousOperationTimer(AseState::BTA_LE_AUDIO_ASE_STATE_QOS_CONFIGURED,
                                       leAudioDevice, bidirection_ase);
    }
  }
};
} // namespace
