...
class LeAudioGroupStateMachineImpl : public LeAudioGroupStateMachine {
 public:
  ...
  void ProcessHciNotifSetupIsoDataPath(LeAudioDeviceGroup* group,
    ...
    AddCisToStreamConfiguration(
        group, ase->cis_conn_hdl, *ase->codec_config.audio_channel_allocation,
        ase->codec_config.channel_count, ase->direction);
  ...
  void ProcessHciNotifRemoveIsoDataPath(LeAudioDeviceGroup* group,
    ...
    if (ases_pair.source &&
      ...
      ases_pair.source->data_path_state =
          AudioStreamDataPathState::CIS_DISCONNECTING;
      RemoveCisFromStreamConfiguration(
          group, leAudioDevice, conn_hdl,
  ...
  void ProcessHciNotifCisDisconnected(
    ...
    if (ases_pair.source) {
      ases_pair.source->data_path_state =
          AudioStreamDataPathState::CIS_ASSIGNED;
      RemoveCisFromStreamConfiguration(
          group, leAudioDevice, event->cis_conn_hdl,
          le_audio::types::kLeAudioDirectionSource);
    }
    group->CigUnassignCis(leAudioDevice);
 ...
 private:
  ...
  void AddCisToStreamConfiguration(LeAudioDeviceGroup* group,
                                   ...
                                   uint8_t channel_count, uint8_t direction) {
    auto* stream_conf = &group->stream_conf;
    auto& streams = (direction == le_audio::types::kLeAudioDirectionSink)
                        ? stream_conf->sink_streams
                        : stream_conf->source_streams;

    LOG_INFO(
        " CIS Connection Handle: %d"
        ", Audio Channel Allocation: %d"
        ", Channel Count: %d"
        ", Direction: %d",
        +cis_conn_hdl, +audio_channel_allocation, +channel_count, +direction);
    ...
    if (iter == streams.end()) {
      streams.emplace_back(
          std::make_pair(cis_conn_hdl, audio_channel_allocation));

      if (direction == le_audio::types::kLeAudioDirectionSink) {
        stream_conf->sink_num_of_devices++;
        stream_conf->sink_num_of_channels += channel_count;

        LOG_INFO(
            " Updated Sink Stream Configuration."
        ...
        stream_conf->source_num_of_channels += channel_count;

        LOG_INFO(
            " Updated Source Stream Configuration."
  ...
  void RemoveCisFromStreamConfiguration(LeAudioDeviceGroup* group,
    ...
    LOG_INFO(
        " CIS Connection Handle: %d"
        ", Direction: %d",
        +cis_conn_hdl, +direction);

    if (streams.empty() == false) {
      streams.erase(
          std::remove_if(
              streams.begin(), streams.end(),
              [leAudioDevice, &cis_conn_hdl, &stream_conf,
               direction](auto& pair) {
                if (!cis_conn_hdl) {
        ...
        if (ase->data_path_state ==
          ...
          RemoveDataPathByCisHandle(leAudioDevice, ase->cis_conn_hdl);
                   ...
                   ase->data_path_state ==
                       AudioStreamDataPathState::CIS_PENDING) {
          RemoveCisFromStreamConfiguration(group, leAudioDevice,
                                           ase->cis_conn_hdl, ase->direction);
          group->CigUnassignCis(leAudioDevice);
