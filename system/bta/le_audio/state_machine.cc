...
using le_audio::types::LeAudioCodecConfigCore;
...
class LeAudioGroupStateMachineImpl : public LeAudioGroupStateMachine {
 ...
 private:
  ...
  void AddCisToStreamConfiguration(LeAudioDeviceGroup* group,
    ...
    params.num_of_channels += config_base.GetChannelCountPerIsoStream();

    if (!config_base.audio_channel_allocation.has_value()) {
      LOG_WARN("ASE has invalid audio location");
    }
    auto ase_audio_channel_allocation =
        config_base.audio_channel_allocation.value_or(0);
    ...
    if (params.sample_frequency_hz == 0) {
      params.sample_frequency_hz = config_base.GetSamplingFrequencyHz();
    } else {
      ASSERT_LOG(
          params.sample_frequency_hz == config_base.GetSamplingFrequencyHz(),
    ...
    if (params.octets_per_codec_frame == 0) {
      params.octets_per_codec_frame = *config_base.octets_per_codec_frame;
    } else {
      ASSERT_LOG(
          params.octets_per_codec_frame == *config_base.octets_per_codec_frame,
    ...
    if (params.frame_duration_us == 0) {
      params.frame_duration_us = config_base.GetFrameDurationUs();
    } else {
      ASSERT_LOG(params.frame_duration_us == config_base.GetFrameDurationUs(),
                 "frame_duration_us: %d!=%d", params.frame_duration_us,
                 config_base.GetFrameDurationUs());
    }
