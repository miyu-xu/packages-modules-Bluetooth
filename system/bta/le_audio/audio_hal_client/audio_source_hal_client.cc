...
class SourceImpl : public AudioHalClient::LeAudioSource {
 public:
  // Interface implementation
  bool Start(const LeAudioCodecConfiguration& codec_configuration,
             AudioHalClient::LeAudioSourceCallbacks* audioReceiver) override;
  void Stop() override;
  ...
  bool Acquire(LeAudioClientInterface* halInterface);
  void Release(LeAudioClientInterface* halInterface);
  ...
  bluetooth::audio::le_audio::LeAudioClientInterface::Sink* halSinkInterface_ =
      nullptr;
  AudioHalClient::LeAudioSourceCallbacks* audioSourceCallbacks_;
  std::mutex audioSourceCallbacksMutex_;
};

SourceImpl* sUnicast_ = nullptr;
SourceImpl* sBroadcast_ = nullptr;
LeAudioClientInterface* sHalInterface = nullptr;

bool SourceImpl::Acquire(LeAudioClientInterface* halInterface) {
...
void SourceImpl::Release(LeAudioClientInterface* halInterface) {
...
bool SourceImpl::OnResumeReq(bool start_media_task) {
  ...
  bt_status_t status = do_in_main_thread(
      FROM_HERE,
      base::BindOnce(&AudioHalClient::LeAudioSourceCallbacks::OnAudioResume,
...
bool SourceImpl::OnSuspendReq() {
  ...
  bt_status_t status = do_in_main_thread(
      FROM_HERE,
      base::BindOnce(&AudioHalClient::LeAudioSourceCallbacks::OnAudioSuspend,
                     base::Unretained(audioSourceCallbacks_),
...
bool SourceImpl::OnMetadataUpdateReq(const source_metadata_t& source_metadata) {
  ...
  bt_status_t status = do_in_main_thread(
      FROM_HERE,
      base::BindOnce(
          &AudioHalClient::LeAudioSourceCallbacks::OnAudioMetadataUpdate,
...
AudioHalClient::LeAudioSource* AudioHalClient::AcquireUnicastSource() {
  if (sUnicast_) {
    LOG_ERROR("Already acquired");
    return nullptr;
  }

  /* Get pointer to singleton LE audio client interface */
  if (sHalInterface == nullptr) {
    sHalInterface = LeAudioClientInterface::Get();
    if (sHalInterface == nullptr) {
  ...
  sUnicast_ = new SourceImpl(false);
  if (!sUnicast_->Acquire(sHalInterface)) {
...
AudioHalClient::LeAudioSource* AudioHalClient::AcquireBroadcastSource() {
  if (sBroadcast_) {
    LOG_ERROR("Already acquired");
    return nullptr;
  }
  ...
  sBroadcast_ = new SourceImpl(true);
  if (!sBroadcast_->Acquire(sHalInterface)) {
    LOG_ERROR("Could not acquire Braodcast Source on LE Audio HAL enpoint");
    delete sBroadcast_;
    return nullptr;
  }
...
void AudioHalClient::Release(AudioHalClient::LeAudioSource* instance) {
  if (instance != nullptr) {
    if (instance == sUnicast_) {
      sUnicast_->Release(sHalInterface);
      sUnicast_ = nullptr;
    } else if (instance == sBroadcast_) {
      sBroadcast_->Release(sHalInterface);
      sBroadcast_ = nullptr;
    }
    delete instance;
  }
...
AudioHalClient* AudioHalClient::Get() {
  static AudioHalClient* interface = nullptr;
  if (interface == nullptr) interface = new AudioHalClient();

  return interface;
}

void AudioHalClient::DebugDump(int fd) {
