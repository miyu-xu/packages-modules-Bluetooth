...
class SourceImpl : public LeAudioBroadcastSourceAudioHalClient {
 ...
 public:
  ...
  bool Start(const LeAudioCodecConfiguration& codec_configuration,
             LeAudioBroadcastSourceAudioHalClient::Callbacks* audioReceiver,
  ...
  void UpdateAudioConfigToHal(
      const ::bluetooth::le_audio::stream_config& config) override;
  ...
  bluetooth::audio::le_audio::LeAudioClientInterface::Sink* halSinkInterface_ =
      nullptr;
  LeAudioBroadcastSourceAudioHalClient::Callbacks* audioSourceCallbacks_ = nullptr;
...
void SourceImpl::Release() {
  ...
  if (halSinkInterface_) {
    ...
    halSinkInterface_->CleanupBroadcast();
    auto halInterface = audio::le_audio::LeAudioClientInterface::Get();
    if (halInterface != nullptr) {
      halInterface->ReleaseBroadcastSink(halSinkInterface_);
    } else {
      log::error("Can't get LE Audio HAL interface");
    }
...
bool SourceImpl::OnResumeReq(bool /*start_media_task*/) {
  ...
  bt_status_t status =
    do_in_main_thread(base::BindOnce(&LeAudioBroadcastSourceAudioHalClient::Callbacks::OnAudioResume,
...
bool SourceImpl::OnSuspendReq() {
  ...
  bt_status_t status =
      do_in_main_thread(base::BindOnce(&LeAudioBroadcastSourceAudioHalClient::Callbacks::OnAudioSuspend,
                     audioSourceCallbacks_->weak_factory_.GetWeakPtr()));
  if (status == BT_STATUS_SUCCESS) {
    return true;
  }
...
bool SourceImpl::OnMetadataUpdateReq(
  ...
  bt_status_t status = do_in_main_thread(base::BindOnce(
          &LeAudioBroadcastSourceAudioHalClient::Callbacks::OnAudioMetadataUpdate,
...
std::unique_ptr<LeAudioBroadcastSourceAudioHalClient>
LeAudioBroadcastSourceAudioHalClient::AcquireUnicast() {
...
std::unique_ptr<LeAudioBroadcastSourceAudioHalClient>
LeAudioBroadcastSourceAudioHalClient::AcquireBroadcast() {
...
void LeAudioBroadcastSourceAudioHalClient::DebugDump(int fd) {
