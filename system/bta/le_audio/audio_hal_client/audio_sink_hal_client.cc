...
class SinkImpl : public AudioHalClient::LeAudioSink {
 public:
  // Interface implementation
  bool Start(const LeAudioCodecConfiguration& codecConfiguration,
             AudioHalClient::LeAudioSinkCallbacks* audioReceiver) override;
  void Stop();
  ...
  AudioHalClient::LeAudioSinkCallbacks* audioSinkCallbacks_ = nullptr;
};

SinkImpl* sUnicast_ = nullptr;
LeAudioClientInterface* sHalInterface = nullptr;

bool SinkImpl::Acquire(LeAudioClientInterface* halInterface) {
...
void SinkImpl::Release(LeAudioClientInterface* halInterface) {
  ...
  sHalInterface->ReleaseSource(halSourceInterface_);
...
bool SinkImpl::OnResumeReq(bool start_media_task) {
  ...
  bt_status_t status = do_in_main_thread(
      FROM_HERE,
      base::BindOnce(&AudioHalClient::LeAudioSinkCallbacks::OnAudioResume,
...
bool SinkImpl::OnSuspendReq() {
  ...
  bt_status_t status = do_in_main_thread(
      FROM_HERE,
      base::BindOnce(&AudioHalClient::LeAudioSinkCallbacks::OnAudioSuspend,
                     base::Unretained(audioSinkCallbacks_),
...
bool SinkImpl::OnMetadataUpdateReq(const sink_metadata_t& sink_metadata) {
  ...
  bt_status_t status = do_in_main_thread(
      FROM_HERE,
      base::BindOnce(
          &AudioHalClient::LeAudioSinkCallbacks::OnAudioMetadataUpdate,
...
AudioHalClient::LeAudioSink* AudioHalClient::AcquireUnicastSink() {
  if (sUnicast_ != nullptr) {
...
void AudioHalClient::Release(AudioHalClient::LeAudioSink* instance) {
  LOG_INFO();
...
void AudioHalClient::LeAudioSink::DebugDump(int fd) {
