...
class SourceAudioHalAsrc::ClockRecovery : ::bluetooth::hal::NocpIsoHandler {
  ...
  void OnEvent(uint32_t timestamp_us, int num_completed) override {
...

    {
      ...
      min_buffer_level_ = std::min(min_buffer_level_, int(num_produced_ - num_completed_));
    }
    ...
    LOG(INFO) << base::StringPrintf("Deviation: %6d us (%3.0f ppm)",
              ...
              << base::StringPrintf("Buffer level: %d", min_buffer_level) << std::endl;
  }

 public:
  ClockRecovery(unsigned interval_us)
    : interval_(interval_us), num_produced_(0), num_completed_(0),
      min_buffer_level_(INT_MAX), max_buffer_level_(INT_MIN),
  ...
  void UpdateOutputStats(unsigned out_count, double sample_rate, int drift_us) {
    ...
    max_buffer_level_ = std::max(max_buffer_level_, int(num_produced_ - num_completed_));
...
class SourceAudioHalAsrc::Resampler {
  ...
  void Upsample(unsigned ratio,
      const T* in, int in_stride, size_t in_len, size_t* in_count,
      T* out, int out_stride, size_t out_len, size_t* out_count) {
...
inline int32_t SourceAudioHalAsrc::Resampler::Filter(
...
inline int32_t SourceAudioHalAsrc::Resampler::Filter(
  const int32_t *in, const int32_t *h, int16_t mu, const int16_t *d) {
...
SourceAudioHalAsrc::SourceAudioHalAsrc(
    ...
    : sample_rate_(sample_rate), bit_depth_(bit_depth), interval_us_(interval_us),
  ...
  // It needs 3 buffers (one almost full, an entire one, and a last which can be started).
...
template <typename T>
__attribute__((no_sanitize("integer")))
void SourceAudioHalAsrc::Resample(
...
__attribute__((no_sanitize("integer")))
std::vector<const std::vector<uint8_t>*> SourceAudioHalAsrc::Run(
  ...
  clock_recovery_->UpdateOutputStats(
    out.size(), ratio * sample_rate_, int(output_us - local_us));

  if (0)
    LOG(INFO) << base::StringPrintf("[%6u.%06u]  Fs: %.2f Hz  drift: %d us",
                   output_us / (1000*1000), output_us % (1000*1000),
                   ratio * sample_rate_, int(output_us - local_us)) << std::endl;
