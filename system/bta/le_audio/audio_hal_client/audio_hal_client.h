...
class AudioHalClient {
 public:
  class LeAudioSinkCallbacks {
   public:
    virtual ~LeAudioSinkCallbacks() = default;
  ...
  class LeAudioSink {
   ...
   protected:
    LeAudioSink() = default;
    virtual ~LeAudioSink() = default;
  };

  class LeAudioSourceCallbacks {
   public:
    virtual ~LeAudioSourceCallbacks() = default;
  ...
  class LeAudioSource {
   ...
   public:
    virtual bool Start(const LeAudioCodecConfiguration& codecConfiguration,
                       LeAudioSourceCallbacks* audioReceiver) = 0;
    virtual void Stop() = 0;
   ...
   protected:
    LeAudioSource() = default;
    virtual ~LeAudioSource() = default;
  };

  virtual ~AudioHalClient() = default;
  virtual LeAudioSink* AcquireUnicastSink();
  virtual LeAudioSource* AcquireUnicastSource();
  virtual LeAudioSource* AcquireBroadcastSource();
  virtual void Release(LeAudioSource* instance);
  virtual void Release(LeAudioSink* instance);

  static AudioHalClient* Get();
  static void DebugDump(int fd);

 protected:
  AudioHalClient() = default;
};
}  // namespace le_audio
