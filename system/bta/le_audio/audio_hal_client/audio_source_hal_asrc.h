...
class LeAudioSourceAudioHalAsrc {
 ...
 public:
  ...
  // The Asynchronous Sample Rate Conversion (ASRC) is set up from the PCM stream
  // characteristics and the length, expressed in us, of the buffers.
  //
  // A transmission `burst` is proposed, to fulfill the audio pipeline buffering.
  // After an initial delay of `burst_delay_ms`, a burst of `num_burst_buffers`
  // is generated. By experience, it looks like some controllers discard and
  // acknowledge the first packets without following transmission intervals.
  // This behavior leads to trash an initial buffering. The `burst_delay_ms`
  // helps to ensure that the synchronization with the transmission intervals
  // is done.

  LeAudioSourceAudioHalAsrc(
    int channels, int sample_rate, int bit_depth, int interval_us,
