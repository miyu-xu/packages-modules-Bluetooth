...
namespace le_audio {
using LeAudioRecomendationActionCb = base::RepeatingCallback<void(
...
class LeAudioHealthStatus {
 public:
  ...
  virtual void RegisterCallback(LeAudioRecomendationActionCb cb) = 0;
  ...
  struct group_stats {
    group_stats(int group_id)
        : group_id_(group_id),
          latest_recomendation_(LeAudioHealthBasedAction::NONE),
    ...
    int group_id_;
    LeAudioHealthBasedAction latest_recomendation_;
