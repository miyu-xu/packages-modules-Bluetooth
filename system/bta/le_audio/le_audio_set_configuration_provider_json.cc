...
struct AudioSetConfigurationProviderJson {
  AudioSetConfigurationProviderJson() {
    LOG_ASSERT(LoadContent(kLeAudioSetConfigs, kLeAudioSetScenarios))
        << __func__ << ": Unable to load le audio set configuration files.";
  }
 ...
 private:
  ...
  bool LoadConfigurationsFromFiles(const char* schema_file,
    ...
    DLOG(INFO) << __func__ << ": Updating " << flat_configs->size()
               << " config entries.";
  ...
  bool LoadScenariosFromFiles(const char* schema_file,
    ...
    DLOG(INFO) << __func__ << ": Updating " << flat_scenarios->size()
               << " scenarios.";
...
AudioSetConfigurationProvider::GetConfigurations(
  ...
  if (CodecManager::GetInstance()->GetCodecLocation() == types::CodecLocation::ADSP) {
    DLOG(INFO) << __func__
               << "Get offload config for the context type: " << (int)content_type;
  ...
  DLOG(INFO) << __func__
             << "Get software config for the context type: " << (int)content_type;
