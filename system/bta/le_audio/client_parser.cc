...
bool PrepareAseCtpCodecConfig(const std::vector<struct ctp_codec_conf>& confs,
  ...
  for (const struct ctp_codec_conf& conf : confs) {
    ...
    auto codec_spec_conf_len = conf.codec_config.size();
    UINT8_TO_STREAM(msg, codec_spec_conf_len);
    memcpy(msg, conf.codec_config.data(), codec_spec_conf_len);
    msg += codec_spec_conf_len;
...
int ParseSinglePac(std::vector<struct acs_ac_record>& pac_recs, uint16_t len,
  ...
  rec.codec_spec_caps_raw.resize(codec_spec_cap_len);
  memcpy(rec.codec_spec_caps_raw.data(), value, codec_spec_cap_len);
