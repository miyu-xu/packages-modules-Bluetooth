...
std::unique_ptr<::le_audio::broadcaster::BroadcastConfiguration>
CodecManager::GetBroadcastConfig(
    const std::vector<std::pair<::le_audio::types::LeAudioContextType,
                                uint8_t>>& subgroup_quality,
    std::optional<const le_audio::types::PublishedAudioCapabilities*> pacs)
    const {
  if (!pimpl_)
    return std::make_unique<::le_audio::broadcaster::BroadcastConfiguration>(
        ::le_audio::broadcaster::GetBroadcastConfig(subgroup_quality));
