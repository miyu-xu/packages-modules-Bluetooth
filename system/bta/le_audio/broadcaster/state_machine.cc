...
class BroadcastStateMachineImpl : public BroadcastStateMachine {
 public:
  ...
  bool Initialize() override {
    ...
    auto& le_audio_codec_config =
        sm_config_.codec_wrapper.GetLeAudioCodecConfiguration();
    if (le_audio_codec_config.num_channels > sNumBisMax) {
      DLOG(ERROR) << int{le_audio_codec_config.num_channels}
  ...
  bluetooth::le_audio::BroadcastId GetBroadcastId() override {
 ...
 private:
  ...
  const std::array<msg_handler_t, BroadcastStateMachine::STATE_MAX>
      start_msg_handlers{/* in STOPPED state */
                         [this](const void*) { EnableAnnouncement(); },
                         /* in CONFIGURED state */
  ...
  const std::array<msg_handler_t, BroadcastStateMachine::STATE_MAX>
      stop_msg_handlers{
          ...
          /* in CONFIGURED state */
          [this](const void*) { DisableAnnouncement(); },
          /* in STREAMING state */
          [this](const void*) {
  ...
  void CreateAnnouncementCb(uint8_t instance_id, int8_t tx_power,
    ...
    /* If this callback get's called the instance_id is valid even though the
...
std::ostream& operator<<(std::ostream& os,
  ...
  static const char* char_value_[BroadcastStateMachine::MESSAGE_MAX] = {
      "START", "SUSPEND", "STOP"};
...
std::ostream& operator<<(std::ostream& os,
  ...
  static const char* char_value_[BroadcastStateMachine::STATE_MAX] = {
      "STOPPED", "CONFIGURED", "STREAMING"};
  os << char_value_[static_cast<uint8_t>(state)];
  return os;
}
