...
struct BroadcastSubgroupBisCodecConfig {
  BroadcastSubgroupBisCodecConfig(
      ...
      : num_bis(num_bis),
        ...
        vendor_codec_specific(vendor_codec_specific) {}

  bool operator==(const BroadcastSubgroupBisCodecConfig& other) const {
    return (num_bis == other.num_bis) &&
           (codec_specific == other.codec_specific) &&
           (vendor_codec_specific == other.vendor_codec_specific);
  }
  ...
  uint8_t GetNumBis() const { return num_bis; }
  ...
  const std::optional<std::vector<uint8_t>>& GetVendorCodecSpecific() const {
    return vendor_codec_specific;
  }

  bool HasVendorCodecSpecific() const {
    return vendor_codec_specific.has_value();
  }

  uint8_t GetNumChannels() const { return num_bis * GetNumChannelsPerBis(); }

  uint32_t GetSamplingFrequencyHz() const {
    return codec_specific.GetAsCoreCodecConfig().GetSamplingFrequencyHz();
  }

  uint8_t GetNumChannelsPerBis() const {
    return codec_specific.GetAsCoreCodecConfig().GetChannelCountPerIsoStream();
  }
 ...
 private:
  ...
  std::optional<std::vector<uint8_t>> vendor_codec_specific;
};
...
struct BroadcastSubgroupCodecConfig {
  BroadcastSubgroupCodecConfig(
      ...
      std::optional<std::vector<uint8_t>> subgroup_vendor_codec_config =
          std::nullopt)
      : codec_id(codec_id),
        bis_codec_configs(bis_codec_configs),
        subgroup_vendor_codec_config(subgroup_vendor_codec_config),
        bits_per_sample(bits_per_sample) {}

  /* We need this copy-assignment operator as we currently use global copy of
   * a wrapper for the currently active Broadcast. Maybe we should consider
   * using shared pointer instead.
   */
  BroadcastSubgroupCodecConfig& operator=(
      const BroadcastSubgroupCodecConfig& other) {
    codec_id = other.codec_id;
    bis_codec_configs = other.bis_codec_configs;
    bits_per_sample = other.bits_per_sample;
    subgroup_vendor_codec_config = other.subgroup_vendor_codec_config;
    return *this;
  };

  bool operator==(const BroadcastSubgroupCodecConfig& other) const {
    if (subgroup_vendor_codec_config.has_value() !=
        other.subgroup_vendor_codec_config.has_value())
      return false;

    if (subgroup_vendor_codec_config.has_value()) {
      if (subgroup_vendor_codec_config->size() !=
          other.subgroup_vendor_codec_config->size())
        return false;
    }

    if (0 != memcmp(subgroup_vendor_codec_config->data(),
                    other.subgroup_vendor_codec_config->data(),
                    subgroup_vendor_codec_config->size())) {
      return false;
    }

    return (codec_id == other.codec_id) &&
           (bis_codec_configs == other.bis_codec_configs) &&
           (bits_per_sample == other.bits_per_sample);
  }
  ...
  types::LeAudioLtvMap GetCommonBisCodecSpecData() const {
    auto common_ltv = bis_codec_configs[0].GetCodecSpecData();
    for (auto it = bis_codec_configs.begin() + 1; it != bis_codec_configs.end();
         ++it) {
  ...
  std::optional<std::vector<uint8_t>> GetVendorCodecSpecData() const {
    return subgroup_vendor_codec_config;
  }

  std::optional<std::vector<uint8_t>> GetBisVendorCodecSpecData(
      uint8_t bis_idx) const {
    // TODO: Add error logs here if there is no bises at all
    //       OR ASSERT better
    auto config = bis_codec_configs.at(0);
    if ((bis_idx != 0) && (bis_idx < bis_codec_configs.size())) {
      config = bis_codec_configs.at(bis_idx);
    }
  ...
  uint16_t GetBisOctetsPerCodecFrame(uint8_t bis_idx) const {
    ...
    // LOG_ERROR("Could not get the number of octets per codec frame!");
    return 0;
  }
  ...
  std::optional<types::LeAudioLtvMap> GetBisCodecSpecData(
    ...
    // TODO: Remove the subgroup config from BIS config
  ...
  uint8_t GetNumBis() const {
    uint8_t value = 0;
    // Iterate over BISes
    for (auto const& cfg : bis_codec_configs) {
      value += cfg.GetNumBis();
    }
    return value;
  }
  ...
  uint8_t GetNumBis(uint8_t bis_idx) const {
    if (bis_idx < bis_codec_configs.size()) {
      return bis_codec_configs.at(bis_idx).GetNumBis();
    }
    return 0;
  }
  ...
  uint8_t GetBitsPerSample() const { return bits_per_sample; }

  size_t GetAllBisConfigCount() const { return bis_codec_configs.size(); }
...
struct BroadcastQosConfig {
  ...
  bool operator==(const BroadcastQosConfig& other) const {
    return (retransmission_number == other.retransmission_number) &&
           (max_transport_latency == other.max_transport_latency);
  }
...
struct BroadcastConfiguration {
  ...
  void GetAudioHalClientConfig(LeAudioCodecConfiguration* hal_config_out) const;
