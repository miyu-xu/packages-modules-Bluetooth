...
struct BroadcastCodecWrapper {
  BroadcastCodecWrapper(types::LeAudioCodecId codec_id,
                        ...
                        uint32_t codec_bitrate, uint8_t blocks_per_sdu = 1)
      : codec_id(codec_id),
  ...
  uint16_t GetMaxSduSizePerChannel() const {
    if (codec_id.coding_format == types::kLeAudioCodingFormatLC3) {
      const int dt_us = source_codec_config.data_interval_us;
      const int br_bps = GetBitrate();
      return lc3_frame_bytes(dt_us, br_bps) * blocks_per_sdu;
    }
