...
using le_audio::broadcaster::BroadcastConfiguration;
...
using le_audio::broadcaster::BroadcastSubgroupCodecConfig;
...
class LeAudioBroadcasterImpl : public LeAudioBroadcaster, public BigCallbacks {
 ...
 public:
  ...
  static BasicAudioAnnouncementData prepareBasicAnnouncement(
    ...
    if (subgroup_configs.size() != metadata_group.size()) {
      LOG_ERROR(
    ...
    while (subgroup_idx < subgroup_configs.size() &&
      ...
      for (uint8_t bis_idx = 0;
           bis_idx < subgroup_config.GetAllBisConfigCount(); ++bis_idx) {
        for (uint8_t bis_num = 0; bis_num < subgroup_config.GetNumBis(bis_idx);
             ++bis_num) {
          ++bis_count;
          ...
          auto vendor_config =
              subgroup_config.GetBisVendorCodecSpecData(bis_idx);
          if (vendor_config) {
            bis_config.vendor_codec_specific_params = vendor_config.value();
          }

          // Check for non vendor LTVs
          auto config_ltv = subgroup_config.GetBisCodecSpecData(bis_idx);
          if (config_ltv) {
            // Remove the part which is common with the parrent subgroup
            // parameters
          ...
          // spec requires the indices to be indexed from 1 in the entire BIG.
          bis_config.bis_index = static_cast<uint8_t>(bis_count);
  ...
  void CreateAudioBroadcast(
    ...
    std::vector<std::pair<le_audio::types::AudioContexts, uint8_t>>
        subgroup_requirements;
    for (auto& idx : subgroup_quality) {
      subgroup_requirements.push_back({context_type, idx});
    }

    auto config =
        CodecManager::GetInstance()->GetBroadcastConfig(subgroup_requirements);
    ...
    void OnStateMachineEvent(uint32_t broadcast_id,
      ...
      switch (state) {
        ...
        case BroadcastStateMachine::State::STREAMING:
          /* Audio framework config will be derived from the first active
          ...
          if (getStreamerCount() == 1) {
            ...
            if (instance->broadcasts_.count(broadcast_id) != 0) {
              ...
              LeAudioCodecConfiguration cfg;
              static_cast<const BroadcastConfiguration*>(data)
                  ->GetAudioHalClientConfig(&cfg);
  ...
  static class LeAudioSourceCallbacksImpl
   ...
   public:
    ...
    void CheckAndReconfigureEncoders(
      ...
      le_audio::LeAudioCodecConfiguration codec_config;
      broadcast_config.GetAudioHalClientConfig(&codec_config);
