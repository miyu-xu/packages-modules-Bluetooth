...
class LeAudioBroadcasterImpl : public LeAudioBroadcaster, public BigCallbacks {
 ...
 public:
  ...
  static PublicBroadcastAnnouncementData preparePublicAnnouncement(
      uint8_t features, LeAudioLtvMap metadata) {
  ...
  static BasicAudioAnnouncementData prepareBasicAnnouncement(
      ...
      std::vector<LeAudioLtvMap> metadata_group) {
    ...
    for (uint8_t i = 0; i < metadata_group.size(); ++i) {
      BasicAudioAnnouncementSubgroup config = {
          ...
          .metadata = metadata_group[i].Values(),
          .bis_configs = {},
      };
  ...
  void UpdateMetadata(
    ...
    for (uint8_t i = 0; i < broadcast_settings.subgroup_settings.size(); ++i) {
      /* Prepare the announcement format */
      bool is_metadata_valid;
      auto ltv = LeAudioLtvMap::Parse(
          broadcast_settings.subgroup_settings[i].metadata.data(),
          broadcast_settings.subgroup_settings[i].metadata.size(),
          is_metadata_valid);
  ...
  void CreateAudioBroadcast(
      const btle_audio_broadcast_settings_t broadcast_settings) override {
    bool is_public = broadcast_settings.is_public;
    std::string broadcast_name = broadcast_settings.broadcast_name;
    auto broadcast_code = broadcast_settings.broadcast_code;
    ...
    if (is_public) {
      ...
      public_ltv = LeAudioLtvMap::Parse(
          broadcast_settings.public_metadata.data(),
          broadcast_settings.public_metadata.size(), is_metadata_valid);
      if (!is_metadata_valid) {
        LOG_ERROR("Invalid metadata provided.");
        return;
      }
    ...
    for (uint8_t i = 0; i < broadcast_settings.subgroup_settings.size(); ++i) {
      // Check subgroup preferred quality info
      if (broadcast_settings.subgroup_settings[i].quality ==
          bluetooth::le_audio::QUALITY_STANDARD) {
        public_features |= le_audio::types::kLeAudioQualityStandard;
      } else if (broadcast_settings.subgroup_settings[i].quality ==
                 bluetooth::le_audio::QUALITY_HIGH) {
        public_features |= le_audio::types::kLeAudioQualityHigh;
      }
    ...
    if (CodecManager::GetInstance()->GetCodecLocation() ==
      ...
      if (is_public) {
        msg.public_announcement =
            preparePublicAnnouncement(public_features, std::move(public_ltv));
      }
