...
class LeAudioBroadcasterImpl : public LeAudioBroadcaster, public BigCallbacks {
 ...
 public:
  ...
  void UpdateAudioActiveStateOnSubgroups() {
    for (auto const& kv_it : broadcasts_) {
      auto& broadcast = kv_it.second;
      ...
      log::info("broadcast_id={}, audio_active_state={}",
                broadcast->GetBroadcastId(), audio_active_state);

      auto updateLtv = [](bool audio_active_state, LeAudioLtvMap& ltv) -> bool {
        auto existing_audio_active_state = ltv.Find(
        ...
        if (existing_audio_active_state &&
          ...
          if (audio_active_state !=
            ...
            ltv.Add(bluetooth::le_audio::types::
      ...
      if (updateLtv(audio_active_state, public_ltv)) {
        ...
        broadcast->UpdatePublicBroadcastAnnouncement(
            broadcast->GetBroadcastId(), broadcast->GetBroadcastName(),
            public_announcement);
      }
    }
  }
    ...
    if (broadcasts_[broadcast_id]->IsPublicBroadcast()) {
      ...
      if (com::android::bluetooth::flags::
        ...
        bool audio_active_state = (audio_state_ == AudioState::ACTIVE) &&
                                  (broadcasts_[broadcast_id]->GetState() ==
  ...
  void setBroadcastTimers() {
    if (audio_state_ == AudioState::SUSPENDED) {
      ...
      alarm_set_on_mloop(
          ...
          [](void*) {
            if (instance) instance->SuspendAudioBroadcasts();
          },
          nullptr);
      ...
      alarm_set_on_mloop(
          ...
          [](void*) {
            if (instance) instance->StopAudioBroadcasts();
          },
          nullptr);
    }
  }
