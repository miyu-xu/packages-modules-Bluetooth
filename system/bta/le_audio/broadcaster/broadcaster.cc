...
class LeAudioBroadcasterImpl : public LeAudioBroadcaster, public BigCallbacks {
 ...
 public:
  ...
  static BasicAudioAnnouncementData prepareBasicAnnouncement(
    ...
    for (size_t i = 0; i < metadata_group.size(); ++i) {
  ...
  void UpdateMetadata(
    ...
    for (uint8_t i = 0; i < subgroup_metadata.size(); ++i) {
      /* Prepare the announcement format */
      bool is_metadata_valid;
      auto ltv =
          LeAudioLtvMap::Parse(subgroup_metadata[i].data(),
                               subgroup_metadata[i].size(), is_metadata_valid);
  ...
  void CreateAudioBroadcast(
      ...
      const std::vector<uint8_t>& public_metadata,
      const std::vector<std::vector<uint8_t>>& subgroup_settings) override {
    ...
    for (size_t i = 0; i < subgroup_settings.size(); ++i) {
      // Check subgroup preferred quality info from 1st byte
      uint8_t quality = subgroup_settings[i][0];
      ...
      /* Prepare the announcement format */
      bool is_metadata_valid;
      // Parse the meta data from index 1, the 1st byte is preferred quality
      // setting
      std::vector<uint8_t> metadata(std::next(subgroup_settings[i].begin()),
                                    subgroup_settings[i].end());
