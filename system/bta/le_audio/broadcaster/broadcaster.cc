...
class LeAudioBroadcasterImpl : public LeAudioBroadcaster, public BigCallbacks {
 ...
 public:
  ...
  void UpdateMetadata(uint32_t broadcast_id,
    ...
    /* Adds multiple contexts and CCIDs regardless of the incoming audio context.
     * Android has only two CCIDs, one for Media and one for Conversational
     * context. Even though we are not broadcasting Conversational streams,
     * some PTS test cases wants multiple CCIDs.
     */
