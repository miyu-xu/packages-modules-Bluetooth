  ...
  void StartAudioBroadcast(uint8_t instance_id) override {
    ...
    if (broadcasts_.count(instance_id) != 0) {
      if (!audio_instance_) {
        ...
        if (!audio_instance_) {
          LOG(ERROR) << __func__ << " could not aquire le audio";
          return;
        }
      }
  ...
  static class LeAudioClientAudioSinkReceiverImpl
   ...
   public:
    ...
    void CheckAndReconfigureEncoders() {
      ...
      /* TODO: Use multichannel encoding, optimize. */
      LOG_ASSERT(!codec_wrapper_.SupportsMultichannelEncoding())
          << "Only single channel encoding is currently supported.";

      auto lc3_encoder_config =
          std::get<Lc3Config>(codec_wrapper_.GetEncoderConfig());
      ...
      while (encoders_.size() < codec_wrapper_.GetNumChannels()) {
        encoders_.emplace_back(
            std::make_unique<Lc3Encoder>(lc3_encoder_config));
      }
    ...
    void encodeLc3Channel(std::unique_ptr<Lc3Encoder>& encoder,
                          std::vector<uint8_t>& out_buffer,
                          const std::vector<uint8_t>& data,
                          int initial_channel_offset, int channel_data_stride) {
      uint16_t byte_count = encoder->lc3Config.getByteCountFromBitrate(
          codec_wrapper_.GetBitrate());
      uint16_t num_frames = encoder->lc3Config.NF;

      /* Resize out_buffer if needed - hopefully called only for the first
       * sample
       */
      if (out_buffer.size() < byte_count) out_buffer.resize(byte_count);

      /* Extract single channel from the received data */
      static std::vector<uint16_t> split_channel_buffer;
      if (split_channel_buffer.size() < num_frames)
        split_channel_buffer.resize(num_frames);

      const uint8_t* data_end = data.data() + data.size();
      for (uint16_t i = 0; i < num_frames; ++i) {
        const uint8_t* sample =
            data.data() + initial_channel_offset + (i * channel_data_stride);

        if (sample < data_end) {
          split_channel_buffer[i] =
              (int16_t)((*(sample + 1) << 8) + *sample) >> 1;
        } else {
          /* No source data left - fill with silence */
          split_channel_buffer[i] = 0;
        }
      }

      auto encoder_status = encoder->run((int16_t*)split_channel_buffer.data(),
                                         byte_count, out_buffer.data());
      if (encoder_status != Lc3Encoder::ERROR_FREE) {
        LOG(ERROR) << "Error while encoding"
                   << "\terror: " << static_cast<int>(encoder_status);
      }
    }
    ...
    virtual void OnAudioDataReady(const std::vector<uint8_t>& data) override {
      ...
      const auto& codec_config = codec_wrapper_.GetLeAudioCodecConfiguration();
      const int bytes_per_sample = (codec_config.bits_per_sample / 8);
      const int bytes_per_multichan_sample =
          bytes_per_sample * codec_config.num_channels;

      /* Constants for the current data buffer */
      const int num_total_bytes_per_channel =
          data.size() / codec_config.num_channels;
      const int num_total_samples_per_channel =
          num_total_bytes_per_channel / bytes_per_sample;

      if (num_total_samples_per_channel == 0) {
        LOG(ERROR) << __func__ << ": Not enough data";
        return;
      }

      /* Check how many ISO packets we should prepare */
      if (num_total_samples_per_channel != encoders_[0]->lc3Config.NF) {
        if (num_total_samples_per_channel > encoders_[0]->lc3Config.NF) {
          /* Note: What should we do besides just printing an error? */
          LOG(ERROR) << __func__ << ": Frames dropped!";
        }
      }

      /* Prepare encoded data for all channels */
      for (uint8_t chan = 0; chan < codec_config.num_channels; ++chan) {
        /* TODO: Use encoder agnostic wrapper */
        encodeLc3Channel(encoders_[chan], enc_audio_buffers_[chan], data,
                         chan * bytes_per_sample, bytes_per_multichan_sample);
      }
   ...
   private:
    ...
    std::vector<std::unique_ptr<Lc3Encoder>> encoders_;
