...
class BroadcasterTest : public Test {
 protected:
  ...
  uint32_t InstantiateBroadcast(
    ...
    settings_data[0] = 0;
    settings_data.insert(std::next(settings_data.begin()), metadata.begin(),
                         metadata.end());
    std::vector<std::vector<uint8_t>> settings_array;
    for (uint8_t i = 0; i < num_of_groups; i++) {
      settings_array.push_back(settings_data);
    }
    // Add two subgroup settings with the same content
    LeAudioBroadcaster::Get()->CreateAudioBroadcast(
        true, test_broadcast_name, code, default_public_metadata,
        settings_array);
