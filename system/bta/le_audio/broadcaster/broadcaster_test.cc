...
// Dummy LC3 codec implementation just to satisfy the linker
Lc3Config::Lc3Config(uint16_t Fs_, Lc3Config::FrameDuration N_ms_, uint8_t Nc_)
    : errorStatus(0),
      Fs(Fs_),
      Fs_ind(0),
      N_ms(N_ms_),
...
TEST_F(BroadcasterTest, StartAudioBroadcast) {
  ...
  std::vector<uint8_t> sample_data(128, 0);
