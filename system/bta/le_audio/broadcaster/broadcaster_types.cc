...
std::vector<uint8_t> BroadcastCodecWrapper::GetCodecSpecData() const {
  ...
  if (codec_id.coding_format == kLeAudioCodecIdLc3.coding_format) {
    const Lc3Config& lc3_config = std::get<Lc3Config>(source_encoder_config);
    uint16_t bc = lc3_config.getByteCountFromBitrate(codec_bitrate);
...
std::ostream& operator<<(
  ...
  os << ", LeAudioCodecConfiguration="
     << "{NumChannels=" << +config.GetLeAudioCodecConfiguration().num_channels
     << ", SampleRate=" << +config.GetLeAudioCodecConfiguration().sample_rate
     << ", BitsPerSample="
     << +config.GetLeAudioCodecConfiguration().bits_per_sample
     << ", DataIntervalUs="
     << +config.GetLeAudioCodecConfiguration().data_interval_us << "}";
  os << "]";
  return os;
}
