...
void PrepareAdvertisingData(
  ...
  if (is_public) {
    ...
    // 6: datalen(1) + adtype(1) + serviceuuid(2) + features(1) + metalen(1)
    adv_data.resize(old_size + 6);
    ...
    if (!public_announcement.metadata.empty()) {
      EmitMetadata(public_announcement.metadata, adv_data);
    } else {
      // Set meta length to 0 if no meta data present
      UINT8_TO_STREAM(data_ptr, 0);
    }
