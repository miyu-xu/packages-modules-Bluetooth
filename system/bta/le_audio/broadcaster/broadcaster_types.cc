...
static void EmitCodecConfiguration(
  ...
  if (config.vendor_codec_specific_params) {
    memcpy(p_value, config.vendor_codec_specific_params->data(),
           config.vendor_codec_specific_params->size());
    p_value += config.vendor_codec_specific_params->size();
  } else {
...
void BroadcastConfiguration::GetAudioHalClientConfig(
    le_audio::LeAudioCodecConfiguration* out_hal_config) const {
  // Get the maximum number of channels
  out_hal_config->num_channels = GetNumChannelsMax();
  // Get the max sampling frequency
  out_hal_config->sample_rate = GetSamplingFrequencyHzMax();
  // Get the minimum data interval
  out_hal_config->data_interval_us = GetSduIntervalUs();
  // Use the default 16 bits per sample resolution in the audio framework
  out_hal_config->bits_per_sample = 16;
}
...
std::ostream& operator<<(
  ...
  os << "BISes=[";
  for (auto const& bis_config : config.bis_codec_configs) {
    os << bis_config << ", ";
  }
  os << "]";
