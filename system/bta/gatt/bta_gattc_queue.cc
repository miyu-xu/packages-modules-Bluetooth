...
struct gatt_read_multi_simulate_op_data {
  ...
  uint8_t read_index;
  std::array<uint8_t, GATT_MAX_ATTR_LEN> values;
  uint16_t values_end;
  bool variable_len;
};
...
void BtaGattQueue::gatt_read_multi_op_simulate(uint16_t conn_id, tGATT_STATUS status,
  ...
  // all handles read, or bad status
  GATT_READ_MULTI_OP_CB tmp_cb = data->cb;
  void* tmp_cb_data = data->cb_data;

  std::array<uint8_t, GATT_MAX_ATTR_LEN> value_copy = data->values;
...
void BtaGattQueue::gatt_execute_next_op(uint16_t conn_id) {
  ...
  if (op.type == GATT_READ_CHAR) {
    ...
    if (op.variable_len && gatt_profile_get_eatt_support_by_conn_id(conn_id)) {
      ...
      data->read_index = 0;
      uint16_t handle = op.handles.handles[data->read_index];
