...
void BtaGattServerQueue::gatts_execute_next_op(uint16_t conn_id) {
  LOG_DEBUG("%s: conn_id=0x%x", __func__, conn_id);

  if (gatts_op_queue.empty()) {
    LOG_DEBUG("%s: op queue is empty", __func__);
    return;
  }
  ...
  if (ptr != congestion_queue.end()) {
    ...
    LOG_DEBUG("%s: congestion queue exist, conn_id: %d, is_congested: %d",
              __func__, conn_id, is_congested);
    if (is_congested) {
      LOG_DEBUG("%s: lower layer is congested", __func__);
      return;
    }
  }
  ...
  if (map_ptr == gatts_op_queue.end()) {
    LOG_DEBUG("%s: Queue is null", __func__);
    return;
  }

  if (map_ptr->second.empty()) {
    LOG_DEBUG("%s: queue is empty for conn_id: %d", __func__, conn_id);
    return;
  }

  if (gatts_op_queue_executing.count(conn_id)) {
    LOG_DEBUG("%s: can't enqueue next op, already executing", __func__);
    return;
  }

  gatts_operation op = map_ptr->second.front();
  LOG_DEBUG("%s: op.type=%d, attr_id=%d", __func__, op.type, op.attr_id);
...
void BtaGattServerQueue::NotificationCallback(uint16_t conn_id) {
  ...
  if (map_ptr == gatts_op_queue.end() || map_ptr->second.empty()) {
    LOG_DEBUG("%s: no more operations queued for conn_id %d", __func__,
              conn_id);
    return;
  }
...
void BtaGattServerQueue::CongestionCallback(uint16_t conn_id, bool congested) {
  LOG_DEBUG("%s: conn_id: %d, congested: %d", __func__, conn_id, congested);
