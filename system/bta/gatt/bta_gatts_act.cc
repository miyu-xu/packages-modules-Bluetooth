...
void bta_gatts_indicate_handle(tBTA_GATTS_CB* p_cb, tBTA_GATTS_DATA* p_msg) {
  ...
  if (p_srvc_cb) {
    if (GATT_GetConnectionInfor(p_msg->api_indicate.hdr.layer_specific,
      ...
      log::error("Unknown connection_id={:x} fail sending notification",
    ...
    log::error("Not an registered servce attribute ID: {:x}",
...
void bta_gatts_close(tBTA_GATTS_CB* /* p_cb */, tBTA_GATTS_DATA* p_msg) {
  ...
  if (GATT_GetConnectionInfor(p_msg->hdr.layer_specific, &gatt_if, remote_bda,
    ...
    log::error("Unknown connection_id={:x}", p_msg->hdr.layer_specific);
  }
}
...
static void bta_gatts_send_request_cback(uint16_t conn_id, uint32_t trans_id,
  ...
  if (GATT_GetConnectionInfor(conn_id, &gatt_if, cb_data.req_data.remote_bda,
    ...
    log::verbose("conn_id={:x}, trans_id={}, req_type={}", conn_id, trans_id,
                 req_type);
    ...
    log::error("request received on unknown conn_id={:x}", conn_id);
  }
}
...
static void bta_gatts_conn_cback(tGATT_IF gatt_if, const RawAddress& bdaddr,
  ...
  log::verbose("bda={} gatt_if= {}, conn_id={:x} connected={}", bdaddr, gatt_if,
               conn_id, connected);
