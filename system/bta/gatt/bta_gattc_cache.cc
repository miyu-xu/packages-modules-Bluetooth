...
static void bta_gattc_explore_next_service(uint16_t conn_id,
  ...
  tBTA_GATTC_CLCB* p_clcb = bta_gattc_find_clcb_by_conn_id(conn_id);
  if (!p_clcb) {
    log::error("unknown conn_id={:x}", conn_id);
    return;
  }
...
#if (BTA_GATT_DEBUG == TRUE)
    log::verbose("Found ATT service uuid={}, s_handle={:x}, e_handle={:x}",
                 service_uuid, start_handle, end_handle);
#endif

    if (!GATT_HANDLE_IS_VALID(start_handle) ||
        !GATT_HANDLE_IS_VALID(end_handle)) {
      log::error("invalid start_handle={:x}, end_handle={:x}", start_handle,
                 end_handle);
...
static void bta_gattc_read_ext_prop_desc_cmpl(
  ...
  if (status != GATT_SUCCESS) {
    log::warn("Discovery on server failed: {:x}", status);
