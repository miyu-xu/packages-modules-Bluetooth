...
static void bta_gattc_generate_hash_file_name(char* buffer, size_t buffer_len,
  ...
  snprintf(buffer, buffer_len,
           ...
           GATT_HASH_PATH_PREFIX, hash[0], hash[1], hash[2], hash[3], hash[4],
...
static void bta_gattc_read_db_hash_cmpl(tBTA_GATTC_CLCB* p_clcb,
  ...
  if (p_data->status == GATT_SUCCESS) {
    ...
    // Only load cache for bonded device if no database hash on server side.
    if (btm_sec_is_a_bonded_dev(p_clcb->p_srcb->server_bda)) {
