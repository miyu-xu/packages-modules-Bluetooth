...
RobustCachingSupport GetRobustCachingSupport(const tBTA_GATTC_CLCB* p_clcb,
  ...
  if (!db.IsEmpty()) {
    ...
    LOG_INFO("GetRobustCachingSupport database hash characteristic not found, so UNSUPPORTED");
    return RobustCachingSupport::UNSUPPORTED;
  }

  if (p_clcb->transport == BT_TRANSPORT_LE && !BTM_RemoteVersionIsParsed(p_clcb->bda)) {
