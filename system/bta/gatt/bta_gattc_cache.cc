...
RobustCachingSupport GetRobustCachingSupport(const tBTA_GATTC_CLCB* p_clcb,
  ...
  // We should only look at the supplied database if we are bonded and did
  // discovery
  if (btm_sec_is_a_bonded_dev(p_clcb->bda) && !db.IsEmpty()) {
  ...
  // Otherwise, we may wish to exclude the use of robust caching for
  // interop reasons.
  ...
  // embedded device having LMP version lower than 5.1 (0x0a) do not
  // support GATT Caching, unless we have seen the database hash characteristic
  // in a prior discovery
  if (lmp_version < 0x0a) {
    LOG_WARN(
