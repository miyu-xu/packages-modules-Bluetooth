...
// don't permanently store potentially corrupt data. Thus, we should return
// MAYBE_VALID.
enum class ServiceDiscoveryStatus { VALID, MAYBE_VALID };
...
std::ostream& operator<<(std::ostream& os,
  ...
  switch (status) {
    case ServiceDiscoveryStatus::VALID:
      return os << "VALID";
    case ServiceDiscoveryStatus::MAYBE_VALID:
      return os << "MAYBE_VALID";
  }
}
...
void bta_gattc_sdp_callback(tSDP_STATUS sdp_status, const void* user_data) {
  ...
  if ((sdp_status != SDP_SUCCESS) && (sdp_status != SDP_DB_FULL)) {
    bta_gattc_explore_srvc_finished(
        cb_data->sdp_conn_id, p_srvc_cb,
        bluetooth::common::init_flags::
                gatt_treat_failed_sdp_as_invalid_is_enabled()
            ? ServiceDiscoveryStatus::MAYBE_VALID
            : ServiceDiscoveryStatus::VALID);
...
static void bta_gattc_read_ext_prop_desc_cmpl(
  ...
  if (status != GATT_SUCCESS) {
    ...
    if (bluetooth::common::init_flags::
            gatt_allow_invalid_services_is_enabled()) {
      bta_gattc_explore_srvc_finished(p_clcb->bta_conn_id, p_srvc_cb,
                                      ServiceDiscoveryStatus::MAYBE_VALID);
    } else {
