...
#define BTA_GATTC_DB_HASH_SIZE 16
...
static void bta_gattc_read_db_hash_cmpl(tBTA_GATTC_CLCB* p_clcb,
  ...
  if (p_data->status == GATT_SUCCESS) {
    ...
    Octet16 remote_hash;
    if (len == BTA_GATTC_DB_HASH_SIZE) {
      memcpy(remote_hash.data(), data, BTA_GATTC_DB_HASH_SIZE);

      Octet16 local_hash = p_clcb->p_srcb->gatt_database.Hash();
      matched = (local_hash == remote_hash);

      LOG_DEBUG("lhash=%s", base::HexEncode(local_hash.data(), local_hash.size()).c_str());
      LOG_DEBUG("rhash=%s", base::HexEncode(remote_hash.data(), remote_hash.size()).c_str());
