...
static void bta_gattc_generate_hash_file_name(char* buffer, size_t buffer_len,
  ...
  snprintf(buffer, buffer_len,
           "%s%s", GATT_HASH_PATH_PREFIX, base::HexEncode(hash.data(), 16).c_str());
}
 ...
 *
 ******************************************************************************/
bool bta_gattc_hash_load(const Octet16& hash, tBTA_GATTC_SERV* p_srcb) {
  char fname[255] = {0};
 ...
 *
 ******************************************************************************/
void bta_gattc_cache_link(const RawAddress& server_bda,
                                 const Octet16& hash) {
