...
void bta_gattc_send_mtu_response(tBTA_GATTC_CLCB* p_clcb,
  ...
  /* For backward compatibility, if user requested less than GATT can
   * handle, just notify the requested mtu
   */
  if (current_mtu > p_data->api_mtu.mtu) {
    cb_data.cfg_mtu.mtu = p_data->api_mtu.mtu;
  } else {
