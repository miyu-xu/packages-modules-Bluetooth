...
void bta_gattc_clcb_dealloc(tBTA_GATTC_CLCB* p_clcb) {
  ...
  p_clcb->bda = {};
  p_clcb->transport = 0;
  p_clcb->p_rcb = nullptr;
  p_clcb->p_srcb = nullptr;
...
BtaEnqueuedResult_t bta_gattc_enqueue(tBTA_GATTC_CLCB* p_clcb,
  ...
  LOG_WARN("Already has a pending command to executer. Enquing for later.");
  p_clcb->p_q_cmd_queue.emplace_back(std::move(p_data));
