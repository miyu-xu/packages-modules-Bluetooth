...
static void bta_gattc_addr_consolidate_cback(uint16_t conn_id,
                                             const RawAddress& identity_addr,
                                             const RawAddress& rpa) {
  log::debug("XXX: conn_id={} identity_addr={} rpa={}", conn_id,
             ADDRESS_TO_LOGGABLE_CSTR(identity_addr),
             ADDRESS_TO_LOGGABLE_CSTR(rpa));

  tBTA_GATTC_CONN* p_conn = bta_gattc_conn_find(rpa);
  if (p_conn != NULL) {
    log::debug("XXX: p_conn found. before={} after={}",
               ADDRESS_TO_LOGGABLE_CSTR(p_conn->remote_bda),
               ADDRESS_TO_LOGGABLE_CSTR(identity_addr));
  ...
  if (p_clcb != NULL) {
    ...
    log::debug("XXX: p_clcb found. before={} after={}",
               ADDRESS_TO_LOGGABLE_CSTR(p_clcb->bda),
               ADDRESS_TO_LOGGABLE_CSTR(identity_addr));
  }
  ...
  if (p_srcb != NULL) {
    ...
    log::debug("XXX: p_srcb found. before={} after={}",
               ADDRESS_TO_LOGGABLE_CSTR(p_srcb->server_bda),
               ADDRESS_TO_LOGGABLE_CSTR(identity_addr));
  }
