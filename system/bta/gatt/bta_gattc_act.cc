...
void bta_gattc_disc_cmpl(tBTA_GATTC_CLCB* p_clcb,
  ...
  if (!btm_cb.enc_adv_data_enabled || p_clcb->status != GATT_SUCCESS ||
      !p_clcb->p_srcb || !btm_sec_is_a_bonded_dev(p_clcb->p_srcb->server_bda)) {
    return;
  }
  ...
  if (p_dev_rec && (p_dev_rec->sec_flags & BTM_SEC_LE_ENCRYPTED)) {
    ...
    tGATT_TCB* p_tcb =
        gatt_find_tcb_by_addr(p_clcb->p_srcb->server_bda, BT_TRANSPORT_LE);
    if (p_tcb) {
