...
void bta_gattc_conn(tBTA_GATTC_CLCB* p_clcb, const tBTA_GATTC_DATA* p_data) {
  ...
  if (p_clcb->p_srcb->gatt_database.IsEmpty() ||
    ...
    if (p_clcb->p_srcb->state == BTA_GATTC_SERV_IDLE) {
      ...
      // if robust caching is known to be supported, any time when connection is
      // established, always check the db hash first, not just load the stored
      // database.
      ...
      if (db.IsEmpty() || !btm_sec_is_a_bonded_dev(p_clcb->bda) ||
          robust_caching_support == RobustCachingSupport::SUPPORTED) {
        // If the peer device might support robust caching, or if we don't
        // know its services yet, then we should do discovery (which may
        // short-circuit through a hash match, but might also do the full
        // discovery).
        p_clcb->p_srcb->state = BTA_GATTC_SERV_DISC;

        // Always try reading the DB hash unless we know with high confidence
        // it's not supported (if we are wrong for a bonded device, we'll get
        // the service changed indication anyway, so we won't *stay* wrong)
