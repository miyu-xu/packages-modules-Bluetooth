...
static void bta_gattc_continue_with_version_and_cache_known(tBTA_GATTC_CLCB* p_clcb, RobustCachingSupport cache_support, bool is_svc_chg);
...
void bta_gattc_continue_discovery_if_needed(const RawAddress& bd_addr, tBT_TRANSPORT transport,
                   uint16_t acl_handle) {
  ...
  tBTA_GATTC_CLCB* p_clcb = bta_gattc_find_clcb_by_conn_id(p_srcb->blocked_conn_id);
  if (!p_clcb) {
...
void bta_gattc_continue_with_version_and_cache_known(tBTA_GATTC_CLCB* p_clcb, RobustCachingSupport cache_support, bool is_svc_chg) {
  ...
  if (bta_gattc_is_robust_caching_enabled() &&
    ...
    LOG(INFO) << __func__
              << ": pending service discovery, read db hash first conn_id:" << loghex(p_clcb->bta_conn_id);
    p_clcb->p_srcb->srvc_hdl_db_hash = false;
    return;
  }
