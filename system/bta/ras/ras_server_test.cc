  ...
  void SetUp() override {
    ...
    EXPECT_CALL(mock_gatt_server_interface_, AddService(_, _, _))
            .WillOnce(testing::DoAll(
                    ...
                    testing::SaveArg<1>(&captured_service), testing::SaveArg<2>(&captured_cb),
...
TEST_F(RasServerTest, ReadCharacteristic) {
  // OnMtuChangedFromServer should be triggered after receiving BTA_GATTS_READ_CHARACTERISTIC_EVT
  EXPECT_CALL(mock_gatt_server_interface_, SendRsp(_, _, _, _)).Times(1);
