...
TEST_F(BtaAvCfgTest, BtaAvCfg_onlySourceEnabledV14_configIsValid) {
  ...
  const uint8_t* meta_event_ids = bta_av_config.getPMetaEvtIds();
  EXPECT_EQ(meta_event_ids[0], AVRC_EVT_PLAY_STATUS_CHANGE);
  EXPECT_EQ(meta_event_ids[1], AVRC_EVT_TRACK_CHANGE);
  EXPECT_EQ(meta_event_ids[2], AVRC_EVT_PLAY_POS_CHANGED);
  EXPECT_EQ(meta_event_ids[3], AVRC_EVT_AVAL_PLAYERS_CHANGE);
  EXPECT_EQ(meta_event_ids[4], AVRC_EVT_ADDR_PLAYER_CHANGE);
  EXPECT_EQ(meta_event_ids[5], AVRC_EVT_UIDS_CHANGE);
  EXPECT_EQ(meta_event_ids[6], AVRC_EVT_NOW_PLAYING_CHANGE);

  // Disabling AVRCP volume would have the same config as AVRCP volume for
  // source only profile.
  const BtaAvConfig& bta_av_config_2 =
      BtaAvCfgFactory::createCustomConfig(true, false, AVRC_REV_1_4, false);
  EXPECT_EQ(bta_av_config, bta_av_config_2);
}
...
TEST_F(BtaAvCfgTest, BtaAvCfg_onlySourceEnabledV13_configIsValid) {
  ...
  // source only profile.
  const BtaAvConfig& bta_av_config_2 =
      BtaAvCfgFactory::createCustomConfig(true, false, AVRC_REV_1_3, false);
  EXPECT_EQ(bta_av_config, bta_av_config_2);
}
...
TEST_F(BtaAvCfgTest, BtaAvCfg_onlySinkEnabledV14_configIsValid) {
  ...
  const uint8_t* meta_event_ids = bta_av_config.getPMetaEvtIds();
  EXPECT_EQ(meta_event_ids[0], AVRC_EVT_VOLUME_CHANGE);
}

TEST_F(BtaAvCfgTest,
       BtaAvCfg_onlySinkEnabledV14AvrcpVolumeDisabled_configIsValid) {
  // Test when source is enabled and sink is disabled
  const BtaAvConfig& bta_av_config =
      BtaAvCfgFactory::createCustomConfig(false, true, AVRC_REV_1_4, false);

  EXPECT_EQ(bta_av_config.getNumEvtIds(), 0);
  const uint8_t* meta_event_ids = bta_av_config.getPMetaEvtIds();
  EXPECT_EQ(meta_event_ids, nullptr);
}
...
TEST_F(BtaAvCfgTest, BtaAvCfg_SourceSinkEnabledV14_configIsValid) {
  ...
  EXPECT_EQ(meta_event_ids[5], AVRC_EVT_UIDS_CHANGE);
  EXPECT_EQ(meta_event_ids[6], AVRC_EVT_NOW_PLAYING_CHANGE);
  EXPECT_EQ(meta_event_ids[7], AVRC_EVT_VOLUME_CHANGE);
}
...
TEST_F(BtaAvCfgTest, BtaAvCfg_SourceSinkEnabledV13_configIsValid) {
  ...
  EXPECT_EQ(meta_event_ids[1], AVRC_EVT_TRACK_CHANGE);
  EXPECT_EQ(meta_event_ids[2], AVRC_EVT_PLAY_POS_CHANGED);
  EXPECT_EQ(meta_event_ids[3], AVRC_EVT_VOLUME_CHANGE);
}

TEST_F(BtaAvCfgTest,
       BtaAvCfg_SourceSinkEnabledV13AbsoluteVolumeDisabled_configIsValid) {
  // Test when source is enabled and sink is disabled
  const BtaAvConfig& bta_av_config =
      BtaAvCfgFactory::createCustomConfig(true, true, AVRC_REV_1_3, false);
