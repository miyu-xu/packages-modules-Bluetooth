...
TEST_F(BtaAvCfgTest, BtaAvCfg_onlySinkEnabledV14_configIsValid) {
  ...
  EXPECT_EQ(bta_av_config.getPMetaEvtIds()[0], AVRC_EVT_VOLUME_CHANGE);
}
...
TEST_F(BtaAvCfgTest, BtaAvCfg_SourceSinkEnabledV14_configIsValid) {
  ...
  EXPECT_EQ(bta_av_config.getPMetaCoIds()[1],
            static_cast<uint32_t>(AVRC_CO_BROADCOM));
  uint8_t tests[] = {AVRC_EVT_PLAY_STATUS_CHANGE, AVRC_EVT_TRACK_CHANGE,
  ...
  EXPECT_EQ(bta_av_config.getPMetaEvtIds()[0],
            static_cast<uint8_t>(AVRC_EVT_PLAY_STATUS_CHANGE));
  EXPECT_EQ(bta_av_config.getPMetaEvtIds()[0], AVRC_EVT_PLAY_STATUS_CHANGE);
  EXPECT_EQ(bta_av_config.getPMetaEvtIds()[1], AVRC_EVT_TRACK_CHANGE);
  EXPECT_EQ(tests[1], AVRC_EVT_TRACK_CHANGE);
  EXPECT_EQ(bta_av_config.getPMetaEvtIds()[2], AVRC_EVT_PLAY_POS_CHANGED);
  EXPECT_EQ(tests[2], AVRC_EVT_PLAY_POS_CHANGED);
  EXPECT_EQ(bta_av_config.getPMetaEvtIds()[3], AVRC_EVT_AVAL_PLAYERS_CHANGE);
  EXPECT_EQ(tests[3], AVRC_EVT_AVAL_PLAYERS_CHANGE);
  EXPECT_EQ(bta_av_config.getPMetaEvtIds()[4], AVRC_EVT_ADDR_PLAYER_CHANGE);
  EXPECT_EQ(tests[4], AVRC_EVT_ADDR_PLAYER_CHANGE);
  EXPECT_EQ(bta_av_config.getPMetaEvtIds()[5], AVRC_EVT_UIDS_CHANGE);
  EXPECT_EQ(tests[5], AVRC_EVT_UIDS_CHANGE);
  EXPECT_EQ(bta_av_config.getPMetaEvtIds()[6], AVRC_EVT_NOW_PLAYING_CHANGE);
  EXPECT_EQ(tests[6], AVRC_EVT_NOW_PLAYING_CHANGE);
  EXPECT_EQ(bta_av_config.getPMetaEvtIds()[7], AVRC_EVT_VOLUME_CHANGE);
  EXPECT_EQ(tests[7], AVRC_EVT_VOLUME_CHANGE);
}
...
TEST_F(BtaAvCfgTest, BtaAvCfg_SourceSinkEnabledV13_configIsValid) {
  ...
  EXPECT_EQ(bta_av_config.getPMetaEvtIds()[1], AVRC_EVT_TRACK_CHANGE);
  EXPECT_EQ(bta_av_config.getPMetaEvtIds()[2], AVRC_EVT_PLAY_POS_CHANGED);
  EXPECT_EQ(bta_av_config.getPMetaEvtIds()[3], AVRC_EVT_VOLUME_CHANGE);
}
