...
#define LOG_TAG "bt_bta_dm_disc_test"
...
TEST_F(BtaInitializedTest, BtaDmSdpPerformer_FindServices) {
  std::unique_ptr<BtaDmSdpPerformer> sdp_performer =
      std::make_unique<BtaDmSdpPerformer>(kRawAddress);
  sdp_performer->FindServices();
}
...
TEST_F_WITH_FLAGS(BtaInitializedTest,
  ...
  std::unique_ptr<MockBtaDmSdpPerformer> sdp_performer;
  bta_dm_disc_override_sdp_performer_for_testing(
      [&](RawAddress bd_addr) {
        sdp_performer = std::make_unique<MockBtaDmSdpPerformer>(bd_addr);

        std::unique_ptr<BtaDmSdpPerformer> ptr{sdp_performer.get()};

        EXPECT_CALL(*sdp_performer, FindServices()).Times(::testing::Exactly(1)).WillOnce(InvokeWithoutArgs([&](){
          //TODO: this causes double free errors
