...
tSDP_DISC_REC SdpDiscRec = {};
tSDP_DISC_REC* sdpFindServiceUUIDInDb = nullptr;

bool sdpFindProtocolListElemInRec = false;
        ...
        .record =
            {
                ...
                .SDP_FindProtocolListElemInRec =
                  ...
                  return sdpFindProtocolListElemInRec;
                },
...
TEST_F(BtaJvTest,
  ...
  *user_data = kSlotId;
  ...
  sdpFindServiceUUIDInDb = &SdpDiscRec;
  ...
  bluetooth::legacy::testing::bta_jv_start_discovery_cback(
      kRawAddress, SDP_SUCCESS, (const void*)user_data);
  sdpFindServiceUUIDInDb = nullptr;
}
...
TEST_F(BtaJvTest, bta_jv_start_discovery__already_active) {
  bta_jv_cb.sdp_active = tBTA_JV_SDP_ACT::YES;
  ...
  bta_jv_enable([](tBTA_JV_EVT event, tBTA_JV* p_data, uint32_t id) {
    switch (event) {
      case BTA_JV_DISCOVERY_COMP_EVT:
        ASSERT_EQ(p_data->disc_comp.status, BTA_JV_BUSY);
        ASSERT_EQ(kSlotId, id);
        break;
...
TEST_F(BtaJvTest, bta_jv_start_discovery__cancelling) {
  bta_jv_cb.sdp_active = tBTA_JV_SDP_ACT::CANCEL;
  bluetooth::Uuid uuid_list[1] = {
      bluetooth::Uuid::GetRandom(),
  };
  uint16_t num_uuid = (uint16_t)(sizeof(uuid_list) / sizeof(uuid_list[0]));
