...
class BtaAgTest : public testing::Test {
 protected:
  ...
  const char test_strings[5][13] = {"test,0,4,6,7", "test,9"};
...
TEST_F(BtaAgSwbTest, parse_qac_at_command) {
  ...
  tBTA_AG_PEER_CODEC codec = bta_ag_parse_qac(&p_scb, (char*)test_strings[0]);
  ...
  codec = bta_ag_parse_qac(&p_scb, (char*)test_strings[1]);
  ASSERT_TRUE((codec & BTM_SCO_CODEC_NONE) == 0);
}
