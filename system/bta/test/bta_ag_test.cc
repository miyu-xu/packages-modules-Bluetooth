...
const char* test_flags[] = {"INIT_aptx_voice=true", nullptr};
...
class BtaAgTest : public testing::Test {
 protected:
  ...
  const char test_strings[5][13] = {"test,0,4,6,7", "0,4,6,7", "test,9",
                                    "9,8,7"};
...
TEST_F(BtaAgSwbTest, parse_qac_at_command) {
  ...
  ASSERT_TRUE(codec & BTA_AG_SCO_SWB_SETTINGS_Q0_MASK);
  ASSERT_TRUE(codec & BTA_AG_SCO_SWB_SETTINGS_Q1_MASK);
  ASSERT_TRUE(codec & BTA_AG_SCO_SWB_SETTINGS_Q2_MASK);
  ASSERT_TRUE(codec & BTA_AG_SCO_SWB_SETTINGS_Q3_MASK);

  codec = bta_ag_parse_qac((char*)test_strings[1]);
  assert(codec & BTA_AG_SCO_SWB_SETTINGS_Q0_MASK);
  assert(codec & BTA_AG_SCO_SWB_SETTINGS_Q1_MASK);
  assert(codec & BTA_AG_SCO_SWB_SETTINGS_Q2_MASK);
  assert(codec & BTA_AG_SCO_SWB_SETTINGS_Q3_MASK);

  codec = bta_ag_parse_qac((char*)test_strings[2]);
  assert((codec & BTM_SCO_CODEC_NONE) == 0);

  codec = bta_ag_parse_qac((char*)test_strings[3]);
  assert((codec & BTM_SCO_CODEC_NONE) == 0);
}

TEST_F(BtaAgSwbTest, enable_swb_codec_fail) {
  const char* test_flag[] = {"INIT_aptx_voice=false", nullptr};
  bluetooth::common::InitFlags::Load(test_flag);
  ASSERT_EQ(BT_STATUS_FAIL, enable_aptx_swb_codec(true));
  ASSERT_FALSE(get_aptx_swb_codec_status());
}

TEST_F(BtaAgSwbTest, enable_swb_codec_success) {
  ASSERT_EQ(BT_STATUS_SUCCESS, enable_aptx_swb_codec(true));
  ASSERT_TRUE(get_aptx_swb_codec_status());
}
...
TEST_F(BtaAgActTest, set_codec_q0_success) {
  ...
  const tBTA_AG_DATA data = {.api_setcodec.codec = BTA_AG_SCO_SWB_SETTINGS_Q0};

  bta_ag_cb.p_cback = [](tBTA_AG_EVT event, tBTA_AG* p_data) {
    tBTA_AG_VAL* val = (tBTA_AG_VAL*)p_data;
    ASSERT_EQ(val->num, BTA_AG_SCO_SWB_SETTINGS_Q0);
    ASSERT_EQ(val->hdr.status, BTA_AG_SUCCESS);
  };

  p_scb->peer_codecs = BTA_AG_SCO_SWB_SETTINGS_Q0;
  p_scb->sco_codec = BTM_SCO_CODEC_NONE;
  p_scb->codec_updated = false;

  bta_ag_setcodec(p_scb, data);
  ASSERT_EQ(p_scb->sco_codec, BTA_AG_SCO_SWB_SETTINGS_Q0);
}

TEST_F(BtaAgActTest, set_codec_q1_fail_unsupported) {
  tBTA_AG_SCB* p_scb = &bta_ag_cb.scb[0];
  const tBTA_AG_DATA data = {.api_setcodec.codec = BTA_AG_SCO_SWB_SETTINGS_Q1};

  bta_ag_cb.p_cback = [](tBTA_AG_EVT event, tBTA_AG* p_data) {
    tBTA_AG_VAL* val = (tBTA_AG_VAL*)p_data;
    ASSERT_EQ(val->num, BTA_AG_SCO_SWB_SETTINGS_Q1);
...
TEST_F(BtaAgCmdTest, at_hfp_cback__qac_ev_codec_disabled) {
  ...
  bta_ag_at_hfp_cback(&p_scb, BTA_AG_AT_QAC_EVT, 0, (char*)&test_strings[0][0],
                      (char*)&test_strings[0][12], BTA_AG_SCO_SWB_SETTINGS_Q0);
  ASSERT_FALSE(p_scb.codec_updated);
  ASSERT_FALSE(p_scb.is_swb_codec);
  ASSERT_EQ(1, get_func_call_count("PORT_WriteData"));
}

TEST_F(BtaAgCmdTest, at_hfp_cback__qac_ev_codec_enabled) {
  tBTA_AG_SCB p_scb = {.app_id = 0,
                       .peer_addr = addr,
                       .peer_codecs = BTA_AG_SCO_SWB_SETTINGS_Q0_MASK};

  ASSERT_EQ(BT_STATUS_SUCCESS, enable_aptx_swb_codec(true));
  bta_ag_at_hfp_cback(&p_scb, BTA_AG_AT_QAC_EVT, 0, (char*)&test_strings[0][0],
                      (char*)&test_strings[0][12], BTA_AG_SCO_SWB_SETTINGS_Q0);
  ASSERT_TRUE(p_scb.codec_updated);
  ASSERT_TRUE(p_scb.is_swb_codec);
...
TEST_F(BtaAgCmdTest, at_hfp_cback__qcs_ev_codec_q0_enabled) {
  tBTA_AG_SCB p_scb = {.app_id = 0,
                       .peer_addr = addr,
                       .sco_codec = BTA_AG_SCO_SWB_SETTINGS_Q0,
...
TEST_F(BtaAgCmdTest, handle_swb_at_event__qcs_ev_codec_q1_fallback_to_q0) {
  tBTA_AG_SCB p_scb = {.app_id = 0,
                       .peer_addr = addr,
                       .sco_codec = BTA_AG_SCO_SWB_SETTINGS_Q1,
                       .codec_fallback = false,
                       .is_swb_codec = true,
                       .sco_idx = BTM_INVALID_SCO_INDEX};
  ...
  ASSERT_EQ(BT_STATUS_SUCCESS, enable_aptx_swb_codec(true));
  bta_ag_at_hfp_cback(&p_scb, BTA_AG_AT_QCS_EVT, 0, (char*)&test_strings[0][0],
                      (char*)&test_strings[0][12], BTA_AG_SCO_SWB_SETTINGS_Q1);
...
TEST_F(BtaAgScoTest, codec_negotiate__aptx_state_off) {
  ...
  ASSERT_EQ(BT_STATUS_SUCCESS, enable_aptx_swb_codec(false));
  bta_ag_codec_negotiate(p_scb);
  ASSERT_EQ(1, get_func_call_count("BTM_ReadRemoteFeatures"));
  ASSERT_EQ(1, get_func_call_count("PORT_WriteData"));
  ASSERT_EQ(1, get_func_call_count("alarm_set_on_mloop"));
...
TEST_F(BtaAgScoTest, codec_negotiate__aptx_disabled) {
  ...
  ASSERT_EQ(BT_STATUS_FAIL, enable_aptx_swb_codec(false));
  bta_ag_codec_negotiate(p_scb);
  ASSERT_EQ(1, get_func_call_count("BTM_ReadRemoteFeatures"));
