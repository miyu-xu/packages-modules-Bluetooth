    ...
    case BTA_AG_AT_BAC_EVT:
      ...
      if ((p_scb->peer_features & BTA_AG_PEER_FEAT_CODEC) &&
        ...
        if (bluetooth::common::init_flags::aptx_voice_is_enabled()) {
          aptx_voice = p_scb->is_aptx_swb_codec == true;
          APPL_TRACE_DEBUG("%s: BTA_AG_AT_BAC_EVT aptx_voice=%d", __func__,
                           aptx_voice);
        }
        ...
        if (swb_supported && (p_scb->peer_codecs & BTM_SCO_CODEC_LC3) &&
          ...
          APPL_TRACE_DEBUG("Received AT+BAC, updating sco codec to AptX Voice");
...
void bta_ag_send_qac(tBTA_AG_SCB* p_scb, tBTA_AG_DATA* p_data) {
  if (!get_swb_codec_status(bluetooth::headset::BTHF_SWB_CODEC_VENDOR_APTX,
                            &p_scb->peer_addr)) {
    APPL_TRACE_DEBUG("%s: send +QAC codecs unsupported", __func__);
    bta_ag_send_result(p_scb, BTA_AG_LOCAL_RES_QAC, SWB_CODECS_UNSUPPORTED, 0);
    return;
  } else {
    APPL_TRACE_DEBUG("%s: send +QAC codecs supported", __func__);
