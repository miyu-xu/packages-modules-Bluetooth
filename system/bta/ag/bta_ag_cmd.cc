...
void bta_ag_at_hfp_cback(tBTA_AG_SCB* p_scb, uint16_t cmd, uint8_t arg_type,
  ...
  switch (cmd) {
    ...
    case BTA_AG_LOCAL_EVT_BCC: {
      if (!bta_ag_sco_is_active_device(p_scb->peer_addr)) {
        LOG_WARN("AT+BCC rejected as %s is not the active device",
                 p_scb->peer_addr.ToStringForLogging().c_str());
        bta_ag_send_error(p_scb, BTA_AG_ERR_OP_NOT_ALLOWED);
        break;
      }

      // Reject AT+BCC if 1. the device is LEA active, 2. dual mode audio is
      // enabled, and 3. LEA is preferred for DUPLEX
      if (bluetooth::os::GetSystemPropertyBool(
        ...
        if (LeAudioClient::Get()->isDuplexPreferenceLeAudio(p_scb->peer_addr)) {
          LOG_INFO(
              "NOT opening sco for EVT BTA_AG_LOCAL_EVT_BCC on dual mode "
              "device %s",
              p_scb->peer_addr.ToStringForLogging().c_str());
          bta_ag_send_error(p_scb, BTA_AG_ERR_OP_NOT_ALLOWED);
          break;
        } else {
          LOG_INFO("Accepting AT+BCC for dual mode device %s",
                   p_scb->peer_addr.ToStringForLogging().c_str());
        }
      }
    ...
    case BTA_AG_IN_CALL_RES:
      ...
      if (bta_ag_sco_is_open(p_scb) || !bta_ag_inband_enabled(p_scb) ||
        ...
        if (bluetooth::os::GetSystemPropertyBool(
                bluetooth::os::kIsDualModeAudioEnabledProperty, false)) {
          LOG_INFO(
              "NOT opening sco for EVT BTA_AG_IN_CALL_RES because the dual "
              "mode audio feature is enabled");
        } else {
          LOG_INFO(
              "Opening sco for EVT BTA_AG_IN_CALL_RES because the dual "
      ...
      if (!(p_scb->features & BTA_AG_FEAT_NOSCO)) {
        ...
        if (result.data.audio_handle == bta_ag_scb_to_idx(p_scb) &&
          ...
          if (bluetooth::os::GetSystemPropertyBool(
                  bluetooth::os::kIsDualModeAudioEnabledProperty, false)) {
            LOG_INFO(
                "NOT opening sco for EVT BTA_AG_IN/OUT_CALL_ORIG_RES "
                "because the dual mode audio feature is enabled");
          } else {
            LOG_INFO(
                "Opening sco for EVT BTA_AG_IN/OUT_CALL_ORIG_RES "
      ...
      if (p_scb->post_sco == BTA_AG_POST_SCO_CALL_END) {
        ...
        if (bta_ag_sco_is_open(p_scb) || !bta_ag_inband_enabled(p_scb) ||
          ...
          if (bluetooth::os::GetSystemPropertyBool(
                  bluetooth::os::kIsDualModeAudioEnabledProperty, false)) {
            LOG_INFO(
                "NOT opening sco for EVT BTA_AG_IN_CALL_RES because the "
                "dual mode audio feature is enabled");
          } else {
            LOG_INFO(
                "Opening sco for EVT BTA_AG_IN_CALL_RES because the "
      ...
      if (!(p_scb->features & BTA_AG_FEAT_NOSCO)) {
        if (result.data.audio_handle == bta_ag_scb_to_idx(p_scb) &&
          ...
          if (bluetooth::os::GetSystemPropertyBool(
                  bluetooth::os::kIsDualModeAudioEnabledProperty, false)) {
            LOG_INFO(
                "NOT opening sco for EVT BTA_AG_IN_CALL_CONN_RES because "
                "the dual mode audio feature is enabled");
          } else {
            LOG_INFO(
                "Opening sco for EVT BTA_AG_IN_CALL_CONN_RES because "
    ...
    case BTA_AG_OUT_CALL_ORIG_RES:
      ...
      if (result.data.audio_handle == bta_ag_scb_to_idx(p_scb) &&
        ...
        if (bluetooth::os::GetSystemPropertyBool(
                bluetooth::os::kIsDualModeAudioEnabledProperty, false)) {
          LOG_INFO(
              "NOT opening sco for EVT BTA_AG_OUT_CALL_ORIG_RES because "
              "the dual mode audio feature is enabled");
        } else {
          LOG_INFO(
              "Opening sco for EVT BTA_AG_OUT_CALL_ORIG_RES because "
              "the dual mode audio feature is not enabled");
          bta_ag_sco_open(p_scb, tBTA_AG_DATA::kEmpty);
        }
      }
    ...
    case BTA_AG_OUT_CALL_ALERT_RES:
      ...
      if (result.data.audio_handle == bta_ag_scb_to_idx(p_scb) &&
        ...
        if (bluetooth::os::GetSystemPropertyBool(
                bluetooth::os::kIsDualModeAudioEnabledProperty, false)) {
          LOG_INFO(
              "NOT opening sco for EVT BTA_AG_OUT_CALL_ALERT_RES because "
              "the dual mode audio feature is enabled");
        } else {
          LOG_INFO(
              "Opening sco for EVT BTA_AG_OUT_CALL_ALERT_RES because "
              "the dual mode audio feature is not enabled");
          bta_ag_sco_open(p_scb, tBTA_AG_DATA::kEmpty);
        }
      }
      ...
      if (!(p_scb->features & BTA_AG_FEAT_NOSCO)) {
        if (result.data.audio_handle == bta_ag_scb_to_idx(p_scb)) {
          if (bluetooth::os::GetSystemPropertyBool(
                  bluetooth::os::kIsDualModeAudioEnabledProperty, false)) {
            LOG_INFO(
                "NOT opening sco for EVT BTA_AG_MULTI_CALL_RES because the "
                "dual mode audio feature is enabled");
          } else {
            LOG_INFO(
                "Opening sco for EVT BTA_AG_MULTI_CALL_RES because the "
                "dual mode audio feature is not enabled");
            bta_ag_sco_open(p_scb, tBTA_AG_DATA::kEmpty);
          }
        } else if (result.data.audio_handle == BTA_AG_HANDLE_NONE) {
      ...
      if (!(p_scb->features & BTA_AG_FEAT_NOSCO)) {
        if (result.data.audio_handle == bta_ag_scb_to_idx(p_scb)) {
          if (bluetooth::os::GetSystemPropertyBool(
                  bluetooth::os::kIsDualModeAudioEnabledProperty, false)) {
            LOG_INFO(
                "NOT opening sco for EVT BTA_AG_OUT_CALL_CONN_RES because "
                "the dual mode audio feature is enabled");
          } else {
            LOG_INFO(
                "Opening sco for EVT BTA_AG_OUT_CALL_CONN_RES because "
                "the dual mode audio feature is not enabled");
            bta_ag_sco_open(p_scb, tBTA_AG_DATA::kEmpty);
          }
        } else if (result.data.audio_handle == BTA_AG_HANDLE_NONE) {
