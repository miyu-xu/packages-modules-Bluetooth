...
void bta_ag_at_hfp_cback(tBTA_AG_SCB* p_scb, uint16_t cmd, uint8_t arg_type,
  ...
  switch (cmd) {
    ...
    case BTA_AG_LOCAL_EVT_BCC: {
      ...
      bta_ag_send_ok(p_scb);
      if (bluetooth::os::GetSystemPropertyBool(kIsDualModeAudioEnabledProperty,
                                               false)) {
        LOG_INFO(
            "NOT opening sco for EVT BTA_AG_LOCAL_EVT_BCC because the dual "
            "mode audio feature is enabled");
      } else {
        LOG_INFO(
            "Opening sco for EVT BTA_AG_LOCAL_EVT_BCC because the dual "
    ...
    case BTA_AG_IN_CALL_RES:
      ...
      if (bta_ag_sco_is_open(p_scb) || !bta_ag_inband_enabled(p_scb) ||
        ...
        if (bluetooth::os::GetSystemPropertyBool(
                kIsDualModeAudioEnabledProperty, false)) {
          LOG_INFO(
