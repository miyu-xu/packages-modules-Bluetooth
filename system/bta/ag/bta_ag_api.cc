...
tBTA_STATUS BTA_AgEnable(tBTA_AG_CBACK* p_cback) {
  ...
  do_in_main_thread(FROM_HERE,
                    base::BindRepeating(&bta_ag_api_enable, p_cback));
  return BTA_SUCCESS;
}
 ...
 ******************************************************************************/
void BTA_AgDisable() {
  do_in_main_thread(FROM_HERE, base::BindRepeating(&bta_ag_api_disable));
}
...
void BTA_AgRegister(tBTA_SERVICE_MASK services, tBTA_AG_FEAT features,
                    ...
                    uint8_t app_id) {
  do_in_main_thread(
      FROM_HERE, base::BindRepeating(&bta_ag_api_register, services, features,
...
void BTA_AgDeregister(uint16_t handle) {
  do_in_main_thread(
      FROM_HERE,
      base::BindRepeating(&bta_ag_sm_execute_by_handle, handle,
...
void BTA_AgClose(uint16_t handle) {
  do_in_main_thread(FROM_HERE, base::BindRepeating(&bta_ag_sm_execute_by_handle,
                                                   handle, BTA_AG_API_CLOSE_EVT,
                                                   tBTA_AG_DATA::kEmpty));
}
...
void BTA_AgResult(uint16_t handle, tBTA_AG_RES result,
  ...
  do_in_main_thread(
      FROM_HERE, base::BindRepeating(&bta_ag_api_result, handle, result, data));
}
...
void BTA_AgSetScoOffloadEnabled(bool value) {
  do_in_main_thread(
      FROM_HERE, base::BindRepeating(&bta_ag_set_sco_offload_enabled, value));
}

void BTA_AgSetScoAllowed(bool value) {
  do_in_main_thread(FROM_HERE,
                    base::BindRepeating(&bta_ag_set_sco_allowed, value));
}

void BTA_AgSetActiveDevice(const RawAddress& active_device_addr) {
  do_in_main_thread(
      FROM_HERE,
      base::BindRepeating(&bta_ag_api_set_active_device, active_device_addr));
}
