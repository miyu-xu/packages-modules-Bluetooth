...
static bool is_lc3_swb_supported;

void set_lc3_swb_codec_support_status(bool supported) {
  LOG_DEBUG("%d", supported);
  is_lc3_swb_supported = supported;
  if (is_lc3_swb_supported) {
    aptx_swb_codec_status = false;
  }
}

bt_status_t enable_aptx_swb_codec(bool enable) {
  if (bluetooth::common::init_flags::aptx_voice_is_enabled() &&
      !is_lc3_swb_supported) {
    LOG_INFO("%d", enable);
