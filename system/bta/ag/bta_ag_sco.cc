...
static void bta_ag_sco_disc_cback(uint16_t sco_idx) {
  ...
  if (handle != 0) {
    ...
    if (bluetooth::common::init_flags::aptx_voice_is_enabled() &&
      ...
      APPL_TRACE_DEBUG("%s: aptx_voice=%d, inuse_codec=%d", __func__,
                       aptx_voice, bta_ag_cb.sco.p_curr_scb->inuse_codec);
    }
    /* Restore settings */
...
void bta_ag_codec_negotiate(tBTA_AG_SCB* p_scb) {
  ...
  if (bluetooth::common::init_flags::aptx_voice_is_enabled()) {
    ...
    APPL_TRACE_DEBUG("%s: aptx_voice=%d, is_aptx_swb_codec=%d", __func__,
  ...
  if (((p_scb->codec_updated || p_scb->codec_fallback) &&
    ...
    if (get_swb_codec_status(bluetooth::headset::BTHF_SWB_CODEC_VENDOR_APTX,
      ...
      APPL_TRACE_DEBUG("%s: Sending +QCS, sco_codec=%d, is_aptx_swb_codec=%d",
      ...
      APPL_TRACE_DEBUG("%s: Sending +BCS, sco_codec=%d, is_aptx_swb_codec=%d",
                       __func__, p_scb->sco_codec, p_scb->is_aptx_swb_codec);
...
void bta_ag_sco_conn_close(tBTA_AG_SCB* p_scb,
  ...
  if (bluetooth::common::init_flags::aptx_voice_is_enabled()) {
    aptx_voice = p_scb->codec_fallback &&
                 p_scb->sco_codec == BTA_AG_SCO_APTX_SWB_SETTINGS_Q0;
    APPL_TRACE_DEBUG("%s: aptx_voice=%d, codec_fallback=%d, sco_codec=%d",
                     __func__, aptx_voice, p_scb->codec_fallback,
                     p_scb->sco_codec);
  }
