...
static void bta_ag_sco_disc_cback(uint16_t sco_idx) {
  ...
  if (handle != 0) {
    ...
    /* no match found */
    LOG_DEBUG("no scb for ag_sco_disc_cback");
  ...
  if (event == BTM_ESCO_CONN_REQ_EVT) {
    ...
    if (remote_bda && bta_ag_sco_is_active_device(*remote_bda) && p_scb &&
      ...
      if (!bta_ag_cb.sco.p_curr_scb) {
        LOG_INFO("%s: Accept Conn Request (sco_inx 0x%04x)", __func__, sco_inx);
        ...
        LOG_DEBUG("bta_ag_esco_connreq_cback: Begin XFER");
...
void bta_ag_create_pending_sco(tBTA_AG_SCB* p_scb, bool is_local) {
  ...
  if (is_local) {
    ...
    LOG_DEBUG("%s: initiated SCO connection", __func__);
  } else {
    ...
    LOG_DEBUG("%s: listening for SCO connection", __func__);
  }
}
...
void bta_ag_sco_close(tBTA_AG_SCB* p_scb,
  ...
  if ((p_scb->sco_idx != BTM_INVALID_SCO_INDEX) ||
    ...
    LOG_DEBUG("bta_ag_sco_close: sco_inx = %d", p_scb->sco_idx);
...
void bta_ag_sco_conn_rsp(tBTA_AG_SCB* p_scb,
  ...
  LOG_DEBUG("%s: eSCO %d, state %d", __func__,
...
void bta_ag_set_sco_allowed(bool value) {
  sco_allowed = value;
  LOG_DEBUG("%s", sco_allowed ? "sco now allowed" : "sco now not allowed");
}
