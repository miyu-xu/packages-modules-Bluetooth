...
#if (DISABLE_SWB == FALSE && DISABLE_WBS == FALSE)
  if ((p_scb->sco_codec == BTM_SCO_CODEC_LC3) && !p_scb->codec_fallback &&
      hfp_hal_interface::get_swb_supported()) {
    esco_codec = UUID_CODEC_LC3;
  } else if ((p_scb->sco_codec == BTM_SCO_CODEC_MSBC) &&
             !p_scb->codec_fallback && hfp_hal_interface::get_wbs_supported()) {
    esco_codec = UUID_CODEC_MSBC;
  }
#elif (DISABLE_SWB == FALSE)
  if ((p_scb->sco_codec == BTM_SCO_CODEC_LC3) && !p_scb->codec_fallback &&
      hfp_hal_interface::get_swb_supported()) {
    esco_codec = UUID_CODEC_LC3;
  }
