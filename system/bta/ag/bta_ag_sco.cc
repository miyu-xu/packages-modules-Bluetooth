...
static void bta_ag_sco_disc_cback(uint16_t sco_idx) {
  ...
  if (handle != 0) {
    ...
    if (bta_ag_cb.sco.p_curr_scb->inuse_codec == UUID_CODEC_MSBC ||
      ...
      if (bta_ag_cb.sco.p_curr_scb->try_zero_retransmission_effort == false) {
        bta_ag_cb.sco.p_curr_scb->try_zero_retransmission_effort = true;
        bta_ag_cb.sco.p_curr_scb->state = BTA_AG_SCO_CODEC_ST;
...
void bta_ag_create_sco(tBTA_AG_SCB* p_scb, bool is_orig) {
  ...
  if (IS_FLAG_ENABLED(
          fall_back_to_sco_connection_if_esco_connection_is_failed) &&
      p_scb->try_zero_retransmission_effort == true) {
    p_scb->try_zero_retransmission_effort = false;
