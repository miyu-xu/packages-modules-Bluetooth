...
void bta_ag_rfc_do_open(tBTA_AG_SCB* p_scb, const tBTA_AG_DATA& data) {
  ...
  if (status == PORT_SUCCESS) {
    ...
    bta_collect_rfc_metrics_after_port_fail(
            static_cast<tPORT_RESULT>(status), p_scb->sdp.sdp_initiated, p_scb->sdp.status,
            p_scb->peer_addr, 0, BTA_SEC_AUTHENTICATE | BTA_SEC_ENCRYPT, true,
            (p_scb->sdp.sdp_start_ms - p_scb->sdp.sdp_end_ms));
