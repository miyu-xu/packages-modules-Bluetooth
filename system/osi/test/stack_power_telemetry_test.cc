 ...
 public:
  ...
  void resetNodes() {
    ...
    for (int i = 0; i < kLogEntriesSize; i++) {
      log_data_containers_[i] = LogDataContainer();
    }
  }
  ...
  int getRecordIndex() { return idx_containers; }
...
TEST_F(PowerTelemetryTest, test_getCurrentLogDataContainer) {
  ...
  ASSERT_EQ(1, MockPowerTelemetry::GetInstance().getRecordIndex());
}
...
TEST_F(PowerTelemetryTest, test_recordLogDataContainer) {
  ...
  for (int i = 0; i < kLogEntriesSize - 1; i++) {
    MockPowerTelemetry::GetInstance().RecordLogDataContainer();
    ASSERT_EQ(i + 1, MockPowerTelemetry::GetInstance().getRecordIndex());
  }

  // Create 1 more node. index integer should be 0
  MockPowerTelemetry::GetInstance().RecordLogDataContainer();
  ASSERT_EQ(0, MockPowerTelemetry::GetInstance().getRecordIndex());
}
...
TEST_F(PowerTelemetryTest, test_LogBleAdvDetails) {
  ...
  MockPowerTelemetry::GetInstance().LogBleAdvDetails(false);
  ASSERT_EQ(1, (int)ldc.adv_list.size());
  ASSERT_NE(0, ldc.adv_list.back().end_time_stamp);
...
TEST_F(PowerTelemetryTest, test_LogTxPower) {
  ...
  // init data
  MockPowerTelemetry::GetInstance().LogLinkDetails(handle, bdaddr, isConnected,
                                                   true);
...
TEST_F(PowerTelemetryTest, test_LogHciCmdEvtDetails) {
  ...
  MockPowerTelemetry::GetInstance().LogHciCmdEvtDetails(kDumpHciCmd);
  ...
  MockPowerTelemetry::GetInstance().LogHciCmdEvtDetails(kDumpHciEvt);
  ASSERT_EQ(1, (int)MockPowerTelemetry::GetInstance().getCmdCount());
...
TEST_F(PowerTelemetryTest, test_LogTrafficData) {
  ...
  MockPowerTelemetry::GetInstance().LogTrafficData();
  ASSERT_EQ(1, (int)MockPowerTelemetry::GetInstance().getRecordIndex());
}
...
TEST_F(PowerTelemetryTest, test_LogAclPktDetails) {
  ...
  MockPowerTelemetry::GetInstance().LogAclPktDetails(kAclTxPacket, 10);
  ASSERT_EQ(1, (int)MockPowerTelemetry::GetInstance().getAclTxPkt());
  ASSERT_EQ(10, (int)MockPowerTelemetry::GetInstance().getAclTxLen());

  MockPowerTelemetry::GetInstance().LogAclPktDetails(kAclRxPacket, 11);
...
TEST_F(PowerTelemetryTest, test_LogChannelConnected) {
  ...
  ASSERT_EQ(kStateConnected, ldc.channel_map[bdaddr].back().state);
...
TEST_F(PowerTelemetryTest, test_LogChannelDisconnected) {
  ...
  ASSERT_EQ(kStateDisconnected, ldc.channel_map[bdaddr].back().state);
...
TEST_F(PowerTelemetryTest, test_LogTxBytes) {
  ...
  MockPowerTelemetry::GetInstance().LogTxBytes(kChannelRfcomm, 0, 0, bdaddr,
                                               10);
  ASSERT_EQ(10, (int)MockPowerTelemetry::GetInstance().getRfcTxBytes());

  MockPowerTelemetry::GetInstance().LogTxBytes(kChannelL2cap, 0, 0, bdaddr, 11);
...
TEST_F(PowerTelemetryTest, test_LogRxBytes) {
  ...
  MockPowerTelemetry::GetInstance().LogRxBytes(kChannelRfcomm, 0, 0, bdaddr,
                                               10);
  ASSERT_EQ(10, (int)MockPowerTelemetry::GetInstance().getRfcRxBytes());

  MockPowerTelemetry::GetInstance().LogRxBytes(kChannelL2cap, 0, 0, bdaddr, 11);
...
TEST_F(PowerTelemetryTest, test_feature_flag) {
  ...
  MockPowerTelemetry::GetInstance().LogTrafficData();
  ASSERT_EQ(0, (int)MockPowerTelemetry::GetInstance().getRecordIndex());
