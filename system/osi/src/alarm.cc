...
<<<<<<< PATCH SET (b8969e Add "bluetooth.core.gap.le.privacy.wakeup_for_rotation" flag)
#include <inttypes.h>
=======
>>>>>>> BASE      (1305e5 Merge "RootCanal: Typo in LE Apcf Transport Discovery Data c)
#include <malloc.h>
#include <pthread.h>
#include <signal.h>
#include <string.h>
...
static bool lazy_initialize(void) {
  ...
  // wakeup to perform RPA rollovers for privacy reasons.
  if (!mobile_device) {
    wakeup_enabled =
  ...
  if (!wakeup_enabled ||
    ...
    if (!timer_create_internal(CLOCK_BOOTTIME, &wakeup_timer)) {
      LOG_ERROR("%s unable to create wakeup_timer: %s", __func__,
                strerror(errno));
      goto error;
    }
  }
