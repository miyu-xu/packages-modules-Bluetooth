...
static bool config_parse_entry(config_t* config, std::string section,
                               std::string line);
  ...
  while (std::getline(config_file, line)) {
    ...
    if (line.front() == '[') {
      ...
      while ((pos = line.find(delimiter)) != std::string::npos) {
        ...
        config_parse_entry(config, section, entry);
        line = line.erase(0, pos + delimiter.length());
      }
      config_parse_entry(config, section, line);
    }
  }
  return true;
}
...
static bool config_parse_entry(config_t* config, std::string section,
                               std::string line) {
  LOG(INFO) << __func__ << " line: " << line.c_str();
