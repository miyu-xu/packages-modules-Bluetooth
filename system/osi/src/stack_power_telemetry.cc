...
PowerTelemetry* PowerTelemetry::GetInstance() {
  static PowerTelemetry power_telemetry;
  return &power_telemetry;
}

LogDataContainer* PowerTelemetry::GetCurrentLogDataContainer() {
  return (LogDataContainer*)log_data_containers_.back();
}
...
void PowerTelemetry::LogChannelConnected(int channel_type, int src_id,
  ...
  if (ldc->channel_map.count(bdaddr) == 0) {
    ...
    ldc->channel_map.insert(std::pair<RawAddress, list<ChannelDetails>>(
...
void PowerTelemetry::PowerTelemetryDump(int fd) {
  ...
  for (auto ldc : log_data_containers_) {
    ...
    dprintf(fd, "%-22s %-22s %-12d %-12ld %-12d %-12ld\n",
            ...
            ldc->acl_pkt_ds.tx_pkt_count, ldc->acl_pkt_ds.tx_total_bytes,
            ldc->acl_pkt_ds.rx_pkt_count, ldc->acl_pkt_ds.rx_total_bytes);
  }
  ...
  for (auto ldc : log_data_containers_) {
    for (auto& itr : ldc->channel_map) {
      ...
      list<ChannelDetails> channel_details_list = itr.second;
      for (auto& channel_details : channel_details_list) {
        ...
        dprintf(fd, "%-7s %-7d %-7d %-8d %-22s %-22s %-14s",
                ...
                (channel_details.state == STATE_DISCONNECTED) ? "DISCONNECTED"
                                                              : "CONNECTED");
        if (log_per_channel_ == true) {
          dprintf(fd, "%-10ld %-10ld %-22s %-22s", channel_details.tx_bytes,
                  channel_details.rx_bytes,
  ...
  for (auto ldc : log_data_containers_) {
    dprintf(fd, "%-22s %-22s %-10ld %-10ld\n", ldc->start_time_stamp.c_str(),
            ldc->end_time_stamp.c_str(), ldc->l2c_data.tx_bytes,
            ldc->l2c_data.rx_bytes);
  }
  ...
  for (auto ldc : log_data_containers_) {
    dprintf(fd, "%-22s %-22s %-10ld %-10ld\n", ldc->start_time_stamp.c_str(),
            ldc->end_time_stamp.c_str(), ldc->rfc_data.tx_bytes,
            ldc->rfc_data.rx_bytes);
  }
  ...
  for (auto ldc : log_data_containers_) {
    for (auto itr : ldc->sniff_activity_map) {
      ...
      dprintf(fd, "%-8d %-19s %-19d %-24ld %-19d %-24ld\n", handle,
              ...
              sniff_data.active_duration, sniff_data.sniff_count,
              sniff_data.sniff_duration);
    }
  }
