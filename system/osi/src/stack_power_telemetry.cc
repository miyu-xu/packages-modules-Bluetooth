...
void power_telemetry::PowerTelemetry::LogLinkDetails(uint16_t handle,
  ...
  std::map<uint16_t, LinkDetails>& link_map =
      isAclOrSCO ? ldc.acl_link_map : ldc.sco_link_map;
  std::list<LinkDetails>& link_list =
      isAclOrSCO ? ldc.acl_link_list : ldc.sco_link_list;

  if (link_map.count(handle) != 0) {
    link_details = link_map[handle];
  }
