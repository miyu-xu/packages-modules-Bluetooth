...
std::shared_ptr<power_telemetry::LogDataContainer>
power_telemetry::PowerTelemetry::GetCurrentLogDataContainer() {
  return log_data_containers_.back();
}

void power_telemetry::PowerTelemetry::RecordLogDataContainer() {
  std::shared_ptr<power_telemetry::LogDataContainer> ldc =
  ...
  if (log_data_containers_.size() == LOG_DATA_ENTRIES_IN_MEMORY) {
    ldc = log_data_containers_.front();
    log_data_containers_.pop_front();
  }

  ldc = std::make_shared<power_telemetry::LogDataContainer>();
