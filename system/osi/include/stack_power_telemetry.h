...
class PowerTelemetry {
 public:
  PowerTelemetry() {
    ...
    std::shared_ptr<LogDataContainer> ldc =
        std::make_shared<LogDataContainer>();
    log_data_containers_.push_back(ldc);
    log_per_channel_ = osi_property_get_bool(
        std::string(LOG_PER_CHANNEL_PROPERTY).c_str(), false);
  }
  std::shared_ptr<LogDataContainer> GetCurrentLogDataContainer();
 ...
 protected:
  std::list<std::shared_ptr<LogDataContainer>> log_data_containers_;
