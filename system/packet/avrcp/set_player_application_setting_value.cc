...
uint8_t SetPlayerApplicationSettingValueRequest::GetNumPlayerAttributes()
    const {
...
SetPlayerApplicationSettingValueRequest::GetPlayerAttributesRequested() const {
  auto it = begin() + VendorPacket::kMinSize() +
            static_cast<size_t>(1);  // Skip number of attributes
  std::vector<PlayerAttribute> attribute_list;

  for (; it < end();) {
    attribute_list.push_back(static_cast<PlayerAttribute>(it.extract8()));
    it++;
  }
...
std::vector<uint8_t>
SetPlayerApplicationSettingValueRequest::GetPlayerAttributeValuesRequested()
    const {
  auto it = begin() + VendorPacket::kMinSize() +
            static_cast<size_t>(1);  // Skip number of attributes
  ...
  for (++it; it < end();) {
    values_list.push_back(static_cast<uint8_t>(it.extract8()));
    it++;
  }
...
bool SetPlayerApplicationSettingValueRequest::IsValid() const {
  ...
  size_t num_attributes = GetNumPlayerAttributes();
  auto attr_start = begin() + VendorPacket::kMinSize() + static_cast<size_t>(1);

  return (num_attributes * 2 * sizeof(uint8_t)) == (size_t)(end() - attr_start);
}
...
std::string SetPlayerApplicationSettingValueRequest::ToString() const {
  ...
  ss << "  └ Num Attributes = " << loghex(GetNumPlayerAttributes())
     << std::endl;

  auto attribute_list_ = GetPlayerAttributesRequested();
  auto values_list_ = GetPlayerAttributeValuesRequested();
