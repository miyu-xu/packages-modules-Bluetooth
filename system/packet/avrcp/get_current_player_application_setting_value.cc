...
size_t GetCurrentPlayerApplicationSettingValueResponseBuilder::size() const {
  ...
  len += sizeof(uint8_t);     // Number of attributes
  len += attributes_.size();  // Attributes
  len += values_.size();      // Attributes values
  return len;
}
...
uint8_t GetCurrentPlayerApplicationSettingValueRequest::GetNumPlayerAttributes()
    const {
...
GetCurrentPlayerApplicationSettingValueRequest::GetPlayerAttributesRequested()
    const {
  auto it = begin() + VendorPacket::kMinSize();
...
std::string GetCurrentPlayerApplicationSettingValueRequest::ToString() const {
  ...
  auto attr_list = GetPlayerAttributesRequested();
