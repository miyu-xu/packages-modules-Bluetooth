...
ListPlayerApplicationSettingValuesResponseBuilder::MakeBuilder(
    uint8_t num_of_values, std::vector<uint8_t> values) {
  std::unique_ptr<ListPlayerApplicationSettingValuesResponseBuilder> builder(
      new ListPlayerApplicationSettingValuesResponseBuilder(num_of_values,
                                                            values));
...
size_t ListPlayerApplicationSettingValuesResponseBuilder::size() const {
  ...
  len += sizeof(num_of_values_);            // Number of setting values
  len += num_of_values_ * sizeof(uint8_t);  // Setting value size
  return len;
}
...
bool ListPlayerApplicationSettingValuesResponseBuilder::Serialize(
  ...
  AddPayloadOctets1(pkt, (uint8_t)num_of_values_);
  for (auto value : values_) {
    AddPayloadOctets1(pkt, (uint8_t)value);
  }

  return true;
}

PlayerAttribute ListPlayerApplicationSettingValuesRequest::GetPlayerAttribute()
    const {
...
std::string ListPlayerApplicationSettingValuesRequest::ToString() const {
  ...
  ss << "  └ Player Setting Attribute = " << GetPlayerAttribute() << std::endl;
  ss << std::endl;
