...
class Report {
 ...
 public:
  ...
  std::string get() const { return maker.str(); }

  void clear() { maker.str(""); }
};

inline bool operator==(AttributeEntry a, AttributeEntry b) {
  return (a.attribute() == b.attribute()) && (a.value() == b.value());
}

inline bool operator!=(AttributeEntry a, AttributeEntry b) { return !(a == b); }
...
class AttributesResponseBuilderTestUser {
 ...
 private:
  ...
  size_t expected_size() { return Builder::kHeaderSize() + this->current_size; }

 public:
  std::string getReport() const { return this->report.get(); }

  AttributesResponseBuilderTestUser(size_t m_size, Maker maker)
      : maker(maker), builder(maker(m_size)), mtu(m_size) {
    this->report.write(
        "AttributesResponseBuilderTestUser: start test for mtu \"%zu\"\n",
        this->mtu);
  }
  ...
  void startTest(size_t m_size) {
    ...
    this->report.write(
        "AttributesResponseBuilderTestUser: starts test for mtu \"%zu\"\n",
        this->mtu);
    this->order_test_result = true;
    this->test_result = true;
  }

  bool testResult() const { return this->test_result; }

  bool testOrder() { return this->order_test_result; }

  void finishTest() {
    this->reset();
    if (order_control.size() != sended_order.size()) {
      this->report.write(
          "testOrder FAIL: the count of entries which should send (%zu)\
                        is not equal to sended entries%zu)) \n input:\n%s\n sended:\n%s\n",
          order_control.size(), sended_order.size(),
          to_string(order_control).c_str(), to_string(sended_order).c_str());
    ...
    this->report.write(
        "AttributesResponseBuilderTestUser: ends test for mtu \"%zu\"\n",
        this->mtu);
  }
 ...
 private:
  void wholeEntry(size_t f, AttributeEntry&& entry) {
    ...
    if (this->builder->size() != this->expected_size()) {
      this->report.write(
          "AttributesResponseBuilderTestUser FAIL for \"%s\":"
  ...
  void fractionEntry(size_t f, AttributeEntry&& entry) {
    ...
    if (builder->entries_.size() == 0) {
      report.write(
          "AttributesResponseBuilderTestUser FAIL: MTU %zu too small\n",
          this->mtu);
      test_result = false;
      this->order_control.push_back(entry);
      this->reset();
...
class FructionEntryBuildTestHelper {
 public:
  ...
  FructionEntryBuildTestHelper(size_t mtu, Maker m) : helper(mtu, m) {}
  ...
  Result runTest(const TestCollection& test_data, size_t mtu) {
    ...
    return std::make_tuple(this->helper.testResult(), this->helper.testOrder(),
  ...
  template <class TestCollection>
  Result runTestWithUnexpectedPrintf(const TestCollection& test_data,
                                     size_t mtu, bool expected_fruction = true,
                                     bool expected_ordering = true) {
    auto result = this->runTest(test_data, mtu);
    std::string report;
    bool fruction_pass;
    bool ordering_pass;
    std::tie(fruction_pass, ordering_pass, report) = result;
    if (fruction_pass != expected_fruction ||
        ordering_pass != expected_ordering) {
