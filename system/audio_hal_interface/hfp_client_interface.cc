...
CodecId get_codec_id_by_peer_codec(tBTA_AG_PEER_CODEC sco_codec) {
  if (sco_codec & BTM_SCO_CODEC_LC3) {
    return CodecId::Core::LC3;
  }
  if (sco_codec & BTM_SCO_CODEC_MSBC) {
    return CodecId::Core::MSBC;
  }
  if (sco_codec & BTM_SCO_CODEC_CVSD) {
...
std::unordered_map<int, ::hfp::sco_config> HfpClientInterface::Offload::GetHfpScoConfig() {
