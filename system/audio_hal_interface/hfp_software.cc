...
void get_audio_configuration(AudioConfigurationAIDL* audio_config) {
  auto cb = get_hfp_active_device_callback();
  if (cb == nullptr) return;
  AudioConfigurationAIDL local_audio_config;
  if (is_aidl_sink_offload_session()) {
    local_audio_config.set<AudioConfigurationAIDL::hfpConfig>(
        get_hfp_config_from_peer_codec(cb->sco_codec, cb->conn_handle));
  } else {
    // Populate default pcm configuration
    local_audio_config.set<AudioConfigurationAIDL::pcmConfig>(
...
void HfpClientInterface::Sink::StartSession() {
  ...
  auto cb = get_hfp_active_device_callback();
  if (cb == nullptr) return;
  AudioConfigurationAIDL* audio_config = nullptr;
  get_audio_configuration(audio_config);
