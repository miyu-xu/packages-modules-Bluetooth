...
std::unique_ptr<A2dpClientInterface> a2dp_client_interface;
...
bool is_hal_enabled() {
  if (a2dp_client_interface) {
    return a2dp_client_interface->IsEnabled();
  }
  return false;
}
...
bool is_hal_offloading() {
  if (a2dp_client_interface) {
    return a2dp_client_interface->IsOffloadEnabled();
  }
  return false;
}
...
bool init(bluetooth::common::MessageLoopThread* message_loop,
  ...
  if (a2dp_client_interface != nullptr && offload_enabled &&
      !a2dp_client_interface->IsOffloadAvailable()) {
    log::info("Update a2dp_client_interface with HardwareOffload. Reinitializing HAL.",
              offload_enabled);
    a2dp_client_interface->Cleanup(true);
    a2dp_client_interface.reset();
  }

  if (a2dp_client_interface != nullptr && a2dp_client_interface->IsEnabled()) {
  ...
  if (HalVersionManager::GetHalTransport() == BluetoothAudioHalTransport::HIDL) {
    ...
    a2dp_client_interface = std::make_unique<A2dpClientInterface>(
  ...
  if (a2dp_client_interface && A2dpClientInterface::remote_delay != 0) {
    log::info("restore DELAY {} ms", static_cast<float>(A2dpClientInterface::remote_delay / 10.0));
    a2dp_client_interface->SetRemoteDelay(A2dpClientInterface::remote_delay);
...
void cleanup() {
  log::info("");
  if (a2dp_client_interface) {
    a2dp_client_interface->Cleanup(false);
  }
  a2dp_client_interface.reset();
}
...
bool setup_codec(A2dpCodecConfig* a2dp_config, uint16_t peer_mtu,
  ...
  log::info("");
  if (a2dp_client_interface) {
    return a2dp_client_interface->UpdateAudioConfigToHal(a2dp_config, peer_mtu,
                                                         preferred_encoding_interval_us);
  }
  return false;
}
...
void start_session() {
  log::info("");
  if (a2dp_client_interface) {
    a2dp_client_interface->StartSession();
  }
}
...
void end_session() {
  log::info("");
  if (a2dp_client_interface) {
    a2dp_client_interface->StopSession();
  }
}
...
void ack_stream_started(Status status) {
  log::info("");
  if (a2dp_client_interface) {
    a2dp_client_interface->ConfirmStreamStarted(status);
  }
}
...
void ack_stream_suspended(Status status) {
  log::info("");
  if (a2dp_client_interface) {
    a2dp_client_interface->ConfirmStreamSuspended(status);
  }
}
...
size_t read(uint8_t* p_buf, uint32_t len) {
  if (a2dp_client_interface) {
    return a2dp_client_interface->Read(p_buf, len);
  }
  return 0;
}
...
void set_audio_low_latency_mode_allowed(bool allowed) {
  ...
  if (a2dp_client_interface) {
    a2dp_client_interface->SetLowLatencyMode(A2dpClientInterface::is_low_latency_mode_allowed);
  } else {
...
std::optional<btav_a2dp_codec_index_t> sink_codec_index(const uint8_t* p_codec_info) {
  if (a2dp_client_interface) {
    return a2dp_client_interface->SinkCodecIndex(p_codec_info);
  }
  return std::nullopt;
}
...
std::optional<btav_a2dp_codec_index_t> source_codec_index(const uint8_t* p_codec_info) {
  if (a2dp_client_interface) {
    return a2dp_client_interface->SourceCodecIndex(p_codec_info);
  }
  return std::nullopt;
}
...
std::optional<const char*> codec_index_str(btav_a2dp_codec_index_t codec_index) {
  if (a2dp_client_interface) {
    return a2dp_client_interface->CodecIndexStr(codec_index);
  }
  return std::nullopt;
}
...
bool supports_codec(btav_a2dp_codec_index_t codec_index) {
  if (a2dp_client_interface) {
    return a2dp_client_interface->SupportsCodec(codec_index);
  }
  return false;
}
...
bool codec_info(btav_a2dp_codec_index_t codec_index, bluetooth::a2dp::CodecId* codec_id,
  ...
  if (a2dp_client_interface) {
    return a2dp_client_interface->CodecInfo(codec_index, codec_id, codec_info, codec_config);
  }
  return false;
}
...
std::optional<a2dp_configuration> get_a2dp_configuration(
        ...
        btav_a2dp_codec_config_t const& user_preferences) {
  if (a2dp_client_interface) {
    return a2dp_client_interface->GetA2dpConfiguration(peer_address, remote_seps, user_preferences);
  }
  return std::nullopt;
}
...
tA2DP_STATUS parse_a2dp_configuration(btav_a2dp_codec_index_t codec_index,
                                      ...
                                      std::vector<uint8_t>* vendor_specific_parameters) {
  if (a2dp_client_interface) {
    return a2dp_client_interface->ParseA2dpConfiguration(codec_index, codec_info, codec_parameters,
