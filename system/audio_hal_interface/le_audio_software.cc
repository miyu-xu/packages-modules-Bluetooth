...
LeAudioClientInterface::Sink* LeAudioClientInterface::GetSink(
  ...
  if (is_broadcasting_session_type && HalVersionManager::GetHalTransport() ==
      BluetoothAudioHalTransport::HIDL) {
    LOG(WARNING) << __func__ << ", No support for broadcasting Le Audio on HIDL";
    return nullptr;
  }

  /* TODO: Add support for simultanous unicast and broadcast transports */
  if (broadcast_sink_ && unicast_sink_) {
    LOG(INFO) << __func__ << ", No support for simultanous unicast and "
              " broadacst session";
    return nullptr;
  }
  ...
  if (HalVersionManager::GetHalTransport() ==
    ...
    aidl::SessionType session_type = is_broadcasting_session_type ?
    ...
    if (CodecManager::GetInstance()->GetCodecLocation() !=
        CodecLocation::HOST) {
      session_type = is_broadcasting_session_type ?
