...
void setParams(LeAudioClientInterface::PcmParameters* params, FuzzedDataProvider* fdp) {
...
void setOffloadConfig(bluetooth::le_audio::offload_config* config, FuzzedDataProvider* fdp) {
...
void setBroadcastOffloadConfig(bluetooth::le_audio::broadcast_offload_config* config,
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  bluetooth::common::MessageLoopThread messageLoopThread(kMessageLoopThreadName);
  ...
  if (!sink->IsBroadcaster()) {
    ...
    std::string propertyValue = android::base::GetProperty(kPropertyName, "false");
    if (fdp.ConsumeBool()) {
  ...
  while (fdp.remaining_bytes()) {
    auto invokeLeAudioSoftwareAPI = fdp.PickValueInArray<const std::function<void()>>(
            {[&]() {
               source->SetRemoteDelay(fdp.ConsumeIntegral<uint16_t>() /* delay_report_ms */);
             },
             ...
             [&]() { sink->SetRemoteDelay(fdp.ConsumeIntegral<uint16_t>() /* delay_report_ms */); },
             [&]() { sink->SetPcmParameters(params); }, [&]() { sink->ConfirmStreamingRequest(); },
