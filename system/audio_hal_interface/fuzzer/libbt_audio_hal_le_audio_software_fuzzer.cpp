...
bluetooth::audio::le_audio::StreamCallbacks streamCallbacks = {
    ...
    [](const source_metadata_v7_t&, le_audio::DsaMode) {
      return true;
    } /*onMetadataUpdate*/,
...
void setOffloadConfig(le_audio::offload_config* config,
                      FuzzedDataProvider* fdp) {
  uint8_t streamMapSize = fdp->ConsumeIntegral<uint8_t>();
  for (uint8_t i = 0; i < streamMapSize; ++i) {
    le_audio::stream_map_info stream_map(
...
void setBroadcastOffloadConfig(le_audio::broadcast_offload_config* config,
                               FuzzedDataProvider* fdp) {
  uint8_t streamMapSize = fdp->ConsumeIntegral<uint8_t>();
  for (uint8_t i = 0; i < streamMapSize; ++i) {
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  if (!sink->IsBroadcaster()) {
    le_audio::DsaModes dsaModes;
    if (fdp.ConsumeBool()) {
      dsaModes.push_back(le_audio::DsaMode::DISABLED);
    }
    if (fdp.ConsumeBool()) {
      dsaModes.push_back(le_audio::DsaMode::ACL);
    }
    if (fdp.ConsumeBool()) {
      dsaModes.push_back(le_audio::DsaMode::ISO_SW);
    }
    if (fdp.ConsumeBool()) {
      dsaModes.push_back(le_audio::DsaMode::ISO_HW);
    }
    ...
    auto invokeLeAudioSoftwareAPI =
        fdp.PickValueInArray<const std::function<void()>>(
            {[&]() {
             ...
             [&]() {
               le_audio::offload_config config;
             ...
             [&]() {
               le_audio::broadcast_offload_config config;
