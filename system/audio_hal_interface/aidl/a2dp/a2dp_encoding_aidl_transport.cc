 ...
 *
 ***/
A2dpTransport::A2dpTransport(StreamCallbacks const* stream_callbacks)
...
/***
 *
 * A2dpEncodingTransport functions and variables
 *
 ***/
A2dpEncodingTransport::A2dpEncodingTransport(
        SessionType session_type, bluetooth::audio::a2dp::StreamCallbacks const* stream_callbacks)
    : IBluetoothTransportInstance(session_type, (AudioConfiguration){}) {
  transport_ = new A2dpTransport(stream_callbacks);
}

A2dpEncodingTransport::~A2dpEncodingTransport() { delete transport_; }

Status A2dpEncodingTransport::StartRequest(bool is_low_latency) {
  return transport_->StartRequest(is_low_latency);
}

Status A2dpEncodingTransport::SuspendRequest() { return transport_->SuspendRequest(); }

void A2dpEncodingTransport::StopRequest() { transport_->StopRequest(); }
...
tA2DP_CTRL_CMD A2dpEncodingTransport::GetPendingCmd() const { return transport_->GetPendingCmd(); }

void A2dpEncodingTransport::ResetPendingCmd() { transport_->ResetPendingCmd(); }

void A2dpEncodingTransport::ResetPresentationPosition() { transport_->ResetPresentationPosition(); }

void A2dpEncodingTransport::LogBytesRead(size_t bytes_read) {
  transport_->LogBytesRead(bytes_read);
}

// delay reports from AVDTP is based on 1/10 ms (100us)
void A2dpEncodingTransport::SetRemoteDelay(uint16_t delay_report) {
  transport_->SetRemoteDelay(delay_report);
}
