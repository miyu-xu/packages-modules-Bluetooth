...
size_t BluetoothAudioClientInterface::ReadAudioData(uint8_t* p_buf, uint32_t len) {
  ...
  if (com::android::bluetooth::flags::a2dp_fmq_read_blocking()) {
    ...
    auto status = data_mq_->readBlocking(reinterpret_cast<int8_t*>(p_buf), len,
                                         kDefaultDataReadTimeoutMs * 1000000);

    if (!status) {
      log::warn("failed to read {} bytes after {} ms", len, kDefaultDataReadTimeoutMs);
      return 0;
    }

    sink_->LogBytesRead(len);
    return len;
  }
