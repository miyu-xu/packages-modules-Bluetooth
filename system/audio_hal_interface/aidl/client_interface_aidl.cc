...
const uint8_t FETCH_AUDIO_PROVIDER_RETRY_NO = 3;
const uint32_t FETCH_AUDIO_PROVIDER_RETRY_SLEEP_MS = 50;
...
void BluetoothAudioClientInterface::FetchAudioProvider() {
  ...
  // Preventing breakage due to audioserver restarting in the middle of fetch
  // with a retry mechanism.
  // When audioserver restart, IBluetoothAudioProviderFactory service is also
  // re-registered.
  for (int retry_no = 0; retry_no < FETCH_AUDIO_PROVIDER_RETRY_NO; ++retry_no) {
    auto provider_factory = IBluetoothAudioProviderFactory::fromBinder(::ndk::SpAIBinder(
            AServiceManager_waitForService(kDefaultAudioProviderFactoryInterface.c_str())));
    ...
    if (!aidl_retval.isOk()) {
      log::error("BluetoothAudioHal::openProvider failure: {}", aidl_retval.getDescription());
      // Sleep 50ms to allow graceful restart.
      std::this_thread::sleep_for(std::chrono::milliseconds(FETCH_AUDIO_PROVIDER_RETRY_SLEEP_MS));
    } else {
