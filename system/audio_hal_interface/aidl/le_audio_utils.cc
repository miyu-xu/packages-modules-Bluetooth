    ...
    if (configs.size() > 0) {
      ...
      while (current_config != configs.end()) {
        ...
        while (++last_equal_config != configs.end()) {
          if (current_codec != last_equal_config->codec) {
            break;
          }
        }
        // Channel configuration
        cfg_str << std::distance(current_config, last_equal_config) << "-"
                << +current_codec.GetChannelCountPerIsoStream() << "chan";
        cfg_str << "-" << dir_str << "Ase-";
        // Codec Id
        ...
        // There is some different config left
        current_config = last_equal_config + 1;
      }
    }
    return cfg_str.str();
  };
