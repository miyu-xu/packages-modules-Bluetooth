...
template <class T>
bool ContainedInBitmask(const T& bitmask, const T& target) {
...
bool sbc_offloading_capability_match(const SbcCapabilities& sbc_capability,
  ...
  if (!ContainedInBitmask(sbc_capability.channelMode, sbc_config.channelMode) ||
      !ContainedInBitmask(sbc_capability.allocMethod, sbc_config.allocMethod) ||
...
bool aac_offloading_capability_match(const AacCapabilities& aac_capability,
  ...
  if (!ContainedInBitmask(aac_capability.channelMode, aac_config.channelMode) ||
      !ContainedInBitmask(aac_capability.objectType, aac_config.objectType) ||
...
bool aptx_offloading_capability_match(const AptxCapabilities& aptx_capability,
  ...
  if (!ContainedInBitmask(aptx_capability.channelMode,
...
bool ldac_offloading_capability_match(const LdacCapabilities& ldac_capability,
  ...
  if (!ContainedInBitmask(ldac_capability.channelMode,
...
bool UpdateOffloadingCapabilities(
  ...
  for (auto capability : audio_hal_capabilities) {
    if ((static_cast<uint32_t>(
             capability.get<AudioCapabilities::codecCapabilities>().codecType) &
         codec_type_masks) != 0) {
      LOG(INFO) << __func__
...
bool IsCodecOffloadingEnabled(const CodecConfiguration& codec_config) {
  for (auto preference : offloading_preference) {
    if (codec_config.codecType !=
        preference.get<AudioCapabilities::codecCapabilities>().codecType) {
      continue;
    }
    auto codec_capability =
        preference.get<AudioCapabilities::codecCapabilities>();
