...
bool hal_bcast_capability_to_stack_format(
  ...
  auto& lc3_capabilities = hal_bcast_capability.leAudioCodecCapabilities.get<
      BroadcastCapability::LeAudioCodecCapabilities::lc3Capabilities>();

  if (lc3_capabilities->empty() || lc3_capabilities->size() > 1) {
    LOG(WARNING) << __func__ << ": The number of config is unsupported.";
  }

  auto supportedChannel = hal_bcast_capability.supportedChannel;
  auto sample_rate_hz = (*lc3_capabilities)[0]->samplingFrequencyHz[0];
  auto frame_duration_us = (*lc3_capabilities)[0]->frameDurationUs[0];
  auto octets_per_frame = (*lc3_capabilities)[0]->octetsPerFrame[0];
