...
#define LOG_TAG "TestAIDLProviderInfo"
...
CodecId::Vendor test_foobar_codec_id = {.id = 0x00112233, .codecId = 0x1122};
CodecId::Vendor test_unknown_foobar_codec_id = {.id = 0x12345678,
                                                .codecId = 0x1234};
...
class ProviderInfoTest : public Test {
 public:
  ...
  void GetProviderInfoForTesting(bool include_source_codecs,
    ...
    provider_info = ProviderInfo::GetProviderInfo();
 ...
 protected:
  void SetUp() override {
    ...
    CreateTestA2dpCodecInfo(
        ...
        std::vector<uint8_t>{0x06, 0x00, 0x00, 0x3f, 0xff, 0x02,
                             0x25}, /* capabilities */
    ...
    CreateTestA2dpCodecInfo(
        ...
        std::vector<uint8_t>{0x08, 0x00, 0x02, 0x80, 0x01, 0x8c, 0x83, 0xe8,
                             0x00}, /* capabilities */
        std::vector<ChannelMode>{ChannelMode::MONO, ChannelMode::STEREO,
                                 ChannelMode::DUALMONO}, /* channelMode */
        std::vector<int32_t>{44100, 48000}, /* samplingFrequencyHz */
    ...
    CreateTestA2dpCodecInfo(
        ...
        std::vector<uint8_t>{0x09, 0x00, 0xff, 0xe0, 0x00, 0x00, 0x00, 0x01,
                             0x00, 0x3c}, /* capabilities */
        std::vector<ChannelMode>{ChannelMode::MONO, ChannelMode::STEREO,
                                 ChannelMode::DUALMONO}, /* channelMode */
        std::vector<int32_t>{44100, 48000}, /* samplingFrequencyHz */
        std::vector<int32_t>{16, 24, 32},   /* bitdepth */
    ...
    CreateTestA2dpCodecInfo(
        ...
        std::vector<uint8_t>{0x09, 0x00, 0xff, 0x33, 0x22, 0x11, 0x00, 0x22,
                             0x11, 0x3c}, /* capabilities */
        std::vector<ChannelMode>{ChannelMode::MONO, ChannelMode::STEREO,
                                 ChannelMode::DUALMONO}, /* channelMode */
        std::vector<int32_t>{44100, 48000}, /* samplingFrequencyHz */
        std::vector<int32_t>{16, 24, 32},   /* bitdepth */
...
TEST_F_WITH_FLAGS(ProviderInfoTest, TestGetProviderInfoReturnsNullptr,
...
TEST_F_WITH_FLAGS(ProviderInfoTest, TestGetCodec,
                  REQUIRES_FLAGS_ENABLED(ACONFIG_FLAG(
                      TEST_BT, a2dp_offload_codec_extensibility))) {
  GetProviderInfoForTesting(true, false);
  ...
  if (received_codec_info_sbc.has_value()) {
  ...
  if (received_codec_info_aac.has_value()) {
  ...
  if (received_codec_info_opus.has_value()) {
  ...
  if (received_codec_info_foobar.has_value()) {
...
TEST_F_WITH_FLAGS(ProviderInfoTest, TestSourceCodecIndexByCodecId,
  ...
  a2dp_codec_index_opt =
      provider_info->SourceCodecIndex(test_unknown_foobar_codec_id);
...
TEST_F_WITH_FLAGS(ProviderInfoTest, TestSourceCodecIndexByCapabilities,
  ...
  auto& a2dpInfo =
      codecInfoArray[0].transport.get<CodecInfo::Transport::Tag::a2dp>();
  a2dp_codec_index_opt =
      provider_info->SourceCodecIndex(a2dpInfo.capabilities.data());
  ASSERT_TRUE(a2dp_codec_index_opt.has_value());
  ASSERT_EQ(a2dp_codec_index_opt.value(), BTAV_A2DP_CODEC_INDEX_SOURCE_SBC);

  a2dpInfo = codecInfoArray[1].transport.get<CodecInfo::Transport::Tag::a2dp>();
  a2dp_codec_index_opt =
      provider_info->SourceCodecIndex(a2dpInfo.capabilities.data());
  ASSERT_TRUE(a2dp_codec_index_opt.has_value());
  ASSERT_EQ(a2dp_codec_index_opt.value(), BTAV_A2DP_CODEC_INDEX_SOURCE_AAC);

  a2dpInfo = codecInfoArray[2].transport.get<CodecInfo::Transport::Tag::a2dp>();
  a2dp_codec_index_opt =
      provider_info->SourceCodecIndex(a2dpInfo.capabilities.data());
  ASSERT_TRUE(a2dp_codec_index_opt.has_value());
  ASSERT_EQ(a2dp_codec_index_opt.value(), BTAV_A2DP_CODEC_INDEX_SOURCE_OPUS);

  a2dpInfo = codecInfoArray[3].transport.get<CodecInfo::Transport::Tag::a2dp>();
  a2dp_codec_index_opt =
      provider_info->SourceCodecIndex(a2dpInfo.capabilities.data());
  ASSERT_TRUE(a2dp_codec_index_opt.has_value());
  ASSERT_EQ(a2dp_codec_index_opt.value(), BTAV_A2DP_CODEC_INDEX_SOURCE_EXT_MIN);
...
TEST_F_WITH_FLAGS(ProviderInfoTest,
  ...
  a2dp_codec_index_opt = provider_info->SourceCodecIndex(
      test_unknown_foobar_codec_id.id, test_unknown_foobar_codec_id.codecId);
  ASSERT_FALSE(a2dp_codec_index_opt.has_value());
}
...
TEST_F_WITH_FLAGS(ProviderInfoTest, TestCodecIndexStr,
  ...
  ASSERT_EQ(provider_info->CodecIndexStr(static_cast<btav_a2dp_codec_index_t>(
                test_unknown_foobar_codec_id.id)),
            std::nullopt);
}
