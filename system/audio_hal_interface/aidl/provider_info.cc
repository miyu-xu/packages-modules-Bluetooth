...
bool ProviderInfo::CodecCapabilities(btav_a2dp_codec_index_t codec_index,
  ...
  std::vector<uint8_t> const& capabilities =
      codec->transport.get<CodecInfo::Transport::a2dp>().capabilities;
  switch (codec->id.getTag()) {
    case CodecId::a2dp: {
      auto id = codec->id.get<CodecId::a2dp>();
      codec_info[0] = static_cast<uint8_t>(id);
      memcpy(codec_info + 1, capabilities.data(), capabilities.size());
      return true;
    }
    case CodecId::vendor: {
      ...
      codec_info[0] = A2DP_MEDIA_CT_NON_A2DP;
      codec_info[1] = static_cast<uint8_t>(vendor_id >> 0);
      codec_info[2] = static_cast<uint8_t>(vendor_id >> 8);
      codec_info[3] = static_cast<uint8_t>(vendor_id >> 16);
      codec_info[4] = static_cast<uint8_t>(vendor_id >> 24);
      codec_info[5] = static_cast<uint8_t>(codec_id >> 0);
      codec_info[6] = static_cast<uint8_t>(codec_id >> 8);
      memcpy(codec_info + 7, capabilities.data(), capabilities.size());
      return true;
    }
    case CodecId::core:
    default:
