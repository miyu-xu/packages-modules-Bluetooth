...
bool a2dp_get_selected_hal_codec_config(CodecConfiguration* codec_config) {
  ...
  if (a2dp_config == nullptr) {
    ...
    *codec_config = ::bluetooth::audio::aidl::codec::kInvalidCodecConfiguration;
    return false;
  }
  ...
  switch (current_codec.codec_type) {
    ...
    default:
      ...
      *codec_config =
          ::bluetooth::audio::aidl::codec::kInvalidCodecConfiguration;
      return false;
  }
...
bool setup_codec() {
  ...
  if (active_hal_interface->GetTransportInstance()->GetSessionType() ==
      SessionType::A2DP_HARDWARE_OFFLOAD_ENCODING_DATAPATH) {
    audio_config.set<AudioConfiguration::codecConfig>(codec_config);
  } else {
