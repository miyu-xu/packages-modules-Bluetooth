...
bool LeAudioClientInterface::ReleaseSink(SinkStub* sink) {
  if (sink != sink_) {
...
SourceStub* LeAudioClientInterface::GetSource(
    StreamCallbacks stream_cb,
    bluetooth::common::MessageLoopThread* message_loop) {
  if (source_ == nullptr) {
    source_ = new Source();
...
bool LeAudioClientInterface::ReleaseSource(SourceStub* source) {
  if (source != source_) {
