...
enh_esco_params_t esco_parameters_for_codec(esco_codec_t codec, bool offload) {
  ...
  if (true) {
#else
  if (IS_FLAG_ENABLED(restrict_pkt_types_on_higher_settings)) {
#endif
    ...
    if (param.packet_types != new_packet_types) {
      log::info(
          "Applying restricted packet types for codec %d: 0x%04x -> 0x%04x",
          codec, param.packet_types, new_packet_types);
