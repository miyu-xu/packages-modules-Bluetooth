...
static bool interop_config_add_or_remove(interop_db_entry_t* db_entry, bool add) {
  ...
  switch (db_entry->bl_type) {
    ...
    case INTEROP_BL_TYPE_NAME: {
      ...
      if (add) {
        ...
        interop_config_remove(interop_feature_string_(feature),
                             db_entry->entry_type.name_entry.name);
      }
      break;
    }
    ...
    case INTEROP_BL_TYPE_VNDR_PRDT: {
      ...
      if (add) {
        interop_config_set_str(interop_feature_string_(feature), m_vnr_pdt,
                               VNDR_PRDT_BASED);
      } else {
        interop_config_remove(interop_feature_string_(feature),
                             m_vnr_pdt);
    ...
    case INTEROP_BL_TYPE_SSR_MAX_LAT: {
      ...
      if (add) {
        interop_config_set_str(interop_feature_string_(feature), m_ssr_max_lat,
                               SSR_MAX_LAT_BASED);
      } else {
        interop_config_remove(interop_feature_string_(feature),
                             m_ssr_max_lat);
    ...
    case INTEROP_BL_TYPE_LMP_VERSION: {
      ...
      if (add) {
        interop_config_set_str(interop_feature_string_(feature), m_lmp_version,
                               LMP_VERSION_BASED);
      } else {
        interop_config_remove(interop_feature_string_(feature),
                             m_lmp_version);
