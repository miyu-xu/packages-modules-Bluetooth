...
static void interop_config_write(UNUSED_ATTR UINT16 event,
...
static void interop_init_feature_name_id_map() {
  ...
  for (feature = BEGINNING_OF_INTEROP_LIST; feature < END_OF_INTEROP_LIST;
    ...
    feature_name_id_map.insert(
        std::pair<std::string, int>(feature_name, feature));
  }
}
...
static int get_feature(const std::string& feature_name) {
  int feature_idx;

  for (feature_idx = BEGINNING_OF_INTEROP_LIST;
       feature_idx < END_OF_INTEROP_LIST; feature_idx++) {
    if (!feature_name.compare(std::string(
            interop_feature_string_((interop_feature_t)feature_idx))))
      return feature_idx;
  }
  return -1;
}
...
static void interop_database_add_(interop_db_entry_t* db_entry, bool persist) {
  ...
  switch (db_entry->bl_type) {
    case INTEROP_BL_TYPE_ADDR: {
      interop_feature_t feature = db_entry->entry_type.addr_entry.feature;
      interop_addr_entry_t addr_entry = db_entry->entry_type.addr_entry;
      char bdstr[18] = {'\0'};
      snprintf(bdstr, sizeof(bdstr), "%02x:%02x:%02x:%02x:%02x:%02x",
               addr_entry.addr.address[0], addr_entry.addr.address[1],
               addr_entry.addr.address[2], addr_entry.addr.address[3],
               addr_entry.addr.address[4], addr_entry.addr.address[5]);
      bdstr[addr_entry.length * 3 - 1] = {'\0'};
      interop_config_set_str(interop_feature_string_(feature), bdstr,
                             ADDR_BASED);
      interop_config_flush();
      break;
    }
    case INTEROP_BL_TYPE_NAME: {
      interop_feature_t feature = db_entry->entry_type.name_entry.feature;
    ...
    case INTEROP_BL_TYPE_MANUFACTURE: {
      interop_feature_t feature = db_entry->entry_type.mnfr_entry.feature;
      char m_facturer[KEY_MAX_LENGTH] = {'\0'};
      snprintf(m_facturer, sizeof(m_facturer), "0x%04x",
               db_entry->entry_type.mnfr_entry.manufacturer);
      interop_config_set_str(interop_feature_string_(feature), m_facturer,
                             MNFR_BASED);
      interop_config_flush();
      break;
    }
    case INTEROP_BL_TYPE_VNDR_PRDT: {
      interop_feature_t feature = db_entry->entry_type.vnr_pdt_entry.feature;
      ...
      interop_config_set_str(interop_feature_string_(feature), m_vendor,
                             VNDR_PRDT_BASED);
      interop_config_flush();
      break;
    }
    case INTEROP_BL_TYPE_SSR_MAX_LAT: {
      ...
      char m_vendor[KEY_MAX_LENGTH] = {'\0'};
      char bdstr[18] = {'\0'};
      snprintf(bdstr, sizeof(bdstr), "%02x:%02x:%02x",
                ssr_entry.addr.address[0], ssr_entry.addr.address[1],
                ssr_entry.addr.address[2]);
      bdstr[3 * 3 - 1] = {'\0'};

      snprintf(m_vendor, sizeof(m_vendor), "%s-0x%04x", bdstr,
    ...
    case INTEROP_BL_TYPE_VERSION: {
      interop_feature_t feature = db_entry->entry_type.version_entry.feature;
      char m_vendor[KEY_MAX_LENGTH] = {'\0'};
    ...
    case INTEROP_BL_TYPE_LMP_VERSION: {
      ...
      char bdstr[18] = {'\0'};
      snprintf(bdstr, sizeof(bdstr), "%02x:%02x:%02x",
               lmp_version_entry.addr.address[0], lmp_version_entry.addr.address[1],
               lmp_version_entry.addr.address[2]);
      bdstr[3 * 3 - 1] = {'\0'};

      snprintf(m_version, sizeof(m_version), "%s-0x%02x-0x%04x", bdstr,
               db_entry->entry_type.lmp_version_entry.lmp_ver,
               db_entry->entry_type.lmp_version_entry.lmp_sub_ver);
      interop_config_set_str(interop_feature_string_(feature), m_version,
                             LMP_VERSION_BASED);
      interop_config_flush();
      break;
    }
  ...
  while (node != list_end(interop_list)) {
    ...
    switch (db_entry->bl_type) {
      ...
      case INTEROP_BL_TYPE_ADDR_RANGE: {
        ...
        //src->addr_start has the actual address, which need to be searched in the range
        if ((src->feature == cur->feature) &&
...
static bool interop_database_remove_(interop_db_entry_t* entry) {
  ...
  if (!interop_database_match_(
    ...
    LOG_ERROR("%s Entry not found in the list", __func__);
    return false;
  }
  ...
  switch (entry->bl_type) {
    case INTEROP_BL_TYPE_ADDR: {
      ...
      char bdstr[18] = {'\0'};
      snprintf(bdstr, sizeof(bdstr), "%02x:%02x:%02x:%02x:%02x:%02x",
               src->addr.address[0], src->addr.address[1], src->addr.address[2],
               src->addr.address[3], src->addr.address[4],
               src->addr.address[5]);
    ...
    case INTEROP_BL_TYPE_NAME: {
      interop_name_entry_t* src = &entry->entry_type.name_entry;
      interop_feature_t feature = src->feature;
      interop_config_remove(interop_feature_string_(feature), src->name);
      interop_config_flush();
      break;
    }
    case INTEROP_BL_TYPE_MANUFACTURE: {
      ...
      interop_feature_t feature = src->feature;
      char m_facturer[KEY_MAX_LENGTH] = {'\0'};
      snprintf(m_facturer, sizeof(m_facturer), "0x%04x", src->manufacturer);
      interop_config_remove(interop_feature_string_(feature), m_facturer);
      interop_config_flush();
      break;
    }
    case INTEROP_BL_TYPE_VNDR_PRDT: {
      ...
      interop_feature_t feature = src->feature;
      char m_vendor[KEY_MAX_LENGTH] = {'\0'};
      snprintf(m_vendor, sizeof(m_vendor), "0x%04x-0x%04x", src->vendor_id,
               src->product_id);
      interop_config_remove(interop_feature_string_(feature), m_vendor);
      interop_config_flush();
      break;
    }
    case INTEROP_BL_TYPE_SSR_MAX_LAT: {
      interop_hid_ssr_max_lat_t ssr_entry = entry->entry_type.ssr_max_lat_entry;
      interop_feature_t feature = ssr_entry.feature;
      ...
      snprintf(m_vendor, sizeof(m_vendor), "%s-0x%04x", bdstr, ssr_entry.max_lat);
      interop_config_remove(interop_feature_string_(feature), m_vendor);
      interop_config_flush();
      break;
    }
    case INTEROP_BL_TYPE_VERSION: {
      ...
      interop_feature_t feature = src->feature;
      char m_version[KEY_MAX_LENGTH] = {'\0'};
      snprintf(m_version, sizeof(m_version), "0x%04x", src->version);
      interop_config_remove(interop_feature_string_(feature), m_version);
      interop_config_flush();
      break;
    }
    case INTEROP_BL_TYPE_LMP_VERSION: {
      interop_lmp_version_t lmp_version_entry = entry->entry_type.lmp_version_entry;
      interop_feature_t feature = lmp_version_entry.feature;
      char m_version[KEY_MAX_LENGTH] = {'\0'};
      char bdstr[18] = {'\0'};
      ...
      snprintf(m_version, sizeof(m_version), "%s-0x%02x-0x%04x", bdstr,
               lmp_version_entry.lmp_ver, lmp_version_entry.lmp_sub_ver);
      interop_config_remove(interop_feature_string_(feature), m_version);
      interop_config_flush();
      break;
    }
    default:
      LOG_ERROR("bl_ype: %d not handled", entry->bl_type);
      status = false;
      break;
  }
...
static bool get_vendor_product_id(char* vendorstr, uint16_t* vendor,
  ...
  if ((token = strtok_r(vendorstr, VENDOR_VALUE_SEPARATOR, &saveptr)) != NULL) {
    ...
    *vendor = (uint16_t)strtoul(token, &e, 16);
    if (*e || errno == EINVAL || errno == ERANGE) return false;
  }
...
static bool get_addr_range(char* str, RawAddress* addr_start, RawAddress* addr_end) {
  ...
  if ((token = strtok_r(str, VENDOR_VALUE_SEPARATOR, &saveptr)) != NULL) {
    ...
    RawAddress::FromString(addr_start_str, *addr_start);
    if (errno == EINVAL || errno == ERANGE) return false;
  }
  ...
  if ((token = strtok_r(NULL, VENDOR_VALUE_SEPARATOR, &saveptr)) != NULL) {
    ...
    RawAddress::FromString(addr_end_str, *addr_end);
    if (errno != EINVAL || errno != ERANGE) ret_value = true;
  }
  return ret_value;
}
...
static bool get_addr_lmp_ver(char* str, char* bdaddrstr, uint8_t* lmp_ver,
  ...
  if ((token = strtok_r(NULL, VENDOR_VALUE_SEPARATOR, &saveptr)) != NULL) {
    ...
    *lmp_sub_ver = (uint16_t)strtoul(token, &e, 16);
    if ((e != NULL) || errno != EINVAL || errno != ERANGE) return true;
  }
  return false;
}
...
bool load_to_database(int feature, const char* key, const char* value,
  ...
  if (!strncasecmp(value, ADDR_BASED, strlen(ADDR_BASED))) {
    ...
    for (int i = 6; i > len; i--) strlcat(bdstr, append_str, sizeof(bdstr));

    RawAddress::FromString(bdstr, addr);

    if (!RawAddress::IsValidAddress(bdstr)) {
      LOG_WARN("key %s or Bluetooth Address %s is invalid, not added to interop list",
          key, bdstr);
      return false;
    }
    ...
    if (strlen(key) != VALID_MNFR_STR_LEN) {
      LOG_WARN("ignoring %s due to invalid Manufacturer id in config file", key);
      return false;
    }
    ...
    if (strlen(key) != VALID_VNDR_PRDT_LEN) {
      LOG_WARN("ignoring %s due to invalid vendor/product id in config file", key);
      return false;
    }
    ...
    if (strlen(key) != VALID_SSR_LAT_LEN) {
      LOG_WARN("ignoring %s due to invalid key for ssr max lat in config file", key);
      return false;
    }
    ...
    if (strlen(key) != VALID_LMP_VERSION_LEN) {
      LOG_WARN("ignoring %s due to invalid key for lmp ver in config file", key);
      return false;
    }
    ...
    len = (strlen(bdaddr_str) + 1) / 3;
    if (len < 3 && len > 6) {
      LOG_WARN("Ignoring as invalid entry for Address %s",
               bdaddr_str);
      return false;
    }
    ...
    if (!RawAddress::IsValidAddress(addr_start.ToString())
        || !RawAddress::IsValidAddress(addr_end.ToString())) {
      LOG_WARN("key %s addr_start %s or addr end  %s is invalid, not added to interop list",
          key, addr_start.ToString().c_str(), addr_end.ToString().c_str());
      return false;
    }

    LOG_DEBUG("key: %s addr_start %s or addr end  %s is added to interop list",
        key, addr_start.ToString().c_str(), addr_end.ToString().c_str());
...
static void interop_config_write(UNUSED_ATTR UINT16 event,
  ...
  int fd = open(INTEROP_DYNAMIC_FILE_PATH, O_WRONLY | O_APPEND);
  if (fd != -1) {
    fsync(fd);
    close(fd);
  }
...
bool interop_database_match_manufacturer(const interop_feature_t feature,
  ...
  if (interop_database_match_(
          ...
          (interop_entry_type)(INTEROP_ENTRY_TYPE_STATIC |
                               INTEROP_ENTRY_TYPE_DYNAMIC))) {
    LOG_WARN("Device with manufacturer id: %d is a match for interop workaround %s",
...
bool interop_database_match_name(const interop_feature_t feature,
  ...
  if (interop_database_match_(
          ...
          (interop_entry_type)(INTEROP_ENTRY_TYPE_STATIC |
                               INTEROP_ENTRY_TYPE_DYNAMIC))) {
    LOG_WARN("Device with name: %s is a match for interop workaround %s",
...
bool interop_database_match_addr(const interop_feature_t feature,
  ...
  memcpy(&entry.entry_type.addr_range_entry.addr_start, addr, sizeof(RawAddress));
  entry.entry_type.addr_range_entry.feature = (interop_feature_t)feature; 
...
bool interop_database_match_vndr_prdt(const interop_feature_t feature,
  ...
  if (interop_database_match_(
          ...
          (interop_entry_type)(INTEROP_ENTRY_TYPE_STATIC |
                               INTEROP_ENTRY_TYPE_DYNAMIC))) {
    LOG_WARN("Device with vendor_id: %d product_id: %d is a match for interop workaround %s",
...
bool interop_database_remove_name(const interop_feature_t feature,
  ...
  if (interop_database_remove_(&entry)) {
    LOG_WARN("Device with name: %s is removed from interop workaround %s",
             name, interop_feature_string_(feature));
    return true;
  }
...
bool interop_database_remove_manufacturer(const interop_feature_t feature,
  ...
  entry.entry_type.mnfr_entry.manufacturer = manufacturer;
  if (interop_database_remove_(&entry)) {
    LOG_WARN("Device with manufacturer id: %d is removed from interop workaround %s",
        manufacturer, interop_feature_string_(feature));
    return true;
  }
...
bool interop_database_remove_vndr_prdt(const interop_feature_t feature,
  ...
  if (interop_database_remove_(&entry)) {
    LOG_WARN("Device with vendor_id: %d product_id: %d is removed from interop workaround %s",
        vendor_id, product_id, interop_feature_string_(feature));
    return true;
  }
  return false;
...
bool interop_database_remove_version(const interop_feature_t feature,
  ...
  if (interop_database_remove_(&entry)) {
    LOG_WARN("Device with version: 0x%04x is removed from interop workaround %s",
        version, interop_feature_string_(feature));
    return true;
  }
  return false;
