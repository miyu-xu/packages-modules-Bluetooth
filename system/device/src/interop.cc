...
static const char *INTEROP_STATIC_FILE_PATH = "/apex/com.android.btservices/etc/bluetooth/interop_database.conf";
...
static int interop_config_init(void)
{
  ...
  if (!stat(INTEROP_STATIC_FILE_PATH, &sts) && sts.st_size) {
    if(!(config_static = config_new(INTEROP_STATIC_FILE_PATH))) {
      LOG_WARN("%s unable to load static config file for : %s",
  ...
  if (!stat(INTEROP_DYNAMIC_FILE_PATH, &sts) && sts.st_size) {
    if(!(config_dynamic = config_new(INTEROP_DYNAMIC_FILE_PATH))) {
      LOG_WARN("%s unable to load dynamic config file for : %s",
...
static int get_feature(const std::string& feature_name)
{
  ...
  for ( feature_idx = BEGINING_OF_INTEROP_LIST;
        feature_idx < END_OF_INTEROP_LIST; feature_idx++ ) {
//    if (!strcmp( section, interop_feature_string_((interop_feature_t)feature)))
    if(!feature_name.compare(std::string(interop_feature_string_((interop_feature_t)feature_idx))))
      return feature_idx;
  }
  return -1;
}
...
bool load_to_database(int feature, const char *key, const char *value, interop_entry_type entry_type)
{
  if ( !strncasecmp( value, ADDR_BASED, strlen(ADDR_BASED)) ) {
    ...
    len = (strlen(key) + 1) / 3;
    if ( len < 3 && len > 6 ) {
      LOG_WARN("%s Ignoring as invalid entry for Address %s",
      __func__, key);
      return false;
    }
    ...
    if (!RawAddress::IsValidAddress(bdstr)) {
      LOG_WARN("%s key %s or Bluetooth Address %s is invalid, not added to interop list",
      __func__, key, bdstr);
      return false;
    }
  ...
  LOG_WARN(" feature:: %d, key :: %s, value :: %s",
                    feature, key, value);
  return true;
}
...
static void load_config()
{
  if ( interop_config_init() != -1) {
    ...
    for (const section_t& sec : config_dynamic.get()->sections) {
      ...
      if ( (feature = get_feature(sec.name)) != -1 ) {
        for (const entry_t& entry : sec.entries) {
          load_to_database(feature, entry.key.c_str(), entry.value.c_str(), INTEROP_ENTRY_TYPE_DYNAMIC);
        }
      }
    }
                      ...
                      uint16_t manufacturer)
{
  ...
  if (interop_database_match_(&entry, &ret_entry, (interop_entry_type)(INTEROP_ENTRY_TYPE_STATIC | INTEROP_ENTRY_TYPE_DYNAMIC))) {
    LOG_WARN("%s() Device with manufacturer id: %d is a match for interop "
...
bool interop_database_match_name( const interop_feature_t feature, const char *name)
{
  ...
  if (interop_database_match_(&entry, &ret_entry, (interop_entry_type)(INTEROP_ENTRY_TYPE_STATIC  | INTEROP_ENTRY_TYPE_DYNAMIC))) {
    LOG_WARN("%s() Device with name: %s is a match for interop workaround %s", __func__,
                   ...
                   uint16_t vendor_id, uint16_t product_id)
{
  ...
  entry.entry_type.vnr_pdt_entry.product_id = product_id;
  if (interop_database_match_(&entry, &ret_entry,  (interop_entry_type)(INTEROP_ENTRY_TYPE_STATIC  | INTEROP_ENTRY_TYPE_DYNAMIC))) {
    LOG_WARN("%s() Device with vendor_id: %d product_id: %d is a match for "
...
bool interop_database_match_version(const interop_feature_t feature, uint16_t version)
{
  ...
  if (interop_database_match_(&entry, &ret_entry,  (interop_entry_type)(INTEROP_ENTRY_TYPE_STATIC  |
    ...
    LOG_WARN("%s() Device with version: 0x%04x is a match for interop workaround %s", __func__, version,
      interop_feature_string_(feature));
    return true;
  }
                          ...
                          uint16_t manufacturer)
{
  ...
  entry.entry_type.mnfr_entry.manufacturer = manufacturer;
  if (interop_database_remove_(&entry)) {
    LOG_WARN("%s() Device with manufacturer id: %d is removed from interop "
...
bool interop_database_remove_feature(const interop_feature_t feature)
{
  ...
  for (const section_t& sec : config_dynamic.get()->sections) {
    if (feature == get_feature(sec.name)) {
      LOG_DEBUG("%s(): found feature - %s",__func__, interop_feature_string_(feature));
          ...
          uint16_t vendor_id, uint16_t product_id)
{
  ...
  if (interop_database_remove_(&entry)) {
    LOG_WARN("%s() Device with vendor_id: %d product_id: %d is removed from"
      "interop workaround %s", __func__, vendor_id, product_id,
      interop_feature_string_(feature));
    return true;
  }
...
bool interop_database_remove_version(const interop_feature_t feature, uint16_t version)
{
  ...
  if (interop_database_remove_(&entry)) {
    LOG_WARN("%s() Device with version: 0x%04x is removed from"
...
bool interop_database_get_whitelisted_media_players_list( const interop_feature_t feature, list_t** p_bl_devices)
{
  ...
  for (const section_t& sec : config_static.get()->sections) {
    if (feature == get_feature(sec.name)) {
      for (const entry_t& entry : sec.entries) {
        LOG_WARN("%s(): found feature - %s",__func__, interop_feature_string_(feature));
