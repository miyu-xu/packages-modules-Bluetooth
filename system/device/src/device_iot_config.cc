...
future_t* device_iot_config_module_init(void) {
  ...
  LOG_INFO("%s: devices loaded %d", __func__, device_iot_config_devices_loaded);
...
int device_iot_config_get_device_num(config_t* conf) {
  CHECK_LOGGING_ENABLED(0);
...
void device_iot_config_write(uint16_t event, UNUSED_ATTR char* p_param) {
  ...
  device_iot_config_restrict_device_num(config.get());
  device_iot_config_sections_sort_by_entry_key(config.get(),
...
bool device_iot_config_is_factory_reset(void) {
  ...
  osi_property_get(PROPERTY_FACTORY_RESET, factory_reset, "false");
