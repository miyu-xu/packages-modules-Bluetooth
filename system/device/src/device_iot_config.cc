...
static std::unique_ptr<config_t> device_iot_config_open(const char* filename);
...
static future_t* init(void) {
  ...
  osi_property_get("persist.bluetooth.device_iot_config.enablelogging", enabled,
                   "false");
  ...
  if (is_factory_reset()) delete_iot_config_files();
  ...
  if (!config) {
    LOG_ERROR("%s unable to load bak file; creating empty config.", __func__);
  ...
  if (device_iot_config_source == NEW_FILE) {
    ...
    version = config_get_int(*config.get(), INFO_SECTION, VERSION_KEY, -1);
    if (version == -1) {
      version = DEVICE_IOT_INFO_FIRST_VERSION;
      config_set_int(config.get(), INFO_SECTION, VERSION_KEY,
                     DEVICE_IOT_INFO_FIRST_VERSION);
    }
  }

  if (version != DEVICE_IOT_INFO_CURRENT_VERSION) {
    LOG_INFO("%s: version in file is %d, CURRENT_VERSION is %d ", __func__,
             version, DEVICE_IOT_INFO_CURRENT_VERSION);
  ...
  device_iot_config_devices_loaded =
      device_iot_config_get_device_num(config.get());

  // Read or set config file creation timestamp
  const std::string* time_str = config_get_string(*config.get(), INFO_SECTION,
                                                  FILE_CREATED_TIMESTAMP, NULL);
  if (time_str != NULL) {
    strlcpy(device_iot_config_time_created, time_str->c_str(),
            TIME_STRING_LENGTH);
  } else {
...
bool device_iot_config_has_section(const char* section) {
  ...
  std::unique_lock<std::mutex> lock(config_lock);
  return config_has_section(*config.get(), section);
}
...
bool device_iot_config_exist(const char* section, const char* key) {
  ...
  std::unique_lock<std::mutex> lock(config_lock);
  return config_has_key(*config.get(), section, key);
}

static bool device_iot_config_has_key_value(const char* section,
                                            const char* key,
  ...
  const std::string* stored_value =
      config_get_string(*config.get(), section, key, NULL);

  if (!stored_value || strcmp(value_str, stored_value->c_str()) != 0)
    return false;
...
bool device_iot_config_get_int(const char* section, const char* key,
  ...
  std::unique_lock<std::mutex> lock(config_lock);
  bool ret = config_has_key(*config.get(), section, key);
  if (ret) *value = config_get_int(*config.get(), section, key, *value);
...
bool device_iot_config_set_int(const char* section, const char* key,
                               int value) {
  CHECK_LOGGING_ENABLED(false);
  ...
  LOG_VERBOSE("%s: sec=%s, key=%s, val=%d", __func__, section, key, value);
  std::unique_lock<std::mutex> lock(config_lock);
  char value_str[32] = {0};
...
bool device_iot_config_int_add_one(const char* section, const char* key) {
  ...
  LOG_VERBOSE("%s: sec=%s, key=%s", __func__, section, key);
  int result = 0;
  std::unique_lock<std::mutex> lock(config_lock);
  result = config_get_int(*config.get(), section, key, result);
  if (result >= 0) {
    result += 1;
  } else {
    result = 0;
...
bool device_iot_config_get_hex(const char* section, const char* key,
  ...
  sscanf_ret = sscanf(stored_value->c_str(), "%x", &result);
  if (sscanf_ret != 1) return false;
...
bool device_iot_config_set_hex(const char* section, const char* key, int value,
  ...
  LOG_VERBOSE("%s: sec=%s, key=%s, val=0x%x", __func__, section, key, value);
  char value_str[32] = {0};
  if (byte_num == 1)
...
bool device_iot_config_set_str(const char* section, const char* key,
  ...
  LOG_VERBOSE("%s: sec=%s, key=%s, val=%s", __func__, section, key, value);
  std::unique_lock<std::mutex> lock(config_lock);
...
bool device_iot_config_set_bin(const char* section, const char* key,
  ...
  LOG_VERBOSE("%s: key = %s", __func__, key);
  if (length > 0) CHECK(value != NULL);

  char* str = (char*)osi_calloc(length * 2 + 1);
  if (str == NULL) {
    LOG_ERROR("%s unable to allocate a str.", __func__);
    return false;
  }
  ...
  std::unique_lock<std::mutex> lock(config_lock);
  if (device_iot_config_has_key_value(section, key, str)) return true;
...
static void device_iot_config_save(void) {
  ...
  alarm_set(config_timer, CONFIG_SETTLE_PERIOD_MS, timer_iot_config_save_cb,
            NULL);
}
...
bool device_iot_config_clear(void) {
  ...
  bool ret = config_save(*config.get(), IOT_CONFIG_FILE_PATH);
...
static void restrict_device_num() {
  ...
  int curr_num = device_iot_config_get_device_num(config.get());
  ...
  LOG_INFO("%s: curr_num=%d, need_remove_num=%d", __func__, curr_num,
           need_remove_devices_num);
...
static bool compare_key(const entry_t& first, const entry_t& second) {
  ...
  if (!first_is_profile_key && !second_is_profile_key) {
    ...
    return strcasecmp(first.key.c_str(), second.key.c_str());
  } else {
...
static void device_iot_config_write(uint16_t event, UNUSED_ATTR char* p_param) {
  ...
  LOG_INFO("%s: evt=%d", __func__, event);
  std::unique_lock<std::mutex> lock(config_lock);
  if (event == IOT_CONFIG_SAVE_TIMER_FIRED_EVT) set_modified_time();

  rename(IOT_CONFIG_FILE_PATH, IOT_CONFIG_BACKUP_PATH);
  restrict_device_num();
  config_sections_sort_by_entry_key(config.get(), compare_key);
  config_save(*config.get(), IOT_CONFIG_FILE_PATH);
}
...
static int device_iot_config_get_device_num(config_t* conf) {
  ...
  for (auto it = conf->sections.begin(); it != conf->sections.end(); ++it) {
    if (RawAddress::IsValidAddress(it->name)) {
      devices++;
    }
  }
  return devices;
