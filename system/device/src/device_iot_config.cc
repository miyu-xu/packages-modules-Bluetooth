...
static future_t* init(void) {
  ...
  osi_property_get("persist.bluetooth.device_iot_config.enablelogging", enabled, "false");
  ...
  if (device_iot_config_source == NEW_FILE) {
    ...
    if (version == -1) {
      ...
      config_set_int(config.get(), INFO_SECTION, VERSION_KEY, DEVICE_IOT_INFO_FIRST_VERSION);
    }
  }
  ...
  device_iot_config_devices_loaded = device_iot_config_get_device_num(config.get());
  ...
  if (time_str != NULL) {
    strlcpy(device_iot_config_time_created, time_str->c_str(), TIME_STRING_LENGTH);
  } else {
...
static bool device_iot_config_has_key_value(const char* section, const char* key, const char* value_str) {
  ...
  const std::string* stored_value = config_get_string(*config.get(), section, key, NULL);
...
bool device_iot_config_get_int(const char* section, const char* key, int* value) {
  ...
  if (ret)
    *value = config_get_int(*config.get(), section, key, *value);
...
bool device_iot_config_get_bin(const char* section, const char* key,
  ...
  std::unique_lock<std::mutex> lock(config_lock);
  const std::string* value_string = config_get_string(*config.get(), section, key, NULL);
...
size_t device_iot_config_get_bin_length(const char* section, const char* key) {
  ...
  std::unique_lock<std::mutex> lock(config_lock);
  const std::string* value_str = config_get_string(*config.get(), section, key, NULL);
