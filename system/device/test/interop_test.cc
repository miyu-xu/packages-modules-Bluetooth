...
TEST(InteropTest, test_lookup_miss) {
  ...
  EXPECT_FALSE(interop_match_addr(INTEROP_BROWSE_PLAYER_ALLOW_LIST, &test_address));
  ...
  EXPECT_FALSE(interop_match_addr(INTEROP_PHONE_POLICY_INCREASED_DELAY_CONNECT_OTHER_PROFILES, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_PHONE_POLICY_REDUCED_DELAY_CONNECT_OTHER_PROFILES, &test_address));
  ...
  EXPECT_FALSE(interop_match_addr(INTEROP_SETUP_SCO_WITH_NO_DELAY_AFTER_SLC_DURING_CALL, &test_address));
...
TEST(InteropTest, test_dynamic_db_clear) {
  module_init(&interop_module);
  ...
  //interop_database_remove_addr(INTEROP_AUTO_RETRY_PAIRING, &test_address);
  EXPECT_FALSE(interop_match_addr(INTEROP_AUTO_RETRY_PAIRING, &test_address));
...
TEST(InteropTest, test_name_hit) {
  ...
  EXPECT_TRUE(interop_match_name(INTEROP_SETUP_SCO_WITH_NO_DELAY_AFTER_SLC_DURING_CALL,
      "Geely_BT"));
...
TEST(InteropTest, test_name_miss) {
  ...
  EXPECT_FALSE(
      interop_match_name(INTEROP_SETUP_SCO_WITH_NO_DELAY_AFTER_SLC_DURING_CALL, "TEST"));
...
TEST(InteropTest, test_vndr_prdt_hit) {
  module_init(&interop_module);
  ...
  EXPECT_TRUE(
      interop_database_match_vndr_prdt(INTEROP_REMOVE_HID_DIG_DESCRIPTOR, vendor_id, product_id));

  vendor_id = 0x05ac;
  product_id = 0x0255;

  EXPECT_TRUE(
      interop_database_match_vndr_prdt(INTEROP_CHANGE_HID_VID_PID, vendor_id, product_id));

  module_clean_up(&interop_module);
}

TEST(InteropTest, test_vndr_prdt_miss) {
  module_init(&interop_module);
...
TEST(InteropTest, test_manufacturer_hit) {
  module_init(&interop_module);

  uint16_t manufacturer = 0x004C;

  EXPECT_TRUE(
      interop_database_match_manufacturer(INTEROP_DISABLE_SNIFF_DURING_SCO, manufacturer));

  module_clean_up(&interop_module);
}

TEST(InteropTest, test_manufacturer_miss) {
  module_init(&interop_module);
...
TEST(InteropTest, test_ssr_max_latency_hit) {
  module_init(&interop_module);
...
TEST(InteropTest, test_ssr_max_latency_miss) {
  module_init(&interop_module);
...
TEST(InteropTest, test_dynamic_addr) {
  module_init(&interop_module);
...
TEST(InteropTest, test_dynamic_name) {
  module_init(&interop_module);
...
TEST(InteropTest, test_dynamic_vndr_prdt) {
  module_init(&interop_module);
...
TEST(InteropTest, test_dynamic_addr_get_ssr_max_lat) {
  module_init(&interop_module);
...
TEST(InteropTest, test_dynamic_manufacturer) {
  module_init(&interop_module);
...
TEST(InteropTest, test_dynamic_addr_get_lmp_version) {
  module_init(&interop_module);
  ...
  EXPECT_FALSE(
      interop_database_match_addr_get_lmp_ver(INTEROP_DISABLE_SNIFF_DURING_SCO,
                                             &test_address,
                                             &lmp_version,&lmp_sub_version));
...
TEST(InteropTest, test_dynamic_did_version) {
  module_init(&interop_module);
  ...
  EXPECT_FALSE(
      interop_database_match_version(INTEROP_DISABLE_SNIFF_DURING_SCO,
                                     did_version));
  interop_database_add_version(INTEROP_DISABLE_SNIFF_DURING_SCO,
                               did_version);
