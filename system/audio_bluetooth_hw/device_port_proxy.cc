...
=======
  ...
  auto low_latency_allowed_cb = [port = this](uint16_t cookie, bool allowed) {
    if (!port->in_use()) {
      LOG(ERROR)
          << "low_latency_allowed_cb: BluetoothAudioPortAidl is not in use";
      return;
    }
...
bool BluetoothAudioPortAidl::SetLatencyMode(audio_latency_mode_t mode) {
  ...
  BluetoothAudioSessionControl::SetLatencyMode(session_type_,
                                               hidl_latency_mode_to_aidl(mode));

  return true;
}

int BluetoothAudioPortAidl::GetRecommendedLatencyModes(
  ...
  if (num_buffer < a2dp_modes.size()) {
    LOG(WARNING) << __func__ << ": audio buffer size " << num_buffer << " <  "
                 << a2dp_modes.size() << "  A2DP latency modes";
  } else {
  ...
  *num_modes = 0;
  ...
  if (*num_modes != num_buffer) {
    LOG(WARNING) << __func__ << ": correct num_modes " << num_buffer << " to "
                 << *num_modes;
  }
  return 0;
}

int BluetoothAudioPortAidl::SetLatencyModeCallback(
