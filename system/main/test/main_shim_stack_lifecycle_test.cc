...
class TestStackManager {
 public:
  TestStackManager() {
    // Start is done after test adds the modules
  }
  ...
  // if stack manger has not started or shutdown
  template <typename T>
...
class Client {
 public:
  ...
  // Make sure all the client handlers are running
  void Await() {
...
TEST_F(StackLifecycleWithDefaultModulesUnitTest, NOP) {}
