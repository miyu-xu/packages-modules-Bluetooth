...
#define LOG_TAG bt_layer_tags[trace_layer]
  ...
  switch (TRACE_GET_TYPE(trace_set_mask)) {
    case TRACE_TYPE_ERROR:
      LOG_ERROR_INT("%s:%d - %s %s", filename, linenum, func, buffer);
      break;
    case TRACE_TYPE_WARNING:
      LOG_WARN_INT("%s:%d - %s %s", filename, linenum, func, buffer);
      break;
    case TRACE_TYPE_API:
    case TRACE_TYPE_EVENT:
      LOG_INFO_INT("%s:%d - %s %s",filename, linenum, func, buffer);
      break;
    case TRACE_TYPE_DEBUG:
      LOG_DEBUG_INT("%s:%d - %s %s",filename, linenum, func, buffer);
      break;
    default:
      /* we should never get this */
      LOG_ERROR_INT("!BAD TRACE TYPE!%s:%d:%s %s",filename, linenum, func, buffer);
