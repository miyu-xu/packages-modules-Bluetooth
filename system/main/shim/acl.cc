...
struct ConnectionDescriptor {
  ...
  std::string ToString() const {
    return std::format("peer:{} handle:0x{:04x} is_locally_initiated:{} creation_time:{} teardown_time:{} disconnect_reason:{}", GetPrivateRemoteAddress(), handle_, is_locally_initiated_ ? "true" : "false", common::StringFormatTimeWithMilliseconds(kConnectionDescriptorTimeFormat,
                                                     creation_time_)
...
public:
  ...
  void OnRoleChange(hci::ErrorCode hci_status, hci::Role new_role) override {
    ...
    BTM_LogHistory(
            ...
            std::format("classic New_role:{} status:{}", hci::RoleText(new_role), hci::ErrorCodeText(hci_status)));
  }
