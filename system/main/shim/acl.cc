...
void shim::legacy::Acl::OnLeConnectSuccess(
  ...
  hci::Address peer_address = connection->peer_address_;
  ...
  // TODO Differentiate between Resolvable Private Address and Random Address
  if (IsRpa(address_with_type)) {
    LOG_WARN("Connection address is rpa:%s identity_addr:%s",
             address_with_type.ToString().c_str(),
             peer_address.ToString().c_str());
    pimpl_->shadow_acceptlist_.Remove(
        {peer_address, hci::AddressType::PUBLIC_DEVICE_ADDRESS});
  } else {
