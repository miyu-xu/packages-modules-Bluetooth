...
struct timed_wakelock {
  ...
  // Acquire wakelock for a timeout defined in sysprop.
  // Acquiring the same wakelock resets the timer. Wakelock is auto
  // released after the timeout.
  void acquire() {
    uint64_t timeout_ms =
            (uint64_t)osi_property_get_int32("bluetooth.core.acl.wakelock_timeout", 0);
    // Ignore request if timeout is 0.
    if (timeout_ms == 0) {
      return;
    }
    if (!timer_) {
      timer_ = alarm_new("wakelock_release_timeout");
    }
    if (!alarm_is_scheduled(timer_)) {
      log::debug("Wakelock acquired", timeout_ms);
  ...
  void release() {
    ...
    if (alarm_is_scheduled(timer_)) {
      log::debug("Wakelock released remain: {} ms", alarm_get_remaining_ms(timer_));
      alarm_cancel(timer_);
      WakelockManager::Get().Release();
    }
  }
