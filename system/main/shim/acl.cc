...
class ShimAclConnection {
  ...
  ShimAclConnection(const HciHandle handle, SendDataUpwards send_data_upwards, os::Handler* handler,
    ...
    queue_up_end_->RegisterDequeue(
            handler_,
            common::Bind(&ShimAclConnection::data_ready_callback, common::Unretained(this)));
  }
...
void shim::legacy::Acl::OnLeConnectSuccess(
  ...
  pimpl_->handle_to_le_connection_map_.emplace(
          handle,
          std::make_unique<LeShimAclConnection>(
                  acl_interface_.on_send_data_upwards,
                  std::bind(&shim::legacy::Acl::OnLeLinkDisconnected, this, std::placeholders::_1,
                            std::placeholders::_2),
