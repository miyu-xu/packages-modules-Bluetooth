...
using bluetooth::hci::AdvertisingFilterOnFoundOnLostInfo;
using bluetooth::hci::ApcfAction;
using bluetooth::hci::ApcfFilterType;
using bluetooth::hci::Enable;
using bluetooth::hci::ScannerId;
using bluetooth::hci::ScanningCallback;

class DefaultScanningCallback : public ::ScanningCallback {
  void OnScannerRegistered(const bluetooth::hci::Uuid app_uuid,
                           ScannerId scanner_id, ScanningStatus status) {
    LogUnused();
  }
  ...
  void OnTimeout() { LogUnused(); }
  void OnFilterEnable(Enable enable, uint8_t status) { LogUnused(); }
  void OnFilterParamSetup(uint8_t available_spaces, ApcfAction action,
                          uint8_t status) {
    LogUnused();
  }
  void OnFilterConfigCallback(ApcfFilterType filter_type,
                              uint8_t available_spaces, ApcfAction action,
                              uint8_t status) {
    LogUnused();
  }
 ...
 private:
  void LogUnused() const {
    LOG_WARN("Classic scanning callbacks have not been registered");
  }
} default_scanning_callback_;

::ScanningCallbacks* bluetooth::shim::default_scanning_callback =
    (::ScanningCallbacks*)&default_scanning_callback_;
