 ...
 *                  does not trigger scan start/stop, but if scan is ever
 *started, this callback would get called with scan results. Additionally, this
 *callback is not reset on each scan start/stop. It's intended to be used by LE
 *Audio related profiles, that would find yet unpaired members of CSIS set, or
 *broadcasts.
 *
