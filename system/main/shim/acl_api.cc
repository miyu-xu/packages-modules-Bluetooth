...
void bluetooth::shim::ACL_ConfigureLePrivacy(bool is_le_privacy_enabled) {
  hci::LeAddressManager::AddressPolicy address_policy = is_le_privacy_enabled
              ...
              do_in_main_thread(FROM_HERE,
                                base::BindOnce(
                                        [](hci::ErrorCode status) {
                                          // NOTE: we intentionally don't supply the address, to
                                          // match the legacy behavior. Callsites that want the
                                          // address should use StartRemoteNameRequest() directly,
                                          // rather than going through this shim.
