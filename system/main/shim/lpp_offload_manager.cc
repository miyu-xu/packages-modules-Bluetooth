...
public:
  ...
  bool SetAclCredits(int linkType, int credit) {
    return bluetooth::shim::GetLppOffloadManager()->SetAclCredits(linkType, credit);
  }

  bool NotifyAclConnectionStateChange(int aclHandle, int linkType, int state) {
    return bluetooth::shim::GetLppOffloadManager()->NotifyAclConnectionStateChange(aclHandle,
                                                                                   linkType, state);
  }
  ...
  bool NotifyAclPowerModeChange(int aclHandle, int powerMode, int interval) {
    return bluetooth::shim::GetLppOffloadManager()->NotifyAclPowerModeChange(aclHandle, powerMode,
                                                                             interval);
  }
