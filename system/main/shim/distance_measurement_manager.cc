...
class DistanceMeasurementInterfaceImpl
 ...
 public:
  ...
  void OnChannelSoundingSubevent(bluetooth::hci::Address address,
                                 uint16_t procedure_counter,
                                 uint8_t procedure_done_status,
                                 std::vector<uint8_t> raw_data) {
    bluetooth::ras::GetRasServer()->PushProcedureData(
        bluetooth::ToRawAddress(address), procedure_counter,
        static_cast<bluetooth::ras::ProcedureDoneStatus>(procedure_done_status),
        raw_data);
  }
