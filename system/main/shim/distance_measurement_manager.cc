...
public:
  ...
  /**
   * Gets the BLE connection handle, calling this from main_thread.
  ...
  void StartDistanceMeasurement(RawAddress identity_addr, uint16_t interval, uint8_t method) {
    do_in_main_thread(base::BindOnce(
            &DistanceMeasurementInterfaceImpl::StartDistanceMeasurement_internal,
            base::Unretained(
                    (DistanceMeasurementInterfaceImpl*)shim::get_distance_measurement_instance()),
            identity_addr, interval, method));
  }

  void StartDistanceMeasurement_internal(RawAddress identity_addr, uint16_t interval,
                                         uint8_t method) {
  ...
  void StopDistanceMeasurement(RawAddress identity_addr, uint8_t method) {
    do_in_main_thread(base::BindOnce(
            &DistanceMeasurementInterfaceImpl::StopDistanceMeasurement_internal,
            base::Unretained(
                    (DistanceMeasurementInterfaceImpl*)shim::get_distance_measurement_instance()),
            identity_addr, method));
  }

  void StopDistanceMeasurement_internal(RawAddress identity_addr, uint8_t method) {
