...
constexpr unsigned kHeadlessIcon = 0x1F5E3;
...
void remote_device_properties(bt_status_t status, RawAddress* bd_addr,
  ...
  if (interface_api_callback_map_.find(__func__) !=
      interface_api_callback_map_.end()) {
  ...
  LOG_INFO(
      ...
      "properties:%p",
      __func__, num_callbacks, bt_status_text(status).c_str(),
      bd_addr->ToString().c_str(), num_properties, properties);
}
...
void HeadlessStack::SetUp() {
  ...
  LOG_CONSOLE("%lc Headless stack has started up successfully", kHeadlessIcon);
}
...
void HeadlessStack::TearDown() {
  ...
  LOG_CONSOLE("%lc Headless stack has shutdown successfully", kHeadlessIcon);
}
