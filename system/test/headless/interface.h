...
// TODO Move this to device_cb.cc
inline std::deque<bluetooth::Uuid> get_uuids_from_property(
    bt_property_t* property) {
  CHECK(property != nullptr);

  ASSERT_LOG(property->type == BT_PROPERTY_UUIDS,
             "Recieved unexpect non-UUID property");

  std::deque<bluetooth::Uuid> uuids;
  // TODO need to copy the data here too
  bluetooth::Uuid* p_uuid = reinterpret_cast<bluetooth::Uuid*>(property->val);
  const size_t num_uuids = property->len / sizeof(bt_property_t);
  LOG_CONSOLE("CMM CMM %zu ", num_uuids);
  for (size_t i = 0; i < num_uuids; i++, p_uuid++) {
    bluetooth::Uuid uuid =
