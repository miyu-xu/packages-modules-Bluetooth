...
struct ACL_AcceptLeConnectionFrom {
  ...
  std::function<bool(const tBLE_BD_ADDR& legacy_address_with_type, bool is_direct)> body{
    [](const tBLE_BD_ADDR& legacy_address_with_type, bool is_direct) { return return_value; }
  };
...
struct ACL_GetAdvertisingSetConnectedTo {
  ...
  std::function<std::optional<uint8_t>(const RawAddress& addr)> body{ [](const RawAddress& addr) {
    return return_value;
  } };
