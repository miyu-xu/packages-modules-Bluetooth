...
struct L2CA_DataWrite {
  std::function<uint8_t(uint16_t cid, BT_HDR* p_data)> body{ [](uint16_t cid, BT_HDR* p_data) {
    return 0;
  } };
...
struct L2CA_SetIdleTimeoutByBdAddr {
  std::function<bool(const RawAddress& bd_addr, uint16_t timeout, tBT_TRANSPORT transport)> body{
    [](const RawAddress& bd_addr, uint16_t timeout, tBT_TRANSPORT transport) { return false; }
  };
...
struct L2CA_GetPeerFeatures {
  std::function<bool(const RawAddress& bd_addr, uint32_t* p_ext_feat, uint8_t* p_chnl_mask)> body{
    [](const RawAddress& bd_addr, uint32_t* p_ext_feat, uint8_t* p_chnl_mask) { return false; }
  };
...
struct L2CA_GetLeHandle {
  std::function<uint16_t(const RawAddress& rem_bda)> body{ [](const RawAddress& rem_bda) {
    return 0;
  } };
...
struct L2CA_SetChnlFlushability {
  std::function<bool(uint16_t cid, bool is_flushable)> body{ [](uint16_t cid, bool is_flushable) {
    return false;
  } };
...
struct L2CA_GetBleConnRole {
  std::function<hci_role_t(const RawAddress& bd_addr)> body{ [](const RawAddress& bd_addr) {
