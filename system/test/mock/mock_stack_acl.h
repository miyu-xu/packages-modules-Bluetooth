...
struct BTM_IsRemoteVersionReceived {
  std::function<bool(const RawAddress& addr)> body{ [](const RawAddress& /* addr */) {
    return false;
  } };
...
struct BTM_is_sniff_allowed_for {
  std::function<bool(const RawAddress& peer_addr)> body{ [](const RawAddress& /* peer_addr */) {
    return false;
  } };
...
struct acl_create_le_connection {
  std::function<bool(const RawAddress& bd_addr)> body{ [](const RawAddress& /* bd_addr */) {
    return false;
  } };
...
struct acl_peer_supports_ble_connection_parameters_request {
  std::function<bool(const RawAddress& remote_bda)> body{ [](const RawAddress& /* remote_bda */) {
    return false;
  } };
...
struct BTM_GetMaxPacketSize {
  std::function<uint16_t(const RawAddress& addr)> body{ [](const RawAddress& /* addr */) {
    return 0;
  } };
...
struct BTM_ReadRemoteFeatures {
  std::function<uint8_t*(const RawAddress& addr)> body{ [](const RawAddress& /* addr */) {
    return nullptr;
  } };
...
struct acl_packets_completed {
  std::function<void(uint16_t handle, uint16_t credits)> body{ [](uint16_t /* handle */,
                                                                  uint16_t /* credits */) { ; } };
...
struct acl_process_supported_features {
  std::function<void(uint16_t handle, uint64_t features)> body{ [](uint16_t /* handle */,
                                                                   uint64_t /* features */) { ; } };
...
struct acl_set_disconnect_reason {
  std::function<void(tHCI_STATUS acl_disc_reason)> body{ [](tHCI_STATUS /* acl_disc_reason */) {
    ;
  } };
...
struct btm_acl_process_sca_cmpl_pkt {
  std::function<void(uint8_t len, uint8_t* data)> body{ [](uint8_t /* len */, uint8_t* /* data */) {
    ;
  } };
...
struct btm_acl_role_changed {
  std::function<void(tHCI_STATUS hci_status, const RawAddress& bd_addr, tHCI_ROLE new_role)> body{
    [](tHCI_STATUS /* hci_status */, const RawAddress& /* bd_addr */, tHCI_ROLE /* new_role */) {
      ;
    }
  };
...
struct btm_read_remote_ext_features_complete_raw {
  std::function<void(uint8_t* p, uint8_t evt_len)> body{ [](uint8_t* /* p */,
                                                            uint8_t /* evt_len */) { ; } };
...
struct btm_read_remote_ext_features_failed {
  std::function<void(uint8_t status, uint16_t handle)> body{ [](uint8_t /* status */,
                                                                uint16_t /* handle */) { ; } };
...
struct btm_read_rssi_complete {
  std::function<void(uint8_t* p, uint16_t evt_len)> body{ [](uint8_t* /* pm */,
                                                             uint16_t /* evt_len */) { ; } };
...
struct hci_btm_set_link_supervision_timeout {
  std::function<void(tACL_CONN& link, uint16_t timeout)> body{ [](tACL_CONN& /* link */,
                                                                  uint16_t /* timeout */) { ; } };
