...
struct BTA_DmSetVisibility {
  ...
  std::function<bool(bt_scan_mode_t mode)> body{ [this](bt_scan_mode_t /* mode */) {
    return return_value;
  } };
...
struct BTA_dm_acl_up {
  std::function<void(const RawAddress bd_addr, tBT_TRANSPORT transport, uint16_t acl_handle)> body{
    [](const RawAddress /* bd_addr */, tBT_TRANSPORT /* transport */, uint16_t /* acl_handle */) {}
  };
...
struct BTA_dm_report_role_change {
  std::function<void(const RawAddress bd_addr, tHCI_ROLE new_role, tHCI_STATUS hci_status)> body{
    [](const RawAddress /* bd_addr */, tHCI_ROLE /* new_role */, tHCI_STATUS /* hci_status */) {}
  };
...
struct bta_dm_ble_scan {
  std::function<void(bool start, uint8_t duration_sec)> body{ [](bool /* start */,
...
struct bta_dm_check_if_only_hd_connected {
  bool return_value{ false };
  std::function<bool(const RawAddress& peer_addr)> body{ [this](const RawAddress& /* peer_addr */) {
    return return_value;
  } };
...
struct bta_dm_eir_update_uuid {
  std::function<void(uint16_t uuid16, bool adding)> body{ [](uint16_t /* uuid16 */,
                                                             bool /* adding */) {} };
