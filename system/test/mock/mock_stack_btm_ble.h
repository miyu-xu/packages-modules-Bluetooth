...
struct BTM_SetBleDataLength {
  ...
  std::function<tBTM_STATUS(const RawAddress& bd_addr, uint16_t tx_pdu_length)> body{
    [](const RawAddress& /* bd_addr */, uint16_t /* tx_pdu_length */) { return return_value; }
  };
...
struct btm_ble_get_acl_remote_addr {
  ...
  std::function<bool(uint16_t hci_handle, RawAddress& conn_addr, tBLE_ADDR_TYPE* p_addr_type)> body{
    [](uint16_t /* hci_handle */, RawAddress& /* conn_addr */, tBLE_ADDR_TYPE* /* p_addr_type */) {
      return return_value;
    }
  };
...
// Return: void
struct doNothing {
  std::function<void(uint8_t* data, uint16_t len)> body{ [](uint8_t* /* data */,
                                                            uint16_t /* len */) {} };
