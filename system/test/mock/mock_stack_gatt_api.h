...
struct GATTC_UpdateUserAttMtuIfNeeded {
  std::function<GATTC_UpdateUserAttMtuIfNeeded(
      const RawAddress& remote_bda, tBT_TRANSPORT transport, uint16_t user_mtu)>
      body{[](const RawAddress& remote_bda, tBT_TRANSPORT transport,
              uint16_t user_mtu) { return; }};
  GATTC_UpdateUserAttMtuIfNeeded operator()(const RawAddress& remote_bda,
                                            tBT_TRANSPORT transport,
                                            uint16_t user_mtu) {
    return body(remote_bda, transport, user_mtu);
  };
};
