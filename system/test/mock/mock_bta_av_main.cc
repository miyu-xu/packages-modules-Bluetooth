...
void bta_av_restore_switch(void) { increment_mock_function_call_count(__func__); }
void bta_av_sm_execute(tBTA_AV_CB* p_cb, uint16_t event, tBTA_AV_DATA* p_data) {
  increment_mock_function_call_count(__func__);
}
void bta_debug_av_dump(int fd) { increment_mock_function_call_count(__func__); }
void tBTA_AV_SCB::OnConnected(const RawAddress& peer_address) {
  increment_mock_function_call_count(__func__);
}
void tBTA_AV_SCB::OnDisconnected() { increment_mock_function_call_count(__func__); }
