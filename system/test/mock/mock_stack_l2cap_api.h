...
struct l2c_get_transport_from_fixed_cid {
  std::function<tBT_TRANSPORT(uint16_t fixed_cid)> body{ [](uint16_t /* fixed_cid */) {
    return 0;
  } };
...
struct L2CA_SendFixedChnlData {
  std::function<uint16_t(uint16_t fixed_cid, const RawAddress& rem_bda, BT_HDR* p_buf)> body{
    [](uint16_t /* fixed_cid */, const RawAddress& /* rem_bda */, BT_HDR* /* p_buf */) { return 0; }
  };
  uint16_t operator()(uint16_t fixed_cid, const RawAddress& rem_bda, BT_HDR* p_buf) {
...
struct L2CA_MarkLeLinkAsActive {
  std::function<bool(const RawAddress& rem_bda)> body{ [](const RawAddress& /* rem_bda */) {
    return false;
  } };
...
struct L2CA_isMediaChannel {
  std::function<bool(uint16_t handle, uint16_t channel_id, bool is_local_cid)> body{
    [](uint16_t /* handle */, uint16_t /* channel_id */, bool /* is_local_cid */) { return false; }
  };
