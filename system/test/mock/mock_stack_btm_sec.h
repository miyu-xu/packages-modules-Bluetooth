...
struct BTM_IsAuthenticated {
  ...
  std::function<bool(const RawAddress& bd_addr, tBT_TRANSPORT transport)> body{
    [](const RawAddress& /* bd_addr */, tBT_TRANSPORT /* transport */) { return return_value; }
  };
...
struct BTM_SecBondCancel {
  static tBTM_STATUS return_value;
  std::function<tBTM_STATUS(const RawAddress& bd_addr)> body{ [](const RawAddress& /* bd_addr */) {
    return return_value;
  } };
...
struct BTM_SecClrService {
  ...
  std::function<uint8_t(uint8_t service_id)> body{ [](uint8_t /* service_id */) {
    return return_value;
  } };
...
struct btm_get_dev_class {
  ...
  std::function<const DEV_CLASS(const RawAddress& bda)> body{ [](const RawAddress& /* bda */) {
    return return_value;
  } };
...
struct btm_io_capabilities_rsp {
  std::function<void(const tBTM_SP_IO_RSP evt_data)> body{ [](const tBTM_SP_IO_RSP /* evt_data */) {
  } };
...
struct btm_sec_auth_complete {
  std::function<void(uint16_t handle, tHCI_STATUS status)> body{ [](uint16_t /* handle */,
                                                                    tHCI_STATUS /* status */) {} };
...
struct btm_sec_is_a_bonded_dev {
  static bool return_value;
  std::function<bool(const RawAddress& bda)> body{ [](const RawAddress& /* bda */) {
    return return_value;
  } };
