...
// Name: btm_sec_find_dev_by_sec_state
// Params: uint8_t state
// Return: tBTM_SEC_DEV_REC*
struct btm_sec_find_dev_by_sec_state {
  static tBTM_SEC_DEV_REC* return_value;
  std::function<tBTM_SEC_DEV_REC*(uint8_t state)> body{
      [](uint8_t state) { return return_value; }};
  tBTM_SEC_DEV_REC* operator()(uint8_t state) { return body(state); };
};
extern struct btm_sec_find_dev_by_sec_state btm_sec_find_dev_by_sec_state;
