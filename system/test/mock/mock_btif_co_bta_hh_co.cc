...
tBTA_HH_RPT_CACHE_ENTRY* bta_hh_le_co_cache_load(
    const tTypedAddressTransport& /* remote_bda */, uint8_t* /* p_num_rpt */,
    uint8_t /* app_id */) {
  inc_func_call_count(__func__);
  return nullptr;
}
...
void bta_hh_co_data(uint8_t /* dev_handle */, uint8_t* /* p_rpt */,
                    ...
                    const tTypedAddressTransport& /* peer_addr */, uint8_t /* app_id */) {
  inc_func_call_count(__func__);
}
...
void bta_hh_le_co_reset_rpt_cache(const tTypedAddressTransport& /* remote_bda */,
                                  uint8_t /* app_id */) {
  inc_func_call_count(__func__);
}
void bta_hh_le_co_rpt_info(const tTypedAddressTransport& /* remote_bda */,
