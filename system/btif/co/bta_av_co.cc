...
void BtaAvCo::ProcessOpen(tBTA_AV_HNDL bta_av_handle,
  ...
  BtaAvCoState* reference_state = getRelevantState(p_peer);
...
bool BtaAvCo::SetActivePeer(const RawAddress& peer_address,
  ...
  BtaAvCoState* reference_state = nullptr;
  if (IS_FLAG_ENABLED(a2dp_concurrent_source_sink)) {
    if (profile == UUID_SERVCLASS_AUDIO_SOURCE) {
      reference_state = &bta_av_source_state_;
    } else if (profile == UUID_SERVCLASS_AUDIO_SINK) {
      reference_state = &bta_av_sink_state_;
    }
  } else {
    reference_state = &bta_av_legacy_state_;
  }
...
BtaAvCoState* BtaAvCo::getRelevantState(const BtaAvCoPeer* p_peer) {
  BtaAvCoState* reference_state = nullptr;
  if (IS_FLAG_ENABLED(a2dp_concurrent_source_sink)) {
...
bool bta_av_co_set_active_peer(const RawAddress& peer_address) {
  return bta_av_co_cb.SetActivePeer(peer_address, 0);
}

bool bta_av_co_set_active_sink_peer(const RawAddress& peer_address) {
  return bta_av_co_cb.SetActivePeer(peer_address, UUID_SERVCLASS_AUDIO_SINK);
}

bool bta_av_co_set_active_source_peer(const RawAddress& peer_address) {
  return bta_av_co_cb.SetActivePeer(peer_address, UUID_SERVCLASS_AUDIO_SOURCE);
}
