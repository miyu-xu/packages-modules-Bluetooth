...
void BtaAvCo::ProcessDiscoveryResult(tBTA_AV_HNDL bta_av_handle,
  ...
  if (p_peer == nullptr) {
    bluetooth::log::error(
        "could not find peer entry for bta_av_handle 0x{:x} peer {}",
...
tA2DP_STATUS BtaAvCo::ProcessSourceGetConfig(
  ...
  bluetooth::log::verbose(
      "peer(o={}, n_sinks={}, n_rx_sinks={}, n_sup_sinks={})", p_peer->opened,
...
void BtaAvCo::DebugDump(int fd) {
  ...
  } else if (bta_av_source_state_.getActivePeer() != nullptr) {
    A2dpCodecs* a2dp_codecs = bta_av_source_state_.getActivePeer()->GetCodecs();
    if (a2dp_codecs != nullptr) {
      a2dp_codecs->debug_codec_dump(fd);
    }
  } else if (bta_av_sink_state_.getActivePeer() != nullptr) {
...
tA2DP_STATUS bta_av_co_audio_getconfig(tBTA_AV_HNDL bta_av_handle,
  ...
  bluetooth::log::error(
      "peer {} : Invalid peer UUID: 0x{:x} for bta_av_handle 0x{:x}",
