...
BtaAvCoSep* BtaAvCo::SelectProviderCodecConfiguration(BtaAvCoPeer* p_peer,
  const ::bluetooth::audio::a2dp::provider::a2dp_configuration&
        provider_codec_config) {
  ...
  auto* p_sink = FindPeerSink(p_peer, provider_codec_config.codec_index);
  if (p_sink == nullptr) {
  ...
  auto* codec_config =
         reinterpret_cast<A2dpCodecConfigExt*>(
                p_peer->GetCodecs()->findSourceCodecConfig(
                provider_codec_config.codec_index));
