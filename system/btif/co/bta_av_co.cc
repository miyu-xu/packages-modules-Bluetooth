...
void BtaAvCo::UpdateMtu(tBTA_AV_HNDL bta_av_handle,
                        const RawAddress& peer_address, uint16_t mtu) {
  log::info("peer {} bta_av_handle: {:x} mtu: {}", peer_address, bta_av_handle,
            mtu);
  ...
  BtaAvCoPeer* p_peer =
      peer_cache_->FindPeerAndUpdate(bta_av_handle, peer_address);
  if (p_peer == nullptr) {
    log::error("could not find peer entry for bta_av_handle {:x} peer {}",
               bta_av_handle, peer_address);
    return;
  }
  p_peer->mtu = mtu;
...
bool BtaAvCo::SetCodecUserConfig(
  ...
  *p_restart_output = false;
  ...
  if (restart_output) {
    ...
    log::verbose("call BTA_AvReconfig({:x})", p_peer->BtaAvHandle());
