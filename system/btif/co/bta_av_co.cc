...
const BtaAvCoSep* BtaAvCo::SelectSourceCodec(BtaAvCoPeer* p_peer) {
  ...
  if (provider_codec_config.has_value() &&
    ...
    auto hardware_codec_config = provider_codec_config.value();
    ...
    if (p_sink != nullptr) {
      ...
      // XXX make sure codec_config.data() has sufficient size.
      SaveNewCodecConfig(p_peer, hardware_codec_config.codec_config.data(),
...
static bool bta_av_co_should_select_hardware_codec(
  ...
  if (A2DP_GetCodecType(hardware_config.codec_config.data()) ==
