  ...
  if (status == A2DP_SUCCESS) {
    ...
    if (t_local_sep == AVDT_TSEP_SNK) {
      status = A2DP_IsSinkCodecSupported(p_codec_info) ?
          tA2DP_STATUS::A2DP_SUCCESS :
          tA2DP_STATUS::A2DP_WRONG_CODEC;

      if (status == tA2DP_STATUS::A2DP_SUCCESS) {
    ...
    if (t_local_sep == AVDT_TSEP_SRC) {
      ...
      if (status == tA2DP_STATUS::A2DP_SUCCESS && num_protect == 1 && !p_peer->ContentProtectActive()) {
...
tA2DP_STATUS BtaAvCo::SetCodecOtaConfig(BtaAvCoPeer* p_peer, const uint8_t* p_ota_codec_config,
  ...
  auto status = p_peer->GetCodecs()->setCodecOtaConfig(
          p_ota_codec_config, &peer_params, result_codec_config, &restart_input, &restart_output,
          &config_updated);
  if (status != A2DP_SUCCESS) {
