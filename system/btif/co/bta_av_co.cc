...
tA2DP_STATUS BtaAvCo::ProcessSourceGetConfig(
  ...
  if (p_peer->acceptor) {
    ...
    if (btif_av_peer_prefers_mandatory_codec(p_peer->addr)) {
      ...
      if (p_sink != nullptr) {
        LOG_DEBUG("%s: mandatory codec preferred for peer %s", __func__,
                  ADDRESS_TO_LOGGABLE_CSTR(p_peer->addr));
  ...
  *p_sep_info_idx = p_sink->sep_info_idx;
  LOG_DEBUG("%s: peer %s acceptor:%s reconfig_needed:%s", __func__,
            ADDRESS_TO_LOGGABLE_CSTR(p_peer->addr),
...
tA2DP_STATUS BtaAvCo::ProcessSinkGetConfig(tBTA_AV_HNDL bta_av_handle,
  ...
  LOG_VERBOSE("%s: peer %s found (o=%d, n_sources=%d, n_rx_sources=%d, "
              "n_sup_sources=%d)",
...
void BtaAvCo::ProcessSetConfig(tBTA_AV_HNDL bta_av_handle,
  ...
  LOG_VERBOSE("%s: bta_av_handle=0x%x peer_address=%s seid=%d "
  ...
  LOG_VERBOSE("%s: peer %s found (o=%d, n_sinks=%d, n_rx_sinks=%d, "
              "n_sup_sinks=%d)",
              __func__, ADDRESS_TO_LOGGABLE_CSTR(p_peer->addr), p_peer->opened,
...
BtaAvCoSep* BtaAvCo::FindPeerSink(BtaAvCoPeer* p_peer,
  ...
  for (size_t index = 0; index < p_peer->num_sup_sinks; index++) {
    ...
    if (!AudioSepHasContentProtection(p_sink)) {
      LOG_VERBOSE("%s: peer Sink for codec %s does not support "
                  "Content Protection",
...
BtaAvCoSep* BtaAvCo::FindPeerSource(BtaAvCoPeer* p_peer,
  ...
  for (size_t index = 0; index < p_peer->num_sup_sources; index++) {
    ...
    if (!AudioSepHasContentProtection(p_source)) {
      LOG_VERBOSE("%s: peer Source for codec %s does not support "
                  "Content Protection",
                  __func__, A2DP_CodecIndexStr(codec_index));
      continue;
    }
