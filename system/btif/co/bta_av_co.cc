...
BtaAvCoPeer* BtaAvCo::FindPeer(tBTA_AV_HNDL bta_av_handle) {
  ...
  LOG_DEBUG("%s: bta_av_handle = 0x%x index = %d", __func__, bta_av_handle,
            index);
...
BtaAvCoPeer* BtaAvCo::FindPeerAndUpdate(tBTA_AV_HNDL bta_av_handle,
                                        const RawAddress& peer_address) {
  LOG_DEBUG("%s: peer %s bta_av_handle = 0x%x", __func__,
  ...
  LOG_DEBUG("%s: peer %s bta_av_handle = 0x%x previous address %s", __func__,
            ADDRESS_TO_LOGGABLE_CSTR(peer_address), bta_av_handle,
...
tA2DP_STATUS BtaAvCo::ProcessSourceGetConfig(
  ...
  LOG_DEBUG("%s: peer %s bta_av_handle:0x%x codec:%s seid:%d", __func__,
            ADDRESS_TO_LOGGABLE_CSTR(peer_address), bta_av_handle,
            A2DP_CodecName(p_codec_info), seid);
  LOG_DEBUG("%s: num_protect:0x%02x protect_info:0x%02x%02x%02x", __func__,
            *p_num_protect, p_protect_info[0], p_protect_info[1],
            p_protect_info[2]);
  LOG_DEBUG("%s: codec: %s", __func__,
  ...
  LOG_DEBUG("%s: peer(o=%d, n_sinks=%d, n_rx_sinks=%d, n_sup_sinks=%d)",
  ...
  if (A2DP_IsPeerSinkCodecValid(p_codec_info)) {
    ...
    if (p_peer->num_sup_sinks < BTA_AV_CO_NUM_ELEMENTS(p_peer->sinks)) {
      ...
      LOG_DEBUG("%s: saved caps[%x:%x:%x:%x:%x:%x]", __func__, p_codec_info[1],
  ...
  LOG_DEBUG("%s: last Sink codec reached for peer %s (local %s)", __func__,
            ADDRESS_TO_LOGGABLE_CSTR(p_peer->addr),
  ...
  if (p_peer->acceptor) {
    ...
    if (btif_av_peer_prefers_mandatory_codec(p_peer->addr)) {
      ...
      if (p_sink != nullptr) {
        LOG_INFO("%s: mandatory codec preferred for peer %s", __func__,
                 ADDRESS_TO_LOGGABLE_CSTR(p_peer->addr));
  ...
  *p_sep_info_idx = p_sink->sep_info_idx;
  LOG_INFO("%s: peer %s acceptor:%s reconfig_needed:%s", __func__,
           ADDRESS_TO_LOGGABLE_CSTR(p_peer->addr),
           (p_peer->acceptor) ? "true" : "false",
           (p_peer->reconfig_needed) ? "true" : "false");
  if (p_peer->acceptor) {
    if (p_peer->reconfig_needed) {
      LOG_DEBUG("%s: call BTA_AvReconfig(0x%x) for peer %s", __func__,
...
tA2DP_STATUS BtaAvCo::ProcessSinkGetConfig(tBTA_AV_HNDL bta_av_handle,
  ...
  LOG_DEBUG("%s: peer %s found (o=%d, n_sources=%d, n_rx_sources=%d, "
            "n_sup_sources=%d)",
            __func__, ADDRESS_TO_LOGGABLE_CSTR(p_peer->addr), p_peer->opened,
            p_peer->num_sources, p_peer->num_rx_sources,
  ...
  LOG_DEBUG("%s: last Source codec reached for peer %s", __func__,
            ADDRESS_TO_LOGGABLE_CSTR(p_peer->addr));
...
void BtaAvCo::ProcessSetConfig(tBTA_AV_HNDL bta_av_handle,
  ...
  LOG_DEBUG("%s: bta_av_handle=0x%x peer_address=%s seid=%d "
            "num_protect=%d t_local_sep=%d avdt_handle=%d",
            __func__, bta_av_handle, ADDRESS_TO_LOGGABLE_CSTR(peer_address),
            seid, num_protect, t_local_sep, avdt_handle);
  LOG_DEBUG("%s: p_codec_info[%x:%x:%x:%x:%x:%x]", __func__, p_codec_info[1],
            p_codec_info[2], p_codec_info[3], p_codec_info[4], p_codec_info[5],
            p_codec_info[6]);
  LOG_DEBUG("%s: num_protect:0x%02x protect_info:0x%02x%02x%02x", __func__,
  ...
  LOG_DEBUG("%s: peer %s found (o=%d, n_sinks=%d, n_rx_sinks=%d, "
            "n_sup_sinks=%d)",
            __func__, ADDRESS_TO_LOGGABLE_CSTR(p_peer->addr), p_peer->opened,
  ...
  if (status == A2DP_SUCCESS) {
    ...
    if (t_local_sep == AVDT_TSEP_SNK) {
      LOG_DEBUG("%s: peer %s is A2DP Source", __func__,
                ADDRESS_TO_LOGGABLE_CSTR(p_peer->addr));
    ...
    if (t_local_sep == AVDT_TSEP_SRC) {
      LOG_DEBUG("%s: peer %s is A2DP SINK", __func__,
                ADDRESS_TO_LOGGABLE_CSTR(p_peer->addr));
  ...
  if (status != A2DP_SUCCESS) {
    LOG_DEBUG("%s: peer %s reject s=%d c=%d", __func__,
  ...
  LOG_DEBUG("%s: peer %s accept reconf=%d", __func__,
...
void BtaAvCo::ProcessOpen(tBTA_AV_HNDL bta_av_handle,
  ...
  LOG_DEBUG("%s: peer %s bta_av_handle: 0x%x mtu:%d", __func__,
...
void BtaAvCo::ProcessStart(tBTA_AV_HNDL bta_av_handle,
  ...
  LOG_DEBUG("%s: bta_av_handle: 0x%x add_rtp_header: %s", __func__,
...
BT_HDR* BtaAvCo::GetNextSourceDataPacket(const uint8_t* p_codec_info,
  ...
  LOG_DEBUG("%s: codec: %s", __func__, A2DP_CodecName(p_codec_info));
...
void BtaAvCo::ProcessAudioDelay(tBTA_AV_HNDL bta_av_handle,
                                const RawAddress& peer_address,
                                uint16_t delay) {
  LOG_DEBUG("%s: peer %s bta_av_handle: 0x%x delay:0x%x", __func__,
...
bool BtaAvCo::ReportSinkCodecState(BtaAvCoPeer* p_peer) {
  LOG_DEBUG("%s: peer_address=%s", __func__,
            ADDRESS_TO_LOGGABLE_CSTR(p_peer->addr));
...
bool BtaAvCo::ContentProtectIsScmst(const uint8_t* p_protect_info) {
  ...
  if (*p_protect_info >= AVDT_CP_LOSC) {
    ...
    if (cp_id == AVDT_CP_SCMS_T_ID) {
      LOG_DEBUG("%s: SCMS-T found", __func__);
      return true;
    }
  }
  return false;
...
bool BtaAvCo::AudioProtectHasScmst(uint8_t num_protect,
  ...
  LOG_DEBUG("%s: SCMS-T not found", __func__);
  return false;
}
...
bool BtaAvCo::AudioSepHasContentProtection(const BtaAvCoSep* p_sep) {
  ...
  LOG_DEBUG("%s: not required", __func__);
  return true;
}
...
const BtaAvCoSep* BtaAvCo::SelectSinkCodec(BtaAvCoPeer* p_peer) {
  ...
  for (const auto& iter : p_peer->GetCodecs()->orderedSinkCodecs()) {
    LOG_DEBUG("%s: trying codec %s", __func__, iter->name().c_str());
    p_source = AttemptSinkCodecSelection(*iter, p_peer);
    if (p_source != nullptr) {
      LOG_DEBUG("%s: selected codec %s", __func__, iter->name().c_str());
      break;
    }
    LOG_DEBUG("%s: cannot use codec %s", __func__, iter->name().c_str());
  }
...
BtaAvCoSep* BtaAvCo::FindPeerSink(BtaAvCoPeer* p_peer,
  ...
  for (size_t index = 0; index < p_peer->num_sup_sinks; index++) {
    ...
    if (!AudioSepHasContentProtection(p_sink)) {
      LOG_DEBUG("%s: peer Sink for codec %s does not support "
                "Content Protection",
...
BtaAvCoSep* BtaAvCo::FindPeerSource(BtaAvCoPeer* p_peer,
  ...
  for (size_t index = 0; index < p_peer->num_sup_sources; index++) {
    ...
    if (!AudioSepHasContentProtection(p_source)) {
      LOG_DEBUG("%s: peer Source for codec %s does not support "
                "Content Protection",
                __func__, A2DP_CodecIndexStr(codec_index));
      continue;
    }
...
const BtaAvCoSep* BtaAvCo::AttemptSourceCodecSelection(
  ...
  if (p_sink == nullptr) {
    LOG_DEBUG("%s: peer Sink for codec %s not found", __func__,
  ...
  if (!p_peer->GetCodecs()->setCodecConfig(
    ...
    LOG_DEBUG("%s: cannot set source codec %s", __func__,
              codec_config.name().c_str());
    return nullptr;
  }
...
const BtaAvCoSep* BtaAvCo::AttemptSinkCodecSelection(
  ...
  if (p_source == nullptr) {
    LOG_DEBUG("%s: peer Source for codec %s not found", __func__,
              codec_config.name().c_str());
    return nullptr;
  }
  if (!p_peer->GetCodecs()->setSinkCodecConfig(
          ...
          true /* select_current_codec */)) {
    LOG_DEBUG("%s: cannot set sink codec %s", __func__,
              codec_config.name().c_str());
    return nullptr;
  }
...
size_t BtaAvCo::UpdateAllSelectableSourceCodecs(BtaAvCoPeer* p_peer) {
  LOG_DEBUG("%s: peer %s", __func__, ADDRESS_TO_LOGGABLE_CSTR(p_peer->addr));

  size_t updated_codecs = 0;
  for (const auto& iter : p_peer->GetCodecs()->orderedSourceCodecs()) {
    LOG_DEBUG("%s: updating selectable codec %s", __func__,
              iter->name().c_str());
...
tA2DP_STATUS bta_av_co_audio_getconfig(tBTA_AV_HNDL bta_av_handle,
  ...
  LOG_DEBUG("%s: peer %s bta_av_handle=0x%x peer_uuid=0x%x", __func__,
