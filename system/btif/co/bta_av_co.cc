...
class BtaAvCo {
 ...
 private:
  ...
  const BtaAvCoSep* AttemptSourceCodecSelection(
      ...
      const std::optional<::bluetooth::audio::a2dp::a2dp_configuration>&
          provider_config);
...
const BtaAvCoSep* BtaAvCo::AttemptSourceCodecSelection(
  ...
  // Attempt to set software codec configuration without saving it
  ...
  if (provider_config.has_value() &&
      bta_av_co_should_select_hardware_codec(codec_config,
                                             provider_config.value())) {
    auto hardware_config = provider_config.value();
    LOG_INFO("Provider configuration: %s", hardware_config.toString().c_str());
    BtaAvCoSep* p_sink_offload =
        FindPeerSink(p_peer, hardware_config.codec_index);
    if (p_sink_offload != nullptr) {
      LOG_INFO("Stack source codec %s overridden by provider codec %s",
               codec_config.name().c_str(), hardware_config.toString().c_str());
      p_sink = p_sink_offload;
      std::memcpy(new_codec_config, hardware_config.codec_config,
                  AVDT_CODEC_SIZE);
    } else {
      // Peer sink for hardware offload codec not found,
      // set and save the software codec instead
      LOG_VERBOSE("%s: peer Sink for codec (%s) not found", __func__,
                  hardware_config.toString().c_str());
      if (!p_peer->GetCodecs()->setCodecConfig(
              p_sink->codec_caps, true /* is_capability */, new_codec_config,
