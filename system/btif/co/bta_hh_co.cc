...
static size_t uhid_calc_msg_len(const struct uhid_event* ev, size_t var_len) {
  switch (ev->type) {
    ...
    case UHID_START:
      return 4 + sizeof(struct uhid_start_req);
    case UHID_OUTPUT:
      return 4 + sizeof(struct uhid_output_req);
    case UHID_GET_REPORT:
      return 4 + sizeof(struct uhid_get_report_req);
    case UHID_SET_REPORT_REPLY:
      return 4 + sizeof(struct uhid_set_report_reply_req);
    // these messages has variable amount of data. We only need to write the
    // necessary length.
    case UHID_GET_REPORT_REPLY:
      return 4 + sizeof(struct uhid_get_report_reply_req) - UHID_DATA_MAX + var_len;
    case UHID_CREATE2:
      return 4 + sizeof(struct uhid_create2_req) - HID_MAX_DESCRIPTOR_SIZE + var_len;
    case UHID_INPUT2:
      return 4 + sizeof(struct uhid_input2_req) - UHID_DATA_MAX + var_len;
    case UHID_SET_REPORT:
      return 4 + sizeof(struct uhid_set_report_req) - UHID_DATA_MAX + var_len;
    default:
...
static bool to_uhid_thread(int fd, const tBTA_HH_TO_UHID_EVT* ev, size_t data_len) {
  ...
  size_t len = data_len + 1;  // type is 1 byte
  ssize_t ret;
  OSI_NO_INTR(ret = write(fd, ev, len));
...
void bta_hh_co_send_hid_info(btif_hh_device_t* p_dev, const char* dev_name, uint16_t vendor_id,
  ...
  if (dscp_len > HID_MAX_DESCRIPTOR_SIZE) {
