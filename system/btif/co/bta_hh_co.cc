...
void uhid_set_non_blocking(int fd) {
  ...
  if (fcntl(fd, F_SETFL, opts) < 0)
    LOG_INFO("%s() Setting non-blocking flag failed (%s)", __func__,
             strerror(errno));
}
...
static int uhid_read_event(btif_hh_device_t* p_dev) {
  ...
  switch (ev.type) {
    case UHID_START:
      LOG_DEBUG("UHID_START from uhid-dev\n");
      p_dev->ready_for_data = true;
      break;
    case UHID_STOP:
      LOG_DEBUG("UHID_STOP from uhid-dev\n");
      p_dev->ready_for_data = false;
      break;
    case UHID_OPEN:
      LOG_DEBUG("UHID_OPEN from uhid-dev\n");
      p_dev->ready_for_data = true;
      break;
    case UHID_CLOSE:
      LOG_DEBUG("UHID_CLOSE from uhid-dev\n");
      p_dev->ready_for_data = false;
      break;
    case UHID_OUTPUT:
      ...
      LOG_DEBUG("UHID_OUTPUT: Report type = %d, report_size = %d",
    ...
    case UHID_OUTPUT_EV:
      ...
      LOG_DEBUG("UHID_OUTPUT_EV from uhid-dev\n");
      break;
...
static inline pthread_t create_thread(void* (*start_routine)(void*),
                                      void* arg) {
  LOG_DEBUG("create_thread: entered");
  ...
  LOG_DEBUG("create_thread: thread created successfully");
  return thread_id;
}
...
void bta_hh_co_data(uint8_t dev_handle, uint8_t* p_rpt, uint16_t len,
  ...
  LOG_DEBUG("%s: dev_handle = %d, subclass = 0x%02X, mode = %d, "
...
void bta_hh_co_send_hid_info(btif_hh_device_t* p_dev, const char* dev_name,
  ...
  LOG_WARN("%s: fd = %d, name = [%s], dscp_len = %d", __func__, p_dev->fd,
           dev_name, dscp_len);
  LOG_WARN("%s: vendor_id = 0x%04x, product_id = 0x%04x, version= 0x%04x,"
           "ctry_code=0x%02x",
...
void bta_hh_le_co_rpt_info(const RawAddress& remote_bda,
  ...
  if (idx < BTA_HH_NV_LOAD_MAX) {
    ...
    LOG_DEBUG("%s() - Saving report; dev=%s, idx=%d", __func__,
...
tBTA_HH_RPT_CACHE_ENTRY* bta_hh_le_co_cache_load(const RawAddress& remote_bda,
  ...
  *p_num_rpt = len / sizeof(tBTA_HH_RPT_CACHE_ENTRY);
  ...
  LOG_DEBUG("%s() - Loaded %d reports; dev=%s", __func__, *p_num_rpt,
...
void bta_hh_le_co_reset_rpt_cache(const RawAddress& remote_bda,
  ...
  LOG_DEBUG("%s() - Reset cache for bda %s", __func__,
            ADDRESS_TO_LOGGABLE_CSTR(remote_bda));
}
