...
void bta_hh_le_co_rpt_info(const RawAddress& remote_bda,
  ...
  if (len >= sizeof(tBTA_HH_RPT_CACHE_ENTRY) && len <= sizeof(sReportCache)) {
    btif_config_get_bin(bdstr, BTIF_STORAGE_KEY_HID_REPORT, (uint8_t*)sReportCache, &len);
  ...
  if (idx < BTA_HH_NV_LOAD_MAX) {
    ...
    btif_config_set_bin(bdstr, BTIF_STORAGE_KEY_HID_REPORT, (const uint8_t*)sReportCache,
                        idx * sizeof(tBTA_HH_RPT_CACHE_ENTRY));
    btif_config_set_int(bdstr, BTIF_STORAGE_KEY_HID_REPORT_VERSION, BTA_HH_CACHE_REPORT_VERSION);
...
tBTA_HH_RPT_CACHE_ENTRY* bta_hh_le_co_cache_load(const RawAddress& remote_bda,
  ...
  int cache_version = -1;
  btif_config_get_int(bdstr, BTIF_STORAGE_KEY_HID_REPORT_VERSION, &cache_version);
