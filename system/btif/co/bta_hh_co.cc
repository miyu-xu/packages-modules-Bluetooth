    ...
    if (p_dev->set_rpt_id_queue) {
      ...
      *context = req.id;
      ...
      if (fixed_queue_try_enqueue(p_dev->set_rpt_id_queue, (void*)context)) {
        btif_hh_setreport(p_dev, map_rtype_uhid_hh[req.rtype], req.size,
                          req.data);
        return true;
      } else {
        osi_free(context);
...
void bta_hh_co_set_rpt_rsp(uint8_t dev_handle, uint8_t status) {
  ...
  if (p_dev->fd >= 0) {
    ...
    if (context) {
      ...
      ev.u.set_report_reply.id = *context;
      ev.u.set_report_reply.err = status;
      uhid_write(p_dev->fd, &ev);
      osi_free(context);
    } else {
      LOG_WARNING("No pending UHID_SET_REPORT");
    }
  } else {
...
void bta_hh_co_get_rpt_rsp(uint8_t dev_handle, uint8_t status,
  ...
  if (p_dev->fd >= 0) {
    ...
    if (len > 0 && len <= UHID_DATA_MAX) {
      ...
      ev.u.feature_answer.id = *context;
