...
static int uhid_read_outbound_event(btif_hh_uhid_t* p_uhid) {
  ...
  switch (ev.type) {
    case UHID_START:
      ...
      btif_hh_cb.uhid_history.record(p_uhid->link_spec, std::string("UHID_START"), std::string());
    ...
    case UHID_STOP:
      ...
      btif_hh_cb.uhid_history.record(p_uhid->link_spec, std::string("UHID_STOP"), std::string());
      if (!com::android::bluetooth::flags::hid_report_queuing()) {
    ...
    case UHID_OPEN:
      ...
      btif_hh_cb.uhid_history.record(p_uhid->link_spec, std::string("UHID_OPEN"), std::string());
      if (com::android::bluetooth::flags::hid_report_queuing()) {
        uhid_on_open(p_uhid);
      } else {
        p_uhid->ready_for_data = true;
    ...
    case UHID_CLOSE:
      ...
      btif_hh_cb.uhid_history.record(p_uhid->link_spec, std::string("UHID_CLOSE"), std::string());
      p_uhid->ready_for_data = false;
      if (com::android::bluetooth::flags::hid_report_queuing()) {
...
static void uhid_fd_close(btif_hh_uhid_t* p_uhid) {
  if (p_uhid->fd >= 0) {
    ...
    btif_hh_cb.uhid_history.record(p_uhid->link_spec, std::string("UHID_DESTROY"), std::string());
    close(p_uhid->fd);
    p_uhid->fd = -1;
...
void bta_hh_co_set_rpt_rsp([[maybe_unused]] uint8_t dev_handle, [[maybe_unused]] uint8_t status) {
  ...
  btif_hh_cb.uhid_history.record(p_dev->link_spec, std::string("UHID_SET_REPORT_REPLY"),
                                 std::string());
  if (com::android::bluetooth::flags::hid_report_queuing()) {
    tBTA_HH_TO_UHID_EVT to_uhid = {};
...
void bta_hh_co_get_rpt_rsp(uint8_t dev_handle, uint8_t status, const uint8_t* p_rpt, uint16_t len) {
  ...
  btif_hh_cb.uhid_history.record(p_dev->link_spec, std::string("UHID_GET_REPORT_REPLY"),
                                 std::string());
  if (com::android::bluetooth::flags::hid_report_queuing()) {
    tBTA_HH_TO_UHID_EVT to_uhid = {};
