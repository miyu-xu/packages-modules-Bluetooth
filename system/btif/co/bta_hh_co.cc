...
static const map_rtype_uhid_hh[] = {BTHH_FEATURE_REPORT, BTHH_OUTPUT_REPORT,
                                    BTHH_INPUT_REPORT};
...
static bool uhid_get_report_req_handler(btif_hh_device_t* p_dev, uhid_get_report_req &req) {
...
static bool uhid_set_report_req_handler(btif_hh_device_t* p_dev, uhid_set_report_req &req) {
  LOG_DEBUG("Report type = %d, id = %d", req.rtype, req.rnum);

  if (req.rtype <= UHID_INPUT_REPORT) {
    if (p_dev->get_spt_id_queue) {
      uint32_t* context = (uint32_t*)osi_malloc(sizeof(uint32_t));
      *context = req.id;
    ...
    case UHID_GET_REPORT:
      if (ret < (ssize_t)(sizeof(ev.type) + sizeof(ev.u.get_report))) {
        LOG_ERROR("UHID_GET_REPORT: Invalid size read from uhid-dev: %zd < %zu",
                  ret, sizeof(ev.type) + sizeof(ev.u.get_report));
        return -EFAULT;
      }

      if (!uhid_get_report_req_handler(p_dev, ev.u.get_report)) {
        return -EFAULT;
      }
