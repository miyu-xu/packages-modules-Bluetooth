...
#define BTA_HH_UHID_POLL_PERIOD_US 50
/* Max number of polling interrupt allowed (100 milliseconds) */
#define BTA_HH_UHID_INTRRUPT_COUNT_MAX \
  (100 * (1000 / BTA_HH_UHID_POLL_PERIOD_US))
...
static void* btif_hh_poll_event_thread(void* arg) {
  ...
  while (p_dev->hh_keep_polling) {
    ...
    do {
      if (counter++ > BTA_HH_UHID_INTRRUPT_COUNT_MAX) {
        LOG_ERROR("Polling interrupted");
        break;
      }
      ret = poll(pfds, 1, BTA_HH_UHID_POLL_PERIOD_US);
