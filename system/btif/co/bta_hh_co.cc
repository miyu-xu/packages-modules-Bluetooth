...
/* poll timeout with the aflags hid_report_queuing */
...
static void uhid_write_input_queue(btif_hh_uhid_t* p_uhid) {
...
static void* btif_hh_poll_event_thread(void* arg) {
  ...
  if (com::android::bluetooth::flags::hid_report_queuing()) {
    ...
    log::assert_that(p_uhid->get_rpt_id_queue,
                     "assert failed: p_uhid->get_rpt_id_queue");
#if ENABLE_UHID_SET_REPORT
    p_uhid->set_rpt_id_queue = fixed_queue_new(SIZE_MAX);
    log::assert_that(p_uhid->set_rpt_id_queue,
                     "assert failed: p_uhid->set_rpt_id_queue");
#endif  // ENABLE_UHID_SET_REPORT
