...
void on_create_record_event(int id) {
  ...
  if (sdp_slot != nullptr && (record = sdp_slot->record_data) != nullptr) {
    ...
    if (handle != -1) {
      ...
      if (service_id > 0) {
        /**
         * {@link btif_enable_service} calls {@link btif_dm_enable_service}, which calls
         * {@link btif_in_execute_service_request}.
         *     - {@link btif_enable_service} sets the mask {@link btif_enabled_services}.
         *     - {@link btif_dm_enable_service} invokes the java callback to return uuids
         * based on the enabled services mask.
