...
bt_status_t sdp_server_init() {
  LOG_DEBUG("Sdp Server %s", __func__);
  init_sdp_slots();
...
static const sdp_slot_t* start_create_sdp(int id) {
  ...
  if (sdp_slots[id].state != SDP_RECORD_ALLOCED) {
    ...
    LOG_ERROR("%s() failed - state for id %d is "
...
bt_status_t create_sdp_record(bluetooth_sdp_record* record,
  ...
  LOG_DEBUG("%s() handle = 0x%08x", __func__, handle);
...
bt_status_t remove_sdp_record(int record_id) {
  ...
  LOG_DEBUG("Sdp Server %s id=%d to handle=0x%08x", __func__, record_id,
            handle);
  ...
  LOG_DEBUG("Sdp Server %s - record already removed - or never created",
            __func__);
  ...
  if (sdp_slot != nullptr && (record = sdp_slot->record_data) != nullptr) {
    ...
    switch (record->hdr.type) {
      ...
      case SDP_TYPE_RAW:
        ...
        break;
      default:
        LOG_DEBUG("Record type %d is not supported", record->hdr.type);
        break;
    }
    if (handle != -1) {
...
static int add_maps_sdp(const bluetooth_sdp_mas_record* rec) {
  ...
  if (!status) {
    ...
    LOG_DEBUG("%s():  SDP Registered (handle 0x%08x)", __func__, sdp_handle);
  }
  return sdp_handle;
}
...
static int add_pbaps_sdp(const bluetooth_sdp_pse_record* rec) {
  ...
  if (bluetooth::common::init_flags::
    ...
    status &= get_legacy_stack_sdp_api()->handle.SDP_AddAttribute(
        ...
        (uint8_t*)&supported_repositories_1_1);
    LOG_DEBUG(" supported_repositories_1_1: 0x%x", supported_repositories_1_1);
