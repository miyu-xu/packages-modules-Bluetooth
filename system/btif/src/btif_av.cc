...
class BtifAvSource {
 public:
  ...
  /**
   * Trigger SUSPEND or STOP A2DP stream event on all peer devices.
   * Returns true if succeeded.
   * @param event SUSPEND or STOP event
   * @param av_stream_idle True if no peer devices suspended nor stopped.
   */
  bool HoldPeersStreamsEvent(btif_av_sm_event_t event, bool& av_stream_idle);
...
bool BtifAvSource::AllowedToConnect(const RawAddress& peer_address) const {
  ...
  if (com::android::bluetooth::flags::a2dp_concurrent_source_sink()) {
    const int sink_connected_peers_size = btif_av_sink.NumberOfPeers();
    log::info(
        "connected={}, max_connected_peers_={}, sink_connected_peers_size={}",
        connected, max_connected_peers_, btif_av_sink.NumberOfPeers());
...
bool BtifAvSource::HoldPeersStreamsEvent(btif_av_sm_event_t event,
                                         bool& av_stream_idle) {
  std::lock_guard<std::recursive_mutex> lock(btifavsource_peers_lock_);
  if (event != BTIF_AV_SUSPEND_STREAM_REQ_EVT &&
      event != BTIF_AV_STOP_STREAM_REQ_EVT) {
    log::warn("Wrong hold event: {}", (int)event);
    return false;
  }
  av_stream_idle = true;
  for (auto it : peers_) {
...
bool BtifAvSink::AllowedToConnect(const RawAddress& peer_address) const {
  ...
  if (!com::android::bluetooth::flags::a2dp_concurrent_source_sink() &&
    ...
    log::info("connected={}, max_connected_peers_={}, source_peers={}",
              connected, max_connected_peers_, btif_av_source.NumberOfPeers());
    /* if source device connected, don't connect sink device */
    return (connected < max_connected_peers_) &&
           (btif_av_source.NumberOfPeers() == 0);
  }

  if (com::android::bluetooth::flags::a2dp_concurrent_source_sink()) {
    const int source_connected_peers_size = btif_av_source.NumberOfPeers();
    log::info(
    ...
    if (btif_av_both_enable()) {
      ...
      if (peer == nullptr) {
        if (peer_sep == AVDT_TSEP_SRC) {
          ...
          if (!(btif_av_source.NumberOfPeers() == 0)) {
            log::verbose(
                "peer_sep invalid, and already has sink peer, so try create a "
                "new sink peer");
            peer = btif_av_source.FindOrCreatePeer(peer_address, bta_handle);
          } else if (!(btif_av_sink.NumberOfPeers() == 0)) {
            log::verbose(
...
void btif_av_stream_stop(const RawAddress& peer_address) {
  ...
  if (btif_av_source.HoldPeersStreamsEvent(BTIF_AV_STOP_STREAM_REQ_EVT,
                                           av_stream_idle) &&
      av_stream_idle) {
...
void btif_av_stream_suspend(void) {
  ...
  bool av_stream_idle = true;
  if (btif_av_source.HoldPeersStreamsEvent(BTIF_AV_SUSPEND_STREAM_REQ_EVT,
                                           av_stream_idle) &&
      av_stream_idle) {
    btif_a2dp_on_stopped(nullptr, A2dpType::kSource);
  }
