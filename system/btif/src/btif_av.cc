 ...
 public:
  ...
  bool SetActivePeer(const RawAddress& peer_address,
    ...
    if (peer == nullptr || !peer->IsConnected()) {
      LOG_ERROR("%s: Error setting %s as active Source peer", __func__, peer_address.ToString().c_str());
      peer_ready_promise.set_value();
      return false;
    }
...
class BtifAvSink {
 public:
  ...
  bool SetActivePeer(const RawAddress& peer_address,
    ...
    BtifAvPeer* peer = FindPeer(peer_address);
    if (peer == nullptr || !peer->IsConnected()) {
      LOG_ERROR("%s: Error setting %s as active Sink peer", __func__, peer_address.ToString().c_str());
      peer_ready_promise.set_value();
      return false;
    }
...
const RawAddress& BtifAvPeer::ActivePeerAddress() const {
  ...
  LOG_ALWAYS_FATAL("%s: A2DP peer %s is neither Source nor Sink", __func__, PeerAddress().ToString().c_str());
  ...
  switch (event) {
    ...
    case BTA_AV_START_EVT: {
      ...
      if (peer_.IsSink()) {
        if (!peer_.CheckFlags(BtifAvPeer::kFlagPendingStart |
          ...
          LOG_WARN("%s: Peer %s : trigger Suspend as remote initiated", __func__, peer_.PeerAddress().ToString().c_str());
          should_suspend = true;
        } else if (!peer_.IsActivePeer()) {
          LOG_WARN("%s: Peer %s : trigger Suspend as non-active", __func__, peer_.PeerAddress().ToString().c_str());
          should_suspend = true;
        }
    ...
    case BTA_AV_RECONFIG_EVT:
      if (p_av->reconfig.status != BTA_AV_SUCCESS) {
        LOG_WARN("%s: Peer %s : failed reconfiguration", __func__, peer_.PeerAddress().ToString().c_str());
        if (peer_.CheckFlags(BtifAvPeer::kFlagPendingStart)) {
          LOG_ERROR("%s: Peer %s : cannot proceed to do AvStart", __func__, peer_.PeerAddress().ToString().c_str());
          peer_.ClearFlags(BtifAvPeer::kFlagPendingStart);
      ...
      if (peer_.IsActivePeer()) {
        LOG_INFO("%s : Peer %s : Reconfig done - calling startSession() to audio HAL",
            __func__, peer_.PeerAddress().ToString().c_str());
        std::promise<void> peer_ready_promise;
      ...
      if (peer_.CheckFlags(BtifAvPeer::kFlagPendingStart)) {
        LOG_INFO("%s : Peer %s : Reconfig done - calling BTA_AvStart(%s)",
            __func__, peer_.PeerAddress().ToString().c_str(),
            loghex(peer_.BtaHandle()).c_str());
        BTA_AvStart(peer_.BtaHandle());
      }
      break;
...
static bt_status_t connect_int(RawAddress* peer_address, uint16_t uuid) {
  ...
  if (status != BT_STATUS_SUCCESS) {
    LOG_ERROR("%s: can't post connection task to main_thread", __func__);
  }
  return status;
}
...
static bt_status_t sink_set_active_device(const RawAddress& peer_address) {
  ...
  if (status == BT_STATUS_SUCCESS) {
    ...
    LOG_WARN("%s: BTIF AV Sink fails to change peer", __func__);
  }
  return status;
}
...
static bt_status_t src_set_active_sink(const RawAddress& peer_address) {
  ...
  if (status == BT_STATUS_SUCCESS) {
    ...
    LOG_WARN("%s: BTIF AV Source fails to change peer", __func__);
  }
  return status;
}
...
static bt_status_t codec_config_src(
  ...
  if (peer_address.IsEmpty()) {
    LOG_WARN("%s: BTIF AV Source needs peer to config", __func__);
  ...
  if (status == BT_STATUS_SUCCESS) {
    ...
    LOG_WARN("%s: BTIF AV Source fails to config codec", __func__);
  }
  return status;
}
