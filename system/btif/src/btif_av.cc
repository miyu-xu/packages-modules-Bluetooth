...
void BtifAvStateMachine::StateIdle::OnEnter() {
  ...
  if (peer_.IsSink()) {
    do_in_main_thread(base::BindOnce(&BtifAvSource::DeleteIdlePeers,
                                                base::Unretained(&btif_av_source)));
  } else if (peer_.IsSource()) {
    do_in_main_thread(base::BindOnce(&BtifAvSink::DeleteIdlePeers,
...
static void bta_av_source_callback(tBTA_AV_EVT event, tBTA_AV* p_data) {
  ...
  do_in_main_thread(base::BindOnce(&btif_av_handle_bta_av_event,
