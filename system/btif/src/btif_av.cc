...
static BtifAvPeer* btif_av_find_peer(const RawAddress& peer_address,
                                     const A2dpType a2dp_type) {
  if (IS_FLAG_ENABLED(a2dp_concurrent_source_sink)) {
    if (btif_av_source.Enabled() && a2dp_type == A2dpType::kSource) {
      BtifAvPeer* sourcePeer = btif_av_source_find_peer(peer_address);
      if (sourcePeer != nullptr) {
        return sourcePeer;
      }
    }
    if (btif_av_sink.Enabled() && a2dp_type == A2dpType::kSink) {
...
static BtifAvPeer* btif_av_find_active_peer(const A2dpType a2dp_type) {
  if (IS_FLAG_ENABLED(a2dp_concurrent_source_sink)) {
    if (btif_av_source.Enabled() && a2dp_type == A2dpType::kSource)
  ...
  switch (event) {
    ...
    case BTA_AV_RC_OPEN_EVT: {
      ...
      if (peer_.IsSink()) {
        can_connect = btif_av_source.AllowedToConnect(peer_.PeerAddress());
        if (!can_connect) {
          LOG_ERROR("Source profile doesn't allow connection to peer:%s",
                    ADDRESS_TO_LOGGABLE_CSTR(peer_.PeerAddress()));
          if (btif_av_src_sink_coexist_enabled())
        ...
        can_connect = btif_av_sink.AllowedToConnect(peer_.PeerAddress());
        if (!can_connect) {
          LOG_ERROR("Sink profile doesn't allow connection to peer:%s",
                    ADDRESS_TO_LOGGABLE_CSTR(peer_.PeerAddress()));
          if (btif_av_src_sink_coexist_enabled())
            BTA_AvCloseRc(((tBTA_AV*)p_data)->rc_open.rc_handle);
          else
...
static void btif_report_connection_state(const RawAddress& peer_address,
  ...
  if (btif_av_src_sink_coexist_enabled() && btif_av_both_enable()) {
    BtifAvPeer* peer = btif_av_find_peer(peer_address, a2dp_type);
    if (peer == nullptr) {
...
bool btif_av_stream_ready(const A2dpType a2dp_type) {
  ...
  BtifAvPeer* peer = btif_av_find_active_peer(a2dp_type);
  if (peer == nullptr) {
...
void btif_av_clear_remote_suspend_flag(const A2dpType a2dp_type) {
  auto clear_remote_suspend_flag = [](const A2dpType a2dp_type) {
    BtifAvPeer* peer = btif_av_find_active_peer(a2dp_type);
    if (peer == nullptr) {
      LOG_WARN("%s: No active peer found", __func__);
      return;
  ...
  // switch to main thread to prevent a race condition of accessing peers
  do_in_main_thread(FROM_HERE,
                    base::BindOnce(clear_remote_suspend_flag, a2dp_type));
}
...
void btif_av_acl_disconnected(const RawAddress& peer_address) {
  ...
  LOG_INFO("%s: Peer %s : ACL Disconnected", __func__,
           ADDRESS_TO_LOGGABLE_CSTR(peer_address));
  if (IS_FLAG_ENABLED(a2dp_concurrent_source_sink) || btif_av_both_enable()) {
    BtifAvPeer* peer = btif_av_find_peer(peer_address, A2dpType::kUnknown);
    if (peer != nullptr) {
      if (peer->IsSource()) {
