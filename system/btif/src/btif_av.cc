...
typedef struct { bool use_latency_mode; } btif_av_start_stream_req_t;

typedef struct { bool is_low_latency; } btif_av_set_latency_req_t;
 ...
 public:
  ...
  bool SetActivePeer(const RawAddress& peer_address,
    ...
    LOG(INFO) << __PRETTY_FUNCTION__
              << ": peer: " << ADDRESS_TO_LOGGABLE_STR(peer_address);
...
class BtifAvSink {
 public:
  ...
  bool SetActivePeer(const RawAddress& peer_address,
    ...
    if (btif_av_src_sink_coexist_enabled()) btif_av_source_delete_active_peer();
    BtifAvPeer* peer = FindPeer(peer_address);
    if (peer == nullptr || !peer->IsConnected()) {
      LOG(ERROR) << __func__ << ": Error setting "
                 << ADDRESS_TO_LOGGABLE_STR(peer_address)
