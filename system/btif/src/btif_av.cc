...
class BtifAvSource {
 public:
  ...
  btif_av_reconf_stream_req_t GetReconfigureStreamData() { return std::move(reconf_stream_data_); }
...
bool BtifAvStateMachine::StateIdle::ProcessEvent(uint32_t event, void* p_data) {
  ...
  switch (event) {
    ...
    case BTIF_AV_RECONFIGURE_REQ_EVT: {
      ...
      req_data.reconf_ready_promise.set_value();
      break;
    }
  ...
  switch (event) {
    ...
    case BTIF_AV_RECONFIGURE_REQ_EVT: {
      ...
      if (peer_.CheckFlags(BtifAvPeer::kFlagPendingStart)) {
        ...
        if (btif_av_source.Enabled()) {
          auto req_data = btif_av_source.GetReconfigureStreamData();
          btif_av_source.UpdateCodecConfig(peer_.PeerAddress(), req_data.codec_preferences,
...
bt_status_t btif_av_source_set_codec_config_preference(
  ...
  if (peer_ready_future.wait_for(std::chrono::seconds(1)) != std::future_status::ready) {
    log::error("BTIF AV Source fails to config codec due to timeout");
    return BT_STATUS_FAIL;
  }
