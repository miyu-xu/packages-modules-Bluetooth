...
public:
  ...
  bool SetActivePeer(const RawAddress& peer_address, std::promise<void> peer_ready_promise) {
    ...
    if(!peer_address.IsEmpty() && peer && (peer->IsSink() &&
       AllowedToConnect(peer_address)) && !active_peer_.IsEmpty() &&
       active_peer && active_peer->CheckFlags(BtifAvPeer::kFlagPendingStart)) {
