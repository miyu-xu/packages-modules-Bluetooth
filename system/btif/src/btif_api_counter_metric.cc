...
std::map<std::string, size_t> get_counter_ordered_by_name(
    const std::unordered_map<std::string, size_t>& map) {
  std::lock_guard<std::mutex> lock(mutex_);
  std::map<std::string, size_t> ordered_entry(map.begin(), map.end());
  return ordered_entry;
}

std::vector<std::pair<std::string, size_t>> get_ordered_api_counter_metric(
    std::unordered_map<std::string, size_t>& map) {
  const auto ordered_entry = get_counter_ordered_by_name(map);
  std::vector<std::pair<std::string, size_t>> vector;
  for (const auto& it : ordered_entry) {
    vector.push_back(std::make_pair(it.first.data(), it.second));
  }
  return vector;
}
