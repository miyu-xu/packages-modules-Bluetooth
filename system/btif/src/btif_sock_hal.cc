...
public:
  ...
  void SocketClose(uint64_t socket_id) const override {
    ...
    if (is_l2cap_socket_found(socket_id)) {
      ...
      log::error("SocketClose: Unable to find socket with socket_id:{}", socket_id);
    }
  }
};
