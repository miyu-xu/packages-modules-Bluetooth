...
static void hh_open_handler(tBTA_HH_CONN& conn) {
  ...
  p_dev->link_spec.addrt.bda = conn.link_spec.addrt.bda;
 ...
 ******************************************************************************/
bool btif_hh_add_added_dev(const tAclLinkSpec& transport,
  ...
  for (i = 0; i < BTIF_HH_MAX_ADDED_DEV; i++) {
    if (btif_hh_cb.added_devices[i].link_spec.addrt.bda.IsEmpty()) {
      ...
      btif_hh_cb.added_devices[i].link_spec = transport;
 ...
 ** Returns          void
 ******************************************************************************/
void btif_hh_remove_device(const tAclLinkSpec& transport) {
  int i;
  ...
  for (i = 0; i < BTIF_HH_MAX_ADDED_DEV; i++) {
    ...
    if (p_added_dev->link_spec.addrt.bda == transport.addrt.bda) {
      BTA_HhRemoveDev(p_added_dev->dev_handle);
      btif_storage_remove_hid_info(p_added_dev->link_spec.addrt.bda);
      p_added_dev->link_spec.addrt.bda = RawAddress::kEmpty;
...
bt_status_t btif_hh_virtual_unplug(const tAclLinkSpec* link_spec) {
  ...
  if ((p_dev != NULL) && (p_dev->dev_status == BTHH_CONN_STATE_CONNECTED) &&
    ...
    if ((btif_hh_cb.pending_conn_address.addrt.bda == link_spec->addrt.bda) &&
...
bt_status_t btif_hh_connect(const tAclLinkSpec* link_spec) {
  ...
  if (added_dev != NULL) {
    if (added_dev->dev_handle == BTA_HH_INVALID_HANDLE) {
      ...
      added_dev->link_spec.addrt.bda = RawAddress::kEmpty;
  ...
  btif_hh_cb.status = BTIF_HH_DEV_CONNECTING;
  btif_hh_cb.pending_conn_address.addrt.bda = link_spec->addrt.bda;
      ...
      for (i = 0; i < BTIF_HH_MAX_ADDED_DEV; i++) {
        if (btif_hh_cb.added_devices[i].link_spec.addrt.bda ==
          ...
          if (p_data->dev_info.status == BTA_HH_OK) {
            ...
            btif_hh_cb.added_devices[i].link_spec.addrt.bda =
                RawAddress::kEmpty;
            btif_hh_cb.added_devices[i].dev_handle = BTA_HH_INVALID_HANDLE;
          }
          break;
