...
// Todo: Remove when simpler_hid_state_machine is released
...
static void hh_connect_complete_(tBTA_HH_CONN& conn, bthh_connection_state_t state) {
...
static void hh_connect_complete(tBTA_HH_CONN& conn, bthh_connection_state_t state) {
  if (!com::android::bluetooth::flags::simpler_hid_state_machine()) {
    hh_connect_complete_(conn, state);
    return;
  }
