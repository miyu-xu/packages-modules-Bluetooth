...
static const char* get_cached_name(tAclLinkSpec& link_spec) {
  const char* cached_name = nullptr;
  bt_bdname_t bdname = {};
  bt_property_t prop_name = {};
  BTIF_STORAGE_FILL_PROPERTY(&prop_name, BT_PROPERTY_BDNAME, sizeof(bt_bdname_t), &bdname);
  if (btif_storage_get_remote_device_property(&link_spec.addrt.bda, &prop_name) ==
      BT_STATUS_SUCCESS) {
...
static bool btif_hh_copy_hid_info(tBTA_HH_DEV_DSCP_INFO* dest, tBTA_HH_DEV_DSCP_INFO* src) {
...
static void hh_get_dscp_handler(tBTA_HH_DEV_DSCP_INFO& dscp_info) {
  ...
  const char* cached_name = get_cached_name(p_dev->link_spec);
  ...
  if (hh_add_device(p_dev->link_spec, p_dev->attr_mask, true)) {
    ...
    bt_status_t ret;
    btif_hh_copy_hid_info(&dscp_info, &dscp_info);
...
bt_status_t btif_hh_virtual_unplug(const tAclLinkSpec& link_spec) {
  ...
  // Remove the already added device
  if (com::android::bluetooth::flags::remove_input_device_on_vup() &&
      btif_hh_find_added_dev(link_spec)) {
