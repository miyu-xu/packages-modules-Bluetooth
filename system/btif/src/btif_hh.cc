...
static void hh_connect_complete(uint8_t handle, tAclLinkSpec& link_spec,
  ...
  HAL_CBACK(bt_hh_callbacks, connection_state_cb, &link_spec.addrt.bda,
            link_spec.addrt.type, link_spec.transport, state);
}
...
static void hh_open_handler(tBTA_HH_CONN& conn) {
  ...
  if (IS_FLAG_ENABLED(allow_switching_hid_and_hogp) &&
    ...
    if ((p_dev != NULL) && (p_dev->dev_status != BTHH_CONN_STATE_ACCEPTING &&
      ...
      log::warn("Reject Incoming HID Connection");
...
void btif_hh_load_added_dev(const tAclLinkSpec& link_spec,
...
void btif_hh_remove_device(const tAclLinkSpec& link_spec) {
  ...
  while ((p_dev = btif_hh_find_dev_by_link_spec(link_spec)) != NULL) {
    ...
    do_in_jni_thread(base::Bind(
        [](RawAddress bd_addr, tBLE_ADDR_TYPE dev_type,
...
bt_status_t btif_hh_virtual_unplug(const tAclLinkSpec* link_spec) {
  ...
  if ((p_dev != NULL) && (p_dev->dev_status == BTHH_CONN_STATE_CONNECTED) &&
    ...
    if ((btif_hh_cb.pending_link_spec.addrt.bda == link_spec->addrt.bda) &&
      ...
      do_in_jni_thread(base::Bind(
          [](RawAddress bd_addrcb, tBLE_ADDR_TYPE dev_type,
             tBT_TRANSPORT transport) {
...
bt_status_t btif_hh_connect(const tAclLinkSpec* link_spec) {
  ...
  do_in_jni_thread(base::Bind(
      [](RawAddress bd_addr, tBLE_ADDR_TYPE dev_type, tBT_TRANSPORT transport) {
        HAL_CBACK(bt_hh_callbacks, connection_state_cb, &bd_addr, dev_type,
...
void btif_hh_disconnect(tAclLinkSpec* link_spec) {
  ...
  const btif_hh_device_t* p_dev =
      btif_hh_find_connected_dev_by_link_spec(*link_spec);
  if (p_dev == nullptr) {
  ...
  switch (event) {
    ...
    case BTA_HH_CLOSE_EVT:
      ...
      if (p_dev != NULL) {
        HAL_CBACK(bt_hh_callbacks, connection_state_cb,
                  &(p_dev->link_spec.addrt.bda), p_dev->link_spec.addrt.type,
                  p_dev->link_spec.transport, BTHH_CONN_STATE_DISCONNECTING);
        ...
        HAL_CBACK(bt_hh_callbacks, connection_state_cb,
                  &(p_dev->link_spec.addrt.bda), p_dev->link_spec.addrt.type,
                  p_dev->link_spec.transport, p_dev->dev_status);
      } else {
    ...
    case BTA_HH_VC_UNPLUG_EVT:
      ...
      if (p_dev->link_spec.transport == BT_TRANSPORT_LE) {
        log::error(
            "BTA_HH_VC_UNPLUG_EVT: Error, not expected for LE transport...");
        return;
      }
  ...
  switch (event) {
    case BTIF_HH_CONNECT_REQ_EVT: {
      ...
      if (btif_hh_connect(p_link_spec) == BT_STATUS_SUCCESS) {
        HAL_CBACK(bt_hh_callbacks, connection_state_cb,
                  &(p_link_spec->addrt.bda), p_link_spec->addrt.type,
                  p_link_spec->transport, BTHH_CONN_STATE_CONNECTING);
      } else
        HAL_CBACK(bt_hh_callbacks, connection_state_cb,
                  &(p_link_spec->addrt.bda), p_link_spec->addrt.type,
                  p_link_spec->transport, BTHH_CONN_STATE_DISCONNECTED);
    } break;
    ...
    case BTIF_HH_DISCONNECT_REQ_EVT: {
      ...
      HAL_CBACK(bt_hh_callbacks, connection_state_cb, &(p_link_spec->addrt.bda),
                p_link_spec->addrt.type, p_link_spec->transport,
...
<<<<<<< PATCH SET (735e21 Added logical changes for transport selection)
                              tBT_TRANSPORT transport, bool reconnect_allowed) {
=======
                              tBT_TRANSPORT transport,
                              bool /* reconnect_allowed */) {
>>>>>>> BASE      (442151 HID host connection state changes and connection policy upda)
  CHECK_BTHH_INIT();
  log::verbose("BTHH");
  btif_hh_device_t* p_dev;
  tAclLinkSpec link_spec;
  ...
  if (IS_FLAG_ENABLED(allow_switching_hid_and_hogp) && !reconnect_allowed) {
    ...
    if (dev) {
      ...
      if (dev->dev_status == BTHH_CONN_STATE_ACCEPTING ||
          dev->dev_status == BTHH_CONN_STATE_CONNECTING) {
        dev->dev_status = BTHH_CONN_STATE_DISCONNECTED;
