...
static void toggle_os_keylockstates(int fd, int changedlockstates) {
  LOG_DEBUG("%s: fd = %d, changedlockstates = 0x%x", __func__, fd,
            changedlockstates);
  uint8_t hidreport[9];
  int reportIndex;
  ...
  LOG_VERBOSE("Writing hidreport #1 to os: "
              "%s:  %x %x %x",
  ...
  memset(hidreport, 0, 9);
  hidreport[0] = 1;
  LOG_VERBOSE("Writing hidreport #2 to os: "
              "%s:  %x %x %x",
              __func__, hidreport[0], hidreport[1], hidreport[2]);
  LOG_VERBOSE("%s:  %x %x %x", __func__, hidreport[3], hidreport[4],
              hidreport[5]);
...
static void update_keyboard_lockstates(btif_hh_device_t* p_dev) {
  ...
  LOG_DEBUG("%s: setting report on dev_handle %d to 0x%x", __func__,
...
static void sync_lockstate_on_connect(btif_hh_device_t* p_dev) {
  ...
  LOG_DEBUG("%s: Syncing keyboard lock states after "
            "reconnect...",
            __func__);
...
static bt_status_t set_info(RawAddress* bd_addr, bthh_hid_info_t hid_info) {
  ...
  LOG_VERBOSE("BTHH: %s: sub_class = 0x%02x, app_id = %d, vendor_id = 0x%04x, "
