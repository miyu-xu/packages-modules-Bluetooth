...
bt_status_t btsock_sco_init(thread_t* thread_) {
  ...
  if (!sco_sockets) return BT_STATUS_SOCKET_STREAM_ERROR;
...
bt_status_t btsock_sco_listen(int* sock_fd, int /* flags */) {
  ...
  if (!sco_socket) return BT_STATUS_SOCKET_STREAM_ERROR;
...
bt_status_t btsock_sco_connect(const RawAddress* bd_addr, int* sock_fd,
  ...
  return (sco_socket != NULL) ? BT_STATUS_SUCCESS : BT_STATUS_SOCKET_STREAM_ERROR;
}
