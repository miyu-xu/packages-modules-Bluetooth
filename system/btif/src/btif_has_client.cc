...
class HearingAaccessClientServiceInterfaceImpl : public HasClientInterface,
  ...
  void Connect(const RawAddress& addr) override {
    ...
    do_in_jni_thread(
        std::bind(&btif_storage_set_leaudio_has_acceptlist, addr, true));
  }
  ...
  void Disconnect(const RawAddress& addr) override {
    ...
    do_in_jni_thread(
        std::bind(&btif_storage_set_leaudio_has_acceptlist, addr, false));
  }
  ...
  void RemoveDevice(const RawAddress& addr) override {
    ...
    do_in_jni_thread(std::bind(&btif_storage_remove_leaudio_has, addr));
  }
  ...
  void OnConnectionState(ConnectionState state,
    ...
    do_in_jni_thread(std::bind(&HasClientCallbacks::OnConnectionState,
                               callbacks_, state, addr));
  }

  void OnDeviceAvailable(const RawAddress& addr, uint8_t features) override {
    do_in_jni_thread(std::bind(&HasClientCallbacks::OnDeviceAvailable,
                               callbacks_, addr, features));
  }

  void OnFeaturesUpdate(const RawAddress& addr, uint8_t features) override {
    do_in_jni_thread(std::bind(&HasClientCallbacks::OnFeaturesUpdate,
                               callbacks_, addr, features));
  }

  void OnActivePresetSelected(std::variant<RawAddress, int> addr_or_group_id,
                              uint8_t preset_index) override {
    do_in_jni_thread(std::bind(&HasClientCallbacks::OnActivePresetSelected,
  ...
  void OnActivePresetSelectError(std::variant<RawAddress, int> addr_or_group_id,
    ...
    do_in_jni_thread(std::bind(&HasClientCallbacks::OnActivePresetSelectError,
                               callbacks_, std::move(addr_or_group_id),
                               result_code));
  }
  ...
  void OnPresetInfo(std::variant<RawAddress, int> addr_or_group_id,
    ...
    do_in_jni_thread(std::bind(&HasClientCallbacks::OnPresetInfo, callbacks_,
                               std::move(addr_or_group_id), change_id,
                               std::move(detail_records)));
  }

  void OnPresetInfoError(std::variant<RawAddress, int> addr_or_group_id,
                         uint8_t preset_index, ErrorCode result_code) override {
    do_in_jni_thread(std::bind(&HasClientCallbacks::OnPresetInfoError,
                               callbacks_, std::move(addr_or_group_id),
  ...
  void OnSetPresetNameError(std::variant<RawAddress, int> addr_or_group_id,
                            ...
                            ErrorCode result_code) override {
    do_in_jni_thread(std::bind(&HasClientCallbacks::OnSetPresetNameError,
                               callbacks_, std::move(addr_or_group_id),
                               preset_index, result_code));
  }
