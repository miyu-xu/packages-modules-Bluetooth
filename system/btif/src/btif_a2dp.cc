...
void btif_a2dp_on_idle(const RawAddress& peer_addr, const A2dpType a2dpType) {
  LOG_VERBOSE(
      "Peer stream endpoint type:%s",
      peer_stream_endpoint_text(btif_av_get_peer_sep(a2dpType)).c_str());
  if (btif_av_get_peer_sep(a2dpType) == AVDT_TSEP_SNK) {
    btif_a2dp_source_on_idle();
  } else if (btif_av_get_peer_sep(a2dpType) == AVDT_TSEP_SRC) {
    btif_a2dp_sink_on_idle();
  }
}

bool btif_a2dp_on_started(const RawAddress& peer_addr,
                          tBTA_AV_START* p_av_start, const A2dpType a2dpType) {
...
void btif_a2dp_on_stopped(tBTA_AV_SUSPEND* p_av_suspend,
  ...
  if (btif_av_get_peer_sep(a2dpType) == AVDT_TSEP_SRC) {
