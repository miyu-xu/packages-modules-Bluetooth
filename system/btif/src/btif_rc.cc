...
void btif_rc_check_pending_cmd (const RawAddress& peer_address) {
  ...
  if (p_dev->launch_cmd_pending && p_dev->rc_connected) {
    if ((p_dev->launch_cmd_pending & RC_PENDING_ACT_REG_VOL) && btif_av_peer_is_sink(p_dev->rc_addr)) {
      if (bluetooth::avrcp::AvrcpService::Get() != nullptr) {
        bluetooth::avrcp::AvrcpService::Get()->RegisterVolChanged(peer_address);
      }
    }
    if ((p_dev->launch_cmd_pending & RC_PENDING_ACT_GET_CAP) && btif_av_peer_is_source(p_dev->rc_addr)) {
      p_dev->rc_features = p_dev->peer_tg_features;
      getcapabilities_cmd (AVRC_CAP_COMPANY_ID, p_dev);
    }
    if ((p_dev->launch_cmd_pending & RC_PENDING_ACT_REPORT_CONN) && btif_av_peer_is_source(p_dev->rc_addr)) {
     if (bt_rc_ctrl_callbacks != NULL) {
...
void handle_rc_passthrough_cmd(tBTA_AV_REMOTE_CMD* p_remote_cmd) {
   ...
   * this PLAY */
  if ((p_remote_cmd->rc_id == AVRC_ID_PLAY) && (!btif_active_av_is_connected())) {
...
static void handle_notification_response(tBTA_AV_META_MSG* pmeta_msg,
  ...
  if (android::sysprop::bluetooth::A2dp::a2dp_src_sink_coexist().value_or(false) &&
    ...
    BTIF_TRACE_ERROR("%s: legacy TG don't handle absolute volume change. leave it to new avrcp", __func__);
    return;
  }
