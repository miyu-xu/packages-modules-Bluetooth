...
static void initialize_transaction(btif_rc_device_cb_t* p_dev, int lbl) {
  ...
  if (lbl < MAX_TRANSACTIONS_PER_SESSION) {
    ...
    transaction_set->transaction[lbl].label = lbl;
    transaction_set->transaction[lbl].in_use = false;
    transaction_set->transaction[lbl].context.label = MAX_LABEL;
    transaction_set->transaction[lbl].context.opcode = AVRC_OP_INVALID;
    memset(&transaction_set->transaction[lbl].context.command, 0,
           sizeof(rc_command_context_t));
    transaction_set->transaction[lbl].timer = nullptr;
  }
}
