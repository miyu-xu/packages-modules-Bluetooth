...
static void handle_rc_ctrl_features_all(btif_rc_device_cb_t* p_dev) {
  if (btif_av_is_connected_addr(p_dev->rc_addr, A2dpType::kSink)) {
    ...
    log::info("{} is not connected, pending, add by lgf", p_dev->rc_addr);
...
static void btif_rc_ctrl_upstreams_rsp_cmd(uint8_t event, tAVRC_COMMAND* pavrc_cmd, uint8_t label,
  ...
  switch (event) {
    case AVRC_PDU_SET_ABSOLUTE_VOLUME:
      ...
      if (btif_av_src_sink_coexist_enabled() && btif_av_is_connected(A2dpType::kSource)) {
    ...
    case AVRC_PDU_REGISTER_NOTIFICATION:
       ...
       * if peer sink is streaming, the abs volume will changeed */
      if (btif_av_src_sink_coexist_enabled() && btif_av_is_connected(A2dpType::kSource)) {
        log::debug("already connected sink dev, ignore register abs");
        return;
      }
