...
void btif_rc_get_addr_by_handle(uint8_t handle, RawAddress& rc_addr) {
  LOG_DEBUG("%s: handle: 0x%x", __func__, handle);
  for (int idx = 0; idx < BTIF_RC_NUM_CONN; idx++) {
    if ((btif_rc_cb.rc_multi_cb[idx].rc_state !=
      ...
      LOG_DEBUG("%s: btif_rc_cb.rc_multi_cb[idx].rc_handle: 0x%x", __func__,
...
void fill_avrc_attr_entry(tAVRC_ATTR_ENTRY* attr_vals, int num_attrs,
  ...
  for (int attr_cnt = 0; attr_cnt < num_attrs; attr_cnt++) {
    ...
    LOG_DEBUG("%s: attr_id: 0x%x, charset_id: 0x%x, str_len: %d, str: %s",
              __func__, (unsigned int)attr_vals[attr_cnt].attr_id,
              attr_vals[attr_cnt].name.charset_id,
              attr_vals[attr_cnt].name.str_len, attr_vals[attr_cnt].name.p_str);
  }
}

void rc_cleanup_sent_cmd(void *p_data) { LOG_DEBUG("%s: ", __func__); }
...
void handle_rc_ctrl_features_all(btif_rc_device_cb_t* p_dev) {
  ...
  if (btif_av_is_connected_addr(p_dev->rc_addr)) {
    ...
    LOG_DEBUG("%s: %s is not connected, pending", __func__,
  ...
  if (bt_rc_ctrl_callbacks != NULL) {
    LOG_DEBUG("%s: Update rc features to CTRL: %d", __func__, rc_features);
...
void handle_rc_ctrl_psm(btif_rc_device_cb_t* p_dev) {
  ...
  LOG_DEBUG("%s: Update rc cover art psm to CTRL: %d", __func__, cover_art_psm);
  if (bt_rc_ctrl_callbacks != NULL) {
    do_in_jni_thread(FROM_HERE,
...
void handle_rc_features(btif_rc_device_cb_t* p_dev) {
  ...
  LOG_DEBUG("%s: AVDTP Source Active Peer Address: %s "
  ...
  LOG_DEBUG("%s: rc_features: 0x%x", __func__, rc_features);
  HAL_CBACK(bt_rc_callbacks, remote_features_cb, p_dev->rc_addr, rc_features);

  LOG_DEBUG("%s: Checking for feature flags in btif_rc_handler with label: %d",
...
void handle_rc_browse_connect(tBTA_AV_RC_BROWSE_OPEN* p_rc_br_open) {
  LOG_DEBUG("%s: rc_handle %d status %d", __func__, p_rc_br_open->rc_handle,
            p_rc_br_open->status);
  btif_rc_device_cb_t* p_dev =
  ...
  if (p_rc_br_open->status == BTA_AV_SUCCESS) {
    ...
    if (btif_av_src_sink_coexist_enabled()) {
      if (btif_av_peer_is_connected_source(p_dev->rc_addr)) {
        ...
        LOG_INFO("%s: pending rc browse connection event", __func__);
      }
    } else {
      do_in_jni_thread(FROM_HERE,
...
void handle_rc_connect(tBTA_AV_RC_OPEN* p_rc_open) {
  LOG_DEBUG("%s: rc_handle: %d", __func__, p_rc_open->rc_handle);
  ...
  if (p_dev->rc_connected) {
    LOG_ERROR("%s: Got RC OPEN in connected state, Connected RC: %d \
            and Current RC: %d",
              __func__, p_dev->rc_handle, p_rc_open->rc_handle);
    if (p_dev->rc_handle != p_rc_open->rc_handle &&
        p_dev->rc_addr != p_rc_open->peer_addr) {
      LOG_DEBUG("%s: Got RC connected for some other handle", __func__);
      BTA_AvCloseRc(p_rc_open->rc_handle);
      return;
    }
  }
  ...
  LOG_DEBUG("%s: handle_rc_connect in features=%#x, out features=%#x, "
  ...
  if (btif_av_src_sink_coexist_enabled() &&
    ...
    p_dev->launch_cmd_pending |= RC_PENDING_ACT_REPORT_CONN;
    LOG_INFO("%s: pending rc connection event", __func__);
    return;
  }
  if (bt_rc_ctrl_callbacks != NULL) {
    do_in_jni_thread(FROM_HERE,
...
void handle_rc_disconnect(tBTA_AV_RC_CLOSE* p_rc_close) {
  btif_rc_device_cb_t* p_dev = NULL;
  LOG_DEBUG("%s: rc_handle: %d", __func__, p_rc_close->rc_handle);
...
void handle_rc_passthrough_cmd(tBTA_AV_REMOTE_CMD* p_remote_cmd) {
  ...
  LOG_DEBUG("%s: p_remote_cmd->rc_id: %d", __func__, p_remote_cmd->rc_id);
  ...
  LOG_DEBUG("%s: rc_features: %d, cmd->rc_id: %d, pressed: %d", __func__,
...
void handle_rc_passthrough_rsp(tBTA_AV_REMOTE_RSP* p_remote_rsp) {
  ...
  LOG_DEBUG("%s: rc_id: %d state: %s", __func__, p_remote_rsp->rc_id, status);
...
void handle_rc_vendorunique_rsp(tBTA_AV_REMOTE_RSP* p_remote_rsp) {
  ...
  if (p_dev->rc_features & BTA_AV_FEAT_RCTG) {
    ...
    LOG_DEBUG("%s: vendor_id: %d status: %s", __func__, vendor_id, status);
...
void handle_rc_metamsg_cmd(tBTA_AV_META_MSG* pmeta_msg) {
  ...
  if (NULL == pmeta_msg) {
    LOG_INFO("%s: Exiting as pmeta_msg is NULL", __func__);
    return;
  }

  if (NULL == pmeta_msg->p_msg) {
    LOG_INFO("%s: Exiting as pmeta_msg->p_msg is NULL", __func__);
    return;
  }

  LOG_INFO("%s: pmeta_msg: opcode: %x, code: %x", __func__,
  ...
  status = AVRC_ParsCommand(pmeta_msg->p_msg, &avrc_command, scratch_buf,
                            sizeof(scratch_buf));
  LOG_DEBUG("%s: Received vendor command.code,PDU and label: %d, %d, %d",
  ...
  if (status != AVRC_STS_NO_ERROR) {
    ...
    LOG_INFO("%s: Passing received metamsg command to app. pdu: %s", __func__,
...
void btif_rc_handler(tBTA_AV_EVT event, tBTA_AV* p_data) {
  LOG_DEBUG("%s: event: %s", __func__, dump_rc_event(event));
  btif_rc_device_cb_t* p_dev = NULL;
  switch (event) {
    case BTA_AV_RC_OPEN_EVT: {
      LOG_DEBUG("%s: Peer_features: 0x%x Cover Art PSM: 0x%x", __func__,
    ...
    case BTA_AV_RC_BROWSE_CLOSE_EVT: {
      LOG_DEBUG("%s: BTA_AV_RC_BROWSE_CLOSE_EVT", __func__);
    } break;

    case BTA_AV_REMOTE_CMD_EVT: {
      if (bt_rc_callbacks != NULL) {
        LOG_DEBUG("%s: rc_id: 0x%x key_state: %d", __func__,
    ...
    case BTA_AV_REMOTE_RSP_EVT: {
      LOG_DEBUG("%s: RSP: rc_id: 0x%x key_state: %d", __func__,
    ...
    case BTA_AV_RC_FEAT_EVT: {
      LOG_DEBUG("%s: Peer_features: %x", __func__,
      ...
      LOG_DEBUG("%s peer_ct_features:0x%x, peer_tg_features=0x%x", __func__,
    ...
    case BTA_AV_RC_PSM_EVT: {
      LOG_DEBUG("%s: Peer cover art PSM: %x", __func__,
    ...
    case BTA_AV_META_MSG_EVT: {
      if (bt_rc_callbacks != NULL) {
        LOG_DEBUG("%s: BTA_AV_META_MSG_EVT code: %d label: %d", __func__,
                  p_data->meta_msg.code, p_data->meta_msg.label);
        LOG_DEBUG("%s: company_id: 0x%x len: %d handle: %d", __func__,
        ...
        LOG_DEBUG("%s BTA_AV_META_MSG_EVT code:%d label:%d opcode %d ctype %d",
    ...
    default:
      LOG_DEBUG("%s: Unhandled RC event : 0x%x", __func__, event);
  }
}
...
void btif_rc_check_handle_pending_play(const RawAddress& peer_addr,
  ...
  LOG_DEBUG("%s: bSendToApp: %d", __func__, bSendToApp);
  if (p_dev->rc_pending_play) {
    if (bSendToApp) {
      tBTA_AV_REMOTE_CMD remote_cmd;
      LOG_DEBUG("%s: Sending queued PLAYED event to app", __func__);
...
static void send_metamsg_rsp(btif_rc_device_cb_t* p_dev, int index,
  ...
  LOG_INFO("%s: rc_handle: %d, index: %d, label: %d, code: 0x%02x, pdu: %s",
  ...
  if ((pmetamsg_resp->rsp.pdu == AVRC_PDU_REGISTER_NOTIFICATION) &&
    ...
    LOG_DEBUG("%s: rc_handle: %d. event_id: 0x%02d bNotify: %u", __func__,
...
static void btif_rc_upstreams_evt(uint16_t event, tAVRC_COMMAND* pavrc_cmd,
                                  uint8_t ctype, uint8_t label,
                                  btif_rc_device_cb_t* p_dev) {
  LOG_INFO("%s: pdu: %s handle: 0x%x ctype: %x label: %x event ID: %x",
  ...
  switch (event) {
    ...
    case AVRC_PDU_INFORM_DISPLAY_CHARSET: {
      tAVRC_RESPONSE avrc_rsp;
      LOG_INFO("%s: AVRC_PDU_INFORM_DISPLAY_CHARSET", __func__);
      if (p_dev->rc_connected) {
    ...
    case AVRC_PDU_GET_FOLDER_ITEMS: {
      ...
      LOG_INFO("%s: AVRC_PDU_GET_FOLDER_ITEMS num_attr: %d, start_item [%d] \
                end_item [%d]",
    ...
    case AVRC_PDU_REQUEST_CONTINUATION_RSP: {
      LOG_INFO("%s() REQUEST CONTINUATION: target_pdu: 0x%02d", __func__,
    ...
    case AVRC_PDU_ABORT_CONTINUATION_RSP: {
      LOG_INFO("%s() ABORT CONTINUATION: target_pdu: 0x%02d", __func__,
    ...
    case AVRC_PDU_GET_ITEM_ATTRIBUTES: {
      ...
      LOG_DEBUG("%s: GET_ITEM_ATTRIBUTES: num_attr: %d", __func__, num_attr);
...
static void btif_rc_ctrl_upstreams_rsp_cmd(uint8_t event,
                                           ...
                                           uint8_t label,
                                           btif_rc_device_cb_t* p_dev) {
  LOG_DEBUG("%s: pdu: %s: handle: 0x%x", __func__, dump_rc_pdu(pavrc_cmd->pdu),
            p_dev->rc_handle);
  switch (event) {
...
static void btif_rc_upstreams_rsp_evt(uint16_t event,
                                      ...
                                      uint8_t label,
                                      btif_rc_device_cb_t* p_dev) {
  LOG_INFO("%s: pdu: %s: handle: 0x%x ctype: %x label: %x", __func__,
...
static bt_status_t get_play_status_rsp(const RawAddress& bd_addr,
  ...
  LOG_DEBUG("%s: song len %d song pos %d", __func__, song_len, song_pos);
...
static bt_status_t register_notification_rsp(
  ...
  tAVRC_RESPONSE avrc_rsp;
  LOG_INFO("%s: event_id: %s", __func__,
  ...
  for (int idx = 0; idx < BTIF_RC_NUM_CONN; idx++) {
    ...
    LOG_DEBUG("%s: Avrcp Event id is registered: event_id: %x handle: 0x%x",
              __func__, event_id, btif_rc_cb.rc_multi_cb[idx].rc_handle);
...
static bt_status_t get_folder_items_list_rsp(const RawAddress& bd_addr,
  ...
  LOG_DEBUG("%s: uid_counter %d num_items %d", __func__, uid_counter,
            num_items);
  CHECK_RC_CONNECTED(p_dev);
  ...
  if (avrc_rsp.get_items.status != AVRC_STS_NO_ERROR) {
    ...
    for (item_cnt = 0; item_cnt < num_items; item_cnt++) {
      ...
      LOG_DEBUG("%s: item_cnt: %d len: %d", __func__, item_cnt, len_before);
      status = AVRC_BldResponse(p_dev->rc_handle, &avrc_rsp, &p_msg);
      LOG_DEBUG("%s: Build rsp status: %d len: %d", __func__, status,
                (p_msg ? p_msg->len : 0));
...
static bt_status_t set_browsed_player_rsp(const RawAddress& bd_addr,
  ...
  LOG_DEBUG("%s: rsp_status: 0x%02X avrc_rsp.br_player.status: 0x%02X",
  ...
  if (AVRC_STS_NO_ERROR == avrc_rsp.get_items.status) {
    ...
    LOG_DEBUG("%s: folder_depth: 0x%02X num_items: %d", __func__, folder_depth,
              num_items);
    ...
    if (folder_depth > 0) {
      ...
      for (item_cnt = 0; item_cnt < folder_depth; item_cnt++) {
        LOG_DEBUG("%s: iteration: %d", __func__, item_cnt);
...
static bt_status_t set_volume(uint8_t volume) {
  LOG_DEBUG("%s: volume: %d", __func__, volume);
  ...
  for (int idx = 0; idx < BTIF_RC_NUM_CONN; idx++) {
    ...
    LOG_DEBUG("%s: Peer supports absolute volume. newVolume: %d", __func__,
              volume);
    ...
    LOG_DEBUG("%s: msgreq being sent out with label: %d", __func__,
              p_transaction->label);
...
static void register_volumechange(btif_rc_device_cb_t* p_dev) {
  ...
  if (MAX_LABEL == p_dev->rc_vol_label) {
    ...
    if (NULL != p_transaction) {
      LOG_DEBUG("%s: already in progress for label: %d", __func__,
                p_dev->rc_vol_label);
      return;
    }
    status = get_transaction(p_dev, context, &p_transaction);
  ...
  LOG_DEBUG("%s: label: %d", __func__, p_dev->rc_vol_label);
  ...
  if (AVRC_STS_NO_ERROR == BldResp && p_msg) {
    BTA_AvMetaCmd(p_dev->rc_handle, p_transaction->label, AVRC_CMD_NOTIF,
                  p_msg);
    LOG_DEBUG("%s: BTA_AvMetaCmd called", __func__);
  } else {
...
static void handle_rc_metamsg_rsp(tBTA_AV_META_MSG* pmeta_msg,
  ...
  if (AVRC_OP_VENDOR == pmeta_msg->p_msg->hdr.opcode &&
    ...
    if (AVRC_PDU_REGISTER_NOTIFICATION == avrc_response.rsp.pdu &&
        ...
        (AVRC_RSP_REJ == pmeta_msg->code ||
         AVRC_RSP_NOT_IMPL == pmeta_msg->code)) {
      LOG_DEBUG("%s remove AbsoluteVolume feature flag.", __func__);
      p_dev->rc_features &= ~BTA_AV_FEAT_ADV_CTRL;
      handle_rc_features(p_dev);
      return;
    }
  ...
  LOG_INFO("%s: Passing received metamsg response to app. pdu: %s", __func__,
...
static bt_status_t build_and_send_vendor_cmd(tAVRC_COMMAND* avrc_cmd,
  ...
  if (status == AVRC_STS_NO_ERROR && p_msg != NULL) {
    ...
    LOG_DEBUG("%s: %s msgreq being sent out with label: %d", __func__,
...
static bt_status_t build_and_send_browsing_cmd(tAVRC_COMMAND* avrc_cmd,
  ...
  LOG_DEBUG("%s: Send pdu_id=%s, label=%d", __func__,
            dump_rc_pdu(avrc_cmd->pdu), p_transaction->label);
  BTA_AvMetaCmd(p_dev->rc_handle, p_transaction->label, AVRC_CMD_CTRL, p_msg);
  start_transaction_timer(p_dev, p_transaction->label, BTIF_RC_TIMEOUT_MS);
  return BT_STATUS_SUCCESS;
...
static void handle_get_capability_response(tBTA_AV_META_MSG* pmeta_msg,
  ...
  if (p_rsp->capability_id == AVRC_CAP_EVENTS_SUPPORTED) {
    ...
    LOG_INFO("%s: AVRC_CAP_COMPANY_ID: ", __func__);
    for (xx = 0; xx < p_rsp->count; xx++) {
      LOG_INFO("%s: company_id: %d", __func__, p_rsp->param.company_id[xx]);
    }
  }
}
...
static void handle_notification_response(tBTA_AV_META_MSG* pmeta_msg,
  ...
  if (pmeta_msg->code == AVRC_RSP_INTERIM) {
    ...
    LOG_DEBUG("%s: Interim response: 0x%2X ", __func__, p_rsp->event_id);
    switch (p_rsp->event_id) {
      ...
      case AVRC_EVT_AVAL_PLAYERS_CHANGE:
        LOG_DEBUG("%s: AVRC_EVT_AVAL_PLAYERS_CHANGE", __func__);
        do_in_jni_thread(
            FROM_HERE,
    ...
    if (p_event == NULL && !p_dev->rc_app_settings.query_started) {
      ...
      if (p_dev->rc_features & BTA_AV_FEAT_APP_SETTING) {
        ...
        LOG_DEBUG("%s: App setting not supported, complete procedure",
                  __func__);
    ...
    LOG_DEBUG("%s: Notification completed: 0x%2X ", __func__, p_rsp->event_id);
...
static void handle_get_folder_items_response(tBTA_AV_META_MSG* pmeta_msg,
  ...
  if (p_rsp->status == AVRC_STS_NO_ERROR) {
    ...
    for (uint8_t i = 0; i < item_count; i++) {
      ...
      LOG_DEBUG("%s folder item type %d", __func__, avrc_item->item_type);
      switch (avrc_item->item_type) {
        case AVRC_ITEM_MEDIA:
          LOG_DEBUG("%s setting type to %d", __func__, BTRC_ITEM_MEDIA);
        ...
        case AVRC_ITEM_FOLDER:
          LOG_DEBUG("%s setting type to BTRC_ITEM_FOLDER", __func__);
          get_folder_item_type_folder(avrc_item, btrc_item);
          break;

        case AVRC_ITEM_PLAYER:
          LOG_DEBUG("%s setting type to BTRC_ITEM_PLAYER", __func__);
    ...
    LOG_DEBUG("%s get_folder_items_cb sent to JNI thread", __func__);
  } else {
...
void get_folder_item_type_media(const tAVRC_ITEM* avrc_item,
  ...
  /* Copy the name */
  LOG_DEBUG("%s max len %d str len %d", __func__, BTRC_MAX_ATTR_STR_LEN,
  ...
  for (int i = 0; i < avrc_item_media->attr_count; i++) {
    ...
    LOG_DEBUG("%s media attr id 0x%x", __func__, avrc_attr_pair->attr_id);
...
static void handle_avk_rc_metamsg_rsp(tBTA_AV_META_MSG* pmeta_msg) {
  ...
  LOG_DEBUG("%s: opcode: %d rsp_code: %d  ", __func__,
            pmeta_msg->p_msg->hdr.opcode, pmeta_msg->code);
  ...
  if ((AVRC_OP_VENDOR == pmeta_msg->p_msg->hdr.opcode) &&
    ...
    LOG_DEBUG("%s parse status %d pdu = %d rsp_status = %d", __func__, status,
    ...
    LOG_DEBUG("%s AVRC_OP_BROWSE pdu %d", __func__, avrc_response.pdu);
    ...
    LOG_DEBUG("%s: Invalid Vendor Command code: %d len: %d. Not processing it.",
              __func__, pmeta_msg->code, pmeta_msg->len);
    return;
  }
  LOG_DEBUG("%s: release transaction %d", __func__, pmeta_msg->label);
  release_transaction(p_dev, pmeta_msg->label);
}
...
static void handle_avk_rc_metamsg_cmd(tBTA_AV_META_MSG* pmeta_msg) {
  ...
  if ((AVRC_OP_VENDOR == pmeta_msg->p_msg->hdr.opcode) &&
    ...
    LOG_DEBUG("%s Received vendor command.code %d, PDU %d label %d", __func__,
    ...
    if (status != AVRC_STS_NO_ERROR) {
      ...
      if (avrc_cmd.pdu == AVRC_PDU_REGISTER_NOTIFICATION) {
        ...
        LOG_INFO("%s: Register notification event_id: %s", __func__,
                 dump_rc_notification_event_id(event_id));
      } else if (avrc_cmd.pdu == AVRC_PDU_SET_ABSOLUTE_VOLUME) {
        LOG_INFO("%s: Abs Volume Cmd Recvd", __func__);
      }
...
static bt_status_t getcapabilities_cmd(uint8_t cap_id,
                                       btif_rc_device_cb_t* p_dev) {
  LOG_DEBUG("%s: cap_id: %d", __func__, cap_id);
  CHECK_RC_CONNECTED(p_dev);
...
static bt_status_t list_player_app_setting_value_cmd(
    uint8_t attrib_id, btif_rc_device_cb_t* p_dev) {
  LOG_DEBUG("%s: attrib_id: %d", __func__, attrib_id);
  CHECK_RC_CONNECTED(p_dev);
...
static bt_status_t get_player_app_setting_cmd(uint8_t num_attrib,
                                              uint8_t* attrib_ids,
                                              btif_rc_device_cb_t* p_dev) {
  LOG_DEBUG("%s: num_attrib: %d", __func__, num_attrib);
  CHECK_RC_CONNECTED(p_dev);
...
static bt_status_t get_now_playing_list_cmd(const RawAddress& bd_addr,
                                            uint32_t start_item,
                                            uint32_t end_item) {
  LOG_DEBUG("%s start, end: (%d, %d)", __func__, start_item, end_item);
...
static bt_status_t change_folder_path_cmd(const RawAddress& bd_addr,
  ...
  LOG_DEBUG("%s: direction %d", __func__, direction);
  btif_rc_device_cb_t* p_dev = btif_rc_get_device_by_bda(bd_addr);
  CHECK_RC_CONNECTED(p_dev);
...
static bt_status_t play_item_cmd(const RawAddress& bd_addr, uint8_t scope,
  ...
  LOG_DEBUG("%s: scope %d uid_counter %d", __func__, scope, uid_counter);
  btif_rc_device_cb_t* p_dev = btif_rc_get_device_by_bda(bd_addr);
  CHECK_RC_CONNECTED(p_dev);
  CHECK_BR_CONNECTED(p_dev);
...
static bt_status_t get_player_app_setting_attr_text_cmd(
    uint8_t* attrs, uint8_t num_attrs, btif_rc_device_cb_t* p_dev) {
  LOG_DEBUG("%s: num attrs: %d", __func__, num_attrs);
  CHECK_RC_CONNECTED(p_dev);
...
static bt_status_t get_player_app_setting_value_text_cmd(
    uint8_t* vals, uint8_t num_vals, btif_rc_device_cb_t* p_dev) {
  LOG_DEBUG("%s: num_vals: %d", __func__, num_vals);
  CHECK_RC_CONNECTED(p_dev);
...
static bt_status_t register_notification_cmd(uint8_t event_id,
                                             uint32_t event_value,
                                             btif_rc_device_cb_t* p_dev) {
  LOG_DEBUG("%s: event_id: %d event_value %d", __func__, event_id, event_value);
  CHECK_RC_CONNECTED(p_dev);
...
static bt_status_t get_metadata_attribute_cmd(uint8_t num_attribute,
                                              const uint32_t* p_attr_ids,
                                              btif_rc_device_cb_t* p_dev) {
  LOG_DEBUG("%s: num_attribute: %d attribute_id: %d", __func__, num_attribute,
            p_attr_ids[0]);
...
static bt_status_t set_volume_rsp(const RawAddress& bd_addr, uint8_t abs_vol,
  ...
  LOG_DEBUG("%s: abs_vol: %d", __func__, abs_vol);
...
static bt_status_t volume_change_notification_rsp(
  ...
  tAVRC_RESPONSE avrc_rsp;
  BT_HDR* p_msg = NULL;
  LOG_DEBUG("%s: rsp_type: %d abs_vol: %d", __func__, rsp_type, abs_vol);
  ...
  status = AVRC_BldResponse(p_dev->rc_handle, &avrc_rsp, &p_msg);
  if (status == AVRC_STS_NO_ERROR) {
    LOG_DEBUG("%s: msgreq being sent out with label: %d", __func__, label);
    uint8_t* data_start = (uint8_t*)(p_msg + 1) + p_msg->offset;
...
static bt_status_t send_groupnavigation_cmd(const RawAddress& bd_addr,
  ...
  tAVRC_STS status = BT_STATUS_UNSUPPORTED;
  rc_transaction_t* p_transaction = NULL;
  LOG_DEBUG("%s: key-code: %d, key-state: %d", __func__, key_code, key_state);
  btif_rc_device_cb_t* p_dev = btif_rc_get_device_by_bda(bd_addr);
  ...
  if (p_dev->rc_features & BTA_AV_FEAT_RCTG) {
    ...
    if ((BT_STATUS_SUCCESS == tran_status) && (NULL != p_transaction)) {
      ...
      *(start)++ = 0;
      ...
      status = BT_STATUS_SUCCESS;
      start_transaction_timer(p_dev, p_transaction->label, BTIF_RC_TIMEOUT_MS);
      LOG_DEBUG("%s: Send command, key-code=%d, key-state=%d, label=%d",
    ...
    status = BT_STATUS_FAIL;
    LOG_DEBUG("%s: feature not supported", __func__);
  }
  return (bt_status_t)status;
}
  ...
  for (uint8_t i = 0; i < MAX_TRANSACTIONS_PER_SESSION; i++) {
    if (!transaction_set->transaction[i].in_use) {
      ...
      *ptransaction = &(transaction_set->transaction[i]);
      LOG_DEBUG("%s: Assigned transaction=%s", __func__,
...
void release_transaction(btif_rc_device_cb_t* p_dev, uint8_t lbl) {
  LOG_DEBUG("%s: p_dev=%s, label=%d", __func__,
...
static void btif_rc_transaction_timeout_handler(UNUSED_ATTR uint16_t event,
  ...
  switch (p_context->opcode) {
    ...
    case AVRC_OP_PASS_THRU:
      ...
      break;
    default:
      LOG_WARN("%s: received timeout for unknown opcode=", __func__,
               p_context->opcode);
      return;
  }
