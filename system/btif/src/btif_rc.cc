...
void handle_rc_ctrl_features(btif_rc_device_cb_t* p_dev) {
  ...
  do_in_jni_thread(FROM_HERE,
                   base::BindRepeating(bt_rc_ctrl_callbacks->getrcfeatures_cb,
...
void handle_rc_ctrl_psm(btif_rc_device_cb_t* p_dev) {
  ...
  BTIF_TRACE_DEBUG("%s: Update rc cover art psm to CTRL: %d", __func__,
      cover_art_psm);
  do_in_jni_thread(
      FROM_HERE, base::BindRepeating(bt_rc_ctrl_callbacks->get_cover_art_psm_cb,
...
void handle_rc_passthrough_rsp(tBTA_AV_REMOTE_RSP* p_remote_rsp) {
  ...
  if (bt_rc_ctrl_callbacks != NULL) {
    do_in_jni_thread(
        FROM_HERE, base::BindRepeating(bt_rc_ctrl_callbacks->passthrough_rsp_cb,
...
void handle_rc_vendorunique_rsp(tBTA_AV_REMOTE_RSP* p_remote_rsp) {
  ...
  if (p_dev->rc_features & BTA_AV_FEAT_RCTG) {
    ...
    do_in_jni_thread(
        FROM_HERE,
        base::BindRepeating(bt_rc_ctrl_callbacks->groupnavigation_rsp_cb,
                            vendor_id, key_state));
  } else {
...
static void btif_rc_ctrl_upstreams_rsp_cmd(uint8_t event,
  ...
  switch (event) {
    case AVRC_PDU_SET_ABSOLUTE_VOLUME:
      do_in_jni_thread(
          FROM_HERE,
          base::BindRepeating(bt_rc_ctrl_callbacks->setabsvol_cmd_cb,
                              p_dev->rc_addr, pavrc_cmd->volume.volume, label));
      break;
    case AVRC_PDU_REGISTER_NOTIFICATION:
      if (pavrc_cmd->reg_notif.event_id == AVRC_EVT_VOLUME_CHANGE) {
        do_in_jni_thread(
            FROM_HERE, base::BindRepeating(
                           bt_rc_ctrl_callbacks->registernotification_absvol_cb,
                           p_dev->rc_addr, label));
      }
      break;
  }
  ...
  if (pmeta_msg->code == AVRC_RSP_INTERIM) {
    ...
    switch (p_rsp->event_id) {
      case AVRC_EVT_PLAY_STATUS_CHANGE:
        ...
        do_in_jni_thread(
            FROM_HERE,
            base::BindRepeating(bt_rc_ctrl_callbacks->play_status_changed_cb,
                                p_dev->rc_addr,
      ...
      case AVRC_EVT_AVAL_PLAYERS_CHANGE:
        ...
        do_in_jni_thread(FROM_HERE,
                         base::BindRepeating(
                             bt_rc_ctrl_callbacks->available_player_changed_cb,
                             p_dev->rc_addr));
        break;
      ...
      case AVRC_EVT_PLAY_POS_CHANGED:
        do_in_jni_thread(
            FROM_HERE,
            base::BindRepeating(bt_rc_ctrl_callbacks->play_position_changed_cb,
...
static void handle_app_val_response(tBTA_AV_META_MSG* pmeta_msg,
  ...
  if (p_app_settings->attr_index < p_app_settings->num_attrs) {
    ...
    if (attr_index < p_app_settings->num_attrs) {
      ...
      do_in_jni_thread(
          FROM_HERE,
          base::BindRepeating(bt_rc_ctrl_callbacks->playerapplicationsetting_cb,
...
static void handle_app_attr_val_txt_response(
  ...
  if (p_app_settings->ext_val_index < p_app_settings->num_ext_attrs) {
    ...
    do_in_jni_thread(
        FROM_HERE,
        base::BindRepeating(cleanup_app_attr_val_txt_response, p_app_settings));
...
static void handle_set_app_attr_val_response(tBTA_AV_META_MSG* pmeta_msg,
  ...
  do_in_jni_thread(
      FROM_HERE,
      base::BindRepeating(bt_rc_ctrl_callbacks->setplayerappsetting_rsp_cb,
...
static void handle_get_metadata_attr_response(tBTA_AV_META_MSG* pmeta_msg,
  ...
  if (p_rsp->status == AVRC_STS_NO_ERROR) {
    ...
    do_in_jni_thread(FROM_HERE, base::BindRepeating(
                                    bt_rc_ctrl_callbacks->track_changed_cb,
                                    p_dev->rc_addr, p_rsp->num_attrs, p_attr));
    do_in_jni_thread(FROM_HERE, base::BindRepeating(osi_free, p_attr));
...
static void handle_set_addressed_player_response(tBTA_AV_META_MSG* pmeta_msg,
  ...
  if (p_rsp->status == AVRC_STS_NO_ERROR) {
    do_in_jni_thread(
        FROM_HERE,
        base::BindRepeating(bt_rc_ctrl_callbacks->set_addressed_player_cb,
...
static void handle_get_folder_items_response(tBTA_AV_META_MSG* pmeta_msg,
  ...
  if (p_rsp->status == AVRC_STS_NO_ERROR) {
    ...
    do_in_jni_thread(FROM_HERE,
                     base::BindRepeating(
                         bt_rc_ctrl_callbacks->get_folder_items_cb,
    ...
    do_in_jni_thread(FROM_HERE, base::BindRepeating(cleanup_btrc_folder_items,
                                                    btrc_items, item_count));
...
static void handle_change_path_response(tBTA_AV_META_MSG* pmeta_msg,
  ...
  if (p_rsp->status == AVRC_STS_NO_ERROR) {
    do_in_jni_thread(FROM_HERE, base::BindRepeating(
                                    bt_rc_ctrl_callbacks->change_folder_path_cb,
...
static void handle_set_browsed_player_response(tBTA_AV_META_MSG* pmeta_msg,
  ...
  if (p_rsp->status == AVRC_STS_NO_ERROR) {
    do_in_jni_thread(
        FROM_HERE, base::BindRepeating(
                       bt_rc_ctrl_callbacks->set_browsed_player_cb,
