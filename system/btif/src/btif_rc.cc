...
typedef enum {
  RC_CHANNEL_CONTROL,   // Browse channel
  RC_CHANNEL_BROWSE,  // Control channel
  RC_CHANNEL_TYPE_MAX
} rc_channel_type;
...
typedef struct {
  ...
  rc_transaction_set_t transaction_set[RC_CHANNEL_TYPE_MAX];
...
static bt_status_t get_transaction(btif_rc_device_cb_t* p_dev, rc_transaction_context_t& context,
                                   rc_transaction_t** ptransaction, rc_channel_type channel_type);
static void start_transaction_timer(btif_rc_device_cb_t* p_dev, uint8_t label, uint64_t timeout_ms, rc_channel_type channel_type);
static void btif_rc_transaction_timer_timeout(void* data);
static void release_transaction(btif_rc_device_cb_t* p_dev, uint8_t label, rc_channel_type channel_type);
static std::string dump_transaction(const rc_transaction_t* const transaction);
static rc_transaction_t* get_transaction_by_lbl(btif_rc_device_cb_t* p_dev, uint8_t label, rc_channel_type channel_type)
...
static void handle_rc_passthrough_rsp(tBTA_AV_REMOTE_RSP* p_remote_rsp) {
  ...
  release_transaction(p_dev, p_remote_rsp->label, RC_CHANNEL_CONTROL);
  if (bt_rc_ctrl_callbacks != NULL) {
...
static bt_status_t build_and_send_vendor_cmd(tAVRC_COMMAND* avrc_cmd, tBTA_AV_CODE cmd_code,
  ...
  bt_status_t tran_status = get_transaction(p_dev, context, &p_transaction, RC_CHANNEL_CONTROL);
  ...
  if (status == AVRC_STS_NO_ERROR && p_msg != NULL) {
    ...
    start_transaction_timer(p_dev, p_transaction->label, BTIF_RC_TIMEOUT_MS, RC_CHANNEL_CONTROL);
  } else {
    log::error("failed to build command. status: 0x{:02x}", status);
    release_transaction(p_dev, p_transaction->label, RC_CHANNEL_CONTROL);
  }
  osi_free(p_msg);
...
static bt_status_t build_and_send_browsing_cmd(tAVRC_COMMAND* avrc_cmd,
  ...
  bt_status_t tran_status = get_transaction(p_dev, context, &p_transaction, RC_CHANNEL_BROWSE);
  ...
  if (status != AVRC_STS_NO_ERROR) {
    ...
    release_transaction(p_dev, p_transaction->label, RC_CHANNEL_BROWSE);
    return BT_STATUS_FAIL;
  }
  ...
  start_transaction_timer(p_dev, p_transaction->label, BTIF_RC_TIMEOUT_MS, RC_CHANNEL_BROWSE);
 ...
 *
 **************************************************************************/
static void clear_cmd_timeout(btif_rc_device_cb_t* p_dev, uint8_t label, rc_channel_type channel_type) {
  rc_transaction_t* p_txn;

  p_txn = get_transaction_by_lbl(p_dev, label, channel_type);
  if (p_txn == NULL) {
...
static void handle_avk_rc_metamsg_rsp(tBTA_AV_META_MSG* pmeta_msg) {
    ...
    switch (avrc_response.pdu) {
      case AVRC_PDU_REGISTER_NOTIFICATION:
        ...
        if (pmeta_msg->code == AVRC_RSP_INTERIM) {
          ...
          clear_cmd_timeout(p_dev, pmeta_msg->label, RC_CHANNEL_CONTROL);
          return;
        }
        break;
  ...
  if (AVRC_OP_BROWSE == pmeta_msg->p_msg->hdr.opcode)
    release_transaction(p_dev, pmeta_msg->label, RC_CHANNEL_BROWSE);
  else
    release_transaction(p_dev, pmeta_msg->label, RC_CHANNEL_CONTROL);
}
 ...
 *      Returns          void
 ******************************************************************************/
static void initialize_transaction(btif_rc_device_cb_t* p_dev, uint8_t lbl, rc_channel_type channel_type) {
  if (p_dev == nullptr) {
    return;
  }
  rc_transaction_set_t* transaction_set = &(p_dev->transaction_set[channel_type]);
  ...
  if (lbl < MAX_TRANSACTIONS_PER_SESSION) {
    ...
    if (alarm_is_scheduled(transaction_set->transaction[lbl].timer)) {
      ...
      clear_cmd_timeout(p_dev, lbl, channel_type);
    }
...
void init_all_transactions(btif_rc_device_cb_t* p_dev) {
  ...
  for (uint8_t j = 0;j < RC_CHANNEL_TYPE_MAX;j++) {
 ...
 * Returns          bt_status_t
 ******************************************************************************/
rc_transaction_t* get_transaction_by_lbl(btif_rc_device_cb_t* p_dev, uint8_t lbl, rc_channel_type channel_type) {
  if (p_dev == nullptr) {
    return nullptr;
  }
    ...
    if (!transaction_set->transaction[i].in_use) {
      ...
      *ptransaction = &(transaction_set->transaction[i]);
      log::verbose("Assigned transaction, dev={}, type={}, transaction={}", p_dev->rc_addr,
                   (uint8_t)channel_type, dump_transaction(*ptransaction));
      return BT_STATUS_SUCCESS;
    }
  }
...
static void start_transaction_timer(btif_rc_device_cb_t* p_dev, uint8_t label,
                                    uint64_t timeout_ms, rc_channel_type channel_type) {
  rc_transaction_t* transaction = get_transaction_by_lbl(p_dev, label, channel_type);
 ...
 * Returns          bt_status_t
 ******************************************************************************/
void release_transaction(btif_rc_device_cb_t* p_dev, uint8_t lbl, rc_channel_type channel_type) {
  if (p_dev == nullptr) {
  ...
  rc_transaction_t* transaction = get_transaction_by_lbl(p_dev, lbl, channel_type);

  /* If the transaction is in use... */
  if (transaction != NULL) {
    log::verbose("Released transaction, dev={}, label={}", p_dev->rc_addr, lbl);
    initialize_transaction(p_dev, lbl, channel_type);
  } else {
...
static void btif_rc_transaction_timeout_handler(uint16_t /* event */, char* data) {
  ...
  if (p_context->opcode == AVRC_OP_BROWSE)
    release_transaction(p_dev, label, RC_CHANNEL_BROWSE);
  else
    release_transaction(p_dev, label, RC_CHANNEL_CONTROL);
}
...
void btif_debug_rc_dump(int fd) {
  ...
  for (int i = 0; i < BTIF_RC_NUM_CONN; ++i) {
    btif_rc_device_cb_t* device_cb = &btif_rc_cb.rc_multi_cb[i];
    if (device_cb->rc_state != BTRC_CONNECTION_STATE_DISCONNECTED) {
      dprintf(fd, "    %s:\n", device_cb->rc_addr.ToRedactedStringForLogging().c_str());

      rc_transaction_set_t* transaction_set = &(device_cb->transaction_set[RC_CHANNEL_CONTROL]);
      std::unique_lock<std::recursive_mutex> lock(transaction_set->label_lock);
      ...
      transaction_set = &(device_cb->transaction_set[RC_CHANNEL_BROWSE]);
