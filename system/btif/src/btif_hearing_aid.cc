...
class HearingAidInterfaceImpl
  ...
  void OnConnectionState(ConnectionState state,
    ...
    do_in_jni_thread(FROM_HERE,
                     base::BindOnce(&HearingAidCallbacks::OnConnectionState,
  ...
  void OnDeviceAvailable(uint8_t capabilities, uint64_t hiSyncId,
    ...
    do_in_jni_thread(
        FROM_HERE,
        base::BindOnce(&HearingAidCallbacks::OnDeviceAvailable,
                       Unretained(callbacks), capabilities, hiSyncId, address));
  }
  ...
  void Disconnect(const RawAddress& address) override {
    ...
    do_in_main_thread(FROM_HERE,
                      base::BindOnce(&HearingAid::Disconnect, address));
    do_in_jni_thread(FROM_HERE,
                     base::BindOnce(&btif_storage_set_hearing_aid_acceptlist,
                                    address, false));
  }
  ...
  void AddToAcceptlist(const RawAddress& address) override {
    ...
    do_in_main_thread(FROM_HERE,
                      base::BindOnce(&HearingAid::AddToAcceptlist, address));
    do_in_jni_thread(FROM_HERE,
                     base::BindOnce(&btif_storage_set_hearing_aid_acceptlist,
                                    address, true));
  }
  ...
  void SetVolume(int8_t volume) override {
    ...
    do_in_main_thread(FROM_HERE,
                      base::BindOnce(&HearingAid::SetVolume, volume));
  }
