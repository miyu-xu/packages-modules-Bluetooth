...
static void btif_core_storage_adapter_write(bt_property_t* prop) {
  LOG_DEBUG("type: %d, len %d, 0x%p", prop->type, prop->len, prop->val);
...
void btif_set_adapter_property(bt_property_t* property) {
  LOG_DEBUG("btif_set_adapter_property type: %d, len %d, 0x%p", property->type,
  ...
  switch (property->type) {
    case BT_PROPERTY_BDNAME: {
      ...
      LOG_DEBUG("set property name : %s", (char *)bd_name);
    ...
    case BT_PROPERTY_ADAPTER_SCAN_MODE: {
      ...
      LOG_DEBUG("set property scan mode : %x", mode);
