...
void btif_enable_bluetooth_evt() {
  ...
  if (!com::android::bluetooth::flags::load_did_config_from_sysprops()) {
    ...
    tSDP_DI_RECORD record = {
      .vendor = uint16_t(GET_SYSPROP(DeviceIDProperties, vendor_id, LMP_COMPID_GOOGLE)),
      .vendor_id_source = uint16_t(
              GET_SYSPROP(DeviceIDProperties, vendor_id_source, DI_VENDOR_ID_SOURCE_BTSIG)),
...
static bt_status_t btif_in_get_adapter_properties(void) {
  ...
  /* SCAN_MODE */
  BTIF_STORAGE_FILL_PROPERTY(&properties[num_props], BT_PROPERTY_ADAPTER_SCAN_MODE, sizeof(mode),
                             &mode);
  btif_storage_get_adapter_property(&properties[num_props]);
  num_props++;
