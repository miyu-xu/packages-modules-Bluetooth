...
class LeAudioClientInterfaceImpl : public LeAudioClientInterface,
  ...
  void OnHealthBaseRecomendationAction(
      const RawAddress& address,
      bluetooth::le_audio::LeAudioHealthBasedAction action) override {
    do_in_jni_thread(
        FROM_HERE,
        Bind(&LeAudioClientCallbacks::OnHealthBaseRecomendationAction,
             Unretained(callbacks), address, action));
  }

  void OnHealthBaseGroupRecomendationAction(
      int group_id,
      bluetooth::le_audio::LeAudioHealthBasedAction action) override {
    do_in_jni_thread(
        FROM_HERE,
        Bind(&LeAudioClientCallbacks::OnHealthBaseGroupRecomendationAction,
