...
class LeAudioClientInterfaceImpl : public LeAudioClientInterface,
  ...
  void OnHealthBaseRecommendationAction(
      const RawAddress& address,
      bluetooth::le_audio::LeAudioHealthBasedAction action) override {
    do_in_jni_thread(
        FROM_HERE,
        Bind(&LeAudioClientCallbacks::OnHealthBaseRecommendationAction,
             Unretained(callbacks), address, action));
  }

  void OnHealthBaseGroupRecommendationAction(
      int group_id,
      bluetooth::le_audio::LeAudioHealthBasedAction action) override {
    do_in_jni_thread(
        FROM_HERE,
        Bind(&LeAudioClientCallbacks::OnHealthBaseGroupRecommendationAction,
