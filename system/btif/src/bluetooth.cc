...
struct MSBCCodec : bluetooth::core::CodecInterface {
  ...
  bool encodePacket(int16_t* input, uint8_t* output) {
    hfp_msbc_encode_frames(input, output);
  }

  bool decodePacket(const uint8_t* i_buf, int16_t* o_buf, size_t out_len) {
    hfp_msbc_decoder_decode_packet(i_buf, o_buf, out_len);
  }
};
    ...
    switch (service_id) {
      ...
      default:
        BTIF_TRACE_ERROR("%s: Unknown service %d being %s", __func__,
                         service_id, (b_enable) ? "enabled" : "disabled");
