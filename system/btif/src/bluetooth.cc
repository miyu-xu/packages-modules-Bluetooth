...
static int disconnect_le_acl(const RawAddress& bd_addr) {
  log::verbose("{}", bd_addr);
  if (!interface_ready()) {
    return BT_STATUS_NOT_READY;
  }

  do_in_main_thread(base::BindOnce(btif_dm_disconnect_le_acl, bd_addr));
  return BT_STATUS_SUCCESS;
}
...
EXPORT_SYMBOL bt_interface_t bluetoothInterface = {
        ...
        .disconnect_le_acl = disconnect_le_acl,
        .le_rand = le_rand,
