  ...
  if (IS_FLAG_ENABLED(gdx_device_discovery)) {
    return bluetooth::shim::Stack::GetInstance()
                   ->CallOnModule<
                     ...
                     mod->StartDiscovery(get_jni()->Bind([](bt_discovery_state_t
                                                                state) {
                       log::assert_that(
                           is_on_jni_thread(),
                           "Device discovery state must be run on jni thread");
                                         ...
                                         get_jni()->Bind(
                                             [](std::vector<std::shared_ptr<
                                               ...
                                               log::assert_that(
                                                   ...
                                                   "must be run on jni thread");
                                               auto properties =
                                                   property::BtPropertyLegacy(
                                                       bt_properties);
                         ...
                         mgr->CancelDiscovery(
                             get_jni()->Bind([](bt_discovery_state_t state) {
                               log::assert_that(is_on_jni_thread(),
                                                "Device discovery state must "
                                                "be run on jni thread");
                               HAL_CBACK(bt_hal_cbacks,
