...
static int create_bond_out_of_band(const RawAddress* bd_addr, int transport,
  ...
  do_in_main_thread(base::BindOnce(btif_dm_create_bond_out_of_band, *bd_addr, transport,
                                              *p192_data, *p256_data));
  return BT_STATUS_SUCCESS;
}
...
static void metadata_changed(const RawAddress& remote_bd_addr, int key,
  ...
  do_in_main_thread(base::BindOnce(btif_dm_metadata_changed, remote_bd_addr, key,
                                              std::move(value)));
}
