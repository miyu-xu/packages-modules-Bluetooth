...
static bool prop2cfg(const RawAddress* remote_bd_addr, bt_property_t* prop) {
  ...
  switch (prop->type) {
    ...
    case BT_PROPERTY_ADAPTER_SCAN_MODE:
      btif_config_set_int(BTIF_STORAGE_SECTION_ADAPTER, BTIF_STORAGE_KEY_SCANMODE,
                          *(int*)prop->val);
      break;
  ...
  switch (prop->type) {
    case BT_PROPERTY_REMOTE_DEVICE_TIMESTAMP:
      if (prop->len >= (int)sizeof(int)) {
        ret = btif_config_get_int(bdstr, BTIF_STORAGE_KEY_TIMESTAMP, (int*)prop->val);
      }
      break;
    case BT_PROPERTY_BDNAME: {
      int len = prop->len;
      if (remote_bd_addr) {
        ret = btif_config_get_str(bdstr, BTIF_STORAGE_KEY_NAME, (char*)prop->val, &len);
      } else {
    ...
    case BT_PROPERTY_REMOTE_FRIENDLY_NAME: {
      int len = prop->len;
      ret = btif_config_get_str(bdstr, BTIF_STORAGE_KEY_ALIAS, (char*)prop->val, &len);
      if (ret && len && len <= prop->len) {
        prop->len = len - 1;
      } else {
        prop->len = 0;
    ...
    case BT_PROPERTY_ADAPTER_SCAN_MODE:
      if (prop->len >= (int)sizeof(int)) {
        ret = btif_config_get_int(BTIF_STORAGE_SECTION_ADAPTER, BTIF_STORAGE_KEY_SCANMODE,
                                  (int*)prop->val);
      }
      break;
    ...
    case BT_PROPERTY_CLASS_OF_DEVICE:
      if (prop->len >= (int)sizeof(int)) {
        ret = btif_config_get_int(bdstr, BTIF_STORAGE_KEY_DEV_CLASS, (int*)prop->val);
      }
      break;
    case BT_PROPERTY_TYPE_OF_DEVICE:
      if (prop->len >= (int)sizeof(int)) {
        ret = btif_config_get_int(bdstr, BTIF_STORAGE_KEY_DEV_TYPE, (int*)prop->val);
      }
      break;
    case BT_PROPERTY_UUIDS: {
      char value[1280];
    ...
    case BT_PROPERTY_REMOTE_MODEL_NUM: {
      int len = prop->len;
      ret = btif_config_get_str(bdstr, BTIF_STORAGE_KEY_DIS_MODEL_NUM, (char*)prop->val, &len);
      if (ret && len && len <= prop->len) {
        prop->len = len - 1;
      } else {
        prop->len = 0;
...
static void btif_read_le_key(const uint8_t key_type, const size_t key_len, RawAddress bd_addr,
  ...
  if (btif_storage_get_ble_bonding_key(bd_addr, key_type, (uint8_t*)&key, key_len) ==
      BT_STATUS_SUCCESS) {
    if (add_key) {
      if (!*device_added) {
...
bt_status_t btif_storage_add_bonded_device(RawAddress* remote_bd_addr, LinkKey link_key,
  ...
  std::string bdstr = remote_bd_addr->ToString();
  bool ret = btif_config_set_int(bdstr, BTIF_STORAGE_KEY_LINK_KEY_TYPE, (int)key_type);
  ret &= btif_config_set_int(bdstr, BTIF_STORAGE_KEY_PIN_LENGTH, (int)pin_length);
...
void btif_storage_load_le_devices(void) {
  ...
  for (uint16_t i = 0; i < bonded_devices.num_devices; i++) {
    ...
    tBTA_LE_KEY_VALUE key = {};
    if (btif_storage_get_ble_bonding_key(bonded_devices.devices[i], BTM_LE_KEY_PID, (uint8_t*)&key,
...
bt_status_t btif_storage_load_bonded_devices(void) {
  ...
  /* Now send the adapter_properties_cb with all adapter_properties */
  {
    ...
    /* BONDED_DEVICES */
    RawAddress* devices_list =
            (RawAddress*)osi_malloc(sizeof(RawAddress) * bonded_devices.num_devices);
