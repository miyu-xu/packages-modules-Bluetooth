...
void btif_storage_set_encr_data_cccd(const RawAddress& bd_addr, uint8_t cccd) {
  do_in_jni_thread(FROM_HERE, Bind(
                                  [](const RawAddress& bd_addr, uint8_t cccd) {
                                    btif_config_set_int(
                                        bd_addr.ToString(),
                                        BTIF_STORAGE_KEY_ENCR_DATA_CCCD, cccd);
                                  },
                                  bd_addr, cccd));
}
...
bt_status_t btif_storage_set_enc_key_material(const RawAddress& remote_bd_addr,
                                              uint8_t* value,
                                              uint8_t key_length) {
  return btif_config_set_bin(remote_bd_addr.ToString(),
...
bt_status_t btif_storage_get_enc_key_material(const RawAddress* remote_bd_addr,
  ...
  LOG(INFO) << __func__;
  std::string key =
      (remote_bd_addr == NULL) ? "Adapter" : remote_bd_addr->ToString();
  return btif_config_get_bin(key, BTIF_STORAGE_KEY_ENCR_DATA, key_value,
                             (size_t*)&key_length)
             ? BT_STATUS_SUCCESS
             : BT_STATUS_FAIL;
}
...
bt_status_t btif_storage_remove_enc_key_material(
  ...
  if (remote_bd_addr == NULL) {
    ...
    BTIF_TRACE_DEBUG(" %s in bd addr:%s", __func__,
                     ADDRESS_TO_LOGGABLE_CSTR(*remote_bd_addr));
  }

  if (btif_config_exist(key, BTIF_STORAGE_KEY_ENCR_DATA) &&
