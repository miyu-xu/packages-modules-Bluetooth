  ...
  switch (event) {
    ...
    case BTA_AG_AT_BAC_EVT:
      ...
      if (hfp_hal_interface::get_swb_supported() &&
        ...
        LOG_DEBUG("%s: btif_hf override-Preferred Codec to LC3", __func__);
        ...
        LOG_DEBUG("%s: btif_hf override-Preferred Codec to mSBC", __func__);
        BTA_AgSetCodec(btif_hf_cb[idx].handle, BTM_SCO_CODEC_MSBC);
      } else {
        LOG_DEBUG("%s btif_hf override-Preferred Codec to CVSD", __func__);
...
bt_status_t HeadsetInterface::PhoneStateChange(
  ...
  */
  if (((num_active + num_held) > 0) && (control_block.num_active == 0) &&
    ...
    tBTA_AG_RES_DATA ag_res = {};
    LOG_VERBOSE("%s: Active/Held call notification received without call setup "
                "update",
                __func__);
