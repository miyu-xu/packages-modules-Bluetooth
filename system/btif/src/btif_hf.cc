  ...
  switch (event) {
      ...
      if (p_data->val.num == BTM_SCO_CODEC_CVSD) {
        ...
        if (bluetooth::common::init_flags::aptx_voice_is_enabled()) {
          ...
          if (p_data->val.num == BTA_AG_SCO_SWB_SETTINGS_Q0 ||
              p_data->val.num == BTA_AG_SCO_SWB_SETTINGS_Q1 ||
              p_data->val.num == BTA_AG_SCO_SWB_SETTINGS_Q2 ||
              p_data->val.num == BTA_AG_SCO_SWB_SETTINGS_Q3) {
            config = BTHF_SWB_YES;
          } else {
            config = BTHF_SWB_NO;
          }
          bt_hf_callbacks->SwbCallback(BTHF_SWB_CODEC_APTX, config,
                                       &btif_hf_cb[idx].connected_bda);
        } else {
    ...
    case BTA_AG_AT_QCS_EVT:
      if (bluetooth::common::init_flags::aptx_voice_is_enabled()) {
        ...
        bt_hf_callbacks->SwbCallback(
            BTHF_SWB_CODEC_APTX,
            p_data->val.num <= BTA_AG_SCO_SWB_SETTINGS_Q3 ? BTHF_SWB_YES
                                                          : BTHF_SWB_NO,
            &btif_hf_cb[idx].connected_bda);
        break;
      }
...
class HeadsetInterface : Interface {
 public:
  ...
  bool GetSwbCodecStatus(bthf_swb_codec_t swbCodec) override;
  bt_status_t EnableSwb(bthf_swb_codec_t swbCodec, bool enable) override;
...
bt_status_t HeadsetInterface::EnableSwb(bthf_swb_codec_t swbCodec,
                                        bool enable) {
  return enable_aptx_swb_codec(enable);
}

bool HeadsetInterface::GetSwbCodecStatus(bthf_swb_codec_t swbCodec) {
  bool status = false;
  switch (swbCodec) {
    case BTHF_SWB_CODEC_APTX:
      status = get_aptx_swb_codec_status();
      break;
    case BTHF_SWB_CODEC_LC3:
      FALLTHROUGH_INTENDED;
    default:
      LOG(ERROR) << __func__ << " failed: swbCodec: " << swbCodec;
      break;
  }
  return status;
}
