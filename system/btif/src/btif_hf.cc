  ...
  switch (event) {
    ...
    case BTA_AG_OPEN_EVT:
      ...
      if (btif_hf_cb[idx].is_initiator) {
        ...
        // Check the incoming open event status and the outoging connection
        // state.
        if ((p_data->open.status != BTA_AG_SUCCESS) &&
            btif_hf_cb[idx].state != BTHF_CONNECTION_STATE_CONNECTING) {
          // Check if the incoming open event and the outoging connection are
          // for the same device.
        ...
        // There is an outgoing connection.
        // Check the outoging connection state and address.
