...
#define CHECK_BTHF_CLIENT_INIT()                                               \
  do {                                                                         \
    if (bt_hf_client_callbacks == NULL) {                                      \
      ...
      LOG_DEBUG("BTHF CLIENT: %s", __func__);                                  \
    }                                                                          \
  } while (0)
...
#define CHECK_BTHF_CLIENT_SLC_CONNECTED(cb)                                    \
  do {                                                                         \
    if (bt_hf_client_callbacks == NULL) {                                      \
      ...
      LOG_WARN(                                                                \
          "BTHF CLIENT: %s: SLC connection not up. state=%s",   \ __func__,    \
...
static bt_status_t connect(const RawAddress* bd_addr) {
  LOG_DEBUG("HFP Client version is  %s", btif_hf_client_version);
...
static bt_status_t send_at_cmd(const RawAddress* bd_addr, int cmd, int val1,
  ...
  LOG_DEBUG("%s: Cmd %d val1 %d val2 %d arg %s", __func__, cmd, val1, val2,
            (arg != NULL) ? arg : "<null>");
...
static bt_status_t send_android_at(const RawAddress* bd_addr, const char* arg) {
  ...
  LOG_DEBUG("%s: val1 %s", __func__, arg);
...
bt_status_t btif_hf_client_execute_service(bool b_enable) {
  LOG_DEBUG("%s: enable: %d", __func__, b_enable);
  ...
  if (b_enable) {
    ...
    LOG_DEBUG("%s: support codec negotiation %d ", __func__, features);
