...
#define CHECK_BTHF_CLIENT_INIT()                                               \
  do {                                                                         \
    if (bt_hf_client_callbacks == NULL) {                                      \
      ...
      LOG_INFO("BTHF CLIENT: %s", __func__);                                   \
    }                                                                          \
  } while (0)
 ...
 ******************************************************************************/
btif_hf_client_cb_t* btif_hf_client_get_cb_by_handle(uint16_t handle) {
  LOG_DEBUG("%s: cb by handle %d", __func__, handle);
...
static bt_status_t connect(const RawAddress* bd_addr) {
  LOG_INFO("HFP Client version is  %s", btif_hf_client_version);
...
static bt_status_t send_at_cmd(const RawAddress* bd_addr, int cmd, int val1,
  ...
  LOG_INFO("%s: Cmd %d val1 %d val2 %d arg %s", __func__, cmd, val1, val2,
           (arg != NULL) ? arg : "<null>");
...
static bt_status_t send_android_at(const RawAddress* bd_addr, const char* arg) {
  ...
  LOG_INFO("%s: val1 %s", __func__, arg);
...
static void btif_hf_client_upstreams_evt(uint16_t event, char* p_param) {
  ...
  if (cb == NULL && event == BTA_HF_CLIENT_OPEN_EVT) {
    LOG_DEBUG("%s: event BTA_HF_CLIENT_OPEN_EVT allocating block", __func__);
  ...
  LOG_DEBUG("%s: event=%s (%u)", __func__, dump_hf_client_event(event), event);
  ...
  switch (event) {
    case BTA_HF_CLIENT_OPEN_EVT:
      if (p_data->open.status == BTA_HF_CLIENT_SUCCESS) {
        ...
        LOG_WARN("%s: HF CLient open failed, but another device connected. "
...
bt_status_t btif_hf_client_execute_service(bool b_enable) {
  LOG_INFO("%s: enable: %d", __func__, b_enable);
  ...
  if (b_enable) {
    ...
    LOG_INFO("%s: support codec negotiation %d ", __func__, features);
