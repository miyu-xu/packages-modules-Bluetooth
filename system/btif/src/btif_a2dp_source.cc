...
private:
  /// Guards access to the internal state of the a2dp source module.
  /// Members may be read / modified from multiple thread contexts:
  ///  - bt_main_thread
...
static MessageLoopThread* local_thread() {
  return com::bluetooth::flags::a2dp_source_threading_fix() ? get_main_thread()
...
bool btif_a2dp_source_end_session(const RawAddress& peer_address) {
  log::info("peer_address={} state={}", peer_address, btif_a2dp_source_cb.StateStr());
  do_in_main_thread(base::BindOnce(&btif_a2dp_source_end_session_delayed, peer_address));
