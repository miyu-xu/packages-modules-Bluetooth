...
static void btif_a2dp_source_startup_delayed() {
  ...
  if (!btif_a2dp_source_thread.EnableRealTimeScheduling()) {
    ...
    LOG_ALWAYS_FATAL("%s: unable to enable real time scheduling");
#endif
  }
...
bool btif_a2dp_source_start_session(const RawAddress& peer_address,
  ...
  LOG_INFO("%s: peer_address=%s state=%s", __func__,
      peer_address.ToString().c_str(), btif_a2dp_source_cb.StateStr().c_str());
  ...
  if (btif_a2dp_source_thread.DoInThread(
    ...
    LOG_ALWAYS_FATAL("%s: peer_address=%s state=%s fails to context switch",
        __func__, peer_address.ToString().c_str(), btif_a2dp_source_cb.StateStr().c_str());
    return false;
  }
}
...
bool btif_a2dp_source_restart_session(const RawAddress& old_peer_address,
  ...
  LOG_INFO("%s: old_peer_address=%s new_peer_address=%s is_streaming=%s state=%s",
      __func__,  old_peer_address.ToString().c_str(),
      new_peer_address.ToString().c_str(), (is_streaming ? "true" : "false"),
...
void btif_a2dp_source_encoder_user_config_update_req(
  ...
  LOG_INFO("%s: peer_address=%s state=%s %zu codec_preference(s)", __func__,
      peer_address.ToString().c_str(), btif_a2dp_source_cb.StateStr().c_str(), codec_user_preferences.size());
  if (!btif_a2dp_source_thread.DoInThread(
          FROM_HERE,
...
static void btif_a2dp_source_encoder_user_config_update_event(
  ...
  for (auto codec_user_config : codec_user_preferences) {
    success = bta_av_co_set_codec_user_config(peer_address, codec_user_config,
                                              &restart_output);
    if (success) {
      LOG_INFO("%s: peer_address=%s state=%s codec_preference={%s} restart_output=%s",
          __func__, peer_address.ToString().c_str(), btif_a2dp_source_cb.StateStr().c_str(),
          codec_user_config.ToString().c_str(), (restart_output ? "true" : "false"));
      break;
    }
  }
