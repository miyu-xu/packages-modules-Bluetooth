...
bool btif_a2dp_source_init(void) {
  ...
  btif_a2dp_source_thread.DoInThread(
      FROM_HERE, base::BindRepeating(&btif_a2dp_source_init_delayed));
  return true;
}
...
bool btif_a2dp_source_startup(void) {
  ...
  btif_a2dp_source_thread.DoInThread(
      FROM_HERE, base::BindRepeating(&btif_a2dp_source_startup_delayed));
...
bool btif_a2dp_source_end_session(const RawAddress& peer_address) {
  ...
  btif_a2dp_source_thread.DoInThread(
      FROM_HERE,
      base::BindRepeating(&btif_a2dp_source_end_session_delayed, peer_address));
...
void btif_a2dp_source_cleanup(void) {
  ...
  btif_a2dp_source_thread.DoInThread(
      FROM_HERE, base::BindRepeating(&btif_a2dp_source_cleanup_delayed));
...
static void btif_a2dp_source_setup_codec(const RawAddress& peer_address) {
  ...
  btif_a2dp_source_thread.DoInThread(
      FROM_HERE,
      base::BindRepeating(&btif_a2dp_source_setup_codec_delayed, peer_address));
}
...
static void btif_a2dp_source_cleanup_codec() {
  ...
  btif_a2dp_source_stop_audio_req();
  btif_a2dp_source_thread.DoInThread(
      FROM_HERE, base::BindRepeating(&btif_a2dp_source_cleanup_codec_delayed));
}
...
void btif_a2dp_source_start_audio_req(void) {
  ...
  btif_a2dp_source_thread.DoInThread(
      FROM_HERE, base::BindRepeating(&btif_a2dp_source_audio_tx_start_event));
}
...
void btif_a2dp_source_stop_audio_req(void) {
  ...
  btif_a2dp_source_thread.DoInThread(
      FROM_HERE, base::BindRepeating(&btif_a2dp_source_audio_tx_stop_event));
}
...
void btif_a2dp_source_feeding_update_req(
  ...
  btif_a2dp_source_thread.DoInThread(
      FROM_HERE,
      base::BindRepeating(&btif_a2dp_source_audio_feeding_update_event,
                          codec_audio_config));
}
...
static bool btif_a2dp_source_audio_tx_flush_req(void) {
  ...
  btif_a2dp_source_thread.DoInThread(
      FROM_HERE, base::BindRepeating(&btif_a2dp_source_audio_tx_flush_event));
  return true;
}
