 ...
 public:
  ...
  BtifA2dpSource()
      : tx_audio_queue(nullptr),
        tx_flush(false),
        sw_audio_encoding_started(false),
  ...
  bool sw_audio_encoding_started;
...
void btif_a2dp_source_on_stopped(tBTA_AV_SUSPEND* p_av_suspend) {
  ...
  btif_a2dp_source_cb.sw_audio_encoding_started = false;
...
#endif
          btif_a2dp_source_cb.encoder_interface->get_encoder_interval_ms()));
  btif_a2dp_source_cb.sw_audio_encoding_started = true;
...
static uint32_t btif_a2dp_source_read_callback(uint8_t* p_buf, uint32_t len) {
  ...
  if (btif_a2dp_source_cb.sw_audio_encoding_started && bytes_read < len) {
