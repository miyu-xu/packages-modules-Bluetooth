...
static void btif_a2dp_source_audio_tx_stop_event(void) {
  ...
  if (bluetooth::audio::a2dp::is_hal_enabled()) {
    btif_a2dp_control_log_bytes_read(bluetooth::audio::a2dp::read(p_buf, sizeof(p_buf)));
...
static void btif_a2dp_source_update_metrics(void) {
  ...
  if (enqueue_stats.total_updates > 1) {
    ...
    metrics.total_scheduling_count = enqueue_stats.overdue_scheduling_count +
            enqueue_stats.premature_scheduling_count + enqueue_stats.exact_scheduling_count;
