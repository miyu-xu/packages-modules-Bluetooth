...
static void btif_dm_search_services_evt(tBTA_DM_SEARCH_EVT event,
  ...
  switch (event) {
      ...
      if (p_data->disc_res.num_uuids != 0 || num_eir_uuids != 0) {
        ...
        if (bd_addr == pairing_cb.bd_addr ||
          ...
          if (le_audio_capable && a2dp_sink_capable &&
            ...
            /* Next step is either succesfull BLE service discovery, or failure,
        ...
        /* Next step is either succesfull BLE service discovery, or failure, no
    ...
    case BTA_DM_GATT_OVER_LE_RES_EVT: {
      ...
      if (event == BTA_DM_GATT_OVER_LE_RES_EVT) {
        ...
        if ((bd_addr == pairing_cb.bd_addr ||
          ...
          if (pairing_cb.gatt_on_le !=
              btif_dm_pairing_cb_t::service_discovery_state::SCHEDULED) {
            LOG_ERROR(
                "gatt_on_le should be SCHEDULED, did someone clear the control "
                "block ?");
          }
          pairing_cb.gatt_on_le =
              btif_dm_pairing_cb_t::service_discovery_state::FINISHED;
      ...
      /* If services were returned as part of SDP discovery, we will send them
       * with rest of SDP results */
  ...
  if (p_auth_cmpl->success) {
    ...
    if (btm_get_bond_type_dev(bd_addr) ==
      ...
      if (pairing_cb.gatt_on_le ==
        ...
        LOG_DEBUG(
            "Skipping LE transport discovery - was already scheduled or "
            "finished");
      }
    }
  } else {
