...
bt_status_t btif_in_execute_service_request(tBTA_SERVICE_ID service_id,
                                            bool b_enable) {
  LOG_DEBUG("%s service_id: %d", __func__, service_id);
  ...
  if (pairing_cb.is_local_initiated && !p_pin_req->min_16_digit) {
    if (check_cod(&bd_addr, COD_AV_HEADSETS) ||
      ...
      if (!interop_match_addr(INTEROP_DISABLE_AUTO_PAIRING, &bd_addr) &&
        ...
        LOG_DEBUG("%s() Attempting auto pair", __func__);
        pin_code.pin[0] = 0x30;
        pin_code.pin[1] = 0x30;
        pin_code.pin[2] = 0x30;
        pin_code.pin[3] = 0x30;
...
static void btif_dm_ssp_cfm_req_evt(tBTA_DM_SP_CFM_REQ* p_ssp_cfm_req) {
  ...
  LOG_INFO("%s: just_works:%d, loc_auth_req=%d, rmt_auth_req=%d", __func__,
  ...
  if (p_ssp_cfm_req->just_works) {
    ...
    if (is_incoming &&
        pairing_cb.bond_type == tBTM_SEC_DEV_REC::BOND_TYPE_TEMPORARY) {
      LOG_INFO("%s: Auto-accept JustWorks pairing for temporary incoming",
               __func__);
...
static void btif_dm_auth_cmpl_evt(tBTA_DM_AUTH_CMPL* p_auth_cmpl) {
  ...
  if ((p_auth_cmpl->success) && (p_auth_cmpl->key_present)) {
    if ((p_auth_cmpl->key_type < HCI_LKEY_TYPE_DEBUG_COMB) ||
        ...
        pairing_cb.bond_type == tBTM_SEC_DEV_REC::BOND_TYPE_PERSISTENT) {
      bt_status_t ret;
      LOG_DEBUG("%s: Storing link key. key_type=0x%x, bond_type=%d", __func__,
      ...
      LOG_DEBUG("%s: Temporary key. Not storing. key_type=0x%x, bond_type=%d",
                __func__, p_auth_cmpl->key_type, pairing_cb.bond_type);
      if (pairing_cb.bond_type == tBTM_SEC_DEV_REC::BOND_TYPE_TEMPORARY) {
        LOG_DEBUG("%s: sending BT_BOND_STATE_NONE for Temp pairing", __func__);
  ...
  if (p_auth_cmpl->success) {
    ...
    switch (p_auth_cmpl->fail_reason) {
      ...
      case HCI_ERR_UNSPECIFIED:
        LOG_DEBUG(" %s() Authentication fail reason %d", __func__,
                  p_auth_cmpl->fail_reason);
    ...
    if (check_cod_hid_major(bd_addr, COD_HID_POINTING)) {
      ...
      LOG_DEBUG("%s(): removing hid pointing device from nvram", __func__);
      is_bonded_device_removed = false;
    }
...
static void btif_dm_search_devices_evt(tBTA_DM_SEARCH_EVT event,
  ...
  LOG_INFO("%s event=%s", __func__, dump_dm_search_event(event));
  ...
  switch (event) {
    case BTA_DM_DISC_RES_EVT: {
      ...
      if (strlen((const char*)p_search_data->disc_res.bd_name)) {
        ...
        if (btif_storage_get_remote_device_property(
          ...
          LOG_DEBUG("%s, BTA_DM_DISC_RES_EVT, cod in storage = 0x%08x",
                    __func__, cod);
        } else {
          LOG_DEBUG("%s, BTA_DM_DISC_RES_EVT, no cod in storage", __func__);
          cod = 0;
        }
        if (cod != 0) {
          BTIF_STORAGE_FILL_PROPERTY(&properties[1], BT_PROPERTY_BDADDR, sizeof(bdaddr), &bdaddr);
          BTIF_STORAGE_FILL_PROPERTY(&properties[2], BT_PROPERTY_CLASS_OF_DEVICE, sizeof(uint32_t), &cod);
          LOG_DEBUG("%s: Now we have name and cod, report to JNI", __func__);
    ...
    case BTA_DM_INQ_RES_EVT: {
      ...
      LOG_DEBUG("%s() %s device_type = 0x%x\n", __func__,
...

      {
        ...
        LOG_DEBUG("%s cod is 0x%06x", __func__, cod);
        if (cod != 0) {
          BTIF_STORAGE_FILL_PROPERTY(&properties[num_properties],
        ...
        LOG_DEBUG("%s clock_offset is 0x%x", __func__,
...
static void btif_dm_upstreams_evt(uint16_t event, char* p_param) {
  ...
  LOG_INFO("%s: ev: %s", __func__, dump_dm_event(event));
  ...
  switch (event) {
    ...
    case BTA_DM_LINK_UP_EVT:
      ...
      LOG_DEBUG("BTA_DM_LINK_UP_EVT. Sending BT_ACL_STATE_CONNECTED");
    ...
    case BTA_DM_BLE_KEY_EVT:
      LOG_DEBUG("BTA_DM_BLE_KEY_EVT key_type=0x%02x ",
      ...
      now*/
      if (pairing_cb.state != BT_BOND_STATE_BONDING) {
        LOG_DEBUG("Bond state not sent to App so far.Notify the app now");
    ...
    case BTA_DM_BLE_AUTH_CMPL_EVT:
      LOG_DEBUG("BTA_DM_BLE_AUTH_CMPL_EVT. ");
...
static void bta_energy_info_cb(tBTM_BLE_TX_TIME_MS tx_time,
                               ...
                               tBTA_STATUS status) {
  LOG_DEBUG("energy_info_cb-Status:%d,state=%d,tx_t=%ld, rx_t=%ld, "
...
void btif_dm_create_bond(const RawAddress bd_addr, int transport) {
  LOG_INFO("%s: bd_addr=%s, transport=%d", __func__,
...
void btif_dm_create_bond_le(const RawAddress bd_addr,
                            tBLE_ADDR_TYPE addr_type) {
  LOG_INFO("%s: bd_addr=%s, addr_type=%d, transport=%d", __func__,
...
void btif_dm_cancel_bond(const RawAddress bd_addr) {
  LOG_INFO("%s: bd_addr=%s", __func__, ADDRESS_TO_LOGGABLE_CSTR(bd_addr));
...
#endif
  {
    LOG_DEBUG("%s: Removing HH device", __func__);
...
void btif_dm_pin_reply(const RawAddress bd_addr, uint8_t accept,
  ...
  LOG_INFO("%s: accept=%d", __func__, accept);
  ...
  if (pairing_cb.is_le_only) {
    ...
    LOG_DEBUG("btif_dm_pin_reply: passkey: %d", passkey);
...
bt_status_t btif_dm_get_adapter_property(bt_property_t* prop) {
  LOG_INFO("%s: type=0x%x", __func__, prop->type);
  switch (prop->type) {
...
void btif_dm_get_remote_services(RawAddress remote_addr, const int transport) {
  LOG_INFO("%s: transport=%d, remote_addr=%s", __func__, transport,
...
void btif_dm_proc_io_req(tBTM_AUTH_REQ* p_auth_req, bool is_orig) {
  ...
  */
  ...
  LOG_DEBUG("+%s: p_auth_req=%d", __func__, *p_auth_req);
  if (pairing_cb.is_local_initiated) {
    ...
    */
    LOG_DEBUG("%s: setting p_auth_req to peer's: %d", __func__,
              pairing_cb.auth_req);
...
void btif_dm_proc_io_rsp(UNUSED_ATTR const RawAddress& bd_addr,
  ...
  if (auth_req & BTA_AUTH_BONDS) {
    LOG_DEBUG("%s auth_req:%d", __func__, auth_req);
...
void btif_dm_set_oob_for_io_req(tBTM_OOB_DATA* p_has_oob_data) {
  ...
  LOG_DEBUG("%s: *p_has_oob_data=%d", __func__, *p_has_oob_data);
}
...
void btif_dm_load_local_oob(void) {
  ...
  LOG_DEBUG("%s: prop_oob = %s", __func__, prop_oob);
  if (prop_oob[0] != '3') {
    if (is_empty_128bit(oob_cb.p192_data.c)) {
      LOG_DEBUG("%s: read OOB, call BTA_DmLocalOob()", __func__);
      BTA_DmLocalOob();
    }
  }
}
...
bool btif_dm_proc_rmt_oob(const RawAddress& bd_addr, Octet16* p_c,
  ...
  if (prop_oob[0] == '1')
    ...
    path = path_a;
  if (!path) {
    LOG_DEBUG("%s: can't open path!", __func__);
    return false;
  }

  FILE* fp = fopen(path, "rb");
  if (fp == NULL) {
    LOG_DEBUG("%s: failed to read OOB keys from %s", __func__, path);
    return false;
  }

  LOG_DEBUG("%s: read OOB data from %s", __func__, path);
...
static void btif_dm_ble_sec_req_evt(tBTA_DM_BLE_SEC_REQ* p_ble_req,
  ...
  if (!is_consent && pairing_cb.state == BT_BOND_STATE_BONDING) {
    LOG_DEBUG("%s Discard security request", __func__);
    return;
  }
...
static void btif_dm_ble_sc_oob_req_evt(tBTA_DM_SP_RMT_OOB* req_oob_type) {
  ...
  RawAddress bd_addr = req_oob_type->bd_addr;
  LOG_DEBUG("%s: bd_addr: %s", __func__, ADDRESS_TO_LOGGABLE_CSTR(bd_addr));
  LOG_DEBUG("%s: oob_cb.bdaddr: %s", __func__,
...
void btif_dm_on_disable() {
  ...
  if (is_bonding_or_sdp()) {
    LOG_DEBUG("%s: Cancel pending pairing request", __func__);
