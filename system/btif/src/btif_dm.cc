...
void btif_dm_start_discovery(void) {
  ...
  if (bta_dm_is_search_request_queued()) {
    if (bta_dm_search_cb.device_search.time_started_ms == 0) {
      ...
      double last_request_time_ms = timestamper_in_ms.GetTimestamp() - bta_dm_search_cb.device_search.time_started_ms;
      if (last_request_time_ms > bta_dm_search_cb.device_search.timeout_ms) {
        if (bluetooth::common::init_flags::disc_device_timemout_is_enabled()) {
          LOG_ALWAYS_FATAL("Device discovery is taking too long s:%.3f", last_request_time_ms/1000.0);
        }
        LOG_ERROR("Device start discovery is taking a long time s:%.3f", last_request_time_ms/1000.0);
      }
    }
  ...
  if (bta_dm_search_cb.device_search.time_started_ms != 0) {
    LOG_ERROR("Pending search callback clear with non-zero device discovery timestamp");
  }
