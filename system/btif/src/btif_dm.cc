          ...
          while ((p_service_data = AdvertiseDataParser::GetFieldByType(
            ...
            if (uuid == 0xfdf0 /* ASHA service*/) {
              ...
              if (service_data_len < 8) {
                ...
                LOG_INFO("ASHA_capability: %u", ASHA_capability);

                // add bit mask so that ASHA_capability won't be zero
                uint16_t ASHA_capability_mask = 1 << 8;
                ASHA_capability += ASHA_capability_mask;
                BTIF_STORAGE_FILL_PROPERTY(&properties[num_properties],
                                           BT_PROPERTY_REMOTE_ASHA_CAPABILITY,
                                           sizeof(uint16_t), &ASHA_capability);

                const uint8_t* p_truncated_hisyncid = &(p_service_data[4]);
                STREAM_TO_UINT32(ASHA_truncated_HiSyncID, p_truncated_hisyncid);
