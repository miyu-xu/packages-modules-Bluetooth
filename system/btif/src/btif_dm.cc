...
static void btif_on_service_discovery_results(
  ...
  if ((result == BTA_SUCCESS) && !uuids_param.empty()) {
    ...
    bt_status_t existing_lookup_result = btif_get_existing_uuids(&pairing_cb.bd_addr, existing_uuids);
...
void btif_on_gatt_results(RawAddress bd_addr, BD_NAME bd_name,
  ...
  if (!transport_le) {
    ...
    * send them with rest of SDP results in on_service_discovery_results */
    ...
    if (pairing_cb.sdp_over_classic ==
            btif_dm_pairing_cb_t::ServiceDiscoveryState::SCHEDULED && com::android::bluetooth::flags::bta_dm_discover_both()) {
      /* Don't report services yet, they will be reported together once SDP finishes. */
