...
static void btif_dm_search_devices_evt(tBTA_DM_SEARCH_EVT event, tBTA_DM_SEARCH* p_search_data) {
  ...
  switch (event) {
    ...
    case BTA_DM_INQ_RES_EVT: {
      ...
      bool report_le_advertising_uuids =
              com::android::bluetooth::flags::report_uuids_from_le_advertising_data() &&
              (p_search_data->inq_res.device_type == BT_DEVICE_TYPE_BLE);

      // TODO: Remove these logs when submitting.
      log::info("report_le_advertising_uuids={}", report_le_advertising_uuids);
      log::info(" ---- flag={}, type={}", flag_report_le_advertise_data_service_uuids,
      ...
      if (p_search_data->inq_res.p_eir) {
        ...
        if (report_le_advertising_uuids) {
          if (!get_btm_client_interface().eir.BTM_GetEirUuidList(
                      p_search_data->inq_res.p_eir, p_search_data->inq_res.eir_len,
                      Uuid::kNumBytes128, &num_uuids_128bits, uuid_list_128bits,
                      max_num_uuid_128bits)) {
...

      {
        ...
        if (num_uuids_16bits > 0) {
          ...
          log::debug("16-bit UUIDs for {}:", bdaddr);
        ...
        if (num_uuids_128bits > 0) {
          ...
          log::debug("128-bit UUIDs for {}:", bdaddr);
