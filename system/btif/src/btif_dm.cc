  ...
  switch (event) {
    case BTA_DM_DISC_RES_EVT: {
      ...
      if ((bd_addr == pairing_cb.bd_addr ||
          ...
          pairing_cb.gatt_over_le !=
              btif_dm_pairing_cb_t::ServiceDiscoveryState::FINISHED) {
        /* Check if device was discovered as LE capable by Fast Pair or have LE
         * Audio in CoD. If any, it's LE Audio capable */
        if (metadata_cb.le_audio_cache.Find(bd_addr) ||
            check_cod_le_audio(bd_addr)) {
