...
static void btif_dm_ssp_cfm_req_evt(tBTA_DM_SP_CFM_REQ* p_ssp_cfm_req) {
  ...
  LOG_DEBUG("%s: just_works:%d, loc_auth_req=%d, rmt_auth_req=%d", __func__,
  ...
  if (p_ssp_cfm_req->just_works) {
    ...
    if (is_incoming &&
        pairing_cb.bond_type == tBTM_SEC_DEV_REC::BOND_TYPE_TEMPORARY) {
      LOG_DEBUG("%s: Auto-accept JustWorks pairing for temporary incoming",
                __func__);
...
static void btif_dm_search_devices_evt(tBTA_DM_SEARCH_EVT event,
  ...
  LOG_DEBUG("%s event=%s", __func__, dump_dm_search_event(event));
...
void btif_dm_sec_evt(tBTA_DM_SEC_EVT event, tBTA_DM_SEC* p_data) {
  ...
  LOG_DEBUG("%s: ev: %s", __func__, dump_dm_event(event));
...
static void bta_energy_info_cb(tBTM_BLE_TX_TIME_MS tx_time,
                               ...
                               tBTA_STATUS status) {
  LOG_VERBOSE("energy_info_cb-Status:%d,state=%d,tx_t=%u, rx_t=%u, "
              "idle_time=%u,used=%u",
...
void btif_dm_create_bond(const RawAddress bd_addr, int transport) {
  LOG_DEBUG("%s: bd_addr=%s, transport=%d", __func__,
...
void btif_dm_create_bond_le(const RawAddress bd_addr,
                            tBLE_ADDR_TYPE addr_type) {
  LOG_DEBUG("%s: bd_addr=%s, addr_type=%d", __func__,
...
void btif_dm_pin_reply(const RawAddress bd_addr, uint8_t accept,
  ...
  LOG_DEBUG("%s: accept=%d", __func__, accept);
...
bt_status_t btif_dm_get_adapter_property(bt_property_t* prop) {
  LOG_DEBUG("%s: type=0x%x", __func__, prop->type);
  switch (prop->type) {
...
void btif_dm_get_remote_services(RawAddress remote_addr, const int transport) {
  LOG_DEBUG("%s: transport=%d, remote_addr=%s", __func__, transport,
