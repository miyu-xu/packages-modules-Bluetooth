...
static void btif_update_remote_properties(const RawAddress& bdaddr,
  ...
  if (strlen((const char*)bd_name)) {
    properties.push_back(bt_property_t{BT_PROPERTY_BDNAME, static_cast<int>(strlen((char *) bd_name)),
                            static_cast<void *>(bd_name)});
    status =
  ...
  properties.emplace_back(BT_PROPERTY_CLASS_OF_DEVICE, static_cast<int>(sizeof(cod)),
                          static_cast<void *>(&cod));
  ...
  properties.emplace_back(bt_property_t{BT_PROPERTY_TYPE_OF_DEVICE, sizeof(dev_type),
                          &dev_type});
  status = btif_storage_set_remote_device_property(&bdaddr, &properties.back());
  ...
  switch (event) {
    ...
    case BTA_DM_INQ_RES_EVT: {
...

      {
        ...
        /* RawAddress */
        bt_properties.emplace_back(BT_PROPERTY_BDADDR, sizeof(bdaddr), &bdaddr);

        /* BD_NAME */
        /* Don't send BDNAME if it is empty */
        if (bdname.name[0]) {
          bt_properties.emplace_back(
        ...
        if (cod != 0) {
          ...
          bt_properties.emplace_back(BT_PROPERTY_CLASS_OF_DEVICE, sizeof(cod),
                                     &cod);
        }
        ...
        bt_properties.emplace_back(BT_PROPERTY_TYPE_OF_DEVICE, sizeof(dev_type),
                                   &dev_type);
        /* RSSI */
        bt_properties.emplace_back(BT_PROPERTY_REMOTE_RSSI, sizeof(int8_t),
                                   &(p_search_data->inq_res.rssi));

        /* CSIP supported device */
        bt_properties.emplace_back(BT_PROPERTY_REMOTE_IS_COORDINATED_SET_MEMBER,
                                   sizeof(bool),
        ...
        bt_properties.emplace_back(BT_PROPERTY_REMOTE_ASHA_CAPABILITY,
                                   sizeof(int16_t), &asha_capability);

        bt_properties.emplace_back(BT_PROPERTY_REMOTE_ASHA_TRUNCATED_HISYNCID,
                                   sizeof(uint32_t),
                                   &asha_truncated_hi_sync_id);
        ...
        if (check_eir_appearance(p_search_data, &appearance)) {
          bt_properties.emplace_back(BT_PROPERTY_APPEARANCE, sizeof(appearance),
                                     &appearance);
        }
...
void btif_on_gatt_results(RawAddress bd_addr, BD_NAME bd_name,
  ...
  prop.emplace_back(BT_PROPERTY_UUIDS, Uuid::kNumBytes128 * uuids.size(),
                    (void*)property_value.data());
  ...
  if (!com::android::bluetooth::flags::
    ...
    prop.emplace_back(BT_PROPERTY_BDNAME, strnlen((char*)bd_name, BD_NAME_LEN),
                      bd_name);
