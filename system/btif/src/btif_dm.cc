  ...
  switch (event) {
    ...
    case BTA_DM_INQ_RES_EVT: {
...

      {
        ...
        // default to be not as a set follower
        bool is_set_follower = false;
        BTIF_STORAGE_FILL_PROPERTY(&properties[num_properties],
                                   BT_PROPERTY_REMOTE_IS_SET_FOLLOWER,
                                   sizeof(bool), &is_set_follower);
        num_properties++;
        ...
        while ((p_service_data = AdvertiseDataParser::GetFieldByType(
          ...
          if (uuid == 0xfdf0 /* ASHA service*/) {
            ...
            if (service_data_len < 8) {
              ...
              uint8_t is_ASHA_follower_device = (p_service_data[3] & 0x04) != 0;
              LOG_INFO("is_ASHA_follower_device: %d", is_ASHA_follower_device);
              if (is_ASHA_follower_device) {
                BTIF_STORAGE_FILL_PROPERTY(&properties[num_properties],
                                           BT_PROPERTY_REMOTE_IS_SET_FOLLOWER,
                                           sizeof(bool),
                                           &is_ASHA_follower_device);
              }
            }
            break;
          }
