...
static void btif_dm_cb_create_bond_with_addrtype(const RawAddress bd_addr,
...
/*******************************************************************************
 *
 * Function         btif_dm_cb_create_bond_with_addrtype
 *
 * Description      Create bond initiated with address type from the BTIF thread
      ...
      case HCI_ERR_LMP_RESPONSE_TIMEOUT:
        if (interop_match_addr(INTEROP_AUTO_RETRY_PAIRING, &bd_addr) &&
          ...
          if (addr_type == BLE_ADDR_RANDOM) {
            btif_dm_cb_create_bond_with_addrtype(bd_addr, addr_type,
                                                 BT_TRANSPORT_AUTO);
          } else {
...
void btif_dm_create_bond_with_addrtype(const RawAddress bd_addr,
  ...
  pairing_cb.timeout_retries = NUM_TIMEOUT_RETRIES;
  btif_dm_cb_create_bond_with_addrtype(bd_addr, addr_type, transport);
}
