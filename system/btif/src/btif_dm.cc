  ...
  switch (event) {
    case BTA_DM_DISC_RES_EVT: {
      ...
      */
      ...
      const tBTA_STATUS bta_status =
          static_cast<tBTA_STATUS>(p_data->disc_res.result);
      BTM_LogHistory(kBtmLogTagSdp, bd_addr, "Discovered services",
                     base::StringPrintf("bta_status:%s sdp_uuids:%zu eir_uuids:%zu", bta_status_text(bta_status).c_str(),
