  ...
  switch (event) {
    ...
    case BTA_DM_INQ_RES_EVT: {
...

      {
        ...
        // default to be not hidden from users
        bool is_hidden_from_user = false;
        BTIF_STORAGE_FILL_PROPERTY(&properties[num_properties],
                                   BT_PROPERTY_REMOTE_IS_HIDDEN_FROM_USER,
                                   sizeof(bool), &is_hidden_from_user);
        num_properties++;
        ...
        while ((p_service_data = AdvertiseDataParser::GetFieldByType(
          ...
          if (uuid == 0xfdf0 /* ASHA service*/) {
            ...
            if (service_data_len < 8) {
              ...
              if (hide_ASHA_device) {
                LOG_INFO("hide ASHA device");
                BTIF_STORAGE_FILL_PROPERTY(
                    &properties[num_properties],
                    BT_PROPERTY_REMOTE_IS_HIDDEN_FROM_USER, sizeof(bool),
                    &hide_ASHA_device);
              }
            }
            break;
