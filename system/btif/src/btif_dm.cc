...
static void btif_dm_update_whitelisted_media_players() {
    uint8_t num_wlplayers = 0;
    uint8_t i = 0, buf_len = 0;
    bt_property_t wlplayers_prop;
    list_t *wl_players = list_new(osi_free);
    if (!wl_players) {
       LOG_ERROR("%s: unable to allocate space for whitelist players",
                 __func__);
       return;
    }
    LOG_DEBUG("btif_dm_update_whitelisted_media_players");

    wlplayers_prop.len = 0;
    if (interop_get_whitelisted_media_players_list(&wl_players)) {
        num_wlplayers = list_length(wl_players);
        LOG_DEBUG("%s: %d - WL media players found", __func__, num_wlplayers);
        ...
        if (num_wlplayers > 0) {
            ...
            invoke_adapter_properties_cb(BT_STATUS_SUCCESS, 1, &wlplayers_prop);
        }
   } else {
        LOG_DEBUG("%s: Whitelisted media players not found", __func__);
   }
}
...
void BTIF_dm_enable() {
  ...
  */
  ...
  btif_dm_update_whitelisted_media_players();
