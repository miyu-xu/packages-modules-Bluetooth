  ...
  switch (event) {
    ...
    case BTA_DM_INQ_RES_EVT: {
...

      {
        ...
        // default to not hide in UI
        bool is_hide_in_ui = false;
        BTIF_STORAGE_FILL_PROPERTY(&properties[num_properties],
                                   BT_PROPERTY_REMOTE_IS_HIDDEN_IN_UI,
                                   sizeof(bool), &is_hide_in_ui);
        num_properties++;

        // based on ASHA advertising service data to decide whether hide in UI
        ...
        while ((p_service_data = AdvertiseDataParser::GetFieldByType(
          ...
          if (uuid == 0xfdf0 /* ASHA service*/) {
            ...
            for (uint i = 2; i < service_data_len; i++) {
              LOG_INFO("ASHA service data: %u", p_service_data[i]);
            }

            if (service_data_len < 8) {
              LOG_WARN("ASHA device service_data_len too short");
            } else {
              uint8_t hide_ASHA_device = (p_service_data[3] & 0x04) != 0;
              if (hide_ASHA_device) {
                LOG_INFO("hide_ASHA_device");
                BTIF_STORAGE_FILL_PROPERTY(&properties[num_properties],
                                           BT_PROPERTY_REMOTE_IS_HIDDEN_IN_UI,
                                           sizeof(bool), &hide_ASHA_device);
              }
              break;
            }
          }
