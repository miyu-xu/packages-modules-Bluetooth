...
static void btif_update_remote_version_property(RawAddress* p_bd) {
  ...
  const bool version_info_valid = BTM_ReadRemoteVersion(*p_bd, &lmp_ver, &mfct_set, &lmp_subver);
...
static void btif_dm_cb_create_bond(const RawAddress bd_addr, tBT_TRANSPORT transport) {
  ...
  if (!com::android::bluetooth::flags::connect_hid_after_service_discovery() && is_hid &&
      (device_type & BT_DEVICE_TYPE_BLE) == 0) {
    tAclLinkSpec link_spec;
    link_spec.addrt.bda = bd_addr;
    link_spec.addrt.type = addr_type;
    link_spec.transport = transport;
    const bt_status_t status =
            GetInterfaceToProfiles()->profileSpecific_HACK->btif_hh_connect(link_spec);
...
static void btif_dm_pin_req_evt(tBTA_DM_PIN_REQ* p_pin_req) {
  ...
  if (BTM_GetPeerDeviceTypeFromFeatures(p_pin_req->bd_addr) == BT_DEVICE_TYPE_DUMO) {
...
static void btif_dm_ssp_cfm_req_evt(tBTA_DM_SP_CFM_REQ* p_ssp_cfm_req) {
  ...
  log::verbose("addr:{}, just_works:{}, loc_auth_req={}, rmt_auth_req={}", p_ssp_cfm_req->bd_addr,
               p_ssp_cfm_req->just_works, p_ssp_cfm_req->loc_auth_req, p_ssp_cfm_req->rmt_auth_req);
  /* Remote properties update */
  if (BTM_GetPeerDeviceTypeFromFeatures(p_ssp_cfm_req->bd_addr) == BT_DEVICE_TYPE_DUMO) {
    dev_type = BT_DEVICE_TYPE_DUMO;
...
static void btif_dm_ssp_key_notif_evt(tBTA_DM_SP_KEY_NOTIF* p_ssp_key_notif) {
  ...
  /* Remote properties update */
  if (BTM_GetPeerDeviceTypeFromFeatures(p_ssp_key_notif->bd_addr) == BT_DEVICE_TYPE_DUMO) {
    dev_type = BT_DEVICE_TYPE_DUMO;
...
static bool btif_is_gatt_service_discovery_post_pairing(const RawAddress bd_addr) {
  if (!com::android::bluetooth::flags::reset_pairing_only_for_related_service_discovery()) {
    if (bd_addr == pairing_cb.bd_addr || bd_addr == pairing_cb.static_bdaddr) {
      if (pairing_cb.gatt_over_le != btif_dm_pairing_cb_t::ServiceDiscoveryState::SCHEDULED) {
        log::error(
...
void btif_on_gatt_results(RawAddress bd_addr, BD_NAME bd_name,
  ...
  if (!com::android::bluetooth::flags::separate_service_and_device_discovery() &&
    ...
    prop.push_back(bt_property_t{
            BT_PROPERTY_BDNAME, static_cast<int>(strnlen((char*)bd_name, BD_NAME_LEN)), bd_name });
...
void btif_dm_get_remote_services(RawAddress remote_addr, const int transport) {
  ...
  BTA_DmDiscover(remote_addr,
                 service_discovery_callbacks{ .on_gatt_results = btif_on_gatt_results,
