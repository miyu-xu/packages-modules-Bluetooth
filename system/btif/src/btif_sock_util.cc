...
int sock_send_all(int sock_fd, const uint8_t* buf, int len) {
  ...
  while (s) {
    ...
    if (ret <= 0) {
      LOG_ERROR("sock fd:%d send errno:%d, ret:%d", sock_fd, errno, ret);
      return -1;
    }
    buf += ret;
    s -= ret;
...
int sock_recv_all(int sock_fd, uint8_t* buf, int len) {
  ...
  while (r) {
    ...
    if (ret <= 0) {
      LOG_ERROR("sock fd:%d recv errno:%d, ret:%d", sock_fd, errno, ret);
      return -1;
    }
    buf += ret;
    r -= ret;
...
int sock_send_fd(int sock_fd, const uint8_t* buf, int len, int send_fd) {
  ...
  LOG_DEBUG("close fd:%d after sent", send_fd);
