...
static void btif_storage_load_bonded_hid_device(tAclLinkSpec link_spec,
                                                int db_version) {
  ...
  btif_hh_load_added_dev(link_spec, attr_mask, sub_class, app_id, dscp_info,
                         reconnect_allowed);
}

static void btif_storage_load_bonded_hogp_device(tAclLinkSpec link_spec) {
  auto name = link_spec.addrt.bda.ToString();
  int value;
  bool reconnect_allowed = true;
...
bt_status_t btif_storage_load_bonded_hid_info(void) {
  for (const auto& bd_addr : btif_config_get_paired_devices()) {
    ...
    if (db_version == 0) {
      btif_storage_load_bonded_hid_device(link_spec, db_version);
    } else {
      btif_storage_load_bonded_hid_device(link_spec, db_version);
