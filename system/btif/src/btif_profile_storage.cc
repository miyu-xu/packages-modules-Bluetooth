...
bt_status_t btif_storage_add_hid_device_info(
  ...
  btif_config_set_int(bdstr, BTIF_STORAGE_KEY_HID_S_S_R_MAX_LATENCY, ssr_max_latency);
  btif_config_set_int(bdstr, BTIF_STORAGE_KEY_HID_S_S_R_MIN_TIMEOUT, ssr_min_tout);
  if (dl_len > 0) btif_config_set_bin(bdstr, BTIF_STORAGE_KEY_HID_DESCRIPTOR, dsc_list, dl_len);
...
bt_status_t btif_storage_load_bonded_hid_info(void) {
  for (const auto& bd_addr : btif_config_get_paired_devices()) {
    ...
    int value;
    if (!btif_config_get_int(name, BTIF_STORAGE_KEY_HID_ATTR_MASK, &value)) continue;
...
void btif_storage_load_bonded_hearing_aids() {
  for (const auto& bd_addr : btif_config_get_paired_devices()) {
    ...
    uint16_t codecs = 0;
    if (btif_config_get_int(name, BTIF_STORAGE_KEY_HEARING_AID_CODECS, &value)) codecs = value;
...
void btif_storage_set_leaudio_autoconnect(const RawAddress& addr,
  ...
  do_in_jni_thread(FROM_HERE, Bind(
                                  [](const RawAddress& addr, bool autoconnect) {
                                    ...
                                    btif_config_set_int(
                                        bdstr, BTIF_STORAGE_KEY_LEAUDIO_AUTOCONNECT,
                                        autoconnect);
                                  },
                                  addr, autoconnect));
}
      ...
      Bind(
          [](const RawAddress& addr, int sink_location, int source_location) {
            ...
            LOG_DEBUG("saving le audio device: %s",
                      ADDRESS_TO_LOGGABLE_CSTR(addr));
            btif_config_set_int(bdstr, BTIF_STORAGE_KEY_LEAUDIO_SINK_AUDIOLOCATION,
                                sink_location);
...
void btif_storage_load_bonded_leaudio() {
  for (const auto& bd_addr : btif_config_get_paired_devices()) {
    ...
    if (btif_config_get_int(
            name, BTIF_STORAGE_KEY_LEAUDIO_SOURCE_SUPPORTED_CONTEXT_TYPE, &value))
    ...
    buffer_size =
        btif_config_get_bin_length(name, BTIF_STORAGE_KEY_LEAUDIO_SINK_PACS_BIN);
    ...
    buffer_size =
        btif_config_get_bin_length(name, BTIF_STORAGE_KEY_LEAUDIO_SOURCE_PACS_BIN);
  ...
  do_in_jni_thread(
      ...
      Bind(
          [](const RawAddress& address, std::vector<uint8_t> presets_bin,
            ...
            btif_config_set_int(name, BTIF_STORAGE_KEY_LEAUDIO_HAS_FEATURES, features);
            btif_config_set_int(name, BTIF_STORAGE_KEY_LEAUDIO_HAS_ACTIVE_PRESETS, active_preset);
  ...
  do_in_jni_thread(FROM_HERE,
                   Bind(
                       [](const RawAddress& address, uint8_t active_preset) {
                         ...
                         btif_config_set_int(name, BTIF_STORAGE_KEY_LEAUDIO_HAS_ACTIVE_PRESETS,
                                             active_preset);
                       },
...
bool btif_storage_get_leaudio_has_features(const RawAddress& address,
  ...
  int value;
  if (!btif_config_get_int(name, BTIF_STORAGE_KEY_LEAUDIO_HAS_FEATURES, &value)) return false;
...
void btif_storage_load_bonded_leaudio_has_devices() {
  for (const auto& bd_addr : btif_config_get_paired_devices()) {
    ...
    if (!btif_config_exist(name, BTIF_STORAGE_KEY_LEAUDIO_HAS_IS_ACCEPTLISTED) &&
        !btif_config_exist(name, BTIF_STORAGE_KEY_LEAUDIO_HAS_FEATURES))
      continue;
    ...
    uint8_t features = 0;
    if (btif_config_get_int(name, BTIF_STORAGE_KEY_LEAUDIO_HAS_FEATURES, &value)) features = value;
...
void btif_storage_remove_leaudio_has(const RawAddress& address) {
  ...
  btif_config_remove(addrstr, BTIF_STORAGE_KEY_LEAUDIO_HAS_FEATURES);
  btif_config_remove(addrstr, BTIF_STORAGE_KEY_LEAUDIO_HAS_ACTIVE_PRESETS);
...
bool btif_storage_get_leaudio_has_presets(const RawAddress& address,
  ...
  int value;
  if (!btif_config_get_int(name, BTIF_STORAGE_KEY_LEAUDIO_HAS_ACTIVE_PRESETS, &value)) return false;
  active_preset = value;

  auto bin_sz = btif_config_get_bin_length(name, BTIF_STORAGE_KEY_LEAUDIO_HAS_SERIALIZED_PRESETS);
  presets_bin.resize(bin_sz);
  if (!btif_config_get_bin(name, BTIF_STORAGE_KEY_LEAUDIO_HAS_SERIALIZED_PRESETS, presets_bin.data(),
                           &bin_sz))
    return false;
  ...
  do_in_jni_thread(FROM_HERE, Bind(
                                  [](const RawAddress& addr, bool autoconnect) {
                                    ...
                                    VLOG(2) << "Storing CSIS device: "
                                            << ADDRESS_TO_LOGGABLE_CSTR(addr);
                                    btif_config_set_int(
                                        bdstr, BTIF_STORAGE_KEY_CSIS_AUTOCONNECT,
                                        autoconnect);
                                  },
                                  addr, autoconnect));
}
