...
bt_status_t btif_storage_add_hid_device_info(
  ...
  if (!IS_FLAG_ENABLED(allow_switching_hid_and_hogp) ||
    ...
    uint8_t dbVersion = 1;
    btif_config_set_int(bdstr, BTIF_STORAGE_KEY_HID_DB_VERSION, dbVersion);
    if (dl_len > 0)
...
bt_status_t btif_storage_set_hid_connection_policy(
    const tAclLinkSpec* link_spec, bool reconnect_allow) {
  std::string bdstr = link_spec->addrt.bda.ToString();
  // Connection allowed Key is stored with value:
  // Y:yes
  // N:no
  // E:empty
  std::string policy = reconnect_allow ? "Y" : "N";

  if (link_spec->transport == BT_TRANSPORT_LE) {
    btif_config_set_str(bdstr, BTIF_STORAGE_KEY_HOGP_CONNECTION_ALLOWED,
                        policy);
    btif_config_set_str(bdstr, BTIF_STORAGE_KEY_HID_CONNECTION_ALLOWED, "E");
  } else if (link_spec->transport == BT_TRANSPORT_BR_EDR){
    btif_config_set_str(bdstr, BTIF_STORAGE_KEY_HID_CONNECTION_ALLOWED, policy);
    btif_config_set_str(bdstr, BTIF_STORAGE_KEY_HOGP_CONNECTION_ALLOWED, "E");
  } else {
...
bt_status_t btif_storage_get_hid_connection_policy(
    const tAclLinkSpec* link_spec, bool* hogp_preferred,
    bool* reconnect_allow) {
  std::string bdstr = link_spec->addrt.bda.ToString();
  *hogp_preferred = false;
  *reconnect_allow = false;
  ...
  // N:no
  // E:empty
  btif_config_get_str(bdstr, BTIF_STORAGE_KEY_HOGP_CONNECTION_ALLOWED, key_value,
                      &key_size);
  if (*key_value == 'Y') {
    *reconnect_allow = true;
    *hogp_preferred = true;
  } else if (*key_value == 'N') {
    *hogp_preferred = true;
  }
  if (!*hogp_preferred) {
    btif_config_get_str(bdstr, BTIF_STORAGE_KEY_HID_CONNECTION_ALLOWED, key_value,
                        &key_size);

    *reconnect_allow = (*key_value == 'Y') ? true : false;
  }
...
bt_status_t btif_storage_clear_hid_connection_policy(
  ...
  btif_config_remove(bdstr, BTIF_STORAGE_KEY_HID_CONNECTION_ALLOWED);
  btif_config_remove(bdstr, BTIF_STORAGE_KEY_HOGP_CONNECTION_ALLOWED);
  return BT_STATUS_SUCCESS;
}
