...
bt_status_t btif_storage_get_hid_connection_policy(
  ...
  *reconnect_allow = false;
  ...
  btif_config_get_str(bdstr, BTIF_STORAGE_KEY_HOGP_CONNECTION_ALLOWED, policy,
                      &size);
  if (policy[0] == 'Y') {
  ...
  if (!*hogp_preferred) {
    btif_config_get_str(bdstr, BTIF_STORAGE_KEY_HID_CONNECTION_ALLOWED, policy,
                        &size);

    *reconnect_allow = (policy[0] == 'Y') ? true : false;
  }
