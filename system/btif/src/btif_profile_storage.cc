...
bt_status_t btif_storage_add_hid_device_info(
  ...
  btif_config_set_int(bdstr, BTIF_STORAGE_KEY_HID_S_S_R_MAX_LATENCY,
                      ssr_max_latency);
  btif_config_set_int(bdstr, BTIF_STORAGE_KEY_HID_S_S_R_MIN_TIMEOUT,
                      ssr_min_tout);
  if (dl_len > 0)
...
void btif_storage_load_bonded_leaudio() {
  for (const auto& bd_addr : btif_config_get_paired_devices()) {
    ...
    *if (btif_config_get_int(name, BTIF_STORAGE_KEY_LEAUDIO_SINK_AUDIOLOCATION,
                             &value)) sink_audio_location = value;
  ...
  do_in_jni_thread(
      ...
      Bind(
          [](const RawAddress& address, std::vector<uint8_t> presets_bin,
            ...
            btif_config_set_int(name, BTIF_STORAGE_KEY_LEAUDIO_HAS_FEATURES,
                                features);
            btif_config_set_int(name,
                                BTIF_STORAGE_KEY_LEAUDIO_HAS_ACTIVE_PRESETS,
                                active_preset);
  ...
  do_in_jni_thread(FROM_HERE,
                   Bind(
                       [](const RawAddress& address, uint8_t active_preset) {
                         ...
                         btif_config_set_int(
                             name, BTIF_STORAGE_KEY_LEAUDIO_HAS_ACTIVE_PRESETS,
                             active_preset);
                       },
...
bool btif_storage_get_leaudio_has_features(const RawAddress& address,
  ...
  int value;
  if (!btif_config_get_int(name, BTIF_STORAGE_KEY_LEAUDIO_HAS_FEATURES, &value))
    return false;
  ...
  do_in_jni_thread(FROM_HERE,
                   Bind(
                       [](const RawAddress& address, uint8_t features) {
                         ...
                         btif_config_set_int(
                             name, BTIF_STORAGE_KEY_LEAUDIO_HAS_FEATURES,
                             features);
                       },
                       address, features));
}
...
void btif_storage_load_bonded_leaudio_has_devices() {
  for (const auto& bd_addr : btif_config_get_paired_devices()) {
    ...
    uint8_t features = 0;
    if (btif_config_get_int(name, BTIF_STORAGE_KEY_LEAUDIO_HAS_FEATURES,
                            &value))
      features = value;
...
bool btif_storage_get_leaudio_has_presets(const RawAddress& address,
  ...
  int value;
  if (!btif_config_get_int(name, BTIF_STORAGE_KEY_LEAUDIO_HAS_ACTIVE_PRESETS,
                           &value))
    return false;
  active_preset = value;
