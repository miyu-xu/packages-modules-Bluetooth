...
void btif_a2dp_sink_update_decoder(const uint8_t* p_codec_info) {
  ...
  LOG_INFO("%s: p_codec_info[%x:%x:%x:%x:%x:%x]", __func__, p_codec_info[1],
...
static void btif_a2dp_sink_handle_inc_media(BT_HDR* p_msg) {
  if ((btif_av_get_peer_sep() == AVDT_TSEP_SNK) ||
    ...
    LOG_DEBUG("%s: state changed happened in this tick", __func__);
    return;
  }
...
static void btif_a2dp_sink_avk_handle_timer() {
  ...
  LOG_DEBUG("%s: process frames begin", __func__);
  while (true) {
    p_msg = (BT_HDR*)fixed_queue_try_dequeue(btif_a2dp_sink_cb.rx_audio_queue);
    if (p_msg == NULL) {
      break;
    }
    LOG_DEBUG("%s: number of packets in queue %zu", __func__,
  ...
  LOG_DEBUG("%s: process frames end", __func__);
}
...
static void btif_a2dp_sink_decoder_update_event(
  ...
  LOG_INFO("%s", __func__);
  LockGuard lock(g_mutex);
  LOG_DEBUG("%s: p_codec_info[%x:%x:%x:%x:%x:%x]", __func__,
  ...
  LOG_DEBUG("%s: reset to Sink role", __func__);
  ...
  LOG_DEBUG("%s: create audio track", __func__);
...
uint8_t btif_a2dp_sink_enqueue_buf(BT_HDR* p_pkt) {
  ...
  if (btif_a2dp_sink_cb.decode_alarm == nullptr &&
    ...
    LOG_DEBUG("%s: Initiate decoding. Current focus state:%d", __func__,
...
static void btif_a2dp_sink_set_focus_state_event(
  ...
  LOG_DEBUG("%s: setting focus state to %d", __func__, state);
