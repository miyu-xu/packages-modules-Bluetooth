...
static void cleanup_rfc_slot(rfc_slot_t* slot) {
  if (slot->fd != INVALID_FD) {
    ...
    log::info(
        ...
        slot->role ? SOCKET_ROLE_LISTEN : SOCKET_ROLE_CONNECTION,
...
static void on_srv_rfc_listen_started(tBTA_JV_RFCOMM_START* p_start,
  ...
  log::info(
      "address={}, state={}, role={}, server_name={}, scn={}",
      ADDRESS_TO_LOGGABLE_CSTR(slot->addr), SOCKET_CONNECTION_STATE_LISTENING,
      slot->role ? SOCKET_ROLE_LISTEN : SOCKET_ROLE_CONNECTION,
      slot->role ? slot->service_name : slot->service_uuid.ToString().c_str(),
      slot->scn);
  btif_sock_connection_logger(
...
static uint32_t on_srv_rfc_connect(tBTA_JV_RFCOMM_SRV_OPEN* p_open,
  ...
  log::info("address={}, state={}, role={}, server_name={}, scn={}",
            ADDRESS_TO_LOGGABLE_CSTR(accept_rs->addr),
            (int)SOCKET_CONNECTION_STATE_CONNECTED,
            (int)(slot->role ? SOCKET_ROLE_LISTEN : SOCKET_ROLE_CONNECTION),
            accept_rs->role ? accept_rs->service_name
                            : accept_rs->service_uuid.ToString().c_str(),
            accept_rs->scn);
  btif_sock_connection_logger(
...
static void on_cli_rfc_connect(tBTA_JV_RFCOMM_OPEN* p_open, uint32_t id) {
  ...
  log::info(
      "address={}, state={}, role={}, server_name={}, scn={}",
      ADDRESS_TO_LOGGABLE_CSTR(slot->addr), SOCKET_CONNECTION_STATE_CONNECTED,
      slot->role ? SOCKET_ROLE_LISTEN : SOCKET_ROLE_CONNECTION,
      slot->role ? slot->service_name : slot->service_uuid.ToString().c_str(),
      slot->scn);
  btif_sock_connection_logger(
