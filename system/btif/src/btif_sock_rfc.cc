...
typedef struct {
  ...
  bool offload;         // is offload purpose?
} rfc_slot_t;
...
static rfc_slot_t* create_srv_accept_rfc_slot(rfc_slot_t* srv_rs, const RawAddress* addr,
  ...
  accept_rs->reg_id = srv_rs->reg_id;
  accept_rs->offload = srv_rs->offload;
...
bt_status_t btsock_rfc_listen(const char* service_name, const Uuid* service_uuid, int channel,
                              int* sock_fd, int flags, int app_uid, int reg_id, bool offload) {
...
static void cleanup_rfc_slot(rfc_slot_t* slot) {
  if (slot->fd != INVALID_FD) {
    ...
    log::info(
      ...
      GetInterfaceToProfiles()->events->invoke_socket_state_changed_cb(slot->reg_id, slot->conn_uuid,
                                                                    BT_STATUS_SUCCESS,
                                                                    slot->f.server ? SOCKET_ROLE_LISTEN : SOCKET_ROLE_CONNECTION,
                                                                    SOCKET_CONNECTION_STATE_DISCONNECTED,
                                                                    BTSOCK_RFCOMM,
                                                                    0, 0, 0, 0, 0, 0, 0, 0, 0, slot->offload);
...
static bool send_app_connect_signal(int fd, const RawAddress* addr, int channel, int status,
                                    int send_fd, const Uuid& conn_uuid) {
...
static uint32_t on_srv_rfc_connect(tBTA_JV_RFCOMM_SRV_OPEN* p_open, uint32_t id) {
  ...
  GetInterfaceToProfiles()->events->invoke_socket_state_changed_cb(accept_rs->reg_id, accept_rs->conn_uuid,
                                                                   BT_STATUS_SUCCESS,
                                                                   ...
                                                                   p_open->tx_mtu,
                                                                   0, 0,
                                                                   p_open->local_credit, p_open->remote_credit,
                                                                   p_open->local_cid, p_open->remote_cid,
                                                                   p_open->acl_handle,
                                                                   accept_rs->offload);
  ...
  send_app_connect_signal(srv_rs->fd, &accept_rs->addr, srv_rs->scn, 0, accept_rs->app_fd, accept_rs->conn_uuid);
...
static void on_cli_rfc_connect(tBTA_JV_RFCOMM_OPEN* p_open, uint32_t id) {
  ...
  if (send_app_connect_signal(slot->fd, &slot->addr, slot->scn, 0, -1, slot->conn_uuid)) {
