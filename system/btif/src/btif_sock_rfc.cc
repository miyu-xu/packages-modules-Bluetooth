...
static void cleanup_rfc_slot(rfc_slot_t* slot) {
  if (slot->fd != INVALID_FD) {
    ...
    btif_sock_connection_logger(
            ...
            slot->connection_start_time_ms, slot->socket_error_code, slot->data_path, slot->hub_id,
            slot->endpoint_id);
...
static void on_srv_rfc_listen_started(tBTA_JV_RFCOMM_START* p_start, uint32_t id) {
  ...
  btif_sock_connection_logger(slot->addr, slot->id, BTSOCK_RFCOMM,
                              ...
                              slot->app_uid, slot->scn, 0, 0, slot->service_name, 0,
                              BTSOCK_ERROR_NONE, slot->data_path, slot->hub_id, slot->endpoint_id);
}
...
static uint32_t on_srv_rfc_connect_offload(tBTA_JV_RFCOMM_SRV_OPEN* p_open, uint32_t id,
  ...
  btif_sock_connection_logger(
          ...
          accept_rs->scn, 0, 0, accept_rs->service_name, 0, BTSOCK_ERROR_NONE, accept_rs->data_path,
          accept_rs->hub_id, accept_rs->endpoint_id);
