...
static bool btsock_rfc_get_offload_mtu(int* rx_mtu, int app_max_rx_packet_size) {
  ...
  *sock_fd = slot->app_fd;  // Transfer ownership of fd to caller.
  ...
  if (com::android::bluetooth::flags::socket_settings_api()) {
    ...
    if (data_path == BTSOCK_DATA_PATH_HARDWARE_OFFLOAD) {
      if (!btsock_rfc_get_offload_mtu(&slot->mtu, max_rx_packet_size)) {
...
bool is_rfc_socket_found(uint64_t socket_id) {
  std::unique_lock<std::recursive_mutex> lock(slot_lock);
...
void on_btsocket_rfc_opened_complete(uint64_t socket_id, bool success) {
  ...
  if (slot->listen_fd != -1) {
    ...
    log::info(
            "Connected to RFCOMM connection for device: {}, channel: {}, app_uid: {}, id: {}, "
            "socket_id: {}",
            slot->addr, slot->scn, slot->app_uid, slot->id, slot->socket_id);
    btif_sock_connection_logger(slot->addr, slot->id, BTSOCK_RFCOMM,
    ...
    case BTA_JV_CREATE_RECORD_EVT: {
      ...
      if (com::android::bluetooth::flags::socket_settings_api()) {  // Added with aosp/3349376
        if (slot->data_path == BTSOCK_DATA_PATH_HARDWARE_OFFLOAD) {
          ...
          cfg->rx_mtu = (slot->mtu > 0) ? slot->mtu : 0;
        }
      }
...
static void handle_discovery_comp(tBTA_JV_STATUS status, int scn, uint32_t id) {
  ...
  if (BTA_JvRfcommConnect(slot->security, scn, slot->addr, rfcomm_cback, slot->id,
                          std::move(cfg)) != tBTA_JV_STATUS::SUCCESS) {
    log::warn(
...
bool btsock_rfc_read_signaled_on_connected_socket(int /* fd */, int flags, uint32_t /* id */,
                                                  rfc_slot_t* slot) {
  if (!slot->f.connected) {
...
bool btsock_rfc_read_signaled_on_listen_socket(int fd, int /* flags */, uint32_t /* id */,
                                               rfc_slot_t* slot) {
  int size = 0;
...
void btsock_rfc_signaled_flagged(int fd, int flags, uint32_t id) {
