 ...
 ******************************************************************************/
void btif_pan_init() {
  LOG_DEBUG("jni_initialized = %d, btpan_cb.enabled:%d", jni_initialized,
            btpan_cb.enabled);
  stack_initialized = true;

  if (jni_initialized && !btpan_cb.enabled) {
    LOG_DEBUG("Enabling PAN....");
...
static bt_status_t btpan_jni_init(const btpan_callbacks_t* callbacks) {
  LOG_DEBUG("stack_initialized = %d, btpan_cb.enabled:%d", stack_initialized,
            btpan_cb.enabled);
  callback = *callbacks;
...
static void btif_in_pan_generic_evt(uint16_t event, char* p_param) {
  LOG_INFO("%s: event=%d", __func__, event);
  switch (event) {
...
static int tap_if_up(const char* devname, const RawAddress* addr) {
  ...
  close(sk);
  LOG_DEBUG("network interface: %s is up", devname);
  return 0;
}
...
int btpan_tap_open() {
  ...
  if (fd < 0) {
    LOG_DEBUG("could not open %s, err:%d", clonedev, errno);
    return fd;
  }
  ...
  if (err < 0) {
    LOG_DEBUG("ioctl error:%d, errno:%s", err, strerror(errno));
    close(fd);
    return err;
  }
...
static void btpan_close_conn(btpan_conn_t* conn) {
  LOG_INFO("btpan_close_conn: %p", conn);

  if (conn && conn->state == PAN_STATE_OPEN) {
    LOG_DEBUG("btpan_close_conn: PAN_STATE_OPEN");
...
void btpan_close_handle(btpan_conn_t* p) {
  LOG_DEBUG("btpan_close_handle : close handle %d", p->handle);
  p->handle = -1;
  p->local_role = -1;
  p->remote_role = -1;
  memset(&p->peer, 0, 6);
...
static inline bool should_forward(tETH_HDR* hdr) {
  ...
  if (proto == ETH_P_IP || proto == ETH_P_ARP || proto == ETH_P_IPV6)
    return true;
  LOG_DEBUG("unknown proto:%x", proto);
  return false;
}
...
static void bta_pan_callback_transfer(uint16_t event, char* p_param) {
  ...
  switch (event) {
    ...
    case BTA_PAN_OPENING_EVT: {
      btpan_conn_t* conn;
      LOG_DEBUG("BTA_PAN_OPENING_EVT handle %d, addr: %s",
