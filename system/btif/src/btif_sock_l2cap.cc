...
static void btsock_l2cap_free_l(l2cap_socket* sock) {
  ...
  log::info(
    ...
    GetInterfaceToProfiles()->events->invoke_socket_state_changed_cb(sock->reg_id, sock->conn_uuid,
                                                                    BT_STATUS_SUCCESS,
                                                                    sock->server ? SOCKET_ROLE_LISTEN : SOCKET_ROLE_CONNECTION,
                                                                    SOCKET_CONNECTION_STATE_DISCONNECTED,
                                                                    sock->is_le_coc ? BTSOCK_L2CAP_LE : BTSOCK_L2CAP,
                                                                    0, 0, 0, 0, 0, 0, 0, 0, 0, sock->offload);
...
static void on_srv_l2cap_psm_connect_l(tBTA_JV_L2CAP_OPEN* p_open, l2cap_socket* sock) {
  ...
  accept_rs->offload = sock->offload;
  ...
  GetInterfaceToProfiles()->events->invoke_socket_state_changed_cb(accept_rs->reg_id, accept_rs->conn_uuid,
                                                                   ...
                                                                   accept_rs->channel,
                                                                   p_open->tx_mtu,
                                                                   p_open->local_coc_mps, p_open->remote_coc_mps,
...
bt_status_t btsock_l2cap_listen(const char* name, int channel, int* sock_fd, int flags,
                                int app_uid, int reg_id, bool offload) {
  return btsock_l2cap_listen_or_connect(name, NULL, channel, sock_fd, flags, 1, app_uid, reg_id, offload);
}

bt_status_t btsock_l2cap_connect(const RawAddress* bd_addr, int channel, int* sock_fd, int flags,
                                 int app_uid, int reg_id, bool offload) {
  return btsock_l2cap_listen_or_connect(NULL, bd_addr, channel, sock_fd, flags, 0, app_uid, reg_id, offload);
}
