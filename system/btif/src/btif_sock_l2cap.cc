...
static bool btsock_l2cap_get_offload_mtu(uint16_t* rx_mtu, uint16_t app_max_rx_packet_size) {
  ...
  uint16_t hal_mtu = static_cast<uint16_t>(socket_cap.leCocCapabilities.mtu);
  ...
  if (flags & SOCK_THREAD_FD_RD) {
    if (!sock->server) {
      ...
      if (ioctl_success && size) {
        ...
        ssize_t count;
        OSI_NO_INTR(count = recv(fd, reinterpret_cast<uint8_t*>(&accept_signal),
                                 sizeof(accept_signal), MSG_NOSIGNAL | MSG_DONTWAIT | MSG_TRUNC));
