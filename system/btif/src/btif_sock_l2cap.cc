...
static void btsock_l2cap_free_l(l2cap_socket* sock) {
  ...
  if (sock->data_path_in_use == SOCKET_DATA_PATH_SOFTWARE_OFFLOAD ||
    ...
    bluetooth::hci::SocketContext socket_context = {
            ...
            .state = bluetooth::hci::SocketConnectionState::ST_DISCONNECTED,
  ...
  if (accept_rs->data_path == SOCKET_DATA_PATH_SOFTWARE_OFFLOAD ||
    ...
    if (accept_rs->is_auto_switch) {
      bluetooth::hci::SocketContext socket_context = {
              ...
              .state = bluetooth::hci::SocketConnectionState::ST_CONNECTED,
...
static bool get_rx_mtu_for_le_coc_offload(hci::SocketDataPath data_path, uint16_t* rx_mtu) {
  ...
  for (const auto& prop : socket_prop.protocolProperties) {
