...
static int pth = -1;
static tL2CAP_CFG_INFO l2cap_etm_cfg{.fcr_present = true, .fcr = kDefaultErtmOptions};
...
static bt_status_t btsock_l2cap_listen_or_connect(const char* name,
  ...
  if (listen) {
    btsock_l2cap_server_listen(sock);
      ...
      /* Setup ETM settings: mtu will be set below */
      std::unique_ptr<tL2CAP_CFG_INFO> cfg = std::make_unique<tL2CAP_CFG_INFO>(l2cap_etm_cfg);
...
bt_status_t btsock_l2cap_set_etm_default_cfg(bt_l2cap_mode mode, bool mandatory) {
  ...
  switch (mode) {
    case BT_L2CAP_FCR_BASIC_MODE:
      l2cap_etm_cfg = {.fcr_present = mandatory, .fcr = kDefaultBasicOptions};
      status = BT_STATUS_SUCCESS;
      break;
    case BT_L2CAP_FCR_ERTM_MODE:
      l2cap_etm_cfg = {.fcr_present = mandatory, .fcr = kDefaultErtmOptions};
      status = BT_STATUS_SUCCESS;
      break;
    case BT_L2CAP_FCR_LE_COC_MODE:
      l2cap_etm_cfg = {.fcr_present = mandatory, .fcr = kDefaultLECOCOptions};
      status = BT_STATUS_SUCCESS;
      break;
    default:
