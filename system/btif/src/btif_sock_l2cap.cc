...
static void btsock_l2cap_server_listen(l2cap_socket* sock) {
  ...
  if (sock->data_path == BTSOCK_DATA_PATH_HW_OFFLOAD) {
...
static bool btsock_l2cap_get_offload_mtu(uint16_t* rx_mtu, uint16_t app_max_rx_packet_size) {
  ...
  uint16_t hal_mtu = static_cast<uint16_t>(socket_cap.le_coc_capabilities.mtu);
  if (hal_mtu < L2CAP_SDU_LENGTH_LE_MIN || hal_mtu > L2CAP_SDU_LENGTH_LE_MAX) {
    log::error("Invalid hal mtu: {}", hal_mtu);
    return false;
  }
  uint16_t mtu = std::min(hal_mtu, app_max_rx_packet_size);
...
static bt_status_t btsock_l2cap_listen_or_connect(const char* name, const RawAddress* addr,
  ...
  if (data_path == BTSOCK_DATA_PATH_HW_OFFLOAD) {
...
// std::mutex locked by caller
bool btsock_l2cap_read_signaled_on_connection_socket(int fd, int flags, uint32_t user_id,
                                                     l2cap_socket* sock) {
  if (!sock->connected) {
    return false;
  }
...
void btsock_l2cap_signaled(int fd, int flags, uint32_t user_id) {
  ...
  if (flags & SOCK_THREAD_FD_RD) {
    if (!sock->server) {
      ...
      if (!btsock_l2cap_read_signaled_on_connection_socket(fd, flags, user_id, sock)) {
        drop_it = true;
      }
    } else {
...
static void on_cl_l2cap_psm_connect_offload_l(tBTA_JV_L2CAP_OPEN* p_open, l2cap_socket* sock) {
  ...
  bluetooth::hal::SocketContext socket_context = {
          ...
          .protocol = bluetooth::hal::SocketProtocol::LE_COC,
          .channel_info.le_coc_channel_info.local_cid = p_open->local_cid,
          .channel_info.le_coc_channel_info.remote_cid = p_open->remote_cid,
          .channel_info.le_coc_channel_info.psm = static_cast<uint16_t>(sock->channel),
          .channel_info.le_coc_channel_info.local_mtu = sock->rx_mtu,
          .channel_info.le_coc_channel_info.remote_mtu = sock->tx_mtu,
          .channel_info.le_coc_channel_info.local_mps = p_open->local_coc_mps,
          .channel_info.le_coc_channel_info.remote_mps = p_open->remote_coc_mps,
          .channel_info.le_coc_channel_info.initial_rx_credits = p_open->local_coc_credit,
          .channel_info.le_coc_channel_info.initial_tx_credits = p_open->remote_coc_credit,
...
static void on_srv_l2cap_psm_connect_offload_l(tBTA_JV_L2CAP_OPEN* p_open, l2cap_socket* sock) {
  ...
  bluetooth::hal::SocketContext socket_context = {
          ...
          .channel_info.le_coc_channel_info.local_cid = p_open->local_cid,
          .channel_info.le_coc_channel_info.remote_cid = p_open->remote_cid,
          .channel_info.le_coc_channel_info.psm = static_cast<uint16_t>(accept_rs->channel),
          .channel_info.le_coc_channel_info.local_mtu = accept_rs->rx_mtu,
          .channel_info.le_coc_channel_info.remote_mtu = accept_rs->tx_mtu,
          .channel_info.le_coc_channel_info.local_mps = p_open->local_coc_mps,
          .channel_info.le_coc_channel_info.remote_mps = p_open->remote_coc_mps,
          .channel_info.le_coc_channel_info.initial_rx_credits = p_open->local_coc_credit,
          .channel_info.le_coc_channel_info.initial_tx_credits = p_open->remote_coc_credit,
