...
static bt_status_t btsock_l2cap_listen_or_connect(const char* name, const RawAddress* addr,
  ...
  if (socket_name) {
    int name_length = (strlen(socket_name) > strlen(sock->socket_name) - 1)
                              ? strlen(sock->socket_name) - 1
                              : strlen(socket_name);
    strncpy(sock->socket_name, socket_name, name_length);
    sock->socket_name[name_length] = '\0';
  }
  sock->hub_id = hub_id;
