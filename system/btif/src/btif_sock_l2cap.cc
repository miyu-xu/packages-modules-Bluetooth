...
typedef struct l2cap_socket {
  ...
  int listen_our_fd;          // listen socket fd from our side
  ...
  uint64_t connection_id;  // socket connection ID
  int data_path;           // socket data path
...
static uint64_t btif_l2cap_sock_generate_connection_id();
static void on_l2cap_disconnect_notify_offload(uint64_t connection_id);
...
static void btsock_l2cap_free_l(l2cap_socket* sock) {
  ...
  uint64_t connection_id = sock->connection_id;
  log::info(
          "Disconnected L2CAP connection for device: {}, channel: {}, app_uid: {}, "
          "id: {}, is_le: {}, connection_id: {}",
          sock->addr, sock->channel, sock->app_uid, sock->id, sock->is_le_coc, sock->connection_id);
  ...
  osi_free(sock);
  on_l2cap_disconnect_notify_offload(connection_id);
}
...
static l2cap_socket* btsock_l2cap_alloc_l(const char* name, const RawAddress* addr, char is_server,
  ...
  sock->connection_id = 0;
  sock->data_path = SOCKET_DATA_PATH_NO_OFFLOAD;
  sock->hub_id = 0;
  sock->endpoint_id = 0;
...
static bool send_app_connect_signal(int fd, const RawAddress* addr, int channel, int status,
                                    ...
                                    const Uuid& conn_uuid, uint64_t connection_id) {
  ...
  cs.connection_id = connection_id;
  if (send_fd != -1) {
...
static void on_l2cap_disconnect_notify_offload(uint64_t connection_id) {
  bluetooth::shim::GetLppOffloadManager()->SocketClosed(connection_id);
}

static void on_srv_l2cap_connect_notify_offload(tBTA_JV_L2CAP_OPEN* p_open, l2cap_socket* sock,
                                                l2cap_socket* accept_rs) {
  bluetooth::hci::SocketContext socket_context = {
          .socketId = accept_rs->connection_id,
          .name = accept_rs->socket_name,
          .aclConnectionHandle = p_open->acl_handle,
          .protocol = bluetooth::hci::SocketProtocol::LE_COC,
          .channelInfo.leCocChannelInfo.localCid = p_open->local_cid,
          .channelInfo.leCocChannelInfo.remoteCid = p_open->remote_cid,
          ...
          .channelInfo.leCocChannelInfo.initialRxCredits = p_open->local_coc_credit,
          .channelInfo.leCocChannelInfo.initialTxCredits = p_open->remote_coc_credit,
...
static void on_srv_l2cap_psm_connect_l(tBTA_JV_L2CAP_OPEN* p_open, l2cap_socket* sock) {
  ...
  accept_rs->rx_mtu = sock->rx_mtu = p_open->rx_mtu;
  ...
  accept_rs->connection_id = btif_l2cap_sock_generate_connection_id();
  ...
  accept_rs->listen_our_fd = sock->our_fd;
  ...
  log::info(
          ...
          "id: {}, is_le: {}, connection_id: {}, rx_mtu: {}",
          sock->addr, sock->channel, sock->app_uid, sock->id, sock->is_le_coc,
          accept_rs->connection_id, accept_rs->rx_mtu);
  ...
  if (com::android::bluetooth::flags::bt_offload_socket_api() &&
      sock->data_path == SOCKET_DATA_PATH_HW_OFFLOAD) {
    on_srv_l2cap_connect_notify_offload(p_open, sock, accept_rs);
  } else {
    // start monitor the socket
    btsock_thread_add_fd(pth, sock->our_fd, BTSOCK_L2CAP, SOCK_THREAD_FD_EXCEPTION, sock->id);
...
static void on_cl_l2cap_connect_notify_offload(tBTA_JV_L2CAP_OPEN* p_open, l2cap_socket* sock) {
  bluetooth::hci::SocketContext socket_context = {
          .socketId = sock->connection_id,
...
static void on_cl_l2cap_psm_connect_l(tBTA_JV_L2CAP_OPEN* p_open, l2cap_socket* sock) {
  ...
  Uuid uuid = Uuid::From128BitBE(bluetooth::os::GenerateRandom<Uuid::kNumBytes128>());
  sock->conn_uuid = uuid;
  sock->connection_id = btif_l2cap_sock_generate_connection_id();

  if (com::android::bluetooth::flags::bt_offload_socket_api() &&
      sock->data_path == SOCKET_DATA_PATH_HW_OFFLOAD) {
    on_cl_l2cap_connect_notify_offload(p_open, sock);
  } else {
    if (!send_app_psm_or_chan_l(sock)) {
    ...
    if (!send_app_connect_signal(sock->our_fd, &sock->addr, sock->channel, 0, -1, sock->rx_mtu,
                                 p_open->tx_mtu, sock->conn_uuid, sock->connection_id)) {
    ...
    log::info(
            ...
            "id: {}, is_le: {}, connection_id: {}, rx_mtu: {}",
            sock->addr, sock->channel, sock->app_uid, sock->id, sock->is_le_coc,
            sock->connection_id, sock->rx_mtu);
    btif_sock_connection_logger(sock->addr, sock->id,
                                sock->is_le_coc ? BTSOCK_L2CAP_LE : BTSOCK_L2CAP,
                                SOCKET_CONNECTION_STATE_CONNECTED,
                                sock->server ? SOCKET_ROLE_LISTEN : SOCKET_ROLE_CONNECTION,
...
static void btsock_l2cap_server_listen(l2cap_socket* sock) {
  ...
  if (sock->data_path == SOCKET_DATA_PATH_HW_OFFLOAD) {
...
static bool btsock_l2cap_get_lpp_rx_mtu(uint16_t* rx_mtu, uint16_t app_max_rx_packet_size) {
  hci::SocketCapabilities socket_cap;
  bluetooth::shim::GetLppOffloadManager()->GetSocketCapabilities(&socket_cap);
  if (!socket_cap.leCocCapabilities.numberOfSupportedSockets) {
    return false;
  }
  uint16_t mtu = std::min(static_cast<uint16_t>(socket_cap.leCocCapabilities.mtu),
                          L2CAP_SDU_LENGTH_LE_MAX);
  mtu = std::min(mtu, app_max_rx_packet_size);
  mtu = std::max(mtu, L2CAP_SDU_LENGTH_LE_MIN);
  *rx_mtu = mtu;
  return true;
}
...
static bt_status_t btsock_l2cap_listen_or_connect(const char* name, const RawAddress* addr,
  ...
  if (is_le_coc) {
    if (data_path == SOCKET_DATA_PATH_HW_OFFLOAD) {
      uint16_t lpp_rx_mtu;
      if (btsock_l2cap_get_lpp_rx_mtu(&lpp_rx_mtu, static_cast<uint16_t>(max_rx_packet_size))) {
        sock->rx_mtu = lpp_rx_mtu;
      } else {
        return BT_STATUS_UNSUPPORTED;
      }
    } else {
      sock->rx_mtu = L2CAP_SDU_LENGTH_LE_MAX;
    }
  } else {
    sock->rx_mtu = L2CAP_SDU_LENGTH_MAX;
  }
...
/* only call with std::mutex taken */
static l2cap_socket* btsock_l2cap_find_by_connection_id_l(uint64_t connection_id) {
  l2cap_socket* sock = socks;

  while (sock) {
    if (sock->connection_id == connection_id) {
      return sock;
    }
    sock = sock->next;
  }
...
void on_btsocket_l2cap_opened_complete(uint64_t connection_id, bool success) {
  ...
  std::unique_lock<std::mutex> lock(state_lock);
  sock = btsock_l2cap_find_by_connection_id_l(connection_id);
  if (!sock) {
    log::error("Unable to find l2cap socket with connection_id:{}", connection_id);
    return;
  }
  if (!success) {
    log::error("L2CAP opened complete failed with connection_id:{}", connection_id);
    btsock_l2cap_free_l(sock);
    return;
  }
  // If the socket was accepted from listen socket, use listen_our_fd.
  if (sock->listen_our_fd == -1) {
    send_app_connect_signal(sock->listen_our_fd, &sock->addr, sock->channel, 0, sock->app_fd,
                            sock->rx_mtu, sock->tx_mtu, sock->conn_uuid, sock->connection_id);
    ...
    if (!send_app_connect_signal(sock->our_fd, &sock->addr, sock->channel, 0, -1, sock->rx_mtu,
                                 sock->tx_mtu, sock->conn_uuid, sock->connection_id)) {
      log::error("Unable to connect l2cap socket to application socket_id:{}", sock->id);
      return;
    }
...
void on_btsocket_l2cap_close(uint64_t connection_id) {
  ...
  log::info("L2CAP close request for connection_id:{}", connection_id);
  btsock_l2cap_free_l(sock);
}
...
static uint64_t btif_l2cap_sock_generate_connection_id() {
  ...
  do {
    connection_id = bluetooth::os::GenerateRandomUint64();
  } while (!connection_id);
