...
typedef struct l2cap_socket {
  ...
  int data_mode;         // socket data offload mode requested
  int data_mode_in_use;  // socket data offload mode in use
  char socket_name[32];  // descriptive socket name
...
static l2cap_socket* btsock_l2cap_alloc_l(const char* name, const RawAddress* addr, char is_server,
  ...
  sock->data_mode = SOCKET_DATA_MODE_OFFLOAD_OFF;
  sock->data_mode_in_use = SOCKET_DATA_MODE_OFFLOAD_OFF;
  sock->hub_id = -1;
  sock->endpoint_id = -1;
...
static void on_srv_l2cap_psm_connect_l(tBTA_JV_L2CAP_OPEN* p_open, l2cap_socket* sock) {
  ...
  accept_rs->data_mode = sock->data_mode;
