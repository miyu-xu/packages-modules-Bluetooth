...
TEST_F(BtifCoreTest, default_callbacks) {}

class BtifInitializedTest : public BtifCoreTest {
 protected:
  void SetUp() override {
    BtifCoreTest::SetUp();
...
class BtifCoreWithControllerTest : public BtifInitializedTest {
  void SetUp() override {
    BtifInitializedTest::SetUp();
    ON_CALL(controller_, SupportsSniffSubrating).WillByDefault(Return(true));
  }

  void TearDown() override { BtifInitializedTest::TearDown(); }
};

std::promise<int> promise0;
void callback0(int val) { promise0.set_value(val); }

TEST_F(BtifInitializedTest, test_nop) {}

TEST_F(BtifInitializedTest, test_post_on_bt_simple0) {
  const int val = 123;
...
TEST_F(BtifInitializedTest, test_post_on_bt_jni_simple1) {
...
TEST_F(BtifInitializedTest, test_post_on_bt_jni_simple2) {
  std::promise<void> promise;
  std::future<void> future = promise.get_future();
...
TEST_F(BtifInitializedTest, test_post_on_bt_jni_simple3) {
  const int val = 456;
...
TEST_F(BtifInitializedTest, dump_av_sm_event_name) {
...
TEST_F(BtifInitializedTest, dump_dm_search_event) {
...
TEST_F(BtifInitializedTest, dump_property_type) {
...
TEST_F(BtifInitializedTest, dump_dm_event) {
...
TEST_F(BtifInitializedTest, dump_hf_event) {
  std::vector<std::pair<uint8_t, std::string>> events = {
...
TEST_F(BtifInitializedTest, dump_hf_client_event) {
  std::vector<std::pair<int, std::string>> events = {
...
TEST_F(BtifInitializedTest, bta_hh_event_text) {
  std::vector<std::pair<int, std::string>> events = {
...
TEST_F(BtifInitializedTest, dump_hd_event) {
  std::vector<std::pair<uint16_t, std::string>> events = {
...
TEST_F(BtifInitializedTest, dump_thread_evt) {
...
TEST_F(BtifInitializedTest, dump_av_conn_state) {
  std::vector<std::pair<uint16_t, std::string>> events = {
...
TEST_F(BtifInitializedTest, dump_av_audio_state) {
  std::vector<std::pair<uint16_t, std::string>> events = {
...
TEST_F(BtifInitializedTest, dump_adapter_scan_mode) {
...
TEST_F(BtifInitializedTest, dump_bt_status) {
...
TEST_F(BtifInitializedTest, dump_rc_event) {
  std::vector<std::pair<uint8_t, std::string>> events = {
      std::make_pair(BTA_AV_RC_OPEN_EVT, "BTA_AV_RC_OPEN_EVT"),
      std::make_pair(BTA_AV_RC_CLOSE_EVT, "BTA_AV_RC_CLOSE_EVT"),
      std::make_pair(BTA_AV_RC_BROWSE_OPEN_EVT, "BTA_AV_RC_BROWSE_OPEN_EVT"),
...
TEST_F(BtifInitializedTest, dump_rc_notification_event_id) {
  std::vector<std::pair<uint8_t, std::string>> events = {
...
TEST_F(BtifInitializedTest, dump_rc_pdu) {
