...
TEST_F(BtifUtilsTest, dump_property_type) {
  std::vector<std::pair<bt_property_type_t, std::string>> types = {
    ...
    std::make_pair(BT_PROPERTY_ADAPTER_SCAN_MODE, "BT_PROPERTY_ADAPTER_SCAN_MODE"),
...
TEST_F(BtifUtilsTest, dump_av_conn_state) {
  std::vector<std::pair<uint16_t, std::string>> events = {
    ...
    std::make_pair(BTAV_CONNECTION_STATE_DISCONNECTING, "BTAV_CONNECTION_STATE_DISCONNECTING"),
  };
  for (const auto& event : events) {
...
TEST_F(BtifUtilsTest, dump_adapter_scan_mode) {
  std::vector<std::pair<bt_scan_mode_t, std::string>> events = {
    ...
    std::make_pair(BT_SCAN_MODE_CONNECTABLE_DISCOVERABLE, "BT_SCAN_MODE_CONNECTABLE_DISCOVERABLE"),
  };
  for (const auto& event : events) {
  ...
  do_in_main_thread(FROM_HERE,
                    BindOnce(
                            [](std::unique_ptr<std::promise<void>> done_promise) {
                              ...
                              EXPECT_NE(std::string(line_buf).find("Event queue is empty"),
                                        std::string::npos);
