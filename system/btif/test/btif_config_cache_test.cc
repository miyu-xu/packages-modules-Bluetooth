...
TEST(BtifConfigCacheTest, test_setup_btif_config_cache) {
  ...
  test_btif_config_cache.SetString(kBtInfo, "TimeCreated",
                                   "2020-06-05 12:12:12");
  // Metrics section
  test_btif_config_cache.SetString(kBtMetrics, "Salt256Bit", "92a331174d20f2bb");
  // Adapter Section
  test_btif_config_cache.SetString(kBtAdapter, BTIF_STORAGE_KEY_ADDRESS, kBtLocalAddr);
  EXPECT_TRUE(test_btif_config_cache.HasSection(kBtAdapter));

  // bt_device_1
  test_btif_config_cache.SetString(kBtAddr1, BTIF_STORAGE_KEY_NAME, "Headset_1");
  ...
  // bt_device_2
  test_btif_config_cache.SetString(kBtAddr2, BTIF_STORAGE_KEY_NAME, "Headset_2");
  EXPECT_TRUE(test_btif_config_cache.HasKey(kBtAddr2, BTIF_STORAGE_KEY_NAME));

  // bt_device_3
  test_btif_config_cache.SetString(kBtAddr3, BTIF_STORAGE_KEY_NAME, "Headset_3");
  EXPECT_TRUE(test_btif_config_cache.HasKey(kBtAddr3, BTIF_STORAGE_KEY_NAME));

  // bt_device_4
  test_btif_config_cache.SetString(kBtAddr4, BTIF_STORAGE_KEY_NAME, "Headset_4");
...
TEST(BtifConfigCacheTest, test_set_up_config_cache_with_invalid_section) {
  ...
  // kBtAddrInvalid1
  test_btif_config_cache.SetString(kBtAddrInvalid1, BTIF_STORAGE_KEY_NAME, "Headset_1");
  EXPECT_TRUE(test_btif_config_cache.HasKey(kBtAddrInvalid1, BTIF_STORAGE_KEY_NAME));
  ...
  test_btif_config_cache.SetString(kBtAddrInvalid1, BTIF_STORAGE_KEY_LINK_KEY ,
                                   "1122334455667788");
  EXPECT_TRUE(test_btif_config_cache.HasKey(kBtAddrInvalid1, BTIF_STORAGE_KEY_LINK_KEY ));
  EXPECT_TRUE(test_btif_config_cache.HasUnpairedSection(kBtAddrInvalid1));
  EXPECT_FALSE(test_btif_config_cache.HasPersistentSection(kBtAddrInvalid1));

  // kBtAddrInvalid2
  test_btif_config_cache.SetString(kBtAddrInvalid2, BTIF_STORAGE_KEY_NAME, "Headset_1");
  EXPECT_TRUE(test_btif_config_cache.HasKey(kBtAddrInvalid2, BTIF_STORAGE_KEY_NAME));
  ...
  test_btif_config_cache.SetString(kBtAddrInvalid2, BTIF_STORAGE_KEY_LINK_KEY ,
                                   "1122334455667788");
  EXPECT_TRUE(test_btif_config_cache.HasKey(kBtAddrInvalid2, BTIF_STORAGE_KEY_LINK_KEY ));
  EXPECT_TRUE(test_btif_config_cache.HasUnpairedSection(kBtAddrInvalid2));
  EXPECT_FALSE(test_btif_config_cache.HasPersistentSection(kBtAddrInvalid2));

  // kBtAddrInvalid3
  test_btif_config_cache.SetString(kBtAddrInvalid3, BTIF_STORAGE_KEY_NAME, "Headset_1");
  EXPECT_TRUE(test_btif_config_cache.HasKey(kBtAddrInvalid3, BTIF_STORAGE_KEY_NAME));
  ...
  test_btif_config_cache.SetString(kBtAddrInvalid3, BTIF_STORAGE_KEY_LINK_KEY ,
                                   "1122334455667788");
  EXPECT_TRUE(test_btif_config_cache.HasKey(kBtAddrInvalid3, BTIF_STORAGE_KEY_LINK_KEY ));
  EXPECT_TRUE(test_btif_config_cache.HasUnpairedSection(kBtAddrInvalid3));
  EXPECT_FALSE(test_btif_config_cache.HasPersistentSection(kBtAddrInvalid3));

  // kBtAddrInvalid4
  test_btif_config_cache.SetString(kBtAddrInvalid4, BTIF_STORAGE_KEY_NAME, "Headset_1");
  EXPECT_TRUE(test_btif_config_cache.HasKey(kBtAddrInvalid4, BTIF_STORAGE_KEY_NAME));
  ...
  test_btif_config_cache.SetString(kBtAddrInvalid4, BTIF_STORAGE_KEY_LINK_KEY ,
                                   "1122334455667788");
  EXPECT_TRUE(test_btif_config_cache.HasKey(kBtAddrInvalid4, BTIF_STORAGE_KEY_LINK_KEY ));
  EXPECT_TRUE(test_btif_config_cache.HasUnpairedSection(kBtAddrInvalid4));
  EXPECT_FALSE(test_btif_config_cache.HasPersistentSection(kBtAddrInvalid4));

  // kBtSectionInvalid1
  test_btif_config_cache.SetString(kBtSectionInvalid1, BTIF_STORAGE_KEY_NAME, "Headset_1");
  EXPECT_TRUE(test_btif_config_cache.HasKey(kBtSectionInvalid1, BTIF_STORAGE_KEY_NAME));
  EXPECT_TRUE(test_btif_config_cache.HasUnpairedSection(kBtSectionInvalid1));
  // get the LinkKey
  test_btif_config_cache.SetString(kBtSectionInvalid1, BTIF_STORAGE_KEY_LINK_KEY ,
                                   "1122334455667788");
  EXPECT_TRUE(test_btif_config_cache.HasKey(kBtSectionInvalid1, BTIF_STORAGE_KEY_LINK_KEY ));
  EXPECT_TRUE(test_btif_config_cache.HasUnpairedSection(kBtSectionInvalid1));
...
TEST(BtifConfigCacheTest, test_get_set_key_value_test) {
  ...
  // get the LinkKey
  test_btif_config_cache.SetString(kBtAddr1, BTIF_STORAGE_KEY_LINK_KEY , "1122334455667788");
  EXPECT_TRUE(test_btif_config_cache.HasKey(kBtAddr1, BTIF_STORAGE_KEY_LINK_KEY ));
  ...
  EXPECT_DEATH({ test_btif_config_cache.SetString("", BTIF_STORAGE_KEY_NAME, "Headset_1"); },
...
TEST(BtifConfigCacheTest, test_set_values_in_the_same_key) {
  ...
  test_btif_config_cache.SetString(kBtAddr1, BTIF_STORAGE_KEY_NAME, "Headset_1A");
  EXPECT_THAT(test_btif_config_cache.GetString(kBtAddr1, BTIF_STORAGE_KEY_NAME),
              Optional(StrEq("Headset_1A")));

  // add the same key BTIF_STORAGE_KEY_NAME with different value
  test_btif_config_cache.SetString(kBtAddr1, BTIF_STORAGE_KEY_NAME, "Headset_2A");
  EXPECT_THAT(test_btif_config_cache.GetString(kBtAddr1, BTIF_STORAGE_KEY_NAME),
 ...
 * 2. unpaired with the device by removing the BTIF_STORAGE_KEY_LINK_KEY  and check the device be
...
TEST(BtifConfigCacheTest, test_pair_unpair_device_stress_test) {
  ...
  for (int i = 0; i < kTestRepeatCount; ++i) {
    ...
    EXPECT_FALSE(test_btif_config_cache.HasKey(kBtAddr1, BTIF_STORAGE_KEY_LINK_KEY ));
    EXPECT_TRUE(test_btif_config_cache.HasUnpairedSection(kBtAddr1));
    EXPECT_FALSE(test_btif_config_cache.HasPersistentSection(kBtAddr1));
  }
}
...
TEST(BtifConfigCacheTest, test_multi_pair_unpair_with_devices) {
  ...
  test_btif_config_cache.SetString(kBtAddr2, BTIF_STORAGE_KEY_LE_KEY_PENC , "aabbccddeeff9900");
  EXPECT_TRUE(test_btif_config_cache.HasKey(kBtAddr2, BTIF_STORAGE_KEY_LE_KEY_PENC ));

  test_btif_config_cache.SetString(kBtAddr3, BTIF_STORAGE_KEY_NAME, "kBtAddr3");
  test_btif_config_cache.SetString(kBtAddr3, BTIF_STORAGE_KEY_LE_KEY_PID , "a1b2c3d4e5feeeee");
  EXPECT_TRUE(test_btif_config_cache.HasKey(kBtAddr3, BTIF_STORAGE_KEY_LE_KEY_PID ));

  test_btif_config_cache.SetString(kBtAddr4, BTIF_STORAGE_KEY_LE_KEY_PCSRK ,
                                   "aaaabbbbccccdddd");
  EXPECT_TRUE(test_btif_config_cache.HasKey(kBtAddr4, BTIF_STORAGE_KEY_LE_KEY_PCSRK ));

  test_btif_config_cache.SetString(kBtAddr5, BTIF_STORAGE_KEY_NAME, "kBtAddr5");
  test_btif_config_cache.SetString(kBtAddr5, BTIF_STORAGE_KEY_LE_KEY_LENC , "jilkjlkjlkn");
  EXPECT_TRUE(test_btif_config_cache.HasKey(kBtAddr5, BTIF_STORAGE_KEY_LE_KEY_LENC ));

  // checking these 4 devices are in paired list cache and the content are
  // correct.
  EXPECT_TRUE(test_btif_config_cache.HasPersistentSection(kBtAddr1));
  EXPECT_THAT(test_btif_config_cache.GetString(kBtAddr1, BTIF_STORAGE_KEY_LINK_KEY ),
              Optional(StrEq("1122334455667788")));
  EXPECT_TRUE(test_btif_config_cache.HasPersistentSection(kBtAddr2));
  EXPECT_THAT(test_btif_config_cache.GetString(kBtAddr2, BTIF_STORAGE_KEY_LE_KEY_PENC ),
              Optional(StrEq("aabbccddeeff9900")));
  EXPECT_TRUE(test_btif_config_cache.HasPersistentSection(kBtAddr3));
  EXPECT_THAT(test_btif_config_cache.GetString(kBtAddr3, BTIF_STORAGE_KEY_LE_KEY_PID ),
              Optional(StrEq("a1b2c3d4e5feeeee")));
  EXPECT_TRUE(test_btif_config_cache.HasPersistentSection(kBtAddr4));
  EXPECT_THAT(test_btif_config_cache.GetString(kBtAddr4, BTIF_STORAGE_KEY_LE_KEY_PCSRK ),
              Optional(StrEq("aaaabbbbccccdddd")));
  EXPECT_TRUE(test_btif_config_cache.HasPersistentSection(kBtAddr5));
  EXPECT_THAT(test_btif_config_cache.GetString(kBtAddr5, BTIF_STORAGE_KEY_LE_KEY_LENC ),
  ...
  EXPECT_FALSE(test_btif_config_cache.HasKey(kBtAddr2, BTIF_STORAGE_KEY_LE_KEY_PENC ));
  ...
  EXPECT_FALSE(test_btif_config_cache.HasKey(kBtAddr3, BTIF_STORAGE_KEY_LE_KEY_PID ));
  ...
  EXPECT_FALSE(test_btif_config_cache.HasKey(kBtAddr4, BTIF_STORAGE_KEY_LE_KEY_PCSRK ));
  ...
  EXPECT_FALSE(test_btif_config_cache.HasKey(kBtAddr5, BTIF_STORAGE_KEY_LE_KEY_LENC ));
 ...
 * paired with sections with the specific BTIF_STORAGE_KEY_RESTRICTED key and then removed the
 * BTIF_STORAGE_KEY_RESTRICTED key, check if the sections with the specific BTIF_STORAGE_KEY_RESTRICTED  key
 * are removed.
 */
TEST(BtifConfigCacheTest, test_remove_sections_with_key) {
  BtifConfigCache test_btif_config_cache(kCapacity);
  // pair with Headset_1 (kBtAddr1), Headset_2 (kBtAddr1), Heasdet_3 (kBtAddr3)
  // , and Headset_1 (kBtAddr1), Headset_3 (kBtAddr3) have specific BTIF_STORAGE_KEY_RESTRICTED 
  // key
  test_btif_config_cache.SetString(kBtAddr1, BTIF_STORAGE_KEY_NAME, "Headset_1");
  test_btif_config_cache.SetString(kBtAddr1, BTIF_STORAGE_KEY_RESTRICTED , "1");
  test_btif_config_cache.SetString(kBtAddr1, BTIF_STORAGE_KEY_LINK_KEY , "1122334455667788");
  test_btif_config_cache.SetString(kBtAddr2, BTIF_STORAGE_KEY_NAME, "Headset_2");
  test_btif_config_cache.SetString(kBtAddr2, BTIF_STORAGE_KEY_LINK_KEY , "aabbccddeeff9900");
  test_btif_config_cache.SetString(kBtAddr3, BTIF_STORAGE_KEY_NAME, "Headset_3");
  test_btif_config_cache.SetString(kBtAddr3, BTIF_STORAGE_KEY_LINK_KEY , "a1b2c3d4e5feeeee");
  ...
  test_btif_config_cache.RemovePersistentSectionsWithKey(BTIF_STORAGE_KEY_RESTRICTED );
