...
static btrc_ctrl_callbacks_t default_btrc_ctrl_callbacks = {
  ...
  .playerapplicationsetting_changed_cb = [](const RawAddress& /* bd_addr */,
  ...
  .get_folder_items_cb = [](const RawAddress& /* bd_addr */, btrc_status_t /* status */,
                            ...
                            uint8_t /* count */) { FAIL(); },
  .change_folder_path_cb = [](const RawAddress& /* bd_addr */, uint32_t /* count */) { FAIL(); },
  .set_browsed_player_cb = [](const RawAddress& /* bd_addr */, uint8_t /* num_items */,
                              uint8_t /* depth */) { FAIL(); },
  .set_addressed_player_cb = [](const RawAddress& /* bd_addr */, uint8_t /* status */) { FAIL(); },
  .addressed_player_changed_cb = [](const RawAddress& /* bd_addr */, uint16_t /* id */) { FAIL(); },
  .now_playing_contents_changed_cb = [](const RawAddress& /* bd_addr */) { FAIL(); },
  .available_player_changed_cb = [](const RawAddress& /* bd_addr */) { FAIL(); },
  .get_cover_art_psm_cb = [](const RawAddress& /* bd_addr */, const uint16_t /* psm */) { FAIL(); },
};
...
TEST_F(BtifRcWithCallbacksTest, handle_rc_ctrl_features) {
  ...
  ASSERT_EQ(res.feature,
            (BTRC_FEAT_ABSOLUTE_VOLUME | BTRC_FEAT_METADATA | BTRC_FEAT_BROWSE |
