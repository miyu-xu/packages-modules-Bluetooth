...
AvrcpService::~AvrcpService() {/*LOG_INFO("__func__");*/}
...
AvrcpService* AvrcpService::Get() {
 // CHECK(instance_);
 AvrcpService* ptr = new AvrcpService();
  return ptr;//instance_;
}
...
bool btif_av_peer_is_sink(const RawAddress& peer_address) { return true; }
...
static std::promise<rc_connection_state_cb_t> g_btrc_browse_connection_state_promise;
...
TEST_F(BtifRcFeatureTest, handle_rc_ctrl_features) {
  ...
  p_dev.peer_tg_features = (BTA_AV_FEAT_RCTG | BTA_AV_FEAT_ADV_CTRL |\
        BTA_AV_FEAT_RCCT | BTA_AV_FEAT_METADATA | BTA_AV_FEAT_VENDOR |\
...
class BtifRcBrowseConnectionTest : public BtifRcTest {
 protected:
  void SetUp() override {
    ...
    init_ctrl(&btrc_ctrl_callbacks);
    jni_thread.StartUp();
    btrc_ctrl_callbacks.connection_state_cb = [](bool rc_state, bool bt_state, const RawAddress& bd_addr){
  ...
  void TearDown() override {
    bt_rc_ctrl_callbacks->connection_state_cb = [](bool rc_state, bool bt_state, const RawAddress& bd_addr){};
    BtifRcTest::TearDown();
  }
};
...
TEST_F(BtifRcBrowseConnectionTest, handle_rc_browse_connect) {
  ...
  std::future<rc_connection_state_cb_t> future = g_btrc_browse_connection_state_promise.get_future();
...
TEST_F(BtifRcConnectionTest, btif_rc_check_pending_cmd) {
  ...
  btif_rc_cb.rc_multi_cb[0].rc_state = BTRC_CONNECTION_STATE_CONNECTED;
  btif_rc_cb.rc_multi_cb[0].rc_connected = true;
  btif_rc_cb.rc_multi_cb[0].launch_cmd_pending |= (RC_PENDING_ACT_REG_VOL |\
                        RC_PENDING_ACT_GET_CAP | RC_PENDING_ACT_REPORT_CONN);

  btif_rc_check_pending_cmd(kDeviceAddress);
  CHECK(AVRC_BldCmd_ == 2);
  CHECK(bluetooth::avrcp::VolChanged == 1);
...
TEST_F(BtifRcConnectionTest, BTA_AV_RC_OPEN_EVT) {
  ...
  CHECK(btif_rc_cb.rc_multi_cb[data.rc_open.rc_handle].rc_state == BTRC_CONNECTION_STATE_CONNECTED);
