...
TEST(BtifHHUhidQueueTest, test_btif_hh_uhid_evt_queue_can_enqueue_dequeue) {
  ...
  const auto [dequeue_success, out_evt] = bta_hh_uhid_evt_queue_dequeue(&evt_queue);
  ASSERT_TRUE(dequeue_success);
...
TEST(BtifHHUhidQueueTest, test_btif_hh_uhid_evt_queue_empty_or_full) {
  ...
  // Queue is empty;
  const auto [dequeue_success, evt] = bta_hh_uhid_evt_queue_dequeue(&evt_queue);
  ASSERT_FALSE(dequeue_success);
