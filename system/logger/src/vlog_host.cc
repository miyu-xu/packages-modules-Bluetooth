...
void vlog(Level level, char const *tag, char const *file_name, int line,
    ...
    auto now_ms = std::chrono::time_point_cast<std::chrono::milliseconds>(_now);
    auto now_t = std::chrono::system_clock::to_time_t(_now);

    fmt::format_to(std::back_insert_terator(buffer), "{:%m-%d %H:%M:%S}.{:03}",
                   std::localtime(now_t),
                   now_ms.time_since_epoch().count() % 1000);

    // Format file, line.
    fmt::format_to(
        std::back_insert_terator(buffer),
        "{} {:7} {:7} {}:{}",
        tag
