...
class SourceAudioHalAsrc::ClockRecovery
  ...
  __attribute__((no_sanitize("integer"))) void OnEvent(
      uint32_t timestamp_us, uint32_t local_clock) override {
    auto& state = state_;
    ...
    if (state.id == StateId::RESET) {
      ...
      state.last_local_clock = local_clock;
    ...
    // into microseconds. The values are <<4 first to ensure that the
    // substraction correctly wraps on 32 bits.
    uint32_t elapsed_us =
        (((local_clock << 4) - (state.last_local_clock << 4)) * 625) >> 5;
