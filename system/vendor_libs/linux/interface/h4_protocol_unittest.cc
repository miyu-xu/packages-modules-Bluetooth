...
MATCHER_P3(PacketMatches, preamble, preamble_length, payload, "") {
 ...
 protected:
  void SetUp() override {
    ...
    fd_watcher_.WatchFdForNonBlockingReads(
        sockfd[0], [this](int fd) { h4_hci_->OnDataReady(fd); });
  ...
  void SendAndReadUartOutbound(HciPacketType type, char* data) {
    ...
    for (i = 0; i < uart_length; i++) {
      ...
      TEMP_FAILURE_RETRY(
          select(fake_uart_ + 1, &read_fds, nullptr, nullptr, nullptr));
  ...
  void WriteAndExpectInboundScoData(char* payload) {
    ...
    char preamble[4] = {static_cast<uint8_t>(HCI_PACKET_TYPE_SCO_DATA), 20, 17,
                        0};
    ...
    std::mutex mutex;
    std::condition_variable done;
    EXPECT_CALL(sco_cb_, Call(PacketMatches(preamble + 1, sizeof(preamble) - 1,
                                            payload)))
        .WillOnce(Notify(&mutex, &done));
  ...
  void WriteAndExpectInboundEvent(char* payload) {
    ...
    std::mutex mutex;
    std::condition_variable done;
    EXPECT_CALL(event_cb_, Call(PacketMatches(preamble + 1,
  ...
  void WriteAndExpectInboundIsoData(char* payload) {
    // h4 type[1] + handle[2] + size[1]
    char preamble[5] = {static_cast<uint8_t>(HCI_PACKET_TYPE_ISO_DATA), 19, 92,
                        0, 0};
    int length = strlen(payload);
    preamble[3] = length & 0xFF;
    ...
    std::mutex mutex;
    std::condition_variable done;
    EXPECT_CALL(iso_cb_, Call(PacketMatches(preamble + 1, sizeof(preamble) - 1,
                                            payload)))
        .WillOnce(Notify(&mutex, &done));
...
TEST_F(H4ProtocolTest, TestDisconnect) {
  std::mutex mutex;
  std::condition_variable done;
  EXPECT_CALL(disconnect_cb_, Call()).WillOnce(Notify(&mutex, &done));
  close(fake_uart_);
