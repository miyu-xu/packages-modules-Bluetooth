...
void H4Protocol::send_data_to_packetizer() {
  while (buffer_offset_ < input_buffer_.size()) {
    if (hci_packet_type_ == HCI_PACKET_TYPE_UNKNOWN) {
      hci_packet_type_ =
          static_cast<HciPacketType>(input_buffer_.data()[buffer_offset_]);
      buffer_offset_ += 1;
    }
    if (buffer_offset_ < input_buffer_.size()) {
      bool packet_ready = hci_packetizer_.OnDataReady(
          hci_packet_type_, input_buffer_, buffer_offset_);
      if (packet_ready) {
        // Call packet callback and move offset.
        buffer_offset_ += on_packet_ready(hci_packetizer_.GetPacket());
        // Get ready for the next type byte.
        hci_packet_type_ = HCI_PACKET_TYPE_UNKNOWN;
      } else {
        // The data was consumed, but there wasn't a packet.
        buffer_offset_ = input_buffer_.size();
      }
    }
  }
...
void H4Protocol::OnDataReady(int fd) {
  ...
  input_buffer_.resize(kMaxPacketLength);
  ssize_t bytes_read =
      TEMP_FAILURE_RETRY(read(fd, input_buffer_.data(), kMaxPacketLength));
  if (bytes_read == 0) {
  ...
  input_buffer_.resize(bytes_read);
  send_data_to_packetizer();
}
