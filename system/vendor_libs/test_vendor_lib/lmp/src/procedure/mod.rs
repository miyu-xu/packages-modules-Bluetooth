...
pub trait Context {
    ...
    fn secure_simple_pairing_supported(&self) -> bool {
        true
    }
...
async fn repeat<Fut>(f: impl Fn() -> Fut)
where
    Fut: Future<Output = ()>,
{
    loop {
...
pub async fn run(ctx: &impl Context) {
    join!(
        repeat(|| authentication::initiate(ctx)),
        repeat(|| authentication::respond(ctx)),
        repeat(|| encryption::initiate(ctx)),
        repeat(|| encryption::respond(ctx))
    );
}
