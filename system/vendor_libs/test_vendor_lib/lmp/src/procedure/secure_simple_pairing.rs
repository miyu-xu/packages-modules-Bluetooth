...
async fn user_passkey_request(ctx: &impl Context) -> Result<(), ()> {
    let bd_addr = hci::Address { bytes: ctx.peer_addr() };
    ctx.send_hci_event(hci::UserPasskeyRequestBuilder { bd_addr }.build());
...
async fn remote_oob_data_request(ctx: &impl Context) -> Result<(), ()> {
    let bd_addr = hci::Address { bytes: ctx.peer_addr() };
    ctx.send_hci_event(hci::RemoteOobDataRequestBuilder { bd_addr }.build());
...
pub async fn initiate(ctx: &impl Context) -> bool {
    ...
    match authentication_method(initiator, responder) {
        AuthenticationMethod::NumericComparaison => {
            ...
            if user_confirmation.is_err() {
                ctx.send_lmp_packet(
                    lmp::NumericComparaisonFailedBuilder { transaction_id: 0 }.build(),
                );
                ctx.send_hci_event(
        ...
        AuthenticationMethod::PasskeyEntry => {
            if initiator.io_capability == hci::IoCapability::KeyboardOnly {
                ...
                ctx.send_hci_event(
                    hci::UserPasskeyNotificationBuilder { bd_addr, passkey: 0 }.build(),
                );
            }
            for _ in 0..20 {
