...
pub async fn initiate(ctx: &impl Context) {
    ...
    let request = ctx.receive_hci_command::<hci::SetConnectionEncryptionPacket>().await;
    let connection_handle = request.get_connection_handle();
    ctx.send_hci_event(
...
pub async fn respond(ctx: &impl Context) {
    ...
    ctx.send_hci_event(
        hci::EncryptionChangeBuilder {
            status: hci::ErrorCode::Success,
            connection_handle: ctx.peer_connection_handle(),
            encryption_enabled: hci::EncryptionEnabled::On,
        }
        .build(),
    );
