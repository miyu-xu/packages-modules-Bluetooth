...
LinkLayerController::LinkLayerController(const DeviceProperties& properties)
  ...
  lmp_ = link_manager_create((struct link_manager_ops){
      .user_pointer = this,
...
void LinkLayerController::IncomingLmp(
    model::packets::LinkLayerPacketView incoming) {
  Address address = incoming.GetSourceAddress();
  ...
  link_manager_ingest_lmp(lmp_, address.data(), packet.data(), packet.size());
}
...
void LinkLayerController::IncomingPagePacket(
  ...
  link_manager_add_link(lmp_, incoming.GetSourceAddress().data());
...
void LinkLayerController::ForwardToLmp(bluetooth::hci::CommandView command) {
  auto packet = std::vector(command.begin(), command.end());
  link_manager_ingest_hci(lmp_, packet.data(), packet.size());
}
...
ErrorCode LinkLayerController::IoCapabilityRequestReply(
    const Address& /* peer */, uint8_t /* io_capability */,
...
ErrorCode LinkLayerController::PinCodeRequestReply(
    const Address& /* peer */, std::vector<uint8_t> /* pin */) {
  // TODO lmp
  return ErrorCode::SUCCESS;
}
...
ErrorCode LinkLayerController::UserPasskeyRequestReply(
...
ErrorCode LinkLayerController::RemoteOobExtendedDataRequestReply(
    ...
    const std::array<uint8_t, 16>& /* r192 */,
    const std::array<uint8_t, 16>& /* c256 */,
...
ErrorCode LinkLayerController::CreateConnection(const Address& addr, uint16_t,
  ...
  link_manager_add_link(lmp_, addr.data());
