...
class DynamicChannelManager {
   ...
   * - If Dynamic channel on a remote device is already reported as connected via
   * on_open_callback, it won't be reported again
   *
   ...
   * object.
   * - If a PSM is already registered or some other error happens, on_registration_complete will
   ...
   * the return status is not SUCCESS, it means service is not registered due to reasons like PSM
   * already take
   * @param on_open_callback: A callback to indicate success of a connection initiated from a
   * remote device.
   * @param configuration_option: The configuration options for this channel
