...
class FixedChannelManager {
   ...
   * - If an ACL connection does not exist, this method will create an ACL connection. As a
   * result, on_open_callback supplied through RegisterService() will be triggered to provide the
   * actual FixedChannel objects
   * - If HCI connection failed, on_fail_callback will be triggered with FAIL_HCI_ERROR
   * - If fixed channel on a remote device is already reported as connected via on_open_callback
   * and has been acquired via FixedChannel#Acquire() API, it won't be reported again
   ...
   * contains a FixedChannelService object. The registered service can be managed from that
   * object.
   * - If a CID is already registered or some other error happens, on_registration_complete will
   * be triggered with a non-SUCCESS value
   ...
   * @param on_registration_complete: A callback to indicate the service setup has completed. If
   * the return status is not SUCCESS, it means service is not registered due to reasons like CID
   * already take
   * @param on_open_callback: A callback to indicate success of a connection initiated from a
   * remote device.
