...
void Link::OnLeReadRemoteFeaturesComplete(hci::ErrorCode, uint64_t) {}

void Link::OnPhyUpdate(hci::ErrorCode, uint8_t, uint8_t) {}
...
std::shared_ptr<FixedChannelImpl> Link::AllocateFixedChannel(Cid cid, SecurityPolicy) {
...
void Link::OnPendingPacketChange(Cid, bool has_packet) {
  if (has_packet) {
