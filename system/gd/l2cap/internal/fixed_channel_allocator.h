...
class FixedChannelAllocator {
 public:
  ...
  virtual std::shared_ptr<FixedChannelImplType> AllocateChannel(Cid cid) {
    ASSERT_LOG(
        !IsChannelAllocated((cid)), "Cid 0x%x for link %s is already in use", cid, link_->ToStringForLogging().c_str());
    ...
    ASSERT_LOG(elem.second, "Failed to create channel for cid 0x%x link %s", cid, link_->ToStringForLogging().c_str());
  ...
  virtual void FreeChannel(Cid cid) {
    ASSERT_LOG(
        IsChannelAllocated(cid), "Channel is not in use: cid %d, link %s", cid, link_->ToStringForLogging().c_str());
    channels_.erase(cid);
  }
