...
class FixedChannelAllocator {
 public:
  ...
  virtual std::shared_ptr<FixedChannelImplType> AllocateChannel(Cid cid) {
    ASSERT_LOG(
        !IsChannelAllocated((cid)), "Cid 0x%x for link %s is already in use", cid, ToLoggableStr(*link_).c_str());
  ...
  virtual void FreeChannel(Cid cid) {
    ASSERT_LOG(IsChannelAllocated(cid), "Channel is not in use: cid %d, link %s", cid, ToLoggableStr(*link_).c_str());
    channels_.erase(cid);
  }
