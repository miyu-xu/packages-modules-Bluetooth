...
class StorageModule : public bluetooth::Module {
  ...
  // straightforward as a BR/EDR device has only a single public static MAC address. However, for
  // LE devices using private addresses, we only learn its real static address after pairing.
  // Since we still need to store that device's information prior to pairing, we use the
  // first-seen address of that device, no matter random private or static public, as a "key" to
  // store that device's config. This method gives you a device object using this legacy key. If
  // the key does not exist, the device will be lazily created in the config
  ...
  // Normally, underlying config will be saved at most 3 seconds after the first config change in
  // a series of changes This method triggers the delayed saving automatically, the delay is equal
  // to |config_save_delay_|
  void SaveDelayed();
  ...
  // remove all content in this config cache, restore it to the state after the explicit
  // constructor
  void Clear();

  // Create the storage module where:
  // - config_file_path is the path to the config file on disk, a .bak file will be created with
  // the original
