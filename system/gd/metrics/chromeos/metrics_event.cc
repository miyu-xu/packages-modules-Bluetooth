...
PairingState ToPairingState(uint32_t status, uint32_t bond_state, int32_t fail_reason) {
  ...
  // When both status and fail reason are provided and disagree with each other, overwrite staus with the fail reason as
  // fail reason is generated closer to the HCI and provides a more accurate description.
