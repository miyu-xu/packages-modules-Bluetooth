...
// A normalized ENUM definition all profiles
...
int64_t StatusToProfileConnectionState(uint32_t status, StateChangeType type) {
  ...
  if (StateChangeType::STATE_CHANGE_TYPE_CONNECT == type) {
    switch ((BtStatus)status) {
      case BtStatus::BT_STATUS_SUCCESS:
        state = (int64_t)ProfileConnectionState::PROFILE_CONN_STATE_SUCCEED;
        break;
      case BtStatus::BT_STATUS_BUSY:
        state = (int64_t)ProfileConnectionState::PROFILE_CONN_STATE_BUSY_CONNECTING;
        break;
      case BtStatus::BT_STATUS_DONE:
        state = (int64_t)ProfileConnectionState::PROFILE_CONN_STATE_ALREADY_CONNECTED;
        break;
      case BtStatus::BT_STATUS_UNSUPPORTED:
        state = (int64_t)ProfileConnectionState::PROFILE_CONN_STATE_PROFILE_NOT_SUPPORTED;
        break;
      case BtStatus::BT_STATUS_PARM_INVALID:
        state = (int64_t)ProfileConnectionState::PROFILE_CONN_STATE_UNKNOWN_ERROR;
        break;
      case BtStatus::BT_STATUS_AUTH_FAILURE:
        state = (int64_t)ProfileConnectionState::PROFILE_CONN_STATE_CONNECTION_REFUSED;
        break;
      case BtStatus::BT_STATUS_RMT_DEV_DOWN:
        state = (int64_t)ProfileConnectionState::PROFILE_CONN_STATE_REMOTE_UNAVAILABLE;
        break;
      case BtStatus::BT_STATUS_AUTH_REJECTED:
      case BtStatus::BT_STATUS_FAIL:
      case BtStatus::BT_STATUS_NOT_READY:
    ...
    switch ((BtStatus)status) {
      case BtStatus::BT_STATUS_SUCCESS:
        state = (int64_t)ProfileDisconnectionState::PROFILE_CONN_STATE_SUCCEED;
        break;
      case BtStatus::BT_STATUS_BUSY:
        state = (int64_t)ProfileDisconnectionState::PROFILE_CONN_STATE_BUSY_CONNECTING;
        break;
      case BtStatus::BT_STATUS_DONE:
        state = (int64_t)ProfileDisconnectionState::PROFILE_CONN_STATE_ALREADY_CONNECTED;
        break;
      case BtStatus::BT_STATUS_UNSUPPORTED:
        state = (int64_t)ProfileDisconnectionState::PROFILE_CONN_STATE_UNKNOWN_ERROR;
        break;
      case BtStatus::BT_STATUS_PARM_INVALID:
        state = (int64_t)ProfileDisconnectionState::PROFILE_CONN_STATE_INVALID_PARAMS;
        break;
      case BtStatus::BT_STATUS_AUTH_FAILURE:
        state = (int64_t)ProfileDisconnectionState::PROFILE_CONN_STATE_CONNECTION_REFUSED;
        break;
      case BtStatus::BT_STATUS_RMT_DEV_DOWN:
        state = (int64_t)ProfileDisconnectionState::PROFILE_CONN_STATE_UNKNOWN_ERROR;
        break;
...
static void ToProfileConnectionState(uint32_t& profile, uint32_t& state) {
...
ProfileConnectionEvent ToProfileConnectionEvent(std::string addr, uint32_t profile, uint32_t status, uint32_t state) {
  ...
  static std::map<std::string, StateChangeType> pending_type;

  ToProfileConnectionState(profile, state);
  event.profile = (int64_t)profile;

  switch ((ProfilesConnectionState)state) {
    case ProfilesConnectionState::CONNECTED:
      event.type = (int64_t)StateChangeType::STATE_CHANGE_TYPE_CONNECT;
      event.state = (int64_t)ProfileConnectionState::PROFILE_CONN_STATE_SUCCEED;
      pending_type.erase(addr);
      break;
    case ProfilesConnectionState::CONNECTING:
      event.type = (int64_t)StateChangeType::STATE_CHANGE_TYPE_CONNECT;
      event.state = (int64_t)ProfileConnectionState::PROFILE_CONN_STATE_STARTING;
      pending_type[addr] = StateChangeType::STATE_CHANGE_TYPE_CONNECT;
      break;
    case ProfilesConnectionState::DISCONNECTED:
      event.type = pending_type.find(addr) != pending_type.end()
                       ? (int64_t)pending_type[addr]
    ...
    case ProfilesConnectionState::DISCONNECTING:
      ...
      event.state = (int64_t)ProfileConnectionState::PROFILE_CONN_STATE_STARTING;
      pending_type[addr] = StateChangeType::STATE_CHANGE_TYPE_DISCONNECT;
      break;
    default:
