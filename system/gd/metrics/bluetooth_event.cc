<<<<<<< PATCH SET (216145 BluetoothMetrics: Log ACL Completion after Remote Name Reque)
<<<<<<< PATCH SET (f87f9e BluetoothMetrics: Log ACL Completion after Remote Name Reque)
/*
...
State MapErrorCodeToState(ErrorCode reason) {
  ...
  switch (reason) {
    ...
    // Connection exists (should mark it as success?)
...
void LogRemoteNameRequestCompletion(const RawAddress& raw_address) {
  hci::Address address = bluetooth::ToGdAddress(raw_address);
  bluetooth::os::LogMetricBluetoothEvent(address, EventType::REMOTE_NAME_REQUEST, State::SUCCESS);
}
...
>>>>>>> BASE      (f4a085 BluetoothMetrics: Map Error Code to State in Bluetooth Event)
