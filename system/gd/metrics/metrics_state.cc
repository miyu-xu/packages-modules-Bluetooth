...
void LEConnectionMetricState::AddStateChangedEvent(
  ...
  if (connection_origin_type != LETransactionOriginType::ORIGIN_UNSPECIFIED) {
  ...
  switch (transaction_state) {
    case LEConnectionTransactionState::STATE_LE_ACL_END: {
    ...
    case LEConnectionTransactionState::STATE_LE_ACL_CANCEL: {
      acl_state = LEACLConnectionState::LE_ACL_FAILED;
      is_cancelled = true;
      break;
    }
...
bool LEConnectionMetricState::IsStarted() {
  return state == LEConnectionTransactionState::STATE_LE_ACL_START;
}
...
void LEConnectionMetricsRemoteDevice::AddStateChangedEvent(
  ...
  if (address.IsEmpty()) {
      ...
      for (auto &device_metric : device_metrics) {
        if (device_metric->IsStarted() && transaction_state == LEConnectionTransactionState::STATE_LE_ACL_CANCEL) {
        ...
        if (device_metric->IsCancelled() && transaction_state == LEConnectionTransactionState::STATE_LE_ACL_END) {
