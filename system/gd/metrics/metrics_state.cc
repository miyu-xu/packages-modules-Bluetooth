...
void LEConnectionMetricState::Flush() {
  ...
  argument_list.push_back(std::make_pair(os::ArgumentType::ACL_STATUS_CODE, android::bluetooth::hci::StatusEnum::STATUS_CONNECTION_TOUT));
  AddStateChangedEvent(
      LEConnectionOriginType::ORIGIN_UNSPECIFIED,
...
LEConnectionMetricsRemoteDevice::LEConnectionMetricsRemoteDevice(BaseMetricsLoggerModule* baseMetricsLoggerModule) {
...
void MetricsCollector::Flush() {
  if (MetricsCollector::le_connection_metrics_remote_device != nullptr) {
    MetricsCollector::le_connection_metrics_remote_device->Flush();
  }
}
