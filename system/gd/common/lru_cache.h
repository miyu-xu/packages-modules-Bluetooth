...
class LruCache {
  ...
  // evicted value if old value was evicted, std::nullopt if not. The return value will be
  // evaluated to true in a boolean context if a value is contained by std::optional, false
  // otherwise.
  //
  // LRU: Will warm up key
  ...
  // Put a key-value pair to the head of cache, evict the oldest key if cache is at capacity.
  // Eviction is based on key ONLY. Hence, updating a key will not evict the oldest key. This
  // method tries to construct the value in-place. If the key already exist, this method only
  // update the value. Return inserted iterator, whether insertion happens, and evicted value if
  // old value was evicted or std::nullopt
  //
  // LRU: Will warm up key
  template <class... Args>
