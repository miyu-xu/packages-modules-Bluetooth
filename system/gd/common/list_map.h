...
class ListMap {
  ...
  // exists, does nothing. Moved arguments won't be moved when key already exists. Return
  // <iterator, true> when key does not exist, <iterator, false> when key exist and iterator is
  ...
  // Try emplace an element before the end of the list map. If the key already exists, does
  // nothing. Moved arguments won't be moved when key already exists return <iterator, true> when
  // key does not exist, <iterator, false> when key exist and iterator is the position where it
  // was placed
  template <class... Args>
  ...
  // Put a key-value pair to the tail of the map or replace the current value without moving the
  // key if key exists
  ...
  // Remove a key from the list map and return removed value if key exits, std::nullopt if not.
  ...
  // Remove an iterator pointed item from the list map and return the iterator immediately after
  // the erased item
