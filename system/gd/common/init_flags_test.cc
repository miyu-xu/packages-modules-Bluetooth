...
TEST(InitFlagsTest, test_leaudio_enable_health_status_actions) {
  const char* input[] = {"INIT_leaudio_enable_health_status_actions=true", nullptr};
  InitFlags::Load(input);
  ASSERT_TRUE(InitFlags::IsLeAudioHealthBaseActionsEnabled());
}
