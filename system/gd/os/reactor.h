...
class Reactor {
  ...
  // Stop the reactor. Must be invoked from a different thread. Note: all registered reactables
  // will not be unregistered by Stop(). If the reactor is not running, it will be stopped once
  // it's started.
  void Stop();

  // Register a reactable fd to this reactor. Returns a pointer to a Reactable. Caller must use
  // this object to unregister or modify registration. Ownership of the memory space is NOT
  // transferred to user.
