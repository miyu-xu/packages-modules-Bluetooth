...
static void LogClassicPairingAdminAduitEvent(const Address& address, EventCode hci_event, ErrorCode cmd_status) {
  ...
  if (hci_event == EventCode::SIMPLE_PAIRING_COMPLETE) {
    ...
    android_log_event_list(SEC_TAG_BLUETOOTH_CONNECTION)
        << PRIVATE_ADDRESS(address) << /* success */ int32_t(cmd_status == ErrorCode::SUCCESS) << reason
        << LOG_ID_SECURITY;
  }
}
...
void LogMetricClassicPairingEvent(
  ...
  LogClassicPairingAdminAduitEvent(address, static_cast<EventCode>(hci_event), static_cast<ErrorCode>(cmd_status));
}
