...
void LogMetricBluetoothLocalSupportedFeatures(uint32_t page_num, uint64_t features) {
  ...
  if (ret < 0) {
    log::warn(
            "Failed for LogMetricBluetoothLocalSupportedFeatures, page_num {}, features {}, "
            "error "
            "{}",
...
void LogMetricBluetoothRemoteSupportedFeatures(const Address& address, uint32_t page,
  ...
  if (ret < 0) {
    log::warn(
            "Failed for LogMetricBluetoothRemoteSupportedFeatures, metric_id {}, page {}, "
            "features "
            "{}, "
            "connection_handle {}, error {}",
...
void LogMetricBluetoothLEConnection(os::LEConnectionSessionOptions session_options) {
  ...
  if (ret < 0) {
    log::warn(
            "Failed BluetoothLeSessionConnected - Address: {}, ACL Connection State: {}, "
            "Origin "
            "Type:  "
            "{}",
