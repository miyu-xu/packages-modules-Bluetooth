...
bluetooth::os::ReactorEventApi* ReactorEventApiFactory() {
  if (!dev_init_flag_rust_reactor) {
    return bluetooth::os::NewReactorEventEventfd();
  } else {
    ASSERT_LOG(false, "Rust reactor is unimplemented");
    return bluetooth::os::NewReactorEventRust();
  }
}
}  // namespace
...
struct Reactor::Event::impl final {
  impl(ReactorEventApi* event_api) : event_api_(event_api) {}
  ~impl() {
    delete event_api_;
  }

  ReactorEventApi* event_api_{nullptr};
};
...
bool Reactor::Event::Read() {
  return pimpl_->event_api_->Read();
}
int Reactor::Event::Id() const {
  return pimpl_->event_api_->Id();
}

void Reactor::Event::Clear() {
  pimpl_->event_api_->Clear();
}

void Reactor::Event::Close() {
  pimpl_->event_api_->Close();
}
void Reactor::Event::Notify() {
  pimpl_->event_api_->Notify();
}
