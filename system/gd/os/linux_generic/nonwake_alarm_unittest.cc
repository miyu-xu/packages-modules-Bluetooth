...
class NonwakeAlarmTest : public ::testing::Test {
protected:
  void SetUp() override {
    thread_ = new Thread("test_thread", Thread::Priority::NORMAL);
    handler_ = new Handler(thread_);
    alarm_ = std::make_shared<NonwakeAlarm>(handler_);
  }
  ...
  std::shared_ptr<NonwakeAlarm> get_new_alarm() { return std::make_shared<NonwakeAlarm>(handler_); }

  std::shared_ptr<NonwakeAlarm> alarm_;
...
TEST_F(NonwakeAlarmTest, cancel_while_not_armed) { alarm_->Cancel(); }

TEST_F(NonwakeAlarmTest, schedule) {
...
TEST_F(NonwakeAlarmTest, cancel_alarm) {
  alarm_->Schedule(BindOnce([]() { FAIL() << "Should not happen"; }), std::chrono::milliseconds(3));
  alarm_->Cancel();
  std::this_thread::sleep_for(std::chrono::milliseconds(5));
}

TEST_F(NonwakeAlarmTest, cancel_alarm_from_callback) {
  std::promise<void> promise;
  auto future = promise.get_future();
  alarm_->Schedule(BindOnce(
                           [](std::shared_ptr<NonwakeAlarm> alarm, std::promise<void> promise) {
                             alarm->Cancel();
...
TEST_F(NonwakeAlarmTest, schedule_while_alarm_armed) {
...
TEST_F(NonwakeAlarmTest, delete_while_alarm_armed) {
  alarm_->Schedule(BindOnce([]() { FAIL() << "Should not happen"; }), std::chrono::milliseconds(1));
  alarm_.reset();
  std::this_thread::sleep_for(std::chrono::milliseconds(10));
}

class TwoNonwakeAlarmTest : public NonwakeAlarmTest {
protected:
  void SetUp() override {
    NonwakeAlarmTest::SetUp();
    alarm2 = get_new_alarm();
  }

  void TearDown() override {
    alarm2.reset();
    NonwakeAlarmTest::TearDown();
  }

  std::shared_ptr<NonwakeAlarm> alarm2;
};

TEST_F(TwoNonwakeAlarmTest, schedule_from_alarm_long) {
