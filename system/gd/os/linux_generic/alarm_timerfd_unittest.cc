...
TEST_F(AlarmOnTimerFdTest, schedule) {
  ...
  int delay_ms = 10;
  alarm_->Schedule(
      BindOnce(&std::promise<void>::set_value, std::move(promise)),
...
TEST_F(AlarmOnTimerFdTest, cancel_alarm) {
  auto promise = std::make_unique<std::promise<void>>();
  auto future = promise->get_future();
  alarm_->Schedule(BindOnce([]() { FAIL(); }), milliseconds(50));
  alarm_->Cancel();
  ASSERT_NE(std::future_status::ready, future.wait_for(milliseconds(10)));
}
...
TEST_F(AlarmOnTimerFdTest, schedule_while_alarm_armed) {
  auto promise = std::make_unique<std::promise<void>>();
  auto future = promise->get_future();
  alarm_->Schedule(BindOnce([]() { FAIL(); }), milliseconds(100));
  alarm_->Schedule(
