...
#include "os/log.h"
#include "os/power_saving_alarm.h"
#include "os/utils.h"
...
void ConditionalWakeupAlarm::Schedule(OnceClosure task, std::chrono::milliseconds delay,
  ...
  int64_t deadline_ms = deadline.count();
...
void ConditionalWakeupAlarm::on_fire_non_wake() {
  ...
  log::assert_that(times_invoked == static_cast<uint64_t>(1),
                   "Invoked number of times:{} fd_non_wake_:{}", (uint64_t)times_invoked,
                   fd_non_wake_);
...
void ConditionalWakeupAlarm::on_fire_wake() {
  log::info("");
  std::unique_lock<std::mutex> lock(mutex_);
  uint64_t times_invoked;
  auto bytes_read = read(fd_wake_, &times_invoked, sizeof(uint64_t));
  lock.unlock();
  ...
  log::assert_that(bytes_read == static_cast<ssize_t>(sizeof(uint64_t)),
                   "assert failed: bytes_read == static_cast<ssize_t>(sizeof(uint64_t))");
  log::assert_that(times_invoked == static_cast<uint64_t>(1),
                   "Invoked number of times:{} fd_wake_:{}", (uint64_t)times_invoked, fd_wake_);
