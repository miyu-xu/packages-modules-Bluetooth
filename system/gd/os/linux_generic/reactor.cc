...
void Reactor::Unregister(Reactor::Reactable* reactable) {
  ...
  bool delaying_delete_until_callback_finished = false;
  {
    ...
    // If we are unregistering during the callback event from this reactable, we delete it after
    // the callback is executed. reactable->is_executing_ is protected by reactable->mutex_, so
    // it's thread safe.
