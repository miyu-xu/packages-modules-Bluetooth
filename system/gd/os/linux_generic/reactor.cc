...
class EventfdReactor : public ReactorApi {
 public:
  ...
  void Run() override;
  void Stop() override;
  Reactor::Reactable* Register(int fd, common::Closure on_read_ready, common::Closure on_write_ready) override;
  ...
  void ModifyRegistration(
      Reactor::Reactable* reactable, common::Closure on_read_ready, common::Closure on_write_ready) override;
...
struct ReactorEventWithEventfd : public ReactorEventApi {
  ReactorEventWithEventfd() : fd_(eventfd(0, EFD_SEMAPHORE | EFD_NONBLOCK)) {
    ASSERT_LOG(fd_ != -1, "Unable to create nonblocking event file descriptor semaphore");
  }
  ~ReactorEventWithEventfd() {
    if (fd_ != -1) Close();
  }
  bool Read() override {
    uint64_t val = 0;
...
ReactorEventApi* NewReactorEventWithEventfd() {
  return new ReactorEventWithEventfd();
}
...
Reactor::Reactable* EventfdReactor::Register(int fd, common::Closure on_read_ready, common::Closure on_write_ready) {
...
void EventfdReactor::ModifyRegistration(
    Reactor::Reactable* reactable, common::Closure on_read_ready, common::Closure on_write_ready) {
  ASSERT(reactable != nullptr);
