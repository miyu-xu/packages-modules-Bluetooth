...
TEST_F(ConditionalWakeupAlarmTest, cancel_alarm_from_callback) {
  ...
  alarm_->Schedule(
          ...
          kDelay, kDeadline);
  fake_timer_advance(kDeadline.count());
  ASSERT_EQ(std::future_status::ready, future.wait_for(kShortWait));
...
TEST_F(TwoConditionalWakeupAlarmTest, schedule_from_alarm_long) {
  ...
  alarm_->Schedule(BindOnce(
                   ...
                   kDelay, kDeadline);
  fake_timer_advance(kDelay.count());
  EXPECT_EQ(std::future_status::ready, future.wait_for(kShortWait));
  fake_timer_advance(kDelay.count());
  EXPECT_EQ(std::future_status::ready, future2.wait_for(kShortWait));
}
