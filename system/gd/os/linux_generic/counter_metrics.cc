...
const int CONTER_METRICS_PERDIOD_MINUTES = 360; // Drain counters every 6 hours

const ModuleFactory CounterMetrics::Factory = ModuleFactory([]() { return new CounterMetrics(); });

CounterMetrics& CounterMetrics::GetInstance() {
  static CounterMetrics counter_metrics;
...
void CounterMetrics::Start() {
  alarm_ = std::make_unique<RepeatingAlarm>(GetHandler());
  alarm_->Schedule(
      common::Bind(&CounterMetrics::DrainBufferedCounters,
           bluetooth::common::Unretained(this)),
      std::chrono::minutes(CONTER_METRICS_PERDIOD_MINUTES));
...
void CounterMetrics::WriteCounter(int32_t key, int64_t count) {
  LogMetricBluetoothCodePathCounterMetrics(key, count);
}
