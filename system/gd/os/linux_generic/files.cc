...
bool WriteToFile(const std::string& path, const std::string& data) {
  ...
  {
    // Make a temporary variable as inputs to dirname() will be modified and return value points
    // to input char array temp_path_for_dir must not be destroyed until results from dirname is
