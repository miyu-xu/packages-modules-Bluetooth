...
TEST_F(ReactorTest, modify_registration) {
  ...
  auto write_result = eventfd_write(fake_reactable.fd_, FakeReactable::kSetPromise);
  EXPECT_EQ(write_result, 0);
  EXPECT_EQ(future.wait_for(10ms), std::future_status::ready);
  EXPECT_EQ(future.get(), kReadReadyValue);

  /* Disable on_read callback */
  reactor_->ModifyRegistration(reactable, false, true);
  ...
  EXPECT_EQ(write_result, 0);
  EXPECT_NE(future.wait_for(10ms), std::future_status::ready);
