...
// When it's constructed, it will register two reactables on the specified thread; when it's
// destroyed, it will unregister itself from the thread. This alarm tries to piggyback on CPU
// wakeups triggered by others in order to save battery. If there were no CPU wakeups until the
// deadline, this alarm wakes up CPU and runs the task. Note that the deadline wakeup will not work
// if __ANDROID__ is not defined.
...
public:
  ...
  // Note that the deadline wakeup will not work if __ANDROID__ is not defined.
