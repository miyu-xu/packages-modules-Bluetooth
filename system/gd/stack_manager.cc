...
std::chrono::milliseconds StackManager::get_gd_stack_timeout_ms(bool is_start) {
  auto gd_timeout = std::chrono::milliseconds(is_start ? 3000 : 5000);
  {
    auto gd_timeout_prop = os::GetSystemProperty(
        is_start ? "ro.bluetooth.gd_start_timeout" : "ro.bluetooth.gd_stop_timeout");
    if (gd_timeout_prop) {
      auto timeout = common::Uint64FromString(gd_timeout_prop.value());
      if (timeout) {
        gd_timeout = std::chrono::milliseconds(timeout.value());
      }
    }
  }
  return gd_timeout;
