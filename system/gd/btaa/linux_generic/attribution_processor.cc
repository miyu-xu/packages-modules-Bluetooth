...
void AttributionProcessor::OnBtaaPackets(std::vector<BtaaHciPacket> btaa_packets) {
  ...
  for (auto& btaa_packet : btaa_packets) {
    ...
    if (wakeup_) {
      ...
      std::string packageInfo = kUnknownPackageInfo;
      std::string address = btaa_packet.address.ToString();
      if (address_app_mapper_.find(address) != address_app_mapper_.end()) {
        packageInfo = address_app_mapper_[address];
      }
      app_wakeup_aggregator_.Push(std::move(AppWakeupDescriptor(btaa_packet.activity, packageInfo)));
    }
  }
  wakeup_ = false;
}
...
void AttributionProcessor::NotifyActivityAttributionInfo(
  ...
  if (address_app_mapper_.find(deviceAddress) == address_app_mapper_.end()) {
    address_app_mapper_[deviceAddress] = {};
  }
  address_app_mapper_[deviceAddress] = packageName + "/" + std::to_string(uid);
}
...
void AttributionProcessor::Dump(
  ...
  for (auto& it : app_wakeup_aggregator) {
    ...
    wakeup_entry_builder.add_wakeup_time(fb_builder->CreateString(
        bluetooth::common::StringFormatTimeWithMilliseconds(kActivityAttributionTimeFormat, wakeup_time).c_str()));
    wakeup_entry_builder.add_activity(fb_builder->CreateString((ActivityToString(it.entry.activity_))));
    wakeup_entry_builder.add_app(fb_builder->CreateString(it.entry.package_info_));
  ...
  for (auto& it : app_activity_aggregator_) {
    ...
    app_entry_builder.add_app(fb_builder->CreateString(it.first.app));
  ...
  builder.add_title_wakeup(title_device_wakeup);
  builder.add_num_wakeup(device_wakeup_aggregator.size());
  builder.add_wakeup_attribution(device_wakeup_entries);
