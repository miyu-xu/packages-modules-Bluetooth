  ...
  for (auto& btaa_packet : btaa_packets) {
    ...
    if (wakeup_) {
      ...
      if (address_app_mapper_.find(address) != address_app_mapper_.end()) {
        package_info = address_app_mapper_[address];
      }
...
void AttributionProcessor::NotifyActivityAttributionInfo(
  ...
  if (address_app_mapper_.find(device_address) == address_app_mapper_.end()) {
    address_app_mapper_[device_address] = {};
  }
  address_app_mapper_[device_address] = package_name + "/" + std::to_string(uid);
}
