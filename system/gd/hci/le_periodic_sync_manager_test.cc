...
TEST_F(PeriodicSyncManagerTest, handle_new_advertising_sync_after_timeout) {
  uint16_t sync_handle = 0x12;
  Address address;
  Address::FromString("00:11:22:33:44:55", address);
  AddressWithType address_with_type = AddressWithType(address, AddressType::PUBLIC_DEVICE_ADDRESS);
...
TEST_F_WITH_FLAGS(
    PeriodicSyncManagerTest,
    handle_new_advertising_sync_after_timeout_without_sync_established_callback,
    REQUIRES_FLAGS_ENABLED(
        ACONFIG_FLAG(TEST_BT, leaudio_broadcast_assistant_force_queue_advance))) {
  uint16_t sync_handle = 0x12;
  Address address;
  Address::FromString("00:11:22:33:44:55", address);
  AddressWithType address_with_type = AddressWithType(address, AddressType::PUBLIC_DEVICE_ADDRESS);
