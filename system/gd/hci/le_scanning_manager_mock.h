...
class MockScanningCallback : public ScanningCallback {
  ...
  MOCK_METHOD(
      ...
      (int, uint8_t, uint16_t, uint8_t, uint8_t, Address, uint8_t, uint16_t));
  MOCK_METHOD(
      void, OnPeriodicSyncReport, (uint16_t, int8_t, int8_t, uint8_t, std::vector<uint8_t>));
...
class MockLeScanningManager : public LeScanningManager {
 public:
  ...
  MOCK_METHOD(void, StartSync, (uint8_t, const Address&, uint16_t, uint16_t, int));
