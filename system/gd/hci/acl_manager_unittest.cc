...
TEST_F(AclManagerWithCallbacksTest, test_disconnection_after_request) {
  ...
  test_hci_layer_->IncomingEvent(ConnectionRequestBuilder::Create(remote, ClassOfDevice({1,2,3}), ConnectionRequestLinkType::ACL));
  test_hci_layer_->IncomingEvent(ConnectionCompleteBuilder::Create(ErrorCode::REMOTE_USER_TERMINATED_CONNECTION, 0, remote, LinkType::ACL, Enable::DISABLED));
}
...
TEST_F(AclManagerWithCallbacksTest, test_disconnection_after_request_sync) {
  ...
  Address remote = *Address::FromString("12:34:56:78:9a:bc");
  EXPECT_CALL(mock_connection_callbacks_, OnConnectRequest).WillOnce([&request_promise](){request_promise.set_value();});
  test_hci_layer_->IncomingEvent(ConnectionRequestBuilder::Create(remote, ClassOfDevice({1,2,3}), ConnectionRequestLinkType::ACL));
