...
class MockConnectionCallback : public ConnectionCallbacks {
 public:
  void OnConnectSuccess(std::unique_ptr<ClassicAclConnection> connection) override {
    LOG_INFO("On connect success");
    // Convert to std::shared_ptr during push_back()
    connections_.push_back(std::move(connection));
    if (callback_connection_promise_ != nullptr) {
      callback_connection_promise_->set_value();
      callback_connection_promise_.reset();
    }
  }
  ...
  std::unique_ptr<std::promise<void>> callback_connection_promise_;
};
...
class MockLeConnectionCallbacks : public LeConnectionCallbacks {
 public:
  void OnLeConnectSuccess(AddressWithType address_with_type, std::unique_ptr<LeAclConnection> connection) override {
    ...
    if (le_callback_connection_promise_ != nullptr) {
      le_callback_connection_promise_->set_value();
      le_callback_connection_promise_.reset();
    }
  }
  ...
  std::unique_ptr<std::promise<void>> le_callback_connection_promise_;
};
...
class AclManagerWithCallbacksTest : public AclManagerNoCallbacksTest {
 protected:
  ...
  std::future<void> GetConnectionFuture() {
    mock_connection_callbacks_.callback_connection_promise_ = std::make_unique<std::promise<void>>();
    return mock_connection_callbacks_.callback_connection_promise_->get_future();
  }

  std::future<void> GetLeConnectionFuture() {
    mock_le_connection_callbacks_.le_callback_connection_promise_ = std::make_unique<std::promise<void>>();
    return mock_le_connection_callbacks_.le_callback_connection_promise_->get_future();
  }
...
TEST_F(AclManagerNoCallbacksTest, unregister_classic_before_connection_request) {
  ...
  sync_client_handler();
  //  auto connection_future_status = connection_future.wait_for(2s);
...
TEST_F(AclManagerWithCallbacksTest, two_remote_connection_requests_ABAB) {
  ...
  ASSERT_EQ(0UL, NumberOfConnections());
  {
    ...
    ASSERT_EQ(GetLastConnection()->GetAddress(), remote_device[0].address)
...

  {
    ...
    ASSERT_EQ(GetLastConnection()->GetAddress(), remote_device[1].address)
