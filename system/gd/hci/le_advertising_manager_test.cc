...
public:
    MOCK_METHOD4(OnAdvertisingSetStarted,
                 void(int reg_id, uint8_t advertiser_id, int8_t tx_power,
    ...
    MOCK_METHOD3(OnAdvertisingParametersUpdated,
                 void(uint8_t advertiser_id, int8_t tx_power, uint8_t status));
    MOCK_METHOD2(OnPeriodicAdvertisingParametersUpdated,
                 void(uint8_t advertiser_id, uint8_t status));
    ...
    MOCK_METHOD3(OnPeriodicAdvertisingEnabled,
                 void(uint8_t advertiser_id, bool enable, uint8_t status));
...
TEST_F_WITH_FLAGS(LeExtendedAdvertisingManagerTest,
  ...
  for (size_t i = 0; i < adv_opcodes.size(); i++) {
    ...
    if (adv_opcodes[i] == OpCode::LE_SET_EXTENDED_ADVERTISING_PARAMETERS) {
      ...
      test_hci_layer_->IncomingEvent(LeSetExtendedScanResponseDataCompleteBuilder::Create(
              uint8_t{ 1 }, ErrorCode::SUCCESS));
    } else if (adv_opcodes[i] == OpCode::LE_SET_EXTENDED_ADVERTISING_DATA) {
      test_hci_layer_->IncomingEvent(LeSetExtendedAdvertisingDataCompleteBuilder::Create(
              uint8_t{ 1 }, ErrorCode::SUCCESS));
    } else {
      test_hci_layer_->IncomingEvent(CommandCompleteBuilder::Create(
              uint8_t{ 1 }, adv_opcodes[i], std::make_unique<RawBuilder>(success_vector)));
...
TEST_F(LeAndroidHciAdvertisingAPITest, set_data_test) {
  ...
  EXPECT_CALL(
          mock_advertising_callback_,
          OnScanResponseDataSet(advertiser_id_, AdvertisingCallback::AdvertisingStatus::SUCCESS));
  test_hci_layer_->IncomingEvent(LeMultiAdvtCompleteBuilder::Create(
          uint8_t{ 1 }, ErrorCode::SUCCESS, SubOcf::SET_SCAN_RESP));
}
...
TEST_F_WITH_FLAGS(LeExtendedAdvertisingAPITest, set_data_valid_max_251_ad_data_length_test,
                  REQUIRES_FLAGS_ENABLED(ACONFIG_FLAG(TEST_BT, divide_long_single_gap_data))) {
  // Set advertising data
  std::vector<GapData> advertising_data{};
  // set data 251 bytes
...
TEST_F_WITH_FLAGS(LeExtendedAdvertisingAPITest, set_data_valid_252_ad_data_length_fragments_test,
                  REQUIRES_FLAGS_ENABLED(ACONFIG_FLAG(TEST_BT, divide_long_single_gap_data))) {
  // Set advertising data
  std::vector<GapData> advertising_data{};
  // set data 252 bytes
...
TEST_F_WITH_FLAGS(LeExtendedAdvertisingAPITest, set_data_with_invalid_256_ad_data_length_test,
                  REQUIRES_FLAGS_ENABLED(ACONFIG_FLAG(TEST_BT, divide_long_single_gap_data))) {
  // Set advertising data with data that greater than le_maximum_advertising_data_length_
  std::vector<GapData> advertising_data{};
...
TEST_F_WITH_FLAGS(LeExtendedAdvertisingAPITest, set_periodic_data_valid_max_252_ad_data_length_test,
                  REQUIRES_FLAGS_ENABLED(ACONFIG_FLAG(TEST_BT, divide_long_single_gap_data))) {
  // Set advertising data
  std::vector<GapData> advertising_data{};
...
TEST_F_WITH_FLAGS(LeExtendedAdvertisingAPITest,
...
TEST_F_WITH_FLAGS(LeExtendedAdvertisingAPITest, set_periodic_data_invalid_256_ad_data_length_test,
                  REQUIRES_FLAGS_ENABLED(ACONFIG_FLAG(TEST_BT, divide_long_single_gap_data))) {
  // Set advertising data
  std::vector<GapData> advertising_data{};
