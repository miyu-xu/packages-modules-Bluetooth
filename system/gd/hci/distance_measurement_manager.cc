...
struct DistanceMeasurementManager::impl : bluetooth::hal::RangingHalCallback {
  ...
  void init_cs_tracker(const Address& cs_remote_address, uint16_t connection_handle,
    ...
    if (cs_trackers_[connection_handle].repeating_alarm == nullptr) {
      // the tracker was used as reflector.
  ...
  void send_le_cs_read_remote_supported_capabilities(uint16_t connection_handle) {
    hci_layer_->EnqueueCommand(
            ...
            handler_->BindOnceOn(this,
                                 &impl::on_cs_setup_command_status_cb<
                                         LeCsReadRemoteSupportedCapabilitiesStatusView>,
                                 connection_handle));
  }
  ...
  void send_le_cs_security_enable(uint16_t connection_handle) {
    hci_layer_->EnqueueCommand(
            ...
            handler_->BindOnceOn(this,
                                 &impl::on_cs_setup_command_status_cb<LeCsSecurityEnableStatusView>,
                                 connection_handle));
  }
  ...
  void send_le_cs_create_config(uint16_t connection_handle) {
    ...
    hci_layer_->EnqueueCommand(
            ...
            handler_->BindOnceOn(this,
                                 &impl::on_cs_setup_command_status_cb<LeCsCreateConfigStatusView>,
                                 connection_handle));
  }
  ...
  static inline void reset_tracker_on_stopped(CsTracker* cs_tracker) {
  ...
  void send_le_cs_procedure_enable(uint16_t connection_handle, Enable enable) {
    ...
    hci_layer_->EnqueueCommand(
            ...
            handler_->BindOnceOn(
                    this, &impl::on_cs_setup_command_status_cb<LeCsProcedureEnableStatusView>,
                    connection_handle));
  }

  template <class T>
  void on_cs_setup_command_status_cb(uint16_t connection_handle, CommandStatusView status_view) {
    log::assert_that(status_view.IsValid(), "assert failed: view.IsValid()");
    auto view = T::Create(status_view);
