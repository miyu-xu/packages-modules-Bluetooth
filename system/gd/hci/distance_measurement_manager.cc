...
struct DistanceMeasurementManager::impl : bluetooth::hal::RangingHalCallback {
  ...
  struct CsTrackerState {
    static constexpr uint8_t kUnspecified = 0x00;
    static constexpr uint8_t kStopped = 1 << 0;
    static constexpr uint8_t kInit = 1 << 1;
    static constexpr uint8_t kRasConnected = 1 << 2;
    static constexpr uint8_t kWaitForConfigComplete = 1 << 3;
    static constexpr uint8_t kWaitForSecurityEnabled = 1 << 4;
    static constexpr uint8_t kWaitForProcedureEnabled = 1 << 5;
    static constexpr uint8_t kStarted = 1 << 6;
  };

  struct CsTracker {
    uint8_t state = CsTrackerState::kStopped;
    Address address;
    ...
    // TODO: clean up, replace the measurement_ongoing with kStopped
  ...
  void init_cs_requester_tracker(const Address& cs_remote_address, uint16_t connection_handle,
    ...
    it->second.state = CsTrackerState::kInit;
  ...
  void handle_ras_client_connected_event(
    ...
    it->second.state = CsTrackerState::kRasConnected;
  ...
  void send_le_cs_security_enable(uint16_t connection_handle) {
    ...
    cs_requester_trackers_[connection_handle].state = CsTrackerState::kWaitForSecurityEnabled;
    hci_layer_->EnqueueCommand(
  ...
  void send_le_cs_create_config(uint16_t connection_handle, uint8_t config_id) {
    ...
    cs_requester_trackers_[connection_handle].state = CsTrackerState::kWaitForConfigComplete;
  ...
  static void reset_tracker_on_stopped(CsTracker& cs_tracker) {
    ...
    cs_tracker.state = CsTrackerState::kStopped;
  }
  ...
  void send_le_cs_procedure_enable(uint16_t connection_handle, Enable enable) {
    ...
    if (enable == Enable::ENABLED) {
      if (it->second.state == CsTrackerState::kStopped) {
      ...
      it->second.state = CsTrackerState::kWaitForProcedureEnabled;
    } else {  // Enable::DISABLE
      if (it->second.state != CsTrackerState::kWaitForProcedureEnabled &&
          it->second.state != CsTrackerState::kStarted) {
  ...
  void on_cs_security_enable_complete(LeCsSecurityEnableCompleteView event_view) {
    ...
    auto res_it = cs_responder_trackers_.find(connection_handle);
    if (res_it != cs_responder_trackers_.end() &&
        res_it->second.state == CsTrackerState::kWaitForSecurityEnabled) {
      res_it->second.state = CsTrackerState::kWaitForProcedureEnabled;
    }
  }
  ...
  void on_cs_config_complete(LeCsConfigCompleteView event_view) {
    ...
    check_and_handle_conflict(connection_handle, config_id, CsTrackerState::kWaitForConfigComplete);
    uint8_t valid_requester_states = CsTrackerState::kWaitForConfigComplete;
    // any state, as the remote can start over at any time.
    uint8_t valid_responder_states = CsTrackerState::kUnspecified;
    ...
    if (!live_tracker->local_start) {
      live_tracker->state = CsTrackerState::kWaitForSecurityEnabled;
    }
  }
  ...
  CsTracker* get_live_tracker(uint16_t connection_handle, uint8_t config_id,
    ...
    if (req_it != cs_requester_trackers_.end() &&
        req_it->second.state != CsTrackerState::kStopped && req_it->second.config_id == config_id &&
        (valid_requester_states & req_it->second.state) != 0) {
    ...
    if (res_it != cs_responder_trackers_.end() &&
        ...
        (valid_responder_states == CsTrackerState::kUnspecified ||
         (valid_responder_states & res_it->second.state) != 0)) {
  ...
  void check_and_handle_conflict(uint16_t connection_handle, uint8_t event_config_id,
                                 uint8_t expected_requester_state) {
  ...
  void on_cs_procedure_enable_complete(LeCsProcedureEnableCompleteView event_view) {
    ...
    if (event_view.GetState() == Enable::ENABLED) {
      check_and_handle_conflict(connection_handle, config_id,
                                CsTrackerState::kWaitForProcedureEnabled);
      uint8_t valid_requester_states = CsTrackerState::kWaitForProcedureEnabled;
      uint8_t valid_responder_states = CsTrackerState::kStopped | CsTrackerState::kInit |
                                       CsTrackerState::kStarted |
                                       CsTrackerState::kWaitForProcedureEnabled;
      ...
      live_tracker->state = CsTrackerState::kStarted;
      ...
      uint8_t valid_requester_states = CsTrackerState::kStarted;
      uint8_t valid_responder_states = CsTrackerState::kStarted;
      live_tracker = get_live_tracker(connection_handle, config_id, valid_requester_states,
                                      valid_responder_states);
      if (live_tracker == nullptr) {
        log::error("no tracker is available for {}", connection_handle);
  ...
  void handle_remote_data(const Address address, uint16_t connection_handle,
    ...
    if (cs_requester_trackers_[connection_handle].state != CsTrackerState::kStarted) {
