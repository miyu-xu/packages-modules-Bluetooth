...
struct DistanceMeasurementManager::impl {
  ...
  void start_distance_measurement_with_cs(
    ...
    if (cs_trackers_.find(connection_handle != cs_trackers_.end() && cs_trackers_[connection_handle].address != cs_remote_address) {
      LOG_WARN("Remove old tracker for %s ", ADDRESS_TO_LOGGABLE_CSTR(address));
  ...
  void send_le_cs_read_remote_supported_capabilities(uint16_t connection_handle) {
    hci_layer_->EnqueueCommand(
        ...
        handler_->BindOnce(this, check_status<LeCsReadRemoteSupportedCapabilitiesStatusView>));
  }

  void send_le_cs_security_enable(uint16_t connection_handle) {
    hci_layer_->EnqueueCommand(
        LeCsSecurityEnableBuilder::Create(connection_handle),
        handler_->BindOnce(this, check_status<LeCsSecurityEnableStatusView>));
  }
