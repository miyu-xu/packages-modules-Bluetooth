...
static constexpr uint8_t kConfigId = 0x01;
static constexpr uint8_t kMinMainModeSteps = 0x02;
static constexpr uint8_t kMaxMainModeSteps = 0x05;
static constexpr uint8_t kMainModeRepetition = 0x00;
static constexpr uint8_t kMode0Steps = 0x03;
static constexpr uint8_t kChannelMapRepetition = 0x01;
static constexpr uint8_t kCh3cJump = 0x03;
static constexpr uint16_t kMaxProcedureLen = 0xFFFF;
...
static constexpr uint32_t kMinSubeventLen = 0x0004E2;
static constexpr uint32_t kMaxSubeventLen = 0x3d0900;
static constexpr uint8_t kToneAntennaConfigSelection = 0x07;
...
struct DistanceMeasurementManager::impl {
  ...
  void send_le_cs_create_config(uint16_t connection_handle) {
    LOG_INFO("CYDBG send_le_cs_create_config");
  ...
  void send_le_cs_set_procedure_parameters(uint16_t connection_handle) {
    LOG_INFO("CYDBG send_le_cs_set_procedure_parameters");
  ...
  void send_le_cs_procedure_enable(uint16_t connection_handle, uint8_t enable) {
    hci_layer_->EnqueueCommand(
  ...
  void on_cs_config_complete(LeCsConfigCompleteView event_view) {
    LOG_INFO("CYDBG on_cs_config_complete");
    if (!event_view.IsValid()) {
    ...
    LOG_INFO("on_cs_config_complete role is %s", CsRoleText(event_view.GetRole()).c_str());
  ...
  void on_cs_set_procedure_parameters(CommandCompleteView view) {
    LOG_INFO("CYDBG on_cs_set_procedure_parameters");
    ...
    if (cs_trackers_[connection_handle].role == CsRole::INITIATOR) {
      send_le_cs_procedure_enable(complete_view.GetConnectionHandle(), 0x01);
    }
  }
