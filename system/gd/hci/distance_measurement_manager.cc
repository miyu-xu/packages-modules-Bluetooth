...
struct DistanceMeasurementManager::impl : bluetooth::hal::RangingHalCallback {
  struct CsProcedureData {
    ...
    // Initiator's PCT (complex value) measured from mode-2 or mode-3 steps in a CS procedure
    // (in time order)
    std::vector<std::vector<std::complex<double>>> tone_pct_initiator;
    // Reflector's PCT (complex value) measured from mode-2 or mode-3 steps in a CS procedure
    // (in time order)
  ...
  void on_cs_subevent(LeMetaEventView event) {
    ...
    log::debug(
            "Connection_handle {}, procedure_done_status: {}, subevent_done_status: {}, "
            "counter: "
            "{}",
    ...
    if (procedure_done_status == CsProcedureDoneStatus::ABORTED ||
      ...
      log::warn(
              "Received CS Subevent with procedure_abort_reason:{}, "
      ...
      for (uint8_t i = 0; i < subevent_header.num_steps_reported_; i++) {
        ...
        switch (step_mode.mode_type_) {
          ...
          case 2: {
            ...
            // Parse in ascending order of antenna position with tone extension data at
            // the end
  ...
  void check_cs_procedure_complete(CsProcedureData* procedure_data, uint16_t connection_handle) {
    if (procedure_data->local_status == CsProcedureDoneStatus::ALL_RESULTS_COMPLETE &&
      ...
      log::debug(
              "Procedure complete counter:{} data size:{}, main_mode_type:{}, "
              "sub_mode_type:{}",
    ...
    for (auto result_data_structure : result_data_structures) {
      ...
      switch (mode) {
        ...
        case 2: {
          ...
          // Parse in ascending order of antenna position with tone extension data at the
          // end
          uint16_t num_tone_data = num_antenna_paths + 1;
