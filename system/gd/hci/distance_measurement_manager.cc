...
#define PRIVATE_ADDRESS(address) address.ToString().substr(8, 17).insert(0, "XX:XX:XX").c_str()
...
static constexpr uint8_t kRSSIDropOffAt1M = 41;
...
struct DistanceMeasurementManager::impl {
  ...
  void start_distance_measurement(
      Address address, uint16_t frequency, DistanceMeasurementMethod method) {
    LOG_INFO("Address:%s, method:%d", PRIVATE_ADDRESS(address), method);
  ...
  void stop_distance_measurement(Address address, DistanceMeasurementMethod method) {
    LOG_INFO("Address:%s, method:%d", PRIVATE_ADDRESS(address), method);
    switch (method) {
      case METHOD_AUTO:
      case METHOD_RSSI: {
        if (rssi_trackers.find(address) == rssi_trackers.end()) {
          LOG_WARN("Can't find rssi tracker for %s ", PRIVATE_ADDRESS(address));
        } else {
          hci_layer_->EnqueueCommand(
  ...
  void read_rssi_regularly(Address address, uint16_t frequency) {
    if (rssi_trackers.find(address) == rssi_trackers.end()) {
      LOG_WARN("Can't find rssi tracker for %s ", PRIVATE_ADDRESS(address));
      return;
    }
    uint16_t connection_handle = acl_manager_->HACK_GetLeHandle(address);
    if (connection_handle == kIllegalConnectionHandle) {
      LOG_WARN("Can't find connection for %s ", PRIVATE_ADDRESS(address));
      if (rssi_trackers.find(address) != rssi_trackers.end()) {
  ...
  void on_set_transmit_power_reporting_enable_complete(Address address, CommandCompleteView view) {
    ...
    if (rssi_trackers.find(address) == rssi_trackers.end()) {
      ...
      LOG_INFO("Track rssi for address %s", PRIVATE_ADDRESS(address));
  ...
  void on_read_rssi_complete(Address address, CommandCompleteView view) {
    ...
    distance_measurement_callbacks_->OnDistanceMeasurementResult(
        address, distance * 100, 0, DistanceMeasurementMethod::METHOD_RSSI);
  }
...
void DistanceMeasurementManager::StopDistanceMeasurement(
    Address address, DistanceMeasurementMethod method) {
  CallOn(pimpl_.get(), &impl::stop_distance_measurement, address, method);
}

#undef PRIVATE_ADDRESS
