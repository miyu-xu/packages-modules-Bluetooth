...
struct DistanceMeasurementManager::impl : bluetooth::hal::RangingHalCallback {
  ...
  void OnHandleVendorSpecificReplyComplete(uint16_t connection_handle, bool success) {
    ...
    distance_measurement_callbacks_->OnHandleVendorSpecificReplyComplete(
            cs_responder_trackers_[connection_handle].address, success);
  }
  ...
  void OnResult(uint16_t connection_handle, const bluetooth::hal::RangingResult& ranging_result) {
    ...
    distance_measurement_callbacks_->OnDistanceMeasurementResult(
            ...
            0.0, -1, -1, -1, -1, elapsedRealtimeNanos, DistanceMeasurementMethod::METHOD_CS);
  }
  ...
  void init_cs_requester_tracker(const Address& cs_remote_address, uint16_t connection_handle,
                                 hci::Role local_hci_role, uint16_t interval) {
    if (cs_requester_trackers_.find(connection_handle) != cs_requester_trackers_.end() &&
        cs_requester_trackers_[connection_handle].address != cs_remote_address) {
      log::debug("Remove old tracker for {}", cs_remote_address);
      cs_requester_trackers_.erase(connection_handle);
    }
    if (cs_requester_trackers_.find(connection_handle) == cs_requester_trackers_.end()) {
      cs_requester_trackers_[connection_handle].address = cs_remote_address;
    }
    // make sure the repeating_alarm is initialized.
    if (cs_requester_trackers_[connection_handle].repeating_alarm == nullptr) {
      cs_requester_trackers_[connection_handle].repeating_alarm =
              std::make_unique<os::RepeatingAlarm>(handler_);
    }
    cs_requester_trackers_[connection_handle].interval_ms = interval;
    cs_requester_trackers_[connection_handle].local_start = true;
    cs_requester_trackers_[connection_handle].measurement_ongoing = true;
    cs_requester_trackers_[connection_handle].waiting_for_start_callback = true;
    cs_requester_trackers_[connection_handle].local_hci_role = local_hci_role;
    cs_requester_trackers_[connection_handle].config_id = kConfigId;
  }
  ...
  void stop_distance_measurement(const Address address, uint16_t connection_handle,
    ...
    switch (method) {
      ...
      case METHOD_CS: {
        ...
        } else if (cs_requester_trackers_[connection_handle].measurement_ongoing) {
          cs_requester_trackers_[connection_handle].repeating_alarm->Cancel();
  ...
  void handle_ras_server_vendor_specific_reply(
    ...
    if (cs_responder_trackers_[connection_handle].address != address) {
      log::info("the cs tracker address was changed as {}, not {}.",
                cs_responder_trackers_[connection_handle].address, address);
      return;
    }
    if (ranging_hal_->IsBound()) {
  ...
  void handle_ras_server_connected(const Address& identity_address, uint16_t connection_handle,
    ...
    if (cs_responder_trackers_.find(connection_handle) != cs_responder_trackers_.end() &&
        cs_responder_trackers_[connection_handle].address != identity_address) {
      log::debug("Remove old tracker for {}", identity_address);
      cs_responder_trackers_.erase(connection_handle);
    }
    cs_responder_trackers_[connection_handle].address = identity_address;
    cs_responder_trackers_[connection_handle].local_start = false;
    cs_responder_trackers_[connection_handle].local_hci_role = local_hci_role;
  }
  ...
  void handle_ras_server_disconnected(const Address& identity_address, uint16_t connection_handle) {
    ...
    if (cs_responder_trackers_[connection_handle].address != identity_address) {
  ...
  void handle_cs_setup_failure(uint16_t connection_handle, DistanceMeasurementErrorCode errorCode) {
    ...
    if (cs_requester_trackers_[connection_handle].measurement_ongoing) {
      if (cs_requester_trackers_[connection_handle].waiting_for_start_callback) {
        distance_measurement_callbacks_->OnDistanceMeasurementStartFail(
                cs_requester_trackers_[connection_handle].address, errorCode, METHOD_CS);
      } else {
        distance_measurement_callbacks_->OnDistanceMeasurementStopped(
                cs_requester_trackers_[connection_handle].address, errorCode, METHOD_CS);
      }
      cs_requester_trackers_[connection_handle].repeating_alarm->Cancel();
      cs_requester_trackers_[connection_handle].repeating_alarm.reset();
    }
    reset_tracker_on_stopped(cs_requester_trackers_[connection_handle]);
  ...
  void send_le_cs_procedure_enable(uint16_t connection_handle, Enable enable) {
    ...
    if (!cs_requester_trackers_[connection_handle].measurement_ongoing) {
      log::error("error state, no local measurement request.");
      if (cs_requester_trackers_[connection_handle].repeating_alarm) {
        cs_requester_trackers_[connection_handle].repeating_alarm->Cancel();
      }
      return;
    }
    ...
    hci_layer_->EnqueueCommand(
            LeCsProcedureEnableBuilder::Create(
                    connection_handle, cs_requester_trackers_[connection_handle].config_id, enable),
  ...
  void on_cs_read_remote_supported_capabilities_complete(
    ...
    if (cs_responder_trackers_.find(connection_handle) != cs_responder_trackers_.end()) {
      cs_responder_trackers_[connection_handle].remote_support_phase_based_ranging =
              event_view.GetOptionalSubfeaturesSupported().phase_based_ranging_ == 0x01;
    }
    send_le_cs_set_default_settings(connection_handle);

    if (cs_requester_trackers_.find(connection_handle) != cs_requester_trackers_.end() &&
        cs_requester_trackers_[connection_handle].measurement_ongoing) {
      cs_requester_trackers_[connection_handle].remote_support_phase_based_ranging =
              event_view.GetOptionalSubfeaturesSupported().phase_based_ranging_ == 0x01;
      cs_requester_trackers_[connection_handle].setup_complete = true;
  ...
  void on_cs_security_enable_complete(LeCsSecurityEnableCompleteView event_view) {
    ...
    if (cs_requester_trackers_.find(connection_handle) != cs_requester_trackers_.end() &&
        cs_requester_trackers_[connection_handle].measurement_ongoing &&
        !cs_requester_trackers_[connection_handle].config_set) {
      cs_requester_trackers_[connection_handle].config_set = true;
  ...
  CsTracker* get_live_tracker(uint16_t connection_handle, uint8_t config_id) {
    ...
    if (cs_requester_trackers_.find(connection_handle) != cs_requester_trackers_.end() &&
        cs_requester_trackers_[connection_handle].measurement_ongoing &&
        cs_requester_trackers_[connection_handle].config_id == config_id) {
