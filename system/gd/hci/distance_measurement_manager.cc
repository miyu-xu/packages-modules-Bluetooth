...
struct DistanceMeasurementManager::impl {
  struct CsProcedureData {
    CsProcedureData(uint8_t procedure_counter, uint8_t num_antenna_paths)
  ...
  void on_cs_subevent(LeMetaEventView event, bool local) {
    if (!event.IsValid()) {
      LOG_ERROR("Received invalid LeMetaEventView");
      return;
    }
    ...
    uint16_t counter = local ? cs_trackers_[connection_handle].local_counter
                             : cs_trackers_[connection_handle].remote_counter;
    ...
    if (procedure_done_status == CsProcedureDoneStatus::ABORTED ||
      ...
      LOG_WARN(
          "Received CS Subevent with abort reason: %02x, connection_handle:%d, counter:%d, %s",
          abort_reason,
          connection_handle,
          counter,
          local ? "local" : "remote");
    ...
    // Parse data from resultDataStructures
    ...
    if (abort_reason != 0) {
      ...
      CsRole role = cs_trackers_[connection_handle].role;
      if (!local) {
        // Get remote role
        role = cs_trackers_[connection_handle].role == CsRole::INITIATOR ? CsRole::REFLECTOR
                                                                         : CsRole::INITIATOR;
      }
      parse_cs_result_data(result_data_structures, *procedure_data, role);
    }
    // Update procedure status
    if (local) {
      procedure_data->local_status = procedure_done_status;
    } else {
      procedure_data->remote_status = procedure_done_status;
    }
  ...
  void check_cs_procedure_complete(CsProcedureData* procedure_data, uint16_t connection_handle) {
    if (procedure_data->local_status == CsProcedureDoneStatus::ALL_RESULTS_COMPLETE &&
      ...
      LOG_DEBUG(
          "--------------------- Procedure complete counter:%d data size:%d, main_mode_type:%d, "
          "sub_mode_type:%d---------------------",
      ...
      switch (mode) {
        case 0: {
          if (role == CsRole::INITIATOR) {
            auto tone_data_view = LeCsMode0InitatorDataView::Create(packet_bytes_view);
            if (!tone_data_view.IsValid()) {
            ...
            procedure_data.measured_freq_offset.push_back(tone_data_view.GetMeasuredFreqOffset());
          } else {
            auto tone_data_view = LeCsMode0ReflectorDataView::Create(packet_bytes_view);
            if (!tone_data_view.IsValid()) {
              LOG_WARN("Received invalid mode %d data, role:%s", mode, CsRoleText(role).c_str());
              print_raw_data(result_data_structure.step_data_);
              continue;
        ...
        case 2: {
          auto tone_data_view = LeCsMode2DataView::Create(packet_bytes_view);
          if (!tone_data_view.IsValid()) {
            LOG_WARN("Received invalid mode %d data, role:%s", mode, CsRoleText(role).c_str());
            print_raw_data(result_data_structure.step_data_);
            continue;
          ...
          auto tone_data = tone_data_view.GetToneData();
          uint8_t permutation_index = tone_data_view.GetAntennaPermutationIndex();
          // Parse in ascending order of antenna position with tone extension data at the end
          for (uint16_t k = 0; k <= num_antenna_paths; k++) {
