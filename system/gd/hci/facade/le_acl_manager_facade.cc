...
#undef GET_CONNECTION
  ...
  void on_incoming_acl(std::shared_ptr<LeAclConnection> connection, uint16_t handle) {
    LOG_INFO("handle=%d, addr=%s", connection->GetHandle(), ADDRESS_TO_LOGGABLE_CSTR(connection->GetRemoteAddress()));
  ...
  void OnLeConnectSuccess(AddressWithType peer, std::unique_ptr<LeAclConnection> connection) override {
    LOG_INFO("handle=%d, addr=%s", connection->GetHandle(), ADDRESS_TO_LOGGABLE_CSTR(peer));
    std::unique_lock<std::mutex> lock(acl_connections_mutex_);
    ...
    if (role == Role::PERIPHERAL) {
      ...
      ASSERT_LOG(
          per_connection_events_.count(peer) > 0, "No connection request for %s", ADDRESS_TO_LOGGABLE_CSTR(peer));
    }
  ...
  void OnLeConnectFail(AddressWithType address, ErrorCode reason, bool locally_initiated) override {
    LOG_INFO("addr=%s, reason=%s", ADDRESS_TO_LOGGABLE_CSTR(address), ErrorCodeText(reason).c_str());
    std::unique_ptr<BasePacketBuilder> builder = LeConnectionCompleteBuilder::Create(
