...
std::vector<uint8_t> msft_read_supported_features_complete{
    ...
    0x87,
    0x80,  // _payload_: MSFT event prefix provided by Intel
};
TEST(HciPacketsTest, testMsftReadSupportedFeaturesComplete) {
  ...
  ASSERT_EQ((uint8_t)0x02, view.GetPrefixLength());
  ASSERT_EQ(2ul, view.GetPayload().size());
}
