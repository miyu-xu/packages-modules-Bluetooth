...
public:
  ...
  void on_le_connection_complete(LeMetaEventView packet) {
    ...
    if (std::holds_alternative<DataAsUninitializedPeripheral>(role_specific_data)) {
      // the OnLeConnectSuccess event will be sent after receiving the On Advertising Set
      // Terminated event, since we need it to know what local_address / advertising set the
      // peer connected to. In the meantime, we store it as a pending_connection.
  ...
  RoleSpecificData initialize_role_specific_data(Role role) {
    if (role == hci::Role::CENTRAL) {
      ...
      // When we receive "set terminated event", we associate connection handle with
      // advertiser address
      return DataAsUninitializedPeripheral{};
    } else {
      // the exception is if we only support legacy advertising - here, our current address is
      // also our advertised address
    ...
    connections.execute(handle, [=, this](LeConnectionManagementCallbacks* /* callbacks */) {
      ...
      le_acl_connection_interface_->EnqueueCommand(
              LeRemoteConnectionParameterRequestReplyBuilder::Create(
                      handle, request_view.GetIntervalMin(), request_view.GetIntervalMax(),
                      request_view.GetLatency(), request_view.GetTimeout(), 0, 0),
  ...
  void update_connectability_state_after_armed(const ErrorCode& status) {
    switch (connectability_state_) {
      ...
      case ConnectabilityState::DISARMING:
        log::error(
                "Received connectability arm notification for unexpected state:{} "
                "status:{}",
  ...
  void on_extended_create_connection(CommandStatusView status) {
    ...
    log::assert_that(status.GetCommandOpCode() == OpCode::LE_EXTENDED_CREATE_CONNECTION,
  ...
  void arm_connectability() {
    ...
    if (accept_list.empty()) {
      log::info(
              "Ignored request to re-arm le connection state machine when filter accept list "
              "is "
              "empty");
      return;
    }
    ...
    switch (connectability_state_) {
      ...
      case ConnectabilityState::ARMING:
        if (already_in_accept_list) {
          ...
          // Ignored, if we add new device to the filter accept list, create connection
          // command will be sent by OnResume.
