...
public:
  ...
  void choose_connection_mode(size_t num_acl_connections, uint16_t& conn_interval_min,
    ...
    uint32_t aggressive_connection_threshold = get_aggressive_conn_threshold();
    ...
    switch (connection_mode) {
      case ConnectionMode::AGGRESSIVE:
        conn_interval_min = get_min_conn_interval_aggressive();
        conn_interval_max = get_max_conn_interval_aggressive();
        stack::l2cap::get_interface().L2CA_AdjustConnectionIntervals(
                &conn_interval_min, &conn_interval_max, BTM_BLE_CONN_INT_MIN);
        break;
      case ConnectionMode::RELAXED:
      default:
        conn_interval_min = get_min_conn_interval_relaxed();
        conn_interval_max = get_max_conn_interval_relaxed();
        break;
    }
    log::info("Connection mode: {}", connection_mode_to_string(connection_mode));
    log::debug("conn_interval_min={}, conn_interval_max={}", conn_interval_min, conn_interval_max);
  }
