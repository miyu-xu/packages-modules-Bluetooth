 ...
 public:
  ...
  void on_create_connection_cancel_complete(CommandCompleteView view) {
    ...
    if (complete_view.GetStatus() != ErrorCode::SUCCESS) {
      ...
      if (status == ErrorCode::COMMAND_DISALLOWED && pause_connection) {
        LOG_WARN("No HCI_LE_Create_Connection or HCI_LE_Extended_Create_Connection command is pending, AckPause");
        le_address_manager_->AckPause(this);
        return;
      }
    }
