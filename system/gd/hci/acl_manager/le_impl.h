...
struct le_impl : public bluetooth::hci::LeAddressManagerCallback {
  ...
  struct {
   ...
   public:
    ...
    void execute(
      ...
      if (remove_afterwards) remove_connection(handle);
    }
    void send_packet(uint16_t handle, std::function<void(struct acl_manager::assembler* assembler)> cb) {
      std::unique_lock<std::mutex> lock(le_acl_connections_guard_);
      auto connection = le_acl_connections_.find(handle);
      cb((connection != le_acl_connections_.end()) ? connection->second.assembler_ : nullptr);
    }
    le_acl_connection& add(
        uint16_t handle,
  ...
  void send_packet(uint16_t handle, std::function<void(struct acl_manager::assembler* assembler)> cb) {
    callbacks.send_packet(handle, cb);
  }
  ...
  void on_le_connection_complete(LeMetaEventView packet) {
    ...
    auto& connection_proxy = callbacks.add(handle, remote_address, queue->GetDownEnd(), handler_);
    ...
    connection_proxy.le_connection_management_callbacks_ =
        connection->GetEventCallbacks([this](uint16_t handle) { this->callbacks.invalidate(handle); });
  ...
  void on_le_connection_update_complete(LeMetaEventView view) {
    ...
    callbacks.execute(handle, [=](LeConnectionManagementCallbacks* callbacks) {
  ...
  uint16_t HACK_get_handle(Address address) {
    return callbacks.HACK_get_handle(address);
  }
  ...
  void check_for_unregister() {
    if (callbacks.is_empty() && connecting_le_.empty() && address_manager_registered && ready_to_unregister) {
      le_address_manager_->Unregister(this);
