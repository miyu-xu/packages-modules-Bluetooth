...
constexpr bool kEnableBleConnInitMorePhy = true;
...
static const std::string kPropertyEnableBleConnInitMorePhy =
    "bluetooth.core.gap.le.connection_initiating_more_phy.enabled";
 ...
 public:
  ...
  void arm_connectability() {
    ...
    if (controller_->IsSupported(OpCode::LE_EXTENDED_CREATE_CONNECTION)) {
      bool init_more_phy = os::GetSystemPropertyBool(kPropertyEnableBleConnInitMorePhy, kEnableBleConnInitMorePhy);
      ...
      if (controller_->SupportsBle2mPhy() && init_more_phy) {
      ...
      if (controller_->SupportsBleCodedPhy() && init_more_phy) {
