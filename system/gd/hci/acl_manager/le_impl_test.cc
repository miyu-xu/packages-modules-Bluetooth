...
template <typename T>
T CreateCommand(std::shared_ptr<std::vector<uint8_t>> bytes) {
  T command;
  return command;
}
...
class LeImplTest : public ::testing::Test {
 protected:
  ...
  void SendCommandComplete(OpCode op_code, ErrorCode error_code) {
    std::vector<uint8_t> success_vector{static_cast<uint8_t>(error_code)};
    auto bytes = std::make_shared<std::vector<uint8_t>>();
    packet::BitInserter bi(*bytes);
    auto builder =
        CommandCompleteBuilder::Create(uint8_t{1}, op_code, std::make_unique<packet::RawBuilder>(success_vector));
    builder->Serialize(bi);
    auto view = CommandCompleteView::Create(EventView::Create(PacketView<kLittleEndian>(bytes)));
    le_impl_->le_address_manager_->OnCommandComplete(view);
  }
