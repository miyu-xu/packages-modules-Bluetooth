...
class LeImplTest : public ::testing::Test {
 ...
 protected:
  ...
  MockLeConnectionManagementCallbacks connection_managment_callbacks_;
  ...
  void SetUp() override {
    ...
    EXPECT_CALL(mock_le_connection_callbacks_, OnLeConnectSuccess(_, _))
        .WillOnce([&](AddressWithType addr, std::unique_ptr<LeAclConnection> conn) {
          ...
          connection_->RegisterCallbacks(&connection_managment_callbacks_, handler_);
        });
...
TEST_F(LeImplWithConnectionTest, on_le_event__PHY_UPDATE_COMPLETE) {
  ...
  // Send a phy update
  {
    EXPECT_CALL(connection_managment_callbacks_, OnPhyUpdate(_, _, _))
...
TEST_F(LeImplWithConnectionTest, on_le_event__DATA_LENGTH_CHANGE) {
  ...
  {
    EXPECT_CALL(connection_managment_callbacks_, OnDataLengthChange(_, _, _, _))
