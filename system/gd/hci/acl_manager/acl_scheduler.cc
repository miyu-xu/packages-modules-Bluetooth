...
struct AclScheduler::impl {
  ...
  void CancelAclConnection(
    ...
    if (!ok) {
      LOG_ERROR("Attempted to cancel connection to %s that does not exist", ADDRESS_TO_LOGGABLE_CSTR(address));
    }
  }
  ...
  void CancelRemoteNameRequest(Address address, common::ContextualOnceCallback<void()> cancel_request) {
    ...
    if (!ok) {
      LOG_ERROR("Attempted to cancel remote name request to %s that does not exist", ADDRESS_TO_LOGGABLE_CSTR(address));
    }
  };
