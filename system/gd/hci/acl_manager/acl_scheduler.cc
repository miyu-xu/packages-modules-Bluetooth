...
struct AclScheduler::impl {
 public:
  ...
  void EnqueueRemoteNameRequest(Address address, common::ContextualOnceCallback<void()> start_request) {
    pending_outgoing_connections_.push_back(RemoteNameRequestQueueEntry{address, std::move(start_request)});
  }
 ...
 private:
  ...
  template <typename T>
  bool cancel_outgoing_or_queued_connection(
      T matcher,
      common::ContextualOnceCallback<void()> cancel_outgoing,
      common::ContextualOnceCallback<void()> cancelled_queued) {
    // Check if relevant connection is currently outgoing
    if (outgoing_entry_.has_value()) {
      if (matcher(outgoing_entry_.value())) {
        cancel_outgoing.Invoke();
        return true;
      }
    }
