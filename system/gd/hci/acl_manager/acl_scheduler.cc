...
struct AclScheduler::impl {
  ...
  void CancelAclConnection(
    ...
    auto ok = cancel_outgoing_or_queued_connection(
        ...
        [&]([[maybe_unused]] auto entry) { cancel_connection_completed.Invoke(); });
    if (!ok) {
  ...
  void ReportRemoteNameRequestCompletion([[maybe_unused]] Address address) {
    if (!outgoing_entry_.has_value()) {
...
void AclScheduler::ListDependencies([[maybe_unused]] ModuleList* list) const {}
