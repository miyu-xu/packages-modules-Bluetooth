  ...
  return std::visit(
      [](auto&& data) {
        ...
        if constexpr (std::is_same_v<T, DataAsUninitializedPeripheral>) {
          // This case should never happen outside of acl_manager.cc, since once the connection is passed into the
          // OnConnectSuccess callback, it should be fully initialized.
  ...
  return std::visit(
      [](auto&& data) {
        ...
        if constexpr (std::is_same_v<T, DataAsCentral>) {
          return Role::CENTRAL;
        } else if constexpr (std::is_same_v<T, DataAsPeripheral> || std::is_same_v<T, DataAsUninitializedPeripheral>) {
