  ...
  return std::visit(
          [](auto&& data) {
            ...
            if constexpr (std::is_same_v<T, DataAsUninitializedPeripheral>) {
              // This case should never happen outside of acl_manager.cc, since once the
              // connection is passed into the OnConnectSuccess callback, it should be fully
              // initialized.
...
bool LeAclConnection::LeReadRemoteFeatures() {
  pimpl_->tracker.le_acl_connection_interface_->EnqueueCommand(
          ...
          pimpl_->tracker.client_handler_->BindOnce([](CommandStatusView status) {
            log::assert_that(status.IsValid(), "assert failed: status.IsValid()");
            log::assert_that(status.GetCommandOpCode() == OpCode::LE_READ_REMOTE_FEATURES,
                             "assert failed: status.GetCommandOpCode() == "
