...
void LeAclConnection::OnSubrateRequestComplete(CommandStatusView status) {
  ASSERT(status.IsValid());
  ASSERT(status.GetCommandOpCode() == OpCode::LE_SUBRATE_REQUEST);
  ...
  if (hci_status != ErrorCode::SUCCESS) {
    LOG_INFO("SubrateRequest status %s", ErrorCodeText(hci_status).c_str());
    pimpl_->tracker.OnLeSubrateChange(hci_status, 0, 0, 0, 0);
  }
}

void LeAclConnection::SubrateRequest(
  ...
  pimpl_->tracker.le_acl_connection_interface_->EnqueueCommand(
      ...
      pimpl_->tracker.client_handler_->BindOnceOn(this, &LeAclConnection::OnSubrateRequestComplete));
}
