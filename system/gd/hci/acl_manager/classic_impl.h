 ...
 public:
  ...
  void create_and_announce_connection(
    ...
    connections.execute(address, [=](ConnectionManagementCallbacks* callbacks) {
      if (delayed_role_change_ == nullptr) {
        ...
        LOG_INFO("Sending delayed role change for %s", ADDRESS_TO_LOGGABLE_CSTR(delayed_role_change_->GetBdAddr()));
    ...
    acl_scheduler_->ReportAclConnectionCompletion(
        ...
        handler_->BindOnce(
            [=](AclScheduler* scheduler, Address address, ErrorCode status, std::string valid_incoming_addresses) {
              ...
              // TODO: do we need to obfuscate valid_incoming_addresses?
              LOG_WARN(
  ...
  void on_role_change(EventView packet) {
    ...
    if (!sent) {
      if (delayed_role_change_ != nullptr) {
        LOG_WARN(
            "Second delayed role change (@%s dropped)", ADDRESS_TO_LOGGABLE_CSTR(delayed_role_change_->GetBdAddr()));
      }
      LOG_INFO(
