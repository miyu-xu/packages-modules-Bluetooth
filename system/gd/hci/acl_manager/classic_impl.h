...
struct classic_impl : public security::ISecurityManagerListener {
 ...
 public:
  ...
  void create_connection(Address address) {
    ...
    acl_scheduler_->EnqueueAclCreateConnection({address, std::move(packet)}, handler_.BindOnceOn(this, [](auto entry) {
      if (is_classic_link_already_connected(entry.address)) {
        LOG_WARN("already connected: %s", entry.address.ToString().c_str());
        acl_connection_interface_->ReportAclConnectionCompletion(entry.address);
      }
  ...
  void on_connection_complete(EventView packet) {
    ...
    acl_scheduler_->ReportAclConnectionCompletion(
        ...
        handle_->BindOnce(
            [](std::string valid_incoming_addresses, Address address, ErrorCode status) {
              ASSERT_LOG(
