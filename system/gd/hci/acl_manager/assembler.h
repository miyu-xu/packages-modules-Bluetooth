...
size_t GetL2capPduSize(packet::PacketView<packet::kLittleEndian> first) {
  if (first.size() < 2) {
    LOG_INFO("Controller sent a fragment of %d bytes.", static_cast<int>(first.size()));
    return kL2capBasicFrameHeaderSize;  // We need at least 4 bytes to send it to L2CAP
  }
  return (static_cast<size_t>(first[1]) << 8u) + first[0];
}
...
struct assembler {
  ...
  void on_incoming_packet(AclView packet) {
    ...
    if (GetL2capPduSize(recombination_stage_) + kL2capBasicFrameHeaderSize <
        recombination_stage_.size()) {
      LOG_INFO("Packet size doesn't match L2CAP header, dropping it.");
      recombination_stage_ = PacketViewForRecombination();
      return;
    } else if (
        GetL2capPduSize(recombination_stage_) + kL2capBasicFrameHeaderSize >
        recombination_stage_.size()) {
