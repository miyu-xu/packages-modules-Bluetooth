...
TEST_F(AclSchedulerTest, ThreeConnectionsQueue) {
  ...
  EXPECT_EQ(future1.wait_for(timeout), std::future_status::timeout);
  ...
  EXPECT_EQ(future1.wait_for(timeout), std::future_status::ready);
  EXPECT_EQ(future2.wait_for(timeout), std::future_status::timeout);

  // second connection fails, so third one should start
  acl_scheduler_->ReportOutgoingAclConnectionFailure();

  // the third connection has started
  EXPECT_EQ(future2.wait_for(timeout), std::future_status::ready);
}
...
TEST_F(AclSchedulerTest, TiebreakForOutgoingConnection) {
  ...
  // this preserves working behavior, it is not based on any principled decision (so if you need to break this test, go
  // for it)
  EXPECT_EQ(future.wait_for(timeout), std::future_status::ready);
}
