...
struct DataAsPeripheral {
  ...
  AddressWithType local_address;
  // the advertising set ID that the peer connected to - in LE, our role is peripheral iff the peer initiated a
  // connection to our advertisement
...
using RoleSpecificData = std::variant<DataAsUninitializedPeripheral, DataAsCentral, DataAsPeripheral>;
