...
void LeConnectlistManager::RegisterTimeoutCallback(
    common::ContextualCallback<void(AddressWithType, ErrorCode)> on_timeout) {
  on_timeout_ = on_timeout;
}

void LeConnectlistManager::UnregisterTimeoutCallback() {
  on_timeout_ = {};
}
...
ConnectionResult LeConnectlistManager::OnConnectionComplete(
  ...
  if (role == hci::Role::CENTRAL) {
    ...
    auto connecting_addr_with_type = connecting_le_.find(remote_address);
    if (connecting_addr_with_type == connecting_le_.end()) {
      LOG_WARN("No prior connection request for %s", ADDRESS_TO_LOGGABLE_CSTR(remote_address));
    }
