...
class PeriodicSyncManager {
 public:
  ...
  void HandleLePeriodicAdvertisingReport(LePeriodicAdvertisingReportView event_view) {
    ...
    LOG(INFO) << __func__ << " Data Status " << (uint16_t)event_view.GetDataStatus();
    LOG(INFO) << __func__ << " Data Size " << base::HexEncode(event_view.GetData().data(), event_view.GetData().size());
    ...
    LOG(INFO) << __func__ << " Pseudo Address " << pseudo_address.ToString();
    ...
    std::vector<uint8_t> complete_advertising_data = event_view.GetData();
    // std::vector<uint8_t> complete_advertising_data;
    // complete_advertising_data.insert(complete_advertising_data.end(), complete_advertising_data_->begin(), complete_advertising_data_->end());
    ...
    for(int i = 0; i < (int)complete_advertising_data.size(); i++) {
      if (complete_advertising_data[i] == (uint8_t)GapDataType::ENCRYPTED_ADVERTISING_DATA) {
        encrypted_data = true;
        LOG(INFO) << __func__ << " BDA: "<<pseudo_address.ToString().c_str()<< " Found Periodic Encrypted Data Index " << i;
      }
    }
    LOG(INFO) << " BDA: " << pseudo_address.ToString().c_str() << " Data: " <<
 ...
 private:
  // std::list<PeriodicSyncStates>::iterator GetAddressFromHandle(uint16_t handle) {
  //   for (auto it = periodic_syncs_.begin(); it != periodic_syncs_.end(); it++) {
  //     if (it->sync_handle == handle) && it->
  //   }
  // }
