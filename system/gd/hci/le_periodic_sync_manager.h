 ...
 public:
  ...
  void HandleLePeriodicAdvertisingReport(LePeriodicAdvertisingReportView event_view) {
    ...
    std::vector<uint8_t> complete_advertising_data(
        complete_advertising_data_->begin(), complete_advertising_data_->end());
    if (bluetooth::common::init_flags::encrypted_advertising_is_enabled()) {
      ...
      Address pseudo_address = bluetooth::shim::legacy::identity_to_pseudo_random(
          advertiser_address, address_type, false);
      ...
      for (int i = 0; i < (int)complete_advertising_data.size(); i++) {
        if (complete_advertising_data[i] == (uint8_t)GapDataType::ENCRYPTED_ADVERTISING_DATA &&
          ...
          if (bluetooth::common::init_flags::encrypted_advertising_log_is_enabled() &&
              ...
              LOG_INFO(
                  "BDA: %s Found Periodic Encrypted Data Index %d ",
                  ADDRESS_TO_LOGGABLE_CSTR(pseudo_address),
                  i);
          }
        }
      }
      LOG_INFO(
          "BDA: %s Periodic Data  %s ",
          ADDRESS_TO_LOGGABLE_CSTR(pseudo_address),
          base::HexEncode(complete_advertising_data.data(), complete_advertising_data.size())
              .c_str());
      if (encrypted_data) {
        enc_adv_data_map = scanning_reassembler_->GetEncAdvFieldsInfo(
            complete_advertising_data.data(), complete_advertising_data.size());

        LOG_INFO(
            "Advertising Data Before Decryption %s",
            base::HexEncode(complete_advertising_data.data(), complete_advertising_data.size())
                .c_str());
        std::vector<uint8_t> decrypted_data = scanning_reassembler_->ProcessEncryptedData(
