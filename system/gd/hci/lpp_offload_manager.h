...
public:
  ...
  virtual void SetAclCredits(int link, int credit) override;
  virtual void NotifyAclConnectionStateChange(uint16_t handle, int link, int state) override;
  virtual void NotifyAclLeDataLengthChange(uint16_t handle, int txDataLen, int rxDataLen) override;
  virtual void NotifyAclPowerModeChange(uint16_t handle, int mode, uint16_t interval) override;
