...
struct LppOffloadManager::impl : bluetooth::hal::SocketHalCallback {
  ...
  bool set_acl_credits(int link, int credit) {
    if (!is_lpp_offload_enabled_) {
      return false;
    }
    log::info("");
    return socket_hal_->SetAclCredits(link, credit);
  }

  bool notify_acl_connection_state_change(int handle, int link, int state) {
    if (!is_lpp_offload_enabled_) {
      return false;
    }
    log::info("");
    return socket_hal_->NotifyAclConnectionStateChange(handle, link, state);
  }

  bool notify_acl_le_data_length_change(int handle, int txDataLen, int rxDataLen) {
    if (!is_lpp_offload_enabled_) {
      return false;
    }
    log::info("");
    return socket_hal_->NotifyAclLeDataLengthChange(handle, txDataLen, rxDataLen);
  }

  bool notify_acl_pm_change(int handle, int mode, int interval) {
    if (!is_lpp_offload_enabled_) {
      return false;
    }
    log::info("");
    return socket_hal_->NotifyAclPowerModeChange(handle, mode, interval);
  }
...
bool LppOffloadManager::SetAclCredits(int link, int credit) {
  return pimpl_->set_acl_credits(link, credit);
}

bool LppOffloadManager::NotifyAclConnectionStateChange(int handle, int link, int state) {
  return pimpl_->notify_acl_connection_state_change(handle, link, state);
}

bool LppOffloadManager::NotifyAclLeDataLengthChange(int handle, int txDataLen, int rxDataLen) {
  return pimpl_->notify_acl_le_data_length_change(handle, txDataLen, rxDataLen);
}

bool LppOffloadManager::NotifyAclPowerModeChange(int handle, int mode, int interval) {
  return pimpl_->notify_acl_pm_change(handle, mode, interval);
}
