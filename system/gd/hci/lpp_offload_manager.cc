...
struct LppOffloadManager::impl : bluetooth::hal::SocketHalCallback {
  ...
  void set_acl_credits(int link, int credit) {
    if (!is_lpp_offload_enabled_) {
      return;
    }
    log::info("");
    socket_hal_->SetAclCredits(link, credit);
  }

  void notify_acl_connection_state_change(uint16_t handle, int link, int state) {
    if (!is_lpp_offload_enabled_) {
      return;
    }
    log::info("");
    socket_hal_->NotifyAclConnectionStateChange(handle, link, state);
  }

  void notify_acl_le_data_length_change(uint16_t handle, int txDataLen, int rxDataLen) {
    if (!is_lpp_offload_enabled_) {
      return;
    }
    log::info("");
    socket_hal_->NotifyAclLeDataLengthChange(handle, txDataLen, rxDataLen);
  }

  void notify_acl_pm_change(uint16_t handle, int mode, uint16_t interval) {
    if (!is_lpp_offload_enabled_) {
      return;
    }
    log::info("");
    socket_hal_->NotifyAclPowerModeChange(handle, mode, interval);
  }
...
void LppOffloadManager::SetAclCredits(int link, int credit) {
  CallOn(pimpl_.get(), &impl::set_acl_credits, link, credit);
}

void LppOffloadManager::NotifyAclConnectionStateChange(uint16_t handle, int link, int state) {
  CallOn(pimpl_.get(), &impl::notify_acl_connection_state_change, handle, link, state);
}

void LppOffloadManager::NotifyAclLeDataLengthChange(uint16_t handle, int txDataLen, int rxDataLen) {
  CallOn(pimpl_.get(), &impl::notify_acl_le_data_length_change, handle, txDataLen, rxDataLen);
}

void LppOffloadManager::NotifyAclPowerModeChange(uint16_t handle, int mode, uint16_t interval) {
  CallOn(pimpl_.get(), &impl::notify_acl_pm_change, handle, mode, interval);
}

void LppOffloadManager::GetOffloadFeaturesSupported(LppOffloadFeatures& features) {
  return pimpl_->get_offload_features_supported(features);
}
