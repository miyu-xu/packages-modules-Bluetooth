...
static const Uuid kBaseLe = Uuid::From128BitLE(
    Uuid::UUID128Bit{{0xfb, 0x34, 0x9b, 0x5f, 0x80, 0x00, 0x00, 0x80, 0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}});
...
//  auto uuid2 = Uuid::FromLegacyConfigString(UUID_BASE_STR);
...
TEST(UuidTest, inequalities) {
  ...
  auto uuid1 = Uuid::FromString(UUID_EMP_STR);
  auto uuid2 = Uuid::FromString(UUID_ONES_STR);
...
TEST(UuidTest, endianness) {
  Uuid uuidBe = kBase;
  Uuid uuidLe = kBaseLe;
  Uuid uuid = uuidLe.To128BitBE();

  ASSERT_TRUE(uuid == uuidBe);
}
