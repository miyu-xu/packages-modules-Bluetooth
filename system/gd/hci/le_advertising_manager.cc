...
struct LeAdvertisingManager::impl : public bluetooth::hci::LeAddressManagerCallback {
  ...
  void start(os::Handler* handler, hci::HciLayer* hci_layer, hci::Controller* controller,
    ...
    if (num_instances_ > 2) {
      size_t original_num_instances = num_instances_;
      num_instances_ -= 2;
      LOG_INFO(
          "num_instances_ reduced from %d to %d to reserve 2 instances for connection",
          (uint16_t)(original_num_instances),
          (uint16_t)num_instances_);
    }
