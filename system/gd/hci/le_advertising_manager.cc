  ...
  AddressWithType new_advertiser_address(AdvertiserId id) {
    switch (advertising_sets_[id].address_type) {
      case AdvertiserAddressType::PUBLIC:
        // note: this doesn't work if we have a STATIC address mode (as opposed to PUBLIC), but
        // this never happens on dual-mode stacks since there we must use the classic address
  ...
  void create_extended_advertiser(
    ...
    switch (advertising_sets_[id].address_type) {
      ...
      case AdvertiserAddressType::NONRESOLVABLE_RANDOM:
        ...
        break;
      case AdvertiserAddressType::PUBLIC:
        // no need to set address, do nothing
        break;
      default:
        LOG_ALWAYS_FATAL(
