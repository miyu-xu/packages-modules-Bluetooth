...
struct LeAdvertisingManager::impl : public bluetooth::hci::LeAddressManagerCallback {
  ...
  GapData EncryptedAdvertising(Advertiser* adv_inst, std::vector<GapData> data) {
    ...
    if (enc_adv_data_log_enabled) {
      ...
      str << "\nRandomizer: " << base::HexEncode(adv_inst->randomizer.data(), adv_inst->randomizer.size())
    ...
    if (!set_scan_rsp) {
      ...
      for (unsigned int i = 0; i < data.size(); i++) {
        str << "\nData: " << base::HexEncode(data[i].data_.data(), data[i].data_.size())
            << " Data Type: " << data[i].data_type_
            << " Size: " << data[i].data_.size();
      }
      for (unsigned int i = 0; i < data_encrypt.size(); i++) {
        str << "\nData Encrypt: " << base::HexEncode(data_encrypt[i].data_.data(), data_encrypt[i].data_.size())
            << " Data Type: " << data_encrypt[i].data_type_
