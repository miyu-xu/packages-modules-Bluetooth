...
auto enc_adv_data_log_enabled =
                    os::GetSystemProperty("persist.vendor.btstack.enable.enc_adv_data_log");
...
struct LeAdvertisingManager::impl : public bluetooth::hci::LeAddressManagerCallback {
    ...
    switch (advertising_api_type_) {
      case (AdvertisingApiType::LEGACY): {
        if (config.advertising_type == AdvertisingType::ADV_IND ||
          ...
          set_data(id, true, config.scan_response, config.scan_response_enc);
        }
        set_data(id, false, config.advertisement, config.advertisement_enc);
        if (!paused) {
  ...
  void create_extended_advertiser_with_id(
    ...
    advertising_sets_[id].handler = handler;
    advertising_sets_[id].include_adi = config.periodic_advertising_parameters.include_adi;
    advertising_sets_[id].address_type = GetAdvertiserAddressTypeFromRequestedTypeAndPolicy(
        config.requested_advertiser_address_type, le_address_manager_->GetAddressPolicy());
    advertising_sets_[id].current_address = new_advertiser_address(id);
    ...
    if (!config.periodic_data.empty() || !config.periodic_data_enc.empty()) {
      ...
      set_periodic_data(id, config.periodic_data, config.periodic_data_enc);
  ...
  void rotate_advertiser_address(AdvertiserId advertiser_id) {
    ...
    if (advertising_api_type_ == AdvertisingApiType::EXTENDED) {
      ...
      if (!adv_inst->advertisement_enc.empty()) {
        LOG_INFO("Encrypted Advertisement");
        impl::set_data(advertiser_id, false, adv_inst->advertisement, adv_inst->advertisement_enc);
        impl::set_data(advertiser_id, true, adv_inst->scan_response, adv_inst->scan_response_enc);
        if(!adv_inst->periodic_data_enc.empty()) {
          LOG_INFO("Encrypted Periodic");
          impl::set_periodic_data(advertiser_id, adv_inst->periodic_data, adv_inst->periodic_data_enc);
        }
      } else if (!adv_inst->scan_response_enc.empty()) {
        LOG_INFO("Encrypted Scan Response");
        impl::set_data(advertiser_id, true, adv_inst->scan_response, adv_inst->scan_response_enc);
      } else if(!adv_inst->periodic_data_enc.empty()) {
        LOG_INFO("Encrypted Periodic Only");
        impl::set_periodic_data(advertiser_id, adv_inst->periodic_data, adv_inst->periodic_data_enc);
      }
    }
  }
  ...
  GapData EncryptedAdvertising(Advertiser* adv_inst, std::vector<GapData> data) {
    ...
    if (!adv_inst->enc_key_value.empty()) {
      ...
      std::optional<std::vector<uint8_t>> keyiv = std::move(storage_module_->GetBin("Adapter", "ENC_KEY_MATERIAL"));
      key_iv.clear();
    ...
    if (enc_adv_data_log_enabled) {
      if (!key.empty() && key.data() != NULL) {
        LOG_INFO("Encr Data Key Material (Key): %s", base::HexEncode(key.data(),key.size()).c_str());
      }
      if (!iv.empty() && iv.data() != NULL) {
        LOG_INFO("Encr Data Key Material (IV): %s", base::HexEncode(iv.data(),iv.size()).c_str());
      }
      LOG_INFO("Randomizer: %s\n Input: %s\n Nonce: %s\n Input AD: %s\n",
                    base::HexEncode(adv_inst->randomizer.data(), adv_inst->randomizer.size()).c_str(),
                    base::HexEncode(in.data(), in.size()).c_str(),
                    base::HexEncode(nonce.data(), nonce.size()).c_str(),
                    base::HexEncode(ad.data(), ad.size()).c_str());
    }
    ...
    if (enc_adv_data_log_enabled) {
      ...
      LOG_INFO("MIC: %s", base::HexEncode(out_tag.data(), out_tag.size()).c_str());
      LOG_INFO("Result: %d", result);
    }
    ED_AD_Data.data_.insert(ED_AD_Data.data_.end(), adv_inst->randomizer.rbegin(), adv_inst->randomizer.rend());
    ...
    if (enc_adv_data_log_enabled) {
      LOG_INFO("ED AD Data: %s",base::HexEncode(ED_AD_Data.data_.data(), ED_AD_Data.data_.size()).c_str());
    }
  ...
  void set_data(AdvertiserId advertiser_id, bool set_scan_rsp, std::vector<GapData> data, std::vector<GapData> data_encrypt) {
    // The Flags data type shall be included when any of the Flag bits are non-zero and the
    // advertising packet is connectable and discoverable.

    Advertiser *adv_inst = &advertising_sets_[advertiser_id];
    if (!set_scan_rsp) {
      LOG_INFO("Advertising Data");
      for (unsigned int i = 0; i < data.size(); i++) {
        LOG_INFO("Data: %s, Data Type: %hhu, Size: %lu ",
                base::HexEncode(data[i].data_.data(), data[i].data_.size()).c_str(),
                data[i].data_type_, data[i].data_.size());
      }
      for (unsigned int i = 0; i < data_encrypt.size(); i++) {
        LOG_INFO("Data Encrypt: %s, Data Type: %hhu, Size: %lu ",
                base::HexEncode(data_encrypt[i].data_.data(), data_encrypt[i].data_.size()).c_str(),
                data_encrypt[i].data_type_, data_encrypt[i].data_.size());
      }
      adv_inst->advertisement = data;
      adv_inst->advertisement_enc = data_encrypt;
    } else {
      LOG_INFO(" Scan Response Data");
      for (unsigned int i = 0; i < data.size(); i++) {
        LOG_INFO("Data: %s, Data Type: %hhu, Size: %lu ",
                base::HexEncode(data[i].data_.data(), data[i].data_.size()).c_str(),
                data[i].data_type_, data[i].data_.size());
    ...
    if(!data_encrypt.empty()) {
      hci_layer_->EnqueueCommand(
              hci::LeRandBuilder::Create(),
              module_handler_->BindOnceOn(this,
                  &impl::encrypted_advertising_complete<LeRandCompleteView>,
                  adv_inst,
                  advertiser_id,
                  set_scan_rsp,
                  data,
  ...
  void set_periodic_data(AdvertiserId advertiser_id, std::vector<GapData> data, std::vector<GapData> data_encrypt) {
    uint16_t data_len = 0;
    // check data size
    Advertiser *adv_inst = &advertising_sets_[advertiser_id];
    LOG_INFO("Periodic Advertising Data");
    for (unsigned int i = 0; i < data.size(); i++) {
      LOG_INFO("Data: %s, Data Type: %hhu, Size: %lu ",
                base::HexEncode(data[i].data_.data(), data[i].data_.size()).c_str(),
                data[i].data_type_, data[i].data_.size());
    ...
    if(!data_encrypt.empty()) {
      hci_layer_->EnqueueCommand(
              hci::LeRandBuilder::Create(),
              module_handler_->BindOnceOn(this,
                  &impl::encrypted_periodic_advertising_complete<LeRandCompleteView>,
                  adv_inst,
                  advertiser_id,
                  data,
                  data_encrypt));
  ...
  void GenerateKeyIV(EncrDataKey *KeyIV, int iteration, CommandCompleteView view) {
    ...
    if (iteration == 1) {
      ...
      LOG_INFO("Key: %s", base::HexEncode(KeyIV->key.data(), KeyIV->key.size()).c_str());
      LOG_INFO("IV: %s", base::HexEncode(KeyIV->iv.data(), KeyIV->iv.size()).c_str());
      ...
      storage_module_->SetBin("Adapter", "ENC_KEY_MATERIAL", completeKeyIV);
      bluetooth::shim::legacy::GAP_DB_Callback(completeKeyIV, GATT_UUID_GAP_ENC_KEY_MATERIAL);
    }
    else {
      std::optional<std::vector<uint8_t>> keyiv = std::move(storage_module_->GetBin("Adapter", "ENC_KEY_MATERIAL"));
      std::vector<uint8_t> enc_key_material(keyiv->begin(),keyiv->end());
      bluetooth::shim::legacy::GAP_DB_Callback(enc_key_material, GATT_UUID_GAP_ENC_KEY_MATERIAL);
    }
  ...
  void encrypted_periodic_advertising_complete(Advertiser* adv_inst, AdvertiserId advertiser_id,
    ...
    std::vector<uint8_t> temp_rand;
    uint64_t rand = status_view.GetRandomNumber();
    memcpy(finalresult, &rand, 5);
    for(int i = 0; i < 5; i++) {
      temp_rand.push_back(finalresult[i]);
    }
    ...
    LOG_INFO("Periodic Advertising Data %s", base::HexEncode(advertising_data.data(), advertising_data.size()).c_str());
    for (size_t i = 0; i < data.size(); i++) {
      if (data[i].size() > kLeMaximumFragmentLength) {
        LOG_WARN("AD data len shall not greater than %d", kLeMaximumFragmentLength);
        if (advertising_callbacks_ != nullptr) {
  ...
  void encrypted_advertising_complete(Advertiser* adv_inst, AdvertiserId advertiser_id,
      bool set_scan_rsp, std::vector<GapData> data, std::vector<GapData> data_encrypt,
      CommandCompleteView view) {
    ASSERT(view.IsValid());
    auto status_view = LeRandCompleteView::Create(view);
    ASSERT(status_view.IsValid());
    ...
    LOG_INFO("Advertising Data %s", base::HexEncode(advertising_data.data(), advertising_data.size()).c_str());
    if (advertising_api_type_ != AdvertisingApiType::EXTENDED && !check_advertising_data(data, false)) {
      if (set_scan_rsp) {
        advertising_callbacks_->OnScanResponseDataSet(
            advertiser_id, AdvertisingCallback::AdvertisingStatus::DATA_TOO_LARGE);
    ...
    if (!adv_inst->started) {
      if (!paused) {
        enable_advertiser(advertiser_id, true, adv_inst->duration, adv_inst->max_extended_advertising_events);
      } else {
        EnabledSet curr_set;
        curr_set.advertising_handle_ = advertiser_id;
  ...
  void enc_key_material_cb_cmpl(storage::StorageModule* storage_module_, hci::HciLayer* hci_layer_, os::Handler* handler) {
    std::optional<std::vector<uint8_t>> keyiv = std::move(storage_module_->GetBin("Adapter", "ENC_KEY_MATERIAL"));
    std::vector<uint8_t> enc_key_material;
    enc_key_material.insert(enc_key_material.end(), keyiv->begin(), keyiv->end());
    if (!storage_module_->HasProperty("Adapter", "ENC_KEY_MATERIAL") || enc_key_material.size() < ENC_KEY_MATERIAL_LEN) {
      LOG_INFO(" Encrypted Data Key Material not in Config");
      hci_layer_->EnqueueCommand(LeRandBuilder::Create(), handler->BindOnceOn(this, &impl::GenerateKeyIV<LeRandCompleteView>, key_iv, 1));
      hci_layer_->EnqueueCommand(LeRandBuilder::Create(), handler->BindOnceOn(this, &impl::GenerateKeyIV<LeRandCompleteView>, key_iv, 2));
      hci_layer_->EnqueueCommand(LeRandBuilder::Create(), handler->BindOnceOn(this, &impl::GenerateKeyIV<LeRandCompleteView>, key_iv, 3));
    } else {
...
void LeAdvertisingManager::enc_key_material_cb() {
  pimpl_->enc_key_material_cb_cmpl(
    GetDependency<storage::StorageModule>(),
    GetDependency<hci::HciLayer>(),
    GetHandler());
}
...
void LeAdvertisingManager::SetData(AdvertiserId advertiser_id, bool set_scan_rsp, std::vector<GapData> data, std::vector<GapData> data_encrypt) {
  CallOn(pimpl_.get(), &impl::set_data, advertiser_id, set_scan_rsp, data, data_encrypt);
}
...
void LeAdvertisingManager::SetPeriodicData(AdvertiserId advertiser_id, std::vector<GapData> data, std::vector<GapData> data_encrypt) {
  CallOn(pimpl_.get(), &impl::set_periodic_data, advertiser_id, data, data_encrypt);
}
