...
struct LeAdvertisingManager::impl : public bluetooth::hci::LeAddressManagerCallback {
  ...
  void multi_advertising_state_change(hci::VendorSpecificEventView event) {
    ...
    ASSERT(view.IsValid());
    LOG_ERROR("Instance: 0x%x StateChangeReason: 0x%x Handle: 0x%x Adress: %s", view.GetAdvertisingInstance(),
        view.GetStateChangeReason(), view.GetConnectionHandle(), advertising_sets_[view.GetAdvertisingInstance()].current_address.ToString().c_str());
    // view.GetStateChangeReason() 0x00: Connection received
    if(view.GetStateChangeReason() == 0x00)
    acl_manager_->OnAdvertisingSetTerminated(ErrorCode::SUCCESS, view.GetConnectionHandle(), advertising_sets_[view.GetAdvertisingInstance()].current_address);
  }
