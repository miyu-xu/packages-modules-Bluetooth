...
struct LeAdvertisingManager::impl : public bluetooth::hci::LeAddressManagerCallback {
  ...
  void handle_event(LeMetaEventView event) {
    switch (event.GetSubeventCode()) {
      ...
      default:
        LOG_INFO(
            "Unknown subevent in scanner %s",
            hci::SubeventCodeText(event.GetSubeventCode()).c_str());
    }
  }
  ...
  void handle_scan_request(LeScanRequestReceivedView event_view) {
    ...
    registered_handler_->Post(common::BindOnce(
        scan_callback_, event_view.GetScannerAddress(), event_view.GetScannerAddressType()));
  }
  ...
  void handle_set_terminated(LeAdvertisingSetTerminatedView event_view) {
    ...
    LOG_VERBOSE(
        "Received LE Advertising Set Terminated with status %s",
        ErrorCodeText(event_view.GetStatus()).c_str());
      ...
      if (advertising_sets_[advertiser_id].duration == 0 &&
        ...
        if (was_rotating_address) {
          ...
          advertising_sets_[advertiser_id].address_rotation_alarm->Schedule(
              common::BindOnce(
                  &impl::set_advertising_set_random_address_on_timer,
  ...
  AddressWithType gen_advertiser_address(AdvertiserId id) {
    if (le_address_manager_->SupportsPrivacy()) {
      if (advertising_sets_[id].requested_address_type ==
        ...
        return le_address_manager_->GetNonResolvableAddress();
      } else {
  ...
  void create_advertiser(
    ...
    switch (advertising_api_type_) {
      case (AdvertisingApiType::LEGACY): {
        ...
        advertising_sets_[id].current_address = gen_advertiser_address(id);
        if (!paused) {
  ...
  void create_extended_advertiser(
    ...
    switch (config.requested_advertiser_address_type) {
      ...
      case AdvertiserAddressType::NONRESOLVABLE_RANDOM:
        if (le_address_manager_->SupportsPrivacy()) {
          AddressWithType address_with_type = gen_advertiser_address(id);
          le_advertising_interface_->EnqueueCommand(
              hci::LeSetAdvertisingSetRandomAddressBuilder::Create(
                  id, address_with_type.GetAddress()),
          ...
          advertising_sets_[id].address_rotation_alarm =
              std::make_unique<os::Alarm>(module_handler_);
          ...
          le_advertising_interface_->EnqueueCommand(
              ...
              module_handler_->BindOnce(
                  impl::check_status<LeSetAdvertisingSetRandomAddressCompleteView>));
        }
        break;
  ...
  void rotate_advertiser_address(AdvertiserId advertiser_id) {
    if (advertising_api_type_ == AdvertisingApiType::EXTENDED) {
      AddressWithType address_with_type = gen_advertiser_address(advertiser_id);
      le_advertising_interface_->EnqueueCommand(
          hci::LeSetAdvertisingSetRandomAddressBuilder::Create(
              advertiser_id, address_with_type.GetAddress()),
          module_handler_->BindOnceOn(
              this,
              &impl::on_set_advertising_set_random_address_complete<
                  LeSetAdvertisingSetRandomAddressCompleteView>,
      ...
      if (set_scan_rsp) {
        le_advertising_interface_->EnqueueCommand(
            hci::LeSetExtendedScanResponseDataBuilder::Create(
                advertiser_id, operation, kFragment_preference, data),
            module_handler_->BindOnce(
                impl::check_status<LeSetExtendedScanResponseDataCompleteView>));
      } else {
        le_advertising_interface_->EnqueueCommand(
            hci::LeSetExtendedAdvertisingDataBuilder::Create(
                advertiser_id, operation, kFragment_preference, data),
            module_handler_->BindOnce(
                impl::check_status<LeSetExtendedAdvertisingDataCompleteView>));
      }
    }
  }
  ...
  void set_periodic_parameter(
    ...
    le_advertising_interface_->EnqueueCommand(
        ...
        module_handler_->BindOnceOn(
            this,
            &impl::check_status_with_id<LeSetPeriodicAdvertisingParamCompleteView>,
            advertiser_id));
  }
  ...
  void on_read_advertising_physical_channel_tx_power(CommandCompleteView view) {
    ...
    if (complete_view.GetStatus() != ErrorCode::SUCCESS) {
      LOG_INFO(
          "Got a command complete with status %s",
  ...
  void on_set_extended_advertising_parameters_complete(AdvertiserId id, CommandCompleteView view) {
    ...
    if (advertising_sets_[id].started && id_map_[id] != kIdLocal) {
      advertising_callbacks_->OnAdvertisingParametersUpdated(
          id, advertising_sets_[id].tx_power, advertising_status);
    }
  }

  template <class View>
  void on_set_periodic_advertising_enable_complete(
      bool enable, AdvertiserId id, CommandCompleteView view) {
    ASSERT(view.IsValid());
  ...
  void check_status_with_id(AdvertiserId id, CommandCompleteView view) {
    ...
    if (status_view.GetStatus() != ErrorCode::SUCCESS) {
      LOG_INFO(
          "Got a command complete with status %s", ErrorCodeText(status_view.GetStatus()).c_str());
      advertising_status = AdvertisingCallback::AdvertisingStatus::INTERNAL_ERROR;
    }
...
AdvertiserId LeAdvertisingManager::ExtendedCreateAdvertiser(
  ...
  if (config.interval_min > config.interval_max) {
    LOG_INFO(
        "Advertising interval: min (%hu) > max (%hu)", config.interval_min, config.interval_max);
    CallOn(
        pimpl_.get(),
        &impl::start_advertising_fail,
        reg_id,
...
void LeAdvertisingManager::RegisterAdvertiser(
  ...
  if (id == kInvalidId) {
    std::move(callback).Run(
        kInvalidId, AdvertisingCallback::AdvertisingStatus::TOO_MANY_ADVERTISERS);
  } else {
...
void LeAdvertisingManager::SetPeriodicParameters(
    AdvertiserId advertiser_id, PeriodicAdvertisingParameters periodic_advertising_parameters) {
  CallOn(
      pimpl_.get(), &impl::set_periodic_parameter, advertiser_id, periodic_advertising_parameters);
}
...
void LeAdvertisingManager::EnablePeriodicAdvertising(
    AdvertiserId advertiser_id, bool enable, bool include_adi) {
