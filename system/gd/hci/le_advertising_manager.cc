...
struct LeAdvertisingManager::impl : public bluetooth::hci::LeAddressManagerCallback {
  ...
  void start(os::Handler* handler, hci::HciLayer* hci_layer, hci::Controller* controller,
    ...
    if (controller_->SupportsBleExtendedAdvertising()) {
      ...
      if (vendor_version != 0 && vendor_version <= 11 && os::IsRootCanalEnabled()) {
        log::info(
                "LeReadAdvertisingPhysicalChannelTxPower is not supported on Android R "
                "RootCanal, "
                "default to 0");
  ...
  void create_extended_advertiser_with_id(
    ...
    if (advertising_sets_[id].current_address.GetAddressType() !=
      ...
      // if we aren't using the public address type at the HCI level, we need to set the
      // random address
      le_advertising_interface_->EnqueueCommand(
  ...
  bool check_advertising_data(std::vector<GapData> data, bool include_flag) {
    ...
    // advertising packet is connectable and discoverable. It will be added by set_data()
    // function, we should count it here.
        ...
        if (set_scan_rsp) {
          le_advertising_interface_->EnqueueCommand(
                  hci::LeSetExtendedScanResponseDataBuilder::Create(advertiser_id, operation,
                                                                    kFragment_preference, data),
                  module_handler_->BindOnce(
                          check_complete<LeSetExtendedScanResponseDataCompleteView>));
        } else {
          le_advertising_interface_->EnqueueCommand(
                  hci::LeSetExtendedAdvertisingDataBuilder::Create(advertiser_id, operation,
                                                                   kFragment_preference, data),
                  module_handler_->BindOnce(
                          check_complete<LeSetExtendedAdvertisingDataCompleteView>));
        }
      }
    }
  }
    ...
    if (com::android::bluetooth::flags::divide_long_single_gap_data()) {
      ...
      if (operation == Operation::COMPLETE_ADVERTISEMENT || operation == Operation::LAST_FRAGMENT) {
        ...
        le_advertising_interface_->EnqueueCommand(
                hci::LeSetPeriodicAdvertisingDataBuilder::Create(advertiser_id, operation, data),
                module_handler_->BindOnce(
                        check_complete<LeSetPeriodicAdvertisingDataCompleteView>));
      }
    }
  }
  ...
  // *after* the resume commands from address manager, which is racey (even if it might not
  // matter).
  //
  ...
  void check_status_with_id(bool send_callback, AdvertiserId id, CommandCompleteView view) {
    ...
    // Do not trigger callback if the advertiser not stated yet, or the advertiser is not
    // register from Java layer
    if (advertising_callbacks_ == nullptr || !advertising_sets_[id].started ||
        id_map_[id] == kIdLocal) {
      return;
    }
