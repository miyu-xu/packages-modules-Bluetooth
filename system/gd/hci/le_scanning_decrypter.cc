...
bool LeScanningDecrypter::ExtractEncryptedData(
  ...
  *has_encrypted_data =
          ContainsEncryptedData(adv_data.data(), adv_data.size());
  if(!*has_encrypted_data) {
    LOG_INFO("enc_adv_data_map is empty");
    return false;
  }
  LOG_DEBUG("Found Encrypted Data: %s",
...
bool LeScanningDecrypter::ContainsEncryptedData(const uint8_t* ad,
                                                size_t ad_len) {
  size_t position = 0;
...
std::optional<std::vector<uint8_t>> LeScanningDecrypter::DecryptEncryptedData(
  ...
  if (out.size() > 0 && (out[0] > 0)) {
    ...
    LOG_INFO("Decryption NOT successful :%s",
              base::HexEncode(out.data(), out.size()).c_str());
    return std::nullopt;
  }
}
