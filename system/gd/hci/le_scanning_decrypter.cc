...
bool LeScanningDecrypter::ExtractEncryptedData(
  ...
  *has_encrypted_data = false;
  ...
  if (*has_encrypted_data) {
    ...
    for (size_t k = 0; k < enc_key_material_vec.size(); k++) {
      std::vector<uint8_t> key(enc_key_material_vec[k].begin(), enc_key_material_vec[k].begin() + 16);
      std::vector<uint8_t> iv(enc_key_material_vec[k].begin() + 16, enc_key_material_vec[k].end());
...
std::vector<uint8_t> LeScanningDecrypter::DecryptEncryptedData(
    std::vector<uint8_t> const &adv_data,
