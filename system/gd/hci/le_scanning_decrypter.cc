...
bool LeScanningDecrypter::ExtractEncryptedData(
  ...
  *has_encrypted_data = ContainsEncryptedData(adv_data.data(), adv_data.size());
  if (!*has_encrypted_data) {
    LOG_INFO("enc_adv_data_map is empty");
    return false;
  }
  LOG_DEBUG("Found Encrypted Data: %s", base::HexEncode(adv_data.data(), adv_data.size()).c_str());
...
std::optional<std::vector<uint8_t>> LeScanningDecrypter::DecryptEncryptedData(
...
#ifdef ENABLE_ENC_ADV_DEBUG_LOG
  if (!randomizer.empty()) {
    LOG_DEBUG("Randomizer: %s", base::HexEncode(randomizer.data(), randomizer.size()).c_str());
  }
  if (!nonce.empty()) {
    LOG_DEBUG("Nonce: %s", base::HexEncode(nonce.data(), nonce.size()).c_str());
  }
  if (!payload.empty()) {
    LOG_DEBUG("Payload: %s", base::HexEncode(payload.data(), payload.size()).c_str());
  }
  if (!MIC.empty()) {
    LOG_DEBUG("MIC: %s", base::HexEncode(MIC.data(), MIC.size()).c_str());
  }
#endif
  ...
  if (out.size() > 0 && (out[0] > 0)) {
    ...
    LOG_INFO("Decryption NOT successful :%s", base::HexEncode(out.data(), out.size()).c_str());
    return std::nullopt;
  }
}
