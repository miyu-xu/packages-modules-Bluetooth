...
void LeScanningDecrypter::DecryptEncryptedData(
    std::vector<uint8_t> &adv_data,
    std::map<int, int> &enc_adv_data_map,
    std::vector<uint8_t> &enc_key_material,
    std::map<int, std::vector<uint8_t>> *decrypted_data_map) {

  std::vector<uint8_t> key(enc_key_material.begin(), enc_key_material.begin() + 16);
  std::vector<uint8_t> iv(enc_key_material.begin() + 16, enc_key_material.end());
  ...
  // if (aeadCTX == nullptr) return is_decryption_success;
  int pos_index = 0;
