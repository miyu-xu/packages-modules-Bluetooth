...
class HciLayerTest : public ::testing::Test {
 protected:
  ...
  void FailIfResetNotSent() {
    auto command_future = hal_->GetSentCommandFuture();
    auto reset_sent_status = command_future.wait_for(1s);
...
TEST_F(HciLayerTest, controller_debug_info_requested_on_hci_timeout) {
  ...
  auto command_future = hal_->GetSentCommandFuture();
  auto debug_cmd_sent_status = command_future.wait_for(1s);
  ASSERT_EQ(debug_cmd_sent_status, std::future_status::ready);
  auto sent_command = hal_->GetSentCommand();
