...
class LeScanningReassembler {
 public:
  ...
  void SetIgnoreScanResponses(bool enable);
 ...
 private:
    ...
    struct AdvertisingInfo {
      ...
      AdvertisingInfo(Address address, DirectAdvertisingAddressType address_type, uint8_t sid);
    ...
    struct AdvertisingFragment {
      ...
      AdvertisingFragment(const AdvertisingInfo& info, const std::vector<uint8_t>& data)
    ...
    constexpr size_t kMaximumCacheSize = 16;
    std::list<AdvertisingFragment> cache_;

    /// Advertising cache management methods.
    const std::vector<uint8_t>& AppendFragment(
        const AdvertisingInfo& info, const std::vector<uint8_t>& data);
    void RemoveFragment(const AdvertisingInfo& info);
    bool ContainsFragment(const AdvertisingInfo& info);
    std::list<Item>::iterator FindFragment(const AdvertisingInfo& info);
};
