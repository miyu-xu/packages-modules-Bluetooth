...
struct Controller::impl {
  ...
  void Start(hci::HciLayer* hci) {
    ...
    if (is_supported(OpCode::LE_READ_MAXIMUM_ADVERTISING_DATA_LENGTH) &&
      ...
      log::info(
              "LE_READ_MAXIMUM_ADVERTISING_DATA_LENGTH not supported, defaulting to 31 "
              "(0x1F)");
...
#define OP_CODE_MAPPING(name)                                                     \
  ...
  bool is_supported(OpCode op_code) {
    switch (op_code) {
      ...
      // Before MSFT extension is fully supported, return false for the following
      // MSFT_OPCODE_XXXX for now.
