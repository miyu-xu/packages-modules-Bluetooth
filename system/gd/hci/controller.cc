...
void Controller::impl::Dump(
  ...
  auto acl_buffer_size_data = CreateBufferSizeData(*fb_builder, acl_buffer_length_, acl_buffers_);

  auto sco_buffer_size_data = CreateBufferSizeData(*fb_builder, sco_buffer_length_, sco_buffers_);
  ...
  auto local_supported_commands_data = fb_builder->CreateVector(
      std::vector<uint8_t>(local_supported_commands_.begin(), local_supported_commands_.end()));
