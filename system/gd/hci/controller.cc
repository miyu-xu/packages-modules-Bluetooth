...
struct Controller::impl {
  ...
  void Start(hci::HciLayer* hci) {
    ...
    if (is_supported(OpCode::READ_LOCAL_SUPPORTED_CODECS_V2)) {
      hci_->EnqueueCommand(
          ReadLocalSupportedCodecsV2Builder::Create(),
          handler->BindOnceOn(this, &Controller::impl::read_local_supported_codecs_v2_handler));
    }
  ...
  void read_local_supported_codecs_v2_handler(CommandCompleteView view) {
    auto complete_view = ReadLocalSupportedCodecsV2CompleteView::Create(view);
    ASSERT(complete_view.IsValid());
    ErrorCode status = complete_view.GetStatus();
    ASSERT_LOG(
        status == ErrorCode::SUCCESS, "Status 0x%02hhx, %s", status, ErrorCodeText(status).c_str());
    local_supported_codecs_ = complete_view.GetSupportedCodecs();
    local_supported_vendor_codecs_ = complete_view.GetVendorSpecificCodecs();
  }
...
#undef OP_CODE_MAPPING
  ...
  std::vector<CodecConfiguration> local_supported_codecs_{};
  std::vector<VendorCodecConfiguration> local_supported_vendor_codecs_{};
...
std::vector<CodecConfiguration> Controller::GetLocalSupportedCodecs() const {
  return impl_->local_supported_codecs_;
}
