...
struct MsftExtensionManager::impl {
  ...
  void msft_adv_monitor_remove(uint8_t monitor_handle, MsftAdvMonitorRemoveCallback cb) {
    msft_adb_monitor_remove_cb_ = cb;
    hci_layer_->EnqueueCommand(
        MsftLeCancelMonitorAdvBuilder::Create(static_cast<OpCode>(msft_.opcode.value()), monitor_handle),
  ...
  void msft_adv_monitor_enable(bool enable, MsftAdvMonitorEnableCallback cb) {
    msft_adb_monitor_enable_cb_ = cb;
    hci_layer_->EnqueueCommand(
  ...
  void on_msft_adv_monitor_add_complete(CommandCompleteView view) {
    ASSERT(view.IsValid());
    auto status_view = MsftLeMonitorAdvCommandCompleteView::Create(MsftCommandCompleteView::Create(view));
    ASSERT(status_view.IsValid());
    ...
    msft_adb_monitor_add_cb_.Run(status_view.GetMonitorHandle(), (uint8_t)status_view.GetStatus());
  }

  void on_msft_adv_monitor_remove_complete(CommandCompleteView view) {
    ASSERT(view.IsValid());
    auto status_view = MsftLeCancelMonitorAdvCommandCompleteView::Create(MsftCommandCompleteView::Create(view));
    ASSERT(status_view.IsValid());
    ...
    msft_adb_monitor_remove_cb_.Run((uint8_t)status_view.GetStatus());
  }

  void on_msft_adv_monitor_enable_complete(CommandCompleteView view) {
    ASSERT(view.IsValid());
    auto status_view = MsftLeSetAdvFilterEnableCommandCompleteView::Create(MsftCommandCompleteView::Create(view));
    ASSERT(status_view.IsValid());
    ...
    msft_adb_monitor_enable_cb_.Run((uint8_t)status_view.GetStatus());
  }
  ...
  Msft msft_;
  MsftAdvMonitorAddCallback msft_adb_monitor_add_cb_;
  MsftAdvMonitorRemoveCallback msft_adb_monitor_remove_cb_;
  MsftAdvMonitorEnableCallback msft_adb_monitor_enable_cb_;
};
...
void MsftExtensionManager::MsftAdvMonitorAdd(const MsftAdvMonitor& monitor, MsftAdvMonitorAddCallback cb) {
  CallOn(pimpl_.get(), &impl::msft_adv_monitor_add, monitor, cb);
}

void MsftExtensionManager::MsftAdvMonitorRemove(uint8_t monitor_handle, MsftAdvMonitorRemoveCallback cb) {
