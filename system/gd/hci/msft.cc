...
struct MsftExtensionManager::impl {
  ...
  void msft_start() {
    ...
    // Register the msft event handler through vendor_specific_event_manager_.
    for (const auto event : MsftExtensionEvents) {
      vendor_specific_event_manager_->RegisterEventHandler(
          event, module_handler_->BindOn(this, &MsftExtensionManager::impl::handle_msft_events));
    }
  }
  ...
  void on_msft_read_supported_features_complete(CommandCompleteView view) {
    ...
    if (prefix.size() > kMsftEventPrefixLengthMax) LOG_WARN("The MSFT prefix length %hu is too large", prefix.size());

    LOG_INFO("MSFT features 0x%16.16llx prefix length %hu", (unsigned long long)msft_.features, prefix.size());
  }
