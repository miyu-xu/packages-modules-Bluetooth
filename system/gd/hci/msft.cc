...
struct MsftExtensionManager::impl {
  ...
  void handle_msft_events(VendorSpecificEventView view) {
    ...
    auto pay_view = MsftEventPayloadView::Create(
    ...
    switch (ev_code) {
      case MsftEventCode::MSFT_RSSI_EVENT:
        handle_rssi_event(MsftRssiEventPayloadView::Create(pay_view));
        break;
      case MsftEventCode::MSFT_LE_MONITOR_DEVICE_EVENT:
        handle_le_monitor_device_event(MsftLeMonitorDeviceEventPayloadView::Create(pay_view));
        break;
      default:
  ...
  void on_msft_adv_monitor_add_complete(CommandCompleteView view) {
    ...
    msft_adv_monitor_add_cb_.Run(status_view.GetMonitorHandle(), (uint8_t)status_view.GetStatus());
  }
  ...
  void on_msft_adv_monitor_remove_complete(CommandCompleteView view) {
    ...
    msft_adv_monitor_remove_cb_.Run((uint8_t)status_view.GetStatus());
  }
  ...
  void on_msft_adv_monitor_enable_complete(CommandCompleteView view) {
    ...
    msft_adv_monitor_enable_cb_.Run((uint8_t)status_view.GetStatus());
  }
