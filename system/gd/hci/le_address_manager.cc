...
void LeAddressManager::AddDeviceToResolvingList(
  ...
  auto is_peer_irk_non_zero = std::any_of(peer_irk.begin(), peer_irk.end(), [](uint8_t u) { return u != 0; });

  auto is_debuggable = os::GetSystemProperty(kIsDebuggableProperty);
  if (is_debuggable.has_value() && is_debuggable.value() == "1") {
    ASSERT_LOG(is_peer_irk_non_zero, "Peer IRK with all zeroes");
  }

  if (!is_peer_irk_non_zero) {
    ...
    RemoveDeviceFromResolvingList(peer_identity_address_type, peer_identity_address);
    return;
  }
