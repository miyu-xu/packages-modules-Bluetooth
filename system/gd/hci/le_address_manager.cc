...
PrivateAddressIntervalRange LeAddressManager::GetNextPrivateAddressIntervalRange() {
  ...
  auto min_ms = minimum_rotation_time_ +
                (std::chrono::milliseconds(os::GenerateRandom()) % random_part_max_length);
  min_ms = min(min_ms, maximum_rotation_time_);

  auto max_ms = maximum_rotation_time_ -
                (std::chrono::milliseconds(os::GenerateRandom()) % random_part_max_length);
  max_ms = max(min_ms, max(max_ms, minimum_rotation_time_));

  PrivateAddressIntervalRange result = PrivateAddressIntervalRange();
  result.min = min_ms;
  result.max = max_ms;

  // For readable logging, the durations are rounded down to integer seconds.
  auto min_minutes = std::chrono::duration_cast<std::chrono::minutes>(min_ms);
  auto min_seconds = std::chrono::duration_cast<std::chrono::seconds>(min_ms - min_minutes);
  auto max_minutes = std::chrono::duration_cast<std::chrono::minutes>(max_ms);
  auto max_seconds = std::chrono::duration_cast<std::chrono::seconds>(max_ms - max_minutes);
  log::info("min={}m{}s, max={}m{}s", min_minutes.count(), min_seconds.count(), max_minutes.count(),
            max_seconds.count());
