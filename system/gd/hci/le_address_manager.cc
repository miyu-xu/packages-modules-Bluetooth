...
void LeAddressManager::OnCommandComplete(bluetooth::hci::CommandCompleteView view) {
  ...
  switch (op_code) {
    case OpCode::LE_SET_PRIVACY_MODE: {
      auto complete_view = LeSetPrivacyModeCompleteView::Create(view);
    ...
    case OpCode::LE_ADD_DEVICE_TO_RESOLVING_LIST: {
      auto complete_view = LeAddDeviceToResolvingListCompleteView::Create(view);
      if (!complete_view.IsValid()) {
        LOG_ERROR("Received %s complete with invalid packet", hci::OpCodeText(op_code).c_str());
      }
      auto status = complete_view.GetStatus();
    ...
    case OpCode::LE_REMOVE_DEVICE_FROM_RESOLVING_LIST: {
      auto complete_view = LeRemoveDeviceFromResolvingListCompleteView::Create(view);
      if (!complete_view.IsValid()) {
        LOG_ERROR("Received %s complete with invalid packet", hci::OpCodeText(op_code).c_str());
      }
      auto status = complete_view.GetStatus();
    ...
    case OpCode::LE_CLEAR_RESOLVING_LIST: {
      auto complete_view = LeClearResolvingListCompleteView::Create(view);
      if (!complete_view.IsValid()) {
        LOG_ERROR("Received %s complete with invalid packet", hci::OpCodeText(op_code).c_str());
      }
      auto status = complete_view.GetStatus();
    ...
    case OpCode::LE_ADD_DEVICE_TO_CONNECT_LIST: {
      auto complete_view = LeAddDeviceToConnectListCompleteView::Create(view);
      if (!complete_view.IsValid()) {
        LOG_ERROR("Received %s complete with invalid packet", hci::OpCodeText(op_code).c_str());
      }
      auto status = complete_view.GetStatus();
    ...
    case OpCode::LE_SET_ADDRESS_RESOLUTION_ENABLE: {
      auto complete_view = LeSetAddressResolutionEnableCompleteView::Create(view);
      if (!complete_view.IsValid()) {
        LOG_ERROR("Received %s complete with invalid packet", hci::OpCodeText(op_code).c_str());
      }
      auto status = complete_view.GetStatus();
    ...
    case OpCode::LE_CLEAR_CONNECT_LIST: {
      auto complete_view = LeClearConnectListCompleteView::Create(view);
      if (!complete_view.IsValid()) {
        LOG_ERROR("Received %s complete with invalid packet", hci::OpCodeText(op_code).c_str());
      }
      auto status = complete_view.GetStatus();
