...
void LeAddressManager::AddDeviceToFilterAcceptList(
  ...
  Command command = { CommandType::ADD_DEVICE_TO_ACCEPT_LIST,
...
void LeAddressManager::OnCommandComplete(bluetooth::hci::CommandCompleteView view) {
  ...
  switch (op_code) {
    case OpCode::LE_SET_RANDOM_ADDRESS: {
      // The command was sent before any client registered, we can make sure all the clients
      // paused when command complete.
      if (address_policy_ == AddressPolicy::USE_STATIC_ADDRESS) {
        log::info(
                "Received LE_SET_RANDOM_ADDRESS complete and Address policy is "
                "USE_STATIC_ADDRESS, "
                "return");
        return;
      }
