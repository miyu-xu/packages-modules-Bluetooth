...
class NullScanningCallback : public ScanningCallback {
  void OnScannerRegistered(
      const bluetooth::hci::Uuid app_uuid, ScannerId scanner_id, ScanningStatus status) override {
...
struct LeScanningManager::impl : public bluetooth::hci::LeAddressManagerCallback {
  ...
  void start_sync(
      uint8_t sid, const Address& address, uint16_t skip, uint16_t timeout, int reg_id) {
    if (!is_periodic_advertising_sync_transfer_sender_support_) {
      LOG_WARN("PAST sender not supported on this device");
      int status = static_cast<int>(ErrorCode::UNSUPPORTED_FEATURE_OR_PARAMETER_VALUE);
      scanning_callbacks_->OnPeriodicSyncStarted(reg_id, status, -1, sid, 1, address, 0, 0);
      return;
    }
  }
  ...
  void sync_tx_parameters(
      const Address& addr, uint8_t mode, uint16_t skip, uint16_t timeout, int reg_id) {
    if (!is_periodic_advertising_sync_transfer_sender_support_) {
      LOG_WARN("PAST sender not supported on this device");
      int status = static_cast<int>(ErrorCode::UNSUPPORTED_FEATURE_OR_PARAMETER_VALUE);
      scanning_callbacks_->OnPeriodicSyncStarted(reg_id, status, -1, -1, 1, addr, 0, 0);
      return;
    }
  }
...
void LeScanningManager::StartSync(
    uint8_t sid, const Address& address, uint16_t skip, uint16_t timeout, int reg_id) {
  CallOn(pimpl_.get(), &impl::start_sync, sid, address, skip, timeout, reg_id);
}
...
void LeScanningManager::SyncTxParameters(
    const Address& addr, uint8_t mode, uint16_t skip, uint16_t timeout, int reg_id) {
  CallOn(pimpl_.get(), &impl::sync_tx_parameters, addr, mode, skip, timeout, reg_id);
}
