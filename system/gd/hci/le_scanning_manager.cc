  ...
  void process_advertising_package_content(
    ...
    if (bluetooth::common::init_flags::encrypted_advertising_is_enabled()) {
      ...
      std::vector<uint8_t> complete_adv_data(
          complete_advertising_data->begin(),
          complete_advertising_data->end());
      bool encrypted_data_exists = false;
      bool is_decrypt_success = scanning_decrypter_.ExtractEncryptedData(
          complete_adv_data,
          enc_key_material,
          &decrypted_data,
      ...
      if(encrypted_data_exists) {
        if (decrypted_data.empty() && !is_decrypt_success) {
          if (bluetooth::common::init_flags::encrypted_advertising_log_is_enabled()) {
