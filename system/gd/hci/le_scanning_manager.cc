...
struct LeScanningManager::impl : public LeAddressManagerCallback {
  ...
  void handle_directed_advertising_report(LeDirectedAdvertisingReportView event_view) {
    ...
    // uint16_t extended_event_type = transform_to_extended_event_type(
    //    {.connectable = true, .directed = true, .legacy = true});

    // TODO(b/) add supported for directed advertising report events.
  ...
  void process_advertising_package_content(
    ...
    // Concatenate the data with exisiting fragments.
    ...
    if (is_legacy && is_scannable && !is_scan_response) {
      return;
    }

    // - For extended advertising, when the current data is marked
    //   incomplete OR when a scan response is expected.
    //   Note that scan responses are ignored when the scanning is configured
    //   with the BCM vendor command since the filter might drop
    //   the scan response.
    bool using_filter_scanning =
        filter_policy_ == LeScanningFilterPolicy::FILTER_ACCEPT_LIST_ONLY &&
        api_type_ == ScanApiType::ANDROID_HCI;

    if (!is_legacy && (data_status == DataStatus::CONTINUING ||
                       (is_scannable && !using_filter_scanning && !is_scan_response))) {
      return;
    }

    // The complete advertising data has been received,
    ...
    // Remove empty and overrunning entries from the advertising data.
