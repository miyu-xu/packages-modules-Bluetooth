      ...
      switch (filter.filter_type) {
        ...
        case ApcfFilterType::TDS_DATA: {
          update_tds_data_filter(apcf_action, filter_index, filter.org_id, filter.tds_flags,
              filter.tds_flags_mask, filter.data, filter.data_mask);
          break;
        }
  ...
  void update_tds_data_filter(
    ...
    if (action != ApcfAction::CLEAR) {
      ...
      if (transport_data.size() != 0) {
        ...
        if(is_tds_data_filter_supported_ && org_id != 0x02) {
          combined_data.insert(combined_data.end(), transport_data_mask.begin(), transport_data_mask.end());
        }
      }
    }

    LOG_WARN("org id: %d, tds_flags: %d, tds_flags_mask = %d,"
            "transport_data size: %lu, transport_data_mask size: %lu",
            org_id, tds_flags, tds_flags_mask, transport_data.size(),
