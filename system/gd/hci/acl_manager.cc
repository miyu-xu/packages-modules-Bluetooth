...
struct AclManager::impl {
  ...
  void dequeue_and_route_acl_packet_to_connection() {
    ...
    bool was_sent = false;
    classic_impl_->send_packet(handle, [&packet, &was_sent](struct acl_manager::assembler* assembler) {
      if (assembler != nullptr) {
        was_sent = true;
        assembler->on_incoming_packet(*packet);
      }
    });
    le_impl_->send_packet(handle, [&packet, &was_sent](struct acl_manager::assembler* assembler) {
      if (assembler != nullptr) {
        was_sent = true;
        assembler->on_incoming_packet(*packet);
      }
