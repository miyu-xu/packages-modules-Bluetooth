...
struct AclManager::impl {
  ...
  void Start() {
    ...
    {
      std::unique_lock<std::mutex> guard(dumpsys_guard_);
      classic_impl_ =
  ...
  mutable std::mutex dumpsys_guard_;
};
...
void AclManager::impl::Dump(
  ...
  const auto le_connectability_state = (le_impl_ != nullptr) ? (int)le_impl_->connectability_state_ : 0;
