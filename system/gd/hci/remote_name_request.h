...
// The RemoteNameRequestModule handles Remote Name Requests, which produce both Remote Name Request Completed events,
...
// interoperate with the GD ACL scheduler. Thus, we intentionally do not merge identical requests, cache responses, or
// handle request timeouts - we leave this to our callers. When GD clients start to use this module, richer
...
class RemoteNameRequestModule : public bluetooth::Module {
 public:
  ...
  // Due to controller bugs (b/184239841), an ACL connection completion is sometimes reported in place of an RNR
  // completion This method lets the ACL manager inform the RNR module if this happens, since we don't get the
  // appropriate HCI event.
