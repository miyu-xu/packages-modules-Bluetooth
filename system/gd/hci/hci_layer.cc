...
struct HciLayer::impl {
  ...
  void handle_command_response(EventView event, std::string logging_id) {
    ...
    if (is_vendor_specific && (is_status && !command_queue_.front().waiting_for_status_) &&
      ...
      // If this is a command status of a vendor specific command, and command complete is
      // expected, we can't treat this as hard failure since we have no way of probing this
      // lack of support at earlier time. Instead we let the command complete handler handle a
      // empty Command Complete packet, which will be interpreted as invalid response.
...
#ifdef TARGET_FLOSS
    // Although UNKNOWN_CONNECTION might be a controller issue in some command status, we treat
    // it as a disconnect event to maintain consistent connection state between stack and
    // controller since there might not be further HCI Disconnect Event after this status event.
  ...
  void send_next_command() {
    ...
    if (hci_timeout_alarm_ != nullptr) {
      hci_timeout_alarm_->Schedule(
              BindOnce(&impl::on_hci_timeout, common::Unretained(this), op_code), kHciTimeoutMs);
    } else {
