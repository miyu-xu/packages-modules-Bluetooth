...
struct HciLayer::impl {
  ...
  void handle_command_response(EventView event, std::string logging_id) {
    ...
    if (static_cast<int>(op_code) & (0x3f << 10) && is_status && !command_queue_.front().waiting_for_status_ &&
      ...
      // the command complete handler handle an intentionally invalid Command Complete packet.
