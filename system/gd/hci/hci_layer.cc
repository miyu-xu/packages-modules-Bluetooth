...
const std::chrono::milliseconds HciLayer::kDefaultHciTimeoutMs = std::chrono::milliseconds(2000);
const std::chrono::milliseconds HciLayer::kDefaultHciTimeoutRestartMs =
...
static void abort_after_time_out(OpCode op_code) {
  log::fatal(
      ...
      OpCodeText(op_code),
      HciLayer::getHciTimeoutRestartMs().count());
}
...
std::chrono::milliseconds HciLayer::getHciTimeoutMs() {
  static auto sHciTimeoutMs = std::chrono::milliseconds(bluetooth::os::GetSystemPropertyUint32Base(
      "bluetooth.hci.timeout_milliseconds", kDefaultHciTimeoutMs.count()));
  return sHciTimeoutMs;
}

std::chrono::milliseconds HciLayer::getHciTimeoutRestartMs() {
  static auto sRestartHciTimeoutMs =
      std::chrono::milliseconds(bluetooth::os::GetSystemPropertyUint32Base(
          "bluetooth.hci.restart_timeout_milliseconds", kDefaultHciTimeoutRestartMs.count()));
