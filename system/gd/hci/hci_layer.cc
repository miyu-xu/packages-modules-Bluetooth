...
void HciLayer::Disconnect(uint16_t handle, ErrorCode reason) {
  if (classic.disconnect_handler != nullptr) classic.disconnect_handler->Invoke(handle, reason);
  if (le.disconnect_handler != nullptr) le.disconnect_handler->Invoke(handle, reason);
}
