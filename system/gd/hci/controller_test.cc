    ...
    switch (command.GetOpCode()) {
      ...
      case (OpCode::DYNAMIC_AUDIO_BUFFER): {
        ...
        if (dab_command->GetDabCommand() == DabCommand::GET_AUDIO_BUFFER_TIME_CAPABILITY) {
          ...
          uint16_t buffer_time_ms = set_command->GetBufferTimeMs();
          event_builder =
              DabSetAudioBufferTimeCompleteBuilder::Create(1, ErrorCode::SUCCESS, buffer_time_ms);
        }
      } break;
...
TEST_F(Controller103Test, feature_spec_version_103_test) {
