...
struct RemoteNameRequestModule::impl {
 public:
  ...
  void StartRemoteNameRequest(
    ...
    // This callback needs to be shared between the *start* callback and the *cancel_completed* callback, so we refcount
    // it for safety. But since the scheduler guarantees that exactly one of these callbacks will be invokes, this is
    // safe.
    auto on_remote_name_complete_ptr = std::make_shared<RemoteNameCallback>(std::move(on_remote_name_complete));
    ...
    acl_scheduler_->EnqueueRemoteNameRequest(
        ...
        handler_->BindOnce(
            [&](Address address, std::shared_ptr<RemoteNameCallback> on_remote_name_complete_ptr) {
              LOG_INFO("Dequeued remote name request to %s since it was cancelled", address.ToString().c_str());
