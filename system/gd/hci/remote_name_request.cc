...
struct RemoteNameRequestModule::impl {
 public:
  ...
  void CancelRemoteNameRequest(Address address) {
    LOG_INFO("Enqueuing cancel of remote name request to %s", address.ToRedactedStringForLogging().c_str());
  ...
  void ReportRemoteNameRequestCancellation(Address address) {
    if (pending_) {
      ...
      LOG_ERROR("Received unexpected CONNECTION_COMPLETE when no Remote Name Request OR ACL connection is outstanding");
    }
  }
 ...
 private:
  ...
  void on_remote_name_request_complete(EventView view) {
    ...
    if (pending_) {
      LOG_INFO(
          "Received REMOTE_NAME_REQUEST_COMPLETE from %s", packet.GetBdAddr().ToRedactedStringForLogging().c_str());
      pending_ = false;
