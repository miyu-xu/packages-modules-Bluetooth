...
struct RemoteNameRequestModule::impl {
  ...
  void StartRemoteNameRequest(
    ...
    // callback, so we refcount it for safety. But since the scheduler guarantees that exactly
    // one of these callbacks will be invokes, this is safe.
  ...
  void ReportRemoteNameRequestCancellation(Address address) {
    if (pending_) {
      log::info(
              "Received CONNECTION_COMPLETE (corresponding INCORRECTLY to an RNR "
              "cancellation) "
              "from {}",
              address.ToRedactedStringForLogging());
      pending_ = false;
  ...
  void on_start_remote_name_request_status(Address address, CompletionCallback on_completion,
    ...
    // unexpectedly sent a Remote Name Req Complete HCI event without the corresponding HCI
    // command.
#ifndef TARGET_FLOSS
  ...
  void on_remote_host_supported_features_notification(EventView view) {
    ...
    if (pending_ && on_remote_host_supported_features_notification_) {
      ...
      log::error(
              "Received unexpected REMOTE_HOST_SUPPORTED_FEATURES_NOTIFICATION when no "
              "Remote "
              "Name "
              "Request is outstanding");
    } else { // callback is not set, which indicates we have processed the feature
             // notification.
      log::error(
              "Received more than one REMOTE_HOST_SUPPORTED_FEATURES_NOTIFICATION during "
              "Remote "
              "Name "
              "Request");
    }
