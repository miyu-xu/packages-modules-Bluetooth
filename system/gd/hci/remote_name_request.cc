 ...
 public:
  ...
  void Start() {
    ...
    hci_layer_->RegisterEventHandler(
        ...
        handler_->BindOn(this, &RemoteNameRequestModule::impl::on_remote_host_supported_features_notification));
    hci_layer_->RegisterEventHandler(
  ...
  void StartRemoteNameRequest(
      ...
      common::ContextualOnceCallback<void(ErrorCode)> on_completion,
      common::ContextualOnceCallback<void(RemoteHostSupportedFeaturesNotificationView)>
          on_remote_host_supported_features_notification,
      common::ContextualOnceCallback<void(RemoteNameRequestCompleteView)> on_remote_name_complete) {
    LOG_INFO("Enqueuing remote name request to %s", address.ToString().c_str());
    acl_scheduler_->EnqueueRemoteNameRequest(
        ...
        handler_->BindOnce(
            [](Address address) {
              LOG_ERROR(
                  "UNIMPLEMENTED Dequeued remote name request to %s since it was cancelled",
  ...
  void CancelRemoteNameRequest(Address address) {
    LOG_INFO("Enqueuing cancel of remote name request to %s", address.ToString().c_str());
 ...
 private:
  void actually_start_remote_name_request(
      ...
      common::ContextualOnceCallback<void(RemoteHostSupportedFeaturesNotificationView)>
          on_remote_host_supported_features_notification,
      common::ContextualOnceCallback<void(RemoteNameRequestCompleteView)> on_remote_name_complete) {
    LOG_INFO("Starting remote name request to %s", address.ToString().c_str());
    ASSERT(pending_ == false);
    pending_ = true;
    on_remote_host_supported_features_notification_ = std::move(on_remote_host_supported_features_notification);
    on_remote_name_complete_ = std::move(on_remote_name_complete);
    hci_layer_->EnqueueCommand(
        std::move(request),
        handler_->BindOnceOn(this, &impl::on_start_remote_name_request_status, address, std::move(on_completion)));
  }

  void on_start_remote_name_request_status(
      Address address, common::ContextualOnceCallback<void(ErrorCode)> on_completion, CommandStatusView status) {
    ASSERT(pending_ == true);
    ASSERT(status.GetCommandOpCode() == OpCode::REMOTE_NAME_REQUEST);
    LOG_INFO(
        "Got status %hhu when starting remote name request to to %s", status.GetStatus(), address.ToString().c_str());
  ...
  void actually_cancel_remote_name_request(Address address) {
    ASSERT(pending_ == true);
    LOG_INFO("Cancelling remote name request to %s", address.ToString().c_str());
    hci_layer_->EnqueueCommand(
        RemoteNameRequestCancelBuilder::Create(address),
        handler_->BindOnce(&acl_manager::check_command_complete<RemoteNameRequestCancelCompleteView>));
  }
  ...
  void on_remote_host_supported_features_notification(EventView view) {
    ...
    if (pending_) {
      LOG_INFO("Received REMOTE_HOST_SUPPORTED_FEATURES_NOTIFICATION from %s", packet.GetBdAddr().ToString().c_str());
      on_remote_host_supported_features_notification_.Invoke(packet);
    } else {
      LOG_ERROR(
          "Received unexpected REMOTE_HOST_SUPPORTED_FEATURES_NOTIFICATION when no Remote Name Request is outstanding");
    }
  }
  ...
  void on_remote_name_request_complete(EventView view) {
    ...
    ASSERT(packet.IsValid());
    if (pending_) {
      LOG_INFO("Received REMOTE_NAME_REQUEST_COMPLETE from %s", packet.GetBdAddr().ToString().c_str());
      pending_ = false;
      on_remote_name_complete_.Invoke(packet);
      acl_scheduler_->ReportRemoteNameRequestCompletion(packet.GetBdAddr());
    } else {
      LOG_ERROR("Received unexpected REMOTE_NAME_REQUEST_COMPLETE when no Remote Name Request is outstanding");
    }
  }
  ...
  common::ContextualOnceCallback<void(RemoteNameRequestCompleteView)> on_remote_name_complete_;
};

const ModuleFactory RemoteNameRequestModule::Factory = ModuleFactory([]() { return new RemoteNameRequestModule(); });
