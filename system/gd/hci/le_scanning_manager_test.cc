...
TEST_F(LeScanningManagerTest, is_ad_type_filter_support_false_test) {
  ASSERT_FALSE(le_scanning_manager->IsAdTypeFilterSupport());
}

TEST_F(LeScanningManagerTest, scan_filter_add_ad_type_not_support_test) {
  test_hci_layer_->SetCommandFuture(1);
...
TEST_F(LeAndroidHciScanningManagerTest, is_ad_type_filter_support_true_test) {
  ASSERT_TRUE(le_scanning_manager->IsAdTypeFilterSupport());
}
