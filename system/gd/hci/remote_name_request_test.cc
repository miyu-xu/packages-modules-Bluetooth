 ...
 protected:
  void SetUp() override {
    ...
    remote_name_request_module_ =
        static_cast<RemoteNameRequestModule*>(fake_registry_.GetModuleUnderTest(&RemoteNameRequestModule::Factory));
  ...
  template <typename... T>
  common::ContextualOnceCallback<void(T...)> capturingPromiseCallback(std::promise<std::tuple<T...>> promise) {
    return client_handler_->BindOnce(
        [](std::promise<std::tuple<T...>> promise, T... args) { promise.set_value(std::make_tuple(args...)); },
        std::move(promise));
  }
...
TEST_F(RemoteNameRequestModuleTest, CorrectCommandSent) {
  ...
  remote_name_request_module_->StartRemoteNameRequest(
      address1,
      RemoteNameRequestBuilder::Create(address1, PageScanRepetitionMode::R0, 3, ClockOffsetValid::INVALID),
...
TEST_F(RemoteNameRequestModuleTest, FailToSendCommand) {
  ...
  test_hci_layer_->IncomingEvent(RemoteNameRequestStatusBuilder::Create(ErrorCode::STATUS_UNKNOWN, 1));
...
TEST_F(RemoteNameRequestModuleTest, SendCommandThenCancelIt) {
  ...
  auto discovery_command = DiscoveryCommandView::Create(command);
  ASSERT_TRUE(discovery_command.IsValid());
  auto cancel_command = RemoteNameRequestCancelView::Create(DiscoveryCommandView::Create(discovery_command));
...
TEST_F(RemoteNameRequestModuleTest, SendCommandThenCancelItCallback) {
  ...
  remote_name_request_module_->CancelRemoteNameRequest(address1);
  test_hci_layer_->GetCommand();
  test_hci_layer_->IncomingEvent(RemoteNameRequestCancelCompleteBuilder::Create(1, ErrorCode::SUCCESS, address1));

  // verify that the completion has NOT yet been invoked (we need to wait for the RNR itself to complete)
  EXPECT_THAT(future.wait_for(timeout), std::future_status::timeout);

  // let the RNR complete with a failure
  test_hci_layer_->IncomingEvent(
      RemoteNameRequestCompleteBuilder::Create(ErrorCode::UNKNOWN_CONNECTION, address1, remote_name1));

  // only now should the name callback be invoked
  EXPECT_THAT(future, IsSetWithValue(Eq(std::make_tuple(ErrorCode::UNKNOWN_CONNECTION, remote_name1))));
}
...
TEST_F(RemoteNameRequestModuleTest, DISABLED_SendCommandThenCancelItCallbackInteropWorkaround) {
  // Some controllers INCORRECTLY give us an ACL Connection Complete event, rather than a Remote Name Request Complete
  // event, if we issue a cancellation. We should nonetheless handle this properly.
  ...
  test_hci_layer_->IncomingEvent(
      ConnectionCompleteBuilder::Create(ErrorCode::UNKNOWN_CONNECTION, 0, address1, LinkType::ACL, Enable::DISABLED));

  // we expect the name callback to be invoked nonetheless
  EXPECT_THAT(future, IsSetWithValue(Eq(std::make_tuple(ErrorCode::UNKNOWN_CONNECTION, std::array<uint8_t, 248>{}))));
}

// This test should be replaced with the above one, so we test the integration of AclManager and RnrModule
...
TEST_F(RemoteNameRequestModuleTest, HostSupportedEvents) {
  ...
  test_hci_layer_->IncomingEvent(RemoteHostSupportedFeaturesNotificationBuilder::Create(address1, 1234));
...
TEST_F(RemoteNameRequestModuleTest, QueuingRemoteNameRequestsSecondOneStarts) {
  ...
  remote_name_request_module_->StartRemoteNameRequest(
      address2,
      RemoteNameRequestBuilder::Create(address2, PageScanRepetitionMode::R1, 4, ClockOffsetValid::VALID),
      emptyCallback<ErrorCode>(),
      impossibleCallback<uint64_t>(),
  ...
  // verify that the second request has now started (so we are participating in the ACL scheduling process)
  auto command = test_hci_layer_->GetCommand();
  auto discovery_command = DiscoveryCommandView::Create(command);
  ASSERT_TRUE(discovery_command.IsValid());
  auto rnr_command = RemoteNameRequestView::Create(DiscoveryCommandView::Create(discovery_command));
...
TEST_F(RemoteNameRequestModuleTest, QueuingRemoteNameRequestsSecondOneCancelledWhileQueued) {
  ...
  EXPECT_THAT(future, IsSetWithValue(Eq(std::make_tuple(ErrorCode::PAGE_TIMEOUT, std::array<uint8_t, 248>{}))));
}
