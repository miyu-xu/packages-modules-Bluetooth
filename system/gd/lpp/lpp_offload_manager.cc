...
struct LppOffloadManager::impl {
  ...
  void start(os::Handler* handler, hal::SocketHal* socket_hal) {
    ...
    is_lpp_offload_disabled_ = os::GetSystemPropertyBool("bluetooth.lpp_offload.disable", false);
    log::info("is_lpp_offload_disabled_ {}", is_lpp_offload_disabled_);
    if (is_lpp_offload_disabled_) {
      return;
    }
