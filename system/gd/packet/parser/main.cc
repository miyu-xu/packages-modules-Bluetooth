...
bool parse_declarations_one_file(const char* argv0,
                                 const std::filesystem::path& input_file,
  ...
  std::string errs;
  if (!builder.newCharReader()->parse(&*json.begin(), &*json.end(), &root,
                                      &errs)) {
...
int main(int argc, const char** argv) {
  ...
  while (!input_files.empty()) {
    ...
    if (!parse_declarations_one_file(argv[0], input_files.front(),
                                     &declarations)) {
