...
bool parse_fields(Json::Value fields, Declarations& declarations,
                  FieldList& list) {
    ...
    if (kind == "checksum_field") {
      ...
      if (field["width"]) {
        ...
        if (auto type_def = declarations.GetTypeDef(enum_id)) {
          EnumDef* enum_def =
              (type_def->GetDefinitionType() == TypeDef::Type::ENUM
                   ? (EnumDef*)type_def
                   : nullptr);
      ...
      if (width.isNumeric()) {
        if (size.isNumeric()) {
          list.AppendField(
              new ArrayField(id, width.asInt(), size.asInt(), loc));
        } else {
          list.AppendField(
              new VectorField(id, width.asInt(), size_modifier, loc));
        }
      } else {
        ...
        for (const auto field : *group) {
          ...
          if (constraint != constraints.end()) {
            if (field->GetFieldType() == ScalarField::kFieldType) {
              ...
              list.AppendField(new FixedScalarField(
                  field->GetSize().bits(),
                  std::get<int64_t>(constraint->second), loc));
              ...
              if (enum_def == nullptr) {
                ERRORLOC(loc)
                    << "No enum found of type " << field->GetDataType();
                return false;
              }
              if (!enum_def->HasEntry(
                      std::get<std::string>(constraint->second))) {
                ERRORLOC(loc)
                    << "Enum " << field->GetDataType() << " has no enumeration "
                    << std::get<std::string>(constraint->second);
                return false;
              }

              list.AppendField(new FixedEnumField(
                  enum_def, std::get<std::string>(constraint->second), loc));
            } else {
              ERRORLOC(loc) << "Unimplemented constraint of type "
                            << field->GetFieldType();
              return false;
            }
...
bool Declarations::FromJson(Json::Value json) {
  ...
  auto endianness_loc =
      ParseLocation(json["endianness"]["loc"]["start"]["line"].asInt());
  ...
  for (auto declaration : json["declarations"]) {
    ...
    if (kind == "packet_declaration") {
      ...
      if (!parse_constraints(declaration["constraints"], constraints))
        return false;
      ...
      if (declaration["parent_id"].isString()) {
        ...
        if (parent == nullptr) {
          ERRORLOC(loc) << "Could not find packet " << parent_id
                        << " used as parent for " << id;
          return false;
        }
      }
      ...
      if (declaration["parent_id"].isString()) {
        ...
        if (parent == nullptr) {
          ERRORLOC(loc) << "Could not find struct " << parent_id
                        << " used as parent for " << id;
          return false;
        }
