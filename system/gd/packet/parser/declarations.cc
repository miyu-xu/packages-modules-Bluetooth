...
bool parse_constraints(Json::Value constraints, ConstraintMap& map) {
  for (auto constraint : constraints) {
    ...
    auto kind = value["kind"].asString();
    auto loc = ParseLocation(constraint["loc"]["start"]["line"].asInt());

    if (kind == "identifier") {
      auto name = value["name"].asString();
      map.insert(std::pair(id, std::variant<int64_t, std::string>(name)));
    } else if (kind == "interger") {
      auto integer = value["value"].asInt64();
      map.insert(std::pair(id, std::variant<int64_t, std::string>(integer)));
    } else {
      ERRORLOC(loc) << "unexpected constraint kind " << kind;
      return false;
    }
  }
  return true;
  ...
  for (auto field : fields) {
    ...
    if (kind == "checksum_field") {
      ...
      if (width.isNumeric()) {
        ...
        list.AppendField(new FixedScalarField(width.asInt(), value, loc));
      } else {
