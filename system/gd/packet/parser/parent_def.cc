...
void ParentDef::AssignSizeFields() {
  for (const auto& field : fields_) {
    ...
    for (auto it = fields_.begin(); *it != size_field; it++) {
      ...
      if (*it == var_len_field) {
        ERROR(var_len_field, size_field) << "Size/count field must come before the "
                                            "variable length field it describes.";
      }
    }
...
Size ParentDef::GetSize(bool without_payload) const {
  ...
  for (const auto& field : fields_) {
    if (without_payload &&
        (field->GetFieldType() == PayloadField::kFieldType ||
         field->GetFieldType() == BodyField::kFieldType)) {
      continue;
    }

    // The offset to the field must be passed in as an argument for dynamically sized custom
    // fields.
