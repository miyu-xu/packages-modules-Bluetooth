...
public:
  ...
  const PacketView<true> multi_view = AppendedPacketView(
          ...
          { PacketView<true>(
                    { View(std::make_shared<const vector<uint8_t>>(count_2), 0, count_2.size()) }),
            PacketView<true>({ View(std::make_shared<const vector<uint8_t>>(count_3), 0,
                                    count_3.size()) }) });
};
...
INSTANTIATE_TEST_CASE_P(chopomatic, SubviewPassTest,
                        ::testing::Values(
                                // {begin, end} pairs for subsets into the PacketView
                                SubviewTestParam{ 0, 0 }, SubviewTestParam{ 0, boundary_1 },
                                SubviewTestParam{ 0, boundary_1 + 1 },
                                SubviewTestParam{ 0, boundary_2 },
