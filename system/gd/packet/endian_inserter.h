...
class EndianInserter {
 ...
 protected:
  ...
  template <
      typename TrivialIntegerT,
      typename std::enable_if<std::is_trivial<TrivialIntegerT>::value, int>::type = 0>
  void insert(TrivialIntegerT value, BitInserter& it, size_t num_bits) const {
    ASSERT(num_bits <= (sizeof(TrivialIntegerT) * 8));
  ...
  // Write a vector of TrivialIntegerT using the iterator
  template <typename TrivialIntegerT>
  void insert_vector(const std::vector<TrivialIntegerT>& vec, BitInserter& it) const {
    static_assert(
        std::is_trivial<TrivialIntegerT>::value,
