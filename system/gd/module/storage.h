...
public:
  ...
  virtual Device GetDeviceByLegacyKey(hci::Address legacy_key_address) = 0;

  // A classic (BR/EDR) or dual mode device can be uniquely located by its classic (BR/EDR) MAC
  // address
  virtual Device GetDeviceByClassicMacAddress(hci::Address classic_address) = 0;
  ...
  virtual Device GetDeviceByLeIdentityAddress(hci::Address le_identity_address) = 0;

  // Get a list of bonded devices from config
  virtual std::vector<Device> GetBondedDevices() = 0;
