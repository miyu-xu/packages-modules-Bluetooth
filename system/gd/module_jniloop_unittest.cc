...
namespace gd_module_jniloop_unittest {
...
void TestJniModule::call_on_handler_protected_method(int handler_tid, int a, int b, int c) {
...
void TestJniModule::call_on_main_external_function(int mainloop_tid, int a, double b, char c) {
  external_function_promise = std::promise<pid_t>();
  auto future = external_function_promise.get_future();
  PostFunctionOnJni(&gd_module_jniloop_unittest::external_function, a, b, c);
  ASSERT_EQ(future.wait_for(std::chrono::seconds(3)), std::future_status::ready);
  ASSERT_EQ(future.get(), mainloop_tid);
}

// Private implementation method executed on main loop
void TestJniModule::call_on_main(int mainloop_tid, int a, int b, int c) {
...
void TestJniModule::call_on_main_repost(int mainloop_tid, int a, int b, int c) {
  private_impl_promise = std::promise<pid_t>();
  auto future = private_impl_promise.get_future();
  PostMethodOnJni(
...
void TestJniModule::call_on_main_recurse(int mainloop_tid, int depth, int b, int c) {
  private_impl_promise = std::promise<pid_t>();
  auto future = private_impl_promise.get_future();
  PostMethodOnJni(
...
TEST_F(ModuleGdxWithJniStackTest, test_call_on_main) {
  Mod()->call_on_main(jniloop_tid_, 1, 2, 3);
}

TEST_F(ModuleGdxWithJniStackTest, test_call_external_function) {
  Mod()->call_on_main_external_function(jniloop_tid_, 1, 2.3, 'c');
}

TEST_F(ModuleGdxWithJniStackTest, test_call_on_main_repost) {
  Mod()->call_on_main_repost(jniloop_tid_, 1, 2, 3);
}

TEST_F(ModuleGdxWithJniStackTest, test_call_on_main_recurse) {
  Mod()->call_on_main_recurse(jniloop_tid_, 1, 2, 3);
}
