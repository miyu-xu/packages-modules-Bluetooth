...
void fill_property(
  ...
  switch (type) {
    case BT_PROPERTY_BDNAME: {
      ...
      ASSERT_EQ(kBdNameSize, properties.back()->Size());
    } break;

    case BT_PROPERTY_BDADDR:
      properties.push_back(BtPropertyFactory::Build<RawAddress>(type, kRawAddress));
    ...
    default:
      ASSERT_LOG(false, "Illegal property:%u", type);
      break;
  }
}
...
void verify_property(const bt_property_type_t& type, const bt_property_t& property) {
  ...
  switch (property.type) {
    case BT_PROPERTY_BDNAME:
      ASSERT_EQ((int)kBdNameSize, property.len);
