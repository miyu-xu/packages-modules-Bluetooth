...
constexpr size_t kBdNameSize = 249U;
...
struct BtProperty {
  ...
  // Serialize the property structure into the provided vector
  static void Serialize(
      const std::vector<std::shared_ptr<BtProperty>>& bt_properties,
      std::vector<bt_property_t>& properties);

  // Serialize the property structure into the provided memory
  static void Serialize(
      const std::vector<std::shared_ptr<BtProperty>>& bt_properties,
...
template <typename T>
struct BtPropertySimple : BtProperty {
 ...
 private:
  friend struct BtPropertyFactory;
...
template <typename T>
struct BtPropertyVector : BtProperty {
  virtual size_t Size() const override {
...
std::shared_ptr<BtPropertyVector<T>> BtPropertyVector<T>::Factory(
  ...
  return std::make_shared<BtPropertyVector<T>>(BtPropertyVector<T>(type, name, kBdNameSize));
}
