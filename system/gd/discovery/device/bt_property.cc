...
void bluetooth::property::BtProperty::Serialize(
    const std::vector<std::shared_ptr<BtProperty>>& properties,
    std::vector<bt_property_t>& bt_properties) {
  bt_properties.resize(properties.size());
  std::vector<bt_property_t>::iterator it = bt_properties.begin();
...
void bluetooth::property::BtProperty::Serialize(
    const std::vector<std::shared_ptr<BtProperty>>& properties,
    bt_property_t* bt_properties,
    size_t size) {
  ASSERT(bt_properties != nullptr);
  ASSERT(size >= properties.size());

  for (const auto& prop : properties) {
    bt_properties->type = prop->Type();
    bt_properties->len = (int)prop->Size();
    bt_properties->val = (void*)prop->Val();
    bt_properties++;
  }
}
