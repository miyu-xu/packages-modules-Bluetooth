...
using bluetooth::discovery::device::AdvData;
...
TEST(AdvDataTest, partial_uuid16) {
...
TEST(AdvDataTest, test_data_packets__device_id) {
  ...
  ASSERT_TRUE(adv_data.GetDeviceId(device_ids));
  ASSERT_EQ(1U, device_ids.size());
  ASSERT_EQ(0x01, device_ids[0][0]);
}

TEST(AdvDataTest, test_data_packets__manufacturer_data) {
  ...
  ASSERT_TRUE(adv_data.GetManufacturerSpecificData(mfr_data));
  ASSERT_EQ(1U, mfr_data.size());
  ASSERT_EQ(0, mfr_data[0][0]);
}

TEST(AdvDataTest, test_data_packets__security_manager_oob_flags) {
  ASSERT_EQ(1U, selected_packets.count("pkt26171"));
  const auto& pkt = selected_packets["pkt26171"];
  const AdvData adv_data(std::vector<uint8_t>(&pkt[kEirOffset], &pkt[kEirOffset] + kEirSize));

  std::vector<std::vector<uint8_t>> oob_flags;
  ASSERT_TRUE(adv_data.GetManufacturerSpecificData(oob_flags));
  ASSERT_EQ(1U, oob_flags.size());
  ASSERT_EQ(0, oob_flags[0][0]);
}

TEST(AdvDataTest, test_data_packets__service_uuids16) {
  ...
  ASSERT_TRUE(adv_data.GetServiceUuids16(service_uuids16));
  ASSERT_EQ(1U, service_uuids16.size());
  ASSERT_EQ(0xfdf0, service_uuids16[0].uuid);
}

TEST(AdvDataTest, test_data_packets__service_uuids32) {
  for (const auto& pkt : data_packets) {
    const AdvData adv_data(std::vector<uint8_t>(&pkt[kEirOffset], &pkt[kEirOffset] + kEirSize));
    std::vector<discovery::device::service_uuid32_t> service_uuids32;
    ASSERT_FALSE(adv_data.GetServiceUuids32(service_uuids32));
  }
}

TEST(AdvDataTest, test_data_packets__tx_power_level) {
  ASSERT_EQ(1U, selected_packets.count("pkt34639"));
  const auto& pkt = selected_packets["pkt34639"];
  const AdvData adv_data(std::vector<uint8_t>(&pkt[kEirOffset], &pkt[kEirOffset] + kEirSize));

  std::vector<int8_t> levels;
  ASSERT_TRUE(adv_data.GetTxPowerLevel(levels));
  ASSERT_EQ(1U, levels.size());
  ASSERT_EQ(4, levels[0]);
}

TEST(AdvDataTest, test_data_packets__slave_connection_interval_range) {
  for (const auto& pkt : data_packets) {
    const AdvData adv_data(std::vector<uint8_t>(&pkt[kEirOffset], &pkt[kEirOffset] + kEirSize));
    std::vector<discovery::device::service_uuid32_t> service_uuids32;
    ASSERT_FALSE(adv_data.GetServiceUuids32(service_uuids32));
...
TEST(AdvDataTest, test_data_packets__service_solicitation16) {
  for (const auto& pkt : data_packets) {
    const AdvData adv_data(std::vector<uint8_t>(&pkt[kEirOffset], &pkt[kEirOffset] + kEirSize));
    std::vector<std::uint16_t> uuids;
    ASSERT_FALSE(adv_data.GetServiceSolicitation16(uuids));
  }
}

TEST(AdvDataTest, test_data_packets__service_solicitation32) {
  for (const auto& pkt : data_packets) {
    const AdvData adv_data(std::vector<uint8_t>(&pkt[kEirOffset], &pkt[kEirOffset] + kEirSize));
    std::vector<std::uint32_t> uuids;
    ASSERT_FALSE(adv_data.GetServiceSolicitation32(uuids));
  }
}

TEST(AdvDataTest, test_data_packets__service_solicitation128) {
  for (const auto& pkt : data_packets) {
    const AdvData adv_data(std::vector<uint8_t>(&pkt[kEirOffset], &pkt[kEirOffset] + kEirSize));
    std::vector<hci::Uuid> uuids;
    ASSERT_FALSE(adv_data.GetServiceSolicitation128(uuids));
  }
}

TEST(AdvDataTest, test_data_packets__lave_connection_interval_range) {
  for (const auto& pkt : data_packets) {
    const AdvData adv_data(std::vector<uint8_t>(&pkt[kEirOffset], &pkt[kEirOffset] + kEirSize));
    std::vector<bluetooth::discovery::device::slave_connection_interval_range_t> interval;
    ASSERT_FALSE(adv_data.GetSlaveConnectionIntervalRange(interval));
  }
}

TEST(AdvDataTest, test_data_packets__appearance) {
  for (const auto& pkt : data_packets) {
    const AdvData adv_data(std::vector<uint8_t>(&pkt[kEirOffset], &pkt[kEirOffset] + kEirSize));
    std::vector<uint16_t> appearance;
    ASSERT_FALSE(adv_data.GetAppearance(appearance));
  }
}

TEST(AdvDataTest, test_data_packets__advertising_interval) {
  for (const auto& pkt : data_packets) {
    const AdvData adv_data(std::vector<uint8_t>(&pkt[kEirOffset], &pkt[kEirOffset] + kEirSize));
    std::vector<uint16_t> advertising_interval;
    ASSERT_FALSE(adv_data.GetAdvertisingInterval(advertising_interval));
  }
}

TEST(AdvDataTest, test_data_packets__le_supported_features) {
  for (const auto& pkt : data_packets) {
    const AdvData adv_data(std::vector<uint8_t>(&pkt[kEirOffset], &pkt[kEirOffset] + kEirSize));
    std::vector<discovery::device::service_uuid32_t> service_uuids32;
    std::vector<std::vector<uint8_t>> le_supported_features;
    ASSERT_FALSE(adv_data.GetLeSupportedFeatures(le_supported_features));
  }
}

TEST(AdvDataTest, test_select_packets__pktAsha) {
  ASSERT_EQ(1U, selected_packets.count("pktAsha"));
  const auto& pkt = selected_packets["pktAsha"];
  const AdvData adv_data(std::vector<uint8_t>(&pkt[kEirOffset], &pkt[kEirOffset] + kEirSize));
  ...
  ASSERT_TRUE(adv_data.GetTxPowerLevel(tx_power_level));
...
TEST(AdvDataTest, test_select_packets__pkt34639) {
  ASSERT_EQ(1U, selected_packets.count("pkt34639"));
  const auto& pkt = selected_packets["pkt34639"];
  const AdvData adv_data(std::vector<uint8_t>(&pkt[kEirOffset], &pkt[kEirOffset] + kEirSize));
  ...
  ASSERT_FALSE(adv_data.GetUuids32(uuids32));
  ASSERT_EQ(0U, uuids32.size());

  std::vector<hci::Uuid> uuids128;
  ASSERT_TRUE(adv_data.GetUuids128(uuids128));
