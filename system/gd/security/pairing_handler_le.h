...
class PairingHandlerLe {
  ...
  WaitEncryptionChanged() {
    ...
    return PairingFailure(
            "Was expecting Encryption Change or Key Refresh Complete but received something "
            "else");
  }
  ...
  std::optional<PairingEvent> WaitUiPasskey() {
    ...
    // It's possible to receive PAIRING_CONFIRM from remote device while waiting for the
    // passkey. Store it until it's needed.
