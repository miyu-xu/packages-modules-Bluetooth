      ...
      switch (responder_io_capability) {
        case hci::IoCapability::DISPLAY_ONLY:
          ...
          if (!GetRecord()->RequiresMitmProtection()) {
            GetChannel()->SendCommand(hci::UserConfirmationRequestReplyBuilder::Create(
                    GetRecord()->GetPseudoAddress()->GetAddress()));
            // NOTE(optedoblivion) BTA needs a callback for when auto accepting
            // JustWorks If we auto accept from the ClassicPairingHandler in GD then we
            // won't get a callback to this shim function. We will have to call it
            // anyway until we eliminate the need
    ...
    case hci::IoCapability::DISPLAY_YES_NO:
      switch (responder_io_capability) {
        ...
        case hci::IoCapability::KEYBOARD_ONLY:
          ...
          break;
        case hci::IoCapability::NO_INPUT_NO_OUTPUT:
          // NumericComparison, auto confirm Responder, Yes/No confirm Initiator. Don't
    ...
    case hci::IoCapability::NO_INPUT_NO_OUTPUT:
      switch (responder_io_capability) {
        case hci::IoCapability::DISPLAY_ONLY:
          ...
          break;
        case hci::IoCapability::DISPLAY_YES_NO:
          // NumericComparison, Initiator auto confirm, Responder Yes/No confirm, no show
          // conf val
          log::info("Numeric Comparison: A auto confirm");
          if (!GetRecord()->RequiresMitmProtection()) {
            GetChannel()->SendCommand(hci::UserConfirmationRequestReplyBuilder::Create(
