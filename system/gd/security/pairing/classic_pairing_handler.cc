...
void ClassicPairingHandler::OnPairingPromptAccepted(const bluetooth::hci::AddressWithType&, bool) {
  // NOTE: This is not used by Classic, only by LE
  LOG_ALWAYS_FATAL("This is not supported by Classic Pairing Handler, only LE");
}

void ClassicPairingHandler::OnConfirmYesNo(const bluetooth::hci::AddressWithType&, bool confirmed) {
  if (confirmed) {
...
void ClassicPairingHandler::OnNameRequestComplete(hci::Address address, bool) {
