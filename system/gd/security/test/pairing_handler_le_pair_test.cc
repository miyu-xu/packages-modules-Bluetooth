...
Address ADDRESS_CENTRAL{
  { 0x26, 0x64, 0x76, 0x86, 0xab, 0xba }
};
AddressType ADDRESS_TYPE_CENTRAL = AddressType::RANDOM_DEVICE_ADDRESS;
Address IDENTITY_ADDRESS_CENTRAL{
  { 0x12, 0x34, 0x56, 0x78, 0x90, 0xaa }
};
...
Address ADDRESS_PERIPHERAL{
  { 0x33, 0x58, 0x24, 0x76, 0x11, 0x89 }
};
AddressType ADDRESS_TYPE_PERIPHERAL = AddressType::RANDOM_DEVICE_ADDRESS;
Address IDENTITY_ADDRESS_PERIPHERAL{
  { 0x21, 0x43, 0x65, 0x87, 0x09, 0x44 }
};
...
class PairingHandlerPairTest : public testing::Test {
  ...
  void SetUp() {
    ...
    l2cap_->GetQueueAUpEnd()->RegisterDequeue(
            handler_,
            common::Bind(&PairingHandlerPairTest::dequeue_callback_central,
                         common::Unretained(this)));
    l2cap_->GetQueueBUpEnd()->RegisterDequeue(
            handler_,
            common::Bind(&PairingHandlerPairTest::dequeue_callback_peripheral,
                         common::Unretained(this)));
  ...
  void RecordPairingPromptHandling(UIMock& ui_mock, std::unique_ptr<PairingHandlerLe>* handler) {
    EXPECT_CALL(ui_mock, DisplayPairingPrompt(_, _))
            ...
            .WillOnce(InvokeWithoutArgs([handler]() {
              ...
              {
                // By grabbing the lock, we ensure initialization of both pairing handlers
                // is finished.
...
TEST_F(PairingHandlerPairTest, test_secure_connections_passkey_entry) {
  ...
  // In this test either central or peripheral display the UI prompt first. This variable makes
  // sure both prompts are displayed before passkey is confirmed. Since both UI handlers are same
  // thread, it's safe.
