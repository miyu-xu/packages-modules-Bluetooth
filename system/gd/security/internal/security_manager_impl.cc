...
void SecurityManagerImpl::Init() {
   ...
   * device can override. */
  auto minimum_rotation_time =
          std::chrono::minutes(GET_SYSPROP(Ble, random_address_rotation_interval_min, 7));
  auto maximum_rotation_time =
          std::chrono::minutes(GET_SYSPROP(Ble, random_address_rotation_interval_max, 15));
...
void SecurityManagerImpl::OnSmpCommandLe(hci::AddressWithType device) {
  ...
  if (temp_cmd_view.GetCode() == Code::PAIRING_REQUEST && my_role == hci::Role::PERIPHERAL) {
    ...
    // TODO: this doesn't have to be a unique ptr, if there is a way to properly std::move it
    // into place where it's stored
...
void SecurityManagerImpl::OnConnectionOpenLe(
  ...
  channel->RegisterOnCloseCallback(
          security_handler_,
          common::BindOnce(&SecurityManagerImpl::OnConnectionClosedLe, common::Unretained(this),
                           channel->GetDevice()));
  channel->GetQueueUpEnd()->RegisterDequeue(
          security_handler_,
          common::Bind(&SecurityManagerImpl::OnSmpCommandLe, common::Unretained(this),
                       channel->GetDevice()));
