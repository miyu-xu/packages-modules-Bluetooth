...
void SecurityManagerImpl::Init() {
  ...
  /* Default to 7 minutes minimum, 15 minutes maximum for random address refreshing; device can override. */
  auto minimum_rotation_time = std::chrono::minutes(GET_SYSPROP(Ble, private_addr_rotation_interval_min, 7));
  auto maximum_rotation_time = std::chrono::minutes(GET_SYSPROP(Ble, private_addr_rotation_interval_max, 15));
