...
void SecurityManagerImpl::Init() {
  ...
  auto irk_prop = storage_module_->GetBin(BTIF_STORAGE_SECTION_ADAPTER , BTIF_STORAGE_KEY_LE_LOCAL_KEY_IRK);
  if (!irk_prop.has_value()) {
    auto rand16 = bluetooth::os::GenerateRandom<16>();
    std::vector<uint8_t> new_irk{rand16.begin(), rand16.end()};
    storage_module_->SetBin(BTIF_STORAGE_SECTION_ADAPTER , BTIF_STORAGE_KEY_LE_LOCAL_KEY_IRK, new_irk);
    irk_prop = storage_module_->GetBin(BTIF_STORAGE_SECTION_ADAPTER , BTIF_STORAGE_KEY_LE_LOCAL_KEY_IRK);
  }

  Address controllerAddress = controller_->GetMacAddress();
  auto address_prop = storage_module_->GetProperty(BTIF_STORAGE_SECTION_ADAPTER , BTIF_STORAGE_KEY_ADDRESS);
  if (!address_prop || address_prop.value() != controllerAddress.ToString()) {
    storage_module_->SetProperty(BTIF_STORAGE_SECTION_ADAPTER , BTIF_STORAGE_KEY_ADDRESS, controllerAddress.ToString());
  }
