...
class SecurityModuleFacadeService : public SecurityModuleFacade::Service,
  ...
  void OnConnectionOpenLe(std::unique_ptr<l2cap::le::FixedChannel> channel) {
    channel->RegisterOnCloseCallback(
            security_handler_,
            common::BindOnce(&SecurityModuleFacadeService::OnConnectionClosedLe,
  ...
  ::grpc::Status SetOutOfBandData(::grpc::ServerContext* /* context */,
    ...
    // We can't simply iterate till end of string, because we have an empty byte added at the
    // end. We know confirm and random are fixed size, 16 bytes
    std::array<uint8_t, 16> le_sc_c;
