...
class HciHalHost : public HciHal {
 ...
 protected:
  ...
  void Start() override {
    ...
    hci_incoming_thread_.GetReactor()->ModifyRegistration(reactable_, true, false);
 ...
 private:
  ...
  void write_to_fd(HciPacket packet) {
    ...
    if (hci_outgoing_queue_.size() == 1) {
      hci_incoming_thread_.GetReactor()->ModifyRegistration(reactable_, true, true);
    }
  }
