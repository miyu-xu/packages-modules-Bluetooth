...
constexpr uint16_t kL2capSduLengthLeMin = 23;
constexpr uint16_t kL2capSduLengthLeMax = 65535;
...
protected:
  ...
  hal::SocketCapabilities GetSocketCapabilities() override {
    ...
    if (socket_capabilities.leCocCapabilities.numberOfSupportedSockets) {
      if (socket_capabilities.leCocCapabilities.mtu < kL2capSduLengthLeMin ||
          socket_capabilities.leCocCapabilities.mtu > kL2capSduLengthLeMax) {
        log::error("Invalid leCocCapabilities.mtu");
        return kEmptySocketCapabilities;
      }
    }
