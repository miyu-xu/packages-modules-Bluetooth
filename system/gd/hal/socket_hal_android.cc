...
protected:
  ...
  bool SetAclCredits(int link, int credit) override {
    log::info("link {}, credit {}", link, credit);
    return true;
  }

  bool NotifyAclConnectionStateChange(int handle, int link, int state) override {
    log::info("handle {}, link {}, state {}", handle, link, state);
    return true;
  }

  bool NotifyAclLeDataLengthChange(int handle, int txDataLen, int rxDataLen) override {
    log::info("handle {}, txDataLen {}, rxDataLen {}", handle, txDataLen, rxDataLen);
    return true;
  }

  bool NotifyAclPowerModeChange(int handle, int mode, int interval) override {
