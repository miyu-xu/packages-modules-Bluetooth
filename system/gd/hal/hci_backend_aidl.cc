...
class AidlHci : public HciBackend {
 public:
  AidlHci(std::shared_ptr<HciBackendCallbacks> callbacks, const char* service_name) {
...
std::unique_ptr<HciBackend> HciBackend::CreateAidl(
    std::shared_ptr<HciBackendCallbacks> callbacks, const char* service_name) {
  return std::make_unique<AidlHci>(callbacks, service_name);
}
