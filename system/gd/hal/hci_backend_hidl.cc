...
class HidlHci : public HciBackend {
 ...
 public:
  HidlHci(std::shared_ptr<HciBackendCallbacks> callbacks, Handler* module_handler) {
...
std::unique_ptr<HciBackend> HciBackend::CreateHidl(
    std::shared_ptr<HciBackendCallbacks> callbacks, Handler* handler) {
  return std::make_unique<HidlHci>(callbacks, handler);
}
