...
class HciProxyCallbacks : public HciBackendCallbacks {
 ...
 public:
  ...
  HciProxyCallbacks(SnoopLogger* btsnoop_logger, LinkClocker* link_clocker)
...
class HciProxy : public HciHal {
 ...
 protected:
  ...
  void Start() override {
    ...
    callbacks_ = std::make_shared<HciProxyCallbacks>(btsnoop_logger_, link_clocker_);

    if (AServiceManager_isDeclared(kBluetoothAidlHalServiceName)) {
      backend_ = HciBackend::CreateAidl(callbacks_, kBluetoothAidlHalServiceName);
    } else {
      backend_ = HciBackend::CreateHidl(callbacks_, GetHandler());
    }
 ...
 private:
  std::shared_ptr<HciProxyCallbacks> callbacks_;
  std::unique_ptr<HciBackend> backend_;
  SnoopLogger* btsnoop_logger_ = nullptr;
  LinkClocker* link_clocker_ = nullptr;
};

const ModuleFactory HciHal::Factory = ModuleFactory([]() { return new HciProxy(); });
