...
std::vector<uint8_t> HciHalImpl::ManageNumberOfCompletedPackets(HciEventReader& e) {
  ...
  std::vector<ConnCompleted> nocp_stack;

  auto num_handles = e.read<uint8_t>();
  nocp_stack.reserve(num_handles);

  {
    ...
    for (; num_handles > 0; --num_handles) {
      ...
      for (; it != committed_.end() && n > 0; --n) {
        n_stack += (it->origin == PacketOrigin::kStack);
        it = committed_.erase(it);
      }

      if (n_stack > 0) nocp_stack.emplace_back(ConnCompleted{conn_handle, n_stack});
    }
    ScheduleTransfer();
  }
  ...
  if (nocp_stack.size() == 0) return std::vector<uint8_t>();

  auto out = HciEventWriter(HciEventCode::kNumberOfCompletedPackets);
  out.write(uint8_t(nocp_stack.size()));
  for (auto& pair : nocp_stack) {
    out.write(pair.conn_handle);
    out.write(pair.num_completed);
  }
  return out.flush();
}
