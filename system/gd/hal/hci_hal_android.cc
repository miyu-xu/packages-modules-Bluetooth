...
void HciHalImpl::OnDisconnectionComplete(HciEventReader& e) {
...

  {
    ...
    if (it_connections == connections_.end()) {
      log::error("Disconnection occured on unknown connection {:04x}", conn_handle);
      return;
    }
    connections_.erase(it_connections);

    auto it_commited =
        std::find_if(commited_.begin(), commited_.end(), [conn_handle](AclCommit& c) {
          return c.conn_handle == conn_handle;
        });
    while (it_commited != commited_.end()) it_commited = commited_.erase(it_commited);
    ScheduleTransfer();
  }
}
...
std::vector<uint8_t> HciHalImpl::ManageNumberOfCompletedPackets(HciEventReader& e) {
...

  {
    ...
    for (auto it_nocp = nocp.begin(); it_nocp != nocp.end();) {
      ...
      auto it = std::find_if(commited_.begin(), commited_.end(), [conn_handle](AclCommit& c) {
        return c.conn_handle == conn_handle;
      });
      for (auto n = it_nocp->num_completed; it != commited_.end() && n > 0; --n) {
...
void HciHalImpl::ScheduleTransfer(void) {
  size_t num_commited[PacketOrigin::kNumValues] = {0};
  for (const auto& c : commited_) ++num_commited[c.origin];
  ...
  for (auto origin : {PacketOrigin::kA2dp, PacketOrigin::kStack}) {
    ...
    while (num_commited[origin] + q.size() > max_packets_[origin]) {
    ...
    int n = std::min(
        max_commits_[origin] - num_commited[origin],
        max_controller_packets_ - int(commited_.size()));
    ...
    while (!q.empty() && n > 0) {
      ...
      // otherwise drop packet (disconnection occured ?)
      ...
      commited_.push_back({conn_handle, origin});
      --n;
    }
  }
}
