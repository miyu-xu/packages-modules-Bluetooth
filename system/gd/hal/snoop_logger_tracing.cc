...
void SnoopLoggerTracing::InitializePerfetto() {
  ...
  log::info("BTSL: Initialize Perfetto");
}
...
void SnoopLoggerTracing::TracePacket(BundleKey key, uint32_t count, int32_t length,
                                     uint64_t duration_ms) {
  SnoopLoggerTracing::Trace(
          [&key, &count, &length, &duration_ms](SnoopLoggerTracing::TraceContext ctx) {
            log::info("BTSL: Adding to trace");
            ...
            bt_event->set_count(count);
            bt_event->set_length(length);
            bt_event->set_duration(duration_ms);
            ...
            log::info("BTSL: bt_event_size={}", sizeof(bt_event));
          });
}
