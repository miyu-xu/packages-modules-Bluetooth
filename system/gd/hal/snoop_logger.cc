...
size_t SnoopLogger::GetMaxPacketsPerBuffer() {
  ...
  auto is_non_user_build = (os::GetSystemProperty(kRoBuildType) != "user");

  size_t btsnooz_max_memory_usage_bytes = (is_non_user_build ? 1024 : 256) * 1024;
...
bool SnoopLogger::IsBtSnoopLogPersisted() {
  auto is_non_user_build = (os::GetSystemProperty(kRoBuildType) != "user");
  return is_non_user_build && os::GetSystemPropertyBool(kBtSnoopLogPersists, false);
}
