...
void delete_old_btsnooz_files(const std::string& log_path, const std::chrono::milliseconds log_life_time) {
  ...
  if (opt_created_ts.has_value()) {
    ...
    timespec created_ts = opt_created_ts.value();
    auto d = seconds{created_ts.tv_sec} + nanoseconds{created_ts.tv_nsec};
    auto created_tp = time_point<system_clock>(duration_cast<system_clock::duration>(d));
...
std::chrono::milliseconds SnoopLogger::GetSnoozLogLifeTime() {
  return kBtSnoozLogLifeTime;
}

std::chrono::milliseconds SnoopLogger::GetSnoozLogDeleteAlarmInterval() {
