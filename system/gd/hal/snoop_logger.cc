  ...
  switch (type) {
    ...
    case SnoopLogger::PacketType::ACL: {
      ...
      if (packet.size() > len_hci_acl) {
        uint16_t l2cap_cid = static_cast<uint16_t>(packet[kL2capCidOffset]) |
...
void SnoopLogger::Capture(const HciPacket& immutable_packet, Direction direction, PacketType type) {
                              ...
                              .type = static_cast<uint8_t>(type) };
  {
    ...
    // std::ofstream::flush() pushes user data into kernel memory. The data will be written even
    // if this process crashes. However, data will be lost if there is a kernel panic, which is
    // out of scope of BT snoop log. NOTE: std::ofstream::write() followed by
    // std::ofstream::flush() has similar effect as UNIX write(fd, data, len)
