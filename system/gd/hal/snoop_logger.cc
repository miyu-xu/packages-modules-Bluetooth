...
const std::string SnoopLogger::kBtSnoopLogFilterTypesProperty = "persist.bluetooth.btsnooplogfilter.types";
 ...
 *  rfcommchannelfiltered - Truncates RFCOMM UIH packet to fixed (L2CAP_HEADER_SIZE) number of bytes
 *  snoopheadersfiltered  - Truncates ACL packets (non-fragment) to fixed (MAX_HCI_ACL_LEN) number of bytes
 *  a2dppktsfiltered      - Discards A2DP media packets (non-split mode)
 *  profilesfiltered      - Filters PBAP, MAP and HFP packets (CPBR, CLCC) based on filter mode:
...
void SnoopLogger::EnableProfilesFilters() {
  ...
  if (profile_pbap_mode) {
    ...
    kBtSnoopLogFilterProfiles[SnoopLogger::kBtSnoopLogFilterProfilePbap] = kBtSnoopLogFilterProfileModeDisabled;
  }
  ...
  if (profile_map_mode) {
    ...
    kBtSnoopLogFilterProfiles[SnoopLogger::kBtSnoopLogFilterProfileMap] = kBtSnoopLogFilterProfileModeDisabled;
  }
}
...
void SnoopLogger::CalculateAclPacketLength(uint32_t* length, uint8_t* packet, bool is_received) {
  uint32_t def_len = ((((uint16_t)packet[ACL_LENGTH_OFFSET + 1]) << 8) + packet[ACL_LENGTH_OFFSET]) + 5;
...
uint32_t SnoopLogger::FilterProfilesHandleHfp(uint8_t* packet, uint32_t length, uint32_t totlen, uint32_t offset) {
  ...
  if ((totlen - offset) > cpbr_pat_len) {
    if (memcmp(&packet[offset], cpbr_pattern.c_str(), cpbr_pat_len) == 0) {
      ...
      packet[L2CAP_PDU_LENGTH_OFFSET] = offset + cpbr_pat_len - (ACL_HEADER_LENGTH + BASIC_L2CAP_HEADER_LENGTH);
    }
    ...
    if (memcmp(&packet[offset], clcc_pattern.c_str(), clcc_pat_len) == 0) {
      ...
      packet[L2CAP_PDU_LENGTH_OFFSET] = offset + clcc_pat_len - (ACL_HEADER_LENGTH + BASIC_L2CAP_HEADER_LENGTH);
    }
  }
      ...
      if (filters.IsRfcommChannel(is_received, l2c_chan)) {
        ...
        if (current_profile != FILTER_PROFILE_NONE) {
          ...
          if (filters.IsHfpProfile(is_received, l2c_chan, addr)) {
            length = FilterProfilesHandleHfp(packet, length, totlen, offset);
          } else {
            length = PayloadStrip(current_profile, packet, offset, totlen - offset);
          }
        }
...
void SnoopLogger::SetRfcommPortOpen(uint16_t conn_handle, uint16_t local_cid, uint8_t dlci, uint16_t uuid, bool flow) {
  ...
  auto& filters = profiles_filter_table[conn_handle];
  {
    ...
    filters.SetupProfilesFilter(
        kBtSnoopLogFilterProfiles[kBtSnoopLogFilterProfilePbap] != kBtSnoopLogFilterProfileModeDisabled,
        kBtSnoopLogFilterProfiles[kBtSnoopLogFilterProfileMap] != kBtSnoopLogFilterProfileModeDisabled);
  }
...
bool HciPacketToHexBytes(std::vector<uint8_t> packet, char* hexstr) {
  if (hexstr != NULL) {
    for (int i = 0; i < (int)packet.size(); i++) {
      sprintf(hexstr + i * 6, "0x%02x, ", (uint8_t)packet.at(i));
    }
    return 0;
  } else {
    std::cout << "hexstr empty" << std::endl;
    return 1;
  }
}
                             ...
                             .type = static_cast<uint8_t>(type)};
  {
    ...
    if (btsnoop_mode_ == kBtSnoopLogModeFiltered) {
      if (type == PacketType::ACL) {
        ...
        if (length != 0 && IsFilterEnabled(kBtSnoopLogFilterTypeRfcommChannelFiltered)) {
    ...
    if (socket_ != nullptr) {
      ...
      socket_->Write(packet.data(), length - 1);
    }
