...
void SnoopLogger::Start() {
  ...
  if (is_enabled_) {
    ...
    //   auto snoop_logger_socket = std::make_unique<SnoopLoggerSocket>(&socket_if);
    //   snoop_logger_socket_ = std::make_unique<SnoopLoggerSocketThread>(std::move(snoop_logger_socket));
