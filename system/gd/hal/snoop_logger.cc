...
static std::string GetBtSnoopMode() {
  ...
  if (os::GetSystemPropertyBool(SnoopLogger::kIsDebuggableProperty, false)) {
    auto default_mode_property = os::GetSystemProperty(SnoopLogger::kBtSnoopDefaultLogModeProperty);
    if (default_mode_property) {
      btsnoop_mode = std::move(default_mode_property.value());
    } else {
  ...
  auto btsnoop_mode_prop = os::GetSystemProperty(SnoopLogger::kBtSnoopLogModeProperty);
  if (btsnoop_mode_prop) {
    btsnoop_mode = std::move(btsnoop_mode_prop.value());
  }
  // Only allow a subset of values:
  if (btsnoop_mode == SnoopLogger::kBtSnoopLogModeKernel ||
      btsnoop_mode == SnoopLogger::kBtSnoopLogModeDisabled ||
      btsnoop_mode == SnoopLogger::kBtSnoopLogModeFiltered ||
