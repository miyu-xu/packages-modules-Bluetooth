...
void SnoopLogger::Start() {
  ...
  if (is_enabled_) {
    OpenNextSnoopLogFile();
    snoop_logger_socket_ = std::make_unique<SnoopLoggerSocket>();
    snoop_logger_socket_->Start();
    RegisterSocket(snoop_logger_socket_.get());
  }
...
void SnoopLogger::Stop() {
  ...
  if (snoop_logger_socket_) {
    snoop_logger_socket_->Stop();
  }
...
size_t SnoopLogger::GetMaxPacketsPerBuffer() {
  ...
  auto is_debuggable_prop = os::GetSystemProperty(kIsDebuggableProperty);
  bool is_debuggable = is_debuggable_prop && common::BoolFromString(is_debuggable_prop.value());
