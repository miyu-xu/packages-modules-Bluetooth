...
static bool create_log_directories() {
  std::filesystem::path deafult_path = os::ParameterProvider::SnoopLogFilePath();
  std::filesystem::path default_dir_path = deafult_path.parent_path();
  if (default_dir_path.parent_path().filename().compare("logs") != 0) {
    default_dir_path /= "logs";
  }
