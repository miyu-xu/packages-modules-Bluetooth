...
uint16_t Mgmt::get_vs_opcode(uint16_t vendor_specification) {
  ...
  do {
    ...
    if (ret > 0) {
      ...
      if (ret < 0) {
        log::error("Failed to call MGMT opcode 0x{:4.4x}, errno {}", ev.opcode, -errno);
        close(fd);
        return ret_opcode;
      };
      break;
  ...
  if (ret <= 0) {
    log::info(
        "Skip because mgmt socket is not writable: ev.opcode 0x{:4.4x} ret {}", ev.opcode, ret);
    close(fd);
    return ret_opcode;
  }
  ...
  do {
    ...
    if (ret > 0) {
      ...
      log::error("Timeout while waiting for response of calling MGMT opcode: 0x{:4.4x}", ev.opcode);
      ret = -1;
    }
  } while (ret > 0);
  close(fd);
