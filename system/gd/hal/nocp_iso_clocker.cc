...
void NocpIsoClocker::OnHciEvent(const HciPacket& packet) {
  ...
  switch (packet[0]) {
    ...
    case HCI_EVT_COMMAND_COMPLETE: {
      ...
      if (cmd_opcode != HCI_CMD_SET_CIG_PARAMETERS)
        return;
    ...
    case HCI_EVT_NUMBER_OF_COMPLETED_PACKETS: {
      ...
      if (payload_length < size_t(1 + 4*num_handles))
        return;

      for (i=0; i < num_handles &&
        ((item[0] | (item[1] << 8)) & 0xfff) != cis_handle_; i++, item += 4);
      if (i >= num_handles)
        return;

      auto timestamp = std::chrono::system_clock::now().time_since_epoch();
      unsigned timestamp_us = std::chrono::microseconds(timestamp).count();
...
void NocpIsoClocker::Register(NocpIsoHandler *handler) { g_handler = handler; }
void NocpIsoClocker::Unregister() { g_handler = &g_empty_handler; }

const ModuleFactory NocpIsoClocker::Factory = ModuleFactory([]() {
  return new NocpIsoClocker();
});
