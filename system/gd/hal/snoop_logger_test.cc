...
class TestSnoopLoggerModule : public SnoopLogger {
 public:
  ...
  SnoopLoggerSocket* GetSocket() {
    return snoop_logger_socket_->GetSocket();
  }
};
...
TEST_F(SnoopLoggerModuleTest, custom_socket_register_enabled_capture_recv_test) {
  ...
  auto sls = std::make_unique<SnoopLoggerSocket>(&socket_impl, SnoopLoggerSocket::DEFAULT_LOCALHOST_, new_port);
  SnoopLoggerSocketThread slsThread(std::move(sls));
  slsThread.Start();

  ASSERT_TRUE(slsThread.ThreadIsRunning());
