...
TEST_F(SnoopLoggerModuleTest, default_socket_enabled_capture_recv_test) {
  ...
  // Set Receive Timeout
  constexpr struct timeval socket_timeout = {
      .tv_sec = 0,
      .tv_usec = 5000,
  };
  ret = setsockopt(socket_fd, SOL_SOCKET, SO_RCVTIMEO, &socket_timeout, sizeof(socket_timeout));
  ASSERT_TRUE(ret == 0);
  ...
  ASSERT_EQ(bytes_read, static_cast<int>(sizeof(kHfpAtNrec0)));
  ASSERT_TRUE(std::memcmp(recv_buf3, kHfpAtNrec0.data(), sizeof(kHfpAtNrec0)) == 0);
...
TEST_F(SnoopLoggerModuleTest, custom_socket_register_enabled_capture_recv_test) {
  ...
  int new_port = 8873;
  auto sls = SnoopLoggerSocket(SnoopLoggerSocket::DEFAULT_LOCALHOST_, new_port);
  sls.Start();

  ASSERT_TRUE(sls.ThreadIsRunning());

  snoop_logger->RegisterSocket(&sls);
...
TEST_F(SnoopLoggerModuleTest, custom_socket_interface_register_logging_disabled_test) {
  ...
  auto mock = SnoopLoggerSocketMock();
