...
TEST_F(SnoopLoggerModuleTest, a2dp_packets_filtered_test) {
  ...
  ASSERT_TRUE(bluetooth::os::SetSystemProperty(
      SnoopLogger::kBtSnoopLogFilterTypesProperty, SnoopLogger::kBtSnoopLogFilterTypeA2dpPktsFiltered));
  auto filterTypesProperties = bluetooth::os::GetSystemProperty(SnoopLogger::kBtSnoopLogFilterTypesProperty);
  ASSERT_TRUE(
      filterTypesProperties &&
      (filterTypesProperties->find(SnoopLogger::kBtSnoopLogFilterTypeA2dpPktsFiltered) != std::string::npos));

  auto* snoop_logger = new TestSnoopLoggerModule(
      temp_snoop_log_.string(), temp_snooz_log_.string(), 10, SnoopLogger::kBtSnoopLogModeFiltered, false);
  ...
  snoop_logger->Capture(kA2dpMediaPacket, SnoopLogger::Direction::OUTGOING, SnoopLogger::PacketType::ACL);
  ...
  ASSERT_EQ(std::filesystem::file_size(temp_snoop_log_filtered), sizeof(SnoopLogger::FileHeaderType));
...
TEST_F(SnoopLoggerModuleTest, a2dp_packets_filtered_negative_test) {
  ...
  ASSERT_EQ(
      ...
      sizeof(SnoopLogger::FileHeaderType) + sizeof(SnoopLogger::PacketHeaderType) + kA2dpMediaPacket.size());
  ASSERT_TRUE(std::filesystem::remove(temp_snoop_log_filtered));
}

TEST_F(SnoopLoggerModuleTest, headers_filtered_test) {
  ASSERT_TRUE(bluetooth::os::SetSystemProperty(
      SnoopLogger::kBtSnoopLogFilterTypesProperty, SnoopLogger::kBtSnoopLogFilterTypeHeadersFiltered));
  auto filterTypesProperties = bluetooth::os::GetSystemProperty(SnoopLogger::kBtSnoopLogFilterTypesProperty);
  ASSERT_TRUE(
      filterTypesProperties &&
      (filterTypesProperties->find(SnoopLogger::kBtSnoopLogFilterTypeHeadersFiltered) != std::string::npos));
  ...
  // Verify states after test
  ASSERT_TRUE(std::filesystem::exists(temp_snoop_log_filtered));
  LOG_INFO("const size: %d", (int)(sizeof(SnoopLogger::FileHeaderType) + sizeof(SnoopLogger::PacketHeaderType)));

  // Packet should be filtered
  ASSERT_EQ(
      std::filesystem::file_size(temp_snoop_log_filtered),
      sizeof(SnoopLogger::FileHeaderType) + sizeof(SnoopLogger::PacketHeaderType) + snoop_logger->GetMaxFilteredSize());
  ASSERT_TRUE(std::filesystem::remove(temp_snoop_log_filtered));
}
...
TEST_F(SnoopLoggerModuleTest, rfcomm_channel_filtered_sabme_ua_test) {
  ...
  ASSERT_TRUE(bluetooth::os::SetSystemProperty(
      SnoopLogger::kBtSnoopLogFilterTypesProperty, SnoopLogger::kBtSnoopLogFilterTypeRfcommChannelFiltered));
  auto filterTypesProperties = bluetooth::os::GetSystemProperty(SnoopLogger::kBtSnoopLogFilterTypesProperty);
  ASSERT_TRUE(
      filterTypesProperties &&
      (filterTypesProperties->find(SnoopLogger::kBtSnoopLogFilterTypeRfcommChannelFiltered) != std::string::npos));
  ...
  snoop_logger->Capture(kRfcommSabme, SnoopLogger::Direction::INCOMING, SnoopLogger::PacketType::ACL);
  ...
  // Packets shoud not be filtered because because they are SAMBE and UA events.
  ASSERT_EQ(
      std::filesystem::file_size(temp_snoop_log_filtered),
      sizeof(SnoopLogger::FileHeaderType) + 2 * sizeof(SnoopLogger::PacketHeaderType) + kRfcommSabme.size() +
          kRfcommUa.size());
  ASSERT_TRUE(std::filesystem::remove(temp_snoop_log_filtered));
}
...
TEST_F(SnoopLoggerModuleTest, rfcomm_channel_filtered_acceptlisted_dlci_test) {
  ...
  snoop_logger->Capture(kRfcommDlci, SnoopLogger::Direction::INCOMING, SnoopLogger::PacketType::ACL);
  snoop_logger->ClearL2capAcceptlist(conn_handle, local_cid, remote_cid);
  ASSERT_TRUE(std::filesystem::exists(temp_snoop_log_filtered));
  ...
  ASSERT_EQ(
      std::filesystem::file_size(temp_snoop_log_filtered),
      sizeof(SnoopLogger::FileHeaderType) + sizeof(SnoopLogger::PacketHeaderType) + kRfcommDlci.size());
  ASSERT_TRUE(std::filesystem::remove(temp_snoop_log_filtered));
}
...
TEST_F(SnoopLoggerModuleTest, rfcomm_channel_filtered_not_acceptlisted_dlci_test) {
  ...
  ASSERT_EQ(
      std::filesystem::file_size(temp_snoop_log_filtered),
      sizeof(SnoopLogger::FileHeaderType) + sizeof(SnoopLogger::PacketHeaderType) + snoop_logger->GetL2capHeaderSize());
  ASSERT_TRUE(std::filesystem::remove(temp_snoop_log_filtered));
}
...
TEST_F(SnoopLoggerModuleTest, rfcomm_channel_filtered_not_acceptlisted_l2cap_channel_test) {
  ...
  snoop_logger->Capture(kRfcommL2capChannel, SnoopLogger::Direction::INCOMING, SnoopLogger::PacketType::ACL);
  snoop_logger->ClearL2capAcceptlist(conn_handle, local_cid, remote_cid);
...
TEST_F(SnoopLoggerModuleTest, rfcomm_channel_filtered_acceptlisted_l2cap_channel_test) {
  ...
  ASSERT_EQ(
      std::filesystem::file_size(temp_snoop_log_filtered),
      sizeof(SnoopLogger::FileHeaderType) + sizeof(SnoopLogger::PacketHeaderType) + kRfcommL2capChannel.size());
  ASSERT_TRUE(std::filesystem::remove(temp_snoop_log_filtered));
}
...
TEST_F(SnoopLoggerModuleTest, profiles_filtered_hfp_hf_test) {
  ...
  ASSERT_TRUE(bluetooth::os::SetSystemProperty(
      SnoopLogger::kBtSnoopLogFilterTypesProperty, SnoopLogger::kBtSnoopLogFilterTypeProfilesFiltered));
  auto filterTypesProperties = bluetooth::os::GetSystemProperty(SnoopLogger::kBtSnoopLogFilterTypesProperty);
  ASSERT_TRUE(
      filterTypesProperties &&
      (filterTypesProperties->find(SnoopLogger::kBtSnoopLogFilterTypeProfilesFiltered) != std::string::npos));
  // Set profiles to filter (pbap and map)
  ASSERT_TRUE(bluetooth::os::SetSystemProperty(
      SnoopLogger::kBtSnoopLogFilterProfilesProperty,
      SnoopLogger::kBtSnoopLogFilterProfilePbap + "," + SnoopLogger::kBtSnoopLogFilterProfileMap));
  auto filterProfilesProperties = bluetooth::os::GetSystemProperty(SnoopLogger::kBtSnoopLogFilterProfilesProperty);
  ASSERT_TRUE(
      filterProfilesProperties &&
      (filterProfilesProperties->find(SnoopLogger::kBtSnoopLogFilterProfilePbap) != std::string::npos) &&
      (filterProfilesProperties->find(SnoopLogger::kBtSnoopLogFilterProfileMap) != std::string::npos));
  // Set pbap and map filtering modes
  ASSERT_TRUE(bluetooth::os::SetSystemProperty(
      SnoopLogger::kBtSnoopLogFilterProfilesPbapModeProperty, SnoopLogger::kBtSnoopLogFilterProfileModeMagic));
  ...
  ASSERT_TRUE(
      ...
      (filterPbapModeProperty->find(SnoopLogger::kBtSnoopLogFilterProfileModeMagic) != std::string::npos));
  ASSERT_TRUE(bluetooth::os::SetSystemProperty(
      SnoopLogger::kBtSnoopLogFilterProfilesMapModeProperty, SnoopLogger::kBtSnoopLogFilterProfileModeMagic));
  auto filterMapModeProperty = bluetooth::os::GetSystemProperty(SnoopLogger::kBtSnoopLogFilterProfilesMapModeProperty);
  ASSERT_TRUE(
      filterMapModeProperty &&
      (filterMapModeProperty->find(SnoopLogger::kBtSnoopLogFilterProfileModeMagic) != std::string::npos));
  ...
  std::vector<uint8_t> kPhoneNumber = {
      0x0b, 0x00, 0x30, 0x00, 0x2c, 0x00, 0x40, 0x30, 0x19, 0xff, 0x4f, 0x01, 0x0d, 0x0a, 0x2b, 0x43, 0x4c, 0x43,
      0x43, 0x3a, 0x20, 0x31, 0x2c, 0x31, 0x2c, 0x34, 0x2c, 0x30, 0x2c, 0x30, 0x2c, 0x22, 0x2b, 0x39, 0x39, 0x31,
      0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x22, 0x2c, 0x31, 0x34, 0x35, 0x0d, 0x0a, 0x49,
  };

  snoop_logger->Capture(kPhoneNumber, SnoopLogger::Direction::OUTGOING, SnoopLogger::PacketType::ACL);
  ...
  ASSERT_EQ(
      std::filesystem::file_size(temp_snoop_log_filtered),
      sizeof(SnoopLogger::FileHeaderType) + sizeof(SnoopLogger::PacketHeaderType) + HEADER_SIZE +
          strlen(clcc_pattern.c_str()));
  ASSERT_TRUE(std::filesystem::remove(temp_snoop_log_filtered));
}
...
TEST_F(SnoopLoggerModuleTest, profiles_filtered_pbap_magic_test) {
  ...
  auto* snoop_logger = new TestSnoopLoggerModule(
      temp_snoop_log_.string(), temp_snooz_log_.string(), 15, SnoopLogger::kBtSnoopLogModeFiltered, false);
  ...
  for (int i = 0; i < (int)kTestData.size(); i++) {
    snoop_logger->Capture(kTestData[i], (SnoopLogger::Direction)(i % 2), SnoopLogger::PacketType::ACL);
  }
  ...
  ASSERT_EQ(
      std::filesystem::file_size(temp_snoop_log_filtered),
      sizeof(SnoopLogger::FileHeaderType) +
          (int)kTestData.size() * (sizeof(SnoopLogger::PacketHeaderType) + HEADER_SIZE + strlen(magic_string.c_str())));
...
TEST_F(SnoopLoggerModuleTest, profiles_filtered_pbap_header_test) {
  ...
  // Set pbap and map filtering modes
  ASSERT_TRUE(bluetooth::os::SetSystemProperty(
      SnoopLogger::kBtSnoopLogFilterProfilesPbapModeProperty, SnoopLogger::kBtSnoopLogFilterProfileModeHeader));
  auto filterPbapModeProperty =
      bluetooth::os::GetSystemProperty(SnoopLogger::kBtSnoopLogFilterProfilesPbapModeProperty);
  ASSERT_TRUE(
      filterPbapModeProperty &&
      (filterPbapModeProperty->find(SnoopLogger::kBtSnoopLogFilterProfileModeHeader) != std::string::npos));
  ASSERT_TRUE(bluetooth::os::SetSystemProperty(
      SnoopLogger::kBtSnoopLogFilterProfilesMapModeProperty, SnoopLogger::kBtSnoopLogFilterProfileModeHeader));
  auto filterMapModeProperty = bluetooth::os::GetSystemProperty(SnoopLogger::kBtSnoopLogFilterProfilesMapModeProperty);
  ASSERT_TRUE(
      filterMapModeProperty &&
      (filterMapModeProperty->find(SnoopLogger::kBtSnoopLogFilterProfileModeHeader) != std::string::npos));
...
TEST_F(SnoopLoggerModuleTest, profiles_filtered_pbap_fullfilter_test) {
  ...
  // Set pbap and map filtering modes
  ASSERT_TRUE(bluetooth::os::SetSystemProperty(
      SnoopLogger::kBtSnoopLogFilterProfilesPbapModeProperty, SnoopLogger::kBtSnoopLogFilterProfileModeFullfillter));
  auto filterPbapModeProperty =
      bluetooth::os::GetSystemProperty(SnoopLogger::kBtSnoopLogFilterProfilesPbapModeProperty);
  ASSERT_TRUE(
      filterPbapModeProperty &&
      (filterPbapModeProperty->find(SnoopLogger::kBtSnoopLogFilterProfileModeFullfillter) != std::string::npos));
  ASSERT_TRUE(bluetooth::os::SetSystemProperty(
      SnoopLogger::kBtSnoopLogFilterProfilesMapModeProperty, SnoopLogger::kBtSnoopLogFilterProfileModeFullfillter));
  auto filterMapModeProperty = bluetooth::os::GetSystemProperty(SnoopLogger::kBtSnoopLogFilterProfilesMapModeProperty);
  ASSERT_TRUE(
      filterMapModeProperty &&
      (filterMapModeProperty->find(SnoopLogger::kBtSnoopLogFilterProfileModeFullfillter) != std::string::npos));
