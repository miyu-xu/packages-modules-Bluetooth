...
int SnoopLoggerSocket::InitializeCommunications() {
  ...
  syscall_if_->FDSet(notification_listen_fd, &save_sock_fds_);
...
bool SnoopLoggerSocket::ProcessIncomingRequest() {
  ...
  } else if ((notification_listen_fd != -1) && syscall_if_->FDIsSet(notification_listen_fd, &sock_fds)) {
...
int SnoopLoggerSocket::NotifySocketListener() {
  ...
  RUN_NO_INTR(ret = syscall_if_->Write(notification_write_fd, &buffer, 1));
  if (ret < 0) {
