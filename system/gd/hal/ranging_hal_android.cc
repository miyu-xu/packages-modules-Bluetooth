...
public:
  ...
  void WriteProcedureData(const uint16_t connection_handle, hci::CsRole local_cs_role,
                          const CsProcedureData& procedure_data) override {
    ...
    channel_sounding_procedure_data.procedureCounter = procedure_data.counter;
  ...
  static std::vector<SubeventResultData> get_subevent_result_data(
    ...
    for (auto subevent_result : subevent_results) {
      ...
      aidl_subevent_result.startAclConnEventCounter =
              subevent_result->start_acl_conn_event_counter_;
      aidl_subevent_result.referencePowerLevelDbm =
              static_cast<int8_t>(subevent_result->reference_power_level_);
      aidl_subevent_result.numAntennaPaths = subevent_result->num_antenna_paths_;
      aidl_subevent_result.frequencyCompensation = ConvertToSigned(
              subevent_result->frequency_compensation_, kInitiatorMeasuredOffsetBits);
      aidl_subevent_result.subeventAbortReason =
              static_cast<SubeventAbortReason>(subevent_result->subevent_abort_reason_);
      aidl_subevent_result.timestampNanos = subevent_result->timestamp_nanos_;
      aidl_subevent_result.stepData = get_group_step_data(subevent_result->step_data_, cs_role);
  ...
  static std::vector<StepData> get_group_step_data(
    ...
    for (auto step_specific_data : step_specific_data_list) {
      StepData step_data;
      step_data.stepChannel = step_specific_data.step_channel_;
      step_data.stepMode = static_cast<ModeType>(step_specific_data.mode_type_);
  ...
  static void get_step_mode_data(
    ...
    if (std::holds_alternative<Mode0Data>(mode_specific_data)) {
      ...
      mode_zero_data.packetQuality = mode_0_data.packet_quality_;
      mode_zero_data.packetRssiDbm = mode_0_data.packet_rssi_;
      mode_zero_data.packetAntenna = mode_0_data.packet_antenna_;
    ...
    if (std::holds_alternative<Mode3Data>(mode_specific_data)) {
      ...
      mode_three_data.modeOneData = convert_mode_1_data(mode_3_data.mode1_data_, cs_role);
      mode_three_data.modeTwoData = convert_mode_2_data(mode_3_data.mode2_data_);
  ...
  static ModeOneData convert_mode_1_data(const Mode1Data& mode_1_data, hci::CsRole cs_role) {
    ...
    mode_one_data.packetQuality = mode_1_data.packet_quality_;
    mode_one_data.packetNadm = static_cast<Nadm>(mode_1_data.packet_nadm_);
    mode_one_data.packetRssiDbm = mode_1_data.packet_rssi_;
    ...
    mode_one_data.packetAntenna = mode_1_data.packet_antenna_;
    ...
    if (mode_1_data.i_packet_pct1_.has_value()) {
      mode_one_data.packetPct1.emplace(
              ConvertToSigned(mode_1_data.i_packet_pct1_.value(), kIQSampleBits),
              ConvertToSigned(mode_1_data.q_packet_pct1_.value(), kIQSampleBits));
    }
    if (mode_1_data.i_packet_pct2_.has_value()) {
      mode_one_data.packetPct2.emplace(
              ConvertToSigned(mode_1_data.i_packet_pct2_.value(), kIQSampleBits),
              ConvertToSigned(mode_1_data.q_packet_pct2_.value(), kIQSampleBits));
    }
  ...
  static ModeTwoData convert_mode_2_data(const Mode2Data& mode_2_data) {
    ...
    mode_two_data.antennaPermutationIndex = mode_2_data.antenna_permutation_index_;
    for (const auto& tone_data_with_quality : mode_2_data.tone_data_with_qualities_) {
      ...
      mode_two_data.tonePctIQSamples.emplace_back(
              ConvertToSigned(tone_data_with_quality.i_sample_, kIQSampleBits),
              ConvertToSigned(tone_data_with_quality.q_sample_, kIQSampleBits));
    }
