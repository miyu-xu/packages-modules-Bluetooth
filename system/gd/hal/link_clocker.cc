...
L2capCreditIndEvents::~L2capCreditIndEvents() {
  ...
  g_credit_ind_handler.links[0].connection_handle = -1;
  g_credit_ind_handler.links[1].connection_handle = -1;
}
...
void LinkClocker::OnAclDataReceived(const HciPacket& packet) {
  ...
  int handle = packet[0] | (packet[1] << 8);
  int packet_boundary_flag = (handle >> 12) & 0x3;
  handle &= 0xfff;
  int data_total_length = std::min(size_t(packet[2] | (packet[3] << 8)), packet.size() - 4);
  ...
  // | [2..3]  Channel ID
  // | [4+]    PDU
  int pdu_length = std::min(data[0] | (data[1] << 8), data_total_length - 4);
  ...
  while (pdu_length >= 4) {
    ...
    int data_length = std::min(data[2] | (data[3] << 8), pdu_length - 4);
