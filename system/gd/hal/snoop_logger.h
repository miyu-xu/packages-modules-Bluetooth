...
class SnoopLogger : public ::bluetooth::Module {
 ...
 protected:
  ...
  uint32_t FilterProfilesHandleHfp(uint8_t* packet, uint32_t length, uint32_t totlen, uint32_t offset);
  void FilterProfilesRfcommChannel(uint8_t* packet, uint8_t *current_offset, uint32_t *length, 
    profile_type_t *current_profile, bluetooth::hal::ProfilesFilter *filters, bool is_received,
