...
class InternalHciCallbacks : public IBluetoothHciCallbacks_1_1 {
 public:
  InternalHciCallbacks(SnoopLogger* btsnoop_logger, NocpClocker* nocp_clocker)
      : btsnoop_logger_(btsnoop_logger), nocp_clocker_(nocp_clocker) {
  ...
  Return<void> hciEventReceived(const hidl_vec<uint8_t>& event) override {
    ...
    nocp_clocker_->OnHciEvent(received_hci_packet);
 ...
 private:
  ...
  NocpClocker* nocp_clocker_ = nullptr;
};
...
class AidlHciCallbacks : public ::aidl::android::hardware::bluetooth::BnBluetoothHciCallbacks {
 public:
  AidlHciCallbacks(SnoopLogger* btsnoop_logger, NocpClocker* nocp_clocker)
      : btsnoop_logger_(btsnoop_logger), nocp_clocker_(nocp_clocker) {
    init_promise_ = new std::promise<void>();
  }
...
class HciHalHidl : public HciHal {
 ...
 protected:
  ...
  void Start() override {
    ...
    nocp_clocker_ = GetDependency<NocpClocker>();
  ...
  void start_aidl() {
    ...
    if (aidl_hci_ != nullptr) {
      ...
      aidl_callbacks_ =
          ::ndk::SharedRefBase::make<AidlHciCallbacks>(btsnoop_logger_, nocp_clocker_);
  ...
  void start_hidl() {
    ...
    hidl_callbacks_ = new InternalHciCallbacks(btsnoop_logger_, nocp_clocker_);
