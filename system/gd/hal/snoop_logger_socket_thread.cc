...
void SnoopLoggerSocketThread::Start() {
  LOG_DEBUG("");
...
bool SnoopLoggerSocketThread::WaitThreadIsRunning() {
...
void SnoopLoggerSocketThread::Run() {
  LOG_DEBUG("");

  socket_->InitializeCommunications();
  ...
  while (socket_->ProcessIncomingRequest()) {
  }
