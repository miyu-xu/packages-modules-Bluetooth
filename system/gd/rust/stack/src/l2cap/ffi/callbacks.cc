...
void L2CA_DataWrite_in_main_thread(uint16_t cid, rust::Slice<const uint8_t> data, OneshotU16& completion) {
  BT_HDR* p_buf = (BT_HDR*)osi_malloc(sizeof(BT_HDR) + L2CAP_MIN_OFFSET + data.size());
  p_buf->offset = L2CAP_MIN_OFFSET;
  memcpy(p_buf + 1, data.data(), data.size());
  auto result = L2CA_DataWrite(cid, p_buf);
  oneshot_send_u16(completion, result);
}

void L2CA_DataWrite_from_rust(uint16_t cid, rust::Slice<const uint8_t> data, OneshotU16& completion) {
  do_in_main_thread(FROM_HERE, base::Bind(L2CA_DataWrite_in_main_thread, cid, data, std::ref(completion)));
}
