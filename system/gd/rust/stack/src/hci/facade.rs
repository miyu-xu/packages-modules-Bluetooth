...
use bt_packets::hci::{
    Acl, Command, EventCode, Event, Iso, LeMetaEvent, Sco,
    SubeventCode,
};
...
impl HciFacadeService {
    ...
    pub async fn register_event(&mut self, code: u32) {
        self.events.register(EventCode::try_from(u8::try_from(code).unwrap()).unwrap(), self.evt_tx.clone()).await;
    }

    /// Register for the le event & plug in the channel to get them back on
    pub async fn register_le_event(&mut self, code: u32) {
        self.events
            .register_le(SubeventCode::try_from(u8::try_from(code).unwrap()).unwrap(), self.le_evt_tx.clone())
            .await;
    }
}
