...
1. Acronyms defined by the Bluetooth specification (e.g. `L2CAP`) may be used
   anywhere, but the expanded version
   (e.g. `logicalLinkControlAndAdaptationLayerProtocol`) must be added to a
   central “glossary” which cites the section of specification.
   * These acronyms may not be shortened in any context (i.e. `RFCOMM` may not
     be shortened to `rfc`)
2. General computing acronyms (e.g. `tx` instead of `transmit`) are generally
   allowed and should be centrally documented depending on obscurity at the
   discretion of code authors and reviewers.
...
3. Acronyms and abbreviations specific to the codebase are generally
   ...
   specificity.
   * Terms used across the codebase should be documented centrally
   * Terms used within specific files or directories only may be documented in
     that file or directory
   * These shortenings are *not* allowed to be used as part of a source of
     truth. That is, names of enums, structs, traits, files, etc. must be
     completely spelled out (except for Bluetooth Specification acronyms)
4. In documentation files the first occurrence of ALL acronyms should be spelled
   out and linked to a glossary or other source of truth.
...
1. A short summary of what is (and should) be included in the file
2. Any relevant parts of the specification that the reader should be familiar with
...
3. If the file is a “main” file as part of an executable it should contains
...
function of that system out. It is heavily encouraged that a documentation
author ensures that their dependency’s documentation is of sufficient quality
that the reader will be able to understand their documentation.
