...
#[dbus_proxy_obj(BluetoothSocketCallback, "org.chromium.bluetooth.SocketCallback")]
impl IBluetoothSocketManagerCallbacks for IBluetoothSocketManagerCallbacksDBus {
    #[dbus_method("IncomingSocketReady")]
    fn incoming_socket_ready(&mut self, socket: BluetoothServerSocket, status: BtStatus) {
        dbus_generated!()
    }

    #[dbus_method("IncomingSocketClosed")]
    fn incoming_socket_closed(&mut self, listener_id: SocketId, reason: BtStatus) {
        dbus_generated!()
    }

    #[dbus_method("HandleIncomingConnection")]
    fn handle_incoming_connection(&mut self, listener_id: SocketId, connection: BluetoothSocket) {
        dbus_generated!()
    }

    #[dbus_method("OutgoingConnectionResult")]
    fn outgoing_connection_result(&mut self, result: BtStatus, socket: Option<BluetoothSocket>) {
        dbus_generated!()
    }
}
