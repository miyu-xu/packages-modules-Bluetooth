    ...
    fn cmd_gatt(&mut self, args: &Vec<String>) {
        ...
        enforce_arg_len(args, 1, "gatt <commands>", || match &args[0][0..] {
            ...
            "client-connect" => {
                ...
                println!("Initiating GATT client connect. client_id: {}, addr: {}, is_direct: {},"
                         " transport: {:?}, oppurtunistic: {}, phy: {:?}",
                         client_id, addr, is_direct, ransport, oppurtunistic, phy);
            ...
            "set-direct-connect" => {
                if args.len() < 2 {
                    println!("usage: gatt set-direct-connect <true|false>");
                    return;
                }

                let is_direct = match String::from(&args[1]).parse::<bool>() {
                    Ok(b) => b,
                    _ => {
            ...
            "set-connect-transport" => {
                if args.len() < 2 {
                    println!("usage: gatt set-connect-transport <Bredr|LE|Auto>");
                    return;
                }

                let transport = match &args[1][0..] {
            ...
            "set-connect-opportunistic" => {
                if args.len() < 2 {
                    println!("usage: gatt set-connect-opportunistic <true|false>");
                    return;
                }

                let opportunistic = match String::from(&args[1]).parse::<bool>() {
                    Ok(b) => b,
                    _ => {
                        println!("Failed to parse {}", args[1]);
                        return;
            ...
            "set-connect-phy" => {
                if args.len() < 2 {
                    println!("usage: gatt set-connect-phy <Phy1m|Phy2m|PhyCoded>");
                    return;
                }

                let phy = match &args[1][0..] {
                    "Phy1m" => LePhy::Phy1m,
                    "Phy2m" => LePhy::Phy2m,
            ...
            "write-characteristic" => {
                ...
                let addr = String::from(&args[1]);
                let handle = match String::from(&args[2]).parse::<i32>() {
                    Ok(handle) => handle,
                    _ => {
                ...
                let value = match hex::decode(&args[3]) {
                    Ok(value) => value,
                    _ => {
