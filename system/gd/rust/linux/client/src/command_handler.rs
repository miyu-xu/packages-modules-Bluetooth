    ...
    command_options.insert(
        ...
        CommandOption {
            rules: vec![
                ...
                String::from("gatt server-disconnect <server_id>"),
    ...
    fn cmd_gatt(&mut self, args: &Vec<String>) -> CommandResult {
        ...
        match &command[..] {
            ...
            "server-connect" => {
                ...
                self.context.lock().unwrap().gatt_server_context.client_addr =
            ...
            "server-disconnect" => {
                ...
                let server_id = String::from(get_arg(args, 1)?)
                    .parse::<i32>()
                    .or(Err("Failed to parse server_id"))?;
                let client_addr = match context.gatt_server_context.client_addr.as_deref() {
                    Some(addr) => addr,
                    None => return Err("GATT Server has not yet connected to a client".into()),
                };
                ...
                if !self
                    ...
                    .as_mut()
                    .unwrap()
                    .server_disconnect(server_id, client_addr.to_string())
                {
                    return Err("Disconnection was unsuccessful".into());
                }

                self.context.lock().unwrap().gatt_server_context.client_addr = None;
            }
