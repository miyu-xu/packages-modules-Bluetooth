...
use crate::callbacks::{AdvertisingSetCallback, BtGattCallback};
        ...
        enforce_arg_len(args, 1, "advertise <commands>", || match &args[0][0..] {
            "on" => {
                ...
                self.context
                    ...
                    .as_mut()
                    .unwrap()
                    .start_advertising_set(params, data, None, None, None, 0, 0, callback);
            }
            "off" => {
                let adv_sets = self.context.lock().unwrap().adv_sets.clone();
                for (_, adv_id) in adv_sets.into_iter() {
                    self.context
                        .lock()
                        .unwrap()
                        .gatt_dbus
