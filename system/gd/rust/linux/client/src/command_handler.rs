...
/// (1) send a chunk of data every |send_interval| time, repeated |num_frame| times,
/// (2) wait another |disconnect_delay| time, dump any incoming data during this time.
...
impl Default for SocketSchedule {
    fn default() -> Self {
        Self {
            num_frame: 0,
            send_interval: Duration::from_secs(0),
            disconnect_delay: Duration::from_secs(0),
        }
    }
}
    ...
    fn cmd_socket(&mut self, args: &Vec<String>) -> CommandResult {
        ...
        match &command[..] {
            "set-on-connect-schedule" => {
                ...
                self.context.lock().unwrap().socket_test_schedule = schedule;
            }
            "listen" => {
