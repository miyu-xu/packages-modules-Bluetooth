    ...
    fn cmd_gatt(&mut self, args: &Vec<String>) -> CommandResult {
        ...
        match &command[..] {
            ...
            "server-send-response" => {
                ...
                // SK - But the following does not work...
                let request = &self.lock_context().pending_gatt_request;
                if request.is_none() {
                    return Err("No pending request to send response to".into());
                }
                let r = request.as_ref().unwrap();
                // SK - ...because the program stalls here until disconnection
                ...
                */
                ...
                // SK - Regardless, the program stalls here and does not return.
            }
