    ...
    command_options.insert(
        ...
        CommandOption {
            rules: vec![
                ...
                String::from("adapter discoverable <on|off>"),
                String::from("adapter discoverable limited <duration>"),
            ...
            description: String::from(
                "Enable/Disable/Show default bluetooth adapter. (e.g. adapter enable)\n
                 ...
                 Discoverable Limited (e.g. adapter discoverable limited 60), the valid range is 1~60 (second).\n
    ...
    fn cmd_adapter(&mut self, args: &Vec<String>) -> CommandResult {
        ...
        match &command[..] {
            ...
            "discoverable" => match &get_arg(args, 1)?[..] {
                "on" => {
                    let discoverable =
                        self.lock_context().adapter_dbus.as_mut().unwrap().set_discoverable(
                            BtDiscMode::GeneralDiscoverable.into(),
                            0, /*not used*/
                        );
                    print_info!(
                ...
                "limited" => {
                    ...
                    let discoverable = self
                        ...
                        .as_mut()
                        .unwrap()
                        .set_discoverable(BtDiscMode::LimitedDiscoverable.into(), duration);
                    print_info!(
                ...
                "off" => {
                    let discoverable =
                        self.lock_context().adapter_dbus.as_mut().unwrap().set_discoverable(
                            BtDiscMode::NonDiscoverable.into(),
                            0, /*not used*/
                        );
                    print_info!(
