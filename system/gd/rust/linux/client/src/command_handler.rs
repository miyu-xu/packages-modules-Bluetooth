    ...
    fn cmd_hid(&mut self, args: &Vec<String>) -> CommandResult {
        ...
        match &command[..] {
            "get-report" => {
                ...
                self.context.lock().unwrap().qa_dbus.as_mut().unwrap().get_report(
                    addr,
                    report_type,
                    report_id,
                );
            ...
            "set-report" => {
                ...
                self.context.lock().unwrap().qa_dbus.as_mut().unwrap().set_report(
                    addr,
                    report_type,
                    report_value,
                );
            ...
            "send-data" => {
                ...
                self.context.lock().unwrap().qa_dbus.as_mut().unwrap().send_data(addr, data);
            }
            _ => return Err(CommandError::InvalidArgs),
        };
