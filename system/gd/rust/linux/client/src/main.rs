...
impl ClientContext {
    ...
    fn get_devices(& self) -> Vec<String> {
        ...
        result.extend(self.found_devices.keys().map(|key| String::from(key)).collect::<Vec<String>>());
        result.extend(self.bonded_devices.keys().filter(|key| {
            !self.found_devices.contains_key(&String::from(*key))
        }).map(|key| String::from(key)).collect::<Vec<String>>());
