...
impl UInput {
    ...
    pub fn send_key(&mut self, key: u8, state: u8) {
        ...
        if uinput_key == 0 {
            warn!("AVRCP key: {} is not supported", key);
            return;
        }

        if self.is_initialized() {
            if self.send_event(EV_KEY.try_into().unwrap(), uinput_key, state.into()) < 0
                || self.send_event(EV_SYN.try_into().unwrap(), SYN_REPORT.try_into().unwrap(), 0)
                    < 0
            {
                error!("Failed to send uinput event: {}", std::io::Error::last_os_error());
            }
        }
    }
}
