...
pub fn extract_flags(bytes: &[u8]) -> u8 {
    for slice in iterate_adv_data(bytes, FLAGS) {
        if slice.len() == 1 {
            return slice[0];
        }
    }
...
pub fn extract_service_uuids(bytes: &[u8]) -> Vec<Uuid128Bit> {
    ...
    for slice in iterate_adv_data(bytes, COMPLETE_LIST_128_BIT_SERVICE_UUIDS) {
        for i in 0..slice.len() / 16 {
            match Uuid::try_from(slice[i * 16..(i + 1) * 16].to_vec()) {
...
pub fn extract_name(bytes: &[u8]) -> String {
    for slice in iterate_adv_data(bytes, COMPLETE_LOCAL_NAME) {
        return String::from_utf8_lossy(slice).to_string();
    }

    for slice in iterate_adv_data(bytes, SHORTENED_LOCAL_NAME) {
        return String::from_utf8_lossy(slice).to_string();
    }
...
pub fn extract_service_data(bytes: &[u8]) -> HashMap<String, Vec<u8>> {
    ...
    for slice in iterate_adv_data(bytes, SERVICE_DATA_128_BIT_UUID) {
        if slice.len() < 16 {
            continue;
        }
        match Uuid::try_from(slice[0..16].to_vec()) {
            Ok(uuid) => {
                service_data.insert(uuid.to_string(), slice[16..].to_vec());
            }
            Err(..) => {}
        }
    }
...
pub fn extract_manufacturer_data(bytes: &[u8]) -> HashMap<u16, Vec<u8>> {
    ...
    for slice in iterate_adv_data(bytes, MANUFACTURER_SPECIFIC_DATA) {
        if slice.len() < 2 {
            continue;
        }
        manufacturer_data.insert(((slice[0] as u16) << 8) | slice[1] as u16, slice[2..].to_vec());
    }
