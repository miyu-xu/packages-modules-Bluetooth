...
pub fn extract_service_uuids(bytes: &[u8]) -> Vec<Uuid128Bit> {
    ...
    for slice in iterate_adv_data(bytes, COMPLETE_LIST_128_BIT_SERVICE_UUIDS) {
        for chunk in slice.chunks(16) {
            match Uuid::try_from(chunk.to_vec()) {
...
pub fn extract_service_data(bytes: &[u8]) -> HashMap<String, Vec<u8>> {
    iterate_adv_data(bytes, SERVICE_DATA_128_BIT_UUID)
        .filter_map(|slice| if slice.len() < 16 { None } else { Some(slice) })
        .map(|slice| {
            (
                Uuid::try_from(slice[0..16].to_vec()).unwrap_or_default().to_string(),
                slice[16..].to_vec(),
            )
        })
        .collect()
...
pub fn extract_manufacturer_data(bytes: &[u8]) -> HashMap<u16, Vec<u8>> {
    iterate_adv_data(bytes, MANUFACTURER_SPECIFIC_DATA)
        .filter_map(|slice| if slice.len() < 2 { None } else { Some(slice) })
        .map(|slice| {
            (u16::from_be_bytes(slice[0..2].try_into().unwrap_or_default()), slice[2..].to_vec())
        })
        .collect()
}
