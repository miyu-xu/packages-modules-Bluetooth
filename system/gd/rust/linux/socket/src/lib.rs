...
impl BtSocket {
    ...
    pub fn read_mgmt_packet(&mut self) -> Option<MgmtPacket> {
        ...
        unsafe {
            ...
            debug!("read_mgmt_packet got {} bytes: {:?}", bytes_read, buf);

            let data_size: usize = (bytes_read - (MGMT_PKT_HEADER_SIZE as isize)).try_into().unwrap();
...
#[no_mangle]
pub extern "C" fn btsocket_create_context() -> BtSocket {
    BtSocket::new()
}

#[no_mangle]
pub extern "C" fn btsocket_open_socket(context: *mut BtSocket) -> i32 {
    unsafe { (*context).open() }
}

#[no_mangle]
pub extern "C" fn btsocket_bind_mgmt_nohdev(context: *mut BtSocket) -> i32 {
    unsafe { (*context).bind_channel(HciChannels::Control, HCI_DEV_NONE) }
}

#[no_mangle]
pub extern "C" fn btsocket_bind_userchannel(context: *mut BtSocket, hci_dev: u16) -> i32 {
    unsafe { (*context).bind_channel(HciChannels::User, hci_dev) }
}

#[no_mangle]
pub extern "C" fn btsocket_take_fd(context: *mut BtSocket) -> i32 {
    unsafe { (*context).take_fd() }
}
