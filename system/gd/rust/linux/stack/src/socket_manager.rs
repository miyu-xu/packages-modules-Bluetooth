    ...
    fn socket_listen(
        ...
        if let Some(uuid) = socket_info.uuid {
            if !self.admin.lock().unwrap().is_service_allowed(uuid.into()) {
                debug!("service {} is blocked by admin policy", uuid);
    ...
    pub fn handle_admin_policy_changed(&mut self) {
        let forbidden_sockets = self
            ...
            .filter(|sock| {
                sock.uuid
                    .and_then(|uuid| {
                        Some(!self.admin.lock().unwrap().is_service_allowed(uuid.into()))
                    })
                    // Don't need to close L2cap socket (indicated by no uuid).
                    .unwrap_or(false)
            })
            .map(|sock| (sock.socket_id, sock.tx.clone()))
            .collect::<Vec<(u64, Sender<SocketRunnerActions>)>>();

        self.runtime.spawn(async move {
            for (id, tx) in forbidden_sockets {
                debug!("socket id {} is not allowed by admin policy, closing", id);
