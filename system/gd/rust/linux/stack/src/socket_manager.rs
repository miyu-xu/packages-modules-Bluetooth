...
impl Default for SocketResult {
    fn default() -> Self {
        SocketResult::new(BtStatus::Success, INVALID_SOCKET_ID)
    }
}
...
impl BluetoothServerSocket {
    ...
    fn to_err_string(&self) -> String {
        format!(
            ...
            self.name.as_ref().unwrap_or(&String::new()).clone(),
            match self.uuid {
...
impl BluetoothSocket {
    ...
    // Pretty print for an err string
    fn to_err_string(&self) -> String {
        format!(
            "[{}]:{} (type: {:?}) (uuid: {})",
            self.remote_device.address.clone(),
            self.port,
            self.sock_type.clone(),
            match self.uuid {
                Some(u) => UuidHelper::to_string(&u.uu),
...
pub enum SocketActions {
    ...
    IncomingSocketReady(CallbackId, BluetoothServerSocket, BtStatus),
    IncomingSocketClosed(CallbackId, SocketId, BtStatus),
    HandleIncomingConnection(CallbackId, SocketId, BluetoothSocket),

    // Last event is for connecting socket.
    OutgoingConnectionResult(CallbackId, BtStatus, Option<BluetoothSocket>),
}
...
impl BluetoothSocketManager {
        ...
        match result {
            Ok(file) => {
                ...
                topstack::get_runtime().spawn(async move {
                    let _ = tx
                        .send(Message::SocketManagerActions(SocketActions::IncomingSocketReady(
                            cbid,
                            forwarded_socket,
                            forwarded_status,
                        )))
            ...
            Err(_) => {
                ...
                log::error!(
                    "Failed to listen on {}. Status={:?}",
                    socket_info.to_err_string(),
                    status
                );
        ...
        loop {
            ...
            match m {
                SocketRunnerActions::AcceptTimeout(socket_id, may_timeout) => {
                    ...
                    accepting = Some(tokio::spawn(async move {
                        loop {
                            ...
                            if !readable {
                                let _ = tx
                                    .send(Message::SocketManagerActions(
                                        SocketActions::IncomingSocketReady(
                                            cbid,
                                            cloned_socket_info,
                                            BtStatus::Fail,
                                        ),
                            ...
                            // CMSG to grab the sockets also transfered over
                            // this socket.
                            let sock = {
                            ...
                            match sock {
                                Some(s) => {
                                    let _ = tx
                                        .send(Message::SocketManagerActions(
                                            SocketActions::HandleIncomingConnection(cbid, s.id, s),
                                        ))
                                        .await;
                                }
                                ...
                                None => {
                                    log::error!(
                                        ...
                                        cloned_socket_info.to_err_string()
                                    );
                ...
                SocketRunnerActions::Close(socket_id) => {
                    ...
                    let _ = rpc_tx
                        .send(Message::SocketManagerActions(SocketActions::IncomingSocketClosed(
                            cbid,
                            socket_info.id,
                            BtStatus::Success,
                        )))
    ...
    ///
    /// This task will always result in a |SocketActions::OutgoingConnectionResult| message being
    ...
    async fn connecting_task(
        ...
        let stream = match connector.stream {
            ...
            None => {
                let _ =
                    tx.send(Message::SocketManagerActions(
                        SocketActions::OutgoingConnectionResult(cbid, BtStatus::Fail, None),
                    ))
                    .await;
                return;
            }
        ...
        let ready = match time::timeout(connection_timeout, stream.readable()).await {
            Ok(inner) => match inner {
                ...
                Err(e) => {
                    ...
                    // return failure.
                    log::debug!(
                        "Connecting socket to {} failed: {:?}",
                        connector.socket_info.to_err_string(),
                        e
                    );
                    let _ = tx
                        .send(Message::SocketManagerActions(
            ...
            Err(_) => {
                ...
                log::info!(
                    "Connecting socket to {} timed out",
                    connector.socket_info.to_err_string()
                );

                let _ =
                    tx.send(Message::SocketManagerActions(
                        SocketActions::OutgoingConnectionResult(cbid, BtStatus::NotReady, None),
                    ))
                    .await;
        ...
        if let Ok(n) = stream.try_read(&mut data) {
            if let Ok(cc) = socket::ConnectionComplete::try_from(&data[0..n]) {
                ...
                if status != BtStatus::Success {
                    let _ = tx
                        .send(Message::SocketManagerActions(
                            SocketActions::OutgoingConnectionResult(cbid, status.clone(), None),
                        ))
                        .await;
                } else {
                    ...
                    let _ = tx
                        .send(Message::SocketManagerActions(
                            SocketActions::OutgoingConnectionResult(
                                cbid,
                                status.clone(),
                                Some(sock),
                            ),
    ...
    pub fn handle_actions(&mut self, action: SocketActions) {
        match action {
            SocketActions::IncomingSocketReady(cbid, server_socket, status) => {
                if let Some(callback) = self.callbacks.get_by_id(cbid) {
                    callback.incoming_socket_ready(server_socket, status);
                }
            }

            SocketActions::IncomingSocketClosed(cbid, socket_id, status) => {
                if let Some(callback) = self.callbacks.get_by_id(cbid) {
                    callback.incoming_socket_closed(socket_id, status);
                }
            }

            SocketActions::HandleIncomingConnection(cbid, socket_id, socket) => {
                if let Some(callback) = self.callbacks.get_by_id(cbid) {
                    callback.handle_incoming_connection(socket_id, socket);
                }
            }

            SocketActions::OutgoingConnectionResult(cbid, status, socket) => {
                if let Some(callback) = self.callbacks.get_by_id(cbid) {
                    callback.outgoing_connection_result(status, socket);
                }
            }
        }
    }
