...
impl BluetoothSocketManager {
    ...
    /// Helper function that waits for given stream to be readable and then reads the stream into the provided buffer.
        ...
        match time::timeout(timeout, stream.readable()).await {
            Ok(inner) => match inner {
                ...
                Err(e) => {
                    ...
                    return (BtStatus::Fail, 0);
                }
            },
            Err(_) => {
                // Timed out waiting for stream to be readable.
                return (BtStatus::NotReady, 0);
            }
        };

        match stream.try_read(buf) {
            Ok(n) => {
                return (BtStatus::Success, n);
            }
            _ => {
                return (BtStatus::Fail, 0);
            }
    ...
    async fn connecting_task(
        ...
        if status != BtStatus::Success || i32::from_be_bytes(channel_bytes) <= 0 {
            log::info!(
        ...
        let mut data = [0; socket::CONNECT_COMPLETE_SIZE + 1];
        let (status, n) = Self::wait_and_read_stream(connection_timeout, &stream, &mut data).await;
        if status != BtStatus::Success {
            log::info!(
        ...
        match socket::ConnectionComplete::try_from(&data[0..n]) {
            Ok(cc) => {
                let status = BtStatus::from(cc.status as u32);
                if status != BtStatus::Success {
                    let _ = tx
