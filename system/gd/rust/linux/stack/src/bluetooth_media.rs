...
impl BluetoothMedia {
    ...
    fn notify_media_capability_updated(&mut self, addr: RawAddress) {
        ...
        let is_profile_cleared =
            self.connected_profiles.get(&addr).cloned().unwrap_or_default().is_empty();
        ...
        let connected_profiles = self.connected_profiles.get(&addr).unwrap_or_default();
