            ...
            HfpCallbacks::MicVolumeUpdate(volume, addr) => {
                ...
                if let Some(uhid) = self.uhid.get_mut(&addr) {
                    if volume == 0 && !uhid.muted {
                        ...
                        if uhid.muted {
                            uhid.muted = false;
                            // TODO: b/333847153 remove it after RingCentral fixes the problem on its end.
                            // RingCentral mute feature is set to toggle instead of on/off control.
                            // To workaround this, we set the mute flag to 1 to trigger a toggle event.
                            self.uhid_send_phone_mute_input_report(&addr, true);
