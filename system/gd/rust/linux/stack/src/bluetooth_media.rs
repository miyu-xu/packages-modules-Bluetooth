...
// PROFILE_DISCOVERY_TIMEOUT_SEC of waiting time.
...
pub enum MediaActions {
    Connect(String, Profile),
    Disconnect(String, Profile),
}
...
pub struct BluetoothMedia {
    ...
    absolute_volume: bool,
    uinput: UInput,
    pending_profiles: HashMap<RawAddress, HashSet<Profile>>,
...
impl BluetoothMedia {
                ...
                match state {
                    BtavConnectionState::Connected => {
                        ...
                        self.pending_profiles
                            ...
                            .remove(&Profile::A2dpSink);

                        self.connected_profiles
                            .entry(addr)
                            .or_insert_with(HashSet::new)
                            .insert(Profile::A2dpSink);
        ...
        match cb {
            AvrcpCallbacks::AvrcpDeviceConnected(addr, supported) => {
                ...
                self.pending_profiles
                    .entry(addr)
                    .or_insert_with(HashSet::new)
                    .remove(&Profile::AvrcpController);

                self.connected_profiles
                    .entry(addr)
                    .or_insert_with(HashSet::new)
                    .insert(Profile::AvrcpController);
            ...
            AvrcpCallbacks::AvrcpDeviceDisconnected(addr) => {
                ...
                // TODO: better support for mutli-device
    ...
    pub fn dispatch_media_actions(&mut self, action: MediaActions) {
        match action {
            MediaActions::Connect(address, profile) => self.connect(address, profile),
            MediaActions::Disconnect(address, profile) => self.disconnect(address, profile),
        }
    }
    ...
    fn notify_media_capability_updated(&mut self, addr: RawAddress) {
        fn device_added_cb(
            ...
            pending_profiles: HashSet<Profile>,
        ) {
            // Once it gets here, it is the only caller to notify the audio stack.
            let mut guard = device_added_tasks.lock().unwrap();
            guard.insert(addr, None);

            if !pending_profiles.is_empty() {
                warn!(
                    "Notify media capability added with pending profiles: {:?}",
                    pending_profiles
                );
            }
        ...
        let connected_profiles = match self.connected_profiles.get(&addr) {
            ...
            None => {
                warn!(
                    "[{}]: Trying to update capabilities of an unknown device.",
                    addr.to_string()
                );
                return;
            }
        ...
        match guard.get(&addr) {
            Some(task) => match task {
                ...
                None => {
                    if connected_profiles.is_empty() {
                        ...
                        warn!("[{}]: Trying to update an added device.", addr.to_string());
                    }
                    return;
                }
            },
            None => {
                if connected_profiles.is_empty() {
                    warn!("[{}]: Trying to update an unknown device.", addr.to_string());
                    return;
                }
            }
        }
        ...
        let pending_profiles = self.pending_profiles.get(&addr).unwrap_or(&HashSet::new()).clone();
        ...
        let task = topstack::get_runtime().spawn(async move {
            ...
            device_added_cb(device_added_tasks, addr, callbacks, device, pending_profiles);
        });
...
impl IBluetoothMedia for BluetoothMedia {
    ...
    fn connect(&mut self, address: String, profile: Profile) {
        ...
        if let Some(connected_profiles) = self.connected_profiles.get(&addr) {
            if connected_profiles.contains(&profile) {
                return;
            }
        }

        let pending_profiles = self.pending_profiles.entry(addr).or_insert_with(HashSet::new);

        match profile {
            Profile::A2dpSink => {
                match self.a2dp.as_mut() {
                    Some(a2dp) => {
                        pending_profiles.insert(profile);
                        a2dp.connect(addr);
                    }
