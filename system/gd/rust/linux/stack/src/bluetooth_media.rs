...
impl BluetoothMedia {
    ...
    fn notify_media_capability_updated(&mut self, addr: RawAddress) {
                ...
                match task {
                    Some((handler, ts)) => {
                        ...
                        if connected_profiles.is_empty() {
                            warn!("[{}]: Device disconnected all profiles, skip notifying the audio stack and clear device state.", addr.to_string());
                            return;
                        }
                    }
                    None => {
                        if connected_profiles.is_empty() {
                            ...
                            warn!("[{}]: Device has already been added with missing profiles due to timeout.", addr.to_string());
                            // TODO(jrwu): Update remaining profiles without removing it?
                        }
                        return;
                    }
                }
        ...
        // Connect to all profiles on first connection.
        if now_ts == first_conn_ts {
            // Redundant requests will be ignored by the state machine.
...
impl IBluetoothMedia for BluetoothMedia {
    ...
    fn connect(&mut self, address: String) {
        if let Some(addr) = RawAddress::from_string(address.clone()) {
            let profiles = self.adapter_get_audio_profiles(addr);
    ...
    fn disconnect(&mut self, address: String) {
        if let Some(addr) = RawAddress::from_string(address.clone()) {
            ...
            for profile in profiles {
                match profile {
                    uuid::Profile::A2dpSink => self.a2dp.as_mut().unwrap().disconnect(addr),
                    uuid::Profile::Hfp => self.hfp.as_mut().unwrap().disconnect(addr),
                    uuid::Profile::AvrcpController => self.avrcp.as_mut().unwrap().disconnect(addr),
                    _ => warn!("Unknown profile."),
                }
            }
        } else {
