...
impl BluetoothMedia {
    ...
    fn adapter_get_remote_name(&self, addr: RawAddress) -> String {
        let device = BluetoothDevice::new(
            addr.to_string(),
            // get_remote_name needs a BluetoothDevice just for its address so
            // construct one with a fake name.
            "Classic Device".to_string(),
        );
        let adapter = self.adapter.as_ref().unwrap().lock().unwrap();
        match adapter.get_remote_name(device).as_str() {
            "" => addr.to_string(),
            name => name.to_string(),
        }
    }
