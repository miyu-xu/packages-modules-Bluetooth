...
impl BluetoothMedia {
        ...
        match cb {
            ...
            HfpCallbacks::AnswerCall(addr) => {
                if !self.phone_ops_enabled || !self.mps_qualification_enabled {
                    warn!("Unexpected answer call. phone_ops_enabled or mps_qualification_enabled does not enabled.");
                    return;
                }
            ...
            HfpCallbacks::HangupCall(addr) => {
                if !self.phone_ops_enabled || !self.mps_qualification_enabled {
                    warn!("Unexpected hangup call. phone_ops_enabled or mps_qualification_enabled does not enabled.");
                    return;
                }
            ...
            HfpCallbacks::DialCall(number, addr) => {
                ...
                self.dialing_call_impl(number, addr);
            }
            HfpCallbacks::CallHold(command, addr) => {
                ...
                let success = match command {
                    CallHoldCommand::ReleaseHeld => self.release_held_impl(addr),
                    CallHoldCommand::ReleaseActiveAcceptHeld => {
                        self.release_active_accept_held_impl(addr)
                    }
                    CallHoldCommand::HoldActiveAcceptHeld => {
                        self.hold_active_accept_held_impl(addr)
                    }
    ...
    pub fn dispatch_uhid_hfp_output_callback(&mut self, address: String, id: u8, data: u8) {
        ...
        if id == BLUETOOTH_TELEPHONY_UHID_REPORT_ID {
            ...
            if call_state == UHID_OUTPUT_NONE {
                ...
                self.dialing_call_impl("".into(), Some(addr));
                self.answer_call_impl();
                self.uhid_send_hook_switch_input_report(&addr, true);
            }
        }
    ...
    fn dialing_call_impl(&mut self, number: Option<String>, addr: Option<RawAddress>) -> bool {
        if !number || self.phone_state.num_active > 0 || self.phone_state.state != CallState::Idle {
            if addr {
                self.simple_at_response(false, addr.clone());
...
impl IBluetoothTelephony for BluetoothMedia {
    ...
    fn dialing_call(&mut self, number: String) -> bool {
        ...
        return self.dialing_call_impl(Some(number), None);
    }
