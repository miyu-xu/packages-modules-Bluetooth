...
impl BluetoothMedia {
    ...
    fn uhid_send_phone_mute_input_report(&mut self, addr: &RawAddress, muted: bool) {
        ...
        if let Some(uhid) = self.uhid.get_mut(addr) {
            ...
            if muted {
                ...
                // Some applications do not respond to phone mute=0 and treat
                // the phone mute=1 event as a toggle rather than an on off control.
                data |= UHID_INPUT_PHONE_MUTE;
                self.uhid_send_input_event_report(&addr, data);
                data &= !UHID_INPUT_PHONE_MUTE;
                self.uhid_send_input_event_report(&addr, data);
    ...
    fn initiate_active_call(&mut self) {
        if self.phone_state.num_active != 0 {
            warn!("Unexpected usage. phone_state.num_active can only be 0 when calling initiate_active_call");
            return;
        }

        if self.phone_state.state != CallState::Idle {
            warn!("Unexpected usage. phone_state.state can only be idle when calling initiate_active_call");
            return;
        }
