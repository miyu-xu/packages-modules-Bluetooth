...
impl IBluetoothMedia for BluetoothMedia {
    ...
    fn connect(&mut self, address: String) {
        ...
        let a2dp = match self.a2dp.as_mut() {
            None => {
    ...
    fn start_audio_request(&mut self) {
        match self.a2dp.as_mut() {
            ...
            None => warn!("Uninitialized A2DP to set volume"),
        }
    }
    ...
    fn start_sco_call(&mut self, address: String) {
        ...
        let hfp = match self.hfp.as_mut() {
            None => {
                warn!("Uninitialized HFP to connect audio");
                return;
            }
            Some(hfp) => hfp,
        };
    ...
    fn stop_sco_call(&mut self, address: String) {
        ...
        match self.hfp.as_mut() {
            ...
            None => warn!("Uninitialized HFP to disconnect audio"),
        }
    }
    ...
    fn get_presentation_position(&mut self) -> PresentationPosition {
        let position = match self.a2dp.as_mut() {
            ...
            None => {
                warn!("Uninitialized A2DP to set volume");
                Default::default()
            }
        };
        PresentationPosition {
