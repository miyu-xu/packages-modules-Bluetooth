                ...
                match state {
                    ...
                    BtavConnectionState::Disconnected => match self.a2dp_states.remove(&addr) {
                        Some(_) => {
                            ...
                            // Notify device removal if it has been added and this is the first
                            // event of critical profile disconnection.
                            if self.disconnecting_devices.insert(addr) {
                                let guard = self.device_added_tasks.lock().unwrap();
                                if let Some(task) = guard.get(&addr) {
                                    if task.is_none() {
                                        self.callbacks.lock().unwrap().for_all_callbacks(
                                            |callback| {
    ...
    fn notify_media_capability_updated(&mut self, addr: RawAddress) {
        ...
        match guard.get(&addr) {
            Some(task) => match task {
                ...
                // The audio stack has been notified of device connection.
                None => {
                    if connected_profiles.is_empty() {
            ...
            // First profile connection since there was ever no profile being connected.
            None => {
                if connected_profiles.is_empty() {
                    error!(
        ...
        // If the device has disconnected one of A2DP/HFP, wait until all
