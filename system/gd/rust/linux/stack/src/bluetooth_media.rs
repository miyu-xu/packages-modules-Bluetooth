...
impl IBluetoothMedia for BluetoothMedia {
    ...
    fn connect(&mut self, address: String) {
        ...
        if let (Some(a2dp), Some(hfp)) = (self.a2dp.as_mut(), self.hfp.as_mut()) {
            a2dp.connect(addr);
            hfp.connect(addr);
        } else {
            warn!("Uninitialized stack to connect {}", address);
        }
    }
    ...
    fn set_active_device(&mut self, address: String) {
        ...
        match self.a2dp.as_mut() {
            ...
            None => warn!("Uninitialized stack to set active device"),
        }
    }
    ...
    fn disconnect(&mut self, address: String) {
        ...
        if let (Some(a2dp), Some(hfp)) = (self.a2dp.as_mut(), self.hfp.as_mut()) {
            ...
            warn!("Uninitialized stack to disconnect {}", address);
        }
    }
    ...
    fn set_audio_config(
        ...
        match self.a2dp.as_mut() {
            None => {
                warn!("Uninitialized stack to set audio config");
                false
            }
            Some(a2dp) => {
    ...
    fn set_volume(&mut self, volume: u8) {
        ...
        match self.avrcp.as_mut() {
            ...
            None => warn!("Uninitialized stack to set volume"),
        }
    }
    ...
    fn start_sco_call(&mut self, address: String) {
        ...
        let hfp = match self.hfp.as_mut() {
            None => {
                warn!("Uninitialized stack to connect audio");
                return;
            }
            Some(hfp) => hfp,
        };
    ...
    fn stop_sco_call(&mut self, address: String) {
        ...
        match self.hfp.as_mut() {
            ...
            None => warn!("Uninitialized stack to disconnect audio"),
        }
    }
    ...
    fn get_presentation_position(&mut self) -> PresentationPosition {
        let position = match self.a2dp.as_mut() {
            ...
            None => {
                warn!("Uninitialized stack to set volume");
                Default::default()
            }
        };
        PresentationPosition {
