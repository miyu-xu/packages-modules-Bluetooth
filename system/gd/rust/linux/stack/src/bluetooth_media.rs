...
const BLE_UNKNOWN_GROUP_ID: i32 = -1;

/// Max number of retries for CSIS to bond set members of a connected group.
/// Refer to |pairDeviceByCsip| in |CachedBluetoothDeviceManager.java|.
const CSIS_BONDING_MAX_RETRIES: u32 = 30;
...
pub trait IBluetoothMediaCallback: RPCProxy {
    ...
    /// Triggered when the first member of the specified BLE group has connected
    ...
    fn on_ble_group_connected(&mut self, group_id: i32, name: String);

    /// Triggered when the last connected member of the specified BLE group has
    ...
    fn on_ble_group_disconnected(&mut self, group_id: i32);
...
pub struct BluetoothMedia {
    ...
    le_audio_delayed_audio_conf_updates: HashMap<i32, BLEAudioConf>,
    ...
    pub fn dispatch_le_audio_callbacks(&mut self, cb: LeAudioClientCallbacks) {
        match cb {
            ...
            LeAudioClientCallbacks::ConnectionState(state, addr) => {
                ...
                let group_id =
                    *self.le_audio_node_to_group.get(&addr).unwrap_or(&BLE_UNKNOWN_GROUP_ID);
                if group_id == BLE_UNKNOWN_GROUP_ID {
                    warn!(
                ...
                match state {
                    BtLeAudioConnectionState::Connected => {
                        if is_only_connected_member {
                            self.callbacks.lock().unwrap().for_all_callbacks(|callback| {
                                callback.on_ble_group_connected(
                                    group_id,
                    ...
                    BtLeAudioConnectionState::Disconnected => {
                        if self.le_audio_states.remove(&addr).is_some() {
                            if is_only_connected_member {
                                self.callbacks.lock().unwrap().for_all_callbacks(|callback| {
                                    callback.on_ble_group_disconnected(group_id);
                                });
                            }
                        }
                    }
...
impl IBluetoothMedia for BluetoothMedia {
    ...
    fn connect_le_group_by_member_address(&mut self, address: String) {
        let addr = match RawAddress::from_string(address.clone()) {
            None => {
                warn!("connect_le_group_by_member_address: invalid device address");
                return;
            }
            Some(addr) => addr,
        };
    ...
    fn disconnect_le_group_by_member_address(&mut self, address: String) {
        let addr = match RawAddress::from_string(address.clone()) {
            None => {
                warn!("disconnect_le_group_by_member_address: invalid device address");
                return;
            }
            Some(addr) => addr,
        };

        let group_id = *self.le_audio_node_to_group.get(&addr).unwrap_or(&BLE_UNKNOWN_GROUP_ID);
        if group_id == BLE_UNKNOWN_GROUP_ID {
            warn!(
                "disconnect_le_group_by_member_address: [{}]: address belongs to no group",
                DisplayAddress(&addr)
            );
            return;
        }
        ...
        info!(
            "disconnect_le_group_by_member_address: [{}]: available profiles: {:?}.",
            DisplayAddress(&addr),
            available_profiles
        );
    ...
    fn connect_le(&mut self, address: String) {
        ...
        let addr = match RawAddress::from_string(address.clone()) {
            None => {
                warn!("connect_le: invalid device address");
                return;
            }
            Some(addr) => addr,
        };
        ...
        if *self.le_audio_states.get(&addr).unwrap_or(&BtLeAudioConnectionState::Disconnected)
            == BtLeAudioConnectionState::Connected
        {
            info!("connect_le: already connected.");
            return;
        }
        ...
        info!(
            "connect_le: [{}]: connecting, available profiles: {:?}.",
            DisplayAddress(&addr),
            available_profiles
        );
        ...
        match self.le_audio.as_mut() {
            ...
            None => {
                warn!("connect_le: [{}]: uninitialized LeAudio to connect", DisplayAddress(&addr));
            }
        };
    }
    ...
    fn disconnect_le(&mut self, address: String) {
        let addr = match RawAddress::from_string(address.clone()) {
            None => {
                warn!("disconnect_le: invalid device address");
                return;
            }
            Some(addr) => addr,
        };
        ...
        if *self.le_audio_states.get(&addr).unwrap_or(&BtLeAudioConnectionState::Disconnected)
            == BtLeAudioConnectionState::Disconnected
        {
            info!("disconnect_le: [{}]: already disconnected", DisplayAddress(&addr));
            return;
        }

        info!("disconnect_le: [{}]: disconnecting", DisplayAddress(&addr));
        ...
        match self.le_audio.as_mut() {
            ...
            None => {
                warn!(
                    "disconnect_le: [{}]: uninitialized LeAudio to disconnect",
                    DisplayAddress(&addr)
                );
            }
        };
    ...
    fn connect_vc(&mut self, address: String) {
        ...
        let addr = match RawAddress::from_string(address.clone()) {
            None => {
                warn!("connect_vc: invalid device address");
                return;
            }
            Some(addr) => addr,
        };
    ...
    fn disconnect_vc(&mut self, address: String) {
        let addr = match RawAddress::from_string(address.clone()) {
            None => {
                warn!("disconnect_vc: invalid device address");
                return;
            }
            Some(addr) => addr,
        };
