...
pub trait IBluetoothMedia {
    ...
    fn start_audio_request(&mut self, listener: File) -> bool;
    fn stop_audio_request(&mut self, listener: File);
...
impl IBluetoothMedia for BluetoothMedia {
    ...
    fn start_audio_request(&mut self, listener: File) -> bool {
        if self.a2dp_audio_connection_listener.is_some() {
            warn!("start_audio_request_impl: replacing an unresolved listener");
        }
    ...
    fn stop_audio_request(&mut self, listener: File) {
        ...
        if self.a2dp_audio_connection_listener.is_some() {
            warn!("stop_audio_request_impl: replacing an unresolved listener");
        }
