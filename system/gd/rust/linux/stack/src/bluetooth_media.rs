...
impl BluetoothMedia {
    ...
    fn notify_media_capability_added(&self, addr: RawAddress) {
        fn dedup_added_cb(
            ...
            if let Some(task) = device_added_tasks.lock().unwrap().insert(addr, None) {
                match task {
                    // There is a pending task that could be caller itself.
                    Some(handler) => {
                        if abort_pending {
                            handler.abort();
                        }
            ...
            (_, _) => {
                ...
                if guard.get(&addr).is_none() {
                    ...
                    let task = topstack::get_runtime().spawn(async move {
                        ...
                        if dedup_added_cb(device_added_tasks, addr, callbacks, cap, hfp_cap, false) {
                            warn!(
