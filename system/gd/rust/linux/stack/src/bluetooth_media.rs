...
impl BluetoothMedia {
    ...
    pub fn dispatch_uhid_telephony_use_callback(&mut self, address: String, state: bool) {
        ...
        if state == false {
            // As there's a HID call for each WebHID call, even if it has been answered in the app or
            // pre-exists, and that an app which disconnects from WebHID may not have trigger the
            // UHID_OUTPUT_NONE, we need to remove all pending HID calls on telephony use release
            // to keep lower HF layer in sync and not prevent A2DP streaming
