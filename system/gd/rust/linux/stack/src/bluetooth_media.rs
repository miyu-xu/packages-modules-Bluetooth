...
impl BluetoothMedia {
    ...
    fn notify_media_capability_added(&self, addr: RawAddress) {
        fn dedup_added_cb(
            ...
            // Device is already added or removed when the pending task sleeps
            if (!task.is_none() && task.as_ref().unwrap().is_none())
                || (task.is_none() && is_delayed)
            {
                return false;
            }

            // Call the callbacks no matter this is triggered by the instant
            // call or delayed task.
            ...
            // Abort pending task. task.unwrap.is_none should have been handled.
            if !task.is_none() && !is_delayed {
                task.unwrap().unwrap().abort();
            }
