...
impl BluetoothMedia {
                ...
                match state {
                    ...
                    BthfConnectionState::Disconnected => {
                        ...
                        match self.hfp_states.remove(&addr) {
                            Some(_) => self.notify_media_capability_removed(addr),
                            _ => warn!("[{}] Unknown address hfp disconnected.", addr.to_string()),
                        }
                        return;
                    }
    ...
    fn notify_media_capability_added(&self, addr: RawAddress) {
        fn dedup_added_cb(
            ...
            if !device_added.lock().unwrap().insert(addr) {
                return false;
            }
        ...
        match (cur_a2dp_cap, cur_hfp_cap) {
            ...
            (_, _) => {
                ...
                topstack::get_runtime().spawn(async move {
                    ...
                    if dedup_added_cb(device_added, addr, callbacks, cap, hfp_cap) {
                        warn!(
    ...
    fn notify_media_capability_removed(&self, addr: RawAddress) {
        ...
        if self.device_added.lock().unwrap().remove(&addr) {
            // It is enforced and intended that we remove the Bluetooth device from the
            // audio service if any capacity is removed.
            self.for_all_callbacks(|callback| {
