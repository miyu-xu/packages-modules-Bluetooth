...
pub struct Callbacks<T: Send + ?Sized> {
    ...
    objectpaths: HashMap<String, u32>,
    tx: Sender<Message>,
...
impl<T: RPCProxy + Send + ?Sized> Callbacks<T> {
    ...
    pub fn new(tx: Sender<Message>, disconnected_message: fn(u32) -> Message) -> Self {
        Self { callbacks: HashMap::new(), objectpaths: HashMap::new(), tx, disconnected_message }
    }

    /// Stores a new callback and monitors for callback disconnect. If the callback object path
    ...
    pub fn add_callback(&mut self, mut callback: Box<T>) -> u32 {
        match self.objectpaths.get(&callback.get_object_id()) {
            Some(cbid) => {
                return *cbid;
            }
            None => {}
        ...
        self.objectpaths.insert(callback.get_object_id(), id);
    ...
    pub fn remove_callback(&mut self, id: u32) -> bool {
        match self.callbacks.get_mut(&id) {
            Some(callback) => {
                ...
                self.objectpaths.remove(&callback.get_object_id());
...
mod tests {
    ...
    fn test_add_and_remove() {
        ...
        // Attempting to add another callback with same object path should return the same cbid
