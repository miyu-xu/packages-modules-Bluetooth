...
impl SoftwareRotationAdvertiseManagerImpl {
    ...
    /// Clears the removed or disabled advertisers from the queue and invoke callback.
    ...
    fn run_rotate(&mut self) {
        ...
        // We are Advertising. Check whether current is the only one enabled.
        ...
        if !current_is_enabled {
            // If current is now disabled or removed, stop it and then
            // let |refresh_queue| handle the callback.
            self.stop_current_advertising();
            self.refresh_queue();
            self.start_next_advertising();
        } else {
            // Current is still enabled, refresh the other advertisers in the queue.
            self.refresh_queue();
...
impl BtifGattAdvCallbacks for SoftwareRotationAdvertiseManagerImpl {
    fn on_advertising_set_started(
        ...
        // 3. Disable or remove the advertiser if failed
        ...
        // At this point we're fine to suspend.
        if self.suspend_mode != SuspendMode::Normal {
            self.stop_current_advertising();
            self.set_suspend_mode(SuspendMode::Suspended);
            return;
