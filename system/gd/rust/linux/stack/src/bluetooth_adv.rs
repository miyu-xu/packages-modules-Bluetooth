...
/// Unlike the extended advertising stack we can't propagate the BTIF Advertiser ID to DBus clients
/// because there can be at most 1 advertier under BTIF layer at the same time.
...
struct SoftwareRotationAdvertiseInfo {
    ...
    /// Filled in on the first time the advertising set started.
    tx_power: Option<i32>,
...
enum SoftwareRotationAdvertiseState {
    /// No advertising is running in LibBluetooth.
    Stopped,
    /// A StartAdvertisingSet call to LibBluetooth is pending.
    Pending(SoftwareRotationAdvertierId),
    /// An advertising is running in LibBluetooth, i.e., an OnAdvertisingSetStarted is received.
...
struct SoftwareRotationAdvertiseManagerImpl {
    ...
    /// When they are removed from the queue, on_advertising_enabled callback needs to be sent.
    /// Note that the advertising running in LibBluetooth should *NOT* be in the queue.
...
impl SoftwareRotationAdvertiseManagerImpl {
    ...
    /// Clears removed or disabled advertising from the queue and invoke callback.
    fn refresh_queue(&mut self) {
        let now = Instant::now();
        let adv_queue = self.adv_queue.clone();
        self.adv_queue.clear();
        for id in adv_queue.iter() {
            let Some(info) = self.adv_info.get_mut(&id) else {
                continue;
            };
            let is_expired = info.expire_time.map_or(false, |t| t < now);
            if is_expired {
                info.enabled = false;
    ...
    fn stop_current_advertising(&mut self) {
        self.state = match &self.state {
            ...
            SoftwareRotationAdvertiseState::Stopped => return,
        };
    }
    ...
    fn run_rotate(&mut self) {
        ...
        // We are Advertising. Check whether current is the only enabled one.
        // If so, don't need to rotate.
        ...
        if !current_is_enabled {
            // If current should be disabled, stop it and rotate.
            self.stop_current_advertising();
            self.refresh_queue();
            self.start_next_advertising();
        } else {
            // Current is still enabled, refresh the rest in the queue.
            self.refresh_queue();
...
impl IBluetoothAdvertiseManager for SoftwareRotationAdvertiseManagerImpl {
    ...
    fn unregister_callback(&mut self, callback_id: u32) -> bool {
        self.adv_info.retain(|id, info| info.callback_id != callback_id);
    ...
    fn start_advertising_set(
        ...
        // expire_time will be determined on the adv is started the first time.
    ...
    fn enable_advertising_set(
        ...
        info.enabled = enable;
        // We won't really call enable to BTIF. Calculate the expire time right now.
        info.expire_time = gen_expire_time_from_now(duration);
        // This is actually not used as the expire_time is already determined.
        // Still fill it in to keep the data updated.
        info.duration = duration;

        if enable && !self.adv_queue.contains(&info.id) && current_id != Some(info.id) {
            // The adv was not enabled and not in the queue. Invoke callback and push it.
            self.callbacks.get_by_id_mut(info.callback_id).map(|cb| {
                cb.on_advertising_enabled(info.id, false, AdvertisingStatus::Success);
            });
    ...
    fn on_advertising_set_started(
        ...
        if let Some(info) = self.adv_info.get_mut(&reg_id) {
            if info.tx_power.is_none() {
                // It is the first time this advertising "started". Send callback.
                if status == AdvertisingStatus::Success {
                ...
                // Not the first time. This is a normal rotation.
                if status == AdvertisingStatus::Success {
                    // If this advertising is already disabled, stop it right now.
                    if !info.enabled {
                        self.stop_current_advertising();
                    }
                } else {
                    // We are not able to report the failure through on_advertising_set_started
                    // because it's already sent before. Disable it instead.
                    info.enabled = false;
                    // Push to the queue and let it be removed on refresh.
                    self.adv_queue.push_back(reg_id);
                }
            }
        } else {
            // This advertising is already removed. Stop it right now.
            self.stop_current_advertising();
        }

        // Rotate again if the next adv is the first time. We need to consume all
        // "first time" adv before suspended to make sure callbacks are sent.
