...
use crate::uuid::{parse_uuid_string, UuidBytes, UuidHelper};
...
impl AdvertiseData {
    ...
    fn append_uuids(dest: &mut Vec<u8>, ad_types: &Vec<u8>, uuids: &Vec<Uuid>) {
        ...
        for uuid in uuids {
            let uuid_bytes = UuidHelper::get_shortest_bytes(&uuid.uu);
            match uuid_bytes {
                UuidBytes::Uuid16Bit(mut bytes) => {
                    bytes.reverse();
                    bytes_list[0].extend(bytes);
                }
                UuidBytes::Uuid32Bit(mut bytes) => {
                    bytes.reverse();
                    bytes_list[1].extend(bytes);
                }
                UuidBytes::Uuid128Bit(mut bytes) => {
                    bytes.reverse();
                    bytes_list[2].extend(bytes);
                }
            }
        }
