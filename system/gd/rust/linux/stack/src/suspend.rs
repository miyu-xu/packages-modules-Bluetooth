...
struct SuspendState {
    ...
    le_rand_timeout_timer: Option<JoinHandle<()>>,
...
impl Suspend {
    ...
    fn suspend_ready(&mut self, suspend_id: i32) {
        ...
        if suspend_state.delay_timer.is_some()
            || suspend_state.disconnect_timeout_timer.is_some()
            || suspend_state.le_rand_timeout_timer.is_some()
        {
    ...
    fn device_disconnected(&mut self, addr: RawAddress) {
        ...
        Self::device_disconnected_complete(
            self.tx.clone(),
            &mut suspend_state,
    ...
    fn device_disconnected_complete(
        tx: Sender<Message>,
...
impl ISuspend for Suspend {
    ...
    fn suspend(&mut self, suspend_type: SuspendType, suspend_id: i32) {
        ...
        // Send le_rand command and wait until completed.
        if let Some(h) = suspend_state.le_rand_timeout_timer.take() {
            log::warn!("Suspend: Found a leftover timer for le_rand");
            h.abort();
        }
        ...
        let tx = self.tx.clone();
        let suspend_state_cloned = self.suspend_state.clone();
        suspend_state.le_rand_timeout_timer = Some(tokio::spawn(async move {
            time::sleep(Duration::from_millis(2000)).await;
            ...
            suspend_state_cloned.lock().unwrap().le_rand_timeout_timer = None;
            let _result =
                tx.send(Message::SuspendActions(SuspendActions::SuspendReady(suspend_id))).await;
        }));
    }
    ...
    fn resume(&mut self) -> bool {
        ...
        if let Some(h) = suspend_state.le_rand_timeout_timer.take() {
            log::warn!("Resume: Found a leftover timer for le_rand");
            h.abort();
        }
        let tx = self.tx.clone();
