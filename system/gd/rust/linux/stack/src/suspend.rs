...
impl SuspendState {
    pub fn new(
        ...
        Self { le_rand_expected: bool, suspend_expected: bool, resume_expected: bool }
    }
}
...
impl Suspend {
    pub fn new(
        ...
        Self {
            ...
            suspend_state: Arc::new(Mutex::new(SuspendState::new(false, false, false))),
        }
    }
    ...
    pub(crate) fn resume_ready(&self, suspend_id: u32) {
        self.callbacks.for_all_callbacks(|callback| {
...
impl ISuspend for Suspend {
    ...
    fn resume(&mut self) -> bool {
        ...
        self.suspend_timeout_joinhandle = Some(tokio::spawn(async move {
            ...
            tokio::spawn(async move {
                let _result = tx.send(Message::ResumeReady(1)).await;
            });
        }));
