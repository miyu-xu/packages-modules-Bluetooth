    ...
    fn suspend(&self, suspend_type: SuspendType) -> u32 {
        ...
        match suspend_type {
            ...
            SuspendType::AllowWakeFromHid => {
                ...
                // self.intf.lock().unwrap().set_event_filter_allow_classic_hid_only()
                // self.intf.lock().unwrap().set_event_mask_allow_classic_hid_only()
                // TODO(230604670): check if A2DP is connected
                // TODO(224603198): save all advertiser information
                // self.is_connected_suspend = true;
            }
            ...
            SuspendType::NoWakesAllowed => {
                ...
                // self.is_connected_suspend = false;
            }
            SuspendType::Other => {
