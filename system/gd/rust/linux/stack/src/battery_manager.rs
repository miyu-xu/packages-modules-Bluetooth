...
/// Helper representation of a collection of batteries to simplify
...
impl BatteryManager {
    ...
    /// Handles all callback messages to acoid deadlocks.
    pub fn handle_callback(&mut self, callback: BatteryConsumerCallbacks) {
        match callback {
            BatteryConsumerCallbacks::OnBatteryInfoUpdated(remote_address, batteries) => {
                let batteries = batteries.map(|battery| battery.batteries);
...
impl Batteries {
    ...
    /// Updates a battery matching all non-battery-level fields if
    /// found, otherwise adds new_battery verbatim.
    pub fn add_or_update_battery(&mut self, new_battery: Battery) {
        match self.batteries.iter_mut().find(|battery| {
            battery.remote_address == new_battery.remote_address
    ...
    /// Returns a Batteries object containing the highest quality pick
...
impl IBatteryConsumerCallback for BatteryConsumerCallback {
    // All callback messages get relayed to the main thread for
    // processing.
