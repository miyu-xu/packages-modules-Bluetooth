...
pub trait IBatteryManagerCallback: RPCProxy {
    ...
    /// Invoked on completion on an explicit read, such as from a refresh.
...
pub trait IBatteryManager {
    ...
    /// Forces a refresh of battery information for a device. Returns
    /// false if BAS is unable to initiate the refresh.
    fn refresh_battery_information(&self, remote_address: String) -> bool;
}

/// Repesentation of the BatteryManager.
pub struct BatteryManager {
    bas: Arc<Mutex<Box<BatteryService>>>,
    /// Sender used for callbacks to relay messages to the main thread.
    tx: Sender<Message>,
...
impl BatteryManager {
    ...
    pub fn new(bas: Arc<Mutex<Box<BatteryService>>>, tx: Sender<Message>) -> BatteryManager {
        ...
        Self {
            ...
            tx: tx.clone(),
            callbacks: callbacks,
            notifications_enabled: HashSet::new(),
        }
    }
