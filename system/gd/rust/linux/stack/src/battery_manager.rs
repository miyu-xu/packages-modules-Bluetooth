...
/// Describes an individual battery measurement, possibly one of many for a
/// given device.
#[derive(Debug, Clone)]
pub struct Battery {
    pub percentage: u32,
...
/// Helper representation of a collection of BatterySet to simplify passing
...
impl IBatteryManager for BatteryManager {
    ...
    fn get_battery_information(&self, remote_address: String) -> Option<BatterySet> {
        Some(self.battery_provider_manager.lock().unwrap().get_battery_info(remote_address)?)
    }
}
