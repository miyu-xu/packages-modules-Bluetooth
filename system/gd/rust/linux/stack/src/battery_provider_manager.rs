...
pub trait IBatteryProviderManager {
    /// Registers a BatteryProvider and generates a unique batttery provider ID
    /// for future calls.
    ...
    /// Unregisters a BatteryProvider, potentially removes battery information
    /// for the remote device if there are no other providers.
    ...
    fn set_battery_info(&mut self, battery_provider_id: u32, battery: Battery);
}

/// Represents the BatteryProviderManager, a central point for collecting
/// battery information from numerous sources.
...
impl BatteryProviderManager {
    /// Constructs a new BatteryProviderManager with callbacks communicating on
    /// tx.
...
impl IBatteryProviderManager for BatteryProviderManager {
    ...
    fn set_battery_info(&mut self, _battery_provider_id: u32, battery: Battery) {
        ...
        if !battery_info.contains_key(&battery.address) {
            battery_info.insert(battery.address.clone(), Batteries::new());
        }
        if let Some(batteries) = battery_info.get_mut(&battery.address) {
