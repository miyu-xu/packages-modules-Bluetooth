...
impl BatteryService {
    ...
    fn init_device(&self, remote_address: String) {
        log::info!("melhuishj: init_device for {}", remote_address.clone());
        let client_id = match *self.client_id.lock().unwrap() {
            Some(id) => id,
            None => {
                log::info!(
                    "melhuishj: init_device failed for {} due to no client_id",
                    remote_address
                );
                return;
            }
        ...
        let bas_uuid = match parse_uuid_string(BAS) {
            ...
            None => {
                log::info!(
                    "melhuishj: init_device failed for {} due to unable to parse BAS",
                    remote_address
                );
                return;
            }
        };
        log::info!("melhuishj: beginning client_connect for {}", remote_address.clone());
...
impl IBatteryService for BatteryService {
    ...
    fn get_battery_level(&self, remote_address: String) -> Option<u32> {
        log::info!("Attempting to return battery level for {}", remote_address.clone());
    ...
    fn refresh_battery_level(&self, remote_address: String) {
        let client_id = match *self.client_id.lock().unwrap() {
            ...
            None => {
                log::info!(
                    "melhuishj: refresh_battery_level failed for {} due to no client_id",
                    remote_address
                );
                return;
            }
...
impl GattCallback {
    ...
    fn set_battery_level(&self, remote_address: String, value: Vec<u8>) {
        log::info!("Setting battery level for {}", remote_address.clone());
...
impl IBluetoothGattCallback for GattCallback {
    fn on_client_registered(&self, status: GattStatus, client_id: i32) {
        log::info!("melhuishj: on_client_registered with client_id {}", client_id);
    ...
    fn on_client_connection_state(
        ...
        log::info!(
            "melhuishj: on_client_connection_state for {} with GattStatus {:?}",
            addr.clone(),
            status.clone()
        );
        if !connected {
            log::info!("melhuishj: not connected to {} with GattStatus {:?}", addr, status);
            return;
        }
        let client_id = match *self.client_id.lock().unwrap() {
            Some(id) => id,
            None => {
                log::info!("melhuishj: init_device failed for {} due to no client_id", addr);
                return;
            }
        };
        log::info!("melhuishj: beginning to discover_services for {}", addr.clone());
        self.gatt.lock().unwrap().discover_services(client_id, addr); //, BAS.to_string());
    }
    ...
    fn on_search_complete(
        ...
        log::info!("melhuishj: finished searching for BAS on {}", addr.clone());
        let bas_uuid = match parse_uuid_string(BAS) {
            Some(uuid) => uuid,
            None => return,
        };
