...
impl IBluetooth for Bluetooth {
    ...
    fn is_dual_mode_audio_sink_device(&self, device: BluetoothDevice) -> bool {
        let group_id = self.bluetooth_media.lock().unwrap().get_group_id(device.address);
        ...
        for addr in self
            ...
            .lock()
            .unwrap()
            .get_group_devices(group_id)
            .unwrap_or(&HashSet::new())
            .iter()
        {
            let uuids = self.get_remote_uuids(BluetoothDevice::new(*addr, "".to_string()));
            if uuids.contains(&get_unwrapped_uuid(Profile::LeAudio))
