...
impl BtifBluetoothCallbacks for Bluetooth {
    ...
    fn ssp_request(
        ...
        // For Just-Works pairing, if it is initiated by us => auto accept the request. Otherwise, auto reject.
        if variant == BtSspVariant::Consent {
            let initiated_by_us = self.bonding_devices.contains_key(&remote_addr);
    ...
    fn bond_state(
        ...
        if &bond_state == &BtBondState::NotBonded {
            ...
            self.bonding_devices.remove(&address);
        }
...
impl IBluetooth for Bluetooth {
    ...
    fn create_bond(&mut self, device: BluetoothDevice, transport: BtTransport) -> bool {
        ...
        self.bonding_devices.insert(&address);
