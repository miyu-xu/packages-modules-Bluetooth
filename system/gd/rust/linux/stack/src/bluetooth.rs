...
use bt_topshim::btif::{
    ...
    BtBondState, BtDeviceType, BtDiscoveryState, BtHciErrorCode, BtLocalLeFeatures, BtPinCode,
...
impl Bluetooth {
    ...
    fn get_remote_device_if_found_mut(
        ...
        if self.bonded_devices.contains_key(&device.address) {
            return self.bonded_devices.get_mut(&device.address);
        } else if self.found_devices.contains_key(&device.address) {
            return self.found_devices.get_mut(&device.address);
        } else {
            return None;
        }
    }
    ...
    fn set_remote_device_property(
        ...
        self.get_remote_device_if_found_mut(&device)
            .and_then(|d: &mut BluetoothDeviceContext| d.properties.insert(property_type, property))
            .map(|_| ())
            .ok_or(())
    }
}
...
impl IBluetooth for Bluetooth {
    ...
    fn get_remote_type(&self, device: BluetoothDevice) -> u32 {
        match self.get_remote_device_property(&device, &BtPropertyType::TypeOfDevice) {
            Some(BluetoothProperty::TypeOfDevice(device_type)) => {
                return BtDeviceType::to_u32(&device_type).unwrap_or_default()
            }
            _ => return 0,
        }
    }
    ...
    fn set_remote_alias(&mut self, device: BluetoothDevice, new_alias: String) {
        self.set_remote_device_property(
            &device,
