...
impl Bluetooth {
    ...
    fn disable_profile(&mut self, profile: &Profile) {
        if !self.uuid_helper.is_profile_supported(profile) {
            return;
        }
    ...
    fn is_profile_enabled(&self, profile: &Profile) -> Option<bool> {
        ...
        match profile {
            ...
            Profile::Hfp => Some(self.bluetooth_media.lock().unwrap().is_hfp_enabled()),
            _ => None,
        }
    }

    pub fn is_uuid_supported(&self, uuid: &Uuid128Bit) -> bool {
        match self.uuid_helper.is_known_profile(uuid) {
            Some(profile) => self.uuid_helper.is_profile_supported(profile),
            None => false,
        }
    }
        ...
        for profile in self.uuid_helper.get_supported_profiles() {
            ...
            if let Some(enabled) = self.is_profile_enabled(&profile) {
                let allowed = allowed_services.len() == 0
                    || allowed_services
                        .contains(&self.uuid_helper.get_profile_uuid(&profile).unwrap().clone());
    ...
    pub(crate) fn trigger_freshness_check(&mut self) {
        ...
        for d in stale_devices {
            ...
            self.bluetooth_admin.lock().unwrap().on_device_cleared(d.clone());
        }
...
impl BtifBluetoothCallbacks for Bluetooth {
    ...
    fn device_found(&mut self, _n: i32, properties: Vec<BluetoothProperty>) {
        ...
        self.bluetooth_admin.lock().unwrap().on_device_found(device.info.clone());
    }
    ...
    fn remote_device_properties_changed(
        ...
        match device {
            Some(d) => {
                d.update_properties(properties.clone());
                d.seen();
                ...
                self.bluetooth_admin
                    .lock()
                    .unwrap()
                    .on_remote_device_properties_changed(info.clone(), properties.clone());
            }
            None => (),
        };
    }
...
impl IBluetooth for Bluetooth {
    ...
    fn connect_all_enabled_profiles(&mut self, device: BluetoothDevice) -> bool {
        ...
        for uuid in uuids.iter() {
            match self.uuid_helper.is_known_profile(uuid) {
                Some(p) => {
                    if self.uuid_helper.is_profile_supported(&p) {
                        match p {
