...
impl Bluetooth {
    ...
    fn check_new_property_and_potentially_connect_profiles(
        ...
        let device = self.remote_devices.get(&addr);
        if device.is_none() || !device.unwrap().connect_to_new_profiles {
            return;
        }
        ...
        log::info!("Verbose debug: addr {:?}, new UUIDs {:?}", addr, new_uuids);
        ...
        log::info!("Verbose debug: connecting to newly discovered profiles {:?}", new_uuids);
        let tx = self.tx.clone();
        tokio::spawn(async move {
            let _ = tx
    ...
    fn connect_profiles_internal(&mut self, uuids: &Vec<Uuid>, device: BluetoothDevice) {
        ...
        log::info!("Verbose debug: addr {:?} has uuids {:?}", addr, uuids);
...
impl BtifBluetoothCallbacks for Bluetooth {
    ...
    fn remote_device_properties_changed(
        ...
        self.check_new_property_and_potentially_connect_profiles(addr.clone(), &properties);
