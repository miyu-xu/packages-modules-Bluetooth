...
pub trait IBluetooth {
    ...
    fn set_discoverable(&mut self, mode: u32, duration: u32) -> bool;
...
impl IBluetooth for Bluetooth {
    ...
    fn set_discoverable(&mut self, mode: u32, duration: u32) -> bool {
        ...
        let disc_mode = match mode.try_into() {
            Err(_) => return false,
            Ok(mode) => mode,
        };

        // Checks if the duration is valid.
        if disc_mode == BtDiscMode::LimitedDiscoverable && (duration <= 0 || duration > 60) {
            warn!("Invalid duration for setting the device into limited discoverable mode. The valid duration: 1~60 (sec).");
            return false;
        }
        ...
        if disc_mode == BtDiscMode::LimitedDiscoverable {
