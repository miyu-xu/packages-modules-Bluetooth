                        ...
                        match p {
                            Profile::Hid | Profile::Hogp => {
                                ...
                                metrics::profile_connection_state_changed(
                                    addr.unwrap(),
                                    metrics::MetricsProfileConnectionIntent::Connect,
                                    *p as u32,
                                    BtStatus::Success,
...
impl BtifHHCallbacks for Bluetooth {
    fn connection_state(&mut self, address: RawAddress, state: BthhConnectionState) {
        ...
        // When the hid host connection state is (dis)connecting, a (dis)connect request is
        // received from the remote device. Hence the intent of whether the status update is for
        // connecting or disconnecting is clear. However, a disconnected state can be generated
        // inconsistent with the intent. e.g., remote device issued a connection requested but
        // rejected and reported as disconnected due to HID device limit. Defer to metrics process
        // unit to resolve the discrepancy.
        let intent = match state {
            BthhConnectionState::Connected | BthhConnectionState::Connecting => {
                metrics::MetricsProfileConnectionIntent::Connect
            }
            BthhConnectionState::Disconnecting => {
                metrics::MetricsProfileConnectionIntent::Disconnect
            }
            _ => metrics::MetricsProfileConnectionIntent::Unknown,
        };
