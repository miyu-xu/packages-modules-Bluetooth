            ...
            match m.unwrap() {
                ...
                Message::OnDeviceDisconnected(device) => {
                    battery_service
                        .lock()
                        .unwrap()
                        .handle_action(BatteryServiceActions::Disconnect(device.address));
                }
                ...
                Message::ProfileDisconnected(device_address) => {
                    ...
                    // Ideally we would also check that there are no
                    // open sockets for this device but Floss does not
                    // manage socket state so there is no reasonable
                    // way for us to know whether a socket is open or
                    // not.
                            ...
                            .is_empty()
                    {
                        battery_service
                            .lock()
                            .unwrap()
                            .handle_action(BatteryServiceActions::Disconnect(device_address));
                    }
                }
            }
        }
