        ...
        topstack::get_runtime().spawn_blocking(move || {
            ...
            loop {
                ...
                match OutputEvent::try_from(event) {
                    ...
                    Err(e) => {
                        match e {
                            StreamError::Io(e) => log::error!("WebHID: IO error: {}", e),
                            StreamError::UnknownEventType(e) => {
                                log::error!("WebHID: Unknown event type: {}", e)
                            }
                        }
                        break;
                    }
            ...
            debug!("WebHID: UHid reading loop completed");
        });
