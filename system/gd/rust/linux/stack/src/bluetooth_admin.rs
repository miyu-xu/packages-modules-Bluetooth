...
impl BluetoothAdmin {
    pub fn new(path: Option<String>, tx: Sender<Message>) -> BluetoothAdmin {
    ...
    fn get_affected_status(&self, blocked_services: &Vec<Uuid128Bit>) -> bool {
        ...
        blocked_services
            .iter()
            .find(|&uuid| match self.uuid_helper.is_known_profile(uuid) {
                Some(p) => self.uuid_helper.is_profile_supported(p),
                _ => false,
            })
            .is_some()
    }
    ...
    fn new_device_policy_effect(&self, uuids: Option<Vec<Uuid128Bit>>) -> Option<PolicyEffect> {
        if let Some(uuids) = uuids {
            ...
            Some(PolicyEffect { service_blocked, affected })
        } else {
            None
        }
    }
    ...
    pub fn on_device_cleared(&mut self, remote_device: BluetoothDevice) {
        self.device_policy_affect_cache.remove(&remote_device.clone());
    }
    ...
    pub fn on_remote_device_properties_changed(
        ...
        let cur_effect = self.device_policy_affect_cache.get(&remote_device.clone());
...
mod tests {
    ...
    fn test_set_service_allowed() {
        ...
        let mut admin = BluetoothAdmin::new(None, tx.clone());
