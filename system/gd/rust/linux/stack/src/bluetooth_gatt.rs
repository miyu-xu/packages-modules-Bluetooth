...
impl BluetoothGatt {
    ...
    /// Initialises AdvertiseManager.
    ///
    /// This needs to be called after Bluetooth is ready. Query |is_le_ext_adv_supported| outside
    /// this function (before locking BluetoothGatt) to avoid deadlock.
    ///
    /// TODO(b/242083290): Correctly fire IsReady message for Adv API in this function after the
    /// API is fully splitted out. For now Gatt is delayed for 500ms (see self.init_profiles())
    /// which shall be enough for Bluetooth to become ready.
