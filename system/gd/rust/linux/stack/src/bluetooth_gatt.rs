...
impl BluetoothGatt {
    ...
    fn msft_run_queue_and_update_scan(&mut self) {
        if !matches!(self.msft_command_pending, MsftCommandPending::NoPending) {
            // There's already a pending command so just return and wait. After the pending command
            // is done, the MSFT callbacks would reset |self.msft_command_pending| and call this.
            return;
        }
