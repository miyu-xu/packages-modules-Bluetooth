...
impl BluetoothGatt {
    ...
    pub fn init_profiles(&mut self, tx: Sender<Message>, adapter: Arc<Mutex<Box<Bluetooth>>>) {
        println!("woot woot");
        log::info!("melhuishj: init_profiles");
...
impl IBluetoothGatt for BluetoothGatt {
    ...
    fn register_client(
        ...
        log::info!("melhuishj: registering client for uuid {}", app_uuid.clone());
        let uuid = parse_uuid_string(app_uuid).expect("uuid could not be parsed");
        ...
        log::info!(
            "melhuishj: client registered for uuid {:?} gatt_status: {:?}",
            &uuid,
            gatt_status
        );
    }
...
impl BtifGattClientCallbacks for BluetoothGatt {
    fn register_client_cb(&mut self, status: GattStatus, client_id: i32, app_uuid: Uuid) {
        log::info!("melhuishj: register_client_cb");
        ...
        log::info!("melhuishj: register_client_cb done");
    }
