    ...
    fn start_scan(
        ...
        tokio::spawn(async move {
            if let Some(filter) = filter {
                let monitor_handle =
                    match gatt_async.lock().await.msft_adv_monitor_add((&filter).into()).await {
                        Ok((handle, 0)) => handle,
                        _ => {
            ...
            gatt_async.lock().await.update_scan().await;
        });
    ...
    fn stop_scan(&mut self, scanner_id: u8) -> BtStatus {
        ...
        tokio::spawn(async move {
            if let Some(handle) = monitor_handle {
                let _res = gatt_async.lock().await.msft_adv_monitor_remove(handle).await;
            }
