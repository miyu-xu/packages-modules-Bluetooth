...
pub struct BluetoothGatt {
    ...
    is_active_scan: Option<bool>, // The controller scan type. None means not scanning.
    msft_enabled: bool,           // The controller MSFT state.

    // To add or remove an MSFT monitor, push the command into |msft_command_queue| and then call
    // |msft_run_queue_and_update_scan|. |msft_run_queue_and_update_scan| would take care of
    // dispatching them to LibBluetooth.
...
impl BluetoothGatt {
    ...
    pub fn new(intf: Arc<Mutex<BluetoothInterface>>, tx: Sender<Message>) -> BluetoothGatt {
        BluetoothGatt {
            ...
            msft_command_pending: MsftCommandPending::Idle,
    ...
    fn update_scan(&mut self, force_restart: bool) {
        let mut is_active_scan = None; // None means no need to scan
    ...
    /// Consumes the MSFT command queue and update scan at the end.
    ///
    ...
    fn msft_run_queue_and_update_scan(&mut self) {
        if !matches!(self.msft_command_pending, MsftCommandPending::Idle) {
            return;
        }
        match self.msft_command_queue.pop_front() {
            ...
            Some(MsftCommandQueue::Remove(monitor_handle)) => {
                ...
                self.msft_command_pending = MsftCommandPending::Remove;
            }
        }
    }
...
impl BtifGattScannerInbandCallbacks for BluetoothGatt {
    ...
    fn inband_msft_adv_monitor_add_callback(&mut self, monitor_handle: u8, status: u8) {
        ...
        let MsftCommandPending::Add(scanner_id, ref monitor) = self.msft_command_pending else {
            error!("Unexpected MsftAdvMonitorAddCallback, pending={:?}", self.msft_command_pending);
            return;
        };
        let monitor = monitor.clone();
        self.msft_command_pending = MsftCommandPending::Idle;

        if status != 0 {
            error!("Error adding advertisement monitor {:?}, status={}", monitor, status);
        } else {
            match monitor.condition {
                ...
                _ => warn!("Unhandled MSFT monitor added {:?}", monitor),
            }
        }
        self.msft_run_queue_and_update_scan();
    }
    ...
    fn inband_msft_adv_monitor_remove_callback(&mut self, status: u8) {
        ...
        let MsftCommandPending::Remove = self.msft_command_pending else {
            error!(
                "Unexpected MsftAdvMonitorRemoveCallback, pending={:?}",
                self.msft_command_pending
            );
            return;
        };
        self.msft_command_pending = MsftCommandPending::Idle;
    ...
    fn inband_msft_adv_monitor_enable_callback(&mut self, status: u8) {
        ...
        let MsftCommandPending::Enable(enabled) = self.msft_command_pending else {
            error!(
                "Unexpected MsftAdvMonitorEnableCallback, pending={:?}",
                self.msft_command_pending
            );
            return;
        };
