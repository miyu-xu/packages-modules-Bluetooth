...
impl BluetoothGatt {
    ...
    pub(crate) fn start_active_scan(&mut self, scanner_id: u8) -> BtStatus {
        let interval = sysprop::get_i32(sysprop::PropertyI32::LeInquiryScanInterval);
        let window = sysprop::get_i32(sysprop::PropertyI32::LeInquiryScanWindow);

        let settings =
            ScanSettings { interval: interval, window: window, scan_type: ScanType::Active };
