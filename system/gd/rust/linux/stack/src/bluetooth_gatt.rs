...
pub trait IBluetoothGatt {
    ...
    /// * `periodic_parameters` - Periodic advertising parameters. If None, periodic advertising will not be started.
    /// * `periodic_data` - Periodic advertising data.
    /// * `duration` - Advertising duration, in 10ms unit. Valid range is from 1 (10ms) to 65535 (655.35s). 0ms advertising should continue until stopped.
...
/// Interface for advertiser callbacks to clients, passed to `IBluetoothGatt::start_advertising_set`.
...
pub struct AdvertisingSetParameters {
    ...
    /// The advertising interval. Bluetooth LE Advertising interval, in 0.625ms unit. The valid range is from 160 (100ms) to 16777215 (10,485.759375 s). Recommended values are: 160 (100ms), 400 (250ms), 1600 (1s).
    pub interval: i32,
    /// Transmission power of Bluetooth LE Advertising, in dBm. The valid range is [-127, 1]. Recommended values are: -21, -15, 7, 1.
    pub tx_power_level: i32,
    /// Own address type for advertising to control public or privacy mode. The valid types are: -1 (default), 0 (public), 1 (random).
...
pub struct AdvertiseData {
    /// A list of service UUIDs within the advertisement that are used to identify the Bluetooth GATT services.
...
pub struct PeriodicAdvertisingParameters {
    ...
    pub include_tx_power: bool,
    /// Periodic advertising interval in 1.25ms unit. Valid values are from 80 (100ms) to 65519 ms (81.89875s). Value from range [interval, interval+20ms] will be picked as the actual value.
    pub interval: i32,
}
