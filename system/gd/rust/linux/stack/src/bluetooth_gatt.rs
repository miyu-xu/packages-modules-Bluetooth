...
impl ServerContextMap {
    ...
    fn get_by_server_id(&self, server_id: i32) -> Option<&Server> {
        self.servers.iter().find(|server| server.id.is_some() && server.id.unwrap() == server_id)
    }
    ...
    fn remove(&mut self, id: i32) {
        // Remove any callbacks
        if let Some(c) = self.get_by_server_id(id) {
            let cbid = c.cbid;
            self.remove_callback(cbid);
        }
...
impl BtifGattServerCallbacks for BluetoothGatt {
    fn register_server_cb(&mut self, status: GattStatus, server_id: i32, app_uuid: Uuid) {
        ...
        let server = self.server_context_map.get_by_uuid(&app_uuid.uu);
        match server {
            Some(s) => {
                let cbid = s.cbid;
            ...
            None => {
                warn!("Warning: Server not registered for UUID {}", app_uuid);
            }
        }
    }
}
