...
pub trait IBluetoothGatt {
    ...
    fn register_scanner(&mut self, callback_id: u32);
...
pub struct BluetoothGatt {
    ...
    // The number of scanners is practically low, so keep it simple by using Vec instead of HashMap
    // for simplicity.
...
impl BluetoothGatt {
    ...
    pub fn remove_scanner_callback(&mut self, callback_id: u32) -> bool {
        // Could be written in a more Rust idiomatic way once Vec::drain_filter stabilizes.
        let mut i = 0;
        while i < self.scanners.len() {
            if self.scanners[i].callback_id == callback_id {
                let scanner = self.scanners.remove(i);
                if let Some(scanner_id) = scanner.scanner_id {
                    ...
                    log::warn!("Cannot unregister a scanner without id");
                }
            } else {
                i += 1;
            }
...
impl IBluetoothGatt for BluetoothGatt {
    ...
    fn register_scanner(&mut self, callback_id: u32) {
        ...
        self.scanners.push(ScannerInfo { uuid, callback_id, scanner_id: None });
...
impl BtifGattScannerCallbacks for BluetoothGatt {
    fn on_scanner_registered(&mut self, uuid: Uuid, scanner_id: u8, status: u8) {
        ...
        if status != 0 {
            ...
            self.scanners.retain(|scanner| scanner.uuid != uuid);
            return;
        }

        let scanner_info = self.scanners.iter_mut().find(|scanner| scanner.uuid == uuid);
