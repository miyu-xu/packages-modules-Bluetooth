...
pub struct BluetoothGatt {
    ...
    // LibBluetooth doesn't allow multiple GATT write characteristic to the same remote device at
    // the same time. |write_characteristic_permits| records the lower layer status and
    // |write_characteristic_pending| stores the pending requests from the clients.
