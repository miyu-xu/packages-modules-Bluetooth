...
impl ServerContextMap {
    ...
    fn get_by_conn_id(&self, conn_id: i32) -> Option<&Server> {
        match self.connections.iter().find(|conn| conn.conn_id == conn_id) {
            Some(conn) => self.get_by_server_id(conn.server_id),
            None => None,
        }
    }
    ...
    fn add_service(&mut self, server_id: i32, service: BluetoothGattService) {
        self.get_mut_by_server_id(server_id).map(|s: &mut Server| s.services.push(service));
    }
    ...
    fn get_request_handle_from_id(&self, request_id: i32) -> Option<i32> {
        self.requests.iter().find(|request| request.id == request_id).map(|request| request.handle)
    }
}
...
impl IBluetoothGatt for BluetoothGatt {
    ...
    fn clear_services(&self, server_id: i32) {
        match self.server_context_map.get_by_server_id(server_id) {
            Some(s) => {
    ...
    fn send_response(
        ...
        let handle = match self.server_context_map.get_request_handle_from_id(request_id) {
            None => return false,
            Some(handle) => handle,
        };

        let len = value.len() as u16;
        let data: [u8; 600] = match value.try_into() {
            Ok(v) => v,
            Err(_v) => return false,
        };
...
impl BtifGattServerCallbacks for BluetoothGatt {
    ...
    fn request_read_characteristic_cb(
        ...
        let cbid = self.server_context_map.get_by_conn_id(conn_id).map(|server| server.cbid);
        match cbid {
            Some(cbid) => {
                if let Some(cb) = self.server_context_map.get_callback_from_callback_id(cbid) {
