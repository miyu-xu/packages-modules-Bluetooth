...
/// The UUID corresponding to the BatteryLevel characteristic defined by the
/// BatteryService specification.
pub const CHARACTERISTIC_BATTERY_LEVEL: &str = "00002A1-9000-0100-0800-000805F9B34FB";

/// The UUID that identifies BAS-provided battery info.
pub const BAS_BATTERY_PROVIDER_UUID: &str = "51cbb9cf-35d4-4925-9053-96b9a08057ce";
...
/// thread. Newly supported callbacks should add a corresponding entry here.
...
/// API for Floss implementation of the Bluetooth Battery Service (BAS). BAS is
/// built on GATT and this implementation wraps all of the GATT calls and
/// handles tracking battery information for the client.
pub trait IBatteryService {
    ...
    /// Returns the battery info of the remote device if available in
    /// BatteryService's cache.
    fn get_battery_info(&self, remote_address: String) -> Option<BatterySet>;

    /// Forces an explicit read of the device's battery level, including
    /// initiating battery level tracking if not yet performed.
...
pub trait IBatteryServiceCallback: RPCProxy {
    /// Called when the status of BatteryService has changed. Trying to read
    /// from devices that do not support BAS will result in this method being
    /// called with BatteryServiceNotSupported.
...
impl BatteryService {
    ...
    pub fn handle_callback(&mut self, callback: GattBatteryCallbacks) {
        match callback {
            ...
            GattBatteryCallbacks::OnCharacteristicRead(addr, status, _handle, value) => {
                ...
                let battery_info = self.set_battery_info(addr.clone(), value.clone());
                self.callbacks.for_all_callbacks(|callback| {
            ...
            GattBatteryCallbacks::OnNotify(addr, _handle, value) => {
                let battery_info = self.set_battery_info(addr.clone(), value);
                self.callbacks.for_all_callbacks(|callback| {
                    callback.on_battery_info_updated(addr.clone(), battery_info.clone());
                });
            }
    ...
    /// Attempt to (re)establish connection to a device when it becomes
    /// connected.
    ...
    fn set_battery_info(&mut self, remote_address: String, value: Vec<u8>) -> BatterySet {
        ...
        let battery_set = self.battery_sets.entry(remote_address.clone()).or_insert_with(|| {
            BatterySet::new(
                remote_address,
                BAS_BATTERY_PROVIDER_UUID.to_string(),
                "BAS".to_string(),
            )
        });
