...
fn main() {
    ...
    let proto_root =
        PathBuf::from(env::current_dir().unwrap()).join("proto").canonicalize().unwrap();

    let out_dir = PathBuf::from(env::var("OUT_DIR").unwrap());
    let proto_out_dir = out_dir.join("proto_out");
    let proto_input_files = [proto_root.join("suspend.proto")];
    let proto_include_dirs = [proto_root.clone()];
    ...
    println!("proto out dir = {:?}", proto_out_dir);
