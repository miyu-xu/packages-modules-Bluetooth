    ...
    async fn monitor_name_owner_changed(
        ...
        self.conn.start_receive(
            ...
            Box::new(move |msg, _conn| {
                // We use `set_signal_match_mode(true)`, so we should always return true from this
                // closure to indicate that we have handled the message and not let other handlers
                // handle this signal.
