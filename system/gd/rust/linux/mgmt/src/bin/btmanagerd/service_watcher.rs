...
const DBUS_OM_INTERFACE: &str = "org.freedesktop.DBus.ObjectManager";
...
impl ServiceWatcher {
    ...
    async fn monitor_name_owner_changed(
        ...
        self.conn.start_receive(
            ...
            Box::new(move |msg, _conn| {
                ...
                if name.is_none() || old_owner.is_none() || new_owner.is_none() {
                    // Invalid NameOwnerChanged signal, ignore.
                    return true;
                }

                let name = name.unwrap();
                let old_owner = old_owner.unwrap();
    ...
    pub async fn start_watch_interface(
        ...
        let mr = MatchRule::new_signal(DBUS_OM_INTERFACE, DBUS_INTERFACES_ADDED);
        self.conn.add_match_no_cb(&mr.match_str()).await.unwrap();
        self.conn.start_receive(
            mr,
            Box::new(move |msg, _conn| {
