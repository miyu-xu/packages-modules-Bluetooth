    ...
    fn apply_action(&mut self, value: String) -> Result<String, DeviceKeyError> {
        ...
        fn apply_conversion(conv: &Converter, value: String) -> Result<String, DeviceKeyError> {
            match conv {
                ...
                Converter::ReplaceWithSpace => Ok(value.replace(";", " ")),
                Converter::ReplaceWithSemiColon => Ok(value.replace(" ", ";")),
            }
        }
        ...
        let mut map: HashMap<&str, DeviceKey> = if is_hid_file {
            ...
            match sec.as_str() {
                GENERAL_SECTION_NAME => [
                    ...
                    (
                        "Services",
                        DeviceKey::new("Service", KeyAction::Apply(Converter::ReplaceWithSpace)),
                    ),
                    (
                        "AddressType",
...
fn convert_floss_conf(filename: &str) {
    ...
    let mut info_map: HashMap<&str, DeviceKey> = [
        ...
        (
            ...
            DeviceKey::new(
                "Services",
                KeyAction::ApplyToSection(Converter::ReplaceWithSemiColon, GENERAL_SECTION_NAME),
            ),
        ),
        (
            "AddrType",
    ...
    for (sec, props) in floss_map {
        ...
        for (k, v) in props {
            ...
            if k == "LE_KEY_PENC" {
                ...
                bluez_info.set(LTK_SECTION_NAME, "Rand", Some(format!("{}", rand)));
                bluez_info.set(LTK_SECTION_NAME, "EDiv", Some(format!("{}", ediv)));
                bluez_info.set(LTK_SECTION_NAME, "Authenticated", Some(format!("{}", auth)));
                bluez_info.set(LTK_SECTION_NAME, "EncSize", Some(format!("{}", len)));
                continue;
            }
    ...
    let globbed = match glob(format!("{}/{}/*:*", BT_LIBDIR, adapter_addr).as_str()) {
        Ok(v) => v,
        _ => {
            return;
        }
