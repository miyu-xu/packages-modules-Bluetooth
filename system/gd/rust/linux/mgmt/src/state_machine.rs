...
/// Device path of hci device in sysfs. This will uniquely identify an Bluetooth
...
fn get_hci_index_from_pid_path(path: &str) -> Option<RealHciIndex> {
    ...
    match re.captures(path)?.get(1)?.as_str().parse().ok() {
        Some(v) => Some(RealHciIndex(v)),
        None => None,
    }
}
        ...
        match m.unwrap() {
            ...
            Message::AdapterStateChange(action) => {
                ...
                match &action {
                    ...
                    AdapterStateActions::HciDevicePresence(devpath, i, presence) => {
                        ...
                        // If the real hci changed, we need to set the previous presence to the
                        // opposite of the current presence so that we don't no-op the action.
                        ...
                        (next_state, adapter_change_action) =
                            context.state_machine.action_on_hci_presence_changed(hci, *presence);
                        ...
                        bluetooth_manager
                            .lock()
                            .unwrap()
                            .callback_hci_device_change(hci.to_i32(), *presence)
                    }
                };
...
mod tests {
    ...
    fn test_updated_virtual_id() {
        ...
        // When devpath and real hci agree, expect a stable virtual index.
        assert_eq!(
            state_machine.get_updated_virtual_id("/fake/bt0".into(), RealHciIndex(12)),
            VirtualHciIndex(0)
        );
