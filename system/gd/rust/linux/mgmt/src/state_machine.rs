...
pub enum ProcessState {
    ...
    Restarting = 4, // We are not notified that the Bluetooth is stopped
}
...
impl StateMachineInternal {
    ...
    pub fn action_restart_bluetooth(&mut self, hci: VirtualHciIndex) -> CommandTimeoutAction {
        ...
        match state {
            ...
            ProcessState::TurningOn => {
                self.modify_state(hci, |s: &mut AdapterState| s.state = ProcessState::On);
                CommandTimeoutAction::CancelTimer
            }
            _ => CommandTimeoutAction::DoNothing,
        }
    ...
    /// Handles a bluetooth started event. Always return the acction to cancel timer even with unknown
    /// interfaces.
    ...
    /// If unexpected, Bluetooth probably crashed, returns an action to reset the timer to restart timeout.
