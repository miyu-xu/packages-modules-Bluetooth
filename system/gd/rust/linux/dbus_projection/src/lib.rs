...
#[macro_export]
macro_rules! gen_dbus_exporter_from_builders {
    ( $dbus_iface_name:expr, $fn_ident:ident, $mixin_type:ty, $($builder:expr),* ) => {
            pub fn $fn_ident(
                ...
                cr.register($dbus_iface_name, |ibuilder| {
                    $(
                        $builder(conn.clone(), disconnect_watcher.clone(), ibuilder);
                    )*
                })
            }
    };
