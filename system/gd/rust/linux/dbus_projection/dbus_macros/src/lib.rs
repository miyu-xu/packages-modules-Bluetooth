...
pub fn generate_dbus_arg(_item: TokenStream) -> TokenStream {
    let gen = quote! {
        ...
        impl<K: 'static + Eq + Hash + RefArgToRust<RustType = K>, V: 'static + RefArgToRust<RustType = V>>
            RefArgToRust for std::collections::HashMap<K, V>
        {
            ...
            fn ref_arg_to_rust(
                ...
                let mut iter = arg.as_iter().unwrap().fuse();
                let mut key = iter.next();
                let mut val = iter.next();
                while !key.is_none() && !val.is_none() {
                    let k = key.unwrap().box_clone();
                    let k = <K as RefArgToRust>::ref_arg_to_rust(&k, _name.clone() + " key")?;
                    let v = val.unwrap().box_clone();
                    let v = <V as RefArgToRust>::ref_arg_to_rust(&v, _name.clone() + " value")?;
                    map.insert(k, v);
                    key = iter.next();
                    val = iter.next();
                }
        ...
        impl<K: Eq + Hash + DBusArg, V: DBusArg> DBusArg for std::collections::HashMap<K, V>
            where <K as DBusArg>::DBusType: 'static + Eq + Hash + dbus::arg::RefArg + RefArgToRust<RustType = <K as DBusArg>::DBusType>
        {
            ...
            ) -> Result<std::collections::HashMap<K::DBusType, V::DBusType>, Box<dyn std::error::Error>>
            {
                let mut map = std::collections::HashMap::new();
                for (key, val) in data {
                    let key = K::to_dbus(key)?;
                    let val = V::to_dbus(val)?;
                    map.insert(key, val);
                }
                Ok(map)
            }
