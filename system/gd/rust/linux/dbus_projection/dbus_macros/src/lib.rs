...
/// in a trait to an IfaceBuilder object, which can be used later with other registerers to
/// generate exporter.
///
/// # Args
///
...
pub fn generate_dbus_method_registerer(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    // type to be Arc<Mutex<Box<T>>>. Otherwise, we accept any type T and depend
    // on the field name lookup to throw an error.
    let obj_type = quote! { Box<#mixin_type> };

    let register_methods = _generate_register_methods(Some(mixin_type), Some(mixin_name), item);

    let gen = quote! {
        pub fn #fn_ident (conn: std::sync::Arc<dbus::nonblock::SyncConnection>,
            disconnect_watcher: std::sync::Arc<std::sync::Mutex<dbus_projection::DisconnectWatcher>>, ibuilder:&mut IfaceBuilder<#obj_type>) {
                #register_methods
        }
    };
