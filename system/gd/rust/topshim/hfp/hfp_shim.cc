...
//namespace headset = ::bluetooth::headset;
...
class DBusHeadsetCallbacks : public headset::Callbacks {
 public:
  ...
  void ConnectionStateCallback(headset::bthf_connection_state_t state, RawAddress* bd_addr) override {
    LOG_WARN("ConnectionStateCallback addr:%s", bd_addr);
    topshim::rust::internal::connection_state_cb(state, bd_addr);
  }

  void AudioStateCallback([[maybe_unused]] headset::bthf_audio_state_t state, [[maybe_unused]] RawAddress* bd_addr) override {}

  void VoiceRecognitionCallback([[maybe_unused]] headset::bthf_vr_state_t state, [[maybe_unused]] RawAddress* bd_addr) override {
  }
  ...
  void NoiseReductionCallback([[maybe_unused]] headset::bthf_nrec_t nrec, [[maybe_unused]] RawAddress* bd_addr) override {}
  ...
  void AtChldCallback(headset::bthf_chld_type_t chld, RawAddress* bd_addr) override {
    LOG_WARN("AtChldCallback addr:%s", bd_addr);
  }

  void AtCnumCallback([[maybe_unused]] RawAddress* bd_addr) override {}

  void AtCindCallback(RawAddress* bd_addr) override {
    LOG_WARN("AtCindCallback addr:%s", bd_addr);
    headset_->CindResponse(0,0,0,headset::BTHF_CALL_STATE_ACTIVE, 0,0,0, bd_addr);
  }
  ...
  void AtBindCallback(char* at_string, RawAddress* bd_addr) override {
    LOG_WARN("AtBindCallback addr:%s", bd_addr);

  }

  void AtBievCallback(
      [[maybe_unused]] headset::bthf_hf_ind_type_t ind_id,
