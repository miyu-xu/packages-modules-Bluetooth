...
class DBusHeadsetCallbacks : public headset::Callbacks {
 public:
  ...
  void WbsCallback(headset::bthf_wbs_config_t wbs, RawAddress* bd_addr) override {
    LOG_INFO("WbsCallback %d from %s", wbs, ADDRESS_TO_LOGGABLE_CSTR(*bd_addr));
  ...
  void AtCindCallback(RawAddress* bd_addr) override {
    ...
    LOG_WARN("Respond +CIND: 0,0,1,5,0,5,0 to AT+CIND? from %s", ADDRESS_TO_LOGGABLE_CSTR(*bd_addr));
  ...
  void AtCopsCallback(RawAddress* bd_addr) override {
    LOG_WARN("Respond +COPS: 0 to AT+COPS? from %s", ADDRESS_TO_LOGGABLE_CSTR(*bd_addr));
  ...
  void AtBiaCallback(bool service, bool roam, bool signal, bool battery, RawAddress* bd_addr) override {
    LOG_WARN("AT+BIA=,,%d,%d,%d,%d,from addr %s", service, signal, roam, battery, ADDRESS_TO_LOGGABLE_CSTR(*bd_addr));
  }
