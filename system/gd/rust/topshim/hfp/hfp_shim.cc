...
bool HfpIntf::get_wbs_supported(RustRawAddress bt_addr) {
  RawAddress addr = rusty::CopyFromRustAddress(bt_addr);
  /* If addr is not in wbs_supported, it could be a device that does not support
   * codec-negotiation, in which case we can give the default value: false
   */
  return internal::wbs_supported[addr.ToString()];
}
