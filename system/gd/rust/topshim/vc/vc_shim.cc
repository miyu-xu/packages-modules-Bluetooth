...
class DBusVolumeControlCallbacks : public vc::VolumeControlCallbacks {
 public:
  ...
  void OnVolumeStateChanged(
    ...
    LOG_INFO(
        "%s from %s, volume=%u, mute=%u, is_autonomous=%u",
        __func__,
  ...
  void OnGroupVolumeStateChanged(
    ...
    LOG_INFO(
        "%s from %d, volume=%u, mute=%u, is_autonomous=%u",
        __func__,
        group_id,
        volume,
        mute,
  ...
  void OnDeviceAvailable(const RawAddress& address, uint8_t num_offset) override {
    LOG_INFO("%s from %s, num_offset=%u", __func__, ADDRESS_TO_LOGGABLE_CSTR(address), num_offset);
  ...
  void OnExtAudioOutVolumeOffsetChanged(
    ...
    LOG_INFO(
        "%s from %s, ext_output_id=%u, offset=%d",
        __func__,
        ADDRESS_TO_LOGGABLE_CSTR(address),
        ext_output_id,
        offset);
  ...
  void OnExtAudioOutLocationChanged(
    ...
    LOG_INFO(
        "%s from %s, ext_output_id=%u, location=%u",
        __func__,
        ADDRESS_TO_LOGGABLE_CSTR(address),
        ext_output_id,
        location);
  ...
  void OnExtAudioOutDescriptionChanged(
    ...
    LOG_INFO(
        "%s from %s, ext_output_id=%u, descr=%s",
        __func__,
        ADDRESS_TO_LOGGABLE_CSTR(address),
        ext_output_id,
        descr.c_str());
...
// void VolumeControlIntf::set_volume(RawAddress addr, uint8_t volume) {
//   return intf_->SetVolume(addr, volume);
// }
...
// std::string descr) {
//   return intf_->SetExtAudioOutDescription(addr, ext_output_id, descr);
// }
}  // namespace rust
}  // namespace topshim
}  // namespace bluetooth
