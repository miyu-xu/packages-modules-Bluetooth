...
class AvrcpMediaInterfaceImpl : public MediaInterface {
  ...
  void SetPlaybackStatus(const PlayState& state) {
    ...
    if (mediaCb_) {
      mediaCb_->SendMediaUpdate(/*track_changed*/ false,
                                /*play_state*/ true,
                                /*queuefalse*/ false);
    }
  }
  ...
  void SetMetadata(const std::string& title, const std::string& artist, const std::string& album,
    ...
    if (title.length() || artist.length() || album.length()) {
      ...
      // Floss's media implementation does not fully support the "Now Playing List," as Floss
      // does not receive additional media information other than the current playing one.
      // However, not all peripherals request metadata through the "Get Element Attributes"
      // request. Instead, many get such information through the "Track Changed Notification."
      // Hence, fill the playlist with one item here and have the Current Song ID always point
      // to the only entry to support the track changed notification.
      nowPlayingList_.clear();
      nowPlayingList_.emplace_back(songInfo_);
      currentSongId_ = songInfo_.media_id;
      if (mediaCb_) {
        mediaCb_->SendMediaUpdate(/*track_changed*/ true,
                                  /*play_state*/ false,
                                  /*queuefalse*/ false);
      }
    }
...
uint32_t A2dpIntf::connect(RawAddress addr) const { return intf_->connect(addr); }
uint32_t A2dpIntf::disconnect(RawAddress addr) const { return intf_->disconnect(addr); }
int A2dpIntf::set_silence_device(RawAddress addr, bool silent) const {
  return intf_->set_silence_device(addr, silent);
}
int A2dpIntf::set_active_device(RawAddress addr) const { return intf_->set_active_device(addr); }
...
void A2dpIntf::cleanup() const { intf_->cleanup(); }
