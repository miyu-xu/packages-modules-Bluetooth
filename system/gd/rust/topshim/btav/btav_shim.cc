...
class AvrcpMediaInterfaceImpl : public MediaInterface {
 public:
  ...
  void SetMetadata(const std::string& title, const std::string& artist, const std::string& album, int64_t length_us) {
    ...
    if (title.length()) song_info.attributes.emplace(avrcp::AttributeEntry(avrcp::Attribute::TITLE, title));
    if (artist.length()) song_info.attributes.emplace(avrcp::AttributeEntry(avrcp::Attribute::ARTIST_NAME, artist));
    if (album.length()) song_info.attributes.emplace(avrcp::AttributeEntry(avrcp::Attribute::ALBUM_NAME, album));
