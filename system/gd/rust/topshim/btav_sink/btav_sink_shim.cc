...
std::unique_ptr<A2dpSinkIntf> GetA2dpSinkProfile(const unsigned char* btif) {
  ...
  auto a2dp_sink = std::make_unique<A2dpSinkIntf>(reinterpret_cast<const btav_sink_interface_t*>(
          btif_->get_profile_interface("a2dp_sink")));
  internal::g_a2dp_sink_if = a2dp_sink.get();
  return a2dp_sink;
}

int A2dpSinkIntf::init() const { return intf_->init(&internal::g_a2dp_sink_callbacks, 1); }

int A2dpSinkIntf::connect(RawAddress addr) const { return intf_->connect(addr); }

int A2dpSinkIntf::disconnect(RawAddress addr) const { return intf_->disconnect(addr); }
