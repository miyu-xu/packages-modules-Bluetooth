...
impl From<BtSdpRecord> for BtSdpType {
    fn from(record: BtSdpRecord) -> Self {
        match record {
            BtSdpRecord::HeaderOverlay(header) => header.hdr.sdp_type,
            BtSdpRecord::MapMas(record) => record.hdr.hdr.sdp_type,
            BtSdpRecord::MapMns(record) => record.hdr.hdr.sdp_type,
            BtSdpRecord::PbapPse(record) => record.hdr.hdr.sdp_type,
            BtSdpRecord::PbapPce(record) => record.hdr.hdr.sdp_type,
            BtSdpRecord::OppServer(record) => record.hdr.hdr.sdp_type,
            BtSdpRecord::SapServer(record) => record.hdr.hdr.sdp_type,
            BtSdpRecord::Dip(record) => record.hdr.hdr.sdp_type,
        }
    }
}
