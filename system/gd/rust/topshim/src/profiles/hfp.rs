...
pub mod ffi {
    ...
    unsafe extern "C++" {
        ...
        unsafe fn interop_disregard_hfp_profile(name: *const c_char) -> bool;
...
pub fn interop_disregard_hfp_profile(name: String) -> bool {
    let c_name = std::ffi::CString::new(name).unwrap();
    // Call an unsafe function in c++. This is necessary for bridge C++ interop API with floss(rust).
    unsafe { return ffi::interop_disregard_hfp_profile(c_name.as_ptr()) }
}
