...
impl BluetoothProperty {
    pub fn get_type(&self) -> BtPropertyType {
        match &*self {
            ...
            BluetoothProperty::RemoteIsCoordinatedSetMember(_) => BtPropertyType::RemoteIsCoordinatedSetMember,
    ...
    fn get_data_ptr<'a>(&'a self, data: &'a mut [u8]) -> *mut u8 {
        ...
        match &*self {
            ...
            BluetoothProperty::VendorProductInfo(vpi) => {
                ...
                let slice = unsafe {
                    std::slice::from_raw_parts(ptr as *mut u8, mem::size_of::<BtVendorProductInfo>())
                };
                data.copy_from_slice(&slice);
            },
