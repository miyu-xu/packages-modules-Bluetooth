...
use bt_topshim::profiles::hf_client::{
    BthfClientCallbacks, BthfClientCallbacksDispatcher, HfClient,
};
use bt_topshim_facade_protobuf::facade::{
    ConnectAudioRequest, ConnectAudioResponse, DisconnectAudioRequest, DisconnectAudioResponse,
    EventType, FetchEventsRequest, FetchEventsResponse, StartSlcRequest, StartSlcResponse,
    StopSlcRequest, StopSlcResponse,
};
...
impl HfClientService for HfClientServiceImpl {
    fn start_slc(
        ...
        ctx.spawn(async move {
            ...
            if let Some(mut addr) = RawAddress::from_string(bt_addr) {
                let status = hf_client.lock().unwrap().connect(&mut addr);
