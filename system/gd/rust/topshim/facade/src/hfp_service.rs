...
impl HfpService for HfpServiceImpl {
    fn start_slc(&mut self, ctx: RpcContext<'_>, req: StartSlcRequest, sink: UnarySink<Empty>) {
        ...
        ctx.spawn(async move {
            if let Some(addr) = RawAddress::from_bytes(req.connection.cookie.clone()) {
                ...
                sink.fail(RpcStatus::with_message(
                    ...
                    format!(
                        ...
                        from_utf8(&req.connection.cookie).unwrap()
                    ),
                ))
                .await
                .unwrap();
        ...
        ctx.spawn(async move {
            if let Some(addr) = RawAddress::from_bytes(req.connection.cookie.clone()) {
                ...
                sink.fail(RpcStatus::with_message(
                    ...
                    format!(
                        "Invalid Request Address: {}",
                        str::from_utf8(&req.connection.cookie).unwrap()
                    ),
                ))
                .await
                .unwrap();
    ...
    fn connect_audio(
        ...
        ctx.spawn(async move {
            if let Some(addr) = RawAddress::from_bytes(req.connection.cookie.clone()) {
                hfp.lock().unwrap().connect_audio(addr);
