...
use crate::utils::converters::{bluetooth_property_to_event_data, create_event_data_from_string};
            ...
            while let Some(event) = rx.lock().await.recv().await {
                match event {
                    BaseCallbacks::AdapterState(_state) => {
                        ...
                        rsp.params.insert(
                            String::from("state"),
                            create_event_data_from_string(String::from("ON")),
                        );
                    ...
                    BaseCallbacks::LeRandCallback(random) => {
                        ...
                        rsp.params.insert(
                            String::from("data"),
                            create_event_data_from_string(random.to_string()),
                        );
                        sink.send((rsp, WriteFlags::default())).await.unwrap();
                    }
                    BaseCallbacks::GenerateLocalOobData(transport, data) => {
                        ...
                        rsp.params.insert(
                            ...
                            create_event_data_from_string(String::from(if data.is_valid {
                                "1"
                            } else {
                                "0"
                            })),
                        ...
                        rsp.params.insert(
                            ...
                            create_event_data_from_string(format!("{}", transport)),
                        );
                        rsp.params.insert(
                            String::from("address"),
                            create_event_data_from_string(encode_hex(&data.address)),
                        );
                        rsp.params.insert(
                            String::from("confirmation"),
                            create_event_data_from_string(encode_hex(&data.c)),
                        );
                        rsp.params.insert(
                            String::from("randomizer"),
                            create_event_data_from_string(encode_hex(&data.r)),
                        );
                        sink.send((rsp, WriteFlags::default())).await.unwrap();
                    }
                    BaseCallbacks::AdapterProperties(status, _, properties) => {
                        ...
                        rsp.params.insert(
                            String::from("status"),
                            create_event_data_from_string(format!("{:?}", status)),
                        );
