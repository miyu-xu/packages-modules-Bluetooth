...
mod tests {
    ...
    async fn test_bt_keystore_interface() {
        ...
        let choice_ptr: cxx::UniquePtr<ffi::BluetoothKeystoreInterface> =
            ffi::new_bt_keystore_interface();
        let address = Arc::new(Mutex::new(format!("{:p}", &choice_ptr)));
        ...
        for i in 1..3 {
            ...
            tokio::spawn(async move {
                let mut a = addr.lock().await;
                if i == 1 {
                    let ptr: cxx::UniquePtr<ffi::BluetoothKeystoreInterface> =
                        ffi::new_bt_keystore_interface();
                    *a = format!("{:p}", &ptr);
                    param_provider_cur.lock().await.set_bt_keystore_interface(ptr).await;
                } else {
                    let ptr: cxx::UniquePtr<ffi::BluetoothKeystoreInterface> =
                        ffi::new_bt_keystore_interface();
                    *a = format!("{:p}", &ptr);
        ...
        let answer = *hex_to_dec(Arc::clone(&address).lock().await.to_string());
