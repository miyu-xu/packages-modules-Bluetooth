...
macro_rules! init_flags {
    (flags: { $($flag:ident $(: $type:ty)? $(= $default:tt)?,)* }
     ...
     extra_parsed_flags: { $($extra_flag:tt => $extra_flag_fn:ident($($extra_args:tt),*),)*}
     dependencies: { $($parent:ident => $child:ident),* }) => {
        ...
        /// Set alls other flags and extra_field to their default type value
...
/// Return true is `tag` is enabled in the flag
...
init_flags!(
    ...
    // extra_fields are not a 1 to 1 match with a INIT flag
    extra_fields: {
        logging_debug_explicit_tag_settings : ExplicitTagSettings,
    }
    extra_parsed_flags: {
        "INIT_logging_debug_enabled_for_tags" => parse_logging_tag(true),
        "INIT_logging_debug_disabled_for_tags" => parse_logging_tag(false),
        "--hci" => parse_hci_adapter(),
    }
    dependencies: {
        gd_core => gd_security
    }
...
mod tests {
    use super::*;
    lazy_static! {
        /// do not run concurrent test as they all use the same global init_flag struct and
        /// accessor
    ...
    fn explicit_flag() {
        ...
        assert!(is_debug_logging_enabled_for_tag("unkown_flag"));
