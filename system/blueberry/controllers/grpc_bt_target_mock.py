...
#pytype : disable = attribute - error
      response
= self.stub.SetDiscoverableMode(request)
      logging.info('set discoverageble response: %s', response) return 0 except grpc
          .RpcError as rpc_error : print(rpc_error) return -1

                                   def
                                   factory_reset_bluetooth(self) -> None
    : logging
          .info('factory reset TO BE IMPLEMENTED')

              def get_bluetooth_mac_address(self)
          ->str : logging.info('mac_address: %s', self.mac_address) return self.mac_address
