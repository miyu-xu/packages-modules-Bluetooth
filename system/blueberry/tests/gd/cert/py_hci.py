...
class PyHciAclConnection(IEventStream):
    ...
    def send_first(self, data: bytes):
        self.send(hci.PacketBoundaryFlag.FIRST_AUTOMATICALLY_FLUSHABLE,
                  hci.BroadcastFlag.POINT_TO_POINT, data)

    def send_continuing(self, data):
        self.send(hci.PacketBoundaryFlag.CONTINUING_FRAGMENT, hci.BroadcastFlag.POINT_TO_POINT,
                  data)
...
class PyHci(Closable):
    ...
    def __init__(self, device, acl_streaming=False):
        ...
        if acl_streaming:
            self.register_for_events(hci.EventCode.ROLE_CHANGE, hci.EventCode.CONNECTION_REQUEST,
                                     hci.EventCode.CONNECTION_COMPLETE,
                                     hci.EventCode.CONNECTION_PACKET_TYPE_CHANGED)
    ...
    def initiate_connection(self, remote_addr):
        self.send_command(
            hci.CreateConnection(
                bd_addr=bluetooth.Address(remote_addr),
                packet_type=0xcc18,
    ...
    def accept_connection(self):
        ...
        self.send_command(
            hci.AcceptConnectionRequest(
                bd_addr=connection_request.get().bd_addr,
    ...
    def set_random_le_address(self, addr):
        self.send_command(hci.LeSetRandomAddress(random_address=addr))
    ...
    def add_device_to_resolving_list(self, peer_address_type, peer_address, peer_irk, local_irk):
        self.send_command(
            hci.LeAddDeviceToResolvingList(
                peer_identity_address_type=peer_address_type,
    ...
    def create_advertisement(self,
        ...
        self.send_command(hci.LeSetAdvertisingSetRandomAddress(
            advertising_handle=handle,
