...
class HciMatchers(object):
    ...
    def _extract_matching_le_event(packet_bytes, subevent_code):
        ...
        if (not isinstance(event, hci.LeMetaEvent) or
            event.subevent_code != subevent_code):
            return None
    ...
    def _extract_le_connection_complete(packet_bytes):
        ...
        return HciMatchers._extract_matching_le_event(packet_bytes,
                                                      hci.SubeventCode.ENHANCED_CONNECTION_COMPLETE)
...
class NeighborMatchers(object):
    ...
    def _is_matching_inquiry_result(packet, address):
        hci_event = HciMatchers.ExtractEventWithCode(packet, hci.EventCode.INQUIRY_RESULT)
    ...
    def _is_matching_inquiry_result_with_rssi(packet, address):
        hci_event = HciMatchers.ExtractEventWithCode(packet, hci.EventCode.INQUIRY_RESULT_WITH_RSSI)
        if hci_event is None:
            return False
    ...
    def _is_matching_extended_inquiry_result(packet, address):
        hci_event = HciMatchers.ExtractEventWithCode(packet, hci.EventCode.EXTENDED_INQUIRY_RESULT)
        if hci_event is None:
            return False
