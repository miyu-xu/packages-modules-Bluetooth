...
class PyHalAdvertisement(object):
    ...
    def set_data(self, complete_name):
        ...
        if self.legacy:
            self.py_hal.send_hci_command(
                hci.LeSetAdvertisingData(advertising_data=advertising_data))
    ...
    def set_scan_response(self, shortened_name):
        ...
        if self.legacy:
            self.py_hal.send_hci_command(
                hci.LeSetScanResponseData(advertising_data=advertising_data))
    ...
    def stop(self):
        if self.legacy:
            self.py_hal.send_hci_command(
                hci.LeSetAdvertisingEnable(advertising_enable=hci.Enable.DISABLED))
            self.py_hal.wait_for_complete(hci.OpCode.LE_SET_ADVERTISING_ENABLE)
        else:
            self.py_hal.send_hci_command(
                hci.LeSetExtendedAdvertisingEnable(
...
class PyHal(Closable):
    ...
    def send_acl(self, handle, pb_flag, b_flag, data: bytes):
        acl = hci.Acl(handle=handle, packet_boundary_flag=pb_flag,
                      broadcast_flag=b_flag,
                      payload=data)
        self.device.hal.SendAcl(common.Data(payload=acl.serialize()))
    ...
    def set_random_le_address(self, addr):
        self.send_hci_command(hci.LeSetRandomAddress(random_address=addr))
    ...
    def create_advertisement(self,
        ...
        self.send_hci_command(hci.LeSetAdvertisingSetRandomAddress(
            advertising_handle=handle,
