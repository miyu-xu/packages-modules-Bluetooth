...
class PyHalAdvertisement(object):
    ...
    def set_scan_response(self, shortened_name):
        advertising_data = [hci.GapData(data_type=hci.GapDataType.SHORTENED_LOCAL_NAME, data=list(complete_name))]
...
class PyHal(Closable):
    ...
    def add_to_filter_accept_list(self, remote_addr):
        self.send_hci_command(
            hci.LeAddDeviceToFilterAcceptList(address_type=remote_addr, address=bluetooth.Address(remote_addr)))
