...
class LeAclManagerTest(gd_base_test.GdBaseTestClass):
    ...
    def setup_test(self):
        ...
        self.cert_hci_event_stream = EventStream(self.cert.hci.StreamEvents(empty_proto.Empty()))
    ...
    def dut_connects_cert_resolvable(self, check_address):
        ...
        dut_address = self.dut.hci_controller.GetMacAddressSimple().decode("utf-8")

        self.enqueue_hci_command(
            hci_packets.LeAddDeviceToResolvingListBuilder(
                hci_packets.PeerAddressType.PUBLIC_DEVICE_OR_IDENTITY_ADDRESS, dut_address,
        ...
        self.enqueue_hci_command(hci_packets.ReadBdAddrBuilder())

        # Cert gets ConnectionComplete with a handle and sends ACL data
        cert_address = hci_packets.Address()

        def get_address(packet):
            packet_bytes = packet.payload
            nonlocal cert_address
            if b'\x0e\x0a\x01\x09' in packet_bytes:
                bd_addr_view = hci_packets.ReadBdAddrCompleteView(
                    hci_packets.CommandCompleteView(
                        hci_packets.EventView(bt_packets.PacketViewLittleEndian(list(packet_bytes)))))
                cert_address = bd_addr_view.GetBdAddr()
                return True
            return False

        self.cert_hci_event_stream.assert_event_occurs(get_address)

        self.dut.hci_le_acl_manager.AddDeviceToResolvingList(
            le_acl_manager_facade.IrkMsg(
                peer=common.BluetoothAddressWithType(
                    address=common.BluetoothAddress(address=bytes(cert_address, "utf-8")),
