...
class GattConnectTest(Sl4aSl4aBaseTestClass):
    ...
    """
        ...
        gatt_callback = self.central.sl4a.gattCreateGattCallback()
        logging.info("Gatt Connect to mac address {}.".format(mac_address_pre_restart))
        bluetooth_gatt = self.central.sl4a.gattClientConnectGatt(gatt_callback, mac_address_pre_restart, autoconnect,
                                                                 transport, opportunistic, GattPhyMask.PHY_LE_1M_MASK)
        self.bluetooth_gatt_list.append(bluetooth_gatt)
        expected_event = GattCallbackString.GATT_CONN_CHANGE.format(gatt_callback)
        try:
            event = self.central.ed.pop_event(expected_event, self.default_timeout)
            logging.error("Connection callback updated unexpectedly: {}".format(event))
            return False
        except Empty:
            logging.info("No connection update as expected.")

        # Step 5: Cancel connection request.
        self.central.sl4a.gattClientDisconnect(bluetooth_gatt)
        close_gatt_client(self.central, bluetooth_gatt)
        if bluetooth_gatt in self.bluetooth_gatt_list:
            self.bluetooth_gatt_list.remove(bluetooth_gatt)
        ...
        gatt_callback = self.central.sl4a.gattCreateGattCallback()
        logging.info("Gatt Connect to mac address {}.".format(mac_address_post_restart))
        bluetooth_gatt = self.central.sl4a.gattClientConnectGatt(gatt_callback, mac_address_post_restart, autoconnect,
                                                                 transport, opportunistic, GattPhyMask.PHY_LE_1M_MASK)
        self.bluetooth_gatt_list.append(bluetooth_gatt)
        expected_event = GattCallbackString.GATT_CONN_CHANGE.format(gatt_callback)
        ...
        try:
            event = self.central.ed.pop_event(expected_event, self.default_timeout)
            logging.info("Connection callback updated successfully: {}".format(event))
            if event['data']['State'] != GattConnectionState.STATE_CONNECTED:
                logging.error("Could not establish a connection to the peripheral.")
                return False
        except Empty:
            logging.error("No connection update was found.")
            return False
