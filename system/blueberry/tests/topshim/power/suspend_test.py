...
class SuspendTest(TopshimBaseTest):
    ...
    def __verify_no_wake_suspend(self):
        ...
        self.dut().set_suspend_le_connection_parameters()
        self.dut().le_rand()
    ...
    def __verify_wakeful_suspend(self, is_a2dp_connected):
        ...
        self.dut().stop_scanning()
        self.dut().disconnect_all_acls()
        #constexpr uint16_t kScanIntervalSuspend = 0x0400; /* 640 ms = 1024 *0.625 */
        #constexpr uint16_t kScanWindowSuspend = 0x0012;   /* 11.25 ms = 18 *0.625 */
        interval = 0x0400
        window = 0x0012
        orig_interval, orig_window = self.dut().get_le_connection_parameters()
        print("herp", orig_interval, orig_window)
        self.dut().set_le_connection_parameters(interval, window)
        print("derp", interval, window)
        interval, window = self.dut().get_le_connection_parameters()
        print("zerp", interval, window)
        self.dut().allow_wake_by_hid()
