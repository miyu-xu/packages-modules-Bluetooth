...
class ClassicSecurityTest(TopshimBaseTest):
    ...
    def test_create_classic_bond(self):
        ...
        state, _ = self.dut().create_bond(address=address, transport=TRANSPORT_CLASSIC)
        assertThat(state).isEqualTo("Bonding")
        state, conn_addr = self.dut().wait_for_bond_state_change()
