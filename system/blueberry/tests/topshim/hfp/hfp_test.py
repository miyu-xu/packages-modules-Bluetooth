...
class HfpTest(TopshimBaseTest):
    ...
    def test_hfp_connect_with_bond(self):
        ...
        self.dut().create_bond(address=address, transport=TRANSPORT_CLASSIC)
        self.dut().wait_for_bond_state_change()

        # Actual test scenario of HFP connection.
        state, _ = self.dut().start_slc(address=address)
        ...
        # Test teardown and dut and cert reset.
        self.dut().remove_bonded_device(address)
        self.dut().wait_for_hfp_connection_state_change()
        self.cert().remove_bonded_device(address)
    ...
    def test_hfp_disconnect_with_bond(self):
        ...
        self.dut().start_slc(address=address)
        ...
        state, _ = self.dut().stop_slc(address)
