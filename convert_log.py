...
ostream_assert_matcher = re.compile(r"""
(?<=[ ])                    # positive look-behind for space character
(LOG_ASSERT)\(
(                           # start of group 2
(.|\n)*?
...
printf_assert_matcher = re.compile(r"""
(?<=[ ])                    # positive look-behind for space character
ASSERT_LOG\(
(?P<cond>                   # start of cond
(.|\n)*?
)                           # end of cond
,
(?P<format>                 # start of format
...
def sanitize_format_string(fmt: str) -> str:
...
def sanitize_format_arg(arg: str) -> str:
    # Remove '.c_str()' suffix.
...
def ostream_converter(match: re.Match) -> str:
    ...
    for part in parts:
        ...
        if part[0] == '"' and part[-1] == '"':
            ...
            (fmt, args) = convert_format(match.group('format'), match.group('args'))
            format_string += fmt
            format_args.extend(sanitize_format_arg(arg) for arg in args)
    ...
    format_string = sanitize_format_string(format_string)
    format_args = [sanitize_format_arg(arg) for arg in format_args]
    format_args = ''.join(", " + arg for arg in format_args)
    return f"{ostream_levels[level]}(\"{format_string}\"{format_args})"


def ostream_assert_converter(match: re.Match) -> str:
    parts = re.split(r"\s<<\s", match.group(2))
    cond = parts[0].strip()[:-1] # cond ends with a closing parenthesis
    parts = parts[1:]
    parts = [part.strip() for part in parts if part.strip()]
    ...
    if not parts:
        cond_str = re.sub(r"\s+", " ", cond)
        return f"log::assert_that({cond}, \"assert failed: {cond_str}\")"
    ...
    format_args = [sanitize_format_arg(arg) for arg in format_args]
    format_args = ''.join(", " + arg for arg in format_args)
    return f"log::assert_that({cond}, \"{format_string}\"{format_args})"
...
def printf_assert_converter(match: re.Match) -> str:
    ...
    if cond == 'false':
        return f"log::fatal(\"{format_string}\"{format_args})"

    return f"log::assert_that({cond}, \"{format_string}\"{format_args})"


def assert_converter(match: re.Match) -> str:
    cond = match.group('cond')
    cond_str = re.sub(r"\s+", " ", cond)
    return f"log::assert_that({cond}, \"assert failed: {cond_str}\")"
...
def run(files: List[Path]):
    ...
    for file in files:
        ...
        replacements.extend(convert(file, source, printf_assert_matcher, printf_assert_converter))
        replacements.extend(convert(file, source, ostream_assert_matcher, ostream_assert_converter))
        replacements.extend(convert(file, source, assert_matcher, assert_converter))
