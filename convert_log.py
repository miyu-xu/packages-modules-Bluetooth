...
def strip_c_str(arg: str) -> str:
    # Remove '.c_str()' from format arguments.
    return re.sub(r'\.c_str\(\)$', '', arg)
...
def strip_func_formatter(fmt: str) -> str:
    ...
    if re.search('(?<!^)XXFUNCXX', fmt):
        print("__func__ found in the middle of the format string", file=sys.stderr)

    return re.sub(r'XXFUNCXX(\(\))? ?[:,]? ?', '', fmt)

def sanitize(fmt: str) -> str:
...
def convert_format(fmt: str, args: str) -> (str, List[str]):
    ...
    for specifier in format_matcher.finditer(fmt):
        ...
        if specifier.group(0) == '%%':
            ...
            if (arg == '__func__' or
                ...
                format_string += "{}"
                format_args.append(strip_c_str(arg))
            ...
            format_args.append(strip_c_str(next(args)))
    ...
    format_string = sanitize(format_string)
