...
def vlog_converter(match: re.Match) -> str:
    ...
    return convert_ostream(vlog_levels[level], match.group(3))


def alog_converter(match: re.Match) -> str:
    level = match.group(1)
    if level not in alog_levels:
        raise Exception(f"unknown ALOG level {level} ({match.start()} - {match.end()})")

    return convert_ostream(alog_levels[level], match.group(2))
...
def convert(path: Path, source: str, matcher: re.Pattern, converter) -> List[Replacement]:
    ...
    while True:
        ...
        try:
            ...
            print(f"Exception raised convert log at offset {match.start()} in file {path}")
            raise exn
...
def run(files: List[Path]):
    ...
    for file in files:
        ...
        replacements.extend(convert(file, source, alog_matcher, alog_converter))
