...
class Dck:
    ...
    def Register(self, wait_for_ready: Optional[bool] = None, timeout: Optional[float] = None) -> Awaitable[empty_pb2.Empty]:
        return self.channel.unary_unary(  # type: ignore
            '/pandora.Dck/Register',
...
class DckServicer:
    async def Register(self, request: empty_pb2.Empty, context: grpc.ServicerContext) -> empty_pb2.Empty:
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)  # type: ignore
        context.set_details("Method not implemented!")  # type: ignore
        raise NotImplementedError("Method not implemented!")


def add_DckServicer_to_server(servicer: DckServicer, server: grpc.aio.Server) -> None:
    rpc_method_handlers = {
    ...
    generic_handler = grpc.method_handlers_generic_handler(  # type: ignore
        'pandora.Dck', rpc_method_handlers)
