...
class OOBService(OOBServicer):
    ...
    def __init__(self, device: Device) -> None:
        ...
        self.log = utils.BumbleServerLoggerAdapter(
            logging.getLogger(), {'service_name': 'oob', 'device': device}
        )
        self.device = device
    ...
    @utils.rpc
    async def ShareOobData(
        self, request: OobDataRequest, context: grpc.ServicerContext
    ) -> OobDataResponse:
        ...
        return OobDataResponse(
            oob=bytes(bytearray.fromhex(extracted_oob))
        )
