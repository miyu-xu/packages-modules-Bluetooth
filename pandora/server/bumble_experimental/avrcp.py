...
from pandora_experimental.avrcp_pb2 import (
    MediaAttribute,
    GetElementAttributesRequest,
    GetElementAttributesResponse,
)
...
class AvrcpService(AVRCPServicer):
    ...
    @utils.rpc
    async def GetElementAttributes(self, request: GetElementAttributesRequest,
                                   context: grpc.ServicerContext) -> GetElementAttributesResponse:
        if not self.avrcp_connected:
            await self.avrcp_protocol.connect(self.connection)
            self.avrcp_connected = True

        attributes = []
        for attribute in await self.avrcp_protocol.get_element_attributes(request.identifier,
                                                                          request.requested_attributes):
            attributes.append(
                MediaAttribute(attribute_id=attribute.attribute_id, attribute_value=attribute.attribute_value))

        return GetElementAttributesResponse(attribute_values=attributes)
