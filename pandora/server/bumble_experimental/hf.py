...
peerdevice = "DA:4C:10:DE:17:00/P"
...
class HFService(HFServicer):
    ...
    @utils.rpc
    async def Aclconnect(self, request: empty_pb2.Empty, context: grpc.ServicerContext) -> empty_pb2.Empty:
        logging.error(f'Aclconnect RPC Function')
        # Connect ACL if not connected
        await perform_acl_connection(self.device, peerdevice)
