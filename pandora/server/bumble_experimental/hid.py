...
def on_report(characteristic, value):
    logging.info(color('Report:', 'cyan'), value.hex(), 'from', characteristic)
...
# -----------------------------------------------------------------------------
async def keyboard_device(device):
    ...
    await device.start_advertising(own_address_type=OwnAddressType.RESOLVABLE_OR_PUBLIC, auto_restart=True)
...
class HIDService(HIDServicer):
    ...
    async def RegisterHidDevice(self, request: empty_pb2.Empty, context: grpc.ServicerContext) -> empty_pb2.Empty:
        logging.info(f'Hid Register')
        await keyboard_device(self.device)
